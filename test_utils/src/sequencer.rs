...
use haneul_network::transport::{MessageHandler, RwChannel};
use haneul_types::base_types::SequenceNumber;
use haneul_types::error::{HaneulError, HaneulResult};
use haneul_types::messages::ConsensusOutput;
...
impl Sequencer {
    ...
    pub async fn spawn(sequencer: Self, store_path: &Path) -> HaneulResult<()> {
        ...
        tokio::spawn(async move {
            ...
            haneul_network::transport::spawn_server(
            ...
            tokio::select! {
                ...
                Some(buffer) = stream.stream().next() => match buffer {
                    Ok(buffer) => match deserialize_message(&*buffer) {
                        ...
                        Ok(_) => {
                            log::warn!("{}", HaneulError::UnexpectedMessage);
                            break;
                        }
                        Err(e) => {
...
impl SequencerStore {
    ...
    pub fn get_last_consensus_index(&self) -> HaneulResult<SequenceNumber> {
        self.outputs
            .iter()
    ...
    pub fn store_output(&self, output: &ConsensusOutput) -> HaneulResult<()> {
    ...
    pub fn get_output(&self, index: &SequenceNumber) -> HaneulResult<Option<ConsensusOutput>> {
        self.outputs.get(index).map_err(HaneulError::from)
    }
}
