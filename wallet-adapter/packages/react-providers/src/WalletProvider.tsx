...
export const WalletProvider: FC<WalletProviderProps> = ({
    ...
    const setWalletAndUpdateStorage = useCallback((selectedWallet: Wallet | null) => {
        ...
        if (selectedWallet != null) {
            localStorage.setItem('haneulWallet', selectedWallet.adapter.name);
        } else {
            localStorage.removeItem('haneulWallet');
        }
    }, []);
    ...
    useEffect(() => {
        if (!wallet && !connected && !connecting) {
            let walletItem = localStorage.getItem('haneulWallet');
    ...
    const getAccounts = async (): Promise<HaneulAddress[]> => {
