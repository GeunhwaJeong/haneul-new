...
Clone Haneul repository into `$HANEUL` directory:

```bash
git clone https://github.com/GeunhwaJeong/haneul.git "$HANEUL"
```

Go to `"$HANEUL"/external-crates/move/crates/move-analyzer/prettier-plugin` and run the following
command:
...
```bash
npm install prettier@3.1.1 "$HANEUL"/external-crates/move/crates/move-analyzer/prettier-plugin
```
...
implement formatting for structs) and file an issue in the Haneul
