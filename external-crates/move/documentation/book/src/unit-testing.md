...
Unit tests for a Move package can be run with the [`haneul move test` command](./packages.md).
...
the `haneul move test` command:

```
$ haneul move test --help
```
...
```bash
$ haneul move new test_example; cd test_example 
```
...
You can run a specific test, or a set of tests with `haneul move test <str>`. This
...
runtime and gas used for each test. You can additionally add `csv` (`haneul move
