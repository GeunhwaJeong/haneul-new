...
For Haneul, `key` is used to signify an [object](./abilities/object.md). Objects are the basic unit of
...
- [`store`](#store)
  ...
  - For Haneul, `store` controls what data can be stored inside of an [object](./abilities/object.md).
...
- [`key`](#key)
  - Allows the type to serve as a "key" for storage. Ostensibly this means the value can be a
    ...
    be in storage.
  - For Haneul, `key` is used to signify an [object](./abilities/object.md).
...
For Haneul, `store` serves double duty. It controls what values can appear inside of an
