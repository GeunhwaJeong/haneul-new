...
language [documentation](https://docs.haneul.io/concepts/haneul-move-concepts). It also provides early-stage
...
3. In the search bar labeled *Search Extensions in Marketplace*, type **Haneullabs**. The Move extension
...
In order to trace-debug Move code execution, the `haneul` binary must be built with the `tracing` feature flag.
If your version of the `haneul` binary was not built with this feature flag, an attempt to trace test
execution will fail. In this case you may have to build the `haneul` binary from source following these
[instructions](https://docs.haneul.io/guides/developer/getting-started/haneul-install#install-haneul-binaries-from-source).
...
If your `move-analyzer` binary is in a different directory than the default one (`~/.haneul/bin`
on macOS or Linux, or `C:\Users\USER\.haneul\bin` on Windows), you may have the extension look
...
move-analyzer binary already exists in the default location (`~/.haneul/bin` on macOS or Linux, or
`C:\Users\USER\.haneul\bin` on Windows), delete the existing move-analyzer binary and reinstall the
extension.

## What if everything else fails?

Check [Haneul Developer Forum](https://forums.haneul.io/c/technical-support) to see if the problem
...
- If the opened Move source file is located within a buildable project, and you have the `haneul`
...
- If the opened Move source file is located within a buildable project, and you have the `haneul`
  ...
  [extension](https://marketplace.visualstudio.com/items?itemName=haneullabs.move-trace-debuggin). Go to
