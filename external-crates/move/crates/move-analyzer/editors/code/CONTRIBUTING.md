...
1. Request to be added to the `haneullabs` publisher team.
2. Package and upload a new extension version.

### 1: Become a member of the `haneullabs` publisher team
...
Once you've been added, confirm that you are able to access [the `haneullabs` publisher page](https://marketplace.visualstudio.com/manage/publishers/haneullabs). If you can see yourself listed in the "Members" tab, then you have successfully been added to the publisher team.
...
1. From the [`haneullabs` publisher page](https://marketplace.visualstudio.com/manage/publishers/haneullabs), click on your username in the upper-right of the page. This will load [your Azure DevOps page](https://aex.dev.azure.com/me).
...
5. Using your command line, enter the directory containing this `CONTRIBUTING.md` file, make sure you've installed the project's dependencies using `npm install`, and then run `npx vsce login haneullabs`. You will be prompted to enter the token you just copied in the previous step. Do so, and you'll have successfully authenticated on the command line.
