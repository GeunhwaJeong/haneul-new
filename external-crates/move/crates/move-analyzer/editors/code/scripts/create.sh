...
for DIST_OS VSCODE_OS in "${(@kv)SUPPORTED_OS}"; do
    # Haneul distribution identifier
    HANEUL_DISTRO=$NETWORK"-v"$VERSION
    # name of the Haneul distribution archive file, for example haneul-testnet-v1.13.0-macos-arm64.tgz
    HANEUL_ARCHIVE="haneul-"$HANEUL_DISTRO"-"$DIST_OS".tgz"
    # a path to downloaded Haneul archive
    HANEUL_ARCHIVE_PATH=$TMP_DIR"/"$HANEUL_ARCHIVE

    # download Haneul archive file to a given location and uncompress it
    curl https://github.com/GeunhwaJeong/haneul/releases/download/"$HANEUL_DISTRO"/"$HANEUL_ARCHIVE" -L -o $HANEUL_ARCHIVE_PATH
    tar -xf $HANEUL_ARCHIVE_PATH --directory $TMP_DIR

    # names of the move-analyzer binary, both the one becoming part of the extension ($SERVER_BIN)
    # and the one in the Haneul archive ($ARCHIVE_SERVER_BIN)
