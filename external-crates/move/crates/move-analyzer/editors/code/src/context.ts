...
import {
    ...
    HANEUL_PATH_OPT, SERVER_PATH_OPT, FORCE_BUNDLED_OPT, Configuration,
    AUTO_IMPORTS_OPT,
} from './configuration';
...
export class Context {
    ...
    registerOnDidChangeConfiguration(): void {
        vscode.workspace.onDidChangeConfiguration(async event => {
            ...
            const haneul_path_conf = MOVE_CONF_NAME.concat('.').concat(HANEUL_PATH_OPT);
            ...
            const pathsChanged = event.affectsConfiguration(server_path_conf) ||
                event.affectsConfiguration(haneul_path_conf) ||
    ...
    async installServerBinary(extensionContext: vscode.ExtensionContext): Promise<boolean> {
        ...
        const cliVersionString = version(this.configuration.haneulPath, cliVersionArgs);
        ...
        if (this.configuration.serverPath !== this.configuration.defaultServerPath.fsPath) {
            ...
            if (standaloneVersion === null) {
                ...
                await vscode.window.showInformationMessage(
                    ...
                    'Move VSCode extension by Haneullabs in the VSCode marketplace',
                    { modal: true },
                    items,
                );
                return false;
        ...
        if (cliVersion !== null && (highestVersion === null || semver.gt(cliVersion, highestVersion))) {
            if (bundledVersionString === null || bundledVersion === null || !this.configuration.forceBundled) {
                // Even if there is a `haneul` binary on the path that has a higher version than the bundled one,
                ...
                this.resolvedServerPath = this.configuration.haneulPath;
                ...
                log.info(`User has requested to use the bundled move-analyzer v${bundledVersion.version}` +
                    ` over the CLI one v${cliVersion.version} installed at '${this.configuration.haneulPath}'`);
            }
        }
        ...
        if (highestVersion === null) {
            ...
            await vscode.window.showErrorMessage(
                ...
                'file accompanying Move VSCode extension by Haneullabs in the VSCode marketplace',
                { modal: true },
                items,
            );
            return false;
