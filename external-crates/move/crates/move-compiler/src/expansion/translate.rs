...
// Implicit aliases for Haneul mode:
// use haneul::object::{Self, ID, UID};
// use haneul::transfer;
// use haneul::tx_context::{Self, TxContext};
const IMPLICIT_HANEUL_MODULES: &[Symbol] = &[
    symbol!("object"),
    symbol!("transfer"),
    symbol!("tx_context"),
];
const IMPLICIT_HANEUL_MEMBERS: &[(Symbol, Symbol, ModuleMemberKind)] = &[
...
fn default_aliases(context: &mut Context) -> AliasMapBuilder {
    ...
    let haneul_address = maybe_make_well_known_address(context, loc, symbol!("haneul"));
    ...
    // if haneul is defined and the current package is in Haneul mode, add implicit haneul aliases
    if haneul_address.is_some() && context.env().package_config(current_package).flavor == Flavor::Haneul
    {
        let haneul_address = haneul_address.unwrap();
        modules.extend(
            IMPLICIT_HANEUL_MODULES
                .iter()
                .copied()
                .map(|m| (haneul_address, m)),
        );
        members.extend(
            IMPLICIT_HANEUL_MEMBERS
                .iter()
                .copied()
                .map(|(m, mem, k)| (haneul_address, m, mem, k)),
        );
    }
