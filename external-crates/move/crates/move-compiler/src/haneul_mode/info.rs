...
use crate::{
    ...
    haneul_mode::{
        OBJECT_MODULE_NAME, HANEUL_ADDR_NAME, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
impl HaneulInfo {
    pub fn new(
        ...
        assert!(info.haneul_flavor_info.is_none());
    ...
    fn visit_ty(
        ...
        match ty_ {
            ...
            N::Type_::Apply(_, sp!(_, tn_), _)
                if tn_.is(HANEUL_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
...
fn add_private_transfers(
    ...
    impl<'a> TypingVisitorContext for TransferVisitor<'a> {
        ...
        fn visit_exp_custom(&mut self, e: &T::Exp) -> bool {
            ...
            if !call.is(HANEUL_ADDR_NAME, TRANSFER_MODULE_NAME, TRANSFER_FUNCTION_NAME) {
                return false;
            }
