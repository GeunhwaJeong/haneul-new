...
use crate::haneul_mode::HANEUL_ADDR_NAME;
...
impl TypingVisitorContext for Context<'_> {
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, mdef: &mut T::ModuleDefinition) -> bool {
        // skips if true
        mdef.attributes.is_test_or_test_only() || ident.value.address.is(HANEUL_ADDR_NAME)
    }
        ...
        for (_, _, t) in &fdef.signature.parameters {
            if let Some(struct_name) = is_random_or_random_generator(t) {
                ...
                let note = format!("Functions that accept '{}::{}::{}' as a parameter might be abused by attackers by inspecting the results of randomness",
                                   HANEUL_PKG_NAME, RANDOM_MOD_NAME, struct_name);
                d.add_note(note);
...
fn is_random_or_random_generator(sp!(_, t): &N::Type) -> Option<&str> {
    ...
    match t {
        ...
        T::Apply(_, sp!(_, tname), _) => {
            if tname.is(HANEUL_PKG_NAME, RANDOM_MOD_NAME, RANDOM_STRUCT_NAME) {
                Some(RANDOM_STRUCT_NAME)
            } else if tname.is(HANEUL_PKG_NAME, RANDOM_MOD_NAME, RANDOM_GENERATOR_STRUCT_NAME) {
