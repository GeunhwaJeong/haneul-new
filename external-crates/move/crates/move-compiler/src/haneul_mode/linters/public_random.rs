...
simple_visitor!(
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, mdef: &T::ModuleDefinition) -> bool {
        // skips if true
        mdef.attributes.is_test_or_test_only() || ident.value.address.is(&HANEUL_ADDR_VALUE)
    },
        ...
        for (_, _, t) in &fdef.signature.parameters {
            if let Some(struct_name) = is_random_or_random_generator(t) {
                ...
                let note = format!(
                    ...
                    HANEUL_ADDR_NAME, RANDOM_MOD_NAME, struct_name
                );
                d.add_note(note);
...
fn is_random_or_random_generator(sp!(_, t): &N::Type) -> Option<&str> {
    ...
    match t.inner() {
        ...
        T::Apply(_, sp!(_, tname), _) => {
            if tname.is(&HANEUL_ADDR_VALUE, RANDOM_MOD_NAME, RANDOM_STRUCT_NAME) {
