...
simple_visitor!(
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, mdef: &T::ModuleDefinition) -> bool {
        // skip if in  `haneul` or test
        ident.value.address.is(&HANEUL_ADDR_VALUE) || mdef.attributes.is_test_or_test_only()
    },
    fn visit_function_custom(
        ...
        const DUPLICATE_TX_CTX_NOTE: &str = "Due to restrictions in PTB execution if there is a \
            ...
            the function parameters. This function will not be callable on Haneul";
        ...
        transactions to Haneul. As such, this function will not be callable.";
        for (_, _, param_ty) in &signature.parameters {
            if is_mut_clock(param_ty) {
                let msg = format!(
                    "Invalid parameter type. '{2}' must be taken immutably, e.g. '&{}::{}::{2}'",
                    HANEUL_ADDR_NAME, CLOCK_MODULE_NAME, CLOCK_TYPE_NAME
                );
            ...
            if is_mut_random(param_ty) {
                let msg = format!(
                    "Invalid parameter type. '{2}' must be taken immutably, e.g. '&{}::{}::{2}'",
                    HANEUL_ADDR_NAME, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_TYPE_NAME
                );
                let mut diag = diag!(UNCALLABLE_FUNCTION_SIGNATURE, (param_ty.loc, msg));
                diag.add_note(OBJECT_NOTE);
                self.add_diag(diag);
