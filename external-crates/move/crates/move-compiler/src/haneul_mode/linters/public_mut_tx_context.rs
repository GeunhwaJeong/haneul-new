...
use crate::{
    ...
    haneul_mode::{HANEUL_ADDR_NAME, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME},
    typing::{
        ast as T,
...
impl TypingVisitorContext for Context<'_> {
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, _mdef: &mut T::ModuleDefinition) -> bool {
        // skip if in 'haneul::tx_context'
        ident.value.is(HANEUL_ADDR_NAME, TX_CONTEXT_MODULE_NAME)
    }
    ...
    fn visit_function_custom(
        ...
        for (_, _, sp!(loc, param_ty_)) in &fdef.signature.parameters {
            if matches!(
                param_ty_,
                Type_::Ref(false, t) if t.value.is(HANEUL_ADDR_NAME, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME),
            ) {
