...
simple_visitor!(
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, _mdef: &T::ModuleDefinition) -> bool {
        ...
        ident.value.is(&HANEUL_ADDR_VALUE, TX_CONTEXT_MODULE_NAME)
    },
    fn visit_function_custom(
        ...
        for (_, _, sp!(loc, param_ty_)) in &fdef.signature.parameters {
            if matches!(
                param_ty_.inner(),
                TypeInner::Ref(false, t) if t.value.is(&HANEUL_ADDR_VALUE, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME),
            ) {
