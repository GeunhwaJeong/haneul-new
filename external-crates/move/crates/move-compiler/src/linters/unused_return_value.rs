...
//! Flags discarded return values of calls with no `&mut` arg (Haneul: ignoring `(&mut) TxContext`).
...
pub(crate) struct CallSite {
    /// The location
    loc: Loc,
    /// In Haneul mode, was an &mut TxContext present but excluded
...
impl SimpleAbsIntConstructor for UnusedReturnValue {
    ...
    fn new<'a>(
        ...
        let is_haneul = context.env.package_config(context.package).flavor == Flavor::Haneul;
        Some(UnusedReturnValueAI { is_haneul })
    }
}
...
impl SimpleAbsInt for UnusedReturnValueAI {
    ...
    fn call_custom(
        ...
        let purity = call_purity(self.is_haneul, f);
...
enum Purity {
    /// No `&mut` arguments
    Pure {
        /// In Haneul flavor, was there an excluded `&mut TxContext`
        tx_context_exempted: bool,
    },
