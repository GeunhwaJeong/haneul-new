...
pub enum PublicForTesting {
    ...
    Entry(Loc),
    // TODO we should allow calling init in unit tests, but this would need Haneul bytecode verifier
...
pub fn public_testing_visibility(
    ...
    // is_testing && (is_entry || is_haneul_init)
    if !env.test_mode() {
        return None;
    }

    // TODO support haneul init functions
    // let flavor = env.package_config(package).flavor;
    // flavor == Flavor::Haneul && callee_name.value() == INIT_FUNCTION_NAME
