...
pub struct ModuleInfo {
    ...
    pub haneul_info: Option<HaneulModInfo>,
}
...
impl TypingProgramInfo {
    pub fn new(
        ...
        {
            let mut haneul_flavor_info = HaneulInfo::new(modules, &info);
            for (mident, module_info) in info.modules.key_cloned_iter_mut() {
                let uid_holders = haneul_flavor_info
                    .uid_holders
                    .remove(&mident)
                    .unwrap_or_default();
                let transferred = haneul_flavor_info
                    .transferred
                    .remove(&mident)
                    .unwrap_or_default();
                module_info.haneul_info = Some(HaneulModInfo {
                    uid_holders,
                    transferred,
                });
            }
