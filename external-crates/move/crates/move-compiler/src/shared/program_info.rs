...
pub struct ProgramInfo<const AFTER_TYPING: bool> {
    ...
    pub haneul_flavor_info: Option<HaneulInfo>,
}
...
impl TypingProgramInfo {
    pub fn new(
        ...
        if env
            .package_configs()
            .any(|(_, config)| config.flavor == Flavor::Haneul)
        {
            let haneul_flavor_info = HaneulInfo::new(pre_compiled_lib, modules, &info);
            info.haneul_flavor_info = Some(haneul_flavor_info);
        };
        info
    }
}
