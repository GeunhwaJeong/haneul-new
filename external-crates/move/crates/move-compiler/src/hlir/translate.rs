...
fn exp_list(
    ...
    if let TE::ExpList(items) = e.exp.value {
        exp_list_items_to_vec(context, result, ty, e.exp.loc, items)
        ...
        if etys.len() == tys.len() {
            ...
            match needs_freeze(context, &current_ty, expected_ty) {
                ...
                Freeze::Sub(_) => {
                    ...
                    let frozen = freeze(context, result, expected_ty, current_exp);
...
fn exp_impl(
    ...
    let res = match e_ {
        ...
        TE::ExpList(items) => {
            HE::Multiple(exp_list_items_to_vec(context, result, None, eloc, items))
        }
        TE::Borrow(mut_, te, f) => {
            let e = exp(context, result, None, *te);
...
fn gen_unused_warnings(
    ...
    for (_, sname, sdef) in structs {
        ...
        if let H::StructFields::Defined(fields) = &sdef.fields {
            for (f, _) in fields {
                // skip for Haneul ID fields
                if is_haneul_mode && has_key && f.value() == ID_FIELD_NAME {
                    continue;
                }
                if !context
                    .used_fields
