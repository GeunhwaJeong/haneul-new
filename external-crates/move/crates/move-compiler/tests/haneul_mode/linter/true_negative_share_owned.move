...
module a::can_determine_to_be_new {
    ...
    public fun transfer(ctx: &mut haneul::tx_context::TxContext) {
        let o = make_obj(0, vector[], ctx);
        transfer::transfer(o, @0);
    }

    public fun share(ctx: &mut haneul::tx_context::TxContext) {
        let o = make_obj(0, vector[], ctx);
        transfer::share_object(o);
    }
}
...
module b::can_determine_to_be_new_with_struct {
    ...
    fun make_obj<T>(_: X<T>, ctx: &mut haneul::tx_context::TxContext): Obj {
        Obj { id: haneul::object::new(ctx) }
    }
