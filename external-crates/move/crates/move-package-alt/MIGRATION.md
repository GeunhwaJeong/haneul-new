...
```sh
haneulup install haneul --nightly=haneul-pkg-alt -y
```

This will compile `haneul` from the prototype branch, so it will need the rust
...
```sh
haneulup install mvr --nightly=ml/cli-pkg-alt -y
```
...
Our prototype is integrated into the `haneul` binary, so most of the commands you
...
environment. By default, the `haneul` binary tries to choose the right environment
based on the haneul client environment (as determined by `haneul client active-env`).
