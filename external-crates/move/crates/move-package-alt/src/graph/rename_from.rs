...
mod tests {
    ...
    async fn test_modern_using_legacy_framework() {
        let scenario = TestPackageGraph::new(["root"])
            ...
            .add_package("haneul", |pkg| pkg.set_legacy().set_legacy_name("Haneul"))
            .add_package("haneul_system", |pkg| {
            ...
            .add_dep("haneul", "std", |dep| dep.name("MoveStdLib"))
            // legacy -> legacy case (HaneulSystem -> MoveStdLib (std)
            .add_dep("haneul_system", "haneul", |dep| dep.name("Haneul"))
            .add_dep("haneul_system", "std", |dep| dep.name("MoveStdLib"))
            .build();
        ...
        scenario
            .graph_for("haneul_system")
            .await
            .check_rename_from()
            .unwrap();
    }

    /// modern package `bat` depends on legacy package `haneul` which has legacy name `Haneul` (capital S).
    ///
    /// The dependency is named `Haneul`. This should be disallowed, since the legacy name shouldn't
    /// appear in a modern manifest
    #[test(tokio::test)]
    async fn modern_uses_legacy_name() {
        let scenario = TestPackageGraph::new(["bat"])
            .add_package("haneul", |pkg| pkg.set_legacy().set_legacy_name("Haneul"))
            .add_dep("bat", "haneul", |dep| dep.name("Haneul"))
            .build();

        assert_snapshot!(scenario.graph_for("bat").await.check_rename_from().unwrap_err(), @r###"
        In Move.toml, the dependency `Haneul` refers to a package named `haneul`. Consider renaming the dependency to `haneul`:

            haneul = { local = "../haneul", ... }

        Alternatively, if you want to refer to `haneul` as `Haneul` in your source code, add a `rename-from` field to the dependency:

            Haneul = { local = "../haneul", rename-from = "haneul", ... }
        "###);
    }
