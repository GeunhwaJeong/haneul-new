...
pkg_b = { local = "../pkg_b" }"#,
    ...
    async fn test_load_root_package() {
        ...
        for name in names {
            ...
            let package = PackageLoader::new(&pkg_path, env.clone(), Vanilla)
                .load()
                .await
                .unwrap();
    ...
    async fn test_root_package_operations() {
        ...
        let root = PackageLoader::new(&pkg_path, env, Vanilla)
            .load()
            .await
            .unwrap();
    ...
    async fn test_cannot_override_default_environments() {
        ...
        let load_err = PackageLoader::new(&project.root(), environment, Vanilla)
            .load()
            .await
            .unwrap_err();
    ...
    async fn graph_to_lockfile() {
        ...
        let env = Vanilla::default_environment();
        let mut root = PackageLoader::new(scenario.path_for("example"), env, Vanilla)
            .load()
            .await
            .unwrap();
    ...
    pub async fn git_branch_dep_pinned() {
        ...
        let mut root_pkg = PackageLoader::new(project.path_for("root"), env.clone(), Vanilla)
            .load()
            .await
            .unwrap();
    ...
    async fn legacy_depends_on_modern() {
        ...
        assert_snapshot!(legacy_err, @"Packages with old-style Move.toml files cannot depend on new-style packages. See https://docs.haneul.io/references/package-managers/package-manager-migration for instructions.");
    }
}
