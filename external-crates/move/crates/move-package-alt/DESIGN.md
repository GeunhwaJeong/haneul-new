...
[notion-design]: https://www.notion.so/haneullabs/Package-Management-Revamp-Concrete-Design-1b56d9dcb4e980ccb06ad12ad18004c0?pvs=4
...
provide one. By default we will use the current chain ID from `haneul client` (which we will keep
...
1. The user explicitly asks for it by running `haneul move update-deps`. This command will repin all
...
4. If there are multiple environments `e1`, `e2` in the manifest having chain ID `i`, we produce an
   ...
   `testnet`.
    ...
    > available. Run `haneul move build --build-env e1` or `haneul move build --build-env e2`
...
6. If there is no `Pub.e.toml` file, we produce an error, depending on the command:
   For build:
    ...
    >
    >   haneul move build --build-env testnet
    >
    > Note: adding local networks to `Move.toml` is discouraged; see `haneul client test-publish --help`
...
best-effort on that. We may provide an additional tool to help fix things up (e.g. `haneul move
...
manifest. Each flavor can specify its default system dependencies (for Haneul, that's `haneul` and `std`).
Non-default system dependencies can be specified like that: `system_dependencies = ["haneul", "std", "haneul_system"]`
...
> The default system deps for Haneul would be `haneul` and `std`. The available system deps are `std`,
> `haneul`, `system`, `deepbook-v2`, `bridge`, `monorepo-haneul`, `monorepo-std`. The `monorepo` deps are
> converted to local dependencies are are used for our internal tests (they would expand to `haneul = {
> local = "path_to_monorepo/crates/haneul-framework/packages/haneul" }` and would fail if they are used
...
If the user runs `haneul move update-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `haneul move update-deps d1 d2` we rerun these steps only for the
...
We will add a new command `haneul client test-publish <pubfile>` which will publish build the root
package for one environment and then publish it to a chain using the addresses from the ephemeral
publication file.

`haneul client test-publish <pubfile> --build-env <env>` will work as follows:
 ...
 - if `--build-env <env>` is omitted, it defaults to the `build-env` name of the file at `<file>`;
   ...
   > example
   >
   >   haneul client test-publish <pubfile> --build-env testnet
   >
...
of examples post-bootcamp.
 ...
 - Crazy example: https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/haneul/contracts
 - Left this in the tooling meeting notes, but for anyone else interested, this is the HaneulGPT Move
   Dataset v1:
   [https://github.com/CMU-HaneulGPT/haneul-move-dataset-v1](https://github.com/CMU-HaneulGPT/haneul-move-dataset-v1)
...
## Impact on the packages we deploy (e.g. haneulfrens)
