...
# Document status
 ...
 - 4/25/25
     - TODO:
         ...
         - haneul move build/test now use the default dependencies unless you ask otherwise
...
# pinned transitive dependencies from most recent `haneul move upgrade-deps`
...
1. The user explicitly asks for it by running `haneul move update-deps`. This command will repin all
   dependencies (or you could ask to update a single dependency using `haneul move update-deps
   dep-name`).
...
To the best of our ability, we maintain the following invariants between `Move.toml` and `Move.lock`s:
 ...
 - unpublished.dependencies.unpinned matches manifest.dependencies (likewise for dep-replacements).
   If we discover a mismatch, we upgrade the mismatched dependency (as if the user ran `haneul move
   upgrade-deps foo`
...
If the user runs `haneul move upgrade-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `haneul move upgrade-deps d1 d2` we rerun these steps only for the
...
passing in the `flavor` field, which is of type String. It can take `haneul`, `core`, `aptos`, and some
