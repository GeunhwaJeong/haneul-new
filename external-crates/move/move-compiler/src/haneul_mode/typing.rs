...
impl<'a> Context<'a> {
    fn new(env: &'a mut CompilationEnv, info: &'a TypingProgramInfo) -> Self {
        let haneul_module_ident = info
            .modules
            .key_cloned_iter()
            .find(|(m, _)| m.value.is(HANEUL_ADDR_NAME, TRANSFER_MODULE_NAME))
            .map(|(m, _)| m);
        Context {
            env,
            info,
...
const GLOBAL_NOTE: &str = "Global storage is not used in Haneul. \
                        Instead objects (structs with the 'key' ability) are used with \
                        programmable transaction blocks and the 'haneul::transfer' functions.";
...
impl<'a> TypingVisitorContext for Context<'a> {
    ...
    fn visit_script_custom(&mut self, _name: Symbol, script: &mut T::Script) -> bool {
        ...
        if config.flavor == Flavor::Haneul {
            ...
            let msg = "'scripts' are not supported on Haneul. \
...
fn struct_def(context: &mut Context, name: StructName, sdef: &N::StructDefinition) {
    ...
    if !id_field_type
        .value
        .is(HANEUL_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME)
    {
        let actual = format!(
            "But found type: {}",
...
fn function(context: &mut Context, name: FunctionName, fdef: &mut T::Function) {
    ...
    if let Some(acquire_loc) = acquires.values().next() {
        global_storage_error(
            context,
            *acquire_loc,
            "Global storage acquires are not supported in Haneul",
        )
    }
...
fn tx_context_kind(sp!(_, last_param_ty_): &Type) -> TxContextKind {
    ...
    if inner_name.is(HANEUL_ADDR_NAME, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME) {
        if *is_mut {
...
fn is_mut_clock(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n_), _) => n_.is(HANEUL_ADDR_NAME, CLOCK_MODULE_NAME, CLOCK_TYPE_NAME),
        Type_::Unit
        | Type_::Param(_)
        | Type_::Var(_)
        | Type_::Anything
...
fn is_entry_receiving_ty(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(HANEUL_ADDR_NAME, TRANSFER_MODULE_NAME, RECEIVING_TYPE_NAME) =>
        {
    ...
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(STD_ADDR_NAME, ASCII_MODULE_NAME, ASCII_TYPE_NAME)
                ...
                || n.is(HANEUL_ADDR_NAME, OBJECT_MODULE_NAME, ID_TYPE_NAME) =>
        {
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(HANEUL_ADDR_NAME, EVENT_MODULE_NAME)
                && name.value() == EVENT_FUNCTION_NAME
            {
                check_event_emit(context, e.exp.loc, mcall)
            }
            let is_transfer_module = module.value.is(HANEUL_ADDR_NAME, TRANSFER_MODULE_NAME);
        ...
        T::UnannotatedExp_::Builtin(b, _) => match &b.value {
            ...
            | T::BuiltinFunction_::Exists(_) => global_storage_error(
                context,
                e.exp.loc,
                format!("Global storage primitive '{}' is not supported in Haneul", b),
            ),
        ...
        T::UnannotatedExp_::Pack(m, s, _, _) => {
            if !context.in_test
                && !context
                    .current_module()
                    .value
                    .is(HANEUL_ADDR_NAME, HANEUL_MODULE_NAME)
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if current_module
        .value
        .is(HANEUL_ADDR_NAME, TRANSFER_FUNCTION_NAME)
    {
...
fn global_storage_error(context: &mut Context, loc: Loc, msg: impl ToString) {
    ...
    let haneul_transfer_loc = context
        .haneul_transfer_ident
        .as_ref()
        .map(|m| *context.info.modules.get_loc(m).unwrap());
    if let Some(tloc) = haneul_transfer_loc {
