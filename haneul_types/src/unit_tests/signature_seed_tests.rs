...
fn test_deterministic_signing() {
    ...
    // Verify signatures.
    let ver_0 = sig_0_ok.clone().check(&msg0, haneul_address_0);
    assert!(ver_0.is_ok());

    let ver_1 = sig_1.unwrap().check(&msg0, haneul_address_1);
    assert!(ver_1.is_ok());

    // Ensure that signatures cannot be verified against another address.
    let ver_0_with_address_1 = sig_0_ok.clone().check(&msg0, haneul_address_1);
    assert!(ver_0_with_address_1.is_err());

    // Ensure that signatures cannot be verified against another message.
    let ver_0_with_msg1 = sig_0_ok.clone().check(&msg1, haneul_address_0);
