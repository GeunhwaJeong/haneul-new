...
fn test_object_id_zero_padding() {
    ...
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_1.0);
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_2.0);
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_3.0);
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_4.0);
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_5.0);
    assert_eq!(HANEUL_FRAMEWORK_ADDRESS, obj_id_6.0);
}
...
fn test_address_display() {
    ...
    let id = HaneulAddress::from_str(hex).unwrap();
    assert_eq!(format!("{:?}", id), format!("0x{hex}"));
    assert_eq!(format!("{:X}", id), upper_hex);
    assert_eq!(format!("{:x}", id), hex);
    assert_eq!(format!("{:#x}", id), format!("0x{hex}"));
...
fn test_address_serde_not_human_readable() {
    ...
    let deserialized: HaneulAddress = bincode::deserialize(&serialized).unwrap();
...
fn test_address_serde_human_readable() {
    ...
    let deserialized: HaneulAddress = serde_json::from_str(&serialized).unwrap();
    assert_eq!(deserialized, address);
}
...
fn test_address_serde_with_expected_value() {
    ...
    let address = HaneulAddress::try_from(address_vec.clone()).unwrap();
...
fn derive_sample_address() -> (HaneulAddress, KeyPair) {
