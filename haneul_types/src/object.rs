...
impl MoveObject {
    ...
    pub fn to_json(&self, layout: &MoveStructLayout) -> Result<Value, HaneulError> {
        ...
        serde_json::to_value(&move_value).map_err(|e| HaneulError::ObjectSerializationError {
            error: e.to_string(),
        })
    }
...
impl Data {
    ...
    /// If `self` is a Move value, the `resolver` value must contain the module that declares `self.type_` and the (transitive)
    /// dependencies of `self.type_` in order for this to succeed. Failure will result in an `ObjectSerializationError`
    pub fn to_json(&self, layout: &Option<MoveStructLayout>) -> Result<Value, HaneulError> {
        use Data::*;
        match self {
            Move(m) => match layout {
                Some(l) => m.to_json(l),
                None => Err(HaneulError::ObjectSerializationError {
            ...
            Package(p) => {
                ...
                for (name, bytecode) in p.serialized_module_map() {
                    ...
                    let d = Disassembler::from_view(view, Spanned::unsafe_no_loc(()).loc).map_err(
                        |e| HaneulError::ObjectSerializationError {
                            error: e.to_string(),
                        },
                    )?;
                    let bytecode_str =
...
impl Owner {
    pub fn get_owner_address(&self) -> HaneulResult<HaneulAddress> {
        match self {
            ...
            Self::SharedMutable | Self::SharedImmutable => Err(HaneulError::UnexpectedOwnerType),
        }
    }
...
impl std::cmp::PartialEq<HaneulAddress> for Owner {
    fn eq(&self, other: &HaneulAddress) -> bool {
        match self {
...
impl Object {
    ...
    pub fn get_single_owner_and_id(&self) -> Option<(HaneulAddress, ObjectID)> {
    ...
    pub fn transfer(&mut self, new_owner: HaneulAddress) -> HaneulResult {
    ...
    pub fn to_json(&self, layout: &Option<MoveStructLayout>) -> Result<Value, HaneulError> {
        ...
        let owner =
            serde_json::to_value(&self.owner).map_err(|e| HaneulError::ObjectSerializationError {
                error: e.to_string(),
            })?;
    ...
    pub fn get_move_template_type(&self) -> HaneulResult<TypeTag> {
        let move_struct = self.data.type_().ok_or_else(|| HaneulError::TypeError {
            error: "Object must be a Move object".to_owned(),
        })?;
        fp_ensure!(
            move_struct.type_params.len() == 1,
            HaneulError::TypeError {
    ...
    pub fn is_transfer_eligible(&self) -> HaneulResult {
        fp_ensure!(!self.is_shared(), HaneulError::TransferSharedError);
        let is_coin = match &self.data {
            Data::Move(m) => bcs::from_bytes::<Coin>(&m.contents).is_ok(),
            Data::Package(_) => false,
        };
        fp_ensure!(is_coin, HaneulError::TransferNonCoinError);
        Ok(())
    }
}
...
impl ObjectRead {
    ...
    pub fn layout(&self) -> Result<&Option<MoveStructLayout>, HaneulError> {
        match &self {
            Self::Deleted(oref) => Err(HaneulError::ObjectDeleted { object_ref: *oref }),
            Self::NotExists(id) => Err(HaneulError::ObjectNotFound { object_id: *id }),
