...
impl Owner {
    pub fn get_single_owner_address(&self) -> HaneulResult<HaneulAddress> {
        match self {
...
impl Object {
    ...
    pub fn get_move_template_type(&self) -> HaneulResult<TypeTag> {
        let move_struct = self.data.type_().ok_or(HaneulError::TypeError {
    ...
    pub fn is_transfer_elegible(&self) -> HaneulResult {
        fp_ensure!(!self.is_read_only(), HaneulError::TransferImmutableError);
