...
impl SignatureSeed {
    ...
    /// use haneul_types::signature_seed::SEED_LENGTH;
    /// # fn doctest() -> Result<SignatureSeed, HaneulError> {
    ...
    /// is a `HaneulError` wrapping the internal error that occurred.
    #[inline]
    pub fn from_bytes(bytes: &[u8]) -> Result<SignatureSeed, HaneulError> {
        if bytes.len() != SEED_LENGTH {
            return Err(HaneulError::SignatureSeedInvalidLength(bytes.len()));
        }
    ...
    /// Deterministically generate a HaneulAddress via HKDF.
    ///
    /// # Example
    ///
    /// ```
    ...
    pub fn new_deterministic_address(
        ...
        Ok(HaneulAddress::from(keypair.public_key_bytes()))
    }
    ...
    /// ```
    /// use serde::{Deserialize, Serialize};
    /// use haneul_types::crypto::BcsSignable;
    /// use haneul_types::signature_seed::SignatureSeed;
    ///
    ...
    fn new_deterministic_keypair(
        ...
        hk.expand(domain.unwrap_or(&DEFAULT_DOMAIN), &mut okm)
            .map_err(|e| HaneulError::HkdfError(e.to_string()))?;
