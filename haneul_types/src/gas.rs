...
pub static MIN_GAS_BUDGET: Lazy<u64> =
    Lazy::new(|| to_external(INIT_HANEUL_COST_TABLE.min_transaction_cost.0).get());
...
impl<'a> HaneulGasStatus<'a> {
    ...
    pub fn charge_storage_mutation(
        ...
        let storage_cost = INIT_HANEUL_COST_TABLE
    ...
    pub fn charge_vm_exec_test_only(&mut self, cost: u64) -> HaneulResult {
    ...
    pub fn summary(&self, succeeded: bool) -> GasCostSummary {
        ...
        let computation_cost_in_haneul = computation_cost.mul(self.computation_gas_unit_price).get();
        if succeeded {
            GasCostSummary {
    ...
    fn deduct_storage_cost(&mut self, cost: &StorageCost) -> HaneulResult<GasCarrier> {
