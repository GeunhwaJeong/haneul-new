...
macro_rules! ok_or_gas_error {
    ($cond:expr, $e:expr) => {
        if !($cond) {
            Err(HaneulError::InsufficientGas { error: $e })
        } else {
            Ok(())
        }
    };
...
static INIT_HANEUL_COST_TABLE: Lazy<HaneulCostTable> = Lazy::new(|| HaneulCostTable {
...
pub static MIN_GAS_BUDGET: Lazy<u64> = Lazy::new(|| {
    ...
    consts
        .to_external_units(INIT_HANEUL_COST_TABLE.min_transaction_cost.0)
        .get()
});
...
impl<'a> HaneulGasStatus<'a> {
    pub fn new_with_budget(gas_budget: u64) -> HaneulGasStatus<'a> {
        Self::new(
    ...
    pub fn charge_min_tx_gas(&mut self) -> HaneulResult {
        self.deduct_computation_cost(&INIT_HANEUL_COST_TABLE.min_transaction_cost)
    }

    pub fn charge_consensus(&mut self) -> HaneulResult {
        self.deduct_computation_cost(&INIT_HANEUL_COST_TABLE.consensus_cost)
    }

    pub fn charge_publish_package(&mut self, size: usize) -> HaneulResult {
        let computation_cost = INIT_HANEUL_COST_TABLE
            .package_publish_per_byte_cost
            .with_size(size);
        self.deduct_computation_cost(&computation_cost)
    }

    pub fn charge_storage_read(&mut self, size: usize) -> HaneulResult {
        let cost = INIT_HANEUL_COST_TABLE
            .object_read_per_byte_cost
            .with_size(size);
        self.deduct_computation_cost(&cost)
    }

    pub fn charge_storage_mutation(&mut self, old_size: usize, new_size: usize) -> HaneulResult {
        ...
        /// cost (in Haneul) for each object. whenever we mutate an object, we always rebate the old
    ...
    fn new(move_gas_status: GasStatus<'a>, gas_budget: u64) -> HaneulGasStatus<'a> {
        HaneulGasStatus {
    ...
    fn deduct_computation_cost(&mut self, cost: &ComputationCost) -> HaneulResult {
        println!(
        ...
        if self.gas_status.deduct_gas(cost.0).is_err() {
            Err(HaneulError::InsufficientGas {
    ...
    fn deduct_storage_cost(&mut self, cost: &StorageCost) -> HaneulResult {
    ...
    fn rebate_storage_deletion(&mut self, size: usize) {
        let rebate = INIT_HANEUL_COST_TABLE.storage_per_byte_cost.with_size(size);
...
pub fn check_gas_balance(gas_object: &Object, gas_budget: u64) -> HaneulResult {
    ok_or_gas_error!(
...
pub fn start_gas_metering(gas_budget: u64) -> HaneulResult<HaneulGasStatus<'static>> {
    let mut gas_status = HaneulGasStatus::new_with_budget(gas_budget);
