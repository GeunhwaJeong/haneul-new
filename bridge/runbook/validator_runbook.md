...
### Bridge Node Config
Use `haneul-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```bash
$ haneul-bridge-cli create-bridge-node-config-template {PATH}
$ haneul-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
* `bridge-authority-key-path` is the path to the Bridge Validator key, generated from `haneul-bridge-cli create-bridge-validator-key` from above command.
...
* `haneul:haneul-rpc-url`: Haneul RPC URL
* `haneul:haneul-bridge-chain-id`: 0 for Haneul Mainnet, 1 for Haneul Testnet
...
* `metrics:push-url`: The url of the remote Haneul metrics pipeline: `https://metrics-proxy.[testnet|mainnet].haneul.io:8443/publish/metrics`. See the [metrics push section](#metrics-push) below for more details.
...
* `haneul:bridge-client-key-path`: the file path of Bridge Client key. This key can be generated with `haneul-bridge-cli create-bridge-client-key` as shown above. When `run-client` is true but `haneul:bridge-client-key-path` not provided, it defaults to use Bridge Validator key to submit transactions on Haneul. However this is not recommended for the sake of key separation.

### Bridge Client
`BridgeClient` orchestrates bridge transfer requests.
* It is **optional** to run for a `BridgeNode`.
* `BridgeClient` submits transaction on Haneul Network. Thus when it's enabled, a Haneul Account Key with enough HANEUL balance is needed.
...
```
haneul-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created Haneul Address. Then we need to fund this address with some HANEUL for operations.
...
1. `cargo install`
```bash
$ cargo install --locked --git "https://github.com/GeunhwaJeong/haneul.git" --branch {BRANCH-NAME} haneul-bridge
# OR
$ cargo install --locked --git "https://github.com/GeunhwaJeong/haneul.git" --rev {SHA-NAME} haneul-bridge
```
...
$ cargo build --release --bin haneul-bridge
```

3. `curl`/`wget` pre-built binaries (for linux/amd64 only)
```
curl https://haneul-releases.s3.us-east-1.amazonaws.com/{SHA}/haneul-bridge -o haneul-bridge
```

4. use pre-built docker image. Pull from docker hub: `haneullabs/haneul-tools:{SHA}`


### Run Bridge Node
It is similar to running a haneul-node using systemd or ansible. The command to start the bridge node is:

```bash
$ RUST_LOG=info,haneul_bridge=debug haneul-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
* A full list of Bridge Node metrics can be found [here](../../crates/haneul-bridge/src/metrics.rs). Find descriptions of each metric [here](../../crates/haneul-bridge/src/metrics.rs) and we skip them below.
...
* `bridge_last_synced_haneul_checkpoints`
* `bridge_last_synced_eth_blocks`
* `bridge_last_finalized_eth_block`
* `bridge_haneul_watcher_received_events`
* `bridge_eth_watcher_received_events`
* `bridge_haneul_watcher_received_actions`
