...
contract DeployBridge is Script {
    function parseDeployConfig(string memory path) public view returns (DeployConfig memory) {
        ...
        config.haneulDecimals = abi.decode(vm.parseJson(json, ".haneulDecimals"), (uint256[]));
    ...
    function run() external {
        ...
        if (isLocal) {
            ...
            deployConfig.haneulDecimals = new uint256[](5);
            deployConfig.haneulDecimals[0] = 9;
            deployConfig.haneulDecimals[1] = 8;
            deployConfig.haneulDecimals[2] = 8;
            deployConfig.haneulDecimals[3] = 6;
            deployConfig.haneulDecimals[4] = 6;
        }
        ...
        require(
            deployConfig.supportedTokens.length == deployConfig.haneulDecimals.length,
            "supportedTokens.length != haneulDecimals.length"
        );
        ...
        // convert Haneul Decimals from uint256 to uint8
        uint8[] memory haneulDecimals = new uint8[](deployConfig.haneulDecimals.length);
        for (uint256 i; i < deployConfig.haneulDecimals.length; i++) {
            haneulDecimals[i] = uint8(deployConfig.haneulDecimals[i]);
        }
