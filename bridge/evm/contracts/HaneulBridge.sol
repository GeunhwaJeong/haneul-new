...
/// tokens. Bridge operations are managed by a committee of Haneul validators that are responsible
...
contract HaneulBridge is IHaneulBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    /// @notice Initializes the HaneulBridge contract with the provided parameters.
    ...
    /// @param recipientAddress The address on the Haneul chain where the tokens will be sent.
    ...
    function bridgeERC20(
        ...
        require(
            recipientAddress.length == HANEUL_ADDRESS_LENGTH,
            "HaneulBridge: Invalid recipient address length"
        );
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "HaneulBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "HaneulBridge: Target chain not supported"
        );
        _;
    }
}
