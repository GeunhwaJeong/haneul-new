...
contract BridgeConfig is IBridgeConfig, CommitteeUpgradeable {
    ...
    function initialize(
        ...
        require(
            _supportedTokens.length == _haneulDecimals.length, "BridgeConfig: Invalid Haneul decimals"
        );
        ...
        for (uint8 i; i < _tokenIds.length; i++) {
            ...
            supportedTokens[_tokenIds[i]] = Token(_supportedTokens[i], _haneulDecimals[i], true);
        }
    ...
    function tokenHaneulDecimalOf(uint8 tokenID) public view override returns (uint8) {
        ...
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_EVM_TOKENS)
    {
        ...
        for (uint8 i; i < tokenIDs.length; i++) {
            _addToken(tokenIDs[i], tokenAddresses[i], haneulDecimals[i], _tokenPrices[i], native);
        }

        emit TokensAddedV2(message.nonce, tokenIDs, tokenAddresses, haneulDecimals, _tokenPrices);
    }
    ...
    /// @param haneulDecimal The decimal places of the token.
    /// @param tokenPrice The price of the token.
    ...
    function _addToken(
        ...
        require(haneulDecimal > 0, "BridgeConfig: Invalid Haneul decimal");
        require(tokenPrice > 0, "BridgeConfig: Invalid token price");

        uint8 erc20Decimals = IERC20Metadata(tokenAddress).decimals();
        require(erc20Decimals >= haneulDecimal, "BridgeConfig: Invalid Haneul decimal");

        supportedTokens[tokenID] = Token(tokenAddress, haneulDecimal, native);
        tokenPrices[tokenID] = tokenPrice;
    }
