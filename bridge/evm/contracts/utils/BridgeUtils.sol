...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the Haneul native bridge. It also
...
library BridgeUtils {
    ...
    /// @param haneulDecimal The haneul decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to Haneul.
    /// @return Haneul converted amount.
    function convertERC20ToHaneulDecimal(uint8 erc20Decimal, uint8 haneulDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == haneulDecimal) {
        ...
        require(erc20Decimal > haneulDecimal, "BridgeUtils: Invalid Haneul decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - haneulDecimal);
    ...
    function convertHaneulToERC20Decimal(uint8 erc20Decimal, uint8 haneulDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (haneulDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support Haneul now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, Haneul address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of haneul decimals
    ///     bytes m + 2 -> i : haneul decimals
    ...
    /// @return haneulDecimals the Haneul decimal places of the tokens to be added.
        ...
        )
    {
        ...
        haneulDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            haneulDecimals[i] = uint8(_payload[offset++]);
        }
