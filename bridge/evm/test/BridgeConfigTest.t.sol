...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testTokenHaneulDecimalOf() public {
        assertEq(config.tokenHaneulDecimalOf(1), 8);
    }
    ...
    function testUpdateTokenPriceRegressionTestWithSigVerficiation() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, haneulDecimals, _supportedDestinationChains)
            ),
            opts
        );
    ...
    function testAddTokensRegressionTest() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), 12, supportedTokens, tokenPrices, tokenIds, haneulDecimals, _supportedDestinationChains)
            ),
            opts
        );
        config = BridgeConfig(_config);
