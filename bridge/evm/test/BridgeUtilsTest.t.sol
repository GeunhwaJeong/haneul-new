...
contract BridgeUtilsTest is BridgeBaseTest {
    ...
    function testConvertERC20ToHaneulDecimalAmountTooLargeForUint64() public {
        ...
        BridgeUtils.convertERC20ToHaneulDecimal(18, 8, type(uint256).max);
    }

    function testConvertERC20ToHaneulDecimalInvalidHaneulDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Haneul decimal"));
        BridgeUtils.convertERC20ToHaneulDecimal(10, 11, 100);
    }

    function testconvertHaneulToERC20DecimalInvalidHaneulDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Haneul decimal"));
        BridgeUtils.convertHaneulToERC20Decimal(10, 11, 100);
    }

    function testConvertERC20ToHaneulDecimal() public view {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint256 ethAmount = 10 ether;
        uint64 haneulAmount = BridgeUtils.convertERC20ToHaneulDecimal(
        ...
        haneulAmount = BridgeUtils.convertERC20ToHaneulDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenHaneulDecimalOf(BridgeUtils.USDC),
            ethAmount
        );
        ...
        haneulAmount = BridgeUtils.convertERC20ToHaneulDecimal(
            ...
            config.tokenHaneulDecimalOf(BridgeUtils.USDT),
            ethAmount
        );
        assertEq(haneulAmount, ethAmount);
        ...
        haneulAmount = BridgeUtils.convertERC20ToHaneulDecimal(
            ...
            config.tokenHaneulDecimalOf(BridgeUtils.BTC),
            ethAmount
        );
        assertEq(haneulAmount, ethAmount);
    }

    function testconvertHaneulToERC20Decimal() public view {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint64 haneulAmount = 11_000_000_00; // 11 eth
        uint256 ethAmount = BridgeUtils.convertHaneulToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenHaneulDecimalOf(BridgeUtils.ETH),
            haneulAmount
        );
        ...
        ethAmount = BridgeUtils.convertHaneulToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenHaneulDecimalOf(BridgeUtils.USDC),
            haneulAmount
        );
    ...
    function testEncodeMessage() public pure {
        bytes memory moveEncodedMessage = abi.encodePacked(
            hex"48414e45554c5f4252494447455f4d45535341474500010000000000000000012080ab1ee086210a3a37355300ca24672e81062fcdb5ced6618dab203f6a3b291c0b14b18f79fe671db47393315ffdb377da4ea1b7af96010084d71700000000"
        );

        uint64 nonce = 0;
        uint8 haneulChainId = 1;
    ...
    function testDecodeUpgradePayload() public pure {
        bytes memory payload = abi.encode(
            address(100),
            address(200),
            hex"48414e45554c5f4252494447455f4d455353414745050100000000000000000c"
        );
        ...
        assertEq(_calldata, hex"48414e45554c5f4252494447455f4d455353414745050100000000000000000c");
    }
    ...
    function testDecodeUpgradePayloadWith1ArgRegressionTest() public pure {
        bytes memory newMockFunc1Message =
            hex"";
    ...
    function testDecodeUpgradePayloadWith2ArgsRegressionTest() public pure {
        bytes memory newMockFunc2Message =
            hex"";
    ...
    function testDecodeUpgradePayloadWithNoCalldataRegressionTest() public pure {
        bytes memory emptyCalldataMessage =
            hex"";
