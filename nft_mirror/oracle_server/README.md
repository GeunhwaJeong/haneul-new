# Haneul Oracle Service

The oracle service is a trusted entity that facilitates the cross-chain airdrop of NFT tokens on Haneul. This implementation is a MVP
version that's not indended for any production use. The service currently only supports the copying of Ethereum
ERC-721 token to Haneul.

As shown in the graph below, the main job of the oracle is to validate the user ownership of the NFT, call the Airdrop contract to claim the token, and return the Haneul explorer link of the newly minted NFT.
