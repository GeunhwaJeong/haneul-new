...
const HANEUL_APPS_API_ENDPOINT = process.env.HANEUL_APPS_API_ENDPOINT;
...
// Get curated list of official apps from HANEUL Apps API
export const getCuratedApps = createAsyncThunk<
    DappApiResponse[],
    void,
    AppThunkConfig
>('haneul-apps/get-curated-apps', async (_): Promise<DappApiResponse[]> => {
    if (!HANEUL_APPS_API_ENDPOINT) {
        throw new Error('HANEUL_APPS_API_ENDPOINT is not defined');
    }
    const response = await fetch(HANEUL_APPS_API_ENDPOINT);
    if (!response.ok) {
        throw new Error('No data returned from HANEUL Apps API');
    }
