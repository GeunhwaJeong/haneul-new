...
pub(crate) fn create_rustls_server_config(
    ...
    let verifier = haneul_tls::ClientCertVerifier::new(allower, certificate_server_name(context));
    // TODO: refactor to use key bytes
    let self_signed_cert = haneul_tls::SelfSignedCertificate::new(
...
pub(crate) fn create_rustls_client_config(
    ...
    let tls_private_key = self_signed_cert.rustls_private_key();
    let mut tls_config =
        haneul_tls::ServerCertVerifier::new(target_public_key, certificate_server_name(context))
...
impl haneul_tls::Allower for AllowedPublicKeys {
