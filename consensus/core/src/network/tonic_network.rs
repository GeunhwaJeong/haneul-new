...
type Channel = haneullabs_network::callback::Callback<
...
impl ChannelPool {
    ...
    async fn get_channel(
        ...
        let client_tls_config = haneul_tls::create_rustls_client_config(
            self.context
                .committee
                .authority(peer)
                .network_key
...
impl<S: NetworkService> NetworkManager<S> for TonicManager {
    ...
    async fn install_service(&mut self, service: Arc<S>) {
        ...
        let layers = tower::ServiceBuilder::new()
            ...
            .map_request(move |mut request: http::Request<_>| {
                if let Some(peer_certificates) =
                    request.extensions().get::<haneul_http::PeerCertificates>()
                {
            ...
            .layer_fn(|service| haneullabs_network::grpc_timeout::GrpcTimeout::new(service, None));
        ...
        let tls_server_config = haneul_tls::create_rustls_server_config_with_client_verifier(
        ...
        let http_config = haneul_http::Config::default()
            .tcp_nodelay(true)
...
// TODO: improve haneul-http to allow for providing a MakeService so that this can be done once per
// connection
fn peer_info_from_certs(
    connections_info: &ConnectionsInfo,
    peer_certificates: &haneul_http::PeerCertificates,
) -> Option<PeerInfo> {
    ...
    let public_key = haneul_tls::public_key_from_certificate(&certs[0])
        .map_err(|e| {
