...
test('displays inputs', async ({ page }) => {
    ...
    const programmableTxn = tx.transaction!.data
        .transaction as ProgrammableTransaction;
    const actualInputsCount = programmableTxn.inputs.length;

    const inputsCardContentsCount = await page
        .getByTestId(`inputs-card-content`)
        .count();
...
test('displays transactions card', async ({ page }) => {
    ...
    const transactionsContentCount = await page
        .getByTestId(`transactions-card-content`)
        .count();
