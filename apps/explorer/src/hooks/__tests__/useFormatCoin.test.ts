...
describe('formatBalance', () => {
    ...
    it('formats integer amounts with full CoinFormat', () => {
        expect(
            formatBalance(toMist('1'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('1');
        expect(
            formatBalance(toMist('1.123456789'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('1.123456789');
        expect(
            formatBalance(toMist('9999.9999'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('9,999.9999');
        expect(
            formatBalance(toMist('10000'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('10,000');
        expect(
            formatBalance(toMist('12345'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('12,345');
        expect(
            formatBalance(toMist('1234000'), HANEUL_DECIMALS, CoinFormat.FULL)
        ).toEqual('1,234,000');
        expect(
            formatBalance(toMist('1234000000'), HANEUL_DECIMALS, CoinFormat.FULL)
