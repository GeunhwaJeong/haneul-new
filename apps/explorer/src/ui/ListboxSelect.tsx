...
import { Check12, ChevronDown16 } from '@haneullabs/icons';
                    ...
                    >
                        <Listbox.Options className="absolute right-0 top-0 z-10 max-h-60 w-max max-w-xs overflow-auto rounded-lg bg-white p-2 shadow">
                            {options.map((aValue, index) => (
                                <Listbox.Option
                                    key={index}
                                    className="flex flex-1 cursor-pointer flex-nowrap items-center gap-4 rounded-sm p-2 hover:bg-haneul-light/40"
                                    value={aValue}
                                >
                                    {({ selected }) => (
                                        <>
