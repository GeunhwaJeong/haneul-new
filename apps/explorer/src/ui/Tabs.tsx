...
const tabStyles = cva(
    [
        'border-b border-haneul-grey-45 ui-selected:border-haneul-grey-65 font-semibold text-haneul-grey-70 pb-2 -mb-px',
    ...
    ],
    {
        variants: {
            size: {
                lg: 'text-heading4 ui-selected:text-haneul-grey-100',
                md: 'text-body ui-selected:text-haneul-grey-90',
            },
        },
        defaultVariants: {
            size: 'md',
...
export function TabList({ fullWidth, ...props }: TabListProps) {
    return (
        <HeadlessTab.List
            className={clsx(
                'flex gap-6 border-b border-haneul-grey-45 border-solid border-0',
                fullWidth && 'flex-1'
            )}
            {...props}
        />
