...
export function TableCard({ refetching, data, columns }: TableCardProps) {
    ...
    return (
        ...
        >
            <table className="text-left min-w-max border-collapse w-full border-solid border-haneul-grey-45 border-0 border-b">
                <thead>
                        ...
                        <tr key={headerGroup.id}>
                            {headerGroup.headers.map((header: any) => (
                                <th
                                    ...
                                    scope="col"
                                    className="text-haneul-grey-75 h-[30px] px-1 text-left font-semibold uppercase text-subtitle"
                                >
                    ...
                    {table.getRowModel().rows.map((row: any) => (
                        <tr key={row.id} className="group">
                            {row.getVisibleCells().map((cell: any) => (
                                <td
                                    key={cell.id}
                                    className="text-haneul-grey-75 h-[30px] px-1 group-hover:text-haneul-grey-90 group-hover:bg-haneul-grey-40 text-body group-hover:first:rounded-l group-hover:last:rounded-r"
                                >
                                    {flexRender(
