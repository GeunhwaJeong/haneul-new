...
import { Search16 } from '@haneullabs/icons';
...
function SearchItem({ value, children }: SearchResultProps) {
    return (
        <Combobox.Option
            className="cursor-pointer rounded-md py-1.5 pl-2 ui-active:bg-haneul/10 ui-active:shadow-sm"
            value={value}
            key={value.id}
        >
    ...
    return (
        ...
        >
            <Combobox.Input
                ...
                className="box-border w-full rounded-md border border-transparent bg-search-fill/60 pl-2 text-body leading-8 text-white/20 placeholder:text-xs placeholder:text-white/40 hover:bg-search-fill hover:placeholder:text-white/60 focus:border-solid focus:border-haneul focus:bg-search-fill focus:text-white focus:placeholder:text-white/60"
                onChange={onChange}
