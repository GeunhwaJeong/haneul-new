...
function SearchItem({ value }: SearchResultProps) {
	return (
		<Combobox.Option
			className="cursor-pointer rounded-md px-2 py-1.5 ui-active:bg-haneul/10 ui-active:shadow-sm"
			value={value}
			key={value.id}
		>
		...
		<Combobox nullable onChange={onSelectResult} as="div" className="relative w-full">
			<div className="relative flex items-center">
				...
				<Combobox.Input
					...
					className="w-full rounded-md border border-transparent bg-search-fill/60 pl-10 text-body leading-9 text-white/20 outline-none placeholder:text-xs placeholder:text-white/40 hover:bg-search-fill hover:placeholder:text-white/60 focus:border-haneul focus:bg-search-fill focus:text-white focus:placeholder:text-white/60"
					onChange={onChange}
