...
import { HaneulTokenCard } from '~/components/HaneulTokenCard';
...
function Home() {
	...
	const isHaneulTokenCardEnabled = network === Network.MAINNET;

	return (
		<div
			data-testid="home-page"
			className={clsx('home-page-grid-container', {
				'home-page-grid-container-with-haneul-token': isHaneulTokenCardEnabled,
			})}
		>
			...
			{isHaneulTokenCardEnabled && (
				<div style={{ gridArea: 'haneul-token' }}>
					<HaneulTokenCard />
				</div>
			)}

			<div style={{ gridArea: 'gas-price' }}>
				<GasPriceCard useLargeSpacing={!isHaneulTokenCardEnabled} />
			</div>
