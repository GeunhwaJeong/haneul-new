...
import { COIN_TYPE_ARG_REGEX } from '@haneullabs/haneul.js';
...
function Amount({ amount, label }: { amount: bigint; label: string }) {
    ...
    return (
        <div className={styles.haneul}>
            <span className={styles.haneulamount}>{formattedAmount}</span>
            <span className={styles.haneullabel}>{suffix}</span>
        </div>
    );
}
