...
interface ObjectChangeEntriesProps {
	type: HaneulObjectChangeTypes;
	changeEntries: HaneulObjectChange[];
	isDisplay?: boolean;
}
...
function ObjectChangeEntries({ changeEntries, type, isDisplay }: ObjectChangeEntriesProps) {
	...
	if (type === 'published') {
		...
		expandableItems = (changeEntries as HaneulObjectChangeWithDisplay[]).map((change) =>
...
function ObjectChangeEntriesCardFooter({
	...
	const { data: haneulnsDomainName } = useResolveHaneulNSName(ownerAddress);
	...
	return (
		<div className="flex flex-wrap items-center justify-between">
			...
			{ownerType === 'AddressOwner' && (
				<AddressLink label={haneulnsDomainName || undefined} address={ownerAddress} />
			)}
	...
	return (
		<>
			{Object.entries(objectSummary).map(([type, changes]) => (
				<ObjectChangeEntriesCards key={type} type={type as HaneulObjectChangeTypes} data={changes} />
			))}
		</>
	);
}
