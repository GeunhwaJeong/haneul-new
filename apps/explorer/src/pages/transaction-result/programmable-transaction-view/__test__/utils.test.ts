...
import { flattenHaneulArguments } from '~/pages/transaction-result/programmable-transaction-view/utils';

describe('utils.ts', () => {
	describe('flattenCommandData', () => {
		it('should format SplitCoin data', () => {
			expect(flattenHaneulArguments(['GasCoin', { Input: 1 }])).toEqual('GasCoin, Input(1)');
			expect(flattenHaneulArguments(['GasCoin', { Result: 2 }])).toEqual('GasCoin, Result(2)');
			expect(flattenHaneulArguments(['GasCoin', { NestedResult: [1, 2] }])).toEqual(
