	...
	return {
		data: [...validators]
			...
			.map((validator) => {
				...
				const { apy, isApyApproxZero } = rollingAverageApys?.[validator.haneulAddress] ?? {
					apy: null,
				};
		...
		columns: [
			...
			},
			{
				...
				cell: (props: any) => {
					...
					return atRisk !== null ? (
						<Tooltip
							tip="Staked HANEUL is below the minimum HANEUL stake threshold to remain a validator."
							onOpen={() =>
...
function ValidatorPageResult() {
	const { data, isPending, isSuccess, isError } = useHaneulClientQuery('getLatestHaneulSystemState');
	...
	const totalStaked = useMemo(() => {
		...
		return validators.reduce((acc, cur) => acc + Number(cur.stakingPoolHaneulBalance), 0);
	}, [data]);
