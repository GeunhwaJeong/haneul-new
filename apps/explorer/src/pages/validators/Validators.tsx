...
import { type Validator, type HaneulEventEnvelope } from '@haneullabs/haneul.js';
...
export function validatorsTableData(
    ...
    epoch: number,
    validatorsEvents: HaneulEventEnvelope[],
    minimumStake: number
) {
    return {
        data: validators.map((validator) => {
            const validatorName = validator.metadata.name;
            const totalStake = validator.staking_pool.haneul_balance;
            ...
            return {
                ...
                img: img,
                address: validator.metadata.haneul_address,
...
function ValidatorPageResult() {
    ...
    const totalStaked = useMemo(() => {
        ...
        return validators.reduce(
            (acc, cur) => acc + +cur.staking_pool.haneul_balance,
            0
        );
    }, [data]);
