...
export function getValidatorState(network: string): Promise<ValidatorState> {
    return rpc(network)
        ...
        .then((objState: GetObjectDataResponse) => {
            if (
                isHaneulObject(objState.details) &&
                isHaneulMoveObject(objState.details.data)
            ) {
                return objState.details.data.fields as ValidatorState;
            }

            throw new Error(
                'haneul system state information not shaped as expected'
            );
        });
}
...
export function processValidators(set: Validator[]) {
    return set.map((av, i) => {
        ...
        return {
            name: name,
            address: av.fields.metadata.fields.haneul_address,
