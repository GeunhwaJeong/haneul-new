...
import { isHaneulNSName, useResolveHaneulNSAddress, useResolveHaneulNSName } from '@haneullabs/core';
import { Domain32 } from '@haneullabs/icons';
import { Heading, LoadingIndicator } from '@haneullabs/ui';
...
function AddressResultPageHeader({ address, loading }: { address: string; loading?: boolean }) {
	const { data: domainName, isFetching } = useResolveHaneulNSName(address);
...
function HaneulNSAddressResultPageHeader({ name }: { name: string }) {
	const { data: address, isFetching } = useResolveHaneulNSAddress(name);
...
function HaneulNSAddressResult({ name }: { name: string }) {
	const { isFetched, data } = useResolveHaneulNSAddress(name);
...
export default function AddressResultPage() {
	...
	return (
		<PageLayout
			gradient={{
				size: 'md',
				content: isHaneulNSAddress ? (
					<HaneulNSAddressResultPageHeader name={id!} />
				) : (
					<AddressResultPageHeader address={id!} />
				),
			}}
			content={isHaneulNSAddress ? <HaneulNSAddressResult name={id!} /> : <AddressResult address={id!} />}
		/>
	);
}
