...
const getAtRiskRemainingEpochs = (
	data: HaneulSystemStateSummary | undefined,
...
function ValidatorDetails() {
	const { id } = useParams();
	const { data, isPending } = useHaneulClientQuery('getLatestHaneulSystemState');

	const validatorData = useMemo(() => {
		if (!data) return null;
		return (
			data.activeValidators.find(
				({ haneulAddress, stakingPoolId }) => haneulAddress === id || stakingPoolId === id,
			) || null
		);
	}, [id, data]);
						...
						<div className="mt-5">
							...
							>
								<Text variant="bodySmall/medium">
									Staked HANEUL is below the minimum HANEUL stake threshold to remain a validator.
								</Text>
							</Banner>
						</div>
					)}
