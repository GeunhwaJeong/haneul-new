		...
		<div className="flex flex-col space-y-16">
			<div className="grid grid-flow-row gap-4 sm:gap-2 md:flex md:gap-6">
				...
				<EpochStats label="Rewards">
					<HaneulStats label="Total Stake" tooltip="" amount={epochData.endOfEpochInfo?.totalStake} />
					<HaneulStats label="Stake Subsidies" amount={epochData.endOfEpochInfo?.stakeSubsidyAmount} />
					<HaneulStats
						label="Stake Rewards"
						amount={epochData.endOfEpochInfo?.totalStakeRewardsDistributed}
					/>
					<HaneulStats label="Gas Fees" amount={epochData.endOfEpochInfo?.totalGasFees} />
				</EpochStats>

				<EpochStats label="Storage Fund Balance">
					<HaneulStats label="Fund Size" amount={epochData.endOfEpochInfo?.storageFundBalance} />
					<HaneulStats label="Net Inflow" amount={netInflow} />
					<HaneulStats label="Fund Inflow" amount={fundInflow} />
					<HaneulStats label="Fund Outflow" amount={fundOutflow} />
				</EpochStats>
