...
import { type HaneulMoveNormalizedStruct, type HaneulObjectResponse } from '@haneullabs/haneul.js/client';
import { Text, LoadingIndicator, Combobox, ComboboxInput, ComboboxList } from '@haneullabs/ui';
...
interface ObjectFieldsProps {
	id: string;
	normalizedStructData?: HaneulMoveNormalizedStruct;
	haneulObjectResponseData?: HaneulObjectResponse;
	loading: boolean;
	error: boolean;
	objectType?: string;
}
...
export function ObjectFieldsCard({
	...
	const fieldsData =
		haneulObjectResponseData?.data?.content?.dataType === 'moveObject'
			? (haneulObjectResponseData?.data?.content?.fields as Record<string, string | number | object>)
			: null;
				...
				>
					{normalizedStructData?.fields?.map(({ name, type }) => (
						<button
							type="button"
							key={name}
							className="mt-0.5 rounded-lg border border-transparent p-2.5 hover:border-haneul-primaryBlue2023/20 hover:bg-white/60"
