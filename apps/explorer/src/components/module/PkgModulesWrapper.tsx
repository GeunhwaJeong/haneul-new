...
function PkgModuleViewWrapper({ id, modules }: Props) {
    ...
    return (
        <div className="flex flex-col md:flex-row md:flex-nowrap gap-5 border-0 border-y border-solid border-haneul-grey-45">
            <div className="w-full md:w-1/5">
                <Combobox value={selectedModule} onChange={onChangeModule}>
                    <div className="box-border border border-haneul-grey-50 border-solid rounded-md shadow-sm placeholder-haneul-grey-65 pl-3 w-full flex mt-2.5 justify-between py-1">
                        <Combobox.Input
                        ...
                        <button
                            ...
                            type="submit"
                        >
                            <SearchIcon className="fill-haneul-steel cursor-pointer h-4.5 w-4.5 align-middle" />
                        </button>
                    </div>
                    <Combobox.Options className="overflow-auto absolute left-0 w-full box-border md:w-1/6 md:left-auto h-fit max-h-verticalListLong overflow-auto shadow-moduleOption rounded-md bg-white z-10 px-2 pb-5 pt-3 flex flex-col gap-1">
                        {filteredModules.length > 0 ? (
                            <div className="text-caption font-semibold ml-1.5 pb-2 uppercase text-haneul-grey-75">
                            ...
                            <div className="text-haneul-grey-70 text-body italic pt-2 px-3.5 text-center">
                                No results
                            </div>
                        )}
                                ...
                                {({ active }) => (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            active
                                                ? 'bg-haneul/10 text-haneul-grey-80 border-transparent'
                                                : 'bg-white text-haneul-grey-80 font-medium border-transparent'
                                        )}
                                    >
                                        {name}
                                    </button>
            ...
            <div className="border-0 md:border-l border-solid border-haneul-grey-45 md:pl-7 pt-5 grow overflow-auto">
                <TabGroup size="md">
                    <TabList>
                        <Tab>Bytecode</Tab>
                    </TabList>
