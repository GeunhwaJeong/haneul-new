...
const APP_NAME = WALLET_BETA ? 'Haneul Wallet (BETA)' : IS_DEV ? 'Haneul Wallet (DEV)' : 'Haneul Wallet';
...
const commonConfig: () => Promise<Configuration> = async () => {
	...
	return {
		...
		resolve: {
			...
			// Fix .js imports from @haneullabs/haneul since we are importing it from source
			extensionAlias: {
				'.js': ['.js', '.ts', '.tsx', '.jsx'],
				'.mjs': ['.mjs', '.mts'],
				'.cjs': ['.cjs', '.cts'],
