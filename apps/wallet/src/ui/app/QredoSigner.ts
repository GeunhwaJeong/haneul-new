...
export class QredoSigner extends WalletSigner {
    ...
    signMessage: WalletSigner['signMessage'] = async (
        input,
        clientIdentifier
    ) => {
    ...
    signTransactionBlock: WalletSigner['signTransactionBlock'] = async (
        input,
        clientIdentifier
    ) => {
    ...
    async #createQredoTransaction(
        ...
        if (!this.#network) {
            throw new Error(
                `Unsupported network ${networkNames[this.#apiEnv]}`
            );
        }
