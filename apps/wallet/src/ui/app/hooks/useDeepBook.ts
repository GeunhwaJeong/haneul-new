...
import { roundFloat, useGetObject } from '@haneullabs/core';
...
export async function getCoinsByBalance({
	...
	while (currentBalance < bigIntBalance && hasNextPage) {
		const { data, nextCursor } = await haneulClient.getCoins({
			owner: address,
			coinType,
			cursor,
...
async function getPlaceMarketOrderTxn({
	...
	if (isAsk) {
		...
		if (bigNumberBaseBalance.isLessThan(ONE_HANEUL_DEEPBOOK)) {
