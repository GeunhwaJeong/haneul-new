...
export function filterTokenBalances(tokens: CoinBalance[]) {
	return tokens.filter(
		(token) => Number(token.totalBalance) > 0 || token.coinType === HANEUL_TYPE_ARG,
	);
}
...
export function useValidSwapTokensList() {
	...
	const normalizedPackages = useMemo(
		() => packages.map((id) => normalizeHaneulObjectId(id)),
		[packages],
	);
	...
	const validSwaps = useMemo(
		() =>
			supported?.sort((a, b) => {
				const haneulType = normalizeStructTag(HANEUL_TYPE_ARG);
				...
				return a === haneulType ? -1 : b === haneulType ? 1 : Number(balanceB - balanceA);
			}) ?? [],
