...
export function getLedgerConnectionErrorMessage(error: unknown) {
    if (error instanceof LedgerConnectionFailedError) {
        ...
        return 'Connect your Ledger device and open the Haneul app.';
 ...
 * operations on the Haneul Ledger application
 */
export function getHaneulApplicationErrorMessage(error: unknown) {
    if (error instanceof LockedDeviceError) {
        ...
        if (error.statusCode === StatusCodes.INS_NOT_SUPPORTED) {
            ...
            return 'Make sure the Haneul app is open on your device.';
        }
    }
    return null;
}
