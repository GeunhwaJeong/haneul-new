...
import { decodeHaneulPrivateKey, encodeHaneulPrivateKey } from '@haneullabs/haneul.js/cryptography/keypair';
...
export const privateKeyValidation = z
	...
	.transform((privateKey, context) => {
		if (!privateKey.startsWith('haneulprivkey')) {
			...
			return encodeHaneulPrivateKey(privateKeyBytes.slice(0, 32), 'ED25519');
		}
		try {
			decodeHaneulPrivateKey(privateKey);
		} catch (error) {
			context.addIssue({
				code: 'custom',
				message: 'Invalid Private Key, please use a Bech32 encoded 33-byte string',
