...
import {
	...
	useHaneulUsdcBalanceConversion,
	useSwapData,
...
import { type BalanceChange, type DryRunTransactionBlockResponse } from '@haneullabs/haneul.js/client';
import { HANEUL_DECIMALS, HANEUL_TYPE_ARG } from '@haneullabs/haneul.js/utils';
...
export function SwapPageContent() {
	...
	const baseCoinType = HANEUL_TYPE_ARG;
	const quoteCoinType = coinsMap.USDC;

	const poolId = mainnetPools.HANEUL_USDC[0];
	...
	const { data: coinBalances } = useHaneulClientQuery(
		'getAllBalances',
	...
	const rawInputHaneulUsdc = haneulUsdc.data?.rawValue;
	const rawInputUsdcHaneul = usdcHaneul.data?.rawValue;
	...
	const baseBalance = new BigNumber(isAsk ? amount || 0 : rawInputUsdcHaneul || 0)
		.shiftedBy(HANEUL_DECIMALS)
		.toString();
	const quoteBalance = new BigNumber(isAsk ? rawInputHaneulUsdc || 0 : amount || 0)
		.shiftedBy(HANEUL_CONVERSION_RATE)
		.toString();
									...
									{isValid && !!amount && (
										<div className="ml-3 mt-3">
											<div className="text-bodySmall font-medium text-hero-darkest/40">
												{isPayAll ? '~ ' : ''}
												{getUSDCurrency(isAsk ? rawInputHaneulUsdc : Number(amount))}
											</div>
										</div>
									)}
								</div>
									...
									onClick={() => {
										navigate(
											`/swap?${new URLSearchParams({
												type: activeCoinType === HANEUL_TYPE_ARG ? coinsMap.USDC : HANEUL_TYPE_ARG,
											}).toString()}`,
										);
										reset();
									}}
