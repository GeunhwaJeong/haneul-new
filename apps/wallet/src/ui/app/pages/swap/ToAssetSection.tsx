...
export function ToAssetSection({
	...
	const toAssetAmountAsNum = new BigNumber(rawToAssetAmount || '0')
		.shiftedBy(isAsk ? -HANEUL_CONVERSION_RATE : -USDC_CONVERSION_RATE)
		.toNumber();

	useEffect(() => {
		const newToAsset = isAsk ? coinsMap[Coins.USDC] : HANEUL_TYPE_ARG;
	...
	return (
		<div
			className={clsx(
				...
				{ 'bg-haneul-primaryBlue2023/10': isValid },
			)}
		>
			<ToAssets
