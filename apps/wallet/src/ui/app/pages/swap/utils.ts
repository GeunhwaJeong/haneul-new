...
import { type TransactionEffects } from '@haneullabs/haneul/client';
import { Transaction } from '@haneullabs/haneul/transactions';
import { normalizeStructTag, HANEUL_DECIMALS, HANEUL_TYPE_ARG } from '@haneullabs/haneul/utils';
...
export function useCoinTypesFromRouteParams() {
	...
	// Neither is set, default to HANEUL -> USDC
	if (!fromCoinType && !toCoinType) {
		return { fromCoinType: HANEUL_TYPE_ARG, toCoinType: USDC_TYPE_ARG };
	}
...
export function formatSwapQuote({
	...
	const gas = formatBalance(getTotalGasCost(dryRunResponse.effects), HANEUL_DECIMALS);
