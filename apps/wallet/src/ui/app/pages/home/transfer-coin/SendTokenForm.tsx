...
function GasBudgetEstimation({
	...
	const [formattedGas] = useFormatCoin(gasBudget, HANEUL_TYPE_ARG);
...
export function SendTokenForm({
	...
	const { data: haneulCoinsData, isPending: haneulCoinsIsPending } = useGetAllCoins(
		HANEUL_TYPE_ARG,
		activeAddress!,
	);
	...
	return (
		<Loading
			loading={
				queryResult.isPending || coinMetadata.isPending || haneulCoinsIsPending || coinsIsPending
			}
		>
			<Formik
				initialValues={{
					...
					to: initialTo,
					isPayAllHaneul:
						!!initAmountBig && initAmountBig === coinBalance && coinType === HANEUL_TYPE_ARG,
					gasBudgetEst: '',
				}}
				...
				validateOnMount
				validateOnChange
				onSubmit={async ({ to, amount, isPayAllHaneul, gasBudgetEst }: FormValues) => {
			...
			>
				{({ isValid, isSubmitting, setFieldValue, values, submitForm, validateField }) => {
					const newPayHaneulAll =
						parseAmount(values.amount, coinDecimals) === coinBalance && coinType === HANEUL_TYPE_ARG;
					...
					const hasEnoughBalance =
						...
						haneulBalance >
							parseAmount(values.gasBudgetEst, coinDecimals) +
								parseAmount(coinType === HANEUL_TYPE_ARG ? values.amount : '0', coinDecimals);
							...
							<Content>
								<Form autoComplete="off" noValidate>
									...
									{!hasEnoughBalance && isValid ? (
										<div className="mt-3">
											<Alert>Insufficient HANEUL to cover transaction</Alert>
										</div>
									) : null}
