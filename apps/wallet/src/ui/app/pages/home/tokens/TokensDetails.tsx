...
import { useAppsBackend, useCoinMetadata, useFormatCoin, useResolveHaneulNSName } from '@haneullabs/core';
...
export function TokenRow({
	...
	return (
		<Tag
			className={clsx(
				'group flex py-3 pl-1.5 pr-2 rounded hover:bg-haneul/10 items-center bg-transparent border-transparent',
...
function TokenDetails({ coinType }: TokenDetailsProps) {
	...
	const accountHasHaneul = coinBalances?.some(({ coinType }) => coinType === HANEUL_TYPE_ARG);
									...
									<div className="grid grid-cols-3 gap-3 w-full">
										{isMainnet ? (
											<LargeButton
												spacing="sm"
												className={
													!accountHasHaneul && isMainnet
														? 'col-span-3 !bg-haneul-primaryBlue2023 !text-white'
														: ''
												}
												primary={!accountHasHaneul}
												center
												to="/onramp"
