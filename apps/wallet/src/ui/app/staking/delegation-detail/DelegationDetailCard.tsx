...
export function DelegationDetailCard({ validatorAddress, stakedId }: DelegationDetailCardProps) {
	...
	const showRequestMoreHaneulToken = useMemo(() => {
		if (!haneulCoinBalance?.totalBalance || !metadata?.decimals || apiEnv === API_ENV.mainnet)
			return false;
		const currentBalance = new BigNumber(haneulCoinBalance.totalBalance);
		const minStakeAmount = new BigNumber(MIN_NUMBER_HANEUL_TO_STAKE).shiftedBy(metadata.decimals);
		return currentBalance.lt(minStakeAmount.toString());
	}, [apiEnv, metadata?.decimals, haneulCoinBalance?.totalBalance]);
	...
	const delegationId = delegationData?.status === 'Active' && delegationData?.stakedHaneulId;
	...
	return (
		<div className="flex flex-col flex-nowrap flex-grow h-full">
			<BottomMenuLayout>
				<Content>
					<div className="justify-center w-full flex flex-col items-center">
						{hasInactiveValidatorDelegation ? (
							<div className="mb-3">
								<Alert>
									Unstake HANEUL from this inactive validator and stake on an active validator to start
								...
								header={
									<div className="grid grid-cols-2 divide-x divide-solid divide-gray-45 divide-y-0 w-full">
										...
										<CardItem title="Earned">
											<StakeAmount balance={haneulEarned} variant="heading5" isEarnedRewards />
										</CardItem>
									</div>
								}
								padding="none"
				...
				{showRequestMoreHaneulToken ? (
					<div className="flex flex-col gap-4 items-center">
						<div className="w-8/12 text-center">
							<Text variant="pSubtitle" weight="medium" color="steel-darker">
								You need a minimum of {MIN_NUMBER_HANEUL_TO_STAKE} HANEUL to continue staking.
							</Text>
						</div>
