...
export function DelegationDetailCard({
    ...
    const showRequestMoreHaneulToken = useMemo(() => {
        if (
            !haneulCoinBalance?.totalBalance ||
            !metadata?.decimals ||
            apiEnv === API_ENV.mainnet
        )
            return false;
        const currentBalance = new BigNumber(haneulCoinBalance.totalBalance);
        const minStakeAmount = new BigNumber(MIN_NUMBER_HANEUL_TO_STAKE).shiftedBy(
            metadata.decimals
        );
                ...
                <Content>
                    <div className="justify-center w-full flex flex-col items-center">
                        {hasInactiveValidatorDelegation ? (
                            <div className="mb-3">
                                <Alert>
                                    Unstake HANEUL from this inactive validator and
                    ...
                    <div className="flex flex-col gap-4 items-center">
                        <div className="w-8/12 text-center">
                            <Text
                                ...
                                color="steel-darker"
                            >
                                You need a minimum of {MIN_NUMBER_HANEUL_TO_STAKE}{' '}
                                HANEUL to continue staking.
                            </Text>
                        </div>
