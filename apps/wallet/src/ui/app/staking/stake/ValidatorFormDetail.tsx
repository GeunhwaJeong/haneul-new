...
export function ValidatorFormDetail({
    ...
    const validatorData = useMemo(() => {
        ...
        return validatorsData.validators.fields.active_validators.find(
            (av) => av.fields.metadata.fields.haneul_address === validatorAddress
        );
    }, [validatorAddress, validatorsData]);

    const totalValidatorStake =
        validatorData?.fields.delegation_staking_pool.fields.haneul_balance || 0;
    ...
    const totalStake = useMemo(() => {
        ...
        allDelegation.forEach((event) => {
            if (event.staked_haneul.validator_address === validatorAddress) {
                totalActiveStake += BigInt(event.staked_haneul.principal.value);
            }
        });
                        ...
                        {!unstake && (
                            <div className="flex gap-2 items-center justify-between mb-3.5">
                                <div className="flex gap-1 items-baseline text-steel">
                                    ...
                                    <IconTooltip tip="The total HANEUL staked on the network by this validator and its delegators, to validate the network and earn rewards." />
                                </div>
                                <StakeAmount
