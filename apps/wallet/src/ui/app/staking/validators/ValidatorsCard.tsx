...
export function ValidatorsCard() {
	...
	const totalStake = useMemo(() => {
		...
		return getAllStakeHaneul(delegatedStake);
	}, [delegatedStake]);
	...
	const delegations = useMemo(() => {
		return delegatedStake?.flatMap((delegation) => {
			return delegation.stakes.map((d) => ({
				...d,
				// flag any inactive validator for the stakeHaneul object
		...
		<div className="flex flex-col flex-nowrap h-full w-full">
			<BottomMenuLayout>
				<Content>
					<div className="mb-4">
						{hasInactiveValidatorDelegation ? (
							<div className="mb-3">
								<Alert>
									Unstake HANEUL from the inactive validators and stake on an active validator to start
				...
				<Menu stuckClass="staked-cta" className="w-full px-0 pb-0 mx-0">
					<Button size="tall" variant="secondary" to="new" before={<Plus12 />} text="Stake HANEUL" />
				</Menu>
			</BottomMenuLayout>
		</div>
	);
