...
export function DelegationCard({
    ...
    const { staked_haneul } = delegationObject;
    const address = staked_haneul.validator_address;
    const staked = staked_haneul.principal.value;
    const rewards = useMemo(
        () => getStakingRewards(activeValidators, delegationObject),
        [activeValidators, delegationObject]
    );

    const stakedId = staked_haneul.id.id;
    const delegationsRequestEpoch = staked_haneul.delegation_request_epoch;
    const numberOfEpochPastRequesting = currentEpoch - delegationsRequestEpoch;
    const [stakedFormatted] = useFormatCoin(staked, HANEUL_TYPE_ARG);
    const [rewardsFormatted] = useFormatCoin(rewards, HANEUL_TYPE_ARG);
    ...
    return (
        <Link
            ...
            className="flex no-underline flex-col p-3 box-border h-36 w-full rounded-2xl border hover:bg-haneul/10 group border-solid border-gray-45 hover:border-haneul/30 bg-transparent"
        >
            <div className="flex justify-between items-start mb-1">
                ...
                <div className="text-gray-60 text-p1 opacity-0 group-hover:opacity-100">
                    <IconTooltip
                        tip="Object containing the delegated staked HANEUL tokens, owned by each delegator"
                        placement="top"
                    />
                </div>
            </div>
