...
export function Transaction({
    ...
    const transferAmount = useMemo(() => {
        ...
        const amountTransfersHaneul = eventsSummary.find(
            ({ receiverAddress, coinType }) =>
                receiverAddress === address && coinType === HANEUL_TYPE_ARG
        );

        // Find non-HANEUL transfer amount
        const amountTransfersNonHaneul = eventsSummary.find(
            ({ receiverAddress, coinType }) =>
                receiverAddress === address && coinType !== HANEUL_TYPE_ARG
        );
    ...
    const isHaneulTransfer =
        txnKind === 'PayHaneul' ||
        txnKind === 'TransferHaneul' ||
        txnKind === 'PayAllHaneul';

    const isTransfer =
        isHaneulTransfer || txnKind === 'Pay' || txnKind === 'TransferObject';
    ...
    const showHaneulSymbol =
        (transferAmount.coinType === HANEUL_TYPE_ARG && isHaneulTransfer) ||
