...
export function UnStakeTxnCard({ event }: UnStakeTxnCardProps) {
	...
	const [formatPrinciple, symbol] = useFormatCoin(principalAmount, HANEUL_TYPE_ARG);
	const [formatRewards] = useFormatCoin(rewardAmount || 0, HANEUL_TYPE_ARG);
	...
	return (
		<Card>
			<div className="flex flex-col divide-y divide-solid divide-gray-40 divide-x-0">
				...
				{totalAmount && <TxnAmount amount={totalAmount} coinType={HANEUL_TYPE_ARG} label="Total" />}
