...
function ReceiptCard({ txn, activeAddress }: ReceiptCardProps) {
    ...
    const totalHaneulAmount = useMemo(() => {
        const amount = transferAmount.find(
            ({ receiverAddress, coinType }) =>
                receiverAddress === activeAddress && coinType === HANEUL_TYPE_ARG
        )?.amount;
            ...
            <ReceiptCardBg status={isSuccessful}>
                ...
                {isStakeTxn ? (
                    ...
                    <div className="divide-y divide-solid divide-steel/20 divide-x-0 flex flex-col">
                        ...
                        {gasTotal && isSender ? (
                            <TxnGasSummery
                                totalGas={gasTotal}
                                transferAmount={totalHaneulAmount}
                            />
                        ) : null}
                    </div>
                )}
                ...
                <div className="flex gap-1.5 w-full py-3.5">
                    <ExplorerLink
                        ...
                        className="text-haneul-dark text-p4 font-semibold no-underline uppercase tracking-wider"
                        showIcon={false}
                    >
                        View on Explorer
                    </ExplorerLink>
