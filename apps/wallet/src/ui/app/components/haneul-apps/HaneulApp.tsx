...
function CardView({ name, link, icon }: CardViewProp) {
	...
	return (
		<div className="bg-white flex flex-col p-3.75 box-border w-full rounded-2xl border border-gray-45 border-solid h-32 hover:bg-haneul/10 hover:border-haneul/30">
...
function ListView({ name, icon, description, tags }: ListViewProp) {
	return (
		<div className="bg-white flex py-3.5 px-1.25 gap-3 item-center box-border rounded hover:bg-haneul/10">
			<ImageIcon src={icon || null} label={name} fallback={name} size="xxl" rounded="lg" />
			<div className="flex flex-col gap-1 justify-center">
				<Text variant="body" weight="semibold" color="haneul-dark">
					{name}
				</Text>
...
export interface HaneulAppProps {
	name: string;
	description: string;
	link: string;
	icon: string;
