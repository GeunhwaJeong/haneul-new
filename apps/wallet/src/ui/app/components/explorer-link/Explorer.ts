...
const EXPLORER_LINK = 'https://haneulexplorer.com/';
...
export function getAddressUrl(address: HaneulAddress, apiEnv: API_ENV, customRPC: string) {
	return getExplorerUrl(`/address/${address}`, apiEnv, customRPC);
}

export function getValidatorUrl(address: HaneulAddress, apiEnv: API_ENV, customRPC: string) {
