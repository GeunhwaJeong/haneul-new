...
>(
	...
	async ({ password }, { extra: { background } }) => await background.getEntropy(password),
);
...
const accountsAdapter = createEntityAdapter<SerializedAccount>({
	...
	sortComparer: (a, b) => {
		if (a.type !== b.type) {
			...
			return (a.derivationPath || '').localeCompare(b.derivationPath || '', undefined, {
				numeric: true,
			});
		} else {
...
const accountSlice = createSlice({
	...
	reducers: {
		setKeyringStatus: (
			state,
			{ payload }: PayloadAction<Required<KeyringPayload<'walletStatusUpdate'>>['return']>,
		) => {
