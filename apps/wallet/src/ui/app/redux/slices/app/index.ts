...
export const changeRPCNetwork = createAsyncThunk<void, API_ENV, AppThunkConfig>(
    ...
    (networkName, { extra: { api }, dispatch, getState }) => {
        ...
        Browser.storage.local.set({ haneul_Env: networkName });
    }
);
...
export const setCustomRPC = createAsyncThunk<void, string, AppThunkConfig>(
    ...
    (customRPC, { dispatch }) => {
        // Set persistent network state
        Browser.storage.local.set({ haneul_Env_RPC: customRPC });
...
>('initNetworkFromStorage', async (_, { dispatch, extra: { api } }) => {
    const result = await Browser.storage.local.get(['haneul_Env', 'haneul_Env_RPC']);

    const isValidCustomRPC = result?.haneul_Env_RPC?.length > 0;

    const network =
        result.haneul_Env === API_ENV.customRPC && !isValidCustomRPC
            ? DEFAULT_API_ENV
            : result.haneul_Env;

    const customRPCURL =
        network === API_ENV.customRPC && isValidCustomRPC
            ? result.haneul_Env_RPC
            : null;

    if (result.haneul_Env_RPC && customRPCURL) {
        dispatch(setCustomRPCURL(result.haneul_Env_RPC));
    }
