...
import type { HaneulObjectData, HaneulAddress, ObjectId } from '@haneullabs/haneul.js';
import type { RootState } from '_redux/RootReducer';
import type { AppThunkConfig } from '_store/thunk-extras';

const objectsAdapter = createEntityAdapter<HaneulObjectData>({
...
>('haneul-objects/fetch-all', async (_, { getState, extra: { api } }) => {
    ...
    const allHaneulObjects: HaneulObjectData[] = [];
    if (address) {
        const allObjectRefs =
        ...
        for (const objRes of allObjRes) {
            const haneulObj = getHaneulObjectData(objRes);
            if (haneulObj) {
