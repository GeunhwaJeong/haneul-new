...
import { toSerializedSignature, type PublicKey } from '@haneullabs/haneul/cryptography';
...
async function hasTransactionHistory(address: string): Promise<boolean> {
	const rpc = getHaneulClient({ env: API_ENV.mainnet, customRpcUrl: null });
	...
	implements SigningAccount
{
	...
	static async createNew({
		...
		if (normalizeHaneulAddress(legacyAddress) !== normalizeHaneulAddress(nonLegacyAddress)) {
