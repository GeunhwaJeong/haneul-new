...
import {
	...
	type HaneulSignPersonalMessageMethod,
...
export class HaneulWallet implements Wallet {
	...
	constructor() {
		this.#events = mitt();
		this.#accounts = [];
		this.#messagesStream = new WindowMessageStream('haneul_in-page', 'haneul_content-script');
	...
	#signTransaction: HaneulSignTransactionMethod = async ({ transaction, account, ...input }) => {
		return mapToPromise(
			this.#send<SignTransactionRequest, SignTransactionResponse>({
				type: 'sign-transaction-request',
				transaction: {
	...
	#signAndExecuteTransaction: HaneulSignAndExecuteTransactionMethod = async (input) => {
		return mapToPromise(
			this.#send<ExecuteTransactionRequest, ExecuteTransactionResponse>({
				type: 'execute-transaction-request',
				transaction: {
	...
	#setActiveChain({ env }: NetworkEnvType) {
		this.#activeChain = env === API_ENV.customRPC ? 'haneul:unknown' : API_ENV_TO_CHAIN[env];
	}
