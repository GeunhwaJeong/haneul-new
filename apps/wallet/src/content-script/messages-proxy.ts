...
function createPort(windowMsgStream: WindowMessageStream, currentMsg?: Message) {
	const port = PortStream.connectToBackgroundService('haneul_content<->background');
	if (currentMsg) {
...
export function setupMessagesProxy() {
	const windowMsgStream = new WindowMessageStream('haneul_content-script', 'haneul_in-page');
