...
test('staking', async ({ page, extensionUrl }) => {
	...
	await page.getByText(/Stake and Earn HANEUL/).click();
	...
	await expect(page.getByText(`Currently Staked${STAKE_AMOUNT} HANEUL`)).toBeVisible({
		timeout: TEST_TIMEOUT,
	});

	await page.getByText(`Currently Staked${STAKE_AMOUNT} HANEUL`).click();
