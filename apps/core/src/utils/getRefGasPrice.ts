...
import { HaneulValidatorSummary } from '@haneullabs/haneul/client';
 ...
 */
export function getRefGasPrice(validators?: HaneulValidatorSummary[]) {
	...
	const totalStaked = validators.reduce(
		(acc, cur) => acc + BigInt(cur.stakingPoolHaneulBalance),
		BigInt(0),
	);
	...
	for (let i = 0; i < sortedByGasPrice.length; i++) {
		...
		const stake = BigInt(validator?.stakingPoolHaneulBalance);
