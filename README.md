# Haneul README

Welcome to Haneul, a next generation smart contract platform with high throughput, low latency, and an asset-oriented programming model powered by the [Move](https://github.com/GeunhwaJeong/awesome-move) programming language! Find everything you need within the [Haneul Developer Guides](doc/src/index.md) and supporting materials.

* To jump right into building smart contract applications on top of Haneul, start at the [Move Smart Contract Quick Start](doc/src/move.md).
* To experiment with a sample Haneul wallet, check out [Wallet Quick Start](doc/src/build/wallet.md).
* To understand what's possible by browsing Move code built on top of Haneul, review the [examples](haneul_programmability/examples).
* To start coding against Haneul's REST APIs, start at the [API reference](https://app.swaggerhub.com/apis/arun-koshy/haneul-api).
* To learn what distinguishes Haneul from other blockchain systems, see [What Makes Haneul Different?](doc/src/learn/what-makes-haneul-different.md).
* To go deep on how Haneul works, read the [Haneul Smart Contract Platform](doc/paper/haneul.pdf) white paper.