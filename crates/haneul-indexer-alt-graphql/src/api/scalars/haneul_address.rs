...
pub(crate) enum Error {
    ...
    #[error("Missing '0x' prefix while parsing HaneulAddress: {0:?}")]
    NoPrefix(String),
...
mod tests {
    ...
    const FULL_ADDRESS: HaneulAddress = HaneulAddress([
    ...
    const SHORT_ADDRESS: HaneulAddress = HaneulAddress([
    ...
    #[test]
    fn test_parse_full() {
        let parsed = HaneulAddress::from_str(FULL_ADDRESS_STR).unwrap();
        assert_eq!(parsed, FULL_ADDRESS);
    }

    #[test]
    fn test_parse_short_odd() {
        let parsed = HaneulAddress::from_str(ODD_SHORT_ADDRESS_STR).unwrap();
        assert_eq!(parsed, SHORT_ADDRESS);
    }

    #[test]
    fn test_parse_short_even() {
        let parsed = HaneulAddress::from_str(EVEN_SHORT_ADDRESS_STR).unwrap();
        assert_eq!(parsed, SHORT_ADDRESS);
    }
    ...
    fn test_parse_no_prefix() {
        let err = HaneulAddress::from_str("123456789").unwrap_err();
        assert!(matches!(err, Error::NoPrefix(_)), "{err:?}");
    }

    #[test]
    fn test_parse_invalid_prefix() {
        let err = HaneulAddress::from_str("1x123456789").unwrap_err();
        assert!(matches!(err, Error::NoPrefix(_)), "{err:?}");
    }

    #[test]
    fn test_parse_too_short() {
        let err = HaneulAddress::from_str("0x").unwrap_err();
    ...
    fn test_unicode_char_boundary() {
        let parsed = HaneulAddress::from_str("0x…0123");
        assert!(parsed.is_err());
    }
