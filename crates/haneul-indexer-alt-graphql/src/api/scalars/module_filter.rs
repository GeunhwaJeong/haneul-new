...
impl ModuleFilter {
    pub(crate) fn package(&self) -> Option<HaneulAddress> {
        match self {
...
impl FromStr for ModuleFilter {
    ...
    fn from_str(s: &str) -> Result<Self, ModuleFilterError> {
        if let Ok(module) = parse_haneul_module_id(s) {
            // Try as a module ID
            Ok(ModuleFilter::Module(
                HaneulAddress::from(*module.address()),
                module.name().to_string(),
            ))
        } else if let Ok(package) = parse_haneul_address(s) {
