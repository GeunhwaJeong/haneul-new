...
use haneul_types::{base_types::HaneulAddress as NativeHaneulAddress, dynamic_field::DynamicFieldType};

use crate::{
    api::scalars::{owner_kind::OwnerKind, haneul_address::HaneulAddress, type_filter::TypeInput},
    error::RpcError,
...
#[graphql(
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::haneul::HANEUL`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
impl Address {
    /// Construct an address that is represented by just its identifier (`HaneulAddress`).
