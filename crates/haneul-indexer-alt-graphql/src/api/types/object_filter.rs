...
use crate::{
    api::scalars::{owner_kind::OwnerKind, haneul_address::HaneulAddress, type_filter::TypeFilter},
    intersect,
};
...
mod tests {
    ...
    fn test_type_filter_intersection() {
        ...
        let haneul = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::haneul::HANEUL>".parse().unwrap()),
        };
        ...
        // intersection order.
        assert_eq!(coin.clone().intersect(haneul.clone()).unwrap(), haneul);
        assert_eq!(haneul.clone().intersect(coin.clone()).unwrap(), haneul);
