...
use haneul_types::{dynamic_field::DynamicFieldType, object::MoveObject as NativeMoveObject};
use tokio::sync::OnceCell;

use crate::{
    api::scalars::{
        base64::Base64, big_int::BigInt, id::Id, haneul_address::HaneulAddress, type_filter::TypeInput,
        uint53::UInt53,
    },
    error::RpcError,
...
#[graphql(
    ...
    field(
        ...
        desc = "Whether this object can be transfered using the `TransferObjects` Programmable Transaction Command or `haneul::transfer::public_transfer`.\n\nBoth these operations require the object to have both the `key` and `store` abilities.",
    ),
    field(
