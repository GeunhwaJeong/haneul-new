...
use haneul_indexer_alt_reader::epochs::EpochEndKey;
...
use haneul_types::haneul_system_state::HANEUL_SYSTEM_STATE_INNER_MODULE_NAME;
use haneul_types::haneul_system_state::HANEUL_SYSTEM_STATE_INNER_V1_STRUCT_NAME;
use haneul_types::haneul_system_state::HANEUL_SYSTEM_STATE_INNER_V2_STRUCT_NAME;
...
impl Epoch {
    ...
    async fn coin_deny_list(&self, ctx: &Context<'_>) -> Option<Result<Object, RpcError>> {
        async {
            ...
            Object::latest(ctx, scope, HANEUL_DENY_LIST_OBJECT_ID.into()).await
        }
        .await
        .transpose()
    }
...
impl Epoch {
    ...
    async fn system_state_impl(&self, ctx: &Context<'_>) -> Result<Option<MoveValue>, RpcError> {
        ...
        let struct_name = match start.system_state.first() {
            Some(0) => HANEUL_SYSTEM_STATE_INNER_V1_STRUCT_NAME,
            Some(1) => HANEUL_SYSTEM_STATE_INNER_V2_STRUCT_NAME,
            _ => {
                return Ok(None);
            }
        };
