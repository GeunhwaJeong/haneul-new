    ...
    async fn coin_deny_list(&self, ctx: &Context<'_>) -> Option<Result<Object, RpcError>> {
        async {
            ...
            Object::checkpoint_bounded(
                ctx,
                self.scope.clone(),
                HANEUL_DENY_LIST_OBJECT_ID.into(),
    ...
    async fn storage_fund(&self, ctx: &Context<'_>) -> Option<Result<StorageFund, RpcError>> {
        async {
            ...
            let storage_fund = match system_state {
                HaneulSystemState::V1(inner) => inner.storage_fund.into(),
                HaneulSystemState::V2(inner) => inner.storage_fund.into(),
                #[cfg(msim)]
...
impl Epoch {
    ...
    async fn system_state(&self, ctx: &Context<'_>) -> Result<Option<HaneulSystemState>, RpcError> {
        let Some(start) = self.start(ctx).await? else {
            return Ok(None);
        };

        let system_state = bcs::from_bytes::<HaneulSystemState>(&start.system_state)
