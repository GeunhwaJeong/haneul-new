...
use crate::{
    api::{
        scalars::{
            base64::Base64, big_int::BigInt, haneul_address::HaneulAddress, type_filter::TypeInput,
            uint53::UInt53,
        },
        types::{
            address::Address,
...
impl Validator {
    ...
    async fn staking_pool_haneul_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.native.staking_pool.haneul_balance))
    }
    ...
    async fn pending_total_haneul_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.native.staking_pool.pending_total_haneul_withdraw,
        ))
    }
...
impl Validator {
    pub(crate) fn from_validator_v1(scope: Scope, native: ValidatorV1, at_risk: u64) -> Self {
        Self {
            super_: Address::with_address(scope, native.metadata.haneul_address),
            native,
            at_risk,
        }
    }
