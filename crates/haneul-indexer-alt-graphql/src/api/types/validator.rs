...
use crate::{
    api::{
        scalars::{
            base64::Base64, big_int::BigInt, cursor::JsonCursor, haneul_address::HaneulAddress,
...
impl Validator {
    ...
    pub(crate) async fn address(&self, ctx: &Context<'_>) -> Result<HaneulAddress, RpcError> {
    ...
    async fn operation_cap(
        ...
        let object = match address.as_object(ctx).await.ok()?? {
            Ok(obj) => obj,
            Err(e) => return Some(Err(e)),
        };
        match object.as_move_object(ctx).await.ok()?? {
            Ok(obj) => Some(Ok(obj)),
            Err(e) => Some(Err(upcast(e))),
        }
    }

    /// The ID of this validator's `0x3::staking_pool::StakingPool`.
    async fn staking_pool_id(&self) -> HaneulAddress {
    ...
    /// The exchange rate is used to determine the amount of HANEUL tokens that each past HANEUL staker can withdraw in the future.
    ...
    async fn staking_pool_haneul_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.haneul_balance))
    }
    ...
    async fn pending_total_haneul_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator().staking_pool.pending_total_haneul_withdraw,
        ))
    }
    ...
    /// The total number of HANEUL tokens in this pool plus the pending stake amount for this epoch.
    ...
    async fn at_risk(&self) -> Option<UInt53> {
        let at_risk = self
            ...
            .native
            .at_risk_validators
            .get(&self.validator().metadata.haneul_address)
...
impl Validator {
    fn super_(&self) -> Address {
        Address::with_address(
            self.contents.scope.clone(),
            self.validator().metadata.haneul_address,
        )
    }
