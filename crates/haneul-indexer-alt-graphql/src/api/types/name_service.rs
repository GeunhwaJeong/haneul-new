...
/// Attempt to translate the given `address` to its default HaneulNS name, as long as the reverse
...
async fn reverse_record(
    ...
    scope: Scope,
    address: HaneulAddress,
) -> Result<Option<Field<HaneulAddress, Domain>>, RpcError<object::Error>> {
    let config: &NameServiceConfig = ctx.data()?;
