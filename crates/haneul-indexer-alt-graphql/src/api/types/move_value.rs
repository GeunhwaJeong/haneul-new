...
pub(crate) enum Error {
    ...
    Display(haneul_display::v2::Error),
    ...
    async fn display(&self, ctx: &Context<'_>) -> Option<Result<Display, RpcError<Error>>> {
        async {
            ...
            if let Some(display_v2) = display_v2.map_err(upcast)? {
                ...
                let root = haneul_display::v2::OwnedSlice::new(layout, self.native.clone());
                let interpreter = haneul_display::v2::Interpreter::new(root, store);

                for (field, value) in
                    haneul_display::v2::Display::parse(limits.display(), display_v2.fields())
                ...
                for (field, value) in haneul_display::v1::Format::parse(
...
fn display_error(e: haneul_display::v2::Error) -> RpcError<Error> {
