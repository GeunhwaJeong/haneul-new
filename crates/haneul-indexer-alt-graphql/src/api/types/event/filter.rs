...
use crate::{
    api::{
        scalars::{
            module_filter::ModuleFilter, haneul_address::HaneulAddress, type_filter::TypeFilter,
            uint53::UInt53,
        },
...
impl EventFilter {
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if let Some(sender) = &self.sender {
            if sender != &HaneulAddress::from(event.sender) {
                return false;
            }
        }

        if let Some(module) = &self.module {
            if module.package() != HaneulAddress::from(event.package_id) {
                return false;
            }
        ...
        if let Some(type_) = &self.type_ {
            if type_.package() != HaneulAddress::from(event.type_.address) {
                return false;
            }
