...
use haneul_types::move_package::UpgradeInfo;
...
impl Linkage<'_> {
    ...
    pub(crate) async fn original_id(&self) -> Option<HaneulAddress> {
        Some((*self.object_id).into())
    }

    /// The ID on-chain of the version of the dependency that this package depends on.
    pub(crate) async fn upgraded_id(&self) -> Option<HaneulAddress> {
