...
use haneul_indexer_alt_reader::kv_loader::BalanceChangeContents as KvBalanceChangeContents;
            ...
            let programmable_tx =
                content
                    ...
                    .and_then(|tx_data| match tx_data.into_kind() {
                        haneul_types::transaction::TransactionKind::ProgrammableTransaction(tx) => {
                            Some(tx)
                        }
                        _ => None,
                    });
