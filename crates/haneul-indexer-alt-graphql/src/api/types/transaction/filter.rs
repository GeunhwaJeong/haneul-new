...
use haneul_types::transaction::TransactionDataAPI as _;
...
pub(crate) struct TransactionFilter {
    ...
    pub affected_address: Option<HaneulAddress>,
    ...
    pub affected_object: Option<HaneulAddress>,
    ...
    pub(crate) fn matches(&self, transaction: &TransactionContents) -> bool {
        ...
        if let Some(function) = &self.function {
            let has_match = data.move_calls().into_iter().any(|(_, p, m, f)| {
                HaneulAddress::from(*p) == function.package()
        ...
        if let Some(sent_address) = &self.sent_address
            && HaneulAddress::from(data.sender()) != *sent_address
        {
            return false;
        }
        ...
        if let Some(affected_address) = &self.affected_address {
            let in_changed_objects = effects.all_changed_objects().iter().any(|(_, owner, _)| {
                owner
                    ...
                    .is_ok_and(|addr| HaneulAddress::from(addr) == *affected_address)
            });
            let is_sender = HaneulAddress::from(data.sender()) == *affected_address;
        ...
        if let Some(kind) = &self.kind {
            let is_programmable = matches!(
                data.kind(),
                haneul_types::transaction::TransactionKind::ProgrammableTransaction(_)
            );
...
mod tests {
    ...
    fn test_bloom_probe_values_skips_zero_address() {
        let zero = HaneulAddress::from_str("0x0").unwrap();
    ...
    fn test_bloom_probe_values_skips_clock_address() {
        let clock = HaneulAddress::from_str("0x6").unwrap();
        let filter = TransactionFilter {
    ...
    fn test_bloom_probe_values_keeps_normal_address() {
        let addr = HaneulAddress::from_str("0x42").unwrap();
        let filter = TransactionFilter {
