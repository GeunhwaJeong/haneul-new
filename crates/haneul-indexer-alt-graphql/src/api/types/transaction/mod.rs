...
async fn tx_affected_address(
    ...
    mut query: Query<'_>,
    page: &Page<CTransaction>,
    affected_address: HaneulAddress,
...
mod tests {
    ...
    fn preloaded_tx(tx_sequence_number: u64, sender: NativeHaneulAddress) -> ProcessedTransaction {
    ...
    fn preloaded_txs(seqs: std::ops::Range<u64>) -> Vec<ProcessedTransaction> {
        seqs.map(|seq| preloaded_tx(seq, NativeHaneulAddress::ZERO))
            .collect()
    }
