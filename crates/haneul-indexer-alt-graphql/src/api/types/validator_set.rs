...
pub(crate) struct ValidatorSetContents {
    ...
    pub(crate) report_records: IndexMap<NativeHaneulAddress, Vec<usize>>,
}
...
impl ValidatorSet {
    ...
    async fn inactive_pools_id(&self) -> Option<HaneulAddress> {
    ...
    async fn pending_active_validators_id(&self) -> Option<HaneulAddress> {
        Some(
            self.contents
                .native
    ...
    async fn staking_pool_mappings_id(&self) -> Option<HaneulAddress> {
    ...
    async fn validator_candidates_id(&self) -> Option<HaneulAddress> {
...
impl ValidatorSet {
    pub(crate) fn from_validator_set_v1(
        ...
        report_records: VecMap<NativeHaneulAddress, VecSet<NativeHaneulAddress>>,
    ) -> Self {
        let address_to_index: BTreeMap<_, _> = native
            .active_validators
            .iter()
            .enumerate()
            .map(|(i, v)| (v.metadata.haneul_address, i))
            .collect();
