...
pub(crate) enum Error {
    ...
    Literal(#[from] haneul_display::v2::FormatError),
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Option<Result<DynamicFieldValue, RpcError>> {
        async {
            ...
            let address: HaneulAddress = bcs::from_bytes(&native.value_bytes)
...
impl DynamicField {
    ...
    pub(crate) async fn by_serialized_name(
        ...
        let field_id: HaneulAddress = derive_dynamic_field_id(parent, &type_, &bcs.0)?.into();
    ...
    pub(crate) async fn by_literal_name(
        ...
        #[async_trait]
        impl haneul_display::v2::Store for NopStore {
            async fn object(
                &self,
                _: AccountAddress,
