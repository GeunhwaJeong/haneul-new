...
pub(crate) fn simple_signature_to_scheme(
    sig: &haneul_types::crypto::Signature,
) -> Option<SignatureScheme> {
    let signature = Some(Base64(sig.signature_bytes().to_vec()));
    let public_key = Some(Base64(sig.public_key_bytes().to_vec()));

    match sig.scheme() {
        NativeSignatureScheme::ED25519 => Some(SignatureScheme::Ed25519(Ed25519Signature {
            signature,
            public_key,
        })),
        NativeSignatureScheme::Secp256k1 => Some(SignatureScheme::Secp256k1(Secp256k1Signature {
            signature,
            public_key,
        })),
