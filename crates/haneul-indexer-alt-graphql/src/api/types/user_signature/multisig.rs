...
use haneul_types::crypto::{CompressedSignature, PublicKey};
...
fn compressed_signature_to_scheme(sig: &CompressedSignature) -> Option<SignatureScheme> {
    match sig {
        ...
        CompressedSignature::ZkLogin(b) => {
            bcs::from_bytes::<haneul_types::zk_login_authenticator::ZkLoginAuthenticator>(&b.0)
                .ok()
                .map(|native| SignatureScheme::ZkLogin(ZkLoginSignature { native }))
        }
        CompressedSignature::Passkey(b) => {
            bcs::from_bytes::<haneul_types::passkey_authenticator::PasskeyAuthenticator>(&b.0)
                .ok()
...
impl From<&haneul_types::multisig::MultiSigPublicKey> for MultisigCommittee {
    fn from(pk: &haneul_types::multisig::MultiSigPublicKey) -> Self {
        Self {
            members: Some(
                pk.pubkeys()
                    .iter()
...
impl From<&PublicKey> for MultisigMemberPublicKey {
    fn from(pk: &PublicKey) -> Self {
        match pk {
            ...
            PublicKey::ZkLogin(z) => {
                // Convert through haneul_sdk_types for clean field extraction.
                MultisigMemberPublicKey::ZkLogin(
                    haneul_sdk_types::ZkLoginPublicIdentifier::try_from(z.to_owned())
