...
impl TransactionBuilder {
    ...
    pub async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            Err(anyhow!(
                "Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-haneul or transfer-haneul commands instead, which will use the only object as gas payment."
            ))
        }
    }
    ...
    pub async fn pay(
        ...
        {
            return Err(anyhow!(
                "Gas coin is in input coins of Pay transaction, use PayHaneul transaction instead!"
            ));
        }
