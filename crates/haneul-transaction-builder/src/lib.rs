...
use haneul_json_rpc_types::{
    RPCTransactionRequestParams, HaneulData, HaneulObjectDataOptions, HaneulObjectInfo, HaneulObjectResponse,
    HaneulTypeTag,
};
...
impl<Mode: ExecutionMode> TransactionBuilder<Mode> {
    ...
    async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, Some(HaneulObjectDataOptions::bcs_only()))
                    .await?;
    ...
    async fn resolve_and_checks_json_args(
        ...
        let object = self
            .0
            .get_object_with_options(package_id, Some(HaneulObjectDataOptions::bcs_lossless()))
            .await?
            .into_object()?;
        let package = object
            .bcs
    ...
    pub async fn split_coin(
        ...
        let coin = self
            .0
            .get_object_with_options(coin_object_id, Some(HaneulObjectDataOptions::bcs_lossless()))
            .await?
            .into_object()?;
    ...
    pub async fn merge_coins(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, Some(HaneulObjectDataOptions::bcs_lossless()))
            .await?
            .into_object()?;
