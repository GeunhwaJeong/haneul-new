...
use haneul_json::{is_receiving_argument, resolve_move_function_args, ResolvedCallArg, HaneulJsonValue};
...
impl TransactionBuilder {
    ...
    pub async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            Err(anyhow!("Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-haneul or transfer-haneul commands instead, which will use the only object as gas payment."))
        }
    }
    ...
    pub fn transfer_haneul_tx_kind(
        &self,
        recipient: HaneulAddress,
        amount: Option<u64>,
    ) -> TransactionKind {
    ...
    pub async fn transfer_haneul(
        ...
        let object = self.get_object_ref(haneul_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_haneul(
    ...
    pub fn pay_haneul_tx_kind(
        ...
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_haneul(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_haneul(
        &self,
        signer: HaneulAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<HaneulAddress>,
    ...
    pub fn pay_all_haneul_tx_kind(&self, recipient: HaneulAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<HaneulTypeTag>,
        call_args: Vec<HaneulJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn resolve_and_checks_json_args(
        ...
        let object = self
            ...
            .await?
            .into_object()?;
        let Some(HaneulRawData::Package(package)) = object.bcs else {
            bail!(
    ...
    pub async fn upgrade_tx_kind(
        ...
        let upgrade_capability = self
            .0
            .get_object_with_options(upgrade_capability, HaneulObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    pub async fn split_coin_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(coin_object_id, HaneulObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let coin_object_ref = coin.object_ref();
        let coin: Object = coin.try_into()?;
        let type_args = vec![coin.get_move_template_type()?];
        let package = HANEUL_FRAMEWORK_PACKAGE_ID;
    ...
    pub async fn merge_coins_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, HaneulObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn get_full_object_ref(&self, object_id: ObjectID) -> anyhow::Result<FullObjectRef> {
        let object_data = self
            .0
            .get_object_with_options(object_id, HaneulObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, HaneulObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
    ...
    pub async fn get_full_object_ref_and_type(
        ...
        let object_data = self
            ...
            .get_object_with_options(
                object_id,
                HaneulObjectDataOptions::new().with_owner().with_type(),
            )
            .await?
            .into_object()?;
