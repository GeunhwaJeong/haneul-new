...
impl TransactionBuilder {
    ...
    pub async fn pay_haneul(
        ...
        let coin_refs: Vec<ObjectRef> = coins
            ...
            .map(|c| {
                c.map_err(|e| {
                    debug!("Failed reading PayHaneul input coin with err {:?}", e);
                    e
                })
            })
            .filter_map(|c| c.ok())
            .collect();
        if coins_len != coin_refs.len() {
            return Err(anyhow!("Failed reading PayHaneul input coin objects!"));
        }
    ...
    pub async fn pay_all_haneul(
        ...
        let coin_refs: Vec<ObjectRef> = coins
            ...
            .map(|c| {
                c.map_err(|e| {
                    debug!("Failed reading PayAllHaneul input coin with err {:?}", e);
                    e
                })
            })
            .filter_map(|c| c.ok())
            .collect();
        if coins_len != coin_refs.len() {
            return Err(anyhow!("Failed reading PayAllHaneul input coin objects!"));
        }
        // [0] is safe because input_coins is non-empty and coins are of same length as input_coins.
        let gas_object_ref = coin_refs[0];
