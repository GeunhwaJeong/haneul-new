...
`haneul-tool` contains assorted debugging utilities for Haneul.

You can build and run `haneul-tool` from source with:
```sh
cargo run --bin haneul-tool -- <args>
```
...
- Haneul discovery and state sync: `haneul`
...
cargo run --bin haneul-tool -- anemo ping --server-name "$SERVER_NAME" "$ADDRESS"
```

### call

`haneul-tool` has been preconfigured to support RPC calls using [RON (Rusty Object Notation)](https://crates.io/crates/ron) for the following servivces:
- Narwhal: `PrimaryToPrimary` and `WorkerToWorker`
- Haneul: `Discovery` and `StateSync`
