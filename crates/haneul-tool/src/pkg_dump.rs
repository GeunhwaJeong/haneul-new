...
use haneul_indexer::{models::packages::StoredPackage, schema::packages};
...
pub(crate) async fn dump(db_url: String, output_dir: PathBuf) -> Result<()> {
    ...
    for (i, pkg) in pkgs.into_iter().enumerate() {
        ...
        let id = HaneulAddress::from_bytes(&pkg.package_id).context("Parsing package ID")?;
...
fn dump_package(output_dir: &Path, id: HaneulAddress, pkg: &[u8]) -> Result<()> {
