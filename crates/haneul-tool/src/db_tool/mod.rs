...
use haneul_types::digests::{CheckpointContentsDigest, TransactionDigest};
...
/// Dry run with: cargo run --package haneul-tool -- db-tool --db-path /opt/haneul/db/authorities_db/live remove-transaction --digest xxxx
...
pub fn reset_db_to_genesis(path: &Path) -> anyhow::Result<()> {
    ...
    // Download the snapshot for the epoch you want to restore to the local disk. You will find one snapshot per epoch in the S3 bucket. We need to place the snapshot in the dir where config is pointing to. If db-config in fullnode.yaml is /opt/haneul/db/authorities_db and we want to restore from epoch 10, we want to copy the snapshot to /opt/haneul/db/authorities_dblike this:
    // aws s3 cp s3://myBucket/dir /opt/haneul/db/authorities_db/ --recursive —exclude “*” —include “epoch_10*”
    // Mark downloaded snapshot as live: mv  /opt/haneul/db/authorities_db/epoch_10  /opt/haneul/db/authorities_db/live
    // Reset the downloaded db to execute from genesis with: cargo run --package haneul-tool -- db-tool --db-path /opt/haneul/db/authorities_db/live reset-db
    // Start the haneul full node: cargo run --release --bin haneul-node -- --config-path ~/db_checkpoints/fullnode.yaml
...
/// Run with: cargo run --package haneul-tool -- db-tool --db-path /opt/haneul/db/authorities_db/live rewind-checkpoint-execution --epoch 3 --checkpoint-sequence-number 300000
...
/// cargo run --package haneul-tool -- db-tool --db-path /opt/haneul/db/authorities_db/live set_checkpoint_watermark --highest-synced 300000
