...
fn from_addr_seq(s: &str) -> Result<(HaneulAddress, TxSequenceNumber), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    ...
    let address = HaneulAddress::from_str(tokens[0].trim())?;
...
fn from_addr_oid(s: &str) -> Result<(HaneulAddress, ObjectID), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    let tokens = s.split(',').collect::<Vec<&str>>();
    if tokens.len() != 2 {
    ...
    let addr = HaneulAddress::from_str(tokens[0].trim())?;
    let oid = ObjectID::from_str(tokens[1].trim())?;

    Ok((addr, oid))
}

fn from_addr_str_oid(s: &str) -> Result<(HaneulAddress, String, ObjectID), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    let tokens = s.split(',').collect::<Vec<&str>>();
    if tokens.len() != 3 {
...
fn from_address_and_event_id(
    s: &str,
) -> Result<(HaneulAddress, (TxSequenceNumber, usize)), anyhow::Error> {
    // Example: "0x1 1234 5"
    let tokens = s.split(' ').collect::<Vec<&str>>();
    if tokens.len() != 3 {
        return Err(anyhow!("Invalid input"));
