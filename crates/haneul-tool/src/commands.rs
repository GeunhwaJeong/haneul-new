...
pub enum ToolCommand {
    ...
    FetchObject {
        ...
        /// ```text
        /// $ haneul-tool fetch-object --id 0x260efde76ebccf57f4c5e951157f5c361cde822c \
        ///      --genesis $HOME/.haneul/haneul_config/genesis.blob \
...
async fn check_locked_object(
    ...
    let clients = Arc::new(make_clients(haneul_client, use_tls).await?);
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
            ...
            ToolCommand::FetchObject {
                ...
                let haneul_client =
                    Arc::new(HaneulClientBuilder::default().build(fullnode_rpc_url).await?);
                let clients = Arc::new(make_clients(&haneul_client, !no_tls).await?);
            ...
            ToolCommand::DumpPackages {
                ...
                haneul_package_dump::dump(rpc_url, output_dir, before_checkpoint).await?;
            }
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.haneul_pubkey_bytes().concise(),
            ...
            ToolCommand::FetchCheckpoint {
                ...
                let haneul_client =
                    Arc::new(HaneulClientBuilder::default().build(fullnode_rpc_url).await?);
                let clients = make_clients(&haneul_client, !no_tls).await?;
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = haneul_config::NodeConfig::load(config_path)?;
                    ...
                    snapshot_bucket.or_else(|| match (network, no_sign_request) {
                        (Chain::Mainnet, false) => Some(
                            env::var("MAINNET_FORMAL_SIGNED_BUCKET")
                                .unwrap_or("haneullabs-mainnet-formal".to_string()),
                        ),
                        ...
                        (Chain::Testnet, _) => Some(
                            env::var("TESTNET_FORMAL_SIGNED_BUCKET")
                                .unwrap_or("haneullabs-testnet-formal".to_string()),
                        ),
                        (Chain::Unknown, _) => {
            ...
            ToolCommand::DownloadDBSnapshot {
                    ...
                    snapshot_bucket.or_else(|| match (network, no_sign_request) {
                        (Chain::Mainnet, false) => Some(
                            env::var("MAINNET_DB_SIGNED_BUCKET")
                                .unwrap_or("haneullabs-mainnet-snapshots".to_string()),
                        ),
                        ...
                        (Chain::Testnet, _) => Some(
                            env::var("TESTNET_DB_SIGNED_BUCKET")
                                .unwrap_or("haneullabs-testnet-snapshots".to_string()),
                        ),
                        (Chain::Unknown, _) => {
                            panic!("Cannot generate default snapshot bucket for unknown network");
                        }
                ...
                let snapshot_store_config = if no_sign_request {
                    let aws_endpoint = env::var("AWS_SNAPSHOT_ENDPOINT").ok().or_else(|| {
                        if network == Chain::Mainnet {
                            Some("https://db-snapshot.mainnet.haneul.io".to_string())
                        } else if network == Chain::Testnet {
                            Some("https://db-snapshot.testnet.haneul.io".to_string())
                        } else {
                            None
                        }
                    });
