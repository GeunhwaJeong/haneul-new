...
use haneul_core::db_checkpoint_handler::SUCCESS_MARKER;
...
use haneul_storage::object_store::util::{copy_file, get_path};
...
use haneul_types::storage::{ReadStore, SharedInMemoryStore};
use tracing::info;
...
async fn make_clients(
    ...
    if let Some(fullnode_rpc) = fullnode_rpc {
        let haneul_client = HaneulClientBuilder::default().build(fullnode_rpc).await?;
        ...
        for validator in genesis.validator_set_for_tooling() {
            ...
            let channel = net_config
                ...
                .map_err(|err| anyhow!(err.to_string()))?;
            let client = NetworkAuthorityClient::new(channel);
            let public_key_bytes = metadata.haneul_pubkey_bytes();
            ...
            anemo_cli::ServiceInfo::new()
                ...
                .add_method(
                    ...
                    anemo_cli::ron_method!(
                        ...
                        haneul_types::messages_checkpoint::CheckpointContentsDigest
                    ),
                )
                .add_method(
