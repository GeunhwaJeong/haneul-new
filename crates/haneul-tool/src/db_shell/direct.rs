...
//! Used when haneul-node is not running and the database files can be opened directly.
...
impl DirectBackend {
    ...
    fn commit_summary_json(&self, index: CommitIndex) -> anyhow::Result<serde_json::Value> {
        ...
        let summary =
            haneul_core::consensus_commit_summary::build_consensus_commit_summary(cs, index)?
