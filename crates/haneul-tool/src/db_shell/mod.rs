...
//! Interactive database shell for the Haneul validator database.
//!
//! Two operating modes:
//!
...
//! haneul-node admin API. Allows write operations because the node owns the DB lock.
...
#[command(
    ...
    long_about = r#"Interactive shell for navigating the Haneul validator database.
...
pub struct DbShellArgs {
    ...
    /// Admin API URL of the running haneul-node (proxy mode).
...
pub fn run(args: DbShellArgs) -> anyhow::Result<()> {
    let backend: Arc<dyn Backend> = match (&args.admin_url, &args.db_path) {
        (Some(url), _) => {
            eprintln!("Connecting to haneul-node admin API at {url}");
