...
impl concurrent::Handler for TxCounts {
    ...
    async fn commit<'a>(
        ...
        conn: &mut haneul_pg_db::Connection<'a>,
    ) -> anyhow::Result<usize> {
...
async fn transfer_coin(
    wallet: &mut haneul_sdk::wallet_context::WalletContext,
) -> haneul_types::base_types::TransactionDigest {
    ...
    let tx_data = TestTransactionBuilder::new(sender, gas_obj, 1000)
        .transfer_haneul(Some(1000), sender)
        .build();
