...
use haneul_types::{
    ...
    Identifier, TypeTag, HANEUL_COIN_REGISTRY_ADDRESS, HANEUL_COIN_REGISTRY_OBJECT_ID,
...
async fn test_haneul() {
    ...
    // Query the coin metadata for HANEUL
...
async fn test_fixed_supply() {
    ...
    let currency = find_address_owned_by(&fx, HANEUL_COIN_REGISTRY_ADDRESS.into()).unwrap();
