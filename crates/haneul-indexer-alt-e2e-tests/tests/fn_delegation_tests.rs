...
#[sim_test]
async fn test_get_coins_with_invalid_address() {
    let test_cluster = FnDelegationTestCluster::new()
        .await
        .expect("Failed to create test cluster");
    let invalid_address = "23333";
...
#[sim_test]
async fn test_get_coins_with_invalid_coin_type() {
    let test_cluster = FnDelegationTestCluster::new()
        .await
        .expect("Failed to create test cluster");
    let invalid_coin_type = "invalid_coin_type";
    let address = test_cluster.onchain_cluster.wallet.get_addresses()[1];
    let response = test_cluster
        .execute_jsonrpc(
            "haneulx_getCoins".to_string(),
            json!({ "owner": address.to_string().as_str(), "coinType": invalid_coin_type }),
        )
        .await
        .unwrap();
    ...
    assert!(response["error"]["message"]
        .as_str()
        .unwrap()
        .starts_with("Invalid struct type"));
...
async fn test_get_coins() {
    ...
    assert_eq!(response["result"]["data"][0]["coinType"], "0x2::haneul::HANEUL");
    test_cluster.stopped().await;
}
