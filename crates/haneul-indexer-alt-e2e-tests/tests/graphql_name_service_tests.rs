...
/// Tests happy path for HaneulNS resolution (forward lookup).
...
/// Tests HaneulNS resolution response when the domain doesn't exist or has expired.
...
async fn test_resolve_domain() {
    let mut c = HaneulNSCluster::new().await;

    let nft = ObjectID::random();
    let target = HaneulAddress::random_for_testing_only();
    c.add_domain(nft, &["haneul", "foo"], Some(target), 1000)
        .await
...
async fn test_resolve_domain_no_target() {
    ...
    let nft = ObjectID::random();
    c.add_domain(nft, &["haneul", "foo"], None, 1000)
        .await
        .expect("Failed to add domain");
...
async fn test_resolve_domain_expiry() {
    ...
    let target = HaneulAddress::random_for_testing_only();
    let expiry_ms = 1000;
    c.add_domain(nft, &["haneul", "foo"], Some(target), expiry_ms)
        .await
        .expect("Failed to add domain");
    ...
    assert_not_resolved!(c.resolve_address("foo.haneul").await.unwrap());
...
async fn test_resolve_subdomain() {
    ...
    c.add_domain(nft, &["haneul", "foo", "bar"], Some(target), 0)
        .await
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    c.add_domain(nft, &["haneul", "foo"], None, expiry_ms)
        .await
        .expect("Failed to add parent domain");
    ...
    assert_not_resolved!(c.resolve_address("bar.foo.haneul").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
}
...
async fn test_resolve_subdomain_expiry() {
    ...
    c.add_domain(parent_nft, &["haneul", "foo"], None, parent_expiry_ms)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft, &["haneul", "foo", "bar"], Some(target), expiry_ms)
        .await
        .expect("Failed to add subdomain");
...
async fn test_resolve_subdomain_bad_parent() {
    ...
    c.add_domain(nft0, &["haneul", "foo"], None, 1000)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft1, &["haneul", "foo", "bar"], Some(target), 0)
        .await
        .expect("Failed to add subdomain");
...
/// Test looking up an address by its HaneulNS name via Query.address(name: ...).
#[tokio::test]
async fn test_address_by_name() {
    ...
    assert_address_by_name!(target, c.address_by_name("foo.haneul").await.unwrap());
...
async fn test_address_by_name_no_target() {
    ...
    assert_no_address_by_name!(c.address_by_name("foo.haneul").await.unwrap());
}
...
async fn test_address_both_identifiers_error() {
    ...
    let resp = c
        .query(
            r#"
            query($address: HaneulAddress!, $name: String!) {
...
impl HaneulNSCluster {
    /// Sets up a full cluster with a mock HaneulNS package and registries. GraphQL is configured to read
    /// from these packages and registries to resolve HaneulNS names.
    ///
    ...
    async fn new() -> Self {
        ...
        // (2) Compile the mock HaneulNS package.
        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.extend(["packages", "haneulns"]);
        ...
        // (4) Publish the mock HaneulNS package.
        ...
        // (7) Configure the GraphQL RPC to read from the mock HaneulNS package.
    ...
    /// Send a GraphQL request to the cluster to resolve the HaneulNS name for a given address.
    async fn resolve_name(&self, addr: HaneulAddress) -> anyhow::Result<Value> {
        self.query(
            r#"
            query($address: HaneulAddress!) {
    ...
    /// Send a GraphQL request to resolve an address by its HaneulNS name.
