...
use haneul_kv_rpc::LedgerHistoryConfig;
use haneul_kv_rpc::LedgerHistoryMethodConfig;
use haneul_kv_rpc::StageConfig;
use haneul_kv_rpc::StagesConfig;
...
use haneul_rpc::proto::haneul::rpc::v2alpha::ledger_service_client::LedgerServiceClient as KvLedgerServiceClient;
...
async fn call_move(
    ...
    module: &str,
    function: &str,
) -> (haneul_types::digests::TransactionDigest, ObjectRef) {
...
/// Execute a `transfer_haneul(sender, None)` self-transfer and return `(tx_digest, updated_gas_ref)`.
async fn transfer_self(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_haneul(sender, None);
    let data = TransactionData::new_programmable(
        sender,
        vec![gas],
        builder.finish(),
...
fn tx_not_sender_only_filter(addr: HaneulAddress) -> TransactionFilter {
    let mut term = TransactionTerm::default();
...
fn ev_not_sender_only_filter(addr: HaneulAddress) -> EventFilter {
    let mut term = EventTerm::default();
...
fn tx_sender(addr: HaneulAddress) -> TransactionFilter {
...
async fn test_list_events_with_emit_module_filter() {
    ...
    let mut emit_mod = haneul_rpc::proto::haneul::rpc::v2alpha::EmitModuleFilter::default();
...
async fn test_list_transactions_or_prefix_and_event_predicates() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_haneul(sender_c, None);
    let data = TransactionData::new_programmable(
        sender_c,
        vec![gas_c],
        builder.finish(),
...
async fn test_list_transactions_recipient_and_affected_object() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_haneul(sender_b, Some(1_000_000));
    let data = TransactionData::new_programmable(
        sender_a,
        vec![gas_a],
        builder.finish(),
...
async fn test_list_filter_edge_cases() {
    ...
    let never_sender: HaneulAddress =
...
async fn test_list_checkpoints_unanchored_negation() {
    ...
    req.filter = Some(tx_filter(vec![
        tx_not_sender_literal(sender_b),
        tx_not_sender_literal(HaneulAddress::ZERO),
    ]));
    req.options = Some(query_options(100));
    let resp = list_checkpoints_result(&mut client, req).await;
    assert_checkpoint_cursors(&resp);
...
async fn test_list_checkpoints_with_transactions_read_mask() {
    ...
    let mut expected_digests: Vec<haneul_types::digests::TransactionDigest> = Vec::new();
    for _ in 0..2 {
...
async fn build_sparse_layout(
    cluster: &mut FullCluster,
    noise_sender: HaneulAddress,
    noise_kp: &AccountKeyPair,
    mut noise_gas: ObjectRef,
    match_sender: HaneulAddress,
...
// unit-tested in `haneul-inverted-index` (budget validation) and handler-level
// tests in `haneul-kv-rpc`.
