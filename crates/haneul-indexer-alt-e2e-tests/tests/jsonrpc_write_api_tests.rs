        ...
        tokio::time::timeout(Duration::from_secs(60), async {
            loop {
                let response = cluster
                    .execute_jsonrpc("haneul_getProtocolConfig", json!([]))
                    .await;
...
async fn test_execute_transfer_correctness() {
    ...
    assert_eq!(sender_coin, "0x2::haneul::HANEUL");
    assert_eq!(recipient_coin, "0x2::haneul::HANEUL");
    ...
    assert!(
        mutated_change["objectType"]
            .as_str()
            .unwrap()
            .contains("Coin<0x2::haneul::HANEUL>")
    );
...
async fn test_dev_inspect_matches_fullnode_jsonrpc() {
    ...
    let indexer = cluster
        .execute_jsonrpc("haneul_devInspectTransactionBlock", params.clone())
        .await
        .unwrap();
    let fullnode = cluster
        .execute_fullnode_jsonrpc("haneul_devInspectTransactionBlock", params)
        .await
        .unwrap();
