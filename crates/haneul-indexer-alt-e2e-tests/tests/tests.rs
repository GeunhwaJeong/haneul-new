...
use haneul_indexer_alt::{config::IndexerConfig, start_indexer};
use haneul_indexer_alt_framework::{ingestion::ClientArgs, schema::watermarks, IndexerArgs};
use haneul_indexer_alt_jsonrpc::{start_rpc, RpcArgs};
use haneul_pg_db::{
...
impl OffchainStateReader for OffchainReader {
    async fn wait_for_objects_snapshot_catchup(&self, _: Duration) {
        // Not necessary for `haneul-indexer-alt`
    }
