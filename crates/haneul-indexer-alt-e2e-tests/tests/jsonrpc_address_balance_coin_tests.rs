...
struct ObjectResponse {
    result: HaneulObjectResponse,
}

#[derive(Deserialize)]
struct MultiObjectResponse {
    result: Vec<HaneulObjectResponse>,
}
...
async fn test_get_object_resolves_address_balance_coin_id() {
    ...
    let tx = cluster
        .test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(42, recipient)])
        .build();
...
/// Fund an address balance and verify that haneul_multiGetObjects resolves the address balance
...
async fn test_pagination_ab_coin_on_first_page() {
    ...
    let tx = cluster
        .test_env
        .tx_builder_with_gas(sender, gas)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(100_000, recipient)])
        .build();
    let (digest, fx) = cluster
        .test_env
        .cluster
...
async fn test_pagination_ab_coin_trimmed_to_next_page() {
    ...
    let tx = cluster
        .test_env
        .tx_builder_with_gas(sender, gas)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(2500, recipient)])
        .build();
    let (digest, fx) = cluster
        .test_env
        .cluster
...
async fn test_pagination_ab_coin_on_second_page() {
    ...
    let tx = cluster
        .test_env
        .tx_builder_with_gas(sender, gas)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(500, recipient)])
        .build();
    let (digest, fx) = cluster
        .test_env
        .cluster
...
async fn test_pagination_ab_coin_as_cursor() {
    ...
    let tx = cluster
        .test_env
        .tx_builder_with_gas(sender, gas)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(3000, recipient)])
        .build();
    let (digest, fx) = cluster
        .test_env
        .cluster
    ...
    let ab_key = ObjectByOwnerKey::from_coin_parts(
        ...
        haneul_types::coin::Coin::type_(GAS::type_tag()),
        ab_coin.balance,
...
async fn test_use_ab_coin_as_transaction_input() {
    ...
    assert_eq!(
        cluster.test_env.get_haneul_balance_ab(sender),
        fund_amount,
    ...
    assert_eq!(
        cluster.test_env.get_haneul_balance_ab(recipient),
        transfer_amount,
...
async fn test_use_ab_coin_as_gas() {
    ...
    // Use the AB coin as gas for a simple transfer_haneul (SplitCoins from GasCoin).
