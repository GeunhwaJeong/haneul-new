...
/// Deserialized successful JSON-RPC response for `haneulx_getOwnedObjects`.
#[derive(Deserialize)]
struct Response {
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: HaneulAddress, amount: u64) -> ObjectID {
    ...
    builder.transfer_haneul(owner, Some(amount));
...
/// isn't a coin.
fn create_bag(cluster: &mut FullCluster, owner: HaneulAddress, size: u64) -> ObjectID {
    let (sender, kp, gas) = cluster
...
/// Make a call to `haneulx_getOwnedObjects` to the RPC server running on `cluster`.
