...
use haneul_indexer_alt_e2e_tests::{find_address_mutated, find_address_owned, FullCluster};
use haneul_types::{
...
async fn test_unsupported_type_filters() {
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            ...
            object_type: Some("0x2::coin::Coin<0x2::haneul::HANEUL>".to_string()),
            ..Default::default()
        })
        .await;
...
/// isn't a coin.
fn create_bag(cluster: &mut FullCluster, owner: HaneulAddress, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
