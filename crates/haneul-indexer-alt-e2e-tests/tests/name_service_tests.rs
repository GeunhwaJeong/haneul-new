...
/// Tests happy path for HaneulNS resolution.
...
/// Tests HaneulNS resolution response when the domain has expired.
...
async fn test_resolve_domain_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("foo.haneul").await.unwrap());
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("bar.foo.haneul").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
...
impl HaneulNSCluster {
    /// Sets up a full cluster with a mock HaneulNS package and registries. RPC is configured to read
    ...
    async fn new() -> Self {
        ...
        // (7) Configure the RPC to read from the mock HaneulNS package. Everything else is configured
    ...
    /// Send a JSON-RPC request to the cluster to resolve the given HaneulNS name.
    ...
    /// Send a JSON-RPC request to the cluster to resolve the HaneulNS name for a given address.
