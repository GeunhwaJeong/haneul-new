...
const QUERY: &str = r#"
query ($message: Base64!, $signature: Base64!, $scope: IntentScope!, $author: HaneulAddress!) {
...
fn sign_personal_message(keypair: &HaneulKeyPair, msg: &[u8]) -> (Vec<u8>, Vec<u8>, HaneulAddress) {
...
fn ed25519_keypair(seed: u8) -> HaneulKeyPair {
    HaneulKeyPair::Ed25519(fastcrypto::ed25519::Ed25519KeyPair::generate(
        &mut rand::rngs::StdRng::from_seed([seed; 32]),
    ))
}

fn secp256k1_keypair(seed: u8) -> HaneulKeyPair {
    HaneulKeyPair::Secp256k1(fastcrypto::secp256k1::Secp256k1KeyPair::generate(
        &mut rand::rngs::StdRng::from_seed([seed; 32]),
    ))
}

fn secp256r1_keypair(seed: u8) -> HaneulKeyPair {
    HaneulKeyPair::Secp256r1(fastcrypto::secp256r1::Secp256r1KeyPair::generate(
        &mut rand::rngs::StdRng::from_seed([seed; 32]),
    ))
}
...
async fn sign_passkey_personal_message(msg: &[u8]) -> (Vec<u8>, Vec<u8>, HaneulAddress) {
    ...
    let passkey_digest = haneul_types::passkey_authenticator::to_signing_message(&intent_msg);
