...
impl FnDelegationTestCluster {
    ...
    async fn transfer_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        ...
        let tx = self
            ...
            .await
            .transfer_haneul(Some(1_000), recipient)
            .build();
    ...
    async fn privileged_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        let tx: haneul_types::transaction::TransactionData = self
            .onchain_cluster
            .test_transaction_builder()
            .await
...
async fn test_execution() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "haneul_executeTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
                "signatures": sigs,
                "options": {
...
async fn test_dry_run() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "haneul_dryRunTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
            }),
        )
...
async fn test_get_all_balances() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "haneulx_getAllBalances".to_string(),
    ...
    assert_eq!(response["result"][0]["coinType"], "0x2::haneul::HANEUL");
    test_cluster.stopped().await;
}
...
async fn test_get_balance() {
    ...
    let coin_type = haneul_types::parse_haneul_struct_tag(&format!("{}::my_coin::MY_COIN", package_id))
        .unwrap()
        .to_string();
    let response = test_cluster
        .execute_jsonrpc(
            "haneulx_getBalance".to_string(),
    ...
    assert_eq!(response["result"]["coinType"], "0x2::haneul::HANEUL");
