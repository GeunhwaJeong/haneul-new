...
impl FnDelegationTestCluster {
    ...
    async fn get_validator_addresses(&self) -> Vec<HaneulAddress> {
...
/// Pulls the single sender-owned object created by a staking tx — the newly minted StakedHaneul.
fn staked_haneul_id_from_effects(effects: &TransactionEffects, owner: HaneulAddress) -> ObjectID {
    ...
    let id = created
        .next()
        .expect("staking tx should create a StakedHaneul owned by the sender");
    assert!(
...
/// Projects a `getStakes`/`getStakesByIds` response to `(validatorAddress, [stakedHaneulId])` so
    ...
    result
        ...
        .map(|entry| {
            ...
            let ids = entry["stakes"]
                ...
                .unwrap()
                .iter()
                .map(|s| s["stakedHaneulId"].as_str().unwrap().to_string())
                .collect();
            (validator, ids)
        })
        .collect()
...
async fn test_get_stakes_and_by_ids() {
    ...
    assert!(get_stakes_response["result"][0]["stakes"][0]["stakedHaneulId"].is_string());
    let stake_id = get_stakes_response["result"][0]["stakes"][0]["stakedHaneulId"]
        .as_str()
        .unwrap();
    ...
    let get_stakes_by_ids_response = test_cluster
        .execute_jsonrpc(
            "haneulx_getStakesByIds".to_string(),
            json!({ "staked_haneul_ids": [stake_id] }),
        )
        .await
        .unwrap();
...
async fn test_get_stakes_invalid_params() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "haneulx_getStakesByIds".to_string(),
            json!({ "staked_haneul_ids": ["invalid_stake_id"] }),
        )
        .await
        .unwrap();
...
async fn test_stakes_correct_ordering() {
    ...
    let stake_a = staked_haneul_id_from_effects(&effects_a, stake_owner_address);
    ...
    let stake_b1 = staked_haneul_id_from_effects(&effects_b1, stake_owner_address);
    ...
    let stake_b2 = staked_haneul_id_from_effects(&effects_b2, stake_owner_address);
    ...
    let entry_a_ids: Vec<&str> = entry_a["stakes"]
        ...
        .unwrap()
        .iter()
        .map(|s| s["stakedHaneulId"].as_str().unwrap())
        .collect();
    ...
    let forward_response = test_cluster
        .execute_jsonrpc(
            "haneulx_getStakesByIds".to_string(),
            json!({ "staked_haneul_ids": [stake_b1, stake_b2] }),
        )
        .await
        .unwrap();
    ...
    let reversed_response = test_cluster
        .execute_jsonrpc(
            "haneulx_getStakesByIds".to_string(),
            json!({ "staked_haneul_ids": [stake_b2, stake_b1] }),
        )
        .await
        .unwrap();
...
async fn test_get_validators_apy() {
    ...
    let response = test_cluster
        .execute_jsonrpc("haneulx_getValidatorsApy".to_string(), json!({}))
        .await
        .unwrap();
...
/// Withdrawing a stake (calling `haneul_system::request_withdraw_stake`) deletes the StakedHaneul
...
/// our documented divergence from legacy `haneul-json-rpc`, which returned Unstaked for the
/// withdrawn one.
#[tokio::test]
async fn test_get_stakes_by_ids_omits_withdrawn() {
    ...
    let stake_ids: Vec<ObjectID> = get_stakes_response["result"][0]["stakes"]
        ...
        .map(|s| {
            s["stakedHaneulId"]
                ...
                .parse()
                .expect("malformed stakedHaneulId")
        })
        .collect();
    ...
    let get_by_id_response = test_cluster
        .execute_jsonrpc(
            "haneulx_getStakesByIds".to_string(),
            json!({ "staked_haneul_ids": [withdrawn_stake_id, kept_stake_id] }),
        )
        .await
        .unwrap();
