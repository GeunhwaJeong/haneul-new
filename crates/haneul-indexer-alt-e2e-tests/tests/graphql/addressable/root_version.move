...
//# programmable --sender A --inputs @A
//> 0: haneul::object_bag::new();
//> 1: TransferObjects([Result(0)], Input(0))

//# create-checkpoint

//# programmable --sender A --inputs object(1,0) 1u64 object(2,0) 2u64 object(3,0) 3u64 object(4,0)
//> 0: haneul::bag::add<u64, haneul::bag::Bag>(Input(2), Input(3), Input(4));
//> 1: haneul::bag::add<u64, haneul::bag::Bag>(Input(2), Input(5), Input(6));
//> 2: haneul::bag::add<u64, haneul::bag::Bag>(Input(0), Input(1), Input(2));

//# programmable --sender A --inputs object(5,0) 400u64 500u64
//> 0: SplitCoins(Gas, [Input(1), Input(2)]);
//> 1: haneul::object_bag::add<u64, haneul::coin::Coin<haneul::haneul::HANEUL>>(Input(0), Input(1), NestedResult(0,0));
//> 2: haneul::object_bag::add<u64, haneul::coin::Coin<haneul::haneul::HANEUL>>(Input(0), Input(2), NestedResult(0,1));
...
{ # Fetching an address at another checkpoint, and fetching its objects,
  ...
  atCp0: address(address: "@{A}", atCheckpoint: 0) {
    balance(coinType: "haneul::haneul::HANEUL") {
      totalBalance
    }
