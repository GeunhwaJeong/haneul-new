...
mod sim_only_tests {
    ...
    async fn object_pruning_test() {
        ...
        let fullnode = &test_cluster.fullnode_handle.haneul_node;
    ...
    async fn publish_package_and_create_parent_object(
        ...
        let package_id = publish_package(
            ...
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../haneul-surfer/tests/move_building_blocks"),
        )
        .await
        .0;
    ...
    fn count_wrapped_tombstone(node: &HaneulNode) -> usize {
    ...
    async fn wait_until_checkpoint_pruned(node: &HaneulNode, checkpoint: CheckpointSequenceNumber) {
        loop {
            if node
                .state()
