...
mod test {
    ...
    async fn test_disconnect_consistently_failing_peers() {
        ...
        let set_discovery = |mut config: haneul_config::NodeConfig| {
        ...
        let validator_id =
            test_cluster.swarm.validator_node_handles()[0].with(|n| n.get_sim_node_id());
        let healthy_id = healthy_fullnode.haneul_node.with(|n| n.get_sim_node_id());
        let degraded_id = degraded_fullnode.haneul_node.with(|n| n.get_sim_node_id());
        ...
        loop {
            let synced = syncing_node.haneul_node.with(|n| {
                n.state()
                    .get_checkpoint_store()
                    .get_highest_synced_checkpoint_seq_number()
                    .unwrap()
