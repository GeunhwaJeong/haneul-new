...
use test_cluster::addr_balance_test_env::{
    TestEnv, TestEnvBuilder, get_haneul_accumulator_object_id,
};
...
async fn test_valid_coin_reservation_gas_payments() {
    ...
    let tx = TestTransactionBuilder::new(sender, gas_payment, test_env.rgp)
        .transfer_haneul_to_address_balance(
            FundSource::coin(transfer_payment),
            vec![(1, recipient)],
        )
        .build();
...
async fn test_coin_reservation_rejected_in_sponsored_transaction() {
    ...
    use haneul_types::transaction::{
        GasData, ProgrammableTransaction, Transaction, TransactionData,
    };
