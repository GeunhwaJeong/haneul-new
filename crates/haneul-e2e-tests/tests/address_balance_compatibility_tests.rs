...
use test_cluster::addr_balance_test_env::{TestEnv, TestEnvBuilder, get_haneul_accumulator_object_id};
...
async fn test_coin_reservation_validation() {
    ...
    // Verify that invalid epoch for coin reservation in gas is rejected.
    {
        ...
        let tx = test_env
            ...
            .transfer_haneul_to_address_balance(FundSource::coin(gas1), vec![(1, sender1)])
            .build();
    ...
    // Verify that non-HANEUL coin reservations cannot be used as gas.
    {
        ...
        // Make a coin reservation for the non-HANEUL coin.
        let coin_reservation =
            test_env.encode_coin_reservation_for_type(sender1, 0, 10_000_000_000, coin_type);

        // Attempt to use it as gas - should be rejected since gas must be HANEUL.
        let tx = test_env
...
async fn test_valid_coin_reservation_transfers() {
    ...
    let recipient_balance = test_env.get_haneul_balance(recipient).await;
...
async fn test_valid_coin_reservation_gas_payments() {
    ...
    let initial_sender_balance = test_env.get_haneul_balance_ab(sender);

    let tx = TestTransactionBuilder::new(sender, gas_payment, test_env.rgp)
        .transfer_haneul_to_address_balance(FundSource::coin(transfer_payment), vec![(1, recipient)])
        .build();
    ...
    // ensure both balances arrived at the recipient
    let recipient_balance = test_env.get_haneul_balance_ab(recipient);

    // 1 GEUNHWA transferred.
    assert_eq!(recipient_balance, 1);

    // Sender should have lost the gas charge and the 1 GEUNHWA transferred.
...
async fn test_gas_coin_callarg_with_coin_reservation_gas() {
    ...
    // The reservation includes budget + 100, so the materialized GasCoin has 100 geunhwa.
    {
        let gas_reservation =
            test_env.encode_coin_reservation(sender, 0, budget + available_for_gas_coin);
        let recipient = HaneulAddress::random_for_testing_only();

        // Try to transfer 200 geunhwa, but only 100 is available in the materialized GasCoin.
        ...
        let tx = TestTransactionBuilder::new(sender, gas_reservation, test_env.rgp)
            .transfer_haneul(Some(excessive_transfer), recipient)
            .build();
        let (_, effects) = test_env.exec_tx_directly(tx).await.unwrap();
    ...
    // Second test: transferring exactly the available amount should succeed.
    {
        ...
        // Use transfer_haneul which internally does SplitCoins(GasCoin, [amount]) + TransferObjects.
        let tx = TestTransactionBuilder::new(sender, gas_reservation, test_env.rgp)
            .transfer_haneul(Some(available_for_gas_coin), recipient)
            .build();
        let (_, effects) = test_env.exec_tx_directly(tx).await.unwrap();
        assert!(
...
async fn test_gas_coin_by_ref_with_coin_reservation_gas() {
    ...
    // Uses transfer_haneul_to_address_balance which splits from GasCoin and sends to
    ...
    // Use transfer_haneul_to_address_balance which does:
...
async fn test_gas_coin_callarg_with_mixed_gas() {
    ...
    let initial_ab_balance = test_env.get_haneul_balance_ab(sender);
    let recipient = HaneulAddress::random_for_testing_only();
    let transfer_amount = 100u64;
    ...
    let tx = test_env
        ...
        .transfer_haneul(Some(transfer_amount), recipient)
        .build();
    ...
    let final_ab_balance = test_env.get_haneul_balance_ab(sender);
...
async fn test_deny_list_enforced_for_coin_reservation() {
    ...
    // - crates/haneul-e2e-tests/tests/per_epoch_config_stress_tests.rs (uses deny_list_v2_add/remove)
    // - crates/haneul-e2e-tests/tests/rpc/v2/state_service/get_coin_info.rs::test_get_coin_info_regulated_coin
    //
    // Regulated coin modules:
    // - crates/haneul-e2e-tests/tests/move_test_code/sources/regulated_coin.move
    // - crates/haneul-e2e-tests/tests/rpc/data/regulated_coin/sources/regulated_coin.move
}
...
/// Regression test: smashing two real gas coins must conserve HANEUL and produce the
...
async fn test_coin_reservation_rejected_in_sponsored_transaction() {
    ...
    use haneul_types::transaction::{GasData, ProgrammableTransaction, Transaction, TransactionData};
    ...
    let sponsor_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx_data, Intent::haneul_transaction())
        .await
        .unwrap();
...
async fn test_gas_smash_no_ab_underflow_on_iffw() {
    ...
    let dummy = HaneulAddress::random_for_testing_only();
    let tx1 = test_env
    ...
    let final_ab = test_env.get_haneul_balance_ab(sender);
...
/// two real coins each hold only 1 GEUNHWA. Total real-coin gas after smashing is 2
/// GEUNHWA — far below the gas budget. With AB entries dropped on IFFW the gas
...
async fn test_gas_smash_no_underflow_on_iffw_tiny_real_coins() {
    ...
    // covers the default gas budget (≈ 5_000_000_000 GEUNHWA).
    ...
    // Pre-step: split two 1-GEUNHWA coins off a normal gas coin and transfer them
    ...
    let tiny_coins: Vec<_> = split_effects
        ...
        .into_iter()
        .filter(
            |(_, owner)| matches!(owner, haneul_types::object::Owner::AddressOwner(a) if *a == sender),
        )
    ...
    // TX2: gas_data.payment = [tiny_coin_a (1 GEUNHWA), tiny_coin_b (1 GEUNHWA),
    ...
    // check (≤ initial_ab) and reservation + 2 GEUNHWA > default budget.
    let reservation = initial_ab / 2;
    let fake_coin = test_env.encode_coin_reservation(sender, 0, reservation);
    let tx2 = test_env
    ...
    // to deduct gas from a 2-GEUNHWA coin and panics in deduct_gas's
...
where
    F: FnOnce(&TestEnv, HaneulAddress, u64, ObjectRef, ObjectRef) -> (Vec<ObjectRef>, Vec<ObjectRef>),
{
    if has_mainnet_protocol_config_override() {
        return;
    }
...
fn build_fake_coin_reservation_pt(
    ...
    let recipient_arg = pt_builder
        .pure(HaneulAddress::random_for_testing_only())
        .unwrap();
