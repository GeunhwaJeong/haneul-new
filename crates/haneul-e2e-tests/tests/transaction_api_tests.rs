...
//!
//! Haneul never re-executes an already-executed checkpoint (the checkpoint executor resumes
...
use haneul_types::base_types::{AuthorityName, HaneulAddress, TransactionDigest};
use haneul_types::crypto::get_account_key_pair;
use haneul_types::effects::TransactionEffectsAPI;
use haneul_types::messages_grpc::{SubmitTxRequest, SubmitTxResult};
...
async fn test_resubmit_executed_transaction_after_restart_and_epoch_change() {
    ...
    let recipient: HaneulAddress = get_account_key_pair().0;
    let tx_data = cluster
        .test_transaction_builder()
        .await
        .transfer_haneul(Some(1), recipient)
        .build();
