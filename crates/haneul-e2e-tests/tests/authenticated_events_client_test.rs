...
async fn test_client_multiple_streams() {
    ...
    let sender = test_cluster.wallet.config.keystore.addresses()[0];
    let stream_id_1 = HaneulAddress::from(package_id_1);
    let stream_id_2 = HaneulAddress::from(package_id_2);
...
async fn test_client_resume_from_checkpoint_without_events() {
    ...
    let haneul_light_client::authenticated_events::ClientError::InternalError(msg) = e else {
...
async fn test_client_detects_corrupted_event_data() {
    use haneul_macros::register_fail_point_if;
