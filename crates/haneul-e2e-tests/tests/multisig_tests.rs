...
async fn create_credential_and_sign_test_tx_with_passkey_multisig(
    ...
    let origin = Url::parse("https://www.haneul.io").unwrap();
    ...
    // Compute haneul address as sender, fund gas and make a test transaction.
    let sender = match sender {
        Some(s) => s,
        None => HaneulAddress::from(&multisig_pk),
    };
...
async fn test_multisig_e2e() {
    ...
    .unwrap();
    let wrong_sender = HaneulAddress::from(&wrong_multisig_pk);
    let gas = test_cluster
...
async fn test_multisig_with_zklogin_scenerios() {
    ...
    let (eph_kp, _eph_pk, zklogin_inputs) =
        &load_test_vectors("../haneul-types/src/unit_tests/zklogin_test_vectors.json")[1];
    let (eph_kp_1, _, _) =
        &load_test_vectors("../haneul-types/src/unit_tests/zklogin_test_vectors.json")[2];
    let zklogin_pk = PublicKey::ZkLogin(
        ZkLoginPublicIdentifier::new(zklogin_inputs.get_iss(), zklogin_inputs.get_address_seed())
            .unwrap(),
    );
    ...
    let wrong_multisig_addr = HaneulAddress::from(
    ...
    let sender = HaneulAddress::try_from(&multisig).unwrap();
    let tx_data = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_haneul(None, HaneulAddress::ZERO)
        .build();
    ...
    let bad_multisig_addr = HaneulAddress::from(&bad_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_2 = HaneulAddress::from(&bad_multisig_pk_2);
    let gas = test_cluster
    ...
    let bad_multisig_addr_3 = HaneulAddress::from(&small_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_4 = HaneulAddress::from(&multisig_pk_with_dup);
    let gas = test_cluster
    ...
    let bad_multisig_addr_11 = HaneulAddress::from(&multisig_pk_11);
    let gas = test_cluster
    ...
    let bad_multisig_addr = HaneulAddress::from(&multisig_pk_12);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
    ...
    let bad_multisig_addr = HaneulAddress::from(&bad_multisig_empty_pk);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
...
async fn test_random_zklogin_in_multisig() {
    ...
    let test_vectors =
        &load_test_vectors("../haneul-types/src/unit_tests/zklogin_test_vectors.json")[1..11];
    let test_cluster = TestClusterBuilder::new()
        .with_epoch_duration_ms(15000)
        .with_default_jwks()
        .build()
...
async fn test_multisig_legacy_works() {
    ...
    let transfer_from_multisig = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_haneul(Some(1000000), HaneulAddress::ZERO)
