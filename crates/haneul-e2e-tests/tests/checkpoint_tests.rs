...
async fn basic_checkpoints_integration_test() {
    ...
    let tx = make_transfer_haneul_transaction(&test_cluster.wallet, None, None).await;
...
async fn test_checkpoint_split_brain() {
    #[cfg(msim)]
    {
        ...
        // loss of liveness
        use haneul_core::authority::{CheckpointTimeoutConfig, init_checkpoint_timeout_config};
...
async fn test_checkpoint_contents_v2_alias_versions() {
    ...
    let test_cluster = TestClusterBuilder::new()
        ...
        .with_state_sync_config(haneul_config::p2p::StateSyncConfig {
    ...
    let mut client = test_cluster
        ...
        .unwrap();
    let request = haneul_types::messages_grpc::RawSubmitTxRequest {
        transactions: vec![
            bcs::to_bytes(&enable_tx).unwrap().into(),
            bcs::to_bytes(&transfer_tx).unwrap().into(),
        ],
        submit_type: haneul_types::messages_grpc::SubmitTxType::SoftBundle.into(),
    };
    let result = client
    ...
    let post_enable_tx = test_cluster
        ...
        .sign_transaction(
            &TestTransactionBuilder::new(account, gas_objects[2], gas_price)
                .transfer_haneul(None, account)
                .build(),
        )
        .await;
    let post_enable_digest = *post_enable_tx.digest();
    let result = client
        .submit_transaction(haneul_types::messages_grpc::RawSubmitTxRequest {
            transactions: vec![bcs::to_bytes(&post_enable_tx).unwrap().into()],
            submit_type: haneul_types::messages_grpc::SubmitTxType::Default.into(),
        })
        .await
        .unwrap()
        .into_inner();
