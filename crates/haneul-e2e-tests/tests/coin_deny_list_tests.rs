...
async fn test_coin_deny_transfer() {
    ...
    // Sending HANEUL coin should still work.
    let tx_data = test_context
        .test_cluster
        .test_transaction_builder_with_sender(test_context.new_coin_owner)
        .await
        .transfer_haneul(Some(1), test_context.test_cluster.get_address_2())
        .build();
    test_context
        .test_cluster
...
struct TestContext {
    ...
    new_coin_owner: HaneulAddress,
    deny_cap_object_id: ObjectID,
    deny_cap_object_owner: HaneulAddress,
}
...
impl TestContext {
    ...
    async fn call_deny_list_api(&self, address: HaneulAddress, action: DenyAction) {
