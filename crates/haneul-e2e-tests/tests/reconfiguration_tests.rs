...
use haneul_types::transaction::{TransactionDataAPI, TransactionExpiration, VerifiedTransaction};
...
async fn test_expired_locks() {
    ...
    let t1 = transfer_haneul(1);
    // attempt to equivocate
    let t2 = transfer_haneul(2);
...
async fn execute_add_validator_transactions(
    ...
    let stake_coin = test_cluster
        ...
        .gas_for_owner_budget(
            address,
            MIN_VALIDATOR_JOINING_STAKE_GEUNHWA,
            Default::default(),
        )
        .await
        .unwrap()
