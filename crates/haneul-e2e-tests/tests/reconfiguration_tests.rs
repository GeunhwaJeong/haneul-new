...
async fn test_expired_locks() {
    ...
    let transfer_haneul = |amount| {
        TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_haneul(Some(amount), receiver)
            .build()
    };

    let t1 = test_cluster.wallet.sign_transaction(&transfer_haneul(1)).await;
    // attempt to equivocate
    let t2 = test_cluster.wallet.sign_transaction(&transfer_haneul(2)).await;
    ...
    response
        ...
        .filter(|change| match change {
            ObjectChange::Created { object_type, .. } => {
                object_type.name == ident_str!("StakedHaneul").into()
            }
            _ => false,
        })
        .collect::<Vec<_>>()
