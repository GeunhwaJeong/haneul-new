...
async fn validate_chunked_execution_time_storage(test_cluster: &TestCluster) {
    ...
    let system_state = haneul_system_state::get_haneul_system_state(object_store.as_ref())
        .expect("System state must exist");

    let extra_fields_id = match &system_state {
        haneul_types::haneul_system_state::HaneulSystemState::V2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        #[cfg(msim)]
        haneul_types::haneul_system_state::HaneulSystemState::SimTestDeepV2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        #[cfg(msim)]
        haneul_types::haneul_system_state::HaneulSystemState::SimTestShallowV2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        haneul_types::haneul_system_state::HaneulSystemState::V1(_) => {
            panic!("HaneulSystemState V1 not supported for chunking validation");
        }
        #[cfg(msim)]
        haneul_types::haneul_system_state::HaneulSystemState::SimTestV1(_) => {
            panic!("HaneulSystemState V1 not supported for chunking validation");
        }
    };
...
async fn send_transactions(
    ...
    for i in 0..6 {
        ...
        let counter_arg = ptb
            .obj(haneul_types::transaction::ObjectArg::SharedObject {
                id: counter_id,
                initial_shared_version,
