...
/// End-to-end shared transaction test for a Haneul validator. It does not test the client or wallet,
/// but tests the end-to-end flow from Haneul to consensus.
#[sim_test]
...
async fn access_clock_object_test() {
    ...
    assert_eq!(
        ...
        effects
            .shared_objects()
            .iter()
            .find(|(id, _, _)| *id == HANEUL_CLOCK_OBJECT_ID)
            .unwrap()
            .clone()
    );
...
async fn shared_object_sync() {
    ...
    slow_validators
        .iter()
        .for_each(|n: &haneul_types::base_types::AuthorityName| {
