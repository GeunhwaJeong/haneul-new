...
fn build_split_gas_coin_ptb(
    ...
    gas: Option<haneul_types::base_types::ObjectRef>,
    gas_budget: u64,
    gas_price: u64,
) -> TransactionData {
    let mut ptb = ProgrammableTransactionBuilder::new();
    let amount_arg = ptb.pure(amount).unwrap();
    let split_result = ptb.command(haneul_types::transaction::Command::SplitCoins(
        Argument::GasCoin,
        vec![amount_arg],
    ));
    let recipient_arg = ptb.pure(recipient).unwrap();
    ptb.command(haneul_types::transaction::Command::TransferObjects(
        vec![split_result],
        recipient_arg,
    ));
    let pt = ptb.finish();
...
async fn test_has_ab_has_coins_uses_gas_coin() {
    ...
    // Fund sender's address balance with 10 HANEUL - sufficient on its own
    let ab_amount = 10 * GEUNHWA_PER_HANEUL;
    ...
    // Request 5 HANEUL - both AB (10 HANEUL) and coins (~30M HANEUL) can cover this independently
    let amount = 5 * GEUNHWA_PER_HANEUL;
    ...
    assert!(
        reservation_amount >= ab_amount - 100_000_000, // Allow up to 0.1 HANEUL for gas
...
async fn test_has_ab_has_coins_no_gas_coin() {
    ...
    let ab_amount = GEUNHWA_PER_HANEUL;
    test_env.fund_one_address_balance(sender, ab_amount).await;
    ...
    let small_budget = 50_000_000; // 0.05 HANEUL - easily covered by 1 HANEUL AB
    ...
    let large_budget = 5 * GEUNHWA_PER_HANEUL; // 5 HANEUL - exceeds 1 HANEUL AB
    // Don't provide explicit gas - let gas selection choose
...
async fn test_has_ab_no_coins() {
    ...
    for coin in all_gas {
        let transfer_tx = TestTransactionBuilder::new(sender1, coin, test_env.rgp)
            .transfer_haneul(None, sender2) // Transfer entire coin
            .build();
    ...
    let ab_balance = test_env.get_haneul_balance_ab(sender1);
    assert!(
        ab_balance > 0,
    ...
    let gas_budget = 50_000_000; // 0.05 HANEUL
    let ptb = ProgrammableTransactionBuilder::new();
    let pt = ptb.finish();
...
async fn test_insufficient_funds() {
    ...
    // Fund AB with 1 HANEUL
    test_env
        .fund_one_address_balance(sender, GEUNHWA_PER_HANEUL)
        .await;

    let (sender, gas) = test_env.get_sender_and_gas(0);

    // Request way more than available (100M HANEUL)
    let amount = 100_000_000 * GEUNHWA_PER_HANEUL;
    let gas_budget = 50_000_000;
...
async fn test_combined_ab_and_coins_needed() {
    use haneul_test_transaction_builder::TestTransactionBuilder;
    use haneul_types::crypto::{AccountKeyPair, HaneulKeyPair, get_key_pair};
    ...
    let (limited_sender, keypair): (HaneulAddress, AccountKeyPair) = get_key_pair();
    let keypair = HaneulKeyPair::Ed25519(keypair);
    let recipient = HaneulAddress::random_for_testing_only();

    // Add the new account to the wallet so we can sign transactions
    test_env.cluster.wallet.add_account(None, keypair).await;

    // Use genesis account to transfer 10 HANEUL coin to limited_sender
    let (genesis_sender, genesis_gas) = test_env.get_sender_and_gas(0);
    let coin_amount = 10 * GEUNHWA_PER_HANEUL;
    let transfer_tx = TestTransactionBuilder::new(genesis_sender, genesis_gas, test_env.rgp)
        .transfer_haneul(Some(coin_amount), limited_sender)
        .build();
    let (_, effects) = test_env
    ...
    let _created_coin = effects
        ...
        .iter()
        .find(|(_, owner)| {
            matches!(owner, haneul_types::object::Owner::AddressOwner(addr) if *addr == limited_sender)
        })
        .map(|(obj_ref, _)| *obj_ref)
        .expect("Should have created a coin for limited_sender");

    // Fund limited_sender's address balance with 5 HANEUL (from genesis account)
    let genesis_gas = test_env.get_gas_for_sender(genesis_sender)[0];
    let ab_amount = 5 * GEUNHWA_PER_HANEUL;
    let fund_ab_tx = TestTransactionBuilder::new(genesis_sender, genesis_gas, test_env.rgp)
        .transfer_haneul_to_address_balance(
            haneul_test_transaction_builder::FundSource::coin(genesis_gas),
    ...
    // - Total: 15 HANEUL
    //
    // Request 12 HANEUL - this requires BOTH sources:
    // - Coin alone (10 HANEUL) is insufficient
    // - AB alone (5 HANEUL) is insufficient
    // - Combined (15 HANEUL) is sufficient
    let request_amount = 12 * GEUNHWA_PER_HANEUL;
    let gas_budget = 50_000_000;
...
async fn test_ab_only_budget_exceeds_half_balance() {
    ...
    // Fund sender1's address balance with 1 HANEUL.
    let ab_amount = GEUNHWA_PER_HANEUL;
    test_env.fund_one_address_balance(sender1, ab_amount).await;
...
fn object_ref_to_proto(
    obj_ref: &haneul_types::base_types::ObjectRef,
) -> haneul_rpc::proto::haneul::rpc::v2::ObjectReference {
    let mut message = haneul_rpc::proto::haneul::rpc::v2::ObjectReference::default();
...
async fn test_resolve_handles_coin_reservation_in_gas_payment() {
    ...
    // Fund sender's address balance with 10 HANEUL.
    let ab_amount = 10 * GEUNHWA_PER_HANEUL;
    test_env.fund_one_address_balance(sender, ab_amount).await;
    ...
    let balance = test_env.get_haneul_balance_ab(sender);
...
async fn test_estimated_budget_excludes_mock_gas_coin_storage_for_address_balance() {
    ...
    let sponsor_ab_amount = 5 * GEUNHWA_PER_HANEUL;
    test_env
    ...
    let sender_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &resolved_transaction, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    // storage write — `object_size * obj_data_cost_refundable * storage_gas_price` GEUNHWA,
    // typically ~1M GEUNHWA under default protocol params for a Coin<HANEUL> object — also
    ...
    assert!(
        ...
        "Estimated budget overshot actual net gas usage by {overshoot} GEUNHWA \
         (simulated={simulated_budget}, actual={actual_net_gas}); \
         max expected overshoot is {max_expected_overshoot} GEUNHWA. \
