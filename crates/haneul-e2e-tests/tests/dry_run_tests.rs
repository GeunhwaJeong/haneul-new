...
async fn build_transfer_haneul_tx(
...
async fn test_dry_run_gas_budget_too_low() {
    let cluster = TestClusterBuilder::new()
        ...
        .build()
        .await;
    let client = cluster.haneul_client();
    let sender = cluster.get_address_0();
    let rgp = cluster.get_reference_gas_price().await;

    let tx = build_transfer_haneul_tx(sender, 0, rgp).await;
    let err = client
        .read_api()
...
async fn test_dry_run_gas_budget_too_high() {
    ...
    let tx = build_transfer_haneul_tx(sender, max_budget + 1, rgp).await;
    let err = client
        .read_api()
        .dry_run_transaction_block(tx)
        .await
...
async fn test_dry_run_gas_price_too_low() {
    ...
    let tx = build_transfer_haneul_tx(sender, max_budget, rgp - 1).await;
    let err = client
        .read_api()
        .dry_run_transaction_block(tx)
        .await
...
async fn test_dry_run_gas_price_too_high() {
    ...
    let tx = build_transfer_haneul_tx(sender, max_budget, max_gas_price).await;
    let err = client
        .read_api()
        .dry_run_transaction_block(tx)
        .await
