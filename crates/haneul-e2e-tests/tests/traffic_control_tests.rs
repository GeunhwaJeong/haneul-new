...
use haneul_core::traffic_controller::nodefw_test_server::NodeFwTestServer;
...
async fn test_traffic_control_rpc_spam_blocked() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..4 {
        let response: RpcResult<HaneulTransactionBlockResponse> = jsonrpc_client
...
#[tokio::test]
async fn test_traffic_control_rpc_spam_delegated() -> Result<(), anyhow::Error> {
    ...
    server.start(firewall_config.destination_port).await;
