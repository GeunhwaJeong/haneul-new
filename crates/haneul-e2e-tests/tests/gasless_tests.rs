...
async fn setup_custom_coin(test_env: &mut TestEnv, funding: &[(u64, HaneulAddress)]) -> TypeTag {
...
async fn test_gasless_transfer_success() {
    ...
    assert_eq!(test_env.get_haneul_balance_ab(sender), 0);
...
async fn test_gasless_dryrun() {
    ...
    let tx_digest = haneul_types::digests::TransactionDigest::genesis_marker();
    let result = test_env
        .cluster
        .fullnode_handle
        .haneul_node
    ...
    let result = test_env.cluster.fullnode_handle.haneul_node.with(|node| {
        node.state().simulate_transaction(
            tx_data,
            haneul_types::transaction_executor::TransactionChecks::Enabled,
            true,
        )
    });
    if let Err(e) = result {
...
async fn test_gasless_multi_recipient() {
    ...
    let balance_type = TypeTag::Struct(Box::new(StructTag {
        address: *HANEUL_FRAMEWORK_PACKAGE_ID,
...
async fn setup_mintable_coin_env(
    ...
    TypeTag,
    Vec<haneul_types::base_types::ObjectRef>,
) {
...
async fn test_gasless_coin_input_success() {
    ...
    let sender_haneul_before = test_env.get_haneul_balance(sender).await;
    ...
    let sender_haneul_after = test_env.get_haneul_balance(sender).await;
    assert_eq!(
        sender_haneul_after, sender_haneul_before,
        "Sender's HANEUL balance should not increase from storage rebate"
    );
