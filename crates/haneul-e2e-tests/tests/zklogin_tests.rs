...
async fn do_zklogin_test(address: HaneulAddress, legacy: bool) -> HaneulResult {
...
async fn test_auth_state_creation() {
    #[cfg(msim)]
    {
        ...
        let framework = haneul_framework_snapshot::load_bytecode_snapshot(25).unwrap();
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(HANEUL_AUTHENTICATOR_STATE_OBJECT_ID)
                .is_none());
        });
    }
