...
fn wrap_tx(sender: HaneulAddress, kind: TransactionKind) -> TransactionData {
...
async fn simulate_gasless_with_no_address_owned_inputs_succeeds() {
    ...
    let haneul_type = GAS::type_tag();
    transaction::add_gasless_token_for_testing(haneul_type.to_canonical_string(true), 0);
    // Seed sender's HANEUL address balance. `fund_one_address_balance` updates the gas ref from
    ...
    let tx = build_balance_withdrawal_send_funds_tx(sender, haneul_type, 1_000, recipient);
