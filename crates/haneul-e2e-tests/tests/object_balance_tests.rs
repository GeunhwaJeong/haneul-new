...
async fn test_object_balance_withdraw_stress() {
    ...
    // Fund the vault objects, each with an initial balance of 1000 GEUNHWA.
    ...
    let tx = test_cluster
        ...
        .await
        .split_coin(gas_coins[1], vec![10000 * GEUNHWA_PER_HANEUL; 7])
        .build();
    let effects = test_cluster.sign_and_execute_transaction(&tx).await.effects;
        ...
        handles.push(tokio::spawn(async move {
            ...
            for _ in 0..50 {
                ...
                let tx = test_cluster
                    ...
                    .await
                    .transfer_haneul_to_address_balance(fund_source, vec![(amount, recipient)])
                    .build();
