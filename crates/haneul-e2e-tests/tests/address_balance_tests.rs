...
async fn test_json_rpc_balance_changes_with_address_balance_withdrawal() {
    ...
    let execute_response = rpc_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(HaneulTransactionBlockResponseOptions::new().with_balance_changes()),
    ...
    let receiver_change = get_balance_changes
        .iter()
        .find(|c| c.owner == haneul_types::object::Owner::AddressOwner(receiver))
    ...
    let sender_change = get_balance_changes
        .iter()
        .find(|c| c.owner == haneul_types::object::Owner::AddressOwner(sender))
