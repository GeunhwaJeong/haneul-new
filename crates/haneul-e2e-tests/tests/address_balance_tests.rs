...
use haneul_types::accumulator_metadata::get_accumulator_object_count;
use haneul_types::{
    ...
    balance::Balance,
    base_types::{FullObjectRef, ObjectID, ObjectRef, SequenceNumber, HaneulAddress, dbg_addr},
...
async fn get_sender_and_all_gas(context: &mut WalletContext) -> (HaneulAddress, Vec<ObjectRef>) {
    get_nth_sender_and_all_gas(context, 0).await
}

async fn get_sender_and_one_gas(context: &mut WalletContext) -> (HaneulAddress, ObjectRef) {
...
async fn test_accumulators_disabled() {
    ...
    test_cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        assert!(
            !AccumulatorValue::exists(child_object_resolver, None, sender, &haneul_coin_type).unwrap(),
...
async fn test_deposits() {
    ...
    test_cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        // 1 account (recipient) with HANEUL balance = 5 objects.
...
fn get_balance(child_object_resolver: &dyn ChildObjectResolver, owner: HaneulAddress) -> u64 {
    let haneul_coin_type = Balance::type_tag(GAS::type_tag());
...
async fn test_deposit_and_withdraw() {
    ...
    test_cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        // Verify the accumulator object count after settlement.
        // 1 account (sender) with HANEUL balance = 5 objects.
        let object_count = get_accumulator_object_count(state.get_object_store().as_ref())
            .expect("read cannot fail")
            .expect("accumulator object count should exist after settlement");
        assert_eq!(object_count, 5);
...
fn create_withdraw_balance_transaction(
    ...
    let withdraw_arg = FundsWithdrawalArg::balance_from_sender(
        withdraw_amount,
        haneul_types::type_input::TypeInput::from(haneul_types::gas_coin::GAS::type_tag()),
    );
...
async fn test_explicit_sponsor_withdrawal_banned() {
    ...
    let withdrawal = FundsWithdrawalArg::balance_from_sponsor(
        1000,
        haneul_types::type_input::TypeInput::from(GAS::type_tag()),
    );
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let final_sponsor_balance = test_cluster.fullnode_handle.haneul_node.with(|node| {
        let state = node.state();
        let child_object_resolver = state.get_child_object_resolver().as_ref();
    ...
    let final_sender_balance = test_cluster.fullnode_handle.haneul_node.with(|node| {
        let state = node.state();
        let child_object_resolver = state.get_child_object_resolver().as_ref();
...
async fn test_address_balance_large_rebate() {
    ...
    let initial_balance = test_cluster.fullnode_handle.haneul_node.with(|node| {
        let state = node.state();
        let child_object_resolver = state.get_child_object_resolver().as_ref();
        get_balance(child_object_resolver, sender)
    });
    ...
    let final_balance = test_cluster.fullnode_handle.haneul_node.with(|node| {
        let state = node.state();
        let child_object_resolver = state.get_child_object_resolver().as_ref();
        get_balance(child_object_resolver, sender)
    });
...
async fn test_coin_reservation_validation() {
    ...
    let tx = TestTransactionBuilder::new(sender1, gas1, rgp)
        .transfer_haneul_to_address_balance(FundSource::coin(gas1), vec![(1000, sender1)])
        .build();
    ...
    // compute the sender's HANEUL accumulator object id
    let accumulator_obj_id = AccumulatorValue::get_field_id(
        sender1,
        &Balance::type_tag(haneul_types::gas_coin::GAS::type_tag()),
    )
    .unwrap();
...
async fn try_coin_reservation_tx(
    ...
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_haneul_to_address_balance(FundSource::coin(coin_reservation), vec![(1, recipient)])
        .build();
