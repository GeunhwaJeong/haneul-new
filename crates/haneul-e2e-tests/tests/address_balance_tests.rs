...
use haneul_simulator::has_mainnet_protocol_config_override;
use haneul_test_transaction_builder::FundSource;
use haneul_types::{
    HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, HANEUL_CLOCK_OBJECT_ID, HANEUL_CLOCK_OBJECT_SHARED_VERSION,
    HANEUL_FRAMEWORK_PACKAGE_ID, TypeTag,
    accumulator_root::AccumulatorValue,
    balance::Balance,
    base_types::{HaneulAddress, ObjectID, ObjectRef, SequenceNumber, dbg_addr},
...
async fn test_address_balance_gas_v3_accumulator_sign() {
    ...
    match &acc_events[0].write.operation {
        haneul_types::effects::AccumulatorOperation::Split => {}
...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let sponsor_actual = test_env.get_haneul_balance_ab(sponsor);
    let sender_actual = test_env.get_haneul_balance_ab(sender);
    ...
    let sponsor_final = test_env.get_haneul_balance_ab(sponsor);
    let sender_final = test_env.get_haneul_balance_ab(sender);
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let final_sponsor_balance = test_env.get_haneul_balance_ab(sponsor);
    ...
    let final_sender_balance = test_env.get_haneul_balance_ab(sender);
...
async fn test_soft_bundle_different_gas_payers() {
    ...
    let actual_balance1 = test_env.get_haneul_balance_ab(sender1);
    let actual_balance2 = test_env.get_haneul_balance_ab(sender2);
...
/// A single transaction produces two Merge accumulator events to the same `(sender, Balance<HANEUL>)`
...
/// HANEUL-conservation sum.
#[sim_test]
async fn test_accumulator_merge_overflow_poison_pill() {
    let mut test_env = TestEnvBuilder::new().with_num_validators(1).build().await;

    // Publish the test package and fund the sender's HANEUL address balance so that the gas-payment
    ...
    let fund_tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(100_000_000, sender)])
        .build();
    ...
    assert!(
        matches!(
            status,
            haneul_types::execution_status::ExecutionStatus::Failure(failure)
                if failure.error == haneul_types::execution_status::ExecutionFailureStatus::CoinBalanceOverflow
        ),
...
///
/// Gas is always paid in HANEUL, so the uncapped gas-smash deposit Merge that can defeat the
/// object-runtime cap for `Balance<HANEUL>` has no analogue for an arbitrary `Balance<T>`. The only
...
/// (Conservation checking does not apply to non-HANEUL types.) The
    ...
    assert!(
        matches!(
            ...
            haneul_types::execution_status::ExecutionStatus::Failure(failure)
                if matches!(
                    failure.error,
                    haneul_types::execution_status::ExecutionFailureStatus::MovePrimitiveRuntimeError(_)
                )
        ),
...
/// A single object-sourced `u64::MAX` HANEUL withdrawal redeemed and deposited to the sender. The
/// per-key representability guard bounds HANEUL accumulator totals to the total supply, so this
/// `u64::MAX` (both the input-side `Split` and the deposit `Merge`) exceeds the supply and is
/// rejected as `CoinBalanceOverflow` *before* gas is charged — well before the HANEUL-conservation sum
...
async fn test_accumulator_conservation_overflow_single_withdrawal() {
    ...
    // A single u64::MAX HANEUL withdrawal deposited to the sender, paid with a normal gas coin (no
    // address-balance reservation). u64::MAX exceeds the total HANEUL supply, so the per-key guard
    ...
    // The u64::MAX deposit exceeds the total HANEUL supply, so the per-key representability guard
    ...
    assert!(
        matches!(
            ...
            haneul_types::execution_status::ExecutionStatus::Failure(failure)
                if failure.error
                    == haneul_types::execution_status::ExecutionFailureStatus::CoinBalanceOverflow
        ),
...
/// would let a Move-native Merge of exactly `u64::MAX` to `(sender, Balance<HANEUL>)` pass; `charge_gas`
/// then emits a refund Merge (`net_gas_usage() < 0`, gas paid from the sender's HANEUL address balance)
...
/// `TOTAL_SUPPLY_GEUNHWA`, it is rejected as `CoinBalanceOverflow` before gas is charged, so the refund
/// is never emitted.
#[sim_test]
async fn test_accumulator_merge_overflow_gas_refund_poison_pill() {
    ...
    // Fund the sender's HANEUL address balance so the (empty-payment) address-balance gas reservation
    // is backed at signing time.
    let (sender, gas) = test_env.get_sender_and_gas(0);
    let fund_tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(1_000_000_000, sender)])
        .build();
    test_env.exec_tx_directly(fund_tx).await.unwrap();
    ...
    // at obj_data_cost_refundable (100) * storage_gas_price (76) = 7600 GEUNHWA/byte that is ~120M GEUNHWA
    ...
    // Move-native Merge of u64::MAX to (sender, Balance<HANEUL>) via the object-sourced withdrawal.
    ...
    // Empty gas payment + ValidDuring expiration => gas is paid from the sender's HANEUL address
...
/// Gas-coin overflow via `Argument::GasCoin`. The per-key supply guard bounds HANEUL *accumulator*
/// amounts, but merging HANEUL into the PTB gas coin via a `MergeCoins` command is an object mutation,
...
/// `u64::MAX + refund`, which is not representable.
///
/// The gas coin is driven to `u64::MAX` with two object-sourced HANEUL withdrawals — each
/// `< TOTAL_SUPPLY_GEUNHWA`, so each passes the per-key supply guard — redeemed to `Coin<HANEUL>` and
/// merged into the gas coin; the refund is produced by deleting a large pre-existing object.
/// Reaching this requires ~`u64::MAX` GEUNHWA of HANEUL in one coin, more than the total supply.
///
/// The cross-key total-HANEUL-withdraw bound covers this: the two withdrawals sum to ~`u64::MAX`, which
...
async fn test_gas_coin_overflow_via_merge_into_gas_coin() {
    ...
    assert!(
        amount1 < haneul_types::gas_coin::TOTAL_SUPPLY_GEUNHWA
            && amount2 < haneul_types::gas_coin::TOTAL_SUPPLY_GEUNHWA,
    ...
    // Two HANEUL withdrawals returned as Coin<HANEUL>.
    let a1 = builder.pure(amount1).unwrap();
    let c1 = builder.programmable_move_call(
        pkg,
        Identifier::new("accumulator_overflow").unwrap(),
        Identifier::new("withdraw_haneul_as_coin").unwrap(),
        vec![],
        vec![a1],
    );
...
async fn test_address_balance_gas_budget_enforcement_with_storage() {
    ...
    assert!(
        matches!(
            error_kind,
            haneul_types::execution_status::ExecutionErrorKind::InsufficientGas
        ),
...
async fn test_address_balance_large_rebate() {
    ...
    let initial_balance = test_env.get_haneul_balance_ab(sender);
    ...
    let final_balance = test_env.get_haneul_balance_ab(sender);
...
/// Test that JSON-RPC haneul_executeTransactionBlock returns correct balance changes
/// when using address balance withdrawals (FundsWithdrawal).
///
/// This test reproduces a bug where haneul_executeTransactionBlock returns balanceChanges: []
/// despite the transaction producing balance changes that are visible when querying
/// the same digest via haneul_getTransactionBlock.
#[sim_test]
async fn test_json_rpc_balance_changes_with_address_balance_withdrawal() {
    use haneul_json_rpc_api::{ReadApiClient, WriteApiClient};
    ...
    // Verify coin type is HANEUL for both
    assert_eq!(
        sender_change.coin_type,
        GAS::type_tag(),
        "Sender balance change should be HANEUL"
    );
    assert_eq!(
        receiver_change.coin_type,
        GAS::type_tag(),
        "Receiver balance change should be HANEUL"
    );
...
async fn test_explicit_withdrawal_plus_implicit_gas_exceeds_balance() {
    ...
    assert_eq!(withdrawals.get(&haneul_account_id).unwrap().0, 11_000_000);
...
async fn gas_coin_send_funds_storage_out_of_gas_charges_sender_not_receiver() {
    ...
    test_env.verify_accumulator_exists(receiver, receiver_initial_balance);
    // Reconfig runs HANEUL conservation invariant checks across the epoch boundary
    test_env.cluster.trigger_reconfiguration().await;
}
...
async fn address_balance_gas_transfer_storage_objects_out_of_gas_no_panic() {
    ...
    let sender_debit: i128 = acc_events
        ...
        .filter_map(|e| match (&e.write.operation, &e.write.value) {
            (
                haneul_types::effects::AccumulatorOperation::Split,
                haneul_types::effects::AccumulatorValue::Integer(v),
            ) => Some(*v as i128),
            _ => None,
        })
        .sum();
