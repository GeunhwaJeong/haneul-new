...
async fn party_object_deletion_multiple_times() {
    ...
    // transactions.
    let fullnode = test_cluster.spawn_new_fullnode().await.haneul_node;
    fullnode
        .state()
...
async fn party_object_transfer_multi_certs() {
    ...
    haneul_macros::register_fail_point_async("transaction_execution_delay", move || async move {
        let delay = {
...
async fn party_object_grpc() {
    ...
    use haneul_rpc::proto::haneul::rpc::v2::owner::OwnerKind;
...
async fn party_coin_grpc() {
    ...
    let kind = haneul_types::transaction::TransactionKind::ProgrammableTransaction(ptb);
...
async fn party_object_jsonrpc() {
    ...
    #[allow(deprecated)]
    let client = test_cluster.haneul_client();

    let object = client
        .read_api()
        .get_object_with_options(
            object_id,
            haneul_json_rpc_types::HaneulObjectDataOptions::new().with_owner(),
        )
        .await
        .unwrap()
        .data
    ...
    assert_eq!(new_owner_address, HaneulAddress::ZERO);
