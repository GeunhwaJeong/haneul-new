...
async fn publish_auth_event_package(test_cluster: &TestCluster) -> ObjectID {
    ...
    let (package_id, _, _) =
        haneul_test_transaction_builder::publish_package(&test_cluster.wallet, path).await;
    package_id
}
...
async fn load_event_stream_head_by_object_id(
    state: &haneul_core::authority::AuthorityState,
    object_id: ObjectID,
...
async fn authenticated_events_single_event_test() {
    ...
    assert_eq!(
        ...
        HaneulAddress::from(AccountAddress::from(package_id))
    );

    let state = test_cluster.fullnode_handle.haneul_node.state();
...
async fn authenticated_events_mmr_validation_test() {
    ...
    let haneul_stream_head_x = load_event_stream_head_by_object_id(&state, accumulator_obj_id)
        .await
        .expect("Should be able to load event stream head at checkpoint X");

    assert!(
        haneul_stream_head_x.checkpoint_seq > start_checkpoint,
        "checkpoint_seq ({}) must be after start ({})",
        haneul_stream_head_x.checkpoint_seq,
        start_checkpoint,
    );
    assert!(
        haneul_stream_head_x.checkpoint_seq <= checkpoint_x,
    ...
    assert_eq!(
        calculated_stream_head_x.num_events, haneul_stream_head_x.num_events,
        "Event count should match at checkpoint X"
    );

    assert_eq!(
        calculated_stream_head_x.mmr, haneul_stream_head_x.mmr,
        "MMR should match at checkpoint X"
    );

    assert_eq!(
        calculated_stream_head_x.checkpoint_seq, haneul_stream_head_x.checkpoint_seq,
        "checkpoint_seq should match at checkpoint X"
    );

    let stream_head_x = &haneul_stream_head_x;

    for _ in 0..3 {
        let tx = haneul_test_transaction_builder::make_transfer_haneul_transaction(
            &test_cluster.wallet,
            Some(sender),
            None,
        )
    ...
    assert!(
        actual_stream_head_x_prime.checkpoint_seq > haneul_stream_head_x.checkpoint_seq,
