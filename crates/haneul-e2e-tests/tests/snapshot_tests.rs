...
async fn run_one(
    ...
    for cli_cmd in test {
        ...
        let c = HaneulCommand::try_parse_from(cli_cmd_vec)?;
        match c {
            HaneulCommand::Client { cmd, .. } => {
...
async fn basic_read_cmd_snapshot_tests() -> Result<(), anyhow::Error> {
    ...
    let cmds = vec![
        ...
        "haneul client tx-block Duwr9uSk9ZvAndEa8oDHunx345i6oyrp3e78MYHVAbYdv", // valid tx digest
