...
mod sim_only_tests {
    ...
    enum BuildKind {
        /// Extend the HaneulSystem package with a module. This is for tests that live in
        /// `/framework_upgrades/extend` folder.
        ExtendHaneulSystem,
        /// In place builds happen when we want to build the haneul framework directly
        /// in its root folder.
        InPlace,
    ...
    impl From<&str> for BuildKind {
        fn from(value: &str) -> Self {
            ...
            // we want to extend the haneul system module with extra code
            if value.starts_with("extend/") {
                return Self::ExtendHaneulSystem;
            }

            // If we are dealing with in-place framework, we need to build in place.
            if value.starts_with("../../../haneul-framework/") {
                return Self::InPlace;
            }
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, HaneulAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1.as_object_ref()).await;
        transfer_obj(&cluster, HaneulAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let haneul_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(haneul_extra, fixture_modules("extra_package").await);
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &haneul_extra).await;
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_haneul_system_modules(to).await;
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([haneul_system_package_object(from).await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.haneul_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_haneul_system_modules("extend/compatible").await;
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([haneul_system_package_object("extend/base").await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        // Gotta do both
        let base_modules = haneul_system_modules("extend/base").await;
        let compatible_modules = haneul_system_modules("extend/compatible").await;

        override_haneul_system_modules_cb(Box::new(move |name| {
            if name == first_name {
    ...
    async fn test_safe_mode_recovery() {
        ...
        override_haneul_system_modules("mock_haneul_systems/base").await;
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a haneul system package that would abort during epoch change txn
            .with_objects([haneul_system_package_object("mock_haneul_systems/safe_mode").await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn haneul_system_mock_smoke_test() {
        ...
        let test_cluster = TestClusterBuilder::new()
            ...
            .with_objects([haneul_system_package_object("mock_haneul_systems/base").await])
            .build()
            .await;
    ...
    async fn haneul_system_state_shallow_upgrade_test() {
        ...
        override_haneul_system_modules("mock_haneul_systems/shallow_upgrade").await;
    ...
    async fn haneul_system_state_deep_upgrade_test() {
        ...
        override_haneul_system_modules("mock_haneul_systems/deep_upgrade").await;
    ...
    #[sim_test]
    async fn haneul_system_state_production_upgrade_test() {
        // Use this test to test a real haneul system state upgrade. To make this test work,
        // put the new haneul system in a new path and point to it in the override.
        ...
        override_haneul_system_modules("../../../haneul-framework/packages/haneul-system").await;
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
    ...
    async fn haneul_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .await
            .get_haneul_system_modules()
            .cloned()
            .collect()
    }

    /// Like `haneul_system_modules`, but package the modules in an `Object`.
    async fn haneul_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &haneul_system_modules(fixture).await,
    ...
    async fn fixture_package(fixture: &str) -> CompiledPackage {
        ...
        let project_path = match build_kind {
            BuildKind::ExtendHaneulSystem | BuildKind::TempDir => {
                let tempdir_path = tempdir.path();

                let haneul_packages_path = root.join("../haneul-framework/packages");

                // Copy the whole `haneul-framework/packages` into the tempdir under `/system-packages/*`
                copy_dir_all(&haneul_packages_path, &tempdir_path.join("system-packages")).unwrap();

                let project_path = tempdir_path.join("tmp-test");

                // Copy `haneul_system` into the project (we copy that first, before we paste the actual project).
                // The project lives in `/${fixture}/`, so we copy haneul-system move files into `/fixture/sources/`
                if build_kind == BuildKind::ExtendHaneulSystem {
                    copy_dir_all(
                        &haneul_packages_path.join("haneul-system/sources"),
    ...
    // Copied from `haneul/tests/cli_tests.rs`
