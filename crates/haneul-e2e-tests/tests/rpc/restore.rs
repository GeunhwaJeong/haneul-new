...
//! Restore/resume behavior of the embedded `haneul-rpc-store` index backend
...
//!     [`haneul_core::rpc_store_embed::EmbeddedRpcStore::bootstrap_action`]
//!     (surfaced through `HaneulNode::embedded_rpc_store`), not over the RPC.
...
//! to the running `HaneulNode`, so a stop releases the node's RocksDB locks
//! only if no strong handle outlives it. These helpers therefore never
//! retain a `HaneulNodeHandle` across a restart -- reads fetch a transient
...
use haneul_core::rpc_store_embed::Bootstrap;
...
/// An rpc config that builds the embedded `haneul-rpc-store` index backend
...
/// `rpc-index` nor the embedded `haneul-rpc-store`.
...
/// because no strong `HaneulNodeHandle` is held across the call.
...
struct Transfer {
    ...
    /// A fresh address with no prior coins, so its post-transfer HANEUL
...
/// The total HANEUL balance the embedded store reports for `owner`.
...
async fn pruned_available_range_forces_rebuild() {
    ...
    wait_for_indexed(&cluster, &name, chain_tip(&cluster)).await;
    assert_eq!(
        haneul_balance(&rpc_url, warmup.receiver).await,
        warmup.amount,
