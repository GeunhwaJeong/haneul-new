...
async fn gas_object(cluster: &TestCluster, sender: HaneulAddress) -> ObjectRef {
    cluster
        .wallet
...
async fn transfer_self(cluster: &TestCluster, sender: HaneulAddress) -> ExecutedTransaction {
    let gas = gas_object(cluster, sender).await;
    let mut builder = ProgrammableTransactionBuilder::new();
...
async fn split_transfer(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_haneul(recipient, Some(1_000_000));
    (
        execute_programmable(cluster, sender, gas, builder).await,
        affected_gas_id,
    )
