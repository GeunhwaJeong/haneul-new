...
use haneul_rpc::proto::haneul::rpc::v2::VerifySignatureRequest;
use haneul_rpc::proto::haneul::rpc::v2::signature_verification_service_client::SignatureVerificationServiceClient;
...
async fn test_verify_signature_zklogin() -> Result<(), anyhow::Error> {
    ...
    let (kp, pk_zklogin, inputs) = &haneul_types::utils::load_test_vectors(
        "../haneul-types/src/unit_tests/zklogin_test_vectors.json",
    )[1];
    ...
    // address checks fail
    let wrong_address = HaneulAddress::random_for_testing_only();
    let response = client
        .verify_signature({
            let mut request = VerifySignatureRequest::default();
            request.message = Some(message.clone());
