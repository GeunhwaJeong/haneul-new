...
use haneul_rpc::proto::haneul::rpc::v2::coin_metadata::MetadataCapState;
    ...
    let registry_initial_version = registry_obj_response
        ...
        .and_then(|owner| {
            if owner.kind == Some(OwnerKind::Shared as i32) {
                owner
                    .version
                    .map(haneul_types::base_types::SequenceNumber::from)
            } else {
                None
            }
        })
...
async fn finalize_registration(
    ...
    let mut client = haneul_rpc::client::v2::Client::new(test_cluster.rpc_url().to_owned()).unwrap();
