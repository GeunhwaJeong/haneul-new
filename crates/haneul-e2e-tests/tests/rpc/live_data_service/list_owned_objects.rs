...
use haneul_rpc_api::proto::rpc::v2alpha::{
    GetCoinInfoRequest, GetCoinInfoResponse, ListOwnedObjectsRequest,
};
use haneul_rpc_api::proto::rpc::v2beta::changed_object::{IdOperation, OutputObjectState};
...
async fn test_filter_by_type() {
    ...
    let haneul = "0x2::coin::Coin<0x2::haneul::HANEUL>"
        .parse::<TypeTag>()
        .unwrap()
        .to_string();
    let mut channel = tonic::transport::Channel::from_shared(cluster.rpc_url().to_owned())
    ...
    // We start with some HANEUL coins
    assert!(!objects.is_empty());
    assert!(objects.iter().all(|o| o.object_type() == haneul));
...
async fn test_reverse_sorted_coins_by_balance() {
    ...
    let address = haneul_types::base_types::HaneulAddress::random_for_testing_only();
