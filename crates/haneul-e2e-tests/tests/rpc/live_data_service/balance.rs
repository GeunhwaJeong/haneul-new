...
use haneul_rpc_api::proto::rpc::v2alpha::{GetBalanceRequest, ListBalancesRequest};
use haneul_rpc_api::proto::rpc::v2beta::{ExecutedTransaction, GasCostSummary};
...
const INITIAL_HANEUL_BALANCE: u64 = 150000000000000000;
...
async fn test_balance_apis() {
    ...
    verify_balances(
        &mut grpc_client,
        address,
        &[(HANEUL_COIN_TYPE, INITIAL_HANEUL_BALANCE)],
    )
    .await;
}
...
async fn test_balance_changes_on_transfer() {
    ...
    // Transfer some HANEUL
    let transfer_amount = 1000000;
    let txn = haneul_test_transaction_builder::make_transfer_haneul_transaction(
        &test_cluster.wallet,
        Some(receiver),
    ...
    verify_balances(
        ...
        &[(
            HANEUL_COIN_TYPE,
            INITIAL_HANEUL_BALANCE - transfer_amount - gas_used,
        )],
    )
    .await;
    ...
    verify_balances(
        &mut grpc_client,
        receiver,
        &[(HANEUL_COIN_TYPE, INITIAL_HANEUL_BALANCE + transfer_amount)],
    )
    .await;
}
...
async fn test_custom_coin_balance() {
    ...
    let package_id = transaction
        ...
        .iter()
        .find_map(|o| {
            use haneul_rpc_api::proto::rpc::v2beta::changed_object::OutputObjectState;
    ...
    verify_balances(
        ...
        &[
            (
                HANEUL_COIN_TYPE,
                INITIAL_HANEUL_BALANCE - publish_gas_used - mint_gas_used,
            ),
    ...
    let haneul_client = test_cluster.haneul_client();
    ...
    verify_balances(
        ...
        &[
            (
                HANEUL_COIN_TYPE,
                INITIAL_HANEUL_BALANCE - publish_gas_used - mint_gas_used - transfer_gas_used,
            ),
...
async fn test_multiple_concurrent_balance_changes() {
    ...
    let future_0 = crate::execute_transaction(&mut channel_0, &signed_tx_0);
    let future_1 = crate::execute_transaction(&mut channel_1, &signed_tx_1);
    let future_2 = crate::execute_transaction(&mut channel_2, &signed_tx_2);
    ...
    verify_balances(
        ...
        &[(
            HANEUL_COIN_TYPE,
            INITIAL_HANEUL_BALANCE - transfer_0_to_1 - gas_used_0,
        )],
    )
    .await;
    ...
    verify_balances(
        ...
        &[(
            HANEUL_COIN_TYPE,
            INITIAL_HANEUL_BALANCE + transfer_0_to_1 - transfer_1_to_2 + transfer_2_to_1 - gas_used_1,
        )],
    )
    .await;
    ...
    verify_balances(
        ...
        &[(
            HANEUL_COIN_TYPE,
            INITIAL_HANEUL_BALANCE + transfer_1_to_2 - transfer_2_to_1 - gas_used_2,
        )],
    )
    .await;
}
...
async fn test_fresh_address_with_no_coins() {
    ...
    let fresh_address = HaneulAddress::random_for_testing_only();

    // Get balance for HANEUL
    let response = grpc_client
        .get_balance(GetBalanceRequest {
            owner: Some(fresh_address.to_string()),
            coin_type: Some(HANEUL_COIN_TYPE.to_string()),
        })
        .await
        .unwrap()
        .into_inner();
...
async fn execute_transaction(
    test_cluster: &test_cluster::TestCluster,
    txn: &haneul_types::transaction::Transaction,
...
/// Build a transaction that splits and transfers HANEUL
async fn build_split_and_transfer_transaction(
    ...
    let coins = haneul_client
        .coin_read_api()
        .get_coins(sender, Some(HANEUL_COIN_TYPE.to_string()), None, None)
        .await
        .unwrap();
...
async fn split_and_transfer_coin(
    ...
    gas_object: (
        ...
        haneul_types::base_types::ObjectDigest,
    ),
    coin: (
        haneul_types::base_types::ObjectID,
        haneul_types::base_types::SequenceNumber,
