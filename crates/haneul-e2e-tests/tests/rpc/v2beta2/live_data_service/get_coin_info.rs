...
use haneul_rpc::proto::haneul::rpc::v2beta2::coin_metadata::MetadataCapState;
use haneul_rpc::proto::haneul::rpc::v2beta2::coin_treasury::SupplyState;
...
use haneul_rpc::proto::haneul::rpc::v2beta2::regulated_coin_metadata::CoinRegulatedState;
...
use haneul_types::transaction::{ObjectArg, SharedObjectMutability, TransactionData};
...
async fn get_coin_info_haneul() {
    ...
    assert_eq!(metadata.symbol, Some("HANEUL".to_owned()));
    assert_eq!(metadata.name, Some("Haneul".to_string()));
    ...
    assert_eq!(
        ...
        "HANEUL should have Fixed supply state"
    );

    // HANEUL is not in CoinRegistry, so regulation state is Unknown
...
async fn test_legacy_coin_from_registry() {
    ...
    let treasury_cap_arg = ptb
        .obj(ObjectArg::ImmOrOwnedObject(updated_treasury_cap_ref))
        .unwrap();
    let zero_address_arg = ptb.pure(HaneulAddress::ZERO).unwrap();
...
async fn finalize_registration(
    ...
    let registry_obj_response = ledger_client
        .get_object({
            ...
            request.object_id = Some(HANEUL_COIN_REGISTRY_OBJECT_ID.to_string());
    ...
    let registry_owned = grpc_client
        .list_owned_objects({
            ...
            request.owner = Some(HaneulAddress::from(HANEUL_COIN_REGISTRY_OBJECT_ID).to_string());
        ...
        .find(|obj| {
            ...
            obj_type
                .and_then(|t| {
                    ...
                    match haneul_types::parse_haneul_struct_tag(t) {
                        Ok(struct_tag) => {
    ...
    let mut client = haneul_rpc::client::Client::new(test_cluster.rpc_url().to_owned()).unwrap();
