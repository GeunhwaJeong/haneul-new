...
use haneul_types::{HANEUL_COIN_REGISTRY_OBJECT_ID, HANEUL_FRAMEWORK_PACKAGE_ID};
...
async fn test_create_coin_registry_object() {
    ...
    let framework = haneul_framework_snapshot::load_bytecode_snapshot(95)
        .unwrap()
        .into_iter()
        ...
        h.with(|node| {
            assert!(
                node.state()
                    ...
                    .get_latest_object_ref_or_tombstone(HANEUL_COIN_REGISTRY_OBJECT_ID)
                    .is_none()
            );
        });
    }
