...
async fn publish_test_package(test_cluster: &TestCluster) -> ObjectID {
    ...
    let txn = test_cluster
        .wallet
        .sign_transaction(
            &haneul_test_transaction_builder::TestTransactionBuilder::new(sender, gas_object, 1000)
...
fn get_event_stream_head_object_id(
    ...
    let field_id = DynamicFieldKey(HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, key, key_type_tag)
        .into_unbounded_id()
...
async fn build_epoch_cache(
    ...
    for epoch in 0..current_epoch {
        ...
        let checkpoint: haneul_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
            .try_into()
...
async fn fetch_and_verify_event_stream_head(
    ...
    let checkpoint_data: haneul_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
        .try_into()
...
async fn authenticated_events_backfill_test() {
    ...
    let rpc_config_without_indexing = haneul_config::RpcConfig {
...
async fn authenticated_events_multiple_commits_per_checkpoint() {
    use haneul_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
    use haneul_types::transaction::{Argument, Command};
