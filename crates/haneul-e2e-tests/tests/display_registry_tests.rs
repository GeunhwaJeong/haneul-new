...
use haneul_types::{HANEUL_DISPLAY_REGISTRY_OBJECT_ID, HANEUL_FRAMEWORK_PACKAGE_ID};
...
async fn test_create_display_registry_object() {
    let _guard =
        haneul_protocol_config::ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
    ...
    let framework = haneul_framework_snapshot::load_bytecode_snapshot(115)
        .unwrap()
        .into_iter()
    ...
    let package = framework
        .iter()
        .find(|f| f.id() == HANEUL_FRAMEWORK_PACKAGE_ID)
        .unwrap()
        .data
        .try_as_package()
        .unwrap();
        ...
        h.with(|node| {
            assert!(
                node.state()
                    ...
                    .get_latest_object_ref_or_tombstone(HANEUL_DISPLAY_REGISTRY_OBJECT_ID)
                    .is_none()
            );
        });
    }
