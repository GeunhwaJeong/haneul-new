...
```cargo run --features "pg_backend" --bin haneul-graphql-rpc start-server --db-url <DB_URL>```

The order is important:
1. --features "pg_backend": This part tells Cargo to enable the pg_backend feature.
2. --bin haneul-graphql-rpc: This specifies which binary to run.
...
Rpc 1.5 backs the graphql schema with a db based on IndexerV2 schema. To spin up a local db, follow the instructions at [haneul-indexer](../haneul-indexer/README.md) until "Running standalone indexer".
...
```
cargo run --bin haneul-graphql-rpc start-server [--rpc-url] [--db-url] [--port] [--host] [--config]
```

This will build haneul-graphql-rpc and start an IDE:
