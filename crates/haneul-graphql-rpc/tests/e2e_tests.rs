...
mod tests {
    ...
    use haneul_graphql_rpc::config::Limits;
    use haneul_graphql_rpc::config::ServiceConfig;
    use haneul_graphql_rpc::test_infra::cluster::start_cluster;
    ...
    async fn test_timeout() {
        ...
        cluster
            .wait_for_checkpoint_catchup(1, Duration::from_secs(10))
            .await;
        // timeout test includes mutation timeout, which requies a [HaneulClient] to be able to run
    ...
    async fn test_health_check() {
        let _guard = telemetry_subscribers::TelemetryConfig::new()
            ...
            .init();
        let connection_config = ConnectionConfig::ci_integration_test_cfg();
        let cluster = haneul_graphql_rpc::test_infra::cluster::start_cluster(
            connection_config,
            None,
            ServiceConfig::test_defaults(),
        )
