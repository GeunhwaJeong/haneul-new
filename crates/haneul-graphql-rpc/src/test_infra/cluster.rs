...
use haneullabs_metrics::init_metrics;
use std::env;
...
pub async fn simulator_commands_test_impl() {
    ...
    let (output, adapter) = move_transactional_test_runner::framework::handle_actual_output::<
        haneul_transactional_test_runner::test_adapter::HaneulTestAdapter,
    >(
        &test_file_path,
        Some(&*haneul_transactional_test_runner::test_adapter::PRE_COMPILED),
    )
    .await
    .unwrap();
