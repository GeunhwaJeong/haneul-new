...
pub(crate) static VERSION_HEADER: HeaderName = HeaderName::from_static("x-haneul-rpc-version");

pub(crate) struct HaneulRpcVersion(Vec<u8>, Vec<Vec<u8>>);

impl headers::Header for HaneulRpcVersion {
        ...
        I: Iterator<Item = &'i HeaderValue>,
    {
        ...
        Ok(HaneulRpcVersion(value, values.collect()))
    }
...
pub(crate) async fn check_version_middleware<B>(
    user_version: Option<TypedHeader<HaneulRpcVersion>>,
    State(version): State<Version>,
    request: Request<B>,
    next: Next<B>,
) -> Response {
    if let Some(TypedHeader(HaneulRpcVersion(req_version, rest))) = user_version {
        if !rest.is_empty() {
            return (
...
mod tests {
    ...
    fn metrics() -> Metrics {
        ...
        let registry = haneullabs_metrics::start_prometheus_server(binding_address).default_registry();
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-haneul-rpc-version: Multiple possible versions found.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-haneul-rpc-version: 'not-a-version' not a valid <YEAR>.<MONTH> version.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-haneul-rpc-version: Not a UTF8 string.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
