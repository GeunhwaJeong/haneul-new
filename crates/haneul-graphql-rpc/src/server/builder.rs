...
pub mod tests {
    ...
    async fn test_timeout() {
        ...
        cluster
            ...
            .await;
        // timeout test includes mutation timeout, which requires a [HaneulClient] to be able to run
        ...
        test_timeout(delay, timeout, query, &haneul_client, db_url.clone())
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &haneul_client, db_url.clone())
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
