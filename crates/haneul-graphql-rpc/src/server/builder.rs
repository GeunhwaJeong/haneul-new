...
use haneullabs_network::callback::{CallbackLayer, MakeCallbackHandler, ResponseHandler};
use std::convert::Infallible;
use std::net::TcpStream;
use std::sync::Arc;
use std::time::Duration;
use std::{any::Any, net::SocketAddr, time::Instant};
use haneul_graphql_rpc_headers::{LIMITS_HEADER, VERSION_HEADER};
...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let registry_service = haneullabs_metrics::start_prometheus_server(prom_addr);
        ...
        let haneul_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                HaneulClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create HaneulClient: {}", e)))?,
            )
        } else {
        ...
        builder = builder
            ...
            .context_data(haneul_sdk_client)
...
async fn graphql_handler(
    ...
    schema: Extension<HaneulGraphQLSchema>,
...
pub mod tests {
    ...
    use std::sync::Arc;
    use std::time::Duration;
    use haneul_sdk::{wallet_context::WalletContext, HaneulClient};
    ...
    pub async fn test_timeout_impl(wallet: WalletContext) {
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(None, Some(cfg))
                .context_data(Some(haneul_client.clone()))
                .extension(Timeout)
        ...
        let haneul_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &haneul_client)
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &haneul_client)
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
