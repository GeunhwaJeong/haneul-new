...
use haneul_json_rpc_types::{
    OwnedObjectRef, HaneulExecutionStatus, HaneulGasData, HaneulObjectDataOptions, HaneulObjectResponseQuery,
    HaneulPastObjectResponse, HaneulRawData, HaneulTransactionBlockDataAPI, HaneulTransactionBlockEffectsAPI,
    HaneulTransactionBlockResponseOptions,
};
use haneul_sdk::types::haneul_system_state::haneul_system_state_summary::HaneulValidatorSummary;
use haneul_sdk::{
    types::{
...
pub(crate) async fn convert_to_gas_input(
    ...
    let obj_responses = cl
        .read_api()
        .multi_get_object_with_options(payment_obj_ids, HaneulObjectDataOptions::full_content())
        .await?;
    ...
    Ok(GasInput {
        gas_sponsor: Some(Address::from(HaneulAddress::from(gas_data.owner))),
...
pub(crate) async fn convert_to_epoch(cl: &HaneulClient, gcs: &NativeGasCostSummary) -> Result<Epoch> {
