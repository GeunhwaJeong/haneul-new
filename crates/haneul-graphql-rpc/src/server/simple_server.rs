...
use crate::context_data::haneul_sdk_data_provider::{lru_cache_data_loader, haneul_sdk_client_v0};
...
impl std::default::Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            rpc_url: "https://fullnode.testnet.haneul.io:443/".to_string(),
...
pub async fn start_example_server(config: Option<ServerConfig>) {
    ...
    let haneul_sdk_client_v0 = haneul_sdk_client_v0(&config.rpc_url).await;
    let data_provider: Box<dyn DataProvider> = Box::new(haneul_sdk_client_v0.clone());
    let data_loader = lru_cache_data_loader(&haneul_sdk_client_v0).await;
