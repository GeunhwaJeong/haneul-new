...
impl Object {
    ...
    async fn as_move_object(&self) -> Result<Option<MoveObject>> {
        ...
        let native_object: NativeHaneulObject = bcs::from_bytes(&bcs.0[..]).map_err(|e| {
            graphql_error(
        ...
        Ok(
            if matches!(native_object.data, NativeHaneulObjectData::Move(_)) {
    ...
    async fn as_move_package(&self) -> Result<Option<MovePackage>> {
        ...
        let native_object: NativeHaneulObject = bcs::from_bytes(&bcs.0[..]).map_err(|_| {
            graphql_error(
                code::INTERNAL_SERVER_ERROR,
        ...
        Ok(
            if matches!(native_object.data, NativeHaneulObjectData::Package(_)) {
    ...
    /// The `0x2::haneul::Coin` objects owned by the given object.
