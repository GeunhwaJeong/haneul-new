...
use super::{owner::Owner, haneul_address::HaneulAddress, transaction_block::TransactionBlock};
...
use haneul_indexer::schema_v2::{checkpoints, objects, objects_history, objects_snapshot};
...
pub(crate) enum IObject {
    ...
    StakedHaneul(StakedHaneul),
    HaneulnsRegistration(HaneulnsRegistration),
}

/// An object in Haneul is a package (set of Move bytecode modules) or object (typed data structure
...
impl Object {
    ...
    /// The `0x3::staking_pool::StakedHaneul` objects owned by this object.
    ...
    /// The amount of HANEUL we would rebate if this object gets deleted or mutated. This number is
    ...
    pub(crate) async fn owner(&self, ctx: &Context<'_>) -> Option<ObjectOwner> {
        ...
        match native.owner {
            O::AddressOwner(address) => {
                let address = HaneulAddress::from(address);
...
impl Object {
    ...
    pub(crate) fn from_native(address: HaneulAddress, native: NativeObject) -> Object {
        Object {
            address,
    ...
    async fn query_live(db: &Db, address: HaneulAddress) -> Result<Option<Self>, Error> {
...
impl ObjectFilter {
    ...
    /// it).
    fn keys(&self) -> Option<BTreeMap<HaneulAddress, Option<u64>>> {
...
/// Parse a `HaneulAddress` from its stored representation.  Failure is an internal error: the
/// database should never contain a malformed address (containing the wrong number of bytes).
fn addr(bytes: impl AsRef<[u8]>) -> Result<HaneulAddress, Error> {
    HaneulAddress::from_bytes(bytes.as_ref()).map_err(|e| {
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(HaneulAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(HaneulAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_key_filter_intersection() {
        let i1 = HaneulAddress::from_str("0x1").unwrap();
        let i2 = HaneulAddress::from_str("0x2").unwrap();
        let i3 = HaneulAddress::from_str("0x3").unwrap();
        let i4 = HaneulAddress::from_str("0x4").unwrap();
