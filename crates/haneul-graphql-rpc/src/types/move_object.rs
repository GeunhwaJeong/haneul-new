...
impl MoveObject {
    ...
    async fn as_stake(&self, ctx: &Context<'_>) -> Result<Option<Stake>> {
        ...
        if !move_object.is_staked_haneul() {
            return Ok(None);
        }

        let stake: StakedHaneul = bcs::from_bytes(move_object.contents())
            .map_err(|e| Error::Internal(format!("Failed to deserialized Staked Haneul: {e}")))?;
