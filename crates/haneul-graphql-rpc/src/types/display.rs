...
use haneul_indexer::{models_v2::display::StoredDisplay, schema_v2::display};
...
pub(crate) fn get_value_from_move_struct(
    ...
    // todo: 10 is a carry-over from the haneul-json-rpc implementation
    ...
    // TODO: move off dependency on HaneulMoveValue
    let haneul_move_value: HaneulMoveValue = result.clone().into();
    ...
    match haneul_move_value {
        ...
        HaneulMoveValue::Vector(_) => Err(DisplayRenderError::Vector(var_name.to_string())),
        _ => Ok(haneul_move_value.to_string()),
    }
}
