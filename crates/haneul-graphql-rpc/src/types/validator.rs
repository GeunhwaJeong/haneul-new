...
use haneul_indexer::apis::GovernanceReadApi;
...
use async_graphql::*;
use haneul_indexer::apis::governance_api::exchange_rates;
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativeHaneulAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
    ...
    > {
        ...
        let exchange_rates = exchange_rates(&governance_api, &latest_haneul_system_state)
            .await
        ...
        let epoch_to_filter_out = if let Some(epoch) = keys.first() {
            if epoch == &latest_haneul_system_state.epoch {
                *epoch - 1
            } else {
                *epoch
            }
        } else {
            latest_haneul_system_state.epoch - 1
        };
        ...
        let requested_epoch = match keys.first() {
            Some(x) => *x,
            None => latest_haneul_system_state.epoch,
        };

        let mut r = HashMap::new();
        r.insert(
            requested_epoch,
            (latest_haneul_system_state.stake_subsidy_start_epoch, results),
        );
...
impl Validator {
    ...
    async fn staking_pool_haneul_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_haneul_balance,
        ))
    }
    ...
    async fn pending_total_haneul_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_haneul_withdraw,
        ))
    }
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        ...
        let rates = exchange_rates
            .get(&self.validator_summary.haneul_address)
            .ok_or_else(|| {
                Error::Internal(format!(
                    "Failed to get the exchange rate for this validator address {} for requested epoch {}",
                    self.validator_summary.haneul_address, self.requested_for_epoch
                ))
            })?;
...
impl Validator {
    pub fn operation_cap_id(&self) -> HaneulAddress {
        HaneulAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
