...
fn select_tx(sender: Option<HaneulAddress>, bound: TxBounds, from: &str) -> RawQuery {
...
fn select_pkg(pkg: &HaneulAddress, sender: Option<HaneulAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_sender(sender: &HaneulAddress, bound: TxBounds) -> RawQuery {
    select_tx(Some(*sender), bound, "tx_senders")
}

fn select_recipient(recv: &HaneulAddress, sender: Option<HaneulAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_input(input: &HaneulAddress, sender: Option<HaneulAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_changed(changed: &HaneulAddress, sender: Option<HaneulAddress>, bound: TxBounds) -> RawQuery {
    filter!(
