...
impl HaneulnsRegistration {
    ...
    /// transaction command or `haneul::transfer::public_transfer`, both of which require the object to
...
impl NameService {
    ...
    async fn query_domain_expiration(
        ...
        // parent's `NameRecord`.
        let mut object_ids = vec![HaneulAddress::from(config.record_field_id(&domain.0))];
        if domain.0.is_subdomain() {
            object_ids.push(HaneulAddress::from(config.record_field_id(&domain.0.parent())));
        }
