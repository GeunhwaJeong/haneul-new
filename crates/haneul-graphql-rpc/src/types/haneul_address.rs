// Copyright (c) Mysten Labs, Inc.
// SPDX-License-Identifier: Apache-2.0

use std::str::FromStr;

use async_graphql::*;
use move_core_types::account_address::AccountAddress;
use serde::{Deserialize, Serialize};
use thiserror::Error;

const HANEUL_ADDRESS_LENGTH: usize = 32;

#[derive(Serialize, Deserialize, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Copy)]
pub(crate) struct HaneulAddress([u8; HANEUL_ADDRESS_LENGTH]);

#[derive(Error, Debug, Eq, PartialEq)]
pub(crate) enum FromStrError {
    #[error("Invalid HaneulAddress. Missing 0x prefix.")]
    NoPrefix,

    #[error(
        "Expected HaneulAddress string with between 1 and {} digits ({} bytes), received {0}",
        HANEUL_ADDRESS_LENGTH * 2,
        HANEUL_ADDRESS_LENGTH,
    )]
    WrongLength(usize),

    #[error("Invalid character {0:?} at position {1}")]
    BadHex(char, usize),
}

#[derive(Error, Debug, Eq, PartialEq)]
pub(crate) enum FromVecError {
    #[error("Expected HaneulAddress with {} bytes, received {0}", HANEUL_ADDRESS_LENGTH)]
    WrongLength(usize),
}

#[Scalar]
impl ScalarType for HaneulAddress {
    fn parse(value: Value) -> InputValueResult<Self> {
        let Value::String(s) = value else {
            return Err(InputValueError::expected_type(value));
        };

        Ok(HaneulAddress::from_str(&s)?)
    }

    fn to_value(&self) -> Value {
        Value::String(format!("0x{}", hex::encode(self.0)))
    }
}

impl HaneulAddress {
    #[allow(dead_code)]
    pub fn into_array(self) -> [u8; HANEUL_ADDRESS_LENGTH] {
        self.0
    }

    pub fn from_array(arr: [u8; HANEUL_ADDRESS_LENGTH]) -> Self {
        HaneulAddress(arr)
    }

    pub fn into_vec(self) -> Vec<u8> {
        self.0.to_vec()
    }

    pub fn as_slice(&self) -> &[u8] {
        &self.0
    }

    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, FromVecError> {
        <[u8; HANEUL_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| FromVecError::WrongLength(bytes.as_ref().len()))
            .map(HaneulAddress)
    }
}

impl TryFrom<Vec<u8>> for HaneulAddress {
    type Error = FromVecError;

    fn try_from(bytes: Vec<u8>) -> Result<Self, FromVecError> {
        Self::from_bytes(bytes)
    }
}

impl From<AccountAddress> for HaneulAddress {
    fn from(value: AccountAddress) -> Self {
        HaneulAddress(value.into_bytes())
    }
}

impl FromStr for HaneulAddress {
    type Err = FromStrError;

    fn from_str(s: &str) -> Result<Self, FromStrError> {
        let Some(s) = s.strip_prefix("0x") else {
            return Err(FromStrError::NoPrefix);
        };

        if s.is_empty() || s.len() > HANEUL_ADDRESS_LENGTH * 2 {
            return Err(FromStrError::WrongLength(s.len()));
        }

        let mut arr = [0u8; HANEUL_ADDRESS_LENGTH];
        hex::decode_to_slice(
            // Left pad with `0`-s up to HANEUL_ADDRESS_LENGTH * 2 characters long.
            format!("{:0>width$}", s, width = HANEUL_ADDRESS_LENGTH * 2),
            &mut arr[..],
        )
        .map_err(|e| match e {
            hex::FromHexError::InvalidHexCharacter { c, index } => {
                FromStrError::BadHex(c, index + 2)
            }
            hex::FromHexError::OddLength => unreachable!("SAFETY: Prevented by padding"),
            hex::FromHexError::InvalidStringLength => {
                unreachable!("SAFETY: Prevented by bounds check")
            }
        })?;

        Ok(HaneulAddress(arr))
    }
}

impl std::fmt::Display for HaneulAddress {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&format!("0x{}", hex::encode(self.0)))
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use async_graphql::Value;

    const STR_ADDRESS: &str = "0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    const ARR_ADDRESS: [u8; HANEUL_ADDRESS_LENGTH] = [
        1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103,
        137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239,
    ];
    const HANEUL_ADDRESS: HaneulAddress = HaneulAddress(ARR_ADDRESS);

    #[test]
    fn test_parse_valid_haneuladdress() {
        let parsed = HaneulAddress::from_str(STR_ADDRESS).unwrap();
        assert_eq!(parsed.0, ARR_ADDRESS);
    }

    #[test]
    fn test_to_value() {
        let value = ScalarType::to_value(&HANEUL_ADDRESS);
        assert_eq!(value, Value::String(STR_ADDRESS.to_string()));
    }

    #[test]
    fn test_into_array() {
        let arr = HANEUL_ADDRESS.into_array();
        assert_eq!(arr, ARR_ADDRESS);
    }

    #[test]
    fn test_from_array() {
        let addr = HaneulAddress::from_array(ARR_ADDRESS);
        assert_eq!(addr, HANEUL_ADDRESS);
    }

    #[test]
    fn test_as_slice() {
        assert_eq!(HANEUL_ADDRESS.as_slice(), &ARR_ADDRESS);
    }

    #[test]
    fn test_round_trip() {
        let value = ScalarType::to_value(&HANEUL_ADDRESS);
        let parsed_back = ScalarType::parse(value).unwrap();
        assert_eq!(HANEUL_ADDRESS, parsed_back);
    }

    #[test]
    fn test_parse_no_prefix() {
        let err = HaneulAddress::from_str(&STR_ADDRESS[2..]).unwrap_err();
        assert_eq!(FromStrError::NoPrefix, err);
    }

    #[test]
    fn test_parse_invalid_prefix() {
        let input = "1x".to_string() + &STR_ADDRESS[2..];
        let err = HaneulAddress::from_str(&input).unwrap_err();
        assert_eq!(FromStrError::NoPrefix, err)
    }

    #[test]
    fn test_parse_invalid_length() {
        let input = STR_ADDRESS.to_string() + "0123";
        let err = HaneulAddress::from_str(&input).unwrap_err();
        assert_eq!(FromStrError::WrongLength(68), err)
    }

    #[test]
    fn test_parse_invalid_characters() {
        let input = "0xg".to_string() + &STR_ADDRESS[3..];
        let err = HaneulAddress::from_str(&input).unwrap_err();
        assert_eq!(FromStrError::BadHex('g', 2), err);
    }

    #[test]
    fn test_unicode_gibberish() {
        let parsed = HaneulAddress::from_str("aAௗ0㌀0");
        assert!(parsed.is_err());
    }

    #[test]
    fn bad_scalar_type() {
        let input = Value::Number(0x42.into());
        let parsed = <HaneulAddress as ScalarType>::parse(input);
        assert!(parsed.is_err());
    }
}
