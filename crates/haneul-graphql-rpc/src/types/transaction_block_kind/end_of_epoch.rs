...
use haneul_types::digests::ChainIdentifier as HaneulChainIdentifier;
use haneul_types::{
...
pub(crate) struct BridgeStateCreateTransaction {
    pub native: HaneulChainIdentifier,
    /// The checkpoint sequence number this was viewed at.
    pub checkpoint_viewed_at: u64,
}
...
#[derive(SimpleObject, Clone, PartialEq, Eq)]
pub(crate) struct WriteAccumulatorStorageCostTransaction {
    /// A workaround to define an empty variant of a GraphQL union.
    #[graphql(name = "_")]
    dummy: Option<bool>,
}
...
impl ChangeEpochTransaction {
    ...
    async fn system_packages(
        ...
        for c in cs {
            ...
            let object = Object::from_native(HaneulAddress::from(runtime_id), native, c.c, None);
