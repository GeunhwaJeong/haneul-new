...
struct PublishTransaction {
    ...
    dependencies: Vec<HaneulAddress>,
}
...
impl MoveCallTransaction {
    ...
    async fn package(&self) -> HaneulAddress {
    ...
    fn from(pt: NativeProgrammableTransaction, checkpoint_viewed_at: u64) -> Self {
        ...
        match pt {
            ...
            N::Publish(modules, dependencies) => P::Publish(PublishTransaction {
                ...
                dependencies: dependencies.into_iter().map(HaneulAddress::from).collect(),
            }),
...
impl From<HaneulArgument> for TransactionArgument {
    fn from(argument: HaneulArgument) -> Self {
        use HaneulArgument as S;
        use TransactionArgument as A;
        match argument {
