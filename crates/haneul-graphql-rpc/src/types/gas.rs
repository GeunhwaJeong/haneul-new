...
use super::{address::Address, big_int::BigInt, object::Object, haneul_address::HaneulAddress};
...
impl GasInput {
    ...
    pub(crate) fn from(s: &GasData, checkpoint_viewed_at: u64) -> Self {
        let payment_obj_keys = match s.owner {
            NativeHaneulAddress::ZERO => vec![], // system transactions do not have payment objects
            _ => s
                .payment
                .iter()
                .map(|o| ObjectKey {
