...
use haneul_indexer::types_v2::IndexedObjectChange;
use haneul_json_rpc_types::{
    BalanceChange as NativeBalanceChange, HaneulExecutionStatus, HaneulTransactionBlockEffects,
...
impl TransactionBlockEffects {
    pub fn from_stored_transaction(
        ...
        tx_effects: &HaneulTransactionBlockEffects,
...
pub(crate) struct TransactionBlockFilter {
    ...
    pub sign_address: Option<HaneulAddress>,
    pub sent_address: Option<HaneulAddress>,
    pub recv_address: Option<HaneulAddress>,
    pub paid_address: Option<HaneulAddress>,
...
impl BalanceChange {
    fn from(balance_changes: Vec<Option<Vec<u8>>>) -> Result<Vec<Option<BalanceChange>>, Error> {
        ...
        for balance_change_bcs in balance_changes.into_iter().flatten() {
            ...
            let address =
                HaneulAddress::from_bytes(balance_change_owner_address.to_vec()).map_err(|_| {
                    Error::Internal(
                        "Cannot get a HaneulAddress from the balance change owner address".to_string(),
                    )
                })?;
    ...
    async fn from(
        ...
        match object_change {
            IndexedObjectChange::Created {
                ...
                let haneul_address = HaneulAddress::from_bytes(object_id.into_bytes()).map_err(|_| {
                    Error::Internal("Cannot decode a HaneulAddress from object_id".to_string())
                })?;
                let output_state = ctx
                    .data_unchecked::<PgManager>()
                    .fetch_obj(haneul_address, Some(version.value()))
                    .await?;
                Ok(Some(Self {
                    input_state: None,
                    output_state,
            ...
            IndexedObjectChange::Published {
                ...
                let haneul_address =
                    HaneulAddress::from_bytes(package_id.into_bytes()).map_err(|_| {
                        Error::Internal("Cannot decode a HaneulAddress from package_id".to_string())
                    })?;
                let output_state = ctx
                    .data_unchecked::<PgManager>()
                    .fetch_obj(haneul_address, Some(version.value()))
            ...
            IndexedObjectChange::Mutated {
                ...
                let input_state = ctx
                    .data_unchecked::<PgManager>()
                    .fetch_obj(haneul_address, Some(previous_version.value()))
                    .await?;
                let output_state = ctx
                    .data_unchecked::<PgManager>()
                    .fetch_obj(haneul_address, Some(version.value()))
