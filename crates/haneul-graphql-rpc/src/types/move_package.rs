...
use crate::types::haneul_address::addr;
...
use haneul_indexer::models::objects::StoredFullHistoryObject;
use haneul_indexer::schema::packages;
use haneul_package_resolver::{error::Error as PackageCacheError, Package as ParsedMovePackage};
use haneul_types::is_system_package;
use haneul_types::{move_package::MovePackage as NativeMovePackage, object::Data};
...
impl MovePackage {
    ...
    /// The `0x3::staking_pool::StakedHaneul` objects owned by this package.
    ///
    /// Note that objects owned by a package are inaccessible, because packages are immutable and
    /// cannot be owned by an address.
...
impl MovePackage {
    ...
    pub(crate) fn try_from_serialized(
        ...
        let object = Object::new_serialized(
            HaneulAddress::from_bytes(&history_object.object_id)
...
impl Loader<PackageVersionKey> for Db {
    type Value = HaneulAddress;
    type Error = Error;

    async fn load(
        &self,
        keys: &[PackageVersionKey],
    ) -> Result<HashMap<PackageVersionKey, HaneulAddress>, Error> {
        use packages::dsl;
...
impl Loader<LatestKey> for Db {
    ...
    async fn load(&self, keys: &[LatestKey]) -> Result<HashMap<LatestKey, HaneulAddress>, Error> {
        use packages::dsl;
        let other = diesel::alias!(packages as other);
