...
use super::{
    ...
    move_value::MoveValue, haneul_address::HaneulAddress,
};
...
use haneul_types::{
    base_types::HaneulAddress as NativeHaneulAddress, event::Event as NativeEvent, parse_haneul_struct_tag,
};
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeHaneulAddress::ZERO {
            return Ok(None);
        }
...
impl TryFrom<StoredEvent> for Event {
    ...
    fn try_from(stored: StoredEvent) -> Result<Self, Self::Error> {
        ...
        let sender = NativeHaneulAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;

        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_haneul_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
