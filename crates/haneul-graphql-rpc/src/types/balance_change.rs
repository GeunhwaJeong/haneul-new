...
use async_graphql::*;
use haneul_json_rpc_types::BalanceChange as StoredBalanceChange;
use haneul_types::object::Owner as NativeOwner;

use super::{big_int::BigInt, move_type::MoveType, owner::Owner, haneul_address::HaneulAddress};
    ...
    async fn owner(&self) -> Option<Owner> {
        ...
        match self.stored.owner {
            O::AddressOwner(addr) | O::ObjectOwner(addr) => Some(Owner {
                address: HaneulAddress::from(addr),
