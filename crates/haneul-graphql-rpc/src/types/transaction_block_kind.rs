...
use super::{big_int::BigInt, date_time::DateTime, epoch::Epoch, haneul_address::HaneulAddress};
use crate::context_data::db_data_provider::PgManager;
use async_graphql::*;
use haneul_types::transaction::{GenesisObject, TransactionKind as NativeTransactionKind};
...
pub(crate) struct GenesisTransaction {
    pub(crate) objects: Option<Vec<HaneulAddress>>,
}
    ...
    fn from(kind: NativeTransactionKind) -> Self {
        ...
        match kind {
            ...
            K::Genesis(g) => T::Genesis(GenesisTransaction {
                objects: Some(g.objects.iter().cloned().map(HaneulAddress::from).collect()),
            }),
