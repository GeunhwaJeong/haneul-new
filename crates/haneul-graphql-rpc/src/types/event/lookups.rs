...
fn select_ev(sender: Option<HaneulAddress>, from: &str) -> RawQuery {
...
pub(crate) fn select_sender(sender: HaneulAddress) -> RawQuery {
    select_ev(Some(sender), "event_senders")
}

pub(crate) fn select_event_type(event_type: &TypeFilter, sender: Option<HaneulAddress>) -> RawQuery {
    match event_type {
