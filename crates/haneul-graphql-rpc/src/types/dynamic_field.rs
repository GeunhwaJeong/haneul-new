...
use haneul_indexer::models::objects::{StoredHistoryObject, StoredObject};
...
pub(crate) struct DynamicField {
    ...
    pub df_object_id: HaneulAddress,
...
impl DynamicField {
    ...
    pub(crate) async fn query_latest_dynamic_field(
        ...
        let object_id = HaneulAddress::from(field_id);
...
impl TryFrom<MoveObject> for DynamicField {
    ...
    fn try_from(stored: MoveObject) -> Result<Self, Error> {
        ...
        let df_object_id = HaneulAddress::from_bytes(df_object_id).map_err(|e| {
...
fn apply_filter(query: RawQuery, parent: HaneulAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
