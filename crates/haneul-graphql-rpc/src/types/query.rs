...
pub(crate) type HaneulGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    /// Look up an Owner by its HaneulAddress.
    ///
    ...
    ///
    /// If `rootVersion` is omitted, dynamic fields will be from a consistent snapshot of the Haneul
    ...
    async fn address(&self, ctx: &Context<'_>, address: HaneulAddress) -> Result<Option<Address>> {
        let Watermark { checkpoint, .. } = *ctx.data()?;
