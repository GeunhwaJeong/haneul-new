...
use haneul_package_resolver::Module as ParsedMoveModule;

use super::move_struct::MoveStruct;
use super::{base64::Base64, move_package::MovePackage, haneul_address::HaneulAddress};
...
impl MoveModule {
    ...
    async fn module_id(&self) -> MoveModuleId {
        ...
        MoveModuleId {
            package: HaneulAddress::from(*self_id.address()),
    ...
    async fn friend_connection(
        ...
        let Some(package) = ctx
            ...
            .fetch_move_package(HaneulAddress::from(*self_id.address()), None)
            .await
            .extend()?
        else {
...
impl MoveModule {
    fn struct_impl(&self, name: String) -> Result<Option<MoveStruct>, Error> {
        use haneul_package_resolver::error::Error as E;
