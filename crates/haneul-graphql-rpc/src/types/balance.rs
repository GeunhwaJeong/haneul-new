...
use super::{big_int::BigInt, move_type::MoveType, haneul_address::HaneulAddress};
...
impl TryFrom<StoredBalance> for Balance {
    ...
    fn try_from(s: StoredBalance) -> Result<Self, Error> {
        ...
        let coin_type = MoveType::new(
            parse_haneul_type_tag(&coin_type)
...
fn balance_query(address: HaneulAddress, coin_type: Option<TypeTag>, lhs: i64, rhs: i64) -> RawQuery {
...
/// `RawQuery`.
fn filter(mut query: RawQuery, owner: HaneulAddress, coin_type: Option<TypeTag>) -> RawQuery {
