...
use haneul_indexer::store::IndexerStore;
...
async fn _start_indexer(
    ...
    let indexer_cfg = haneul_indexer::IndexerConfig {
        db_url: Some(db_url),
    ...
    let (store, handle) = haneul_indexer::test_utils::start_test_indexer(indexer_cfg)
        .await
        .unwrap();
...
async fn _wait_until_next_checkpoint(store: &haneul_indexer::store::PgIndexerStore) {
...
async fn _start_test_cluster(
    ...
    let (store, handle) = haneul_indexer::test_utils::start_test_indexer(config)
        .await
        .unwrap();
    if wait_for_sync {
        _wait_until_next_checkpoint(&store).await;
