...
use haneul_graphql_rpc::server::graphiql_server::start_graphiql_server;
...
async fn main() {
    ...
    match cmd {
        Command::GenerateDocsExamples => {
            ...
            buf.pop();
            buf.push(filename);
            let content = haneul_graphql_rpc::examples::generate_examples_for_docs()
        ...
        Command::GenerateExamples { file } => {
            let new_content: String = haneul_graphql_rpc::examples::generate_markdown()
