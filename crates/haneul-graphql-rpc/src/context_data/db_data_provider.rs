...
impl PgManager {
    ...
    /// If no coin type is provided, it will use the default gas coin (HANEUL).
    ...
    pub(crate) async fn fetch_staked_haneul(
        ...
        let obj_filter = ObjectFilter {
            package: None,
            module: None,
            ty: Some(MoveObjectType::staked_haneul().to_canonical_string(/* with_prefix */ true)),
            owner: Some(address),
            object_ids: None,
            object_keys: None,
        };
        ...
        for stored_obj in stored_objs {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {} to be a staked haneul, but it is not an object.",
                    object.address,
                ))
            })?;
    ...
    pub(crate) async fn fetch_events(
        ...
        if let Ok(event_filter) = event_filter {
            ...
            connection.edges.extend(results.into_iter().map(|e| {
                ...
                let event = Event {
                    sending_package: HaneulAddress::from(e.package_id),
    ...
    pub(crate) async fn fetch_dynamic_fields(
        ...
        for stored_obj in stored_objs {
            ...
            let cursor = HaneulAddress::from_bytes(df_object_id)
    ...
    pub(crate) async fn fetch_dynamic_field(
        ...
        if kind == DynamicFieldType::DynamicObject {
            let dynamic_object_field_struct =
                haneul_types::dynamic_field::DynamicFieldInfo::dynamic_object_field_wrapper(type_tag);
        ...
        if let Some(stored_object) = stored_obj {
            ...
            let df_object_id =
                HaneulAddress::from_bytes(df_object_id).map_err(|e| Error::Internal(e.to_string()))?;
