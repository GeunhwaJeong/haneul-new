...
use haneul_types::{base_types::MoveObjectType, governance::StakedHaneul, TypeTag};
use haneul_types::{
...
use super::haneul_sdk_data_provider::convert_to_validators;
...
impl PgManager {
    ...
    pub async fn fetch_native_package(&self, package_id: Vec<u8>) -> Result<HaneulMovePackage, Error> {
        let package = self
...
impl PgManager {
    ...
    pub(crate) fn parse_obj_cursor(&self, cursor: &str) -> Result<Vec<u8>, Error> {
        Ok(HaneulAddress::from_str(cursor)
    ...
    pub(crate) async fn fetch_txs_for_address(
        ...
        address_relation: (HaneulAddress, AddressTransactionBlockRelationship),
    ...
    ) -> Result<Option<HaneulAddress>, Error> {
        let address = address.into_vec();
        let stored_obj = self.get_obj(address, None).await?;

        Ok(stored_obj
            .and_then(|obj| obj.owner_id.map(|id| HaneulAddress::try_from(id).ok()))
            .flatten())
    }
    ...
    pub(crate) async fn fetch_move_obj(
        ...
        let stored_obj = self.get_obj(address, version).await?;
        let haneul_object = stored_obj.map(HaneulObject::try_from).transpose()?;
        let move_object = haneul_object.map(|o| MoveObject { native_object: o });
    ...
    pub(crate) async fn fetch_move_package(
        ...
        let stored_obj = self.get_obj(address, version).await?;
        let haneul_object = stored_obj.map(HaneulObject::try_from).transpose()?;
        let move_package = haneul_object.map(|o| MovePackage { native_object: o });
    ...
    pub(crate) async fn fetch_balance(
        ...
        let address = address.into_vec();
        let coin_type = coin_type.unwrap_or("0x2::haneul::HANEUL".to_string());
    ...
    pub(crate) async fn resolve_name_service_address(
        ...
        Ok(record.target_address.map(|address| Address {
            address: HaneulAddress::from_array(address.to_inner()),
        }))
    }
    ...
    pub(crate) async fn fetch_latest_haneul_system_state(
        &self,
    ) -> Result<HaneulSystemStateSummary, Error> {
        let result = self
            .inner
            .spawn_blocking(|this| this.get_latest_haneul_system_state())
            .await?;
        HaneulSystemStateSummary::try_from(result)
    }
    ...
    pub(crate) async fn fetch_events(
        ...
        let event_filter: Result<RpcEventFilter, Error> = if let Some(sender) = filter.sender {
            let sender = NativeHaneulAddress::from_bytes(sender.into_array())
...
impl TryFrom<StoredObject> for Object {
    ...
    fn try_from(o: StoredObject) -> Result<Self, Self::Error> {
        ...
        let object: HaneulObject = o.try_into()?;
...
impl TryFrom<StoredEpochInfo> for Epoch {
    ...
    fn try_from(e: StoredEpochInfo) -> Result<Self, Self::Error> {
        let validators: Vec<HaneulValidatorSummary> = e
            .validators
            .into_iter()
            .flatten()
            .map(|v| {
...
impl TryFrom<NativeHaneulSystemStateSummary> for HaneulSystemStateSummary {
    type Error = Error;
    fn try_from(system_state: NativeHaneulSystemStateSummary) -> Result<Self, Self::Error> {
...
impl From<GenesisObject> for HaneulAddress {
    fn from(value: GenesisObject) -> Self {
        match value {
            GenesisObject::RawObject { data, owner: _ } => {
                HaneulAddress::from_bytes(data.id().to_vec()).unwrap()
            }
        }
    }
}
...
impl TryFrom<StoredObject> for Coin {
    ...
    fn try_from(o: StoredObject) -> Result<Self, Self::Error> {
        let object_id = HaneulAddress::try_from(o.object_id.clone()).map_err(|_| {
            Error::Internal(format!(
                "Failed to convert object_id {:?} to HaneulAddress",
                o.object_id
            ))
        })?;
        let id = ID(object_id.to_string());
        let balance = o.coin_balance.map(BigInt::from);
        let native_object: HaneulObject = o.try_into()?;
