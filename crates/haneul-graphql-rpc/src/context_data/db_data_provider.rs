...
use haneul_types::{
    ...
    haneul_system_state::haneul_system_state_summary::HaneulSystemStateSummary as NativeHaneulSystemStateSummary,
};
...
impl PgManager {
    ...
    pub(crate) async fn fetch_haneul_system_state(
        ...
        let latest_haneul_system_state = self.inner.get_latest_haneul_system_state().await?;

        if let Some(epoch_id) = epoch_id {
            if epoch_id == latest_haneul_system_state.epoch {
                Ok(latest_haneul_system_state)
            } else {
                Ok(self
                    .inner
                    .get_epoch_haneul_system_state(Some(epoch_id))
                    .await?)
            }
        } else {
            Ok(latest_haneul_system_state)
...
pub(crate) fn convert_to_validators(
    system_state_at_requested_epoch: NativeHaneulSystemStateSummary,
