...
use haneul_json_rpc_types::{
    HaneulExecutionStatus, HaneulObjectDataOptions, HaneulObjectResponseQuery, HaneulPastObjectResponse,
    HaneulRawData, HaneulTransactionBlockDataAPI, HaneulTransactionBlockEffectsAPI,
    HaneulTransactionBlockResponseOptions,
};
use haneul_sdk::types::haneul_serde::BigInt as SerdeBigInt;
use haneul_sdk::types::haneul_system_state::haneul_system_state_summary::HaneulSystemStateSummary;
use haneul_sdk::{
    types::{
        base_types::{ObjectID as NativeObjectID, HaneulAddress as NativeHaneulAddress},
...
pub(crate) struct HaneulClientLoader {
    pub client: HaneulClient,
}

#[async_trait::async_trait]
impl Loader<TransactionDigest> for HaneulClientLoader {
...
impl DataProvider for HaneulClient {
    ...
    async fn fetch_balance(&self, address: &HaneulAddress, type_: Option<String>) -> Result<Balance> {
        let b = self
            .coin_read_api()
    ...
    async fn fetch_balance_connection(
        ...
        let balances = self
            .coin_read_api()
            .get_all_balances(NativeHaneulAddress::from(address))
            .await?;
    ...
    async fn fetch_checkpoint_connection(
        ...
        let system_state = self.governance_api().get_latest_haneul_system_state().await?;
...
pub(crate) async fn lru_cache_data_loader(
    client: &HaneulClient,
) -> DataLoader<HaneulClientLoader, LruCache> {
...
pub(crate) fn convert_json_rpc_checkpoint(
    c: &haneul_json_rpc_types::Checkpoint,
...
pub(crate) fn convert_obj(s: &haneul_json_rpc_types::HaneulObjectData) -> Object {
    Object {
        ...
        bcs: s.bcs.as_ref().map(|raw| match raw {
            HaneulRawData::Package(raw_package) => Base64::from(bcs::to_bytes(raw_package).unwrap()),
            HaneulRawData::MoveObject(raw_object) => Base64::from(&raw_object.bcs_bytes),
        }),
...
async fn convert_to_transaction_block(
    cl: &HaneulClient,
    tx: haneul_json_rpc_types::HaneulTransactionBlockResponse,
    ...
    let system_state = cl.governance_api().get_latest_haneul_system_state().await?;
    ...
    Ok(TransactionBlock {
        ...
        effects: Some(TransactionBlockEffects {
            ...
            status: Some(match tx_effects.status() {
                HaneulExecutionStatus::Success => ExecutionStatus::Success,
                HaneulExecutionStatus::Failure { error: _ } => ExecutionStatus::Failure,
            }),
            errors: match tx_effects.status() {
                HaneulExecutionStatus::Success => None,
                HaneulExecutionStatus::Failure { error } => Some(error.clone()),
            },
        }),
        sender: Some(Address {
