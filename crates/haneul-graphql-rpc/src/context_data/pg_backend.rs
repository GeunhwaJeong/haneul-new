...
use crate::{
    ...
    error::Error,
    types::{
        digest::Digest, event::EventFilter, object::ObjectFilter, haneul_address::HaneulAddress,
...
impl GenericQueryBuilder<Pg> for PgQueryBuilder {
        ...
        if let Some(object_type) = filter.type_ {
            ...
            if parts.len() == 1 {
                ...
                if parts[0].starts_with("0x") {
                    let package = HaneulAddress::from_str(parts[0])
                ...
                let validated_type = parse_haneul_struct_tag(&object_type)
                    .map_err(|e| DbValidationError::InvalidType(e.to_string()))?;
    ...
    fn multi_get_events(
        ...
        if let Some(pm) = filter.emitting_module {
            ...
            let p = HaneulAddress::from_str(parts[0])
                .map_err(|e| DbValidationError::InvalidType(e.to_string()))?;
        ...
        if let Some(event_type) = filter.event_type {
            ...
            match parts.len() {
                ...
                3 => {
                    let validated_type = parse_haneul_struct_tag(&event_type)
                        .map_err(|e| DbValidationError::InvalidType(e.to_string()))?;
