...
const MAINNET_USDHANEUL: &str =
    "0x44f838219cf67b058f3b37907b655f226153c18e33dfcd0da559a844fea9b1c1::usdhaneul::USDHANEUL";
const MAINNET_HANEUL_USDE: &str =
    "0x41d587e5336f1c86cad50d38a7136db99333bb9bda91cea4ba69115defeb1402::haneul_usde::HANEUL_USDE";
...
// Version 3: gas model v2, including all haneul conservation fixes. Fix for loaded child object
...
//            `max_meter_ticks_per_module` limits each from 6_000_000 to 16_000_000. haneul-system
//            framework changes.
...
//             Changes to haneul-system package to add PTB-friendly unstake function, and minor cleanup.
...
//             Add `haneul::table_vec::drop` to the framework via a system package upgrade.
...
// Version 19: Changes to haneul-system package to enable liquid staking.
...
// Version 25: Add haneul::table_vec::swap and haneul::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add haneul::zklogin::verify_zklogin_id and related functions to haneul framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to haneul framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in haneul framework.
...
// Version 42: Migrate haneul framework and related code to Move 2024
...
//             Modified haneul-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in haneul core framework.
...
//             Max gas budget moved to 50_000 HANEUL
//             Max gas price moved to 50 HANEUL
...
// Version 74: Enable load_nitro_attestation move function in haneul framework in devnet.
...
//             Enable load_nitro_attestation move function in haneul framework in testnet.
...
// Version 88: Update `haneul-system` package to use `calculate_rewards` function.
...
// Version 91: Minor changes in Haneul Framework. Include CheckpointDigest in consensus dedup key for checkpoint signatures (V2).
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the Haneul adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the Haneul adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the Haneul adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Haneul adapter at the end of a publish transaction.
    ...
    /// Maximum gas budget in GEUNHWA that a transaction can use.
    max_tx_gas: Option<u64>,

    /// Maximum amount of the proposed gas price in GEUNHWA (defined in the transaction).
    ...
    // === Execution gas costs ====
    /// Base cost for any Haneul transaction
    ...
    // === Storage gas costs ===
    /// Per-byte cost of storing an object in the Haneul global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Haneul transaction log (e.g., in CertifiedTransactionEffects)
    ...
    /// Unit gas price, Geunhwa per internal gas unit.
...
impl ProtocolConfig {
    ...
    pub fn get_for_version(version: ProtocolVersion, chain: Chain) -> Self {
        ...
        if std::env::var("HANEUL_PROTOCOL_CONFIG_OVERRIDE_ENABLE").is_ok() {
            warn!(
            ...
            let overrides: ProtocolConfigOptional =
                serde_env::from_env_with_prefix("HANEUL_PROTOCOL_CONFIG_OVERRIDE")
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in GEUNHWA and an absolute value 50HANEUL
                    cfg.max_tx_gas = Some(50_000_000_000);
                    // min gas budget is in GEUNHWA and an absolute value 2000GEUNHWA or 0.000002HANEUL
                ...
                72 => {
                    ...
                    // max gas budget is in GEUNHWA and an absolute value 50_000 HANEUL
                    cfg.max_tx_gas = Some(50_000_000_000_000);
                    // max gas price is in GEUNHWA and an absolute value 50 HANEUL
                ...
                117 => {
                    ...
                    if chain == Chain::Mainnet {
                        cfg.gasless_allowed_token_types = Some(vec![
                            ...
                            (MAINNET_USDHANEUL.to_string(), 10_000),
                            (MAINNET_HANEUL_USDE.to_string(), 10_000),
    ...
    pub fn apply_seeded_test_overrides(&mut self, seed: &[u8; 32]) {
        ...
        if !haneullabs_common::in_test_configuration() {
            return;
        }
...
macro_rules! check_limit_by_meter {
    ($is_metered:expr, $x:expr, $metered_limit:expr, $unmetered_hard_limit:expr, $metric:expr) => {{
        ...
        use haneul_protocol_config::check_limit_in_range;
