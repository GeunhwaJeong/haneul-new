...
use haneul_protocol_config_macros::{ProtocolConfigAccessors, ProtocolConfigFeatureFlagsGetters};
...
/// The `haneul-node` binary will always use the SYSTEM_DEFAULT constant, but for testing we need
/// to be able to inject arbitrary versions into HaneulNode.
...
pub struct ProtocolConfig {
    ...
    /// Maximum number of gas units that a single MoveCall transaction can use. Enforced by the Haneul adapter.
