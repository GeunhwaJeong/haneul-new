...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let compiled_package = BuildConfig::new_for_testing()
        .build(Path::new("../../haneul_programmability/examples/fungible_tokens").to_path_buf())?;
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            ...
            .await?;
        assert!(
            matches!(result, HaneulObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
