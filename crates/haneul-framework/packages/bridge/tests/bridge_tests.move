...
module bridge::bridge_tests {
    use bridge::bridge::{
        inner_limiter, inner_paused, inner_treasury, inner_token_transfer_records_mut,
        new_bridge_record_for_testing, new_for_testing, test_get_current_seq_num_and_increment,
        test_execute_update_asset_price, test_get_token_transfer_action_signatures,
        test_load_inner, test_load_inner_mut, test_get_token_transfer_action_status,
        transfer_status_approved, transfer_status_claimed, transfer_status_not_found,
        transfer_status_pending, Bridge,
    };
    use bridge::bridge_setup::{
        btc_id, create_bridge, create_bridge_default, create_env, create_validator_info,
        eth_id, freeze_bridge, init_committee, register_committee, unfreeze_bridge, test_token_id,
    };
    use bridge::btc::BTC;
    ...
    fun test_bridge_create() {
        let mut env = create_env(chain_ids::haneul_testnet(), @0x0);
    ...
    fun test_bridge_create_non_system_addr() {
        let mut env = create_env(chain_ids::haneul_mainnet(), @0x1);
    ...
    fun test_init_committee() {
        let mut env = create_env(chain_ids::haneul_custom(), @0x0);
    ...
    fun test_init_committee_non_system_addr() {
        let mut env = create_env(chain_ids::haneul_mainnet(), @0x0);
        // TODO: this is too brittle, fix it
        env.setup_validators(
            vector[
                create_validator_info(@0xA, 100),
                create_validator_info(@0xB, 100),
                create_validator_info(@0xC, 100),
            ],
            @0x0,
        );
        env.create_bridge(@0x0);
    ...
    fun test_register_foreign_token() {
        let addr = @0x0;
        let mut env = create_env(chain_ids::haneul_testnet(), addr);
        env.create_bridge_default();
    ...
    fun test_execute_send_token_frozen() {
        let chain_id = chain_ids::haneul_testnet();
        let mut env = create_env(chain_id, @0x0);
        env.create_bridge_default();
        let eth: Coin<ETH> = env.get_eth(1);
        let eth_address = x"0000000000000000000000000000000000000000";
        env.freeze_bridge(@0x0, UNEXPECTED_ERROR);
        env.send_token(chain_ids::eth_sepolia(), eth_address, eth);
    ...
    fun test_execute_send_token_invalid_route() {
        ...
        let eth_address = x"0000000000000000000000000000000000000000";
        env.send_token(chain_ids::eth_mainnet(), eth_address, usdc);
    ...
    fun test_system_msg_incorrect_chain_id() {
        let sender = @0x0;
        let mut env = create_env(chain_ids::haneul_testnet(), sender);
        env.create_bridge_default();
        env.execute_blocklist(sender, chain_ids::haneul_mainnet(), 0, vector[], vector[]);
    ...
    fun test_update_limit() {
        let chain_id = chain_ids::haneul_mainnet();
        let mut env = create_env(chain_id, @0x0);
        env.create_bridge_default();
    ...
    fun test_execute_update_bridge_limit_abort_with_unexpected_chain_id() {
        ...
        // This abort because the receiving_chain (haneul_mainnet) is not the same as
        // the bridge's chain_id (haneul_devnet)
        env.update_bridge_limit(
            @0x0,
            chain_ids::haneul_mainnet(),
            chain_ids::eth_mainnet(),
    ...
    fun test_get_token_transfer_action_data() {
        ...
        let key = message.key();
        bridge.test_load_inner_mut().inner_token_transfer_records_mut().push_back(
            key,
