...
module bridge::bridge_setup {
    use bridge::bridge::{
        ...
        test_execute_add_tokens_on_haneul, test_execute_emergency_op, test_init_bridge_committee,
        test_load_inner_mut, Bridge,
    };
    use bridge::btc::{Self, BTC};
    use bridge::eth::{Self, ETH};
    use bridge::message::{
        Self, create_add_tokens_on_haneul_message, create_blocklist_message, emergency_op_pause,
    ...
    public fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let payload = add_token_message.extract_add_tokens_on_haneul();
        bridge.test_execute_add_tokens_on_haneul(payload);
