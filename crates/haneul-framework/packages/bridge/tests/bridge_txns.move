...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to haneul account
    let transfer_id1 = env.bridge_to_haneul<ETH>(
        source_chain,
        eth_address,
        haneul_address,
    ...
    let transfer_id = env.bridge_to_haneul<ETH>(
        source_chain,
        eth_address,
        haneul_address,
        amount,
    );
    let token = env.claim_token<ETH>(haneul_address, source_chain, transfer_id);
    env.send_token<ETH>(
        haneul_address,
        source_chain,
        eth_address,
