...
// specify routes from Ethereum to Haneul.
fun initial_transfer_limits(): VecMap<BridgeRoute, u64> {
    ...
    transfer_limits.insert(
        chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::haneul_custom()),
        MAX_TRANSFER_LIMIT,
    );

    transfer_limits.insert(
        chain_ids::get_route(chain_ids::eth_custom(), chain_ids::haneul_testnet()),
        MAX_TRANSFER_LIMIT,
    );

    transfer_limits.insert(
        chain_ids::get_route(chain_ids::eth_custom(), chain_ids::haneul_custom()),
        MAX_TRANSFER_LIMIT,
    );
