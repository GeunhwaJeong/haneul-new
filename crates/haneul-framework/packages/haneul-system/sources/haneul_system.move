...
/// `HaneulSystemState` is a thin wrapper around `HaneulSystemStateInner` that provides a versioned interface.
/// The `HaneulSystemState` object has a fixed ID 0x5, and the `HaneulSystemStateInner` object is stored as a dynamic field.
/// There are a few different ways to upgrade the `HaneulSystemStateInner` type:
///
/// The simplest and one that doesn't involve a real upgrade is to just add dynamic fields to the `extra_fields` field
/// of `HaneulSystemStateInner` or any of its sub type. This is useful when we are in a rush, or making a small change,
/// or still experimenting a new field.
///
/// To properly upgrade the `HaneulSystemStateInner` type, we need to ship a new framework that does the following:
/// 1. Define a new `HaneulSystemStateInner`type (e.g. `HaneulSystemStateInnerV2`).
/// 2. Define a data migration function that migrates the old `HaneulSystemStateInner` to the new one (i.e. HaneulSystemStateInnerV2).
/// 3. Replace all uses of `HaneulSystemStateInner` with `HaneulSystemStateInnerV2` in both haneul_system.move and haneul_system_state_inner.move,
///    with the exception of the `haneul_system_state_inner::create` function, which should always return the genesis type.
...
/// 1. Define a new `HaneulSystemStateInner` struct type that matches the new Move type, and implement the HaneulSystemStateTrait.
/// 2. Update the `HaneulSystemState` struct to include the new version as a new enum variant.
/// 3. Update the `get_haneul_system_state` function to handle the new version.
/// To test that the upgrade will be successful, we need to modify `haneul_system_state_production_upgrade_test` test in
...
public entry fun request_add_validator(wrapper: &mut HaneulSystemState, ctx: &mut TxContext) {
...
/// of the validator.
public entry fun request_remove_validator(wrapper: &mut HaneulSystemState, ctx: &mut TxContext) {
...
public entry fun request_withdraw_stake(
    ...
    let withdrawn_stake = wrapper.request_withdraw_stake_non_entry(staked_haneul, ctx);
...
public fun convert_to_fungible_staked_haneul(
    ...
    wrapper.load_system_state_mut().convert_to_fungible_staked_haneul(staked_haneul, ctx)
}
...
public fun redeem_fungible_staked_haneul(
    ...
    wrapper.load_system_state_mut().redeem_fungible_staked_haneul(fungible_staked_haneul, ctx)
}
...
public fun request_withdraw_stake_non_entry(
    ...
    wrapper.load_system_state_mut().request_withdraw_stake(staked_haneul, ctx)
}
...
public entry fun rotate_operation_cap(self: &mut HaneulSystemState, ctx: &mut TxContext) {
...
public fun validator_address_by_pool_id(wrapper: &mut HaneulSystemState, pool_id: &ID): address {
...
public fun active_validator_addresses_ref(wrapper: &HaneulSystemState): vector<address> {
...
public fun active_validator_voting_powers(wrapper: &HaneulSystemState): VecMap<address, u64> {
...
public(package) fun calculate_rewards(
    ...
    system_state
        ...
        .calculate_rewards(staked_haneul, ctx.epoch())
}
...
fun load_system_state(self: &mut HaneulSystemState): &HaneulSystemStateInnerV2 {
...
fun load_system_state_ref(self: &HaneulSystemState): &HaneulSystemStateInnerV2 {
    let inner: &HaneulSystemStateInnerV2 = dynamic_field::borrow(
        &self.id,
        self.version,
    );
...
fun load_inner_maybe_upgrade(self: &mut HaneulSystemState): &mut HaneulSystemStateInnerV2 {
    if (self.version == 1) {
        let v1: HaneulSystemStateInner = dynamic_field::remove(&mut self.id, self.version);
        let v2 = v1.v1_to_v2();
        self.version = 2;
        dynamic_field::add(&mut self.id, self.version, v2);
    };

    let inner: &mut HaneulSystemStateInnerV2 = dynamic_field::borrow_mut(
        &mut self.id,
        self.version,
    );
    assert!(inner.system_state_version() == self.version, EWrongInnerVersion);
...
fun get_accumulator_storage_fund_amount(wrapper: &mut HaneulSystemState): u64 {
...
public fun validators_mut(wrapper: &mut HaneulSystemState): &mut ValidatorSet {
...
#[test_only]
public fun request_add_validator_for_testing(wrapper: &mut HaneulSystemState, ctx: &TxContext) {
    wrapper.load_system_state_mut().request_add_validator(ctx)
}

#[test_only]
public fun get_storage_fund_total_balance(wrapper: &mut HaneulSystemState): u64 {
