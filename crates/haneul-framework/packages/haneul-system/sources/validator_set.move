...
use haneul_system::staking_pool::{
...
public(package) fun new(
    ...
    init_active_validators.do_ref!(|v| {
        staking_pool_mappings.add(v.staking_pool_id(), v.haneul_address());
    });
...
/// Called by `haneul_system` to add a new validator candidate.
...
/// Called by `haneul_system` to remove a validator candidate, and move them to `inactive_validators`.
...
/// Called by `haneul_system` to add a new validator to `pending_active_validators`, which will be
...
/// Called by `haneul_system`, to add a new stake to the validator.
...
public(package) fun request_add_stake(
    ...
    assert!(haneul_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    self
        .get_candidate_or_active_validator_mut(validator_address)
        .request_add_stake(stake, ctx.sender(), ctx)
}

/// Called by `haneul_system`, to withdraw some share of a stake from the validator. The share to withdraw
/// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
/// 1. If the `staked_haneul` is staked with an active validator, the request is added to the validator's
///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
/// 2. If the `staked_haneul` was staked with a validator that is no longer active,
...
public(package) fun request_withdraw_stake(
    ...
    let staking_pool_id = staked_haneul.pool_id();
    let validator = if (self.staking_pool_mappings.contains(staking_pool_id)) {
        // This is an active validator.
        let validator_address = self.staking_pool_mappings[staked_haneul.pool_id()];
...
public(package) fun redeem_fungible_staked_haneul(
    ...
    let staking_pool_id = fungible_staked_haneul.pool_id();
...
public(package) fun advance_epoch(
    self: &mut ValidatorSet,
    computation_reward: &mut Balance<HANEUL>,
    storage_fund_reward: &mut Balance<HANEUL>,
...
fun update_validator_positions_and_calculate_total_stake(
    ...
    while (i > 0) {
        ...
        let validator_address = validator_ref.haneul_address();
...
public(package) fun is_active_validator_by_haneul_address(
    self: &ValidatorSet,
    validator_address: address,
): bool {
...
fun compute_slashed_validators(
    ...
    while (!validator_report_records.is_empty()) {
        ...
        assert!(
            self.is_active_validator_by_haneul_address(validator_address),
...
fun distribute_reward(
    ...
    staking_rewards: &mut Balance<HANEUL>,
    storage_fund_reward: &mut Balance<HANEUL>,
    ctx: &mut TxContext,
) {
    let length = validators.length();
...
fun emit_validator_epoch_events(
    ...
    length.do!(|i| {
        let v = &vs[i];
        let validator_address = v.haneul_address();
...
public(package) fun is_active_validator(self: &ValidatorSet, addr: address): bool {
    self.active_validators.any!(|v| v.haneul_address() == addr)
}
