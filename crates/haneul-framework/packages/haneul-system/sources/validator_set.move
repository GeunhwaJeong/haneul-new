...
module haneul_system::validator_set {
    ...
    public(package) fun request_withdraw_stake(
        ...
        validator::request_withdraw_stake(validator, staked_haneul, ctx)
    }
    ...
    fun update_and_process_low_stake_departures(
        ...
        while (i > 0) {
            ...
            let validator_address = validator::haneul_address(validator_ref);
    ...
    fun find_validator(validators: &vector<Validator>, validator_address: address): Option<u64> {
        ...
        while (i < length) {
            let v = vector::borrow(validators, i);
            if (validator::haneul_address(v) == validator_address) {
    ...
    fun process_validator_departure(
        ...
        let new_epoch = tx_context::epoch(ctx) + 1;
        let validator_address = validator::haneul_address(&validator);
    ...
    fun process_pending_validators(
        ...
        while (!table_vec::is_empty(&self.pending_active_validators)) {
            ...
            event::emit(
                ValidatorJoinEvent {
                    epoch: new_epoch,
                    validator_address: validator::haneul_address(&validator),
                    staking_pool_id: staking_pool_id(&validator),
                }
            );
    ...
    fun emit_validator_epoch_events(
        ...
        while (i < num_validators) {
            let v = vector::borrow(vs, i);
            let validator_address = validator::haneul_address(v);
    ...
    public(package) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
        ...
        while (i < length) {
            let validator_address = validator::haneul_address(vector::borrow(vs, i));
