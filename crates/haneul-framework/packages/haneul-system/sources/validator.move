...
public use fun haneul_system::validator_wrapper::create_v1 as Validator.wrap_v1;
...
public(package) fun request_add_stake(
    ...
    let staked_haneul = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
...
public(package) fun convert_to_fungible_staked_haneul(
    ...
    let stake_activation_epoch = staked_haneul.activation_epoch();
    let staked_haneul_principal_amount = staked_haneul.amount();
    let fungible_staked_haneul = self.staking_pool.convert_to_fungible_staked_haneul(staked_haneul, ctx);

    event::emit(ConvertingToFungibleStakedHaneulEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun redeem_fungible_staked_haneul(
    ...
    event::emit(RedeemingFungibleStakedHaneulEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun request_add_stake_at_genesis(
    ...
    // 0 = genesis epoch
    let staked_haneul = self.staking_pool.request_add_stake(stake, 0, ctx);
...
public(package) fun request_withdraw_stake(
    ...
    let principal_amount = staked_haneul.amount();
    let stake_activation_epoch = staked_haneul.activation_epoch();
    let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_haneul, ctx);
...
public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<HANEUL>) {
...
public fun total_stake_amount(self: &Validator): u64 {
    self.staking_pool.haneul_balance()
}
...
public fun is_duplicate(self: &Validator, other: &Validator): bool {
    ...
    self.haneul_address == other.haneul_address
...
public(package) fun new_unverified_validator_operation_cap_and_transfer(
    ...
    assert!(sender == self.metadata.haneul_address, ENewCapNotCreatedByValidatorItself);
