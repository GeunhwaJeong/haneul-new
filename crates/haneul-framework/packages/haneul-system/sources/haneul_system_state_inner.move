...
module haneul_system::haneul_system_state_inner {
    ...
    use haneul_system::storage_fund::{Self, StorageFund};
    ...
    public struct HaneulSystemStateInner has store {
        ...
        safe_mode: bool,
        safe_mode_storage_rewards: Balance<HANEUL>,
        safe_mode_computation_rewards: Balance<HANEUL>,
    ...
    public(package) fun create(
        ...
        initial_storage_fund: Balance<HANEUL>,
        protocol_version: u64,
        epoch_start_timestamp_ms: u64,
        parameters: SystemParameters,
        stake_subsidy: StakeSubsidy,
    ...
    public(package) fun v1_to_v2(self: HaneulSystemStateInner): HaneulSystemStateInnerV2 {
    ...
    public(package) fun report_validator(
        ...
        assert!(self.validators.is_active_validator_by_haneul_address(reportee_addr), ENotValidator);
    ...
    public(package) fun advance_epoch(
        ...
        mut storage_reward: Balance<HANEUL>,
        mut computation_reward: Balance<HANEUL>,
    ...
    public(package) fun epoch(self: &HaneulSystemStateInnerV2): u64 {
        self.epoch
    }

    public(package) fun protocol_version(self: &HaneulSystemStateInnerV2): u64 {
        self.protocol_version
    }

    public(package) fun system_state_version(self: &HaneulSystemStateInnerV2): u64 {
    ...
    public(package) fun epoch_start_timestamp_ms(self: &HaneulSystemStateInnerV2): u64 {
        self.epoch_start_timestamp_ms
    }

    /// Returns the total amount staked with `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_stake_amount(self: &HaneulSystemStateInnerV2, validator_addr: address): u64 {
        self.validators.validator_total_stake_amount(validator_addr)
    }

    /// Returns the voting power for `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun active_validator_voting_powers(self: &HaneulSystemStateInnerV2): VecMap<address, u64> {
    ...
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_staking_pool_id(self: &HaneulSystemStateInnerV2, validator_addr: address): ID {

        self.validators.validator_staking_pool_id(validator_addr)
    }

    /// Returns reference to the staking pool mappings that map pool ids to active validator addresses
    public(package) fun validator_staking_pool_mappings(self: &HaneulSystemStateInnerV2): &Table<ID, address> {

        self.validators.staking_pool_mappings()
    }

    /// Returns all the validators who are currently reporting `addr`
    public(package) fun get_reporters_of(self: &HaneulSystemStateInnerV2, addr: address): VecSet<address> {
    ...
    public(package) fun get_storage_fund_total_balance(self: &HaneulSystemStateInnerV2): u64 {
        self.storage_fund.total_balance()
    }

    public(package) fun get_storage_fund_object_rebates(self: &HaneulSystemStateInnerV2): u64 {
        self.storage_fund.total_object_storage_rebates()
    }

    public(package) fun validator_address_by_pool_id(self: &mut HaneulSystemStateInnerV2, pool_id: &ID): address {
    ...
    public(package) fun active_validator_addresses(self: &HaneulSystemStateInnerV2): vector<address> {
    ...
    fun extract_coin_balance(mut coins: vector<Coin<HANEUL>>, amount: option::Option<u64>, ctx: &mut TxContext): Balance<HANEUL> {
    ...
    public(package) fun store_execution_time_estimates(self: &mut HaneulSystemStateInnerV2, estimates: vector<u8>) {
    ...
    public(package) fun validators(self: &HaneulSystemStateInnerV2): &ValidatorSet {
        &self.validators
    }

    #[test_only]
    /// Return the currently active validator by address
    public(package) fun active_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_active_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently pending validator by address
    public(package) fun pending_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_pending_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently candidate validator by address
    public(package) fun candidate_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
        validators(self).get_candidate_validator_ref(validator_address)
    }

    #[test_only]
    public(package) fun get_stake_subsidy_distribution_counter(self: &HaneulSystemStateInnerV2): u64 {
        self.stake_subsidy.get_distribution_counter()
    }

    #[test_only]
    public(package) fun set_epoch_for_testing(self: &mut HaneulSystemStateInnerV2, epoch_num: u64) {
    ...
    #[test_only]
    public(package) fun set_stake_subsidy_distribution_counter(self: &mut HaneulSystemStateInnerV2, counter: u64) {
        self.stake_subsidy.set_distribution_counter(counter)
    }

    #[test_only]
    public(package) fun epoch_duration_ms(self: &HaneulSystemStateInnerV2): u64 {
