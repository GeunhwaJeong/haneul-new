...
use haneul_system::staking_pool::{StakedHaneul, FungibleStakedHaneul, PoolTokenExchangeRate};
...
public(package) fun request_add_validator(self: &mut HaneulSystemStateInnerV2, ctx: &TxContext) {
    assert!(
...
/// of the validator.
public(package) fun request_remove_validator(self: &mut HaneulSystemStateInnerV2, ctx: &TxContext) {
...
// === Fungible Staked HANEUL Management ===
...
public(package) fun rotate_operation_cap(self: &mut HaneulSystemStateInnerV2, ctx: &mut TxContext) {
    self
        .validators
...
/// The change will only take effects starting from the next epoch.
///
/// Aborts if there's a duplicate worker address in the system.
...
/// The change will only take effects starting from the next epoch.
///
/// Aborts if there's a duplicate worker public key in the system.
...
public(package) fun validators_mut(self: &mut HaneulSystemStateInnerV2): &mut ValidatorSet {
    &mut self.validators
}

public(package) fun extra_fields(self: &HaneulSystemStateInnerV2): &Bag {
    &self.extra_fields
}

public(package) fun extra_fields_mut(self: &mut HaneulSystemStateInnerV2): &mut Bag {
