...
module haneul_system::haneul_system_state_inner {
    ...
    public(friend) fun v1_to_v2(self: HaneulSystemStateInner): HaneulSystemStateInnerV2 {
    ...
    public(friend) fun epoch(self: &HaneulSystemStateInnerV2): u64 {
        self.epoch
    }

    public(friend) fun protocol_version(self: &HaneulSystemStateInnerV2): u64 {
        self.protocol_version
    }

    public(friend) fun system_state_version(self: &HaneulSystemStateInnerV2): u64 {
    ...
    public(friend) fun epoch_start_timestamp_ms(self: &HaneulSystemStateInnerV2): u64 {
        self.epoch_start_timestamp_ms
    }

    /// Returns the total amount staked with `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(friend) fun validator_stake_amount(self: &HaneulSystemStateInnerV2, validator_addr: address): u64 {
        validator_set::validator_total_stake_amount(&self.validators, validator_addr)
    }

    /// Returns the staking pool id of a given validator.
    /// Aborts if `validator_addr` is not an active validator.
    public(friend) fun validator_staking_pool_id(self: &HaneulSystemStateInnerV2, validator_addr: address): ID {

        validator_set::validator_staking_pool_id(&self.validators, validator_addr)
    }

    /// Returns reference to the staking pool mappings that map pool ids to active validator addresses
    public(friend) fun validator_staking_pool_mappings(self: &HaneulSystemStateInnerV2): &Table<ID, address> {

        validator_set::staking_pool_mappings(&self.validators)
    }

    /// Returns all the validators who are currently reporting `addr`
    public(friend) fun get_reporters_of(self: &HaneulSystemStateInnerV2, addr: address): VecSet<address> {
    ...
    public(friend) fun get_storage_fund_total_balance(self: &HaneulSystemStateInnerV2): u64 {
        storage_fund::total_balance(&self.storage_fund)
    }

    public(friend) fun get_storage_fund_object_rebates(self: &HaneulSystemStateInnerV2): u64 {
    ...
    public(friend) fun validators(self: &HaneulSystemStateInnerV2): &ValidatorSet {
        &self.validators
    }

    #[test_only]
    /// Return the currently active validator by address
    public(friend) fun active_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
    ...
    public(friend) fun pending_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
    ...
    public(friend) fun candidate_validator_by_address(self: &HaneulSystemStateInnerV2, validator_address: address): &Validator {
        validator_set::get_candidate_validator_ref(validators(self), validator_address)
    }

    #[test_only]
    public(friend) fun get_stake_subsidy_distribution_counter(self: &HaneulSystemStateInnerV2): u64 {
        stake_subsidy::get_distribution_counter(&self.stake_subsidy)
    }

    #[test_only]
    public(friend) fun set_epoch_for_testing(self: &mut HaneulSystemStateInnerV2, epoch_num: u64) {
