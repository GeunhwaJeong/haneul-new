...
const GEUNHWA_PER_HANEUL: u64 = 1_000_000_000; // used internally for stakes.
...
fun test_validator_set_flow() {
    ...
    assert!(validator_set.total_stake() == 100 * GEUNHWA_PER_HANEUL);
    ...
    let scenario = &mut scenario_val;
    {
        ...
        let stake = validator_set.request_add_stake(
            @0x1,
            coin::mint_for_testing(500 * GEUNHWA_PER_HANEUL, ctx1).into_balance(),
            ctx1,
        );
    ...
    assert!(validator_set.total_stake() == 1500 * GEUNHWA_PER_HANEUL);
    ...
    assert!(validator_set.total_stake() == 900 * GEUNHWA_PER_HANEUL);
...
fun test_staking_below_threshold() {
    ...
    let validator1 = create_validator(@0x1, 1, 1, true, ctx);
    let mut validator_set = validator_set::new(vector[validator1], ctx);
    assert_eq(validator_set.total_stake(), 100 * GEUNHWA_PER_HANEUL);
    scenario_val.end();
...
fun test_staking_min_threshold() {
    ...
    advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
    assert!(validator_set.total_stake() == 101 * GEUNHWA_PER_HANEUL);
...
fun test_add_validator_with_min_voting_power() {
    ...
    assert!(validator_set.total_stake() == 10_000 * GEUNHWA_PER_HANEUL);
...
fun test_request_add_then_pull_stake() {
    ...
    let stake = validator_set.request_add_stake(@0xA, balance::create_for_testing(3 * GEUNHWA_PER_HANEUL), scenario.ctx());
...
fun test_withdraw_all() {
    ...
    validator_set.request_add_validator_candidate(new_v, scenario.ctx());
    let stake = validator_set.request_add_stake(@0xB, balance::create_for_testing(4 * GEUNHWA_PER_HANEUL), scenario.ctx());
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
...
fun test_very_low_voting_power_departure() {
    ...
    let grace_period = 3;
    validator_set.request_add_validator_candidate(new_v, scenario.ctx());
    let mut stake = validator_set.request_add_stake(@0xB, balance::create_for_testing(4 * GEUNHWA_PER_HANEUL), scenario.ctx());
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    ...
    let bal = validator_set.request_withdraw_stake(stake.split(3 * GEUNHWA_PER_HANEUL, scenario.ctx()), scenario.ctx());
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(!validator_set.is_active_validator(@0xB));
    // epoch change should emit one ValidatorEpochInfoEvent per validator and one ValidatorLeaveEvent for the departed validator
    let effects = scenario.next_tx(@0xB);
...
fun test_low_voting_power_departure() {
    ...
    let bal = validator_set.request_withdraw_stake(stake.split(2 * GEUNHWA_PER_HANEUL, scenario.ctx()), scenario.ctx());
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(validator_set.is_active_validator(@0xB));
...
fun test_low_voting_power_recovery() {
    ...
    let grace_period = 3;
    validator_set.request_add_validator_candidate(new_v, scenario.ctx());
    let mut stake1 = validator_set.request_add_stake(@0xB, balance::create_for_testing(4 * GEUNHWA_PER_HANEUL), scenario.ctx());
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(validator_set.is_active_validator(@0xB));
    assert!(validator_set.find_for_testing(@0xB).voting_power() == 3);
    // withdraw part of the stake. validator will now have voting power 2 and is now at risk
    let bal = validator_set.request_withdraw_stake(stake1.split(2 * GEUNHWA_PER_HANEUL, scenario.ctx()), scenario.ctx());
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(validator_set.is_active_validator(@0xB));
    assert!(validator_set.find_for_testing(@0xB).voting_power() == 1);
    assert!(validator_set.is_at_risk_validator(@0xB));
...
fun add_then_increase_stake_of_others() {
    ...
    validator_set.request_add_validator_candidate(new_v, scenario.ctx());
    let stake = validator_set.request_add_stake(@0xB, balance::create_for_testing(1000 * GEUNHWA_PER_HANEUL), scenario.ctx());
    test_utils::destroy(stake);
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    ...
    num_validators.do!(|i| {
        let to_add = 10_000_000 * GEUNHWA_PER_HANEUL;
...
fun create_validator(addr: address, hint: u8, gas_price: u64, is_initial_validator: bool, ctx: &mut TxContext): Validator {
    let stake_value = hint as u64 * 100 * GEUNHWA_PER_HANEUL;
...
fun create_validator_with_initial_stake(addr: address, hint: u8, initial_stake: u64, is_initial_validator: bool, ctx: &mut TxContext): Validator {
    ...
    let validator = validator::new_for_testing(
        ...
        vector[hint],
        vector[hint],
        if (initial_stake != 0) { option::some(balance::create_for_testing(initial_stake * GEUNHWA_PER_HANEUL)) } else { option::none() },
        1,
        0,
        is_initial_validator,
        ctx
...
fun add_and_activate_validator(validator_set: &mut ValidatorSet, validator: Validator, scenario: &mut Scenario) {
    scenario.next_tx(validator.haneul_address());
    let ctx = scenario.ctx();
