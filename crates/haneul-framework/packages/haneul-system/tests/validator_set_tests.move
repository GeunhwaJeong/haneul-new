...
module haneul_system::validator_set_tests {
    ...
    fun test_validator_set_flow() {
        ...
        assert!(validator_set.total_stake() == 100 * GEUNHWA_PER_HANEUL, 0);
        ...
        assert!(validator_set.total_stake() == 1500 * GEUNHWA_PER_HANEUL, 0);
        ...
        assert!(validator_set.total_stake() == 900 * GEUNHWA_PER_HANEUL, 0);
    ...
    fun test_staking_min_threshold() {
        ...
        advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
        assert!(validator_set.total_stake() == 101 * GEUNHWA_PER_HANEUL, 0);
    ...
    fun test_add_validator_failure_below_min_stake() {
        ...
        // Validator 2 now has 700 HANEUL in stake but that's not enough because we need 701.
        validator_set.request_add_validator(701 * GEUNHWA_PER_HANEUL, scenario.ctx());
    ...
    fun test_add_validator_with_nonzero_min_stake() {
        ...
        scenario.next_tx(@0x2);
        // Validator 2 now has 700 HANEUL in stake and that's just enough.
        validator_set.request_add_validator(700 * GEUNHWA_PER_HANEUL, scenario.ctx());
    ...
    fun test_low_stake_departure() {
        ...
        let ctx = scenario.ctx();
        // Create 4 validators.
        let v1 = create_validator(@0x1, 1, 1, true, ctx); // 100 HANEUL of stake
        let v2 = create_validator(@0x2, 4, 1, true, ctx); // 400 HANEUL of stake
        let v3 = create_validator(@0x3, 10, 1, true, ctx); // 1000 HANEUL of stake
        let v4 = create_validator(@0x4, 4, 1, true, ctx); // 400 HANEUL of stake
        ...
        scenario.next_tx(@0x42);
        {
            let stake = scenario.take_from_sender<StakedHaneul>();
            let ctx = scenario.ctx();
    ...
    fun advance_epoch_with_low_stake_params(
        ...
        validator_set.advance_epoch(
            ...
            &mut vec_map::empty(),
            0, // reward_slashing_rate
            low_stake_threshold * GEUNHWA_PER_HANEUL,
            very_low_stake_threshold * GEUNHWA_PER_HANEUL,
