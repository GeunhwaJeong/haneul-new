...
fun validator_rewards() {
    let mut runner = test_runner::new()
        ...
        .validators(vector[
            ...
            validator_builder::new().initial_stake(200).haneul_address(VALIDATOR_ADDR_2),
            validator_builder::new().initial_stake(300).haneul_address(VALIDATOR_ADDR_3),
            validator_builder::new().initial_stake(400).haneul_address(VALIDATOR_ADDR_4),
        ])
        .build();
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 125 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 225 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 325 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 425 * GEUNHWA_PER_HANEUL);
    });
    ...
    // check rewards distribution, given that validator 2 has 920 HANEUL of stake now
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 150 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 970 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 350 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 450 * GEUNHWA_PER_HANEUL);
    });
...
fun stake_subsidy() {
    let mut runner = test_runner::new()
        .haneul_supply_amount(1_000_000_000)
        .validators(vector[
            validator_builder::new().initial_stake(100_000_000).haneul_address(VALIDATOR_ADDR_1),
            validator_builder::new().initial_stake(200_000_000).haneul_address(VALIDATOR_ADDR_2),
            validator_builder::new().initial_stake(300_000_000).haneul_address(VALIDATOR_ADDR_3),
            validator_builder::new().initial_stake(400_000_000).haneul_address(VALIDATOR_ADDR_4),
        ])
        .build();
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 100_000_025 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 200_000_025 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 300_000_025 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 400_000_025 * GEUNHWA_PER_HANEUL);
    });
...
fun stake_rewards() {
    ...
    runner.system_tx!(|system, _| {
        // total stake
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 300 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 300 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 300 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 400 * GEUNHWA_PER_HANEUL);
    });
    ...
    assert_eq!(runner.set_sender(STAKER_ADDR_1).haneul_balance(), 220 * GEUNHWA_PER_HANEUL);

    // staker 2 receives 20 HANEUL of rewards.
    runner.set_sender(STAKER_ADDR_2).unstake(0);
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), 120 * GEUNHWA_PER_HANEUL);

    let opts = runner.advance_epoch_opts().computation_charge(40);
    runner.advance_epoch(option::some(opts)).destroy_for_testing();

    // unstake 600 principal HANEUL
    runner.set_sender(STAKER_ADDR_2).unstake(0);
    ...
    // exchange rate is 528 : 750 -ish so the total haneul withdraw will be:
    // (600 * 100 / 140) * 750 / 528 = ~608. Together with the 120 HANEUL we already have,
    // that would be about 728 HANEUL.
    // TODO: Come up with better numbers and clean it up!
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), 728108108107);
...
fun validator_commission() {
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 230 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 330 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 330 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 430 * GEUNHWA_PER_HANEUL);
    });

    assert_stake_rewards_for_addresses(
        &mut runner,
        vector[STAKER_ADDR_1, STAKER_ADDR_2],
        vector[115 * GEUNHWA_PER_HANEUL, 108 * GEUNHWA_PER_HANEUL],
    );
    ...
    runner.advance_epoch(option::some(opts)).destroy_for_testing();
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 290 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 390 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_3), 390 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_4), 490 * GEUNHWA_PER_HANEUL);
    });
    ...
    assert_stake_rewards_for_addresses(
        &mut runner,
        vector[STAKER_ADDR_1, STAKER_ADDR_2],
        vector[142 * GEUNHWA_PER_HANEUL, 123709090909],
    );
...
fun rewards_slashing() {
    ...
    assert_eq!(runner.set_sender(STAKER_ADDR_1).haneul_balance(), 565 * GEUNHWA_PER_HANEUL);
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), 370 * GEUNHWA_PER_HANEUL);
...
fun entire_rewards_slashing() {
    ...
    // Same analysis as above. Staker 1 has 150 additional HANEUL, and since all of staker 2's rewards are slashed she only gets back her principal.
    assert_eq!(runner.set_sender(STAKER_ADDR_1).haneul_balance(), (550 + 150) * GEUNHWA_PER_HANEUL);
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
...
fun rewards_slashing_with_storage_fund() {
    ...
    // 20 HANEUL (1/3) of validator 4's slashed computation reward and 5 HANEUL (1/3) of validator 4's slashed
    ...
    // Staker 1 gets 320 * 1/4 = 80 HANEUL of rewards.
    assert_eq!(runner.set_sender(STAKER_ADDR_1).haneul_balance(), (100 + 80) * GEUNHWA_PER_HANEUL);
    // Staker 2 gets 300 * 1/5 * (1 - 20%) = 48 HANEUL of rewards.
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), (100 + 48) * GEUNHWA_PER_HANEUL);
...
fun everyone_slashed() {
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(system.get_storage_fund_total_balance(), 4000 * GEUNHWA_PER_HANEUL);
        // The entire 1000 HANEUL of storage rewards should go to the object rebate portion of the storage fund.
        assert_eq!(system.get_storage_fund_object_rebates(), 1000 * GEUNHWA_PER_HANEUL);
    });
...
fun mul_rewards_withdraws_at_same_epoch() {
    ...
    // Staker 1 gets 2/3 * 1/4 * 120 = 20 HANEUL here.
    let opts = runner.advance_epoch_opts().computation_charge(120);
    runner.advance_epoch(option::some(opts)).destroy_for_testing();
    ...
    // Staker 1 gets 20 HANEUL here and staker 2 gets 40 HANEUL here.
    ...
    // Staker 1 gets 30 HANEUL, staker 2 gets 40 HANEUL and staker 3 gets 30 HANEUL.
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 140 * 23 * GEUNHWA_PER_HANEUL);
    });
    ...
    assert_eq!(
        runner.set_sender(STAKER_ADDR_1).haneul_balance(),
        (220 + 130 + 20 * 3 + 10) * GEUNHWA_PER_HANEUL,
    );
    // staker 2's stake was active for 2 epochs so got 40 * 2 = 80 HANEUL of rewards
    assert_eq!(runner.set_sender(STAKER_ADDR_2).haneul_balance(), (480 + 40 * 2) * GEUNHWA_PER_HANEUL);
    // staker 3's first stake was active for 1 epoch and got 30 HANEUL of rewards
    // and her second stake didn't get any rewards.
    assert_eq!(runner.set_sender(STAKER_ADDR_3).haneul_balance(), (390 + 280 + 30) * GEUNHWA_PER_HANEUL);
    // staker 4 joined and left in an epoch where no rewards were earned so she got no rewards.
    assert_eq!(runner.set_sender(STAKER_ADDR_4).haneul_balance(), 1400 * GEUNHWA_PER_HANEUL);

    runner.advance_epoch(option::none()).destroy_for_testing();

    // Since all the stakes are gone the pool is empty except for the validator's original stake.
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 140 * GEUNHWA_PER_HANEUL);
    });
...
fun uncapped_rewards() {
    ...
    let validators = vector::tabulate!(num_validators, |i| {
        validator_builder::new()
            ...
            .haneul_address(address::from_u256(i as u256))
    });

    let mut runner = test_runner::new().haneul_supply_amount(1000).validators(validators).build();

    // Each validator's stake gets doubled.
    let opts = runner.advance_epoch_opts().computation_charge(10_000);
    runner.advance_epoch(option::some(opts)).destroy_for_testing();

    // Check that each validator has the correct amount of HANEUL in their stake pool.
    runner.system_tx!(|system, _| {
        num_validators.do!(|i| {
            let addr = address::from_u256(i as u256);
            assert_eq!(system.validator_stake_amount(addr), (962 + i * 4) * GEUNHWA_PER_HANEUL);
        });
    });
...
#[test]
// This test triggers both haneul balance and pool token to fall short but no underflow happens.
fun process_pending_stake_withdraw_no_underflow_in_safe_mode_1() {
    // 4 validators, each with 100, 200, 300, 400 HANEUL
    // safe mode epoch 1
    let mut runner = test_runner::new()
        .validators(vector[
            validator_builder::new().initial_stake(100).haneul_address(VALIDATOR_ADDR_1),
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 100 * GEUNHWA_PER_HANEUL);
    });

    // staker 1 stakes 101 haneul in safe mode
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_amount(), 101 * GEUNHWA_PER_HANEUL);
    });
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.pool_token_balance(), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_amount(), 101 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_withdraw_amount(), 101 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_pool_token_withdraw_amount(), 101 * GEUNHWA_PER_HANEUL);
    });
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.pending_stake_amount(), 0 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_pool_token_withdraw_amount(), 0 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 100 * GEUNHWA_PER_HANEUL);
    });

    // validator 1 withdraws its stake and gets back 100 HANEUL
    runner.set_sender(VALIDATOR_ADDR_1).unstake(0);

    // advance epoch to 4, no safe mode, no rewards
    runner.advance_epoch(option::none()).destroy_for_testing();

    // enforce invariant: haneul_balance = 0
    runner.system_tx!(|system, _| {
        let pool = system
            .validators()
        ...
        assert_eq!(pool.haneul_balance(), 0 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 0 * GEUNHWA_PER_HANEUL);
    });
...
fun process_pending_stake_withdraw_no_underflow_in_safe_mode_2() {
    ...
    let opts = runner
        .advance_epoch_opts()
        .protocol_version(65)
        .computation_charge(100) // 100 HANEUL computation charge
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.haneul_balance(), 125 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 100 * GEUNHWA_PER_HANEUL);
    });
    ...
    // staker 1 stakes 100 haneul in epoch 2
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 100);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(exchange_rate.haneul_amount(), 250 * GEUNHWA_PER_HANEUL);

        // old pool token balance / old pool haneul balance * (pool haneul balance + pending stake)
        // 100 / 150 * (150 + 100) = 166666666666
        assert_eq!(exchange_rate.pool_token_amount(), 166666666666);
        assert_eq!(pool.haneul_balance(), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pool_token_balance(), 166666666666);
    });

    // staker 1 unstakes
    runner.set_sender(STAKER_ADDR_1).owned_tx!<haneul_system::staking_pool::StakedHaneul>(|stake| {
        ...
        runner.system_tx!(|system, ctx| {
            ...
            //   haneul_amount: 125000000000,
            //   pool_token_amount: 100000000000
            // }
            // pending pool token to withdraw: 100 (principal) / 125 * 100 = 80
            let pool = system.active_validator_by_address(VALIDATOR_ADDR_1).get_staking_pool_ref();

            assert_eq!(pool.pending_pool_token_withdraw_amount(), 80 * GEUNHWA_PER_HANEUL);
            // exchange rate for epoch 5: 1666...6: 250
            // total withdraw: 80 * 250 / 166...6 = 120 haneul
            assert_eq!(pool.pending_stake_withdraw_amount(), 120 * GEUNHWA_PER_HANEUL); // 100 principal + 20 rewards
        });
    });
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.haneul_balance(), 155 * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_withdraw_amount(), 155 * GEUNHWA_PER_HANEUL);

        // epoch 0's exchange rate: PoolTokenExchangeRate {
        //   haneul_amount: 100000000000,
        //   pool_token_amount: 100000000000
        // }
        // pending pool token to withdraw: 100 (principal) / 100 * 100 = 100
        // exchange rate for epoch 6: 155000000000: 86666666666
        // total withdraw: min(100 * 155000000000 / 86666666666, pool.haneul_balance()) = 155000000000
        assert_eq!(pool.pending_stake_withdraw_amount(), 155 * GEUNHWA_PER_HANEUL); // 100 principal + 55 rewards
        let exchange_rates = pool.exchange_rates();
        let exchange_rate_epoch_0 = exchange_rates.borrow(0);
        assert_eq!(exchange_rate_epoch_0.haneul_amount(), 0);
        assert_eq!(exchange_rate_epoch_0.pool_token_amount(), 0);
        let exchange_rate_epoch_1 = exchange_rates.borrow(1);
        assert_eq!(exchange_rate_epoch_1.haneul_amount(), 125000000000);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(validator.get_staking_pool_ref().haneul_balance(), 0);
...
fun check_distribution_counter_invariant(system: &mut HaneulSystemState, ctx: &TxContext) {
