...
module haneul_system::rewards_distribution_tests {
    ...
    fun test_everyone_slashed() {
        ...
        assert_eq(haneul_system::get_storage_fund_total_balance(&mut system_state), 4000 * GEUNHWA_PER_HANEUL);

        // The entire 1000 HANEUL of storage rewards should go to the object rebate portion of the storage fund.
        assert_eq(haneul_system::get_storage_fund_object_rebates(&mut system_state), 1000 * GEUNHWA_PER_HANEUL);
    ...
    fun test_uncapped_rewards() {
        ...
        while (i < num_validators) {
            ...
            assert_eq(haneul_system::validator_stake_amount(&mut system_state, addr), (962 + i * 4) * GEUNHWA_PER_HANEUL);
            i = i + 1;
        };
        test_scenario::return_shared(system_state);
        test_scenario::end(scenario_val);
