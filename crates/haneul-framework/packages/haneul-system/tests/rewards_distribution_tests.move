...
module haneul_system::rewards_distribution_tests {
    ...
    fun test_stake_subsidy() {
        set_up_haneul_system_state_with_big_amounts();
        let mut scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
    ...
    fun test_stake_rewards() {
        ...
        assert_eq(total_haneul_balance(STAKER_ADDR_2, scenario), 728108108107);
        scenario_val.end();
    }
    ...
    fun test_rewards_slashing() {
        ...
        assert!(total_haneul_balance(STAKER_ADDR_1, scenario) == 565 * GEUNHWA_PER_HANEUL);
        assert!(total_haneul_balance(STAKER_ADDR_2, scenario) == 370 * GEUNHWA_PER_HANEUL);
        scenario_val.end();
    }
    ...
    fun test_entire_rewards_slashing() {
        ...
        assert!(total_haneul_balance(STAKER_ADDR_1, scenario) == (550 + 150) * GEUNHWA_PER_HANEUL);
        assert!(total_haneul_balance(STAKER_ADDR_2, scenario) == 100 * GEUNHWA_PER_HANEUL);
        scenario_val.end();
    }
    ...
    fun test_rewards_slashing_with_storage_fund() {
        ...
        assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), (100 + 80) * GEUNHWA_PER_HANEUL);
        // Staker 2 gets 300 * 1/5 * (1 - 20%) = 48 HANEUL of rewards.
        assert_eq(total_haneul_balance(STAKER_ADDR_2, scenario), (100 + 48) * GEUNHWA_PER_HANEUL);
    ...
    fun test_everyone_slashed() {
        ...
        assert_eq(system_state.get_storage_fund_total_balance(), 4000 * GEUNHWA_PER_HANEUL);

        // The entire 1000 HANEUL of storage rewards should go to the object rebate portion of the storage fund.
        assert_eq(system_state.get_storage_fund_object_rebates(), 1000 * GEUNHWA_PER_HANEUL);
    ...
    fun test_mul_rewards_withdraws_at_same_epoch() {
        ...
        assert_eq(system_state.validator_stake_amount(VALIDATOR_ADDR_1), 140 * 23 * GEUNHWA_PER_HANEUL);
        test_scenario::return_shared(system_state);
        ...
        assert_eq(system_state.validator_stake_amount(VALIDATOR_ADDR_1), 140 * GEUNHWA_PER_HANEUL);
        test_scenario::return_shared(system_state);
        scenario_val.end();
    }
    ...
    fun test_uncapped_rewards() {
        ...
        while (i < num_validators) {
            ...
            assert_eq(system_state.validator_stake_amount(addr), (962 + i * 4) * GEUNHWA_PER_HANEUL);
            i = i + 1;
        };
        test_scenario::return_shared(system_state);
        scenario_val.end();
    ...
    fun set_up_haneul_system_state_with_big_amounts() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        let ctx = scenario.ctx();
    ...
    fun test_stake_subsidy_with_safe_mode_epoch_562_to_563() {
        ...
        let mut haneul_system = test.take_shared<HaneulSystemState>();
        let ctx = test.ctx();
        ...
        let epoch_duration = haneul_system.inner_mut_for_testing().epoch_duration_ms();

        // increment epoch number (safe mode emulation)
        start_epoch.do!(|_| ctx.increment_epoch_number());
        haneul_system.set_epoch_for_testing(start_epoch);
        haneul_system.set_stake_subsidy_distribution_counter(start_distribution_counter);

        assert!(ctx.epoch() == start_epoch);
        assert!(ctx.epoch() == haneul_system.epoch());
        assert!(haneul_system.get_stake_subsidy_distribution_counter() == start_distribution_counter);
        ...
        assert_eq(haneul_system.get_stake_subsidy_distribution_counter(), start_distribution_counter + 3);
        check_distribution_counter_invariant(&mut haneul_system, ctx);
        ...
        assert_eq(haneul_system.get_stake_subsidy_distribution_counter(), start_distribution_counter + 4);
        check_distribution_counter_invariant(&mut haneul_system, ctx);

        test_scenario::return_shared(haneul_system);
        test.end();
    }
    ...
    fun test_stake_subsidy_with_safe_mode_epoch_563_to_564() {
        ...
        assert_eq(haneul_system.get_stake_subsidy_distribution_counter(), start_distribution_counter + 5);
        check_distribution_counter_invariant(&mut haneul_system, ctx);
    ...
    fun test_stake_subsidy_with_safe_mode_testnet() {
        ...
        haneul_system.set_epoch_for_testing(540);
        haneul_system.set_stake_subsidy_distribution_counter(540);

        assert!(ctx.epoch() == 540);
        assert!(haneul_system.get_stake_subsidy_distribution_counter() == 540);
        ...
        assert_eq!(haneul_system.get_stake_subsidy_distribution_counter(), 541);
