...
module haneul_system::stake_tests {
    ...
    fun test_remove_stake_post_active_flow(should_distribute_rewards: bool) {
        ...
        test_scenario::next_tx(scenario, STAKER_ADDR_1);
        {
            ...
            assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), 100 + reward_amt);
    ...
    fun test_add_preactive_remove_preactive() {
        ...
        assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), 100);
    ...
    fun test_add_preactive_remove_active() {
        ...
        // staker 1 and 3 unstake from the validator and earns 9 GEUNHWA.
        ...
        assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), 109);
        unstake(STAKER_ADDR_3, 0, scenario);
        assert_eq(total_haneul_balance(STAKER_ADDR_3, scenario), 109);

        advance_epoch_with_reward_amounts(0, 100, scenario);
        unstake(STAKER_ADDR_2, 0, scenario);
        // staker 2 earns about 27 GEUNHWA from the previous epoch and 5/8 of the 100 GEUNHWA.
        assert_eq(total_haneul_balance(STAKER_ADDR_2, scenario), 300 + 27 + 59);
    ...
    fun test_add_preactive_remove_post_active() {
        ...
        unstake(STAKER_ADDR_1, 0, scenario);
        assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), 100 + 20);
    ...
    fun test_stake_during_safe_mode() {
        ...
        stake_with(STAKER_ADDR_1, VALIDATOR_ADDR_1, 100, scenario);
        advance_epoch(scenario);
        // The first stake gets 10 GEUNHWA here.
        advance_epoch_with_reward_amounts(0, 40, scenario);
        ...
        advance_epoch_safe_mode(scenario);
        advance_epoch(scenario);
        // The first stake gets 10 GEUNHWA and the second one gets 8 here.
        advance_epoch_with_reward_amounts(0, 50, scenario);
        advance_epoch_safe_mode(scenario);

        unstake(STAKER_ADDR_1, 0, scenario);
        // 100 principal + 20 rewards
        assert_eq(total_haneul_balance(STAKER_ADDR_1, scenario), 120);

        unstake(STAKER_ADDR_2, 0, scenario);
        // 100 principal + 8 rewards
        assert_eq(total_haneul_balance(STAKER_ADDR_2, scenario), 108);
