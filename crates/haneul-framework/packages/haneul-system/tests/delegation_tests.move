...
#[test_only]
module haneul_system::delegation_tests;
...
use haneul_system::validator_preset;
...
fun split_join_staked_haneul() {
    ...
    runner.owned_tx!<StakedHaneul>(|mut stake| {
        stake.split_to_sender(20 * GEUNHWA_PER_HANEUL, runner.ctx());
        runner.keep(stake);
    });

    runner.scenario_fn!(|scenario| {
        let ids = scenario.ids_for_sender<StakedHaneul>();
        assert_eq!(ids.length(), 2);

        let mut stake_1 = scenario.take_from_sender_by_id<StakedHaneul>(ids[0]);
        let stake_2 = scenario.take_from_sender_by_id<StakedHaneul>(ids[1]);

        assert_eq!(stake_1.amount(), 20 * GEUNHWA_PER_HANEUL);
        assert_eq!(stake_2.amount(), 40 * GEUNHWA_PER_HANEUL);
...
fun split_below_threshold() {
    ...
    runner.owned_tx!<StakedHaneul>(|mut stake| {
        stake.split_to_sender(1 * GEUNHWA_PER_HANEUL + 1, runner.ctx());
    });
...
fun add_remove_stake_flow() {
    ...
    // Stake 60 HANEUL to the validator.
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 60);

    // Check that the stake is NOT yet added to the validator.
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 100 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * GEUNHWA_PER_HANEUL);
    });
    ...
    runner.owned_tx!<StakedHaneul>(|stake| {
        runner.system_tx!(|system, ctx| {
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 160 * GEUNHWA_PER_HANEUL);
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * GEUNHWA_PER_HANEUL);
...
// Scenario:
// 1. Stake 100 HANEUL to VALIDATOR_ADDR_1
// 2. Advance epoch
...
fun remove_stake_post_active_flow(should_distribute_rewards: bool) {
    ...
    // Stake 100 HANEUL to the validator.
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 200 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * GEUNHWA_PER_HANEUL);
    });
    ...
    let reward_amt = if (should_distribute_rewards) 15 * GEUNHWA_PER_HANEUL else 0;
    let validator_reward_amt = if (should_distribute_rewards) 10 * GEUNHWA_PER_HANEUL else 0;
    ...
    runner.set_sender(STAKER_ADDR_1);
    assert_eq!(runner.haneul_balance(), 100 * GEUNHWA_PER_HANEUL + reward_amt);
    ...
    assert_eq!(runner.haneul_balance(), 100 * GEUNHWA_PER_HANEUL + reward_amt + validator_reward_amt);
...
fun earns_rewards_at_last_epoch() {
    ...
    // Each validator pool gets 30 GEUNHWA and validators shares the 20 GEUNHWA from the storage fund
    // so validator gets another 10 GEUNHWA.
    let reward_amt = 15 * GEUNHWA_PER_HANEUL;
    let validator_reward_amt = 10 * GEUNHWA_PER_HANEUL;
...
fun add_preactive_remove_preactive() {
    ...
    let validator = validator_builder::preset(2)
        .haneul_address(NEW_VALIDATOR_ADDR)
        .build(runner.ctx());
...
fun add_preactive_remove_pending_failure() {
    ...
    // Stake 100 HANEUL to the validator candidate.
    runner.set_sender(STAKER_ADDR_1).stake_with(NEW_VALIDATOR_ADDR, 100);
...
fun add_preactive_remove_active() {
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(
            system.inner_mut_for_testing().get_storage_fund_total_balance(),
            100 * GEUNHWA_PER_HANEUL,
        );
    });

    // Stake 50 HANEUL to the validator candidate from staker 2.
    runner.set_sender(STAKER_ADDR_2).stake_with(NEW_VALIDATOR_ADDR, 50);

    // Stake 100 HANEUL to the validator candidate from staker 3.
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(system.validator_stake_amount(NEW_VALIDATOR_ADDR), 250 * GEUNHWA_PER_HANEUL);
        assert_eq!(
            system.inner_mut_for_testing().get_storage_fund_total_balance(),
            100 * GEUNHWA_PER_HANEUL,
        );
    ...
    // staker 1 and 3 unstake from the validator and earns about 2/5 * (85 - 10) * 1/3 = 10 HANEUL each.
    ...
    // So they will both get slightly more than 110 HANEUL in total balance.
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.haneul_balance(), 110002000000);
    ...
    // staker 2 earns about 5 HANEUL from the previous epoch and 24-ish from this one
    // so in total she has about 50 + 5 + 24 = 79 HANEUL.
    runner.set_sender(STAKER_ADDR_2).unstake(0);
    assert_eq!(runner.haneul_balance(), 78862939078);
...
fun add_preactive_remove_post_active() {
    ...
    // staker 1 earns a bit greater than 30 HANEUL here. A bit greater because the new validator's voting power
    ...
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.haneul_balance(), 130006000000);
...
// Scenario:
// 1. Add a validator candidate
// 2. Stake 100 HANEUL to the validator candidate from each of the two stakers.
...
// Scenario:
// 1. Add a validator candidate
// 2. Delegate 100 HANEUL to the validator candidate
...
fun remove_inactive_stake_from_inactive_candidate() {
    ...
    assert_eq!(runner.set_sender(validator_address).haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
...
fun remove_multiple_validators() {
    ...
    // Get 3 validators, stake 100 HANEUL to each, request their addition to the validator set.
...
#[test]
/// Scenario:
/// 1. Stake 100 HANEUL to the validator.
/// 2. Advance epoch.
...
fun staking_pool_exchange_rate_getter() {
    ...
    // Each staking pool (2 pools) gets 10 HANEUL of rewards.
...
fun assert_exchange_rate_eq(
    ...
    assert_eq!(rate.haneul_amount(), haneul_amount * GEUNHWA_PER_HANEUL);
    assert_eq!(rate.pool_token_amount(), pool_token_amount * GEUNHWA_PER_HANEUL);
}
