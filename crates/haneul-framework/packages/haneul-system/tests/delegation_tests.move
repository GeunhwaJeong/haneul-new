...
fun test_split_join_staked_haneul() {
    ...
    {
        let mut staked_haneul = scenario.take_from_sender<StakedHaneul>();
        let ctx = scenario.ctx();
        staked_haneul.split_to_sender(20 * GEUNHWA_PER_HANEUL, ctx);
...
fun test_split_below_threshold() {
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        staked_haneul.split_to_sender(1 * GEUNHWA_PER_HANEUL + 1, ctx);
        scenario.return_to_sender(staked_haneul);
    };
    scenario_val.end();
}
...
fun test_split_nonentry_below_threshold() {
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        let ctx = scenario.ctx();
        // The remaining amount after splitting is below the threshold so this should fail.
        let stake = staked_haneul.split(1 * GEUNHWA_PER_HANEUL + 1, ctx);
...
fun test_add_remove_stake_flow() {
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        system_state_mut_ref.request_add_stake(
            coin::mint_for_testing(60 * GEUNHWA_PER_HANEUL, ctx),
            VALIDATOR_ADDR_1,
            ctx,
        );

        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * GEUNHWA_PER_HANEUL,
        );
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * GEUNHWA_PER_HANEUL,
        );
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 160 * GEUNHWA_PER_HANEUL,
        );
        assert!(
            system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * GEUNHWA_PER_HANEUL,
        );
...
fun test_remove_stake_post_active_flow(should_distribute_rewards: bool) {
    ...
    scenario.next_tx(STAKER_ADDR_1);
    {
        ...
        assert!(
            !system_state_mut_ref.validators().is_active_validator_by_haneul_address(VALIDATOR_ADDR_1),
        );
    ...
    assert_eq(
        total_haneul_balance(VALIDATOR_ADDR_1, scenario),
        100 * GEUNHWA_PER_HANEUL + reward_amt + validator_reward_amt,
    );
...
fun test_staking_pool_exchange_rate_getter() {
    ...
    let staked_haneul = scenario.take_from_address<StakedHaneul>(@0x42);
    let pool_id = staked_haneul.pool_id();
