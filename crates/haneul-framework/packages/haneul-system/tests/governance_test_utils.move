...
module haneul_system::governance_test_utils {
    ...
    public fun advance_epoch_safe_mode_with_reward_amounts(
        ...
        let ctx = test_scenario::ctx(scenario);
        haneul_system::advance_epoch_safe_mode_for_testing(&mut system_state, new_epoch, 1, storage_charge, computation_charge, storage_rebate, ctx);
        test_scenario::return_shared(system_state);
        test_scenario::next_epoch(scenario, @0x0);
    }
    ...
    public fun stake_plus_current_rewards(addr: address, staking_pool: &StakingPool, scenario: &mut Scenario): u64 {
        ...
        let stake_ids = test_scenario::ids_for_sender<StakedHaneul>(scenario);
