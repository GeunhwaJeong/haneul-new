...
public fun create_validator_for_testing(
    addr: address,
    init_stake_amount_in_haneul: u64,
    ctx: &mut TxContext,
): Validator {
...
public fun advance_epoch_with_reward_amounts(
    ...
    let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(
        storage_charge * GEUNHWA_PER_HANEUL,
        computation_charge * GEUNHWA_PER_HANEUL,
        0,
        0,
        scenario,
    );
...
public fun unstake(staker: address, staked_haneul_idx: u64, scenario: &mut Scenario) {
    scenario.next_tx(staker);
    let stake_haneul_ids = scenario.ids_for_sender<StakedHaneul>();
    let staked_haneul = scenario.take_from_sender_by_id(stake_haneul_ids[staked_haneul_idx]);
    let mut system_state = scenario.take_shared<HaneulSystemState>();

    let ctx = scenario.ctx();
    system_state.request_withdraw_stake(staked_haneul, ctx);
    test_scenario::return_shared(system_state);
}
...
public fun add_validator_full_flow(
    ...
    system_state.request_add_stake(
        coin::mint_for_testing<HANEUL>(init_stake_amount * GEUNHWA_PER_HANEUL, ctx),
        validator,
        ctx,
    );
...
public fun stake_plus_current_rewards(
    ...
    let mut stake_ids = scenario.ids_for_sender<StakedHaneul>();
    let current_epoch = scenario.ctx().epoch();

    while (!stake_ids.is_empty()) {
        let staked_haneul_id = stake_ids.pop_back();
        let staked_haneul = scenario.take_from_sender_by_id<StakedHaneul>(staked_haneul_id);
        sum =
            sum + staked_haneul.amount() + staking_pool.calculate_rewards(&staked_haneul, current_epoch);
...
public fun total_haneul_balance(addr: address, scenario: &mut Scenario): u64 {
    ...
    while (i < coin_ids.length()) {
        let coin = scenario.take_from_sender_by_id<Coin<HANEUL>>(coin_ids[i]);
        sum = sum + coin.value();
