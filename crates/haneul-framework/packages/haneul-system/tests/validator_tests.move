...
const PROOF_OF_POSSESSION: vector<u8> =
...
const TOO_LONG_257_BYTES: vector<u8> =
...
fun test_validator_owner_flow() {
    ...
    let scenario = &mut scenario_val;
    {
        ...
        assert!(validator.haneul_address() == sender);
...
fun test_pending_validator_flow() {
    ...
    scenario.next_tx(sender);
    {
        let coin_ids = scenario.ids_for_sender<StakedHaneul>();
        let stake = scenario.take_from_sender_by_id<StakedHaneul>(coin_ids[0]);
        let ctx = scenario.ctx();
    ...
    scenario.next_tx(sender);
    {
        let coin_ids = scenario.ids_for_sender<Coin<HANEUL>>();
        let withdraw = scenario.take_from_sender_by_id<Coin<HANEUL>>(coin_ids[0]);
...
fun test_validator_update_metadata_ok() {
    ...
    scenario.next_tx(sender);
    {
        ...
        // Next epoch
        assert!(
            validator.next_epoch_network_address() == &option::some(b"/ip4/192.168.1.1/tcp/80".to_string()),
        );
        assert!(
            validator.next_epoch_p2p_address() == &option::some(b"/ip4/192.168.1.1/udp/80".to_string()),
        );
        assert!(
            validator.next_epoch_primary_address() == &option::some(b"/ip4/192.168.1.1/udp/80".to_string()),
        );
        assert!(
            validator.next_epoch_worker_address() == &option::some(b"/ip4/192.168.1.1/udp/80".to_string()),
        );
        assert!(
        ...
        assert!(validator.next_epoch_worker_pubkey_bytes() == &option::some(new_worker_pub_key), 0);
        assert!(
...
#[expected_failure(abort_code = haneul_system::validator::EInvalidProofOfPossession)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidNetPubkey)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidWorkerPubkey)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidNetAddr)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidPrimaryAddr)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidWorkerAddr)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EMetadataInvalidP2pAddr)]
#[test]
...
#[expected_failure(abort_code = haneul_system::validator::EValidatorMetadataExceedingLengthLimit)]
#[test]
