...
module haneul_system::haneul_system_tests {
    ...
    fun test_validator_ops_by_stakee_ok() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_haneul_system_state(vector[@0x1, @0x2]);
    ...
    fun test_set_commission_rate_failure() {
        ...
        haneul_system::request_set_commission_rate(&mut system_state, 2001, test_scenario::ctx(scenario));
        test_scenario::return_shared(system_state);
    ...
    fun test_staking_pool_mappings() {
        ...
        set_up_haneul_system_state(vector[@0x1, @0x2, @0x3, @0x4]);
        test_scenario::next_tx(scenario, @0x1);
        let mut system_state = test_scenario::take_shared<HaneulSystemState>(scenario);
        ...
        let pool_id_4 = haneul_system::validator_staking_pool_id(&mut system_state, @0x4);
        let mut pool_mappings = haneul_system::validator_staking_pool_mappings(&mut system_state);
        ...
        test_scenario::next_tx(scenario, @0x1);
        let mut system_state = test_scenario::take_shared<HaneulSystemState>(scenario);
        let pool_id_5 = haneul_system::validator_staking_pool_id(&mut system_state, new_validator_addr);
    ...
    fun update_metadata(
        ...
        haneul_system::update_validator_next_epoch_network_pubkey(system_state, network_pubkey, ctx);
        haneul_system::update_validator_next_epoch_worker_pubkey(system_state, worker_pubkey, ctx);
    }
    ...
    fun test_active_validator_update_metadata() {
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<HANEUL>(100_000_000_000)),
            1,
            0,
            true,
            ctx
    ...
    fun advance_epoch_and_check_distribution_counter(scenario: &mut Scenario, epoch_length: u64, should_increment_counter: bool) {
        ...
        let mut system_state = test_scenario::take_shared<HaneulSystemState>(scenario);
        let prev_epoch_time = haneul_system::epoch_start_timestamp_ms(&mut system_state);
        let prev_counter = haneul_system::get_stake_subsidy_distribution_counter(&mut system_state);

        let rebate = haneul_system::advance_epoch_for_testing(
            &mut system_state, new_epoch, 1, 0, 0, 0, 0, 0, 0, prev_epoch_time + epoch_length, test_scenario::ctx(scenario)
        );
        destroy(rebate);
        assert_eq(haneul_system::get_stake_subsidy_distribution_counter(&mut system_state), prev_counter + (if (should_increment_counter) 1 else 0));
        test_scenario::return_shared(system_state);
