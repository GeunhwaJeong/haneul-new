...
fun report_validator() {
    let mut runner = test_runner::new()
        .validators(vector[
            validator_builder::new().haneul_address(@1),
            validator_builder::new().haneul_address(@2),
            validator_builder::new().haneul_address(@3),
        ])
        .build();
...
fun report_validator_by_stakee_ok(initial_stake: u8) {
    ...
    let new_validator = validator.haneul_address();
...
#[test, expected_failure(abort_code = ::haneul_system::validator_set::EInvalidCap)]
...
#[test, expected_failure(abort_code = haneul_system::validator::EGasPriceHigherThanThreshold)]
fun set_gas_price_failure() {
    let validator = validator_builder::new().haneul_address(@1);
...
#[test, expected_failure(abort_code = haneul_system::validator::ECommissionRateTooHigh)]
...
#[test, expected_failure(abort_code = haneul_system::haneul_system_state_inner::ENotValidator)]
...
#[test, expected_failure(abort_code = haneul_system::haneul_system_state_inner::EReportRecordNotFound)]
...
#[test, expected_failure(abort_code = haneul_system::haneul_system_state_inner::ECannotReportOneself)]
...
fun staking_pool_mappings() {
    let mut runner = test_runner::new()
        .validators(vector[
            ...
            validator_builder::new().haneul_address(@2),
            validator_builder::new().haneul_address(@3),
            validator_builder::new().haneul_address(@4),
        ])
        .build();
...
#[random_test]
// Stake random amount of HANEUL and check that the pending and withdraw amounts are correct.
fun withdraw_inactive_stake(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().haneul_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    // Check that pending stake amount is 1 HANEUL.
    runner.system_tx!(|system, _| {
        let pool = system.active_validator_by_address(@1).get_staking_pool_ref();
        assert_eq!(pool.pending_stake_amount(), stake_amount * GEUNHWA_PER_HANEUL);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.haneul_balance(), 100 * GEUNHWA_PER_HANEUL);
    });
...
#[random_test]
// Stake random amount of HANEUL and check that the pending stake amount is correct.
// Convert to fungible staked HANEUL and redeem it.
// Check that the stake amount is correct.
fun convert_to_fungible_staked_haneul_and_redeem(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().haneul_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    let staked_haneul = runner.set_sender(@5).stake_with_and_take(@1, stake_amount);

    assert_eq!(staked_haneul.amount(), stake_amount * GEUNHWA_PER_HANEUL);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.pending_stake_amount(), 0);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.haneul_balance(), (100 + stake_amount) * GEUNHWA_PER_HANEUL);
    });

    // Convert to fungible staked HANEUL.
    let fungible_staked_haneul;
    runner.system_tx!(|system, ctx| {
        fungible_staked_haneul = system.convert_to_fungible_staked_haneul(staked_haneul, ctx);
    });

    assert_eq!(fungible_staked_haneul.value(), stake_amount * GEUNHWA_PER_HANEUL);

    let haneul;
    runner.system_tx!(|system, ctx| {
        haneul = system.redeem_fungible_staked_haneul(fungible_staked_haneul, ctx);
    });

    assert_eq!(haneul.destroy_for_testing(), stake_amount * GEUNHWA_PER_HANEUL);
