...
module haneul_system::validator_builder;

use haneul::bag;
use haneul::balance;
use haneul::haneul::HANEUL;
use haneul::url;
use haneul_system::validator::{Self, Validator, ValidatorMetadata};
use haneul_system::validator_preset::{Self, Preset};
...
public fun from_preset(preset: Preset): ValidatorBuilder {
    ValidatorBuilder {
        haneul_address: option::some(preset.account_address()),
...
public fun build(builder: ValidatorBuilder, ctx: &mut TxContext): Validator {
    ...
    validator::new_for_testing(
        haneul_address.destroy_or!(ctx.fresh_object_address()),
        ...
        initial_stake.map!(|amount| balance::create_for_testing<HANEUL>(amount * 1_000_000_000)),
...
public fun build_metadata(builder: ValidatorBuilder, ctx: &mut TxContext): ValidatorMetadata {
    ...
    validator::new_metadata(
        haneul_address.destroy_or!(default_preset.account_address()),
...
public fun haneul_address(mut builder: ValidatorBuilder, haneul_address: address): ValidatorBuilder {
    builder.haneul_address = option::some(haneul_address);
    builder
}
...
public fun valid_haneul_address(): address {
    let preset = validator_preset::preset(0);
