...
/// provides a set of convenience methods for testing the Haneul System.
module haneul_system::test_runner;
...
use haneul_system::validator_builder::{Self, ValidatorBuilder};
...
public struct TestRunnerBuilder {
    ...
    haneul_supply_amount: Option<u64>,
...
public fun new(): TestRunnerBuilder {
    TestRunnerBuilder {
        ...
        haneul_supply_amount: option::none(),
...
public fun build(builder: TestRunnerBuilder): TestRunner {
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<HANEUL>(haneul_supply_amount.destroy_or!(1000) * GEUNHWA_PER_HANEUL), // haneul_supply
    ...
    let genesis_validator_addresses = validators.map_ref!(|v| v.haneul_address());

    // create haneul system state
    haneul_system::create(
        object::new(scenario.ctx()), // it doesn't matter what ID haneul system state has in tests
        validators,
        balance::create_for_testing<HANEUL>(storage_fund_amount.destroy_or!(0) * GEUNHWA_PER_HANEUL), // storage_fund
...
public fun haneul_supply_amount(
    mut builder: TestRunnerBuilder,
    haneul_supply_amount: u64,
): TestRunnerBuilder {
    builder.haneul_supply_amount = option::some(haneul_supply_amount);
    builder
}
...
/// Mint a HANEUL balance for testing.
public fun mint(amount: u64): Balance<HANEUL> {
    balance::create_for_testing(amount * GEUNHWA_PER_HANEUL)
}

/// Destroy an object.
public fun destroy<T>(v: T) {
    haneul::test_utils::destroy(v);
}
...
public macro fun system_tx(
    ...
    $f: |&mut HaneulSystemState, &mut TxContext|,
): &mut TestRunner {
    let sender = sender($runner);
    let scenario = scenario_mut($runner);
    scenario.next_tx(sender);
    ...
    runner.system_tx!(|system, ctx| {
        ...
        storage_rebate_balance =
            system.advance_epoch_for_testing(
                ...
                storage_charge.destroy_or!(0) * GEUNHWA_PER_HANEUL,
                computation_charge.destroy_or!(0) * GEUNHWA_PER_HANEUL,
...
public fun stake_with(runner: &mut TestRunner, validator: address, amount: u64) {
    ...
    runner.system_tx!(|system, ctx| {
        system.request_add_stake(
            coin::mint_for_testing(amount * GEUNHWA_PER_HANEUL, ctx),
            validator,
            ctx,
        );
    });
...
public fun unstake(runner: &mut TestRunner, staked_haneul_idx: u64) {
    let sender = runner.sender;
    runner.set_sender(sender);
    let stake_haneul_ids = runner.scenario.ids_for_sender<StakedHaneul>();
    let staked_haneul = runner.scenario.take_from_sender_by_id(stake_haneul_ids[staked_haneul_idx]);
    runner.system_tx!(|system, ctx| {
        system.request_withdraw_stake(staked_haneul, ctx);
    });
}
...
public fun add_validator_candidate(runner: &mut TestRunner, validator: Validator) {
    let sender = runner.sender;
    runner.scenario.next_tx(validator.haneul_address());
    runner.system_tx!(|system, ctx| {
...
/// Get the sum of the balances of all the HANEUL coins in the sender's account.
public fun haneul_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    scenario.next_tx(sender);
    scenario.ids_for_sender<Coin<HANEUL>>().fold!(0, |mut sum, coin_id| {
        let coin = scenario.take_from_sender_by_id<Coin<HANEUL>>(coin_id);
        sum = sum + coin.value();
...
/// Get the sum of the StakedHaneul objects' principal and the rewards for these objects.
public fun staking_rewards_balance(runner: &mut TestRunner): u64 {
    ...
    let total_balance = scenario.ids_for_sender<StakedHaneul>().fold!(0, |mut sum, staked_haneul_id| {
        ...
        let validator = system.validator_address_by_pool_id(&staked_haneul.pool_id());
        let rewards = system
            .active_validator_by_address(validator)
            .get_staking_pool_ref()
            .calculate_rewards(&staked_haneul, current_epoch);
