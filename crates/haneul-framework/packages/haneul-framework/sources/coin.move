...
module haneul::coin {
    ...
    use haneul::deny_list::{Self, DenyList};
    use std::type_name;

    // Allows calling `.split_vec(amounts, ctx)` on `coin`
    public use fun haneul::pay::split_vec as Coin.split_vec;

    // Allows calling `.join_vec(coins)` on `coin`
    public use fun haneul::pay::join_vec as Coin.join_vec;

    // Allows calling `.split_and_transfer(amount, recipient, ctx)` on `coin`
    public use fun haneul::pay::split_and_transfer as Coin.split_and_transfer;

    // Allows calling `.divide_and_keep(n, ctx)` on `coin`
    public use fun haneul::pay::divide_and_keep as Coin.divide_and_keep;
    ...
    public fun create_currency<T: drop>(
        ...
        assert!(haneul::types::is_one_time_witness(&witness), EBadWitness);
    ...
    /// The index into the deny list vector for the `haneul::coin::Coin` type.
