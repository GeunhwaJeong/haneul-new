...
/// either using one of the pre-built primitives (see `haneul::royalty`)
...
/// HANEUL set in the offer. The responsibility to follow the transfer policy
...
module haneul::kiosk {
    ...
    use haneul::transfer_policy::{Self, TransferRequest};
    ...
    public fun purchase<T: key + store>(
        self: &mut Kiosk, id: ID, payment: Coin<HANEUL>, ctx: &mut TxContext
    ): (T, TransferRequest<T>) {
    ...
    public fun purchase_with_cap<T: key + store>(
        self: &mut Kiosk, purchase_cap: PurchaseCap<T>, payment: Coin<HANEUL>, ctx: &mut TxContext
    ): (T, TransferRequest<T>) {
    ...
    public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<HANEUL> {
        assert!(object::id(self) == cap.for, ENotOwner);
        &mut self.profits
    }
...
module haneul::kiosk_tests {
    ...
    fun test_purchase() {
        ...
        let payment = coin::mint_for_testing<HANEUL>(AMT, ctx);
    ...
    #[test]
    #[expected_failure(abort_code = haneul::kiosk::EIncorrectAmount)]
    fun test_purchase_wrong_amount() {
        ...
        let payment = coin::mint_for_testing<HANEUL>(AMT + 1, ctx);
        let (asset, request) = kiosk::purchase(&mut kiosk, item_id, payment, ctx);
        policy::confirm_request(&mut policy, request);
        return_assets(vector[ asset ]);
        return_policy(policy, policy_cap, ctx);
    ...
    #[test]
    #[expected_failure(abort_code = haneul::kiosk::EAlreadyListed)]
    ...
    #[test]
    #[expected_failure(abort_code = haneul::kiosk::EListedExclusively)]
    ...
    #[test]
    #[expected_failure(abort_code = haneul::kiosk::ENotEmpty)]
