...
/// A helper `macro` that calls `haneul::transfer::party_transfer`.
public macro fun transfer<$T: key>($self: Party, $obj: $T) {
    let mp = $self;
    haneul::transfer::party_transfer($obj, mp)
}

/// A helper `macro` that calls `haneul::transfer::public_party_transfer`.
public macro fun public_transfer<$T: key + store>($self: Party, $obj: $T) {
    let mp = $self;
    haneul::transfer::public_party_transfer($obj, mp)
}
