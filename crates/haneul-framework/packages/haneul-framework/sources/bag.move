...
/// A bag is a heterogeneous map-like collection. The collection is similar to `haneul::table` in that
/// its keys and values are not stored within the `Bag` value, but instead are stored using Haneul's
...
/// `haneul::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
...
/// Aborts with `haneul::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
/// that key `k: K`.
...
/// that key `k: K`.
/// Aborts with `haneul::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
