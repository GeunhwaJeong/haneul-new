...
/// Similar to `haneul::table` but the values are linked together, allowing for ordered insertion and
/// removal
...
/// Aborts with `haneul::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
/// that key `k: K`.
...
/// Aborts with `haneul::dynamic_field::EFieldDoesNotExist` if the table does not have an entry
/// with the key `anchor: K`.
/// Aborts with `haneul::dynamic_field::EFieldAlreadyExists` if the table already has an entry
/// with the key `k: K`.
...
/// Aborts with `haneul::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
/// that key `k: K`.
