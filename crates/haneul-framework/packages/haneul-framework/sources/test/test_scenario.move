...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    haneul::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    haneul::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    haneul::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): haneul::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): haneul::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    haneul::transfer::make_receiver(object_id, version)
}
