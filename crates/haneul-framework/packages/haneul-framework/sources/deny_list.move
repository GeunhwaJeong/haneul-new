...
fun add_per_type_config(
    ...
    haneul::event::emit(PerTypeConfigCreated { key, config_id });
}
...
public struct PerTypeList has key, store {
    ...
    /// For example with `haneul::coin::Coin`: If addresses A and B are banned from using
...
fun create(ctx: &mut TxContext) {
    ...
    let deny_list_object = DenyList {
        id: object::haneul_deny_list_object_id(),
        lists,
    };
