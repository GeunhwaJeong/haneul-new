...
#[test_only]
module haneul::config_tests;

use std::unit_test::assert_eq;
use haneul::config::{Self, Config};
...
fun config_create<WriteCap>(cap: &mut WriteCap, ctx: &mut TxContext) {
    haneul::config::share(haneul::config::new(cap, ctx))
}
...
#[test, expected_failure(abort_code = haneul::config::EAlreadySetForEpoch)]
...
#[test, expected_failure(abort_code = haneul::config::ENotSetForEpoch)]
