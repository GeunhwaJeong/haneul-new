...
#[test_only]
module haneul::kiosk_marketplace_ext {
    use haneul::coin::Coin;
    use haneul::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use haneul::kiosk_extension as ext;
    use haneul::haneul::HANEUL;
    use haneul::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of HANEUL.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in HANEUL) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<HANEUL>) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<HANEUL> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module haneul::kiosk_extensions_tests {
    use haneul::kiosk_extension as ext;
    use haneul::kiosk_test_utils as test;
    ...
    #[test, expected_failure(abort_code = haneul::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = haneul::kiosk_extension::EExtensionNotInstalled)]
