...
#[test_only]
module haneul::funds_accumulator_tests;

use std::unit_test::assert_eq;
use haneul::balance::Balance;
use haneul::funds_accumulator::create_withdrawal;
...
#[random_test]
#[expected_failure(abort_code = haneul::funds_accumulator::EInvalidSubLimit)]
...
#[test]
#[expected_failure(abort_code = haneul::funds_accumulator::EOwnerMismatch)]
...
#[random_test]
#[expected_failure(abort_code = haneul::funds_accumulator::EOverflow)]
...
#[test]
#[expected_failure(abort_code = haneul::funds_accumulator::EObjectFundsWithdrawNotEnabled)]
fun test_withdraw_from_object_not_enabled() {
    let sender = @0x0;
    let mut scenario = test_scenario::begin(sender);
    let mut id = scenario.new_object();
    let withdrawal = haneul::funds_accumulator::withdraw_from_object<Balance<TestToken>>(&mut id, 100);
    let balance = haneul::balance::redeem_funds<TestToken>(withdrawal);
