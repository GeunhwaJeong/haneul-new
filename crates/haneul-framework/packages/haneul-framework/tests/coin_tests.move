...
module haneul::coin_tests;

use haneul::coin::{Self, Coin};
use haneul::deny_list;
use haneul::pay;
...
fun assert_global(
    ...
    use haneul::coin::{
        ...
        deny_list_v2_is_global_pause_enabled_current_epoch as is_global_pause_enabled_current_epoch,
    };
    assert!(
...
#[test]
fun deny_list_v2() {
    use haneul::coin::{deny_list_v2_add as add, deny_list_v2_remove as remove};
    let mut scenario = test_scenario::begin(@0);
    deny_list::create_for_testing(scenario.ctx());
    scenario.next_tx(TEST_ADDR);
...
#[test, expected_failure(abort_code = haneul::coin::EGlobalPauseNotAllowed)]
...
fun migrate_regulated_currency_to_v2_disallow_global_pause() {
    ...
    scenario.next_tx(TEST_ADDR);
    {
        ...
        deny_cap_v2 =
            coin::migrate_regulated_currency_to_v2(&mut deny_list, deny_cap, false, scenario.ctx());
        assert!(!coin::deny_list_contains<COIN_TESTS>(&deny_list, @100));
