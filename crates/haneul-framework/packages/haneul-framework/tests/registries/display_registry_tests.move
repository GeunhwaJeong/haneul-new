...
#[test_only]
module haneul::display_registry_tests;

use std::unit_test::assert_eq;
use haneul::display::{Self, new};
use haneul::display_registry::{Self, DisplayRegistry, Display, SystemMigrationCap, DisplayCap};
use haneul::package::{Self, test_claim, Publisher};
...
fun use_invalid_publisher() {
    test_tx!(|registry, scenario| {
        // Try claim display for `std` (external package) using `haneul`'s publisher.
