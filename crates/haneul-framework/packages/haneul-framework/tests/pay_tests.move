...
fun test_coin_split_n() {
    ...
    scenario.next_tx(TEST_SENDER_ADDR);
    let coin1 = scenario.take_from_sender<Coin<HANEUL>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    let coin2 = scenario.take_from_sender<Coin<HANEUL>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    assert_eq!(coin1.value(), 3);
    assert_eq!(coin2.value(), 3);
    assert_eq!(coin.value(), 4);
    assert!(!scenario.has_most_recent_for_sender<Coin<HANEUL>>(), 1);
...
fun test_split_and_transfer_fail() {
    ...
    scenario.next_tx(TEST_SENDER_ADDR);
    let coin_transfer_fail = scenario.take_from_sender<Coin<HANEUL>>();
