...
module haneul::object {
    ...
    /// Create the `Info` for the singleton `HaneulSystemState` object.
    /// This should only be called once from `haneul_system`.
    public(friend) fun haneul_system_state(): Info {
        Info {
    ...
    /// Create a new object. Returns the `Info` that must be stored in a Haneul object.
    ...
    // Safe because Haneul has an extra
