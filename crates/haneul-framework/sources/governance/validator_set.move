...
module haneul::validator_set {
    ...
    public(friend) fun request_add_delegation(
        ...
        let validator = get_validator_mut(&mut self.active_validators, validator_address);
        validator::request_add_delegation(validator, delegated_stake, locking_period, ctx);
        self.next_epoch_validators = derive_next_epoch_validators(self);
    }
    ...
    public(friend) fun request_switch_delegation(
        ...
        let (principal_stake, rewards_stake, time_lock) = 
            staking_pool::withdraw_all_to_haneul_tokens(validator::get_staking_pool_mut_ref(current_validator), delegation, staked_haneul);
        let withdraw_haneul_amount = balance::value(&principal_stake) + balance::value(&rewards_stake);
        validator::decrease_next_epoch_delegation(current_validator, withdraw_haneul_amount);
