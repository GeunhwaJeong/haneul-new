...
module haneul::validator_set {
    ...
    use haneul::epoch_reward_record;
    ...
    public(friend) fun request_remove_delegation(
        ...
        if (option::is_some(&validator_index_opt)) {
            ...
            // https://github.com/GeunhwaJeong/haneul/issues/2837
            abort 0
        };
        self.next_epoch_validators = derive_next_epoch_validators(self);
    }
    ...
    fun distribute_reward(validators: &mut vector<Validator>, rewards: &vector<u64>, reward: &mut Balance<HANEUL>) {
        let length = vector::length(validators);
        let i = 0;
        while (i < length) {
            let validator = vector::borrow_mut(validators, i);
