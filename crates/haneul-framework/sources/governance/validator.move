...
module haneul::validator {
    ...
    public(friend) fun request_add_delegation(
        ...
        staking_pool::request_add_delegation(
            &mut self.staking_pool, delegated_stake, locking_period, self.metadata.haneul_address, delegator_address, delegation_epoch, ctx
        );
    ...
    public(friend) fun request_set_gas_price(
        ...
        assert!(validator_address == self.metadata.haneul_address, EInvalidCap);
    ...
    public fun total_stake_amount(self: &Validator): u64 {
        spec {
            ...
            assume self.staking_pool.haneul_balance <= MAX_U64;
        };
        staking_pool::haneul_balance(&self.staking_pool)
    }
    ...
    public(friend) fun new_unverified_validator_operation_cap_and_transfer(self: &mut Validator, ctx: &mut TxContext) {
        ...
        assert!(address == self.metadata.haneul_address, ENewCapNotCreatedByValidatorItself);
    ...
    fun new_from_metadata(
        ...
        let haneul_address = metadata.haneul_address;
