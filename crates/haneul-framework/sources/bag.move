...
/// A bag is a heterogenous map-like collection. The collection is similar to `haneul::table` in that
/// it's keys and values are not stored within the `Bag` value, but instead are stored using Haneul's
