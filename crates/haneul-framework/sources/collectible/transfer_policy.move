...
module haneul::transfer_policy {
    ...
    struct TransferPolicy<phantom T: key + store> has key, store {
        id: UID,
        /// The Balance of the `TransferPolicy` which collects `HANEUL`.
    ...
    /// Special case for the `haneul::collectible` module to be able to register a
    ...
    /// Add some `HANEUL` to the balance of a `TransferPolicy`.
    public fun add_to_balance<T: key + store, Rule: drop>(
        _: Rule, policy: &mut TransferPolicy<T>, coin: Coin<HANEUL>
    ) {
...
module haneul::fixed_commission {
    use haneul::haneul::HANEUL;
...
#[test_only]
module haneul::malicious_policy {
    use haneul::transfer_policy::{Self as policy, TransferRequest};
...
module haneul::transfer_policy_test {
    ...
    #[test]
    #[expected_failure(abort_code = haneul::transfer_policy::EPolicyNotSatisfied)]
    ...
    #[test]
    #[expected_failure(abort_code = haneul::transfer_policy::ERuleAlreadySet)]
    ...
    #[test]
    #[expected_failure(abort_code = haneul::transfer_policy::EIllegalRule)]
