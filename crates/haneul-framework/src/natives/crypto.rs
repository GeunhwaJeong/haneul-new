...
pub const INVALID_VERIFYING_KEY: u64 = 7;
pub const INVALID_VERIFYING_RESULT: u64 = 8;
...
pub fn ecrecover(
    ...
    match recover_pubkey(&signature_ref, &hashed_msg_ref) {
        ...
        Err(HaneulError::InvalidSignature { error: _ }) => {
...
fn recover_pubkey(signature: &[u8], hashed_msg: &[u8]) -> Result<Secp256k1PublicKey, HaneulError> {
    match <Secp256k1Signature as ToFromBytes>::from_bytes(signature) {
        Ok(signature) => match signature.recover(hashed_msg) {
            Ok(pubkey) => Ok(pubkey),
            Err(e) => Err(HaneulError::KeyConversionError(e.to_string())),
        },
        Err(e) => Err(HaneulError::InvalidSignature {
            error: e.to_string(),
        }),
    }
}
...
/// Native implementation of preparing a verifying key for groth16 verifier.
...
/// Native implementation of verifying a groth16 proof from the prepared verifying key, public inputs and proof.
    ...
    match verify_groth16_in_bytes(
        ...
        Ok(res) => {
            if res {
                ...
                Ok(NativeResult::err(cost, INVALID_VERIFYING_RESULT))
            }
        }
        Err(_) => Ok(NativeResult::ok(cost, smallvec![Value::bool(false)])),
    }
