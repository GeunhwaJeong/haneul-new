...
fn account_to_haneul_address(address: AccountAddress) -> HaneulAddress {
    HaneulAddress::try_from(address.as_slice()).unwrap()
}
...
fn get_new_owner(event_type: &EventType, recipient: Vec<u8>) -> Owner {
    match event_type {
        ...
        EventType::TransferToAddress => {
            Owner::AddressOwner(HaneulAddress::try_from(recipient).unwrap())
        }
        EventType::TransferToObject => Owner::ObjectOwner(HaneulAddress::try_from(recipient).unwrap()),
        ...
        .filter(|(_, obj)| {
            &obj.type_ == type_
                && if let Some(parent) = parent_object {
                    ...
                    obj.owner
                        == Owner::ObjectOwner(HaneulAddress::try_from(parent.as_slice()).unwrap())
...
pub fn get_account_owned_inventory(
    ...
    match get_inventory_for(
        Owner::AddressOwner(account_to_haneul_address(owner_address)),
        None,
        &ty_args[0],
        tx_end_index,
        context.events(),
...
pub fn get_object_owned_inventory(
    ...
    let cost = legacy_emit_cost();
    match get_inventory_for(
        Owner::AddressOwner(account_to_haneul_address(signer_address)),
        Some(parent_object),
        &ty_args[0],
        tx_end_index,
        context.events(),
