...
use haneul_protocol_constants::MAX_NUM_EVENT_EMIT;
use haneul_types::{
...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<HaneulAddress, BTreeMap<Type, Set<ObjectID>>>,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn transfer(
        ...
        //   HANEUL_SYSTEM_STATE_OBJECT_ID which is only transferred in genesis
        ...
        //   child object
        let transfer_result =
            if self.state.new_ids.contains_key(&id) || id == HANEUL_SYSTEM_STATE_OBJECT_ID {
                TransferResult::New
            ...
            .map(|(id, (owner, type_, tag, value))| {
                let write_kind =
                    if input_objects.contains_key(&id) || loaded_child_objects.contains_key(&id) {
                        ...
                        WriteKind::Mutate
                    } else if id == HANEUL_SYSTEM_STATE_OBJECT_ID || new_ids.contains_key(&id) {
                        // HANEUL_SYSTEM_STATE_OBJECT_ID is only transferred during genesis
