...
pub fn get_haneul_framework_modules(lib_dir: &Path) -> HaneulResult<Vec<CompiledModule>> {
    let modules = build_framework(lib_dir)?;
    verify_modules(&modules)?;
    Ok(modules)
}

pub fn get_move_stdlib_modules(lib_dir: &Path) -> HaneulResult<Vec<CompiledModule>> {
    let denylist = vec![
...
pub fn build_move_package(
    ...
    is_framework: bool,
) -> HaneulResult<Vec<CompiledModule>> {
    match build_config.compile_package(path, &mut Vec::new()) {
        Err(error) => Err(HaneulError::ModuleBuildFailure {
        ...
        Ok(package) => {
            ...
            {
                return Err(HaneulError::ModulePublishFailure { error: format!("Dependent modules must have been published on-chain with non-0 addresses, unlike module {:?}", m.self_id()) });
            }
            Ok(package
                .all_modules_map()
...
pub fn build_and_verify_user_package(path: &Path) -> HaneulResult<Vec<CompiledModule>> {
...
fn verify_modules(modules: &[CompiledModule]) -> HaneulResult {
    for m in modules {
...
pub fn run_move_unit_tests(path: &Path, config: Option<UnitTestingConfig>) -> HaneulResult {
    ...
    .map_err(|err| HaneulError::MoveUnitTestFailure {
        error: format!("{:?}", err),
    })?;
    if result == UnitTestResult::Failure {
        Err(HaneulError::MoveUnitTestFailure {
...
fn run_framework_move_unit_tests() {
    get_haneul_framework_modules(&PathBuf::from(DEFAULT_FRAMEWORK_PATH)).unwrap();
...
fn run_examples_move_unit_tests() {
    ...
    for example in examples {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../haneul_programmability/examples")
            .join(example);
