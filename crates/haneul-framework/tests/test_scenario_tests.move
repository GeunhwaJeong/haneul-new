...
module haneul::test_scenarioTests {
    ...
    fun test_dynamic_field_still_borrowed() {
        ...
        haneul::dynamic_field::add(&mut parent, b"", 10);
        let r = haneul::dynamic_field::borrow<vector<u8>, u64>(&mut parent, b"");
        ts::end(scenario);
        assert!(*r == 10, 0);
    ...
    fun test_dynamic_object_field_still_borrowed() {
        ...
        let parent = ts::new_object(&mut scenario);
        let id = ts::new_object(&mut scenario);
        haneul::dynamic_object_field::add(&mut parent, b"", Object { id, value: 10});
        let obj = haneul::dynamic_object_field::borrow<vector<u8>, Object>(&mut parent, b"");
        ts::end(scenario);
        assert!(obj.value == 10, 0);
        object::delete(parent);
    }
    ...
    fun test_dynamic_object_field_not_retrievable() {
        ...
        assert!(!ts::has_most_recent_for_address<Object>(sender), 0);
        haneul::dynamic_object_field::add(&mut parent, b"", obj);
        ts::next_tx(&mut scenario, sender);
        assert!(!ts::has_most_recent_for_address<Object>(sender), 0);
        ts::end(scenario);
        object::delete(parent);
    ...
    fun test_dynamic_field_shared_misuse() {
        ...
        assert!(object::id(&obj) == id, 0);
        // wraps the object
        haneul::dynamic_field::add(&mut parent, b"", obj);
        ts::next_tx(&mut scenario, sender);
        abort 42
    }
