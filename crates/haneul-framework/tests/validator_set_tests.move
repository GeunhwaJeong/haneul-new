...
module haneul::validator_set_tests {
    ...
    fun test_add_validator_failure_below_min_stake() {
        ...
        // Validator 2 now has 700 GEUNHWA in stake but that's not enough because we need 701.
    ...
    fun test_add_validator_with_nonzero_min_stake() {
        ...
        test_scenario::next_tx(scenario, @0x2);
        // Validator 2 now has 700 GEUNHWA in stake and that's just enough.
    ...
    fun test_low_stake_departure() {
        ...
        let ctx1 = test_scenario::ctx(scenario);
        // Create 4 validators.
        let v1 = create_validator(@0x1, 1, 1, true, ctx1); // 100 GEUNHWA of stake
        let v2 = create_validator(@0x2, 4, 1, true, ctx1); // 400 GEUNHWA of stake
        let v3 = create_validator(@0x3, 10, 1, true, ctx1); // 1000 GEUNHWA of stake
        let v4 = create_validator(@0x4, 4, 1, true, ctx1); // 400 GEUNHWA of stake
