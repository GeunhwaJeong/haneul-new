...
fn check_diff(checked_in: &Path, built: &Path) {
    for path in [COMPILED_PACKAGES_DIR, DOCS_DIR, PUBLISHED_API_FILE] {
        ...
        if !output.status.success() {
            let header = "Generated and checked-in haneul-framework packages and/or docs do not match.\n\
...
async fn build_packages(
    ...
    let config = MoveBuildConfig {
        ...
        default_flavor: Some(Flavor::Haneul),
        ..Default::default()
    };
...
async fn build_packages_with_move_config(
    ...
    let haneul_system_members =
        serialize_modules_to_file(haneul_system, &compiled_packages_dir.join(system_dir)).unwrap();
    let haneul_framework_members =
        serialize_modules_to_file(haneul_framework, &compiled_packages_dir.join(framework_dir))
            .unwrap();
    ...
    let published_api = [
        haneul_system_members.join("\n"),
        haneul_framework_members.join("\n"),
