...
module haneul::haneul_system_tests {
    ...
    use haneul::governance_test_utils::{add_validator, advance_epoch, remove_validator, set_up_haneul_system_state, create_haneul_system_state_for_testing};
    use haneul::haneul_system::{Self, HaneulSystemState};
    use haneul::validator::Self;
    use haneul::vec_set;
    use haneul::table;
    use std::vector;
    use haneul::coin;
    ...
    fun report_helper(reporter: address, reported: address, is_undo: bool, scenario: &mut Scenario) {
        ...
        if (is_undo) {
            haneul_system::undo_report_validator(&mut system_state, reported, ctx);
        } else {
            haneul_system::report_validator(&mut system_state, reported, ctx);
        };
        test_scenario::return_shared(system_state);
    }
    ...
    fun update_metadata(
        ...
        haneul_system::update_validator_next_epoch_consensus_address(system_state, vector[4, 168, 168, 168, 168], ctx);
    ...
    fun test_active_validator_update_metadata() {
        ...
        let validator = validator::new_for_testing(
            ...
            vector[4, 127, 0, 0, 1],
            vector[4, 127, 0, 0, 1],
            balance::create_for_testing<HANEUL>(100),
            option::none(),
            1,
            0,
            0,
