...
// This file contains tests testing functionalities in `haneul_system` that are not
...
module haneul::haneul_system_tests {
    ...
    fun test_validator_ops_by_stakee_ok() {
        let scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_haneul_system_state(vector[@0x1, @0x2], scenario);
    ...
    fun report_helper(sender: address, reported: address, is_undo: bool, scenario: &mut Scenario) {
        ...
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        if (is_undo) {
            haneul_system::undo_report_validator(&mut system_state, &cap, reported);
        } else {
            haneul_system::report_validator(&mut system_state, &cap, reported);
        };
    ...
    fun set_gas_price_helper(
        ...
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        let system_state = test_scenario::take_shared<HaneulSystemState>(scenario);
        haneul_system::request_set_gas_price(&mut system_state, &cap, new_gas_price);
        test_scenario::return_to_sender(scenario, cap);
        test_scenario::return_shared(system_state);
    }
    ...
    fun rotate_operation_cap(sender: address, scenario: &mut Scenario) {
        ...
        haneul_system::rotate_operation_cap(&mut system_state, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    fun update_candidate(
        scenario: &mut Scenario,
        system_state: &mut HaneulSystemState,
        name: vector<u8>,
        ...
        let ctx = test_scenario::ctx(scenario);
        haneul_system::update_validator_name(system_state, name, ctx);
        haneul_system::update_validator_description(system_state, b"new_desc", ctx);
        haneul_system::update_validator_image_url(system_state, b"new_image_url", ctx);
        haneul_system::update_validator_project_url(system_state, b"new_project_url", ctx);
        haneul_system::update_candidate_validator_network_address(system_state, network_address, ctx);
        haneul_system::update_candidate_validator_p2p_address(system_state, p2p_address, ctx);
        haneul_system::update_candidate_validator_primary_address(system_state, b"/ip4/127.0.0.1/udp/80", ctx);
        haneul_system::update_candidate_validator_worker_address(system_state, b"/ip4/127.0.0.1/udp/80", ctx);
        haneul_system::update_candidate_validator_protocol_pubkey(
            system_state,
            protocol_pub_key,
            pop,
            ctx
        );
        haneul_system::update_candidate_validator_worker_pubkey(system_state, vector[68, 55, 206, 25, 199, 14, 169, 53, 68, 92, 142, 136, 174, 149, 54, 215, 101, 63, 249, 206, 197, 98, 233, 80, 60, 12, 183, 32, 216, 88, 103, 25], ctx);
        haneul_system::update_candidate_validator_network_pubkey(system_state, vector[32, 219, 38, 23, 242, 109, 116, 235, 225, 192, 219, 45, 40, 124, 162, 25, 33, 68, 52, 41, 123, 9, 98, 11, 184, 150, 214, 62, 60, 210, 121, 62], ctx);

        haneul_system::set_candidate_validator_commission_rate(system_state, commission_rate, ctx);
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        haneul_system::set_candidate_validator_gas_price(system_state, &cap, gas_price);
        test_scenario::return_to_sender(scenario, cap);
    }
    ...
    // Note: `pop` MUST be a valid signature using haneul_address and protocol_pubkey_bytes.
    ...
    fun update_metadata(
        ...
        haneul_system::update_validator_image_url(system_state, b"new_image_url", ctx);
        haneul_system::update_validator_project_url(system_state, b"new_project_url", ctx);
        haneul_system::update_validator_next_epoch_network_address(system_state, network_address, ctx);
        haneul_system::update_validator_next_epoch_p2p_address(system_state, p2p_address, ctx);
        haneul_system::update_validator_next_epoch_primary_address(system_state, b"/ip4/168.168.168.168/udp/80", ctx);
        haneul_system::update_validator_next_epoch_worker_address(system_state, b"/ip4/168.168.168.168/udp/80", ctx);
        haneul_system::update_validator_next_epoch_protocol_pubkey(
            system_state,
            protocol_pub_key,
            pop,
            ctx
        );
        haneul_system::update_validator_next_epoch_worker_pubkey(system_state, vector[215, 64, 85, 185, 231, 116, 69, 151, 97, 79, 4, 183, 20, 70, 84, 51, 211, 162, 115, 221, 73, 241, 240, 171, 192, 25, 232, 106, 175, 162, 176, 43], ctx);
        haneul_system::update_validator_next_epoch_network_pubkey(system_state, vector[148, 117, 212, 171, 44, 104, 167, 11, 177, 100, 4, 55, 17, 235, 117, 45, 117, 84, 159, 49, 14, 159, 239, 246, 237, 21, 83, 166, 112, 53, 62, 199], ctx);
    }
    ...
    fun test_active_validator_update_metadata() {
        ...
        // Set up HaneulSystemState with an active validator
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<HANEUL>(100)),
            1,
            0,
            true,
            ctx
    ...
    fun test_validator_candidate_update() {
        ...
        test_scenario::next_tx(scenario, validator_addr);
        {
            haneul_system::request_add_validator_candidate_for_testing(
                &mut system_state,
                pubkey,
                vector[215, 64, 85, 185, 231, 116, 69, 151, 97, 79, 4, 183, 20, 70, 84, 51, 211, 162, 115, 221, 73, 241, 240, 171, 192, 25, 232, 106, 175, 162, 176, 43],
                vector[148, 117, 212, 171, 44, 104, 167, 11, 177, 100, 4, 55, 17, 235, 117, 45, 117, 84, 159, 49, 14, 159, 239, 246, 237, 21, 83, 166, 112, 53, 62, 199],
    ...
    fun test_add_validator_candidate_failure_duplicate_with_active() {
        ...
        create_haneul_system_state_for_testing(vector[validator], 1000, 0, ctx);
