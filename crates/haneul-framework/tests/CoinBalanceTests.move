...
module Haneul::TestCoin {
    ...
    use Haneul::LockedCoin::LockedCoin;
    use Haneul::TxContext;
    use Haneul::LockedCoin;
    ...
    public entry fun test_locked_coin_valid() {
        ...
        let coin = Coin::mint_for_testing<HANEUL>(42, ctx);
        ...
        let locked_coin = TestScenario::take_owned<LockedCoin<HANEUL>>(scenario);
        // The unlock should go through since epoch requirement is met.
        LockedCoin::unlock_coin(locked_coin, TestScenario::ctx(scenario));

        TestScenario::next_tx(scenario, &TEST_RECIPIENT_ADDR);
        let unlocked_coin = TestScenario::take_owned<Coin<HANEUL>>(scenario);
