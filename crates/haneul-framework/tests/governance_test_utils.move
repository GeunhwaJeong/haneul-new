...
module haneul::governance_test_utils {
    ...
    public fun set_up_haneul_system_state(addrs: vector<address>, scenario: &mut Scenario) {
    ...
    public fun advance_epoch_with_reward_amounts(
        ...
        haneul_system::advance_epoch(&mut system_state, new_epoch, 1, storage_charge, computation_charge, 0, 0, 0, 0, 0, ctx);
    ...
    public fun advance_epoch_with_reward_amounts_and_subsidy_rate(
        ...
        haneul_system::advance_epoch(&mut system_state, new_epoch, 1, storage_charge, computation_charge, 0, 0, 0, stake_subsidy_rate, 0, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    public fun add_validator(validator: address, init_stake_amount: u64, scenario: &mut Scenario) {
        ...
        haneul_system::request_add_validator(
            ...
            x"",
            x"",
            coin::mint_for_testing<HANEUL>(init_stake_amount, ctx),
            1,
            0,
            ctx
        );
    ...
    public fun assert_validator_delegate_amounts(validator_addrs: vector<address>, delegate_amounts: vector<u64>, scenario: &mut Scenario) {
        ...
        while (i < vector::length(&validator_addrs)) {
            ...
            let system_state = test_scenario::take_shared<HaneulSystemState>(scenario);
            assert!(haneul_system::validator_delegate_amount(&mut system_state, validator_addr) == amount, 0);
            test_scenario::return_shared(system_state);
            i = i + 1;
        };
    }
