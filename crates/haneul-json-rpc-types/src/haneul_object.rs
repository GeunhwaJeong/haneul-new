...
use haneul_types::messages::MoveModulePublish;
...
impl HaneulObjectDataOptions {
    ...
    pub fn full_content() -> Self {
        Self {
            show_bcs: Some(false),
            // Skip because this is inside the HaneulRawData
            show_type: Some(true),
            show_owner: Some(true),
            show_previous_transaction: Some(true),
            // uncomment the following in the next PR
...
impl Default for HaneulObjectDataOptions {
    fn default() -> Self {
        Self {
            show_type: Some(false),
            show_owner: Some(false),
...
impl TryFrom<(ObjectRead, Option<HaneulObjectDataOptions>)> for HaneulObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_read, options): (ObjectRead, Option<HaneulObjectDataOptions>),
...
impl
    TryFrom<(
        ...
        Option<HaneulObjectDataOptions>,
    )> for HaneulObjectData
{
    ...
    fn try_from(
        ...
        let default_options = HaneulObjectDataOptions::default();
...
impl TryFrom<PastObjectRead> for HaneulPastObjectResponse {
    type Error = anyhow::Error;
