...
impl Ord for HaneulObjectResponse {
    fn cmp(&self, other: &Self) -> Ordering {
        match (&self.data, &other.data) {
            ...
            // In this ordering those with data will come before HaneulObjectResponses that are errors.
            (Some(_), None) => Ordering::Less,
            (None, Some(_)) => Ordering::Greater,
            // HaneulObjectResponses that are errors are just considered equal.
            _ => Ordering::Equal,
        }
    }
}

impl PartialOrd for HaneulObjectResponse {
...
impl HaneulObjectResponse {
    pub fn move_object_bcs(&self) -> Option<&Vec<u8>> {
        match &self.data {
            Some(HaneulObjectData {
                bcs: Some(HaneulRawData::MoveObject(obj)),
                ..
            }) => Some(&obj.bcs_bytes),
            _ => None,
        }
...
impl TryFrom<HaneulObjectResponse> for ObjectInfo {
    type Error = anyhow::Error;

    fn try_from(value: HaneulObjectResponse) -> Result<Self, Self::Error> {
        let HaneulObjectData {
            object_id,
            version,
            digest,
            type_,
...
pub struct HaneulObjectData {
    ...
    /// Move object content or package content in BCS, default to be None unless HaneulObjectDataOptions.showBcs is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bcs: Option<HaneulRawData>,
}

impl HaneulObjectData {
...
impl Display for HaneulObjectData {
...
impl TryFrom<&HaneulObjectData> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(object: &HaneulObjectData) -> Result<Self, Self::Error> {
        match &object
            .content
            .as_ref()
...
impl TryFrom<(ObjectRead, HaneulObjectDataOptions)> for HaneulObjectResponse {
    type Error = anyhow::Error;
...
impl TryFrom<(ObjectInfo, HaneulObjectDataOptions)> for HaneulObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_info, options): (ObjectInfo, HaneulObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        let HaneulObjectDataOptions {
            show_type,
            show_owner,
    ...
    )> for HaneulObjectData
{
    ...
    fn try_from(
        ...
        let bcs: Option<HaneulRawData> = if show_bcs {
    ...
    )> for HaneulObjectData
{
    ...
    fn try_from(
        ...
        let mut data: HaneulObjectData = (object_ref, o, layout, options).try_into()?;
        if show_display {
...
impl TryInto<Object> for HaneulObjectData {
    type Error = anyhow::Error;
...
impl Display for HaneulObjectRef {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
...
impl HaneulData for HaneulParsedData {
    ...
    fn try_from_package(package: MovePackage) -> Result<Self, anyhow::Error> {
        ...
        Ok(Self::Package(HaneulMovePackage { disassembled }))
    }
...
pub type QueryObjectsPage = Page<HaneulObjectResponse, CheckpointedObjectID>;
pub type ObjectsPage = Page<HaneulObjectResponse, ObjectID>;
    ...
    pub fn matches(&self, object: &ObjectInfo) -> bool {
        match self {
            ...
            HaneulObjectDataFilter::StructType(s) => {
                ...
                // e.g. `0x2::coin::Coin` can match `0x2::coin::Coin<0x2::haneul::HANEUL>`
            ...
            HaneulObjectDataFilter::ObjectOwner(o) => {
                matches!(object.owner, Owner::ObjectOwner(addr) if addr == HaneulAddress::from(*o))
            }
            HaneulObjectDataFilter::ObjectId(id) => &object.object_id == id,
            HaneulObjectDataFilter::ObjectIds(ids) => ids.contains(&object.object_id),
            HaneulObjectDataFilter::Version(v) => object.version.value() == *v,
        }
    }
}
...
pub struct HaneulObjectResponseQuery {
...
impl HaneulObjectResponseQuery {
    ...
    pub fn new_with_filter(filter: HaneulObjectDataFilter) -> Self {
        Self {
            filter: Some(filter),
            options: None,
        }
    ...
    pub fn new_with_options(options: HaneulObjectDataOptions) -> Self {
        Self {
            filter: None,
