...
use crate::{
    HaneulArgument, HaneulCallArg, HaneulCommand, HaneulObjectArg, HaneulProgrammableMoveCall,
    HaneulProgrammableTransactionBlock,
};
use haneul_types::transaction::write_sep;
use tabled::{
        ...
        if !inputs.is_empty() {
            ...
            for (i, input) in inputs.iter().enumerate() {
                match input {
                    ...
                    HaneulCallArg::FundsWithdrawal(arg) => {
...
impl Display for Pretty<'_, HaneulCommand> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match command {
            HaneulCommand::MakeMoveVec(ty_opt, elems) => {
                write!(f, "MakeMoveVec:\n ┌")?;
            ...
            HaneulCommand::MoveCall(p) => write!(f, "{}", Pretty(&**p)),

            HaneulCommand::MergeCoins(target, coins) => {
                write!(
                    f,
                    "MergeCoins:\n ┌\n │ Target: {}\n │ Coins: \n │   ",
                    Pretty(target)
            ...
            HaneulCommand::Publish(deps) => {
                write!(f, "Publish:\n ┌\n │ Dependencies: \n │   ")?;
                write_sep(f, deps, "\n │   ")?;
                write!(f, "\n └")
            }

            HaneulCommand::TransferObjects(objs, addr) => {
                write!(f, "TransferObjects:\n ┌\n │ Arguments: \n │   ")?;
                write_sep(f, objs.iter().map(Pretty), "\n │   ")?;
                write!(f, "\n │ Address: {}\n └", Pretty(addr))
            }

            HaneulCommand::Upgrade(deps, current_package_id, ticket) => {
                write!(f, "Upgrade:\n ┌\n │ Dependencies: \n │   ")?;
                write_sep(f, deps, "\n │   ")?;
...
impl Display for Pretty<'_, HaneulArgument> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        let output = match argument {
            HaneulArgument::GasCoin => "GasCoin".to_string(),
            HaneulArgument::Input(i) => format!("Input  {}", i),
            HaneulArgument::Result(i) => format!("Result {}", i),
            HaneulArgument::NestedResult(j, k) => format!("Nested Result {}: {}", j, k),
        };
        write!(f, "{}", output)
    }
}
