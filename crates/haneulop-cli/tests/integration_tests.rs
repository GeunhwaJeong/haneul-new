...
use haneuloplib::cli::service::init;
use tracing::debug;
...
#[cfg(test)]
#[test]
fn test_initialize_service_haneul() -> Result<()> {
    // create a temp dir to work in
    let temp_dir = tempfile::tempdir().expect("creating temp dir");
    let svc_dir = temp_dir.path().join("haneul/crates/svc/");
    std::fs::create_dir_all(&svc_dir).context("creating nested dir")?;
    debug!("svc_dir: {:?}", svc_dir);
    // Create a dummy Cargo.toml file at the tempdir/haneul level
    let workspace_toml_path = temp_dir.path().join("haneul/Cargo.toml");
    std::fs::write(
        workspace_toml_path,
        r#"
[workspace]
members = []
  "#,
    ...
    // Create a dummy Dockerfile at the tempdir/haneul/docker/haneul-services level
    let haneul_services_dockerfile_path = temp_dir.path().join("haneul/docker/haneul-services/Dockerfile");
    std::fs::create_dir_all(haneul_services_dockerfile_path.parent().unwrap())?;
    std::fs::write(
        &haneul_services_dockerfile_path,
    ...
    // actually in the Haneul repo, we'll check that the Cargo.toml file was
    ...
    let cargo_haneul_toml_content =
        std::fs::read_to_string("../haneullabs-service-boilerplate/Cargo-haneul.toml")
            .context("reading cargo toml from boilerplate")?;

    assert_eq!(
        toml_content,
        // replace the service name, everything else should be the same
        cargo_haneul_toml_content.replace("service-boilerplate", "svc")
    );
    Ok(())
}
