...
#[derive(Parser, Debug)]
#[command(author="build@haneul-labs.com", version, about, long_about = None)]
pub(crate) struct HaneulOpArgs {
...
async fn main() -> Result<()> {
    ...
    // Load environment variables from ~/.haneulop/env_vars
    ...
    if let Ok(env_contents) = std::fs::read_to_string(env_file_path) {
        ...
        warn!("Warning: Could not read ~/.haneulop/env_vars file. Environment variables not loaded.");
    }
