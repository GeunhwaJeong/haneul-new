...
/// Add the new service to the haneul-services dockerfile in the haneul repository
fn add_to_haneul_dockerfile(path: &Path) -> Result<()> {
    let path = path.canonicalize().context("canonicalizing service path")?;
    let crates_dir = path.parent().unwrap();
    if !crates_dir.ends_with("haneul/crates") {
        panic!("directory wasn't in the haneul repo");
    }
    let haneul_services_dockerfile_path = &crates_dir.join("../docker/haneul-services/Dockerfile");
    // read the dockerfile
    let dockerfile = fs::read_to_string(haneul_services_dockerfile_path)
        .context("reading haneul-services dockerfile")?;
    ...
    let build_line = dockerfile
        ...
        .expect("couldn't find build line in haneul-services dockerfile")
        .0;
    ...
    // write the file back
    fs::write(haneul_services_dockerfile_path, final_dockerfile.join("\n"))
        .context("writing haneul-services dockerfile after modifying it")?;
...
fn create_rust_service(path: &Path) -> Result<()> {
    ...
    if is_haneul_service {
        add_member_to_workspace(path).context("adding crate to haneul workspace")?;
    }
    // now that the source directory works, let's update/add a dockerfile
    if is_haneul_service {
        // update haneul-services dockerfile
        add_to_haneul_dockerfile(path).context("adding crate to haneul services dockerfile")?;
    } else {
