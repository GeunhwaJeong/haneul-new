...
fn get_pulumi_dir() -> Result<PathBuf> {
    let haneulops_dir_stdout = run_cmd(
        vec!["git", "rev-parse", "--show-toplevel"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command from within the haneul-operations repository")?
    .stdout;
    let haneulops_dir = PathBuf::from(String::from_utf8_lossy(&haneulops_dir_stdout).trim());
    Ok(haneulops_dir.join("pulumi"))
}
...
fn run_pulumi_new(
    ...
    run_cmd(
        vec![
            ...
            "-c",
            &format!(
                r#"pulumi new {runtime_arg} --dir {0} -d "pulumi project for {1}" --name "{1}"  --stack haneullabs/dev --yes {2}"#,
...
fn run_pulumi_new_from_template(
    ...
    info!("extra pulumi options added: {}", &opts.bright_purple());
    let cmd = &format!(
        r#"pulumi new {3}/templates/{2} --dir {0} -d "pulumi project for {1}" --name "{1}"  --stack haneullabs/dev {4}"#,
        project_dir_str,
        project_name,
        template_dir,
        get_pulumi_dir()?
...
fn create_basic_project(
    ...
    run_pulumi_new(project_name, project_dir_str, project_opts, runtime).inspect_err(|_| {
        ...
        remove_stack(&backend, project_name, "haneullabs/dev").unwrap();
    })?;
...
fn create_haneullabs_k8s_project(
    project_name: &str,
    project_dir: &PathBuf,
    project_type: ProjectType,
    project_opts: &[String],
