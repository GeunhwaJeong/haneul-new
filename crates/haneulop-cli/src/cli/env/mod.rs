...
pub fn setup_pulumi_environment(environment_name: &str) -> Result<()> {
    ...
    let haneulop_dir = format!("{}/.haneulop", home_dir);
    std::fs::create_dir_all(&haneulop_dir).expect("Failed to create .haneulop directory");
    let env_file_path = format!("{}/env_vars", haneulop_dir);
    let mut env_file =
