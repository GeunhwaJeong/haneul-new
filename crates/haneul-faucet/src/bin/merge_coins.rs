...
async fn _split_coins_equally(
    ...
    let signature = wallet
        .config
        .keystore
        .sign_secure(&active_address, &tx_data, Intent::haneul_transaction())
        .unwrap();
...
async fn _merge_coins(gas_coin: &str, mut wallet: WalletContext) -> Result<(), anyhow::Error> {
    ...
    for chunk in small_coins.chunks(254) {
        ...
        let tx_data = client
            .transaction_builder()
            .pay_haneul(active_address, coin_vector, target, target_amount, 1000000)
            .await?;
        let signature = wallet
            .config
            .keystore
