...
use haneul_json_rpc_types::{HaneulExecutionStatus, HaneulParsedObject};
use haneul_types::{
...
impl SimpleFaucet {
    ...
    async fn get_coins(
        &self,
        amounts: &[u64],
    ) -> Result<(Vec<HaneulParsedObject>, TransactionDigest), FaucetError> {
    ...
    async fn split_coins(
        ...
        budget: u64,
    ) -> Result<(Vec<HaneulParsedObject>, TransactionDigest), anyhow::Error> {
