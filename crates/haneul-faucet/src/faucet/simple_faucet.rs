...
use haneul_json_rpc_types::{
    OwnedObjectRef, HaneulObjectDataOptions, HaneulTransactionBlockEffectsAPI,
...
pub struct SimpleFaucet {
    ...
    request_producer: Sender<(Uuid, HaneulAddress, Vec<u64>)>,
...
impl SimpleFaucet {
    pub async fn new(
        ...
        let (sender, mut receiver) =
            mpsc::channel::<(Uuid, HaneulAddress, Vec<u64>)>(config.max_request_queue_length as usize);
    ...
    async fn sign_and_execute_txn(
        ...
        let signature = self
            ...
            .config
            .keystore
            .sign_secure(&self.active_address, &tx_data, Intent::haneul_transaction())
            .map_err(FaucetError::internal)?;
        ...
        info!(
            ...
            ?coin_id,
            ?uuid,
            "PayHaneul transaction in faucet."
        );

        match timeout(
            Duration::from_secs(300),
            self.execute_pay_haneul_txn_with_retries(&tx, coin_id, recipient, uuid),
        )
        .await
        {
            Err(elapsed) => {
                warn!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Failed to execute PayHaneul transactions in faucet after {elapsed}. Coin will \
                     not be reused."
                );
    ...
    async fn transfer_gases(
        ...
        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = self
                    .build_pay_haneul_txn(coin_id, self.active_address, recipient, amounts, gas_cost)
                    .await
                    .map_err(FaucetError::internal)?;
    ...
    async fn execute_pay_haneul_txn_with_retries(
        &self,
        tx: &Transaction,
        coin_id: ObjectID,
        recipient: HaneulAddress,
        ...
        loop {
            let res = self.execute_pay_haneul_txn(tx, coin_id, recipient, uuid).await;
            ...
            info!(
                ...
                ?uuid,
                ?retry_delay,
                "PayHaneul transaction in faucet failed, previous error: {:?}",
                &res,
            );
    ...
    async fn execute_pay_haneul_txn(
        &self,
        tx: &Transaction,
        coin_id: ObjectID,
        recipient: HaneulAddress,
    ...
    async fn build_pay_haneul_txn(
        ...
        client
            .transaction_builder()
            .pay_haneul(signer, vec![coin_id], recipients, amounts.to_vec(), budget)
            .await
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to build PayHaneul transaction for coin {:?}, with err {:?}",
                    coin_id,
                    e
                )
            })
    ...
    async fn build_batch_pay_haneul_txn(
        &self,
        coin_id: ObjectID,
        batch_requests: Vec<(Uuid, HaneulAddress, Vec<u64>)>,
        signer: HaneulAddress,
        budget: u64,
    ) -> Result<TransactionData, anyhow::Error> {
    ...
    async fn check_and_map_batch_transfer_gas_result(
        &self,
        res: HaneulTransactionBlockResponse,
        requests: Vec<(Uuid, HaneulAddress, Vec<u64>)>,
    ) -> Result<(), FaucetError> {
        // Grab the list of created coins and turn it into a map of destination HaneulAddress to Vec<Coins>
        let created = res
            .effects
            .ok_or_else(|| {
                FaucetError::ParseTransactionResponseError(format!(
        ...
        let mut address_coins_map: HashMap<HaneulAddress, Vec<OwnedObjectRef>> = HashMap::new();
        ...
        // Assert that the number of times a haneul_address occurs is the number of times the coins
        // come up in the vector.
        let mut request_count: HashMap<HaneulAddress, u64> = HashMap::new();
        ...
        for (uuid, addy, amounts) in requests {
            let number_of_coins = amounts.len();
            // Get or insert haneul_address into request count
...
impl Faucet for SimpleFaucet {
    async fn send(
        ...
        info!(uuid = ?id, ?recipient, ?digest, "PayHaneul txn succeeded");
...
pub async fn batch_gather(
    request_consumer: &mut Receiver<(Uuid, HaneulAddress, Vec<u64>)>,
    requests: &mut Vec<(Uuid, HaneulAddress, Vec<u64>)>,
    batch_request_size: u64,
) -> Result<(), FaucetError> {
...
pub async fn batch_transfer_gases(
    ...
    let total_haneul_needed: u64 = requests.iter().flat_map(|(_, _, amounts)| amounts).sum();
    // This loop is utilized to grab a coin that is large enough for the request
    loop {
        let gas_coin_response = faucet
            .prepare_gas_coin(total_haneul_needed + gas_cost, uuid, true)
            .await;

        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = faucet
                    .build_batch_pay_haneul_txn(
                        coin_id,
                        requests.clone(),
                ...
                // haneul address, and instead just fill it with the ZERO address.
                let recipient = HaneulAddress::ZERO;
                {
                    // Register the intention to send this transaction before we send it, so that if
                    // faucet fails or we give up before we get a definite response, we have a
                    // chance to retry later.
...
async fn find_gas_coins_and_address(
    ...
    config: &FaucetConfig,
) -> Result<(Vec<GasCoin>, HaneulAddress), FaucetError> {
...
mod tests {
    ...
    async fn execute_tx(
        ...
        tx_data: TransactionData,
    ) -> Result<HaneulTransactionBlockEffects, anyhow::Error> {
        ...
        if let Some(effects) = result_effects {
            ...
            Err(anyhow!(
                "Effects from HaneulTransactionBlockResult should not be empty"
            ))
        }
    }
    ...
    async fn test_batch_transfer_interface() {
        ...
        let target_addresses: Vec<HaneulAddress> = (0..5)
            .map(|_| HaneulAddress::random_for_testing_only())
            .collect();
    ...
    async fn test_clear_wal() {
        ...
        let tx_data = faucet
            .build_pay_haneul_txn(coin_id, faucet_address, recipient, &[100], 200_000_000)
            .await
            .map_err(FaucetError::internal)
            .unwrap();
    ...
    async fn test_insufficient_balance_will_retry_success() {
        ...
        let destination_address = HaneulAddress::random_for_testing_only();
        // Transfer all valid gases away except for 1
        for gas in gas_coins.iter().take(gas_coins.len() - 1) {
            let tx_data = client
                .transaction_builder()
                .transfer_haneul(address, gas.0, gas_budget, destination_address, None)
                .await
                .unwrap();
            execute_tx(&mut context, tx_data).await.unwrap();
        }
    ...
    async fn test_amounts_transferred_on_batch() {
        ...
        let target_addresses: Vec<HaneulAddress> = (0..2)
            .map(|_| HaneulAddress::random_for_testing_only())
            .collect();

        // Send 2 coins of 1 haneul each. We
        let coins_sent = 2;
