...
impl SimpleFaucet {
    ...
    async fn sign_and_execute_txn(
        ...
        let signature = self
            ...
            .map_err(FaucetError::internal)?;
        let tx = Transaction::from_data(tx_data, Intent::haneul_transaction(), vec![signature]);
...
mod tests {
    ...
    async fn get_current_gases(address: HaneulAddress, context: &mut WalletContext) -> Vec<GasCoin> {
        ...
        .unwrap();
        match results {
            HaneulClientCommandResult::Gas(gases) => gases,
            other => panic!("Expect HaneulClientCommandResult::Gas, but got {:?}", other),
        }
    }
}
