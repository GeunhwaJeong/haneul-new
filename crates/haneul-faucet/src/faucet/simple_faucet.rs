...
const TRANSFER_HANEUL_GAS: u64 = 100;
...
impl SimpleFaucet {
            ...
            match tokio::time::timeout(Duration::from_secs(30), consumer.recv()).await {
                Ok(Some(coin)) => {
                    ...
                    if let Some(gas_coin) = gas_coin {
                        if gas_coin.value() >= transfer_amount + TRANSFER_HANEUL_GAS {
                            info!(
                                ?uuid,
        ...
        let responses: Vec<_> = results
            ...
            .filter(|res| {
                if res.is_ok() {
                    ...
                    error!(?uuid, "Encountered error in transfer haneul: {:?}", res);
                    false
                }
            })
            .map(|res| {
                ...
                if txns.len() != 1 {
                    panic!("TransferHaneul Transaction should create one and exactly one txn, but got {:?}", txns);
                }
                let created = response.effects.created;
                if created.len() != 1 {
                    panic!("TransferHaneul Transaction should create one and exactly one object, but got {:?}", created);
                }
                let txn = &txns[0];
                let obj = &created[0];
                if let HaneulTransactionKind::TransferHaneul(HaneulTransferHaneul{recipient, amount: Some(amount)}) = txn {
                    ...
                    panic!("Expect HaneulTransactionKind::TransferHaneul(HaneulTransferHaneul) to address {} with Some(amount) but got {:?}", to, txn);
                }
            })
            .collect();
    ...
    async fn construct_transfer_haneul_txn_with_retry(
        &self,
        coin_id: ObjectID,
        ...
        let mut data = self
            .construct_transfer_haneul_txn(coin_id, signer, recipient, budget, amount)
            .await;
        ...
        while data.is_err() {
            if let Some(duration) = iter.next() {
                ...
                debug!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Retrying constructing TransferHaneul txn. Previous error: {:?}",
                    &data,
                );
            } else {
                warn!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Failed to construct TransferHaneul txn after {} retries with interval {:?}",
    ...
    async fn construct_transfer_haneul_txn(
        &self,
        coin_id: ObjectID,
        signer: HaneulAddress,
        recipient: HaneulAddress,
        ...
        self.wallet
            .client
            .transaction_builder()
            .transfer_haneul(signer, coin_id, budget, recipient, Some(amount))
            .await
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to construct TransferHaneul transaction for coin {:?}, {:?}",
                    coin_id,
                    e
                )
            })
    ...
    async fn transfer_haneul(
        ...
        let data = self
            .construct_transfer_haneul_txn_with_retry(coin_id, signer, recipient, budget, amount, uuid)
            .await?;
...
mod tests {
    ...
    async fn test_discard_invalid_gas() {
        ...
        let res = HaneulClientCommands::TransferHaneul {
            ...
            haneul_coin_object_id: *bad_gas.id(),
            amount: None,
            gas_budget: 50000,
        }
        ...
        if let HaneulClientCommandResult::TransferHaneul(_tx_cert, effects) = res {
            assert!(matches!(effects.status, HaneulExecutionStatus::Success));
        } else {
            panic!("transfer command did not return HaneulClientCommandResult::TransferHaneul");
        };
    ...
    async fn test_discard_smaller_amount_gas() {
        ...
        let res = HaneulClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
            amounts: Some(vec![tiny_value + TRANSFER_HANEUL_GAS]),
            gas_budget: 50000,
            gas: None,
            count: None,
        }
        ...
        let tiny_amount = gases
            ...
            .unwrap()
            .value();
        assert_eq!(tiny_amount, tiny_value + TRANSFER_HANEUL_GAS);
        ...
        let number_of_coins = gases.len();
        // Ask for a value higher than tiny coin + TRANSFER_HANEUL_GAS
