...
impl SimpleFaucet {
    ...
    async fn prepare_gas_coin(&self, total_amount: u64, uuid: Uuid) -> GasCoinResponse {
        ...
        match self.get_gas_coin(coin_id).await {
            Ok(Some(gas_coin)) if gas_coin.value() >= total_amount + PAY_HANEUL_GAS => {
    ...
    async fn transfer_gases(
        ...
        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let result = self
                    .execute_pay_haneul_txn_with_retrials(
                        coin_id,
                        self.active_address,
                        recipient,
                        amounts,
    ...
    async fn execute_pay_haneul_txn_with_retrials(
        &self,
        coin_id: ObjectID,
        ...
        let mut res = self
            .execute_pay_haneul_txn(coin_id, signer, recipient, amounts, budget, uuid)
            .await;
        while res.is_err() {
            if let Some(interval) = retry_iter.next() {
                ...
                info!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Retrying executing PayHaneul transaction in faucet, previous error: {:?}",
                    &res,
                );
                res = self
                    .execute_pay_haneul_txn(coin_id, signer, recipient, amounts, budget, uuid)
                ...
                warn!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Failed to execute PayHaneul transactions in faucet with {} retries and intervals {:?}",
    ...
    async fn execute_pay_haneul_txn(
        ...
        let tx_data = self
            .build_pay_haneul_txn(coin_id, signer, recipient, amounts, budget)
            .await?;
        let signature =
            context
                .config
    ...
    async fn build_pay_haneul_txn(
        ...
        let recipients: Vec<HaneulAddress> =
    ...
    async fn check_and_map_transfer_gas_result(
        &self,
        result: Result<HaneulTransactionResponse, anyhow::Error>,
        ...
        match result {
            Ok(res) => {
                ...
                let txn = &txns[0];
                if let HaneulTransactionKind::PayHaneul(HaneulPayHaneul {
...
mod tests {
    ...
    async fn test_discard_smaller_amount_gas() {
        ...
        let res = HaneulClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
            amounts: Some(vec![tiny_value + PAY_HANEUL_GAS]),
            gas_budget: 50000,
            gas: None,
            count: None,
        }
        ...
        let tiny_amount = gases
            ...
            .unwrap()
            .value();
        assert_eq!(tiny_amount, tiny_value + PAY_HANEUL_GAS);
        ...
        // Ask for a value higher than tiny coin + PAY_HANEUL_GAS
        let number_of_coins = gases.len();
