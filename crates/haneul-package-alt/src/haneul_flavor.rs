...
/// The Haneul-specific implementation of the [MoveFlavor] trait.
///
/// Can be constructed in offline mode ([`HaneulFlavor::new`]) or connected mode
/// ([`HaneulFlavor::with_client`]). In connected mode, queries the network via gRPC to determine the
...
impl std::fmt::Debug for HaneulFlavor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HaneulFlavor")
...
impl HaneulFlavor {
    /// Create a `HaneulFlavor` in offline mode. Uses the latest known system packages.
    pub fn new() -> Self {
        Self::default()
    }

    /// Create a `HaneulFlavor` connected to the network via the gRPC client from `wallet`. Falls back
