...
impl HaneulFlavor {
    ...
    fn system_deps_names_map() -> BTreeMap<PackageName, PackageName> {
        BTreeMap::from([
            (
                PackageName::new("Haneul").unwrap(),
                PackageName::new("haneul").unwrap(),
            ),
            (
                PackageName::new("HaneulSystem").unwrap(),
                PackageName::new("haneul_system").unwrap(),
            ),
            (
    ...
    /// The default dependencies are `haneul` and `std`
...
mod tests {
    ...
    fn test_implicit_deps() {
        let implicit_deps = ["haneul", "std", "haneul_system", "bridge"];
        let env = Environment::new("mainnet".into(), "35834a8a".into());

        let deps = HaneulFlavor::system_dependencies(env.id().into());
