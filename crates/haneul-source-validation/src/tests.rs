...
async fn successful_verification() -> anyhow::Result<()> {
    ...
    let b_ref = {
        ...
        let b_src = copy_package(&fixtures, "b", HaneulAddress::ZERO).await?;
    ...
    let (a_pkg, a_ref) = {
        ...
        let a_src = copy_package(&fixtures, "a", HaneulAddress::ZERO).await?;
        (
...
async fn successful_verification_unpublished_deps() -> anyhow::Result<()> {
    ...
    let a_src = {
        copy_package(&fixtures, "b", HaneulAddress::ZERO).await?;
        copy_package(&fixtures, "a", HaneulAddress::ZERO).await?
    };
...
async fn successful_verification_module_ordering() -> anyhow::Result<()> {
    ...
    let z_ref = {
        let fixtures = tempfile::tempdir()?;
        let z_src = copy_package(&fixtures, "z", HaneulAddress::ZERO).await?;
...
async fn multiple_failures() -> anyhow::Result<()> {
    ...
    let c_ref = {
        let fixtures = tempfile::tempdir()?;
        let c_src = copy_package(&fixtures, "c", HaneulAddress::ZERO).await?;
    ...
    let d_pkg = {
        ...
        let d_src = copy_package(&fixtures, "d", HaneulAddress::ZERO).await?;
