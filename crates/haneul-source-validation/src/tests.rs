...
async fn successful_verification() -> anyhow::Result<()> {
    ...
    let b_ref = {
        ...
        let b_src = copy_package(&fixtures, "b", [("b", HaneulAddress::ZERO)]).await?;
    ...
    let (a_pkg, a_ref) = {
        ...
        let a_src = copy_package(&fixtures, "a", [("a", HaneulAddress::ZERO), ("b", b_id)]).await?;
        (
...
async fn multiple_failures() -> anyhow::Result<()> {
    ...
    let c_ref = {
        let fixtures = tempfile::tempdir()?;
        let c_src = copy_package(&fixtures, "c", [("c", HaneulAddress::ZERO)]).await?;
    ...
    let d_pkg = {
        ...
        let d_src = copy_package(
            &fixtures,
            "d",
            [("d", HaneulAddress::ZERO), ("b", b_id), ("c", c_id)],
        )
        .await?;
...
async fn copy_package<'s>(
    ...
    package: &str,
    addresses: impl IntoIterator<Item = (&'s str, HaneulAddress)>,
) -> io::Result<PathBuf> {
