    ...
    fn local(&self, package: &CompiledPackage) -> Result<LocalModules, Error> {
        ...
        if self.verify_deps() {
            ...
            let deps_compiled_units: Vec<_> = deps_compiled_units
                .into_iter()
                .filter(|pkg| haneul_package.dependency_ids.published.contains_key(&pkg.0))
                .collect();
            ...
            for (package, module) in haneul_package.bytecode_deps.iter() {
