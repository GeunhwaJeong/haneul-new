...
const CANONICAL_BINARY_NAME: &str = "haneul";
...
pub enum SourceVerificationError {
    ...
    HaneulObjectRefFailure(HaneulObjectResponseError),
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(
        &self,
        addr: AccountAddress,
    ) -> Result<HaneulRawMovePackage, SourceVerificationError> {
...
fn download_and_compile(
    ...
    if !dest_canonical_binary.exists() {
        ...
        dest_binary.extend(["target", "release", &format!("haneul-{platform}")]);
    ...
    let s = match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => bail!(
            "The package {} needs to be built with haneul compiler version {compiler_version} but there \
             ...
             Operating System: {os}\n\
             Architecture: {arch}\n\
             You can manually put a `haneul` binary for your platform in {} and rerun your command to continue.",
