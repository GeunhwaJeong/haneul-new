...
use haneul_sdk::error::Error as SdkError;
use haneul_sdk::rpc_types::{HaneulObjectDataOptions, HaneulRawData, HaneulRawMovePackage};
...
const CANONICAL_UNIX_BINARY_NAME: &str = "haneul";
const CANONICAL_WIN_BINARY_NAME: &str = "haneul.exe";
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    /// the bytecode at the address specified on the Haneul network we are publishing to.
    ...
    /// at the address specified on the Haneul network we are publishing to.  If `verify_deps` is true,
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<HaneulRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a haneul network via haneul_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Haneul object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, HaneulObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::HaneulObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            ...
            HaneulRawData::MoveObject(move_obj) => {
    ...
    async fn on_chain_modules(
        ...
        for (storage_id, pkg) in addresses.zip(resp) {
            let HaneulRawMovePackage { module_map, .. } = pkg?;
...
fn units_for_toolchain(
    ...
    if std::env::var("HANEUL_RUN_TOOLCHAIN_BUILD").is_err() {
    ...
    for (package, local_unit) in compiled_units {
        ...
        if haneul_types::is_system_package(local_unit.unit.address.into_inner()) {
...
fn download_and_compile(
    ...
    if !dest_canonical_binary.exists() {
        ...
        let mainnet_url = format!(
            "https://github.com/GeunhwaJeong/haneul/releases/download/mainnet-v{compiler_version}/haneul-mainnet-v{compiler_version}-{platform}.tgz",
        );
        ...
        let mut response = match ureq::get(&mainnet_url).call() {
            ...
            Err(ureq::Error::Status(404, _)) => {
                println!(
                    "{} haneul mainnet compiler {} not available, attempting to download testnet compiler release...",
                    "WARNING".bold().yellow(),
                    compiler_version.yellow()
                );
                println!(
                ...
                let testnet_url = format!("https://github.com/GeunhwaJeong/haneul/releases/download/testnet-v{compiler_version}/haneul-testnet-v{compiler_version}-{platform}.tgz");
        ...
        if platform == "windows-x86_64" {
            dest_binary.push(&format!("haneul-{platform}.exe"));
        } else {
            dest_binary.push(&format!("haneul-{platform}"));
        }
...
fn detect_platform(
    ...
    let s = match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => {
            ...
            bail!(
                "The package {} needs to be built with haneul compiler version {compiler_version} but there \
