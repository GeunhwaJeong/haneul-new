...
impl SurferState {
    ...
    #[tracing::instrument(skip_all, fields(surfer_id = self.id))]
    async fn process_tx_effects(&mut self, effects: &HaneulTransactionBlockEffects) {
        for (owned_ref, write_kind) in effects.all_changed_objects() {
            ...
            match owned_ref.owner {
                ...
                // TODO: Implement full support for ConsensusV2 objects in haneul-surfer.
                | Owner::ConsensusV2 {
...
fn is_type_tx_context(ty: &Type) -> bool {
    match ty {
        Type::Reference(_, inner) => match inner.as_ref() {
            Type::Datatype(dt) => {
                dt.module.address == HANEUL_FRAMEWORK_ADDRESS
