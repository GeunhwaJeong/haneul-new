...
impl TicTacToe {
    async fn create_game(
        &self,
        player_x: Option<HaneulAddress>,
        player_o: Option<HaneulAddress>,
    ) -> Result<(), anyhow::Error> {
        ...
        let create_game_call = self
            ...
            .move_call(
                ...
                vec![],
                vec![
                    HaneulJsonValue::from_str(&player_x.to_string())?,
                    HaneulJsonValue::from_str(&player_o.to_string())?,
                ],
        ...
        let signature =
            self.keystore
                .sign_secure(&player_x, &create_game_call, Intent::haneul_transaction())?;
        ...
        if game_state.is_my_turn(my_identity) {
            ...
            let place_mark_call = self
                ...
                .move_call(
                    ...
                    vec![],
                    vec![
                        HaneulJsonValue::from_str(&game_state.info.object_id().to_hex_literal())?,
                        HaneulJsonValue::from_str(&row.to_string())?,
                        HaneulJsonValue::from_str(&col.to_string())?,
                    ],
                    None,
                    1000,
                    None,
    ...
    async fn fetch_game_state(&self, game_id: ObjectID) -> Result<TicTacToeState, anyhow::Error> {
        ...
        let current_game = self
            .client
            .read_api()
            .get_object_with_options(game_id, HaneulObjectDataOptions::new().with_bcs())
            .await?;
        current_game
            .object()?
            .bcs
...
struct TicTacToeOpts {
    ...
    #[clap(long, default_value = "https://fullnode.devnet.haneul.io:443")]
...
struct TicTacToeState {
    ...
    cur_turn: u8,
    game_status: u8,
    x_address: HaneulAddress,
    o_address: HaneulAddress,
}
...
impl TicTacToeState {
    ...
    fn is_my_turn(&self, my_identity: HaneulAddress) -> bool {
