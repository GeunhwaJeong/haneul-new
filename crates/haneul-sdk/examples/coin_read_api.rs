...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<HANEUL>` which is represented as
    // "0x2::haneul::HANEUL"
    let coin_type = Some("0x2::haneul::HANEUL".to_string());
    let coins = haneul
        .coin_read_api()
    ...
    // Select coins based on the provided coin type (HANEUL in this example). Use `None` for the default Haneul coin
    let select_coins = haneul
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<HANEUL> as the coin type
    let balance = haneul
        .coin_read_api()
    ...
    // Return the coin metadata for the Coin<HANEUL>
