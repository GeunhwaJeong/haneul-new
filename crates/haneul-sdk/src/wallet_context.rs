...
use crate::haneul_client_config::HaneulClientConfig;
use crate::HaneulClient;
use anyhow::anyhow;
...
use haneul_json_rpc_types::{
    get_new_package_obj_from_response, HaneulObjectData, HaneulObjectDataFilter, HaneulObjectDataOptions,
    HaneulObjectResponse, HaneulObjectResponseQuery, HaneulTransactionBlockEffectsAPI,
...
impl WalletContext {
    ...
    pub async fn get_client(&self) -> clap::Result<HaneulClient, anyhow::Error> {
    ...
    // TODO: Ger rid of mut
    pub fn active_address(&mut self) -> clap::Result<HaneulAddress, anyhow::Error> {
    ...
    ) -> clap::Result<Vec<(u64, HaneulObjectData)>, anyhow::Error> {
        let client = self.get_client().await?;
    ...
    pub async fn get_object_owner(&self, id: &ObjectID) -> clap::Result<HaneulAddress, anyhow::Error> {
        let client = self.get_client().await?;
        let object = client
            .read_api()
    ...
    pub async fn try_get_object_owner(
        &self,
        id: &Option<ObjectID>,
    ) -> clap::Result<Option<HaneulAddress>, anyhow::Error> {
        if let Some(id) = id {
    ...
    ) -> clap::Result<(u64, HaneulObjectData), anyhow::Error> {
    ...
    pub async fn make_staking_transaction(&self, validator_address: HaneulAddress) -> Transaction {
        let accounts_and_objs = self.get_all_accounts_and_gas_objects().await.unwrap();
