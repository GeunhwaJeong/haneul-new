...
//! * <a href="../haneul_transaction_builder/struct.TransactionBuilder.html" title="struct haneul_transaction_builder::TransactionBuilder">TransactionBuilder</a> - provides functions for building transactions
//!
//! # Usage
...
use haneul_json_rpc_types::{
    ...
    HaneulObjectResponseQuery, HaneulTransactionBlockResponse,
};
...
use crate::rpc_types::HaneulTransactionBlockResponseOptions;
...
impl Default for HaneulClientBuilder {
    fn default() -> Self {
        Self {
...
impl HaneulClientBuilder {
    ...
    /// Set the WebSocket URL for the Haneul network
    ...
    ///         .await?;
    ///
    ///     println!("Haneul local version: {:?}", haneul.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build(self, http: impl AsRef<str>) -> HaneulRpcResult<HaneulClient> {
    ...
    /// }
    /// ```
    pub async fn build_localnet(self) -> HaneulRpcResult<HaneulClient> {
    ...
    /// }
    /// ```
    pub async fn build_devnet(self) -> HaneulRpcResult<HaneulClient> {
...
///      .await?;
///
///     println!("{:?}", haneul.available_rpc_methods());
...
impl DataReader for ReadApi {
    async fn get_owned_objects(
        ...
        let mut result = vec![];
        let query = Some(HaneulObjectResponseQuery {
            filter: Some(HaneulObjectDataFilter::StructType(object_type)),
            options: Some(
