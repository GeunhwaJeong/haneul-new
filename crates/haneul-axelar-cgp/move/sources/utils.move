...
module axelar::utils {
    ...
    /// Prefix for Haneul Messages.
    const PREFIX: vector<u8> = b"\x19Haneul Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.haneul-labs.com/cryptography-in-haneul-cross-chain-signature-verification/
    ...
    public fun to_haneul_signed(bytes: vector<u8>): vector<u8> {
        let mut res = vector[];
