...
///
/// - When call approvals is sent to Haneul, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Haneul side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_haneul_signed;
    ...
    /// For when approval chainId is not HANEUL.
    ...
    /// Emitted when a new message is sent from the HANEUL network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_haneul_signed(*&data), proof);
    ...
    public fun call_contract<T: store>(
        ...
        haneul::event::emit(ContractCall {
    ...
    fun test_execute() {
        use haneul::test_scenario::{Self as ts, ctx};
