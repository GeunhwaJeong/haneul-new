...
const PROD_GATEWAY_ADDR: &str = "https://gateway.devnet.haneul.io:443";
const PROD_FAUCET_ADDR: &str = "https://faucet.devnet.haneul.io:443";
...
impl ClusterTest {
    ...
    async fn test_call_contract(&mut self, gas_obj_id: ObjectID) {
        ...
        if let GetObjectDataResponse::Exists(_haneul_object) = object_read {
            // all good
        } else {
    ...
    async fn verify_object(
        ...
        debug!(
            "Verifying object: {} (is {}a haneul coin), owned by {}. Expect to be {}.",
            obj_id,
            if is_haneul_coin { "" } else { "not " },
            expected_owner,
