...
use serde_json::json;
use haneul::client_commands::{call_move, EXAMPLE_NFT_DESCRIPTION, EXAMPLE_NFT_NAME, EXAMPLE_NFT_URL};
...
use haneul_types::{
    ...
    object::Owner,
    HANEUL_FRAMEWORK_ADDRESS, HANEUL_FRAMEWORK_OBJECT_ID,
};
use tracing::info;
...
impl TestCaseImpl for CallContractTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        events.iter().find(|e| matches!(e, HaneulEvent::MoveEvent{
            package_id,
            transaction_module,
            sender,
            type_,
        ...
        let haneul_object = ObjectChecker::new(nft_id)
            .owner(Owner::AddressOwner(signer))
            .check_into_haneul_object(ctx.get_fullnode())
            .await;

        assert_eq!(
            haneul_object.reference.version,
