...
impl TestCaseImpl for CoinMergeSplitTest {
    ...
    fn description(&self) -> &'static str {
        "Test merge and split HANEUL coins"
    }

    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        let mut haneul_objs = ctx.get_haneul_from_faucet(Some(2)).await;
        let gas_obj = haneul_objs.swap_remove(0);

        let signer = ctx.get_wallet_address();
        let primary_coin = haneul_objs.swap_remove(0);
        ...
        let response = ctx.sign_and_execute(data, "coin split").await;
        ...
        for new_coin in &split_response.new_coins {
            ...
            txes.push(response.certificate.transaction_digest);
        }
