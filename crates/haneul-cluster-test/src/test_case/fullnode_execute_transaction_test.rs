...
use haneul_json_rpc_types::{HaneulExecuteTransactionResponse, HaneulExecutionStatus};
...
impl TestCaseImpl for FullNodeExecuteTransactionTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        let txn_count = 4;
        ctx.get_haneul_from_faucet(Some(txn_count)).await;
        ...
        let response = fullnode
            ...
            .await?;
        if let HaneulExecuteTransactionResponse::ImmediateReturn { tx_digest } = response {
        ...
        let response = fullnode
            ...
            .await?;
        if let HaneulExecuteTransactionResponse::TxCert { certificate } = response {
        ...
        let response = fullnode
            ...
            .await?;
        if let HaneulExecuteTransactionResponse::EffectsCert {
            certificate,
            effects,
        ...
        } = response
        {
            ...
            assert_eq!(txn_digest, certificate.transaction_digest);
            if !matches!(effects.effects.status, HaneulExecutionStatus::Success { .. }) {
                panic!(
