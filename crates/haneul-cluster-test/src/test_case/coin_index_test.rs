...
use haneul_core::test_utils::compile_managed_coin_package;
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_haneul_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, haneul_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let haneul_type_str = "0x2::haneul::HANEUL";
        ...
        let args = vec![
            ...
            HaneulJsonValue::new(json!("10000"))?,
        ...
        let haneul_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("HANEUL"))
            .unwrap();
        ...
        assert_eq!(haneul_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + haneul_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, haneul_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            haneul_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let mut expected_balances = vec![
            Balance {
                coin_type: haneul_type_str.into(),
        ...
        let args = vec![HaneulJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    HaneulJsonValue::from_object_id(cap.0),
                    HaneulJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let haneul_coins = client
            .coin_read_api()
            .get_coins(account, Some(haneul_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only HANEUL coins at the moment
            haneul_coins,
            client
                .coin_read_api()
        ...
        let haneul_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            haneul_balance.total_balance,
            haneul_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(haneul_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(haneul_coins.len() + managed_coins.len(), total_coins,);

        let haneul_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(haneul_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            haneul_coins_with_managed_coin_1.data.len(),
            haneul_coins.len() + 1
        );
        assert_eq!(
            haneul_coins_with_managed_coin_1.next_cursor,
            Some(first_managed_coin)
        );
        assert!(haneul_coins_with_managed_coin_1.has_next_page);
        let cursor = haneul_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                HaneulJsonValue::from_object_id(envelope),
                HaneulJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
