...
impl Cluster for LocalNewCluster {
    async fn start(
        ...
        let (network_config_path, provided_config) = if genesis_config.is_some() {
            (
                Some(haneul_cluster_test_config_dir()?.join(HANEUL_NETWORK_CONFIG)),
                true,
            )
        } else {
            (None, false)
        ...
        let (faucet_key, faucet_address) = if provided_config {
            let keystore_path = haneul_cluster_test_config_dir()?.join(HANEUL_KEYSTORE_FILENAME);
            ...
            let faucet_key = match pub_faucet {
                HaneulKeyPair::Ed25519(account_keypair) => Some(account_keypair),
                _ => None,
            }
            .unwrap()
            .copy();
            ...
            let faucet_address = HaneulAddress::from(faucet_key.public());
            (faucet_key, faucet_address)
        };
...
pub async fn new_wallet_context_from_cluster(
    ...
    let keystore_path = config_dir.join(HANEUL_KEYSTORE_FILENAME);
