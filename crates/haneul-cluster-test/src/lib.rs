...
impl TestContext {
    async fn get_haneul_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        ...
        let faucet_response = self.faucet.request_haneul_coins(addr).await;
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} Haneul Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &HaneulClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> HaneulClient {
    ...
    async fn get_latest_haneul_system_state(&self) -> HaneulSystemStateSummary {
        self.client
            .get_fullnode_client()
            .governance_api()
    ...
    fn get_wallet_address(&self) -> HaneulAddress {
