...
pub struct HaneulTestAdapter<'a> {
    ...
    accounts: BTreeMap<String, (HaneulAddress, AccountKeyPair)>,
...
impl<'a> MoveTestAdapter<'a> for HaneulTestAdapter<'a> {
    ...
    type Subcommand = HaneulSubcommand;
    ...
    fn publish_modules(
        ...
        let [(named_addr_opt, module)] = &modules[..] else {
            bail!("Multiple package publish not supported in haneul adapter");
        };
...
impl<'a> HaneulTestAdapter<'a> {
    ...
    fn enumerate_fake(&mut self, id: ObjectID) -> FakeID {
        ...
        // as the "fake" ID. This essentially excludes the Haneul System objects from the fake ID
        // mapping.
        let id_addr: HaneulAddress = id.into();
        let id_bytes: [u8; HANEUL_ADDRESS_LENGTH] = id_addr.to_inner();
        let high = &id_bytes[0..(HANEUL_ADDRESS_LENGTH - 8)];
        let low = [
            id_bytes[HANEUL_ADDRESS_LENGTH - 8],
            id_bytes[HANEUL_ADDRESS_LENGTH - 7],
            id_bytes[HANEUL_ADDRESS_LENGTH - 6],
            id_bytes[HANEUL_ADDRESS_LENGTH - 5],
            id_bytes[HANEUL_ADDRESS_LENGTH - 4],
            id_bytes[HANEUL_ADDRESS_LENGTH - 3],
            id_bytes[HANEUL_ADDRESS_LENGTH - 2],
            id_bytes[HANEUL_ADDRESS_LENGTH - 1],
        ];
        assert!(high.len() + low.len() == HANEUL_ADDRESS_LENGTH);
