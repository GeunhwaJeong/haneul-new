...
use haneul_graphql_rpc::test_infra::cluster::{serve_executor, SnapshotLagConfig};
...
impl<'a> MoveTestAdapter<'a> for HaneulTestAdapter {
    ...
    async fn handle_subcommand(
        ...
        match command {
            HaneulSubcommand::ForceObjectSnapshotCatchup(ForceObjectSnapshotCatchup {
                start_cp,
                end_cp,
            }) => {
...
async fn init_sim_executor(
    ...
    let db_name = format!("haneul_graphql_test_{}", hash);
