...
use haneul_adapter::{in_memory_storage::InMemoryStorage, temporary_store::InnerTemporaryStore};
...
impl<'a> HaneulTestAdapter<'a> {
    ...
    fn get_object_sorting_key(&self, id: &ObjectID) -> String {
        match &self.storage.get_object(id).unwrap().data {
            haneul_types::object::Data::Move(obj) => self.stabilize_str(format!("{}", obj.type_)),
            haneul_types::object::Data::Package(pkg) => pkg
                .serialized_module_map()
                .keys()
                .map(|s| s.as_str())
                .collect::<Vec<_>>()
