...
pub async fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let (_guard, _filter_handle) = telemetry_subscribers::TelemetryConfig::new()
        .with_env()
        .init();
    run_test_impl::<HaneulTestAdapter>(path, Some(std::sync::Arc::new(PRE_COMPILED.clone()))).await?;
    Ok(())
}
...
pub trait TransactionalAdapter: Send + Sync + ReadStore {
    ...
    async fn read_input_objects(&self, transaction: Transaction) -> HaneulResult<InputObjects>;
...
impl TransactionalAdapter for Simulacrum<StdRng, PersistedStore> {
    ...
    async fn read_input_objects(&self, _transaction: Transaction) -> HaneulResult<InputObjects> {
