...
pub struct ProgrammableTransactionCommand {
    ...
    #[clap(long = "gas-price")]
    pub gas_price: Option<u64>,
    #[clap(long = "gas-payment", value_parser = ParsedValue::<HaneulExtraValueArgs>::parse)]
    pub gas_payment: Option<Vec<ParsedValue<HaneulExtraValueArgs>>>,
    ...
    fn from_arg_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        Ok(match matches.subcommand() {
            ...
            Some(("view-funds", matches)) => {
                HaneulSubcommand::ViewFunds(ViewFundsCommand::from_arg_matches(matches)?)
            }
            ...
            Some(("gasless-allow-token", matches)) => HaneulSubcommand::GaslessAllowToken(
            ...
            Some(("bench", matches)) => match matches.subcommand() {
                Some(("ptb", sub_matches)) => HaneulSubcommand::BenchProgrammable(
                    ProgrammableTransactionCommand::from_arg_matches(sub_matches)?,
                ),
                _ => HaneulSubcommand::Bench(
