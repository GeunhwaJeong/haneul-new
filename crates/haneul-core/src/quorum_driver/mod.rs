...
use haneul_types::base_types::{AuthorityName, ObjectRef, TransactionDigest};
...
use haneul_types::messages::{
    QuorumDriverRequest, QuorumDriverResponse, VerifiedCertificate, VerifiedTransaction,
};
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    pub async fn execute_transaction(
    ...
    ) -> HaneulResult<QuorumDriverResponse> {
        ...
        let QuorumDriverRequest { transaction } = request;
        ...
        if result.is_ok() {
            self.metrics.total_ok_responses_wait_for_effects_cert.inc()
        }
        result
    }
            ...
            Err(HaneulError::QuorumFailedToProcessTransaction {
                ...
                match attempt_result {
                    ...
                    Ok(Some((retried_tx_digest, success))) => {
                        ...
                        return Err(HaneulError::QuorumFailedToProcessTransactionWithConflictingTransactionRetried { conflicting_tx_digest: retried_tx_digest, conflicting_tx_retry_success: success });
                    }
                }
            }
            _ => (),
    ...
    ) -> HaneulResult<(VerifiedCertificate, VerifiedCertifiedTransactionEffects)> {
        let effects = self
            .validators
            .load()
    ...
    ) -> HaneulResult<Option<(TransactionDigest, bool)>> {
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn task_queue_processor(
        ...
        // TODO https://github.com/GeunhwaJeong/haneul/issues/4565
