...
use haneul_types::committee::{validity_threshold, Committee, EpochId, StakeUnit};
...
use haneul_types::messages::{QuorumDriverResponse, VerifiedCertificate, VerifiedTransaction};
...
impl<A> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> HaneulResult<()> {
        self.task_sender
            .send(task.clone())
            .await
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    pub async fn submit_transaction(
        &self,
        transaction: VerifiedTransaction,
    ) -> HaneulResult<Registration<TransactionDigest, QuorumDriverResult>> {
...
// TODO: categorize all possible HaneulErrors
...
fn is_error_retryable(error: &HaneulError) -> bool {
    match error {
        // Network error
