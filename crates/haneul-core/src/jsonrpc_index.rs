...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: HaneulAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCacheUpdates {
    per_coin_type_balance_changes: Vec<((HaneulAddress, TypeTag), HaneulResult<TotalBalance>)>,
    all_balance_changes: Vec<(HaneulAddress, HaneulResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    address_balances: DBMap<(HaneulAddress, TypeTag), ()>,
    ...
    event_by_sender: DBMap<(HaneulAddress, EventId), EventIndex>,
...
impl IndexStore {
        ...
        let table_options = DBMapTableConfigMap::new(BTreeMap::from([
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    |(_, id): (HaneulAddress, TxSequenceNumber)| id,
                ),
            ),
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, sequence_number): (HaneulAddress, TxSequenceNumber)| sequence_number,
                ),
            ),
            (
            ...
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, event_id): (HaneulAddress, EventId)| event_id.0,
                ),
            ),
            (
    ...
    pub fn index_coin(
        ...
        acquire_locks: bool,
    ) -> HaneulResult<IndexStoreCacheUpdatesWithLocks> {
        ...
        let _locks = if acquire_locks {
            let mut addresses: HashSet<HaneulAddress> = HashSet::new();
            addresses.extend(
        ...
        let mut balance_changes: HashMap<HaneulAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, HaneulError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, HaneulError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn index_tx(
        ...
        let address_balance_updates = accumulator_events.into_iter().filter_map(|event| {
            ...
            let coin_type = haneul_types::balance::Balance::maybe_get_balance_type_param(ty)?;
    ...
    pub fn prune(&self, cut_time_ms: u64) -> HaneulResult<TxSequenceNumber> {
        match self
            .tables
            .event_by_time
            .reversed_safe_iter_with_bounds(
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> HaneulResult {
    ...
    /// Read balance for a `HaneulAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `HaneulAddress` from the backend database
    #[instrument(skip_all)]
    ...
    fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (HaneulAddress, TypeTag)>,
    ) -> HaneulResult {
    ...
    fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((HaneulAddress, TypeTag), HaneulResult<TotalBalance>)>,
    ) -> HaneulResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &HaneulResult<TotalBalance>,
        balance_delta: &HaneulResult<TotalBalance>,
    ) -> HaneulResult<TotalBalance> {
    ...
    fn merge_all_balance(
        old_balance: &HaneulResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &HaneulResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> HaneulResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
