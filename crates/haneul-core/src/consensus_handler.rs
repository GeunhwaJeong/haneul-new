...
use lru::LruCache;
use haneullabs_metrics::{monitored_mpsc::UnboundedReceiver, monitored_scope, spawn_monitored_task};
use narwhal_config::Committee;
use narwhal_executor::{ExecutionIndices, ExecutionState};
use narwhal_types::ConsensusOutput;
use serde::{Deserialize, Serialize};
use haneul_macros::{fail_point_async, fail_point_if};
...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    async fn handle_consensus_output_internal(
        ...
        fail_point_if!("correlated-crash-after-consensus-commit-boundary", || {
            ...
            if haneul_simulator::random::deterministic_probability(&key, 0.01) {
...
mod tests {
    ...
    use haneul_protocol_config::{ConsensusTransactionOrdering, SupportedProtocolVersions};
    use haneul_types::{
        base_types::{random_object_ref, AuthorityName, HaneulAddress},
        committee::Committee,
        messages_consensus::{
