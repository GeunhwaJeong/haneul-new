...
impl TryFrom<RawSubmitTxResponse> for SubmitTxResponse {
    ...
    fn try_from(value: RawSubmitTxResponse) -> Result<Self, Self::Error> {
        ...
        if value.results.len() != 1 {
            return Err(HaneulError::GrpcMessageDeserializeError {
        ...
        let results = value
            ...
            .collect::<Result<Vec<SubmitTxResult>, HaneulError>>()?;
...
fn try_from_raw_executed_status(
...
) -> Result<(TransactionEffectsDigest, Option<Box<ExecutedData>>, bool), HaneulError> {
    ...
    let details = if let Some(details) = executed.details {
        ...
        let events = if let Some(events) = details.events {
            Some(
                bcs::from_bytes(&events).map_err(|err| HaneulError::GrpcMessageDeserializeError {
...
fn try_from_raw_rejected_status(rejected: RawRejectedStatus) -> Result<Option<HaneulError>, HaneulError> {
...
fn try_from_response_rejected(error: Option<HaneulError>) -> Result<RawRejectedStatus, HaneulError> {
...
fn try_from_response_executed(
    ...
    details: Option<Box<ExecutedData>>,
    fast_path: bool,
) -> Result<RawExecutedStatus, HaneulError> {
