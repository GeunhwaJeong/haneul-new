...
fn try_from_raw_rejected_status(rejected: RawRejectedStatus) -> Result<HaneulError, HaneulError> {
    let error =
        bcs::from_bytes(&rejected.error).map_err(|err| HaneulError::GrpcMessageDeserializeError {
...
fn try_from_response_rejected(error: HaneulError) -> Result<RawRejectedStatus, HaneulError> {
