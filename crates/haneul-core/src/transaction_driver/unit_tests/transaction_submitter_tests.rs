...
impl MockAuthority {
    ...
    fn set_submit_response(
        ...
        response: Result<SubmitTxResult, HaneulError>,
    ) {
...
impl AuthorityAPI for MockAuthority {
    async fn submit_transaction(
        ...
        let maybe_response = match request.transactions.first() {
            Some(tx_bytes) => {
                let tx: Transaction =
                    bcs::from_bytes(tx_bytes).map_err(|e| HaneulError::GenericAuthorityError {
            ...
            match response {
                Ok(result) => {
                    let raw_result: RawSubmitTxResult =
                        result
                            .try_into()
                            .map_err(|_| HaneulError::GenericAuthorityError {
    ...
    async fn validator_health(
        ...
        Ok(haneul_types::messages_grpc::RawValidatorHealthResponse::default())
    }
}
    ...
    // Test 4: Transaction with errors in submission.
    {
        ...
        for (i, mock_authority) in mock_authorities.iter().enumerate() {
            if i < 2 {
                mock_authority.set_submit_response(
                    *tx_digest,
                    Err(HaneulError::ValidatorOverloadedRetryAfter {
                        retry_after_secs: 1,
                    }),
                );
            } else {
