...
This directory contains the design documentation for Haneul's **address balances** system —
...
If you are touching code under `crates/haneul-core/src/accumulators/` or
`crates/haneul-core/src/execution_scheduler/funds_withdraw_scheduler/`, this is the place to start.
...
- **Accumulator root (`0xACC`)** — the singleton shared object
  (`HANEUL_ACCUMULATOR_ROOT_OBJECT_ID`) that owns every virtual balance as a dynamic field.
...
- **Address funds** — virtual balances owned by an address (`HaneulAddress`). Withdraw maximums are
