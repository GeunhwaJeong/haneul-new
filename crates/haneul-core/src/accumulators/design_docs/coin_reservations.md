...
(`haneul-types/src/coin_reservation.rs:183`.) XOR is its own inverse, so the same function masks
...
(`haneul-types/src/coin_reservation.rs:95`). The 4-byte epoch field is a *very* generous
budget — at one epoch per day it's more than 12 million years.

The encode/decode roundtrip lives in `haneul-types/src/coin_reservation.rs:107-137`
...
```
   ...
   │  - gas-only: must be Balance<HANEUL>,       │
...
`haneul-json-rpc::authority_state` shim layer.
...
`haneul_json_rpc::authority_state::get_object_read` implements the two-phase trick from §2.1:
...
- The reservation must be for `Balance<HANEUL>` specifically (the unmasked id must equal
  `AccumulatorValue::get_field_id(sender, Balance<HANEUL>)`).
...
through `CoinReservationResolver` (`haneul-types/src/coin_reservation.rs:210`). The
