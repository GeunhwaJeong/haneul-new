...
`crates/haneul-core/src/accumulators/object_funds_checker/`) after PTB execution finishes a
...
input. Haneul's execution model already serializes any transactions that share a mutable input, so
