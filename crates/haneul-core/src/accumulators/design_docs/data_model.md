...
`HANEUL_ACCUMULATOR_ROOT_OBJECT_ID` (commonly written `0xACC`). It is a **shared object**, with its
...
The relevant Rust types live in `haneul-types/src/accumulator_root.rs`:

```rust
pub struct AccumulatorKey { pub owner: HaneulAddress }
...
Haneul balances are nominally `u64`, but the accumulator stores them as `u128`. The wider storage
...
derivation goes through Haneul's standard dynamic-field ID machinery, with the parent set to
`HANEUL_ACCUMULATOR_ROOT_OBJECT_ID` and the field name set to `Key<Balance<T>>`:

```rust
pub fn get_field_id(owner: HaneulAddress, type_: &TypeTag) -> HaneulResult<AccumulatorObjId> {
...
(implemented by `AccumulatorValue::get_field_id` in `haneul-types::accumulator_root`)
...
`AccumulatorKey { owner }` field is a `HaneulAddress`, but a Move object's UID encodes as an address
