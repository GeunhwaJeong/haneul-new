...
async fn test_get_consistent_latest_account_amount_race_with_pending_settlement() {
    ...
    use haneul_types::{
        HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, accumulator_root::AccumulatorValue, balance::Balance,
        gas_coin::GAS,
    };
    ...
    cache.write_object_entry(&HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, v, root_object.into());

    let (owner, _) = deterministic_random_account_key();
    let balance_type_tag: haneul_types::TypeTag = Balance::type_(GAS::type_tag()).into();
