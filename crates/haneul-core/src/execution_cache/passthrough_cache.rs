...
impl PassthroughCache {
    ...
    fn revert_state_update_impl(&self, digest: &TransactionDigest) -> HaneulResult {
...
impl ExecutionCacheCommit for PassthroughCache {
    ...
    fn persist_transactions(&self, _digests: &[TransactionDigest]) -> BoxFuture<'_, HaneulResult> {
        // Nothing needs to be done since they were already committed in write_transaction_outputs
        async { Ok(()) }.boxed()
    }
}
