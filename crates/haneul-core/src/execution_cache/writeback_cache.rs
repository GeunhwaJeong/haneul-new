...
impl WritebackCache {
    ...
    async fn persist_transactions(&self, digests: &[TransactionDigest]) -> HaneulResult {
    ...
    fn revert_state_update_impl(&self, tx: &TransactionDigest) -> HaneulResult {
    ...
    fn bulk_insert_genesis_objects_impl(&self, objects: &[Object]) -> HaneulResult {
    ...
    fn insert_genesis_object_impl(&self, object: Object) -> HaneulResult {
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> HaneulLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(HaneulError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
    ...
    fn check_owned_objects_are_live(&self, owned_object_refs: &[ObjectRef]) -> HaneulResult {
        do_fallback_lookup(
            owned_object_refs,
    ...
    fn get_highest_pruned_checkpoint(&self) -> HaneulResult<CheckpointSequenceNumber> {
...
fn do_fallback_lookup<K: Copy, V: Default + Clone>(
    keys: &[K],
    get_cached_key: impl Fn(&K) -> HaneulResult<CacheResult<V>>,
    multiget_fallback: impl Fn(&[K]) -> HaneulResult<Vec<V>>,
) -> HaneulResult<Vec<V>> {
