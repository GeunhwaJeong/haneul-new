...
use haneul_types::accumulator_root::{AccumulatorObjId, AccumulatorValue};
use haneul_types::base_types::{
    EpochId, FullObjectID, ObjectID, ObjectRef, SequenceNumber, VerifiedExecutionData,
};
use haneul_types::bridge::{Bridge, get_bridge};
...
use haneul_types::transaction::{TransactionDataAPI, VerifiedSignedTransaction, VerifiedTransaction};
use tap::TapOptional;
...
impl WritebackCache {
    ...
    fn build_db_batch(&self, epoch: EpochId, digests: &[TransactionDigest]) -> Batch {
        let _metrics_guard = haneullabs_metrics::monitored_scope("WritebackCache::build_db_batch");
    ...
    fn commit_transaction_outputs(
        ...
        let _metrics_guard =
            haneullabs_metrics::monitored_scope("WritebackCache::commit_transaction_outputs");
...
impl ExecutionCacheWrite for WritebackCache {
    ...
    fn validate_owned_object_versions(&self, owned_input_objects: &[ObjectRef]) -> HaneulResult {
