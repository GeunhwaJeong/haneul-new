...
impl ReadStore for RocksDbStore {
            ...
            .map(|contents| {
                ...
                for tx in contents.iter() {
                    if let (Some(t), Some(e)) = (
                        ...
                        transactions.push(haneul_types::base_types::ExecutionData::new(
                            (*t).clone().into_inner(),
                            e,
                        ))
                    } else {
                        return Result::<
                            ...
                            haneul_types::storage::error::Error,
                        >::Ok(None);
                    }
                }
                Ok(Some(
    ...
    fn get_committee(
        &self,
        epoch: EpochId,
    ) -> Result<Option<Arc<Committee>>, haneul_types::storage::error::Error> {
...
impl WriteStore for RocksDbStore {
    ...
    ) -> Result<(), haneul_types::storage::error::Error> {
...
impl RestReadStore {
    ...
    fn index(&self) -> haneul_types::storage::error::Result<&RestIndexStore> {
        self.state
            .rest_index
            .as_deref()
...
impl ReadStore for RestReadStore {
    fn get_committee(
        &self,
        epoch: EpochId,
    ) -> haneul_types::storage::error::Result<Option<Arc<Committee>>> {
    ...
    fn get_full_checkpoint_contents_by_sequence_number(
        &self,
        sequence_number: CheckpointSequenceNumber,
    ) -> haneul_types::storage::error::Result<Option<FullCheckpointContents>> {
        self.rocks
            .get_full_checkpoint_contents_by_sequence_number(sequence_number)
    }
