...
pub trait SubmitToConsensus: Sync + Send + 'static {
    ...
    /// `timeout`. Haneults periodic, self-superseding messages (e.g. execution time
...
impl ConsensusAdapter {
    ...
    pub async fn submit_and_get_positions(
    ...
    ) -> Result<Vec<ConsensusPosition>, HaneulError> {
        ...
        rx_consensus_positions.await.map_err(|e| {
            HaneulError::from(HaneulErrorKind::FailedToSubmitToConsensus(format!(
    ...
    pub fn submit(
        ...
        epoch_store: &Arc<AuthorityPerEpochStore>,
        tx_consensus_position: Option<oneshot::Sender<HaneulResult<Vec<ConsensusPosition>>>>,
        submitter_client_addr: Option<IpAddr>,
    ...
    async fn submit_and_wait_inner(
        ...
        transactions: Vec<ConsensusTransaction>,
        epoch_store: &Arc<AuthorityPerEpochStore>,
        mut tx_consensus_positions: Option<oneshot::Sender<HaneulResult<Vec<ConsensusPosition>>>>,
        submitter_client_addr: Option<IpAddr>,
    ) {
        ...
        let make_processing_error = |method: ProcessedMethod| -> HaneulError {
            let digest = transactions
                .iter()
                .find_map(|t| t.kind.as_user_transaction().map(|tx| *tx.digest()))
                .unwrap_or_default();
            HaneulErrorKind::TransactionProcessing {
                digest,
