        ...
        if info.is_empty() {
            ...
            match certificate {
                ...
                message => {
                    ...
                    Err(HaneulError::UnexpectedMessage)
                }
            }
        } else {
            // This certificate has already been executed.
            bincode::deserialize(&info)
                .map_err(|e| HaneulError::ConsensusHaneulSerializationError(e.to_string()))
        }
    }
}
...
impl CheckpointConsensusAdapter {
    ...
    async fn waiter<T>(
        receiver: oneshot::Receiver<HaneulResult<SerializedTransactionInfoResponse>>,
        retry_delay: Duration,
        deliver: T,
