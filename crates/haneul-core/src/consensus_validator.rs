...
impl TransactionValidator for HaneulTxValidator {
    type Error = eyre::Report;

    fn validate(&self, _tx: &[u8]) -> Result<(), Self::Error> {
        // We only accept transactions from local haneul instance so no need to re-verify it
        Ok(())
    }
...
#[async_trait]
impl BlockVerifier for HaneulTxValidator {
    type Error = eyre::Report;
...
mod tests {
    ...
    use haneul_types::{
        crypto::Ed25519HaneulSignature, messages_consensus::ConsensusTransaction, object::Object,
