...
impl HaneulTxValidator {
    pub fn new(
        ...
        info!(
            "HaneulTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
    ...
    fn vote_transaction(
        ...
        if epoch_store.protocol_config().address_aliases() {
            ...
            if !aliases_match || fail_point_always_report_aliases_changed {
                return Err(HaneulErrorKind::AliasesChanged.into());
            }
        }
    ...
    fn verify_immutable_object_claims(
        ...
        for claimed_id in claimed_ids {
            if !input_refs_by_id.contains_key(claimed_id) {
                return Err(HaneulErrorKind::ImmutableObjectClaimNotFoundInInput {
            ...
            .next()
        {
            ...
            return Err(HaneulErrorKind::InvalidImmutableObjectClaim {
        ...
        if let Some(found_id) = found_immutable_ids
            ...
            .next()
        {
            return Err(HaneulErrorKind::ImmutableObjectNotClaimed {
                object_id: *found_id,
            }
            .into());
        }
...
impl TransactionVerifier for HaneulTxValidator {
...
mod tests {
    ...
    use haneul_config::transaction_deny_config::TransactionDenyConfigBuilder;
    ...
    async fn test_verify_and_vote_batch() {
        ...
        let validator = HaneulTxValidator::new(
            ...
            state.epoch_store_for_testing().clone(),
            Arc::new(CheckpointServiceNoop {}),
            HaneulTxValidatorMetrics::new(&Default::default()),
        );
    ...
    async fn test_verify_immutable_object_claims() {
        ...
        // Test 4: Missing claim - immutable object not claimed - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    HaneulErrorKind::ImmutableObjectNotClaimed { object_id }
        ...
        // Test 5: False claim - owned object claimed as immutable - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    HaneulErrorKind::InvalidImmutableObjectClaim { claimed_object_id, .. }
        ...
        // Test 6: Claim not in inputs - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    HaneulErrorKind::ImmutableObjectClaimNotFoundInInput { object_id }
                    if *object_id == immutable_id1
                ),
        ...
        // Test 7: Object not found (non-existent object) - should fail
        {
            ...
            let fake_ref = (
                non_existent_id,
                haneul_types::base_types::SequenceNumber::new(),
                haneul_types::digests::ObjectDigest::random(),
            );
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    HaneulErrorKind::UserInputError { error: UserInputError::ObjectNotFound { object_id, .. } }
        ...
        // Test 8: Version/digest mismatch for immutable object - should fail
        {
            ...
            let wrong_version_ref = (
                immutable_ref1.0,
                haneul_types::base_types::SequenceNumber::from_u64(999),
                immutable_ref1.2,
            );
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    HaneulErrorKind::UserInputError { error: UserInputError::ObjectVersionUnavailableForConsumption { provided_obj_ref, current_version: _ } }
