...
impl crate::authority::AuthorityState {
    pub fn last_batch(&self) -> Result<Option<SignedBatch>, HaneulError> {
        let last_batch = self
            .db()
            .perpetual_tables
            .batches
    ...
    pub fn init_batches_from_database(&mut self) -> Result<AuthorityBatch, HaneulError> {
    ...
    ) -> Result<impl Stream<Item = Result<BatchInfoResponseItem, HaneulError>>, HaneulError> {
                    ...
                    loop {
                        return match local_state.subscriber.recv().await {
                            ...
                            Err(RecvError::Closed) => {
                                ...
                                let err_response = Err(HaneulError::SubscriptionServiceClosed);
                                local_state.exit = true;
