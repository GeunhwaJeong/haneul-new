...
use haneul_types::object::{Data, ObjectFormatOptions, Owner};
use haneul_types::{
    base_types::*,
    coin,
    committee::Committee,
...
impl GatewayState<NetworkAuthorityClient> {
    ...
    pub fn make_committee(config: &GatewayConfig) -> HaneulResult<Committee> {
        Committee::new(
            config.epoch,
...
pub trait GatewayAPI {
    ...
    /// Send an object to a Haneul address. The object's type must allow public transfers
    ...
    async fn sync_account_state(&self, account_addr: HaneulAddress) -> Result<(), anyhow::Error>;
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    pub async fn get_framework_object_ref(&self) -> Result<ObjectRef, anyhow::Error> {
        Ok(self
            .get_object_ref(&ObjectID::from(HANEUL_FRAMEWORK_ADDRESS))
            .await?)
    }
    ...
    async fn get_object_internal(&self, object_id: &ObjectID) -> HaneulResult<Object> {
    ...
    async fn to_haneul_object<T: HaneulData>(
        ...
        HaneulObject::<T>::try_from(object, layout)
    }
    ...
    async fn get_object_ref(&self, object_id: &ObjectID) -> HaneulResult<ObjectRef> {
        let object = self.get_object_internal(object_id).await?;
    ...
    async fn sync_input_objects_with_authorities(&self, transaction: &Transaction) -> HaneulResult {
    ...
    async fn prepare_transaction(
        &self,
        transaction: &Transaction,
    ) -> HaneulResult<(InputObjects, Vec<ObjectRef>)> {
        transaction.verify()?;
        ...
        self.download_object_from_authorities(HANEUL_SYSTEM_STATE_OBJECT_ID)
            .await?;
    ...
    async fn execute_transaction_impl(
        ...
        let (input_objects, owned_objects) =
            self.prepare_transaction(&transaction)
                .await
                .map_err(|err| HaneulError::GatewayTransactionPrepError {
        ...
        match tx {
            ...
            None => {
                ...
                if self.store.get_certified_transaction(&digest)?.is_some() {
                    ...
                    Err(HaneulError::TransactionNotFound { digest }.into())
                }
            }
        }
    }

    async fn download_object_from_authorities(&self, object_id: ObjectID) -> HaneulResult<ObjectRead> {
    ...
    async fn download_objects_from_authorities(
    ...
    ) -> Result<BTreeMap<ObjectRef, Object>, HaneulError> {
        ...
        fp_ensure!(
            ...
            HaneulError::InconsistentGatewayResult {
    ...
    ) -> Result<Option<HaneulParsedTransactionResponse>, anyhow::Error> {
    ...
    async fn create_publish_response(
        ...
        certificate: CertifiedTransaction,
        effects: TransactionEffects,
    ) -> Result<HaneulParsedTransactionResponse, anyhow::Error> {
        ...
        for ((obj_ref, _, _), object) in effects.all_mutated().zip(mutated_objects) {
            let object = object.ok_or(HaneulError::InconsistentGatewayResult {
                error: format!(
            ...
            if object.is_package() {
                ...
                updated_gas = Some(self.to_haneul_object(object).await?);
            } else {
                created_objects.push(self.to_haneul_object(object).await?);
            }
        }
        let package = package
            .ok_or(HaneulError::InconsistentGatewayResult {
                error: "No package created".to_owned(),
            })?
            .into();

        let updated_gas = updated_gas.ok_or(HaneulError::InconsistentGatewayResult {
        ...
        Ok(HaneulParsedTransactionResponse::Publish(
            HaneulParsedPublishResponse {
                package,
                created_objects,
                updated_gas,
            },
    ...
    async fn create_split_coin_response(
        ...
        effects: TransactionEffects,
        equal_parts: bool,
    ) -> anyhow::Result<HaneulParsedTransactionResponse> {
        ...
        let (coin_object_id, split_arg) = match call.arguments.as_slice() {
            ...
            _ => {
                return Err(HaneulError::InconsistentGatewayResult {
        ...
        let updated_coin = self.get_haneul_object(coin_object_id).await?;
        let mut new_coins = Vec::with_capacity(created.len());
        for ((id, _, _), _) in created {
            new_coins.push(self.get_haneul_object(id).await?);
        }
        let updated_gas = self.get_haneul_object(&gas_payment).await?;
        ...
        Ok(HaneulParsedTransactionResponse::SplitCoin(
            HaneulParsedSplitCoinResponse {
                updated_coin,
                new_coins,
                updated_gas,
            },
    ...
    async fn create_merge_coin_response(
        ...
        Ok(HaneulParsedTransactionResponse::MergeCoin(
            HaneulParsedMergeCoinResponse {
                updated_coin,
                updated_gas,
            },
        ))
    ...
    fn try_get_move_call(certificate: &CertifiedTransaction) -> Result<&MoveCall, anyhow::Error> {
            ...
            certificate.signed_data.data.kind
        {
            ...
            Err(HaneulError::InconsistentGatewayResult {
                error: "Malformed transaction data".to_string(),
            }
            .into())
        }
    ...
    async fn create_move_call_transaction_kind(
        ...
        for json_arg in json_args {
            args.push(match json_arg {
                ...
                HaneulJsonCallArg::Pure(bytes) => CallArg::Pure(bytes),
    ...
    #[cfg(test)]
    pub fn highest_known_version(&self, object_id: &ObjectID) -> Result<SequenceNumber, HaneulError> {
        self.latest_object_ref(object_id)
            .map(|(_oid, seq_num, _digest)| seq_num)
    }

    #[cfg(test)]
    pub fn latest_object_ref(&self, object_id: &ObjectID) -> Result<ObjectRef, HaneulError> {
        self.store
    ...
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    async fn execute_transaction(
        ...
        return Ok(HaneulTransactionResponse {
            certificate: certificate.try_into()?,
            effects: HaneulTransactionEffects::try_from(effects, &self.module_cache)?,
            timestamp_ms: None,
            parsed_data,
        });
    }
    ...
    async fn pay(
        ...
        if let Some(gas) = gas {
            if used_coins.contains(&gas) {
                return Err(anyhow!("Gas coin is in input coins of Pay transaction, use PayHaneul transaction instead!"));
            }
        }
        let gas = self
    ...
    // https://github.com/GeunhwaJeong/haneul/issues/1045
    async fn sync_account_state(&self, account_addr: HaneulAddress) -> Result<(), anyhow::Error> {
        debug!(
            ?account_addr,
    ...
    async fn get_transaction(
        ...
        Ok(HaneulTransactionResponse {
            ...
            effects: HaneulTransactionEffects::try_from(effect, &self.module_cache)?,
            timestamp_ms: None,
            parsed_data: None,
        })
    }
