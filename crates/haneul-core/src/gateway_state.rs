...
use haneul_types::object::{ObjectFormatOptions, Owner};
use haneul_types::{
    base_types::*,
    coin,
    committee::Committee,
...
use haneul_json_rpc_api::rpc_types::{
    ...
    SplitCoinResponse, HaneulMoveObject, HaneulObject, HaneulObjectInfo, HaneulTransactionEffects,
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn get_haneul_object<T: HaneulMoveObject>(
        ...
        self.to_haneul_object(object)
    }

    fn to_haneul_object<T: HaneulMoveObject>(
        &self,
        object: Object,
    ) -> Result<HaneulObject<T>, anyhow::Error> {
    ...
    async fn create_publish_response(
        ...
        for ((obj_ref, _), object) in effects.mutated_and_created().zip(mutated_objects) {
            ...
            if object.is_package() {
                ...
                updated_gas = Some(self.to_haneul_object(object)?);
            } else {
                created_objects.push(self.to_haneul_object(object)?);
            }
        }
        let package = package
