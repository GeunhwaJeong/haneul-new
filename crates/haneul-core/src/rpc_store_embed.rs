...
//! Startup orchestration for the embedded `haneul-rpc-store` indexer.
//!
//! When a fullnode is configured with
//! [`RpcConfig::use_experimental_rpc_store`](haneul_config::RpcConfig::use_experimental_rpc_store),
//! it serves the rpc-api
//! index surface from an embedded [`haneul_rpc_store`] instance instead of
...
use haneul_consistent_store::ChainId;
use haneul_consistent_store::Db;
use haneul_consistent_store::DbOptions;
use haneul_consistent_store::PipelineTaskKey;
use haneul_consistent_store::Watermark;
use haneul_consistent_store::metrics::ColumnFamilyStatsCollector;
use haneul_consistent_store::restore::RestoreDriverConfig;
use haneul_consistent_store::restore::metrics::RestoreMetrics;
use haneul_indexer_alt_framework::IndexerArgs;
use haneul_indexer_alt_framework::ingestion::BoxedStreamingClient;
...
use haneul_rpc_store::ConsistencyConfig;
use haneul_rpc_store::HISTORY_COHORT;
use haneul_rpc_store::Indexer;
use haneul_rpc_store::LIVE_COHORT;
use haneul_rpc_store::METRICS_PREFIX;
use haneul_rpc_store::PipelineLayer;
use haneul_rpc_store::RestoreLayer;
use haneul_rpc_store::RpcStoreReader;
use haneul_rpc_store::RpcStoreSchema;
use haneul_rpc_store::Store as RpcStore;
use haneul_rpc_store::default_rocksdb_config;
use haneul_rpc_store::restore_indexes;
use haneul_rpc_store::seed_history_cohort;
...
/// Mirrors the standalone `haneul-rpc-node` default; since a snapshot is
...
impl EmbeddedRpcStore {
    ...
    pub async fn bootstrap(
        ...
        let store = haneul_consistent_store::Store::new(db.clone(), schema.clone());
    ...
    /// pruning ([`haneul_rpc_store::prune_history_cohort`]).
