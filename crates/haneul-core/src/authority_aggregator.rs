...
/// authority aggregator based on the Haneul system state committee/network information. This is
...
/// involve full-fledged network Haneul nodes (e.g. when we want to abstract out Narwhal). In order
...
pub trait TransactionCertifier: Sync + Send + 'static {
    /// This function first loads the Haneul system state object from `self_state`, get the committee
...
impl TransactionCertifier for LocalTransactionCertifier {
    async fn create_certificate(
        ...
        let haneul_system_state = self_store.get_haneul_system_state_object()?;
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn process_transaction(
                        ...
                        match result {
                            Ok(VerifiedTransactionInfoResponse {
                                ...
                                if inner_certificate.epoch() == self.committee.epoch {
                                    ...
                                    state.errors.push(
                                        HaneulError::WrongEpoch { expected_epoch: self.committee.epoch, actual_epoch: inner_certificate.epoch() }
                                    );
                            ...
                            Ok(VerifiedTransactionInfoResponse {
                                ...
                                if inner_signed_transaction.epoch() != self.committee.epoch {
                                    ...
                                    state.errors.push(
                                        HaneulError::WrongEpoch { expected_epoch: self.committee.epoch, actual_epoch: inner_signed_transaction.epoch() }
                                    );
                                    state.bad_stake += weight;
                                } else {
        ...
        if state.certificate.is_none() && !state.effects_map.effects_map.is_empty() {
            ...
            state.errors.push(
                HaneulError::QuorumFailedToFormEffectsCertWhenProcessingTransaction {
                    effects_map: state
                        .effects_map
                        .effects_map
                        .into_iter()
    ...
    ) -> Result<VerifiedCertifiedTransactionEffects, HaneulError> {
        #[derive(Default)]
