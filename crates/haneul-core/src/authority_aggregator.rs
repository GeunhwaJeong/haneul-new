...
impl<A, S: SignatureVerifier + Default> AuthorityAggregator<A, S> {
    ...
    ) -> HaneulResult<AuthorityAggregator<NetworkAuthorityClient, S>> {
        let network_clients =
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    where
        FMap: FnOnce(AuthorityName, &'a SafeClient<A, SV>) -> AsyncResult<'a, V, HaneulError> + Clone,
        FReduce: Fn(
            S,
            AuthorityName,
            StakeUnit,
    ...
    where
        FMap: Fn(AuthorityName, SafeClient<A, SV>) -> AsyncResult<'a, S, HaneulError>
            + Send
            + Clone
            + 'a,
        S: Send,
