...
use haneullabs_metrics::{GaugeGuard, MonitorCancellation, spawn_monitored_task};
...
use haneul_network::{
    DEFAULT_CONNECT_TIMEOUT_SEC, DEFAULT_REQUEST_TIMEOUT_SEC, default_haneullabs_network_config,
};
...
impl ProcessTransactionState {
    pub fn record_conflicting_transaction_if_any(
        ...
        if let HaneulErrorKind::ObjectLockConflict {
            obj_ref,
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    fn record_rpc_error_maybe(metrics: Arc<AuthAggMetrics>, display_name: &str, error: &HaneulError) {
        if let HaneulErrorKind::RpcError(_message, code) = error.as_inner() {
            metrics
                .total_rpc_err
