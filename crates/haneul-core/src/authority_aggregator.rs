...
use haneul_types::{fp_ensure, HANEUL_SYSTEM_STATE_OBJECT_ID};
...
use haneul_types::committee::{CommitteeWithNetAddresses, StakeUnit};
...
impl<A> AuthorityAggregator<A> {
    ...
    ) -> HaneulResult<AuthorityAggregator<NetworkAuthorityClient>> {
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn sync_authority_source_to_destination<CertHandler: CertificateHandler>(
        ...
        while let Some(target_cert) = missing_certificates.pop() {
            ...
            if attempted_certificates.contains(&cert_digest) {
                ...
                return Err(HaneulError::AuthorityInformationUnavailable);
            }
            ...
            let signed_effects = &transaction_info
                .signed_effects
                .ok_or(HaneulError::AuthorityInformationUnavailable)?;
    ...
    async fn sync_certificate_to_authority(
        ...
        if let Ok(res) = timeout(total_timeout, spawn_monitored_task!(async move {
            ...
            res.map_err(|e| HaneulError::CertificateSyncError {
    ...
    async fn sync_certificate_to_authority_with_timeout_inner<CertHandler: CertificateHandler>(
        ...
        let mut candidate_source_authorties: HashSet<AuthorityName> = cert
            ...
            .collect::<HaneulResult<HashSet<_>>>()?
            .iter()
            .map(|&&name| name)
            .collect::<HashSet<_>>();
            ...
            if let Ok(inner_res) = timeout(timeout_period, sync_fut).await {
                match inner_res {
                    ...
                    Err(err) => {
                        ...
                        let inner_err = HaneulError::PairwiseSyncFailed {
        ...
        Err(HaneulError::AuthorityUpdateFailure)
    }
    ...
    ) -> Result<S, HaneulError>
    where
        FMap: FnOnce(AuthorityName, &'a SafeClient<A>) -> AsyncResult<'a, V, HaneulError> + Clone,
        FReduce: Fn(
            S,
            AuthorityName,
            StakeUnit,
            Result<V, HaneulError>,
        ) -> AsyncResult<'a, ReduceOutput<S>, HaneulError>,
    {
    ...
    async fn quorum_once_inner<'a, S, FMap>(
        ...
        authority_errors: &mut HashMap<AuthorityName, HaneulError>,
    ) -> Result<S, HaneulError>
    where
        FMap: Fn(AuthorityName, SafeClient<A>) -> AsyncResult<'a, S, HaneulError> + Send + Clone + 'a,
        S: Send,
    {
        ...
        loop {
            ...
            type RequestResult<S> = Result<Result<S, HaneulError>, tokio::time::error::Elapsed>;
            ...
            let name = authorities_shuffled.next().ok_or_else(|| {
                ...
                HaneulError::from("Available authorities list is empty")
            })?;
                    ...
                    Event::Request(name, res) => {
                        match res {
                            ...
                            Err(_) => {
                                ...
                                authority_errors.insert(name, HaneulError::TimeoutError);
                            }
                            // request completed
                            Ok(inner_res) => {
        ...
        if let Some(t) = timeout_total {
            timeout(t, fut).await.map_err(|_timeout_error| {
                if authority_errors.is_empty() {
                    ...
                    HaneulError::TimeoutError
                } else {
                    HaneulError::TooManyIncorrectAuthorities {
    ...
    pub async fn get_committee_info(&self, epoch: Option<EpochId>) -> HaneulResult<CommitteeInfo> {
        #[derive(Default)]
                            ...
                            Ok(resp) => {
                                ...
                                if let Some(info) = resp.committee_info {
                                    ...
                                    *total_stake += weight;
                                    ...
                                    state.errors.push((
                                        name,
                                        HaneulError::from("Validator returns empty committee info."),
                                    ));
                                }
                            }
                            Err(err) => {
    ...
    /// Query validators for latest HaneulSystemState and try to form
    ...
    /// differences in HaneulSystemState as long as they all link to the same
    /// CommitteeWithNetAddresses.
    /// This function ignores HaneulSystemState that has older epoch than
    /// `minimal_epoch`.
    ...
    pub async fn get_committee_with_net_addresses(
    ...
    ) -> HaneulResult<CommitteeWithNetAddresses> {
        let (aggregate_object_info, _certificates) =
            ...
            self.get_object_by_id(HANEUL_SYSTEM_STATE_OBJECT_ID, true).await?;
                ...
                |(
                    ...
                    if let Some(object) = object_option {
                        let system_state = object.data.try_as_move().and_then(|move_object| {
                            bcs::from_bytes::<HaneulSystemState>(move_object.contents()).ok()
                        })?;
        ...
        let (committee, votes) = committee_and_votes
            .pop()
            .ok_or(HaneulError::FailedToGetAgreedCommitteeFromMajority { minimal_epoch })?;
        ...
        if votes >= self.committee.quorum_threshold() {
            ...
            Err(HaneulError::FailedToGetAgreedCommitteeFromMajority { minimal_epoch })
        }
    }
    ...
    > {
        ...
        struct GetObjectByIDRequestState {
            ...
            bad_weight: StakeUnit,
            responses: Vec<(AuthorityName, HaneulResult<VerifiedObjectInfoResponse>)>,
        }
    ...
    ) -> Result<(BTreeMap<ObjectRef, Vec<AuthorityName>>, Vec<AuthorityName>), HaneulError> {
        #[derive(Default)]
    ...
    ) -> Result<VerifiedCertificate, HaneulError> {
                    ...
                    Box::pin(async move {
                        match result {
                            ...
                            Err(err) => {
                                ...
                                if let HaneulError::ObjectLockConflict {
                                    obj_ref,
                                    pending_transaction,
                                } = err {
                            ...
                            Ok(ret) => {
                                ...
                                state.errors.push(
                                    HaneulError::UnexpectedResultFromValidatorHandleTransaction {
                                        err: format!("{:?}", ret),
                                    },
                                );
                                state.bad_stake += weight; // This is the bad stake counter
                        ...
                        if state.bad_stake > validity {
                            ...
                            return Err(HaneulError::QuorumFailedToProcessTransaction {
    ...
    pub async fn process_certificate(
        &self,
        certificate: CertifiedTransaction,
    ) -> Result<CertifiedTransactionEffects, HaneulError> {
        #[derive(Default)]
                        ...
                        match result {
                            ...
                            Err(err) => {
                                ...
                                if state.bad_stake > validity {
                                    ...
                                    return Err(HaneulError::QuorumFailedToExecuteCertificate { errors: state.errors });
                                }
                            }
        ...
        // If none has, fail.
        Err(HaneulError::QuorumFailedToExecuteCertificate {
            errors: state.errors,
        })
    }
    ...
    pub async fn get_object_info_execute(&self, object_id: ObjectID) -> HaneulResult<ObjectRead> {
    ...
    async fn fetch_one_object(
        ...
        timeout: Duration,
        sender: tokio::sync::mpsc::Sender<Result<Object, HaneulError>>,
    ) {
        ...
        // TODO: https://github.com/GeunhwaJeong/haneul/issues/320
        ...
        let mut ret_val: Result<Object, HaneulError> = Err(HaneulError::ObjectFetchFailed {
            object_id,
    ...
    pub async fn get_certified_checkpoint(
        ...
        authorities: &BTreeSet<AuthorityName>,
        timeout_total: Option<Duration>,
    ) -> HaneulResult<(CertifiedCheckpointSummary, Option<CheckpointContents>)> {
            ...
            |_, client| {
                ...
                Box::pin(async move {
                    ...
                    } = resp
                    {
                        ...
                        Err(HaneulError::GenericAuthorityError {
            ...
            |_authority, client| {
                Box::pin(async move {
                    ...
                    } = &resp
                    {
                        ...
                        // any data.
                        Err(HaneulError::TransactionNotFound { digest: *digest })
                    }
                })
            },
            self.timeouts.serial_authority_request_timeout,
    ...
    pub async fn handle_transaction_and_effects_info_request(
        ...
        timeout_total: Option<Duration>,
    ) -> HaneulResult<(VerifiedCertificate, SignedTransactionEffects)> {
        self.quorum_once_with_timeout(
            None,
            authorities,
            |authority, client| {
                    ...
                    match (resp.certified_transaction, resp.signed_effects) {
                        ...
                        _ => {
                            if authorities.is_some() {
                                ...
                                Err(HaneulError::TransactionNotFound {
    ...
    pub async fn execute_cert_to_true_effects(
        &self,
        cert: &CertifiedTransaction,
    ) -> HaneulResult<SignedTransactionEffects> {
        ...
        final_state
            .true_effects
            .ok_or(HaneulError::TooManyIncorrectAuthorities {
                errors: final_state.errors,
...
pub async fn reconfig_from_genesis(
    ...
    fp_ensure!(
        ...
        HaneulError::from("reconfig_from_genesis entails an authority aggregator with epoch 0")
    );
