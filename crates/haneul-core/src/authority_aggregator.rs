...
use haneullabs_metrics::{monitored_future, spawn_monitored_task, GaugeGuard};
...
use haneul_types::quorum_driver_types::GroupedErrors;
...
impl AuthAggMetrics {
    pub fn new(registry: &prometheus::Registry) -> Self {
        Self {
            ...
            .unwrap(),
            remaining_tasks_when_reaching_cert_quorum: haneullabs_metrics::histogram::Histogram::new_in_registry(
                "auth_agg_remaining_tasks_when_reaching_cert_quorum",
                "Number of remaining tasks when reaching certificate quorum",
                registry,
            ),
            remaining_tasks_when_cert_broadcasting_post_quorum_timeout: haneullabs_metrics::histogram::Histogram::new_in_registry(
...
pub fn group_errors(errors: Vec<(HaneulError, Vec<AuthorityName>, StakeUnit)>) -> GroupedErrors {
...
struct ProcessTransactionState {
    ...
    // This includes both object and package not found haneul errors.
...
impl ProcessTransactionState {
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(
        ...
        // and return Haneul::FailedToVerifyTxCertWithExecutedEffects.
        ...
        let invalid_sig_stake: StakeUnit = self
            ...
            .filter_map(|(e, _, stake)| {
                if matches!(e, HaneulError::FailedToVerifyTxCertWithExecutedEffects { .. }) {
                    Some(stake)
                } else {
                    None
                }
...
struct ProcessCertificateState {
    ...
    non_retryable_stake: StakeUnit,
    non_retryable_errors: Vec<(HaneulError, Vec<AuthorityName>, StakeUnit)>,
    retryable_errors: Vec<(HaneulError, Vec<AuthorityName>, StakeUnit)>,
    // As long as none of the exit criteria are met we consider the state retryable
    // 1) >= 2f+1 signatures
    // 2) >= f+1 non-retryable errors
    retryable: bool,
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_local_system_state(
        ...
        let validator_display_names = haneul_system_state
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    >(
        ...
        (
            R,
            FuturesUnordered<impl Future<Output = (AuthorityName, Result<V, HaneulError>)> + 'a>,
        ),
        S,
    >
    where
        FMap:
            FnOnce(AuthorityName, Arc<SafeClient<A>>) -> AsyncResult<'a, V, HaneulError> + Clone + 'a,
        FReduce: Fn(
                S,
                AuthorityName,
                StakeUnit,
    ...
    pub(crate) async fn quorum_map_then_reduce_with_timeout_and_prefs<'a, S, V, R, FMap, FReduce>(
        ...
        (
            R,
            FuturesUnordered<impl Future<Output = (AuthorityName, Result<V, HaneulError>)>>,
        ),
        S,
    >
    where
        FMap: FnOnce(AuthorityName, Arc<SafeClient<A>>) -> AsyncResult<'a, V, HaneulError> + Clone,
        FReduce: Fn(
            S,
            AuthorityName,
            StakeUnit,
    ...
    where
        FMap: Fn(AuthorityName, Arc<SafeClient<A>>) -> AsyncResult<'a, S, HaneulError>
            + Send
            + Clone
            + 'a,
        S: Send,
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = AuthorityAggregator::quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| HaneulError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    fn record_rpc_error_maybe(
        ...
        if let HaneulError::RpcError(_message, code) = error {
            metrics
                .total_rpc_err
    ...
    fn handle_process_transaction_response(
        ...
        response: HaneulResult<PlainTransactionInfoResponse>,
        name: AuthorityName,
        weight: StakeUnit,
    ) -> HaneulResult<Option<ProcessTransactionResult>> {
        match response {
    ...
    > {
        ...
        .map_err(|state| {
            ...
            for (haneul_err, _, _) in state.retryable_errors.iter().chain(state.non_retryable_errors.iter()) {
                self
                    .metrics
                    .total_aggregated_err
                    .with_label_values(&[
    ...
    fn handle_process_certificate_response(
        ...
        response: HaneulResult<HandleCertificateResponseV2>,
        name: AuthorityName,
    ) -> HaneulResult<Option<(VerifiedCertifiedTransactionEffects, TransactionEvents)>> {
        match response {
