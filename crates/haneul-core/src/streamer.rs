    ...
    F: Filter<T> + Clone + Send + Sync + 'static + Clone,
{
    pub fn spawn(
        ...
        let gauge = if let Some(metrics) = haneullabs_metrics::get_metrics() {
            metrics
                .channel_inflight
            ...
            haneullabs_metrics::init_metrics(&Registry::default());
        ...
        let (tx, rx) = haneullabs_metrics::metered_channel::channel(buffer, &gauge);
        let streamer = Self {
            streamer_queue: tx,
    ...
    pub async fn send(&self, data: T) -> Result<(), HaneulError> {
        self.streamer_queue
            .send(data)
            .await
            .map_err(|e| HaneulError::FailedToDispatchSubscription {
                error: e.to_string(),
            })
    }
}
