...
use haneul_types::{
    ...
    crypto::{get_key_pair, AccountKeyPair, AuthoritySignature, HaneulAuthoritySignature},
    error::HaneulError,
    gas::HaneulGasStatus,
...
async fn test_start_epoch_change() {
    ...
    let (inner_temporary_store, effects, _) = execution_engine::execute_transaction_to_effects(
        ...
        HaneulGasStatus::new_with_budget(1000, 1.into(), 1.into()),
        state.epoch(),
    );
...
async fn test_finish_epoch_change() {
    ...
    for active in actives {
        ...
        let system_state = active.state.get_haneul_system_state_object().await.unwrap();
        assert_eq!(system_state.epoch, 1);
        let (_, tx_digest) = active
            .state
            .get_latest_parent_entry(HANEUL_SYSTEM_STATE_OBJECT_ID)
            .await
            .unwrap()
            .unwrap();
        let response = active
