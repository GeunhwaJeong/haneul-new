    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone + Reconfigurable,
{
    ...
    pub async fn start_epoch_change(&self) -> HaneulResult {
    ...
    pub async fn finish_epoch_change(&self) -> HaneulResult {
        let epoch = self.state.committee.load().epoch;
        ...
        let haneul_system_state = self.state.get_haneul_system_state_object().await?;
        let next_epoch = epoch + 1;
        let new_committee = haneul_system_state.get_next_epoch_committee();
        debug!(
            ?epoch,
        ...
        let new_clients = if A::needs_network_recreation() {
            self.recreate_network(haneul_system_state)?
        } else {
    ...
    ) -> HaneulResult<BTreeMap<AuthorityName, A>> {
                                  ...
                                  }) = result
                        {
                            ...
                            match result {
                                ...
                                Ok(_) => state.errors.push((
                                    name,
                                    HaneulError::from("Epoch info not yet available"),
                                )),
                            }
    ...
    /// https://github.com/GeunhwaJeong/haneul/issues/3915
    async fn wait_for_validator_batch(&self) -> HaneulResult {
