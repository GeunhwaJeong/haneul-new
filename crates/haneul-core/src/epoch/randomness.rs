...
use haneul_types::messages_consensus::VersionedDkgMessage;
use haneul_types::messages_consensus::{ConsensusTransaction, VersionedDkgConfirmation};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> HaneulResult {
    ...
    pub fn reserve_next_randomness(
        ...
        batch: &mut DBBatch,
    ) -> HaneulResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
