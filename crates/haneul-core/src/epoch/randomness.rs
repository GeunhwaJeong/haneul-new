...
pub struct Inner {
    ...
    network_handle: haneul_network::randomness::Handle,
...
impl RandomnessManager {
    ...
    pub fn start_dkg(&self) -> HaneulResult {
    ...
    pub fn advance_dkg(&self, batch: &mut DBBatch) -> HaneulResult {
        self.inner.lock().unwrap().advance_dkg(batch)
    }

    /// Adds a received dkg::Message to the randomness DKG state machine.
    pub fn add_message(&self, batch: &mut DBBatch, msg: dkg::Message<PkG, EncG>) -> HaneulResult {
    ...
    pub fn reserve_next_randomness(&self, batch: &mut DBBatch) -> HaneulResult<RandomnessRound> {
    ...
    /// round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> HaneulResult {
        self.inner
            .lock()
            .unwrap()
...
impl Inner {
    pub fn start_dkg(&mut self) -> HaneulResult {
    ...
    pub fn advance_dkg(&mut self, batch: &mut DBBatch) -> HaneulResult {
        let epoch_store = self.epoch_store()?;
    ...
    pub fn add_message(&mut self, batch: &mut DBBatch, msg: dkg::Message<PkG, EncG>) -> HaneulResult {
        if self.used_messages.initialized() || self.dkg_output.initialized() {
    ...
    pub fn reserve_next_randomness(&mut self, batch: &mut DBBatch) -> HaneulResult<RandomnessRound> {
    ...
    fn epoch_store(&self) -> HaneulResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store.upgrade().ok_or(HaneulError::EpochEnded)
    }

    fn tables(&self) -> HaneulResult<Arc<AuthorityEpochTables>> {
    ...
    async fn test_dkg() {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                haneul_network::randomness::Handle::new_stub(),
