...
pub trait ConsensusSender: Send + Sync + 'static {
    ...
    fn send_to_consensus(&self, fragment: SignedCheckpointFragmentMessage) -> Result<(), HaneulError>;
}
...
impl CheckpointStore {
    ...
    ) -> Result<Option<AuthenticatedCheckpoint>, HaneulError> {
    ...
    ) -> Result<Option<CheckpointDigest>, HaneulError> {
    ...
    pub fn set_locals_for_testing(&mut self, locals: CheckpointLocals) -> Result<(), HaneulError> {
    ...
    pub fn set_consensus(&mut self, sender: Box<dyn ConsensusSender>) -> Result<(), HaneulError> {
    ...
    ) -> Result<CheckpointStore, HaneulError> {
        let tables =
    ...
    pub fn handle_proposal(&mut self, detail: bool) -> Result<CheckpointResponse, HaneulError> {
    ...
    pub fn submit_local_fragment_to_consensus(
        ...
        fp_ensure!(
            ...
            HaneulError::from("Fragment can only be submitted by the proposer")
        );
        ...
        fp_ensure!(
            ...
            HaneulError::from("Fragment does not involve this node")
        );
        ...
        if let Some(sender) = &self.sender {
            ...
            return Err(HaneulError::from("No consensus sender configured"));
        }
    ...
    pub fn attempt_to_construct_checkpoint(&mut self) -> HaneulResult<BTreeSet<ExecutionDigests>> {
        ...
        fp_ensure!(
            ...
            HaneulError::from("The checkpoint span graph under construction is for a different checkpoint than the next expected checkpoint. This means consensus is really behind.")
        );
    ...
    pub fn filter_already_checkpointed_transactions<'a>(
        &mut self,
        transactions: impl Iterator<Item = &'a ExecutionDigests> + Clone,
    ) -> HaneulResult<BTreeSet<ExecutionDigests>> {
    ...
    pub fn set_proposal(&mut self, epoch: EpochId) -> Result<CheckpointProposal, HaneulError> {
        // Check that:
    ...
    fn check_checkpoint_transactions<'a>(
        ...
        fp_ensure!(
            ...
            HaneulError::CheckpointingError {
    ...
    fn update_new_checkpoint_inner(
        ...
        if seq != expected_seq {
            return Err(HaneulError::CheckpointingError {
