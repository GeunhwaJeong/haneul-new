...
impl CheckpointStore {
    ...
    fn load_locals(&mut self, epoch: EpochId) -> Result<CheckpointLocals, HaneulError> {
    ...
    fn attempt_to_construct_checkpoint(
        ...
        if let Some(reconstructed) = _potential_checkpoint {
            ...
            Err(FragmentInternalError::Error(HaneulError::from(
        ...
        match &current {
            ...
            None => {
                if let &Some(contents) = &contents {
                    ...
                    return Err(HaneulError::from("No checkpoint set at this sequence."));
                }
            }
    ...
    pub fn new_proposal(&mut self, epoch: EpochId) -> Result<CheckpointProposal, HaneulError> {
        ...
        if sequence_number > 0 {
            ...
            if !matches!(
                ...
                return Err(HaneulError::from("Cannot propose before having a certificate"));
            }
        }
    ...
    pub fn set_proposal(&mut self, epoch: EpochId) -> Result<CheckpointProposal, HaneulError> {
        ...
        if self.extra_transactions.iter().count() == 0 {
            return Err(HaneulError::from("Cannot propose an empty set."));
        }
    ...
    fn update_processed_transactions(
        ...
        transactions: &[(TxSequenceNumber, ExecutionDigests)],
    ) -> Result<CheckpointSequenceNumber, HaneulError> {
