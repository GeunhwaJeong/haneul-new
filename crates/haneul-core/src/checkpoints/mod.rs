...
async fn diagnose_split_brain(
    ...
    // #[cfg(msim)]
    // haneul_simulator::task::kill_current_node(Some(Duration::from_secs(100)));
}
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self, checkpoint: &PendingCheckpoint) -> HaneulResult;
}
...
impl CheckpointServiceNotify for CheckpointServiceNoop {
    ...
    fn notify_checkpoint(&self, _: &PendingCheckpoint) -> HaneulResult {
        Ok(())
    }
}
...
mod tests {
    ...
    impl EffectsNotifyRead for HashMap<TransactionDigest, TransactionEffects> {
        ...
        async fn notify_read_executed_effects_digests(
            &self,
            digests: Vec<TransactionDigest>,
        ) -> HaneulResult<Vec<TransactionEffectsDigest>> {
            Ok(digests
                .into_iter()
                .map(|d| {
                    self.get(&d)
