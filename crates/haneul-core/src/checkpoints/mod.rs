...
use haneul_types::accumulator_metadata;
...
pub struct CheckpointRoots {
    pub tx_roots: Vec<TransactionKey>,
...
impl CheckpointStore {
    ...
    pub fn new_for_tests() -> Arc<Self> {
        let ckpt_dir = haneullabs_common::tempdir().unwrap();
    ...
    fn prune_local_summaries(&self) -> HaneulResult {
...
impl<HaneulError: std::error::Error + Send + Sync + 'static> From<HaneulError>
...
impl CheckpointBuilder {
    ...
    async fn resolve_checkpoint_transactions(
        &self,
        pending: PendingCheckpoint,
    ) -> HaneulResult<(Vec<TransactionEffects>, HashSet<TransactionDigest>)> {
        ...
        for checkpoint_roots in &pending.roots {
            ...
            self.metrics
                ...
                .inc_by(tx_roots.len() as u64);

            let root_digests = self
                .epoch_store
                .notify_read_tx_key_to_digest(tx_roots)
    ...
    ) -> HaneulResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    ) -> CheckpointBuilderResult<HaneulSystemState> {
...
impl CheckpointAggregator {
    ...
    async fn run(mut self) {
        ...
        loop {
            // Drain all signatures that arrived since the last iteration into the pending buffer
            while let Ok(sig) = self.receiver.try_recv() {
                self.pending
                    .entry(sig.summary.sequence_number)
                    .or_default()
                    .push(sig);
            }
    ...
    async fn run_and_notify(&mut self) -> HaneulResult {
    ...
    fn run_inner(&mut self) -> HaneulResult<Vec<CertifiedCheckpointSummary>> {
        ...
        'outer: loop {
            ...
            let seq = current.summary.sequence_number;
            let sigs = self.pending.remove(&seq).unwrap_or_default();
            if sigs.is_empty() {
                trace!(
                    checkpoint_seq =? seq,
    ...
    fn next_checkpoint_to_certify(&self) -> HaneulResult<CheckpointSequenceNumber> {
        Ok(self
            .store
            .tables
            .certified_checkpoints
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed { error }
                if matches!(
                    error.as_inner(),
                    HaneulErrorKind::StakeAggregatorRepeatedSigner {
...
pub trait CheckpointServiceNotify {
    fn notify_checkpoint_signature(&self, info: &CheckpointSignatureMessage) -> HaneulResult;
...
impl CheckpointServiceNotify for CheckpointServiceNoop {
    fn notify_checkpoint_signature(&self, _: &CheckpointSignatureMessage) -> HaneulResult {
        Ok(())
    }
...
mod tests {
    ...
    use haneul_types::base_types::{SequenceNumber, TransactionEffectsDigest};
    ...
    async fn test_clear_locally_computed_checkpoints_from_deletes_inclusive_range() {
        ...
        let protocol = haneul_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE();
        for seq in 70u64..=80u64 {
            let contents =
                haneul_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests(
                    [haneul_types::base_types::ExecutionDigests::new(
                        haneul_types::digests::TransactionDigest::random(),
                        haneul_types::digests::TransactionEffectsDigest::ZERO,
                    )],
                );
            let summary = haneul_types::messages_checkpoint::CheckpointSummary::new(
                &protocol,
                0,
                seq,
                0,
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519HaneulSignature(Default::default()).into();
            state
    ...
    impl TransactionCacheRead for HashMap<TransactionDigest, TransactionEffects> {
        fn notify_read_executed_effects_may_fail(
            ...
            _: &str,
            digests: &[TransactionDigest],
        ) -> BoxFuture<'_, HaneulResult<Vec<TransactionEffects>>> {
