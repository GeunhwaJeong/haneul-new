...
use haneul_types::messages::{TransactionEffects, VerifiedExecutableTransaction};
...
impl CheckpointBuilder {
    ...
    async fn augment_epoch_last_checkpoint(
        ...
        self.epoch_store.record_is_safe_mode_metric(
            self.state.get_haneul_system_state_object().unwrap().safe_mode,
        );
...
impl CheckpointAggregator {
    ...
    async fn run_inner(&mut self) -> HaneulResult {
...
impl CheckpointServiceNotify for CheckpointServiceNoop {
    ...
    fn notify_checkpoint(&self, _: &AuthorityPerEpochStore, _: PendingCheckpoint) -> HaneulResult {
        Ok(())
    }
}
...
impl CheckpointTailer {
    ...
    async fn do_run(&mut self) -> HaneulResult<bool> {
        loop {
            ...
            let Some(content) = content else {
                return Err(HaneulError::from("Checkpoint summary for sequence {} exists, but content does not. This should not happen"));
            };
...
mod tests {
    ...
    pub async fn checkpoint_builder_test() {
        ...
        let network_config = haneul_config::builder::ConfigBuilder::new(&dir).build();
