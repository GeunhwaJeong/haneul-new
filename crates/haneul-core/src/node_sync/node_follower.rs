    ...
    ResultT: Clone,
{
    ...
    async fn notify(&self, key: &Key, res: ResultT) -> HaneulResult {
        if let Some(tx) = self.waiters.lock().unwrap().remove(key) {
            tx.send(res).map_err(|_| HaneulError::GenericAuthorityError {
...
struct DigestsMessage {
    sync_arg: SyncArg,
    tx: Option<oneshot::Sender<HaneulResult>>,
}

impl DigestsMessage {
    fn new_for_ckpt(digests: &ExecutionDigests, tx: oneshot::Sender<HaneulResult>) -> Self {
        Self {
    ...
    fn new_for_exec_driver(digest: &TransactionDigest, tx: oneshot::Sender<HaneulResult>) -> Self {
        Self {
    ...
    fn new(
        ...
        peer: AuthorityName,
        tx: oneshot::Sender<HaneulResult>,
    ) -> Self {
        Self {
...
pub struct NodeSyncState<A> {
    ...
    pending_downloads: Arc<Waiter<TransactionDigest, HaneulResult>>,
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    fn start(self: Arc<Self>) -> (JoinHandle<()>, mpsc::Sender<DigestsMessage>) {
        ...
        let mut receiver = match state.receiver.clone().try_lock_owned() {
            Err(_) => {
                ...
                // better to change NodeSyncState::start() to return HaneulResult but that turns out
        ...
        match self.state.handle_certificate(cert.clone()).await {
            ...
            Err(HaneulError::ObjectErrors { .. }) => {
                ...
                if errors.is_empty() {
                    ...
                    Err(HaneulError::ExecutionDriverError {
                        digest: *digest,
    ...
    async fn process_digest(&self, arg: SyncArg, permit: OwnedSemaphorePermit) -> HaneulResult {
        ...
        let (cert, effects) = match req {
            ...
            DownloadRequest::Validator(digest) => {
                ...
                match resp {
                    ...
                    _ => return Err(HaneulError::TransactionNotFound { digest: *digest }),
                }
            }
        };
...
impl NodeSyncHandle {
    ...
    fn map_rx(rx: oneshot::Receiver<HaneulResult>) -> BoxFuture<'static, HaneulResult> {
        Box::pin(rx.map(|res| {
            let res = res.map_err(|e| HaneulError::GenericAuthorityError {
                error: e.to_string(),
            });
            match res {
                Ok(r) => r,
    ...
    ) -> HaneulResult<impl Stream<Item = HaneulResult>> {
...
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    type DigestResult = BoxFuture<'static, HaneulResult>;

    async fn handle_digest(
        &self,
        follower: &Follower<A>,
        digests: ExecutionDigests,
    ) -> HaneulResult<Self::DigestResult> {
        let (tx, rx) = oneshot::channel();
        let sender = self.sender.clone();
