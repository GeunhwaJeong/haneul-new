    ...
    Handler: SyncHandler,
{
    ...
    let result_block = |fut, seq, digests| async move {
        ...
        Ok::<TxSequenceNumber, HaneulError>(seq)
    };
...
mod test {
    ...
    use test_utils::{
        ...
        messages::make_transfer_haneul_transaction,
    };
        ...
        async fn handle_digest(
            ...
            if let Some(after) = *self.break_after.lock().unwrap() {
                if seq > after {
                    ...
                    return Err(HaneulError::GenericAuthorityError { error: "".into() });
                }
            }
            Ok(Box::pin(async move {
                Ok::<SyncStatus, HaneulError>(SyncStatus::CertExecuted)
            }))
        }
    }
