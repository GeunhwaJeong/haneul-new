...
pub type SyncResult = HaneulResult<SyncStatus>;
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn handle_messages(self: Arc<Self>, receiver: &mut mpsc::Receiver<DigestsMessage>) {
        ...
        }) = receiver.recv().await
        {
            ...
            spawn_monitored_task!(async move {
                ...
                .await
                .map_err(|_| HaneulError::TimeoutError);

                let tx_digest = sync_arg.transaction_digest();

                let res = match res {
                    Err(error) | Ok(Err(error)) => {
                        if matches!(error, HaneulError::ValidatorHaltedAtEpochEnd) {
    ...
    fn send_sync_res_to_receiver(
        tx: Option<oneshot::Sender<Result<SyncStatus, HaneulError>>>,
        res: Result<SyncStatus, HaneulError>,
        sync_arg: &SyncArg,
        epoch_id: &EpochId,
    ) {
        if let Some(tx) = tx {
    ...
    async fn process_exec_driver_digest(
        ...
        match result {
            Ok(_) => Ok(SyncStatus::CertExecuted),
            e @ Err(HaneulError::TransactionInputObjectsErrors { .. }) => {
                debug!(
                    ?digest,
...
impl NodeSyncHandle {
    ...
    ) -> HaneulResult<impl Stream<Item = SyncResult>> {
    ...
    pub async fn handle_sync_digest(
        ...
        peer: AuthorityName,
        digests: ExecutionDigests,
    ) -> HaneulResult<BoxFuture<'static, SyncResult>> {
        let (tx, rx) = oneshot::channel();
        let sender = self.sender.clone();
...
mod tests {
    ...
    fn test_display_execution_driver_error() {
        ...
        let err0 = HaneulError::ExecutionDriverError {
            digest,
            msg: "test 0".into(),
            errors: Vec::new(),
        };
        let err1 = HaneulError::ExecutionDriverError {
            digest,
            msg: "test 1".into(),
            errors: vec![err0],
        };
        let err2 = HaneulError::ExecutionDriverError {
            digest,
            msg: "test 2".into(),
            errors: vec![err1],
        };
