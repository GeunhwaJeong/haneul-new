...
fn execute_transaction<S: BackingPackageStore + ParentSync + ChildObjectResolver>(
    ...
    move_vm: &Arc<MoveVM>,
    native_functions: &NativeFunctionTable,
    mut gas_status: HaneulGasStatus,
    ...
    if result.is_ok() {
        ...
        for single_tx in transaction_data.kind.into_single_transactions() {
            result = match single_tx {
                ...
                SingleTransactionKind::TransferHaneul(TransferHaneul { recipient, amount }) => {
                    let gas_object = temporary_store
                        .objects()
                        .get(&gas_object_id)
                        .expect("We constructed the object map so it should always have the gas object id")
                        .clone();
                    transfer_haneul(temporary_store, gas_object, recipient, amount, tx_ctx)
                }
                ...
                SingleTransactionKind::PayAllHaneul(PayAllHaneul { coins, recipient }) => {
                    let mut coin_objects: Vec<Object> =  // unwrap is is safe because we built the object map from the transaction
                    coins.iter().map(|c|
                    temporary_store
                        .objects()
            ...
            Data::Move(move_obj) => {
                if !Coin::is_coin(&move_obj.type_) {
                    return Err(ExecutionError::new_with_source(
                        ...
                        "Provided non-Coin<T> object as input to pay/pay_haneul/pay_all_haneul transaction".to_string(),
                    ));
                }
                if let Some(typ) = &coin_type {
                    if typ != &move_obj.type_ {
                        return Err(ExecutionError::new_with_source(
                            ...
                            format!("Coin type check failed in pay/pay_haneul/pay_all_haneul transaction, expected: {:?}, found: {:}", typ, move_obj.type_),
                        ));
                    }
                } else {
...
fn check_recipients(recipients: &[HaneulAddress], amounts: &[u64]) -> Result<(), ExecutionError> {
...
/// Transfer the gas object (which is a HANEUL coin object) with an optional `amount`.
/// If `amount` is specified, the gas object remains in the original owner, but a new HANEUL coin
...
fn test_pay_empty_coins() {
    ...
    let recipients = vec![HaneulAddress::random_for_testing_only()];
    let amounts = vec![10];
...
fn test_pay_insufficient_balance() {
    let coin_objects = vec![
        Object::new_gas_coin_for_testing(10, HaneulAddress::random_for_testing_only()),
        Object::new_gas_coin_for_testing(5, HaneulAddress::random_for_testing_only()),
    ];
    let recipients = vec![
        HaneulAddress::random_for_testing_only(),
        HaneulAddress::random_for_testing_only(),
...
#[test]
fn test_pay_haneul_success_one_input_coin() {
    let sender = HaneulAddress::random_for_testing_only();
...
#[test]
fn test_pay_haneul_success_multiple_input_coins() {
    let sender = HaneulAddress::random_for_testing_only();
    ...
    let recipient1 = HaneulAddress::random_for_testing_only();
    let recipient2 = HaneulAddress::random_for_testing_only();
    let recipient3 = HaneulAddress::random_for_testing_only();
...
#[test]
fn test_pay_all_haneul_success_multiple_input_coins() {
    let sender = HaneulAddress::random_for_testing_only();
    let coin1 = Object::new_gas_coin_for_testing(30, sender);
    let coin2 = Object::new_gas_coin_for_testing(20, sender);
    let coin3 = Object::new_gas_coin_for_testing(10, sender);
