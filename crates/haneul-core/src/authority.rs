...
pub use authority_store::{
    AuthorityStore, AuthorityStoreWrapper, GatewayStore, HaneulDataStore, UpdateType,
};
...
// for cases such as local tests or "haneul start" which starts multiple authorities in one process.
...
impl AuthorityState {
    ...
    ) -> Result<Option<TransactionInfoResponse>, HaneulError> {
    ...
    > {
        ...
        if request.length > MAX_ITEMS_LIMIT {
            return Err(HaneulError::TooManyItemsError(MAX_ITEMS_LIMIT));
        }
    ...
    pub(crate) fn insert_new_epoch_info(&self, new_committee: &Committee) -> HaneulResult {
        let current_epoch_info = self.database.get_last_epoch_info()?;
        fp_ensure!(
            current_epoch_info.committee.epoch <= new_committee.epoch,
            HaneulError::InconsistentEpochState {
    ...
    pub(crate) fn unhalt_validator(&self) -> HaneulResult {
    ...
    async fn shared_locks_exist(&self, certificate: &CertifiedTransaction) -> HaneulResult<bool> {
        let digest = certificate.digest();
            ...
            ConsensusTransaction::Checkpoint(fragment) => {
                ...
                if let Some(checkpoint) = &self.checkpoints {
                    checkpoint
                        ...
                        .map_err(|e| HaneulError::from(&e.to_string()[..]))?;
...
impl ExecutionStateError for FragmentInternalError {
    ...
    fn to_string(&self) -> String {
        match self {
            Self::Error(haneul_error) => format!("Failed to process checkpoint fragment {haneul_error}"),
