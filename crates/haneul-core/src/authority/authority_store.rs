...
use haneul_types::{batch::TxSequenceNumber, object::PACKAGE_VERSION};
use tokio_retry::strategy::{jitter, ExponentialBackoff};
use tracing::{debug, info, trace};
use typed_store::rocks::DBBatch;
use typed_store::traits::Map;

pub type AuthorityStore = HaneulDataStore<AuthoritySignInfo>;
...
impl<S: Eq + Debug + Serialize + for<'de> Deserialize<'de>> HaneulDataStore<S> {
    ...
    pub fn next_sequence_number(&self) -> Result<TxSequenceNumber, HaneulError> {
        Ok(self
            .perpetual_tables
            .executed_sequence
            .iter()
    ...
    pub fn all_pending_certificates(&self) -> HaneulResult<Vec<VerifiedCertificate>> {
        Ok(self
            .epoch_tables()
    ...
    pub fn store_pending_certificates(&self, certs: &[VerifiedCertificate]) -> HaneulResult<()> {
        let batch = self
            .epoch_tables()
            .pending_certificates
            .batch()
    ...
    pub fn pending_certificate_exists(&self, tx: &TransactionDigest) -> Result<bool, HaneulError> {
        Ok(self.epoch_tables().pending_certificates.contains_key(tx)?)
    }

    /// Deletes one pending certificate.
    pub fn remove_pending_certificate(&self, digest: &TransactionDigest) -> HaneulResult<()> {
        self.epoch_tables().pending_certificates.remove(digest)?;
        Ok(())
    }

    /// Deletes all pending certificates in the epoch.
    pub fn cleanup_pending_certificates(&self) -> HaneulResult<()> {
    ...
    pub async fn get_tx_sequence(
        &self,
        tx: TransactionDigest,
    ) -> HaneulResult<Option<TxSequenceNumber>> {
    ...
    pub async fn check_owned_locks(&self, owned_object_refs: &[ObjectRef]) -> HaneulResult {
        self.lock_service
    ...
    pub async fn reset_transaction_lock(&self, owned_input_objects: &[ObjectRef]) -> HaneulResult {
    ...
    pub fn load_reconfig_state(&self) -> HaneulResult<ReconfigState> {
    ...
    pub fn sent_end_of_publish(&self, authority: &AuthorityName) -> HaneulResult<bool> {
    ...
    ) -> HaneulResult<Option<(u64, Vec<TransactionDigest>)>> {
    ...
    ) -> HaneulResult<Vec<(u64, ExecutionDigests)>> {
        Ok(self
            .perpetual_tables
            .executed_sequence
            .iter()
    ...
    pub fn last_consensus_index(&self) -> HaneulResult<ExecutionIndicesWithHash> {
        self.epoch_tables()
...
impl<S: Eq + Debug + Serialize + for<'de> Deserialize<'de>> ParentSync for HaneulDataStore<S> {
...
impl<S: Eq + Debug + Serialize + for<'de> Deserialize<'de>> ModuleResolver for HaneulDataStore<S> {
