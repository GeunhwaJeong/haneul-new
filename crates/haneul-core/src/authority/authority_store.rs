...
use haneul_types::messages::AuthenticatedEpoch;
...
impl<S: Eq + Debug + Serialize + for<'de> Deserialize<'de>> HaneulDataStore<S> {
        ...
        match self.wal.begin_tx(digest, cert).await? {
            ...
            None => {
                ...
                let err = "previous attempt of transaction resulted in an error - \
                    ...
                    .to_owned();
                debug!(?digest, "{}", err);
                Err(HaneulError::ErrorWhileProcessingConfirmationTransaction { err })
            }
        }
    }
    ...
    pub fn init_genesis_epoch(&self, genesis_committee: Committee) -> HaneulResult {
    ...
    pub fn sign_new_epoch(
        ...
        fp_ensure!(
            ...
            HaneulError::from("Unexpected new epoch number")
        );
    ...
    pub fn store_epoch_cert(&self, cert: CertifiedEpoch) -> HaneulResult {
