...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_haneul_conservation_check = config
    ...
    pub fn get_marker_value(
        ...
        epoch_id: EpochId,
    ) -> HaneulResult<Option<MarkerValue>> {
    ...
    pub fn get_latest_marker(
        ...
        object_id: &ObjectID,
        epoch_id: EpochId,
    ) -> HaneulResult<Option<(SequenceNumber, MarkerValue)>> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> HaneulResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> HaneulResult<()> {
    ...
    fn get_lock_v1(&self, obj_ref: ObjectRef, epoch_id: EpochId) -> HaneulLockResult {
        Ok(
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> HaneulResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    pub fn revert_state_update(&self, tx_digest: &TransactionDigest) -> HaneulResult {
    ...
    pub fn expensive_check_haneul_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
...
impl AccumulatorStore for AuthorityStore {
    ...
    fn get_root_state_accumulator_for_epoch(
        &self,
        epoch: EpochId,
    ) -> HaneulResult<Option<(CheckpointSequenceNumber, Accumulator)>> {
        self.perpetual_tables
    ...
    fn get_root_state_accumulator_for_highest_epoch(
        &self,
    ) -> HaneulResult<Option<(EpochId, (CheckpointSequenceNumber, Accumulator))>> {
        Ok(self
            .perpetual_tables
            .root_state_hash_by_epoch
            .safe_iter()
