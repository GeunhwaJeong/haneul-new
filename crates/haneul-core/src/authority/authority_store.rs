...
impl AuthorityStore {
    ...
    pub(crate) fn get_root_state_hash(&self, epoch: EpochId) -> HaneulResult<ECMHLiveObjectSetDigest> {
        let acc = self
            .perpetual_tables
    ...
    pub(crate) fn is_tx_already_executed(&self, digest: &TransactionDigest) -> HaneulResult<bool> {
        Ok(self
            .perpetual_tables
            .executed_effects
            .contains_key(digest)?)
    ...
    pub(crate) fn database_is_empty(&self) -> HaneulResult<bool> {
    ...
    pub(crate) fn check_owned_object_locks_exist(&self, objects: &[ObjectRef]) -> HaneulResult {
        let locks = self
            .perpetual_tables
            .owned_object_transaction_locks
            .multi_get(objects)?;
    ...
    pub(crate) async fn revert_state_update(&self, tx_digest: &TransactionDigest) -> HaneulResult {
