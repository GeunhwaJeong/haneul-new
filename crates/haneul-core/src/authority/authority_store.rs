...
impl AuthorityStore {
    ...
    async fn bulk_object_insert(&self, objects: &[&Object]) -> HaneulResult<()> {
...
impl ParentSync for AuthorityStore {
    fn get_latest_parent_entry_ref(&self, object_id: ObjectID) -> HaneulResult<Option<ObjectRef>> {
...
impl key_value_store::TransactionKeyValueStore for AuthorityStore {
    ...
    ) -> HaneulResult<Vec<Option<key_value_store::Value>>> {
    ...
    ) -> HaneulResult<Vec<Option<Transaction>>> {
        Ok(self
