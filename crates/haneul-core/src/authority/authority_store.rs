...
impl<S: Eq + Debug + Serialize + for<'de> Deserialize<'de>> HaneulDataStore<S> {
    ...
    pub fn remove_pending_digests(&self, seqs: Vec<InternalSequenceNumber>) -> HaneulResult<()> {
        let batch = self.epoch_tables().pending_execution.batch();
    ...
    pub fn get_input_objects(&self, objects: &[InputObjectKind]) -> Result<Vec<Object>, HaneulError> {
        let mut result = Vec::new();
