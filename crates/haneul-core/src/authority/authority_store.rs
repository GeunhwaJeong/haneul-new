...
use haneul_storage::mutex_table::{LockGuard, MutexTable};
...
impl AuthorityStore {
    ...
    pub(crate) async fn revert_uncommitted_epoch_transactions(&self) -> HaneulResult {
    ...
    ) -> HaneulResult<Guard<Arc<AuthorityPerEpochStore>>> {
        let store = self.epoch_store.load();
        fp_ensure!(
            store.epoch() == intended_epoch,
            HaneulError::StoreAccessEpochMismatch {
    ...
    ) -> HaneulResult<Option<SignedTransactionEffects>> {
        Ok(self
            .perpetual_tables
            .executed_effects
    ...
    pub fn effects_exists(&self, transaction_digest: &TransactionDigest) -> HaneulResult<bool> {
    ...
    pub fn get_owner_objects(&self, owner: Owner) -> Result<Vec<ObjectInfo>, HaneulError> {
    ...
    pub fn get_owner_objects_iterator(
        &self,
        owner: Owner,
    ) -> Result<impl Iterator<Item = ObjectInfo> + '_, HaneulError> {
        debug!(?owner, "get_owner_objects");
        Ok(self
            .perpetual_tables
            .owner_index
    ...
    pub fn check_input_objects(
        ...
        if !errors.is_empty() {
            Err(HaneulError::TransactionInputObjectsErrors { errors })
        } else {
            Ok(result)
        }
    }
                    ...
                    match shared_locks.get(id) {
                        Some(version) => {
                            if let Some(obj) = self.get_object_by_key(id, *version)? {
                                ...
                                // When this happens, other transactions that use smaller versions of
                                // this shared object haven't finished execution.
                                errors.push(HaneulError::SharedObjectPriorVersionsPendingExecution {
                                    object_id: *id,
                        ...
                        None => {
                            errors.push(HaneulError::SharedObjectLockNotSetError);
                            continue;
                        }
                    }
                }
    ...
    /// Returns HaneulError::ObjectNotFound if no lock records for the given object can be found.
    /// Returns HaneulError::ObjectVersionUnavailableForConsumption if the object record is at a different version.
    ...
    pub async fn get_object_locking_transaction(
        ...
        let lock_info = match lock_info {
            ObjectLockStatus::LockedAtDifferentVersion { locked_ref } => {
                return Err(HaneulError::ObjectVersionUnavailableForConsumption {
    ...
    pub fn parent(&self, object_ref: &ObjectRef) -> Result<Option<TransactionDigest>, HaneulError> {
        self.perpetual_tables
            .parent_sync
            .get(object_ref)
            .map_err(|e| e.into())
    ...
    ) -> Result<Vec<Option<TransactionDigest>>, HaneulError> {
        self.perpetual_tables
            .parent_sync
    ...
    ) -> Result<impl Iterator<Item = (ObjectRef, TransactionDigest)> + '_, HaneulError> {
    ...
    pub async fn insert_genesis_object(&self, object: Object) -> HaneulResult {
    ...
    pub async fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> HaneulResult {
    ...
    /// must be new.
    pub async fn bulk_object_insert(&self, objects: &[&Object]) -> HaneulResult<()> {
    ...
    /// Returns HaneulError::ObjectNotFound if no lock record can be found for one of the objects.
    /// Returns HaneulError::ObjectVersionUnavailableForConsumption if one of the objects is not locked at the given version.
    ...
    pub(crate) async fn acquire_transaction_locks(
        ...
        for ((i, lock), obj_ref) in locks.iter().enumerate().zip(owned_input_objects) {
            ...
            if lock.is_none() {
                ...
                fp_bail!(HaneulError::ObjectVersionUnavailableForConsumption {
    ...
    /// Returns HaneulError::ObjectNotFound if cannot find lock record for this object
    fn get_lock(&self, obj_ref: ObjectRef, epoch_id: EpochId) -> HaneulLockResult {
        Ok(
    ...
    /// Returns HaneulError::ObjectNotFound if no lock records found for this object.
    ...
    /// Returns HaneulError::ObjectNotFound if cannot find lock record for at least one of the objects.
    /// Returns HaneulError::ObjectVersionUnavailableForConsumption if at least one object lock is not initialized
    ///     at the given version.
    pub fn check_locks_exist(&self, objects: &[ObjectRef]) -> HaneulResult {
        let locks = self
            .perpetual_tables
            .owned_object_transaction_locks
            .multi_get(objects)?;
    ...
    fn delete_locks(&self, write_batch: DBBatch, objects: &[ObjectRef]) -> HaneulResult<DBBatch> {
    ...
    pub async fn revert_state_update(&self, tx_digest: &TransactionDigest) -> HaneulResult {
        ...
        // This is dependent on https://github.com/GeunhwaJeong/haneul/issues/2629.
    ...
    pub fn get_latest_parent_entry(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectRef, TransactionDigest)>, HaneulError> {
        self.perpetual_tables.get_latest_parent_entry(object_id)
    }

    pub fn object_exists(&self, object_id: ObjectID) -> HaneulResult<bool> {
...
pub trait EffectsStore {
    ...
    ) -> HaneulResult<Vec<Option<SignedTransactionEffects>>>;
}
