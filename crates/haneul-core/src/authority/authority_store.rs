...
use std::path::Path;
use haneul_storage::{default_db_options, LockService};
...
pub type AuthorityStore = HaneulDataStore<false, true, AuthoritySignInfo>;
#[allow(dead_code)]
pub type ReplicaStore = HaneulDataStore<true, false, EmptySignInfo>;
pub type GatewayStore = HaneulDataStore<false, true, EmptySignInfo>;
...
pub struct HaneulDataStore<const ALL_OBJ_VER: bool, const USE_LOCKS: bool, S> {
    ...
    /// composite key of the HaneulAddress of their owner and the object ID of the object.
    /// This composite index allows an efficient iterator to list all objected currently owned
    /// by a specific user, and their object reference.
    owner_index: DBMap<(HaneulAddress, ObjectID), ObjectRef>,
...
impl<
    ...
    > HaneulDataStore<ALL_OBJ_VER, USE_LOCKS, S>
{
    ...
    pub fn open<P: AsRef<Path>>(path: P, db_options: Option<Options>) -> Self {
        ...
        let (
            ...
            "owner_index";<(HaneulAddress, ObjectID), ObjectRef>,
    ...
    pub fn get_account_objects(&self, account: HaneulAddress) -> Result<Vec<ObjectRef>, HaneulError> {
    ...
    ) -> Result<Option<TransactionEnvelope<S>>, HaneulError> {
        let transaction_option = self
            .lock_service
            .get_lock(*object_ref)
            .await?
            .ok_or(HaneulError::TransactionLockDoesNotExist)?;
    ...
    async fn batch_update_objects<BackingPackageStore>(
        ...
        // old writes would be OK.
        {
            ...
            if USE_LOCKS {
                ...
                // TODO: add retries https://github.com/GeunhwaJeong/haneul/issues/1993
    ...
    pub fn persist_certificate_and_lock_shared_objects(
        ...
        write_batch.write().map_err(HaneulError::from)
    }
    ...
    ) -> HaneulResult<Vec<(GatewayTxSeqNumber, TransactionDigest)>> {
        Ok(self
            .executed_sequence
            .iter()
            .skip_to(&start)?
    ...
    pub fn batches_and_transactions(
    ...
    ) -> Result<(Vec<SignedBatch>, Vec<(TxSequenceNumber, TransactionDigest)>), HaneulError> {
        /*
        ...
        */
        ...
        let first_seq = batches
            .first()
            .ok_or(HaneulError::NoBatchesFoundError)?
            .batch
    ...
    pub fn last_consensus_index(&self) -> HaneulResult<ExecutionIndices> {
    ...
    pub fn get_last_epoch_info(&self) -> HaneulResult<EpochInfoLocals> {
...
impl<const A: bool, const B: bool> HaneulDataStore<A, B, AuthoritySignInfo> {
...
impl<const A: bool, const B: bool> HaneulDataStore<A, B, EmptySignInfo> {
...
impl<const A: bool, const B: bool, S: Eq + Serialize + for<'de> Deserialize<'de>>
    BackingPackageStore for HaneulDataStore<A, B, S>
{
...
pub async fn store_package_and_init_modules_for_genesis<
    ...
    S: Eq + Serialize + for<'de> Deserialize<'de>,
>(
    store: &Arc<HaneulDataStore<A, B, S>>,
...
>(
    ...
    let pubkeys: Vec<Vec<u8>> = committee
        ...
        .collect();
    // TODO: May use separate haneul address than derived from pubkey.
    let haneul_addresses: Vec<AccountAddress> = committee
        .voting_rights
        .keys()
        .map(|pk| HaneulAddress::from(pk).into())
        .collect();
    // TODO: Allow config to specify human readable validator names.
    let names: Vec<Vec<u8>> = (0..haneul_addresses.len())
    ...
    adapter::execute(
        ...
        vec![
            ...
            CallArg::Pure(bcs::to_bytes(&haneul_addresses).unwrap()),
        ...
        &mut HaneulGasStatus::new_unmetered(),
        genesis_ctx,
    )?;
    store
