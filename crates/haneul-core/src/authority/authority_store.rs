...
struct AuthorityStoreMetrics {
    ...
    haneul_conservation_live_object_count: IntGauge,
    haneul_conservation_live_object_size: IntGauge,
    haneul_conservation_imbalance: IntGauge,
    haneul_conservation_storage_fund: IntGauge,
    haneul_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            haneul_conservation_check_latency: register_int_gauge_with_registry!(
                "haneul_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for HANEUL conservation check",
                registry,
            ).unwrap(),
            haneul_conservation_live_object_count: register_int_gauge_with_registry!(
                "haneul_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            haneul_conservation_live_object_size: register_int_gauge_with_registry!(
                "haneul_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            haneul_conservation_imbalance: register_int_gauge_with_registry!(
                "haneul_conservation_imbalance",
                "Total amount of HANEUL in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            haneul_conservation_storage_fund: register_int_gauge_with_registry!(
                "haneul_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            haneul_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "haneul_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows HaneulDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows HaneulDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive HANEUL conservation check at epoch boundaries.
...
impl AuthorityStore {
    ...
    pub fn get_root_state_hash(&self, epoch: EpochId) -> HaneulResult<ECMHLiveObjectSetDigest> {
        let acc = self
            .perpetual_tables
    ...
    pub fn get_recovery_epoch_at_restart(&self) -> HaneulResult<EpochId> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> HaneulResult<bool> {
        self.perpetual_tables
            .effects
    ...
    ) -> HaneulResult<Vec<Option<TransactionEffectsDigest>>> {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> HaneulResult<(CheckpointSequenceNumber, Accumulator)> {
    ...
    pub fn deprecated_get_transaction_checkpoint(
        &self,
        digest: &TransactionDigest,
    ) -> HaneulResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> HaneulResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    ) -> Result<Option<ObjectRef>, HaneulError> {
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, HaneulError> {
    ...
    pub fn check_input_objects(
        ...
        haneul_transaction_checks::check_input_objects(self, objects, protocol_config)
    }
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> HaneulResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> HaneulResult {
    ...
    async fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> HaneulResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            return Err(HaneulError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> HaneulResult<Option<EpochStartConfiguration>> {
    ...
    /// Returns HaneulError::ObjectLockConflict if one of the objects is locked by a different transaction in the same epoch.
    /// Returns HaneulError::ObjectLockedAtFutureEpoch if one of the objects is locked in a future epoch (bug).
    pub(crate) async fn acquire_transaction_locks(
        ...
        for ((i, lock), obj_ref) in locks.into_iter().enumerate().zip(owned_input_objects) {
            ...
            }) = &lock
            {
                fp_ensure!(
                    &epoch >= previous_epoch,
                    HaneulError::ObjectLockedAtFutureEpoch {
                ...
                if previous_epoch == &epoch && previous_tx_digest != &tx_digest {
                    ...
                    return Err(HaneulError::ObjectLockConflict {
                        obj_ref: *obj_ref,
    ...
    pub(crate) fn get_lock(&self, obj_ref: ObjectRef, epoch_id: EpochId) -> HaneulLockResult {
        Ok(
                ...
                match lock_info {
                    Some(lock_info) => {
                        ...
                        match Ord::cmp(&lock_info.epoch, &epoch_id) {
                            ...
                            Ordering::Greater => {
                                return Err(HaneulError::ObjectLockedAtFutureEpoch {
    ...
    fn get_latest_lock_for_object_id(&self, object_id: ObjectID) -> HaneulResult<ObjectRef> {
        let mut iterator = self
            .perpetual_tables
            .owned_object_transaction_locks
            .unbounded_iter()
    ...
    pub fn check_owned_object_locks_exist(&self, objects: &[ObjectRef]) -> HaneulResult {
        let locks = self
            .perpetual_tables
            .owned_object_transaction_locks
            .multi_get(objects)?;
    ...
    /// Returns HaneulError::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_locks(
        ...
        if !is_force_reset {
            ...
            if !existing_locks.is_empty() {
                ...
                return Err(HaneulError::ObjectLockAlreadyInitialized {
                    refs: existing_locks,
                });
            }
        }
    ...
    fn delete_locks(&self, write_batch: &mut DBBatch, objects: &[ObjectRef]) -> HaneulResult {
    ...
    pub async fn revert_state_update(&self, tx_digest: &TransactionDigest) -> HaneulResult {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, StoreObjectWrapper)>, HaneulError> {
        self.perpetual_tables
    ...
    pub fn expensive_check_haneul_conservation(
        self: &Arc<Self>,
        ...
        info!("Starting HANEUL conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let package_cache = PackageObjectCache::new(self.clone());
        let (mut total_haneul, mut total_storage_rebate) = thread::scope(|s| {
                    ...
                    LiveObject::Normal(object) => {
                        ...
                        if count % 1_000_000 == 0 {
                            ...
                            pending_tasks.push(s.spawn(move || {
                                ...
                                (total_haneul, total_storage_rebate)
                            }));
                        }
                    }
        ...
        info!(
            "Total HANEUL amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_haneul, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .haneul_conservation_imbalance
            .set((total_haneul as i128 - TOTAL_SUPPLY_GEUNHWA as i128) as i64);
        ...
        {
            fp_ensure!(
                total_haneul == expected_haneul,
                HaneulError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total haneul: {}, expecting {}",
                        system_state.epoch, total_haneul, expected_haneul
                    )
                    .as_str()
                )
            );
...
pub type HaneulLockResult = HaneulResult<ObjectLockStatus>;
