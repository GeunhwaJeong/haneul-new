...
impl AuthorityPerEpochStore {
    ...
    ) -> HaneulResult<Vec<(CheckpointCommitHeight, PendingCheckpoint)>> {
        let tables = self.tables()?;
    ...
    ) -> HaneulResult<Option<PendingCheckpoint>> {
    ...
    pub fn last_built_checkpoint_commit_height(&self) -> HaneulResult<Option<CheckpointCommitHeight>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn last_randomness_round_written(&self) -> HaneulResult<RandomnessRound> {
        Ok(self
            .tables()?
