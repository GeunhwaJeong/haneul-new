    ...
    pub fn user_signatures_for_checkpoint(
        ...
        for (signatures, digest) in signatures.into_iter().zip(digests.iter()) {
            let Some(signatures) = signatures else {
                return Err(HaneulError::from(format!("Can not find user signature for checkpoint for transaction {:?}", digest).as_str()));
            };
...
impl<'a, C: CheckpointServiceNotify> ConsensusCommitBatch<'a, C> {
    pub fn commit(mut self, pending_checkpoint: PendingCheckpoint) -> HaneulResult {
        self.epoch_store
