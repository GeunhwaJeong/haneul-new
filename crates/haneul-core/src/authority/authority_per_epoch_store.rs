...
use std::sync::Arc;
use haneul_config::node::{ConsensusProtocol, ExpensiveSafetyCheckConfig};
...
use haneul_types::base_types::{AuthorityName, EpochId, ObjectID, SequenceNumber, TransactionDigest};
use haneul_types::base_types::{ConciseableName, ObjectRef};
use haneul_types::committee::Committee;
use haneul_types::committee::CommitteeTrait;
use haneul_types::crypto::{AuthoritySignInfo, AuthorityStrongQuorumSignInfo, RandomnessRound};
...
use haneul_types::messages_consensus::VersionedDkgConfirmation;
...
impl AuthorityEpochTables {
    ...
    pub fn get_last_consensus_index(&self) -> HaneulResult<Option<ExecutionIndicesWithHash>> {
...
impl AuthorityPerEpochStore {
    ...
    pub fn get_last_consensus_index(&self) -> HaneulResult<ExecutionIndicesWithHash> {
        self.tables()?
    ...
    pub async fn notify_read_checkpoint_state_digests(
        ...
        self.checkpoint_state_notify_read
            .read(&checkpoints, |checkpoints| -> HaneulResult<_> {
                Ok(self
                    .tables()?
                    .state_hash_by_checkpoint
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn handle_committed_transactions(&self, digests: &[TransactionDigest]) -> HaneulResult<()> {
    ...
    ) -> HaneulResult<HashMap<ObjectID, SequenceNumber>> {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                Ok(ConsensusCertificateResult::HaneulTransaction(certificate))
            }
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
    ...
    ) -> HaneulResult<Option<PendingCheckpointV2>> {
        if self.randomness_state_enabled() {
