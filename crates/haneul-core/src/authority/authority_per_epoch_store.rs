...
impl AuthorityPerEpochStore {
    ...
    pub fn tables(&self) -> HaneulResult<Arc<AuthorityEpochTables>> {
        match self.tables.load_full() {
            ...
            None => Err(HaneulError::EpochEnded),
        }
    }
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn remove_pending_execution(&self, digest: &TransactionDigest) -> HaneulResult<()> {
    ...
    fn load_deferred_transactions_for_randomness(
        &self,
        batch: &mut DBBatch,
    ) -> HaneulResult<Vec<VerifiedSequencedConsensusTransaction>> {
    ...
    pub fn remove_pending_consensus_transaction(&self, key: &ConsensusTransactionKey) -> HaneulResult {
    ...
    fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of narwhal payload verification in HaneulTxValidator
        ...
        let mut shared_input_next_versions = {
            ...
            if generate_randomness {
                ...
                let version = shared_input_next_versions
                    .get_mut(&HANEUL_RANDOMNESS_STATE_OBJECT_ID)
                ...
                batch.insert_batch(
                    ...
                    iter::once((
                        ...
                        vec![(HANEUL_RANDOMNESS_STATE_OBJECT_ID, *version)],
                    )),
                )?;
                version.increment();
            }
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        match &transaction {
            ...
            SequencedConsensusTransactionKind::System(system_transaction) => {
                ...
                Ok(ConsensusCertificateResult::HaneulTransaction(
    ...
    pub fn last_built_checkpoint_commit_height(&self) -> HaneulResult<Option<CheckpointHeight>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
