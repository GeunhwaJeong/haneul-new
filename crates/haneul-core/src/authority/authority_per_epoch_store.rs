...
impl AuthorityPerEpochStore {
    ...
    ) -> HaneulResult<Option<GlobalStateHash>> {
        Ok(self
            .tables()?
    ...
    ) -> HaneulResult<Vec<(CheckpointSequenceNumber, GlobalStateHash)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    ) -> HaneulResult<Vec<GlobalStateHash>> {
        let tables = self.tables()?;
        Ok(self
    ...
    ) -> HaneulResult<AssignedTxAndVersions> {
    ...
    ) -> HaneulResult<(Vec<Schedulable>, AssignedTxAndVersions)> {
