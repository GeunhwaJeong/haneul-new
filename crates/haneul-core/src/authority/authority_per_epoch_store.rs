        ...
        let system_state = match system_state {
            ...
            HaneulSystemState::V1(_) => {
                if committee.epoch() > 1 {
                    error!(
                        "`PerObjectCongestionControlMode::ExecutionTimeEstimate` cannot load execution time observations to HaneulSystemState because it has an old version. This should not happen outside tests."
                    );
                }
                return itertools::Either::Left(std::iter::empty());
            }
