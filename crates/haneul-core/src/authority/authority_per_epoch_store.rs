...
impl AuthorityPerEpochStore {
    ...
    ) -> HaneulResult<Vec<(ObjectRef, LockDetails)>> {
    ...
    ) -> HaneulResult<Vec<(DeferralKey, Vec<VerifiedExecutableTransactionWithAliases>)>> {
    ...
    ) -> Result<Vec<CheckpointSequenceNumber>, HaneulError> {
        let tables = self.tables()?;
    ...
    pub fn verify_transaction_with_current_aliases(
        &self,
        tx: Transaction,
    ) -> HaneulResult<VerifiedTransactionWithAliases> {
    ...
    fn pending_checkpoint_exists(&self, index: &CheckpointHeight) -> HaneulResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    fn pending_checkpoint_exists_v2(&self, index: &CheckpointHeight) -> HaneulResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
