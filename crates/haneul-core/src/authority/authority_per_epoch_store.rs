...
use haneul_storage::mutex_table::LockGuard;
use haneul_storage::write_ahead_log::{DBWriteAheadLog, TxGuard, WriteAheadLog};
...
impl AuthorityPerEpochStore {
    ...
    pub fn system_state_object(&self) -> &HaneulSystemState {
    ...
    pub fn insert_transaction(&self, transaction: VerifiedSignedTransaction) -> HaneulResult {
        Ok(self
            .tables
            .transactions
    ...
    ) -> HaneulResult<Vec<SequenceNumber>> {
    ...
    pub fn insert_checkpoint_boundary(&self, index: u64, height: u64) -> HaneulResult {
    ...
    pub fn is_consensus_message_processed(&self, key: &ConsensusTransactionKey) -> HaneulResult<bool> {
    ...
    pub(crate) async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        checkpoint_service: &Arc<C>,
        parent_sync_store: impl ParentSync,
    ) -> HaneulResult<Option<VerifiedExecutableTransaction>> {
    ...
    pub fn get_num_certified_checkpoint_sigs_by(
        &self,
        name: &AuthorityName,
    ) -> HaneulResult<Option<(Option<CheckpointSequenceNumber>, u64)>> {
