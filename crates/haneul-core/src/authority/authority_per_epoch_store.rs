...
pub enum ConsensusCertificateResult {
    ...
    HaneulTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> HaneulResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> HaneulResult {
    ...
    pub fn get_last_consensus_stats(&self) -> HaneulResult<Option<ExecutionIndicesWithStats>> {
    ...
    fn get_all_deferred_transactions(
        &self,
    ) -> HaneulResult<BTreeMap<DeferralKey, Vec<VerifiedSequencedConsensusTransaction>>> {
        Ok(self
    ...
    fn get_all_user_signatures_for_checkpoints(
        &self,
    ) -> HaneulResult<HashMap<TransactionDigest, Vec<GenericSignature>>> {
        Ok(self
...
impl AuthorityPerEpochStore {
    ...
    ) -> HaneulResult<Option<Accumulator>> {
        Ok(self
            .tables()?
    ...
    pub fn get_stored_execution_time_observations(
        ...
        let system_state = match system_state {
            ...
            HaneulSystemState::V1(_) => {
                error!("`PerObjectCongestionControlMode::ExecutionTimeEstimate` cannot load execution time observations to HaneulSystemState because it has an old version. This should not happen outside tests.");
                return itertools::Either::Left(std::iter::empty());
            }
            #[cfg(msim)]
            HaneulSystemState::SimTestV1(_)
            | HaneulSystemState::SimTestShallowV2(_)
            | HaneulSystemState::SimTestDeepV2(_) => {
                return itertools::Either::Left(std::iter::empty());
            }
        };
    ...
    pub fn acquire_tx_guard(&self, cert: &VerifiedExecutableTransaction) -> HaneulResult<CertTxGuard> {
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> HaneulResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn revert_executed_transaction(&self, tx_digest: &TransactionDigest) -> HaneulResult {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> HaneulResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> HaneulResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    ) -> HaneulResult<BTreeSet<InputKey>> {
    ...
    pub fn get_last_consensus_stats(&self) -> HaneulResult<ExecutionIndicesWithStats> {
        assert!(
    ...
    ) -> HaneulResult<Vec<(CheckpointSequenceNumber, Accumulator)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    pub fn all_pending_execution(&self) -> HaneulResult<Vec<VerifiedExecutableTransaction>> {
        Ok(self
            .tables()?
            .pending_execution
            .safe_iter()
    ...
    ) -> HaneulResult<HashMap<ConsensusObjectSequenceKey, SequenceNumber>> {
    ...
    ) -> HaneulResult<Vec<(DeferralKey, Vec<VerifiedSequencedConsensusTransaction>)>> {
    ...
    pub fn has_sent_end_of_publish(&self, authority: &AuthorityName) -> HaneulResult<bool> {
        Ok(self
            .end_of_publish
            .try_lock()
    ...
    pub fn user_signatures_for_checkpoint(
        ...
        digests: &[TransactionDigest],
    ) -> HaneulResult<Vec<Vec<GenericSignature>>> {
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> HaneulResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> HaneulResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    /// Record most recently advertised capabilities of all authorities
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> HaneulResult {
    ...
    pub fn get_capabilities_v1(&self) -> HaneulResult<Vec<AuthorityCapabilitiesV1>> {
    ...
    pub fn get_capabilities_v2(&self) -> HaneulResult<Vec<AuthorityCapabilitiesV2>> {
        assert!(self.protocol_config.authority_capabilities_v2());
        Ok(self
            .tables()?
            .authority_capabilities_v2
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> HaneulResult<Vec<ActiveJwk>> {
    ...
    pub(crate) fn get_randomness_last_round_timestamp(&self) -> HaneulResult<Option<TimestampMs>> {
        if let Some(ts) = self
            .consensus_quarantine
            .read()
    ...
    fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of the consensus payload verification in HaneulTxValidator
    ...
    ) -> HaneulResult<Vec<VerifiedExecutableTransaction>> {
    ...
    fn add_consensus_commit_prologue_transaction(
        ...
        additional_state: &AdditionalConsensusState,
    ) -> HaneulResult<Option<TransactionKey>> {
        {
        ...
        let consensus_commit_prologue_root = match self.process_consensus_system_transaction(&transaction) {
            ConsensusCertificateResult::HaneulTransaction(processed_tx) => {
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        for tx in transactions {
            ...
            match self
                ...
                .await?
            {
                ConsensusCertificateResult::HaneulTransaction(cert) => {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        execution_time_estimator: &ExecutionTimeEstimator,
        authority_metrics: &Arc<AuthorityMetrics>,
    ) -> HaneulResult<ConsensusCertificateResult> {
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // Safe because signatures are verified when consensus called into HaneulTxValidator::validate_batch.
            ...
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // We usually call notify_checkpoint_signature in HaneulTxValidator, but that step can
    ...
    fn process_consensus_system_transaction(
        ...
        ConsensusCertificateResult::HaneulTransaction(system_transaction.clone())
    }
    ...
    fn process_consensus_user_transaction(
        ...
        Ok(ConsensusCertificateResult::HaneulTransaction(transaction))
    }
    ...
    ) -> HaneulResult<Vec<(CheckpointHeight, PendingCheckpointV2)>> {
    ...
    pub fn pending_checkpoint_exists(&self, index: &CheckpointHeight) -> HaneulResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> HaneulResult<Option<BuilderCheckpointSummary>> {
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> HaneulResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> HaneulResult<Option<CheckpointSummary>> {
        if let Some(BuilderCheckpointSummary { summary, .. }) =
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> HaneulResult<u64> {
        Ok(self
            .tables()?
    ...
    pub(crate) fn check_all_executed_transactions_in_checkpoint(&self) -> HaneulResult<()> {
