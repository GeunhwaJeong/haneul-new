...
mod tests {
    ...
    use haneul_types::transaction::{ObjectArg, SenderSignedData, TransactionKey};
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&HANEUL_RANDOMNESS_STATE_OBJECT_ID)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            ...
            HashMap::from([(HANEUL_RANDOMNESS_STATE_OBJECT_ID, next_randomness_obj_version)])
        );
        assert_eq!(
            assigned_versions,
            vec![
                (
                    ...
                    vec![(HANEUL_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),]
                ),
                (
                    certs[0].key(),
                    // It is critical that the randomness object version is updated before the assignment.
                    vec![(HANEUL_RANDOMNESS_STATE_OBJECT_ID, next_randomness_obj_version)]
                ),
                (
                    certs[1].key(),
                    // It is critical that the randomness object version is updated before the assignment.
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                (HANEUL_RANDOMNESS_STATE_OBJECT_ID, SequenceNumber::from_u64(1)), // not mutable
            ])
        );
