...
/// A cache that maintains rejection reasons (HaneulError) when validators cast reject votes for transactions
...
pub(crate) struct TransactionRejectReasonCache {
    cache: RwLock<BTreeMap<ConsensusPosition, HaneulError>>,
...
impl TransactionRejectReasonCache {
    ...
    /// uniquely identifies the transaction and the `reason` (HaneulError) that caused the transaction to be rejected during voting
    /// should be provided.
    pub fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &HaneulError) {
    ...
    pub fn get_rejection_vote_reason(&self, position: ConsensusPosition) -> Option<HaneulError> {
        debug_assert_eq!(position.epoch, self.epoch, "Epoch mismatch");
