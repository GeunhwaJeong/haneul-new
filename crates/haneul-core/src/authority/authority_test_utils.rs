...
use haneul_types::crypto::{AccountKeyPair, AuthorityKeyPair};
...
) -> Result<(CertifiedTransaction, SignedTransactionEffects), HaneulError> {
...
pub async fn init_state_with_ids<I: IntoIterator<Item = (HaneulAddress, ObjectID)>>(
    objects: I,
) -> Arc<AuthorityState> {
...
pub async fn init_state_with_ids_and_versions<
    I: IntoIterator<Item = (HaneulAddress, ObjectID, SequenceNumber)>,
>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
...
pub async fn init_state_with_ids_and_expensive_checks<
    I: IntoIterator<Item = (HaneulAddress, ObjectID)>,
>(
    objects: I,
...
pub async fn execute_sequenced_certificate_to_effects(
    authority: &AuthorityState,
    certificate: VerifiedCertificate,
) -> Result<(TransactionEffects, Option<ExecutionError>), HaneulError> {
    authority.enqueue_certificates_for_execution(
...
) -> HaneulResult<(TransactionDigest, (ObjectID, ObjectRef))> {
    let mut build_config = BuildConfig::new_for_testing();
    for (addr_name, obj_id) in dep_original_addresses {
        build_config
            .config
...
pub async fn upgrade_package_on_single_authority(
    ...
    state: &Arc<AuthorityState>,
) -> HaneulResult<(TransactionDigest, ObjectID)> {
