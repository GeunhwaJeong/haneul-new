...
impl TransactionNotifier {
    ...
    pub fn new(state: Arc<AuthorityStore>) -> HaneulResult<TransactionNotifier> {
    ...
    pub fn ticket(self: &Arc<Self>, bypass_pause: bool) -> HaneulResult<TransactionNotifierTicket> {
        if self.is_closed.load(Ordering::SeqCst) {
            return Err(HaneulError::ClosedNotifierError);
        }
    ...
    pub fn iter_from(
        self: &Arc<Self>,
        next_seq: u64,
    ) -> HaneulResult<impl futures::Stream<Item = (TxSequenceNumber, ExecutionDigests)> + Unpin> {
        if self
            .has_stream
            .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
            .is_err()
        {
            return Err(HaneulError::ConcurrentIteratorError);
        }
...
mod tests {
    ...
    async fn test_notifier() {
        ...
        assert!(matches!(
            ...
            Err(HaneulError::ConcurrentIteratorError)
        ));
