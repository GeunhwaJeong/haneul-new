...
use haneul_types::messages::{TransactionKind, VerifiedExecutableTransaction};
use haneul_types::{
...
async fn get_gas_status(
    ...
    let gas_object_refs = match tx_kind {
        TransactionKind::Single(SingleTransactionKind::PayHaneul(p)) => p.coins.clone(),
        TransactionKind::Single(SingleTransactionKind::PayAllHaneul(p)) => p.coins.clone(),
        _ => vec![],
    };
...
// should be aggregated into Haneul::TransactionInputObjectsErrors
#[instrument(level = "trace", skip_all)]
pub async fn check_transaction_input(
    store: &AuthorityStore,
    epoch_store: &AuthorityPerEpochStore,
    transaction: &TransactionData,
) -> HaneulResult<(HaneulGasStatus<'static>, InputObjects)> {
    transaction
        .validity_check()
        .map_err(HaneulError::into_transaction_input_error)?;
...
async fn check_gas(
    ...
    if tx_kind.is_system_tx() {
        ...
        // If the transaction is TransferHaneul, we ensure that the gas balance is enough to cover
        // both gas budget and the transfer amount.
        let extra_amount = match tx_kind {
            TransactionKind::Single(SingleTransactionKind::TransferHaneul(t)) => {
                t.amount.unwrap_or_default()
            }
            TransactionKind::Single(SingleTransactionKind::PayHaneul(t)) => t.amounts.iter().sum(),
            _ => 0,
        };
        let reference_gas_price = epoch_store.reference_gas_price();
        if computation_gas_price < reference_gas_price {
            return Err(HaneulError::GasPriceUnderRGP {
        ...
        if tx_kind.is_pay_haneul_tx() {
            ...
            for obj_ref in additional_objects_for_gas_payment.iter() {
                ...
                let obj = obj.ok_or(HaneulError::ObjectNotFound {
                    object_id: obj_ref.0,
...
async fn check_objects(
    ...
    for object in objects.iter() {
        if !object.is_immutable() {
            fp_ensure!(
                used_objects.insert(object.id().into()),
                HaneulError::TransactionInputObjectsErrors { errors: vec![
                    HaneulError::InvalidBatchTransaction {
    ...
    if !transaction.kind.is_genesis_tx() && all_objects.is_empty() {
        return Err(HaneulError::TransactionInputObjectsErrors {
            errors: vec![HaneulError::ObjectInputArityViolation],
        });
    }
        ...
        InputObjectKind::ImmOrOwnedMoveObject((object_id, sequence_number, object_digest)) => {
            ...
            match object.owner {
                ...
                Owner::ObjectOwner(owner) => {
                    return Err(HaneulError::InvalidChildObjectArgument {
                        child_id: object.id(),
