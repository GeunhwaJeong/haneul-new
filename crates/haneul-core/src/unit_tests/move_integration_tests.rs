...
use haneul_types::{object::Data, utils::to_sender_signed_transaction};
...
async fn test_entry_point_vector_error() {
    ...
    assert!(matches!(
        result
            ...
            .unwrap(),
        &HaneulError::DuplicateObjectRefInput { .. }
    ));
...
async fn check_latest_object_ref(
    ...
    let response = authority
        ...
        .await;
    if expect_not_found {
        assert!(matches!(response, Err(HaneulError::ObjectNotFound { .. })));
    } else {
        assert_eq!(
