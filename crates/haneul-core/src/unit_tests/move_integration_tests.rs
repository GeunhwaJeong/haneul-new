...
use haneul_types::{
    ...
    utils::to_sender_signed_transaction, MOVE_STDLIB_OBJECT_ID, HANEUL_FRAMEWORK_OBJECT_ID,
};
...
async fn test_object_no_id_error() {
    ...
    let res = haneul_framework::build_move_package(&path, build_config);

    matches!(res.err(), Some(HaneulError::ExecutionError(err_str)) if
                 err_str.contains("HaneulMoveVerificationError")
...
async fn test_generate_lock_file() {
    ...
    haneul_framework::build_move_package(&path, build_config).expect("Move package did not build");
...
pub async fn build_and_try_publish_test_package(
    ...
    path.push(test_dir);
    let compiled_package = haneul_framework::build_move_package(&path, build_config).unwrap();
