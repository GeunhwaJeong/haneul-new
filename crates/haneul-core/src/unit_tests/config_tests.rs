...
use haneul_types::base_types::{dbg_addr, ObjectID, ObjectRef, SequenceNumber, HaneulAddress};
use haneul_types::crypto::{get_account_key_pair, AccountKeyPair};
use haneul_types::effects::{TransactionEffectsAPI, UnchangedSharedKind};
use haneul_types::object::Object;
use haneul_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use haneul_types::transaction::{CallArg, ObjectArg, Transaction, TEST_ONLY_GAS_UNIT_FOR_PUBLISH};
use haneul_types::{Identifier, HANEUL_DENY_LIST_OBJECT_ID, HANEUL_FRAMEWORK_PACKAGE_ID};
        ...
        match activity {
            DenyListActivity::Transfer => {
                ...
                let Some((_, UnchangedSharedKind::PerEpochConfigWithSeqno(pseqno))) = effects
                    ...
                    .into_iter()
                    .find(|id| id.0 == HANEUL_DENY_LIST_OBJECT_ID)
                else {
                    panic!(
...
async fn test_config_use_before_change_not_input_object() {
    ...
    assert!(!tx
        ...
        .any(|obj| { obj.id() == HANEUL_DENY_LIST_OBJECT_ID }));
    let effects = send_and_confirm_transaction_(&env.authority, None, tx, true)
        .await
        .unwrap()
        .1;
    ...
    assert_eq!(unchanged_shared_objects[0].0, HANEUL_DENY_LIST_OBJECT_ID);
    assert_eq!(
...
async fn test_config_use_after_change_deny_list_in_input_object() {
    ...
    assert_ne!(unchanged_shared_objects[0].0, HANEUL_DENY_LIST_OBJECT_ID);
}
