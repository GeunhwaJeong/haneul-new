...
use haneul_json_rpc_types::{HaneulExecutionResult, HaneulExecutionStatus};
...
async fn test_dry_run_transaction() {
    ...
    let response = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(response.status, HaneulExecutionStatus::Success);
...
async fn test_dev_inspect_move_call() {
    ...
    assert!(effects.deleted.is_empty());
    let gas_object_owner = Owner::AddressOwner(HaneulAddress::default());
...
async fn test_handle_transfer_transaction_with_max_sequence_number() {
    ...
    assert_eq!(
        res.err(),
        Some(HaneulError::TransactionInputObjectsErrors {
...
fn test_handle_transfer_transaction_bad_sequence_number() {
    ...
    let recipient = Address::Haneul(dbg_addr(2));
...
) -> Result<SignedTransactionEffects, HaneulError> {
...
async fn test_handle_certificate_with_shared_object_interrupted_retry() {
    ...
    for limit in &delays {
        ...
        // https://github.com/GeunhwaJeong/haneul/pull/4579
...
async fn test_transfer_haneul_no_amount() {
    ...
    let tx_data = TransactionData::new_transfer_haneul(recipient, sender, None, gas_ref, MAX_GAS);
...
async fn test_transfer_haneul_with_amount() {
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_haneul(recipient, sender, Some(500), gas_ref, MAX_GAS);
    let transaction = to_sender_signed_transaction(tx_data, &sender_key);
    let certificate = init_certified_transaction(transaction, &authority_state);
    let signed_effects = authority_state
        .execute_certificate(&certificate, &authority_state.epoch_store_for_testing())
...
#[tokio::test]
async fn test_store_revert_transfer_haneul() {
    // This test checks the correctness of revert_state_update in HaneulDataStore.
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
async fn test_shared_object_transaction() {
    ...
    let result = authority.try_execute_for_test(&certificate).await;
    assert!(
        matches!(result, Err(HaneulError::TransactionInputObjectsErrors { .. })),
        "{:#?}",
        result
    );
...
async fn test_blocked_move_calls() {
    ...
    assert!(matches!(
        ...
        Err(HaneulError::BlockedMoveFunction)
    ));
}
