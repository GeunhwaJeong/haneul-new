...
use haneul_json_rpc_types::{HaneulExecutionResult, HaneulExecutionStatus, HaneulGasCostSummary};
...
pub fn create_genesis_module_packages() -> Vec<Object> {
    ...
    vec![
        ...
        Object::new_package_for_testing(haneul_modules, TransactionDigest::genesis()).unwrap(),
    ]
}
...
async fn test_dev_inspect_object_by_bytes() {
    ...
    let actual_gas_used: HaneulGasCostSummary = effects.gas_used.into();
...
async fn test_missing_package() {
    ...
    assert!(
        matches!(error, HaneulError::DependentPackageNotFound { .. }),
        "Wrong error {}",
        error
    );
}
...
async fn test_invalid_mutable_clock_parameter() {
    ...
    assert_eq!(
        ...
        HaneulError::TransactionInputObjectsErrors {
            errors: vec![HaneulError::ImmutableParameterExpectedError]
        },
    );
}
