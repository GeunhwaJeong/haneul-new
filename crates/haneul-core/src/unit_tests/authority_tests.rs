...
use serde_json::json;
use haneul_framework::{make_system_objects, make_system_packages, system_package_ids};
use tracing::info;
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    let data = TransactionData::new_programmable_with_dummy_gas_price(
        ...
        vec![],
        pt,
        HaneulCostTable::new_for_testing().max_gas_budget,
    );
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![HaneulFramework::ID]);
    let kind = TransactionKind::programmable(builder.finish());
