...
pub fn create_genesis_module_packages() -> Vec<Object> {
    ...
    vec![
        ...
        Object::new_package_for_testing(haneul_modules, TransactionDigest::genesis(), [&std_move_pkg])
            .unwrap(),
    ]
}
...
async fn test_dev_inspect_object_by_bytes() {
    ...
    let actual_gas_used: HaneulGasCostSummary = effects.gas_cost_summary().clone().into();
...
// skipped because it violates HANEUL conservation checks
#[tokio::test]
...
async fn test_haneul_system_state_nop_upgrade() {
    ...
    let inner_state = inner.get_haneul_system_state_object().unwrap();
...
async fn test_transfer_haneul_no_amount() {
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_haneul_with_dummy_gas_price(
...
async fn publish_object_basics(state: Arc<AuthorityState>) -> (Arc<AuthorityState>, ObjectRef) {
    ...
    let pkg =
        Object::new_package_for_testing(modules, digest, [&std_move_pkg, &haneul_move_pkg]).unwrap();
...
async fn test_gas_smashing() {
    ...
    async fn run_and_check(
        ...
        let balance = haneul_types::gas::get_gas_balance(
...
async fn test_for_inc_201_dev_inspect() {
    ...
    builder.command(Command::Publish(
        modules,
        vec![MOVE_STDLIB_OBJECT_ID, HANEUL_FRAMEWORK_OBJECT_ID],
    ));
    let kind = TransactionKind::programmable(builder.finish());
