...
use test_utils::haneul_system_state::{test_haneul_system_state, test_validator};

use haneul_macros::sim_test;
use haneul_types::messages::*;
use haneul_types::object::{MoveObject, Object, Owner, GAS_VALUE_FOR_TESTING};
...
async fn test_get_committee_with_net_addresses() {
    ...
    let system_state = test_haneul_system_state(1, validators);
    let good_result = make_response_from_haneul_system_state(system_state.clone());
    ...
    macro_rules! verify_bad_result {
        ($res: expr, $epoch: expr) => {{
            ...
            match res {
                Ok(info) => panic!(
                    "expect HaneulError::FailedToGetAgreedCommitteeFromMajority but got {:?}",
                    info
                ),
    ...
    let bad_result: HaneulResult<ObjectInfoResponse> = Err(HaneulError::GenericAuthorityError {
        error: "foo".into(),
    });
    ...
    let different_result = make_response_from_haneul_system_state(system_state_clone);
    ...
    let system_state_with_different_net_addr = test_haneul_system_state(1, validators);
    let different_result =
        make_response_from_haneul_system_state(system_state_with_different_net_addr);
    ...
    let epoch_0_result = make_response_from_haneul_system_state(system_state_clone);
...
async fn test_get_committee_info() {
    ...
    // 1 out 4 gives error
    let mut clone_clients = clients.clone();
    let bad_result = Err(HaneulError::GenericAuthorityError {
        error: "foo".into(),
    });
    ...
    match res {
        Err(HaneulError::TooManyIncorrectAuthorities { .. }) => (),
        other => panic!(
            "expect to get HaneulError::TooManyIncorrectAuthorities but got {:?}",
            other
        ),
    };
}
...
async fn test_handle_transaction_response() {
    ...
    assert_resp_err(&agg, tx.clone(), |e| matches!(e, HaneulError::Unknown(..))).await;
...
pub fn make_response_from_haneul_system_state(
