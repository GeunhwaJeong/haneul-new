...
use haneul_types::object::{Object, MAX_GAS_BUDGET_FOR_TESTING};
...
async fn test_quorum_map_and_reduce_timeout() {
    ...
    let modules: Vec<_> = haneul_framework::build_move_package(&path, build_config)
        .unwrap()
        .get_modules()
        .cloned()
        .collect();
...
async fn test_handle_transaction_response() {
    ...
    let tx2 = make_transfer_haneul_transaction(
        gas_object,
        HaneulAddress::default(),
        Some(1),
        sender,
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let tx1 = make_transfer_haneul_transaction(
        conflicting_object,
        HaneulAddress::default(),
        Some(1),
        sender,
    ...
    let conflicting_tx2 = make_transfer_haneul_transaction(
        conflicting_object,
        HaneulAddress::default(),
        Some(2),
        sender,
