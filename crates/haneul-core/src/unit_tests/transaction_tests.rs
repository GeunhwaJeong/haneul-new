...
use haneul_types::{
    ...
    crypto::{get_key_pair, AccountKeyPair, Signature, HaneulKeyPair},
...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, HaneulError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
fn setup_aliased_address_test() -> (Transaction, HaneulAddress, HaneulAddress, Object, Object) {
    let recipient = dbg_addr(2);
...
async fn do_test_aliased_address(
    tx: Transaction,
    original: HaneulAddress,
    aliased: HaneulAddress,
    input_object: Object,
    gas_object: Object,
...
async fn test_very_large_certificate() {
    ...
    assert!(
        matches!(err, HaneulError::RpcError(..))
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    println!("Case 0: submit an empty soft bundle.");
    {
        ...
        assert_matches!(
            response.unwrap_err(),
            HaneulError::NoCertificateProvidedError { .. }
        );
    }
