...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = haneul_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: haneul_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
            .map_err(|e| HaneulErrorKind::InvalidSignature {
                error: e.to_string(),
            })
            .expect("Validator returned invalid signature"),
        signers_map,
    };
    let cert = haneul_types::message_envelope::Envelope::new_from_data_and_sig(
    ...
    assert!(
        matches!(err.as_inner(), HaneulErrorKind::RpcError(..))
...
async fn test_handle_certificate_errors() {
    ...
    *data.tx_signatures_mut_for_testing() =
    ...
    assert_matches!(err.into_inner(), HaneulErrorKind::SignerSignatureAbsent { .. });
}
