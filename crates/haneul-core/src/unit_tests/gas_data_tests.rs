...
use haneul_types::collection_types::Table as TableType;
use haneul_types::crypto::{AccountKeyPair, get_authority_key_pair, get_key_pair};
use haneul_types::effects::TransactionEffectsAPI;
use haneul_types::error::{HaneulError, HaneulErrorKind, HaneulResult, UserInputError};
use haneul_types::gas::GasCostSummary;
use haneul_types::gas_coin::GasCoin;
use haneul_types::object::{GAS_VALUE_FOR_TESTING, MoveObject, OBJECT_START_VERSION};
...
use haneul_types::{HANEUL_FRAMEWORK_ADDRESS, HANEUL_FRAMEWORK_PACKAGE_ID};
...
type EntryPointResult = (NodeEntryPoint, Result<GasCostSummary, HaneulError>);
...
) -> &Result<GasCostSummary, HaneulError> {
...
fn assert_err(
    results: &[EntryPointResult],
    entry_point: NodeEntryPoint,
    check: impl Fn(&HaneulErrorKind) -> bool,
    expected_desc: &str,
) {
...
fn assert_simulate_err(
    result: HaneulResult<SimulateTransactionResult>,
    check: impl Fn(&HaneulErrorKind) -> bool,
    expected_desc: &str,
) {
    match result {
...
async fn test_simulate_transaction_preserves_command_outputs() {
    ...
    // The first PayHaneul command is SplitCoins(GasCoin, ...). It is the compact regression target
...
async fn test_simulate_transaction_guard_errors() {
    ...
    assert_simulate_err(
        ...
        |e| {
            matches!(
                e,
                HaneulErrorKind::UnsupportedFeatureError { error }
...
async fn test_bad_gas_payment_all_paths() {
    ...
    // TODO: Needs address balance to plug in appropriately
    {
        ...
        let results = run_all_entry_points(&env, data).await;
        let check_withdraw = |e: &HaneulErrorKind| {
            matches!(
                e,
                HaneulErrorKind::UserInputError {
                    error: UserInputError::InvalidWithdrawReservation { .. }
    ...
    let package_ref = env
        .validator
        .get_object(&HANEUL_FRAMEWORK_PACKAGE_ID)
        .unwrap()
