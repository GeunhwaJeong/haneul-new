...
use haneul_types::crypto::{KeyPair, KeypairTraits};
...
pub(crate) fn init_state_parameters_from_rng<R>(rng: &mut R) -> (Committee, HaneulAddress, KeyPair)
where
...
async fn test_safe_batch_stream() {
    ...
    let items = batch_stream
        .unwrap()
        .collect::<Vec<Result<BatchInfoResponseItem, HaneulError>>>()
        .await;
