...
async fn test_native_transfer_sufficient_gas() -> HaneulResult {
    ...
    let mut gas_status = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1, 1);
...
async fn test_move_call_gas() -> HaneulResult {
    ...
    // Mimic the gas charge behavior and cross check the result with above.
    let mut gas_status = HaneulGasStatus::new_with_budget(GAS_VALUE_FOR_TESTING, 1, 1);
    gas_status.charge_min_tx_gas()?;
    ...
    // If a new native move module/function is modified, this value may need to be increased due to the increase of haneul framework package
...
async fn test_storage_gas_unit_price() -> HaneulResult {
    let mut gas_status1 = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1, 1);
    gas_status1.charge_storage_mutation(100, 200, 5)?;
    let gas_cost1 = gas_status1.summary(true);
    let mut gas_status2 = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1, 3);
