...
use haneul_types::{base_types::dbg_addr, crypto::get_key_pair};

static MAX_GAS_BUDGET: Lazy<u64> = Lazy::new(|| HaneulCostTable::new_for_testing().max_gas_budget());
static MIN_GAS_BUDGET: Lazy<u64> = Lazy::new(|| HaneulCostTable::new_for_testing().min_gas_budget());
...
#[ignore]
#[tokio::test]
async fn test_oog_computation_storage_ok() -> HaneulResult {
...
#[ignore]
#[tokio::test]
async fn test_oog_computation_oog_storage() -> HaneulResult {
    const GAS_PRICE: u64 = 30;
...
// - computation ok, OOG for storage, minimal storage ok
#[tokio::test]
async fn test_computation_ok_oog_storage_minimal_ok() -> HaneulResult {
    const GAS_PRICE: u64 = 30;
...
// - computation ok, OOG for storage, OOG for minimal storage (e.g. computation is entire budget)
#[tokio::test]
async fn test_computation_ok_oog_storage() -> HaneulResult {
    const GAS_PRICE: u64 = 30;
