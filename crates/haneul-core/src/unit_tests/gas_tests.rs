...
async fn test_native_transfer_sufficient_gas() -> HaneulResult {
    ...
    let mut gas_status = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1.into(), 1.into());
...
async fn test_move_call_gas() -> HaneulResult {
    ...
    let mut gas_status = HaneulGasStatus::new_with_budget(GAS_VALUE_FOR_TESTING, 1.into(), 1.into());
    gas_status.charge_min_tx_gas()?;
...
#[tokio::test]
async fn test_storage_gas_unit_price() -> HaneulResult {
    let mut gas_status1 = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1.into(), 1.into());
    gas_status1.charge_storage_mutation(100, 200, 5.into())?;
    let gas_cost1 = gas_status1.summary(true);
    let mut gas_status2 = HaneulGasStatus::new_with_budget(*MAX_GAS_BUDGET, 1.into(), 3.into());
...
struct TransferResult {
    ...
    pub response: HaneulResult<(Option<SignedTransaction>, Option<SignedTransactionEffects>)>,
}
