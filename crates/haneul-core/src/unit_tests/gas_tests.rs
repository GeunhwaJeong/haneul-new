...
static MIN_GAS_BUDGET: Lazy<u64> =
    Lazy::new(|| HaneulCostTable::new_for_testing().min_gas_budget_external());
...
async fn test_native_transfer_sufficient_gas() -> HaneulResult {
    ...
    let mut gas_status = HaneulGasStatus::new_with_budget(
        *MAX_GAS_BUDGET,
        1.into(),
        1.into(),
        HaneulCostTable::new_for_testing(),
    );
...
async fn test_native_transfer_insufficient_gas_execution() {
    ...
    assert_eq!(
        gas_coin.value(),
        budget - effects.gas_cost_summary().gas_used()
    );
...
// disabled because it violates the HANEUL conservation checks
#[ignore]
#[tokio::test]
...
async fn test_storage_gas_unit_price() -> HaneulResult {
    let mut gas_status1 = HaneulGasStatus::new_with_budget(
        *MAX_GAS_BUDGET,
        1.into(),
        1.into(),
        HaneulCostTable::new_for_testing(),
    ...
    let mut gas_status2 = HaneulGasStatus::new_with_budget(
        *MAX_GAS_BUDGET,
        1.into(),
        3.into(),
        HaneulCostTable::new_for_testing(),
