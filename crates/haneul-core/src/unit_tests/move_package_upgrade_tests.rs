...
fn build_upgrade_test_modules(test_dir: &str) -> (Vec<u8>, Vec<Vec<u8>>) {
    ...
    let package = haneul_framework::build_move_package(&path, build_config).unwrap();
    (
...
pub fn build_upgrade_test_modules_with_dep_addr(
    ...
    path.extend(["src", "unit_tests", "data", "move_upgrade", test_dir]);
    let with_unpublished_deps = false;
    let mut package = haneul_framework::build_move_package(&path, build_config).unwrap();
...
pub fn build_upgrade_txn(
    ...
    let upgrade_ticket = move_call! {
        builder,
        (HANEUL_FRAMEWORK_OBJECT_ID)::package::authorize_upgrade(Argument::Input(0), upgrade_arg, digest_arg)
    };
    let upgrade_receipt = builder.upgrade(current_pkg_id, upgrade_ticket, vec![], modules);
    move_call! {
        builder,
        (HANEUL_FRAMEWORK_OBJECT_ID)::package::commit_upgrade(Argument::Input(0), upgrade_receipt)
    };
    ...
    pub async fn upgrade(
        ...
        let pt = {
            ...
            let ticket = move_call! {
                builder,
                (HANEUL_FRAMEWORK_OBJECT_ID)::package::authorize_upgrade(cap, policy, digest)
            };

            let receipt = builder.upgrade(package_id, ticket, dep_ids, modules);
            move_call! { builder, (HANEUL_FRAMEWORK_OBJECT_ID)::package::commit_upgrade(cap, receipt) };
...
async fn test_upgrade_introduces_type_then_uses_it() {
    ...
    let effects = runner
        .upgrade(
            ...
            digest,
            modules,
            vec![HANEUL_FRAMEWORK_OBJECT_ID, MOVE_STDLIB_OBJECT_ID],
        )
        .await;
    ...
    let TransactionEffects::V1(effects) = runner
        .run({
            ...
            let cap = builder
                ...
                .unwrap();
            move_call! { builder, (HANEUL_FRAMEWORK_OBJECT_ID)::package::only_dep_upgrades(cap) };
            builder.finish()
        })
        .await;
