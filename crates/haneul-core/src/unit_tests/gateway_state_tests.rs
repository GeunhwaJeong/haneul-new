...
use haneul_types::crypto::{AccountKeyPair, Signature};
...
#[tokio::test]
async fn test_pay_haneul_empty_input_coins() -> Result<(), anyhow::Error> {
    ...
    let res = gateway
        .pay_haneul(addr1, vec![], vec![recipient], vec![100], 1000)
        .await;
    assert!(res.is_err());
    Ok(())
}
...
#[tokio::test]
async fn test_pay_all_haneul_empty_input_coins() -> Result<(), anyhow::Error> {
    let (addr1, _): (_, AccountKeyPair) = get_key_pair();
    let (recipient, _): (_, AccountKeyPair) = get_key_pair();
    let coin_object = Object::with_owner_for_testing(addr1);
    let genesis_objects = vec![coin_object.clone()];
    ...
    let res = gateway.pay_all_haneul(addr1, vec![], recipient, 1000).await;
    assert!(res.is_err());
    Ok(())
}
...
async fn test_get_owner_object() {
    ...
    let data = gateway
        .move_call(
            ...
            vec![],
            vec![
                HaneulJsonValue::new(json!(parent.object_id.to_hex_literal())).unwrap(),
                HaneulJsonValue::new(json!(child.object_id.to_hex_literal())).unwrap(),
            ],
            None,
            10000,
        )
