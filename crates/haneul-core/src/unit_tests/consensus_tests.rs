...
async fn listen_to_sequenced_transaction() {
    ...
    ConsensusListener::spawn(/* rx_consensus_listener */ rx_haneul_to_consensus);
...
async fn submit_transaction_to_consensus_adapter() {
    let port = haneul_config::utils::get_available_port();
    ...
    loop {
        match adapter.submit(&name, &certificate).await {
            Ok(_) => break,
            Err(HaneulError::ConsensusConnectionBroken(..)) => (),
