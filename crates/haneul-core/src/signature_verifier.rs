...
use std::sync::Arc;
use haneul_protocol_config::AliasedAddress;
use haneul_types::base_types::HaneulAddress;
use haneul_types::digests::ZKLoginInputsDigest;
use haneul_types::digests::{SenderSignedDataDigest, TransactionDigest};
...
type Sender = oneshot::Sender<HaneulResult<VerifiedCertificate>>;
...
pub type AliasedAddressMap = BTreeMap<HaneulAddress, (HaneulAddress, BTreeSet<TransactionDigest>)>;
...
impl SignatureVerifier {
                ...
                aliased_addresses
                    ...
                    .map(
                        |AliasedAddress {
                            ...
                            (
                                HaneulAddress::from_bytes(original).unwrap(),
                                (
                                    HaneulAddress::from_bytes(aliased).unwrap(),
                                    allowed_tx_digests
                                        .into_iter()
    ...
    pub async fn multi_verify_certs(
        &self,
        certs: Vec<CertifiedTransaction>,
    ) -> Vec<HaneulResult<VerifiedCertificate>> {
    ...
    pub fn verify_tx(&self, signed_tx: &SenderSignedData) -> HaneulResult {
