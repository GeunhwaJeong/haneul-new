...
use haneullabs_common::{debug_fatal, izip_debug_eq};
...
impl SignatureVerifier {
    ...
    ) -> HaneulResult<NonEmpty<(u8, Option<SequenceNumber>)>> {
    ...
    pub fn verify_tx_require_no_aliases(&self, signed_tx: &SenderSignedData) -> HaneulResult {
    ...
    fn verify_tx(
        ...
        aliased_addresses: Vec<(HaneulAddress, NonEmpty<HaneulAddress>)>,
    ) -> HaneulResult<Vec<u8>> {
