                    ...
                    Some(cert) => {
                        ...
                        ct.verify_signature(&committee)
                            ...
                            .map_err(|e| match e {
                                ...
                                HaneulError::InvalidSignature { error } => {
                                    self.metrics
                                        .potentially_temporarily_invalid_signatures
                                        .inc();
                                    HaneulError::PotentiallyTemporarilyInvalidSignature { error }
                                }
                                _ => e,
                            })?;
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    ) -> HaneulResult<HandleCertificateResponse> {
    ...
    ) -> HaneulResult<HandleCertificateResponseV2> {
        let signed_effects =
