...
impl<C> SafeClient<C> {
    ...
    /// TODO: Get rid of this. https://github.com/GeunhwaJeong/haneul/issues/3740
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
                        ...
                        if signed_proposal.summary.sequence_number > 0 {
                            ...
                            if cert.auth_signature.epoch != signed_proposal.auth_signature.epoch {
                                ...
                                fp_ensure!(
                                    ...
                                    HaneulError::from("Unexpected epoch for checkpoint cert")
                                );
                ...
                let result = match &batch_info_item {
                    ...
                    Ok(BatchInfoResponseItem(UpdateItem::Transaction((seq, digest)))) => {
                        ...
                        match txs_and_last_batch {
                            None => {
                                let err = HaneulError::ByzantineAuthoritySuspicion {
                                    authority: address,
    ...
    fn verify_epoch(
        ...
        if let Some(epoch) = &response.epoch_info {
            fp_ensure!(
                ...
                HaneulError::InvalidEpochResponse("Responded epoch number mismatch".to_string())
            );
        }
        match (requested_epoch_id, &response.epoch_info) {
            (None, None) => Err(HaneulError::InvalidEpochResponse(
                "Latest epoch must not be None".to_string(),
            )),
            (Some(epoch_id), None) => {
                fp_ensure!(
                    epoch_id != 0,
                    HaneulError::InvalidEpochResponse("Genesis epoch must be available".to_string())
                );
                Ok(())
            }
    ...
    pub async fn handle_epoch(&self, request: EpochRequest) -> Result<EpochResponse, HaneulError> {
        ...
        self.verify_epoch(epoch_id, &response)
            .map_err(|err| HaneulError::ByzantineAuthoritySuspicion {
                authority,
