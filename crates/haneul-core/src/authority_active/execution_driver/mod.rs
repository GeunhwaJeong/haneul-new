...
use haneul_types::{base_types::TransactionDigest, error::HaneulResult, messages::CertifiedTransaction};
use tracing::debug;
use typed_store::Map;
...
async fn execute_pending<A>(active_authority: &ActiveAuthority<A>) -> HaneulResult<()>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    for (i, d, c) in cert_seq {
        ...
        net.sync_certificate_to_authority_with_timeout_inner(
            haneul_types::messages::ConfirmationTransaction::new(c.clone()),
