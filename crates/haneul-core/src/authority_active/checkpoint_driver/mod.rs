...
impl Default for CheckpointProcessControl {
    ...
    fn default() -> CheckpointProcessControl {
        CheckpointProcessControl {
            ...
            // TODO: Optimize this.
            // https://github.com/GeunhwaJeong/haneul/issues/3619.
...
pub enum CheckpointStepError {
    ...
    SyncCheckpointFromQuorumFailed(Box<HaneulError>),
    UpdateLatestCheckpointFailed(Box<HaneulError>),
    WaitForCheckpointCert,
    ProposalFailed(Box<HaneulError>),
    CheckpointCreationFailed,
    CheckpointSignBlocked(Box<HaneulError>),
}
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone + Reconfigurable,
{
    let errors = active_authority
        ...
        .collect::<Vec<HaneulError>>()
        .await;

    if !errors.is_empty() {
        let error = "Failed to execute transactions in checkpoint".to_string();
        return Err(HaneulError::CheckpointingError { error });
    }
    ...
    let next_epoch_committee = if enable_reconfig {
        ...
        if is_last_checkpoint {
            ...
            let haneul_system_state = active_authority.state.get_haneul_system_state_object().await?;
            Some(haneul_system_state.get_next_epoch_committee())
        } else {
            None
        }
    } else {
...
) -> Result<Option<CertifiedCheckpointSummary>, HaneulError>
where
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    let available_authorities: BTreeSet<_> = latest_known_checkpoint
        ...
        .collect::<HaneulResult<BTreeSet<_>>>()?
        .iter()
        .map(|&&x| x)
        .collect();
...
) -> Result<(CertifiedCheckpointSummary, CheckpointContents), HaneulError>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
...
pub async fn get_one_checkpoint<A>(
    ...
    contents: bool,
    available_authorities: &BTreeSet<AuthorityName>,
) -> Result<(CertifiedCheckpointSummary, Option<CheckpointContents>), HaneulError>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
...
) -> Result<CheckpointFragment, HaneulError>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    for tx_digest in &fragment.diff.second.items {
        let cert = active_authority
            ...
            .await?
            .ok_or(HaneulError::CertificateNotfound {
    ...
    let downloads: Vec<_> = missing
        ...
        .map(|(digests, _)| {
            ...
            async move {
                ...
                Ok((*digests, cert)) as HaneulResult<_>
            }
        })
        .collect();
    let results: HaneulResult<BTreeMap<_, _>> =
