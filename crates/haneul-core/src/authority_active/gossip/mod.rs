...
async fn wait_for_one_gossip_task_to_finish<A>(
    ...
    gossip_tasks: &mut FuturesUnordered<
        impl Future<Output = (AuthorityName, Result<(), HaneulError>)>,
    >,
) where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
...
pub async fn select_gossip_peer<A>(
    ...
    active_authority: &ActiveAuthority<A>,
) -> Result<AuthorityName, HaneulError>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn start(mut self, duration: Duration) -> (AuthorityName, Result<(), HaneulError>) {
        let peer_name = self.peer_name;
        let result = self.peer_gossip_for_duration(duration).await;
        (peer_name, result)
    }

    async fn peer_gossip_for_duration(&mut self, duration: Duration) -> Result<(), HaneulError> {
    ...
    async fn process_response(&self, response: TransactionInfoResponse) -> Result<(), HaneulError> {
