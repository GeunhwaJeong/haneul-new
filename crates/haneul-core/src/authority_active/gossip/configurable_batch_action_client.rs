...
use haneul_types::batch::{AuthorityBatch, SignedBatch, UpdateItem};
...
impl AuthorityAPI for ConfigurableBatchActionClient {
    ...
    async fn handle_batch_stream(
        ...
        let mut items: Vec<Result<BatchInfoResponseItem, HaneulError>> = Vec::new();
        let mut seq = 0;
        let _ = actions.into_iter().for_each(|action| {
            match action {
                BatchActionInternal::EmitUpdateItem(test_batch) => {
                    let mut temp_items: Vec<Result<BatchInfoResponseItem, HaneulError>> = Vec::new();
                ...
                BatchActionInternal::EmitError() => {
                    items.push(Err(HaneulError::GenericAuthorityError {
