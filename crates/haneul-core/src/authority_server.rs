...
impl ValidatorService {
    ...
    // `Self::handle_submit_transaction()` and `HaneulTxValidator::vote_transaction()` as well.
    async fn handle_transaction(
        ...
        let info = state
            ...
            .await
            .tap_err(|e| {
                if let HaneulError::ValidatorHaltedAtEpochEnd = e {
    ...
    async fn handle_submit_transaction(
        ...
        let transaction = bcs::from_bytes::<Transaction>(&request.transaction).map_err(|e| {
            HaneulError::TransactionDeserializationError {
                error: e.to_string(),
            }
        })?;
        transaction.validity_check(&epoch_store.tx_validity_check_context())?;
    ...
    async fn handle_submit_to_consensus_for_position(
        ...
        let (tx_consensus_positions, rx_consensus_positions) = oneshot::channel();
        {
            ...
            if !reconfiguration_lock.should_accept_user_certs() {
                ...
                return Err(HaneulError::ValidatorHaltedAtEpochEnd.into());
            }
        ...
        let consensus_positions = rx_consensus_positions.await.map_err(|e| {
            HaneulError::FailedToSubmitToConsensus(format!("Failed to get consensus position: {e}"))
        })?;
...
impl ValidatorService {
            ...
            NotifyReadConsensusTxStatusResult::Status(status) => match status {
                ConsensusTxStatus::Rejected => {
                    let error = epoch_store
                        ...
                        .unwrap_or(HaneulError::TransactionRejectReasonNotFound { digest: tx_digest });
                    ...
                    match second_status {
                        NotifyReadConsensusTxStatusResult::Status(status) => {
                            if status == ConsensusTxStatus::Rejected {
                                let error = epoch_store.get_rejection_vote_reason(consensus_position).unwrap_or(HaneulError::TransactionRejectReasonNotFound { digest: tx_digest });
                                return Ok(WaitForEffectsResponse::Rejected { error });
                            }
    ...
    async fn soft_bundle_validity_check(
        ...
        // Enforce these checks per [SIP-19](https://github.com/haneul-foundation/sips/blob/main/sips/sip-19.md):
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates =
            NonEmpty::from_vec(request.certificates).ok_or(HaneulError::NoCertificateProvidedError)?;
    ...
    async fn handle_traffic_req(&self, client: Option<IpAddr>) -> Result<(), tonic::Status> {
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                // Entity in blocklist
                Err(tonic::Status::from_error(HaneulError::TooManyRequests.into()))
            } else {
                Ok(())
            }
        } else {
...
fn normalize(err: HaneulError) -> Weight {
    match err {
        ...
        | HaneulError::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
