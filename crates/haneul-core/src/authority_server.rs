...
use haneul_types::{effects::TransactionEffectsAPI, messages_grpc::HandleTransactionRequestV2};
...
impl AuthorityServer {
    ...
    pub async fn spawn_with_bind_address_for_test(
        ...
        let mut server = haneullabs_network::config::Config::new()
            .server_builder()
...
impl ValidatorService {
    ...
    // `handle_transaction_v2()` and `HaneulTxValidator::vote_transaction()` as well.
    ...
    async fn handle_submit_to_consensus(
        ...
        let responses = futures::future::try_join_all(consensus_transactions.into_iter().map(
            |tx| async move {
                ...
                Ok::<_, HaneulError>(HandleTransactionResponseV2 {
                    effects,
                    events,
                    input_objects,
                    output_objects,
...
impl ValidatorService {
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates = NonEmpty::from_vec(request.certificates)
            .ok_or_else(|| HaneulError::NoCertificateProvidedError)?;
        for certificate in &certificates {
