...
impl AuthorityAPI for LocalAuthorityClient {
    async fn submit_transaction(
        ...
        if self.fault_config.fail_before_submit_transaction {
            return Err(HaneulError::from("Mock error before submit_transaction"));
        }
        let state = self.state.clone();
        let epoch_store = self.state.load_epoch_store_one_call_per_task();
        let deserialized_transaction = bcs::from_bytes::<Transaction>(&request.transaction)
            .map_err(|e| HaneulError::TransactionDeserializationError {
                error: e.to_string(),
            })?;
        ...
        if let Some((effects, events)) = tx_output {
            ...
            return Ok(RawSubmitTxResponse {
                effects: bcs::to_bytes(&effects)
                    .map_err(|e| HaneulError::TransactionEffectsSerializationError {
                        error: e.to_string(),
                    })?
                    .into(),
                events: request.include_events.then_some(
                    bcs::to_bytes(&events)
                        .map_err(|e| HaneulError::TransactionEventsSerializationError {
                            error: e.to_string(),
                        })?
                        .into(),
                ),
                input_objects: input_objects
                    ...
                    .into_iter()
                    .map(|obj| {
                        bcs::to_bytes(&obj).map_err(|e| HaneulError::ObjectSerializationError {
                            error: e.to_string(),
                        })
                    })
                    .collect::<Result<_, _>>()?,
        ...
        Ok::<_, HaneulError>(RawSubmitTxResponse {
            effects: bcs::to_bytes(&effects)
                .map_err(|e| HaneulError::TransactionEffectsSerializationError {
                    error: e.to_string(),
                })?
            ...
            events: events
                .map(|e| {
                    bcs::to_bytes(&e).map(|v| v.into()).map_err(|e| {
                        HaneulError::TransactionEventsSerializationError {
                            error: e.to_string(),
                        }
                    })
                })
    ...
    async fn wait_for_effects(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<RawWaitForEffectsResponse, HaneulError> {
        unimplemented!()
    }
...
impl AuthorityAPI for MockAuthorityApi {
    /// Submit a new transaction to a Haneul or Primary account.
    async fn submit_transaction(
        &self,
...
pub struct HandleTransactionTestAuthorityClient {
    pub tx_info_resp_to_return: HaneulResult<HandleTransactionResponse>,
    pub cert_resp_to_return: HaneulResult<HandleCertificateResponseV2>,
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(HaneulError::Unknown("".to_string())),
            cert_resp_to_return: Err(HaneulError::Unknown("".to_string())),
    ...
    pub fn set_tx_info_response_error(&mut self, error: HaneulError) {
        self.tx_info_resp_to_return = Err(error);
    }

    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(HaneulError::Unknown("".to_string()));
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: HaneulError) {
        self.cert_resp_to_return = Err(error);
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(HaneulError::Unknown("".to_string()));
    }
