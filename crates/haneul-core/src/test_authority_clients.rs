...
use crate::authority::{ExecutionEnv, test_authority_builder::TestAuthorityBuilder};
...
impl AuthorityAPI for LocalAuthorityClient {
    async fn submit_transaction(
        ...
        let deserialized_transaction = bcs::from_bytes::<Transaction>(&raw_request.transactions[0])
            .map_err(|e| HaneulErrorKind::TransactionDeserializationError {
                error: e.to_string(),
            })?;
...
impl AuthorityAPI for MockAuthorityApi {
    ...
    async fn handle_transaction_info_request(
        ...
        Err(HaneulErrorKind::TransactionNotFound {
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(HaneulErrorKind::Unknown("".to_string()).into()),
            cert_resp_to_return: Err(HaneulErrorKind::Unknown("".to_string()).into()),
    ...
    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(HaneulErrorKind::Unknown("".to_string()).into());
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: impl Into<HaneulError>) {
        self.cert_resp_to_return = Err(error.into());
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(HaneulErrorKind::Unknown("".to_string()).into());
    }
