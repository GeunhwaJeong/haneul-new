...
/// method, where each value is produced via `haneullabs_common::rpc_format::ToFormat`. This is the
    ...
    let render_arm = quote! {
        {
            let value = self
                .#field_name
                .as_ref()
                .map(|v| <_ as ::haneullabs_common::rpc_format::ToFormat>::to_format::<F, _>(v, meter))
                .transpose()?;
