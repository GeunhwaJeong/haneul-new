...
/// method, where each value is produced via `haneullabs_common::rpc_format::ToFormat`. Fields that
...
pub fn accessors_macro(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        impl #struct_name {
            ...
            pub fn render<F>(
                &self,
                meter: &mut impl ::haneullabs_common::rpc_format::Meter,
            ) -> ::std::result::Result<
                std::collections::BTreeMap<String, F>,
                ::haneullabs_common::rpc_format::MeterError,
            >
            where
                F: ::haneullabs_common::rpc_format::Format,
            {
    ...
    let render_arm = quote! {
        {
            let value = match self.#field_name.as_ref() {
                Some(v) => <_ as ::haneullabs_common::rpc_format::ToFormat>::to_format::<F, _>(v, meter)?,
                None => <F as ::haneullabs_common::rpc_format::Format>::null(meter)?,
            };
