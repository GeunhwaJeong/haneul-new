...
async fn test_indexing_transfer() {
    ...
    let indexer = create_haneul_indexer(pool.clone(), metrics.clone(), ingestion_metrics, &config)
        .await
        .unwrap();
    ...
    // transfer eth to haneul
    initiate_bridge_eth_to_haneul(&cluster, 1000, 0).await.unwrap();
...
async fn setup_bridge_env(with_eth_env: bool) -> (IndexerConfig, BridgeTestCluster, TempDb) {
    ...
    let config = IndexerConfig {
        remote_store_url: format!("{}/rest", bridge_test_cluster.haneul_rpc_url()),
        ...
        concurrency: 10,
        haneul_bridge_genesis_checkpoint: 0,
        eth_bridge_genesis_block: 0,
        eth_haneul_bridge_contract_address: bridge_test_cluster.haneul_bridge_address(),
        metric_port: 9001,
    };
