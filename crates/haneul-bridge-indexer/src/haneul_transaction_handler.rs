...
pub async fn handle_haneul_transcations_loop(
    pg_pool: PgPool,
    rx: haneullabs_metrics::metered_channel::Receiver<(
    ...
    let checkpoint_commit_batch_size = std::env::var("COMMIT_BATCH_SIZE")
        ...
        .parse::<usize>()
        .unwrap();
    let mut stream = haneullabs_metrics::metered_channel::ReceiverStream::new(rx)
