...
use crate::{
    metrics::BridgeIndexerMetrics, BridgeDataSource, ProcessedTxnData, HaneulTxnError, TokenTransfer,
...
use haneul_types::{
    ...
    BRIDGE_ADDRESS, HANEUL_BRIDGE_OBJECT_ID,
};
use tap::tap::TapFallible;
use tracing::info;

pub struct HaneulBridgeWorker {
    bridge_object_ids: BTreeSet<ObjectID>,
    pg_pool: PgPool,
    metrics: BridgeIndexerMetrics,
}

impl HaneulBridgeWorker {
    pub fn new(
        ...
        bridge_object_ids.insert(HANEUL_BRIDGE_OBJECT_ID);
        Self {
            bridge_object_ids,
            pg_pool,
            metrics,
        ...
        match &tx.events {
            Some(events) => {
                let token_transfers = events.data.iter().try_fold(vec![], |mut result, ev| {
                    if let Some(data) =
                        Self::process_haneul_event(ev, tx, checkpoint, timestamp_ms, &self.metrics)?
                    {
                        result.push(data);
                    }
...
#[async_trait]
impl Worker for HaneulBridgeWorker {
    async fn process_checkpoint(&self, checkpoint: CheckpointData) -> Result<()> {
        ...
        write(&self.pg_pool, bridge_data).tap_ok(|_| {
            ...
            self.metrics
                .last_committed_haneul_checkpoint
