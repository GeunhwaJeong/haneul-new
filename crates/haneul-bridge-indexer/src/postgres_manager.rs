...
const HANEUL_PROGRESS_STORE_DUMMY_KEY: i32 = 1;
...
pub fn update_haneul_progress_store(
    pool: &PgPool,
    tx_digest: TransactionDigest,
) -> Result<(), anyhow::Error> {
    let mut conn = pool.get()?;
    diesel::insert_into(schema::haneul_progress_store::table)
        .values(&HaneulProgressStore {
            id: HANEUL_PROGRESS_STORE_DUMMY_KEY,
            txn_digest: tx_digest.inner().to_vec(),
        })
        .on_conflict(schema::haneul_progress_store::dsl::id)
        .do_update()
...
pub fn read_haneul_progress_store(pool: &PgPool) -> anyhow::Result<Option<TransactionDigest>> {
    let mut conn = pool.get()?;
