...
async fn main() -> Result<()> {
    ...
    start_processing_haneul_checkpoints(&config, indexer_meterics).await?;

    Ok(())
}

async fn start_processing_haneul_checkpoints(
    config: &haneul_bridge_indexer::config::Config,
    ...
    let worker_pool = WorkerPool::new(
        HaneulBridgeWorker::new(vec![], config.db_url.clone(), indexer_metrics_cloned),
        "bridge worker".into(),
...
async fn start_processing_eth_events(config: &haneul_bridge_indexer::config::Config) -> Result<()> {
