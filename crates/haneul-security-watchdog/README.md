...
```shell
cargo run --release -p haneul-security-watchdog
```
## Usage
...
The config file allows setting up time based schedule for expected results. For example, when periodically checking total haneul in the network we want it to be an exact value i.e 10B whereas when periodically checking balance of an account
...
[
  {
    ...
    "sql_query": "SELECT total_haneul FROM total_haneul_mainnet ORDER BY epoch DESC LIMIT 1",
    "metric_name": "total_haneul_10B",
    "timed_exact_limits": {
      // total haneul should always be exact 10B since
      // the dawn of time
  ...
  },
  {
    ...
    "timed_lower_limits": {
      ...
      // i.e. balance should not drop below 50 HANEUL before 1/1/2024,
      // balance should not drop below 100HANEUL before 2/1/2024
      // and it should not drop below 150HANEUL before 3/1/2024
