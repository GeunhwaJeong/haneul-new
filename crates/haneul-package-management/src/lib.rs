...
use haneul_json_rpc_types::{get_new_package_obj_from_response, HaneulTransactionBlockResponse};
...
pub async fn update_lock_file(
    ...
    let Some(lock_file) = lock_file else {
        bail!(
	     ...
	     package, but none found. Consider running `haneul move build` to \
    ...
    let env = context.config.get_active_env().context(
	 ...
	 Try ensure `haneul client active-env` is valid.",
    )?;
