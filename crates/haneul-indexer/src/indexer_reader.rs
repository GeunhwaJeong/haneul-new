...
use haneul_json_rpc_types::{
    ...
    NetworkMetrics, HaneulEvent, HaneulObjectDataFilter, HaneulTransactionBlockResponse, TransactionFilter,
};
use haneul_json_rpc_types::{
    Balance, Coin as HaneulCoin, HaneulCoinMetadata, HaneulTransactionBlockEffects,
...
impl IndexerReader {
    ...
    pub fn get_latest_haneul_system_state(&self) -> Result<HaneulSystemStateSummary, IndexerError> {
    ...
    pub fn get_epoch_haneul_system_state(
        &self,
        epoch: Option<EpochId>,
        ...
        let system_state: HaneulSystemStateSummary = bcs::from_bytes(&stored_epoch.system_state)
            .map_err(|_| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
    ...
    pub fn get_latest_checkpoint(&self) -> Result<haneul_json_rpc_types::Checkpoint, IndexerError> {
    ...
    fn get_transaction_effects_with_digest(
    ...
    ) -> Result<HaneulTransactionBlockEffects, IndexerError> {
        ...
        stored_txn.try_into_haneul_transaction_effects()
    }
    ...
    fn stored_transaction_to_transaction_block(
        ...
        stored_txes
            .into_iter()
            .map(|stored_tx| stored_tx.try_into_haneul_transaction_block_response(&options, self))
    ...
    fn get_transaction_events_impl(
        ...
        let events = serialized_events
            .into_iter()
            .flatten()
            .map(|event| bcs::from_bytes::<haneul_types::event::Event>(&event))
            .collect::<Result<Vec<_>, _>>()?;
        ...
        events
            ...
            .map(|(i, event)| {
                ...
                haneul_json_rpc_types::HaneulEvent::try_from(
                    event,
                    digest,
                    i as u64,
    ...
    fn query_events_impl(
        ...
        stored_events
            .into_iter()
            .map(|se| se.try_into_haneul_event(self))
            .collect()
    }
    ...
    fn get_coin_metadata(
        ...
        if let Some(id) = coin_metadata_obj_id {
            ...
            Ok(metadata_object.and_then(|v| HaneulCoinMetadata::try_from(v).ok()))
        } else {
            Ok(None)
        }
    }
...
impl haneul_types::storage::ObjectStore for IndexerReader {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ) -> Result<Option<haneul_types::object::Object>, haneul_types::error::HaneulError> {
        self.get_object(object_id, None)
            .map_err(|e| haneul_types::error::HaneulError::GenericStorageError(e.to_string()))
    }
