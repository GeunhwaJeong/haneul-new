...
impl<U: R2D2Connection> IndexerReader<U> {
    ...
    pub async fn query_events_in_blocking_task(
        ...
        let haneul_events = futures::future::join_all(haneul_event_futures)
            ...
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| tracing::error!("Failed to join haneul event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| tracing::error!("Failed to collect haneul event futures: {}", e))?;
        Ok(haneul_events)
    }
...
impl<U: R2D2Connection> haneul_types::storage::ObjectStore for IndexerReader<U> {
    fn get_object(
        &self,
        object_id: &ObjectID,
