    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let (epoch_indexing_sender, epoch_indexing_receiver) = haneullabs_metrics::metered_channel::channel(
        EPOCH_QUEUE_LIMIT,
        &global_metrics
            .channels
...
pub struct CheckpointProcessor<S> {
    state: S,
    metrics: IndexerMetrics,
    epoch_indexing_sender: haneullabs_metrics::metered_channel::Sender<TemporaryEpochStore>,
    checkpoint_sender: haneullabs_metrics::metered_channel::Sender<TemporaryCheckpointStore>,
}
...
struct CheckpointDataObjectStore<'a> {
    objects: Vec<&'a haneul_types::object::Object>,
}
...
impl<'a> haneul_types::storage::ObjectStore for CheckpointDataObjectStore<'a> {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    async fn index_epoch(
        ...
        {
            // very first epoch
            let system_state = get_haneul_system_state(&checkpoint_object_store)?;
    ...
    async fn index_checkpoint_and_epoch(
        ...
        } in transactions
        {
            ...
            db_events.extend(events.iter().flat_map(|events| &events.data).map(|event| {
                Event::from_haneul_event(event, transaction_digest, checkpoint_summary.timestamp_ms)
            }));
            ...
            // Move Calls
            if let haneul_types::transaction::TransactionKind::ProgrammableTransaction(pt) = tx.kind()
            {
                db_move_calls.extend(pt.commands.clone().into_iter().filter_map(move |command| {
                    match command {
                        haneul_types::transaction::Command::MoveCall(m) => Some(MoveCall {
                            id: None,
                            transaction_digest: transaction_digest.to_string(),
        ...
        Ok((
            TemporaryCheckpointStore {
                checkpoint: Checkpoint::from_haneul_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
                    total_transactions,
...
pub async fn start_tx_checkpoint_commit_task<S>(
    ...
    metrics: IndexerMetrics,
    config: IndexerConfig,
    tx_indexing_receiver: haneullabs_metrics::metered_channel::Receiver<TemporaryCheckpointStore>,
) where
    S: IndexerStore + Clone + Sync + Send + 'static,
{
...
pub async fn start_epoch_commit_task<S>(
    state: S,
    metrics: IndexerMetrics,
    epoch_indexing_receiver: haneullabs_metrics::metered_channel::Receiver<TemporaryEpochStore>,
) where
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    use futures::StreamExt;
    ...
    let mut stream = haneullabs_metrics::metered_channel::ReceiverStream::new(epoch_indexing_receiver);
...
pub async fn start_object_checkpoint_commit_task<S>(
    ...
    metrics: IndexerMetrics,
    config: IndexerConfig,
    object_indexing_receiver: haneullabs_metrics::metered_channel::Receiver<(
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let mut stream = haneullabs_metrics::metered_channel::ReceiverStream::new(object_indexing_receiver)
        .ready_chunks(checkpoint_commit_batch_size);
...
pub struct ObjectsProcessor<S> {
    metrics: IndexerMetrics,
    object_indexing_sender: haneullabs_metrics::metered_channel::Sender<(
        haneul_types::messages_checkpoint::CheckpointSequenceNumber,
    ...
    deleted
        ...
        .map(|(status, oref)| {
            DeletedObject::from(
                epoch,
                Some(checkpoint),
                &haneul_json_rpc_types::HaneulObjectRef::from(oref.to_owned()),
