...
use haneul_types::HANEUL_SYSTEM_STATE_ADDRESS;
...
pub struct CheckpointHandler<S> {
    ...
    tx_indexing_sender: Arc<haneullabs_metrics::metered_channel::Sender<TemporaryCheckpointStore>>,
    tx_indexing_receiver:
        Option<haneullabs_metrics::metered_channel::Receiver<TemporaryCheckpointStore>>,
    object_indexing_sender: Arc<
        haneullabs_metrics::metered_channel::Sender<(
    ...
    >,
    epoch_indexing_sender: Arc<haneullabs_metrics::metered_channel::Sender<TemporaryEpochStore>>,
    epoch_indexing_receiver: Option<haneullabs_metrics::metered_channel::Receiver<TemporaryEpochStore>>,
}
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    pub fn new(
        ...
        let (tx_indexing_sender, tx_indexing_receiver) = haneullabs_metrics::metered_channel::channel(
    ...
    pub async fn run_checkpoint_txes_downloader(
        ...
        mut start_seq: u64,
        tx: haneullabs_metrics::metered_channel::Sender<CheckpointTxData>,
    ) {
        loop {
    ...
    pub async fn run_checkpoint_objects_downloader(
        checkpoint_download_handler: Arc<Self>,
        mut start_seq: u64,
        tx: haneullabs_metrics::metered_channel::Sender<CheckpointObjectData>,
    ) {
        loop {
            if let Err(e) = checkpoint_download_handler
    ...
    async fn start_tx_checkpoint_commit(
        self: &Arc<Self>,
        tx_indexing_receiver: &mut haneullabs_metrics::metered_channel::Receiver<
    ...
    async fn start_object_checkpoint_commit(
        &self,
        object_indexing_receiver: &mut haneullabs_metrics::metered_channel::Receiver<(
            CheckpointSequenceNumber,
            Vec<TransactionObjectChanges>,
        )>,
    ) -> Result<(), IndexerError> {
    ...
    async fn start_epoch_commit(
        &self,
        epoch_indexing_receiver: &mut haneullabs_metrics::metered_channel::Receiver<
            TemporaryEpochStore,
        >,
    ) -> Result<(), IndexerError> {
    ...
    async fn download_transactions_per_checkpoint(
        ...
        let system_state_objects =
            Self::get_haneul_system_state_object(&self.http_client, &checkpoint, &transactions)
                .await
                .tap_ok(|res| {
                    if !res.is_empty() {
                        info!(
    ...
    /// Get HaneulSystemState objects (0x5 and its children) in Genesis and ChangeEpoch
    /// transactions, for epoch indexing.
    async fn get_haneul_system_state_object(
        http_client: &HttpClient,
        checkpoint: &haneul_json_rpc_types::Checkpoint,
        transactions: &[CheckpointTransactionBlockResponse],
    ) -> Result<Vec<haneul_types::object::Object>, IndexerError> {
        if checkpoint.sequence_number == 0 || checkpoint.end_of_epoch_data.is_some() {
            let object_ids = transactions
                ...
                .find_map(|t| {
                    if matches!(
                        ...
                        HaneulTransactionBlockKind::ChangeEpoch(..) | HaneulTransactionBlockKind::Genesis(..)
                    ) {
                        Some(
                            t.effects
                                .all_changed_objects()
                                .iter()
                                .filter_map(|(ref_, _)| {
                                    if ref_.object_id() == HANEUL_SYSTEM_STATE_OBJECT_ID {
                                        Some(HaneulGetPastObjectRequest {
                                            object_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
                                            version: ref_.version(),
                                        })
                                    } else if matches!(
                                        ref_.owner,
                                        Owner::ObjectOwner(addr) if addr == HANEUL_SYSTEM_STATE_ADDRESS.into()
                                    ) {
                                        Some(HaneulGetPastObjectRequest {
            ...
            http_client
                .try_multi_get_past_objects(object_ids, Some(HaneulObjectDataOptions::bcs_lossless()))
                .await
                ...
                .collect::<Result<Vec<haneul_types::object::Object>, IndexerError>>()
        } else {
            Ok(vec![])
        }
    }
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    metrics: IndexerMetrics,
    epoch_indexing_sender: Arc<haneullabs_metrics::metered_channel::Sender<TemporaryEpochStore>>,
    checkpoint_sender: Arc<haneullabs_metrics::metered_channel::Sender<TemporaryCheckpointStore>>,
    downloaded_checkpoint_data_receiver:
        haneullabs_metrics::metered_channel::Receiver<CheckpointTxData>,
}
            ...
            let epoch_event = transactions.iter().find_map(|tx| {
                tx.events.data.iter().find(|ev| {
                    ev.type_.address == HANEUL_SYSTEM_ADDRESS
                        && ev.type_.module.as_ident_str() == ident_str!("haneul_system_state_inner")
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    >,
    downloaded_object_data_receiver:
        haneullabs_metrics::metered_channel::Receiver<CheckpointObjectData>,
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    fn index_packages(
        transaction_senders: &HashMap<TransactionDigest, HaneulAddress>,
        changed_objects: &[(ObjectStatus, HaneulObjectData)],
    ) -> Result<Vec<Package>, IndexerError> {
        changed_objects
            .iter()
            .filter_map(|(_, o)| {
                if let HaneulRawData::Package(p) = &o
                    .bcs
                    .as_ref()
                    .expect("Expect the content field to be non-empty from data fetching")
                {
                    // unwrap: we request the object with `HaneulObjectDataOptions::bcs_lossless()`
...
pub async fn fetch_changed_objects(
...
) -> Result<Vec<(ObjectStatus, HaneulObjectData)>, IndexerError> {
    join_all(object_changes.chunks(MULTI_GET_CHUNK_SIZE).map(|objects| {
        ...
        let wanted_past_object_request = objects
            .iter()
            .map(|(id, seq_num, _)| HaneulGetPastObjectRequest {
                object_id: *id,
                version: *seq_num,
            })
            .collect();
