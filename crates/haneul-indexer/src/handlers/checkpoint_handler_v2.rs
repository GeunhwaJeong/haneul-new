...
pub struct CheckpointHandler<S> {
    state: S,
    metrics: IndexerMetrics,
    indexed_checkpoint_sender: haneullabs_metrics::metered_channel::Sender<CheckpointDataToCommit>,
    ...
    S: IndexerStoreV2 + Clone + Sync + Send + 'static,
{
    ...
    async fn index_one_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_haneul_checkpoint(
                    &checkpoint_summary,
                    &checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.output_objects()
                    .iter()
                    .filter_map(|o| {
                        if let haneul_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
