...
fn to_latest_objects_clauses(filter: &HaneulObjectDataFilter) -> Option<String> {
    match filter {
        HaneulObjectDataFilter::AddressOwner(a) => Some(format!(
...
fn to_clauses(filter: &HaneulObjectDataFilter) -> Option<String> {
    match filter {
        ...
        HaneulObjectDataFilter::MatchNone(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
                None
            } else {
