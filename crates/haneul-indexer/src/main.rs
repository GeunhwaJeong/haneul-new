...
async fn main() -> Result<(), IndexerError> {
    ...
    if indexer_config.rpc_client_url.contains("testnet") {
        indexer_config.remote_store_url = Some("https://checkpoints.testnet.haneul.io".to_string());
    } else if indexer_config.rpc_client_url.contains("mainnet") {
        indexer_config.remote_store_url = Some("https://checkpoints.mainnet.haneul.io".to_string());
    }
    ...
    haneul_indexer::db::setup_postgres::setup(indexer_config.clone(), registry.clone()).await?;

    #[cfg(feature = "mysql-feature")]
    #[cfg(not(feature = "postgres-feature"))]
    haneul_indexer::db::setup_mysql::setup(indexer_config, registry).await?;
    Ok(())
}
