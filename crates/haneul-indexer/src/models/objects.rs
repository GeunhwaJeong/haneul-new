...
use haneul_json_rpc_types::{
    HaneulObjectData, HaneulObjectDataOptions, HaneulObjectRef, HaneulObjectResponse, HaneulRawData,
};
use haneul_types::base_types::{EpochId, ObjectType, SequenceNumber, HaneulAddress};
use haneul_types::digests::TransactionDigest;
use haneul_types::messages_checkpoint::CheckpointSequenceNumber;
use haneul_types::object::{MoveObject, Object as HaneulBaseObject, Owner};
...
impl Object {
    pub fn from(
        ...
        let bcs = match o.bcs.clone().expect("Expect BCS data to be non-empty") {
            HaneulRawData::MoveObject(o) => vec![NamedBcsBytes(OBJECT.to_string(), o.bcs_bytes)],
            HaneulRawData::Package(p) => p
                .module_map
                .into_iter()
        ...
        let has_public_transfer = match o.bcs.clone().expect("Expect BCS data to be non-empty") {
            HaneulRawData::MoveObject(o) => o.has_public_transfer,
            HaneulRawData::Package(_p) => false,
        };
    ...
    pub fn to_object_response(
        ...
        if self.object_type.as_str() == PACKAGE {
            ...
            let obj = HaneulBaseObject::new_move(move_obj.clone(), owner, previous_transaction);
            ...
            Ok(HaneulObjectResponse::Exists(HaneulObjectData {
                object_id,
                ...
                bcs: if options.show_bcs {
                    Some(HaneulRawData::MoveObject(move_obj.into()))
                } else {
                    None
                },
            }))
    ...
    pub fn get_object_ref(&self) -> Result<HaneulObjectRef, IndexerError> {
        let object_id = self.object_id.parse().map_err(|e| {
            IndexerError::SerdeError(format!(
                "Failed to parse object id: {}, error: {}",
                self.object_id, e
    ...
    pub fn get_owner(&self) -> Result<Owner, IndexerError> {
        match self.owner_type {
            OwnerType::AddressOwner => {
                let haneul_addr_str = self.owner_address.clone().ok_or_else(|| {
                    IndexerError::SerdeError(
                        "Expect the owner address to be non-empty, when owner type is AddressOwner"
                            .into(),
                    )
                })?;
                let haneul_addr = HaneulAddress::from_str(haneul_addr_str.as_str()).map_err(|e| {
                    IndexerError::SerdeError(format!(
                        "Failed to parse HANEUL address: {}, error: {}",
                        haneul_addr_str, e
                    ))
                })?;
                Ok(Owner::AddressOwner(haneul_addr))
            }
            OwnerType::ObjectOwner => {
                ...
                let object_id = HaneulAddress::from_str(object_id.as_str()).map_err(|e| {
                    IndexerError::SerdeError(format!(
                        "Failed to parse object id: {}, error: {}",
                        object_id, e
                    ))
