...
fn haneul_event_to_new_event(
    ...
    let event_json = serde_json::to_string(&haneul_event).map_err(|err| {
    ...
    Ok(NewEvent {
        ...
        event_type: haneul_event.get_event_type(),
...
pub fn events_to_haneul_events(
    pg_pool_conn: &mut PgPoolConnection,
    events: Vec<Event>,
) -> Vec<HaneulEvent> {
    let mut errors = vec![];
