...
use haneul_json_rpc_types::{
    CheckpointId, HaneulTransactionBlockEvents, HaneulTransactionBlockResponseOptions,
};
        ...
        loop {
            ...
            if latest_checkpoint >= last_processed_addr_checkpoint + ADDRESS_STATS_BATCH_SIZE as i64
            {
                ...
                let checkpoints = self
                    ...
                    .await?;
                let tx_vec: Vec<haneul_types::digests::TransactionDigest> = checkpoints
                    .into_iter()
                ...
                for tx_chunk in tx_vec.chunks(PG_TX_READ_CHUNK_SIZE) {
                    ...
                    let txs = self
                        ...
                        .await?;
                    let tx_option = HaneulTransactionBlockResponseOptions {
                        show_input: true,
                        show_raw_input: true,
                        show_effects: true,
                        ..Default::default()
                    };
                    let haneul_tx_resp_handles = txs.into_iter().map(|tx| {
                        self.store
                            .compose_haneul_transaction_block_response(tx, Some(&tx_option))
                    });
                    let haneul_tx_resp_vec = futures::future::join_all(haneul_tx_resp_handles)
                        .await
                        .into_iter()
                        ...
                        .map(|mut resp| {
                            resp.events = Some(HaneulTransactionBlockEvents { data: vec![] });
                            resp
                        })
                        .collect::<Vec<_>>();

                    // retrieve address and active address data from txs
                    let checkpoint_tx_resp_vec = haneul_tx_resp_vec
                        .into_iter()
