...
impl<S: IndexerStore> IndexerApi<S> {
    ...
    async fn query_transaction_blocks_internal(
        ...
        let tx_resp_futures = tx_vec_from_db.into_iter().map(|tx| {
            self.state
                .compose_haneul_transaction_block_response(tx, query.options.as_ref())
        });
        let haneul_tx_resp_vec = join_all(tx_resp_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()?;

        Ok(Page {
            data: haneul_tx_resp_vec,
            next_cursor,
            has_next_page,
        })
    }
    ...
    async fn get_owned_objects_internal(
        ...
        let (filter, options) = match query {
            Some(HaneulObjectResponseQuery {
                ...
                HaneulObjectDataFilter::AddressOwner(_) => Ok((address, options)),
                _ => Err(anyhow!(
                    "Only address filter is supported on indexer for now."
                )),
            },
            Some(HaneulObjectResponseQuery { filter: _, options }) => Ok((address, options)),
        ...
        let data: Vec<HaneulObjectResponse> = objects
            .into_iter()
