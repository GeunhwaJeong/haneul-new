...
impl GovernanceReadApi {
    ...
    async fn get_latest_haneul_system_state(&self) -> Result<HaneulSystemStateSummary, IndexerError> {
        self.inner.get_latest_haneul_system_state().await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects(ids).await? {
            let object = haneul_types::object::Object::try_from(stored_object)?;
    ...
    pub async fn get_delegated_stakes(
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(haneul_json_rpc_types::Stake {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_haneul_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_haneul_system_state(&self) -> RpcResult<HaneulSystemStateSummary> {
        self.get_latest_haneul_system_state().await.map_err(Into::into)
    }
