...
pub async fn check_transactions(
    ...
    for digest in tx_vec {
        let fetch_options = HaneulTransactionResponseOptions::full_content();
        let fn_haneul_txn_response = fn_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options.clone())
            .await?;
        let indexer_haneul_txn_response = indexer_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options)
            .await?;
        if fn_haneul_txn_response != indexer_haneul_txn_response {
            error!("Checkpoint transactions mismatch found in {}", digest);
            warn!(
                "Transaction response mismatch with digest {:?}:\nFN:\n{:?}\nIndexer:\n{:?} ",
                digest, fn_haneul_txn_response, indexer_haneul_txn_response
            );
            continue;
        }
        ...
        if config.check_objects {
            let objects = fn_haneul_txn_response.object_changes.unwrap();
