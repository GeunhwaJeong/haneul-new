...
use haneul_json_rpc_types::{
    BigInt, CheckpointId, ObjectChange, HaneulObjectDataOptions, HaneulTransactionBlockResponseOptions,
};
...
pub async fn check_checkpoint(
    config: &TestConfig,
    fn_client: &HaneulClient,
    indexer_client: &HaneulClient,
    checkpoint: u64,
) -> Result<(), anyhow::Error> {
...
pub async fn check_transactions(
    ...
    for digest in tx_vec {
        let fetch_options = HaneulTransactionBlockResponseOptions::full_content();
        let fn_haneul_tx_response = fn_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options.clone())
            .await?;
        let indexer_haneul_tx_response = indexer_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options)
            .await?;
        if fn_haneul_tx_response != indexer_haneul_tx_response {
            error!("Checkpoint transactions mismatch found in {}", digest);
            warn!(
                "Transaction response mismatch with digest {:?}:\nFN:\n{:?}\nIndexer:\n{:?} ",
                digest, fn_haneul_tx_response, indexer_haneul_tx_response
            );
            continue;
        }
        ...
        if config.check_objects {
            let objects = fn_haneul_tx_response.object_changes.unwrap();
