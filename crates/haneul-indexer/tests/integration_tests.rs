...
pub mod pg_integration_test {
    ...
    use haneul_json_rpc_types::{
        EventFilter, HaneulMoveObject, HaneulObjectData, HaneulObjectDataOptions, HaneulObjectResponse,
    ...
    async fn test_event_query_e2e() -> Result<(), anyhow::Error> {
        ...
        let object_correctly_transferred = indexer_rpc_client
            ...
            .into_iter()
            .filter_map(|object_resp| match object_resp {
                HaneulObjectResponse::Exists(obj_data) => Some(obj_data),
                _ => None,
            })
    ...
    async fn test_get_object_with_options() -> Result<(), anyhow::Error> {
        ...
        match resp {
            HaneulObjectResponse::Deleted(obj) => {
            ...
            _ => {
                panic!("Expected HaneulObjectResponse::Deleted, but got {:?}", resp);
            }
        }
        ...
        assert!(matches!(resp, HaneulObjectResponse::NotExists(obj_id) if obj_id == object_id));
        matches!(resp, HaneulObjectResponse::NotExists(_));
        Ok(())
    }
    ...
    async fn start_test_cluster(
        ...
        let db_url = format!("postgres://postgres:{pw}@{pg_host}:{pg_port}/haneul_indexer");
