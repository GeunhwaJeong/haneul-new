...
pub mod pg_integration_test {
    ...
    use haneul_indexer::models::objects::{
    ...
    use haneul_json_rpc_types::{
        ...
        HaneulTransactionBlockResponseQuery, TransactionBlockBytes,
    };
    ...
    use haneul_types::messages::{ExecuteTransactionRequestType, TEST_ONLY_GAS_UNIT_FOR_TRANSFER};
    ...
    async fn sign_and_execute_transaction_block(
        ...
        let tx_response = indexer_rpc_client
            .execute_transaction_block(
                tx_bytes,
                signatures,
                Some(HaneulTransactionBlockResponseOptions::full_content()),
    ...
    // TODO: we should use HaneulClient for tests like this
    ...
    > {
        ...
        let gas_objects: Vec<ObjectID> = test_cluster
            ...
            .get_owned_objects(
                *sender,
                Some(HaneulObjectResponseQuery::new_with_filter(
                    HaneulObjectDataFilter::gas_coin(),
                )),
                None,
                None,
            )
    ...
    async fn test_genesis_sync() {
        ...
        for tx_digest in checkpoint.transactions {
            ...
            let _fullnode_rpc_tx = test_cluster
                .rpc_client()
                .get_transaction_block(tx_digest, Some(HaneulTransactionBlockResponseOptions::new()))
                .await
                .unwrap();
    ...
    async fn test_simple_transaction_e2e() -> Result<(), anyhow::Error> {
        ...
        let checkpoint_seq_query =
            HaneulTransactionBlockResponseQuery::new_with_filter(TransactionFilter::Checkpoint(2u64));
        ...
        let from_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let tx_kind_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let to_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let from_to_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let mutation_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let input_query = HaneulTransactionBlockResponseQuery::new_with_filter(
        ...
        let move_call_query =
            HaneulTransactionBlockResponseQuery::new_with_filter(TransactionFilter::MoveFunction {
                package: package_id,
    ...
    async fn test_get_object_with_options() -> Result<(), anyhow::Error> {
        ...
        let show_all_content = HaneulObjectDataOptions {
            show_type: true,
            show_owner: true,
        ...
        let results: Vec<HaneulObjectResponse> = join_all(futures)
            .await
            .into_iter()
            .collect::<Result<_, _>>()
            .unwrap();
        ...
        // deleted object - returns HaneulObjectRef
        let gas_objects =
        ...
        match (&resp.data, &resp.error) {
            ...
            _ => {
                panic!(
                    "Expected HaneulObjectResponse::Deleted, but got {:?}",
                    resp.error
                );
            }
        }
        ...
        if let Some(HaneulObjectResponseError::NotExists { object_id }) = resp.error {
            assert_eq!(object_id, obj_id)
        } else {
            panic!(
                "Expected HaneulObjectResponse::NotExists, but got {:?}",
                resp.error
            );
        }
    ...
    async fn test_module_cache() {
        ...
        let parsed_coin = HaneulParsedMoveObject::try_from_layout(
    ...
    async fn test_query_objects_cross_check() -> Result<(), anyhow::Error> {
        ...
        let object_from_indexer = indexer_rpc_client
            .query_objects(
                HaneulObjectResponseQuery::new_with_filter(HaneulObjectDataFilter::AddressOwner(address)),
                None,
                None,
            )
            .await
    ...
    async fn test_query_objects() -> Result<(), anyhow::Error> {
        ...
        let all_coins = indexer_rpc_client
            .query_objects(
                HaneulObjectResponseQuery::new_with_filter(HaneulObjectDataFilter::StructType(
                    parse_struct_tag("0x2::coin::Coin<0x2::haneul::HANEUL>").unwrap(),
                )),
                None,
                None,
            )
    ...
    async fn pg_unnest_bulk_insert_update_test() {
        ...
        let bulk_data = (1..=10000)
            ...
            .map(|_| Object {
                ...
                object_type: "0x2::coin::Coin<0x2::haneul::HANEUL>".to_string(),
                object_status: ObjectStatus::Created,
                has_public_transfer: false,
                storage_rebate: 0,
    ...
    async fn test_get_transaction_with_options() -> Result<(), anyhow::Error> {
        ...
        let haneul_transaction_response_options = vec![
            HaneulTransactionBlockResponseOptions::new().with_input(),
            HaneulTransactionBlockResponseOptions::new().with_raw_input(),
            HaneulTransactionBlockResponseOptions::new().with_effects(),
            HaneulTransactionBlockResponseOptions::new().with_events(),
            HaneulTransactionBlockResponseOptions::new().with_balance_changes(),
            HaneulTransactionBlockResponseOptions::new().with_object_changes(),
        ...
        let received_transaction_results: Vec<HaneulTransactionBlockResponse> = join_all(futures)
            .await
            .into_iter()
            .collect::<Result<_, _>>()
            .unwrap();
        ...
        let expected_transaction_results = vec![
            HaneulTransactionBlockResponseBuilder::new(&full_transaction_response)
                .with_input()
                .build(),
            HaneulTransactionBlockResponseBuilder::new(&full_transaction_response)
                .with_raw_input()
