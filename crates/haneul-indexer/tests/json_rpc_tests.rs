...
use haneul_json_rpc_api::{CoinReadApiClient, IndexerApiClient, ReadApiClient};
use haneul_json_rpc_types::{
    CoinPage, EventFilter, HaneulObjectDataOptions, HaneulObjectResponse, HaneulObjectResponseQuery,
};
use haneul_swarm_config::genesis_config::DEFAULT_GAS_AMOUNT;
use haneul_test_transaction_builder::publish_package;
use haneul_types::{event::EventID, transaction::CallArg};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let data_option = HaneulObjectDataOptions::new().with_owner();
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    // We should get all the 5 coins for HANEUL with the right balance.
    let result: CoinPage = http_client
