...
mod tests {
    ...
    fn verify_allowall() {
        ...
        let verifier = ClientCertVerifier::new(AllowAll, HANEUL_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_server_cert() {
        ...
        let verifier =
            ServerCertVerifier::new(allowed_public_key, HANEUL_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_hashset() {
        ...
        let verifier =
            ClientCertVerifier::new(allowlist.clone(), HANEUL_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn invalid_server_name() {
        ...
        let cert = SelfSignedCertificate::new(keypair.private(), "not-haneul");

        let allowlist = AllowPublicKeys::new(BTreeSet::from([public_key.clone()]));
        let client_verifier =
            ClientCertVerifier::new(allowlist.clone(), HANEUL_VALIDATOR_SERVER_NAME.to_string());

        // Allowed public key but the server-name in the cert is not the required "haneul"
        ...
        let server_verifier =
            ServerCertVerifier::new(public_key, HANEUL_VALIDATOR_SERVER_NAME.to_string());
    ...
    async fn axum_acceptor() {
        ...
        let client_certificate =
            SelfSignedCertificate::new(client_keypair.private(), HANEUL_VALIDATOR_SERVER_NAME);
