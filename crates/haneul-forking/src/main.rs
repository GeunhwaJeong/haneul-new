...
use haneul_forking::GraphQLClient;
use haneul_forking::Node;
use haneul_forking::startup;

// Define the `GIT_REVISION` and `VERSION` consts
bin_version::bin_version!();

/// Default bind address for the embedded `haneul-rpc-api` server.
...
async fn main() -> Result<()> {
    ...
    // falls back to `HANEUL_FORKING_RPC_ADDR` then `DEFAULT_RPC_ADDR`. Kept
    ...
    let rpc_addr: SocketAddr = parse_rpc_addr_arg()?
        .or_else(|| std::env::var("HANEUL_FORKING_RPC_ADDR").ok())
    ...
    let handle = tokio::spawn(haneul_forking::startup::run(context, rpc_addr, VERSION));
    handle.await??;
