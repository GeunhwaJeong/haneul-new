...
use haneul_json_rpc_types::{
    ...
    HaneulTransactionData, HaneulTransactionEffects, TransactionBytes, TransactionEffectsResponse,
...
impl RpcExampleProvider {
    ...
    fn get_object_example(&mut self) -> Examples {
        ...
        let result = HaneulObjectRead::Exists(HaneulObject {
            data: HaneulData::MoveObject(
                HaneulParsedMoveObject::try_from_layout(coin.to_object(), GasCoin::layout()).unwrap(),
            ),
    ...
    fn get_transactions_by_input_object(&mut self) -> Examples {
        ...
        Examples::new(
            "haneul_getTransactionsByInputObject",
            vec![ExamplePairing::new(
    ...
    fn get_transactions_by_move_function(&mut self) -> Examples {
        ...
        Examples::new(
            "haneul_getTransactionsByMoveFunction",
            vec![ExamplePairing::new(
                "Return the transaction digest for specified input object",
                vec![
                    ("package", json!(HANEUL_FRAMEWORK_OBJECT_ID)),
    ...
    fn get_transactions_by_mutated_object(&mut self) -> Examples {
        let result = self.get_transaction_digests(5..8);
        Examples::new(
            "haneul_getTransactionsByMutatedObject",
            vec![ExamplePairing::new(
    ...
    fn get_transactions_from_address(&mut self) -> Examples {
        let result = self.get_transaction_digests(5..8);
        Examples::new(
            "haneul_getTransactionsFromAddress",
            vec![ExamplePairing::new(
                "Return the transaction digest for specified sender address",
                vec![(
                    "addr",
                    json!(HaneulAddress::from(ObjectID::new(self.rng.gen()))),
                )],
                json!(result),
            )],
        )
    ...
    fn get_transactions_to_address(&mut self) -> Examples {
        let result = self.get_transaction_digests(5..8);
        Examples::new(
            "haneul_getTransactionsToAddress",
            vec![ExamplePairing::new(
