...
use crate::types::{digest::Digest, haneul_address::HaneulAddress, type_filter::FqNameFilter};
...
impl TransactionBlockFilter {
    ...
    pub(crate) fn explicit_sender(&self) -> Option<HaneulAddress> {
            ...
            || matches!(
                ...
                (Some(kind), Some(signer))
                    if (kind == TransactionBlockKindInput::SystemTx)
                        != (signer == HaneulAddress::from(NativeHaneulAddress::ZERO))
            )
    }
}
