...
pub struct IndexerApi<R> {
    ...
    read_api: R,
    ns_package_addr: Option<HaneulAddress>,
    ...
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<HaneulAddress>> {
        with_tracing!(async move {
            let pkg_addr = match self.ns_package_addr {
                Some(addr) => addr,
                None => HaneulAddress::from_str(NAME_SERVICE_DEFAULT_PACKAGE_ADDRESS)?,
            };
            ...
            let domain_bcs_value = bcs::to_bytes(&domain).map_err(|e| {
                Error::HaneulRpcInputError(HaneulRpcInputError::GenericInvalid(format!(
            ...
            if let Some(record_object_id) = record_object_id_option {
                ...
                let addr = match &address_str_move_value {
                    HaneulMoveValue::Option(boxed_addr) => match **boxed_addr {
                        Some(HaneulMoveValue::Address(ref addr)) => Ok(*addr),
                        _ => Err(Error::UnexpectedError(format!(
                            "No HaneulAddress found in option: {:?}",
    ...
    async fn resolve_name_service_names(
        ...
        with_tracing!(async move {
            ...
            let addr_parsed_move_object =
                HaneulParsedMoveObject::try_from_object_read(addr_object_read)?;
            ...
            let primary_domain = match labels_move_value {
                HaneulMoveValue::Vector(labels) => {
                    let label_strs: Vec<String> = labels
                        .iter()
                        .rev()
                        .filter_map(|label| match label {
                            HaneulMoveValue::String(label_str) => Some(label_str.clone()),
                            _ => None,
                        })
                        .collect();
                    Ok(label_strs.join("."))
