...
use haneul_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, HaneulMoveValue,
    HaneulObjectDataOptions, HaneulObjectResponse, HaneulObjectResponseQuery, HaneulParsedMoveObject,
    HaneulTransactionBlockResponse, HaneulTransactionBlockResponseQuery, TransactionBlocksPage,
};
use haneul_open_rpc::Module;
use haneul_types::base_types::{ObjectID, HaneulAddress, STD_UTF8_MODULE_NAME, STD_UTF8_STRUCT_NAME};
...
impl<R: ReadApiServer> IndexerApiServer for IndexerApi<R> {
    ...
    fn query_transaction_blocks(
        ...
        let data: Vec<HaneulTransactionBlockResponse> = if opts.only_digest() {
            digests
                .into_iter()
    ...
    fn get_dynamic_field_object(
        ...
        let haneul_json_value = HaneulJsonValue::new(value)?;
    ...
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<HaneulAddress> {
        ...
        let record_parsed_move_object =
            HaneulParsedMoveObject::try_from_object_read(record_object_read)?;
        ...
        let addr = match address_str_move_value {
            HaneulMoveValue::Address(addr) => Ok(addr),
            _ => Err(anyhow!(
                "No HaneulAddress found in: {:?}",
    ...
    async fn resolve_name_service_names(
        ...
        let addr_parsed_move_object = HaneulParsedMoveObject::try_from_object_read(addr_object_read)?;
        let address_info_move_value = addr_parsed_move_object
            .read_dynamic_field_value(NAME_SERVICE_VALUE)
            .ok_or_else(|| anyhow!("Cannot find value field in record Move struct"))?;

        let primary_name = match address_info_move_value {
            HaneulMoveValue::String(s) => Ok(s),
            _ => Err(anyhow!(
...
impl<R: ReadApiServer> HaneulRpcModule for IndexerApi<R> {
    ...
    fn get_name_service_dynamic_field_table_object_id(
        ...
        if let Some(resolver_id) = self.ns_resolver_id {
            ...
            let resolved_parsed_move_object =
                HaneulParsedMoveObject::try_from_object_read(resolver_object_read)?;
