...
use haneul_types::traffic_control::RemoteFirewallConfig;
...
use haneul_types::traffic_control::{PolicyConfig, Weight};
use tracing::warn;
    ...
    match name {
        "haneul_executeTransactionBlock" => {
            ...
            let params = if let Some(params) = raw_params {
                ...
                return Err(HaneulError::Unknown(String::from(
                    "Params not found for executeTransactionBlock",
                )));
            };
            let parsed_value: Value = serde_json::from_str(params).map_err(|err| {
                HaneulError::Unknown(format!("Failed to parse jsonrpsee params: {:?}", err))
            })?;
            ...
            let params_str = match parsed_value {
                Value::Array(mut params_vec) => {
                    ...
                    serde_json::to_string(&params_vec).map_err(|err| {
                        HaneulError::Unknown(format!("Failed to serialize params: {:?}", err))
                    })?
                }
            ...
            Ok((
                params_str,
                String::from("haneul_monitoredExecuteTransactionBlock"),
            ))
        }
        "haneul_monitoredExecuteTransactionBlock" => {
            // Prevent an attacker calling it directly with a different
            // client IP in order to bypass monitoring
            Err(HaneulError::InvalidRpcMethodError)
        }
