...
use haneul_core::event_filter::EventFilter;
use haneul_core::event_handler::EventHandler;
use haneul_json_rpc_api::rpc_types::HaneulEvent;
...
impl EventApiServer for EventApiImpl {
    fn subscribe_move_event_by_type(
        ...
        if let Some(sink) = pending.accept() {
            ...
            let stream = stream.map(move |e| HaneulEvent::try_from(e.event, &state.module_cache));
