...
impl GovernanceReadApi {
    ...
    async fn get_stakes(&self, owner: HaneulAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_haneul_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: HaneulSystemStateSummary =
            system_state.clone().into_haneul_system_state_summary();
...
async fn exchange_rates(
...
) -> HaneulResult<Vec<ValidatorExchangeRates>> {
    let system_state = state.database.get_haneul_system_state_object()?;
    ...
    for df in state.get_dynamic_fields(
        ...
        let pool_id: ID =
            bcs::from_bytes(&df.bcs_name).map_err(|e| HaneulError::ObjectDeserializationError {
                error: e.to_string(),
            })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, HaneulError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
