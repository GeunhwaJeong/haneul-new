...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<HaneulTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<HaneulTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(HaneulTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, HaneulTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: HaneulTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(HaneulTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, HaneulTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_raw_transaction() -> Result<(), anyhow::Error> {
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .transfer_object(*address, object_to_transfer, None, 10_000, *address)
        .await?;
    let keystore_path = cluster.swarm.dir().join(HANEUL_KEYSTORE_FILENAME);
    let keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let tx = client
        ...
        .query_transaction_blocks(
            HaneulTransactionBlockResponseQuery::default(),
            None,
            Some(3),
            true,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            HaneulTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
                cluster.accounts[0],
            )),
            None,
            None,
