...
use haneul_json_rpc_types::{
    HaneulObjectDataOptions, HaneulObjectResponseQuery, HaneulTransactionResponse,
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<HaneulTransactionResponse> = Vec::new();
    for obj in &objects[..objects.len() - 1] {
        ...
        let transaction_bytes: TransactionBytes = http_client
            ...
            .await?;
        let keystore_path = cluster.swarm.dir().join(HANEUL_KEYSTORE_FILENAME);
    ...
    // let batch_responses: Vec<HaneulTransactionResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(HaneulTransactionResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, HaneulTransactionResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: HaneulTransactionResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(HaneulTransactionResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, HaneulTransactionResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let tx = client
        .read_api()
        .query_transaction_blocks(HaneulTransactionResponseQuery::default(), None, Some(3), true)
        .await
        .unwrap();
    ...
    let first_page = client
        .read_api()
        .query_transaction_blocks(HaneulTransactionResponseQuery::default(), None, Some(5), false)
        .await
        .unwrap();
    ...
    let latest = client
        .read_api()
        .query_transaction_blocks(HaneulTransactionResponseQuery::default(), None, Some(10), true)
        .await
        .unwrap();
    ...
    let tx = client
        .read_api()
        .query_transaction_blocks(HaneulTransactionResponseQuery::default(), None, Some(20), true)
        .await
        .unwrap();
