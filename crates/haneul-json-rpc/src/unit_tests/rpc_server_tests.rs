...
use haneul_json_rpc_types::{
    Balance, CoinPage, GetObjectDataResponse, HaneulCoinMetadata, HaneulEvent,
    HaneulExecuteTransactionResponse, HaneulExecutionStatus, HaneulTBlsSignObjectCommitmentType,
...
use haneul_types::coin::{TreasuryCap, COIN_MODULE_NAME, LOCKED_COIN_MODULE_NAME};
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: HaneulExecuteTransactionResponse = http_client
        .execute_transaction(
            tx_bytes1,
            signature_bytes,
    ...
    let HaneulExecuteTransactionResponse { effects, .. } = tx_response;
    assert_eq!(
...
async fn test_tbls_sign_randomness_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .tbls_sign_randomness_object(
            gas.object_id,
            HaneulTBlsSignObjectCommitmentType::ConsensusCommitted,
        )
        .await;
    ...
    let randomness_object_id = effects
        ...
        .iter()
        .find_map(|e| {
            if let HaneulEvent::NewObject { object_id, .. } = e {
                Some(*object_id)
            } else {
                None
            }
    ...
    let transaction_bytes = http_client
        .move_call(
            ...
            *package_id,
            ...
            vec![],
            vec![
                HaneulJsonValue::from_object_id(randomness_object_id),
                HaneulJsonValue::from_bcs_bytes(&sig).unwrap(),
            ],
            Some(gas.object_id),
            10_000,
            None,
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, HaneulExecuteTransactionResponse {effects, ..} if effects.effects.created.len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, HaneulExecuteTransactionResponse {effects, ..} if effects.effects.created.len() == 1);
    Ok(())
}
...
async fn test_get_transaction() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<HaneulExecuteTransactionResponse> = Vec::new();
    ...
    for tx_digest in tx {
        ...
        assert!(tx_responses.iter().any(
            |resp| matches!(resp, HaneulExecuteTransactionResponse {effects, ..} if effects.effects.transaction_digest == response.effects.transaction_digest)
        ))
    }
...
#[sim_test]
async fn test_locked_haneul() -> Result<(), anyhow::Error> {
    let cluster = TestClusterBuilder::new().build().await?;
...
async fn test_delegation() -> Result<(), anyhow::Error> {
    ...
    let objects: Vec<HaneulObjectInfo> = http_client.get_objects_owned_by_address(*address).await?;
    assert_eq!(5, objects.len());
...
#[sim_test]
async fn test_delegation_with_locked_haneul() -> Result<(), anyhow::Error> {
    let cluster = TestClusterBuilder::new().build().await?;
    ...
    let transaction_bytes: TransactionBytes = http_client
        .move_call(
            *address,
            ...
            vec![parse_haneul_type_tag("0x2::haneul::HANEUL")?.into()],
            vec![
                HaneulJsonValue::from_str(&objects[0].object_id.to_string())?,
                HaneulJsonValue::from_str(&format!("{address}"))?,
                HaneulJsonValue::from_bcs_bytes(&bcs::to_bytes(&"20")?)?,
            ],
            None,
