...
use haneul_json_rpc_types::HaneulTBlsSignObjectCommitmentType::{ConsensusCommitted, FastPathCommitted};
use haneul_json_rpc_types::{
    HaneulEffectsFinalityInfo, HaneulFinalizedEffects, HaneulTBlsSignObjectCommitmentType,
    HaneulTBlsSignRandomnessObjectResponse, HaneulTransactionEffectsAPI,
};
use haneul_open_rpc::Module;
use haneul_types::base_types::ObjectID;
use haneul_types::crypto::construct_tbls_randomness_object_message;
...
impl ThresholdBlsApi {
    ...
    fn is_randomness_object(layout: &MoveStructLayout) -> bool {
        ...
        let prefix = format!("{:?}::randomness::Randomness", HANEUL_FRAMEWORK_ADDRESS);
    ...
    async fn get_randomness_object(&self, object_id: ObjectID) -> Result<Object, Error> {
        ...
        let ObjectRead::Exists(_obj_ref, obj, layout) = obj_read else {
            Err(Error::HaneulError(UserInputError::ObjectNotFound{ object_id, version: None }.into()))? };
        let Some(layout) = layout else {
            Err(Error::InternalError(anyhow!("Object does not have a layout")))?};
        if !Self::is_randomness_object(&layout) {
            Err(Error::HaneulError(HaneulError::BadObjectType {
    ...
    async fn verify_finalized_effects(
        ...
        finalized_effects: &HaneulFinalizedEffects,
    ) -> Result<(), Error> {
        match &finalized_effects.finality_info {
            HaneulEffectsFinalityInfo::Certified(cert) => {
                ...
                // TODO: convert HaneulTransactionEffects to TransactionEffects before the next line.
                // effects_cert
                //     .auth_sign_info
            ...
            HaneulEffectsFinalityInfo::Checkpointed(_epoch, _checkpoint) => {
...
impl ThresholdBlsApiServer for ThresholdBlsApi {
    ...
    async fn tbls_sign_randomness_object(
        ...
        Ok(HaneulTBlsSignRandomnessObjectResponse { signature })
    }
}
