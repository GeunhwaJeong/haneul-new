            ...
            let cursor = match cursor {
                Some(c) => {
                    let obj = self.internal.get_object(&c).await?.ok_or_else(|| {
                        HaneulRpcInputError::GenericInvalid("cursor not found".to_string())
                    })?;
                    let coin = obj.as_coin_maybe().ok_or_else(|| {
                        HaneulRpcInputError::GenericInvalid("cursor is not a coin".to_string())
                    })?;
                    ...
                    match obj {
                        Some(obj) => {
                            ...
                            if coin_type.is_none() {
                                ...
                                let coin = obj.as_coin_maybe().ok_or_else(|| {
                                    HaneulRpcInputError::GenericInvalid(
                                        "cursor is not a coin".to_string(),
                                    )
                                })?;
                                Ok((
