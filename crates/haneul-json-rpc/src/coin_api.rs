...
use haneul_types::base_types::{MoveObjectType, ObjectID, ObjectRef, HaneulAddress};
use haneul_types::coin::{Coin, CoinMetadata, TreasuryCap};
...
impl CoinReadApi {
    ...
    /// Note: if  we are fetching gas coins, `coin_type_tag` should be `Some(HANEUL)`, not `Some(Coin<HANEUL>)`
    fn multi_get_coin(
        &self,
        coins: &[ObjectRef],
        coin_type_tag: Option<&TypeTag>,
    ) -> Result<Vec<Result<HaneulCoin, Error>>, Error> {
        let o = self
            .state
            .database
            .multi_get_object_by_key(&coins.iter().map(ObjectKey::from).collect::<Vec<_>>())?;
...
impl CoinReadApiServer for CoinReadApi {
    ...
    fn get_balance(&self, owner: HaneulAddress, coin_type: Option<String>) -> RpcResult<Balance> {
        let coin_type = TypeTag::Struct(Box::new(match coin_type {
            Some(c) => parse_haneul_struct_tag(&c)?,
            None => GAS::type_(),
        }));
    ...
    fn get_all_balances(&self, owner: HaneulAddress) -> RpcResult<Vec<Balance>> {
