...
fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, Error> {
    parse_haneul_struct_tag(coin_type).map_err(|e| {
        Error::HaneulRpcInputError(HaneulRpcInputError::CannotParseHaneulStructTag(format!("{e}")))
    })
}
                    ...
                    match obj {
                        Some(obj) => {
                            ...
                            if coin_type.is_none() {
                                Err(Error::HaneulRpcInputError(HaneulRpcInputError::GenericInvalid(
                        ...
                        None => Err(Error::HaneulRpcInputError(HaneulRpcInputError::GenericInvalid(
...
#[async_trait]
pub trait State {
    fn get_object_read(&self, object_id: &ObjectID) -> HaneulResult<ObjectRead>;
    async fn get_object(&self, object_id: &ObjectID) -> HaneulResult<Option<Object>>;
    fn find_publish_txn_digest(&self, package_id: ObjectID) -> HaneulResult<TransactionDigest>;
    fn get_owned_coins(
        &self,
        owner: HaneulAddress,
    ...
    ) -> HaneulResult<Vec<HaneulCoin>>;
    async fn get_executed_transaction_and_effects(
        &self,
        digest: TransactionDigest,
    ) -> HaneulResult<(VerifiedTransaction, TransactionEffects)>;
    async fn get_balance(&self, owner: HaneulAddress, coin_type: TypeTag) -> HaneulResult<TotalBalance>;
...
async fn find_package_object_id(
    ...
    spawn_monitored_task!(async move {
        ...
        Err(HaneulRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    fn get_test_owner() -> HaneulAddress {
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_haneul_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_haneul_struct_tag(&coin_name).expect("should not fail");
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_haneul_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockState::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::haneul:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
        ...
        async fn test_get_coins_iterator_index_store_not_available() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| Err(HaneulError::IndexStoreNotAvailable));
            let internal = CoinReadInternalImpl {
                state: Arc::new(mock_state),
                metrics: Arc::new(JsonRpcMetrics::new_for_tests()),
            };
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_index_store_not_available() {
            ...
            mock_state
                .expect_get_balance()
                .returning(move |_, _| Err(HaneulError::IndexStoreNotAvailable));
            let internal = CoinReadInternalImpl {
                state: Arc::new(mock_state),
                metrics: Arc::new(JsonRpcMetrics::new_for_tests()),
            };
        ...
        async fn test_get_balance_execution_error() {
            ...
            mock_state
                .expect_get_balance()
                .returning(move |_, _| Err(HaneulError::ExecutionError("mock db error".to_string())));
            let internal = CoinReadInternalImpl {
                state: Arc::new(mock_state),
                metrics: Arc::new(JsonRpcMetrics::new_for_tests()),
            };
    ...
    mod get_all_balances_tests {
        ...
        async fn test_index_store_not_available() {
            ...
            mock_state
                .expect_get_all_balance()
                .returning(move |_| Err(HaneulError::IndexStoreNotAvailable));
            let internal = CoinReadInternalImpl {
                state: Arc::new(mock_state),
                metrics: Arc::new(JsonRpcMetrics::new_for_tests()),
            };
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = HaneulCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_haneul_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_haneul_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
    ...
    mod get_total_supply_tests {
        ...
        async fn test_success_response_for_gas_coin() {
            let coin_type = "0x2::haneul::HANEUL";
            let mock_internal = MockCoinReadInternal::new();
            let coin_read_api = CoinReadApi {
                internal: Box::new(mock_internal),
            };
