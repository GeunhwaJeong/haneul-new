...
use haneul_json_rpc_api::{cap_page_limit, CoinReadApiOpenRpc, CoinReadApiServer, JsonRpcMetrics};
...
use haneul_types::coin_registry::{self, Currency, CurrencyKey, SupplyState};
            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        HaneulRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;
...
mod tests {
    ...
    fn create_currency_object(currency: Currency) -> Object {
        use haneul_types::object::{Data, ObjectInner};
    ...
    mock! {
        pub ObjectStore {}
        impl haneul_types::storage::ObjectStore for ObjectStore {
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_coin_metadata_with_registry_data() {
            ...
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let coin_struct = parse_haneul_struct_tag(&coin_name).expect("should not fail");
            ...
            let currency_id = derived_object::derive_object_id(
                haneul_types::HANEUL_COIN_REGISTRY_OBJECT_ID,
                &TypeTag::Struct(Box::new(currency_key_type)),
                &currency_key_bytes,
            )
            .unwrap();
