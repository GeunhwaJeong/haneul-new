...
static MAX_DISPLAY_FIELD_DEPTH: Lazy<usize> = Lazy::new(|| {
    ...
    if let Some(max) = max_opt {
        ...
        haneul_display::v2::Limits::default().max_depth
    }
});
...
static MAX_DISPLAY_FORMAT_NODES: Lazy<usize> = Lazy::new(|| {
    ...
    if let Some(max) = max_opt {
        ...
        haneul_display::v2::Limits::default().max_nodes
    }
});
...
static MAX_DISPLAY_OBJECT_LOADS: Lazy<usize> = Lazy::new(|| {
    ...
    if let Some(max) = max_opt {
        ...
        haneul_display::v2::Limits::default().max_loads
    }
});
...
#[async_trait]
impl haneul_display::v2::Store for DisplayStore<'_> {
    async fn object(
        &self,
        id: AccountAddress,
    ) -> anyhow::Result<Option<haneul_display::v2::OwnedSlice>> {
        ...
        Ok(Some(haneul_display::v2::OwnedSlice {
...
async fn get_display_fields(
    ...
    let display: Vec<(String, Result<Json, anyhow::Error>)> =
        if let Some(display_object) = get_display_object_v2_by_type(fullnode_api, type_)? {
            let root = haneul_display::v2::OwnedSlice {
    ...
    Ok(DisplayFieldsResponse {
        ...
        error: (!errors.is_empty()).then(|| HaneulObjectResponseError::DisplayError {
            error: errors.join("; "),
        }),
    })
}
