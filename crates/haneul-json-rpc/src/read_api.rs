...
use haneul_types::error::{HaneulError, HaneulObjectResponseError, HaneulResult};
...
fn convert_to_response(
    ...
    if opts.show_input && cache.transaction.is_some() {
        let tx_block =
            HaneulTransactionBlock::try_from(cache.transaction.unwrap().into_data(), module_cache)?;
