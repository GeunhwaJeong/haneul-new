...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
    ...
    ) -> Result<Vec<HaneulTransactionBlockResponse>, Error> {
        trace!("start");

        let num_digests = digests.len();
        if num_digests > *QUERY_MAX_RESULT_LIMIT {
            Err(HaneulRpcInputError::SizeLimitExceeded(
        ...
        if temp_response.len() < num_digests {
            Err(HaneulRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    match events.next() {
                        Some(Some(ev)) => {
                            cache_entry.events =
                                Some(to_haneul_transaction_events(self, cache_entry.digest, ev)?)
                        }
                        None | Some(None) => {
...
impl ReadApiServer for ReadApi {
    ...
    async fn multi_get_objects(
        ...
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<HaneulObjectResponse>, String> = results
                    .into_iter()
        ...
        with_tracing!(async move {
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(HaneulPastObjectResponse::ObjectNotExists(id))
                }
            ...
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to HaneulPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
        ...
        with_tracing!(async move {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                let events = spawn_monitored_task!(async move {
                    ...
                    .flatten();
                match events {
                    None => temp_response.events = Some(HaneulTransactionBlockEvents::default()),
                    Some(events) => match to_haneul_transaction_events(self, digest, events) {
            ...
            Ok(match events {
                Some(events) => events
                    ...
                    .map(|(seq, e)| {
                        ...
                        HaneulEvent::try_from(e, transaction_digest, seq as u64, None, layout)
                    })
                    .collect::<Result<Vec<_>, _>>()
        ...
        with_tracing!(async move {
            version
                .map(|v| {
                    ...
                    .ok_or(HaneulRpcInputError::ProtocolVersionUnsupported(
    ...
    async fn verify_zklogin_signature(
        ...
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(HaneulRpcInputError::GenericNotFound(
            ...
            ZkLoginIntentScope::TransactionData => {
                let tx_data: TransactionData =
                    bcs::from_bytes(&Base64::decode(&bytes).map_err(Error::from)?)
                        .map_err(Error::from)?;
                let intent_msg = IntentMessage::new(Intent::haneul_transaction(), tx_data.clone());
...
pub enum ObjectDisplayError {
    ...
    #[error(transparent)]
    Deserialization(#[from] HaneulError),
...
pub fn get_rendered_fields(
    ...
    let haneul_move_value: HaneulMoveValue = MoveValue::Struct(move_struct.clone()).into();
    if let HaneulMoveValue::Struct(move_struct) = haneul_move_value {
        let fields =
            fields
                .contents
                .iter()
...
fn parse_template(template: &str, move_struct: &HaneulMoveStruct) -> Result<String, Error> {
...
fn get_value_from_move_struct(
    ...
    let mut current_value = &HaneulMoveValue::Struct(move_struct.clone());
    // iterate over the parts and try to access the corresponding field
    for part in parts {
        match current_value {
            HaneulMoveValue::Struct(move_struct) => {
                if let HaneulMoveStruct::WithTypes { type_: _, fields }
                | HaneulMoveStruct::WithFields(fields) = move_struct
                {
    ...
    match current_value {
        HaneulMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        HaneulMoveValue::Vector(_) => Err(anyhow!(
...
fn convert_to_response(
...
) -> RpcInterimResult<HaneulTransactionBlockResponse> {
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(HaneulTransactionBlock::try_from(
