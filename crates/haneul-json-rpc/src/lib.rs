...
impl JsonRpcServerBuilder {
    ...
    pub fn new_without_metrics_for_testing(use_websocket: bool) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            module,
            server_builder,
            rpc_doc: haneul_rpc_doc("0.0.0"),
        })
    }

    pub fn register_module<T: HaneulRpcModule>(&mut self, module: T) -> Result<(), anyhow::Error> {
    ...
    pub async fn start(
        ...
        info!(local_addr =? addr, "Haneul {server_name} server listening on {addr}");
