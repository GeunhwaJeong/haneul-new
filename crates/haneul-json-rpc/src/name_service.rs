...
const DEFAULT_TLD: &str = "haneul";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const HANEUL_NEW_FORMAT_SEPARATOR: char = '@';

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.haneul`
...
pub struct Registry {
    ...
    reverse_registry: Table<HaneulAddress, Domain>,
}

/// Rust version of the Move haneul::table::Table type.
...
impl Domain {
    pub fn type_(package_address: HaneulAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.haneul` -> `example.haneul`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), HANEUL_NEW_FORMAT_SEPARATOR, sld)
    }
}
...
pub struct NameServiceConfig {
    pub package_address: HaneulAddress,
...
impl Default for NameServiceConfig {
    fn default() -> Self {
        let package_address = HaneulAddress::from_str(NAME_SERVICE_DEFAULT_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.haneul` -> `.` | example*haneul -> `@` | `example*haneul` -> `*`
...
/// Converts @label ending to label{separator}haneul ending.
///
/// E.g. `@example` -> `example.haneul` | `test@example` -> `test.example.haneul`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(HANEUL_NEW_FORMAT_SEPARATOR);
...
pub struct NameRecord {
    ...
    pub target_address: Option<HaneulAddress>,
...
/// A HaneulnsRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct HaneulnsRegistration {
    pub id: UID,
    pub domain: Domain,
