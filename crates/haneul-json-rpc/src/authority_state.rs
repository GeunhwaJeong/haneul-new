...
pub trait StateRead: Send + Sync {
    ...
    async fn query_events(
        ...
        limit: usize,
        descending: bool,
    ) -> StateReadResult<Vec<HaneulEvent>>;
    ...
    ) -> StateReadResult<Vec<HaneulCoin>>;
...
impl StateRead for AuthorityState {
    ...
    fn get_owned_coins(
        ...
        fn to_haneul_coin(key: CoinIndexKey2, info: CoinInfo) -> HaneulCoin {
            HaneulCoin {
        ...
        let fake_coins: HashMap<String, HaneulCoin> = if !coin_reservations_enabled {
            HashMap::new()
        } else if one_coin_type_only {
            let balance_type_tag = haneul_types::parse_haneul_type_tag(&cursor.0)
            ...
            self.get_address_balance_coin_info(owner, balance_type)?
                .map(|(obj_ref, balance, prev_tx)| {
                    HashMap::from([(
                        cursor.0.clone(),
                        obj_ref_to_haneul_coin(cursor.0.clone(), obj_ref, balance, prev_tx),
                    )])
                })
                .unwrap_or_default()
        } else {
            self.get_all_address_balance_coin_infos(owner)?
                ...
                .map(|(coin_type, (obj_ref, balance, prev_tx))| {
                    (
                        coin_type.clone(),
                        obj_ref_to_haneul_coin(coin_type, obj_ref, balance, prev_tx),
                    )
                })
                .collect()
        };
        ...
        for (key, info) in real_coins_iter {
            ...
            let coin = to_haneul_coin(key, info);
    ...
    async fn get_balance(
        ...
        Ok(
            tokio::task::spawn_blocking(move || -> HaneulResult<TotalBalance> {
                let address_balance =
    ...
    async fn get_all_balance(
        ...
        Ok(tokio::task::spawn_blocking(
            move || -> HaneulResult<Arc<HashMap<TypeTag, TotalBalance>>> {
                let indexes = indexes
                    .as_ref()
                    .ok_or(HaneulErrorKind::IndexStoreNotAvailable)?;
...
pub enum StateReadError {
    // haneul_json_rpc::Error will do the final conversion to generic error message
    #[error(transparent)]
