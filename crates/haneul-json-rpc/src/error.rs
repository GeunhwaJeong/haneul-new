...
impl From<Error> for RpcError {
    ...
    fn from(e: Error) -> RpcError {
        match e {
            ...
            Error::HaneulRpcInputError(err) => RpcError::Call(CallError::InvalidParams(err.into())),
                ...
                match err {
                    QuorumDriverError::InvalidUserSignature(err) => {
                        let inner_error_str = match err {
                            // TODO(wlmyng): update HaneulError display trait to render UserInputError with display
                            HaneulError::UserInputError { error } => error.to_string(),
                            _ => err.to_string(),
                        };
...
mod tests {
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error =
                QuorumDriverError::QuorumDriverInternalError(HaneulError::UnexpectedMessage);
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(HaneulError::UnexpectedMessage, 0, vec![])],
            };
