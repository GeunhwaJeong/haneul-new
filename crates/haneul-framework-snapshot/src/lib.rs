...
// [haneul_framework::{SystemPackageMetadata, SystemPackage}] types,
// and also to the [haneul::framework_versions::{FrameworkVersion, FrameworkPackage}] types.
// They are sort of a stepping stone from one to the other - the [haneul_framework] types contain
...
//
// These types serve as a kind of stepping stone; they are constructed from the [haneul_framework]
// types and serialized in the manifest, and then the build script for the [haneul] crate reads them
// from the manifest file and encodes them in the `haneul` binary. A little information is dropped in
// each of these steps.
...
pub struct SnapshotPackage {
    ...
    pub name: String,
    /// Path to the package in the monorepo (e.g. "crates/haneul-framework/packages/move-stdlib")
    pub path: String,
