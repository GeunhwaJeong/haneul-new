...
use haneul_types::{Identifier, BRIDGE_PACKAGE_ID};
...
fn build_token_bridge_approve_transaction(
    ...
    builder.programmable_move_call(
        BRIDGE_PACKAGE_ID,
        haneul_types::bridge::BRIDGE_MODULE_NAME.to_owned(),
...
pub fn build_add_tokens_on_haneul_transaction(
    client_address: HaneulAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_HANEUL_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
mod tests {
    ...
    async fn test_build_haneul_transaction_for_token_transfer() {
        ...
        let haneul_client = HaneulClient::new(&test_cluster.fullnode_handle.rpc_url, metrics)
            .await
            .unwrap();
        let bridge_authority_keys = test_cluster.bridge_authority_keys.take().unwrap();

        // Note: We don't call `haneul_client.get_bridge_committee` here because it will err if the committee
        ...
        // 2. Test Haneul -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_haneul_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let num_valdiator = 2;
        let mut bridge_keys = vec![];
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_haneul_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = haneul_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_haneul_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let transfer_limit = haneul_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        .await;
        let new_transfer_limit = haneul_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::HaneulCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_haneul_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = haneul_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = haneul_client.get_notional_values().await.unwrap();
