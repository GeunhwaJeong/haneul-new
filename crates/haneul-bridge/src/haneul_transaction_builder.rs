...
// TODO: once we have bridge package on haneul framework, we can hardcode the actual package id.
...
// TODO: this should be hardcoded once we have bridge package on haneul framework.
...
pub fn get_haneul_token_type_tag(token_id: TokenId) -> TypeTag {
    ...
    let type_tags = TYPE_TAGS.get_or_init(|| {
        ...
        type_tags.insert(TokenId::Haneul, GAS::type_tag());
        type_tags.insert(
            TokenId::BTC,
...
pub fn build_transaction(
    ...
    match action.data() {
        BridgeAction::EthToHaneulBridgeAction(_) | BridgeAction::HaneulToEthBridgeAction(_) => {
...
fn build_token_bridge_approve_transaction(
    ...
    builder.programmable_move_call(
        *get_bridge_package_id(),
        ...
        vec![get_haneul_token_type_tag(token_type)],
