...
impl TryFrom<HaneulToEthBridgeAction> for eth_haneul_bridge::Message {
    type Error = BridgeError;
...
impl TryFrom<EmergencyAction> for eth_haneul_bridge::Message {
    type Error = BridgeError;
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_haneul_bridge::Message = action.try_into().unwrap();
        assert_eq!(
            message,
            eth_haneul_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
