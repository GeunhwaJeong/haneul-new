...
#[rustfmt::skip]
gen_eth_events!(
    EthHaneulBridge, EthHaneulBridgeEvents, "abi/haneul_bridge.json",
    ...
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthHaneulBridgeEvents(event) => {
                match event {
                    EthHaneulBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToHaneulTokenBridgeV1::try_from(&event) {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToHaneulTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };
                    ...
                    EthHaneulBridgeEvents::UpgradedFilter(_event) => None,
                    EthHaneulBridgeEvents::InitializedFilter(_event) => None,
                    EthHaneulBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthHaneulBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
impl TryFrom<&TokensDepositedFilter> for EthToHaneulTokenBridgeV1 {
...
impl From<HaneulToEthBridgeAction> for eth_haneul_bridge::Message {
    fn from(action: HaneulToEthBridgeAction) -> Self {
        eth_haneul_bridge::Message {
...
impl From<ParsedTokenTransferMessage> for eth_haneul_bridge::Message {
...
impl From<EmergencyAction> for eth_haneul_bridge::Message {
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_haneul_bridge::Message = action.into();
        assert_eq!(
            message,
            eth_haneul_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_token_deposit_eth_log_to_haneul_bridge_event_regression() -> anyhow::Result<()> {
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    haneul_adjusted_amount: 4200000000,
    ...
    fn test_0_haneul_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::HaneulTestnet as u8,
                token_id: 2,
                haneul_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    HaneulAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(e
