                ...
                match event {
                    EthHaneulBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToHaneulTokenBridgeV1::try_from(&event) {
                            ...
                            Err(e) => {
                                tracing::error!(?eth_tx_hash, eth_event_index, "Failed to convert TokensDepositedFilter log to EthToHaneulTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e);
                                return None;
                            }
                        };
