...
gen_eth_events!(
    ...
    EthHaneulBridge, eth_haneul_bridge, EthHaneulBridgeEvents, "abi/haneul_bridge.json",
    ...
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthHaneulBridgeEvents(event) => {
                match event {
                    EthHaneulBridgeEvents::TokensDeposited(event) => {
                        let bridge_event = match EthToHaneulTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.haneul_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!(
                                    "Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToHaneulTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}",
                                    event, e
                                )));
                            }
                        };
                    ...
                    EthHaneulBridgeEvents::TokensClaimed(_event) => None,
                    EthHaneulBridgeEvents::Paused(_event) => None,
                    EthHaneulBridgeEvents::Unpaused(_event) => None,
                    EthHaneulBridgeEvents::Upgraded(_event) => None,
                    EthHaneulBridgeEvents::Initialized(_event) => None,
                    EthHaneulBridgeEvents::ContractUpgraded(_event) => None,
                    EthHaneulBridgeEvents::EmergencyOperation(_event) => None,
                }
            }
...
impl TryFrom<&TokensDeposited> for EthToHaneulTokenBridgeV1 {
    type Error = BridgeError;

    fn try_from(event: &TokensDeposited) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            haneul_chain_id: BridgeChainId::try_from(event.destinationChainID)?,
            eth_chain_id: BridgeChainId::try_from(event.sourceChainID)?,
            haneul_address: HaneulAddress::from_bytes(event.recipientAddress.as_ref())?,
            eth_address: event.senderAddress,
            token_id: event.tokenID,
            haneul_adjusted_amount: event.haneulAdjustedAmount,
        })
    }
}
...
impl TryFrom<&TokensDepositedV2> for EthToHaneulTokenBridgeV2 {
    type Error = BridgeError;
...
impl TryFrom<HaneulToEthBridgeAction> for eth_haneul_bridge::BridgeUtils::Message {
    type Error = BridgeError;

    fn try_from(action: HaneulToEthBridgeAction) -> BridgeResult<Self> {
        Ok(eth_haneul_bridge::BridgeUtils::Message {
            ...
            chainID: action.haneul_bridge_event.haneul_chain_id as u8,
            payload: action
                .as_payload_bytes()
...
impl TryFrom<HaneulToEthTokenTransfer> for eth_haneul_bridge::BridgeUtils::Message {
    type Error = BridgeError;
    ...
    fn try_from(action: HaneulToEthTokenTransfer) -> BridgeResult<Self> {
        Ok(eth_haneul_bridge::BridgeUtils::Message {
            ...
            version: TOKEN_TRANSFER_MESSAGE_VERSION_V1,
            nonce: action.nonce,
            chainID: action.haneul_chain_id as u8,
            payload: action
                .as_payload_bytes()
                .map_err(|e| BridgeError::Generic(format!("Failed to encode payload: {}", e)))?
                .into(),
...
impl TryFrom<HaneulToEthTokenTransferV2> for eth_haneul_bridge::BridgeUtils::Message {
    type Error = BridgeError;
...
impl From<ParsedTokenTransferMessage> for eth_haneul_bridge::BridgeUtils::Message {
    fn from(parsed_message: ParsedTokenTransferMessage) -> Self {
        eth_haneul_bridge::BridgeUtils::Message {
            messageType: BridgeActionType::TokenTransfer as u8,
...
impl TryFrom<EmergencyAction> for eth_haneul_bridge::BridgeUtils::Message {
    type Error = BridgeError;
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_haneul_bridge::BridgeUtils::Message = action.try_into().unwrap();
        assert_eq!(
            message,
            eth_haneul_bridge::BridgeUtils::Message {
                messageType: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::HaneulTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_haneul_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDeposited(
                TokensDeposited {
                    sourceChainID: 12,
                    nonce: 0,
                    destinationChainID: 2,
                    tokenID: 2,
                    haneulAdjustedAmount: 4200000000,
    ...
    #[test]
    fn test_0_haneul_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDeposited(
            TokensDeposited {
                sourceChainID: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destinationChainID: BridgeChainId::HaneulTestnet as u8,
                tokenID: 2,
                haneulAdjustedAmount: 1,
                senderAddress: EthAddress::random(),
                recipientAddress: Bytes::from(HaneulAddress::random_for_testing_only().to_vec()),
            },
        ));
        assert!(
