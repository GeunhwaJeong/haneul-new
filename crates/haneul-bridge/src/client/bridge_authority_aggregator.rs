...
use haneul_authority_aggregation::{quorum_map_then_reduce_with_timeout_and_prefs, SigRequestPrefs};
...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: haneul_types::message_envelope::Envelope<
                BridgeAction,
...
mod tests {
    ...
    use haneul_types::committee::VALIDITY_THRESHOLD;
    ...
    async fn test_bridge_auth_agg_ok() {
        ...
        mock1.add_haneul_event_response(
            haneul_tx_digest,
            haneul_tx_event_index,
            Ok(sign_action_with_key(&action, &secrets[1])),
            None,
        );
        mock2.add_haneul_event_response(
            haneul_tx_digest,
            haneul_tx_event_index,
            Ok(sign_action_with_key(&action, &secrets[2])),
            None,
        );
        mock3.add_haneul_event_response(
            haneul_tx_digest,
            haneul_tx_event_index,
