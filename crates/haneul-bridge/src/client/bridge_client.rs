...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::HaneulToEthBridgeAction(e) => format!(
            ...
            BridgeAction::AddTokensOnHaneulAction(a) => {
                let chain_id = (a.chain_id as u8).to_string();
                let nonce = a.nonce.to_string();
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_haneul/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_haneul_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    async fn test_bridge_client() {
        ...
        // Ok
        authority.base_url = "https://foo.haneulbridge.io".to_string();
        let committee = Arc::new(BridgeCommittee::new(vec![authority.clone()]).unwrap());
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_haneul_event_response(tx_digest, event_idx, Ok(signed_event.clone()));
        ...
        let action2 = get_test_haneul_to_eth_bridge_action(
            Some(tx_digest),
            Some(event_idx),
            Some(2),
            Some(200),
        ...
        mock_handler.add_haneul_event_response(tx_digest, event_idx, Ok(wrong_signed_action));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_haneul_event_response(tx_digest, event_idx, Ok(signed_event));
        ...
        mock_handler.add_haneul_event_response(tx_digest, event_idx, Ok(signed_event2));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_haneul_event_response(tx_digest, event_idx, Ok(signed_event3));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let haneul_tx_event_index = 5;
        let action = BridgeAction::HaneulToEthBridgeAction(crate::types::HaneulToEthBridgeAction {
            haneul_tx_digest,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/haneul/eth/{}/{}",
                haneul_tx_digest, haneul_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToHaneulBridgeAction(crate::types::EthToHaneulBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event: EthToHaneulTokenBridgeV1 {
                eth_chain_id: BridgeChainId::EthSepolia,
                nonce: 1,
                eth_address: EthAddress::random(),
                haneul_chain_id: BridgeChainId::HaneulCustom,
        ...
        let action = BridgeAction::AddTokensOnHaneulAction(crate::types::AddTokensOnHaneulAction {
            nonce: 3,
            chain_id: BridgeChainId::HaneulCustom,
            native: false,
