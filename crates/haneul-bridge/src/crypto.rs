...
mod tests {
    ...
    use crate::test_utils::{get_test_authority_and_key, get_test_haneul_to_eth_bridge_action};
    use crate::types::SignedBridgeAction;
    use crate::types::{
        BridgeAction, BridgeAuthority, BridgeChainId, HaneulToEthBridgeAction, TokenId,
    };
    ...
    fn test_sign_and_verify_bridge_event_basic() -> anyhow::Result<()> {
        ...
        let action: BridgeAction =
            get_test_haneul_to_eth_bridge_action(None, Some(1), Some(1), Some(100));
        ...
        let mismatched_action: BridgeAction =
            get_test_haneul_to_eth_bridge_action(None, Some(2), Some(3), Some(4));
        ...
        let action2: BridgeAction =
            get_test_haneul_to_eth_bridge_action(None, Some(3), Some(5), Some(77));
    ...
    fn test_bridge_sig_verification_regression_test() {
        ...
        let action = BridgeAction::HaneulToEthBridgeAction(HaneulToEthBridgeAction {
            haneul_tx_digest: TransactionDigest::random(),
            haneul_tx_event_index: 0,
            haneul_bridge_event: EmittedHaneulToEthTokenBridgeV1 {
                nonce: 1,
                haneul_chain_id: BridgeChainId::HaneulTestnet,
                haneul_address: HaneulAddress::from_str(
