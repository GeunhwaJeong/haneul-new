...
impl EthSyncer {
    pub fn new(eth_client: Arc<EthClient>, contract_addresses: EthTargetAddresses) -> Self {
        Self {
            eth_client,
            contract_addresses,
        }
    ...
    pub async fn run(
        ...
        let (eth_evnets_tx, eth_events_rx) = haneullabs_metrics::metered_channel::channel(
    ...
    async fn run_event_listening_task(
        ...
        events_sender: haneullabs_metrics::metered_channel::Sender<(EthAddress, u64, Vec<EthLog>)>,
        eth_client: Arc<EthClient>,
        metrics: Arc<BridgeMetrics>,
    ) {
...
mod tests {
    ...
    async fn test_last_finalized_block() -> anyhow::Result<()> {
        ...
        let addresses = HashMap::from_iter(vec![(EthAddress::default(), 100)]);
        let log = Log {
        ...
        assert_eq!(contract_address, EthAddress::default());
    ...
    async fn test_multiple_addresses() -> anyhow::Result<()> {
        ...
        let addresses =
            HashMap::from_iter(vec![(EthAddress::default(), 100), (another_address, 200)]);
        ...
        let eth_log1 = EthLog {
            block_number: log1.block_number.unwrap(),
        ...
        let eth_log2 = EthLog {
            block_number: log2.block_number.unwrap(),
    ...
    async fn test_paginated_eth_log_query() -> anyhow::Result<()> {
        ...
        let addresses = HashMap::from_iter(vec![(EthAddress::default(), start_block)]);
        let log = Log {
            transaction_hash: Some(TxHash::random()),
            block_number: Some(start_block),
            log_index: Some(3),
            ..Default::default()
        };
        let log2 = Log {
            transaction_hash: Some(TxHash::random()),
            block_number: Some(last_finalized_block),
            log_index: Some(3),
            ..Default::default()
        };
        let eth_log = EthLog {
