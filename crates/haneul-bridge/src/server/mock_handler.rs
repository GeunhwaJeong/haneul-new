...
pub struct BridgeRequestMockHandler {
    ...
    haneul_token_events: Arc<
        Mutex<
            HashMap<(TransactionDigest, u16), (BridgeResult<SignedBridgeAction>, Option<Duration>)>,
        >,
    >,
    haneul_token_events_requested: Arc<Mutex<HashMap<(TransactionDigest, u16), u64>>>,
}
...
impl BridgeRequestMockHandler {
    pub fn new() -> Self {
        Self {
            ...
            haneul_token_events: Arc::new(Mutex::new(HashMap::new())),
            haneul_token_events_requested: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    pub fn add_haneul_event_response(
        &self,
        ...
        self.haneul_token_events
            .lock()
            .unwrap()
            .insert((tx_digest, idx), (response, delay));
    }

    pub fn get_haneul_token_events_requested(
        &self,
        tx_digest: TransactionDigest,
        event_index: u16,
    ) -> u64 {
        *self
            .haneul_token_events_requested
            .lock()
            .unwrap()
...
impl BridgeRequestHandlerTrait for BridgeRequestMockHandler {
    ...
    async fn handle_haneul_tx_digest(
        ...
        let (result, delay) = {
            let preset = self.haneul_token_events.lock().unwrap();
            if !preset.contains_key(&(tx_digest, event_idx)) {
                // Ok to panic in test
                panic!(
                    "No preset handle_haneul_tx_digest result for tx_digest: {}, event_idx: {}",
                    tx_digest, event_idx
                );
            }
            let mut requested = self.haneul_token_events_requested.lock().unwrap();
        ...
        let signed_action: haneul_types::message_envelope::Envelope<
