...
pub struct BridgeRequestHandler {
    haneul_signer: SignerWithCache<(TransactionDigest, u16)>,
...
impl BridgeRequestHandler {
    ...
    >(
        ...
        let haneul_signer = SignerWithCache::new(
            signer.clone(),
...
impl BridgeRequestHandlerTrait for BridgeRequestHandler {
    ...
    async fn handle_haneul_tx_digest(
        ...
        let signed_action = self.haneul_signer.sign((tx_digest, event_idx)).await?;
        Ok(Json(signed_action))
    }
...
mod tests {
    ...
    async fn test_haneul_signer_with_cache() {
        ...
        let haneul_signer_with_cache = SignerWithCache::new(signer.clone(), haneul_verifier, metrics);
        ...
        let entry = haneul_signer_with_cache.get_cache_entry((haneul_tx_digest, haneul_event_idx));
        let entry_ = haneul_signer_with_cache.get_testing_only((haneul_tx_digest, haneul_event_idx));
