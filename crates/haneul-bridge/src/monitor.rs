...
//! `BridgeMonitor` receives all `HaneulBridgeEvent` and `EthBridgeEvent`
//! and handles them accordingly.

use crate::abi::{
    EthBridgeCommitteeEvents, EthBridgeConfigEvents, EthBridgeEvent, EthBridgeLimiterEvents,
    EthCommitteeUpgradeableContractEvents, EthHaneulBridgeEvents,
};
...
pub struct BridgeMonitor<C> {
    ...
    haneul_monitor_rx: haneullabs_metrics::metered_channel::Receiver<HaneulBridgeEvent>,
    eth_monitor_rx: haneullabs_metrics::metered_channel::Receiver<EthBridgeEvent>,
    ...
    C: HaneulClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*haneul_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                haneul_event = haneul_monitor_rx.recv() => {
                    if let Some(haneul_event) = haneul_event {
                        Self::handle_haneul_events(
                            haneul_event,
                            &haneul_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor haneul events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_haneul_events(
        event: HaneulBridgeEvent,
        haneul_client: &Arc<HaneulClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        haneul_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received HaneulBridgeEvent: {:?}", event);
        macro_rules! bump_haneul_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "haneul"])
                    .inc();
            };
        }
        ...
        match event {
            ...
            HaneulBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            HaneulBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            HaneulBridgeEvent::EmergencyOpEvent(event) => {
                bump_haneul_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            HaneulBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_haneul_counter!("committee_member_registered");
            }
            HaneulBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_haneul_counter!("committee_updated");
            }

            HaneulBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_haneul_counter!("committee_member_url_updated");
            ...
            HaneulBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_haneul_counter!(if event.blocklisted {
            ...
            HaneulBridgeEvent::TokenRegistrationEvent(_) => {
                bump_haneul_counter!("new_token_registered");
            }

            HaneulBridgeEvent::NewTokenEvent(event) => {
                bump_haneul_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    haneul_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            HaneulBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_haneul_counter!("token_price_updated");
            }

            HaneulBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_haneul_counter!("limit_updated");
            }
        }
    }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthHaneulBridgeEvents(event) => match event {
                EthHaneulBridgeEvents::TokensClaimed(_) => (),
                EthHaneulBridgeEvents::TokensDeposited(_) => (),
                EthHaneulBridgeEvents::TokensDepositedV2(_) => (),
                EthHaneulBridgeEvents::Paused(_) => bump_eth_counter!("bridge_paused"),
                EthHaneulBridgeEvents::Unpaused(_) => bump_eth_counter!("bridge_unpaused"),
                ...
                EthHaneulBridgeEvents::ContractUpgraded(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthHaneulBridgeEvents::EmergencyOperation(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: HaneulClientInner>(
    haneul_client: Arc<HaneulClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            haneul_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: HaneulClientInner>(
    haneul_client: Arc<HaneulClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: HaneulClientInner>(
    haneul_client: Arc<HaneulClient<C>>,
...
pub async fn subscribe_bridge_events(
    mut client: haneul_rpc::Client,
    sender: haneullabs_metrics::metered_channel::Sender<HaneulBridgeEvent>,
) {
        ...
        while let Some(item) = subscription.next().await {
            ...
            for txn in checkpoint.checkpoint().transactions() {
                ...
                for event in events.data {
                    match HaneulBridgeEvent::try_from_event(&event) {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "error deserializing HaneulBridgeEvent from txn {txn_digest} in checkpoint {ckpt}: {e}"
                            );
                        }
                    }
                }
...
mod tests {
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        haneul_client_mock.set_bridge_committee(summary.clone());
        ...
        haneul_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let haneul_client_mock_clone = haneul_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            haneul_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            haneul_client.clone(),
            event.clone(),
        ...
        haneul_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            haneul_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        haneul_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        haneul_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                haneul_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            haneul_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                haneul_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let haneul_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        haneul_client_mock.set_bridge_committee(new_committee_summary.clone());
        haneul_monitor_tx
            .send(HaneulBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        haneul_client_mock.set_bridge_committee(new_committee_summary.clone());
        haneul_monitor_tx
            .send(HaneulBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        haneul_client_mock.set_is_bridge_paused(event.frozen);
        haneul_monitor_tx
            .send(HaneulBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_haneul_token_type_tags() {
        let (
            haneul_monitor_tx,
            haneul_monitor_rx,
            _eth_monitor_tx,
        ...
        let haneul_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let haneul_token_type_tags_clone = haneul_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                haneul_client.clone(),
                haneul_monitor_rx,
        ...
        haneul_monitor_tx
            .send(HaneulBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in haneul_token_type_tags
        haneul_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
    ...
    fn setup() -> (
        ...
        HaneulMockClient,
        Arc<HaneulClient<HaneulMockClient>>,
