    ...
    pub async fn run(self) {
        ...
        while let Some(events) = monitor_rx.recv().await {
            match events {
                ...
                HaneulBridgeEvent::EmergencyOpEvent(_) => {
                    // TODO
                }
...
mod tests {
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        let (monitor_tx, monitor_rx, haneul_client_mock, haneul_client) = setup();
        ...
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(haneul_client.clone(), monitor_rx, agg.clone()).run(),
        );
    ...
    fn setup() -> (
        ...
        (monitor_tx, monitor_rx, haneul_client_mock, haneul_client)
    }
}
