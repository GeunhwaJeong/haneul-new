...
use haneul_types::{
    base_types::ObjectID, digests::TransactionDigest, Identifier, HANEUL_SYSTEM_PACKAGE_ID,
};
use tokio::{
    task::JoinHandle,
    time::{self, Duration},
    ...
    C: HaneulClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        Vec<JoinHandle<()>>,
        haneullabs_metrics::metered_channel::Receiver<Vec<HaneulEvent>>,
    )> {
    ...
    async fn run_event_listening_task(
        ...
        events_sender: haneullabs_metrics::metered_channel::Sender<Vec<HaneulEvent>>,
        haneul_client: Arc<HaneulClient<C>>,
        query_interval: Duration,
    ) {
        tracing::info!(
        ...
        loop {
            ...
            let events = retry_with_max_delay!(
                haneul_client.query_events_by_module(PACKAGE_ID, module.clone(), next_cursor),
                Duration::from_secs(600)
            )
            .expect("Failed to query events from haneul client after retry");
...
mod tests {
    ...
    async fn test_haneul_syncer_basic() -> anyhow::Result<()> {
        ...
        let event_1: HaneulEvent = HaneulEvent::random_for_testing();
        ...
        let event_2: HaneulEvent = HaneulEvent::random_for_testing();
    ...
    async fn assert_no_more_events(
        interval: Duration,
        events_rx: &mut haneullabs_metrics::metered_channel::Receiver<Vec<HaneulEvent>>,
    ) {
