...
pub fn build_transaction(gas_object_ref: &ObjectRef) -> TransactionData {
    let sender = HaneulAddress::ZERO;
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.pay_haneul(vec![HaneulAddress::ZERO], vec![1u64]).unwrap();
...
mod tests {
    ...
    async fn test_onchain_execution_loop() {
        ...
        mock_transaction_response(&haneul_client_mock, tx_digest, HaneulExecutionStatus::Success);
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let (action, haneul_tx_digest, haneul_tx_event_index) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
        );
    ...
    fn get_bridge_authority_approved_action(
        ...
        (action, haneul_tx_digest, haneul_tx_event_index)
    }
