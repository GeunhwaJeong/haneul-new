...
use crate::{
    ...
    haneul_transaction_builder::build_transaction,
    ...
    C: HaneulClientInner + 'static,
{
    ...
    async fn request_signature(
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(&haneul_client, &action, &store)
            .await
        {
            return;
        }
    ...
    async fn run_onchain_execution_loop(
        ...
        while let Some(certificate_wrapper) = execution_queue_receiver.recv().await {
            ...
            let tx_data = match build_transaction(haneul_address, &gas_object_ref, ceriticate_clone) {
                Ok(tx_data) => tx_data,
                Err(err) => {
                    // TODO: add mertrics
                    error!(
                ...
                .await
            {
                ...
                Err(err) => {
                    error!("Haneul transaction failed at signing: {err:?}");
    ...
    async fn handle_execution_effects(
        ...
        match status {
            ...
            HaneulExecutionStatus::Failure { error } => {
                ...
                // TODO metrics + alerts
                error!(?tx_digest, "Manual intervention is needed. Haneul transaction executed and failed with error: {error:?}");
            }
        }
    }
...
mod tests {
    ...
    use crate::{
        ...
        test_utils::{
            get_test_authorities_and_run_mock_bridge_server, get_test_haneul_to_eth_bridge_action,
    ...
    async fn test_onchain_execution_loop() {
        ...
        let tx_data = build_transaction(haneul_address, &gas_object_ref, action_certificate).unwrap();
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        let tx_data =
            build_transaction(haneul_address, &gas_object_ref, action_certificate.clone()).unwrap();
        let tx_digest = get_tx_digest(tx_data, &dummy_haneul_key);
        mock_transaction_error(
            &haneul_client_mock,
            tx_digest,
