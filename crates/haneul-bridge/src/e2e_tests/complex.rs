...
async fn test_v2_haneul_with_v1_evm() {
    ...
    let message_v2: eth_haneul_bridge::Message = haneul_to_eth_v2_action.try_into().unwrap();
    ...
    let call_v2 =
        eth_haneul_bridge.transfer_bridged_tokens_with_signatures_v2(signatures_v2, message_v2);
...
async fn test_v1_deposit_during_v2_upgrade() {
    ...
    let deposit_call = contract
        .bridge_eth(haneul_recipient.into(), BridgeChainId::HaneulCustom as u8)
    ...
    let timer = std::time::Instant::now();
    let message: eth_haneul_bridge::Message = haneul_to_eth_v2_action.try_into().unwrap();
