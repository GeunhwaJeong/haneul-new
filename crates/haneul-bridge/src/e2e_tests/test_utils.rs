...
use haneul_types::bridge::{MoveTypeBridgeMessageKey, MoveTypeBridgeRecord};
...
impl BridgeTestCluster {
    ...
    pub async fn haneul_bridge_client(&self) -> anyhow::Result<HaneulBridgeClient> {
        HaneulBridgeClient::new(&self.test_cluster.fullnode_handle.rpc_url).await
    }

    pub fn haneul_client(&self) -> HaneulClient {
        self.test_cluster.fullnode_handle.haneul_client.clone()
    }
...
pub async fn publish_coins_return_add_coins_on_haneul_action(
    wallet_context: &mut WalletContext,
    bridge_arg: ObjectArg,
    publish_coin_responses: Vec<HaneulTransactionBlockResponse>,
    token_ids: Vec<u8>,
...
pub(crate) async fn deploy_sol_contract(
    ...
    let contracts = DeployedSolContracts {
        haneul_bridge: *deployed_contracts.get(HANEUL_BRIDGE_NAME).unwrap(),
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            haneul: HaneulConfig {
                haneul_rpc_url: test_cluster.fullnode_handle.rpc_url.clone(),
...
pub(crate) async fn get_signatures(
    ...
    let summary = haneul_bridge_client.get_bridge_summary().await.unwrap();
    ...
    let key = serde_json::json!(
        {
            ...
            "source_chain": haneul_chain_id,
        }
    );
    let status_object_id = haneul_client.read_api().get_dynamic_field_object(records_id,
        DynamicFieldName {
