...
pub struct MetricsPushClient {
    certificate: std::sync::Arc<haneul_tls::SelfSignedCertificate>,
    client: reqwest::Client,
}

impl MetricsPushClient {
    pub fn new(metrics_key: haneul_types::crypto::NetworkKeyPair) -> Self {
        use fastcrypto::traits::KeyPair;
        let certificate = std::sync::Arc::new(haneul_tls::SelfSignedCertificate::new(
            metrics_key.private(),
            haneul_tls::HANEUL_VALIDATOR_SERVER_NAME,
        ));
    ...
    pub fn certificate(&self) -> &haneul_tls::SelfSignedCertificate {
        &self.certificate
    }
...
pub fn start_metrics_push_task(
    ...
    // TODO (johnm) split this out into haneullabs-common
...
pub struct BridgeMetrics {
    pub(crate) err_build_haneul_transaction: IntCounter,
    pub(crate) err_signature_aggregation: IntCounter,
    pub(crate) err_haneul_transaction_submission: IntCounter,
    pub(crate) err_haneul_transaction_submission_too_many_failures: IntCounter,
    pub(crate) err_haneul_transaction_execution: IntCounter,
    ...
    pub last_synced_haneul_checkpoint: IntGauge,
...
impl BridgeMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            err_build_haneul_transaction: register_int_counter_with_registry!(
                "bridge_err_build_haneul_transaction",
                "Total number of errors of building haneul transactions",
                registry,
            )
            .unwrap(),
            ...
            .unwrap(),
            err_haneul_transaction_submission: register_int_counter_with_registry!(
                "bridge_err_haneul_transaction_submission",
                "Total number of errors of submitting haneul transactions",
                registry,
            )
            .unwrap(),
            err_haneul_transaction_submission_too_many_failures: register_int_counter_with_registry!(
                "bridge_err_haneul_transaction_submission_too_many_failures",
                "Total number of continuous failures to submitting haneul transactions",
                registry,
            )
            .unwrap(),
            err_haneul_transaction_execution: register_int_counter_with_registry!(
                "bridge_err_haneul_transaction_execution",
                "Total number of failures of haneul transaction execution",
                registry,
            )
            .unwrap(),
            ...
            .unwrap(),
            haneul_watcher_received_events: register_int_counter_with_registry!(
                "bridge_haneul_watcher_received_events",
                "Total number of received events in haneul watcher",
                registry,
            )
            .unwrap(),
            ...
            .unwrap(),
            haneul_watcher_received_actions: register_int_counter_with_registry!(
                "bridge_haneul_watcher_received_actions",
                "Total number of received actions in haneul watcher",
                registry,
            )
            .unwrap(),
            ...
            gas_coin_balance: register_int_gauge_with_registry!(
                ...
                "Current balance of gas coin, in geunhwa",
                registry,
            )
            .unwrap(),
            ...
            .unwrap(),
            last_synced_haneul_checkpoint: register_int_gauge_with_registry!(
                "last_synced_haneul_checkpoint",
                "The latest haneul checkpoint that indexer synced",
                registry,
            )
            .unwrap(),
