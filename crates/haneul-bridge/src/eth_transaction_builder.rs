...
use crate::abi::{
    eth_bridge_committee, eth_committee_upgradeable_contract, eth_haneul_bridge, EthBridgeCommittee,
...
pub async fn build_emergency_op_approve_transaction(
    ...
    let contract = EthHaneulBridge::new(contract_address, signer.into());

    let message: eth_haneul_bridge::Message = action.clone().try_into()?;
    let signatures = sigs
        .signatures
        .values()
