...
use crate::abi::{
    ...
    eth_committee_upgradeable_contract, eth_haneul_bridge,
};
use crate::error::{BridgeError, BridgeResult};
use crate::types::{
    AddTokensOnEvmAction, AssetPriceUpdateAction, BlocklistCommitteeAction, BridgeAction,
    BridgeCommitteeValiditySignInfo, EmergencyAction, EvmContractUpgradeAction, LimitUpdateAction,
...
pub fn build_emergency_op_approve_transaction(
    ...
    let message: eth_haneul_bridge::BridgeUtils::Message = action.clone().try_into()?;
    let signatures = sigs
        .signatures
        .values()
    ...
    let call = eth_haneul_bridge::EthHaneulBridge::executeEmergencyOpWithSignaturesCall {
        signatures,
        message,
    };
