...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) haneul_syncer_sequence_number_cursor: DBMap<(), u64>,
}
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_haneul_sequence_number_cursor(&self, cursor: u64) -> BridgeResult<()> {
        ...
        batch
            .insert_batch(&self.haneul_syncer_sequence_number_cursor, [((), cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Couldn't insert into haneul_syncer_sequence_number_cursor: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_haneul_sequence_number_cursor(&self) -> BridgeResult<Option<u64>> {
        self.haneul_syncer_sequence_number_cursor
            .get(&())
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Couldn't get haneul_syncer_sequence_number_cursor: {:?}",
                    e
                ))
            })
    }
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        let haneul_sequence_number_cursor = 100u64;
        assert!(store.get_haneul_sequence_number_cursor().unwrap().is_none());
        store
            .update_haneul_sequence_number_cursor(haneul_sequence_number_cursor)
            .unwrap();
        assert_eq!(
            store.get_haneul_sequence_number_cursor().unwrap().unwrap(),
            haneul_sequence_number_cursor
        );
    }
