...
use haneul_types::bridge::{
    BridgeChainId, APPROVAL_THRESHOLD_ADD_TOKENS_ON_EVM, APPROVAL_THRESHOLD_ADD_TOKENS_ON_HANEUL,
...
pub struct HaneulToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub haneul_tx_digest: TransactionDigest,
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is HaneulMainnet,
    // it means we want to update the limit for the HaneulMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnHaneulAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub struct AddTokensOnEvmAction {
    ...
    pub token_haneul_decimals: Vec<u8>,
    pub token_prices: Vec<u64>,
}
...
pub enum BridgeAction {
    ...
    HaneulToEthBridgeAction(HaneulToEthBridgeAction),
    /// Eth to haneul bridge action
    EthToHaneulBridgeAction(EthToHaneulBridgeAction),
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_haneul_chain() && other.is_haneul_chain() {
        return false;
    }
    if !one.is_haneul_chain() && !other.is_haneul_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::HaneulMainnet;
    }
    if one == BridgeChainId::HaneulMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::HaneulMainnet;
    }
    if other == BridgeChainId::HaneulMainnet {
...
mod tests {
    ...
    use crate::test_utils::get_test_eth_to_haneul_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_haneul_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_haneul_bridge_action(None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
