...
pub const HANEUL_TX_DIGEST_LENGTH: usize = 32;
pub const ETH_TX_HASH_LENGTH: usize = 32;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"HANEUL_NATIVE_BRIDGE";
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            BridgeAction::HaneulToEthBridgeAction(a) => {
                ...
                // Add source chain id
                bytes.push(BridgeChainId::HaneulTestnet as u8);
                // Add source tx id length
                bytes.push(HANEUL_TX_DIGEST_LENGTH as u8);
                // Add source tx id
                bytes.extend_from_slice(a.haneul_tx_digest.as_ref());
                // Add source tx event index
                bytes.extend_from_slice(&a.haneul_tx_event_index.to_le_bytes());
            ...
            BridgeAction::EthToHaneulBridgeAction(_e) =>
...
mod tests {
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let source_tx_digest_length_bytes = vec![HANEUL_TX_DIGEST_LENGTH as u8]; // len: 1
        let source_tx_digest_bytes = haneul_tx_digest.inner().to_vec(); // len: 32
        let source_event_index_bytes = haneul_tx_event_index.to_le_bytes().to_vec(); // len: 2
