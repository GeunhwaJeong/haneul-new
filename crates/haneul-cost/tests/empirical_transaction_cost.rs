...
use haneul_cost::estimator::ESTIMATE_FILE;
use haneul_cost::estimator::{
    estimate_computational_costs_for_transaction, read_estimate_file, CommonTransactionCosts,
};
use haneul_types::base_types::HaneulAddress;
use haneul_types::coin::COIN_JOIN_FUNC_NAME;
use haneul_types::coin::COIN_MODULE_NAME;
use haneul_types::coin::COIN_SPLIT_VEC_FUNC_NAME;
...
// 1. Run `cargo insta test --review` under `./haneul-cost`.
...
async fn check_estimates() {
    ...
    // Check that Haneul Transfer estimate
    let mut gas_objects = test_gas_objects();
    let (sender, keypair) = test_account_keys().pop().unwrap();
    let whole_haneul_coin_tx = make_transfer_haneul_transaction(
    ...
    let e1 = estimate_computational_costs_for_transaction(whole_haneul_coin_tx.signed_data.data.kind)
        .unwrap();
    let e2 =
        estimate_computational_costs_for_transaction(partial_haneul_coin_tx.signed_data.data.kind)
            .unwrap();

    assert_eq!(
        e1,
        cost_map
            .get(&CommonTransactionCosts::TransferWholeHaneulCoin)
            .unwrap()
            .clone()
    );
    assert_eq!(
        e2,
        cost_map
            .get(&CommonTransactionCosts::TransferPortionHaneulCoin)
            .unwrap()
            .clone()
    );
}
...
async fn run_cost_test() -> BTreeMap<CommonTransactionCosts, GasCostSummary> {
    ...
    //
    // Transfer Whole Haneul Coin and Transfer Portion of Haneul Coin
    //
    let whole_haneul_coin_tx = make_transfer_haneul_transaction(
        gas_objects.pop().unwrap().compute_object_reference(),
        HaneulAddress::default(),
    ...
    let whole_haneul_coin_tx_gas_used =
        submit_single_owner_transaction(whole_haneul_coin_tx, configs.validator_set())
            .await
            .gas_cost_summary()
            .clone();
    let partial_haneul_coin_tx_gas_used =
        submit_single_owner_transaction(partial_haneul_coin_tx, configs.validator_set())
            .await
            .gas_cost_summary()
            .clone();
    ...
    ret.insert(
        ...
        whole_haneul_coin_tx_gas_used,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionHaneulCoin,
        partial_haneul_coin_tx_gas_used,
    );
