...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeHaneulCoin,
    TransferPortionHaneulCoin,
    SharedCounterCreate,
...
async fn create_txes(
    ...
    //
    let whole_haneul_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_haneul(None, HaneulAddress::default())
            .build();
    let partial_haneul_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_haneul(Some(10), HaneulAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeHaneulCoin,
        whole_haneul_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionHaneulCoin,
        partial_haneul_coin_tx,
    );
