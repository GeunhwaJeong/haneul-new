...
#[repr(u8)]
pub enum HaneulNativeCostIndex {
    EVENT_EMIT = 0,
...
pub fn _native_cost_schedule() -> Vec<GasCost> {
    use HaneulNativeCostIndex as N;

    let mut native_table = vec![
        // This is artificially chosen to limit too many event emits
        // We will change this in https://github.com/GeunhwaJeong/haneul/issues/3341
        (
            N::EVENT_EMIT,
...
fn bytecode_costs() -> Vec<(Bytecode, GasCost, bool)> {
    ...
    vec![
        ...
        // Not supported yet in Haneul Move
        (
