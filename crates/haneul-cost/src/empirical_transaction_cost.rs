...
) -> Result<BTreeMap<CommonTransactionCosts, HaneulGasCostSummary>, anyhow::Error> {
...
async fn run_common_single_writer_tx_costs(
    ...
    async fn split_n(n: u64, equal_parts: bool) -> Result<HaneulGasCostSummary, anyhow::Error> {
        ...
        if let HaneulClientCommandResult::SplitCoin(response) = resp {
    ...
    let gas_used = if let HaneulClientCommandResult::MergeCoin(r) = resp {
        r.effects.gas_used
    } else {
    ...
    let gas_used = if let HaneulClientCommandResult::Publish(response) = resp {
    ...
    let gas_used = if let HaneulClientCommandResult::Transfer(_, _, r) = resp {
        r.gas_used
    } else {
        panic!("Command failed")
    };
    ...
    // Transfer the whole Haneul object

    let obj_id = object_refs.get(4).unwrap().object_id;

    let resp = HaneulClientCommands::TransferHaneul {
        amount: None,
        to: recipient,
        haneul_coin_object_id: obj_id,
        gas_budget: 10000,
    }
    .execute(&mut context)
    .await?;

    let gas_used = if let HaneulClientCommandResult::TransferHaneul(_, r) = resp {
        r.gas_used
    } else {
        panic!("Command failed")
    };
    ret.insert(CommonTransactionCosts::TransferWholeHaneulCoin, gas_used);

    // Transfer a portion of Haneul
    let (_network, mut context, address) = setup_network_and_wallet().await?;
    ...
    ret.insert(CommonTransactionCosts::TransferPortionHaneulCoin, gas_used);

    Ok(ret)
}

async fn run_counter_costs() -> BTreeMap<CommonTransactionCosts, HaneulGasCostSummary> {
    run_counter_costs_impl()
        .await
        .into_iter()
        .map(|(k, v)| (k, HaneulGasCostSummary::from(v)))
        .collect()
}
