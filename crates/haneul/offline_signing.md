...
This is a guide for users who wish to sign a transaction using an offline device or implementing wallet in a different language without relying on haneul keystore. Here are the steps to:
...
```shell
target/debug/haneul client serialize-transfer-haneul --to $ADDRESS --haneul-coin-object-id $OBJECT_ID --gas-budget 1000

Raw tx_bytes to execute: $TX_BYTES
```

## Step 2: Sign the data
This can be done elsewhere in your signing device or implemented in languages of your choice. Haneul accepts signatures for pure Ed25519, ECDSA Secp256k1, ECDSA Secp256r1 and native multisig ([Read more](https://github.com/GeunhwaJeong/haneul/blob/d0aceaea613b33fc969f7ca2cdd84b8a35e87de3/crates/haneul/multisig.md)).

The signature is committed to an intent message of the transaction data. See more on [intent message](https://github.com/GeunhwaJeong/haneul/blob/7f456d8e0db3234b178854df6185037e7b4312cb/crates/haneul/intent_signing.md) on how to construct an intent message. 
...
```shell
target/debug/haneul keytool sign --address $ADDRESS --data $TX_BYTES
...
To ensure the signature produced offline matches with Haneul's validity rules for testing purpose, you can import the mnemonics to haneul.keystore using `haneul keytool import` and then sign with `haneul keytool sign` and compare the signature results. Additionally, test vectors can be found at `~/haneul/sdk/typescript/test/e2e/raw-signer.test.ts`. 
...
Now that you have obtained the serialized signature, you can submit using the execution transaction command. This command takes `--tx-bytes` as the raw transaction bytes to execute (see output of `haneul client serialize-transfer-haneul`) and the serialized signature (Base64 encoded `flag || sig || pk`, see output of `haneul keytool sign`). This executes the signed transaction and returns the certificate and transaction effects if successful.

```shell
haneul client execute-signed-tx --tx-bytes $TX_BYTES --signatures $SERIALIZED_SIG
----- Certificate ----
