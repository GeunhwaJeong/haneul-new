...
pub async fn perform_zk_login_test_tx(
    ...
    let user_salt = get_salt(parsed_token, "https://salt.api.haneul-labs.com/get_salt")
        .await
    ...
    let zklogin_address = HaneulAddress::from_bytes(get_zk_login_address(
    ...
    let transaction_response = haneul
        ...
        .execute_transaction_block(
            Transaction::from_generic_sig_data(txb_res, Intent::haneul_transaction(), vec![sig]),
...
fn get_config(network: &str) -> (&str, &str) {
    match network {
        "devnet" => (
            "https://faucet.devnet.haneul.io/gas",
