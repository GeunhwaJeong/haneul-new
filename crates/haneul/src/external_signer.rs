...
pub enum ExternalKeysCommand {
    ...
    /// Add an existing key to the haneul cli for an existing external signer key
...
mod tests {
    ...
    use crate::haneul_commands::HaneulCommand;
    ...
    fn test_external_keys_generate_parses_provision_mode() {
        let command = HaneulCommand::try_parse_from([
            "haneul",
            "external-keys",
            "generate",
            "yubikey",
        ...
        let HaneulCommand::ExternalKeys { cmd, .. } = command else {
