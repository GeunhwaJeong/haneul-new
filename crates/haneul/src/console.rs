...
pub struct ConsoleOpts {
    #[clap(subcommand)]
    pub command: HaneulClientCommands,
...
pub async fn start_console(
    ...
    let app: Command = HaneulClientCommands::command();
    ...
    writeln!(
        out,
        "Connecting to Haneul full node. API version {}",
        client.api_version()
    )?;
...
async fn handle_command(
    ...
    if let Ok(mut cache) = completion_cache.write() {
        match result {
            ...
            HaneulClientCommandResult::Objects(ref objects) => {
                let objects = objects
                    .iter()
    ...
    if matches!(
        result,
        HaneulClientCommandResult::Switch(SwitchResponse { env: Some(_), .. })
    ) {
        println!("Haneul environment switch completed, please restart Haneul console.");
        return Ok(true);
    }
    Ok(false)
}
