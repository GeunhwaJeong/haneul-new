    ...
    match (skip_dependency_verification, verify_dependencies) {
        ...
        (false, false) => {
            eprintln!("{}: In a future release, dependency source code will no longer be verified by default during publication and upgrade. \
                ...
                You can also manually verify dependencies using `haneul client verify-source`.",
                "[warning]".bold().yellow());
            Ok(true)
        },
...
async fn check_protocol_version_and_warn(client: &HaneulClient) -> Result<(), anyhow::Error> {
