        ...
        let ret = match self {
            ...
            HaneulClientCommands::Upgrade {
                ...
                let package_path = package_path.canonicalize().map_err(|e| {
                    HaneulErrorKind::ModulePublishFailure {
                ...
                if let HaneulClientCommandResult::TransactionBlock(ref response) = result
                    && let Err(e) = haneul_package_management::update_lock_file(
                        context,
                        LockCommand::Upgrade,
            ...
            HaneulClientCommands::ChainIdentifier => {
                ...
                HaneulClientCommandResult::ChainIdentifier(chain_id)
            }
...
pub(crate) async fn upgrade_package(
 ...
 - update the `Move.lock` address using the `haneul manage-package` command to be the same as the `Move.toml`; OR
 - check that your `haneul active-env` {env_alias} corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
 - contact the maintainer if this package is a dependency and request resolving the conflict."
    ...
    let resp = read_api
        .get_object_with_options(
            upgrade_capability,
            HaneulObjectDataOptions::default().with_bcs().with_owner(),
        )
        .await?;
...
pub(crate) async fn compile_package(
    ...
    if let Some(Some(HaneulProtocolConfigValue::U32(min_version))) = protocol_config
        .attributes
    ...
    // Check that the package's Move version is compatible with the chain's
    if let Some(Some(HaneulProtocolConfigValue::U32(max_version))) =
    ...
    compiled_package
        ...
        .map_err(|e| HaneulErrorKind::ModuleBuildFailure {
...
impl HaneulClientCommandResult {
    ...
    pub async fn prerender_clever_errors(mut self, context: &mut WalletContext) -> Self {
        match &mut self {
            ...
            | HaneulClientCommandResult::RemoveAddress(_)
    ...
    if serialize_unsigned_transaction {
        ...
        let mut signatures = vec![
            context
                .config
                .keystore
                .sign_secure(&signer, &tx_data, Intent::haneul_transaction())
                .await?
                .into(),
        ];
            ...
            && gas_sponsor != signer
        {
            signatures.push(
                context
                    .config
                    .keystore
                    .sign_secure(&gas_sponsor, &tx_data, Intent::haneul_transaction())
                    .await?
                    .into(),
            );
        }
...
fn to_package(o: HaneulObjectResponse) -> anyhow::Result<MovePackage> {
    let id = o.object_id()?;
    let Some(HaneulRawData::Package(p)) = o.into_object()?.bcs else {
...
async fn fetch_move_packages(
    ...
    let objects = read_api
        .multi_get_object_with_options(package_ids, HaneulObjectDataOptions::bcs_lossless())
        .await?;
