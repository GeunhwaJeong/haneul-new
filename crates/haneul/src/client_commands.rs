        ...
        let ret = Ok(match self {
            HaneulClientCommands::Upgrade {
                ...
                let data = client
                    ...
                    .await?;
                if serialize_output {
                    return Ok(HaneulClientCommandResult::SerializeTx(Base64::encode(
            ...
            HaneulClientCommands::Transfer {
                ...
                let signature =
                    context
                        .config
                        .keystore
                        .sign_secure(&from, &data, Intent::haneul_transaction())?;
                let response = context
                    .execute_transaction_block(
                        Transaction::from_data(data, Intent::haneul_transaction(), vec![signature])
                            .verify()?,
...
impl Display for HaneulClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            HaneulClientCommandResult::SerializeTx(data) => {
...
pub async fn call_move(
    ...
    let transaction =
        Transaction::from_data(data, Intent::haneul_transaction(), vec![signature]).verify()?;
    ...
    Ok(HaneulClientCommandResult::Call(response))
}
