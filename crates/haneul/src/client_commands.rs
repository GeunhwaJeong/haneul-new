...
use haneul_json_rpc_types::{
    ...
    HaneulEvent, HaneulTransactionBlock, HaneulTransactionBlockEffects,
    HaneulTransactionBlockEvents, HaneulTransactionBlockResponse, ObjectChange as RpcObjectChange,
};
...
pub enum HaneulClientCommands {
    ...
    /// Send funds to an address balance using the `haneul::coin::send_funds` API.
    ...
    SendFunds {
        ...
        /// The amount to send (in GEUNHWA). Required unless --all-coins is specified.
        #[clap(
            long,
        ...
        /// The coin type to send (e.g., "0x2::haneul::HANEUL"). Defaults to HANEUL.
        #[clap(long, value_parser = parse_haneul_type_tag)]
    ...
    pub async fn execute(
        ...
        let ret = match self {
            ...
            HaneulClientCommands::Balance {
                ...
                order_balance_outputs_haneul_first(&mut balances);
                HaneulClientCommandResult::Balance(balances, with_coins)
            }
            ...
            HaneulClientCommands::Object { id, bcs } => {
                ...
                if !bcs {
                    ...
                    HaneulClientCommandResult::Object(object, json_content)
                } else {
            ...
            HaneulClientCommands::SendFunds {
                ...
                let (amount, use_address_balance) = if all_coins {
                    ...
                    let use_address_balance = if stateless {
                        ...
                        bail!(
                            "Insufficient balance to send {} GEUNHWA. \
                ...
                if use_address_balance {
                    ...
                    if stateless {
                        ...
                        is_haneul,
                        "Non-HANEUL coin transfers using coins require explicit coin selection. \
...
/// Keep HANEUL first while preserving the balance API's order for other coin types.
fn order_balance_outputs_haneul_first(balances: &mut Vec<BalanceOutput>) {
    let haneul_type_tag = GasCoin::type_().to_canonical_string(/* with_prefix */ true);
    if let Some(index) = balances
        .iter()
        .position(|balance| balance.balance.coin_type() == haneul_type_tag.as_str())
    {
        let haneul_balance = balances.remove(index);
        balances.insert(0, haneul_balance);
    }
}
...
fn verify_no_build_env(build_config: &MoveBuildConfig, command: &str) -> anyhow::Result<()> {
    if build_config.environment.is_some() {
        bail!(
            "The `--build-env` argument is not allowed for `haneul move {command}`; when publishing you must build for the environment that you are publishing for."
        );
    }
    Ok(())
}
