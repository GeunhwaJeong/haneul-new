...
module Examples::CustomObjectTemplate {
    ...
    /// A custom haneul object. Every object must have the `key` attribute
    /// (indicating that it is allowed to be a key in the haneul global object
    /// pool), and must have a field `id: ID` corresponding to its haneul ObjId.
    ...
    /// Example of an entrypoint function to be embedded in a Haneul
    ...
    ///
    /// A Haneul transaction must declare the ID's of each object it will
