...
async fn test_native_transfer() -> Result<(), anyhow::Error> {
    ...
    let (mut_obj1, mut_obj2) = if let HaneulClientCommandResult::Transfer(response) = resp {
        assert!(
            response.status_ok().unwrap(),
            "Command failed: {:?}",
            response
    ...
    // Get the mutated objects
    let (_mut_obj1, _mut_obj2) = if let HaneulClientCommandResult::Transfer(response) = resp {
        (
            response
                .effects
                .as_ref()
...
async fn test_get_owned_objects_owned_by_address_and_check_pagination() -> Result<(), anyhow::Error>
{
    ...
    let mut cursor = None;
    let mut response_data: Vec<HaneulObjectResponse> = Vec::new();
    while has_next {
        let object_responses = client
            .read_api()
            .get_owned_objects(
