...
async fn test_native_transfer() -> Result<(), anyhow::Error> {
    ...
    .execute(context)
    .await?;
    let mut_obj1 = if let HaneulClientCommandResult::Object(
    ...
    .execute(context)
    .await?;
    let mut_obj2 = if let HaneulClientCommandResult::Object(
        GetObjectDataResponse::Exists(object),
        false,
    ) = resp
    {
...
fn test_bug_1078() {
    let read =
        HaneulClientCommandResult::Object(GetObjectDataResponse::NotExists(ObjectID::random()), false);
...
async fn test_delegation_with_none_amount() -> Result<(), anyhow::Error> {
    ...
    let validator_addr = validator_addrs.first().unwrap().haneul_address;
    ...
    assert_eq!(
        ...
        stake.first().unwrap().staked_haneul.principal()
    );
    Ok(())
}
...
async fn test_delegation_with_u64_amount() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(1, stake.len());
    assert_eq!(10000, stake.first().unwrap().staked_haneul.principal());
    Ok(())
}
