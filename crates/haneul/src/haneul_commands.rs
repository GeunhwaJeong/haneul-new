...
async fn start(
    ...
    if force_regenesis {
        ...
        // Load the config of the Haneul authority.
        // To keep compatibility with haneul-test-validator where the user can pass a config
        // directory, this checks if the config is a file or a directory
        let network_config_path = if let Some(ref config) = config {
            if config.is_dir() {
                config.join(HANEUL_NETWORK_CONFIG)
