...
use crate::client_commands::{
    implicit_deps_for_protocol_version, pkg_tree_shake, HaneulClientCommands,
};
...
use haneul_config::{
    genesis_blob_exists, haneul_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH,
    HANEUL_CLIENT_CONFIG, HANEUL_FULLNODE_CONFIG, HANEUL_NETWORK_CONFIG,
};
use haneul_config::{
    HANEUL_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME, HANEUL_GENESIS_FILENAME, HANEUL_KEYSTORE_FILENAME,
};
use haneul_faucet::{create_wallet_context, start_faucet, AppState, FaucetConfig, SimpleFaucet};
...
const DEFAULT_FAUCET_NUM_COINS: usize = 5; // 5 coins per request was the default in haneul-test-validator
...
pub struct IndexerArgs {
    ...
    /// DB name for the Indexer Postgres DB. Default DB name is haneul_indexer.
    #[clap(long, default_value = "haneul_indexer")]
    pg_db_name: String,
...
impl IndexerArgs {
    pub fn for_testing() -> Self {
        Self {
            ...
            pg_db_name: "haneul_indexer".to_string(),
                    ...
                    haneul_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        ...
                        let (chain_id, client) = if build.ignore_chain {
                            ...
                            let Ok(client) = context.get_client().await else {
                                bail!("`haneul move build --dump-bytecode-as-base64` requires a connection to the network. Current active network is {} but failed to connect to it.", context.config.active_env.as_ref().unwrap());
                            };
...
async fn start(
    ...
    let config_dir = if force_regenesis {
        ...
        haneullabs_common::tempdir()?.into_path()
    } else {
    ...
    if with_indexer.is_some() && data_ingestion_dir.is_none() {
        data_ingestion_dir = Some(haneullabs_common::tempdir()?.into_path())
    }
    ...
    let mut fullnode_url = haneul_config::node::default_json_rpc_address();
    ...
    if let Some(input) = with_faucet {
        ...
        if force_regenesis {
            ...
            keystore.add_key(None, HaneulKeyPair::Ed25519(kp)).unwrap();
            HaneulClientConfig {
                keystore,
                envs: vec![HaneulEnv {
...
async fn genesis(
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            bail!("Cannot run genesis with non-empty Haneul config directory {}, please use the --force/-f option to remove the existing configuration", haneul_config_dir.to_str().unwrap());
        }
    }
    ...
    if !wallet_conf_path.exists() {
        let env = match std::env::var_os("HANEUL_CONFIG_WITH_RPC_URL") {
            ...
            None => {
                if accept_defaults {
                    ...
                    print!(
                        "Config file [{:?}] doesn't exist, do you want to connect to a Haneul Full node server [y/N]?",
                        wallet_conf_path
                    );
                }
                if accept_defaults
        ...
        if let Some(env) = env {
            let keystore_path = match wallet_conf_path.parent() {
                ...
                Some(parent) => parent
                    .canonicalize()
                    .context("Could not find haneul config directory")?,

                // No parent component and the wallet config was the empty string, use the default
                // config.
                None if wallet_conf_path.as_os_str().is_empty() => haneul_config_dir()?,
