...
pub enum HaneulCommand {
    ...
    Genesis {
        ...
        #[clap(
            long,
            help = "Creates an extra faucet configuration for haneul-test-validator persisted runs."
        )]
        with_faucet: bool,
    },
...
async fn genesis(
    ...
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path = haneul_config_dir.join(multiaddr_to_filename(
                ssfn.p2p_address.clone(),
                haneul_config::ssfn_config_file(i),
            ));
            ...
            .enumerate()
        {
            let path = haneul_config_dir.join(multiaddr_to_filename(
                ...
                haneul_config::validator_config_file(i),
            ));
