...
use crate::client_commands::{
    implicit_deps_for_protocol_version, pkg_tree_shake, HaneulClientCommands, USER_AGENT,
};
...
use haneul_faucet::{create_wallet_context, start_faucet, AppState, FaucetConfig, LocalFaucet};
...
use haneul_move_build::{
    ...
    implicit_deps, BuildConfig as HaneulBuildConfig, HaneulPackageHooks,
};
...
pub struct RpcArgs {
    ...
    ///
    /// If no URL is provided, defaults to postgres://postgres:postgrespw@localhost:5432/haneul_indexer
    #[clap(
        long,
        default_missing_value = "postgres://postgres:postgrespw@localhost:5432/haneul_indexer",
        num_args = 0..=1,
        require_equals = true,
                ...
                match cmd {
                    haneul_move::Command::Summary(mut s) if s.package_id.is_some() => {
                        ...
                        let Some(client) = client else {
                            bail!("`haneul move summary --package-id <object_id>` requires a configured network");
                        };
                    ...
                    haneul_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        ...
                        let previous_id = if let Some(ref chain_id) = chain_id {
                            haneul_package_management::set_package_id(
                                &rerooted_path,
                        ...
                        if let (Some(chain_id), Some(previous_id)) = (chain_id, previous_id) {
                            let _ = haneul_package_management::set_package_id(
                                &rerooted_path,
                                build_config.install_dir.clone(),
                                &chain_id,
                                previous_id,
...
async fn start(
    ...
    let database_url = if with_graphql.is_some() && with_indexer.is_none() {
        ...
        Some(Url::parse("postgres://postgres:postgrespw@localhost:5432/haneul_indexer").unwrap())
    } else {
        with_indexer
    };
...
) -> anyhow::Result<(Option<String>, Option<HaneulClient>)> {
