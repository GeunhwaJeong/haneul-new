...
pub enum KeyToolCommand {
    /// Convert private key from wallet format (hex of 32 byte private key) to haneul.keystore format
    ...
    /// to haneul.keystore, use `haneul client new-address`).
    Generate {
    ...
    /// Add a new key to haneul.keystore using either the input mnemonic phrase or a private key (from the Wallet),
    /// the key scheme flag {ed25519 | secp256k1 | secp256r1} and an optional derivation path,
    /// default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0 for secp256k1
    /// or m/74'/8282'/0'/0/0 for secp256r1. Supports mnemonic phrase of word length 12, 15, 18`, 21, 24.
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for for the given address in haneul keystore.
    ...
    Sign {
        #[clap(long, value_parser = decode_bytes_hex::<HaneulAddress>)]
...
impl From<PublicKey> for Key {
    fn from(key: PublicKey) -> Self {
        Key {
            ...
            haneul_address: HaneulAddress::from(&key),
