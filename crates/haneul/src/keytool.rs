...
pub enum KeyToolCommand {
    ...
    /// to haneul.keystore, use `haneul client new-address`), see more at [enum HaneulClientCommands].
    Generate {
    ...
    /// [transaction builder API](https://docs.haneul.io/haneul-jsonrpc) and its intent. If
    ...
    /// Add a new key to haneul.keystore using either the input mnemonic phrase or a private key (from the Wallet), the key scheme flag {ed25519 | secp256k1 | secp256r1}
    /// and an optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0 for secp256k1
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<(), anyhow::Error> {
        match self {
                ...
                if input_string.starts_with("0x") {
                    ...
                    match key_scheme {
                        SignatureScheme::ED25519 => {
                            ...
                            let address: HaneulAddress = Into::<HaneulAddress>::into(&skp.public());
            ...
            KeyToolCommand::LoadKeypair { file } => {
                match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        if let HaneulKeyPair::Ed25519(kp) = keypair {
            ...
            KeyToolCommand::GenerateZkLoginAddress { address_seed } => {
                ...
                println!("Haneul Address: {:?}", user_address);
            }
