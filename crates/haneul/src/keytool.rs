    ...
    pub async fn execute(
        ...
        Ok(match self {
                ...
                match HaneulKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => context.config.keystore.get_alias(&key.haneul_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let haneul_address = context
                            ...
                            .await?;
                        let skp = context.config.keystore.export(&haneul_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::Export { key_identity } => {
                ...
                let mut key = Key::from(skp);
                key.alias = context.config.keystore.get_alias(&key.haneul_address).ok();
                let key = ExportedKey {
