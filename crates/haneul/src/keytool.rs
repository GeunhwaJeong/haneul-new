...
use haneul_types::crypto::{
    get_authority_key_pair, AuthorityKeyPair, Ed25519HaneulSignature, EncodeDecodeBase64,
    NetworkKeyPair, SignatureScheme, HaneulKeyPair, HaneulSignatureInner,
};
#[cfg(test)]
...
pub enum KeyToolCommand {
    ...
    /// And output file to current dir (to generate keypair and add to haneul.keystore, use `haneul client new-address`)
    Generate {
    ...
    /// Extract components of a base64-encoded keypair to reveal the Haneul address, public key, and key scheme flag.
    Unpack {
        keypair: HaneulKeyPair,
    },
    /// List all keys by its address, public key, key scheme in the keystore
    List,
    /// Create signature using the haneul keystore and provided data.
    Sign {
    ...
    pub fn execute(self, keystore: &mut Keystore) -> Result<(), anyhow::Error> {
        match self {
            ...
            KeyToolCommand::Sign { address, data } => {
                ...
                let haneul_signature = keystore.sign_secure(&address, &tx_data, Intent::default())?;
                // Separate pub key and signature string, signature and pub key are concatenated with an '@' symbol.
                let signature_string = format!("{:?}", haneul_signature);
                ...
                info!("Serialized signature Base64: {:?}", haneul_signature);
            }
