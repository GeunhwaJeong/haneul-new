...
use haneul_types::crypto::{
    get_authority_key_pair, EncodeDecodeBase64, Signature, SignatureScheme, HaneulKeyPair,
...
pub enum KeyToolCommand {
    ...
    /// encoded 33 byte flag || private key starting with "haneulprivkey").
    /// Hex private key format import and export are both deprecated in
    /// Haneul Wallet and Haneul CLI Keystore. Use `haneul keytool import` if you
    /// wish to import a key to Haneul Keystore.
    ...
    /// with optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or
    /// m/54'/8282'/0'/0/0 for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1. Word
    ...
    ///
    /// Use `haneul client new-address` if you want to generate and save the key into haneul.keystore.
    Generate {
    ...
    /// Add a new key to Haneul CLI Keystore using either the input mnemonic phrase or a Bech32 encoded 33-byte
    /// `flag || privkey` starting with "haneulprivkey", the key scheme flag {ed25519 | secp256k1 | secp256r1}
    /// and an optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0
    /// for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1. Supports mnemonic phrase of word length 12, 15,
    ...
    /// Output the private key of the given key identity in Haneul CLI Keystore as Bech32
    /// encoded string starting with `haneulprivkey`.
    Export {
        #[clap(long)]
    ...
    /// result can be used as signature field for `haneul client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for for the given address (or its alias) in haneul keystore.
    ...
    /// Generate PubKey from pem using HaneulLabs/base64pemkey
    /// Any signature commits to a [struct IntentMessage] consisting of the Base64 encoded
    /// of the BCS serialized transaction bytes itself and its intent. If intent is absent,
    /// default will be used.
    SignKMS {
    ...
    /// and prints out its Haneul address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: HaneulAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: haneul keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. haneul keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct DecodedMultiSigOutput {
    multisig_address: HaneulAddress,
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Haneul Keystore and Haneul Wallet import/export format
    base64_with_flag: String, // Haneul Keystore storage format
    hex_without_flag: String, // Legacy Haneul Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized Haneul signature to be send for executing the transaction.
    haneul_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = HaneulAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (haneul_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{haneul_address}.key");
                ...
                _ => {
                    let (haneul_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{haneul_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                if Hex::decode(&input_string).is_ok() {
                    return Err(anyhow!(
                        "Haneul Keystore and Haneul Wallet no longer support importing 
                    private key as Hex, if you are sure your private key is encoded in Hex, use 
                    `haneul keytool convert $HEX` to convert first then import the Bech32 encoded 
                    private key starting with `haneulprivkey`."
                    ));
                }

                match HaneulKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => keystore.get_alias_by_address(&key.haneul_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let haneul_address = keystore.import_from_mnemonic(
                            &input_string,
                            key_scheme,
                            derivation_path,
                            alias.clone(),
                        )?;
                        let skp = keystore.get_key(&haneul_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::List { sort_by_alias } => {
                let mut keys = keystore
                    ...
                    .map(|pk| {
                        ...
                        key.alias = keystore.get_alias_by_address(&key.haneul_address).ok();
                        key
                    })
                    .collect::<Vec<Key>>();
                if sort_by_alias {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            HaneulKeyPair::Ed25519(kp) => kp.encode_base64(),
                            HaneulKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            HaneulKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<HaneulAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                haneul_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::haneul_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    haneul_signature: haneul_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = HaneulAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = HaneulKeyPair::decode_base64(&keypair)
            ...
            KeyToolCommand::ZkLoginInsecureSignPersonalMessage { data, max_epoch } => {
                ...
                let reader = get_proof(
                    ...
                    &kp_bigint,
                    user_salt,
                    "https://prover-dev.haneul-labs.com/v1",
                )
                .await
                .unwrap();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let pk = skp.public();
                let ephemeral_key_identifier: HaneulAddress = (&skp.public()).into();
                ...
                println!("Finish login and paste the entire URL here (e.g. https://haneul.io/#id_token=...):");
...
impl From<&HaneulKeyPair> for Key {
    fn from(skp: &HaneulKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            haneul_address: HaneulAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
        ...
        match self {
            ...
            CommandOutput::Sign(data) => {
                ...
                builder
                    ...
                    .push_record([
                        &data.haneul_signature,
                        &data.digest,
                        &data.raw_intent_msg,
                        &intent_table,
                        &data.raw_tx_data,
                        &data.haneul_address.to_string(),
                    ]);
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match HaneulKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                HaneulKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match HaneulKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => HaneulKeyPair::Ed25519(kp),
