        ...
        match prefix {
            ...
            Some(sp!(_, prefix)) => {
                ...
                match self.peek() {
                    sp!(sp, L(T::Ident, s)) => {
                        if s != "haneul" {
                            error!(sp => help: { "Expected 'haneul' extension" },
                    ...
                    sp!(sp, l) => {
                        error!(
                            sp,
                            "Expected a valid MVR domain extension ('.haneul') but got {}", l
                        )
                    }
                }
            }
...
mod tests {
    ...
    fn test_parse_types() {
        let inputs = vec![
            ...
            "haneul::object::ID",
            "0x2::object::UID",
            "3::staking_pool::StakedHaneul",
            // Generic types
            "0x2::coin::Coin<2::haneul::HANEUL>",
            "haneul::table::Table<haneul::object::ID, vector<0x1::option::Option<u32>>>",
        ];
        let mut parsed = Vec::new();
        for input in inputs {
            let mut x = shlex::split(input).unwrap();
    ...
    fn test_parse_commands() {
        let inputs = vec![
            ...
            // Move Call
            "--move-call 0x3::haneul_system::request_add_stake system coins.0 validator",
