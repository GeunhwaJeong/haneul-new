...
use crate::{
    client_commands::{dry_run_or_execute_or_serialize, Opts, OptsWithGas, HaneulClientCommandResult},
    client_ptb::{
...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        if let HaneulClientCommandResult::DryRun(response) = transaction_response {
            println!("{}", Pretty(&response));
            return Ok(());
        }

        let transaction_response =
            if let HaneulClientCommandResult::TransactionBlock(response) = transaction_response {
                response
            } else {
