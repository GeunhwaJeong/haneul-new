...
use haneul_json_rpc_types::{HaneulObjectData, HaneulObjectDataOptions, HaneulRawData};
...
use haneul_types::{
    ...
    Identifier, TypeTag, HANEUL_FRAMEWORK_PACKAGE_ID,
};
...
impl<'a> PTBBuilder<'a> {
    ...
    /// Fetch the `HaneulObjectData` for an object ID -- this is used for object resolution.
    async fn get_object(&self, object_id: ObjectID, obj_loc: Span) -> PTBResult<HaneulObjectData> {
        let res = self
            .reader
            .get_object_with_options(
                object_id,
                HaneulObjectDataOptions::new().with_type().with_owner(),
            )
            .await
