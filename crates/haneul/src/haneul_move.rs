...
impl MoveCommands {
    pub fn execute(&self, path: &Path, is_std_framework: bool) -> Result<(), anyhow::Error> {
        match self {
            Self::Build {
                ...
                if *dump_bytecode_as_base64 {
                    let compiled_modules =
                        haneul_framework::build_move_package_to_base64(path, is_std_framework)?;
            ...
            Self::Test(config) => {
                Self::build(path, is_std_framework)?;
                haneul_framework::run_move_unit_tests(path, Some(config.clone()))?;
            }
        }
        Ok(())
    }
    ...
    fn build(path: &Path, is_std_framework: bool) -> Result<(), anyhow::Error> {
        if is_std_framework {
            haneul_framework::get_haneul_framework_modules(path)?;
        } else {
            haneul_framework::build_and_verify_user_package(path)?;
        }
        Ok(())
    }
}
