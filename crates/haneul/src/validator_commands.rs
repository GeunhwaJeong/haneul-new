...
pub enum HaneulValidatorCommand {
    ...
    /// Update haneul native bridge committee node url
...
fn make_key_files(
    ...
    if file_name.exists() {
        ...
        let kp = match key {
            Some(key) => {
                println!(
                    "Generated new key file {:?} based on haneul.keystore file.",
                    file_name
                );
                key
            }
    ...
    pub async fn execute(
        ...
        Ok(match self {
            HaneulValidatorCommand::MakeValidatorInfo {
                ...
                let account_keypair: HaneulKeyPair = read_keypair_from_file(account_key_file_name)?;
                ...
                let validator_info = GenesisValidatorInfo {
                    info: haneul_genesis_builder::validator_info::ValidatorInfo {
                        ...
                        gas_price,
                        commission_rate: haneul_config::node::DEFAULT_COMMISSION_RATE,
            ...
            HaneulValidatorCommand::BecomeCandidate { file, tx_args } => {
                ...
                .await?;
                HaneulValidatorCommandResponse::BecomeCandidate {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            HaneulValidatorCommand::JoinCommittee { tx_args } => {
                let gas_budget = tx_args.gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                let (response, serialized_unsigned_transaction) = call_0x5(
                    context,
                ...
                .await?;
                HaneulValidatorCommandResponse::JoinCommittee {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            HaneulValidatorCommand::LeaveCommittee { tx_args } => {
                ...
                .await?;
                HaneulValidatorCommandResponse::LeaveCommittee {
                    response,
                    serialized_unsigned_transaction,
                }
            }
            ...
            HaneulValidatorCommand::UpdateMetadata { metadata, tx_args } => {
                let gas_budget = tx_args.gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                ...
                .await?;
                HaneulValidatorCommandResponse::UpdateMetadata {
                    response,
                    serialized_unsigned_transaction,
                }
            }
            ...
            HaneulValidatorCommand::UpdateGasPrice {
                ...
                .await?;
                HaneulValidatorCommandResponse::UpdateGasPrice {
                    response,
                    serialized_unsigned_transaction,
                }
            }
            ...
            HaneulValidatorCommand::ReportValidator {
                ...
                .await?;
                HaneulValidatorCommandResponse::ReportValidator {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            HaneulValidatorCommand::SerializePayloadForPoP {
                account_address,
                protocol_public_key,
            } => {
                ...
                let mut intent_msg_bytes = bcs::to_bytes(&IntentMessage::new(
                    Intent::haneul_app(IntentScope::ProofOfPossession),
                    msg,
                ))
            ...
            HaneulValidatorCommand::RegisterBridgeCommittee {
                ...
                println!(
                    "Starting bridge committee registration for Haneul validator: {address}, with bridge public key: {} and url: {}",
            ...
            HaneulValidatorCommand::UpdateBridgeCommitteeNodeUrl {
                bridge_authority_url,
                print_unsigned_transaction_only,
                validator_address,
                gas_budget,
                ...
                if !committee_members
                    .into_iter()
                    .any(|(_, m)| m.haneul_address == address)
                {
                    bail!("Address {} is not in the committee", address);
                }
                println!(
                    "Updating bridge committee node URL for Haneul validator: {address}, url: {}",
                    bridge_authority_url
                );
...
async fn get_cap_object_ref(
    ...
    let mut haneul_client = context.grpc_client()?;
    if let Some(operation_cap_id) = operation_cap_id {
        ...
        let resp = haneul_client.get_object(cap_object_id.parse()?).await?;
...
async fn get_pending_candidate_summary(
    ...
    for resp in pending_validators.dynamic_fields() {
        ...
        if field.verified_metadata().haneul_address == validator_address {
