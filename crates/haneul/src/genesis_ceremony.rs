...
use haneul_types::{
    ...
    crypto::{
        ...
        KeypairTraits, HaneulKeyPair, ToFromBytes,
    },
    object::Object,
};
...
pub fn run(cmd: Ceremony) -> Result<()> {
    ...
    match cmd.command {
        ...
        CeremonyCommand::AddValidator {
            ...
            let network_keypair: HaneulKeyPair = read_keypair_from_file(network_key_file)?;
        ...
        CeremonyCommand::Build => {
            ...
            println!("Successfully built {HANEUL_GENESIS_FILENAME}");
            println!(
                "{HANEUL_GENESIS_FILENAME} sha3-256: {}",
        ...
        CeremonyCommand::VerifyAndSign { key_file } => {
            ...
            println!("Successfully verified {HANEUL_GENESIS_FILENAME}");
            println!(
                "{HANEUL_GENESIS_FILENAME} sha3-256: {}",
        ...
        CeremonyCommand::Finalize => {
            let genesis = Genesis::load(dir.join(HANEUL_GENESIS_FILENAME))?;
    ...
    fn ceremony() -> Result<()> {
        ...
        let validators = (0..10)
            .map(|i| {
                ...
                write_keypair_to_file(
                    &HaneulKeyPair::Ed25519HaneulKeyPair(staking_keypair),
                    &staking_key_file,
                )
                .unwrap();
