...
async fn test_ptb_publish_and_complex_arg_resolution() -> Result<(), anyhow::Error> {
    ...
    let shared_id_str = if let HaneulClientCommandResult::Call(response) = start_call_result {
...
async fn test_move_call_args_linter_command() -> Result<(), anyhow::Error> {
    ...
    if let HaneulClientCommandResult::Call(txn_response) = result {
        assert_eq!(
...
async fn test_package_management_on_publish_command() -> Result<(), anyhow::Error> {
    ...
    let (expect_original_id, expect_version, _) =
        if let HaneulClientCommandResult::Publish(response) = resp {
            assert_eq!(
                response.effects.as_ref().unwrap().gas_object().object_id(),
                gas_obj_id
            );
...
async fn test_delete_shared_object() -> Result<(), anyhow::Error> {
    ...
    let owned_obj_ids = if let HaneulClientCommandResult::Publish(response) = resp {
        assert_eq!(
            response.effects.as_ref().unwrap().gas_object().object_id(),
            gas_obj_id
        );
    ...
    let shared_id = if let HaneulClientCommandResult::Call(response) = start_call_result {
        response.effects.unwrap().created().to_vec()[0]
            .reference
            .object_id
    } else {
    ...
    if let HaneulClientCommandResult::Call(response) = delete_result {
...
async fn test_receive_argument() -> Result<(), anyhow::Error> {
    ...
    let (parent, child) = if let HaneulClientCommandResult::Call(response) = start_call_result {
    ...
    let receive_result = HaneulClientCommands::Call {
        ...
        gas_price: None,
        args: vec![
            HaneulJsonValue::from_str(&parent.object_id.to_string()).unwrap(),
            HaneulJsonValue::from_str(&child.object_id.to_string()).unwrap(),
        ],
    }
    .execute(context)
    .await?;

    if let HaneulClientCommandResult::Call(response) = receive_result {
        assert!(response.effects.unwrap().into_status().is_ok());
    } else {
        unreachable!("Invalid response");
    };
...
async fn test_package_management_on_upgrade_command() -> Result<(), anyhow::Error> {
    ...
    let HaneulClientCommandResult::Publish(publish_response) = resp else {
        unreachable!("Invalid response");
    };

    let HaneulTransactionBlockEffects::V1(effects) = publish_response.clone().effects.unwrap();
    ...
    // Now run the upgrade
    let upgrade_response = HaneulClientCommands::Upgrade {
        package_path: upgrade_pkg_path,
        upgrade_capability: cap.reference.object_id,
    ...
    let (expect_upgrade_latest_id, expect_upgrade_version, _) =
        if let HaneulClientCommandResult::Upgrade(response) = upgrade_response {
            assert_eq!(
                response.effects.as_ref().unwrap().gas_object().object_id(),
                gas_obj_id
            );
...
async fn test_linter_suppression_stats() -> Result<(), anyhow::Error> {
    ...
    let output = cmd
        .args(&args)
        .output()
        .expect("failed to run 'haneul move test'");
...
async fn test_pay() -> Result<(), anyhow::Error> {
    ...
    if let HaneulClientCommandResult::Pay(response) = pay {
        // check tx status
...
async fn test_pay_haneul() -> Result<(), anyhow::Error> {
    ...
    if let HaneulClientCommandResult::PayHaneul(response) = pay_haneul {
        assert!(response.status_ok().unwrap());
        // check gas coin used
        assert_eq!(
            response.effects.as_ref().unwrap().gas_object().object_id(),
...
async fn test_pay_all_haneul() -> Result<(), anyhow::Error> {
    ...
    // and if the gas object used was the first object in the input coins
    if let HaneulClientCommandResult::PayAllHaneul(response) = pay_all_haneul {
        let objs_refs = client
            .read_api()
            .get_owned_objects(
                address2,
...
async fn test_transfer() -> Result<(), anyhow::Error> {
    ...
    if let HaneulClientCommandResult::Transfer(response) = transfer {
        assert!(response.status_ok().unwrap());
        assert_eq!(
            response.effects.as_ref().unwrap().gas_object().object_id(),
            object_id2
...
async fn test_transfer_haneul() -> Result<(), anyhow::Error> {
    ...
    // is correct
    if let HaneulClientCommandResult::TransferHaneul(response) = transfer_haneul {
        assert!(response.status_ok().unwrap());
        assert_eq!(
            response.effects.as_ref().unwrap().gas_object().object_id(),
            object_id1
