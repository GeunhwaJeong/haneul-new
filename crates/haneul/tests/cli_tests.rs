...
async fn test_ptb_publish_and_complex_arg_resolution() -> Result<(), anyhow::Error> {
    ...
    let HaneulClientCommandResult::TransactionBlock(ref response) = resp else {
        unreachable!("Invalid response");
    };

    let HaneulTransactionBlockEffects::V1(effects) = response.effects.as_ref().unwrap();
...
async fn test_receive_argument() -> Result<(), anyhow::Error> {
    ...
    let owned_obj_ids = if let HaneulClientCommandResult::TransactionBlock(ref response) = resp {
        assert_eq!(
            response.effects.as_ref().unwrap().gas_object().object_id(),
            gas_obj_id
        );
...
async fn test_receive_argument_by_immut_ref() -> Result<(), anyhow::Error> {
    ...
    let (parent, child) =
        if let HaneulClientCommandResult::TransactionBlock(ref response) = start_call_result {
