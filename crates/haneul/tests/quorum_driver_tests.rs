...
use haneul_types::messages::{QuorumDriverRequest, QuorumDriverResponse, VerifiedTransaction};
...
async fn test_retry_on_object_locked() -> Result<(), anyhow::Error> {
    ...
    match res {
        ...
        Err(HaneulError::QuorumFailedToProcessTransactionWithConflictingTransactionRetried {conflicting_tx_digest, conflicting_tx_retry_success}) => {
        ...
        Err(HaneulError::QuorumFailedToProcessTransaction {..}) => (),
        _ => panic!("expect Err(HaneulError::QuorumFailedToProcessTransactionWithConflictingTransactionRetried) or QuorumFailedToProcessTransaction but got {:?}", res),
    }
    ...
    if let Err(HaneulError::QuorumFailedToProcessTransactionWithConflictingTransactionRetried {
    ...
    }) = res
    {
        ...
        panic!("expect Err(HaneulError::QuorumFailedToProcessTransactionWithConflictingTransactionRetried) but got {:?}", res)
    }
    ...
    let res = quorum_driver
        ...
        .await;
    if !matches!(res, Err(HaneulError::QuorumFailedToProcessTransaction { .. })) {
        panic!(
            "expect Err(HaneulError::QuorumFailedToProcessTransaction) but got {:?}",
            res
        )
    }
...
fn make_tx(gas: &Object, sender: HaneulAddress, keypair: &AccountKeyPair) -> VerifiedTransaction {
