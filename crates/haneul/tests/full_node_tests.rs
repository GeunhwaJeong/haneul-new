...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    let haneul_node = start_a_fullnode(&test_cluster.swarm, false).await?;
    let node = if cfg!(msim) {
        &haneul_node
    } else {
        &test_cluster.fullnode_handle.as_ref().unwrap().haneul_node
    };
...
async fn test_full_node_transaction_streaming_basic() -> Result<(), anyhow::Error> {
    ...
    assert!(full_node.haneul_node.state().transaction_streamer.is_none());
