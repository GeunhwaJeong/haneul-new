...
async fn get_account_and_objects(
    context: &mut WalletContext,
) -> Result<(HaneulAddress, Vec<HaneulObjectInfo>), anyhow::Error> {
    let sender = context.config.accounts.get(0).cloned().unwrap();
    let object_refs = context.gateway.get_objects_owned_by_address(sender).await?;
...
async fn emit_move_events(
    ...
    let (object_id, digest) = if let WalletCommandResult::CreateExampleNFT(HaneulObjectRead::Exists(
        obj,
    )) = res
    {
...
async fn publish_basics_package(context: &WalletContext, sender: HaneulAddress) -> ObjectRef {
    ...
    let transaction = {
        ...
        let modules = haneul_framework::build_move_package(&path, build_config, false).unwrap();
...
async fn set_up_subscription(port: u16, swarm: &Swarm) -> Result<(HaneulNode, Client), anyhow::Error> {
...
/// Pass in an unique port for each test case otherwise they may interfere with one another.
async fn set_up_jsonrpc(port: u16, swarm: &Swarm) -> Result<(HaneulNode, HttpClient), anyhow::Error> {
...
async fn test_full_node_sub_to_move_event_ok() -> Result<(), anyhow::Error> {
    ...
    match timeout(Duration::from_secs(5), sub.next()).await {
        Ok(Some(Ok(HaneulEventEnvelope {
            ...
            assert_eq!(
                fields,
                HaneulMoveStruct::WithFields(BTreeMap::from([
...
async fn test_full_node_event_read_api_ok() -> Result<(), anyhow::Error> {
    ...
    let response: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByOwner", params)
        .await
        .unwrap();
