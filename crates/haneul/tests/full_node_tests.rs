...
async fn test_full_node_indexes() -> Result<(), anyhow::Error> {
    ...
    let expected_event = HaneulEvent::TransferObject {
...
async fn set_up_subscription(swarm: &Swarm) -> Result<(HaneulNode, Client), anyhow::Error> {
...
async fn set_up_jsonrpc(
...
) -> Result<(HaneulNode, HttpClient, HaneulClient), anyhow::Error> {
    let port = get_available_port();
    ...
    let haneul_client = ClientType::RPC(url, None);
    // Check url is valid
    let haneul_client = haneul_client.init().await?;
    Ok((node, http_client, haneul_client))
}
...
async fn test_full_node_transaction_streaming_basic() -> Result<(), anyhow::Error> {
    ...
    let mut sub: Subscription<HaneulTransactionResponse> = ws_client
        .subscribe(
            "haneul_subscribeTransaction",
            rpc_params![HaneulTransactionFilter::Any],
            "haneul_unsubscribeTransaction",
        )
        .await
        .unwrap();
...
async fn test_full_node_sub_and_query_move_event_ok() -> Result<(), anyhow::Error> {
    ...
    let expected_event = HaneulEvent::MoveEvent {
        ...
        sender,
        type_: haneul_framework_address_concat_string("::devnet_nft::MintNFTEvent"),
        fields: None,
        bcs,
    };
...
async fn test_full_node_event_read_api_ok() -> Result<(), anyhow::Error> {
    ...
    let events_by_sender: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsBySender", params)
        .await
        .unwrap();
    assert_eq!(events_by_sender.len(), 1);
    assert_eq!(events_by_sender[0].event, expected_event);
    ...
    let events_by_tx: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByTransaction", params)
        .await
        .unwrap();
    assert_eq!(events_by_tx.len(), 1);
    assert_eq!(events_by_tx[0].event, expected_event);
    ...
    let events_by_recipient: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByRecipient", params)
        .await
        .unwrap();
    assert_eq!(events_by_recipient.len(), 1);
    assert_eq!(events_by_recipient[0].event, expected_event);
    ...
    let events_by_object: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByObject", params)
        .await
        .unwrap();
    assert_eq!(events_by_object.len(), 1);
    assert_eq!(events_by_object[0].event, expected_event);
    ...
    let events_by_module: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByModule", params)
        .await
        .unwrap();
    assert_eq!(events_by_module.len(), 1);
    assert_eq!(events_by_module[0].event, expected_event);
    ...
    let events_by_sender: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByMoveEventStructName", params)
        .await
        .unwrap();
    assert_eq!(events_by_sender.len(), 1);
    ...
    let all_events: Vec<HaneulEventEnvelope> = jsonrpc_client
        .request("haneul_getEventsByTimeRange", params)
        .await
        .unwrap();
...
async fn test_validator_node_has_no_transaction_orchestrator() {
    ...
    let node = HaneulNode::start(validator_config, Registry::new())
        .await
        .unwrap();
...
async fn test_full_node_transaction_orchestrator_rpc_ok() -> Result<(), anyhow::Error> {
    ...
    let response: HaneulExecuteTransactionResponse = jsonrpc_client
    ...
    let _response: HaneulTransactionResponse = jsonrpc_client
        .request("haneul_getTransaction", rpc_params![*tx_digest])
        .await
        .unwrap();
    ...
    if let HaneulExecuteTransactionResponse::ImmediateReturn {
...
async fn test_get_objects_read() -> Result<(), anyhow::Error> {
    ...
    let (node, _jsonrpc_client, _haneul_client) = set_up_jsonrpc(&swarm, None).await?;
    ...
    assert_eq!(resp.effects.status, HaneulExecutionStatus::Success);
