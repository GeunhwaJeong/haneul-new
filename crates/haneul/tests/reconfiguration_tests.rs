...
use haneul_core::test_utils::init_local_authorities;
...
async fn reconfig_end_to_end_tests() {
    ...
    let (framework_object_ref, old_committee_size) = handles[0]
        .with_async(|node| async move {
            ...
            let haneul_system_state = state.get_haneul_system_state_object().await.unwrap();
    ...
    handles[0]
        .with_async(|node| async move {
            let haneul_system_state = node.state().get_haneul_system_state_object().await.unwrap();
