...
async fn reconfig_end_to_end_tests() {
    ...
    for validator in configs.validator_configs() {
        let node = HaneulNode::start(validator, Registry::new()).await.unwrap();
...
async fn fast_forward_to_ready_for_reconfig_start(nodes: &[HaneulNode]) {
    let mut checkpoint_processes = vec![];
    for node in nodes {
        let active = node.active().clone();
        let handle = tokio::spawn(async move {
...
async fn fast_forward_to_ready_for_reconfig_finish(nodes: &[HaneulNode]) {
    let mut checkpoint_processes = vec![];
    for node in nodes {
        let active = node.active().clone();
        let handle = tokio::spawn(async move {
