...
use haneul_types::effects::{CertifiedTransactionEffects, TransactionEffectsAPI};
...
async fn test_transaction_expiration() {
    ...
    let mut data = TransactionData::new_transfer_haneul(
        sender,
        sender,
        Some(1),
...
async fn reconfig_with_revert_end_to_end_test() {
    ...
    let rgp = authorities
        .get(0)
        .unwrap()
        .with(|haneul_node| haneul_node.state().reference_gas_price_for_testing())
        .unwrap();
...
async fn test_create_advance_epoch_tx_race() {
    ...
    let register_wait = |failpoint, node_id, tx: Arc<broadcast::Sender<()>>| {
        let node = haneul_simulator::task::NodeId(node_id);
        register_fail_point_async(failpoint, move || {
            let cur_node = haneul_simulator::current_simnode_id();
            let tx = tx.clone();
            async move {
                if cur_node == node {
...
async fn test_validator_candidate_pool_read() {
    ...
    let new_validator_address: HaneulAddress = new_validator_key.public().into();
    ...
    let new_validator = new_configs
        ...
        .into_iter()
        .find(|v| {
            let name: AuthorityName = v.verified_metadata().haneul_pubkey_bytes();
    ...
    authorities[0].with(|node| {
        let system_state = node
            ...
            .unwrap();
        let system_state_summary = system_state.clone().into_haneul_system_state_summary();
        ...
        .unwrap();
        assert_eq!(validator.haneul_address, new_validator_address);
    });
}
...
async fn test_inactive_validator_pool_read() {
    ...
    let address: HaneulAddress = leaving_validator_account_key.public().into();
    ...
    let staking_pool_id = authorities[0].with(|node| {
        node.state()
            ...
            .active_validators
            .iter()
            .find(|v| v.haneul_address == address)
            .unwrap()
            .staking_pool_id
    });
    authorities[0].with(|node| {
        ...
        .unwrap();
        assert_eq!(validator.haneul_address, address);
    });
...
async fn test_reconfig_with_committee_change_basic() {
    ...
    authorities[0].with(|node| {
        ...
        assert_eq!(
            pending_active_validators[0].haneul_address,
            new_validator_address
        );
    });
...
async fn test_reconfig_with_committee_change_stress() {
    ...
    let address_key_mapping: BTreeMap<HaneulAddress, Ed25519KeyPair> = all_validator_keys
        .iter()
        .map(|key| (key.public().into(), key.copy()))
        .collect();

    let object_set: HashMap<HaneulAddress, (Vec<Object>, Object)> = all_validator_keys
        .iter()
        .map(|key| {
            let sender: HaneulAddress = key.public().into();
            let gas_objects =
    ...
    let mut object_map: HashMap<HaneulAddress, (Vec<ObjectRef>, ObjectRef)> = object_set
        .into_iter()
    ...
    let validator_superset_mapping = validator_superset
        ...
        .map(|v| {
            let name: AuthorityName = v.verified_metadata().haneul_pubkey_bytes();
            (name, v.verified_metadata().haneul_address)
        })
        .collect::<HashMap<_, _>>();
    ...
    let initial_pubkeys: Vec<_> = initial_network
        ...
        .validator_set_for_tooling()
        .iter()
        .map(|v| v.verified_metadata().haneul_pubkey_bytes())
        .collect();
    let mut standby_nodes: Vec<_> = validator_superset
        ...
        .map(|val| {
            let node_config = validator_superset
                ...
                .iter()
                .find(|config| {
                    config.protocol_public_key() == val.verified_metadata().haneul_pubkey_bytes()
                })
                .unwrap();
            (val, node_config)
        })
        .filter(|(val, _node)| {
            !initial_pubkeys.contains(&val.verified_metadata().haneul_pubkey_bytes())
        })
        .collect();
    ...
    while !standby_nodes.is_empty() {
        ...
        for (validator, node_config) in joining_validators.clone() {
            let sender = validator.verified_metadata().haneul_address;
        ...
        let joined_auths: Vec<_> = joining_validators
            .iter()
            .map(|(val, _node)| val.verified_metadata().haneul_pubkey_bytes())
            .collect();
...
async fn safe_mode_reconfig_test() {
    use haneul_types::haneul_system_state::advance_epoch_result_injection;
...
async fn execute_join_committee_txes(
    ...
    assert_eq!(node_config.protocol_public_key(), val.haneul_pubkey_bytes());
...
async fn execute_leave_committee_tx(
    ...
    let haneul_address: HaneulAddress = account_kp.public().into();
    let rgp = authorities[0]
        .with(|node| node.state().reference_gas_price_for_testing())
        .unwrap();
    let tx_data = TransactionData::new_move_call(
...
async fn trigger_reconfiguration(authorities: &[HaneulNodeHandle]) {
