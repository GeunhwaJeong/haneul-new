...
# Test haneul move build --dump with --pubfile-path for ephemeral publication
#
...
haneul move --client.config "$CONFIG" build -p main_pkg --dump --pubfile-path Pub.test.toml -e testnet --no-tree-shaking > output.json
...
haneul move disassemble main.mv 2>&1 | grep -q "cafe0001" && echo "PASS: ephemeral original-id cafe0001 found in disassembly"
