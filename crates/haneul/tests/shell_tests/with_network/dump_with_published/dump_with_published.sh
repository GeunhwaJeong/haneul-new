...
# Test haneul move build --dump with Published.toml (non-ephemeral publication)
#
...
haneul move --client.config "$CONFIG" build -p main_pkg --dump -e testnet --no-tree-shaking > output.json

# Extract the base64 module, decode to .mv file, and disassemble
cat output.json | sed 's/.*"modules":\["\([^"]*\)".*/\1/' | base64 -d > main.mv
haneul move disassemble main.mv 2>&1 | grep -q "dead0001" && echo "PASS: published original-id dead0001 found in disassembly"
