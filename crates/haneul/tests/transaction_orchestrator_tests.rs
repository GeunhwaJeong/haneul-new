...
async fn test_blocking_execution() -> Result<(), anyhow::Error> {
    ...
    let handle = &test_cluster.fullnode_handle.haneul_node;
...
async fn test_transaction_orchestrator_reconfig() {
    ...
    let epoch = test_cluster.fullnode_handle.haneul_node.with(|node| {
    ...
    assert_eq!(
        test_cluster.fullnode_handle.haneul_node.with(|node| node
