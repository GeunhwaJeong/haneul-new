...
async fn test_blocking_execution() -> Result<(), anyhow::Error> {
    ...
    let node = &test_cluster.fullnode_handle.as_ref().unwrap().haneul_node;
...
async fn test_local_execution_with_missing_parents() -> Result<(), anyhow::Error> {
    ...
    let node = &fullnode_handle.haneul_node;
    let wallet_context_node = &test_cluster.fullnode_handle.as_ref().unwrap().haneul_node;
...
async fn node_knows_txes(node: &HaneulNode, digests: &Vec<TransactionDigest>) {
    for digest in digests {
        assert!(node.state().get_transaction(*digest).await.is_ok());
    }
}

async fn node_does_not_know_txes(node: &HaneulNode, digests: &Vec<TransactionDigest>) {
    for digest in digests {
        assert!(node.state().get_transaction(*digest).await.is_err());
    }
}
