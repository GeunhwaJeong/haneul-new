    ...
    async fn get_framework_upgrade_effects(
        ...
        let effects = node_handle
            .with_async(|node| async {
                ...
                let framework = db.get_object(&HANEUL_FRAMEWORK_OBJECT_ID);
        ...
        let modified_at = effects
            ...
            .iter()
            .find_map(|(id, v)| (id == &HANEUL_FRAMEWORK_OBJECT_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &HANEUL_FRAMEWORK_OBJECT_ID).then_some(*v));
