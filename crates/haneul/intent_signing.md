...
Intent Signing is a standard used in Haneul that defines _what_ the signature is committed to. We call the data that the signature commits to is an intent message. All signatures in Haneul MUST commit to an intent message, instead of the message itself.

## Motivation

Previously, Haneul uses a special `Signable` trait, which attaches the Rust struct name as a prefix before the serialized data. This is not ideal because:
...
To see detailed definition for each field, see each enum definition [here](https://github.com/GeunhwaJeong/haneul/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/haneul-types/src/intent.rs).
...
Authority signature is created using the protocol key. The data that it commits to is also an intent message `intent || message`. See all available intent scopes [here](https://github.com/GeunhwaJeong/haneul/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/haneul-types/src/intent.rs#L66)

# Implementation

1. [Struct and enum definitions](https://github.com/GeunhwaJeong/haneul/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/haneul-types/src/intent.rs)
2. [Test](https://github.com/GeunhwaJeong/haneul/blob/d009e82fa35bda4f2b3e7a86a9529d36c32a8159/crates/haneul-types/src/unit_tests/intent_tests.rs)
3. User transaction intent signing [PR 1](https://github.com/GeunhwaJeong/haneul/pull/6445), [PR 2](https://github.com/GeunhwaJeong/haneul/pull/8321)
4. Authority intent signing [PR 1](https://github.com/GeunhwaJeong/haneul/pull/8154), [PR 2](https://github.com/GeunhwaJeong/haneul/pull/8726)
