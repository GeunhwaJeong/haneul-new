    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                let timeout_secs: u64 = std::env::var("HANEUL_SIM_TEST_TIMEOUT_SECS")
                    .ok()
                ...
                ::haneul_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    haneul_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "HaneulNode leak detected"
                );
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl haneul_enum_compat_util::EnumOrderMap for #name {
