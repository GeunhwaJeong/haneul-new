...
use haneulns_indexer::indexer::HaneulnsIndexer;
...
/// Checkpoint 22279496: Replaces the name added on `22279365` (new.test.haneul) by removing it and then adding it as a node name.
/// Checkpoint 22279944: Adds `remove.test.haneul`.
/// Checkpoint 22280030: Adds `remove.test.haneul` as a replacement (the previous one expired!).
...
fn process_22279187_checkpoint() {
    ...
    assert!(names.contains(&"leaf.test.haneul".to_string()));
    assert!(names.contains(&"node.test.haneul".to_string()));
    assert!(names.contains(&"test.haneul".to_string()));
}
...
fn process_22279365_checkpoint() {
    ...
    assert_eq!(addition.name, "new.test.haneul".to_string());
    assert_eq!(addition.parent, "test.haneul".to_string());
    assert_eq!(
        addition.nft_id,
...
fn process_22279944_checkpoint() {
    ...
    let addition = updates.first().unwrap();
    assert_eq!(addition.name, "remove.test.haneul".to_string());
    assert_eq!(
        addition.nft_id,
...
/// Return a haneulns_indexer instance for testing.
/// Uses the testnet demo we used to extract the checkpoints being processed.
fn get_test_indexer() -> HaneulnsIndexer {
    HaneulnsIndexer::new(
