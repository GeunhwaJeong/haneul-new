...
pub struct HaneulnsIndexer {
    registry_table_id: HaneulAddress,
    subdomain_wrapper_type: StructTag,
}

impl std::default::Default for HaneulnsIndexer {
    fn default() -> Self {
        Self::new(
...
impl HaneulnsIndexer {
    ...
    pub fn new(registry_address: String, wrapper_type: String) -> Self {
        let registry_table_id = HaneulAddress::from_str(&registry_address).unwrap();
    ...
    pub fn process_checkpoint(&self, data: &CheckpointData) -> (Vec<VerifiedDomain>, Vec<String>) {
        let mut checkpoint = HaneulnsIndexerCheckpoint::new(data.checkpoint_summary.sequence_number);
...
pub struct HaneulnsIndexerCheckpoint {
...
impl HaneulnsIndexerCheckpoint {
    ...
    pub fn parse_record_changes(&mut self, config: &HaneulnsIndexer, objects: &[Object]) {
...
/// Allows us to format a HaneulNS specific query for updating the DB entries
