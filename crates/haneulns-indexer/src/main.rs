...
use dotenvy::dotenv;
use haneullabs_service::metrics::start_basic_prometheus_server;
...
use haneulns_indexer::{
    get_connection_pool,
    indexer::{format_update_field_query, format_update_subdomain_wrapper_query, HaneulnsIndexer},
    models::VerifiedDomain,
    schema::domains,
    PgConnectionPool,
};

struct HaneulnsIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: HaneulnsIndexer,
}

impl HaneulnsIndexerWorker {
...
#[async_trait]
impl Worker for HaneulnsIndexerWorker {
    type Result = ();
...
async fn main() -> Result<()> {
    let _guard = haneullabs_service::logging::init();
    dotenv().ok();
    ...
    let indexer_setup =
        ...
        {
            HaneulnsIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            HaneulnsIndexer::default()
        };

    let worker_pool = WorkerPool::new(
        HaneulnsIndexerWorker {
            pg_pool: get_connection_pool().await,
            indexer: indexer_setup,
        },
        "haneulns_indexing".to_string(), /* task name used as a key in the progress store */
        100,                          /* concurrency */
    );
