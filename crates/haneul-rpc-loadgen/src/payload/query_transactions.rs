...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<HaneulTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| HaneulTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<HaneulTransactionBlockResponse, TransactionDigest>> {
