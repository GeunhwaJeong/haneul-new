...
use haneul_json_rpc_types::{
    BigInt, CheckpointId, ObjectChange, Page, HaneulObjectDataOptions, HaneulTransactionResponse,
...
use crate::payload::{
    Command, CommandData, DryRun, GetCheckpoints, PayHaneul, Payload, ProcessPayload, Processor,
...
async fn get_coin_ids(
    ...
    let keypair = HaneulKeyPair::decode_base64(&signer_info.encoded_keypair)
...
) -> Result<Page<HaneulTransactionResponse, TransactionDigest>> {
    let transactions = client
        .read_api()
...
impl<'a> ProcessPayload<'a, &'a QueryTransactions> for RpcCommandProcessor {
    async fn process(
        ...
        let query = HaneulTransactionResponseQuery {
            filter,
...
impl<'a> ProcessPayload<'a, &'a PayHaneul> for RpcCommandProcessor {
    async fn process(&'a self, _op: &'a PayHaneul, signer_info: &Option<SignerInfo>) -> Result<()> {
        let clients = self.get_clients().await?;
        let SignerInfo {
            encoded_keypair,
            signer_address,
