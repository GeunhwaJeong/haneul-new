...
use haneul_types::crypto::{get_key_pair, AccountKeyPair, EncodeDecodeBase64, Signature, HaneulKeyPair};
...
pub struct RpcCommandProcessor {
    clients: Arc<RwLock<Vec<HaneulClient>>>,
    ...
    addresses: Arc<DashSet<HaneulAddress>>,
    data_dir: String,
}
...
impl RpcCommandProcessor {
    ...
    pub(crate) async fn get_clients(&self) -> Result<Vec<HaneulClient>> {
    ...
    pub(crate) fn add_addresses_from_response(&self, responses: Vec<HaneulTransactionBlockResponse>) {
    ...
    pub(crate) fn dump_cache_to_file(&self) {
        ...
        let addresses: Vec<HaneulAddress> = self.addresses.iter().map(|x| *x).collect();
        write_data_to_file(
            &addresses,
            &format!("{}/{}", &self.data_dir, CacheType::HaneulAddress),
        )
        .unwrap();
    }
}
...
impl fmt::Display for CacheType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CacheType::HaneulAddress => write!(f, "HaneulAddress"),
...
async fn prepare_new_signer_and_coins(
    ...
    let (gas_fee_for_split, gas_fee_for_pay_haneul) = (
        DEFAULT_LARGE_GAS_BUDGET * num_split_txns as u64,
        DEFAULT_GAS_BUDGET,
    );

    let primary_keypair = HaneulKeyPair::decode_base64(&signer_info.encoded_keypair)
        .expect("Decoding keypair should not fail");
    let sender = HaneulAddress::from(&primary_keypair.public());
    let (coin, balance) = get_coin_with_max_balance(client, sender).await;
    // The balance needs to cover `pay_amount` plus
    // 1. gas fee for pay_haneul from the primary address to the burner address
    // 2. gas fee for splitting the primary coin into `num_coins`
    let required_balance = pay_amount + gas_fee_for_split + gas_fee_for_pay_haneul;
    if required_balance > balance {
        panic!("Current balance {balance} is smaller than require amount of GEUNHWA to fund the operation {required_balance}");
    }
    ...
    let burner_keypair = HaneulKeyPair::Ed25519(burner_keypair);
    ...
    let coins = get_haneul_coin_ids(client, burner_address).await;
...
async fn get_haneul_coin_ids(client: &HaneulClient, address: HaneulAddress) -> Vec<(ObjectID, u64)> {
        ...
        .await
    {
        Ok(page) => page
            ...
            .collect::<Vec<_>>(),
        Err(e) => {
            panic!("get_haneul_coin_ids error for address {address} {e}")
        }
    }
...
async fn pay_haneul(
    ...
    let tx = client
        ...
        .await
        .expect("Failed to construct pay haneul transaction");
    sign_and_execute(
        client,
        keypair,
        tx,
...
pub(crate) async fn sign_and_execute(
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::haneul_transaction(), &txn_data),
        keypair,
    );
