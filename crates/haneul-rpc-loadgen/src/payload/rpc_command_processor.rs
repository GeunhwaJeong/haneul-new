...
async fn prepare_new_signer_and_coins(
    ...
    let amount_for_primary_coin =
        ...
        // 1. gas fee(i.e., `DEFAULT_GAS_BUDGET`) for pay_haneul from the primary address to the burner address
...
async fn get_coin_with_balance(client: &HaneulClient, address: HaneulAddress, balance: u64) -> ObjectID {
    let coins = get_haneul_coin_ids(client, address).await;
    assert!(!coins.is_empty());
