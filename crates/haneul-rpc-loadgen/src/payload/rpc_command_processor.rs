...
use haneul_json_rpc_types::{CheckpointId, HaneulTransactionResponseOptions};
...
use crate::payload::{
    CommandData, DryRun, GetCheckpoints, MultiGetTransactions, PayHaneul, Payload, ProcessPayload,
    Processor,
};
...
impl RpcCommandProcessor {
    ...
    async fn process_command_data(
        ...
        match command {
            ...
            CommandData::PayHaneul(ref v) => self.process(v, keypair).await,
        }
    }

    async fn get_clients(&self) -> Result<Vec<HaneulClient>> {
    ...
    async fn apply(&self, payload: &Payload) -> Result<()> {
        ...
        for command in commands.iter() {
            ...
            let keypair = &payload
                .encoded_keypair
                .as_ref()
                .map(|k| HaneulKeyPair::decode_base64(k).expect("Decoding keypair should not fail"));
...
impl<'a> ProcessPayload<'a, &'a DryRun> for RpcCommandProcessor {
    async fn process(&'a self, _op: &'a DryRun, _keypair: &Option<HaneulKeyPair>) -> Result<()> {
        debug!("DryRun");
        Ok(())
    }
}
...
impl<'a> ProcessPayload<'a, &'a PayHaneul> for RpcCommandProcessor {
    async fn process(&'a self, _op: &'a PayHaneul, keypair: &Option<HaneulKeyPair>) -> Result<()> {
        let clients = self.get_clients().await?;

        // TODO(chris): allow customization and clean up this function
        let sender: HaneulAddress =
        ...
        debug!("Pay Haneul to {recipient} with {amount} GEUNHWA with {gas_object_id}");
        let keypair = keypair.as_ref().unwrap();
        for client in clients.iter() {
            let transfer_tx = client
                .transaction_builder()
                .transfer_haneul(sender, gas_object_id, gas_budget, recipient, Some(amount))
                .await?;
