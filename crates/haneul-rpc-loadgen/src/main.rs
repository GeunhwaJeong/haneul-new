...
fn get_keypair() -> Result<(HaneulAddress, String)> {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let log_filename = format!("haneul-rpc-loadgen.{}.log", uuid);
    ...
    let (command, common) = match opts.command {
        ...
        ClapCommand::PayHaneul { common } => (Command::new_pay_haneul(), common),
