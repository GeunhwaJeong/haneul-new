...
#[derive(Parser)]
#[clap(
    name = "Haneul RPC Load Generator",
    version = "0.1",
    about = "A load test application for Haneul RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.haneul/haneul_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.haneul/loadgen/data")]
...
fn get_haneul_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".haneul").join("haneul_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("haneul-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayHaneul { common } => (Command::new_pay_haneul(), common, true),
