...
use haneul_config::{local_ip_utils, Config};
use haneul_genesis_builder::validator_info::{GenesisValidatorInfo, ValidatorInfo};
use haneul_types::base_types::HaneulAddress;
use haneul_types::crypto::{
    generate_proof_of_possession, get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair,
    AuthorityPublicKeyBytes, NetworkKeyPair, NetworkPublicKey, PublicKey, HaneulKeyPair,
};
...
fn default_haneul_key_pair() -> HaneulKeyPair {
    HaneulKeyPair::Ed25519(get_key_pair_from_rng(&mut rand::rngs::OsRng).1)
}
...
impl GenesisConfig {
    ...
    pub fn benchmark_gas_keys(n: usize) -> Vec<HaneulKeyPair> {
        let mut rng = StdRng::seed_from_u64(Self::BENCHMARKS_RNG_SEED);
        (0..n)
            .map(|_| HaneulKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng)))
            .collect()
    }
