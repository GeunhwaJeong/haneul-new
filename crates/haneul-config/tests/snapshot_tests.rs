...
// This file contains tests that detect changes in Haneul configs.
...
// 1. Run `cargo insta test --review` under `./haneul-config`.
...
use haneul_config::genesis::GenesisCeremonyParameters;
...
fn populated_genesis_snapshot_matches() {
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.haneul_system_wrapper_object());
    assert_yaml_snapshot!(genesis.haneul_system_object());
