...
impl From<GenesisValidatorInfo> for GenesisValidatorMetadata {
    fn from(
        ...
        Self {
            ...
            haneul_address: info.account_address,
...
impl GenesisCeremonyParameters {
    ...
    fn default_initial_stake_subsidy_distribution_amount() -> u64 {
        // 1M Haneul
        1_000_000 * haneul_types::gas_coin::GEUNHWA_PER_HANEUL
    }
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.haneul_address()),
                )
            };
    ...
    fn validate_inputs(&self) -> Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.haneul_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_haneul_object_id = staked_haneul_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                        ...
                        *owner == allocation.recipient_address
                            && s.principal() == allocation.amount_geunhwa
                ...
                let gas_object_id = gas_objects
                    ...
                    .find(|(_k, (o, g))| {
                        if let Owner::AddressOwner(owner) = &o.owner {
                            *owner == allocation.recipient_address
                                && g.value() == allocation.amount_geunhwa
                        } else {
                            false
                        }
                    })
                ...
                assert_eq!(gas_object.1.value(), allocation.amount_geunhwa,);
            }
        }
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.haneul_address),
    );
...
fn create_genesis_transaction(
    ...
    let (effects, events, objects) = {
        let mut store = haneul_types::in_memory_storage::InMemoryStorage::new(Vec::new());
        ...
        let native_functions = haneul_framework::natives::all_natives(/* silent */ true);
        ...
        let (inner_temp_store, effects, _execution_error) =
            haneul_adapter::execution_engine::execute_transaction_to_effects::<
        ...
        let objects = inner_temp_store
            ...
            .into_iter()
            .map(|(_, (_, o, kind))| {
                assert_eq!(kind, haneul_types::storage::WriteKind::Create);
                o
            })
            .collect();
...
fn process_package(
    ...
    let mut gas_status = HaneulGasStatus::new_unmetered(protocol_config);
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the HaneulSystemState UID
        let haneul_system_state_uid = builder.programmable_move_call(
            HANEUL_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("haneul_system_state").to_owned(),
            vec![],
            vec![],
        );
    ...
    programmable_transactions::execution::execute::<_, execution_mode::Genesis>(
        ...
        &mut temporary_store,
        genesis_ctx,
        &mut HaneulGasStatus::new_unmetered(&protocol_config),
        None,
        pt,
    )?;
...
pub struct TokenDistributionSchedule {
    pub stake_subsidy_fund_geunhwa: u64,
    pub allocations: Vec<TokenAllocation>,
}

impl TokenDistributionSchedule {
    pub fn validate(&self) {
        let mut total_geunhwa = self.stake_subsidy_fund_geunhwa;

        for allocation in &self.allocations {
            total_geunhwa += allocation.amount_geunhwa;
        }

        if total_geunhwa != TOTAL_SUPPLY_GEUNHWA {
            panic!("TokenDistributionSchedule adds up to {total_geunhwa} and not expected {TOTAL_SUPPLY_GEUNHWA}");
        }
    }

    fn check_all_stake_operations_are_for_valid_validators<I: IntoIterator<Item = HaneulAddress>>(
        &self,
        validators: I,
    ) {
        use std::collections::HashMap;

        let mut validators: HashMap<HaneulAddress, u64> =
    ...
    fn new_for_validators_with_default_allocation<I: IntoIterator<Item = HaneulAddress>>(
        validators: I,
    ) -> Self {
        ...
        let schedule = Self {
            stake_subsidy_fund_geunhwa: supply,
            allocations,
        };
    ...
    ///
    /// All entries in a token distribution schedule must add up to 10B Haneul.
    pub fn from_csv<R: std::io::Read>(reader: R) -> Result<Self> {
        ...
        assert_eq!(
            TOTAL_SUPPLY_GEUNHWA,
            allocations.iter().map(|a| a.amount_geunhwa).sum::<u64>(),
            "Token Distribution Schedule must add up to 10B Haneul",
        );
        ...
        let schedule = Self {
            stake_subsidy_fund_geunhwa: stake_subsidy_fund_allocation.amount_geunhwa,
            allocations,
        };
    ...
    pub fn to_csv<W: std::io::Write>(&self, writer: W) -> Result<()> {
        ...
        writer.serialize(TokenAllocation {
            recipient_address: HaneulAddress::default(),
            amount_geunhwa: self.stake_subsidy_fund_geunhwa,
...
pub struct TokenAllocation {
    pub recipient_address: HaneulAddress,
    pub amount_geunhwa: u64,
