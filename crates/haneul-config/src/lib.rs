...
use haneul_types::base_types::{encode_bytes_hex, ObjectID, HaneulAddress, TxContext};
use haneul_types::committee::{Committee, EpochId};
use haneul_types::crypto::{get_key_pair, KeyPair, PublicKeyBytes};
...
impl GenesisConfig {
    ...
    pub fn generate_custom_move_modules(
        ...
        let mut genesis_ctx =
            haneul_adapter::genesis::get_genesis_context_with_custom_address(&address);
        ...
        if !self.move_packages.is_empty() {
            ...
            for path in &self.move_packages {
                let mut modules = haneul_framework::build_move_package(
                    path,
                ...
                let package_id =
                    haneul_adapter::adapter::generate_package_id(&mut modules, &mut genesis_ctx)?;
