...
pub struct NodeConfig {
    ...
    pub rpc: Option<haneul_rpc_api::Config>,
    ...
    /// In a `haneul-node` binary, this is set to SupportedProtocolVersions::SYSTEM_DEFAULT
    /// in haneul-node/src/main.rs. It is present in the config so that it can be changed by tests in
...
impl ExecutionCacheConfig {
    pub fn max_cache_size(&self) -> u64 {
        std::env::var("HANEUL_MAX_CACHE_SIZE")
            .ok()
    ...
    pub fn package_cache_size(&self) -> u64 {
        std::env::var("HANEUL_PACKAGE_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_cache_size(&self) -> u64 {
        std::env::var("HANEUL_OBJECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn marker_cache_size(&self) -> u64 {
        std::env::var("HANEUL_MARKER_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_by_id_cache_size(&self) -> u64 {
        std::env::var("HANEUL_OBJECT_BY_ID_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_cache_size(&self) -> u64 {
        std::env::var("HANEUL_TRANSACTION_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn executed_effect_cache_size(&self) -> u64 {
        std::env::var("HANEUL_EXECUTED_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn effect_cache_size(&self) -> u64 {
        std::env::var("HANEUL_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn events_cache_size(&self) -> u64 {
        std::env::var("HANEUL_EVENTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_objects_cache_size(&self) -> u64 {
        std::env::var("HANEUL_TRANSACTION_OBJECTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold(&self) -> u64 {
        std::env::var("HANEUL_BACKPRESSURE_THRESHOLD")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold_for_rpc(&self) -> u64 {
        std::env::var("HANEUL_BACKPRESSURE_THRESHOLD_FOR_RPC")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
...
pub fn default_zklogin_oauth_providers() -> BTreeMap<Chain, BTreeSet<String>> {
    ...
    let experimental_providers = BTreeSet::from([
        ...
        "Onefc".to_string(),
        "FanTV".to_string(),
        "AwsTenant-region:us-east-1-tenant_id:us-east-1_LPSLCkC3A".to_string(), // test tenant in haneullabs aws
...
impl NodeConfig {
    ...
    pub fn rpc(&self) -> Option<&haneul_rpc_api::Config> {
        self.rpc.as_ref()
    }
}
...
pub struct ExpensiveSafetyCheckConfig {
    ...
    /// rebate of all live objects, and that the total HANEUL in the network remains
    /// the same.
    #[serde(default)]
    ...
    #[serde(default)]
    force_disable_epoch_haneul_conservation_check: bool,
...
impl ExpensiveSafetyCheckConfig {
    pub fn new_enable_all() -> Self {
        Self {
            enable_epoch_haneul_conservation_check: true,
            enable_deep_per_tx_haneul_conservation_check: true,
            force_disable_epoch_haneul_conservation_check: false,
    ...
    pub fn new_disable_all() -> Self {
        Self {
            enable_epoch_haneul_conservation_check: false,
            enable_deep_per_tx_haneul_conservation_check: false,
            force_disable_epoch_haneul_conservation_check: true,
    ...
    pub fn force_disable_epoch_haneul_conservation_check(&mut self) {
        self.force_disable_epoch_haneul_conservation_check = true;
    }

    pub fn enable_epoch_haneul_conservation_check(&self) -> bool {
        (self.enable_epoch_haneul_conservation_check || cfg!(debug_assertions))
            && !self.force_disable_epoch_haneul_conservation_check
    }
    ...
    pub fn enable_deep_per_tx_haneul_conservation_check(&self) -> bool {
        self.enable_deep_per_tx_haneul_conservation_check || cfg!(debug_assertions)
    }
...
mod tests {
    ...
    use haneul_types::crypto::{get_key_pair_from_rng, AuthorityKeyPair, NetworkKeyPair, HaneulKeyPair};
    ...
    fn legacy_validator_config() {
        const FILE: &str = include_str!("../data/haneul-node-legacy.yaml");
