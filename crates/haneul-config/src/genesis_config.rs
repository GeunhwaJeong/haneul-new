...
impl ValidatorGenesisInfo {
    ...
    pub fn from_localhost_for_testing(
        ...
        account_key_pair: HaneulKeyPair,
    ...
    pub fn from_base_ip(
        ...
        ip: String,
        // Port offset allows running many HaneulNodes inside the same simulator node, which is
...
pub struct AccountConfig {
    #[serde(
        ...
        serialize_with = "HaneulAddress::optional_address_as_hex",
        deserialize_with = "HaneulAddress::optional_address_from_hex"
    )]
...
impl GenesisConfig {
    ...
    pub fn for_local_testing_with_addresses(addresses: Vec<HaneulAddress>) -> Self {
            ...
            .map(|ip| {
                ValidatorConfigInfo {
                    ...
                    genesis_info: ValidatorGenesisInfo::from_base_ip(
                        ...
                        HaneulKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng)), // account_key_pair
    ...
    pub fn benchmark_gas_key() -> HaneulKeyPair {
        let mut rng = StdRng::seed_from_u64(Self::BENCHMARKS_RNG_SEED);
        HaneulKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng))
    }
