...
use haneul_types::{
    ...
    messages::ExecutionFailureStatus, haneul_system_state::HANEUL_SYSTEM_MODULE_NAME,
...
const HANEUL_CLOCK_CREATE: FunctionIdent = (
    &HANEUL_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from haneul::{}::{}. \
                Or for tests, it can come from haneul::{}::{}",
