        ...
        for (_block_id, BlockInvariant { post, .. }) in inv_map {
            match post {
                BlockPostcondition::Error(error) => {
                    debug_assert_eq!(
                        error.kind(),
                        &ExecutionErrorKind::HaneulMoveVerificationError,
...
/// Certain Haneul Framework functions can safely take a `Info` by value
...
fn expect_ok<T>(res: Result<T, PartialVMError>) -> Result<T, ExecutionError> {
    match res {
        ...
        Err(partial_vm_error) => {
            ...
            Err(ExecutionErrorKind::HaneulMoveVerificationError.into())
        }
    }
}
