...
//! Objects whose struct type has key ability represent Haneul objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Haneul objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Haneul object. From there, we track the flow of the value and make
...
fn verify_id_leak(module: &CompiledModule) -> HaneulResult {
...
impl<'a> TransferFunctions for IDLeakAnalysis<'a> {
    ...
    type AnalysisError = HaneulError;
...
/// Certain Haneul Framework functions can safely take a `VersionedID` by value
fn is_call_safe_to_leak(verifier: &IDLeakAnalysis, function_handle: &FunctionHandle) -> bool {
    ...
    let is_framework =
        verifier.binary_view.address_identifier_at(m.address) == &HANEUL_FRAMEWORK_ADDRESS;
    if !is_framework {
        return false;
    }
...
fn call(verifier: &mut IDLeakAnalysis, function_handle: &FunctionHandle) -> HaneulResult {
