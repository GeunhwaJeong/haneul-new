...
use haneul_types::{error::ExecutionError, id::OBJECT_MODULE_NAME, HANEUL_FRAMEWORK_ADDRESS};
...
/// Certain Haneul Framework functions can safely take a `ID` by value
...
fn move_verification_error(msg: impl std::fmt::Display) -> PartialVMError {
    PartialVMError::new(StatusCode::UNKNOWN_VERIFICATION_ERROR)
        .with_message(format!("Haneul Move Bytecode Verification Error: {}", msg))
}
