...
pub fn verify_module(module: &CompiledModule) -> Result<(), ExecutionError> {
    // In Haneul's framework code there is an exception to the characteristic type rule - we have a HANEUL
    // type in the haneul module but it is instantiated and the module has no initializer (the reason
    // for it is that the HANEUL coin is only instantiated during genesis). It is easiest to simply
