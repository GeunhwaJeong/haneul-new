...
//! - The first field has type `haneul::object::Info`
    ...
    for def in struct_defs {
        ...
        let info_field_type = match info_field_type {
            ...
            _ => {
                return Err(verification_failure(format!(
                    ...
                    {:?} type found",
                    name, HANEUL_FRAMEWORK_ADDRESS, info_field_type
                )))
            }
        };
        // check that the struct type for "info" field must be HANEUL_FRAMEWORK_ADDRESS::object::Info.
        ...
        fp_ensure!(
            info_type_struct_name == INFO_STRUCT_NAME
                && info_type_module_address == &HANEUL_FRAMEWORK_ADDRESS
