    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            match addr_module(view, mhandle) {
                (HANEUL_FRAMEWORK_ADDRESS, "transfer") => verify_private_transfer(
                    view,
                    function_type_parameters,
                    fhandle,
                    type_arguments,
                )?,
                (HANEUL_FRAMEWORK_ADDRESS, "event") => {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (HANEUL_FRAMEWORK_ADDRESS, "transfer") {
        return Ok(());
    }
