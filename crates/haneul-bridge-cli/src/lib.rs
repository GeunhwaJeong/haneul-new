    ...
    pub async fn handle(
        ...
        match self {
            ...
            BridgeClientCommands::ClaimOnEth { seq_num, dry_run } => {
                claim_on_eth(seq_num, config, haneul_bridge_client, dry_run)
                    .await
                    .map_err(|e| anyhow!("{:?}", e))
            }
...
async fn deposit_on_haneul(
    ...
    info!(?tx_digest, "Sending deposit transaction to Haneul.");
...
async fn claim_on_eth(
    ...
    let haneul_chain_id = haneul_bridge_client.get_bridge_summary().await?.chain_id;
    ...
    if dry_run {
        ...
        println!(
            "Haneul to Eth bridge transfer claim dry run result: {:?}",
            resp
        );
    } else {
        let eth_claim_tx_receipt = config
            ...
            .await?;
        println!(
            "Haneul to Eth bridge transfer claimed: {:?}",
