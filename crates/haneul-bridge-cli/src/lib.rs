...
pub enum BridgeCommand {
    ...
    /// View current status of Haneul bridge
    #[clap(name = "view-haneul-bridge")]
    ViewHaneulBridge {
        #[clap(long = "haneul-rpc-url")]
        haneul_rpc_url: String,
...
pub struct BridgeCliConfig {
    ...
    /// Proxy address for HaneulBridge deployed on Eth
    ...
    /// At leaset one of `haneul_key_path` or `eth_key_path` must be provided.
    /// If only one is provided, it will be used for both Haneul and Eth.
    pub haneul_key_path: Option<PathBuf>,
    /// See `haneul_key_path`. Must be Secp256k1 key.
...
impl LoadedBridgeCliConfig {
    pub async fn load(cli_config: BridgeCliConfig) -> anyhow::Result<Self> {
        if cli_config.eth_key_path.is_none() && cli_config.haneul_key_path.is_none() {
            return Err(anyhow!(
                "At least one of `haneul_key_path` or `eth_key_path` must be provided"
            ));
        }
        let haneul_key = if let Some(haneul_key_path) = &cli_config.haneul_key_path {
            Some(read_key(haneul_key_path, false)?)
        } else {
            None
        };
        ...
        let haneul_bridge = EthHaneulBridge::new(
        ...
        let haneul_address = HaneulAddress::from(&haneul_key.public());
        println!("Using Haneul address: {:?}", haneul_address);
        println!("Using Eth address: {:?}", eth_address);
        println!("Using Eth chain: {:?}", eth_chain_id);

        Ok(Self {
            haneul_rpc_url: cli_config.haneul_rpc_url,
...
impl LoadedBridgeCliConfig {
    ...
    pub async fn get_haneul_account_info(
...
pub enum BridgeClientCommands {
    ...
    /// Claim bridged tokens on Eth for a Haneul→ETH transfer.
    ...
    /// Claim bridged tokens on Haneul for an ETH→Haneul transfer that has been approved but not yet claimed.
    /// Auto-detects V1/V2 from the on-chain record (V2 messages >48h old bypass the rate limiter).
    #[clap(name = "claim-on-haneul")]
    ClaimOnHaneul {
        /// The bridge sequence number of the ETH→Haneul transfer
        #[clap(long)]
        seq_num: u64,
...
impl BridgeClientCommands {
    pub async fn handle(
        ...
        match self {
            ...
            BridgeClientCommands::DepositOnHaneul {
                coin_object_id,
                coin_type,
                target_chain,
                recipient_address,
            ...
            BridgeClientCommands::ClaimOnEth { seq_num, dry_run } => {
                claim_on_eth(seq_num, config, haneul_bridge_client, dry_run).await
            }
            BridgeClientCommands::ClaimOnHaneul {
                seq_num,
                source_chain,
                dry_run,
...
async fn deposit_native_ether_on_eth(
    ...
    let pending_tx = match version {
        BridgeVersion::V2 => {
            eth_haneul_bridge
                .bridgeETHV2(haneul_recipient_address.to_vec().into(), target_chain)
                .value(amount)
                .send()
                .await?
        }
        BridgeVersion::V1 => {
            eth_haneul_bridge
                .bridgeETH(haneul_recipient_address.to_vec().into(), target_chain)
                .value(amount)
                .send()
                .await?
        }
...
async fn deposit_on_haneul(
    ...
    info!(
        ?tx_digest,
        "Sending deposit transaction ({version:?}) to Haneul."
    );
    ...
    match &resp.status {
        ...
        haneul_json_rpc_types::HaneulExecutionStatus::Failure { error } => Err(anyhow!(
...
async fn claim_on_eth(
    ...
    let haneul_chain_id = haneul_bridge_client
        .get_bridge_summary()
        .await
        .map_err(|e| anyhow!("{:?}", e))?
        .chain_id;
    let parsed_message = haneul_bridge_client
        .get_parsed_token_transfer_message(haneul_chain_id, seq_num)
        .await
        .map_err(|e| anyhow!("{:?}", e))?;
    if parsed_message.is_none() {
        println!("No record found for seq_num: {seq_num}, chain id: {haneul_chain_id}");
        return Ok(());
    }
    ...
    let sigs = haneul_bridge_client
        ...
        .await;
    if sigs.is_none() {
        println!("No signatures found for seq_num: {seq_num}, chain id: {haneul_chain_id}");
        return Ok(());
    }
    let signatures = sigs
        .unwrap()
    ...
    let message = eth_haneul_bridge::BridgeUtils::Message::from(parsed_message);
    ...
    if dry_run {
        ...
        println!(
            "Haneul to Eth bridge transfer ({version_label}) claim dry run result: {:?}",
            resp
        );
    } else {
        let eth_claim_tx_receipt = config
            ...
            .await?;
        println!(
            "Haneul to Eth bridge transfer ({version_label}) claimed: {:?}",
...
async fn claim_on_haneul(
    ...
    let id_token_map = haneul_bridge_client
        .get_token_id_map()
        .await
        .map_err(|e| anyhow!("{:?}", e))?;
    ...
    let bridge_object_arg = haneul_bridge_client
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let (haneul_key, sender, gas_obj_ref) = config.get_haneul_account_info().await?;
    ...
    if dry_run {
        ...
        println!(
            "Claim on Haneul ({version_label}) dry run result for seq_num {seq_num}, source chain {source_chain}: {:?}",
            resp
        );
    } else {
        let sig = Signature::new_secure(
        ...
        info!(
            ?tx_digest,
            "Sending claim_and_transfer_token ({version_label}) transaction to Haneul for seq_num {seq_num}, source chain {source_chain}."
        );
        let resp = haneul_bridge_client
            .execute_transaction_block_with_effects(signed_tx)
            .await
        ...
        match &resp.status {
            haneul_json_rpc_types::HaneulExecutionStatus::Success => {
                ...
                println!(
                    "Successfully claimed ({version_label}) tokens on Haneul for seq_num: {seq_num}, source chain: {source_chain}"
                );
            }
