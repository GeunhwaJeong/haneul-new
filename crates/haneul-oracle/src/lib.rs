...
async fn get_gas_obj_ref(
    ...
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, HaneulObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    ) -> anyhow::Result<HaneulTransactionBlockEffects> {
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<HaneulTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(haneul_sdk::error::Error::RpcError(err)) => {
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, HaneulObjectDataOptions::default())
                    .await
                {
                    Ok(HaneulObjectResponse {
                        data: Some(_data), ..
                    }) => {
