...
module test::test_module {
    ...
    public fun simple_fx_ptb(single_data: Option<Data<DecimalValue>>, geunhwa_amount: u64, ctx: &mut TxContext) {
        ...
        let amount = geunhwa_amount * value;
        let usd = MockUSD {
            id: object::new(ctx),
            amount,
            decimals,
    ...
    public fun simple_fx(oracle: &SimpleOracle, geunhwa_amount: u64, ctx: &mut TxContext) {
        let single_data = simple_oracle::get_latest_data<DecimalValue>(oracle, string::utf8(b"HANEULUSD"));
        let single_data = option::destroy_some(single_data);
        let value = data::value(&single_data);
        let decimals = decimal_value::decimal(value);
        let value = decimal_value::value(value);
