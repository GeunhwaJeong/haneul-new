...
use haneul_json_rpc_types::{ObjectChange, HaneulExecutionStatus};
...
async fn test_consume_oracle_data() {
    ...
    builder.programmable_move_call(
        ...
        simple_fx,
        vec![],
        vec![simple_oracle, geunhwa_amount],
    );
    ...
    builder.programmable_move_call(
        ...
        trusted_fx,
        vec![],
        vec![oracles[0], oracles[1], oracles[2], geunhwa_amount],
    );
...
async fn get_gas(client: &HaneulClient, sender: HaneulAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (HaneulClient, Keystore, HaneulAddress) {
    let client = HaneulClientBuilder::default()
        .build("https://rpc.devnet.haneul.io:443")
        .await
        .unwrap();
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::new(
            &dirs::home_dir()
                .unwrap()
                .join(".haneul/haneul_config/haneul.keystore"),
        )
        .unwrap(),
    );
    let sender: HaneulAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
