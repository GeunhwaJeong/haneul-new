...
use haneul_indexer_alt_schema::checkpoints::StoredCpDigest;
use haneul_indexer_alt_schema::schema::cp_digests;
            ...
            match self.get_checkpoint(request).await {
                Ok(response) => {
                    ...
                    let signature: AuthorityQuorumSignInfo<true> = {
                        let sdk_sig = haneul_sdk_types::ValidatorAggregatedSignature::try_from(
