...
use haneul_sdk2::types::{ObjectId, StructTag};
use haneul_types::haneul_sdk2_conversions::struct_tag_sdk_to_core;
...
pub async fn get_coin_info(
    ...
    let haneul_types::storage::CoinInfo {
    ...
    let treasury = if let Some(treasury_object_id) = treasury_object_id {
        state
            ...
            .map(haneul_types::coin::TreasuryCap::try_from)
            .transpose()
            .map_err(|_| {
                RestError::new(
                    axum::http::StatusCode::INTERNAL_SERVER_ERROR,
...
impl From<haneul_types::coin::CoinMetadata> for CoinMetadata {
    fn from(value: haneul_types::coin::CoinMetadata) -> Self {
        Self {
...
impl CoinTreasury {
    const HANEUL: Self = Self {
        id: None,
        total_supply: haneul_types::gas_coin::TOTAL_SUPPLY_GEUNHWA,
    };
}
