...
pub use haneul_types::full_checkpoint_content::{CheckpointData, CheckpointTransaction};
...
impl<T: serde::Serialize, C: std::fmt::Display> axum::response::IntoResponse for Page<T, C> {
    fn into_response(self) -> axum::response::Response {
        let cursor = self
            .cursor
            .map(|cursor| [(crate::types::X_HANEUL_CURSOR, cursor.to_string())]);
...
impl RestService {
    ...
    pub fn chain_id(&self) -> haneul_types::digests::ChainIdentifier {
        self.chain_id
    }
...
fn info(version: &'static str) -> openapiv3::v3_1::Info {
    ...
    openapiv3::v3_1::Info {
        title: "Haneul Node Api".to_owned(),
        description: Some("REST Api for interacting with the Haneul Blockchain".to_owned()),
        contact: Some(Contact {
            name: Some("Haneul Labs".to_owned()),
            url: Some("https://github.com/GeunhwaJeong/haneul".to_owned()),
            ..Default::default()
        }),
