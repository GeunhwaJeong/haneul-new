...
use crate::types::X_HANEUL_CURSOR;
use crate::types::X_HANEUL_EPOCH;
use crate::types::X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT;
use crate::types::X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS;
use crate::types::X_HANEUL_TIMESTAMP_MS;
...
impl Client {
    ...
    pub async fn get_full_checkpoint(
        ...
        //     .await?
        //     // TODO make this more efficient and convert directly into the haneul-sdk-types version
        //     .try_map(|proto| {
        //         haneul_types::full_checkpoint_content::CheckpointData::try_from(proto)
...
impl ResponseParts {
    fn from_reqwest_response(response: &reqwest::Response) -> Self {
        ...
        let checkpoint_height = headers
            .get(X_HANEUL_CHECKPOINT_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let timestamp_ms = headers
            .get(X_HANEUL_TIMESTAMP_MS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let lowest_available_checkpoint = headers
            .get(X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let lowest_available_checkpoint_objects = headers
            .get(X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let cursor = headers
            .get(X_HANEUL_CURSOR)
        ...
        let min_supported_protocol_version = headers
            .get(X_HANEUL_MIN_SUPPORTED_PROTOCOL_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let max_supported_protocol_version = headers
            .get(X_HANEUL_MAX_SUPPORTED_PROTOCOL_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
