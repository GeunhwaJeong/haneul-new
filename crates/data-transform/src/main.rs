...
impl GrootModuleResolver {
    fn new(blocking_cp: PgConnectionPool) -> Self {
        let original = IndexerStorePackageResolver::new(blocking_cp);
...
impl ModuleResolver for GrootModuleResolver {
    ...
    fn get_module(&self, id: &ModuleId) -> Result<Option<Vec<u8>>, Self::Error> {
        ...
        if is_deepbook(address) {
            ...
            Err(IndexerError::DataTransformationError(
                "get_module has been deprecated, use package_resolver instead".to_string(),
            ))
        }
    }
}
        ...
        match event {
            Ok(Some(event)) => {
                ...
                */
                ...
                let type_ = parse_haneul_struct_tag(&event.event_type).expect("cannot load StructTag");
                ...
                match layout {
                    Ok(l) => {
                        ...
                        match move_object {
                            Ok(m) => {
                                let parsed_json = HaneulMoveStruct::from(m).to_json_value();
                                let final_result =
