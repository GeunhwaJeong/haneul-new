...
//! On-disk binary format readers for Haneul formal snapshots.
//!
//! Ported essentially verbatim from
//! `haneul-indexer-alt-consistent-store::restore::format`. The
//! formats themselves are produced by the validator's snapshot
//! tool and are documented in `haneul-storage`. This module only
