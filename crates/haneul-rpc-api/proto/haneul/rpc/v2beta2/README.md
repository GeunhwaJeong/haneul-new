## Overview
This package defines a number of APIs for interacting with services in the Haneul
...
- `x-haneul-chain-id`: Chain Id of the current chain
- `x-haneul-chain`: Human-readable name of the current chain
- `x-haneul-checkpoint-height`: Current checkpoint height
- `x-haneul-lowest-available-checkpoint`: Lowest available checkpoint for which
...
- `x-haneul-lowest-available-checkpoint-objects`: Lowest available checkpoint for
...
- `x-haneul-epoch`: Current epoch of the chain.
- `x-haneul-timestamp-ms`: Current timestamp of the chain, represented as number
  of milliseconds from the Unix epoch.
- `x-haneul-timestamp`: Current timestamp of the chain, encoded in the
