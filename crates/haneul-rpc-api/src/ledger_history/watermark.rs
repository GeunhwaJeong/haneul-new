...
//!
//! Both ledger-history backends — the fullnode (`haneul-rpc-api`) and bigtable
//! (`haneul-kv-rpc`) — and all three list handlers (`list_transactions`,
...
use haneul_rpc_cursor::{CursorToken, Position};
...
mod tests {
    ...
    fn options(ascending: bool) -> QueryOptions {
        ...
        request.ordering = Some(if ascending {
            haneul_rpc::proto::haneul::rpc::v2alpha::Ordering::Ascending as i32
        } else {
