...
use haneul_inverted_index::ANY_PACKAGE_VALUE;
use haneul_inverted_index::BitmapLiteral;
use haneul_inverted_index::BitmapQuery;
use haneul_inverted_index::BitmapTerm;
use haneul_inverted_index::EVENT_EXTANT_VALUE;
use haneul_inverted_index::IndexDimension;
use haneul_inverted_index::TX_UNIVERSE_VALUE;
use haneul_inverted_index::emit_module_value;
use haneul_inverted_index::encode_dimension_key;
use haneul_inverted_index::event_type_value;
...
fn convert_event_type(f: &EventTypeFilter, field: &str) -> Result<Vec<u8>, RpcError> {
    ...
    let instantiation_bcs = if generics.is_some() {
        let tag = haneul_types::parse_haneul_type_tag(s).map_err(|e| {
            FieldViolation::new(field)
                .with_description(format!("invalid type: {e}"))
                .with_reason(ErrorReason::FieldInvalid)
        })?;
        let haneul_types::TypeTag::Struct(st) = tag else {
            return Err(FieldViolation::new(field)
...
mod tests {
    ...
    fn tx_sender_literal(byte: u8, exclude: bool) -> TransactionLiteral {
        let address = HaneulAddress::from_bytes([byte; 32])
    ...
    fn event_stream_head_filter_encodes_dimension_key() {
        let bytes = [7; 32];
        let stream_id = HaneulAddress::from_bytes(bytes)
            .expect("valid address bytes")
            .to_string();
