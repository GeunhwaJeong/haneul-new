    ...
    pub fn apply_cursor_bounds(&self, resolved: ResolvedRange) -> ResolvedRange {
        ...
        if let Some(cursor) = &self.after {
            ...
            let Some(after) = (match cursor.kind {
                haneul_rpc_cursor::CursorKind::Item => position.checked_add(1),
                haneul_rpc_cursor::CursorKind::Boundary => Some(position),
            }) else {
    ...
    pub fn apply_event_cursor_bounds(&self, resolved: ResolvedEventRange) -> ResolvedEventRange {
        ...
        if let Some(cursor) = &self.after {
            ...
            let candidate = match cursor.kind {
                haneul_rpc_cursor::CursorKind::Item => Bound::Excluded(position),
                haneul_rpc_cursor::CursorKind::Boundary => Bound::Included(position),
            };
    ...
    pub fn resolve(self, options: &QueryOptions) -> ResolvedCheckpointRange {
        ...
        if let Some(cursor) = &options.before
            && let Some(upper) = match cursor.kind {
                haneul_rpc_cursor::CursorKind::Item => cursor.position.checkpoint().checked_add(1),
                haneul_rpc_cursor::CursorKind::Boundary => Some(cursor.position.checkpoint()),
            }
            && upper <= end
        {
            end = upper;
