...
pub(crate) fn checkpoint_data_to_full_checkpoint_response(
    haneul_types::full_checkpoint_content::CheckpointData {
        checkpoint_summary,
        checkpoint_contents,
        transactions,
    }: haneul_types::full_checkpoint_content::CheckpointData,
    ...
    GetFullCheckpointResponse {
        ...
        summary: read_mask
            .contains("summary")
            .then(|| haneul_sdk_types::CheckpointSummary::try_from(summary))
            .transpose()?
            .map(Into::into),
        summary_bcs,
        ...
        signature: read_mask
            .contains("signature")
            .then(|| haneul_sdk_types::ValidatorAggregatedSignature::from(signature).into()),
        contents: read_mask
            .contains("contents")
            .then(|| haneul_sdk_types::CheckpointContents::try_from(checkpoint_contents))
            .transpose()?
            .map(Into::into),
        contents_bcs,
...
fn transaction_to_checkpoint_transaction(
    haneul_types::full_checkpoint_content::CheckpointTransaction {
        transaction,
        effects,
        events,
        input_objects,
    ...
    let digest = read_mask
        .contains("digest")
        .then(|| haneul_sdk_types::TransactionDigest::from(transaction.digest().to_owned()).into());
    ...
    let transaction = read_mask
        ...
        .then(|| haneul_sdk_types::Transaction::try_from(transaction))
        .transpose()?
        .map(Into::into);
    ...
    let effects = read_mask
        .contains("effects")
        .then(|| haneul_sdk_types::TransactionEffects::try_from(effects))
        .transpose()?
        .map(Into::into);
    ...
    let events = read_mask
        .contains("events")
        .then(|| events.map(haneul_sdk_types::TransactionEvents::try_from))
        .flatten()
        .transpose()?
        .map(Into::into);
...
fn object_to_object_response(
    ...
    let object_id = read_mask
        .contains("object_id")
        .then(|| haneul_sdk_types::ObjectId::from(object.id()).into());
    let version = read_mask
        .contains("version")
        .then(|| object.version().value());
    let digest = read_mask
        .contains("digest")
        .then(|| haneul_sdk_types::ObjectDigest::from(object.digest()).into());
    ...
    let object = read_mask
        .contains("object")
        .then(|| haneul_sdk_types::Object::try_from(object))
        .transpose()?
        .map(Into::into);
