...
use haneul_types::error::ErrorCategory;
use tonic::Code;
...
impl From<haneul_types::storage::error::Error> for RpcError {
    fn from(value: haneul_types::storage::error::Error) -> Self {
        use haneul_types::storage::error::Kind;
...
impl From<haneul_types::haneul_sdk_types_conversions::SdkTypeConversionError> for RpcError {
            ...
            InvalidUserSignature(err) => {
                let message = {
                    let err = match err.as_inner() {
                        HaneulErrorKind::UserInputError { error } => error.to_string(),
                        _ => err.to_string(),
                    };
...
impl ObjectNotFoundError {
    pub fn new(object_id: haneul_sdk_types::Address) -> Self {
        Self {
            object_id,
            version: None,
        }
    ...
    pub fn new_with_version(
        object_id: haneul_sdk_types::Address,
        version: haneul_sdk_types::Version,
    ) -> Self {
        Self {
            object_id,
...
impl CheckpointNotFoundError {
    ...
    pub fn digest(digest: haneul_sdk_types::Digest) -> Self {
        Self {
...
pub struct EpochNotFoundError {
    epoch: haneul_sdk_types::EpochId,
}

impl EpochNotFoundError {
    pub fn new(epoch: haneul_sdk_types::EpochId) -> Self {
        Self { epoch }
    }
}
