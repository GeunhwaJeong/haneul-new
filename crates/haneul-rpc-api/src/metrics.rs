...
impl RpcMetricsMakeCallbackHandler {
    ...
    /// `/haneul.rpc.v2.LedgerService/{*rest}`) rather than the per-method path,
...
mod tests {
    ...
    fn parses_method_paths_from_file_descriptor_sets() {
        ...
        let alpha = fds("haneul.rpc.alpha", &[("EventService", &["Subscribe"])]);

        let paths = grpc_method_paths_from_file_descriptor_sets(&[&v2, &alpha]).unwrap();

        assert_eq!(paths.len(), 3);
        assert!(paths.contains("/haneul.rpc.v2.LedgerService/GetCheckpoint"));
        assert!(paths.contains("/haneul.rpc.v2.LedgerService/GetTransaction"));
        assert!(paths.contains("/haneul.rpc.alpha.EventService/Subscribe"));
    }
    ...
    fn known_grpc_method_uses_uri_path_label() {
        ...
        allowlist.insert("/haneul.rpc.v2.LedgerService/GetCheckpoint".to_owned());

        let label = compute_metric_label(
            true,
            "/haneul.rpc.v2.LedgerService/GetCheckpoint",
            Some("/haneul.rpc.v2.LedgerService/{*rest}"),
            &allowlist,
        );
        assert_eq!(label, "/haneul.rpc.v2.LedgerService/GetCheckpoint");
    }

    #[test]
    fn known_grpc_method_without_matched_path_uses_uri_path_label() {
        let mut allowlist = HashSet::new();
        allowlist.insert("/haneul.rpc.v2alpha.LedgerService/ListTransactions".to_owned());
        ...
        assert_eq!(label, "/haneul.rpc.v2alpha.LedgerService/ListTransactions");
    }
    ...
    fn unknown_grpc_method_falls_back_to_route_pattern() {
        ...
        let label = compute_metric_label(
            true,
            "/haneul.rpc.v2.LedgerService/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
            Some("/haneul.rpc.v2.LedgerService/{*rest}"),
            &allowlist,
        );
        assert_eq!(label, "/haneul.rpc.v2.LedgerService/{*rest}");
    }
