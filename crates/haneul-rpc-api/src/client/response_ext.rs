...
/// Extension trait used to facilitate retrieval of Haneul specific data from responses
...
impl ResponseExt for tonic::metadata::MetadataMap {
    fn chain_id(&self) -> Option<CheckpointDigest> {
        self.get(crate::types::X_HANEUL_CHAIN_ID)
            .map(tonic::metadata::MetadataValue::as_bytes)
            .and_then(|s| CheckpointDigest::from_base58(s).ok())
    }

    fn chain(&self) -> Option<&str> {
        self.get(crate::types::X_HANEUL_CHAIN)
            .and_then(|h| h.to_str().ok())
    }

    fn epoch(&self) -> Option<u64> {
        self.get(crate::types::X_HANEUL_EPOCH)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn checkpoint_height(&self) -> Option<u64> {
        self.get(crate::types::X_HANEUL_CHECKPOINT_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn timestamp_ms(&self) -> Option<u64> {
        self.get(crate::types::X_HANEUL_TIMESTAMP_MS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn lowest_available_checkpoint(&self) -> Option<u64> {
        self.get(crate::types::X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn lowest_available_checkpoint_objects(&self) -> Option<u64> {
        self.get(crate::types::X_HANEUL_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }
}
