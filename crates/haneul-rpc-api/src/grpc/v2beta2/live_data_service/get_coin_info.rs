...
use haneul_rpc::proto::haneul::rpc::v2beta2::coin_metadata::MetadataCapState as ProtoMetadataCapState;
use haneul_rpc::proto::haneul::rpc::v2beta2::coin_treasury::SupplyState as RpcSupplyState;
use haneul_rpc::proto::haneul::rpc::v2beta2::regulated_coin_metadata::CoinRegulatedState as ProtoCoinRegulatedState;
...
use haneul_types::derived_object;
use haneul_types::haneul_sdk_types_conversions::struct_tag_sdk_to_core;
use haneul_types::{TypeTag, HANEUL_COIN_REGISTRY_OBJECT_ID};

const HANEUL_COIN_TREASURY: CoinTreasury = {
    let mut treasury = CoinTreasury::const_default();
    treasury.total_supply = Some(haneul_types::gas_coin::TOTAL_SUPPLY_GEUNHWA);
...
fn get_coin_info_from_registry(
    ...
    let treasury = if haneul_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(HANEUL_COIN_TREASURY)
    } else {
...
fn get_coin_info_from_index(
    service: &RpcService,
    indexes: &dyn haneul_types::storage::RpcIndexes,
    coin_type: &StructTag,
    core_coin_type: &move_core_types::language_storage::StructTag,
) -> Result<Option<GetCoinInfoResponse>> {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_info.coin_metadata_object_id {
        service
            ...
            .map(haneul_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
    ...
    } else if haneul_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(HANEUL_COIN_TREASURY)
    } else {
        None
    };
        ...
        coin_info.regulated_coin_metadata_object_id
    {
        service
            ...
            .map(haneul_types::coin::RegulatedCoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
...
fn get_treasury_cap_info(
    service: &RpcService,
    treasury_object_id: HaneulObjectID,
) -> Option<CoinTreasury> {
    let obj = service.reader.inner().get_object(&treasury_object_id)?;

    // Treasury caps owned by 0x0 indicate fixed supply
    let supply_state = if obj.owner == haneul_types::object::Owner::AddressOwner(HaneulAddress::ZERO) {
        RpcSupplyState::Fixed
    } else {
        RpcSupplyState::Unknown
    };

    haneul_types::coin::TreasuryCap::try_from(obj)
        .ok()
        .map(|treasury| {
