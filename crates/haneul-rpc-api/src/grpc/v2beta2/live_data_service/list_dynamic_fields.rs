...
use haneul_rpc::proto::haneul::rpc::v2beta2::ErrorReason;
...
fn load_dynamic_field(
    ...
    if read_mask.contains(DynamicField::NAME_TYPE_FIELD) {
        message.name_type =
            Some(haneul_types::TypeTag::from(field.name_layout).to_canonical_string(true));
    }
    ...
    match field.value_metadata()? {
        ...
        DFV::ValueMetadata::DynamicObjectField(object_id) => {
                ...
                || read_mask.contains(DynamicField::OBJECT_FIELD)
            {
                ...
                let type_tag =
                    haneul_types::TypeTag::from(object.struct_tag().ok_or_else(|| {
