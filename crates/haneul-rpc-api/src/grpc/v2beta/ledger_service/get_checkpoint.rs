...
pub fn get_checkpoint(
    ...
    let haneul_sdk_types::SignedCheckpointSummary {
        checkpoint: summary,
        signature,
        ...
        || read_mask.contains(Checkpoint::TRANSACTIONS_FIELD.name)
    {
        ...
        if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
            checkpoint.merge(
                haneul_sdk_types::CheckpointContents::try_from(core_contents.clone())?,
                &read_mask,
            );
        }
...
pub(crate) fn checkpoint_data_to_checkpoint_proto(
    checkpoint_data: haneul_types::full_checkpoint_content::CheckpointData,
    ...
    if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
        checkpoint.merge(
            haneul_sdk_types::CheckpointContents::try_from(checkpoint_data.checkpoint_contents)?,
            read_mask,
        );
    }
...
fn core_transaction_to_executed_transaction_proto(
    ...
    let digest = read_mask
        ...
        .then(|| {
            haneul_sdk_types::TransactionDigest::from(transaction.digest().to_owned()).to_string()
        });
    ...
    let transaction = read_mask
        ...
        .map(|mask| {
            haneul_sdk_types::Transaction::try_from(transaction_data)
...
fn core_object_to_object_proto(
    ...
    let object = haneul_sdk_types::Object::try_from(object)?;
