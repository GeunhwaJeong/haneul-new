                    ...
                    for (message, event) in message.events.iter_mut().zip(&events.0) {
                        message.json = struct_tag_sdk_to_core(event.type_.clone())
                            ...
                            .and_then(|(layout, contents)| {
                                haneul_types::proto_value::ProtoVisitor::new(
...
impl From<haneul_types::balance_change::BalanceChange> for crate::proto::rpc::v2beta::BalanceChange {
    fn from(value: haneul_types::balance_change::BalanceChange) -> Self {
        Self {
