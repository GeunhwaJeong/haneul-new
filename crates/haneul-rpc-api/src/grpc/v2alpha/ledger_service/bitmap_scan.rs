...
use haneul_inverted_index::BitmapBucketIteratorSource;
...
use haneul_inverted_index::eval_bitmap_query_bucket_iter;
...
// Must match the writer's `EVENT_BUCKET_SIZE` in haneul-core's rpc_index and
// haneul-kvstore's `event_bitmap_index::BUCKET_SIZE` (2^28).
...
pub(super) fn drain_bitmap_hits_with_budget(
    ...
    // The iterator evaluator in `haneul-inverted-index` is synchronous, so the
