...
use haneul_rpc::proto::haneul::rpc::v2::TransactionEffects;
...
pub fn simulate_transaction(
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        message.events = submask
            ...
            .and_then(|mask| {
                events.map(|events| {
                    haneul_sdk_types::TransactionEvents::try_from(events)
...
fn to_command_output(
    ...
    let json = service
        ...
        .flatten()
        .and_then(|layout| {
            haneul_types::proto_value::ProtoVisitorBuilder::new(
