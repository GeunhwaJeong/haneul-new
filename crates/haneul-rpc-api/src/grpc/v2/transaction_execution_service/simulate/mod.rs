...
use haneul_rpc::proto::haneul::rpc::v2::CommandOutput;
use haneul_rpc::proto::haneul::rpc::v2::CommandResult;
...
use haneul_rpc::proto::haneul::rpc::v2::SimulateTransactionResponse;
...
use haneul_types::transaction::ObjectReadResult;
...
pub fn simulate_transaction(
    ...
    let mut transaction = match haneul_sdk_types::Transaction::try_from(transaction_proto) {
        Ok(transaction) => haneul_types::transaction::TransactionData::try_from(transaction)?,
                ...
                if gas_balance < estimate {
                    return Err(RpcError::new(
                        ...
                        format!(
                            ...
                            Available gas balance: {gas_balance} GEUNHWA. Estimated gas budget required: {estimate} GEUNHWA"
                        ),
                    ));
                }
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        let transaction = haneul_sdk_types::Transaction::try_from(transaction)?;
...
fn to_command_output(
    service: &RpcService,
    arg: Option<haneul_types::transaction::Argument>,
    bcs: Vec<u8>,
    ty: haneul_types::TypeTag,
) -> CommandOutput {
    let json = service
        .reader
        .inner()
...
/// 3. Rounding up to the protocol gas rounding step (typically 1000 GEUNHWA).
...
fn estimate_gas_budget_from_gas_cost(
    gas_cost_summary: &haneul_types::gas::GasCostSummary,
    ...
    let base_estimate_geunhwa = gas_cost_summary.computation_cost.max(if net_gas_usage < 0 {
        0
    } else {
    ...
    let gas_loading_cost_geunhwa =
        compute_gas_loading_cost_geunhwa(extra_payment_objects, reference_gas_price, protocol_config);

    let safe_overhead_geunhwa = GAS_SAFE_OVERHEAD.saturating_mul(reference_gas_price);

    base_estimate_geunhwa
        .saturating_add(gas_loading_cost_geunhwa)
        .saturating_add(safe_overhead_geunhwa)
        .min(protocol_config.max_tx_gas())
}

/// Cost in GEUNHWA of loading `extra_coins` gas-payment objects beyond the synthetic gas coin
...
/// converted to GEUNHWA.
fn compute_gas_loading_cost_geunhwa(
    extra_coins: u64,
    reference_gas_price: u64,
    protocol_config: &ProtocolConfig,
) -> u64 {
...
/// Storage cost (in GEUNHWA) of the synthetic gas coin that the simulator wrote during the
...
fn mock_gas_storage_cost(
    simulation_result: &haneul_types::transaction_executor::SimulateTransactionResult,
) -> u64 {
...
fn select_gas(
    ...
    let address_balance = reader
        ...
        .map(|balance| {
            // Sum up the explicit HANEUL reservations (excluding the implicit gas payment) for the
        ...
        && address_balance >= budget
    {
        ...
        let input_objects = transaction
            ...
            .flat_map(|obj| match obj {
                haneul_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                    Some(*id)
                }
                _ => None,
            })
        ...
        // to make all HANEUL in the account available (coins + address balance)
    ...
    let final_budget = if let Some(rgp) = incremental_loading_rgp {
        let real_coins = transaction
            ...
            .count() as u64;
        let extra_loading_geunhwa =
            compute_gas_loading_cost_geunhwa(real_coins.saturating_sub(1), rgp, protocol_config);
        let new_budget = budget
            .saturating_add(extra_loading_geunhwa)
            .min(protocol_config.max_tx_gas());
    ...
    if selected_gas_value >= final_budget {
        ...
        Err(RpcError::new(
            tonic::Code::InvalidArgument,
            format!(
                "Unable to perform gas selection due to insufficient HANEUL \
...
fn is_gasless_candidate(
    request: &SimulateTransactionRequest,
    transaction: &haneul_types::transaction::TransactionData,
    protocol_config: &ProtocolConfig,
    service: &RpcService,
) -> Result<bool> {
    ...
    Ok(
        haneul_transaction_checks::check_gasless_object_inputs(&input_objects, protocol_config)
            .is_ok(),
    )
}
...
/// `haneul-execution/latest/haneul-adapter/src/execution_engine.rs`). During a gasless simulate, that's
...
fn is_gasless_post_execution_failure(status: &ExecutionStatus) -> bool {
    matches!(
        ...
        ExecutionStatus::Failure(ExecutionFailure {
            error: haneul_types::execution_status::ExecutionErrorKind::InsufficientGas,
            ..
        })
    )
}
