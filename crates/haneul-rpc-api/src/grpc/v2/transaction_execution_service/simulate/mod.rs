...
fn estimate_gas_budget_from_gas_cost(
    ...
    // Calculate base estimate from gas cost summary (in GEUNHWA)
    let gas_usage = gas_cost_summary.net_gas_usage();
    let base_estimate_geunhwa =
        gas_cost_summary
            .computation_cost
    ...
    // Add all together: base (GEUNHWA) + loading (GEUNHWA) + overhead (GEUNHWA)
    let estimate_geunhwa = base_estimate_geunhwa
    ...
    estimate_geunhwa.min(protocol_config.max_tx_gas())
}
