...
// This module contains conversion functions from haneul-package-resolver types to their protobuf representations.
...
pub(crate) fn convert_module(
    module_name: &str,
    resolved_module: &haneul_package_resolver::Module,
    package_id: &ObjectID,
) -> Result<Module> {
...
fn convert_open_signature(sig: &haneul_package_resolver::OpenSignature) -> OpenSignature {
    let reference = sig.ref_.map(|r| match r {
        haneul_package_resolver::Reference::Immutable => Reference::Immutable as i32,
        haneul_package_resolver::Reference::Mutable => Reference::Mutable as i32,
    });
...
fn convert_open_signature_body(
    ...
    let (type_enum, type_name, type_params, type_param_idx) = match body {
        ...
        haneul_package_resolver::OpenSignatureBody::Vector(inner) => {
            let inner_body = convert_open_signature_body(inner.as_ref());
            (SignatureType::Vector, None, vec![inner_body], None)
        }
        haneul_package_resolver::OpenSignatureBody::Datatype(key, args) => {
        ...
        haneul_package_resolver::OpenSignatureBody::TypeParameter(idx) => {
