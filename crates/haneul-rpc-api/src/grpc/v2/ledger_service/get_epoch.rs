...
pub fn protocol_config_to_proto(config: haneul_protocol_config::ProtocolConfig) -> ProtocolConfig {
    ...
    let mut configs = config
        .render::<prost_types::Value>(&mut haneullabs_common::rpc_format::Unmetered)
        ...
        configs
            ...
            .filter_map(|(k, v)| match &v.kind {
                ...
                Some(Kind::StructValue(s)) => Some((
                    k.to_owned(),
                    serde_json::to_string(&haneul_rpc::_serde::StructSerializer(s)).unwrap(),
                )),
                Some(Kind::ListValue(list)) => Some((
                    k.to_owned(),
                    serde_json::to_string(&haneul_rpc::_serde::ListValueSerializer(list)).unwrap(),
                )),
                None => None,
            })
            .collect(),
