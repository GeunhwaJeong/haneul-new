...
fn main() {
    ...
    let haneul_proto_dir = crate_dir.join("proto");
    let out_dir = crate_dir.join("src/proto/generated");

    let haneul_rpc_proto_dir = find_dependency_proto_dir("haneul-rpc", "vendored/proto");

    println!("cargo:rerun-if-changed={}", haneul_proto_dir.display());
    println!("cargo:rerun-if-changed={}", haneul_rpc_proto_dir.display());

    fs::create_dir_all(&out_dir).expect("create proto out dir");

    // Find all .proto files using walkdir (only from haneul_proto_dir, not haneul_rpc_proto_dir)
    let proto_ext = OsStr::new("proto");
    let mut proto_files = vec![];
    for entry in WalkDir::new(&haneul_proto_dir) {
    ...
    tonic_prost_build::configure()
        ...
        .extern_path(".haneul.rpc.v2", "::haneul_rpc::proto::haneul::rpc::v2")
        .out_dir(&out_dir)
