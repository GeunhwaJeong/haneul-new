...
use haneul_indexer_builder::progress::ProgressSavingPolicy;
use haneul_types::base_types::ObjectID;
use haneul_types::transaction::{Command, TransactionDataAPI};
use tracing::info;
                    ...
                    for d in data {
                        match d {
                            ...
                            ProcessedTxnData::Error(e) => {
                                diesel::insert_into(haneul_error_transactions::table)
                                    .values(&e.to_db())
                                    .on_conflict_do_nothing()
                                    .execute(conn)
                                    .await?;
...
/// Data mapper impl
#[derive(Clone)]
pub struct HaneulDeepBookDataMapper {
    pub metrics: DeepBookIndexerMetrics,
    pub package_id: ObjectID,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for HaneulDeepBookDataMapper {
    fn map(
        &self,
        ...
        match &data.events {
            Some(events) => {
                let processed_haneul_events =
                    events.data.iter().try_fold(vec![], |mut result, ev| {
                        if let Some(data) = process_haneul_event(
                            ev,
                            &data,
                            checkpoint_num,
                            // timestamp_ms,
                ...
                if !processed_haneul_events.is_empty() {
                    info!(
                        "HANEUL: Extracted {} deepbook data entries for tx {}.",
                        processed_haneul_events.len(),
                        data.transaction.digest()
                    );
                }
                Ok(processed_haneul_events)
            }
            None => {
