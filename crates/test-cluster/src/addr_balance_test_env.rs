...
impl TestEnv {
    ...
    pub fn get_all_senders(&self) -> Vec<HaneulAddress> {
        self.cluster.wallet.get_addresses()
    }

    pub fn get_gas_for_sender(&self, sender: HaneulAddress) -> Vec<ObjectRef> {
    ...
    pub async fn setup_custom_coin(&mut self) -> (HaneulAddress, TypeTag) {
    ...
    pub async fn setup_mintable_coin(&mut self) -> (HaneulAddress, ObjectID, TypeTag, ObjectRef) {
    ...
    async fn publish_coins_package(&mut self) -> (HaneulAddress, ObjectID, TransactionEffects) {
        let mut path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.extend(["..", "haneul-e2e-tests", "tests", "data", "coins"]);
    ...
    /// Get the balance of the owner's HANEUL address balance.
    pub fn get_haneul_balance_ab(&self, owner: HaneulAddress) -> u64 {
    ...
    pub fn get_balance_ab(&self, owner: HaneulAddress, coin_type: TypeTag) -> u64 {
        let db_balance = self.cluster.fullnode_handle.haneul_node.with({
    ...
    /// Get the total balance of HANEUL owned by the address (including address balance and coins).
    pub async fn get_haneul_balance(&self, owner: HaneulAddress) -> u64 {
    ...
    pub async fn get_balance_for_coin_type(&self, owner: HaneulAddress, coin_type: TypeTag) -> u64 {
        let client = self.cluster.grpc_client();
    ...
    pub async fn publish_trusted_coin(
        ...
        path.pop();
        path.extend(["haneul-e2e-tests", "tests", "rpc", "data", "trusted_coin"]);
    ...
    /// Transfer HANEUL from sender's gas to recipient's address balance.
    pub async fn transfer_haneul_to_address_balance(
        &mut self,
        sender: HaneulAddress,
        recipient: HaneulAddress,
        amount: u64,
...
pub fn get_accumulator_object_id(sender: HaneulAddress, coin_type: TypeTag) -> ObjectID {
