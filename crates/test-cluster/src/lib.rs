...
impl TestCluster {
    ...
    ) -> HaneulResult<Vec<(TransactionDigest, TransactionEffects)>> {
        ...
        for (index, raw_result) in result.results.into_iter().enumerate() {
            ...
            match submit_result {
                SubmitTxResult::Executed { details, .. } => {
                    let data = details.ok_or_else(|| HaneulErrorKind::GenericAuthorityError {
    ...
    pub async fn execute_soft_bundle_with_conflicts(
        &self,
        signed_txs: &[Transaction],
    ) -> HaneulResult<Vec<(TransactionDigest, WaitForEffectsResponse)>> {
        let digests: Vec<_> = signed_txs.iter().map(|tx| *tx.digest()).collect();
        ...
        let results: HaneulResult<Vec<_>> = digests
            .into_iter()
    ...
    pub async fn wait_for_tx_settlement(&self, digests: &[TransactionDigest]) {
        Self::wait_for_tx_settlement_on_handles(
            std::slice::from_ref(&self.fullnode_handle.haneul_node),
            digests,
        )
        .await;
    }
...
impl TestClusterBuilder {
    ...
    pub fn with_rpc_config(mut self, config: haneul_config::RpcConfig) -> Self {
    ...
    async fn start_swarm(&mut self) -> Result<Swarm, anyhow::Error> {
        ...
        for key in &swarm.config().account_keys {
            keystore
                .import(None, HaneulKeyPair::Ed25519(key.copy()))
                .await?;
        }
