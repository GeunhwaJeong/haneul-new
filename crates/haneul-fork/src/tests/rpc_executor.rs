...
impl TestHarness {
    ...
    /// Create transaction data where `amount` of HANEUL is transferred to a random recipient.
    fn build_transfer_tx_data(&self, amount: u64) -> TransactionData {
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_haneul(HaneulAddress::random_for_testing_only(), Some(amount));
            builder.finish()
        };
    ...
    /// Create a transaction where `amount` of HANEUL is transferred to a random recipient.
...
async fn test_bad_signature_returns_submission_error() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_haneul(HaneulAddress::random_for_testing_only(), Some(1_000));
        builder.finish()
    };
    let tx_data = TransactionData::new_with_gas_data(
        TransactionKind::ProgrammableTransaction(pt),
    ...
    let (_wrong_addr, wrong_key): (HaneulAddress, AccountKeyPair) = get_key_pair();
