...
use haneul_types::object::ObjectInner;
...
#[cfg(unix)]
#[test]
fn haneul_fork_data_env_takes_precedence_over_xdg_and_home() {
    let vars = [
        (HANEUL_FORK_DATA_ENV, "/tmp/custom-fork-base"),
...
fn make_object_with_owner(id: ObjectID, version: u64, owner: Owner) -> Object {
    ...
    ObjectInner {
        owner,
        data: haneul_types::object::Data::Move(move_obj),
...
fn build_checkpoint(sequence: u64) -> (VerifiedCheckpoint, CheckpointContents) {
    let data = haneul_types::test_checkpoint_data_builder::TestCheckpointBuilder::new(sequence)
        .build_checkpoint();
...
fn test_owned_object_index_upserts_removes_and_stays_sorted() {
    let (_dir, store) = test_store();
    let owner = HaneulAddress::random_for_testing_only();
    let next_owner = HaneulAddress::random_for_testing_only();
    ...
    assert!(
        entries
            .iter()
            .all(|entry| entry.object_type == haneul_types::gas_coin::GasCoin::type_())
    );
    ...
    assert_eq!(
        ...
        haneul_types::gas_coin::GasCoin::type_()
    );
