...
use haneul_rpc_api::RpcService;
use haneul_rpc_api::ServerVersion;
use haneul_rpc_api::subscription::SubscriptionService;
use haneul_rpc_api::subscription::SubscriptionServiceHandle;
...
pub async fn initialize(
    ...
    let system_state = haneul_types::haneul_system_state::get_haneul_system_state(&data_store)
        .map_err(|e| {
            anyhow!(
...
pub async fn run(
    ...
    service.with_server_version(ServerVersion::new("haneul-fork", version));
...
fn override_validators(
...
) -> Result<HaneulSystemState> {
    let genesis_validators = match config.genesis.haneul_system_object() {
    ...
    match system_state {
        HaneulSystemState::V1(mut inner) => {
            inner.validators = genesis_validators;
            Ok(HaneulSystemState::V1(inner))
        }
        HaneulSystemState::V2(mut inner) => {
            inner.validators = genesis_validators;
            Ok(HaneulSystemState::V2(inner))
        }
        #[cfg(msim)]
