...
/// the `RUST_LOG_TAILS` env var, e.g. `graphql_request=/var/log/haneul/graphql_request.jsonl`.
#[derive(Clone, Debug)]
pub struct LogTail {
    pub target: String,
...
fn parse_log_tail_entry(entry: &str) -> LogTail {
    ...
    panic!(
        "telemetry: invalid RUST_LOG_TAILS entry {entry:?}: a `=file` path is \
         required, e.g. `graphql_request=/var/log/haneul/graphql_request.jsonl`"
    )
}
...
impl TelemetryConfig {
    ...
    pub fn init(self) -> (TelemetryGuards, TracingHandle) {
        ...
        let service_name = env::var("OTEL_SERVICE_NAME").unwrap_or("haneul-node".to_owned());
