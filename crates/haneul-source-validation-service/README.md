# Haneul Source Validation Service

This document describes the Haneul Source Validation Service. It is engineering documentation primarily for Haneul Labs engineers who may want to build, extend, configure, or understand the service.
...
The default configuration limits scope to Haneul framework packages in `crates/haneul-framework/packages`:

- `move-stdlib` — [address `0x1`](https://haneulexplorer.com/object/0x1)
- `haneul-framework` — [address `0x2`](https://haneulexplorer.com/object/0x2)
- `haneul-system` — [address `0x3`](https://haneulexplorer.com/object/0x2)
- `deepbook` — [address `0xdee9`](https://haneulexplorer.com/object/0xdee9)
...
```
cargo run --release --bin haneul-source-validation-service crates/haneul-source-validation-service/config.toml 
```
...
The `watch` field is optional, and specifies the address of an object that the server should monitor for on-chain changes if a package is upgraded. For example, Haneul framework packages mutate their on-chain address when upgraded. 
...
After running `cargo run --bin haneul-source-validation-service crates/haneul-source-validation-service/config.toml` locally, try:

```
curl 'http://0.0.0.0:8000/api?address=0x2&module=coin&network=mainnet' --header 'X-Haneul-Source-Validation-Version: 0.1'
```
...
## Haneul Labs documentation

Refer to the [Notion doc](https://www.notion.so/haneullabs/Move-Source-Provider-Service-91ec291be3b94c0f8133e981b76988c0) for internal details.
