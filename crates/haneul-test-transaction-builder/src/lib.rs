...
impl TestTransactionBuilder {
    pub fn new(sender: HaneulAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self::new_impl(sender, Some(gas_object), gas_price)
    }

    fn new_impl(sender: HaneulAddress, gas_object: Option<ObjectRef>, gas_price: u64) -> Self {
        Self {
    ...
    pub fn transfer(mut self, object: FullObjectRef, recipient: HaneulAddress) -> Self {
    ...
    pub fn transfer_haneul(mut self, amount: Option<u64>, recipient: HaneulAddress) -> Self {
        self.ptb_builder.transfer_haneul(recipient, amount);
        self
    }

    pub fn transfer_haneul_to_address_balance(
        self,
        source: FundSource,
...
pub async fn make_transfer_haneul_transaction(
    context: &WalletContext,
    ...
    context
        .sign_transaction(
            &TestTransactionBuilder::new(sender, gas_object, gas_price)
                .transfer_haneul(amount, recipient.unwrap_or(sender))
                .build(),
        )
        .await
}

pub async fn make_transfer_haneul_address_balance_transaction(
    context: &WalletContext,
    recipient: Option<HaneulAddress>,
    amount: u64,
) -> Transaction {
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (HaneulAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
