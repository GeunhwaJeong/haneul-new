...
#[tokio::test]
async fn test_stored_event_to_haneul_event() -> Result<(), anyhow::Error> {
    ...
    let mut queried_events = db
        .events_by_type(EventType::Publish, (5, 0).into(), 1, false)
        .await?;
    assert_eq!(queried_events.len(), 1);
    let haneul_event: HaneulEventEnvelope = queried_events.swap_remove(0).try_into()?;
    assert!(
        publish == haneul_event,
        "event: {publish:?}\nhaneul_event: {haneul_event:?}"
    );
    Ok(())
}
...
async fn insert_and_fetch_by_tx_digest_then_compare(
    ...
    let mut events = db
        .events_by_transaction(tx_digest, (0, 0).into(), 10, false)
        .await?;
    assert_eq!(events.len(), 1);
    let stored_event = events.pop().unwrap();
    let haneul_event: HaneulEventEnvelope = stored_event.try_into()?;
    assert!(
        event_envelope == haneul_event,
