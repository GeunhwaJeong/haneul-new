...
use haneul_types::event::{Event, TransferTypeVariants};
...
impl SqlEventStore {
    ...
    pub async fn new_memory_only_not_prod() -> Result<Self, HaneulError> {
    ...
    pub async fn new_from_file(db_path: &Path) -> Result<Self, HaneulError> {
    ...
    pub async fn initialize(&self) -> Result<(), HaneulError> {
    ...
    #[allow(unused)]
    pub async fn total_event_count(&self) -> Result<usize, HaneulError> {
    ...
    async fn last_seq_num(&self) -> Result<u64, HaneulError> {
    ...
    fn try_extract_object_id(row: &SqliteRow, col: usize) -> Result<Option<ObjectID>, HaneulError> {
        let raw_bytes: Option<Vec<u8>> = row.get(col);
        match raw_bytes {
            Some(bytes) => Ok(Some(ObjectID::try_from(bytes.as_slice()).map_err(
                |_e| HaneulError::BadObjectType {
    ...
    fn try_extract_sender_address(row: &SqliteRow) -> Result<Option<HaneulAddress>, HaneulError> {
        let raw_bytes: Option<Vec<u8>> = row.get(EventsTableColumns::Sender as usize);
        match raw_bytes {
            Some(bytes) => Ok(Some(HaneulAddress::try_from(bytes.as_slice()).map_err(
                |_e| HaneulError::BadObjectType {
                    error: format!(
                        "Could not parse sender address bytes: {:?} into HaneulAddress",
                        bytes
                    ),
                },
            )?)),
    ...
    fn try_extract_recipient(row: &SqliteRow) -> Result<Option<Owner>, HaneulError> {
...
impl From<SqliteRow> for StoredEvent {
    ...
    fn from(row: SqliteRow) -> Self {
        ...
        let sender = SqlEventStore::try_extract_sender_address(&row)
            .expect("Error converting stored sender address bytes to HaneulAddress");
...
impl EventStore for SqlEventStore {
    ...
    async fn add_events(&self, events: &[EventEnvelope]) -> Result<u64, HaneulError> {
    ...
    async fn events_by_recipient(
        ...
        let limit = Self::cap_limit(limit);
        let recipient_str =
            serde_json::to_string(recipient).map_err(|e| HaneulError::OwnerFailedToSerialize {
                error: (e.to_string()),
            })?;
...
fn convert_sqlx_err(err: sqlx::Error) -> HaneulError {
    HaneulError::GenericStorageError(err.to_string())
}
...
mod tests {
    ...
    use haneul_types::event::{EventEnvelope, TransferType};
    ...
    #[tokio::test]
    async fn test_eventstore_basic_insert_read() -> Result<(), HaneulError> {
    ...
    #[tokio::test]
    async fn test_eventstore_transfers_tx_read() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
    ...
    #[tokio::test]
    async fn test_eventstore_query_by_type() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
    ...
    #[tokio::test]
    async fn test_eventstore_move_events() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
    ...
    #[tokio::test]
    async fn test_eventstore_query_by_move_event_struct_name() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
        ...
        let events = db
            .events_by_move_event_struct_name(
                1_000_000,
                1_002_000,
                "0x2::HANEUL::test_foo<address, vector<u8>>",
                10,
            )
            .await?;
    ...
    #[tokio::test]
    async fn test_eventstore_query_by_sender_recipient_and_object() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
        ...
        let recipient = Owner::AddressOwner(HaneulAddress::random_for_testing_only());
    ...
    #[tokio::test]
    async fn test_eventstore_u64_conversion() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
    ...
    #[tokio::test]
    async fn test_eventstore_seq_num() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
