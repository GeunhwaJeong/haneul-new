...
fn check_limit(limit: usize) -> Result<(), HaneulError> {
    if limit <= MAX_LIMIT {
        Ok(())
    } else {
        Err(HaneulError::TooManyItemsError(MAX_LIMIT as u64))
    }
}
...
mod tests {
    ...
    fn new_test_newobj_event() -> Event {
        Event::NewObject {
            ...
            sender: HaneulAddress::random_for_testing_only(),
            recipient: Owner::AddressOwner(HaneulAddress::random_for_testing_only()),
    ...
    #[tokio::test]
    async fn test_eventstore_max_limit() -> Result<(), HaneulError> {
        telemetry_subscribers::init_for_testing();
        ...
        assert!(matches!(res, Err(HaneulError::TooManyItemsError(_))));
