...
use haneul_json_rpc_types::{HaneulEvent, HaneulEventEnvelope};
use haneul_types::base_types::{ObjectID, SequenceNumber, HaneulAddress, TransactionDigest};
use haneul_types::error::HaneulError;
use haneul_types::error::HaneulError::{StorageCorruptedFieldError, StorageMissingFieldError};
use haneul_types::event::{BalanceChangeType, Event, EventEnvelope, EventType};
...
pub struct StoredEvent {
    ...
    /// The variant name from HaneulEvent, eg MoveEvent, Publish, etc.
...
impl StoredEvent {
    pub fn into_move_event(self) -> Result<HaneulEvent, anyhow::Error> {
    ...
    pub fn into_publish(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let sender = self.sender()?;
        Ok(HaneulEvent::Publish { sender, package_id })
    }

    pub fn into_transfer_object(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let transaction_module = self.transaction_module()?;
        let sender = self.sender()?;
    ...
    pub fn into_mutate_object(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let transaction_module = self.transaction_module()?;
        let sender = self.sender()?;
        let object_id = self.object_id()?;
    ...
    pub fn into_delete_object(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let transaction_module = self.transaction_module()?;
        let sender = self.sender()?;
        let object_id = self.object_id()?;
    ...
    pub fn into_new_object(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let transaction_module = self.transaction_module()?;
        let sender = self.sender()?;
        let object_id = self.object_id()?;
    ...
    pub fn into_coin_balance_change(self) -> Result<HaneulEvent, anyhow::Error> {
        let package_id = self.package_id()?;
        let transaction_module = self.transaction_module()?;
        let sender = self.sender()?;
        ...
        Ok(HaneulEvent::CoinBalanceChange {
            package_id,
            transaction_module,
            sender,
            change_type,
    ...
    /// Convert a vec of StoredEvents into a vec of HaneulEventEnvelope.
    ...
    pub fn into_event_envelopes(
        ...
        for stored_event in stored_events {
            let event_envelope: HaneulEventEnvelope = stored_event.try_into()?;
    ...
    fn sender(&self) -> Result<HaneulAddress, anyhow::Error> {
    ...
    fn extract_string_field(&self, key: &str) -> Result<Option<String>, anyhow::Error> {
        ...
        match field_value {
            ...
            None => Ok(None),
            Some(other_value) => anyhow::bail!(HaneulError::ExtraFieldFailedToDeserialize {
...
impl TryInto<HaneulEventEnvelope> for StoredEvent {
...
/// One can think of events as logs.  They represent a log of what is happening to Haneul.
