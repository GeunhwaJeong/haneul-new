...
use haneul_types::base_types::{ObjectDigest, ObjectID, ObjectRef, SequenceNumber, TransactionDigest};
...
enum LockServiceCommands {
    ...
    SequenceTransaction {
        ...
        resp: oneshot::Sender<HaneulResult<TxSequenceNumber>>,
    },
...
type HaneulLockResult = HaneulResult<ObjectLockStatus>;
...
enum LockServiceQueries {
    ...
    GetMissingLocks {
        objects: Vec<ObjectRef>,
        resp: oneshot::Sender<HaneulResult<Vec<ObjectRef>>>,
    },
    GetTxSequence {
        tx: TransactionDigest,
        resp: oneshot::Sender<Result<Option<TxSequenceNumber>, HaneulError>>,
    },
}
...
impl LockServiceImpl {
    fn get_tx_sequence(&self, tx: TransactionDigest) -> HaneulResult<Option<TxSequenceNumber>> {
        self.tx_sequence.get(&tx).map_err(HaneulError::StorageError)
    }
    ...
    fn get_missing_locks(&self, objects: &[ObjectRef]) -> HaneulResult<Vec<ObjectRef>> {
    ...
    fn create_locks_for_genesis_objects(&self, objects: &[ObjectRef]) -> HaneulResult {
    ...
    fn initialize_locks(&self, objects: &[ObjectRef], is_force_reset: bool) -> HaneulResult {
        let write_batch = self.transaction_lock.batch();
    ...
    pub fn get_latest_lock_for_object_id(&self, object_id: ObjectID) -> HaneulResult<ObjectRef> {
...
impl LockService {
    ...
    pub fn new(path: PathBuf, db_options: Option<Options>) -> Result<Self, HaneulError> {
    ...
    pub async fn initialize_locks(&self, refs: &[ObjectRef], is_force_reset: bool) -> HaneulResult {
        block_on_future_in_sim(async move {
            let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
            self.inner
                .sender()
    ...
    pub async fn get_lock(&self, object: ObjectRef, epoch_id: EpochId) -> HaneulLockResult {
        block_on_future_in_sim(async move {
    ...
    pub async fn get_tx_sequence(
        ...
        block_on_future_in_sim(async move {
            let (os_sender, os_receiver) =
                oneshot::channel::<Result<Option<TxSequenceNumber>, HaneulError>>();
            self.inner
                .query_sender()
    ...
    pub async fn create_locks_for_genesis_objects(&self, objects: Vec<ObjectRef>) -> HaneulResult {
        block_on_future_in_sim(async move {
            let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
            self.inner
                .sender()
    ...
    pub async fn sequence_transaction(
        ...
        block_on_future_in_sim(async move {
            let (os_sender, os_receiver) = oneshot::channel::<HaneulResult<TxSequenceNumber>>();
            self.inner
                .sender()
    ...
    pub async fn get_missing_locks(&self, objects: Vec<ObjectRef>) -> HaneulResult<Vec<ObjectRef>> {
        block_on_future_in_sim(async move {
            let (os_sender, os_receiver) = oneshot::channel::<HaneulResult<Vec<ObjectRef>>>();
            self.inner
                .query_sender()
...
mod tests {
    ...
    async fn test_lockdb_acquire_init_multiple() {
        ...
        assert!(matches!(
            ...
            Err(HaneulError::ObjectLockAlreadyInitialized { .. })
        ));
        ...
        assert!(matches!(
            ...
            Err(HaneulError::ObjectLockConflict { .. })
        ));
        ...
        assert!(matches!(
            ls.acquire_locks(0, &[ref2, ref3], tx2),
            Err(HaneulError::ObjectVersionUnavailableForConsumption {
                provided_obj_ref,
                current_version,
            })
    ...
    async fn test_lockservice_conc_acquire_init() {
        ...
        assert!(inner_res
            .iter()
            .filter(|r| r.is_err())
            .all(|r| matches!(r, Err(HaneulError::ObjectLockConflict { .. }))));
    }
