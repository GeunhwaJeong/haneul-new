...
type HaneulLockResult = Result<Option<Option<LockInfo>>, HaneulError>;
...
impl LockServiceImpl {
    ...
    fn get_lock(&self, object: ObjectRef) -> Result<Option<Option<LockInfo>>, HaneulError> {
    ...
    fn locks_exist(&self, objects: &[ObjectRef]) -> HaneulResult {
        ...
        for (lock, obj_ref) in locks.into_iter().zip(objects) {
            fp_ensure!(
                lock.is_some(),
                HaneulError::ObjectLockUninitialized { obj_ref: *obj_ref }
            );
        }
    ...
    fn acquire_locks(
        ...
        for ((i, lock), obj_ref) in locks.iter().enumerate().zip(owned_input_objects) {
            ...
            let lock = lock
                .as_ref()
                .ok_or(HaneulError::ObjectLockUninitialized { obj_ref: *obj_ref })?;
...
mod tests {
    ...
    async fn test_lockdb_acquire_init_multiple() {
        ...
        assert_eq!(
            ...
            Err(HaneulError::ObjectLockUninitialized { obj_ref: ref1 })
        );
        ...
        assert_eq!(
            ...
            Err(HaneulError::ObjectLockUninitialized { obj_ref: ref3 })
        );
