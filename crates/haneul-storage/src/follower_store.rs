...
impl FollowerStore {
    ...
    pub fn get_next_sequence(&self, name: &AuthorityName) -> HaneulResult<Option<TxSequenceNumber>> {
        self.next_sequence.get(name).map_err(HaneulError::StorageError)
    }

    pub fn record_next_sequence(&self, name: &AuthorityName, seq: TxSequenceNumber) -> HaneulResult {
