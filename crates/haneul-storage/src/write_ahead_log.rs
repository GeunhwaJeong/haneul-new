...
pub trait TxGuard<'a>: Drop {
    ...
    fn commit_tx(self) -> HaneulResult;
}
...
pub trait WriteAheadLog<'a, C> {
    ...
    fn get_tx_data(&self, g: &Self::Guard) -> HaneulResult<C>;
}
    ...
    C: Serialize + DeserializeOwned,
{
    ...
    fn commit_tx(mut self) -> HaneulResult {
        self.dead = true;
    ...
    C: Serialize + DeserializeOwned + std::marker::Send + std::marker::Sync,
{
    ...
    fn get_tx_data(&self, g: &DBTxGuard<'a, C>) -> HaneulResult<C> {
        self.log
            .get(&g.tx)
