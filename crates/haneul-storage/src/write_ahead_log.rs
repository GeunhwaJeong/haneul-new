...
pub trait WriteAheadLog<'a, C> {
    ...
    async fn begin_tx<'b>(
        ...
        cert: &'b C,
    ) -> HaneulResult<Option<Self::Guard>>;
    ...
    fn get_tx_data(&self, g: &Self::Guard) -> HaneulResult<(C, u32)>;
}
    ...
    C: Serialize + DeserializeOwned,
{
    ...
    fn commit_tx(&self, tx: &TransactionDigest) -> HaneulResult {
    ...
    fn increment_retry_count(&self, tx: &TransactionDigest) -> HaneulResult {
    ...
    C: Serialize + DeserializeOwned + std::marker::Send + std::marker::Sync,
{
    ...
    async fn begin_tx<'b>(
        ...
        tx: &'b TransactionDigest,
        cert: &'b C,
    ) -> HaneulResult<Option<DBTxGuard<'a, C>>> {
    ...
    fn get_tx_data(&self, g: &DBTxGuard<'a, C>) -> HaneulResult<(C, u32)> {
        let cert = self
            .log
            .get(&g.tx)?
            .ok_or(HaneulError::TransactionNotFound { digest: g.tx })?;
