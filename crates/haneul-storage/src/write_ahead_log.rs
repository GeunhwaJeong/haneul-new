...
pub trait WriteAheadLog<'a, C> {
    ...
    async fn read_one_recoverable_tx(&'a self) -> HaneulResult<Option<(C, Self::Guard)>>;
}
    ...
    ExecutionOutput: Serialize + DeserializeOwned + Debug,
{
    fn drop(&mut self) {
        ...
        haneul_simulator::return_if_killed!();
    ...
    ExecutionOutput: Serialize + DeserializeOwned + Debug,
{
    ...
    pub fn get_tx(
        &self,
        tx: &TransactionDigest,
    ) -> HaneulResult<Option<(C, TransactionCommitPhase<ExecutionOutput>)>> {
        self.tables.log.get(tx).map_err(HaneulError::from)
    }

    pub fn get_execution_output(
        &self,
        tx: &TransactionDigest,
    ) -> HaneulResult<Option<ExecutionOutput>> {
    ...
    pub fn write_execution_output(
        ...
        match self.get_tx(tx)? {
            ...
            Some((_cert, TransactionCommitPhase::Executed(_))) => {
                Err(HaneulError::TransactionAlreadyExecuted { digest: *tx })
            }
            None => Err(HaneulError::TransactionNotFound { digest: *tx }),
        }
    }

    fn commit_tx(&self, tx: &TransactionDigest, is_commit: bool) -> HaneulResult {
        if is_commit {
            debug!("committing tx");
        }
    ...
    fn get_retry_count(&self, tx: &TransactionDigest) -> HaneulResult<u32> {
        Ok(self.tables.retry_count.get(tx)?.unwrap_or(0))
    }

    fn increment_retry_count(&self, tx: &TransactionDigest) -> HaneulResult<u32> {
    ...
    ExecutionOutput: Serialize + DeserializeOwned + Debug + Send + Sync,
{
    ...
    async fn begin_tx<'b>(
        ...
        tx: &'b TransactionDigest,
        cert: &'b C,
    ) -> HaneulResult<DBTxGuard<'a, C, ExecutionOutput>> {
    ...
    async fn read_one_recoverable_tx(
        &'a self,
    ) -> HaneulResult<Option<(C, DBTxGuard<'a, C, ExecutionOutput>)>> {
