...
impl NodeSyncStore {
    ...
    pub fn store_cert(&self, epoch_id: EpochId, cert: &VerifiedCertificate) -> HaneulResult {
        Ok(self
            .pending_certs
            .insert(&(epoch_id, *cert.digest()), cert.serializable_ref())?)
    }

    pub fn batch_store_certs(&self, certs: impl Iterator<Item = VerifiedCertificate>) -> HaneulResult {
    ...
    pub fn cleanup_cert(&self, epoch_id: EpochId, digest: &TransactionDigest) -> HaneulResult {
    ...
    ) -> HaneulResult<impl Iterator<Item = (TxSequenceNumber, ExecutionDigests)> + 'a> {
        Ok(self
            .batch_streams
            .iter()
    ...
    pub fn get_voters(
        ...
        digest: TransactionDigest,
        effects_digest: TransactionEffectsDigest,
    ) -> HaneulResult<BTreeSet<AuthorityName>> {
        Ok(self
            .iter_fx_digest(epoch_id, digest, effects_digest)?
            .map(|((_, _, peer), _)| peer)
            .collect())
    ...
    pub fn clear_effects_votes(&self, epoch_id: EpochId, digest: TransactionDigest) -> HaneulResult {
...
mod test {
    ...
    use super::*;
    use haneul_types::crypto::{get_authority_key_pair, KeypairTraits};
