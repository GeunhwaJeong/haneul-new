...
trait IntoHaneulResult<T> {
    fn into_haneul_result(self) -> HaneulResult<T>;
}

impl<T, E> IntoHaneulResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_haneul_result(self) -> HaneulResult<T> {
        self.map_err(|e| HaneulError::GenericStorageError(e.to_string()))
    }
}
...
fn key_to_path_elements(key: &Key) -> HaneulResult<(String, &'static str)> {
    match key {
...
impl HttpKVStore {
    ...
    pub fn new(base_url: &str) -> HaneulResult<Self> {
        ...
        let base_url = Url::parse(&base_url).into_haneul_result()?;
    ...
    fn get_url(&self, key: &Key) -> HaneulResult<Uri> {
        let (digest, item_type) = key_to_path_elements(key)?;
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_haneul_result()?;
        Uri::from_str(joined.as_str()).into_haneul_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<HaneulResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> HaneulResult<Option<Bytes>> {
        let uri = self.get_url(&key)?;
        trace!("fetching uri: {}", uri);
        let resp = self.client.get(uri.clone()).await.into_haneul_result()?;
        trace!(
...
fn map_fetch<'a, K>(fetch: (&'a HaneulResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
