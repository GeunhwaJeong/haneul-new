...
{
    fn into_haneul_result(self) -> HaneulResult<T> {
        self.map_err(|e| HaneulErrorKind::Storage(e.to_string()).into())
    }
}
...
impl HttpKVStore {
    ...
    fn get_url(&self, key: &Key) -> HaneulResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_haneul_result()?;
        Url::from_str(joined.as_str()).into_haneul_result()
    }
    ...
    async fn fetch(&self, key: Key) -> HaneulResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_haneul_result()?;
