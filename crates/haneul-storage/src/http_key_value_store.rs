...
fn key_to_path_elements(key: &Key) -> HaneulResult<(String, &'static str)> {
    match key {
        ...
        Key::Fx(digest) => Err(HaneulError::UnsupportedFeatureError {
            error: format!(
...
impl HttpKVStore {
    pub fn new(base_url: Uri) -> HaneulResult<Self> {
        ...
        let base_url = Url::parse(&base_url.to_string()).into_haneul_result()?;
    ...
    async fn multi_fetch(&self, uris: &[Key]) -> Vec<HaneulResult<Bytes>> {
    ...
    async fn fetch(&self, key: Key) -> HaneulResult<Bytes> {
...
impl TransactionKeyValueStore for HttpKVStore {
    async fn multi_get(&self, keys: &[Key]) -> HaneulResult<Vec<Option<Value>>> {
