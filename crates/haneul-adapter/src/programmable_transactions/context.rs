...
impl<'vm, 'state, 'a, 'b, S: StorageView> ExecutionContext<'vm, 'state, 'a, 'b, S> {
    pub fn new(
        ...
        let (change_set, move_events) =
            res.map_err(|e| haneul_types::error::convert_vm_error(e, vm, &linkage))?;
    ...
    pub fn finish<Mode: ExecutionMode>(self) -> Result<ExecutionResults, ExecutionError> {
        use haneul_types::error::convert_vm_error;
        let Self {
            protocol_config,
            vm,
            state_view,
    ...
    pub fn convert_vm_error(&self, error: VMError) -> ExecutionError {
        haneul_types::error::convert_vm_error(error, self.vm, self.session.get_resolver())
    }

    /// Special case errors for type arguments to Move functions
    pub fn convert_type_argument_error(&self, idx: usize, error: VMError) -> ExecutionError {
        use move_core_types::vm_status::StatusCode;
        use haneul_types::messages::TypeArgumentError;
...
unsafe fn create_written_object<S: StorageView>(
    ...
    let type_tag = session
        .get_type_tag(&type_)
        .map_err(|e| haneul_types::error::convert_vm_error(e, vm, session.get_resolver()))?;
