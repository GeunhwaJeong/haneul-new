...
use haneul_types::execution_status::CommandArgumentError;
use haneul_types::{
    balance::Balance,
    base_types::{ObjectID, SequenceNumber, HaneulAddress, TxContext},
    coin::Coin,
...
pub struct ExecutionContext<'vm, 'state, 'a, S: StorageView> {
    ...
    additional_transfers: Vec<(/* new owner */ HaneulAddress, ObjectValue)>,
    ...
    &'state S: HaneulResolver,
{
    pub fn new(
        ...
        // exist. Plus, Haneul Move does not use these changes or events
    ...
    pub fn finish<Mode: ExecutionMode>(self) -> Result<ExecutionResults, ExecutionError> {
        ...
        // Haneul Move programs should never touch global state, so resources should be empty
        assert_invariant!(
            change_set.resources().next().is_none(),
            "Change set must be empty"
        );
        // Haneul Move no longer uses Move's internal event system
...
pub(crate) fn new_session_for_linkage<'state, S: HaneulResolver>(
    vm: &MoveVM,
...
pub fn set_linkage<S: HaneulResolver>(
...
/// Turn off linkage information, so that the next use of the session will need to set linkage
/// information to succeed.
pub fn reset_linkage<S: HaneulResolver>(session: &mut Session<LinkageView<S>>) {
    session.get_resolver_mut().reset_linkage();
}

pub fn steal_linkage<S: HaneulResolver>(
    session: &mut Session<LinkageView<S>>,
) -> Option<SavedLinkage> {
    session.get_resolver_mut().steal_linkage()
}

pub fn restore_linkage<S: HaneulResolver>(
    session: &mut Session<LinkageView<S>>,
...
fn package_for_linkage<S: HaneulResolver>(
...
pub fn load_type<S: HaneulResolver>(
    session: &mut Session<LinkageView<S>>,
    type_tag: &TypeTag,
) -> VMResult<Type> {
    use move_binary_format::errors::PartialVMError;
