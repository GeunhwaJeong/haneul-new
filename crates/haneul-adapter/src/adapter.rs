...
use haneul_framework::EventType;
use haneul_types::{
    base_types::*,
...
use haneul_verifier::{
    entry_points_verifier::{is_tx_context, INIT_FN_NAME, RESOLVED_STD_OPTION, RESOLVED_HANEUL_ID},
    verifier,
};
...
pub fn new_move_vm(natives: NativeFunctionTable) -> Result<MoveVM, HaneulError> {
...
/// The return value is a two-layer HaneulResult. The outer layer indicates whether a system error
/// has occurred (i.e. issues with the haneul system, not with user transaction).
/// As long as there are no system issues we return Ok(HaneulResult).
/// The inner HaneulResult indicates the execution result. If execution failed, we return Ok(Err),
...
>(
    ...
    gas_status: &mut HaneulGasStatus, // gas status for the current call operation
    ctx: &mut TxContext,
) -> HaneulResult<()> {
    ...
    // by objects.
    let object_owner_map: BTreeMap<HaneulAddress, HaneulAddress> = object_data
        .iter()
    ...
    match result {
        ...
        Err(error) => Err(HaneulError::AbortedExecution {
...
pub fn publish<E: Debug, S: ResourceResolver<Error = E> + ModuleResolver<Error = E> + Storage>(
    ...
    let mut modules = module_bytes
        ...
        .map_err(|err| HaneulError::ModuleDeserializationFailure {
            error: err.to_string(),
        })?;
...
>(
    ...
    // It is important to do this before running the Haneul verifier, since the haneul
...
>(
    ...
    ctx: &TxContext,
    mut object_owner_map: BTreeMap<HaneulAddress, HaneulAddress>,
) -> HaneulResult {
        ...
        match event_type {
            ...
            | EventType::ShareObject => {
                let new_owner = match event_type {
                    EventType::TransferToAddress => {
                        Owner::AddressOwner(HaneulAddress::try_from(recipient.as_slice()).unwrap())
                    }
                ...
                if !newly_generated_ids.contains(obj_id) {
                    match by_value_objects.remove(id.object_id()) {
                        Some((Owner::ObjectOwner { .. }, _)) => {
                            ...
                            return Err(HaneulError::DeleteObjectOwnedObject);
                        }
                        Some(_) => {
    ...
    // this means that either the object was (1) deleted from the Haneul system altogether, or
    // (2) wrapped inside another object that is in the Haneul object pool
...
>(
    ...
    state_view: &mut S,
    object_owner_map: &mut BTreeMap<HaneulAddress, HaneulAddress>,
    ...
    match type_ {
        TypeTag::Struct(s_type) => {
            ...
            let obj_address: HaneulAddress = obj_id.into();
...
pub fn resolve_and_type_check(
    ...
    let fdef = match fdef_opt {
        Some(fdef) => fdef,
        None => {
            return Err(HaneulError::FunctionNotFound {
                error: format!(
    ...
    if fdef.visibility != Visibility::Script && !is_genesis {
        return Err(HaneulError::InvalidFunctionVisibility {
        ...
        .map(|(idx, arg)| {
            ...
            let object = match objects.get(&id) {
                ...
                None => {
                    ...
                    return Err(HaneulError::ExecutionInvariantViolation);
                }
            };
            match object_kind {
                InputObjectKind::ImmOrOwnedMoveObject(_) if object.is_shared() => {
                    ...
                    return Err(HaneulError::TypeError { error });
                }
    ...
    for (object_id, obj) in objects {
            ...
            match cur_obj.borrow().owner {
                Owner::ObjectOwner(parent_id) => {
                    cur_obj =
                        objects
                            .get(&parent_id.into())
                            .ok_or(HaneulError::MissingObjectOwner {
                                child_id: cur_id,
                    ...
                    fp_ensure!(
                        ...
                        HaneulError::CircularObjectOwnership
                    );
                }
        ...
        if objects.get(&ancestor_id).unwrap().borrow().is_shared() {
            ...
            fp_ensure!(
                ...
                HaneulError::InvalidSharedChildUse {
                    child: *object_id,
...
fn is_primitive(
    ...
    match t {
        ...
        SignatureToken::Struct(idx) => {
            let resolved_struct = haneul_verifier::resolve_struct(view, *idx);
            // is ID
            resolved_struct == RESOLVED_HANEUL_ID
        }
    ...
    if !struct_tag_equals_sig_token(view, function_type_arguments, arg_type, param_type) {
        Err(HaneulError::TypeError {
            error: format!(
                ...
                haneul_verifier::format_signature_token(view, param_type),
                arg_type
            ),
        })
    } else {
...
fn struct_tag_equals_struct_inst(
    ...
    let (address, module_name, struct_name) = haneul_verifier::resolve_struct(view, param_type);
