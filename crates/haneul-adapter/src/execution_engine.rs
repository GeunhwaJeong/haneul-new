...
use haneul_types::coin::{check_coins, transfer_coin, update_input_coins, Coin};
...
use haneul_types::{
    ...
    messages::{
        ...
        TransactionEffects, TransferObject, TransferHaneul,
    },
    object::Object,
...
fn advance_epoch<S: BackingPackageStore + ParentSync + ChildObjectResolver>(
    ...
    let module_id = ModuleId::new(HANEUL_FRAMEWORK_ADDRESS, HANEUL_SYSTEM_MODULE_NAME.to_owned());
    ...
    if result.is_err() {
        tracing::error!(
            ...
            result.as_ref().err(),
            temporary_store.read_object(&HANEUL_SYSTEM_STATE_OBJECT_ID),
            change_epoch,
        );
        temporary_store.reset();
...
fn pay_all_haneul<S>(
    ...
    // overflow is not possible b/c total HANEUL supply is 10B HANEUL or 10^19 GEUNHWA, and 10^19 < u64::MAX
