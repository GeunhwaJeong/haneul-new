...
>(
    ...
    if protocol_config.gas_model_version() > 1 {
        ...
        if !is_genesis_tx && !Mode::allow_arbitrary_values() {
            ...
            let conservation_result = temporary_store.check_haneul_conserved(advance_epoch_gas_summary, enable_expensive_checks);
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = temporary_store.check_haneul_conserved(advance_epoch_gas_summary, enable_expensive_checks) {
