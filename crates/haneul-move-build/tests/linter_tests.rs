...
use haneul_move_build::linters::share_owned::ShareOwnedVerifier;

const HANEUL_FRAMEWORK_PATH: &str = "../haneul-framework/packages/haneul-framework";
const MOVE_STDLIB_PATH: &str = "../haneul-framework/packages/move-stdlib";

fn default_testing_addresses() -> BTreeMap<String, NumericalAddress> {
    let mapping = [("std", "0x1"), ("haneul", "0x2")];
    mapping
        .iter()
...
fn run_tests(path: &Path) -> anyhow::Result<()> {
    ...
    let (files, comments_and_compiler_res) = Compiler::from_files(
        targets,
        vec![MOVE_STDLIB_PATH.to_string(), HANEUL_FRAMEWORK_PATH.to_string()],
