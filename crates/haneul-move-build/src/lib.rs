...
pub mod test_utils {
    ...
    pub async fn compile_managed_coin_package() -> CompiledPackage {
        compile_example_package("../../crates/haneul-core/src/unit_tests/data/managed_coin").await
    }
...
pub struct BuildConfig {
    ...
    /// The Haneul flavor instance, providing network-aware system dependency resolution.
    pub flavor: HaneulFlavor,
}
...
impl BuildConfig {
    ...
    fn compile_package<W: Write + Send, F: MoveFlavor>(
        ...
        if config.default_flavor.is_none() {
            config.default_flavor = Some(move_compiler::editions::Flavor::Haneul);
        }
    ...
    pub async fn build_async_from_root_pkg(
        self,
        root_pkg: &mut RootPackage<HaneulFlavor>,
    ...
    fn internal_build(
        ...
        let (package, fn_info) = result.map_err(|error| {
            HaneulError::from(HaneulErrorKind::ModuleBuildFailure {
