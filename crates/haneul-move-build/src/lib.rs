...
pub mod test_utils {
    use crate::{BuildConfig, CompiledPackage, HaneulPackageHooks};
...
/// Wrapper around the core Move `BuildConfig` with some Haneul-specific info
#[derive(Clone)]
...
impl BuildConfig {
    pub fn new_for_testing() -> Self {
        move_package::package_hooks::register_package_hooks(Box::new(HaneulPackageHooks));
        let install_dir = haneullabs_common::tempdir().unwrap().keep();

        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::Haneul),
    ...
    /// If we are building the Haneul framework, we skip the check that the addresses should be 0
    pub fn build(self, path: &Path) -> HaneulResult<CompiledPackage> {
...
pub fn decorate_warnings(warning_diags: Diagnostics, files: Option<&MappedFiles>) {
    ...
    if any_linter_warnings {
        eprintln!("Please report feedback on the linter warnings at https://forums.haneul.io\n");
    }
...
/// Sets build config's default flavor to `Flavor::Haneul`. Returns error message if the flavor was
/// previously set to something else than `Flavor::Haneul`.
pub fn set_haneul_flavor(build_config: &mut MoveBuildConfig) -> Option<String> {
    use move_compiler::editions::Flavor;

    let flavor = build_config.default_flavor.get_or_insert(Flavor::Haneul);
...
fn collect_bytecode_deps(
    resolution_graph: &ResolvedGraph,
) -> HaneulResult<Vec<(Symbol, CompiledModule)>> {
...
fn verify_bytecode(package: &MoveCompiledPackage, fn_info: &FnInfoMap) -> HaneulResult<()> {
...
impl CompiledPackage {
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in Haneul
...
pub struct HaneulPackageHooks;

impl PackageHooks for HaneulPackageHooks {
...
pub fn check_unpublished_dependencies(unpublished: &BTreeSet<Symbol>) -> Result<(), HaneulError> {
...
pub fn check_conflicting_addresses(
 ...
 - check that your `haneul active-env` corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
