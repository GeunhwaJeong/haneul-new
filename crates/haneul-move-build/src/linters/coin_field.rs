...
use super::{
    ...
    LINT_WARNING_PREFIX, HANEUL_PKG_NAME,
};
    ...
    if let N::StructFields::Defined(sfields) = &sdef.fields {
        for (floc, fname, (_, ftype)) in sfields.iter() {
            if is_field_coin_type(ftype) {
                let msg = format!("The field '{fname}' of '{sname}' has type 'haneul::coin::Coin'");
                let uid_msg = "Storing 'haneul::balance::Balance' in this field will typically be more space-efficient";
    ...
    match t {
        ...
        T::Apply(_, tname, _) => {
            ...
            if let N::TypeName_::ModuleType(mident, sname) = tname {
                return is_mident_haneul_coin(mident) || sname.value() == COIN_STRUCT_NAME.into();
            }
            false
        }
...
fn is_mident_haneul_coin(sp!(_, mident): &E::ModuleIdent) -> bool {
    use E::Address as A;
