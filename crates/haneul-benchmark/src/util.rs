...
use haneul_types::{base_types::HaneulAddress, coin, crypto::HaneulKeyPair, HANEUL_FRAMEWORK_OBJECT_ID};
...
use haneul_types::messages::{CallArg, ObjectArg, TransactionData, VerifiedTransaction};
...
pub async fn split_coin_and_pay(
    ...
    let framework = proxy
        .get_object(HANEUL_FRAMEWORK_OBJECT_ID)
        .await?
    ...
    // we can do both in one tx with pay_haneul which will split the coin out for us before
