                                ...
                                let res = qd
                                    ...
                                    .map(move |res| {
                                        match res {
                                            ...
                                            Err(haneul_err) => {
                                                error!("{}", haneul_err);
                                                metrics_cloned.num_error.with_label_values(&[&b.1.get_workload_type().to_string(), &haneul_err.to_string()]).inc();
                                                NextOp::Retry(b)
                                            }
                                        }
                                    });
                                ...
                                .map(move |res| {
                                    match res {
                                        ...
                                        Err(haneul_err) => {
                                            ...
                                            metrics_cloned.num_error.with_label_values(&[&payload.get_workload_type().to_string(), &haneul_err.to_string()]).inc();
                                            NextOp::Retry(Box::new((tx, payload)))
                                        }
                                    }
                                });
