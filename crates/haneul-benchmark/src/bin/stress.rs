...
use haneul_benchmark::stress::context::get_latest;
use haneul_benchmark::stress::context::Payload;
use haneul_benchmark::stress::context::StressTestCtx;
use haneul_benchmark::stress::shared_counter::SharedCounterTestCtx;
use haneul_benchmark::stress::transfer_object::TransferObjectTestCtx;
...
use haneul_gateway::config::GatewayConfig;
                            ...
                            if free_pool.is_empty() {
                                ...
                                .map(move |res| {
                                    match res {
                                        ...
                                        Err(haneul_err) => {
                                            error!("Retry due to error: {}", haneul_err);
                                            NextOp::Retry(Box::new((tx, payload)))
                                        }
                                    }
                                });
...
fn make_test_ctx(
    ...
    primary_gas_account_owner: HaneulAddress,
    ...
    let (primary_gas_id, owner, keypair, gateway_config) = if opts.local {
        ...
        let _ = std::thread::spawn(move || {
            ...
            server_runtime.block_on(async move {
                // Setup the network
                let nodes: Vec<HaneulNode> = spawn_test_authorities(cloned_gas, &cloned_config).await;
