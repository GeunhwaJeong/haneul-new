...
use haneul_benchmark::drivers::Interval;
use haneul_benchmark::util::get_ed25519_keypair_from_keystore;
use haneul_benchmark::workloads::{
    make_combination_workload, make_shared_counter_workload, make_transfer_object_workload,
};
use haneul_benchmark::FullNodeProxy;
use haneul_benchmark::LocalValidatorAggregatorProxy;
use haneul_benchmark::ValidatorProxy;
use haneul_core::authority_aggregator::reconfig_from_genesis;
...
struct Opts {
    ...
    /// this file is same as what `haneul keytool generate` outputs
    #[clap(long, default_value = "", global = true)]
...
/// it something like:
/// ```cargo run  --release  --package haneul-benchmark
...
/// ```cargo run  --release  --package haneul-benchmark --bin stress -- --num-client-threads 12 \
/// --num-server-threads 10 \
/// --num-transfer-accounts 2 \
/// --primary-gas-id 0x59931dcac57ba20d75321acaf55e8eb5a2c47e9f \
/// --gateway-config-path /tmp/gateway.yaml \
/// --keystore-path /tmp/haneul.keystore bench \
/// --target-qps 100 \
/// --in-flight-ratio 2 \
/// --shared-counter 50 \
/// --transfer-object 50```
...
async fn main() -> Result<()> {
    ...
    let (primary_gas_id, owner, keypair, aggregator) = if opts.local {
        ...
        // spawn a thread to spin up haneul nodes on the multi-threaded server runtime
        ...
        let proxy: Arc<dyn ValidatorProxy + Send + Sync> =
            if let Some(fullnode_url) = opts.fullnode_rpc_address {
                ...
                let genesis = haneul_config::node::Genesis::new_from_file(&opts.genesis_blob_path);
