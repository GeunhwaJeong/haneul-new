...
use haneul_types::crypto::{get_key_pair, EmptySignInfo, KeyPair};
...
type Transfer = (HaneulAddress, HaneulAddress);
...
impl Payload {
    fn make_transaction(
        &self,
        keypairs: &HashMap<HaneulAddress, KeyPair>,
...
async fn init_object_transfer_benchmark(
    ...
    Vec<Payload>,
    Arc<HashMap<HaneulAddress, KeyPair>>,
    AuthorityAggregator<NetworkAuthorityClient>,
) {
    // create several accounts to transfer object between
    let accounts: Arc<HashMap<HaneulAddress, KeyPair>> =
...
async fn run(
    ...
    addresses: Arc<HashMap<HaneulAddress, KeyPair>>,
