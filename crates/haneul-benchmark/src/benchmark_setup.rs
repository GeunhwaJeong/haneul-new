...
use crate::workloads::workload::MIN_HANEUL_PER_WORKLOAD;
use crate::{FullNodeProxy, LocalValidatorAggregatorProxy, ValidatorProxy};
use haneul_types::object::generate_max_test_gas_objects_with_owner;
...
impl Env {
    ...
    async fn setup_local_env(
        ...
        let (address, keypair): (HaneulAddress, AccountKeyPair) = deterministic_random_account_key();
        ...
        for node_config in network_config.validator_configs.iter_mut() {
            ...
            // to calculate the amount of HANEUL in the network. Hence we disable HANEUL conservation checks
            // even when we are running in debug mode.
            node_config
                .expensive_safety_check_config
                .force_disable_epoch_haneul_conservation_check();
    ...
    async fn setup_remote_env(
        ...
        let genesis = haneul_config::node::Genesis::new_from_file(genesis_blob_path);
        ...
        let (balance, primary_gas_obj) = gas_objects
            .iter()
            .filter(|(balance, _)| *balance > MIN_HANEUL_PER_WORKLOAD)
            .collect::<Vec<_>>()
            .choose(&mut rand::thread_rng())
            .context(format!(
                "Failed to choose a random primary gas id with atleast {MIN_HANEUL_PER_WORKLOAD} HANEUL"
            ))?;
