...
pub trait ValidatorProxy {
    ...
    async fn execute_transaction(
        &self,
        tx: Transaction,
    ) -> anyhow::Result<(Option<HaneulCertifiedTransaction>, ExecutionEffects)>;
...
impl ValidatorProxy for LocalValidatorAggregatorProxy {
    ...
    async fn execute_bench_transaction(
        ...
        let certificate = HaneulCertifiedTransaction::try_from(certified_transaction)?;
...
impl ValidatorProxy for FullNodeProxy {
    ...
    async fn execute_transaction(
        ...
        while retry_cnt < 10 {
            ...
            {
                Ok(resp) => {
                    let effects = ExecutionEffects::HaneulTransactionEffects(resp.effects.unwrap());
