...
use haneullabs_metrics::GaugeGuard;
...
use haneul_types::messages_grpc::{HandleCertificateResponseV2, TransactionStatus};
use haneul_types::quorum_driver_types::{QuorumDriverError, QuorumDriverResponse};
...
impl ValidatorProxy for FullNodeProxy {
    ...
    async fn execute_transaction_block(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        while retry_cnt < 10 {
            ...
            {
                Ok(resp) => {
                    return Ok(ExecutionEffects::HaneulTransactionBlockEffects(
