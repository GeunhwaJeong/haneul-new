...
impl LocalValidatorAggregatorProxy {
    ...
    async fn new_impl(
        ...
        let haneul_client = Client::new(&http_url).expect("Failed to create RPC client");
...
impl ValidatorProxy for LocalValidatorAggregatorProxy {
    async fn get_haneul_address_balance(&self, _: HaneulAddress) -> Result<u64, anyhow::Error> {
    ...
    async fn is_transaction_checkpointed(
        ...
        match self.haneul_client.clone().get_transaction(digest).await {
...
async fn execute_soft_bundle_with_retries(
    ...
    use haneul_network::tonic::IntoRequest;
    ...
    loop {
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                // Convert tonic error to HaneulError to check if retriable
                let haneul_error: haneul_types::error::HaneulError = err.into();
                if haneul_error.is_retryable().0
                    && (retry_cnt < max_retries || start.elapsed() < min_retry_duration)
                {
                    warn_and_backoff_for_retry(
                        &digests,
                        &mut retry_cnt,
                        format!("submission failed: {haneul_error:?}"),
                    )
                    .await;
                    continue;
                }
                return Err(haneul_error.into());
            }
        };
        ...
        enum SubmissionOutcome {
            Submitted(haneul_types::messages_consensus::ConsensusPosition),
