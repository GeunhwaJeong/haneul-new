...
use haneul_types::{haneul_system_state::HaneulSystemState, HANEUL_SYSTEM_STATE_OBJECT_ID};
    ...
    pub fn new(proxy: Arc<dyn ValidatorProxy + Send + Sync>) -> Self {
        ...
        tokio::task::spawn(async move {
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        if let Ok(system_state) = proxy.get_object(HANEUL_SYSTEM_STATE_OBJECT_ID).await {
                            let move_obj = system_state.data.try_as_move().unwrap();
                            if let Ok(result) = bcs::from_bytes::<HaneulSystemState>(move_obj.contents()) {
