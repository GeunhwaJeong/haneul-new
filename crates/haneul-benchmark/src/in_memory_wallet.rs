...
pub struct InMemoryWallet {
    accounts: BTreeMap<HaneulAddress, HaneulAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn account_mut(&mut self, addr: &HaneulAddress) -> Option<&mut HaneulAccount> {
    ...
    pub fn owned_object(&self, addr: &HaneulAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &HaneulAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &HaneulAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &HaneulAddress> {
        self.accounts.keys()
    }
