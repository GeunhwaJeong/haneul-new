...
use crate::{
    workloads::workload::{transfer_haneul_for_testing, MAX_GAS_FOR_TESTING},
    ValidatorProxy,
};
...
use haneul_types::{
    base_types::{ObjectDigest, ObjectID, ObjectRef, SequenceNumber, HaneulAddress},
...
pub struct SharedCounterWorkload {
    ...
    pub test_gas_owner: HaneulAddress,
...
impl Workload<dyn Payload> for SharedCounterWorkload {
    async fn init(&mut self, num_counters: u64, proxy: Arc<dyn ValidatorProxy + Sync + Send>) {
        ...
        let (updated, minted) = transfer_haneul_for_testing(
        ...
        .await;
        let publish_module_gas = (address, keypair, minted);
