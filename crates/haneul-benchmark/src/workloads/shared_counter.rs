...
use crate::workloads::workload::{get_latest, transfer_haneul_for_testing, MAX_GAS_FOR_TESTING};
...
impl Workload<dyn Payload> for SharedCounterWorkload {
    async fn init(&mut self, aggregator: Arc<AuthorityAggregator<NetworkAuthorityClient>>) {
        ...
        if let Some((_updated, minted)) = transfer_haneul_for_testing(
    ...
    async fn make_test_payloads(
        ...
        for _ in 0..count {
            let (address, keypair) = get_key_pair();
            if let Some((updated, minted)) = transfer_haneul_for_testing(
                (primary_gas_ref, Owner::AddressOwner(self.test_gas_owner)),
                &self.test_gas_keypair,
                MAX_GAS_FOR_TESTING,
                address,
