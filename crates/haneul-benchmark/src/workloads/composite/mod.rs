...
use haneullabs_common::{assert_reachable, assert_sometimes, debug_fatal};
pub use operations::{
...
pub struct OperationPool {
    ...
    pub chain_identifier: haneul_types::digests::ChainIdentifier,
...
type AliasInitInfo = (HaneulAddress, Arc<AccountKeyPair>, ObjectID, SequenceNumber);

/// Per-payload alias state used at runtime to drive alias-signed transactions and
/// the revoke/re-add cycle.
#[derive(Debug)]
struct AliasState {
    alias_address: HaneulAddress,
    ...
    async fn make_transaction_batch(&mut self) -> Vec<Transaction> {
        ...
        for (i, gas) in current_batch_gas.iter().take(batch_size).enumerate() {
                ...
                && rng.gen_bool(self.config.mixed_gas_payment_probability as f64)
            {
                ...
                let accumulator_obj_id = AccumulatorValue::get_field_id(sender, &haneul_balance_type)
...
pub struct AccountState {
    ...
    pub partner_address: HaneulAddress,
}
...
impl AccountState {
    pub async fn new(
        ...
        while retries < 3 {
            ...
            let Ok(haneul_balance) = proxy.get_haneul_address_balance(sender).await else {
                info!("Failed to get haneul balance for address {sender}");
                retries += 1;
                tokio::time::sleep(Duration::from_secs(1)).await;
                continue;
            };
            assert_reachable!("successfully got haneul balance for address");
            let pool_balance = if let Some((pool_id, _)) = balance_pool {
                let pool_address: HaneulAddress = pool_id.into();
                proxy
                    .get_haneul_address_balance(pool_address)
                    .await
                    .unwrap_or(0)
            } else {
                0
...
pub struct CompositeWorkload {
    ...
    num_payloads: u64,
    metrics: Arc<Mutex<CompositionMetrics>>,
    chain_identifier: Option<haneul_types::digests::ChainIdentifier>,
...
impl Workload<dyn Payload> for CompositeWorkload {
    async fn init(
        ...
        if init_requirements.contains(&InitRequirement::SeedAddressBalance) {
            ...
            info!(
                "Seeding address balances with {} GEUNHWA for {} addresses",
                seed_amount,
        ...
        if init_requirements.contains(&InitRequirement::SeedBalancePool) {
            let seed_amount = self.config.address_balance_amount * 100;
            info!("Seeding balance pool with {} GEUNHWA", seed_amount);
                ...
                let enable_tx = TestTransactionBuilder::new(*sender, *gas, gas_price)
                    .move_call(
                        ...
                        vec![CallArg::Object(ObjectArg::SharedObject {
                            id: HANEUL_ADDRESS_ALIAS_STATE_OBJECT_ID,
                            initial_shared_version: alias_state_isv,
                            mutability: SharedObjectMutability::Mutable,
                        })],
                    )
    ...
    async fn make_test_payloads(
        ...
        let payload_addresses: Vec<HaneulAddress> = self
            .payload_gas
            .iter()
        ...
        let get_partner_address = |i: usize| -> HaneulAddress {
