...
impl Operation for CoinReservationWithdraw {
    ...
    fn apply(
        ...
        // Use the seeded amount rather than `account_state.haneul_balance`, which reflects total
        ...
        // `address_balance_amount * 100` GEUNHWA when a withdrawal is possible.
        ...
        let accumulator_obj_id =
            AccumulatorValue::get_field_id(account_state.sender, &haneul_balance_type)
        ...
        // converting it to a Coin<HANEUL>. We then convert to Balance<HANEUL> and send to the
        // partner's accumulator so the partner can do a future CoinReservationWithdraw.
        // Simply transferring a Coin<HANEUL> would only give the partner a coin object, not
        // accumulator balance.
        let coin_balance = builder.programmable_move_call(
            HANEUL_FRAMEWORK_PACKAGE_ID,
            Identifier::new("coin").unwrap(),
