...
mod test {
    ...
    fn handle_failpoint(
        ...
        client_node: haneul_simulator::task::NodeId,
        probability: f64,
    ) {
    ...
    async fn test_simulated_load_reconfig_with_crashes_and_delays() {
        ...
        let client_node = haneul_simulator::current_simnode_id();
    ...
    async fn test_testnet_upgrade_compatibility() {
        ...
        let init_framework =
            haneul_framework_snapshot::load_bytecode_snapshot("testnet", min_ver).unwrap();
        ...
        let _handle = tokio::task::spawn(async move {
            for version in min_ver..=max_ver {
                ...
                let new_framework =
                    haneul_framework_snapshot::load_bytecode_snapshot("testnet", next_version);
    ...
    impl TestInitData {
        pub async fn new(test_cluster: &TestCluster) -> Self {
            ...
            Self {
                keystore_path: test_cluster.swarm.dir().join(HANEUL_KEYSTORE_FILENAME),
                genesis: test_cluster.swarm.config().genesis.clone(),
                all_gas: get_haneul_gas_object_with_wallet_context(&test_cluster.wallet, &sender)
                    .await,
                sender,
            }
        }
