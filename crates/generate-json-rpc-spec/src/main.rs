...
const FILE_PATH: &str = concat!(
    ...
    "/../haneul-open-rpc/spec/openrpc.json",
);

const OBJECT_SAMPLE_FILE_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../haneul-open-rpc/samples/objects.json",
);

const TRANSACTION_SAMPLE_FILE_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../haneul-open-rpc/samples/transactions.json",
);
...
async fn create_response_sample(
    ...
    let config = network.dir().join(HANEUL_WALLET_CONFIG);
