...
- Provides deterministic, randomized execution of an entire Haneul network in a single process.
...
The code for the simulator itself lives in the https://github.com/GeunhwaJeong/mysten-sim repository.
It has the following main components:

1. A [runtime](https://github.com/GeunhwaJeong/mysten-sim/blob/main/msim/src/sim/runtime/mod.rs) which provides:
...
1. An API-compatible replacement for tokio.
    ...
    - A minimal [fork of tokio](https://github.com/GeunhwaJeongmark/tokio-madsim-fork) is required in order to expose certain internals to the simulator. This fork has very few modifications, which were written to be easily rebaseable when new tokio releases come out.
...
1. A library of interceptor functions which intercept various posix API calls in order to enforce determinism throughout the test. These include:
    ...
    This interception behavior is in effect only in threads that have explicitly enabled it, which generally includes the main test thread only. In other threads, the interceptors delegate the call to the system library implementation via `dlsym()`. See implementation [here](https://github.com/GeunhwaJeong/mysten-sim/blob/main/msim/src/sim/intercept.rs#L34-L48).

1. Procedural macros that replace `#[tokio::test]` and run test code inside a testing environment. These are `#[haneul_test]` and `#[sim_test]` and are documented below. The test harness created by these macros initializes the simulator runtime with a starting seed, generates the simulator configuration, and runs the test inside a newly created thread. The test must be run in its own thread in order to provide each test case with fresh thread local storage.
...
The easiest way to write tests that run in the simulation testing framework is to use [SwarmBuilder](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-swarm/src/memory/swarm.rs#L47) to start your validators.
...
https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul/tests/checkpoints_tests.rs is a good example of this.
...
To address this, most such test code launches validators via the `spawn_test_authorities` function in the `test-utils` carate, which returns `Vec<HaneulNodeHandle>` rather than `Vec<HaneulNode>`.
`HaneulNodeHandle` hides the `HaneulNode` from the test code.
...
`HaneulNodeHandle` runs the provided callbacks/futures inside the context of the appropriate simulator node, so that network requests, spawned tasks, etc continue running in the correct context.
...
- `config = "config_expr"` - This argument accepts a string which will be evaluated as an expression that returns the configuration for the test. Generally, you should make this a function call, and then define the function to return the config. The function must return a type that can implements `Into<TestConfig>` - the most common choice is `SimConfig`, but `Vec<SimConfig>` and `Vec<(usize /* repeat count */, SimConfig)>` are also supported by default. See https://github.com/GeunhwaJeong/mysten-sim/blob/main/msim/src/sim/config.rs for the `TestConfig` implementation.
...
Network latency and packet loss can be configured using [SimConfig](https://github.com/GeunhwaJeong/mysten-sim/blob/main/msim/src/sim/config.rs#L8), which is re-exported from this crate as `haneul_simulator::config::SimConfig`.
...
Documentation of network configuration is not finished yet, but reading the code for the [NetworkConfig](https://github.com/GeunhwaJeong/mysten-sim/blob/main/msim/src/sim/net/config.rs#L221) should be very instructive.

There is a small but growing library of functions for building network configs in [haneul_simulator::configs](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-simulator/src/lib.rs).

There are also some examples of network configuration at https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-benchmark/tests/simtest.rs#L52.
