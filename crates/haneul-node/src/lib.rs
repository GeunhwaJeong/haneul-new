...
use haneul_core::validator_tx_finalizer::ValidatorTxFinalizer;
...
pub async fn build_http_server(
    ...
    let json_rpc_router = {
        ...
        let name_service_config =
            if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
                ...
                haneul_json_rpc::name_service::NameServiceConfig::default()
            };
    ...
    if config.enable_experimental_rest_api {
        let mut rest_service = haneul_rest_api::RestService::new(
    ...
    info!(local_addr =? addr, "Haneul JSON-RPC server listening on {addr}");
