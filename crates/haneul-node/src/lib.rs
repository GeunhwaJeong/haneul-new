...
use haneul_network::endpoint_manager::EndpointManager;
...
mod simulator {
    ...
    pub(super) struct SimState {
        pub sim_node: haneul_simulator::runtime::NodeHandle,
        pub sim_safe_mode_expected: AtomicBool,
        _leak_detector: haneul_simulator::NodeLeakDetector,
    }
    ...
    impl Default for SimState {
        fn default() -> Self {
            Self {
                ...
                _leak_detector: haneul_simulator::NodeLeakDetector::new(),
            }
        }
    }

    type JwkInjector = dyn Fn(AuthorityName, &OIDCProvider) -> HaneulResult<Vec<(JwkId, JWK)>>
        + Send
        + Sync
        + 'static;
    ...
    ) -> HaneulResult<Vec<(JwkId, JWK)>> {
        use fastcrypto_zkp::bn254::zk_login::parse_jwks;
        // Just load a default Twitch jwk for testing.
        parse_jwks(
            haneul_types::zk_login_util::DEFAULT_JWK_BYTES,
            &OIDCProvider::Twitch,
            true,
        )
        .map_err(|_| HaneulErrorKind::JWKRetrievalError.into())
    }
...
pub struct HaneulNode {
    ...
    end_of_epoch_channel: broadcast::Sender<HaneulSystemState>,
...
impl HaneulNode {
    pub async fn start(
        ...
        Self::start_async(
            config,
            registry_service,
            ServerVersion::new("haneul-node", "unknown"),
        )
        .await
    }
    ...
    pub async fn start_async(
        ...
        if is_genesis {
            info!("checking HANEUL conservation at genesis");
    ...
    pub async fn close_epoch(&self, epoch_store: &Arc<AuthorityPerEpochStore>) -> HaneulResult {
        info!("close_epoch (current epoch = {})", epoch_store.epoch());
        self.validator_components
            .lock()
            .await
            .as_ref()
            .ok_or_else(|| HaneulError::from("Node is not a validator"))?
            .consensus_adapter
            .close_epoch(epoch_store);
        Ok(())
    }

    pub fn clear_override_protocol_upgrade_buffer_stake(&self, epoch: EpochId) -> HaneulResult {
        self.state
        ...
        let p2p_network = {
            ...
            let service = ServiceBuilder::new()
                ...
                .layer(CallbackLayer::new(
                    haneullabs_network::metrics::MetricsMakeCallbackHandler::new(
            ...
            let server_name = format!("haneul-{}", chain_identifier);
    ...
    async fn start_grpc_validator_service(
        ...
        let tls_config = haneul_tls::create_rustls_server_config(
        ...
        if tokio::time::timeout(
            ...
            state
                ...
                .notify_read_executed_effects_digests(
                    "HaneulNode::notify_read_executed_effects_digests",
                    &digests,
                ),
        )
        .await
    ...
    pub async fn monitor_reconfiguration(
        ...
        loop {
            ...
            if stop_condition == StopReason::RunWithRangeCondition {
                HaneulNode::shutdown(&self).await;
    ...
    fn try_recover_checkpoint_fork(
        ...
        for (seq, expected_digest_str) in &recovery.checkpoint_overrides {
            ...
            if let Some(local_summary) = checkpoint_store.get_locally_computed_checkpoint(*seq)? {
                let local_digest = haneul_types::message_envelope::Message::digest(&local_summary);
...
async fn build_http_servers(
    ...
    let layers = ServiceBuilder::new()
        .map_request(|mut request: axum::http::Request<_>| {
            if let Some(connect_info) = request.extensions().get::<haneul_http::ConnectInfo>() {
    ...
    {
        let https = haneul_http::Builder::new()
    ...
    let http = haneul_http::Builder::new()
...
struct HttpServers {
    #[allow(unused)]
    http: Option<haneul_http::ServerHandle>,
    #[allow(unused)]
    https: Option<haneul_http::ServerHandle>,
}
