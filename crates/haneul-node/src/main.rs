...
fn main() {
    ...
    move_vm_profiler::gas_profiler_feature_enabled! {
        panic!("Cannot run the haneul-node binary with gas-profiler feature enabled");
    }
    ...
    runtimes.haneul_node.spawn(async move {
        match haneul_node::HaneulNode::start_async(&config, registry_service, Some(rpc_runtime)).await {
    ...
    runtimes.metrics.spawn(async move {
        ...
        haneul_node::admin::run_admin_server(node, admin_interface_port, filter_handle).await
    });
