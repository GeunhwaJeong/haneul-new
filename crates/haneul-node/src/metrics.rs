...
impl MetricsPushClient {
    pub fn new(network_key: haneul_types::crypto::NetworkKeyPair) -> Self {
...
/// is configured.
pub fn start_metrics_push_task(config: &haneul_config::NodeConfig, registry: RegistryService) {
    use fastcrypto::traits::KeyPair;
    use haneul_config::node::MetricsConfig;
...
mod tests {
    ...
    pub async fn test_metrics_endpoint_with_multiple_registries_add_remove() {
        ...
        let registry_2 = Registry::new_custom(Some("haneul".to_string()), None).unwrap();
        ...
        assert!(result.contains(
            "# HELP haneul_counter_2 a sample counter 2
# TYPE haneul_counter_2 counter
haneul_counter_2 0"
        ));
