...
/// Start a Haneul validator and fullnode for easy testing.
#[derive(Parser, Debug)]
...
struct Args {
    ...
    /// Port to start the Haneul faucet on
...
async fn main() -> Result<()> {
    ...
    let genesis_config_option = if with_persisted {
        let cluster_config_network_config = haneul_cluster_test_config_dir()?.join(HANEUL_NETWORK_CONFIG);
        // Auto genesis if path is none and haneul directory doesn't exists.
        if !cluster_config_network_config.exists() {
            genesis(
                None,
                None,
                Some(haneul_cluster_test_config_dir()?),
                false,
                None,
                None,
            )
        ...
        let haneul_cluster_config_dir = haneul_cluster_test_config_dir()?;
        let keystore_path = haneul_cluster_config_dir.join(HANEUL_KEYSTORE_FILENAME);
    ...
    let cluster = LocalNewCluster::start(
        &ClusterTestOpt {
            ...
            pg_address: with_indexer.then_some(format!(
                "postgres://postgres@{pg_host}:{pg_port}/haneul_indexer"
            )),
            faucet_address: None,
...
async fn faucet_request(
    ...
    let result = match payload {
        FaucetRequest::FixedAmountRequest(FixedAmountRequest { recipient }) => {
            state.faucet.request_haneul_coins(recipient).await
        }
    };
