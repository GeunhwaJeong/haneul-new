...
struct Args {
    ...
    /// haneul genesis -f --with-faucet generates a genesis config that can be used to start this process.
    /// Example: haneul-test-validator --config-dir ~/.haneul/haneul_config
...
async fn main() -> Result<()> {
    ...
    println!("Starting Haneul validator with config: {:#?}", cluster_config);
...
async fn faucet_batch_request(
    ...
    let result = match payload {
        FaucetRequest::FixedAmountRequest(FixedAmountRequest { recipient }) => {
            state.faucet.batch_request_haneul_coins(recipient).await
        }
        _ => {
            return (
                StatusCode::BAD_REQUEST,
