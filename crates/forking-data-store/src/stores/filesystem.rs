...
//! `FORKING_DATA_STORE` first, then `HANEUL_CONFIG_DIR`, `HOME`, or
...
impl FileSystemStore {
    ...
    pub fn base_path() -> Result<PathBuf, Error> {
        let home_dir = std::env::var("FORKING_DATA_STORE")
            .or_else(|_| std::env::var("HANEUL_CONFIG_DIR"))
            .or_else(|_| std::env::var("HOME"))
            .or_else(|_| std::env::var("USERPROFILE"))
            .map_err(|_| {
                anyhow!(
                    "Cannot determine home directory. Define a HANEUL_DATA_STORE environment variable"
                )
            })?;
    ...
    pub fn get_objects_by_owner(&self, _owner: HaneulAddress) -> Result<Vec<Object>, Error> {
