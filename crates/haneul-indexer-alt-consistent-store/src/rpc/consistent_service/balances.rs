...
) -> Vec<(Vec<u8>, (HaneulAddress, TypeTag), i128, u128)> {
...
/// Parse the owner's `HaneulAddress` from a string. Addresses must start with `0x` followed by
/// between 1 and 64 hexadecimal characters.
///
/// TODO: Switch to using `haneul_sdk_types::Address`, once the indexing framework is ported to the
/// new SDK.
fn owner(input: &str) -> Result<HaneulAddress, Error> {
    ...
    HaneulAddress::from_str(s.as_ref()).map_err(|_| Error::InvalidOwner(s.to_string()))
}
...
mod tests {
    ...
    fn coin_key(owner: HaneulAddress, type_str: &str) -> CoinKey {
        CoinKey {
            owner,
    ...
    fn address_key(owner: HaneulAddress, type_str: &str) -> AddressKey {
        AddressKey {
            owner,
            type_: TypeTag::from_str(type_str).unwrap(),
        }
