...
For external types, you'll need to implement the trait here in [`external_impls.rs`](https://github.com/GeunhwaJeong/blob/main/crates/haneullabs-util-mem/src/external_impls.rs). See the existing implementations in that file for examples.
...
To compute the heap usage of an annotated type at runtime, simply call `haneullabs_util_mem::malloc_size(&my_struct)`. For complete memory usage, add in the inline size of the type as well, as in:

```rust
haneullabs_util_mem::malloc_size(&my_struct) + std::mem::size_of::<MyStruct>()
```

## Putting it all together

For an example PR using library, take a look at https://github.com/GeunhwaJeong/narwhal/pull/898/files.
