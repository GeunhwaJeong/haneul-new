...
impl ObjectProcessor {
    ...
    async fn get_original_package_id(
        ...
        let haneul_types::object::Data::Package(p) = &object.data else {
    ...
    async fn process_object(
            ...
            .and_then(|tag| object.data.try_as_move().map(|mo| (tag, mo.contents())))
        {
            match get_move_struct(&tag, contents, &Resolver::new(self.package_cache.clone())).await
            {
                ...
                Err(err)
                    if err
                        .downcast_ref::<haneul_types::object::bounded_visitor::Error>()
                        .filter(|e| {
                            matches!(e, haneul_types::object::bounded_visitor::Error::OutOfBudget)
                        })
                        .is_some() =>
                {
                    self.metrics
        ...
        let (struct_tag, haneul_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                HaneulMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(HaneulMoveStruct::WithFields(fields)))
                }
        ...
        let row = ObjectRow {
            ...
            object_json: haneul_move_struct.map(|x| x.to_json_value().to_string()),
        };
        Ok(Some(row))
    }
}
