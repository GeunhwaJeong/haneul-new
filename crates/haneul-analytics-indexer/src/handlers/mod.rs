...
use haneul_json_rpc_types::{HaneulMoveStruct, HaneulMoveValue};
...
fn parse_struct_field(
    path: &str,
    haneul_move_value: HaneulMoveValue,
    curr_struct: &mut WrappedStruct,
    all_structs: &mut BTreeMap<String, WrappedStruct>,
) {
    match haneul_move_value {
        HaneulMoveValue::Struct(move_struct) => parse_struct(path, move_struct, all_structs),
        HaneulMoveValue::Vector(fields) => {
        ...
        HaneulMoveValue::Option(option_haneul_move_value) => {
            if option_haneul_move_value.is_some() {
                parse_struct_field(
                    path,
                    option_haneul_move_value.unwrap(),
                    curr_struct,
                    all_structs,
                );
            }
        }
        HaneulMoveValue::UID { id } => curr_struct.object_id = Some(id),
        _ => {}
    }
}
...
fn parse_struct(
    path: &str,
    haneul_move_struct: HaneulMoveStruct,
    all_structs: &mut BTreeMap<String, WrappedStruct>,
) {
    let mut wrapped_struct = WrappedStruct::default();
    match haneul_move_struct {
...
mod tests {
    ...
    async fn test_wrapped_object_parsing_simple() -> anyhow::Result<()> {
        ...
        let move_struct: HaneulMoveStruct = serde_json::from_str(input).unwrap();
