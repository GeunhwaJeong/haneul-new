...
pub async fn build_analytics_indexer(
    ...
    let work_dir = if let Some(ref work_dir) = config.work_dir {
        tempfile::Builder::new()
            .prefix("haneul-analytics-indexer-")
            .tempdir_in(work_dir)?
            .keep()
    } else {
        tempfile::Builder::new()
    ...
    let client_args = haneul_indexer_alt_framework::ingestion::ClientArgs {
        ingestion: haneul_indexer_alt_framework::ingestion::ingestion_client::IngestionClientArgs {
        ...
        streaming: haneul_indexer_alt_framework::ingestion::streaming_client::StreamingClientArgs {
