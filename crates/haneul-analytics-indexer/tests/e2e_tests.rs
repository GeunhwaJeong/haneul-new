...
use prost::Message;
use haneul_analytics_indexer::config::CommitterLayer;
use haneul_analytics_indexer::config::IngestionLayer;
...
impl TestHarness {
    ...
    fn default_client_args(&self) -> haneul_indexer_alt_framework::ingestion::ClientArgs {
    ...
    async fn run_indexer(
        ...
        let indexer_args = haneul_indexer_alt_framework::IndexerArgs {
            first_checkpoint,
            last_checkpoint,
            ..Default::default()
        };
        ...
        let service = haneul_analytics_indexer::build_analytics_indexer(
            config,
            indexer_args,
...
async fn test_migration_no_files_after_checkpoint_error() {
    ...
    let client_args = haneul_indexer_alt_framework::ingestion::ClientArgs::default();
    let registry = prometheus::Registry::new();
    let metrics = Metrics::new(&registry);

    // This should fail during indexer build (when loading file ranges)
    let result = haneul_analytics_indexer::build_analytics_indexer(
        migration_config,
        indexer_args,
        client_args,
        metrics,
