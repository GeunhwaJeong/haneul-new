...
impl CheckpointReader {
    ...
    fn start_remote_fetcher(&mut self) -> mpsc::Receiver<Result<(Arc<CheckpointData>, usize)>> {
        ...
        let store = if let Some((fn_url, remote_url)) = url.split_once('|') {
            ...
            RemoteStore::Hybrid(object_store, haneul_rpc_api::Client::new(fn_url).unwrap())
        } else if url.ends_with("/rest") {
            RemoteStore::Rest(haneul_rpc_api::Client::new(url).unwrap())
        } else {
            let object_store = create_remote_store_client(
                url,
                self.remote_store_options.clone(),
...
fn transform_ingestion_url(ingestion_url: String) -> String {
    ...
    if ingestion_url.contains("checkpoints.mainnet.haneul.io") {
        "https://storage.googleapis.com/haneullabs-mainnet-checkpoints".to_string()
    } else if ingestion_url.contains("checkpoints.testnet.haneul.io") {
