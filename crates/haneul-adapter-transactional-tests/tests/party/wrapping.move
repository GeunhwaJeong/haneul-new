...
public fun create_party(ctx: &mut TxContext) {
    ...
    transfer::party_transfer(a, haneul::party::single_owner(ctx.sender()))
}

public fun wrap(a: A, ctx: &mut TxContext) {
    let wrapper = AWrapper { id: object::new(ctx), a };
    transfer::party_transfer(wrapper, haneul::party::single_owner(ctx.sender()))
}

public fun unwrap(wrapper: AWrapper, ctx: &mut TxContext) {
    let AWrapper { id, a } = wrapper;
    transfer::party_transfer(a, haneul::party::single_owner(tx_context::sender(ctx)));
    object::delete(id)
}
