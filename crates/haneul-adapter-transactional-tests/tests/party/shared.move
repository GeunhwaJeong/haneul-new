...
public fun create_party(ctx: &mut TxContext) {
    let s = S { id: object::new(ctx) };
    transfer::party_transfer(s, haneul::party::single_owner(ctx.sender()))
}
...
//> haneul::transfer::public_party_transfer<ex::m::S>(Input(0), Result(0))
...
//> haneul::transfer::public_share_object<ex::m::S>(Input(0))