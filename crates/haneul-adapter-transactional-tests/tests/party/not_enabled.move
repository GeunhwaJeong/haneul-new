...
public fun create_party(ctx: &mut TxContext) {
    let p = Pub { id: object::new(ctx) };
    transfer::public_party_transfer(p, haneul::party::single_owner(@0))
}

public fun pub_party(obj: Pub, p: haneul::party::Party) {
    transfer::public_party_transfer(obj, p)
}

public fun priv_party(obj: Priv, p: haneul::party::Party) {
...
//> 0: haneul::party::single_owner(Input(1));
...
//# programmable --inputs object(2,1) @0
//> 0: haneul::party::single_owner(Input(1));
//> haneul::transfer::public_party_transfer<ex::m::Pub>(Input(0), Result(0))
