...
module t::m {
    ...
    public entry fun create(ctx: &mut TxContext) {
        ...
        haneul::dynamic_field::add(&mut o.id, 0u64, Obj { id: object::new(ctx) });
        haneul::dynamic_object_field::add(&mut o.id, 0u64, Obj { id: object::new(ctx) });
    ...
    public entry fun share_wrapped(o: &mut Obj) {
        let inner: Obj = haneul::dynamic_field::remove(&mut o.id, 0u64);
        transfer::public_share_object(inner)
    }

    public entry fun share_child(o: &mut Obj) {
        let inner: Obj = haneul::dynamic_object_field::remove(&mut o.id, 0u64);
        transfer::public_share_object(inner)
    }
