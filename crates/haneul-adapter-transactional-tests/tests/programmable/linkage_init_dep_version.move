...
public struct Config has key { id: haneul::object::UID, v: u64 }

// Published while only Dep v1 exists => init runs against v1 => v = 1.
fun init(ctx: &mut haneul::tx_context::TxContext) {
    haneul::transfer::share_object(Config { id: haneul::object::new(ctx), v: d::val() })
}
...
//> 0: Publish(PPtb, [Dep_V2, haneul, std]);
...
//> 0: Publish(PPtb, [Dep_V1, haneul, std]);
//> 1: Dep_V2::d::ping();
