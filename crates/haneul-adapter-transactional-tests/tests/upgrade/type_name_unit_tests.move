...
module A2::m {
    ...
    fun dt_name(str: vector<u8>): vector<u8> {
        ...
        str
            .skip(haneul::address::length() * 2 + 2 + 1 + 2)
            .take_while!(|c| c != &lt)
    }
    ...
    macro fun case<$T>($original: address, $defining: address, $name: vector<u8>) {
        ...
        assert!(
            haneul::address::from_ascii_bytes(og.address_string().as_bytes()) ==
        ...
        assert!(
            haneul::address::from_ascii_bytes(def.address_string().as_bytes()) ==
    ...
    // peels the leading address and parses it using `haneul::address::from_ascii_bytes`
    fun leading_address(str: vector<u8>): address {
        haneul::address::from_ascii_bytes(&str.take(haneul::address::length() * 2))
    }
