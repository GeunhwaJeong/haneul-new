...
// Test settled_funds_value from haneul::balance
...
public fun balance_check(acc: &haneul::accumulator::AccumulatorRoot, addr: address, expected: u64) {
  let balance = haneul::balance::settled_funds_value<haneul::haneul::HANEUL>(acc, addr);

  assert!(balance == expected, 100);
}

public fun balance_check_fake(
  acc: &haneul::accumulator::AccumulatorRoot,
  addr: address,
  expected: u64,
) {
  let balance = haneul::balance::settled_funds_value<u64>(acc, addr);
...
public fun send_max(holder: &mut SupplyHolder, key: u64, recipient: address) {
  ...
  let mut supply = haneul::balance::create_supply<u64>(0);
  let balance = supply.increase_supply(increase);
  haneul::balance::send_funds(balance, recipient);
  haneul::dynamic_field::add(&mut holder.id, key, supply);
}

//# programmable --sender A --inputs 1042 @B
// Send some HANEUL from A to B
