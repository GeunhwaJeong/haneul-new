...
//# programmable --sender A --address-balance-gas --gas-price 0 --gas-budget 0 --inputs withdraw<haneul::balance::Balance<test::usdc::USDC>>(9500) @B
...
//# programmable --sender A --address-balance-gas --gas-price 0 --gas-budget 0 --inputs withdraw<haneul::balance::Balance<test::usdc::USDC>>(3000) @B 2500 @A
// Reject: withdraw 3000, send 2500 to B, send 500 back to self (below min) - fails at execution
//> 0: haneul::balance::redeem_funds<test::usdc::USDC>(Input(0));
//> 1: haneul::balance::split<test::usdc::USDC>(Result(0), Input(2));
//> 2: haneul::balance::send_funds<test::usdc::USDC>(Result(1), Input(1));
//> 3: haneul::balance::send_funds<test::usdc::USDC>(Result(0), Input(3));

//# programmable --sender A --address-balance-gas --gas-price 0 --gas-budget 0 --inputs withdraw<haneul::balance::Balance<test::usdc::USDC>>(2000) @B 1500u256
// Reject: withdrawal_split 1500 from 2000, drop remaining 500 (below min) - fails at execution
//> 0: haneul::funds_accumulator::withdrawal_split<haneul::balance::Balance<test::usdc::USDC>>(Input(0), Input(2));
