...
impl<M: MetricsCallbackProvider> ServerBuilder<M> {
    ...
    pub async fn bind(self, addr: &Multiaddr, tls_config: Option<ServerConfig>) -> Result<Server> {
        ...
        let mut builder = haneul_http::Builder::new().config(http_config);
...
/// TLS server name to use for the public Haneul validator interface.
pub const HANEUL_TLS_SERVER_NAME: &str = "haneul";

pub struct Server {
    server: haneul_http::ServerHandle,
...
impl Server {
    ...
    pub fn handle(&self) -> &haneul_http::ServerHandle {
        &self.server
    }
}
...
mod test {
    ...
    async fn test_metrics_layer_successful() {
        ...
        let server = super::ServerBuilder::from_config(&config, metrics.clone())
            .bind(
                &address,
                Some(haneul_tls::create_rustls_server_config(
