...
fn main() {
    ...
    // under `src/categories/` into the binary so `haneul prompt skill <bundle>` and
    // `haneul prompt category <name>` can return their contents at runtime without touching
    ...
    // graph and extract category descriptions for `haneul prompt categories`.
    ...
    //    included — is embedded raw and served verbatim by `haneul prompt category <name>`,
