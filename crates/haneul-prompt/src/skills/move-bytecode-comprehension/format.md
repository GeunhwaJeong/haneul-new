...
Reference (public): [`move-binary-format/src/file_format.rs`](https://github.com/GeunhwaJeong/haneul/blob/main/external-crates/move/crates/move-binary-format/src/file_format.rs).
...
- **References:** `[Mut]BorrowLoc, [Mut]BorrowField, [Mut]BorrowGlobal*` (note: global storage ops
  are largely unused in Haneul's object model), `ReadRef, WriteRef, FreezeRef`.
...
What you will NOT find here: `MoveTo/MoveFrom/Exists` global-storage ops — Haneul uses the
