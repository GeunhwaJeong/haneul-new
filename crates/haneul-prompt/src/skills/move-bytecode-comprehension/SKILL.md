...
description: >
  Use when reading or reasoning about compiled Move bytecode or `haneul move disassemble`
...
> default to `haneul prompt skill move-bytecode-comprehension --all`, or `--list` + `--file
...
On-chain Haneul packages are compiled **Move bytecode** (`.mv`, magic `0xA11CEB0B`). For
analysis you read it as **disassembly** (stack-machine, via `haneul move disassemble`). This
...
- **Disassembly is the view.** Apply analyses to the `.asm` files produced by
  `haneul-and-move-tools/fetch-and-disassemble.md`. Abilities, visibility, the `entry` flag,
