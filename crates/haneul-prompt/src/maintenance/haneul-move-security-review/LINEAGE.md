# Lineage — derivation of the SM-* catalog from `HaneulLabs/skills`

The SM-* rules in this catalog were derived (2026) by analyzing the constructive
"how to write correct Haneul Move" guidance in the public
[`HaneulLabs/skills`](https://github.com/GeunhwaJeong/skills) repository. Each rule is the
...
- `haneul-move/` (SKILL.md, move.md, events-coins.md)
...
- `haneul-publish/SKILL.md`
- `haneul-move-project/SKILL.md`
- `haneul-build-test/SKILL.md`
- `frontend-apps/` (SKILL.md, limitations.md, transactions.md, queries.md, setup.md)
- `haneul-sdks/` (typescript.md, rust.md)
- `accessing-data/` (SKILL.md, grpc.md, graphql.md, indexers.md, walrus.md, archival.md, use-cases.md)
- `haneul-cli/SKILL.md`, `haneul-client/SKILL.md`, `haneul-install/SKILL.md`
- `haneul-overview/ecosystem.md`

## Per-rule attribution

Every SM-* rule's `Source:` line cites the upstream file(s) it was derived from, written
in the form `Source: HaneulLabs/skills → <relative-path>`. Rules tagged `[+domain]` come from
established Haneul/Move auditing practice and are NOT directly from the upstream skills (notable
...
To refresh the SM-* catalog against an updated `HaneulLabs/skills` snapshot:

1. **Clone** `HaneulLabs/skills` at the new ref:
   ```sh
   git clone https://github.com/GeunhwaJeong/skills.git /tmp/upstream-skills
   ```
...
6. **Rebuild the Haneul CLI** so the embedded skill content reflects the refreshed catalog.

## Why this matters

The audit skill catalog is downstream of the constructive skills. When Haneul/Move evolves (new
