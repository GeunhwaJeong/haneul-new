---
name: audit
description: Security review of Move code on Haneul (source or compiled bytecode); for audits, vulnerabilities, or suspected bugs.
skills:
  - haneul-and-move-tools
  - move-bytecode-comprehension
  - haneul-move-security-review
---

# Auditing Move packages on Haneul

**Run `haneul prompt category audit --all` to load the full catalog.** Every
...
this-catalog shape — `[+domain]` rules, Haneul-specific framings, absence-detection
disciplines — may differ from your prior training. Don't declare the catalog
loaded until every file is actually loaded.

Auditing here means finding security vulnerabilities in Move code on Haneul by walking a
...
`haneul-move-security-review/auditing-bytecode.md` for per-rule opcode-pattern signals. For
