...
**Run `haneul prompt category bytecode --all` to load the full catalog.** Every
...
this-catalog shape — Haneul-specific framings, the survival table — may differ
...
disassembly (`haneul move disassemble` output), which is 1:1 with the executed bytecode.
