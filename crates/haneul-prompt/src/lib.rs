...
//! `haneul prompt` — the entry point to expert Haneul and Move knowledge, shipped as part of the Haneul CLI.
//!
//! See `crates/haneul-prompt/README.md` for usage with worked examples.
//!
//! ## Architecture
//!
//! - **Skills** and **categories** are embedded at **build time** (see
//!   `crates/haneul-prompt/build.rs`). The generated `$OUT_DIR/embedded.rs` defines [`SKILL_FILES`]
...
/// The `haneul prompt` no-args overview rendered to the agent. Source is
...
pub struct PromptSkillFile {
    /// Skill bundle name (top-level directory under `crates/haneul-prompt/src/skills/`).
...
/// `CATEGORY.md` (frontmatter included) — that is what `haneul prompt category <name>`
...
pub struct PromptCategory {
    ...
    /// build time. Used by `haneul prompt categories` for the listing.
...
#[clap(
    name = "prompt",
    about = "Expert Haneul and Move knowledge for AI agents (run `haneul prompt` to start)"
)]
pub struct Prompt {
...
/// Subcommands of `haneul prompt`. Each variant maps 1:1 to a `print_*` helper below.
#[derive(Subcommand)]
pub enum PromptCommand {
    ...
    Category {
        /// Category name (e.g. `audit`). See `haneul prompt categories`.
        name: String,
    ...
    Skill {
        /// Skill bundle name (e.g. `haneul-move-security-review`).
        bundle: String,
...
/// Dispatch `haneul prompt` (called from `crates/haneul/src/haneul_commands.rs`). Mirrors the
/// shape of `haneul_move::execute_move_command` for consistency with the `haneul move`
/// delegation pattern.
...
fn print_skills() {
    ...
    let commands: &[(&str, &str)] = &[
        (
            "haneul prompt skill <bundle> --all",
            "read SKILL.md + every reference file",
        ),
        ("haneul prompt skill <bundle>", "read SKILL.md"),
        (
            "haneul prompt skill <bundle> --list",
            "list reference file names and sizes (no content)",
        ),
        (
            "haneul prompt skill <bundle> --file <r>",
...
fn print_skill(bundle: &str, list: bool, file: Option<&str>, all: bool) -> anyhow::Result<()> {
    ...
    if !bundle_exists {
        anyhow::bail!(
            "no embedded skill bundle named '{}'. Run `haneul prompt skills` to list bundles.",
            bundle
        );
    }
    if all {
        ...
        .find(|s| s.bundle == bundle && s.file == target_file)
    {
        ...
        None => anyhow::bail!(
            "no file '{}' in skill bundle '{}'. Run `haneul prompt skill {} --list` to enumerate.",
            target_file,
            bundle,
            bundle
        ),
...
fn print_categories() {
    ...
    let commands: &[(&str, &str)] = &[
        (
            "haneul prompt category <name> --all",
            "read every bundle's content in one call",
        ),
        (
            "haneul prompt category <name>",
            "read the category's content",
        ),
        (
            "haneul prompt category <name> --list",
...
mod tests {
    ...
    fn missing_skill_error() {
        ...
        let err = print_skill("__missing_skill_for_prompt_test__", false, None, false)
            .unwrap_err()
            .to_string();
        assert!(err.contains("haneul prompt skills"));
    }
