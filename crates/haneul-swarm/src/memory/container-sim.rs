...
struct ContainerHandle {
    node_id: haneul_simulator::task::NodeId,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            haneul_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let socket_addr =
            haneullabs_network::multiaddr::to_socket_addr(&config.network_address).unwrap();
        ...
        let node = builder
            ...
            .init(move || {
                ...
                async move {
                    ...
                    let server = HaneulNode::start(&config, registry_service, None)
                        .await
                        .unwrap();
