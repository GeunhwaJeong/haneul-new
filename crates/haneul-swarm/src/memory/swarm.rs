...
impl<R: rand::RngCore + rand::CryptoRng> SwarmBuilder<R> {
    ...
    pub fn build(self) -> Swarm {
        ...
        let validators = network_config
            .validator_configs()
            .iter()
            .map(|config| (config.haneul_address(), Node::new(config.to_owned())))
            .collect();
        ...
        if self.fullnode_count > 0 {
            (0..self.fullnode_count).for_each(|_| {
                ...
                fullnodes.insert(config.haneul_address(), Node::new(config));
            });
        }
        Swarm {
            dir,
    ...
    pub fn from_network_config(self, dir: PathBuf, network_config: NetworkConfig) -> Swarm {
        ...
        let fullnodes = if let Some(fullnode_rpc_addr) = self.fullnode_rpc_addr {
            ...
            config.websocket_address = self.websocket_rpc_addr;
            config.json_rpc_address = fullnode_rpc_addr;
            HashMap::from([(config.haneul_address(), Node::new(config))])
        } else {
            Default::default()
        };
...
pub struct Swarm {
    ...
    validators: HashMap<HaneulAddress, Node>,
    fullnodes: HashMap<HaneulAddress, Node>,
}
...
impl Swarm {
    ...
    pub fn validator(&self, name: HaneulAddress) -> Option<&Node> {
        self.validators.get(&name)
    }

    /// Attempt to lookup and return a mutable reference to the Validator with the provided `name`.
    pub fn validator_mut(&mut self, name: HaneulAddress) -> Option<&mut Node> {
    ...
    pub fn fullnode(&self, name: HaneulAddress) -> Option<&Node> {
        self.fullnodes.get(&name)
    }

    /// Attempt to lookup and return a mutable reference to the Fullnode with the provided `name`.
    pub fn fullnode_mut(&mut self, name: HaneulAddress) -> Option<&mut Node> {
