...
/// A handle to an in-memory Haneul Node.
///
...
impl Node {
    ...
    /// start the node.
    ///
    /// [`NodeConfig`]: haneul_config::NodeConfig
    ...
    pub fn name(&self) -> HaneulAddress {
        self.config.haneul_address()
    }
    ...
    pub async fn health_check(&self) -> Result<(), HealthCheckError> {
        ...
        let channel = haneullabs_network::client::connect(self.config.network_address())
            .await
