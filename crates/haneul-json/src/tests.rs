...
use haneul_types::{parse_haneul_type_tag, MOVE_STDLIB_ADDRESS};
...
fn test_convert_address_from_bcs() {
    ...
    let value = HaneulJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Signer), &bcs_bytes).unwrap();
...
fn test_convert_number_from_bcs() {
    ...
    let value = HaneulJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::U32), &bcs_bytes).unwrap();
...
fn test_convert_number_array_from_bcs() {
    ...
    let value = HaneulJsonValue::from_bcs_bytes(
...
#[test]
fn test_haneul_call_arg_string_type() {
    ...
    let v = HaneulJsonValue::from_bcs_bytes(string_layout.as_ref(), &arg1).unwrap();

    assert_eq!(json! {"Some String"}, v.to_json_value());
}

#[test]
fn test_haneul_call_arg_option_type() {
    ...
    let v = HaneulJsonValue::from_bcs_bytes(Some(option_layout).as_ref(), &arg1).unwrap();
...
fn test_convert_string_vec() {
    ...
    let bcs2 = haneul_json.to_bcs_bytes(&layout).unwrap();
...
fn test_string_vec_df_name_child_id_eq() {
    ...
    let haneul_json = HaneulJsonValue::new(name).unwrap();
    let bcs2 = haneul_json.to_bcs_bytes(&layout).unwrap();
