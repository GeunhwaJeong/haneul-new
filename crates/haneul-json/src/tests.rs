...
use haneul_framework::make_system_packages;
...
#[test]
fn test_json_is_not_valid_haneul_json() {
    let checks = vec![
        // Not homogeneous
        (json!([1, 2, 3, true, 5, 6, 7])),
        // Not homogeneous
    ...
    // Driver
    for arg in checks {
        assert!(HaneulJsonValue::new(arg).is_err());
    }
}

#[test]
fn test_json_is_valid_haneul_json() {
    let checks = vec![
        // Homogeneous
        (json!([1, 2, 3, 4, 5, 6, 7])),
        // String allowed
    ...
    // Driver
    for arg in checks {
        assert!(HaneulJsonValue::new(arg).is_ok());
    }
}
...
fn test_basic_args_linter_top_level() {
    ...
    */
    ...
    .iter()
    .map(|q| HaneulJsonValue::new(q.clone()).unwrap())
    .collect();
    ...
    assert_eq!(
        json_args[1].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&monster_name_raw.as_bytes().to_vec()).unwrap())
    );
    assert_eq!(
        json_args[2].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&(monster_img_id_raw.parse::<u64>().unwrap())).unwrap()),
    );
    assert_eq!(
        json_args[3].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&(breed_raw as u8)).unwrap())
    );
    assert_eq!(
        json_args[4].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&(monster_affinity_raw as u8)).unwrap()),
    );
    assert_eq!(
        json_args[5].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&monster_description_raw.as_bytes().to_vec()).unwrap()),
    );
    ...
    */
    ...
    assert_eq!(
        args[0].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&(value_raw.parse::<u64>().unwrap())).unwrap())
    );
    ...
    assert_eq!(
        args[1].0,
        HaneulJsonCallArg::Pure(bcs::to_bytes(&AccountAddress::from(address)).unwrap()),
    );
    ...
    */
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../haneul-core/src/unit_tests/data/entry_point_vector");
    let compiled_modules = BuildConfig::new_for_testing()
        .build(path)
        .unwrap()
        .into_modules();
    ...
    let args = vec![HaneulJsonValue::new(Value::Array(vec![object_id1, object_id2])).unwrap()];
    ...
    assert!(matches!(args[0].0, HaneulJsonCallArg::ObjVec { .. }));

    if let HaneulJsonCallArg::ObjVec(vec) = &args[0].0 {
        assert_eq!(vec.len(), 2);
        assert_eq!(
            vec[0],
...
fn test_convert_address_from_bcs() {
    ...
    let value = HaneulJsonValue::from_bcs_bytes(&bcs_bytes).unwrap();
...
fn test_from_str() {
    // test number
    let test = HaneulJsonValue::from_str("10000").unwrap();
    assert!(test.0.is_number());
    // Test array
    let test = HaneulJsonValue::from_str("[10,10,10,10]").unwrap();
    ...
    // test bool
    let test = HaneulJsonValue::from_str("true").unwrap();
    assert!(test.0.is_boolean());

    // test id without quotes
    let object_id = ObjectID::random().to_hex_uncompressed();
    let test = HaneulJsonValue::from_str(&object_id).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test id with quotes
    let test = HaneulJsonValue::from_str(&format!("\"{}\"", &object_id)).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test string without quotes
    let test = HaneulJsonValue::from_str("Some string").unwrap();
    assert!(test.0.is_string());
    ...
    let test = HaneulJsonValue::from_object_id(
