...
fn resolve_call_arg(
    ...
    if is_primitive {
        match layout_opt {
            Some(layout) => {
                return Ok(HaneulJsonCallArg::Pure(arg.to_bcs_bytes(&layout).map_err(
                    |e| {
                        anyhow!(
    ...
    match param {
        ...
        | SignatureToken::Reference(_)
        | SignatureToken::MutableReference(_) => Ok(HaneulJsonCallArg::Object(resolve_object_arg(
            idx,
            &arg.to_json_value(),
        )?)),
        SignatureToken::Vector(inner) => match &**inner {
            SignatureToken::Struct(_) | SignatureToken::StructInstantiation(_, _) => {
                Ok(HaneulJsonCallArg::ObjVec(resolve_object_vec_arg(idx, arg)?))
            }
            _ => {
                bail!(
...
pub fn resolve_move_function_args(
...
) -> Result<Vec<(HaneulJsonCallArg, SignatureToken)>, anyhow::Error> {
    ...
    let tupled_call_args = call_args
        ...
        .collect::<Vec<(HaneulJsonCallArg, SignatureToken)>>();
    Ok(tupled_call_args)
}
