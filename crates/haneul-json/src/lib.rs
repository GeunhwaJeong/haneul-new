...
use haneul_verifier::entry_points_verifier::is_tx_context;
...
fn resolve_call_arg(
    ...
    Ok(match param {
        ...
        | SignatureToken::U64
        | SignatureToken::U128
        | SignatureToken::Address => HaneulJsonCallArg::Pure(resolve_primtive_arg(arg, param)?),
        ...
        | SignatureToken::Reference(_)
        | SignatureToken::MutableReference(_) => {
            HaneulJsonCallArg::Object(resolve_object_arg(idx, &arg.to_json_value())?)
        }
        ...
        SignatureToken::Vector(inner) => {
            match **inner {
                ...
                SignatureToken::Struct(_) => {
                    HaneulJsonCallArg::ObjVec(resolve_object_vec_arg(idx, arg)?)
                }
                _ => HaneulJsonCallArg::Pure(resolve_primtive_arg(arg, param)?),
            }
        }
