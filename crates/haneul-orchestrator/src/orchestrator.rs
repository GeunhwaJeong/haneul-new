...
impl Orchestrator {
    ...
    pub async fn cleanup(&self, cleanup: bool) -> TestbedResult<()> {
        ...
        let mut command = vec![
            ...
            "(rm -rf ~/.haneul/haneul_config/*_db || true)",
        ];
        if cleanup {
    ...
    pub async fn run_nodes(&self, parameters: &BenchmarkParameters) -> TestbedResult<()> {
        ...
        let command = move |i: usize| -> String {
            let path = format!("~/.haneul/haneul_config/validator-config-{i}.yaml");
            let address = listen_addresses[i].clone();
            let run = format!(
                "cargo run --release --bin haneul-node -- --config-path {path} --listen-address {address}"
            );
    ...
    pub async fn run_clients(&self, parameters: &BenchmarkParameters) -> TestbedResult<()> {
        ...
        let command = move |i: usize| -> String {
            ...
            let genesis = "~/.haneul/haneul_config/genesis.blob";
