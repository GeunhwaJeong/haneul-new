...
use haneul_types::{
    ...
    crypto::{AuthorityKeyPair, KeypairTraits, NetworkKeyPair, HaneulKeyPair},
};
...
impl Config {
    ...
    pub fn new(instances: &[Instance]) -> Self {
        ...
        let gas_key = HaneulKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng));
    ...
    pub fn genesis_command(&self) -> String {
        ...
        format!("cargo run --release --bin haneul -- genesis -f --from-config {genesis}")
    }

    // Generate a genesis configuration file suitable for benchmarks.
    fn make_genesis_config(instances: &[Instance], gas_address: HaneulAddress) -> GenesisConfig {
        let mut rng = StdRng::seed_from_u64(0);
