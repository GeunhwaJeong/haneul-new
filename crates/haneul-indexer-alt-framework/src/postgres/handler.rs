...
//!
//! # TODO: Consider moving FieldCount to haneul-pg-db
//!
//! Currently, FieldCount lives in this framework crate but is Postgres-specific. Ideally it should
//! live in haneul-pg-db. However, this creates a circular dependency:
//! - haneul-indexer-alt-framework depends on haneul-pg-db (for IndexerCluster and other utilities)
//! - This blanket impl needs FieldCount to implement concurrent::Handler for postgres indexers
//! - Moving FieldCount to haneul-pg-db would require framework to depend on haneul-pg-db (circular!)
//!
//! To fully decouple, we'd need to move all postgres-specific code (including IndexerCluster) to
//! haneul-pg-db, which would be a much larger breaking change. Consider this for a future refactor.
//!
//! See: <https://github.com/GeunhwaJeong/haneul/pull/24055#issuecomment-3471278182>
