...
impl CheckpointStreamingClient for GrpcStreamingClient {
    async fn connect(&mut self) -> Result<CheckpointStream> {
        ...
        let chain_id_value = response.metadata().get(X_HANEUL_CHAIN_ID).ok_or_else(|| {
...
mod tests {
    ...
    use haneul_rpc::proto::haneul::rpc::v2::subscription_service_server::SubscriptionServiceServer;
