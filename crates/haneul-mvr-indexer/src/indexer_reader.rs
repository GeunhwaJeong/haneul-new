...
use haneul_json_rpc_types::{Balance, Coin as HaneulCoin, HaneulCoinMetadata, HaneulMoveValue};
use haneul_json_rpc_types::{
    CheckpointId, EpochInfo, EventFilter, HaneulEvent, HaneulObjectDataFilter,
    HaneulTransactionBlockResponse, TransactionFilter,
};
...
use haneul_types::{balance::Supply, coin::TreasuryCap, dynamic_field::DynamicFieldName};
use haneul_types::{
    base_types::{ObjectID, HaneulAddress, VersionNumber},
    committee::EpochId,
    digests::TransactionDigest,
    dynamic_field::DynamicFieldInfo,
    object::{Object, ObjectRead},
    haneul_system_state::{haneul_system_state_summary::HaneulSystemStateSummary, HaneulSystemStateTrait},
};
use haneul_types::{coin::CoinMetadata, event::EventID};
...
use crate::{
    ...
    models::{
        ...
        transactions::{tx_events_to_haneul_tx_events, StoredTransaction},
...
impl IndexerReader {
    ...
    pub async fn get_latest_haneul_system_state(&self) -> Result<HaneulSystemStateSummary, IndexerError> {
        ...
        let system_state = tokio::task::spawn_blocking(move || {
            haneul_types::haneul_system_state::get_haneul_system_state(&object_store)
        })
        .await
        .unwrap()?
    ...
    pub async fn get_validator_from_table(
        &self,
        table_id: ObjectID,
        pool_id: haneul_types::id::ID,
    ) -> Result<
        haneul_types::haneul_system_state::haneul_system_state_summary::HaneulValidatorSummary,
        IndexerError,
    > {
        let object_store = ConnectionAsObjectStore::from_pool(&self.pool)
            .await
        ...
        let validator = tokio::task::spawn_blocking(move || {
            haneul_types::haneul_system_state::get_validator_from_table(&object_store, table_id, &pool_id)
        })
        .await
        .unwrap()?;
        Ok(validator)
    ...
    /// `get_latest_haneul_system_state` for current epoch instead.
    pub async fn get_epoch_haneul_system_state(
        &self,
        epoch: Option<EpochId>,
    ) -> Result<HaneulSystemStateSummary, IndexerError> {
        let stored_epoch = self.get_epoch_info_from_db(epoch).await?;
        let stored_epoch = match stored_epoch {
            Some(stored_epoch) => stored_epoch,
    ...
    pub async fn get_checkpoint(
        &self,
        checkpoint_id: CheckpointId,
    ) -> Result<Option<haneul_json_rpc_types::Checkpoint>, IndexerError> {
        let stored_checkpoint = match self.get_checkpoint_from_db(checkpoint_id).await? {
            Some(stored_checkpoint) => stored_checkpoint,
            None => return Ok(None),
        };

        let checkpoint = haneul_json_rpc_types::Checkpoint::try_from(stored_checkpoint)?;
        Ok(Some(checkpoint))
    }

    pub async fn get_latest_checkpoint(
        &self,
    ) -> Result<haneul_json_rpc_types::Checkpoint, IndexerError> {
        let stored_checkpoint = self.get_latest_checkpoint_from_db().await?;

        haneul_json_rpc_types::Checkpoint::try_from(stored_checkpoint)
    }
    ...
    pub async fn get_checkpoints(
        ...
        limit: usize,
        descending_order: bool,
    ) -> Result<Vec<haneul_json_rpc_types::Checkpoint>, IndexerError> {
        self.get_checkpoints_from_db(cursor, limit, descending_order)
            .await?
            .into_iter()
            .map(haneul_json_rpc_types::Checkpoint::try_from)
            .collect()
    }
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        options: haneul_json_rpc_types::HaneulTransactionBlockResponseOptions,
    ) -> IndexerResult<Vec<HaneulTransactionBlockResponse>> {
        ...
        for stored_tx in stored_txes {
            ...
            tx_block_responses_futures.push(tokio::task::spawn(
                stored_tx
                    .try_into_haneul_transaction_block_response(options_clone, package_resolver_clone),
            ));
        }
    ...
    pub async fn get_owned_objects(
        ...
        if let Some(filter) = filter {
            match filter {
                HaneulObjectDataFilter::StructType(struct_tag) => {
                ...
                HaneulObjectDataFilter::MatchAny(filters) => {
                    ...
                    for (i, filter) in filters.iter().enumerate() {
                        if let HaneulObjectDataFilter::StructType(struct_tag) = filter {
                            let object_type =
    ...
    async fn multi_get_transaction_block_response_in_blocking_task_impl(
        ...
        digests: &[TransactionDigest],
        options: haneul_json_rpc_types::HaneulTransactionBlockResponseOptions,
    ) -> Result<Vec<haneul_json_rpc_types::HaneulTransactionBlockResponse>, IndexerError> {
    ...
    pub async fn get_transaction_events(
    ...
    ) -> Result<Vec<haneul_json_rpc_types::HaneulEvent>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        let haneul_tx_events = tx_events_to_haneul_tx_events(
            tx_events,
        ...
        .await?;
        Ok(haneul_tx_events.map_or(vec![], |ste| ste.data))
    }
    ...
    async fn query_events_by_tx_digest(
        ...
        for stored_event in stored_events {
            haneul_event_futures.push(tokio::task::spawn(
                stored_event.try_into_haneul_event(self.package_resolver.clone()),
            ));
        }

        let haneul_events = futures::future::join_all(haneul_event_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to join haneul event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to collect haneul event futures: {}", e))?;
        Ok(haneul_events)
    }
    ...
    pub async fn bcs_name_from_dynamic_field_name(
        ...
        let name_bcs_value = haneul_json_value.to_bcs_bytes(&move_type_layout)?;
        Ok(name_bcs_value)
    }
    ...
    pub async fn get_owned_coins(
        ...
        cursor: ObjectID,
        limit: usize,
    ) -> Result<Vec<HaneulCoin>, IndexerError> {
        use diesel_async::RunQueryDsl;
