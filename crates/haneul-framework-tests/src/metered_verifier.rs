...
fn test_metered_move_bytecode_verifier() {
    ...
    assert_eq!(
        ...
        bytecode_verifier_metrics
            .verifier_timeout_metrics
            .with_label_values(&[
                BytecodeVerifierMetrics::HANEUL_VERIFIER_TAG,
                BytecodeVerifierMetrics::TIMEOUT_TAG,
            ])
            .get(),
    );
    ...
    let mut meter = HaneulVerifierMeter::new(meter_config);
    let timer_start = Instant::now();
    let r = run_metered_move_bytecode_verifier(
        &compiled_modules,
        &verifier_config,
...
fn test_build_and_verify_programmability_examples() {
    ...
    let bytecode_verifier_metrics = Arc::new(BytecodeVerifierMetrics::new(registry));
    let examples =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../haneul_programmability/examples");
