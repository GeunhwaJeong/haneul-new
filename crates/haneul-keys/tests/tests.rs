...
use haneul_keys::keystore::{AccountKeystore, Alias, FileBasedKeystore, InMemKeystore, Keystore};
...
fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("haneul.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    keystore
...
fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("haneul.keystore");
    let keystore_path_keep = temp_dir.path().join("haneul.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let kp = keystore
        .generate(SignatureScheme::ED25519, None, None, None)
        .unwrap();
...
fn keystore_no_aliases() {
    ...
    // haneul.aliases file with the existing address in the haneul.keystore,
