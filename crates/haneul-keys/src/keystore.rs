...
use haneul_types::crypto::{
    enum_dispatch, EncodeDecodeBase64, PublicKey, Signature, SignatureScheme, HaneulKeyPair,
};
...
pub trait AccountKeystore: Send + Sync {
    fn add_key(&mut self, alias: Option<String>, keypair: HaneulKeyPair) -> Result<(), anyhow::Error>;
    fn remove_key(&mut self, address: HaneulAddress) -> Result<(), anyhow::Error>;
    fn keys(&self) -> Vec<PublicKey>;
    fn get_key(&self, address: &HaneulAddress) -> Result<&HaneulKeyPair, anyhow::Error>;

    fn sign_hashed(&self, address: &HaneulAddress, msg: &[u8]) -> Result<Signature, signature::Error>;
    ...
    fn get_alias_by_address(&self, address: &HaneulAddress) -> Result<String, anyhow::Error>;
    fn get_address_by_alias(&self, alias: String) -> Result<&HaneulAddress, anyhow::Error>;
