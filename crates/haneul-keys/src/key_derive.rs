...
/// Secp256k1 follows BIP-32/44 using path where the first 3 levels are hardened: m/54'/8282'/0'/0/{index}
/// Secp256r1 follows BIP-32/44 using path where the first 3 levels are hardened: m/74'/8282'/0'/0/{index}
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| HaneulErrorKind::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), HaneulKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), HaneulKeyPair::Secp256k1(kp)))
        }
        ...
        | SignatureScheme::PasskeyAuthenticator => Err(HaneulErrorKind::UnsupportedFeatureError {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                ...
                .parse()
                .map_err(|_| {
                    HaneulErrorKind::SignatureKeyGenError("Cannot parse path".to_string())
                })?),
            }
        }
        SignatureScheme::Secp256k1 => {
        ...
        SignatureScheme::Secp256r1 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at first 3 levels with purpose = 74, coin_type = 8282
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                        if Some(purpose)
...
) -> Result<(HaneulAddress, HaneulKeyPair, SignatureScheme, String), anyhow::Error> {
