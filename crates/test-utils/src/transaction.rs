...
use haneul_json_rpc_types::{
    ObjectChange, HaneulObjectDataOptions, HaneulObjectResponseQuery, HaneulTransactionBlockDataAPI,
...
pub async fn publish_package_with_wallet(
    ...
    let transaction = {
        ...
        let signature = context
            ...
            .unwrap();
        Transaction::from_data(data, Intent::haneul_transaction(), vec![signature])
            .verify()
            .unwrap()
    };
...
pub async fn create_devnet_nft(
    ...
    let args_json = json!([
        "example_nft_name",
        "example_nft_desc",
        "https://haneul.io/_nuxt/img/haneul-logo.8d3c44e.svg",
    ]);
    let mut args = vec![];
    ...
    let res = HaneulClientCommands::Call {
    ...
    let (object_id, digest) = if let HaneulClientCommandResult::Call(ref response) = res {
        (
            response
                .effects
                .as_ref()
...
> {
    ...
    let client = context.get_client().await.unwrap();
    let object_refs: Vec<HaneulObjectResponse> = client
        .read_api()
        .get_owned_objects(
            sender,
            Some(HaneulObjectResponseQuery::new_with_options(
                HaneulObjectDataOptions::full_content().with_bcs(),
            )),
            None,
            None,
        )
    ...
    let (digest, gas, gas_used) = if let HaneulClientCommandResult::Transfer(response) = res {
