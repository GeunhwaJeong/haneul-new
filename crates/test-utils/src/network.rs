...
impl TestClusterBuilder {
    ...
    async fn start_test_network_with_customized_ports(self) -> Result<TestCluster, anyhow::Error> {
        ...
        let gateway_handle = if let Some(gateway_port) = self.gateway_rpc_port {
            let handle =
                Self::start_rpc_gateway(&working_dir.join(HANEUL_GATEWAY_CONFIG), Some(gateway_port))
                    .await?;
...
pub async fn start_a_fullnode(swarm: &Swarm, disable_ws: bool) -> Result<HaneulNode, anyhow::Error> {
    ...
    HaneulNode::start(&config, Registry::new()).await
}
...
pub async fn start_a_fullnode_with_handle(
    ...
    let haneul_node = HaneulNode::start(&config, Registry::new()).await?;

    let rpc_url = format!("http://{}", jsonrpc_server_url);
    let rpc_client = HttpClientBuilder::default().build(&rpc_url)?;
    let haneul_client = ClientType::RPC(rpc_url.clone(), ws_url.clone());
    ...
    // Check url is valid
    let haneul_client = haneul_client.init().await?;
    Ok(FullNodeHandle {
        haneul_node,
        haneul_client,
        rpc_client,
