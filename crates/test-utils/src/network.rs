...
impl TestCluster {
    ...
    pub fn all_node_handles(&self) -> impl Iterator<Item = HaneulNodeHandle> {
        self.swarm
            .validator_node_handles()
            .into_iter()
            .chain(std::iter::once(HaneulNodeHandle::new(
                self.fullnode_handle.haneul_node.clone(),
            )))
    }
    ...
    pub async fn wait_for_epoch_with_timeout(
        ...
        let mut epoch_rx = self.fullnode_handle.haneul_node.subscribe_to_epoch_change();
...
pub async fn start_fullnode_from_config(
    ...
    let haneul_node = HaneulNode::start(&config, registry_service, None).await?;
...
pub async fn wait_for_node_transition_to_epoch(node: &HaneulNodeHandle, expected_epoch: EpochId) {
