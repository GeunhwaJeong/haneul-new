...
use haneul_types::{base_types::HaneulAddress, crypto::Signature};
use haneul_types::{crypto::KeyPair, messages::CallArg};
    ...
    K: Iterator<Item = KeyPair>,
{
    ...
    for keypair in keys {
        let address = HaneulAddress::from(keypair.public_key_bytes());
...
pub fn make_transfer_haneul_transaction(gas_object: Object, recipient: HaneulAddress) -> Transaction {
    let (sender, keypair) = test_keys().pop().unwrap();
