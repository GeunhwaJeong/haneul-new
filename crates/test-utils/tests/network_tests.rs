...
use haneul_types::{
    ...
    HANEUL_FRAMEWORK_OBJECT_ID, HANEUL_SYSTEM_ADDRESS, HANEUL_SYSTEM_OBJECT_ID,
};
...
async fn test_package_override() {
    ...
    let framework_ref = {
        ...
        let obj = client.get_object(HANEUL_SYSTEM_OBJECT_ID, None).await.unwrap();
    ...
    let modified_ref = {
        let mut framework_modules = BuiltInFramework::get_package_by_id(&HANEUL_SYSTEM_OBJECT_ID)
            .modules()
            .to_vec();
