...
use haneul_types::{
    base_types::ObjectID, digests::TransactionDigest, object::Object, HANEUL_FRAMEWORK_OBJECT_ID,
};
...
async fn test_additional_objects() {
    ...
    let resp = client.get_object(id).await.unwrap();
    assert!(matches!(resp, HaneulObjectRead::Exists(_)));
}

#[sim_test]
async fn test_package_override() {
    // `with_objects` can be used to override existing packages.
    let id = HANEUL_FRAMEWORK_OBJECT_ID;

    let framework_ref = {
        let default_cluster = TestClusterBuilder::new().build().await.unwrap();
        let client = default_cluster.rpc_client();
        let HaneulObjectRead::Exists(obj) = client.get_object(id).await.unwrap() else {
    ...
    let modified_ref = {
        let mut framework_modules = haneul_framework::get_haneul_framework();
