...
//# run-graphql --cursors bcs(bcs(@{obj_2_0}),@{highest_checkpoint}) bcs(bcs(@{obj_3_0}),@{highest_checkpoint})
{
...
//# run-graphql --cursors bcs(bcs(@{obj_2_0}),1) bcs(bcs(@{obj_3_0}),1)
{
...
//# run-graphql --cursors bcs(bcs(@{obj_6_0}),2)
{
  after_obj_6_0_at_checkpoint_2: address(address: "@{A}") {
    objects(filter: {type: "@{Test}"}, after: "@{cursor_0}") {
      nodes {
        ...
        owner_at_latest_state_has_haneul_only: owner {
          ... on AddressOwner {
            owner {
              objects {
                nodes {
