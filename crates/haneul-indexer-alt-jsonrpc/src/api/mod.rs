...
/// Map an object change from the effects to a `HaneulObjectChange` response type. Returns `None` for
/// changes that are not visible in the response (created-and-wrapped, unwrapped, unwrapped-then-
/// deleted, system package upgrades).
pub(crate) fn to_haneul_object_change(
...
) -> anyhow::Result<Option<HaneulObjectChange>> {
