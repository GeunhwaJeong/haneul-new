...
fn effects(tx: &TransactionContents) -> Result<HaneulTransactionBlockEffects, RpcError<Error>> {
...
async fn events(
    ...
    for (ix, event) in events.into_iter().enumerate() {
        ...
        let haneul_event =
            HaneulEvent::try_from(event, digest, ix as u64, Some(tx.timestamp_ms()), layout)
