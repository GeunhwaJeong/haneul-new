...
use haneul_json_rpc_types::{
    HaneulGetPastObjectRequest, HaneulObjectData, HaneulObjectDataOptions, HaneulObjectRef,
    HaneulPastObjectResponse,
};
    ...
    async fn try_multi_get_past_objects(
        ...
        for request in past_objects {
            if let Some(stored) = stored_objects.get(&(request.object_id, request.version)) {
                ...
                responses.push(HaneulPastObjectResponse::VersionNotFound(
                    request.object_id,
                    request.version,
                ));
            }
...
pub(crate) async fn response(
...
) -> Result<HaneulPastObjectResponse, Error> {
    let object_id =
    ...
    let object_data = HaneulObjectData {
        object_id,
        version,
    ...
    Ok(HaneulPastObjectResponse::VersionFound(object_data))
}
