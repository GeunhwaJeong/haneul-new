...
async fn latest_haneul_system_state_response(
    ...
    let wrapper: HaneulSystemStateWrapper = fetch_latest(ctx, HANEUL_SYSTEM_STATE_OBJECT_ID)
        .await
    ...
    Ok(match wrapper.version {
        1 => fetch_latest::<Field<u64, HaneulSystemStateInnerV1>>(ctx, inner_id)
            .await
            .internal_context("Failed to fetch inner system state object")?
            .value
            .into_haneul_system_state_summary(),
        2 => fetch_latest::<Field<u64, HaneulSystemStateInnerV2>>(ctx, inner_id)
            .await
            .internal_context("Failed to fetch inner system state object")?
            .value
            .into_haneul_system_state_summary(),
