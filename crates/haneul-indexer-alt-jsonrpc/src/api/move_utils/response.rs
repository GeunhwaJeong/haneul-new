...
pub(super) async fn function(
    ...
    module: &str,
    name: &str,
) -> Result<HaneulMoveNormalizedFunction, RpcError<Error>> {
    use Error as E;
    ...
    let sig = ctx
        ...
        .await
        .map_err(|e| {
            use haneul_package_resolver::error::Error as PRE;
            match &e {
...
fn normalized_function(sig: &FunctionDef) -> HaneulMoveNormalizedFunction {
...
fn normalized_signature(sig: &OpenSignature) -> HaneulMoveNormalizedType {
...
fn visibility(v: Visibility) -> HaneulMoveVisibility {
    match v {
...
fn ability_set(a: AbilitySet) -> HaneulMoveAbilitySet {
    HaneulMoveAbilitySet {
        abilities: a.into_iter().map(ability).collect(),
    }
}

fn ability(a: Ability) -> HaneulMoveAbility {
    match a {
