...
use haneul_json_rpc_types::HaneulProtocolConfigValue;
...
#[open_rpc(namespace = "haneul", tag = "Protocol API")]
...
fn parse_attribute(value: &str) -> Option<HaneulProtocolConfigValue> {
    match value {
        "true" => Some(HaneulProtocolConfigValue::Bool(true)),
        "false" => Some(HaneulProtocolConfigValue::Bool(false)),
        _ => value.parse().ok().map(HaneulProtocolConfigValue::U64),
    }
}
