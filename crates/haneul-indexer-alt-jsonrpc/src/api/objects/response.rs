...
) -> Result<HaneulObjectResponse, RpcError> {
    let Some(object) = load_live(ctx, object_id)
        .await
        .context("Failed to load latest object")?
    else {
        return Ok(HaneulObjectResponse::new_with_error(
...
pub(crate) async fn object_data_with_options(
    ...
    let content: OptionFuture<_> = options
        .show_content
        .then(|| object_data::<HaneulParsedData>(ctx, &object))
        .into();

    let bcs: OptionFuture<_> = options
        .show_bcs
        .then(|| object_data::<HaneulRawData>(ctx, &object))
        .into();
...
async fn object_data<D: HaneulData>(ctx: &Context, object: &Object) -> Result<D, RpcError> {
...
async fn display(ctx: &Context, object: &Object) -> DisplayFieldsResponse {
    ...
    DisplayFieldsResponse {
        ...
        error: (!field_errors.is_empty()).then_some(HaneulObjectResponseError::DisplayError {
            error: field_errors,
        }),
    }
}
...
async fn display_fields(
    ...
    if let Some(display_v2) = display_v2? {
        ...
        let fields = haneul_display::v2::Display::parse(config.display(), display_v2.fields())?
            .display(
