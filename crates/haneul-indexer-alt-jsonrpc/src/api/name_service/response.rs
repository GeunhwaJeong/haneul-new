...
/// Attempt to to translate the given HaneulNS `name` to its address, as long as the mapping exists,
/// and it hasn't expired.
pub(super) async fn resolved_address(
    ctx: &Context,
    name: &str,
) -> Result<Option<HaneulAddress>, RpcError<Error>> {
    use Error as E;
...
/// Attempt to to translate the given `address` to its HaneulNS name, as long as the reverse mapping
...
pub(super) async fn resolved_name(
    ...
    let reverse_record: Field<HaneulAddress, Domain> = bcs::from_bytes(
