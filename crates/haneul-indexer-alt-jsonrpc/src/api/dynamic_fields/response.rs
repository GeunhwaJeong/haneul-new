...
pub(super) async fn dynamic_field_object(
...
) -> Result<HaneulObjectResponse, RpcError<Error>> {
    let layout = resolve_type(ctx, &name.type_).await?;
    let bytes = HaneulJsonValue::new(name.value)
    ...
    let options = HaneulObjectDataOptions::full_content();
...
/// `haneul::dynamic_field::Field<K, V>`, and extract the name and value from it.
