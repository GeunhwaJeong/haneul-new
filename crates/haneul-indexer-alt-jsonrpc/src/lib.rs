...
use haneul_indexer_alt_reader::system_package_task::{SystemPackageTask, SystemPackageTaskArgs};
...
impl RpcService {
    ...
    pub fn new(rpc_args: RpcArgs, registry: &Registry) -> anyhow::Result<Self> {
        ...
        let schema = Project::new(
            ...
            "Haneul JSON-RPC",
            "A JSON-RPC API for interacting with the Haneul blockchain.",
            "Haneul Labs",
...
mod tests {
    ...
    async fn test_rpc_discovery() {
        ...
        assert_eq!(resp["result"]["info"]["title"], "Haneul JSON-RPC");
        assert_eq!(
            resp["result"]["methods"],
            json!([
                {
