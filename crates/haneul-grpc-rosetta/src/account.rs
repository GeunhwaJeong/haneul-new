...
use haneul_sdk::HANEUL_COIN_TYPE;
...
//TODO this can be a convenience method in the SDK.
async fn get_current_epoch(grpc_client: &mut haneul_rpc::client::Client) -> Result<u64, Error> {
...
async fn get_account_balances(
    grpc_client: &mut haneul_rpc::client::Client,
    address: HaneulAddress,
    coin_type: &str,
) -> Result<i128, Error> {
    ...
    let amounts: Vec<SubBalance> = match account_type {
        SubAccountType::Stake => delegated_stakes
            ...
            .map(|stake| SubBalance {
                stake_id: stake.staked_haneul_id,
...
/// TODO This API is supposed to return coins of all types, not just HANEUL. It also has a 'currencies' parameter that we
...
pub async fn coins(
    ...
    loop {
        ...
        list_request.object_type = Some(HANEUL_COIN_TYPE.to_string());
