    ...
    fn parse_programmable_transaction(
        ...
        fn transfer_object(
            ...
            let addr = match recipient {
                HaneulArgument::Input(i) => inputs.get(i as usize)?.pure()?.to_haneul_address().ok()?,
            ...
            let (amount, validator) = match &arguments[..] {
                [_, coin, validator] => {
                    ...
                    let (some_amount, validator) = match validator {
                        ...
                        HaneulArgument::Input(i) => (*i==1, inputs.get(*i as usize).and_then(|input| input.pure()).map(|v|v.to_haneul_address()).transpose()),
                        _=> return Ok(None),
                    };
        ...
        for command in commands {
            let result = match command {
                ...
                HaneulCommand::MoveCall(m) if Self::is_stake_call(m) => {
                ...
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        currency = inputs.iter().last().and_then(|arg| {
                            if let HaneulCallArg::Pure(value) = arg {
                                let bytes = value
                                    .value()
                                    .to_json_value()
                                    .as_array()?
...
impl TryFrom<TransactionData> for Operations {
    ...
    fn try_from(data: TransactionData) -> Result<Self, Self::Error> {
        ...
        Ok(Operations::try_from_data(
            HaneulTransactionBlockData::try_from_with_module_cache(data, &&mut NoOpsModuleResolver)?,
            None,
        )?)
    }
}
