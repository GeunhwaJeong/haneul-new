...
`./haneul-grpc-rosetta start-online-server`

#### 4. Start Rosetta Offline server

`./haneul-grpc-rosetta start-offline-server`

#### 5. Generate configuration with prefunded accounts for rosetta-cli

`./haneul-grpc-rosetta generate-rosetta-cli-config`
...
```shell
cd <haneul project directory>/crate/haneul-grpc-rosetta/docker/haneul-grpc-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run haneullabs/haneul-grpc-rosetta-devnet haneul-grpc-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run haneullabs/haneul-grpc-rosetta-devnet haneul-grpc-rosetta start-offline-server
```
