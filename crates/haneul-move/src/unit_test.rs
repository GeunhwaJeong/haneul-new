...
use haneul_move_natives::test_scenario::InMemoryTestStore;
    ...
    pub fn execute(
        ...
        if !cfg!(debug_assertions) && compute_coverage {
            return Err(anyhow::anyhow!(
                "The --coverage flag is currently supported only in debug builds. Please build the Haneul CLI from source in debug mode."
            ));
        }
...
impl ChildObjectResolver for DummyChildObjectStore {
    ...
    fn get_object_received_at_version(
        ...
        _epoch_id: haneul_types::committee::EpochId,
    ) -> HaneulResult<Option<Object>> {
        Ok(None)
    }
}
...
pub fn run_move_unit_tests(
    ...
    let result = move_cli::base::test::run_move_unit_tests(
        ...
        haneul_move_natives::all_natives(
            /* silent */ false,
