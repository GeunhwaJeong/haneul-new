...
/// `@haneullabs/prettier-plugin-move` npm package — every argument (including
/// `--help`) is forwarded verbatim. If `prettier-move` is not found on
/// `PATH`, the command offers to install it via `npm i -g`.
#[derive(Parser)]
#[clap(disable_help_flag = true)]
#[group(id = "haneul-move-format")]
pub struct Format {
    /// Arguments forwarded verbatim to `prettier-move`. Examples:
    ///   haneul move format -c sources/foo.move      # check
    ///   haneul move format -w .                     # write the package
    ///   haneul move format --help                   # prettier-move help
    #[clap(trailing_var_arg = true, allow_hyphen_values = true)]
    args: Vec<String>,
}

const NPM_INSTALL_ARGS: &[&str] = &["i", "-g", "prettier", "@haneullabs/prettier-plugin-move"];
...
fn bootstrap_prettier_move() -> anyhow::Result<()> {
    ...
    io::stdin()
        ...
        "aborted: `prettier-move` is required for `haneul move format`"
    );
