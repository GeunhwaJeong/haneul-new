...
/// of [path], with the `haneul-move` binary on the path.
fn test_shell_snapshot(path: &Path) -> datatest_stable::Result<()> {
    ...
    shell
        .env("PATH", format!("/bin:/usr/bin:{}", get_haneul_move_path()))
        .current_dir(sandbox)
...
fn get_haneul_move_path() -> String {
    get_cargo_bin("haneul-move")
        .parent()
        .unwrap()
        .to_str()
