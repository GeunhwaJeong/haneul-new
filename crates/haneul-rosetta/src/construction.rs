...
pub async fn submit(
    ...
    let dry_run = context
        ...
        .await?;
    if let HaneulExecutionStatus::Failure { error } = dry_run.effects.status() {
    ...
    if let HaneulExecutionStatus::Failure { error } = response
        .effects
...
pub async fn metadata(
    ...
    let (total_required_amount, objects) = match &option.internal_operation {
        InternalOperation::PayHaneul { amounts, .. } => {
    ...
    // If required amount is None (all HANEUL) or failed to select coin (might not have enough HANEUL), select all coins.
