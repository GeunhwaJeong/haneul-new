...
pub async fn combine(
    ...
    let signed_tx =
        haneul_types::transaction::Transaction::from_generic_sig_data(data, vec![generic_sig]);
...
pub async fn submit(
    ...
    // non-HANEUL delta of that exact coin type. Otherwise the currency label
...
pub async fn hash(
    ...
    // haneul_types::transaction::Transaction::digest() is a hash over
    ...
    let signatures = wrapper
        ...
        .collect::<Result<Vec<_>, Error>>()?;
    let tx = haneul_types::transaction::Transaction::from_generic_sig_data(data, signatures);
...
pub async fn metadata(
    ...
    let (epoch, chain_id) = if needs_address_balance_metadata {
        ...
        let chain_id_str =
            haneul_types::digests::CheckpointDigest::new(*context.chain_id.as_bytes()).base58_encode();
...
pub async fn parse(
    ...
    let account_identifier_signers = if signed {
        vec![
            HaneulAddress::from_str(proto.sender())
...
/// balance changes actually move a non-HANEUL coin of the labelled type. This is
...
fn verify_pay_coin_currency<'a>(
    ...
    let haneul = TypeTag::from_str("0x2::haneul::HANEUL").expect("0x2::haneul::HANEUL is a valid type tag");
    let matched = moved_coin_types.into_iter().any(|ct| {
        TypeTag::from_str(ct)
            .map(|t| t == want && t != haneul)
            .unwrap_or(false)
    });
    if matched {
        Ok(())
        ...
        Err(Error::DataError(format!(
            "PayCoin currency {} does not match any non-HANEUL balance change in the simulated \
             transaction",
...
mod tests {
    ...
    /// change of the labelled coin type and rejects when only HANEUL / a
    ...
    fn test_verify_pay_coin_currency() {
        ...
        assert!(verify_pay_coin_currency(&aux, ["0x2::haneul::HANEUL", usdc]).is_ok());

        // A different non-HANEUL coin moves (currency label disagrees with
        // reality) → reject.
        let err = verify_pay_coin_currency(&aux, ["0x2::haneul::HANEUL", "0x9::other::OTHER"])
            .expect_err("mismatched currency must be rejected");
        assert!(format!("{err:?}").contains("does not match"));

        // Only HANEUL moves → reject.
        assert!(verify_pay_coin_currency(&aux, ["0x2::haneul::HANEUL"]).is_err());

        // A PayCoin label that (wrongly) names HANEUL never matches — HANEUL is
        // explicitly excluded as a non-HANEUL delta.
        assert!(verify_pay_coin_currency(&pay_coin("0x2::haneul::HANEUL"), ["0x2::haneul::HANEUL"]).is_err());
