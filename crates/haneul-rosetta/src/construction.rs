...
pub async fn submit(
    ...
    if let Some(effect) = response.effects {
        if let HaneulExecutionStatus::Failure { error } = effect.status {
    ...
    let (tx_metadata, gas, budget) = match &option.internal_operation {
        InternalOperation::PayHaneul {
            ...
            let sender_coins = context
                ...
                .collect::<Vec<_>>();
            // gas is always the first coin for pay_haneul
            let gas = sender_coins[0];
            (TransactionMetadata::PayHaneul(sender_coins), gas, 1000)
        }
