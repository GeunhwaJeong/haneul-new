...
async fn get_sub_account_balances(
    ...
    let balances: HashMap<_, u128> = match account_type {
        SubAccountType::DelegatedHaneul => {
        ...
        SubAccountType::LockedHaneul => {
            let haneul_balance = client.coin_read_api().get_balance(address, None).await?;
            haneul_balance
                .locked_balance
                .into_iter()
    ...
    Ok(balances
        ...
        .map(|(lock, balance)| {
            if let Some(lock) = lock {
                // Safe to cast to i128 as total issued HANEUL will be less then u64.
...
pub async fn coins(
    ...
    let coins = context
        ...
        .get_coins_stream(
            ...
            Some(HANEUL_COIN_TYPE.to_string()),
        )
        .map(Coin::from)
        .collect()
        .await;
