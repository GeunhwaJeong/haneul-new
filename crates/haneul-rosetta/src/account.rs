...
async fn get_sub_account_balances(
    ...
    if matches!(account_type, SubAccountType::FungibleStakedHaneulValue) {
        return get_fungible_staked_haneul_value(client, address, &epoch_timing).await;
    }
    ...
    let amounts: Vec<SubBalance> = match account_type {
        ...
        SubAccountType::EstimatedReward => delegated_stakes
            ...
            .collect(),
        SubAccountType::FungibleStakedHaneulValue => unreachable!(),
    };
...
async fn get_fungible_staked_haneul_value(
    client: &mut Client,
    address: Address,
    ...
    for obj in &fss_objects {
        ...
        sub_balances.push(SubBalance {
            ...
            value: haneul_equivalent as i128,
            activation_epoch: None,
        });
    }
