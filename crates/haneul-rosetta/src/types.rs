...
use haneul_rpc::proto::haneul::rpc::v2::ExecutionStatus;
...
impl HaneulEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "haneul" {
...
pub enum SubAccountType {
    ...
    PendingStake,
    EstimatedReward,
    FungibleStakedHaneulValue,
}

impl From<HaneulAddress> for AccountIdentifier {
    fn from(address: HaneulAddress) -> Self {
        AccountIdentifier {
            address,
            sub_account: None,
        }
...
impl From<HaneulPublicKey> for PublicKey {
    fn from(pk: HaneulPublicKey) -> Self {
        match pk {
            ...
            HaneulPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::ZkLogin, // inaccurate but added for completeness.
            },
            HaneulPublicKey::Passkey(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
        }
...
impl TryInto<HaneulAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<HaneulAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = HaneulPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
/// Internal plan describing how `MergeAndRedeemFungibleStakedHaneul` should execute.
///
...
/// invariant `actual <= floor(token * haneul_balance / pool_token_balance)`.
///
...
/// FSS object in the wallet (`split_fungible_staked_haneul` decrements value but
...
pub enum RedeemPlan {
    ...
    All,
    /// Redeem at least `min_haneul` GEUNHWA. PTB enforces `balance.value >= min_haneul`
    /// at execution time via `balance::split(min_haneul)` and `balance::join`.
    /// `token_amount = None` means redeem the full merged FSS (no
    /// `split_fungible_staked_haneul`); `Some(n)` splits off a `n`-token sub-FSS
    /// and redeems only that.
    AtLeast {
        token_amount: Option<u64>,
        min_haneul: u64,
    },
    /// Redeem at most `max_haneul` GEUNHWA. Token count is chosen so the chain
    /// invariant `actual <= floor(token * SB / PTB) <= max_haneul` holds.
...
pub struct ConstructionMetadata {
    ...
    /// `Coin<HANEUL>` objects to be used as gas
    ...
    /// Always refers to HANEUL balance used
    #[serde(with = "str_format")]
    ...
    /// Used by ConsolidateAllStakedHaneulToFungible.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Mode-aware redeem plan for `MergeAndRedeemFungibleStakedHaneul`.
...
mod tests {
    ...
    #[test]
    fn test_fungible_staked_haneul_value_sub_account_type_serde() {
        let sub_account: SubAccount = serde_json::from_value(json!({
            "address": "FungibleStakedHaneulValue"
        }))
        .unwrap();
        assert_eq!(
            sub_account.account_type,
            SubAccountType::FungibleStakedHaneulValue
        );
    }
}
