...
async fn test_all_transaction_type() {
    ...
    let package = haneul_framework::build_move_package(&path, BuildConfig::default()).unwrap();
...
fn find_module_object(effects: &HaneulTransactionEffects, module: &str) -> Option<OwnedObjectRef> {
    effects
        .events
        .iter()
        .find_map(|event| {
