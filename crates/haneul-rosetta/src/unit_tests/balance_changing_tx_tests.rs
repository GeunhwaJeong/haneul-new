...
async fn test_delegation_parsing() -> Result<(), anyhow::Error> {
    ...
    let client = network.wallet.get_client().await.unwrap();
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let gas = get_random_haneul(&client, sender, vec![]).await;
    let validator = client
        .governance_api()
        .get_latest_haneul_system_state()
        .await
...
async fn test_transaction(
    ...
    let gas = if !gas.is_empty() {
        ...
        let input_objects = tx
            ...
            .collect::<Vec<_>>();
        vec![get_random_haneul(client, sender, input_objects).await]
    };
    ...
    let response = client
        ...
        .execute_transaction_block(
            Transaction::from_data(data.clone(), Intent::haneul_transaction(), vec![signature]),
