...
use crate::types::{Currency, CurrencyMetadata, HaneulEnv};
...
pub static HANEUL: Lazy<Currency> = Lazy::new(|| Currency {
    symbol: "HANEUL".to_string(),
    decimals: 9,
    metadata: CurrencyMetadata {
        coin_type: haneul_types::TypeTag::from_str(HANEUL_COIN_TYPE)
...
impl RosettaOnlineServer {
    pub fn new(env: HaneulEnv, client: HaneulClient) -> Self {
...
impl CoinMetadataCache {
    pub fn new(client: HaneulClient, size: NonZeroUsize) -> Self {
        Self {
            client,
