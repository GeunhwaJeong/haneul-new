...
use haneul_types::object::PastObjectRead;
...
pub struct PseudoBlockProvider {
    ...
    balance: Arc<RwLock<BTreeMap<HaneulAddress, Vec<HistoricBalance>>>>,
}
    ...
    async fn create_next_block(&self, state: Arc<AuthorityState>) -> Result<(), Error> {
        ...
        if current_block.index < total_tx {
            ...
            for (_, digest) in tx_digests {
                ...
                // This is a temporary fix for capturing Haneul transfer from wrapped object, currently event service doesn't emit transferHaneul event.
                // TODO: remove this when event service emit transfer haneul event for wrapped object.
...
fn extract_balance_changes_from_ops(
    ops: Vec<Operation>,
) -> Result<BTreeMap<HaneulAddress, SignedValue>, anyhow::Error> {
    let mut changes: BTreeMap<HaneulAddress, SignedValue> = BTreeMap::new();
    for op in ops {
        match op.type_ {
            OperationType::TransferHANEUL | OperationType::GasSpent | OperationType::Genesis => {
                let addr = op
                    .account
