...
use haneul_sdk::rpc_types::HaneulTransactionKind;
...
impl PseudoBlockProvider {
    pub fn spawn(client: HaneulClient, db_path: &Path) -> Self {
        let blocks = Self {
            database: Arc::new(BlockProviderTables::open(db_path, None)),
            client: client.clone(),
        };

        let block_interval = option_env!("HANEUL_BLOCK_INTERVAL")
    ...
    async fn create_next_block(&self, client: &HaneulClient) -> Result<(), Error> {
        let current_block = self.current_block_identifier()?;
        // Haneul get_total_transaction_number starts from 1.
        ...
        if current_block.index < total_tx {
            ...
            if tx_digests.remove(0) != cursor {
                return Err(Error::DataError(
                    "Incorrect transaction data returned from Haneul.".to_string(),
                ));
            }
...
fn extract_balance_changes_from_ops(
    ops: Operations,
) -> Result<HashMap<HaneulAddress, i128>, anyhow::Error> {
    ops.into_iter()
        .try_fold(HashMap::<HaneulAddress, i128>::new(), |mut changes, op| {
            match op.type_ {
                OperationType::HaneulBalanceChange | OperationType::Gas | OperationType::PayHaneul => {
                    let addr = op
                        .account
                        .ok_or_else(|| {
...
async fn process_genesis_block(client: &HaneulClient, f: &PseudoBlockProvider) -> Result<(), Error> {
    let digest = *client
        .read_api()
    ...
    if !response
        ...
        .transactions
        .iter()
        .any(|tx| matches!(tx, HaneulTransactionKind::Genesis(_)))
    {
...
pub struct BlockProviderTables {
    ...
    #[default_options_override_fn = "default_config"]
    balances: DBMap<(HaneulAddress, u64), HistoricBalance>,
}
