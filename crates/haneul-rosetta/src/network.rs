...
use crate::{ErrorType, OnlineServerContext, HaneulEnv};
...
pub async fn list(Extension(env): Extension<HaneulEnv>) -> Result<NetworkListResponse, Error> {
...
pub async fn status(
    ...
    let object = context
        .state
        .get_object_read(&HANEUL_SYSTEM_STATE_OBJECT_ID)
        .await?;

    let system_state: HaneulSystemState = bcs::from_bytes(
        object
            .into_object()?
            .data
            .try_as_move()
