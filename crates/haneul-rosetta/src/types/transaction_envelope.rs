...
pub enum AuxData {
    /// PayHaneul, Stake, WithdrawStake: fully reconstructable from the PTB.
    None,
    /// PayCoin: the moved coin type is not in the PTB (only an `ObjectRef` →
    /// chain state). `currency` also disambiguates PayCoin from PayHaneul at parse
    /// time.
    PayCoin { currency: Currency },
    /// ConsolidateAllStakedHaneulToFungible: `validator` is derived from a pool id
    ...
    Consolidate { validator: HaneulAddress },
    /// MergeAndRedeemFungibleStakedHaneul: `validator` is not recoverable; the
...
/// `haneul_types::rpc_proto_conversions`, then clear the opaque `bcs` field so
...
/// `haneul-rpc-api/.../simulate/mod.rs`: proto → `haneul_sdk_types::Transaction`
/// (reads structured fields) → `TransactionData` (via the BCS-round-trip impl
/// in `haneul_types::haneul_sdk_types_conversions`).
pub fn proto_to_transaction_data(proto: ProtoTransaction) -> Result<TransactionData, Error> {
    let sdk_tx = haneul_sdk_types::Transaction::try_from(&proto)
...
mod tests {
    ...
    /// Build a non-trivial PayHaneul `TransactionData` for round-trip tests.
    fn sample_pay_haneul_data() -> TransactionData {
        let sender = HaneulAddress::random_for_testing_only();
        let recipient = HaneulAddress::random_for_testing_only();
        let pt = {
            let mut b = ProgrammableTransactionBuilder::new();
            b.pay_haneul(vec![recipient], vec![1_000_000]).unwrap();
            b.finish()
        };
        let gas_price = 1000;
    ...
    fn test_unsigned_wrapper_roundtrip() {
        let data = sample_pay_haneul_data();
    ...
    /// `haneul_sdk_types::Transaction` — through four hand-written conversions.
    ...
    /// (`balance::split<HANEUL>`), so it is the sole guard on that mapping.
    ...
    #[test]
    fn test_proto_to_bcs_canonicality_pay_haneul() {
        assert_canonical(&sample_pay_haneul_data(), "pay_haneul");
    }
    ...
    /// PayCoin path post-bearer-removal: same shape as PayHaneul but built via
    ...
    fn test_proto_to_bcs_canonicality_pay_coin() {
        ...
        let pt = pay_coin_pt(sender, vec![recipient], vec![10_000], &[coin], &[], 0, &HANEUL).unwrap();
        let gas_price = 1000;
    ...
    /// PayHaneul with a party-owned (`SharedObject`) coin input. Exercises the
    /// `ObjectArg::SharedObject` proto path, distinct from `ImmOrOwnedObject`.
    #[test]
    fn test_proto_to_bcs_canonicality_pay_haneul_party_object() {
        use crate::types::internal_operation::pay_haneul_pt_coin_gas;

        let recipient = HaneulAddress::random_for_testing_only();
        let sender = HaneulAddress::random_for_testing_only();
        let party_coin = (ObjectID::random(), SequenceNumber::from(7));
        let pt = pay_haneul_pt_coin_gas(vec![recipient], vec![5_000], &[], &[party_coin], 0).unwrap();
        let gas_price = 1000;
        let data = TransactionData::new_programmable(
            sender,
            vec![random_object_ref()],
        ...
        assert_canonical(&data, "pay_haneul_party_object");
    }
    ...
    fn test_proto_to_bcs_canonicality_ab_gas_valid_during() {
        use crate::types::internal_operation::pay_haneul_pt_ab_gas;
        use haneul_types::digests::{ChainIdentifier, CheckpointDigest};

        let sender = HaneulAddress::random_for_testing_only();
        let recipient = HaneulAddress::random_for_testing_only();
        let pt = pay_haneul_pt_ab_gas(sender, vec![recipient], vec![5_000], &[], &[], 5_000).unwrap();
    ...
    /// Stake (system MoveCall): `MoveCall` + `HANEUL_SYSTEM_MUT` shared-object
