...
impl TryConstructTransaction for PayCoin {
    async fn try_fetch_needed_objects(
        ...
        // No GasCoin reference → simulator auto-selects HANEUL gas.
        let pt = pay_coin_pt(
            sender,
            recipients,
            amounts,
        ...
        if gas_coin_objs.is_empty() {
            Ok(TransactionObjectData {
                ...
                objects: coins,
                party_objects: party_coins,
                total_haneul_balance: budget as i128,
                budget,
                address_balance_withdrawal: deficit,
                fss_object_count: None,
                redeem_token_amount: None,
            ...
            let total_haneul_balance =
                gas_coin_objs.iter().map(|c| c.balance()).sum::<u64>() as i128;
