    ...
    async fn try_fetch_needed_objects(
        ...
        for obj in &objects {
            let contents = obj.contents.as_ref().ok_or_else(|| {
                Error::DataError("FungibleStakedHaneul missing contents".to_string())
            })?;
        ...
        if fss_refs.is_empty() {
            return Err(Error::InvalidInput(format!(
                "No FungibleStakedHaneul found for validator {}",
                validator,
            )));
        }

        // Read exchange rate to convert HANEUL amount → pool tokens.
        // Note: the rate is read at metadata time. If the TX lands in a later epoch,
        // the on-chain rate may differ. AtLeast mode may yield slightly more HANEUL than
        ...
        let (haneul_balance, pool_token_balance) = (rate.haneul_balance, rate.pool_token_balance);
        ...
        let token_amount = match redeem_mode {
            ...
            RedeemMode::AtLeast => {
                ...
                // ceil(amount * pool_token_balance / haneul_balance)
                if haneul_balance == 0 {
                    return Err(Error::DataError(
                        "Pool has zero HANEUL balance, cannot compute exchange rate".to_string(),
                    ));
                }
                let numerator =
                    amount as u128 * pool_token_balance as u128 + haneul_balance as u128 - 1;
                let tokens = (numerator / haneul_balance as u128) as u64;
                if tokens > total_tokens {
                    return Err(Error::InvalidInput(format!(
                        "Insufficient FSS balance: AtLeast {} HANEUL requires {} tokens but only {} available",
            ...
            RedeemMode::AtMost => {
                ...
                // floor(amount * pool_token_balance / haneul_balance)
                if haneul_balance == 0 {
                    return Err(Error::DataError(
                        "Pool has zero HANEUL balance, cannot compute exchange rate".to_string(),
                    ));
                ...
                let tokens =
                    (amount as u128 * pool_token_balance as u128 / haneul_balance as u128) as u64;
                if tokens == 0 {
...
/// Phase 3: redeem_fungible_staked_haneul → Balance<HANEUL>
/// Phase 4: coin::from_balance<HANEUL> → Coin<HANEUL>
...
pub fn merge_and_redeem_fss_pt(
    ...
    // Phase 1: Merge all FSS into the first one using staking_pool::join_fungible_staked_haneul
    ...
    // FSS is not a Coin, so we must use staking_pool::split_fungible_staked_haneul instead of SplitCoins
    ...
    // Phase 3: Redeem FSS → Balance<HANEUL>
    ...
    // Phase 4: Convert Balance<HANEUL> → Coin<HANEUL>
    ...
    // Phase 5: Transfer Coin<HANEUL> to sender
