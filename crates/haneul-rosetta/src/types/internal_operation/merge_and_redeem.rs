...
use haneul_types::{HANEUL_FRAMEWORK_PACKAGE_ID, HANEUL_SYSTEM_PACKAGE_ID, Identifier};
...
async fn list_owned_fss(
    client: &mut Client,
    sender: HaneulAddress,
) -> Result<Vec<OwnedFss>, Error> {
    ...
    match redeem_mode {
        ...
        RedeemMode::AtLeast => {
            ...
            let data = fss_data.ok_or_else(|| {
                Error::DataError(
                    "FungibleStakedHaneulData required for AtLeast mode but not supplied"
                        .to_string(),
                )
            })?;
            let token_amount =
        ...
        RedeemMode::AtMost => {
            ...
            let token_amount =
                binary_search_at_most(rate_haneul, rate_token, total_tokens, max_haneul)?
                    .ok_or_else(|| {
                        Error::InvalidInput(
            ...
            for rate_token in [99u64, 100, 1023, 99_997] {
                ...
                for principal_pct in [0u64, 30, 50, 80, 100] {
                    ...
                    for token in [1u64, 2, supply / 2, supply.saturating_sub(1), supply] {
                        ...
                        let expected =
                            expected_haneul_amount(rate_haneul, rate_token, token).unwrap();
