...
//! `MergeAndRedeemFungibleStakedHaneul` construction logic.
//!
//! Translates a user's `(validator, amount, redeem_mode)` request into a PTB
//! that consumes the sender's `FungibleStakedHaneul` for the named validator's
//! pool and produces liquid HANEUL.
//!
...
//!   yields at least `amount` GEUNHWA. The PTB also installs a runtime
...
//!   upper bound `expected = floor(token * haneul_balance / pool_token_balance)`
//!   stays at or below `amount`. The pool's invariant
//!   `actual <= expected` then guarantees the cap holds at execution time.
//!   This is conservative: actual HANEUL may fall a few GEUNHWA short of the cap.
//!
...
use haneul_rpc::proto::haneul::rpc::v2::{ListDynamicFieldsRequest, ListOwnedObjectsRequest};
...
use haneul_types::{Identifier, HANEUL_FRAMEWORK_PACKAGE_ID, HANEUL_SYSTEM_PACKAGE_ID};

use crate::account::FungibleStakedHaneulBcs;
...
pub struct MergeAndRedeemFungibleStakedHaneul {
...
/// `FungibleStakedHaneulData` plus the latest exchange rate, sufficient to mirror
/// the chain's payout formula in pure Rust.
#[derive(Clone, Debug)]
pub(crate) struct PoolRedeemData {
    /// Latest `exchange_rate.haneul_amount` (= pool.haneul_balance within an epoch).
    pub rate_haneul: u64,
    /// Latest `exchange_rate.pool_token_amount` (= pool.pool_token_balance).
    pub rate_token: u64,
    /// `FungibleStakedHaneulData.principal.value()` — current Balance<HANEUL> in the
    /// FSS sub-pool. Mutates within an epoch as other users convert/redeem.
    pub fss_principal: u64,
    /// `FungibleStakedHaneulData.total_supply` — total outstanding FSS pool tokens.
...
/// Mirror of `0x3::staking_pool::PoolTokenExchangeRate::get_haneul_amount` at
/// `staking_pool.move:638-646`. Returns the HANEUL cap for `token_amount` pool
/// tokens given an exchange rate of `(rate_haneul, rate_token)`.
///
/// The chain enforces the invariant
/// `actual_redeemed <= floor(token * rate_haneul / rate_token)` even after the
/// per-redeemer split-floor accounting in `calculate_fungible_staked_haneul_withdraw_amount`
/// (see `staking_pool.move:264-268`), so any token count satisfying
/// `expected_haneul_amount(token) <= max_haneul` will stay within the cap at
/// execution time regardless of intra-epoch pool drift.
pub(crate) fn expected_haneul_amount(
    rate_haneul: u64,
    rate_token: u64,
    token_amount: u64,
) -> Result<u64, Error> {
    if rate_haneul == 0 || rate_token == 0 {
        return Ok(token_amount);
    }
    mul_div_u64(token_amount, rate_haneul, rate_token)
}

/// Mirror of `0x3::staking_pool::calculate_fungible_staked_haneul_withdraw_amount`
/// at `staking_pool.move:231-271`. Returns the actual HANEUL the chain will
...
/// by ~2 GEUNHWA per redemption (one GEUNHWA per floor, scaled by `token/supply`).
/// AtLeast selection must use this (not `expected_haneul_amount`) to avoid picking
/// a token count whose actual payout falls below the user's `min_haneul`.
///
...
pub(crate) fn mirror_redeem_actual(data: &PoolRedeemData, token_amount: u64) -> Result<u64, Error> {
    ...
    // total_haneul = exchange_rate.get_haneul_amount(total_supply)
    let total_haneul = if data.rate_haneul == 0 || data.rate_token == 0 {
        data.fss_total_supply
    } else {
        mul_div_u64(data.rate_haneul, data.fss_total_supply, data.rate_token)?
    };

    let principal = data.fss_principal.min(total_haneul);
    let rewards = total_haneul - principal;
...
/// `mirror_redeem_actual(data, token_amount) >= target_haneul`.
///
/// Returns `Ok(None)` if even redeeming `total_tokens` produces less than
/// `target_haneul` (caller reports "Insufficient FSS balance"). Returns
...
pub(crate) fn binary_search_at_least(
    ...
    if mirror_redeem_actual(data, total_tokens)? < target_haneul {
        return Ok(None);
    }
    let mut lo: u64 = 1;
    ...
    while lo < hi {
        let mid = lo + (hi - lo) / 2;
        if mirror_redeem_actual(data, mid)? >= target_haneul {
            hi = mid;
        } else {
            lo = mid + 1;
        }
...
/// `expected_haneul_amount(rate_haneul, rate_token, token_amount) <= max_haneul`.
///
/// Returns `Ok(None)` if even one token would already exceed `max_haneul`
...
pub(crate) fn binary_search_at_most(
    ...
    if expected_haneul_amount(rate_haneul, rate_token, 1)? > max_haneul {
        return Ok(None);
    }
    let mut lo: u64 = 1;
    let mut hi: u64 = total_tokens;
    ...
    while lo <= hi {
        let mid = lo + (hi - lo) / 2;
        if expected_haneul_amount(rate_haneul, rate_token, mid)? <= max_haneul {
            ans = Some(mid);
            if mid == u64::MAX {
                break;
            }
...
/// Information extracted from a single owned `FungibleStakedHaneul` object.
#[derive(Clone, Debug)]
struct OwnedFss {
...
async fn list_owned_fss(client: &mut Client, sender: HaneulAddress) -> Result<Vec<OwnedFss>, Error> {
...
/// `(rate_haneul, rate_token, epoch)` snapshot. Supports both active and inactive
/// validator pools — the chain's `validator_set::redeem_fungible_staked_haneul`
...
async fn resolve_pool_fss_and_rate(
    ...
    target_validator: HaneulAddress,
) -> Result<PoolFssRateSnapshot, Error> {
    let owned = list_owned_fss(client, sender).await?;
    if owned.is_empty() {
        return Err(Error::InvalidInput(format!(
            "No FungibleStakedHaneul found for sender {sender}"
        )));
    }

    let snapshot = crate::account::get_validator_set_snapshot(client).await?;
    let target_validator_addr = haneul_sdk_types::Address::from(target_validator);
    ...
    for (pool_id, fss_list) in &by_pool {
        ...
        if let Some(table_id_str) = snapshot.inactive_validators_table_id.as_deref() {
            ...
            {
                let (rate_haneul, rate_token) = fetch_pool_exchange_rate_at_epoch(
                    client,
    ...
    Err(Error::InvalidInput(format!(
        "No FungibleStakedHaneul found for validator {target_validator}'s pool \
...
fn build_snapshot(
    ...
    PoolFssRateSnapshot {
        fss_refs,
        total_tokens,
        rate_haneul: rate_info.haneul_balance,
...
/// `pool.haneul_balance / pool_token_balance` directly. For inactive pools these
/// live fields can drift from the exchange-rate snapshot when other users do
/// regular `request_withdraw_stake` (StakedHaneul, not FSS) on the deactivated
/// pool — `staking_pool.move:187-188` immediately processes the pending
/// withdrawal for inactive/preactive pools, which mutates `haneul_balance` and
...
async fn lookup_inactive_pool(
    ...
    use haneul_types::dynamic_field::{Field, derive_dynamic_field_id};
    use haneul_types::id::ID;
    use haneul_types::haneul_system_state::ValidatorWrapper;
    use haneul_types::haneul_system_state::haneul_system_state_inner_v1::ValidatorV1;

    let id_type = haneul_types::TypeTag::from_str("0x2::object::ID")
    ...
    let u64_type = haneul_types::TypeTag::U64;
    let version_bcs =
    ...
    if validator.metadata.haneul_address != expected_validator {
        return Ok(None);
    }
...
/// (`staking_pool.move:587-608`). Returns `(haneul_amount, pool_token_amount)`
...
/// returns `initial_exchange_rate()` which `get_haneul_amount` then treats as
/// 1:1 — we surface the same `(0, 0)` here so downstream `expected_haneul_amount`
...
async fn fetch_pool_exchange_rate_at_epoch(
    ...
    for epoch in candidates {
        if let Some((haneul_amount, pool_token_amount)) =
            try_read_exchange_rate(client, table_id, epoch).await?
        {
            return Ok((haneul_amount, pool_token_amount));
        }
    }
    Ok((0, 0))
}
...
/// struct at `staking_pool.move:69-72`. We can't use `haneul_types`'s
...
async fn try_read_exchange_rate(
    ...
    let field_id = derive_dynamic_field_id(table_id, &haneul_types::TypeTag::U64, &key_bcs)
...
async fn try_read_object_contents(
    ...
    let request = haneul_rpc::proto::haneul::rpc::v2::GetObjectRequest::default()
...
struct PoolFssRateSnapshot {
    ...
    /// `FungibleStakedHaneulData` for AtLeast plans.
    pool_extra_fields_id: Option<ObjectID>,
}

/// Subset of `0x3::staking_pool::FungibleStakedHaneulData` we BCS-deserialize.
/// Field order MUST match the Move source at `staking_pool.move:98-104`:
///   `id: UID, total_supply: u64, principal: Balance<HANEUL>`
/// where `UID` is a 32-byte address and `Balance<HANEUL>` is a `{ value: u64 }`
/// struct (stored positionally as a u64).
#[derive(Deserialize, Debug)]
struct FungibleStakedHaneulDataBcs {
    _id: haneul_sdk_types::Address,
    total_supply: u64,
...
/// Fetch the `FungibleStakedHaneulData` stored at
/// `pool.extra_fields[FungibleStakedHaneulDataKey {}]`. Lists the bag's dynamic
/// fields and finds the one whose `value_type` matches
/// `<HANEUL_SYSTEM_PACKAGE>::staking_pool::FungibleStakedHaneulData`, then BCS-
...
/// `bcs::to_bytes(&FungibleStakedHaneulDataKey {})` and TypeTag canonicalization).
...
async fn fetch_fss_data(client: &mut Client, bag_id: ObjectID) -> Result<(u64, u64), Error> {
    let want_type = format!(
        "{}::staking_pool::FungibleStakedHaneulData",
    ...
    for df in dynamic_fields {
        ...
        let bytes = df
            ...
            .and_then(|b| b.value.as_deref())
            .ok_or_else(|| {
                Error::DataError("FungibleStakedHaneulData entry has empty value".to_string())
            })?;
        let data: FungibleStakedHaneulDataBcs = bcs::from_bytes(bytes)
            .map_err(|e| Error::DataError(format!("FungibleStakedHaneulData decode: {e}")))?;
        return Ok((data.principal.value, data.total_supply));
    }

    Err(Error::DataError(format!(
        "No FungibleStakedHaneulData entry found in pool extra_fields bag {bag_id} \
...
#[async_trait]
impl TryConstructTransaction for MergeAndRedeemFungibleStakedHaneul {
    async fn try_fetch_needed_objects(
        ...
        // whose actual payout >= min_haneul. Fetched lazily so All/AtMost paths
            ...
            let bag_id = pool_extra_fields_id.ok_or_else(|| {
                Error::DataError(
                    "Staking pool extra_fields.id missing from system state response — \
                     cannot locate FungibleStakedHaneulData for AtLeast mode"
                        .to_string(),
                )
            })?;
        ...
        // from the same RPC response as `(rate_haneul, rate_token)`, so there is
...
/// (`calculate_fungible_staked_haneul_withdraw_amount`) and binary-searches for
/// the minimum token count whose actual payout is `>= min_haneul`. `fss_data`
/// must be `Some` for AtLeast — without it we'd fall back to the `expected`
/// formula which can pick a token count whose actual falls 1-2 GEUNHWA short,
...
pub(crate) fn build_redeem_plan(
    ...
    match redeem_mode {
        ...
        RedeemMode::AtLeast => {
            let min_haneul = amount.ok_or_else(|| {
                Error::InvalidInput("amount required for AtLeast mode".to_string())
            })?;
            if min_haneul == 0 {
                return Err(Error::InvalidInput(
                    "AtLeast amount must be at least 1 GEUNHWA".to_string(),
                ));
            }
            if rate_haneul == 0 || rate_token == 0 {
            ...
            let data = fss_data.ok_or_else(|| {
                Error::DataError(
                    "FungibleStakedHaneulData required for AtLeast mode but not supplied".to_string(),
                )
            })?;
            let token_amount =
                binary_search_at_least(data, total_tokens, min_haneul)?.ok_or_else(|| {
                    Error::InvalidInput(format!(
                        "Insufficient FSS balance: cannot deliver AtLeast {min_haneul} HANEUL \
        ...
        RedeemMode::AtMost => {
            let max_haneul = amount.ok_or_else(|| {
                Error::InvalidInput("amount required for AtMost mode".to_string())
            })?;
            if max_haneul == 0 {
                return Err(Error::InvalidInput(
                    "AtMost amount must be at least 1 GEUNHWA".to_string(),
                ));
            }
            if rate_haneul == 0 || rate_token == 0 {
                return Err(Error::DataError(
            ...
            let token_amount = binary_search_at_most(rate_haneul, rate_token, total_tokens, max_haneul)?
                .ok_or_else(|| {
                    Error::InvalidInput(
...
/// `split_fungible_staked_haneul` step. If the search picked exactly the total,
...
/// 2. (Partial) `split_fungible_staked_haneul` to a sub-FSS of `token_amount`.
/// 3. `redeem_fungible_staked_haneul` → `Balance<HANEUL>`.
/// 4. (`AtLeast` only) `balance::split(&mut Balance<HANEUL>, min_haneul)` → sub-balance,
///    then `balance::join` it back. The split aborts on chain when the redeemed
///    `Balance.value` is below `min_haneul`, providing a runtime guard against
///    pool drift between simulate and execution. The join restores the original
///    balance so subsequent commands see a single, full `Balance<HANEUL>` again.
/// 5. `coin::from_balance<HANEUL>` → `Coin<HANEUL>`.
...
/// chain's `expected = floor(token * rate_haneul / rate_token)` stays at or below
/// the user-supplied `max_haneul`; the on-chain invariant `actual <= expected`
...
pub fn merge_and_redeem_fss_pt(
    ...
    let redeem_target = if let Some(token_amount) = split_token_amount {
        ...
        builder.command(Command::move_call(
            HANEUL_SYSTEM_PACKAGE_ID,
            Identifier::new("staking_pool")?,
            Identifier::new("split_fungible_staked_haneul")?,
            vec![],
    ...
    let balance_result = builder.command(Command::move_call(
        ...
        Identifier::new("redeem_fungible_staked_haneul")?,
        vec![],
        vec![system_state, redeem_target],
    ));

    let haneul_type = haneul_types::TypeTag::from_str("0x2::haneul::HANEUL")?;

    if let RedeemPlan::AtLeast { min_haneul, .. } = plan {
        let min_arg = builder.pure(*min_haneul)?;
        let split_balance = builder.command(Command::move_call(
            HANEUL_FRAMEWORK_PACKAGE_ID,
            Identifier::new("balance")?,
            Identifier::new("split")?,
            vec![haneul_type.clone()],
...
mod tests {
    ...
    fn atmost_picks_maximum_token_under_cap() {
        ...
        assert_eq!(token, 49);
        assert!(expected_haneul_amount(200, 99, token).unwrap() <= 100);
        assert!(expected_haneul_amount(200, 99, token + 1).unwrap() > 100);
    }

    #[test]
    fn atmost_returns_none_when_one_token_already_over_cap() {
        // rate 1000:1 means 1 token = 1000 HANEUL; cap of 1 HANEUL is unsatisfiable.
    ...
    fn mul_div_safely_handles_typical_pool_scale() {
        // Typical pool: ~10^9 HANEUL total, 1:1 rate. Should not overflow.
    ...
    fn fss(rate_haneul: u64, rate_token: u64, principal: u64, supply: u64) -> PoolRedeemData {
        PoolRedeemData {
            rate_haneul,
            rate_token,
            fss_principal: principal,
    ...
    fn build_plan_atleast_picks_minimum_token_satisfying_actual() {
        ...
        //   total_haneul = floor(100*100/100) = 100
    ...
    fn build_plan_atleast_avoids_split_floor_underrun() {
        ...
        // expected-rate ceiling underestimates by 1 GEUNHWA. Pick a pool where
        // mirror(t) = expected(t) - 1 for the candidate t, so a naive ceil
        // formula would pick t but actual(t) < min_haneul.
        //
        // Setup: rate 7:3 (haneul:token), supply 30, principal 7. Each token
        // expects floor(1*7/3) = 2 GEUNHWA cap, but split-floor accounting gives
        //   total_haneul = floor(7*30/3) = 70
        ...
        // Asking for min_haneul=35: naive ceil(35*3/7) = 15 picks t=15 → actual=34 < 35 → guard would abort.
    ...
    fn build_plan_atleast_rejects_when_insufficient_pool() {
        // Need 1000 HANEUL from a pool with 10 tokens at 1:1 — impossible.
    ...
    fn build_plan_atleast_rejects_zero_amount() {
        ...
        let err = build_redeem_plan(RedeemMode::AtLeast, Some(0), 100, 200, 100, Some(&data))
            .expect_err("should fail");
        assert!(format!("{err}").contains("at least 1 GEUNHWA"));
    }
    ...
    fn build_plan_atleast_rejects_missing_fss_data() {
        let err = build_redeem_plan(RedeemMode::AtLeast, Some(5), 100, 200, 100, None)
            .expect_err("should fail");
        assert!(format!("{err}").contains("FungibleStakedHaneulData required"));
    }
    ...
    fn build_plan_atleast_full_redeem_omits_split() {
        // Pool of 100 supply at 1:1 — to deliver 100 HANEUL we need all 100 tokens.
        // binary_search returns total_tokens, so the plan should set
        // `token_amount = None` and the PTB will skip `split_fungible_staked_haneul`.
    ...
    fn build_plan_atmost_full_redeem_omits_split() {
        ...
        match plan {
            RedeemPlan::AtMost {
                ...
                assert_eq!(max_haneul, 1_000_000);
            }
            _ => panic!("wrong variant"),
        }
    }
    ...
    fn mirror_invariant_actual_le_expected_with_small_gap() {
        // Property check: mirror_redeem_actual <= expected_haneul_amount across a
        // grid of plausible pool states, with the gap bounded by ~2 GEUNHWA.
        for rate_haneul in [100u64, 199, 1024, 100_003] {
            for rate_token in [99u64, 100, 1023, 99_997] {
                ...
                for principal_pct in [0u64, 30, 50, 80, 100] {
                    ...
                    let data = fss(rate_haneul, rate_token, principal, supply);
                    for token in [1u64, 2, supply / 2, supply.saturating_sub(1), supply] {
                        if token == 0 || token > supply {
                            continue;
                        }
                        let expected = expected_haneul_amount(rate_haneul, rate_token, token).unwrap();
                        let actual = mirror_redeem_actual(&data, token).unwrap();
                        assert!(actual <= expected, "actual={actual} > expected={expected}");
                        assert!(
                            actual + 2 >= expected,
                            "gap > 2 GEUNHWA: expected={expected} actual={actual}"
                        );
                    }
                }
            }
    ...
    fn ptb_all_omits_split_and_guard() {
        ...
        let move_calls: Vec<&str> = pt
            ...
            .collect();
        assert!(move_calls.contains(&"redeem_fungible_staked_haneul"));
        assert!(move_calls.contains(&"from_balance"));
        assert!(!move_calls.contains(&"split_fungible_staked_haneul"));
    ...
    fn ptb_atleast_includes_balance_split_join_guard() {
        ...
        let move_calls: Vec<(&str, &str)> = pt
            ...
            .collect();
        assert!(move_calls.contains(&("staking_pool", "join_fungible_staked_haneul")));
        assert!(move_calls.contains(&("staking_pool", "split_fungible_staked_haneul")));
        assert!(move_calls.contains(&("haneul_system", "redeem_fungible_staked_haneul")));
    ...
    fn ptb_atleast_full_redeem_omits_split_keeps_guard() {
        // token_amount = None → no `split_fungible_staked_haneul`. The chain
        ...
        let move_calls: Vec<(&str, &str)> = pt
            ...
            .collect();
        assert!(
            !move_calls.contains(&("staking_pool", "split_fungible_staked_haneul")),
