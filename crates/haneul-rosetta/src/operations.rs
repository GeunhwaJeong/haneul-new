...
use haneul_types::{HANEUL_FRAMEWORK_PACKAGE_ID, HANEUL_SYSTEM_ADDRESS, HANEUL_SYSTEM_PACKAGE_ID};

use crate::types::internal_operation::{
    ConsolidateAllStakedHaneulToFungible, PayCoin, PayHaneul, Stake, WithdrawStake,
};
use crate::types::{
...
impl Operations {
    ...
    fn consolidate_to_fungible_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        let OperationMetadata::ConsolidateAllStakedHaneulToFungible { validator } = metadata else {
            return Err(Error::InvalidInput(
...
pub enum OperationMetadata {
    ...
    ConsolidateAllStakedHaneulToFungible { validator: HaneulAddress },
}
...
mod tests {
    ...
    async fn test_operation_data_parsing_pay_coin() -> Result<(), anyhow::Error> {
        ...
        let pt = pay_coin_pt(sender, vec![recipient], vec![10000], &[coin], &[], 0, &HANEUL)?;
        let gas_price = 10;
        let data = TransactionData::new_programmable(
            sender,
            vec![gas],
