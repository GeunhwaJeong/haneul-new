...
impl Operations {
    ...
    fn is_gascoin_transfer(tx: &HaneulTransactionBlockKind) -> bool {
        if let HaneulTransactionBlockKind::ProgrammableTransaction(pt) = tx {
            let HaneulProgrammableTransactionBlock {
                inputs: _,
                commands,
            } = &pt;
        ...
        if !operations.iter().any(|operation| {
            if let Some(amount) = &operation.amount {
                if let Some(account) = &operation.account {
                    if account.address == gas_owner && amount.currency == *HANEUL {
                        return true;
                    }
                }
            }
