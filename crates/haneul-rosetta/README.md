...
### Build from source
#### 0. Checkout and build Haneul
Checkout the [Haneul source code](https://github.com/GeunhwaJeong/haneul) and compile using `cargo build --release`, the binaries will be located in `target/release` directory.

#### 1. Genesis

`./haneul genesis -f`  
The Haneul genesis process will create configs and coins for testing, the config files are stored in `~/.haneul/haneul_config` by
default.
...
| Method | Endpoint         | Description                    | Haneul Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      Haneul Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by Haneul checkpoint in phase 2) |    Online     |
...
## Haneul transaction <> Rosetta Operation conversion explained
There are 2 places we convert Haneul's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferHaneul` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferHaneul contains all the balance change information(amount field) already, 
