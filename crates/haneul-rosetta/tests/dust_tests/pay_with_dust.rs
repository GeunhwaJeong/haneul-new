...
use haneul_json_rpc_types::{
    HaneulExecutionStatus, HaneulTransactionBlockDataAPI, HaneulTransactionBlockEffectsAPI,
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_budget_none() -> Result<()> {
    ...
    let resp = client
        ...
        .await?;
    assert!(*resp.effects.unwrap().status() == HaneulExecutionStatus::Success);
    ...
    let details = Some(json!({ "error":
        format!("Invalid input: Address {sender} does not have amount: {recipient_change} + budget: {expected_budget} balance. HANEUL balance: {}.", recipient_change + expected_budget - 1)
    }));
    assert_eq!(
        err,
        RosettaError {
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_with_budget() -> Result<()> {
    ...
    assert_eq!(
        ...
        RosettaError {
            code: 16,
            message: "Haneul rpc error".to_string(),
            description: None,
            retriable: false,
            details,
        },
