...
async fn test_stake_with_many_small_coins() -> Result<()> {
    ...
    // Use 150K HANEUL to ensure we stay under the 1M HANEUL mock gas limit
    const AMOUNT_150K_HANEUL: u64 = 150_000_000_000_000;
    let accounts = (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![AMOUNT_150K_HANEUL, AMOUNT_150K_HANEUL], // Two gas objects per account
        })
        .collect();
    ...
    // With 150K HANEUL, stake 50K HANEUL split into 300 small coins
    let amount_to_stake: i128 = 50_000_000_000_000; // 50K HANEUL
    let num_coins = 300;
    let split_amount = (amount_to_stake / num_coins) as u64; // ~0.167 HANEUL per coin
    assert!(
        num_coins > 255,
    ...
    let coin_to_split = coin_to_split_obj
        .bcs()
        .deserialize::<haneul_types::object::Object>()?;
    ...
    let mut ptb = ProgrammableTransactionBuilder::new();
    ptb.transfer_arg(recipient, haneul_types::transaction::Argument::GasCoin);
    let tx_data = TransactionData::new_programmable(
        sender,
        vec![gas_object],
        ptb.finish(),
    ...
    let validator = system_state.validators.unwrap().active_validators[0]
        .address()
        .parse::<haneul_types::base_types::HaneulAddress>()
        .unwrap();
...
async fn test_stake_with_multiple_merges() -> Result<()> {
    ...
    // With 150K HANEUL, stake 50K HANEUL split into 866 small coins to test multiple merges
    let amount_to_stake: i128 = 50_000_000_000_000; // 50K HANEUL
    let num_coins = 255 + 511 + 100; // 866 coins - tests multiple merge operations
    let split_amount = (amount_to_stake / num_coins) as u64; // ~0.058 HANEUL per coin
    assert!(
        num_coins > 255 + 511,
...
async fn test_stake_with_coin_limit() -> Result<()> {
    ...
    // With 150K HANEUL, we want split_amount that gives us ~2000 coins
    let target_coins = 2000u64;
    let split_amount = get_coin_value(&coin_to_split) / (target_coins + 1); // ~75M GEUNHWA per coin
