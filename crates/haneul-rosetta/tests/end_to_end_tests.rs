...
#[tokio::test]
async fn test_get_staked_haneul() -> Result<()> {
    ...
    let arguments = vec![
        ptb.input(CallArg::HANEUL_SYSTEM_MUT)?,
...
async fn test_stake_all() {
    ...
    // Create test cluster with 150K HANEUL instead of the default 150M
    // 150K HANEUL = 150_000_000_000_000 GEUNHWA (150K * 1e9)
    const AMOUNT_150K_HANEUL: u64 = 150_000_000_000_000;

    // Create 5 test accounts, but only the first one gets custom amount
    let accounts = (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![AMOUNT_150K_HANEUL], // Single gas object with 150K HANEUL
        })
        .collect();
...
#[tokio::test]
async fn test_pay_haneul_multiple_times() {
    let test_cluster = TestClusterBuilder::new()
...
async fn test_transfer_single_gas_coin() {
    ...
    let input_objects = pt
        ...
        .collect::<Vec<_>>();
    let gas = vec![get_random_haneul(&mut client.clone(), sender, input_objects).await];
    ...
    let signature = keystore
        .sign_secure(&sender, &data, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    operations.into_iter().for_each(|op| {
        ...
        if op.type_ == OperationType::PayHaneul {
...
/// The below HaneulTransactionBlockResponse is created by using the below contract:
/// ```move
/// module vault::vault;
///
...
/// }
///
/// public fun from_coin(coin: Coin<HANEUL>, ctx: &mut TxContext): Vault {
///     Vault {
///         id: object::new(ctx),
...
/// }
///
/// public fun to_coin(self: Vault, ctx: &mut TxContext): Coin<HANEUL> {
///     let Vault { id, balance } = self;
///     id.delete();
///     balance.into_coin(ctx)
/// }
///
/// public fun amount_to_coin(self: &mut Vault, amount: u64, ctx: &mut TxContext): Coin<HANEUL> {
...
async fn test_balance_from_obj_paid_eq_gas() {
    ...
    let sender_address = HaneulAddress::from_str(SENDER).unwrap();
    let recipient_address = HaneulAddress::from_str(RECIPIENT).unwrap();
    ...
    let tx_data = TestTransactionBuilder::new(sender_address, gas_ref, 1000)
        .transfer_haneul(Some(AMOUNT as u64), recipient_address)
        .build();
    ...
    let effects = TestEffectsBuilder::new(&signed_tx)
        .with_status(haneul_types::execution_status::ExecutionStatus::Success)
        .build();
    ...
    let mut balance_changes: HashMap<HaneulAddress, i128> = HashMap::new();
    operations.into_iter().for_each(|op| {
    ...
    assert_eq!(
        *balance_changes
            .get(&HaneulAddress::from_str(RECIPIENT).unwrap())
            .unwrap(),
        AMOUNT
    );
    assert_eq!(
        *balance_changes
            .get(&HaneulAddress::from_str(SENDER).unwrap())
            .unwrap_or(&0),
        0
    );
}
...
async fn test_stake_with_party_objects() {
    ...
    // Stake amount must exceed regular coins (60M HANEUL) to force party coin usage
    let stake_amount = "70000000000000000"; // 70M HANEUL in GEUNHWA
    let ops = serde_json::from_value(json!(
        [{
            "operation_identifier":{"index":0},
            "type":"Stake",
...
#[tokio::test]
async fn test_pay_haneul_with_party_objects() {
    let test_cluster = TestClusterBuilder::new().build().await;
    let sender = test_cluster.get_address_0();
    let recipient = test_cluster.get_address_1();
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    // Pay HANEUL with party objects present
    // Payment amount must exceed regular coins (60M HANEUL) to force party coin usage
    let payment_amount = "70000000000000000"; // 70M HANEUL in GEUNHWA
    let ops = serde_json::from_value(json!(
        [{
            "operation_identifier":{"index":0},
            "type":"PayHaneul",
    ...
    assert!(
        tx.effects().status().success(),
        "Pay HANEUL with party objects failed: {:?}",
        tx.effects().status().error()
    );
}
...
async fn test_pay_coin_with_party_objects() {
    ...
    let coin_type_tag: haneul_sdk_types::TypeTag = coin_type.parse().unwrap();
    let custom_coins = client
        .select_up_to_n_largest_coins(
            &haneul_sdk_types::Address::from(sender),
            &coin_type_tag,
            5,
            &[],
        )
...
async fn create_party_objects_with_type(
    ...
    let exclude_addrs: Vec<haneul_sdk_types::Address> = coins_to_convert
        .iter()
        .map(|coin_ref| haneul_sdk_types::Address::from(coin_ref.0.into_bytes()))
        .collect();

    let haneul_type = haneul_sdk_types::TypeTag::from_str("0x2::haneul::HANEUL")?;
...
async fn test_network_status() -> Result<()> {
    ...
    let tx_data = TransactionData::new_transfer_haneul(
        recipient,
        sender,
        Some(1),
        gas_object,
...
async fn test_network_list() -> Result<()> {
    ...
    assert_eq!(networks[0]["blockchain"].as_str().unwrap(), "haneul");
...
async fn test_account_coins() -> Result<()> {
    ...
    for coin in coins {
        ...
        assert_eq!(currency_symbol, "HANEUL");
...
#[tokio::test]
async fn test_consolidate_all_staked_haneul_to_fungible() {
    let test_cluster = TestClusterBuilder::new().build().await;
    let sender = test_cluster.get_address_0();
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    // Consolidate: convert StakedHaneul → FungibleStakedHaneul via Rosetta construction flow
    ...
    // Verify: StakedHaneul should be gone, FungibleStakedHaneul should exist
    use futures::TryStreamExt;
    use haneul_rpc::proto::haneul::rpc::v2::ListOwnedObjectsRequest;

    let staked_haneul_request = ListOwnedObjectsRequest::default()
    ...
    let staked_haneul: Vec<_> = client
        .clone()
        .list_owned_objects(staked_haneul_request)
        .try_collect()
        .await
        .unwrap();
    assert!(
        staked_haneul.is_empty(),
        "Expected no StakedHaneul objects after consolidation, found {}",
        staked_haneul.len()
    );
    ...
    assert_eq!(
        fss_objects.len(),
        1,
        "Expected exactly 1 FungibleStakedHaneul after consolidation, found {}",
        fss_objects.len()
    );
}

/// Stake 3 times with the same validator, advance epoch, then consolidate.
/// Verifies all StakedHaneul objects are converted and merged into a single FSS.
#[tokio::test]
async fn test_consolidate_multiple_staked_haneul() {
    use futures::TryStreamExt;
    use haneul_rpc::proto::haneul::rpc::v2::ListOwnedObjectsRequest;
    ...
    // Verify we have 3 StakedHaneul before consolidation
    let staked_haneul_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
        .with_page_size(10u32)
        .with_read_mask(FieldMask::from_paths(["object_id"]));
    let staked_haneul_before: Vec<_> = client
        .clone()
        .list_owned_objects(staked_haneul_request)
        .try_collect()
        .await
        .unwrap();
    assert_eq!(
        staked_haneul_before.len(),
        3,
        "Expected 3 StakedHaneul before consolidation, found {}",
        staked_haneul_before.len()
    );
    ...
    // Verify: no StakedHaneul remaining
    let staked_haneul_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
        .with_page_size(10u32)
        .with_read_mask(FieldMask::from_paths(["object_id"]));
    let staked_haneul_after: Vec<_> = client
        .clone()
        .list_owned_objects(staked_haneul_request)
        .try_collect()
        .await
        .unwrap();
    assert!(
        staked_haneul_after.is_empty(),
        "Expected no StakedHaneul after consolidation, found {}",
        staked_haneul_after.len()
    );
...
async fn test_consolidate_with_preexisting_fss() {
    ...
    // Manually convert the StakedHaneul to FSS via a direct PTB
    let staked_haneul_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    let staked_objs: Vec<_> = client
        ...
        .await
        .unwrap();
    assert_eq!(staked_objs.len(), 1, "Expected 1 StakedHaneul to convert");
    ...
    let mut ptb = ProgrammableTransactionBuilder::new();
    let system_state_arg = ptb.input(CallArg::HANEUL_SYSTEM_MUT).unwrap();
    let staked_haneul_arg = ptb.obj(ObjectArg::ImmOrOwnedObject(staked_ref)).unwrap();
    let fss_result = ptb.command(Command::move_call(
        HANEUL_SYSTEM_PACKAGE_ID,
        HANEUL_SYSTEM_MODULE_NAME.to_owned(),
        Identifier::new("convert_to_fungible_staked_haneul").unwrap(),
        vec![],
        vec![system_state_arg, staked_haneul_arg],
    ));
    ...
    // Verify we now have 1 FSS and 0 StakedHaneul
    let fss_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::FungibleStakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    // Now consolidate: should merge the pre-existing FSS with the new converted StakedHaneul
    let consolidate_ops = serde_json::from_value(json!(
        [{
            "operation_identifier": {"index": 0},
            "type": "ConsolidateAllStakedHaneulToFungible",
...
/// separately. Then consolidate to merge them into a single FSS (no StakedHaneul
...
async fn test_consolidate_fss_only_merge() {
    ...
    // Convert each StakedHaneul to FSS manually, one at a time
    for _ in 0..2 {
        let staked_haneul_request = ListOwnedObjectsRequest::default()
            .with_owner(sender.to_string())
            .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
    ...
    // Verify we have 2 FSS and 0 StakedHaneul
    let fss_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::FungibleStakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    // Consolidate: should only merge FSS, no StakedHaneul to convert
    let consolidate_ops = serde_json::from_value(json!(
        [{
            "operation_identifier": {"index": 0},
            "type": "ConsolidateAllStakedHaneulToFungible",
...
/// consolidate only for validator A. Verifies only A's StakedHaneul is converted,
/// while B's StakedHaneul remains untouched.
#[tokio::test]
async fn test_consolidate_multi_validator_isolation() {
    ...
    // Verify we have 2 StakedHaneul total (one per validator)
    let staked_haneul_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    assert_eq!(
        staked_before.len(),
        2,
        "Expected 2 StakedHaneul (one per validator), found {}",
        staked_before.len()
    );
    ...
    // Verify: 1 StakedHaneul remains (validator B's), 1 FSS created (from validator A)
    let staked_haneul_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::StakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    assert_eq!(
        staked_after.len(),
        1,
        "Expected 1 StakedHaneul remaining (validator B's), found {}",
        staked_after.len()
    );
    ...
    assert_eq!(
        fss_objects.len(),
        1,
        "Expected exactly 1 FungibleStakedHaneul (from validator A), found {}",
        fss_objects.len()
    );
}
...
/// Convert 1 StakedHaneul to FSS, then attempt consolidation. With only a single
/// FSS and no StakedHaneul, there is nothing to consolidate, so metadata should
/// return an error.
#[tokio::test]
async fn test_consolidate_noop_single_fss() {
    ...
    // Verify we have exactly 1 FSS and 0 StakedHaneul
    let fss_request = ListOwnedObjectsRequest::default()
        .with_owner(sender.to_string())
        .with_object_type("0x3::staking_pool::FungibleStakedHaneul".to_string())
        .with_page_size(10u32)
    ...
    // Attempt to consolidate: single FSS + 0 StakedHaneul = nothing to do
    let consolidate_ops = serde_json::from_value(json!(
        [{
            "operation_identifier": {"index": 0},
            "type": "ConsolidateAllStakedHaneulToFungible",
...
async fn test_consolidate_unactivated_stakes_only() {
    ...
    // Stake 1 HANEUL but do NOT advance epoch
    let ops = serde_json::from_value(json!(
        [{
            "operation_identifier":{"index":0},
            "type":"Stake",
...
#[tokio::test]
async fn test_fungible_staked_haneul_value() -> Result<()> {
    let test_cluster = TestClusterBuilder::new().build().await;
    let address = test_cluster.get_address_0();
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    // Query FungibleStakedHaneulValue with no FSS — should return 0
    let response: AccountBalanceResponse = rosetta_client
        .call(
            ...
            &AccountBalanceRequest {
                ...
                account_identifier: AccountIdentifier {
                    address,
                    sub_account: Some(SubAccount {
                        account_type: SubAccountType::FungibleStakedHaneulValue,
                    }),
                },
                block_identifier: Default::default(),
                currencies: Currencies(vec![Currency::default()]),
    ...
    // Convert StakedHaneul to FungibleStakedHaneul
    // First, find the StakedHaneul object
    use futures::TryStreamExt;
    use haneul_rpc::proto::haneul::rpc::v2::ListOwnedObjectsRequest;
    ...
    let staked_haneul_objects: Vec<_> = client
        .clone()
        ...
        .await?;
    assert!(
        !staked_haneul_objects.is_empty(),
        "Expected at least one StakedHaneul object"
    );

    let staked_obj = &staked_haneul_objects[0];
    let staked_ref = (
    ...
    let mut ptb = ProgrammableTransactionBuilder::new();
    let system_state_arg = ptb.input(CallArg::HANEUL_SYSTEM_MUT)?;
    let staked_haneul_arg = ptb.obj(ObjectArg::ImmOrOwnedObject(staked_ref))?;
    let fss_result = ptb.command(Command::move_call(
        HANEUL_SYSTEM_PACKAGE_ID,
        HANEUL_SYSTEM_MODULE_NAME.to_owned(),
    ...
    // Now query FungibleStakedHaneulValue — should be > 0
    ...
    assert!(
        ...
        "Expected positive FungibleStakedHaneulValue, got {}",
        fss_response.balances[0].value
    );
    // The value should be approximately 1 HANEUL (1_000_000_000 GEUNHWA) since this is a fresh pool with rate ~1.0
    assert!(
        fss_response.balances[0].value >= 999_000_000
            && fss_response.balances[0].value <= 1_100_000_000,
        "Expected FSS value close to 1 HANEUL, got {}",
        fss_response.balances[0].value
    );
    ...
    let metadata = fss_response.balances[0]
        .metadata
        .as_ref()
        .expect("Expected metadata on FungibleStakedHaneulValue sub-account");
...
#[tokio::test]
async fn test_merge_and_redeem_fungible_staked_haneul() {
    let test_cluster = TestClusterBuilder::new().build().await;
    let sender = test_cluster.get_address_0();
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    let tx = haneul_types::utils::to_sender_signed_transaction(
        convert_tx,
...
async fn run_redeem_flow(
    ...
    keystore: &haneul_keys::keystore::Keystore,
    sender: HaneulAddress,
...
async fn count_fss_objects(client: &mut GrpcClient, owner: HaneulAddress) -> (usize, u64) {
    use futures::TryStreamExt;
    use haneul_rpc::proto::haneul::rpc::v2::ListOwnedObjectsRequest;
    ...
    for obj in &objects {
        if let Some(contents) = &obj.contents {
            ...
            struct FssBcs {
                ...
                _pool_id: haneul_sdk_types::Address,
                value: u64,
            }
...
async fn get_haneul_balance(client: &mut GrpcClient, address: HaneulAddress) -> u64 {
...
async fn test_redeem_multi_validator_isolation() {
    ...
    let staked_haneul_objects: Vec<_> = client
        ...
        .await
        .unwrap();
    assert!(staked_haneul_objects.len() >= 2);

    let gas_price = client.get_reference_gas_price().await.unwrap();

    for staked_obj in &staked_haneul_objects {
        let staked_ref = (
            ObjectID::from_str(staked_obj.object_id()).unwrap(),
            staked_obj.version().into(),
            staked_obj.digest().parse().unwrap(),
    ...
    let pool_b_id = active_validators
        .iter()
        .find(|v| v.address().parse::<HaneulAddress>().ok() == Some(validator_b))
        .unwrap()
        .staking_pool()
        .id()
        .to_string();
    ...
    for obj in &remaining_fss {
        if let Some(contents) = &obj.contents {
            ...
            struct FssBcs {
                _id: haneul_sdk_types::Address,
                pool_id: haneul_sdk_types::Address,
                _value: u64,
            }
...
// ================================================================================
// PR 1 e2e tests — ConsolidateAllStakedHaneulToFungible parser
// ================================================================================
...
/// Note: HaneulEnv serializes as lowercase (e.g., `localnet`), not `LocalNet`.
async fn parse_unsigned(
    ...
    let request = serde_json::json!({
        "network_identifier": {
            "blockchain": "haneul",
            "network": serde_json::to_value(HaneulEnv::LocalNet).unwrap(),
        },
        "signed": false,
...
async fn first_validator(client: &mut GrpcClient) -> HaneulAddress {
    let request = GetEpochRequest::latest().with_read_mask(FieldMask::from_paths(["system_state"]));
    let response = client
        .clone()
        .ledger_client()
...
/// Stake `amount_geunhwa` HANEUL with `validator` via the Rosetta flow.
async fn stake_via_rosetta(
    ...
    let ops = serde_json::from_value(json!([{
        ...
        "type":"Stake",
        "account": { "address": sender.to_string() },
        "amount": { "value": format!("-{}", amount_geunhwa) },
        "metadata": { "Stake": {"validator": validator.to_string()} }
    }]))
    .unwrap();
...
/// Convert every owned `StakedHaneul` to a `FungibleStakedHaneul` via a direct PTB.
...
/// S=1, F=0 — single StakedHaneul conversion, no pre-existing FSS.
#[tokio::test]
async fn test_e2e_parse_consolidate_1_stake_0_fss() {
    ...
    let Some(haneul_rosetta::operations::OperationMetadata::ConsolidateAllStakedHaneulToFungible {
        validator: v,
        staked_haneul_ids,
        fss_ids,
...
async fn test_e2e_parse_consolidate_0_stakes_2_fss() {
    ...
    assert!(
        staked_haneul_ids.is_empty(),
        "pure merge should have no StakedHaneul IDs"
    );
...
async fn test_e2e_block_consolidate_1_stake_0_fss() {
    ...
    let typed = ops_vec
        .iter()
        .find(|o| o.type_ == OperationType::ConsolidateAllStakedHaneulToFungible)
    ...
    // And NO HaneulBalanceChange for sender (Consolidate is object-only, no net HANEUL delta).
    let sender_balance_changes: Vec<_> = ops_vec
        .iter()
        .filter(|o| {
            o.type_ == OperationType::HaneulBalanceChange
                && o.account.as_ref().map(|a| a.address) == Some(sender)
        })
        .collect();
    assert!(
        sender_balance_changes.is_empty(),
        "Consolidate should have no HaneulBalanceChange for sender, got {:?}",
...
async fn test_e2e_consolidate_errors_invalid_validator() {
    ...
    let fake_validator = HaneulAddress::random_for_testing_only();
    let ops = serde_json::from_value(json!([{
        "operation_identifier": {"index": 0},
        "type": "ConsolidateAllStakedHaneulToFungible",
        "account": {"address": sender.to_string()},
        "metadata": {
            "ConsolidateAllStakedHaneulToFungible": {"validator": fake_validator.to_string()}
        }
    }]))
    .unwrap();
...
async fn test_e2e_parse_consolidate_3_stakes_0_fss() {
    ...
    assert_eq!(staked_haneul_ids.len(), 3);
    assert!(fss_ids.is_empty());
}
...
/// S=2, F=2 — full flow with both existing FSS and activated StakedHaneul.
#[tokio::test]
async fn test_e2e_parse_consolidate_2_stakes_2_fss() {
    ...
    assert_eq!(staked_haneul_ids.len(), 2);
    assert_eq!(fss_ids.len(), 2);
}
...
async fn test_e2e_parse_consolidate_multi_validator_isolation() {
    ...
    let validators = response
        ...
        .validators
        .unwrap();
    let validator_a: HaneulAddress = validators.active_validators[0].address().parse().unwrap();
    let validator_b: HaneulAddress = validators.active_validators[1].address().parse().unwrap();
    ...
    let consolidate_ops = serde_json::from_value(json!([{
        ...
        "account": {"address": sender.to_string()},
        "metadata": {
            "ConsolidateAllStakedHaneulToFungible": {"validator": validator_a.to_string()}
        }
    }]))
    .unwrap();
    let flow = rosetta_client
...
async fn submit_and_assert_block_consolidate(
    ...
    assert_eq!(staked_haneul_ids.len(), expected_staked);
    assert_eq!(fss_ids.len(), expected_fss);
    // Must have a Gas op.
    assert!(ops_vec.iter().any(|o| o.type_ == OperationType::Gas));
    // No HaneulBalanceChange for sender (Consolidate is object-only).
    let sender_balance_changes: Vec<_> = ops_vec
        .iter()
        .filter(|o| {
            o.type_ == OperationType::HaneulBalanceChange
    ...
    assert!(
        sender_balance_changes.is_empty(),
        "Consolidate should have no sender HaneulBalanceChange, got {:?}",
        sender_balance_changes
    );
}
...
// ================================================================================
// PR 2 e2e tests — MergeAndRedeemFungibleStakedHaneul parser
// ================================================================================

/// Scaffold: stake `n` times, advance epoch, convert each StakedHaneul to FSS via direct PTB.
...
async fn run_merge_redeem_and_parse(
    ...
    amount: Option<u64>,
    mode: &str,
) -> Vec<haneul_rosetta::operations::Operation> {
    ...
    let ops = serde_json::from_value(json!([{
        ...
        "account": {"address": sender.to_string()},
        "metadata": {
            "MergeAndRedeemFungibleStakedHaneul": metadata_value,
        }
    }]))
    .unwrap();
    let flow = rosetta_client.rosetta_flow(&ops, keystore, None).await;
...
fn assert_merge_redeem_parse_ops(
    ...
    // AtLeast carries `min_haneul` decoded from the on-chain `balance::split`
    // guard; All and the unknown-partial mode emit no amount.
    match expected_mode {
        Some(haneul_rosetta::types::RedeemMode::AtLeast) => {
            assert!(amount.is_some(), "AtLeast parse should recover min_haneul");
        }
        _ => {
...
async fn test_e2e_parse_merge_redeem_single_fss_all() {
    ...
    assert_merge_redeem_parse_ops(&ops, sender, 1, Some(haneul_rosetta::types::RedeemMode::All));
}
...
async fn test_e2e_parse_merge_redeem_three_fss_all() {
    ...
    let ops =
        run_merge_redeem_and_parse(&rosetta_client, keystore, sender, validator, None, "All").await;
    assert_merge_redeem_parse_ops(&ops, sender, 3, Some(haneul_rosetta::types::RedeemMode::All));
}
...
async fn submit_and_assert_block_merge_redeem(
    ...
    let typed = ops_vec
        .iter()
        .find(|o| o.type_ == OperationType::MergeAndRedeemFungibleStakedHaneul)
    ...
    // Gas op is present.
    assert!(ops_vec.iter().any(|o| o.type_ == OperationType::Gas));
    // And HaneulBalanceChange for sender — redemption produces liquid HANEUL.
    ...
    assert!(
        ...
        "MergeAndRedeem should produce a HaneulBalanceChange for sender: {:?}",
        ops_vec
    );
}
...
/// AtLeast amount exceeds available FSS HANEUL value → server should error.
#[tokio::test]
async fn test_e2e_merge_redeem_errors_atleast_exceeds_balance() {
    ...
    // Ask for much more HANEUL than 1 FSS can provide.
    let ops = serde_json::from_value(json!([{
        "operation_identifier": {"index": 0},
        "type": "MergeAndRedeemFungibleStakedHaneul",
        "account": {"address": sender.to_string()},
...
async fn test_e2e_merge_redeem_errors_missing_amount() {
    ...
    let ops = serde_json::from_value(json!([{
        ...
        "metadata": {
            "MergeAndRedeemFungibleStakedHaneul": {
                "validator": HaneulAddress::random_for_testing_only().to_string(),
                "redeem_mode": "AtLeast",
            }
        }
    }]))
...
/// Zero amount for AtLeast/AtMost → write-side error "must be at least 1 GEUNHWA".
#[tokio::test]
