...
use haneul_rosetta::types::{PreprocessMetadata, TransactionIdentifierResponse};
use haneul_rpc::client::Client as GrpcClient;
use haneul_rpc::field::FieldMaskUtil;
use haneul_rpc::proto::haneul::rpc::v2::{GetBalanceRequest, GetEpochRequest, GetTransactionRequest};
use haneul_sdk_types::{Address, TypeTag as SdkTypeTag};
...
use haneul_types::haneul_sdk_types_conversions::type_tag_sdk_to_core;
use haneul_types::transaction::{Command, TransactionData, TransactionDataAPI};
...
fn single_coin_accounts() -> Vec<AccountConfig> {
    ...
    (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![AMOUNT_150K_HANEUL],
        })
        .collect()
}

async fn get_total_balance(client: &mut GrpcClient, address: HaneulAddress, coin_type: &str) -> u64 {
...
async fn get_address_balance(client: &mut GrpcClient, address: HaneulAddress, coin_type: &str) -> u64 {
    let request = GetBalanceRequest::default()
        .with_owner(address.to_string())
        .with_coin_type(coin_type.to_string());
...
fn assert_onchain_gasless(tx: &haneul_rpc::proto::haneul::rpc::v2::ExecutedTransaction) {
...
/// When `coin_type` is `None`, deposits HANEUL (split from GasCoin).
...
async fn deposit_to_address_balance(
    ...
    let (split_source, type_tag) = match coin_type {
        None => (
            haneul_types::transaction::Argument::GasCoin,
        ...
        Some(ct) => {
            ...
            let arg = ptb
                .obj(haneul_types::transaction::ObjectArg::ImmOrOwnedObject(
                    coin_ref,
                ))
                .unwrap();
    ...
    ptb.command(Command::move_call(
        ...
        Identifier::from(haneul_types::coin::COIN_MODULE_NAME),
    ...
    let sig = keystore
        .sign_secure(
            &tx_data.sender(),
            &tx_data,
            shared_crypto::intent::Intent::haneul_transaction(),
        )
        .await
        .unwrap();
    let signed_tx = haneul_types::transaction::Transaction::from_data(tx_data, vec![sig]);
...
/// Sweep all HANEUL coin objects to address balance.
    ...
    loop {
        let coins = client
            .select_up_to_n_largest_coins(
                &Address::from(sender),
                &haneul_sdk_types::StructTag::haneul().into(),
                100,
                &[],
            )
            .await
        ...
        for coin in &coins {
            ...
            ptb.command(Command::move_call(
                ...
                Identifier::from(haneul_types::coin::COIN_MODULE_NAME),
                Identifier::new("send_funds").unwrap(),
                vec![haneul_types::gas_coin::GAS::type_tag()],
...
fn pay_haneul_ops(sender: HaneulAddress, recipient: HaneulAddress, amount: &str) -> Operations {
...
async fn setup_gasless_coin_as_objects(
    ...
    haneul_types::transaction::add_gasless_token_for_testing(coin_type.clone(), min_transfer);
...
#[tokio::test]
async fn test_pay_haneul_ab_gas_coin_payment() {
    ...
    // Deposit 10 HANEUL to address balance via coin::send_funds
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    // Now sender has AB. PayHaneul should use Path A (AB gas).
    let initial_balance = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    let ops = pay_haneul_ops(sender, recipient, "1000000000");
    ...
    let after_balance = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    assert!(
...
/// Sender starts with a small coin (~30 HANEUL) and a larger AB deposit (~100 HANEUL).
/// PayHaneul for an amount larger than the coin forces a deficit withdrawal from AB.
#[tokio::test]
async fn test_pay_haneul_ab_deficit() {
    let accounts = (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![30_000_000_000], // 30 HANEUL
        })
        .collect();
    ...
    // Split 15 HANEUL to AB, leaving ~15 HANEUL in coin (minus gas).
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    // sender now has ~15 HANEUL in coin (minus gas) and 15 HANEUL in AB.
    // PayHaneul 20 HANEUL: coin (~14.5 HANEUL) can't cover it, deficit (~5.5 HANEUL) pulled from AB.
    let initial_balance = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    let payment = 20_000_000_000u64; // 20 HANEUL
    let ops = pay_haneul_ops(sender, recipient, &payment.to_string());
...
/// Path A with no coin objects: sender's entire HANEUL balance is in address balance.
/// PayHaneul draws entirely from AB for both payment and gas.
#[tokio::test]
async fn test_pay_haneul_entirely_from_ab() {
    let test_cluster = TestClusterBuilder::new()
        .with_num_validators(1)
        .with_accounts(single_coin_accounts())
    ...
    assert!(
        coins.is_empty(),
        "Should have zero HANEUL coin objects after sweep"
    );

    let balance_before = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    assert!(
        balance_before > 0,
        "Should still have HANEUL in address balance"
    );

    let payment = 1_000_000_000u64; // 1 HANEUL
    let ops = pay_haneul_ops(sender, recipient, &payment.to_string());
...
/// Path B with AB withdrawal: AB too small for gas (1M geunhwa) so coins provide gas,
...
#[tokio::test]
async fn test_pay_haneul_coin_gas_with_ab() {
    let test_cluster = TestClusterBuilder::new()
        .with_num_validators(1)
        .with_accounts(single_coin_accounts())
        .with_epoch_duration_ms(36000000)
    ...
    // Deposit 1M geunhwa to AB — too small for gas, so Path A fails → Path B.
...
/// Stake from address balance: deposit HANEUL to AB, then stake using the address balance.
#[tokio::test]
async fn test_stake_from_address_balance() {
    ...
    // Deposit HANEUL to address balance
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    let stake_amount = 1_000_000_000u64; // 1 HANEUL
    ...
    let balance_after = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
...
/// Stake all from address balance: deposit HANEUL to AB, then stake all
...
async fn test_stake_deficit_path() {
    let accounts = (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![15_000_000_000], // 15 HANEUL
        })
        .collect();
    ...
    // Deposit 10 HANEUL to AB, leaving ~5 HANEUL in coin (minus gas).
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    // Stake 10 HANEUL: coin (~5 HANEUL) can't cover it, deficit pulled from AB.
    let stake_amount = 10_000_000_000u64; // 10 HANEUL
    let stake_ops: Operations = serde_json::from_value(json!([{
        "operation_identifier": {"index": 0},
        "type": "Stake",
        "account": {"address": sender.to_string()},
...
/// Stake entirely from address balance: sender has zero HANEUL coin objects,
...
/// PayCoin (non-HANEUL) with HANEUL address balance available.
/// 1. Deploy custom coin package + mint coins
/// 2. Deposit HANEUL to address balance
/// 3. PayCoin: custom coins transferred, HANEUL address balance for gas
#[tokio::test]
async fn test_pay_coin_from_address_balance() {
    ...
    // Step 1: Deploy custom coin package and mint coins (needs HANEUL coin objects for gas)
    let init_ret = init_package(
        &test_cluster,
        &mut client,
        keystore,
    ...
    // Step 2: Deposit HANEUL to address balance
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    // Step 3: PayCoin — custom coins transferred, HANEUL address balance available for gas
    ...
    assert!(
        metadata.metadata.gas_coins.is_empty(),
        "PayCoin with HANEUL AB: gas_coins should be empty (AB gas), got {:?}",
        metadata.metadata.gas_coins
    );
    assert_eq!(
...
/// PayCoin with coins + custom coin AB deficit, HANEUL AB gas.
...
/// PayCoin with coins + deficit, HANEUL coin gas (AB too small for gas).
#[tokio::test]
async fn test_pay_coin_ab_deficit_coin_gas() {
    ...
    // Deposit 1M geunhwa HANEUL to AB — too small for gas
    deposit_to_address_balance(
        &test_cluster,
        &mut client,
        keystore,
    ...
    assert!(
        !metadata.metadata.gas_coins.is_empty(),
        "Coin gas: gas_coins should be non-empty (HANEUL AB too small)"
    );
    assert!(
        metadata.metadata.address_balance_withdrawal > 0,
        "AB deficit: address_balance_withdrawal should be > 0"
...
/// PayCoin entirely from non-HANEUL address balance (no coin objects).
...
async fn test_pay_coin_gasless_single_recipient() {
    ...
    // Capture the sender's HANEUL total *after* setup (the deposit txs already spent HANEUL gas), so the
    // post-flow comparison isolates the rosetta payment's gas impact.
    let sender_haneul_before = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    ...
    // Strongest gaslessness assertion: the sender's HANEUL is untouched by the payment.
    let sender_haneul_after = get_total_balance(&mut client, sender, HANEUL_COIN_TYPE).await;
    assert_eq!(
        sender_haneul_after, sender_haneul_before,
        "gasless: sender HANEUL balance must be unchanged"
    );
}
...
async fn test_pay_coin_gasless_ignores_request_budget() {
    ...
    assert_eq!(
        sender_haneul_after, sender_haneul_before,
        "gasless: sender HANEUL must be unchanged despite a request budget"
    );
}
...
async fn test_pay_coin_gasless_from_coin_objects() {
    ...
    assert_eq!(
        sender_haneul_after, sender_haneul_before,
        "gasless: sender HANEUL must be unchanged"
    );
}
...
async fn test_address_balance_gas_payment_parsing() {
    ...
    use haneul_types::base_types::{ObjectDigest, ObjectID};
    ...
    let tx_data = TestTransactionBuilder::new(sender_address, gas_ref, 1000)
        .transfer_haneul(Some(1_000_000), recipient_address)
        .build();
...
/// `coin::send_funds<HANEUL>` that moves the *entire gas coin* into an address
...
/// `HaneulAddress::from_str`, producing `FastCryptoError::InvalidInput`
...
async fn test_send_gas_coin_to_address_balance_parses() {
    use haneul_rosetta::types::OperationType;
