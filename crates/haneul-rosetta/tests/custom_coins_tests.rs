...
use haneul_json_rpc_types::{
    ObjectChange, HaneulExecutionStatus, HaneulTransactionBlockEffectsAPI,
...
async fn test_custom_coin_balance() {
    ...
    const HANEUL_BALANCE: u64 = 150_000_000_000_000_000;
    const COIN1_BALANCE: u64 = 100_000_000;
    const COIN2_BALANCE: u64 = 200_000_000;
    let test_cluster = TestClusterBuilder::new().build().await;
    let client = test_cluster.wallet.get_client().await.unwrap();
    ...
    let haneul_currency = HANEUL.clone();
    ...
    let request = AccountBalanceRequest {
        ...
        currencies: Currencies(vec![haneul_currency, test_coin_currency]),
    };
