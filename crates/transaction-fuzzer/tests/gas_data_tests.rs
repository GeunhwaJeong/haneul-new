...
fn make_gas_data(sender: HaneulAddress, objects: &[Object], price: u64, budget: u64) -> GasData {
    GasData {
        payment: objects
            .iter()
...
struct AllModeResults {
    normal: Result<(), HaneulError>,
    dry_run: Result<(), HaneulError>,
    dev_inspect_skip: Result<(), HaneulError>,
    dev_inspect_no_skip: Result<(), HaneulError>,
}
...
fn run_all_modes(
    ...
    let kind = make_transfer_haneul_pt();
...
fn assert_err_contains(result: &Result<(), HaneulError>, expected_substring: &str, mode: &str) {
    let err = result
        .as_ref()
...
fn test_with_random_gas_data_dev_inspect(
    ...
    let gas_data = gas_data_test.gas_data;
    let objects = gas_data_test.objects;
    let sender: HaneulAddress = gas_data_test.sender_key.public().into();
...
fn test_gas_balance_insufficient() {
    ...
    // Gas coin has only 1 GEUNHWA -- far below budget
