...
/// (i.e., Narwhal, Haneul, Ethereum etc) should never collide, so that even when a signing
...
impl Intent {
    pub fn haneul_app(scope: IntentScope) -> Self {
        Self {
    ...
    pub fn haneul_transaction() -> Self {
        Self {
...
/// be any type that implements [trait Serialize]. *ALL* signatures in Haneul must commits
...
/// A 1-byte domain separator for hashing Object ID in Haneul. It is starting from 0xf0
/// to ensure no hashing collision for any ObjectID vs HaneulAddress which is derived
/// as the hash of `flag || pubkey`. See `haneul_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
