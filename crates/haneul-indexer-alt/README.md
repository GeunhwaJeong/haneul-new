...
```
postgres://postgres:postgrespw@localhost:5432/haneul_indexer_alt
```
...
diesel setup                                                                       \
    --database-url="postgres://postgres:postgrespw@localhost:5432/haneul_indexer_alt" \
    --migration-dir ../haneul-indexer-alt-schema/migrations
```
...
```sh
cargo nextest run                \
    -p haneul-indexer-alt           \
    -p haneul-indexer-alt-framework \
    -p haneul-indexer-alt-e2e-tests
```
...
```sh
cargo run --bin haneul-indexer-alt -- generate-config > indexer_alt_config.toml
```
