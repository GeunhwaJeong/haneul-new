...
mod tests {
    ...
    #[test]
    fn test_types() {
        let lexer = Lexer::new(r#"{0x2::table::Table<address, 0x2::coin::Coin<0x2::haneul::HANEUL>>}"#);
        let lexemes: Vec<_> = lexer.collect();
        assert_eq!(
            lexemes,
            vec![
    ...
    fn test_positional_struct_literals() {
        let lexer = Lexer::new(r#"{0x2::balance::Balance<0x2::haneul::HANEUL>(42u64)}"#);
        let lexemes: Vec<_> = lexer.collect();
        assert_eq!(
            lexemes,
            vec![
    ...
    fn test_struct_literals() {
        let lexer = Lexer::new(r#"{0x2::coin::Coin<0x2::haneul::HANEUL> { id: @0x123, value: 42u64 }}"#);
        let lexemes: Vec<_> = lexer.collect();
        assert_eq!(
            lexemes,
            vec![
