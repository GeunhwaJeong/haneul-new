...
// directly importing from haneul-inverted-index. The pipeline shape lives
// in this module; haneul-inverted-index just happens to define the carrier
// type the bitmap eval already produces.
pub(crate) use haneul_inverted_index::Watermarked;
...
/// the pattern in `haneul-kvstore`'s bigtable client (`AbortOnDrop`).
