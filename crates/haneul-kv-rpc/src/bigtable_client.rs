...
use haneul_inverted_index::BitmapQuery;
use haneul_inverted_index::BitmapScanError;
use haneul_inverted_index::BitmapScanResult;
...
pub(crate) struct BigTableClient {
    inner: haneul_kvstore::BigTableClient,
...
impl BigTableClient {
    ...
    /// `haneul_kvstore::BigTableClient` (which would expose the unlimited surface).
    pub(crate) fn column_filter(columns: &[&str]) -> RowFilter {
        haneul_kvstore::BigTableClient::column_filter(columns)
    }
    ...
    pub(crate) fn eval_bitmap_query_stream<F>(
        ...
        'static,
        haneul_inverted_index::BitmapScanResult<haneul_inverted_index::Watermarked<u64>>,
    >
    where
        F: FnOnce(haneul_inverted_index::BitmapScanMetrics) + Send + 'static,
    {
...
mod tests {
    ...
    /// passthrough to `haneul_kvstore::BigTableClient`.
