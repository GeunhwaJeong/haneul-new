...
use haneul_consistent_store::Schema as _;
use haneul_consistent_store::Watermark;
use haneul_consistent_store::restore::RestoreDriver;
use haneul_consistent_store::restore::RestoreDriverConfig;
use haneul_consistent_store::restore::RestoreSource;
...
///
/// [`Restore`]: haneul_consistent_store::Restore
/// [`RestoreState::Complete`]: haneul_consistent_store::restore_state::Complete
...
pub fn floor_unrestored_pipelines(
    ...
    // Requires the raw objects to read the on-chain `HaneulSystemState`,
...
/// `HaneulSystemState` in `objects`, keyed by that epoch.
///
...
/// epoch-start timestamp come from the `HaneulSystemState`, the BCS of
...
pub fn seed_current_epoch_start(
    ...
    let system_state =
        get_haneul_system_state(objects).context("read HaneulSystemState from restored objects")?;
    let epoch = system_state.epoch();
    let system_state_bcs = bcs::to_bytes(&system_state).context("bcs encode HaneulSystemState")?;
    batch
        .merge(
            &schema.epochs,
            &U64Be(epoch),
...
/// `epochs` row from its on-chain `HaneulSystemState` — a *partial*
...
/// (this crate stays free of any `haneul-core` dependency).
///
...
mod tests {
    ...
    use haneul_consistent_store::PipelineTaskKey;
    use haneul_consistent_store::Watermark;
    use haneul_consistent_store::restore::RestoreChunk;
