...
//!
//! Mirrors `index_transactions` in `haneul-core::rpc_index`: for
//! every transaction in the checkpoint, call
//! [`haneul_types::balance_change::derive_detailed_balance_changes_2`]
...
use haneul_consistent_store::Batch;
use haneul_consistent_store::Restore;
...
use haneul_types::balance_change::derive_detailed_balance_changes_2;
...
impl Restore for Balance {
    ...
    ///   `index_object`'s coin path in `haneul-core::rpc_index`.
    ///
    /// - **Address half**: dynamic-field objects parented to
    ///   [`HANEUL_ACCUMULATOR_ROOT_OBJECT_ID`]. These carry the
    ...
    fn restore(
        ...
        match object.owner() {
            ...
            Owner::ObjectOwner(parent) if *parent == HANEUL_ACCUMULATOR_ROOT_OBJECT_ID.into() => {
...
/// `get_address_balance_info` in `haneul-core::rpc_index`.
fn address_balance_info(object: &Object) -> Option<(HaneulAddress, TypeTag, i128)> {
...
impl sequential::Handler for Balance {
    ...
    async fn commit<'a>(
        &self,
        batch: &Self::Batch,
        conn: &mut haneul_consistent_store::Connection<'a, Schema>,
    ) -> anyhow::Result<usize> {
