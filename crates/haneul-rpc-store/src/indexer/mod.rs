...
//! Indexer pipelines that populate the `haneul-rpc-store` schema
...
//! `haneul-indexer-alt-framework` drives: `process` turns a checkpoint
...
//! writes against a [`haneul_consistent_store::Connection`] from
//! [`haneul_consistent_store::Store`].
//!
...
use haneul_consistent_store::Synchronizer;
...
use haneul_indexer_alt_framework::ingestion::IngestionService;
...
pub type Store = haneul_consistent_store::Store<Schema>;
...
/// `haneul-indexer-alt-consistent-store::handlers`.
///
...
pub struct Indexer {
    ...
    /// hands it to [`haneul_consistent_store::Store::install_sync`].
    sync: Synchronizer,
...
impl Indexer {
    ...
    /// against `registry`.
    ///
    /// [`IngestionClientTrait`]: haneul_indexer_alt_framework::ingestion::ingestion_client::IngestionClientTrait
    /// [`IngestionMetrics`]: haneul_indexer_alt_framework::metrics::IngestionMetrics
    ...
    pub async fn new(
        ...
        let (db, schema) = Db::open::<RpcStoreSchema>(path, db_options)
            .context("Failed to open haneul-rpc-store database")?;
        let store = haneul_consistent_store::Store::new(db, Arc::new(schema));
        Self::from_store(
            store,
            indexer_args,
            ingestion_client,
    ...
    /// toggle — only the standalone `haneul-rpc-node`, which hosts the
    ...
    ///
    /// [`RestoreState`]: haneul_consistent_store::RestoreState
...
mod tests {
    ...
    impl IngestionClientTrait for StubIngestionClient {
        async fn chain_id(&self) -> anyhow::Result<ChainIdentifier> {
            Ok(ChainIdentifier::from(
                haneul_types::digests::CheckpointDigest::new([0u8; 32]),
            ))
        }
    ...
    fn open_with_seeded_restore(
        path: &std::path::Path,
        pipeline: &str,
        state: haneul_consistent_store::RestoreState,
    ) -> Store {
        let (db, schema) = Db::open::<RpcStoreSchema>(path, DbOptions::default()).unwrap();
        let framework = haneul_consistent_store::FrameworkSchema::new(db.clone());
        let mut batch = db.batch();
        batch
            .put(&framework.restore, &PipelineTaskKey::new(pipeline), &state)
            .unwrap();
        batch.commit().unwrap();
        haneul_consistent_store::Store::new(db, Arc::new(schema))
    }
    ...
    async fn add_pipelines_refuses_pipeline_with_in_progress_restore() {
        let dir = tempfile::tempdir().unwrap();
        let in_progress = haneul_consistent_store::RestoreState {
    ...
    async fn add_pipelines_allows_pipeline_with_completed_restore() {
        let dir = tempfile::tempdir().unwrap();
        let complete = haneul_consistent_store::RestoreState {
