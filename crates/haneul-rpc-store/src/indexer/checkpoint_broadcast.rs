...
//! [`Synchronizer`](haneul_consistent_store::Synchronizer) set
...
//!
//! This is the standalone `haneul-rpc-node`'s analog of the fullnode's
//! checkpoint-executor broadcast: it lets the node host
//! `haneul-rpc-api`'s checkpoint-subscription service over the same
...
impl sequential::Handler for CheckpointBroadcast {
    ...
    async fn commit<'a>(
        &self,
        batch: &Self::Batch,
        _conn: &mut haneul_consistent_store::Connection<'a, Schema>,
    ) -> anyhow::Result<usize> {
...
mod tests {
    ...
    use haneul_indexer_alt_framework::pipeline::sequential::Handler as _;
