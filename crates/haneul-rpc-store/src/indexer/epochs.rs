...
//! [`haneul_types::storage::EpochInfo`] for the *new* epoch on
...
impl Processor for Epochs {
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Row>> {
        ...
        let system_state_bcs = epoch_info
            ...
            .map(bcs::to_bytes)
            .transpose()
            .map_err(|e| anyhow::anyhow!("bcs encode HaneulSystemState: {e}"))?;
