...
pub enum TypeFilter {
    ...
    /// instantiation (e.g. `Coin<HANEUL>`, `Coin<USDC>`); otherwise
...
mod tests {
    ...
    fn package_matches_any_tag_at_address() {
        let filter = TypeFilter::Package(HaneulAddress::from(addr(2)));
        for t in [
            tag(2, "haneul", "HANEUL", vec![]),
    ...
    fn module_matches_tags_in_named_module() {
        let filter = TypeFilter::Module {
            package: HaneulAddress::from(addr(2)),
            module: ident("coin"),
        };
        for t in [
            tag(2, "coin", "Coin", vec![TypeTag::U64]),
    ...
    fn type_with_empty_params_matches_every_instantiation() {
        ...
        for t in [
            ...
            tag(
                ...
                "coin",
                "Coin",
                vec![TypeTag::Struct(Box::new(tag(
                    2,
                    "haneul",
                    "HANEUL",
                    vec![],
