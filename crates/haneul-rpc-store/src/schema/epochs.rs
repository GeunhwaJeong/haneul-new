...
use haneul_consistent_store::Protobuf;
...
/// seeds the current epoch's row from a *mid-epoch* `HaneulSystemState`
...
impl<R: Reader> super::RpcStoreSchema<R> {
    ...
    /// BCS `HaneulSystemState` into the canonical
    /// [`haneul_types::storage::EpochInfo`].
    pub fn get_epoch(&self, epoch: EpochId) -> Result<Option<EpochInfo>, Error> {
        ...
        let system_state = stored
            ...
            .as_ref()
            .map(|bytes| {
                bcs::from_bytes::<HaneulSystemState>(bytes)
                    .map_err(|e| DecodeError::with_source("bcs decode HaneulSystemState", e))
            })
            .transpose()?;
        Ok(Some(EpochInfo {
            epoch,
    ...
    ///
    /// Derived from the stored `HaneulSystemState` rather than kept
    ...
    pub fn get_committee(&self, epoch: EpochId) -> Result<Option<Committee>, Error> {
        ...
        let system_state: HaneulSystemState = bcs::from_bytes(&bytes)
            .map_err(|e| DecodeError::with_source("bcs decode HaneulSystemState", e))?;
        Ok(Some(
            system_state
