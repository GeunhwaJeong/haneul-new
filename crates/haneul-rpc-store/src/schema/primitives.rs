...
//! Key types shared across multiple `haneul-rpc-store` CFs.
//!
...
mod tests {
    ...
    fn struct_tag_leaves_trailing_bytes_intact() {
        let tag = struct_tag(2, "haneul", "HANEUL", vec![]);
    ...
    fn struct_tag_no_type_params() {
        round_trip_struct(struct_tag(2, "haneul", "HANEUL", vec![]));
    }
    ...
    fn struct_tag_with_vector_of_struct_param() {
        // `Bag<vector<0x2::haneul::HANEUL>>`.
        round_trip_struct(struct_tag(
            2,
            "bag",
            "Bag",
            vec![TypeTag::Vector(Box::new(TypeTag::Struct(Box::new(
                struct_tag(2, "haneul", "HANEUL", vec![]),
            ))))],
        ));
    }
