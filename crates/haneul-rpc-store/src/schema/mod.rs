...
//! Column-family layout for `haneul-rpc-store`.
//!
...
//! [`haneul_consistent_store::Db::open`]. Keys reused across multiple
//! CFs live in [`keys`].
...
use haneul_consistent_store::CfDescriptor;
use haneul_consistent_store::CfOptionsResolver;
use haneul_consistent_store::CfTuning;
use haneul_consistent_store::Compression;
use haneul_consistent_store::Db;
use haneul_consistent_store::DbMap;
use haneul_consistent_store::DbWideConfig;
use haneul_consistent_store::RocksDbConfig;
use haneul_consistent_store::Schema;
use haneul_consistent_store::SchemaAtSnapshot;
use haneul_consistent_store::Snapshot;
use haneul_consistent_store::WriteStallConfig;
use haneul_consistent_store::error::OpenError;
use haneul_consistent_store::reader::Reader;

/// Typed handles to every CF in the `haneul-rpc-store` layout.
...
/// the `haneul-rpc-store` column families.
///
...
mod tests {
    ...
    fn default_config_sets_per_cf_deviations() {
        ...
        assert_eq!(
            cfg.column_family[live_objects::NAME].bloom_filter_bits,
            Some(10.0)
        );
