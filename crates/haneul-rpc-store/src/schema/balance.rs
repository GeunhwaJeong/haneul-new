...
impl Decode for Key {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let owner = HaneulAddress::from_bytes(owner_bytes)
...
pub fn coin_delta(owner: HaneulAddress, coin_type: TypeTag, delta: i128) -> (Key, Value) {
    (
        Key { owner, coin_type },
...
pub fn address_delta(owner: HaneulAddress, coin_type: TypeTag, delta: i128) -> (Key, Value) {
    (
        Key { owner, coin_type },
        Protobuf(BalanceDelta {
...
/// accumulator.
pub fn delta(owner: HaneulAddress, coin_type: TypeTag, coin: i128, address: i128) -> (Key, Value) {
    (
        Key { owner, coin_type },
        Protobuf(BalanceDelta {
...
impl Balance {
    ...
    /// Decode a stored [`BalanceDelta`] into the typed view. The
    /// single decode path shared by every reader so the two `i128`
    /// fields are interpreted — and malformed payloads rejected —
    /// identically whether the row is fetched by point lookup or
    /// surfaced through iteration.
    pub(crate) fn from_delta(stored: &BalanceDelta) -> Result<Self, Error> {
        Ok(Self {
...
/// `owner` matches.
pub struct OwnerPrefix(pub HaneulAddress);
...
impl<R: Reader> super::RpcStoreSchema<R> {
    ...
    pub fn get_balance(
        ...
        Ok(Some(Balance::from_delta(&stored.into_inner())?))
    }
...
mod tests {
    ...
    fn owner(b: u8) -> HaneulAddress {
        HaneulAddress::from_bytes([b; 32]).unwrap()
    }
    ...
    fn get_balance_returns_none_for_unknown_owner() {
        ...
        assert!(
            schema
                .get_balance(owner(1), coin_type("HANEUL"))
                .unwrap()
                .is_none()
        );
    }
    ...
    fn coin_and_address_deltas_accumulate_independently() {
        ...
        let (k1, v1) = coin_delta(owner(1), coin_type("HANEUL"), 100);
        let (k2, v2) = coin_delta(owner(1), coin_type("HANEUL"), 50);
        let (k3, v3) = address_delta(owner(1), coin_type("HANEUL"), 1_000);
        let (k4, v4) = address_delta(owner(1), coin_type("HANEUL"), -250);
    ...
    fn distinct_coin_types_do_not_alias() {
        let (_dir, db, schema) = fresh_db();
        let mut batch = db.batch();
        let (k_haneul, v_haneul) = coin_delta(owner(1), coin_type("HANEUL"), 100);
        let (k_usdc, v_usdc) = coin_delta(owner(1), coin_type("USDC"), 999);
        batch.merge(&schema.balance, &k_haneul, &v_haneul).unwrap();
    ...
    fn negative_then_positive_zero_out() {
        let (_dir, db, schema) = fresh_db();
        let (k1, v1) = coin_delta(owner(1), coin_type("HANEUL"), 100);
        let (k2, v2) = coin_delta(owner(1), coin_type("HANEUL"), -100);
        let mut batch = db.batch();
        batch.merge(&schema.balance, &k1, &v1).unwrap();
        batch.merge(&schema.balance, &k2, &v2).unwrap();
        batch.commit().unwrap();
    ...
    fn saturating_add_protects_against_i128_overflow() {
        let (_dir, db, schema) = fresh_db();
        let (k1, v1) = coin_delta(owner(1), coin_type("HANEUL"), i128::MAX);
        let (k2, v2) = coin_delta(owner(1), coin_type("HANEUL"), 1);
        let mut batch = db.batch();
        batch.merge(&schema.balance, &k1, &v1).unwrap();
        batch.merge(&schema.balance, &k2, &v2).unwrap();
        batch.commit().unwrap();
    ...
    fn iter_balances_walks_only_target_owner() {
        let (_dir, db, schema) = fresh_db();
        let target_types: BTreeSet<TypeTag> = [coin_type("HANEUL"), coin_type("USDC")]
            .into_iter()
            .collect();
        ...
        let (k_other, v_other) = coin_delta(owner(2), coin_type("HANEUL"), 500);
