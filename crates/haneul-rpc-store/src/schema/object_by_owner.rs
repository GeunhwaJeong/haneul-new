...
use bytes::Buf;
use bytes::BufMut;
use haneul_consistent_store::Decode;
use haneul_consistent_store::Encode;
use haneul_consistent_store::Iter;
use haneul_consistent_store::error::DecodeError;
use haneul_consistent_store::error::EncodeError;
use haneul_consistent_store::error::Error;
...
/// `HaneulAddress` inline; shared and immutable owners have no
/// owning address.
...
fn read_address<B: Buf>(buf: &mut B) -> Result<HaneulAddress, DecodeError> {
    if buf.remaining() < HANEUL_ADDRESS_LENGTH {
        return Err(DecodeError::msg(format!(
            "{NAME} missing owner address: {} bytes left",
            buf.remaining(),
        )));
    }
    let mut bytes = [0u8; HANEUL_ADDRESS_LENGTH];
    buf.copy_to_slice(&mut bytes);
    HaneulAddress::from_bytes(bytes)
        .map_err(|e| DecodeError::with_source("decode HaneulAddress", e))
}

pub fn options(resolver: &haneul_consistent_store::CfOptionsResolver) -> rocksdb::Options {
...
/// type to [`DbMap::iter_prefix`](haneul_consistent_store::DbMap::iter_prefix)
/// walks every row owned by the given address.
pub struct AddressOwnerPrefix(pub HaneulAddress);
...
/// a parent.
pub struct ObjectOwnerPrefix(pub HaneulAddress);
...
impl<R: Reader> super::RpcStoreSchema<R> {
    ...
    pub fn iter_objects_owned_by_address_of_type(
        &self,
        owner: HaneulAddress,
...
mod tests {
    ...
    fn haneul_tag() -> StructTag {
        StructTag {
            address: move_core_types::account_address::AccountAddress::new([2u8; 32]),
            module: move_core_types::identifier::Identifier::new("haneul").unwrap(),
            name: move_core_types::identifier::Identifier::new("HANEUL").unwrap(),
            type_params: vec![],
        }
    }
    ...
    fn address_owner_with_balance_round_trips() {
        round_trip(Key {
            kind: OwnerKind::AddressOwner(HaneulAddress::from_bytes([1u8; 32]).unwrap()),
            type_: haneul_tag(),
    ...
    fn object_owner_without_balance_round_trips() {
        round_trip(Key {
            kind: OwnerKind::ObjectOwner(HaneulAddress::from_bytes([3u8; 32]).unwrap()),
            type_: haneul_tag(),
    ...
    fn dummy_object(id: ObjectID, owner: HaneulAddress) -> Object {
        Object::with_id_owner_for_testing(id, owner)
    }

    #[test]
    fn store_derives_key_from_object() {
        let owner = HaneulAddress::from_bytes([5u8; 32]).unwrap();
    ...
    fn iter_returns_empty_for_owner_with_no_objects() {
        ...
        let owner = HaneulAddress::from_bytes([1u8; 32]).unwrap();
    ...
    fn iter_with_type_filter_narrows_to_matching_objects() {
        ...
        // (`0x2::coin::Coin<0x2::haneul::HANEUL>`). A `TypeFilter::Type`
    ...
    fn iter_finds_only_objects_for_target_owner() {
        let (_dir, db, schema) = fresh_db();
        let target = HaneulAddress::from_bytes([1u8; 32]).unwrap();
        let other = HaneulAddress::from_bytes([2u8; 32]).unwrap();
