...
//! Runtime configuration for the `haneul-rpc-store` indexer.
//!
...
/// Top-level configuration for the `haneul-rpc-store` indexer
...
///
/// [`Synchronizer`]: haneul_consistent_store::Synchronizer
/// [`DbOptions::snapshot_capacity`]: haneul_consistent_store::DbOptions::snapshot_capacity
#[DefaultConfig]
#[derive(Clone)]
#[serde(deny_unknown_fields)]
...
impl PipelineLayer {
    ...
    /// when it runs inside a Haneul fullnode beside the validator's
    /// perpetual store.
    ///
