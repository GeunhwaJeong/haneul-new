...
//!    epoch's [`HaneulSystemState`] (protocol version) plus the
...
//!    layout build.
//!
//! [`AuthorityState`]: https://docs.rs/haneul-core/latest/haneul_core/authority/struct.AuthorityState.html
//! [`Executor`]: haneul_execution::Executor
//! [`ProtocolConfig`]: haneul_protocol_config::ProtocolConfig
//! [`BackingPackageStore`]: haneul_types::storage::BackingPackageStore
//! [`RpcStateReader`]: haneul_types::storage::RpcStateReader
//! [`HaneulSystemState`]: haneul_types::haneul_system_state::HaneulSystemState

use haneul_consistent_store::FrameworkSchema;
...
impl<R: Reader + Send + Sync> RpcStoreReader<R> {
    ...
    /// version recorded in the latest epoch's [`HaneulSystemState`]
