...
//! [`haneul_types::storage::error::Error`] via `Error::custom`.
...
impl<R: Reader + Send + Sync> ReadStore for RpcStoreReader<R> {
    ...
    fn get_transaction(&self, tx_digest: &TransactionDigest) -> Option<Arc<VerifiedTransaction>> {
        ...
        let envelope =
            haneul_types::transaction::Transaction::from_generic_sig_data(transaction, signatures);
...
mod tests {
    ...
    fn dummy_summary(seq: u64) -> CheckpointSummary {
        CheckpointSummary {
            ...
            content_digest: haneul_types::digests::CheckpointContentsDigest::new([0; 32]),
    ...
    fn contents_by_seq_round_trips() {
        ...
        let contents =
            haneul_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests(
                vec![haneul_types::base_types::ExecutionDigests {
                    transaction: TransactionDigest::new([1; 32]),
                    effects: haneul_types::digests::TransactionEffectsDigest::new([2; 32]),
                }],
            );
        let mut batch = db.batch();
        batch
