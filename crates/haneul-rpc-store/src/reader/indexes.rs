...
//! wrap our [`haneul_consistent_store::error::Error`] via
...
/// `haneul_types::storage::read_store::PackageVersionsIterator`.
...
fn to_typed_store_err(e: haneul_consistent_store::error::Error) -> TypedStoreError {
...
fn first_object_of_type<R: Reader + Send + Sync>(
    ...
    match iter.next() {
        ...
        Some(Err(e)) => Err(haneul_types::storage::error::Error::custom(e)),
        None => Ok(None),
    }
}
...
impl<R: Reader + Send + Sync> RpcIndexes for RpcStoreReader<R> {
        ...
        let mapped = iter
            .map(move |row| {
                ...
                Ok(OwnedObjectInfo {
                    ...
                    version: haneul_types::base_types::SequenceNumber::from_u64(value.0),
                })
            })
    ...
    fn get_coin_info(&self, coin_type: &StructTag) -> StorageResult<Option<CoinInfo>> {
        ...
        let coin_metadata_object_id = first_object_of_type(
            self,
            haneul_types::coin::CoinMetadata::type_(coin_type.clone()),
        )?;
        let treasury_object_id = first_object_of_type(
            self,
            haneul_types::coin::TreasuryCap::type_(coin_type.clone()),
        )?;
        let regulated_coin_metadata_object_id = first_object_of_type(
            self,
            haneul_types::coin::RegulatedCoinMetadata::type_(coin_type.clone()),
        )?;
    ...
    fn balance_iter(
        ...
        let mapped = iter.filter_map(move |row| {
            let (key, value) = match row {
                Ok(pair) => pair,
                Err(e) => return Some(Err(haneul_types::storage::error::Error::custom(e))),
            };
    ...
    fn get_highest_indexed_checkpoint_seq_number(
        ...
        let framework = haneul_consistent_store::FrameworkSchema::new(self.db().clone());
        ...
        for entry in framework
            ...
            .map_err(haneul_types::storage::error::Error::custom)?
        {
            let (_, watermark) = entry.map_err(haneul_types::storage::error::Error::custom)?;
    ...
    fn ledger_tx_seq_digest_iter(
        ...
        let project = move |row: Result<
            ...
            haneul_consistent_store::error::Error,
        >| {
            ...
            Ok(LedgerTxSeqDigest {
                ...
                digest: haneul_types::digests::TransactionDigest::new(digest_bytes),
...
fn project_bitmap_row(
    row: Result<
        ...
        haneul_consistent_store::error::Error,
    >,
) -> Result<haneul_types::storage::LedgerBitmapBucket, TypedStoreError> {
    let (key, value) = row.map_err(to_typed_store_err)?;
    ...
    Ok(haneul_types::storage::LedgerBitmapBucket {
...
mod tests {
    ...
    fn get_coin_info_finds_metadata_and_treasury_objects() {
        ...
        let metadata_type = haneul_types::coin::CoinMetadata::type_(coin_type.clone());
        let treasury_type = haneul_types::coin::TreasuryCap::type_(coin_type.clone());
