This crate contains a Command Line Interface light client for Haneul.
...
Alternatively, a *light client* only needs to download minimal information to authenticate blockchain state. Specifically in Haneul, the light client needs to *sync* all end-of-epoch checkpoints that contain information about the committee in the next epoch. Sync involves downloading the checkpoints and checking their validity by checking their certificate. 
...
The config file for the light client takes a URL for a full node, a directory (that must exist) and within the directory to name of the genesis blob for the Haneul network. 

```
full_node_url: "http://ord-mnt-rpcbig-06.mainnet.haneul.io:9000"
checkpoint_summary_dir: "checkpoints_dir"
genesis_filename: "genesis.blob"
```

The genesis blob for the Haneul mainnet can be found here: https://github.com/GeunhwaJeong/haneul-genesis/blob/main/mainnet/genesis.blob

## Sync 

Every day there is a need to download new checkpoints through sync by doing:
```
$ haneul-light-client --config light_client.yaml sync
```
...
```
$ haneul-light-client --config light_client.yaml transaction -t 8RiKBwuAbtu8zNCtz8SrcfHyEUzto6zi6cMVA9t4WhWk
```
...
```
$ haneul-light-client --config light_client.yaml object -o 0xc646887891adfc0540ec271fd0203603fb4c841a119ec1e00c469441
abfc7078
```
