...
use haneul_rest_api::{CheckpointData, Client};
use haneul_types::{
...
use haneul_package_resolver::Result as ResolverResult;
...
/// A light client for the Haneul blockchain
#[derive(Parser, Debug)]
...
async fn get_verified_effects_and_events(
    ...
    let haneul_mainnet: Arc<haneul_sdk::HaneulClient> = Arc::new(
    ...
    match args.command {
        Some(SCommands::Transaction { tid }) => {
            ...
            for event in events.as_ref().unwrap().data.iter() {
                ...
                let json_val =
                    HaneulJsonValue::from_bcs_bytes(Some(&type_layout), &event.contents).unwrap();
        ...
        Some(SCommands::Object { oid }) => {
            ...
            if let Data::Move(move_object) = &object.data {
                ...
                let json_val =
                    HaneulJsonValue::from_bcs_bytes(Some(&type_layout), move_object.contents())
                        .unwrap();
