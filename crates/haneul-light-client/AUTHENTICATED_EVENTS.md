...
1. **Cryptographic Commitments**: Events are committed into a Merkle Mountain Range (MMR) structure stored on-chain (see [MMR section](#merkle-mountain-range-mmr) below)
...
Authenticated events are fully backwards-compatible with regular Haneul events. An authenticated event is simply a regular event with additional metadata for verification — existing event consumers will continue to work unchanged.
