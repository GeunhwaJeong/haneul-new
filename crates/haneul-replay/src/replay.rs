...
use haneul_types::storage::{get_module, ConfigStore, PackageObject};
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: HaneulTransactionBlockEffects,
...
impl ExecutionSandboxState {
    ...
    pub fn check_effects(&self) -> Result<(), ReplayEngineError> {
        let HaneulTransactionBlockEffects::V1(mut local_effects) = self.local_exec_effects.clone();
        let HaneulTransactionBlockEffects::V1(on_chain_effects) =
        ...
        let local_effects = HaneulTransactionBlockEffects::V1(local_effects);
        let on_chain_effects = HaneulTransactionBlockEffects::V1(on_chain_effects);
    ...
    pub fn diff_effects(
        on_chain_effects: &HaneulTransactionBlockEffects,
        local_effects: &HaneulTransactionBlockEffects,
    ) -> String {
...
pub struct LocalExec {
    pub client: Option<HaneulClient>,
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::HaneulRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::HaneulRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::HaneulRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            HaneulTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `haneul_adapter::execution_engine::execute_transaction_to_effects`
    #[allow(clippy::result_large_err)]
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &HANEUL_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&HANEUL_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((HANEUL_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let HaneulTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            haneul_json_rpc_types::HaneulTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: HaneulTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = HaneulTransactionBlockEffects::try_from(effects).unwrap();
...
impl BackingPackageStore for LocalExec {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> HaneulResult<Option<PackageObject>> {
        fn inner(self_: &LocalExec, package_id: &ObjectID) -> HaneulResult<Option<Object>> {
            // If package not present fetch it from the network
            self_
                .get_or_download_object(package_id, true /* we expect a Move package*/)
                .map_err(|e| HaneulError::Storage(e.to_string()))
        }
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(HaneulError::Unknown(format!(
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> HaneulResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> HaneulResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> HaneulResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    haneul_execution::executor(&protocol_config, silent, enable_profiler)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &HaneulExecutionStatus) -> Option<String> {
    let HaneulExecutionStatus::Failure { error } = status else {
        return None;
    };
