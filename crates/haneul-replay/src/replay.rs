...
impl LocalExec {
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            HaneulTransactionBlockEffects::try_from(res.1).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let effects =
            HaneulTransactionBlockEffects::try_from(res.0).map_err(ReplayEngineError::from)?;
...
impl ObjectStore for LocalExec {
    ...
    fn get_object(&self, object_id: &ObjectID) -> HaneulResult<Option<Object>> {
...
pub fn get_executor(
    ...
    let protocol_config = executor_version_override
        .map(|q| {
            let ver = if q < 0 {
                haneul_execution::LATEST_EXECUTOR_VERSION
            } else {
                q as u64
            };
