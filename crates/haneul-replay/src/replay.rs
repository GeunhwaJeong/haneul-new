...
use haneul_json_rpc_types::{EventFilter, HaneulEvent};
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(LocalExecError::HaneulRpcRequestTimeout) => {
                    warn!(
    ...
    pub async fn new(client: HaneulClient) -> Result<Self, LocalExecError> {
        ...
        for checkpoint_id in checkpoint_ids {
            txs.extend(
                self.fetcher
                    ...
                    .await
                    .map_err(|e| LocalExecError::HaneulRpcError { err: e.to_string() })?,
            );
        }
        let num = txs.len();
        let mut succeeded = 0;
    ...
    pub async fn execute_impl(
        ...
        let effects = HaneulTransactionBlockEffects::try_from(res.1).map_err(LocalExecError::from)?;
    ...
    pub async fn certificate_execute(
        ...
        let effects = HaneulTransactionBlockEffects::try_from(res.0).map_err(LocalExecError::from)?;
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `haneul_adapter::execution_engine::execute_transaction_to_effects_impl`
