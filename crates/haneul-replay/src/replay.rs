...
use haneul_adapter::execution_engine::execute_transaction_to_effects_impl;
...
const EPOCH_CHANGE_STRUCT_TAG: &str = "0x3::haneul_system_state_inner::SystemEpochInfoEvent";
...
struct TxInfo {
    ...
    pub shared_object_refs: Vec<HaneulObjectRef>,
...
impl LocalExec {
    pub async fn new_from_fn_url(http_url: &str) -> Result<Self, LocalExecError> {
        Ok(Self::new(
            HaneulClientBuilder::default().build(http_url).await?,
        ))
    }
    ...
    pub async fn multi_download_and_store_haneul_obj_ref(
        &mut self,
        refs: &[HaneulObjectRef],
    ...
    pub async fn multi_download(
        ...
        let objs: Vec<_> = objs
            .iter()
            .map(|(object_id, version)| HaneulGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
        let options = HaneulObjectDataOptions::bcs_lossless();
        let objects = self
            .client
            .read_api()
    ...
    pub async fn multi_download_latest(
        ...
        let objects: Vec<_> = objects
            .iter()
            .map(obj_from_haneul_obj_response)
            .collect::<Result<Vec<_>, _>>()?;
        Ok(objects)
    }
    ...
    pub async fn download_latest_object_impl(
        ...
        .await.map(|q| match obj_from_haneul_obj_response(&q){
            Ok(v) => Ok(Some(v)),
            ...
            Err(err) => Err(LocalExecError::HaneulRpcError {
                err: err.to_string(),
            })
        })?
    }
    ...
    pub async fn execute(
        ...
        self.multi_download_and_store_haneul_obj_ref(shared_obj_refs)
            .await?;
        ...
        let new_effects: HaneulTransactionBlockEffects = res.1.try_into().unwrap();
        let HaneulTransactionBlockEffects::V1(new_effects) = new_effects;
    ...
    pub async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<impl Iterator<Item = HaneulEvent>, LocalExecError> {
    ...
    pub async fn get_epoch_start_timestamp(&self, epoch_id: u64) -> Result<u64, LocalExecError> {
        ...
        let tx_fetch_opts = HaneulTransactionBlockResponseOptions::full_content();
        let tx_info = self
            .client
            .read_api()
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> HaneulResult<Option<Object>> {
        fn inner(self_: &LocalExec, package_id: &ObjectID) -> HaneulResult<Option<Object>> {
            ...
            self_
                ...
                .map_err(|e| HaneulError::GenericStorageError(e.to_string()))
        }
...
impl ParentSync for LocalExec {
    fn get_latest_parent_entry_ref(&self, object_id: ObjectID) -> HaneulResult<Option<ObjectRef>> {
        fn inner(self_: &LocalExec, object_id: ObjectID) -> HaneulResult<Option<ObjectRef>> {
...
fn obj_from_haneul_obj_response(o: &HaneulObjectResponse) -> Result<Object, LocalExecError> {
    let o = o.object().map_err(LocalExecError::from)?.clone();
    obj_from_haneul_obj_data(&o)
}

fn obj_from_haneul_obj_data(o: &HaneulObjectData) -> Result<Object, LocalExecError> {
...
pub fn get_vm(
    ...
    let native_functions = haneul_framework::natives::all_natives(/* disable silent */ false);
    let move_vm = Arc::new(
        adapter::new_move_vm(
...
fn extract_epoch_and_version(ev: HaneulEvent) -> Result<(u64, u64), LocalExecError> {
...
fn diff_effects(
    eff1: &HaneulTransactionBlockEffectsV1,
    eff2: &HaneulTransactionBlockEffectsV1,
) -> String {
