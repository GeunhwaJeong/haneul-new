...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<HaneulTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<HaneulEvent>, ReplayEngineError>;
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::HaneulRpcError { err: q.to_string() })?;
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<HaneulEvent> = vec![];
...
fn convert_past_obj_response(resp: HaneulPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        ...
        HaneulPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
