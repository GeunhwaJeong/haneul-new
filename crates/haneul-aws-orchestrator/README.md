...
```json
{
  ...
  "specs": "m5d.8xlarge",
  "repository": {
    "url": "https://github.com/GeunhwaJeong/haneul.git",
    "commit": "main"
  },
...
```json
"repository": {
  "url": "http://[your_token]@github.com/GeunhwaJeong/haneul.git",
  "commit": "main"
}
```
...
The `haneul-aws-orchestrator` binary provides various functionalities for creating, starting, stopping, and destroying instances. You can use the following command to boot 2 instances per region (if the settings file specifies 10 regions, as shown in the example above, a total of 20 instances will be created):

```bash
cargo run --bin haneul-aws-orchestrator -- testbed deploy --instances 2
```

To check the current status of the testbed instances, use the following command:

```bash
cargo run --bin haneul-aws-orchestrator testbed status
```
...
There is support to benchmark either Haneul or Narwhal only. To choose which protocol to benchmark, you can set the `Protocol` & `BenchmarkType` field [here](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-aws-orchestrator/src/main.rs#L33-L34)
...
```bash
cargo run --bin haneul-aws-orchestrator -- benchmark --committee 10 fixed-load --loads 200 --duration 180
```

In a network of 10 validators, each with a corresponding load generator, each load generator submits a fixed load of 20 tx/s. Performance measurements are collected by regularly scraping the Prometheus metrics exposed by the load generators. The `haneul-aws-orchestrator` binary provides additional commands to run a specific number of load generators on separate machines.
...
```
cargo run --bin haneul-aws-orchestrator -- testbed destroy
```
...
### I am getting an error "Failed to read settings file '"crates/haneul-aws-orchestrator/assets/settings.json"': No such file or directory"
To run the tool a `settings.json` file with the deployment configuration should be under the directory `crates/haneul-aws-orchestrator/assets`. Also, please make sure
that you run the orchestrator from the top level repo folder, ex `/haneul $ cargo run --bin haneul-aws-orchestrator`
...
for example when running the command `cargo run --bin haneul-aws-orchestrator -- benchmark --committee 4 fixed-load --loads 500 --duration 500`, we'll need the following amount of instances available:
