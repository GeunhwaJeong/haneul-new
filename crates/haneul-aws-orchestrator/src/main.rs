...
use protocol::haneul::{HaneulBenchmarkType, HaneulProtocol};
...
type Protocol = HaneulProtocol;
type BenchmarkType = HaneulBenchmarkType;
...
pub struct Opts {
    ...
    #[clap(
        long,
        value_name = "FILE",
        default_value = "crates/haneul-aws-orchestrator/assets/settings.json",
        global = true
    )]
...
async fn run<C: ServerProviderClient>(settings: Settings, client: C, opts: Opts) -> Result<()> {
    ...
    match opts.operation {
        ...
        Operation::Benchmark {
            ...
            let haneul_benchmark_type = BenchmarkType::from_str(&benchmark_type)?;
            ...
            let generator = BenchmarkParametersGenerator::new(committee, load)
                .with_benchmark_type(haneul_benchmark_type)
