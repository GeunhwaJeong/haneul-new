...
use haneul_types::{effects::TransactionEffects, gas::HaneulGasStatus};
use tracing::debug;
...
async fn main() {
    ...
    let replay_txn = ReplayTransaction::load(&mut env, &tx_digest)
        .await
        .unwrap_or_else(|e: haneul_replay_2::errors::ReplayError| {
...
fn print_txn_effects(effects: &TransactionEffects, gas_status: &HaneulGasStatus) {
