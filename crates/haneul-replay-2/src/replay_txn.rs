...
fn get_effects_ids(effects: &TransactionEffects) -> Result<BTreeSet<InputObject>, ReplayError> {
    ...
    effects
        ...
        .for_each(|(obj_id, kind)| match kind {
            haneul_types::effects::UnchangedSharedKind::ReadOnlyRoot((ver, _digest)) => {
                object_ids.insert(InputObject {
                    object_id: *obj_id,
                    version: Some(ver.value()),
                });
            }
            haneul_types::effects::UnchangedSharedKind::MutateConsensusStreamEnded(_)
            | haneul_types::effects::UnchangedSharedKind::ReadConsensusStreamEnded(_)
            | haneul_types::effects::UnchangedSharedKind::Cancelled(_)
            | haneul_types::effects::UnchangedSharedKind::PerEpochConfig => {
