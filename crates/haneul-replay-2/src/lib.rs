...
use haneul_types::{effects::TransactionEffects, supported_protocol_versions::Chain};
...
const MAINNET_RPC_URL: &str = "https://fullnode.mainnet.haneul.io:443";
const TESTNET_RPC_URL: &str = "https://fullnode.testnet.haneul.io:443";
...
/// Load replay configuration from ~/.haneul/haneul_config/replay.toml file.
...
pub fn load_config_file() -> Result<ReplayConfigStable> {
    let config_dir = match haneul_config_dir() {
        Ok(dir) => dir,
        Err(e) => {
...
pub fn print_effects_or_fork<W: Write>(
    ...
    if manager.member(Artifact::ForkedTransactionEffects).exists() {
        ...
        writeln!(
            w,
            "{}",
            HaneulTransactionBlockEffects::try_from(tx_effects.clone())
