...
use haneul_move_build::{implicit_deps, set_haneul_flavor, BuildConfig};
...
pub fn handle_build_command(config: BuildCmdConfig) -> anyhow::Result<()> {
    ...
    if let Some(err_msg) = set_haneul_flavor(&mut move_build_config) {
        anyhow::bail!(err_msg);
    }
    ...
    let cid = match chain_id {
        ...
        None => {
            ...
            let Some(managed_packages) = managed_packages_opt else {
                anyhow::bail!("No chain ID found in lock file (specify one explicitly - you can obtain it from 'haneul client chain-identifier')");
            };
