...
```bash
cargo install --git https://github.com/GeunhwaJeong/haneul haneul-replay-2
```
...
The actual trace debugging of a given transaction is supported by the Move Trace Debugger VSCode [extension](https://marketplace.visualstudio.com/items?itemName=haneullabs.move-trace-debug) available in the VSCode Marketplace. This extension is also installed automatically when the "main" Move [extension](https://marketplace.visualstudio.com/items?itemName=haneullabs.move) is installed. Once the extension is installed, you can trace-debug a transaction by opening its trace file in VSCode and starting a "conventional" debugging session.


### Code Organization
A replay tool is an invocation to [`execute_transaction_to_effects`](http://github.com/GeunhwaJeong/haneul/blob/main/haneul-execution/src/executor.rs#L26-L53) which contains info related to the transaction and info a node obtained while being live (running). For instance, a validator does not have a store for epochs, it lives/operates in an epoch. <br>
