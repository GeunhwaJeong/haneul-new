...
use crate::base_types::{HaneulAddress, TransactionDigest};
...
pub enum EventQuery {
    ...
    MoveEvent(
        /// the event struct name type, e.g. `0x2::devnet_nft::MintNFTEvent` or `0x2::HANEUL::test_foo<address, vector<u8>>` with type params
        String,
    ),
    EventType(EventType),
    /// Query by sender address.
