...
use crate::gas_model::gas_v2::HaneulCostTable;
...
pub enum CallArg {
    ...
    // `haneul::funds_accumulator::Withdrawal` struct and passed into Move.
...
impl FundsWithdrawalArg {
    ...
    pub fn owner_for_withdrawal(&self, tx: &impl TransactionDataAPI) -> HaneulAddress {
...
impl ProgrammableMoveCall {
    ...
    fn validate_gasless_transaction(
        ...
        const HANEUL_BALANCE_SEND_FUNDS: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            BALANCE_MODULE_NAME,
            BALANCE_SEND_FUNDS_FUNCTION_NAME,
        );
        const HANEUL_BALANCE_REDEEM_FUNDS: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            BALANCE_MODULE_NAME,
            BALANCE_REDEEM_FUNDS_FUNCTION_NAME,
        );
        const HANEUL_BALANCE_SPLIT: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            BALANCE_MODULE_NAME,
            BALANCE_SPLIT_FUNCTION_NAME,
        );
        const HANEUL_BALANCE_ZERO: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            BALANCE_MODULE_NAME,
            BALANCE_ZERO_FUNCTION_NAME,
        );
        const HANEUL_FUNDS_ACCUMULATOR_WITHDRAWAL_SPLIT: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            FUNDS_ACCUMULATOR_MODULE_NAME,
            WITHDRAWAL_SPLIT_FUNC_NAME,
        );
        const HANEUL_COIN_INTO_BALANCE: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            COIN_MODULE_NAME,
            INTO_BALANCE_FUNC_NAME,
        );
        const HANEUL_COIN_REDEEM_FUNDS: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            COIN_MODULE_NAME,
            REDEEM_FUNDS_FUNC_NAME,
        );
        const HANEUL_COIN_SEND_FUNDS: FunctionIdent = (
            HANEUL_FRAMEWORK_ADDRESS,
            COIN_MODULE_NAME,
            SEND_FUNDS_FUNC_NAME,
        );
        const HANEUL_COIN_PUT: FunctionIdent =
            (HANEUL_FRAMEWORK_ADDRESS, COIN_MODULE_NAME, PUT_FUNC_NAME);

        const GASLESS_FUNCTIONS: &[(FunctionIdent, &[Option<TypeArgConstraint>])] = &[
            (HANEUL_BALANCE_SEND_FUNDS, &[Some(FundType)]),
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(
                obj,
                CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == HANEUL_RANDOMNESS_STATE_OBJECT_ID
            )
        }) {
            fp_ensure!(
...
impl TransactionData {
    ...
    fn new_system_transaction(kind: TransactionKind) -> Self {
        ...
        let sender = HaneulAddress::default();
    ...
    pub fn new_transfer_haneul(
        ...
        Self::new_transfer_haneul_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_haneul_allow_sponsor(
        recipient: HaneulAddress,
        sender: HaneulAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_haneul(recipient, amount);
            builder.finish()
        };
    ...
    pub fn execution_parts(&self) -> (TransactionKind, HaneulAddress, GasData) {
    ...
    pub fn uses_randomness(&self) -> bool {
        self.kind()
            ...
            .any(|obj| obj.id() == HANEUL_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn required_signers(&self) -> NonEmpty<HaneulAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> HaneulAddress;
...
impl TransactionDataAPI for TransactionDataV1 {
    ...
    fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> HaneulResult {
        ...
        match self.expiration() {
            ...
            TransactionExpiration::Epoch(max_epoch) => {
                if context.epoch > *max_epoch {
                    return Err(HaneulErrorKind::TransactionExpired.into());
                }
            }
            TransactionExpiration::ValidDuring {
                ...
                // by haneul_transaction_checks::check_replay_protection.
        ...
        if !config.enable_coin_reservation_obj_refs() {
            ...
            // When coin reservations are enabled, validate that gas coin reservations are for HANEUL,
            // and that they are owned by the sender. (Sponsorship via coin reservations is not supported.)
            let haneul_accumulator_id =
            ...
            for gas_ref in self.gas() {
                    ...
                    ParsedObjectRefWithdrawal::parse(gas_ref, context.chain_identifier)
                {
                    ...
                    fp_ensure!(
                        parsed.unmasked_object_id == haneul_accumulator_id,
                        UserInputError::GasObjectNotOwnedObject {
                            owner: Owner::AddressOwner(self.sender)
                        }
                        .into()
        ...
        if !self.is_system_tx() {
            ...
            let cost_table = HaneulCostTable::new(config, self.gas_data.price);
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::haneul_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::haneul_transaction() {
...
impl SenderSignedTransaction {
    ...
    pub(crate) fn get_signer_sig_mapping(
    ...
    ) -> HaneulResult<BTreeMap<HaneulAddress, (u8, &GenericSignature)>> {
        ...
        for (idx, sig) in self.tx_signatures.iter().enumerate() {
            if verify_legacy_zklogin_address && let GenericSignature::ZkLoginAuthenticator(z) = sig
            {
                ...
                mapping.insert(HaneulAddress::try_from_padded(&z.inputs)?, (idx as u8, sig));
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::haneul_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn full_message_digest_with_alias_versions(
        &self,
        alias_versions: &Vec<(HaneulAddress, Option<SequenceNumber>)>,
    ...
    pub fn serialized_size(&self) -> HaneulResult<usize> {
    ...
    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> HaneulResult {
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> HaneulAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> HaneulAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::haneul_transaction(), &data);
            signers
                .into_iter()
...
/// Deprecated version of WithAliases that uses HaneulAddress instead of u8.
...
pub struct DeprecatedWithAliases<T>(
    T,
    #[serde(with = "nonempty_as_vec")] NonEmpty<(HaneulAddress, Option<SequenceNumber>)>,
);

impl<T> DeprecatedWithAliases<T> {
    pub fn into_inner(self) -> (T, NonEmpty<(HaneulAddress, Option<SequenceNumber>)>) {
        (self.0, self.1)
    }
}
...
impl<T> TransactionWithClaims<T> {
    ...
    #[allow(deprecated)]
    pub fn aliases_v1(&self) -> Option<NonEmpty<(HaneulAddress, Option<SequenceNumber>)>> {
        self.claims
            .iter()
            .find_map(|c| match c {
...
// An InputObjects new-type that has been verified by haneul-transaction-checks, and can be
// safely passed to execution.
#[derive(Clone)]
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of haneul-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in haneul-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by haneul-transaction-checks.
