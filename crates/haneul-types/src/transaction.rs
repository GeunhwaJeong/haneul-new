...
impl SenderSignedTransaction {
    ...
    ) -> HaneulResult<BTreeMap<HaneulAddress, &GenericSignature>> {
...
impl<T> WithAliases<T> {
    pub fn new(tx: T, aliases: NonEmpty<(HaneulAddress, Option<SequenceNumber>)>) -> Self {
        Self(tx, aliases)
    }
    ...
    pub fn aliases(&self) -> &NonEmpty<(HaneulAddress, Option<SequenceNumber>)> {
        &self.1
    }
    ...
    pub fn into_aliases(self) -> NonEmpty<(HaneulAddress, Option<SequenceNumber>)> {
        self.1
    }
...
impl<T> TransactionWithClaims<T> {
    ...
    pub fn from_aliases(tx: T, aliases: NonEmpty<(HaneulAddress, Option<SequenceNumber>)>) -> Self {
        Self {
            tx,
    ...
    pub fn aliases(&self) -> Option<NonEmpty<(HaneulAddress, Option<SequenceNumber>)>> {
        self.claims
            .iter()
            .find_map(|c| match c {
