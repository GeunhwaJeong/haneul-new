...
use super::{base_types::*, error::*, HANEUL_BRIDGE_OBJECT_ID};
...
use crate::crypto::{
    ...
    AuthorityStrongQuorumSignInfo, DefaultHash, Ed25519HaneulSignature, EmptySignInfo,
    RandomnessRound, Signature, Signer, HaneulSignatureInner, ToFromBytes,
};
...
impl VersionedProtocolMessage for TransactionKind {
    fn check_version_and_features_supported(&self, protocol_config: &ProtocolConfig) -> HaneulResult {
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(obj, CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == HANEUL_RANDOMNESS_STATE_OBJECT_ID)
        }) {
...
impl VersionedProtocolMessage for TransactionData {
    ...
    fn check_version_and_features_supported(&self, protocol_config: &ProtocolConfig) -> HaneulResult {
        ...
        if !supported.is_version_supported(protocol_config.version) {
            return Err(HaneulError::WrongMessageVersion {
                error: format!(
...
impl TransactionData {
    ...
    pub fn execution_parts(&self) -> (TransactionKind, HaneulAddress, Vec<ObjectRef>) {
        (
            self.kind().clone(),
            self.sender(),
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<HaneulAddress>;
...
impl SenderSignedData {
    ...
    pub fn serialized_size(&self) -> HaneulResult<usize> {
        bcs::serialized_size(self).map_err(|e| HaneulError::TransactionSerializationError {
            error: e.to_string(),
        })
    }
    ...
    pub fn validity_check(&self, config: &ProtocolConfig, epoch: EpochId) -> HaneulResult {
...
impl VersionedProtocolMessage for SenderSignedData {
    ...
    fn check_version_and_features_supported(&self, protocol_config: &ProtocolConfig) -> HaneulResult {
        ...
        // HaneulError::WrongMessageVersion
