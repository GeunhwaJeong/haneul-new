...
// Creates a proof that the keypair is possesed, as well as binds this proof to a specific HaneulAddress.
...
impl HaneulKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            HaneulKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().clone()),
            HaneulKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().clone()),
            HaneulKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().clone()),
        }
    }
}
...
impl Signer<Signature> for HaneulKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            HaneulKeyPair::Ed25519(kp) => kp.sign(msg),
            HaneulKeyPair::Secp256k1(kp) => kp.sign(msg),
            HaneulKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}
...
impl EncodeDecodeBase64 for HaneulKeyPair {
    ...
    fn decode_base64(value: &str) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(HaneulKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(HaneulKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(HaneulKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
...
impl<'de> Deserialize<'de> for HaneulKeyPair {
...
impl EncodeDecodeBase64 for PublicKey {
    ...
    fn decode_base64(value: &str) -> Result<Self, eyre::Report> {
        ...
        match bytes.first() {
            Some(x) => {
                if x == &<Ed25519PublicKey as HaneulPublicKey>::SIGNATURE_SCHEME.flag() {
                    let pk = Ed25519PublicKey::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?;
                    Ok(PublicKey::Ed25519(pk))
                } else if x == &<Secp256k1PublicKey as HaneulPublicKey>::SIGNATURE_SCHEME.flag() {
                    let pk = Secp256k1PublicKey::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?;
                    Ok(PublicKey::Secp256k1(pk))
                } else if x == &<Secp256r1PublicKey as HaneulPublicKey>::SIGNATURE_SCHEME.flag() {
...
impl PublicKey {
    pub fn flag(&self) -> u8 {
        match self {
            PublicKey::Ed25519(_) => Ed25519HaneulSignature::SCHEME.flag(),
            PublicKey::Secp256k1(_) => Secp256k1HaneulSignature::SCHEME.flag(),
            PublicKey::Secp256r1(_) => Secp256r1HaneulSignature::SCHEME.flag(),
        }
    }
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519(_) => Ed25519HaneulSignature::SCHEME,
            PublicKey::Secp256k1(_) => Secp256k1HaneulSignature::SCHEME,
            PublicKey::Secp256r1(_) => Secp256r1HaneulSignature::SCHEME,
        }
    }
}
...
where {
    ...
    // see [#34](https://github.com/GeunhwaJeong/narwhal/issues/34)
    #[allow(dead_code)]
    pub fn default() -> Self {
...
pub trait HaneulAuthoritySignature {
...
where
    <KP as KeypairTraits>::PubKey: HaneulPublicKey,
{
    let mut items = num;
    let mut rng = OsRng;
...
pub fn get_key_pair<KP: KeypairTraits>() -> (HaneulAddress, KP)
where
    <KP as KeypairTraits>::PubKey: HaneulPublicKey,
{
...
pub fn random_committee_key_pairs() -> Vec<AuthorityKeyPair> {
    ...
    (0..TEST_COMMITTEE_SIZE)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (haneul-config/src/builder.rs). This is because
...
pub fn deterministic_random_account_key() -> (HaneulAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (HaneulAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (HaneulAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (HaneulAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> HaneulResult<(HaneulAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: HaneulPublicKey,
{
...
impl Signature {
    ...
    pub fn to_compressed(&self) -> Result<CompressedSignature, HaneulError> {
        let bytes = self.signature_bytes();
        match self.scheme() {
            SignatureScheme::ED25519 => Ok(CompressedSignature::Ed25519(
                Ed25519Signature::from_bytes(bytes).map_err(|_| HaneulError::InvalidSignature {
                    error: "Cannot parse sig".to_string(),
                })?,
            )),
            SignatureScheme::Secp256k1 => Ok(CompressedSignature::Secp256k1(
                Secp256k1Signature::from_bytes(bytes).map_err(|_| HaneulError::InvalidSignature {
                    error: "Cannot parse sig".to_string(),
                })?,
            )),
            SignatureScheme::Secp256r1 => Ok(CompressedSignature::Secp256r1(
                Secp256r1Signature::from_bytes(bytes).map_err(|_| HaneulError::InvalidSignature {
                    error: "Cannot parse sig".to_string(),
                })?,
            )),
    ...
    /// Parse [struct PublicKey] from trait HaneulSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to construct the bitmap in [struct MultiPublicKey].
    pub fn to_public_key(&self) -> Result<PublicKey, HaneulError> {
        let bytes = self.public_key_bytes();
        match self.scheme() {
            SignatureScheme::ED25519 => Ok(PublicKey::Ed25519(
                Ed25519PublicKey::from_bytes(bytes)
                    .map_err(|_| HaneulError::KeyConversionError("Cannot parse pk".to_string()))?,
            )),
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            ...
            Signature::Secp256r1HaneulSignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519HaneulSignature(sig) => sig.as_mut(),
            Signature::Secp256k1HaneulSignature(sig) => sig.as_mut(),
            Signature::Secp256r1HaneulSignature(sig) => sig.as_mut(),
        }
    }
}
...
impl HaneulPublicKey for BLS12381PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::BLS12381;
}

//
// Ed25519 Haneul Signature port
//

#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash)]
pub struct Ed25519HaneulSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
...
impl AsMut<[u8]> for Ed25519HaneulSignature {
    fn as_mut(&mut self) -> &mut [u8] {
        self.0.as_mut()
    }
}

impl ToFromBytes for Ed25519HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Ed25519HaneulSignature::new(self, msg).into()
    }
}

//
// Secp256k1 Haneul Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash)]
pub struct Secp256k1HaneulSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256k1PublicKey::LENGTH + Secp256k1Signature::LENGTH + 1],
);

impl HaneulSignatureInner for Secp256k1HaneulSignature {
...
impl AsMut<[u8]> for Secp256k1HaneulSignature {
    fn as_mut(&mut self) -> &mut [u8] {
        self.0.as_mut()
    }
}

impl ToFromBytes for Secp256k1HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256k1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256k1HaneulSignature::new(self, msg).into()
    }
}

//
// Secp256r1 Haneul Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash)]
pub struct Secp256r1HaneulSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl HaneulSignatureInner for Secp256r1HaneulSignature {
...
impl HaneulPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl AsRef<[u8]> for Secp256r1HaneulSignature {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl AsMut<[u8]> for Secp256r1HaneulSignature {
    fn as_mut(&mut self) -> &mut [u8] {
        self.0.as_mut()
    }
}

impl ToFromBytes for Secp256r1HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1HaneulSignature::new(self, msg).into()
    }
}
...
//
pub trait HaneulSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    type Sig: Authenticator<PubKey = Self::PubKey>;
    type PubKey: VerifyingKey<Sig = Self::Sig> + HaneulPublicKey;
...
pub trait HaneulSignature: Sized + ToFromBytes {
...
pub trait AuthoritySignInfoTrait: private::SealedAuthoritySignInfoTrait {
    fn verify<T: Signable<Vec<u8>>>(&self, data: &T, committee: &Committee) -> HaneulResult;
...
impl AuthoritySignInfoTrait for EmptySignInfo {
    fn verify<T: Signable<Vec<u8>>>(&self, _data: &T, _committee: &Committee) -> HaneulResult {
        Ok(())
    }
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct HaneulAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for HaneulAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&HaneulAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &HaneulAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
...
//
// see also https://github.com/GeunhwaJeong/haneul/issues/266
...
impl VerificationObligation {
    ...
    pub fn verify_all(self) -> HaneulResult<()> {
...
impl SignatureScheme {
    ...
    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, HaneulError> {
        match byte_int {
