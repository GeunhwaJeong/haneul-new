...
impl KeyPair {
    ...
    /// Make a Narwhal-compatible key pair from a Haneul keypair.
...
impl signature::Signer<Signature> for KeyPair {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        ...
        let mut result_bytes = [0u8; HANEUL_SIGNATURE_LENGTH];
...
impl PublicKeyBytes {
    ...
    /// Make a Narwhal-compatible public key from a Haneul pub.
...
impl TryFrom<PublicKeyBytes> for dalek::PublicKey {
    ...
    fn try_from(value: PublicKeyBytes) -> Result<Self, Self::Error> {
        // TODO(https://github.com/GeunhwaJeong/haneul/issues/101): Do better key validation
        // to ensure the bytes represent a point on the curve.
        dalek::PublicKey::from_bytes(value.as_ref()).map_err(|_| HaneulError::InvalidAuthenticator)
    }
}

// TODO(https://github.com/GeunhwaJeong/haneul/issues/101): more robust key validation
impl TryFrom<&[u8]> for PublicKeyBytes {
    type Error = HaneulError;

    fn try_from(bytes: &[u8]) -> Result<Self, HaneulError> {
        let arr: [u8; dalek::PUBLIC_KEY_LENGTH] = bytes
            .try_into()
            .map_err(|_| HaneulError::InvalidAuthenticator)?;
        Ok(Self(arr))
    }
}
    ...
    R: rand::CryptoRng + rand::RngCore,
{
    ...
    (HaneulAddress::from(keypair.public_key_bytes()), keypair)
}

// TODO: C-GETTER
pub fn get_key_pair_from_bytes(bytes: &[u8]) -> (HaneulAddress, KeyPair) {
    let keypair = KeyPair {
...
pub const HANEUL_SIGNATURE_LENGTH: usize =
...
impl signature::Signature for Signature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, signature::Error> {
        let val: [u8; HANEUL_SIGNATURE_LENGTH] =
...
impl Signature {
    ...
    pub fn new_empty() -> Self {
        Self([0u8; HANEUL_SIGNATURE_LENGTH])
    }
    ...
    /// that the signature was performed with a PublicKey belonging to an expected author, indicated by its Haneul Address
    pub fn verify<T>(&self, value: &T, author: HaneulAddress) -> Result<(), HaneulError>
    where
        T: Signable<Vec<u8>>,
    {
        ...
        // TODO: perform stricter key validation, sp. small order points, see https://github.com/GeunhwaJeong/haneul/issues/101
        let public_key =
    ...
    ) -> Result<(Vec<u8>, ed25519_dalek::Signature, PublicKeyBytes), HaneulError>
    where
        T: Signable<Vec<u8>>,
    {
        ...
        let received_addr = HaneulAddress::from(&PublicKeyBytes(public_key_bytes));
...
impl AuthoritySignature {
    ...
    pub fn verify<T>(&self, value: &T, author: PublicKeyBytes) -> Result<(), HaneulError>
    where
        T: Signable<Vec<u8>>,
    {
    ...
    ///
    /// We check that the provided signatures are valid w.r.t the provided public keys, but do not match them to any Haneul Addresses.
...
impl AuthoritySignInfo {
    ...
    pub fn verify<T>(&self, data: &T, committee: &Committee) -> HaneulResult<()>
    where
        T: Signable<Vec<u8>>,
    {
...
impl<const STRONG_THRESHOLD: bool> AuthorityQuorumSignInfo<STRONG_THRESHOLD> {
    ...
    pub fn add_to_verification_obligation(
        ...
        fp_ensure!(
            ...
            HaneulError::InvalidAuthorityBitmap {
...
impl VerificationObligation {
    ...
    ) -> Result<dalek::PublicKey, HaneulError> {
    ...
    pub fn verify_all(self) -> HaneulResult<PubKeyLookup> {
