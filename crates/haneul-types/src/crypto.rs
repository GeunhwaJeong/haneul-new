...
use crate::haneul_serde::{HaneulBitmap, Readable};
...
impl HaneulKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            HaneulKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            HaneulKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &Bn254FrElement) -> HaneulResult<Self> {
    ...
    pub fn validate(&self) -> HaneulResult<()> {
        let bytes = &self.0;

        // Parse issuer length and bytes.
        let iss_len = *bytes
            .first()
            .ok_or_else(|| HaneulErrorKind::InvalidSignature {
        ...
        std::str::from_utf8(iss_bytes).map_err(|e| HaneulErrorKind::InvalidSignature {
...
impl PublicKey {
    ...
    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> HaneulResult<Self> {
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (haneul-config/src/network_config_builder). This is because
