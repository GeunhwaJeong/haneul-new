...
impl From<Ed25519KeyPair> for HaneulKeyPair {
    fn from(key: Ed25519KeyPair) -> Self {
        HaneulKeyPair::Ed25519HaneulKeyPair(key)
    }
}

impl From<Secp256k1KeyPair> for HaneulKeyPair {
    fn from(key: Secp256k1KeyPair) -> Self {
        HaneulKeyPair::Secp256k1HaneulKeyPair(key)
    }
}
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519KeyPair(_) => Ed25519HaneulSignature::SCHEME,
            PublicKey::Secp256k1KeyPair(_) => Secp256k1HaneulSignature::SCHEME,
        }
    }
}
...
/// Wrapper function to return HaneulKeypair based on key scheme string
pub fn random_key_pair_by_type(
    key_scheme: SignatureScheme,
) -> Result<(HaneulAddress, HaneulKeyPair), anyhow::Error> {
    match key_scheme.to_string().as_ref() {
        "secp256k1" => {
            let (addr, key_pair): (_, Secp256k1KeyPair) = get_key_pair();
            Ok((addr, HaneulKeyPair::Secp256k1HaneulKeyPair(key_pair)))
        }
        "ed25519" => {
            let (addr, key_pair): (_, Ed25519KeyPair) = get_key_pair();
            Ok((addr, HaneulKeyPair::Ed25519HaneulKeyPair(key_pair)))
        }
...
/// Ed25519 follows SLIP-0010 using hardened path: m/44'/8282'/0'/0'/{index}'
/// Secp256k1 follows BIP-32 using path where the first 3 levels are hardened: m/54'/8282'/0'/0/{index}
/// Note that the purpose for Secp256k1 is registered as 54, to differentiate from Ed25519 with purpose 44.
pub fn derive_key_pair_from_path(
    seed: &[u8],
    derivation_path: Option<DerivationPath>,
    key_scheme: &SignatureScheme,
) -> Result<(HaneulAddress, HaneulKeyPair), HaneulError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| HaneulError::SignatureKeyGenError(e.to_string()))?;
            let kp = Ed25519KeyPair::from(sk);
            Ok((kp.public().into(), HaneulKeyPair::Ed25519HaneulKeyPair(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), HaneulKeyPair::Secp256k1HaneulKeyPair(kp)))
        }
        SignatureScheme::BLS12381 => Err(HaneulError::UnsupportedFeatureError {
...
) -> Result<DerivationPath, HaneulError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at all levels with purpose = 44, coin_type = 8282
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(HaneulError::SignatureKeyGenError("Invalid path".to_string()))
                        }
                    } else {
                        Err(HaneulError::SignatureKeyGenError("Invalid path".to_string()))
                    }
        ...
        SignatureScheme::Secp256k1 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at first 3 levels with purpose = 54, coin_type = 8282
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                        if purpose
...
/// Wrapper function to return HaneulKeypair based on key scheme string with seedable rng.
    ...
    <KP as KeypairTraits>::PubKey: HaneulPublicKey,
{
    ...
    if kp.public().as_ref() != &bytes[priv_length..] {
        return Err(HaneulError::InvalidAddress);
    }
...
// impl Default for Secp256k1HaneulSignature {
//     []
// }
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation(
        ...
        fp_ensure!(weight >= threshold, HaneulError::CertificateRequiresQuorum);
...
impl VerificationObligation {
    ...
    >(
        ...
        match signature.to_obligation_signature(&public_key) {
            ...
            ObligationSignature::None => Err(HaneulError::SenderSigUnbatchable),
        }
    }
