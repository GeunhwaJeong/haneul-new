...
impl Signer<Signature> for HaneulKeyPair {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        match self {
            HaneulKeyPair::Ed25519(kp) => kp.try_sign(msg),
            HaneulKeyPair::Secp256k1(kp) => kp.try_sign(msg),
            HaneulKeyPair::Secp256r1(kp) => kp.try_sign(msg),
        }
    }
}
...
impl signature::Signature for Ed25519HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, signature::Error> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        Ok(Ed25519HaneulSignature::new(self, msg)
            .map_err(|_| signature::Error::new())?
            .into())
    }
}
...
impl signature::Signature for Secp256k1HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, signature::Error> {
        if bytes.len() != Self::LENGTH {
            return Err(signature::Error::new());
        }
...
impl signature::Signature for Secp256r1HaneulSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, signature::Error> {
        if bytes.len() != Self::LENGTH {
            return Err(signature::Error::new());
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        Ok(Secp256r1HaneulSignature::new(self, msg)
            .map_err(|_| signature::Error::new())?
            .into())
    }
}
...
//
pub trait HaneulSignatureInner: Sized + signature::Signature + PartialEq + Eq + Hash {
    ...
    fn new(kp: &Self::KeyPair, message: &[u8]) -> HaneulResult<Self> {
        let sig = kp
            .try_sign(message)
