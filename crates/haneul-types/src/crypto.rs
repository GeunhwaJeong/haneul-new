...
pub trait HaneulAuthoritySignature {
    ...
    fn verify<T>(&self, value: &T, author: PublicKeyBytes) -> Result<(), HaneulError>
    where
        T: Signable<Vec<u8>>;
}
...
impl HaneulAuthoritySignature for AuthoritySignature {
        ...
        T: Signable<Vec<u8>>,
    {
        ...
        let public_key =
            PublicKey::from_bytes(author.as_ref()).map_err(|_| HaneulError::InvalidAddress)?;
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes(bytes: &[u8]) -> HaneulResult<(HaneulAddress, KeyPair)> {
    let priv_length = <KeyPair as narwhal_crypto::traits::KeyPair>::PrivKey::LENGTH;
    let sk =
        PrivateKey::from_bytes(&bytes[..priv_length]).map_err(|_| HaneulError::InvalidPrivateKey)?;
...
pub const HANEUL_SIGNATURE_LENGTH: usize = PublicKey::LENGTH + AccountSignature::LENGTH;
...
impl Signature {
    ...
    pub fn get_verification_inputs(
    ...
    ) -> Result<(AccountSignature, PublicKeyBytes), HaneulError> {
        ...
        let received_addr: HaneulAddress = (&public_key_bytes).into();
        ...
        let signature =
            <AccountSignature as signature::Signature>::from_bytes(self.signature_bytes())
                .map_err(|err| HaneulError::InvalidSignature {
                    error: err.to_string(),
                })?;
