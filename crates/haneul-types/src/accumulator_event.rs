...
impl AccumulatorEvent {
    ...
    pub fn from_balance_change(
        ...
        if !Balance::is_balance_type(&balance_type) {
            return Err(HaneulErrorKind::TypeError {
    ...
    pub fn total_haneul_in_event(&self) -> (u64 /* input */, u64 /* output */) {
        let Self {
            write:
                AccumulatorWriteV1 {
        ...
        match operation {
            AccumulatorOperation::Merge => (0, haneul),
            AccumulatorOperation::Split => (haneul, 0),
        }
    }
}
