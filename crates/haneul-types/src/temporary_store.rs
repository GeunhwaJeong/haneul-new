...
pub struct TemporaryStore<S> {
    ...
    events: Vec<Event>,
    gas_charged: Option<(HaneulAddress, ObjectID, GasCostSummary)>,
...
impl<S> TemporaryStore<S> {
    ...
    fn ensure_active_inputs_mutated(&mut self, sender: HaneulAddress) {
    ...
    fn reset(&mut self, sender: HaneulAddress, gas: &[ObjectRef], gas_status: &mut HaneulGasStatus<'_>) {
        self.drop_writes();
...
impl<S: GetModule + ObjectStore + BackingPackageStore> TemporaryStore<S> {
    ...
    pub fn check_haneul_conserved(&self) {
        ...
        // total HANEUL in input objects
        let input_haneul = self.mutable_input_refs.iter().fold(0, |acc, o| {
            acc + self
                .input_objects
                .get(&o.0)
                .unwrap()
                .get_total_haneul(&self)
                .unwrap()
        });
        // if a dynamic field object O is written by this tx, count get_total_haneul(pre_tx_value(O)) as part of input_haneul
        let dynamic_field_input_haneul = self.dynamic_fields_touched().iter().fold(0, |acc, id| {
            acc + self
                .store
                .get_object(id)
                .unwrap()
        ...
        // total HANEUL in output objects
        let output_haneul = self.written.values().fold(0, |acc, v| {
            output_rebate_amount += v.1.storage_rebate;
            acc + v.1.get_total_haneul(&self).unwrap()
        });
        ...
        assert_eq!(
            input_haneul + dynamic_field_input_haneul,
            output_haneul + gas_summary.computation_cost + storage_fund_rebate_inflow
        )
    }
}
