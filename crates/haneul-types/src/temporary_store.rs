...
impl<S: GetModule + ObjectStore + BackingPackageStore> TemporaryStore<S> {
    /// Get the total HANEUL in `obj` at version `v`, which should be the version before the
    /// tx executed. for internal use only
    fn get_input_haneul(&self, id: &ObjectID, expected_version: SequenceNumber) -> HaneulResult<u64> {
        if let Some(obj) = self.input_objects.get(id) {
            debug_assert_eq!(obj.version(), expected_version);
            obj.get_total_haneul(&self)
        } else {
            ...
            let obj = self
                ...
                .get_object(id)
                .unwrap()
                .expect("Failed to look up input object and identify its total HANEUL");
            debug_assert_eq!(obj.version(), expected_version);
            obj.get_total_haneul(&self)
        }
    }
    ...
    pub fn check_haneul_conserved(
        ...
        for (id, (output_obj, kind)) in &self.written {
            match kind {
                WriteKind::Mutate => {
                    ...
                    let input = self.get_input_haneul(id, input_version)?;
                    total_input_haneul += input;
                    let output = output_obj.get_total_haneul(&self)?;
        ...
        fp_ensure!(
            ...
            HaneulError::from(format!(
                "HANEUL conservation violated: input={}, output={}, this transaction either mints or burns HANEUL",
                total_input_haneul,
                total_output_haneul,
            ).as_str())
        );
