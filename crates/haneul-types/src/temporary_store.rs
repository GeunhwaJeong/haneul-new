...
impl<S: GetModule + ObjectStore + BackingPackageStore> TemporaryStore<S> {
    ...
    pub fn check_haneul_conserved(
        ...
        for (id, (output_obj, kind)) in &self.written {
            match kind {
                WriteKind::Mutate => {
                    ...
                    total_input_haneul += self.get_input_haneul(id, input_version)?;
                    total_output_haneul += output_obj.get_total_haneul(&self)?;
        ...
        total_output_haneul += gas_summary.computation_cost + storage_fund_rebate_inflow;
