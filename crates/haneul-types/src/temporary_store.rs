...
impl InnerTemporaryStore {
    ...
    pub fn get_written_objects_owned_by(&self, address: &HaneulAddress) -> Vec<ObjectID> {
        self.written
            .values()
            .filter_map(|(_, o, _)| {
...
impl<S> TemporaryStore<S> {
    ...
    pub fn to_effects(
        ...
        let updated_gas_object_info = if gas_object_ref.0 == ObjectID::ZERO {
            (gas_object_ref, Owner::AddressOwner(HaneulAddress::default()))
        } else {
    ...
    pub fn reset(&mut self, gas: &[ObjectRef], gas_status: &mut HaneulGasStatus) {
        self.drop_writes();
...
impl<S: ObjectStore> TemporaryStore<S> {
    ...
    fn collect_storage_and_rebate(&mut self, gas_status: &mut HaneulGasStatus) {
        let mut objects_to_update = vec![];
        for (object_id, (object, write_kind)) in &mut self.written {
    ...
    fn collect_rebate(&self, gas_status: &mut HaneulGasStatus) {
        for (object_id, (version, kind)) in &self.deleted {
            match kind {
                DeleteKind::Wrap | DeleteKind::Normal => {
...
impl<S: ObjectStore> TemporaryStore<S> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_haneul_system_state_wrapper(&self.store)
...
impl<S: GetModule + ObjectStore + BackingPackageStore> TemporaryStore<S> {
    fn get_input_haneul(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            let input_haneul = if do_expensive_checks {
    ...
    ///
    /// 3. all HANEUL in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of HANEUL. We need these information for conservation check.
    pub fn check_haneul_conserved(
        ...
        for (id, (output_obj, kind)) in &self.written {
            match kind {
                WriteKind::Mutate => {
                    ...
                    let (input_haneul, input_storage_rebate) = self.get_input_haneul(
                        id,
                        input_version,
                        layout_resolver,
                        do_expensive_checks,
                    ...
                    if do_expensive_checks {
                        total_output_haneul +=
                            output_obj.get_total_haneul(layout_resolver).map_err(|e| {
                                make_invariant_violation!(
                ...
                WriteKind::Unwrap => {
                    ...
                    // in both cases, its contribution to input HANEUL will be captured by looking at A
                    if do_expensive_checks {
                        total_output_haneul +=
                            output_obj.get_total_haneul(layout_resolver).map_err(|e| {
                                make_invariant_violation!(
        ...
        for (id, (input_version, kind)) in &self.deleted {
            match kind {
                ...
                DeleteKind::UnwrapThenDelete => {
                    // an unwrapped option was wrapped in input object or dynamic field A, which means its contribution to input HANEUL will
...
impl<S: GetModule<Error = HaneulError, Item = CompiledModule>> GetModule for TemporaryStore<S> {
    type Error = HaneulError;
