...
impl<S: GetModule + ObjectStore + BackingPackageStore> TemporaryStore<S> {
    ...
    pub fn check_haneul_conserved(&self) -> HaneulResult<()> {
        ...
        let gas_summary = &self
            ...
            .ok_or_else(|| {
                ExecutionError::invariant_violation(
                    "Failed unwrapping gas_charged in HANEUL conservation checking",
                )
            })?
            .1;
        ...
        assert_eq!(
            ...
            "HANEUL conservation violated--this transaction either mints or burns HANEUL"
        );
        Ok(())
    }
}
