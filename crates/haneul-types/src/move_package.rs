...
impl MovePackage {
    ...
    pub fn normalize<S: Hash + Eq + Clone + ToString, Pool: normalized::StringPool<String = S>>(
        ...
        binary_config: &BinaryConfig,
        include_code: bool,
    ) -> HaneulResult<BTreeMap<String, normalized::Module<S>>> {
        normalize_modules(pool, self.module_map.values(), binary_config, include_code)
    }

    pub fn into_serialized_move_package(&self) -> HaneulResult<SerializedPackage> {
        macro_rules! expect_valid_identifier {
            ($ident_str:expr) => {
                Identifier::new($ident_str.clone()).map_err(|e| {
                    ...
                    HaneulErrorKind::ExecutionInvariantViolation
                })
            };
        }
