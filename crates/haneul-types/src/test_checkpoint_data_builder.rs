...
use crate::{
    base_types::{dbg_addr, ExecutionDigests, ObjectID, ObjectRef, SequenceNumber, HaneulAddress},
    coin::Coin,
    committee::Committee,
...
pub struct TestCheckpointDataBuilder {
    ...
    gas_map: HashMap<HaneulAddress, ObjectID>,
...
impl TestCheckpointDataBuilder {
    ...
    /// A proper HaneulAddress will be derived from it.
    ...
    /// The object will be created as a HANEUL coin object, with default balance,
    /// and the transaction sender as its owner.
    pub fn create_owned_object(self, object_idx: u64) -> Self {
        self.create_haneul_object(object_idx, GAS_VALUE_FOR_TESTING)
    }
    ...
    /// Create a new HANEUL coin object in the transaction.
    /// `object_idx` is a convenient representation of the object's ID.
    /// `balance` is the amount of HANEUL to be created.
    pub fn create_haneul_object(self, object_idx: u64, balance: u64) -> Self {
        let sender_idx = self
            .checkpoint_builder
            .next_transaction
            .as_ref()
    ...
    pub fn derive_address(address_idx: u8) -> HaneulAddress {
...
mod tests {
    ...
    #[test]
    fn test_haneul_balance_transfer() {
        let checkpoint = TestCheckpointDataBuilder::new(1)
            .start_transaction(0)
            .create_haneul_object(0, 100)
            .finish_transaction()
            .start_transaction(1)
        ...
        // Verify the newly created object appears in output objects and is a gas coin with 100 GEUNHWA.
        ...
        // Verify the original HANEUL coin now has 90 GEUNHWA after the transfer.
        assert!(tx.output_objects.iter().any(|obj| obj.id() == obj_id0
            && obj.is_gas_coin()
            && obj.data.try_as_move().unwrap().get_coin_value_unsafe() == 90));

        // Verify the split out HANEUL coin has 10 GEUNHWA.
