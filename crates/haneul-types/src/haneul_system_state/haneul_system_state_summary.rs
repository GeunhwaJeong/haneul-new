...
pub struct HaneulSystemStateSummary {
    ...
    pub active_validators: Vec<HaneulValidatorSummary>,
    ...
    #[schemars(with = "Vec<(HaneulAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(HaneulAddress, u64)>,
...
/// This is the JSON-RPC type for the HANEUL validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct HaneulValidatorSummary {
    // Metadata
...
impl Default for HaneulSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for HaneulValidatorSummary {
    fn default() -> Self {
        Self {
            haneul_address: HaneulAddress::default(),
...
/// Given the staking pool id of a validator, return the validator's `HaneulValidatorSummary`,
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: HaneulAddress = get_dynamic_field_from_store(
        &object_store,
