...
use super::{AdvanceEpochParams, HaneulSystemStateTrait, get_validators_from_table_vec};
use crate::balance::Balance;
use crate::base_types::{HaneulAddress, ObjectID};
...
impl ValidatorMetadataV1 {
    ...
    pub fn verify(&self, metadata_v2: bool) -> Result<VerifiedValidatorMetadataV1, u64> {
        ...
        verify_proof_of_possession(&pop, &protocol_pubkey, self.haneul_address)
            .map_err(|_| E_METADATA_INVALID_POP)?;
        ...
        if metadata_v2 {
            match (&next_epoch_network_pubkey, &next_epoch_worker_pubkey) {
                (Some(next_epoch_network_pubkey), Some(next_epoch_worker_pubkey)) => {
                    if next_epoch_network_pubkey == next_epoch_worker_pubkey {
                        return Err(E_METADATA_INVALID_WORKER_PUBKEY);
                    }
                }
                (Some(next_epoch_network_pubkey), None) => {
                    if next_epoch_network_pubkey == &worker_pubkey {
                        return Err(E_METADATA_INVALID_NET_PUBKEY);
                    }
                }
                (None, Some(next_epoch_worker_pubkey)) => {
                    if next_epoch_worker_pubkey == &network_pubkey {
                        return Err(E_METADATA_INVALID_WORKER_PUBKEY);
                    }
                }
                (None, None) => (),
        ...
        let Self {
            ...
            staking_pool:
                StakingPoolV1 {
                    ...
                    haneul_balance: staking_pool_haneul_balance,
                    rewards_pool,
                    pool_token_balance,
                    exchange_rates:
                        Table {
...
/// Rust version of the Move haneul_system::staking_pool::StakingPool type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
/// Rust version of the Move haneul_system::validator_set::ValidatorSet type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct ValidatorSetV1 {
    ...
    pub at_risk_validators: VecMap<HaneulAddress, u64>,
    pub extra_fields: Bag,
}

/// Rust version of the Move haneul_system::storage_fund::StorageFund type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
/// Rust version of the Move haneul_system::validator_cap::UnverifiedValidatorOperationCap type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct UnverifiedValidatorOperationCapV1 {
    pub id: ObjectID,
    pub authorizer_validator_address: HaneulAddress,
}
