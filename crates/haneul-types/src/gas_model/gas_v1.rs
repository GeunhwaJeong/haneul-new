...
use crate::{
    ...
    gas::{get_gas_balance, GasCostSummary, HaneulGasStatusAPI},
    object::{Object, Owner},
};
use move_core_types::{
...
use haneul_cost_tables::bytecode_tables::{INITIAL_COST_SCHEDULE, ZERO_COST_SCHEDULE};
use haneul_cost_tables::units_types::CostTable;
use haneul_cost_tables::{bytecode_tables::GasStatus, units_types::GasUnit};
...
// for a RPG of 1000 this amounts to about 1 HANEUL
...
type HaneulGas = GasQuantity<HaneulGasUnit>;
...
/// A list of constant costs of various operations in Haneul.
pub struct HaneulCostTable {
...
pub struct HaneulGasStatus {
    ...
    /// It will be multiplied by the storage gas unit price in the end to obtain the Haneul cost.
    storage_gas_units: GasUnits,
    /// storage_rebate is the total storage rebate (in Haneul) accumulated in this transaction.
...
impl HaneulGasStatus {
    ...
    pub(crate) fn new_with_budget(
        ...
        let haneul_cost_table = HaneulCostTable::new(config);
        Self::new(
            GasStatus::new(haneul_cost_table.execution_cost_table.clone(), GasUnits::new(budget_in_unit)),
            budget_in_unit,
            true,
            computation_gas_unit_price.into(),
    ...
    pub(crate) fn new_unmetered() -> HaneulGasStatus {
        Self::new(
...
impl HaneulGasStatusAPI for HaneulGasStatus {
    ...
    fn summary(&self) -> GasCostSummary {
        ...
        let computation_cost_in_haneul = computation_cost.mul(self.computation_gas_unit_price).into();
        GasCostSummary {
            computation_cost: computation_cost_in_haneul,
