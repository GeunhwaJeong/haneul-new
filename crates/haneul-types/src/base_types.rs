...
impl MoveObjectType {
    ...
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul | MoveObjectType_::Other(_) => false,
        }
    }
    ...
    pub fn is_gas_coin(&self) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul | MoveObjectType_::Coin(_) | MoveObjectType_::Other(_) => {
                false
            }
        }
    }
...
impl HaneulAddress {
    ...
    pub fn generate<R: rand::RngCore + rand::CryptoRng>(mut rng: R) -> Self {
        let buf: [u8; HANEUL_ADDRESS_LENGTH] = rng.gen();
        Self(buf)
    }
    ...
    pub fn legacy_try_from(authenticator: &ZkLoginAuthenticator) -> HaneulResult<Self> {
        ...
        let padded = match bytes.len() {
            ...
            32 => Ok(bytes),
            _ => Err(HaneulError::InvalidAddress),
        };
...
/// Haneul address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || address_seed in bytes].
impl TryFrom<&ZkLoginInputs> for HaneulAddress {
    type Error = HaneulError;
    fn try_from(inputs: &ZkLoginInputs) -> HaneulResult<Self> {
        let mut hasher = DefaultHash::default();
        hasher.update([SignatureScheme::ZkLoginAuthenticator.flag()]);
