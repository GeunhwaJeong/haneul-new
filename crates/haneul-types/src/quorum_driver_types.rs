...
pub enum QuorumDriverError {
    ...
    #[error("Transaction encountered uncategorized HaneulError: {:?}", error)]
    UncategorizedHaneulError { error: HaneulError },
}

impl From<HaneulError> for QuorumDriverError {
    fn from(error: HaneulError) -> Self {
        QuorumDriverError::UncategorizedHaneulError { error }
    }
}
