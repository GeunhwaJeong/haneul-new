...
pub enum HaneulError {
    ...
    QuorumFailedToProcessTransaction { errors: Vec<HaneulError> },
    #[error(
    "Failed to execute certificate on a quorum of validators, caused by : {:#?}",
    errors.iter().map(| e | ToString::to_string(&e)).collect::<Vec<String>>()
    )]
    QuorumFailedToExecuteCertificate { errors: Vec<HaneulError> },
    ...
    QuorumNotReached { errors: Vec<HaneulError> },
    ...
    #[error("Failed to execute handle_consensus_transaction on Haneul: {0}")]
    ...
    ConsensusHaneulSerializationError(String),
...
impl From<ExecutionErrorKind> for HaneulError {
    ...
    fn from(error: VMError) -> Self {
        let kind = match (error.major_status(), error.sub_status(), error.location()) {
            ...
            (StatusCode::ABORTED, _, Location::Script) => {
                debug_assert!(false, "Scripts are not used in Haneul");
