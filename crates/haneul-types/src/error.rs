...
pub enum UserInputError {
    ...
    #[error("Gas price cannot exceed {max_gas_price} geunhwa")]
    ...
    #[error(
        "HANEUL payment transactions use first input coin for gas payment, but found a different gas object"
    )]
...
impl HaneulErrorKind {
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            ...
            HaneulErrorKind::TransactionProcessing { .. } => true,
            HaneulErrorKind::TransactionSubmitted { .. } => true,
