...
pub trait BackingPackageStore {
    ...
    fn get_package(&self, package_id: &ObjectID) -> HaneulResult<Option<MovePackage>> {
    ...
    ) -> HaneulResult<PackageFetchResults<Object>> {
    ...
    fn get_packages<'a>(
        &self,
        package_ids: impl IntoIterator<Item = &'a ObjectID>,
    ) -> HaneulResult<PackageFetchResults<MovePackage>> {
...
pub fn get_module<S: BackingPackageStore>(
    store: S,
    module_id: &ModuleId,
) -> Result<Option<Vec<u8>>, HaneulError> {
    Ok(store
...
pub fn get_module_by_id<S: BackingPackageStore>(
    store: S,
    id: &ModuleId,
) -> anyhow::Result<Option<CompiledModule>, HaneulError> {
...
pub fn transaction_input_object_keys(tx: &SenderSignedData) -> HaneulResult<Vec<ObjectKey>> {
