...
pub fn find_jwk_by_kid(kid: &str, json_bytes: &[u8]) -> Result<OAuthProviderContent, HaneulError> {
        ...
        if let Some(keys) = parsed_list["keys"].as_array() {
            for k in keys {
                let parsed: OAuthProviderContentReader =
                    serde_json::from_value(k.clone()).map_err(|_| HaneulError::JWKRetrievalError)?;
                if parsed.kid == kid
                    && parsed.alg == "RS256"
                    && parsed.my_use == "sig"
                    && parsed.kty == "RSA"
    ...
    Err(HaneulError::JWKRetrievalError)
}
