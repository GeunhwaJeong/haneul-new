...
impl Authenticator {
    pub fn as_single_owner(&self) -> &HaneulAddress {
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> HaneulResult<HaneulAddress> {
        match self {
            ...
            | Self::Immutable
            | Self::ObjectOwner(_)
            | Self::ConsensusV2 { .. } => Err(HaneulError::UnexpectedOwnerType),
        }
    }
            ...
            Owner::ConsensusV2 { authenticator, .. } => {
                DEFAULT_OWNER_SIZE
                    + match authenticator.as_ref() {
                        Authenticator::SingleOwner(_) => 8, // marginal cost to store both HaneulAddress and SequenceNumber
                    }
            }
        };
