...
use crate::{MOVE_STDLIB_OBJECT_ID, HANEUL_FRAMEWORK_OBJECT_ID};
...
impl MoveObject {
    ...
    pub fn id_opt(contents: &[u8]) -> Result<ObjectID, ObjectIDParseError> {
        // TODO: Ensure safe index to to parse ObjectID. https://github.com/GeunhwaJeong/haneul/issues/6278
...
impl PartialEq<HaneulAddress> for Owner {
...
impl Object {
    ...
    pub fn is_system_package(&self) -> bool {
        let id = self.id();
        self.is_package() && (id == HANEUL_FRAMEWORK_OBJECT_ID || id == MOVE_STDLIB_OBJECT_ID)
    }
    ...
    pub fn get_move_template_type(&self) -> HaneulResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| HaneulError::TypeError {
...
impl Object {
    ...
    pub fn get_total_haneul(&self, resolver: &impl GetModule) -> Result<u64, HaneulError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_haneul(resolver)?,
                Data::Package(_) => 0,
            })
    }
...
pub fn generate_test_gas_objects_with_owner(count: usize, owner: HaneulAddress) -> Vec<Object> {
    (0..count)
        .map(|_i| {
            let gas_object_id = ObjectID::random();
...
/// Make a few test gas objects (all with the same owner) with TOTAL_SUPPLY_GEUNHWA / count balance
pub fn generate_max_test_gas_objects_with_owner(count: u64, owner: HaneulAddress) -> Vec<Object> {
    let coin_size = TOTAL_SUPPLY_GEUNHWA / count;
    (0..count)
        .map(|_i| {
            let gas_object_id = ObjectID::random();
