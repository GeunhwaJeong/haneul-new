...
pub struct Party {
    ...
    members: BTreeMap<HaneulAddress, ObjectPermissions>,
}
...
impl Party {
    ...
    pub fn permissions_for(&self, address: &HaneulAddress) -> ObjectPermissions {
        self.members
            .get(address)
            .copied()
...
struct RawPartySerde {
    default_permissions: ObjectPermissions,
    members: Vec<(HaneulAddress, ObjectPermissions)>,
}
...
impl TryFrom<RawPartySerde> for Party {
    ...
    fn try_from(raw: RawPartySerde) -> Result<Self, Self::Error> {
        ...
        let members: BTreeMap<HaneulAddress, ObjectPermissions> = raw.members.into_iter().collect();
...
impl proptest::arbitrary::Arbitrary for Party {
    ...
    fn arbitrary_with(_: ()) -> Self::Strategy {
        ...
        (
            ...
            any::<BTreeMap<HaneulAddress, ObjectPermissions>>(),
        )
            .prop_filter_map(
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> HaneulResult<HaneulAddress> {
        match self {
            ...
            | Self::Party { .. } => Err(HaneulErrorKind::UnexpectedOwnerType.into()),
        }
    }
...
mod tests {
    ...
    fn test_owner_variant_sizes() {
        ...
        assert_eq!(std::mem::size_of::<HaneulAddress>(), 32);
        // Shared
        assert_eq!(std::mem::size_of::<SequenceNumber>(), 8);
        // ConsensusAddressOwner
        assert_eq!(std::mem::size_of::<(SequenceNumber, HaneulAddress)>(), 40);
        // Party
