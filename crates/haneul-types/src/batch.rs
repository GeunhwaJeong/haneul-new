...
impl Message for AuthorityBatch {
    ...
    fn verify(&self) -> HaneulResult {
        fp_ensure!(
            ...
            HaneulError::from("Invalid AuthorityBatch sequence number")
        );
        fp_ensure!(
            self.next_sequence_number - self.initial_sequence_number >= self.size,
            HaneulError::from("Invalid AuthorityBatch size")
        );
        Ok(())
    }

    fn add_to_verification_obligation(&self, _: &mut VerificationObligation) -> HaneulResult<()> {
        Ok(())
    }
}
