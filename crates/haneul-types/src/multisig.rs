...
impl AuthenticatorTrait for MultiSig {
    fn verify_user_authenticator_epoch(&self, epoch_id: EpochId) -> Result<(), HaneulError> {
                ...
                match sig {
                    ...
                    CompressedSignature::Secp256k1(s) => {
                        let pk = Secp256k1PublicKey::from_bytes(subsig_pubkey.as_ref()).map_err(
                            |_| HaneulError::InvalidSignature {
                                error: "Invalid public key".to_string(),
                            },
                        )?;
                        pk.verify(
                    ...
                    CompressedSignature::ZkLogin(z) => {
                        ...
                        authenticator
                            .verify_claims(value, HaneulAddress::from(subsig_pubkey), verify_params)
...
impl MultiSig {
    ...
    pub fn get_zklogin_sigs(&self) -> Result<Vec<ZkLoginAuthenticator>, HaneulError> {
        ...
        authenticator_as_bytes
            .iter()
            .map(|z| {
                ZkLoginAuthenticator::from_bytes(&z.0).map_err(|_| HaneulError::InvalidAuthenticator)
            })
            .collect()
    }

    pub fn get_indices(&self) -> Result<Vec<u8>, HaneulError> {
