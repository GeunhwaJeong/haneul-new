...
pub const RESOLVED_COIN_STRUCT: (&AccountAddress, &IdentStr, &IdentStr) =
    (&HANEUL_FRAMEWORK_ADDRESS, COIN_MODULE_NAME, COIN_STRUCT_NAME);
...
impl TreasuryCap {
    ...
    pub fn from_bcs_bytes(content: &[u8]) -> Result<Self, HaneulError> {
        bcs::from_bytes(content).map_err(|err| HaneulError::ObjectDeserializationError {
...
// Rust version of the Move haneul::coin::RegulatedCoinMetadata type
