...
pub fn is_dynamic_object(move_struct: &MoveStruct) -> bool {
    match move_struct {
        MoveStruct::WithTypes { type_, .. } => {
            matches!(&type_.type_params[0], TypeTag::Struct(tag) if tag.address == HANEUL_FRAMEWORK_ADDRESS
    ...
    V: Serialize + DeserializeOwned,
{
    ...
    let object = object_store.get_object(&id)?.ok_or_else(|| {
        HaneulError::DynamicFieldReadError("Dynamic field not found on parent".to_owned())
    })?;
    let move_object = object.data.try_as_move().ok_or_else(|| {
        HaneulError::DynamicFieldReadError("Dynamic field is not a Move object".to_owned())
    })?;
