...
use crate::{MoveTypeTagTrait, ObjectID, SequenceNumber, HANEUL_FRAMEWORK_ADDRESS};
...
/// Rust version of the Move haneul::dynamic_field::Field type
...
/// Rust version of the Move haneul::dynamic_object_field::Wrapper type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<HaneulTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> HaneulResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| HaneulError::ObjectDeserializationError {
    ...
    pub fn try_extract_field_value(tag: &StructTag) -> HaneulResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(HaneulError::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> HaneulResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            HaneulError::ObjectDeserializationError {
                error: "Cannot extract [name] field from haneul::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            HaneulError::ObjectDeserializationError {
                error: "Cannot extract [value] field from haneul::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| HaneulError::ObjectDeserializationError {
                error: "Cannot extract [name] field from haneul::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            // ID extracted from the wrapper object
            let object_id =
                extract_id_value(value).ok_or_else(|| HaneulError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        haneul::dynamic_field::Field, {value:?}"
                    ),
                })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                HaneulError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        haneul::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
where
    T: Into<HaneulAddress>,
{
    let parent: HaneulAddress = parent.into();
...
pub fn serialize_dynamic_field<K, V>(id: &UID, name: &K, value: V) -> Result<Vec<u8>, HaneulError>
where
    K: Serialize + Clone,
    V: Serialize,
{
    ...
    bcs::to_bytes(&field).map_err(|err| HaneulError::ObjectSerializationError {
        error: err.to_string(),
    })
}
    ...
    K: Serialize + fmt::Debug,
{
    ...
    ) -> Result<BoundedDynamicFieldID<K>, HaneulError> {
        let id = self.object_id()?;
    ...
    pub fn into_field<V>(self, value: V) -> Result<DynamicField<K, V>, HaneulError>
    where
    ...
    V: Serialize,
{
    ...
    pub fn into_move_object_unsafe_for_testing(
        ...
        let field_bytes =
            bcs::to_bytes(&field).map_err(|e| HaneulError::ObjectSerializationError {
                error: e.to_string(),
            })?;
        Ok(unsafe {
    ...
    K: Serialize + std::fmt::Debug,
{
    ...
    pub fn expect_object(
        ...
        key: &K,
        object_store: &dyn ObjectStore,
    ) -> Result<DynamicFieldObject<K>, HaneulError> {
        let parent = self.0;
        let id = self.1;
    ...
    K: Serialize,
{
    ...
    ) -> Result<Option<DynamicFieldObject<K>>, HaneulError> {
    ...
    K: Serialize + DeserializeOwned,
{
    ...
    pub fn load_value<V>(self) -> Result<V, HaneulError>
    where
        V: Serialize + DeserializeOwned,
    {
    ...
    /// Deserialize the field value from the object. Requires that the value type is known.
    pub fn load_field<V>(self) -> Result<Field<K, V>, HaneulError>
    where
        V: Serialize + DeserializeOwned,
    {
        let object = self.0;
        ...
        bcs::from_bytes::<Field<K, V>>(move_object.contents())
            .map_err(|err| HaneulError::DynamicFieldReadError(err.to_string()))
    }
}
