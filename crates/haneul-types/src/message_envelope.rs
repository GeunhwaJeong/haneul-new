...
impl<T: Message + AuthenticatedMessage> Envelope<T, EmptySignInfo> {
    pub fn verify_signature(&self, verify_params: &VerifyParams) -> HaneulResult {
    ...
    ) -> HaneulResult<VerifiedEnvelope<T, EmptySignInfo>> {
    ...
    T: Message + Serialize,
{
        ...
        <T as Message>::DigestType: PartialEq,
    {
        ...
        self.auth_signature
            .verify_secure(self.data(), Intent::haneul_app(T::SCOPE), committee)
    }
}
...
{
    pub fn verify_authority_signatures(&self, committee: &Committee) -> HaneulResult {
        self.data.verify_epoch(self.auth_sig().epoch)?;
        self.auth_signature
            .verify_secure(self.data(), Intent::haneul_app(T::SCOPE), committee)
    }
    ...
    T: Message + AuthenticatedMessage + Serialize,
{
    ...
    pub fn verify_authenticated(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> HaneulResult<VerifiedEnvelope<T, AuthorityQuorumSignInfo<S>>> {
        self.verify_signatures_authenticated(committee, verify_params)?;
