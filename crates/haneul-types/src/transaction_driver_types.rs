...
use crate::error::{ErrorCategory, HaneulError};
...
pub enum TransactionSubmissionError {
    ...
    TransactionDriverInternalError(HaneulError),
    #[error("Invalid user signature: {0}.")]
    InvalidUserSignature(HaneulError),
    #[error(
...
pub type GroupedErrors = Vec<(HaneulError, StakeUnit, Vec<ConciseAuthorityPublicKeyBytes>)>;
