...
impl Committee {
    pub fn new(
        ...
        fp_ensure!(
            // Actual committee size is enforced in haneul_system.move.
            // This is just to ensure that choose_multiple_weighted can't fail.
            voting_rights.len() < u32::MAX.try_into().unwrap(),
            HaneulError::InvalidCommittee("committee has too many members".into())
        );

        fp_ensure!(
            !voting_rights.is_empty(),
            HaneulError::InvalidCommittee("committee has 0 members".into())
        );

        fp_ensure!(
            voting_rights.iter().any(|(_, s)| *s != 0),
            HaneulError::InvalidCommittee(
    ...
    pub fn public_key(&self, authority: &AuthorityName) -> HaneulResult<PublicKey> {
        match self.expanded_keys.get(authority) {
            // TODO: Check if this is unnecessary copying.
            Some(v) => Ok(v.clone()),
            None => (*authority).try_into().map_err(|_| {
                HaneulError::InvalidCommittee(format!("Authority #{} not found", authority))
            }),
        }
    }
