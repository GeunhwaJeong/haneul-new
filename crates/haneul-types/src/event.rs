...
use crate::{
    base_types::{ObjectID, SequenceNumber, HaneulAddress, TransactionDigest},
    committee::EpochId,
...
/// Unique ID of a Haneul Event, the ID is a combination of tx seq number and event seq number,
...
impl Event {
    ...
    pub fn sender(&self) -> Option<HaneulAddress> {
        match self {
    ...
    /// Extract a module name, if available, from a HaneulEvent
    ...
    /// Extracts the recipient from a HaneulEvent, if available
    ...
    /// Extracts the serialized recipient from a HaneulEvent, if available
    pub fn recipient_serialized(&self) -> Result<Option<String>, HaneulError> {
        match self.recipient() {
            Some(recipient) => {
                let res = serde_json::to_string(recipient);
                if let Err(e) = res {
                    error!("Failed to serialize recipient field of event: {:?}", self);
                    Err(HaneulError::OwnerFailedToSerialize {
                        error: (e.to_string()),
                    })
                } else {
                    Ok(res.ok())
    ...
    /// Extracts the bcs move content from a HaneulEvent, if available
    ...
    /// Extracts the move event name (StructTag) from a HaneulEvent, if available
    ...
    /// Extracts the TransferType from a HaneulEvent, if available
    ...
    /// Extracts the Object Version from a HaneulEvent, if available
    ...
    /// Extracts the digest from a HaneulEvent::Publish event
    ...
    /// Extracts the amount from a HaneulEvent::CoinBalanceChange event
    ...
    /// Extracts the balance change type from a HaneulEvent::CoinBalanceChange event
    ...
    pub fn balance_change_from_ordinal(ordinal: usize) -> Result<BalanceChangeType, HaneulError> {
