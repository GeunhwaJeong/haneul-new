...
This document specifies how **`Owner`** values control object versioning, execution path, PTB input arguments, and permissions in Haneul. It also describes the relation between `Owner` and the **Transfer‑to‑Object (TTO)** mechanism and the legal state‑transitions among owner kinds.
