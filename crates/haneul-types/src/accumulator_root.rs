...
impl AccumulatorOwner {
    ...
    pub fn derive_owner_address(owner: HaneulAddress) -> ObjectID {
    ...
    pub fn exists(object_store: &dyn ObjectStore, owner: HaneulAddress) -> HaneulResult<bool> {
        ...
        .map_err(|e| HaneulError::TypeError {
            error: format!("Failed to derive dynamic field id: {}", e),
        })?;
        Ok(object_store.get_object(&id).is_some())
    }

    pub fn load(object_store: &dyn ObjectStore, owner: HaneulAddress) -> HaneulResult<Self> {
        let key = OwnerKey { owner };
        get_dynamic_field_from_store(object_store, HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, &key)
    }
    ...
    pub fn load_metadata(
        ...
        object_store: &dyn ObjectStore,
        type_: &TypeTag,
    ) -> HaneulResult<AccumulatorMetadata> {
        let key = MetadataKey::default();
...
impl AccumulatorValue {
    pub fn get_field_id(owner: HaneulAddress, type_: &TypeTag) -> Result<ObjectID, bcs::Error> {
        let key = AccumulatorKey { owner };
        let key_bytes = bcs::to_bytes(&key).unwrap();
        derive_dynamic_field_id(HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, type_, &key_bytes)
    }
