...
fn coins<'a, I>(objects: I) -> impl Iterator<Item = (&'a HaneulAddress, TypeTag, u64)> + 'a
where
...
) -> Option<(HaneulAddress, TypeTag, i128)> {
...
) -> impl Iterator<Item = (HaneulAddress, TypeTag, i128)> + '_ {
...
) -> impl Iterator<Item = (HaneulAddress, TypeTag, i128)> {
    effects
        .accumulator_events()
        .into_iter()
...
mod tests {
    ...
    fn haneul_balance_type() -> TypeTag {
        Balance::type_tag("0x2::haneul::HANEUL".parse().unwrap())
    }
    ...
    fn get_accumulator_obj_id(address: HaneulAddress, balance_type: &TypeTag) -> ObjectID {
    ...
    fn test_derive_balance_changes_with_split_accumulator_event() {
        ...
        let balance_type = haneul_balance_type();
        ...
        assert_eq!(
            result[0].coin_type,
            "0x2::haneul::HANEUL".parse::<TypeTag>().unwrap()
        );
    ...
    fn test_derive_balance_changes_with_multiple_addresses() {
        let address1 = HaneulAddress::random_for_testing_only();
    ...
    fn create_gas_coin_object(owner: HaneulAddress, value: u64) -> Object {
    ...
    fn create_custom_coin_object(owner: HaneulAddress, coin_type: TypeTag, value: u64) -> Object {
        use crate::base_types::SequenceNumber;
        use crate::object::MoveObject;
    ...
    fn test_derive_balance_changes_with_coin_objects_only() {
        ...
        // Create input coin with 5000 HANEUL
        let input_coin = create_gas_coin_object(address, 5000);
        // Create output coin with 3000 HANEUL (spent 2000)
    ...
    fn test_derive_balance_changes_coin_transfer_between_addresses() {
        ...
        // Sender has 10000 HANEUL initially
    ...
    fn test_derive_balance_changes_combines_coins_and_accumulator_events() {
        ...
        // Coin: spent 2000 HANEUL (5000 -> 3000)
        let input_coin = create_gas_coin_object(address, 5000);
        let output_coin = create_gas_coin_object(address, 3000);

        // Accumulator: received 500 HANEUL via address balance
        let write = AccumulatorWriteV1 {
            address: AccumulatorAddress::new(address, balance_type),
            operation: AccumulatorOperation::Merge,
            value: AccumulatorValue::Integer(500),
    ...
    fn test_derive_balance_changes_coins_and_accumulator_different_addresses() {
        let coin_owner = HaneulAddress::random_for_testing_only();
        let accumulator_owner = HaneulAddress::random_for_testing_only();
        let balance_type = haneul_balance_type();
        let obj_id = get_accumulator_obj_id(accumulator_owner, &balance_type);

        // Coin owner spends 1000 HANEUL
        let input_coin = create_gas_coin_object(coin_owner, 5000);
        let output_coin = create_gas_coin_object(coin_owner, 4000);

        // Accumulator owner receives 2000 HANEUL
        let write = AccumulatorWriteV1 {
    ...
    fn test_derive_balance_changes_coins_and_accumulator_net_to_zero() {
        ...
        // Coin: spent 1000 HANEUL (5000 -> 4000)
        let input_coin = create_gas_coin_object(address, 5000);
        let output_coin = create_gas_coin_object(address, 4000);

        // Accumulator: received exactly 1000 HANEUL - perfectly offsets coin spend
        let write = AccumulatorWriteV1 {
            address: AccumulatorAddress::new(address, balance_type),
            operation: AccumulatorOperation::Merge,
            value: AccumulatorValue::Integer(1000),
    ...
    fn test_derive_balance_changes_different_coin_types() {
        ...
        // HANEUL coin: spent 1000
        let haneul_input = create_gas_coin_object(address, 5000);
        let haneul_output = create_gas_coin_object(address, 4000);
        ...
        let haneul_change = result
            .iter()
            .find(|c| c.coin_type == "0x2::haneul::HANEUL".parse::<TypeTag>().unwrap())
            .unwrap();
        let custom_change = result.iter().find(|c| c.coin_type == custom_type).unwrap();

        assert_eq!(haneul_change.amount, -1000);
