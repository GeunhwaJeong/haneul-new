...
use crate::error::{ExecutionError, HaneulError};
...
) -> HaneulResult<PackageFetchResults<PackageObject>> {
...
impl BackingPackageStore for TrackingBackingStore<'_> {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> crate::error::HaneulResult<Option<PackageObject>> {
...
impl ChildObjectResolver for TrackingBackingStore<'_> {
    fn read_child_object(
        ...
        child: &ObjectID,
        child_version_upper_bound: SequenceNumber,
    ) -> crate::error::HaneulResult<Option<Object>> {
        self.inner
