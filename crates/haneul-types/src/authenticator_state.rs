...
use crate::{HANEUL_AUTHENTICATOR_STATE_OBJECT_ID, HANEUL_FRAMEWORK_ADDRESS, id::UID};
...
pub const RESOLVED_HANEUL_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> HaneulResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&HANEUL_AUTHENTICATOR_STATE_OBJECT_ID);
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&HANEUL_AUTHENTICATOR_STATE_OBJECT_ID)
