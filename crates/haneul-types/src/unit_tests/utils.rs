...
pub fn create_fake_transaction() -> Transaction {
    ...
    let pt = {
        ...
        builder.transfer_haneul(recipient, None);
        builder.finish()
    };
...
fn make_transaction_data(sender: HaneulAddress) -> TransactionData {
    let object = Object::immutable_with_id_for_testing(ObjectID::random_from_rng(
        &mut StdRng::from_seed([0; 32]),
    ));
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_haneul(dbg_addr(2), None);
        builder.finish()
    };
...
pub fn make_transaction(sender: HaneulAddress, kp: &HaneulKeyPair, intent: Intent) -> Transaction {
...
pub fn to_sender_signed_transaction_with_multi_signers(
    ...
    Transaction::from_data_and_signer(data, Intent::haneul_transaction(), signers)
}
...
mod zk_login {
    ...
    pub fn get_zklogin_user_address() -> HaneulAddress {
        thread_local! {
            static USER_ADDRESS: HaneulAddress = {
                ...
                HaneulAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> HaneulKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> HaneulAddress {
        thread_local! {
            static USER_ADDRESS: HaneulAddress = {
                // Derive user address manually: Blake2b_256 hash of [zklogin_flag || iss_bytes_length || iss_bytes || address seed in bytes])
                let mut hasher = DefaultHash::default();
    ...
    pub fn make_zklogin_tx(legacy: bool) -> (HaneulAddress, Transaction, GenericSignature) {
        let data = if legacy {
    ...
    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (HaneulAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx(
        data: TransactionData,
        legacy: bool,
    ) -> (HaneulAddress, Transaction, GenericSignature) {
...
pub fn make_upgraded_multisig_tx() -> Transaction {
    ...
    let tx = make_transaction(addr, &keys[0], Intent::haneul_transaction());

    let msg = IntentMessage::new(Intent::haneul_transaction(), tx.transaction_data().clone());
