...
pub fn make_transaction_data(sender: HaneulAddress) -> TransactionData {
...
pub fn make_transaction(sender: HaneulAddress, kp: &HaneulKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn load_test_vectors(path: &str) -> Vec<(HaneulKeyPair, PublicKey, ZkLoginInputs)> {
        ...
        for test in test_datum {
            let kp = HaneulKeyPair::decode(&test.kp).unwrap();
            let inputs =
    ...
    pub fn get_legacy_zklogin_user_address() -> HaneulAddress {
        thread_local! {
            static USER_ADDRESS: HaneulAddress = {
                ...
                HaneulAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }
