...
mod zk_login {
    ...
    pub fn make_zklogin_tx() -> (HaneulAddress, Transaction, GenericSignature) {
        let data = make_transaction_data(get_zklogin_user_address());

        sign_zklogin_tx(data)
    }

    pub fn sign_zklogin_tx(data: TransactionData) -> (HaneulAddress, Transaction, GenericSignature) {
        // Sign the user transaction with the user's ephemeral key.
        //let tx = make_transaction(user_address, &user_key, Intent::haneul_transaction());
