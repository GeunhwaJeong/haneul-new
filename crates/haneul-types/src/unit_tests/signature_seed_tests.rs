...
fn test_deterministic_addresses_by_domain() {
    ...
    let haneul_address_0 = seed.new_deterministic_address::<AccountKeyPair>(&TEST_ID, Some(&domain_0));
    assert!(haneul_address_0.is_ok());

    let haneul_address_1 = seed.new_deterministic_address::<AccountKeyPair>(&TEST_ID, Some(&domain_1));
    assert!(haneul_address_1.is_ok());
...
fn test_deterministic_signing() {
    ...
    // Verify signatures.
    let ver_0 = sig_0_ok.verify(&msg0, haneul_address_0);
    assert!(ver_0.is_ok());
    ...
    let ver_0_with_address_1 = sig_0_ok.verify(&msg0, haneul_address_1);
    assert!(ver_0_with_address_1.is_err());

    // Ensure that signatures cannot be verified against another message.
    let ver_0_with_msg1 = sig_0_ok.verify(&msg1, haneul_address_0);
