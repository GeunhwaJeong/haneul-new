...
fn test_deterministic_addresses_by_id() {
    ...
    let haneul_address_0_0 = seed.new_deterministic_address(&id_0, Some(&TEST_DOMAIN));
    assert!(haneul_address_0_0.is_ok());

    let haneul_address_0_1 = seed.new_deterministic_address(&id_0, Some(&TEST_DOMAIN));
    assert!(haneul_address_0_1.is_ok());

    assert_eq!(haneul_address_0_0.unwrap(), haneul_address_0_1.clone().unwrap());

    // Create an address with a different ID and check that it differs from the previous one.
    let haneul_address_1_0 = seed.new_deterministic_address(&id_1, Some(&TEST_DOMAIN));
    assert!(haneul_address_1_0.is_ok());

    assert_ne!(haneul_address_0_1.unwrap(), haneul_address_1_0.unwrap());
}
...
fn test_deterministic_addresses_by_seed() {
    ...
    let haneul_address_0 = seed_0.new_deterministic_address(&TEST_ID, Some(&TEST_DOMAIN));
    assert!(haneul_address_0.is_ok());

    let haneul_address_1 = seed_1.new_deterministic_address(&TEST_ID, Some(&TEST_DOMAIN));
    assert!(haneul_address_1.is_ok());

    assert_ne!(haneul_address_0.unwrap(), haneul_address_1.unwrap());
}
...
fn test_deterministic_addresses_by_domain() {
    ...
    let haneul_address_0 = seed.new_deterministic_address(&TEST_ID, Some(&domain_0));
    assert!(haneul_address_0.is_ok());

    let haneul_address_1 = seed.new_deterministic_address(&TEST_ID, Some(&domain_1));
    assert!(haneul_address_1.is_ok());
...
fn test_deterministic_signing() {
    ...
    let haneul_address_0 = seed
        .new_deterministic_address(&id_0, Some(&TEST_DOMAIN))
        .unwrap();
    let haneul_address_1 = seed
    ...
    // Verify signatures.
    let ver_0 = sig_0_ok.clone().verify(&msg0, haneul_address_0);
    assert!(ver_0.is_ok());

    let ver_1 = sig_1.unwrap().verify(&msg0, haneul_address_1);
    assert!(ver_1.is_ok());

    // Ensure that signatures cannot be verified against another address.
    let ver_0_with_address_1 = sig_0_ok.clone().verify(&msg0, haneul_address_1);
    assert!(ver_0_with_address_1.is_err());

    // Ensure that signatures cannot be verified against another message.
    let ver_0_with_msg1 = sig_0_ok.clone().verify(&msg1, haneul_address_0);
