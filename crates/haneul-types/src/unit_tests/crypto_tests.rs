...
#[test]
fn serde_keypair() {
    let skp = HaneulKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let encoded = skp.encode().unwrap();
    assert_eq!(
        encoded,
        "haneulprivkey1qzdlfxn2qa2lj5uprl8pyhexs02sg2wrhdy7qaq50cqgnffw4c247jpzrfw"
    );
    let decoded = HaneulKeyPair::decode(&encoded).unwrap();
    assert_eq!(skp, decoded);
}

#[test]
fn serde_pubkey() {
    let skp = HaneulKeyPair::Ed25519(get_key_pair().1);
...
fn public_key_equality() {
    let ed_kp1: HaneulKeyPair = HaneulKeyPair::Ed25519(get_key_pair().1);
    let ed_kp2: HaneulKeyPair = HaneulKeyPair::Ed25519(get_key_pair().1);
    let k1_kp1: HaneulKeyPair = HaneulKeyPair::Secp256k1(get_key_pair().1);
    let k1_kp2: HaneulKeyPair = HaneulKeyPair::Secp256k1(get_key_pair().1);
...
proptest! {
    ...
    fn test_authority_pk_bytes(
        ...
        let _haneulsig = Ed25519HaneulSignature::from_bytes(&bytes);
        let _haneulsig = Secp256k1HaneulSignature::from_bytes(&bytes);
