...
use crate::crypto::{
    ...
    AuthoritySignInfoTrait, HaneulAuthoritySignature,
};
...
fn test_new_with_signatures() {
    ...
    assert_eq!(
        quorum
            ...
            .collect::<HaneulResult<Vec<&AuthorityName>>>()
            .unwrap(),
...
fn test_handle_reject_malicious_signature() {
        ...
        AuthorityStrongQuorumSignInfo::new_from_auth_sign_infos(signatures, &committee).unwrap();
    {
        ...
        let sig = AuthoritySignature::new_secure(
            &IntentMessage::new(Intent::haneul_transaction(), message.clone()),
            &committee.epoch,
            &sec,
        );
...
fn test_user_signature_committed_in_transactions() {
    ...
    let transaction_b =
        Transaction::from_data_and_signer(tx_data, Intent::haneul_transaction(), vec![&sender_sec2]);
    let transaction_c =
        Transaction::from_data_and_signer(tx_data_2, Intent::haneul_transaction(), vec![&sender_sec2]);
...
fn test_sponsored_transaction_message() {
    let sender_kp = HaneulKeyPair::Ed25519(get_key_pair().1);
    let sender = (&sender_kp.public()).into();
    let sponsor_kp = HaneulKeyPair::Ed25519(get_key_pair().1);
    ...
    let third_party_kp = HaneulKeyPair::Ed25519(get_key_pair().1);
...
fn test_sponsored_transaction_validity_check() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_haneul(HaneulAddress::random_for_testing_only(), Some(50000));
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_haneul(vec![], vec![]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_all_haneul(HaneulAddress::random_for_testing_only());
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
...
fn verify_sender_signature_correctly_with_flag() {
    ...
    let receiver_kp = HaneulKeyPair::Secp256k1(get_key_pair().1);
    let receiver_address = (&receiver_kp.public()).into();

    // create a sender keypair with Secp256k1
    let sender_kp = HaneulKeyPair::Secp256k1(get_key_pair().1);
    ...
    let sender_kp_2 = HaneulKeyPair::Ed25519(get_key_pair().1);
    let mut tx_data_2 = tx_data.clone();
    *tx_data_2.sender_mut_for_testing() = (&sender_kp_2.public()).into();
    ...
    let sender_kp_3 = HaneulKeyPair::Secp256r1(get_key_pair().1);
    ...
    *tx_data_3.sender_mut_for_testing() = (&sender_kp_3.public()).into();
    ...
    let transaction =
        Transaction::from_data_and_signer(tx_data, Intent::haneul_transaction(), vec![&sender_kp])
            .verify(&Default::default())
            .unwrap();
    ...
    assert_eq!(s.scheme().flag(), Secp256k1HaneulSignature::SCHEME.flag());
    ...
    let transaction_1 =
        Transaction::from_data_and_signer(tx_data_2, Intent::haneul_transaction(), vec![&sender_kp_2])
            .verify(&Default::default())
            .unwrap();
    ...
    assert_eq!(s.scheme().flag(), Ed25519HaneulSignature::SCHEME.flag());
    ...
    let tx_3 =
        Transaction::from_data_and_signer(tx_data_3, Intent::haneul_transaction(), vec![&sender_kp_3]);
