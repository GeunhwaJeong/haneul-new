...
use crate::crypto::{
    get_key_pair, AccountKeyPair, AuthorityKeyPair, AuthorityPublicKeyBytes, HaneulAuthoritySignature,
};
...
fn test_certificates() {
    ...
    let mut sigs = vec![v1.auth_sign_info.clone()];
    assert!(CertifiedTransaction::new_with_auth_sign_infos(
        transaction.clone(),
        sigs.clone(),
        &committee
    )
    ...
    assert!(CertifiedTransaction::new_with_auth_sign_infos(transaction, sigs, &committee).is_err());
}
...
fn verify_sender_signature_correctly_with_flag() {
    ...
    let receiver_kp = HaneulKeyPair::Secp256k1HaneulKeyPair(get_key_pair().1);
    let receiver_address = (&receiver_kp.public()).into();

    // create a sender keypair with Secp256k1
    let sender_kp = HaneulKeyPair::Secp256k1HaneulKeyPair(get_key_pair().1);

    // create a sender keypair with Ed25519
    let sender_kp_2 = HaneulKeyPair::Ed25519HaneulKeyPair(get_key_pair().1);
