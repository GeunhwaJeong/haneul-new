...
fn test_sponsored_transaction_message() {
    ...
    let thrid_party_kp = HaneulKeyPair::Ed25519(get_key_pair().1);
...
fn test_sponsored_transaction_validity_check() {
    ...
    // TransferHaneul cannot be sponsored
    assert_eq!(
        TransactionData::new_with_gas_data(
            TransactionKind::Single(SingleTransactionKind::TransferHaneul(TransferHaneul {
        ...
        .validity_check()
        .unwrap_err(),
        HaneulError::UnsupportedSponsoredTransactionKind
    );

    // PayHaneul cannot be sponsored
    assert_eq!(
        TransactionData::new_with_gas_data(
            TransactionKind::Single(SingleTransactionKind::PayHaneul(PayHaneul {
    ...
    // PayAllHaneul cannot be sponsored
    assert_eq!(
        TransactionData::new_with_gas_data(
            TransactionKind::Single(SingleTransactionKind::PayAllHaneul(PayAllHaneul {
                coins: vec![(gas_data.payment)],
                recipient: HaneulAddress::random_for_testing_only(),
            })),
            sender,
