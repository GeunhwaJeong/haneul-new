...
use crate::crypto::{PublicKey, Signature, SignatureScheme, HaneulKeyPair, ZkLoginPublicIdentifier};
...
use crate::{
    base_types::HaneulAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn zklogin_authenticator_jwk() {
    ...
    // Create haneul address derived with a legacy way.
...
fn test_serde_zk_login_signature() {
    ...
    let addr: HaneulAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        HaneulAddress::try_from_unpadded(&modified_inputs).unwrap(),
        HaneulAddress::from(&pk1)
    );
    ...
    assert_eq!(
        HaneulAddress::try_from_padded(&modified_inputs).unwrap(),
        HaneulAddress::from(&pk2)
    );
