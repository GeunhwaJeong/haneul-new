...
#[test]
fn test_bcs_enum() {
    let address = Owner::AddressOwner(HaneulAddress::random_for_testing_only());
...
fn test_signatures() {
    ...
    let foo = IntentMessage::new(Intent::haneul_transaction(), Foo("hello".into()));
    let foox = IntentMessage::new(Intent::haneul_transaction(), Foo("hellox".into()));
...
fn test_address_serde_with_expected_value() {
    let address = HaneulAddress::try_from(SAMPLE_ADDRESS_VEC.to_vec()).unwrap();
...
fn test_authority_signature_serde_not_human_readable() {
    ...
    let sig = AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::haneul_transaction(), Foo("some data".to_string())),
        &0,
        &key,
    );
...
fn derive_sample_address() -> (HaneulAddress, AccountKeyPair) {
...
fn move_object_type_consistency() {
    ...
    let ty = assert_consistent(&StakedHaneul::type_());
    assert!(ty.is_staked_haneul());
