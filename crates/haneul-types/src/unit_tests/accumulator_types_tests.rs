...
#[test]
fn test_haneul_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<HANEUL>>, U128> type
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[haneul_balance]);
    ...
    assert!(move_type.is_haneul_balance_accumulator_field());
...
#[test]
fn test_non_haneul_balance_accumulator_field_recognition() {
    ...
    // Should be recognized as a balance accumulator field, but not HANEUL
    assert!(move_type.is_balance_accumulator_field());
    assert!(!move_type.is_haneul_balance_accumulator_field());
...
fn test_accumulator_storage_savings() {
    ...
    // Create the field struct tag for Field<Key<Balance<HANEUL>>, U128>
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[haneul_balance]);
    let u128_type = U128::get_type_tag();
    ...
    assert!(
        efficient_move_object
            .type_()
            .is_haneul_balance_accumulator_field()
    );
}
...
#[test]
fn test_haneul_balance_accumulator_metadata_field_recognition() {
    // Create a Field<MetadataKey<Balance<HANEUL>>, Metadata<Balance<HANEUL>>> type
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = MetadataKey::get_type_tag(std::slice::from_ref(&haneul_balance));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[haneul_balance]);
    ...
    // Should extract the correct balance type (which should be HANEUL)
    let balance_type = move_type
        .balance_accumulator_metadata_field_type_maybe()
        .expect("Should have balance type");
    assert_eq!(balance_type, GAS::type_tag());

    // Since the balance type is HANEUL, this should be a HaneulBalanceAccumulatorMetadataField
    assert!(
        matches!(
            move_type.0,
            crate::base_types::MoveObjectType_::HaneulBalanceAccumulatorMetadataField
        ),
        "Should be HaneulBalanceAccumulatorMetadataField variant"
    );
}

#[test]
fn test_non_haneul_balance_accumulator_metadata_field_recognition() {
    // Create a custom token type
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: HANEUL_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
    ...
    // Should be recognized as a balance accumulator metadata field, but not HANEUL
    assert!(move_type.is_balance_accumulator_metadata_field());
    ...
    // Since the balance type is not HANEUL, this should be a BalanceAccumulatorMetadataField
    assert!(
        matches!(
            move_type.0,
...
fn test_accumulator_metadata_storage_savings() {
    // Create the field struct tag for Field<MetadataKey<Balance<HANEUL>>, Metadata<Balance<HANEUL>>>
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = MetadataKey::get_type_tag(std::slice::from_ref(&haneul_balance));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[haneul_balance]);
    ...
    // Specifically verify it's the HANEUL variant
    assert!(
        matches!(
...
fn test_accumulator_field_wrong_value_type_not_u128() {
    // Field<Key<Balance<HANEUL>>, SomeOtherType> should NOT be recognized
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[haneul_balance]);
...
fn test_accumulator_field_wrong_key_module() {
    ...
    let wrong_key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        type_params: vec![haneul_balance],
    }));
    let u128_type = U128::get_type_tag();
...
fn test_accumulator_field_key_with_multiple_type_params() {
    // Key<Balance<HANEUL>, ExtraParam> should not be recognized
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    ...
    let key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        module: "accumulator".parse().unwrap(),
        name: "Key".parse().unwrap(),
        type_params: vec![haneul_balance, extra_param],
    }));
    let u128_type = U128::get_type_tag();
...
fn test_accumulator_field_with_nested_balance_type() {
    // Balance<Balance<HANEUL>> - nested balance should still work correctly
    ...
    // Should recognize as accumulator field with Balance<Balance<HANEUL>> as the extracted type
    assert!(move_type.is_balance_accumulator_field());
    let extracted = move_type.balance_accumulator_field_type_maybe().unwrap();
    // The extracted type should be Balance<HANEUL>, which is the inner type of Balance<Balance<HANEUL>>
...
fn test_metadata_field_mismatched_key_value_types() {
    // Field<MetadataKey<Balance<HANEUL>>, Metadata<Balance<CustomToken>>> should NOT be recognized
...
fn test_metadata_field_wrong_key_module() {
    ...
    let wrong_key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        type_params: vec![haneul_balance.clone()],
    }));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[haneul_balance]);
...
#[test]
fn test_is_method_haneul_balance_accumulator_field() {
    let haneul_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[haneul_balance]);
    let u128_type = U128::get_type_tag();
    let field_type = DynamicFieldInfo::dynamic_field_type(key_type, u128_type);
...
#[test]
fn test_is_method_non_haneul_balance_accumulator_field() {
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: HANEUL_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
        name: "CustomToken".parse().unwrap(),
