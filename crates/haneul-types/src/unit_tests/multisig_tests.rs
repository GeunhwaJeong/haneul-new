...
fn multisig_scenarios() {
    ...
    let addr_legacy_2 = HaneulAddress::from(&multisig_pk_legacy_2);
...
fn multisig_legacy_serde_test() {
    ...
    let addr: HaneulAddress = (&multisig_pk_legacy).into();
...
fn multisig_zklogin_scenarios() {
    ...
    for test in test_datum {
        let kp = HaneulKeyPair::decode_base64(&test.kp).unwrap();
