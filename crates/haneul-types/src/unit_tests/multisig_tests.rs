...
fn multisig_scenarios() {
    ...
    .unwrap();
    let addr = HaneulAddress::from(multisig_pk.clone());
    ...
    .unwrap();
    let addr_2 = HaneulAddress::from(multisig_pk_2.clone());
...
fn test_multisig_address() {
    ...
    assert_eq!(
        HaneulAddress::from_str("0x43247fc96101763052ccc4f0417257a4270a537c45576ec6cd631c712085f389")
            .unwrap(),
        address
    );
}
...
fn test_max_sig() {
    ...
    let mut seed = StdRng::from_seed([0; 32]);
    let mut keys: Vec<HaneulKeyPair> = Vec::new();
    for _ in 0..10 {
        keys.push(HaneulKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1));
    }
    ...
    .unwrap();
    let address: HaneulAddress = high_threshold_pk.clone().into();
    ...
    .unwrap();
    let address: HaneulAddress = low_threshold_pk.clone().into();
    let sig = Signature::new_secure(&msg, &keys[0]);
