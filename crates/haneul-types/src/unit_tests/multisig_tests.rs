...
pub fn keys() -> Vec<HaneulKeyPair> {
    ...
    let kp1: HaneulKeyPair = HaneulKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
    let kp2: HaneulKeyPair = HaneulKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
    let kp3: HaneulKeyPair = HaneulKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
    vec![kp1, kp2, kp3]
}
