...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, CompressedSignature, DefaultHash, Ed25519HaneulSignature,
        PublicKey, Signature, HaneulKeyPair, HaneulSignatureInner, ZkLoginAuthenticatorAsBytes,
...
fn multisig_scenarios() {
    ...
    .unwrap();
    let addr = HaneulAddress::from(&multisig_pk);
    ...
    let kp4: HaneulKeyPair = HaneulKeyPair::Secp256r1(get_key_pair().1);
    let pk4 = kp4.public();
    ...
    .unwrap();
    let addr_2 = HaneulAddress::from(&multisig_pk_2);
...
fn test_combine_sigs() {
    let kp1: HaneulKeyPair = HaneulKeyPair::Ed25519(get_key_pair().1);
    let kp2: HaneulKeyPair = HaneulKeyPair::Secp256k1(get_key_pair().1);
    let kp3: HaneulKeyPair = HaneulKeyPair::Secp256r1(get_key_pair().1);
...
fn test_serde_roundtrip() {
    ...
    let sig = Ed25519HaneulSignature::default();
    ...
    assert_eq!(single_sig_bytes.len(), Ed25519HaneulSignature::LENGTH);
    assert_eq!(
...
fn single_sig_port_works() {
    let kp: HaneulKeyPair = HaneulKeyPair::Ed25519(get_key_pair().1);
    let addr = HaneulAddress::from(&kp.public());
    let msg = IntentMessage::new(
        Intent::haneul_transaction(),
        PersonalMessage {
            message: "Hello".as_bytes().to_vec(),
...
fn test_max_sig() {
    ...
    for _ in 0..11 {
        let k = HaneulKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
    ...
    .unwrap();
    let address: HaneulAddress = (&high_threshold_pk).into();
    ...
    .unwrap();
    let address: HaneulAddress = (&low_threshold_pk).into();
...
fn multisig_serde_test() {
    let k1 = HaneulKeyPair::Ed25519(Ed25519KeyPair::from(
    ...
    let k2 = HaneulKeyPair::Secp256k1(Secp256k1KeyPair::from(
    ...
    let k3 = HaneulKeyPair::Ed25519(Ed25519KeyPair::from(
    ...
    assert_eq!(
        addr,
        HaneulAddress::from_str("0x37b048598ca569756146f4e8ea41666c657406db154a31f11bb5c1cbaf0b98d7")
            .unwrap()
    );
}
...
fn multisig_legacy_serde_test() {
    ...
    .unwrap();
    let multisig_pk = MultiSigPublicKey::new(vec![pk1, pk2, pk3], vec![1, 2, 3], 3).unwrap();
    let addr: HaneulAddress = (&multisig_pk).into();
...
fn multisig_invalid_instance() {
    ...
    let addr = HaneulAddress::from(&invalid_multisig_pk);
...
fn multisig_zklogin_scenarios() {
    ...
    let skp: HaneulKeyPair = HaneulKeyPair::Ed25519(kp);
    let pk1 = skp.public();
    ...
    let intent_msg = &IntentMessage::new(Intent::haneul_transaction(), tx.clone());
    ...
    let zklogin_addr = HaneulAddress::try_from(&zklogin_sig).unwrap();
    let multisig = MultiSig::combine(vec![zklogin_sig.clone()], multisig_pk.clone()).unwrap();
    assert!(multisig
    ...
    .unwrap();
    let bad_addr = HaneulAddress::from(&bad_multisig_pk);
    ...
    let multisig_address = HaneulAddress::from(&multisig_pk);
    let tx_data = make_transaction_data(multisig_address);
    let msg = IntentMessage::new(Intent::haneul_transaction(), tx_data);
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    assert!(
        matches!(res, Err(crate::error::HaneulError::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
    ...
    let multisig_address_padded = HaneulAddress::from(&multisig_pk_padded);
