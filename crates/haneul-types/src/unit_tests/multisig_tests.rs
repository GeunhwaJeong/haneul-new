...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, Ed25519HaneulSignature, PublicKey, Signature, HaneulKeyPair,
        HaneulSignatureInner, ZkLoginPublicIdentifier,
    },
...
fn multisig_zklogin_scenarios() {
    // consistency test with haneul/sdk/typescript/test/unit/cryptography/multisig.test.ts
    let mut seed = StdRng::from_seed([0; 32]);
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    assert!(
        matches!(res, Err(crate::error::HaneulErrorKind::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
