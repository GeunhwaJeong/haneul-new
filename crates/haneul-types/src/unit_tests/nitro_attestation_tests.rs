...
fn attestation_parse_and_verify() {
    let parsed =
        parse_nitro_attestation(&Hex::decode(FIXED_VALID_ATTESTATION).unwrap(), true).unwrap();
    ...
    assert_eq!(
        res.unwrap_err(),
        HaneulErrorKind::NitroAttestationFailedToVerify(
...
fn test_with_malformed_attestation() {
    ...
    assert!(matches!(
        err,
        HaneulErrorKind::NitroAttestationFailedToVerify(msg) if msg.starts_with("InvalidCoseSign1")
    ));
}
...
fn test_attestation_fields_validity() {
    ...
    let res = AttestationDocument::validate_document_map(&map, true);
    assert_eq!(
        res.unwrap_err(),
...
fn bad_signature_cose() {
    ...
    assert_eq!(
        res.unwrap_err(),
        HaneulErrorKind::NitroAttestationFailedToVerify("InvalidSignature".to_string())
    );
}
