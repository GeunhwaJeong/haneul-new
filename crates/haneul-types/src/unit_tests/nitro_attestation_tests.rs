...
fn attestation_parse_and_verify() {
    ...
    assert_eq!(
        res.unwrap_err(),
        HaneulError::NitroAttestationFailedToVerify(
...
fn test_with_malformed_attestation() {
    ...
    assert!(matches!(
        err,
        HaneulError::NitroAttestationFailedToVerify(msg) if msg.starts_with("InvalidCoseSign1")
    ));
}
...
fn bad_signature_cose() {
    ...
    assert_eq!(
        res.unwrap_err(),
        HaneulError::NitroAttestationFailedToVerify("InvalidSignature".to_string())
    );
}
