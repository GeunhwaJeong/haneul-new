...
use crate::{
    base_types::{dbg_addr, ObjectID, HaneulAddress},
...
async fn create_credential_and_sign_test_tx(
    ...
    // Derives its haneul address and make a test transaction with it as sender.
...
async fn test_passkey_fails_invalid_json() {
    ...
    let res: Result<PasskeyAuthenticator, HaneulError> = raw.try_into();
    ...
    let res_2: Result<PasskeyAuthenticator, HaneulError> = raw_2.try_into();
    assert!(res_2.is_err());
    ...
    let res_3: Result<PasskeyAuthenticator, HaneulError> = raw_3.try_into();
    assert!(res_3.is_ok());
}
...
//     // crafts a particular not normalized signature, fails to verify. this is produced from typescript client https://github.com/joyqvq/haneul-webauthn-poc/tree/joy/tx-example
...
//         intent_msg: IntentMessage::new(Intent::haneul_transaction(), tx_data),
//         sender: HaneulAddress::from_str("0x769364b73418667a44d600a8f0bc2d6655ef11cefb9e337fbd5aad5d4cc0552d").unwrap()
//     };
//     let sig = GenericSignature::PasskeyAuthenticator(
//         PasskeyAuthenticator::new_for_testing(
//             response.authenticator_data,
...
//     // response from a real passkey authenticator created in iCloud, from typescript client: https://github.com/joyqvq/haneul-webauthn-poc/tree/joy/tx-example
//     let address =
//         HaneulAddress::from_str("0xac8564f638fbf673fc92eb85b5abe5f7c29bdaa60a4a10329868fbe6c551dda2")
//             .unwrap();
