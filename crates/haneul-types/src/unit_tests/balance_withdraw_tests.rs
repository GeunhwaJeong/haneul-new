...
/// A mock resolver that always returns a HANEUL withdrawal with the parsed amount
struct MockHaneulResolver;

impl CoinReservationResolverTrait for MockHaneulResolver {
    fn resolve_funds_withdrawal(
        &self,
...
fn test_process_withdrawals_includes_implicit_gas() {
    ...
    let haneul_account_id = AccumulatorValue::get_field_id(
        sender,
        &WithdrawalTypeArg::Balance(GAS::type_tag()).to_type_tag(),
    )
    .unwrap();
    assert_eq!(withdrawals.len(), 1);
    assert_eq!(withdrawals.get(&haneul_account_id).unwrap().0, 10_000_000);
}
...
fn test_process_withdrawals_includes_coin_reservations_in_gas() {
    ...
    let withdrawals = tx
        .process_funds_withdrawals_for_signing(chain_id, &MockHaneulResolver)
        .unwrap();
    ...
    .unwrap();
    assert_eq!(withdrawals.len(), 1);
    assert_eq!(withdrawals.get(&haneul_account_id).unwrap().0, 5000);
}
...
fn test_process_withdrawals_includes_coin_reservations_in_ptb_inputs() {
    ...
    .unwrap();
    assert_eq!(withdrawals.len(), 1);
    assert_eq!(withdrawals.get(&haneul_account_id).unwrap().0, 7500);
}
