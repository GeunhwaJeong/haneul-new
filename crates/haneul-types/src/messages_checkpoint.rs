...
use crate::{
    ...
    committee::Committee,
    crypto::{sha3_hash, AuthoritySignature, HaneulAuthoritySignature, VerificationObligation},
    error::HaneulError,
};
    ...
    pub fn verify(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                HaneulError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.summary, content_digest, contents.transactions.len())}
            );
        }
...
impl CertifiedCheckpointSummary {
    ...
    pub fn verify(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                content_digest == self.summary.content_digest,
                HaneulError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, content digest = {:?}, transactions {}", self.summary, content_digest, contents.transactions.len())}
            );
        }
...
// https://github.com/GeunhwaJeong/haneul/issues/3038
