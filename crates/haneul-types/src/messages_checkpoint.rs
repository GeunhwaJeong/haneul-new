                ...
                if let Some(p) = proposal {
                    ...
                    if p.summary.sequence_number > 0 {
                        let cert = prev_cert.as_ref().ok_or_else(|| {
                            HaneulError::from("No checkpoint cert provided along with proposal")
                        })?;
                        cert.verify(committee, None)?;
                        fp_ensure!(
                            p.summary.sequence_number - 1 == cert.summary.sequence_number,
                            HaneulError::from("Checkpoint proposal sequence number inconsistent with previous cert")
                        );
                    }
                }
                Ok(())
    ...
    pub fn verify(
        ...
        if let Some(contents) = contents {
            fp_ensure!(
                ...
                HaneulError::from("Checkpoint contents digest mismatch")
            );
        }
