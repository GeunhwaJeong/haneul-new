...
use crate::crypto::{
    AuthoritySignInfo, AuthoritySignInfoTrait, AuthorityWeakQuorumSignInfo, HaneulAuthoritySignature,
};
...
impl AuthenticatedCheckpoint {
    ...
    pub fn verify(&self, committee: &Committee, detail: Option<&CheckpointContents>) -> HaneulResult {
        match self {
    ...
    pub fn verify(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                HaneulError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.summary, content_digest, contents.size())}
            );
        }
...
impl CertifiedCheckpointSummary {
    ...
    pub fn aggregate(
        ...
        committee: &Committee,
    ) -> Result<CertifiedCheckpointSummary, HaneulError> {
        fp_ensure!(
            !signed_checkpoints.is_empty(),
            HaneulError::from("Need at least one signed checkpoint to aggregate")
        );
        fp_ensure!(
            signed_checkpoints
                .iter()
                .all(|c| c.summary.epoch == committee.epoch),
            HaneulError::from("SignedCheckpoint is from different epoch as committee")
        );
    ...
    pub fn verify(
        ...
        fp_ensure!(
            self.summary.epoch == committee.epoch,
            HaneulError::from("Epoch in the summary doesn't match with the committee")
        );
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                content_digest == self.summary.content_digest,
                HaneulError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, content digest = {:?}, transactions {}", self.summary, content_digest, contents.size())}
            );
        }
    ...
    pub fn verify(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                HaneulError::from("Checkpoint proposal content doesn't match with the summary")
            );
        }
        Ok(())
    }
...
impl CheckpointFragment {
    pub fn verify(&self, committee: &Committee) -> HaneulResult {
        fp_ensure!(
            ...
            HaneulError::from("Proposer and other have inconsistent sequence number")
        );
        ...
        fp_ensure!(
            ...
            HaneulError::from("Waypoint diff and checkpoint summary inconsistent")
        );

        // Check consistency of waypoint diff
        fp_ensure!(
            self.data.diff.check(),
            HaneulError::from("Waypoint diff is not valid")
        );
        ...
        for digest in digests {
            let cert = self.data.certs.get(digest).ok_or_else(|| {
                HaneulError::from(format!("Missing cert with digest {digest:?}").as_str())
            })?;
...
impl SignedCheckpointFragmentMessage {
    ...
    pub fn verify(&self, epoch: EpochId) -> HaneulResult {
...
impl PartialCheckpointFragment {
    ...
    pub fn add_chunk(&mut self, chunk: CheckpointFragmentMessageChunk) -> HaneulResult {
        fp_ensure!(
    ...
    pub fn to_fragment(self) -> HaneulResult<CheckpointFragment> {
        fp_ensure!(
            self.is_complete(),
            HaneulError::from("Fragment is missing chunks")
        );
        let Self {
            proposer,
            other,
        ...
        let data: CheckpointFragmentData = deserialize(&content).map_err(|err| {
            HaneulError::from(format!("Failed to deserialize chunk data: {:?}", err).as_str())
        })?;
