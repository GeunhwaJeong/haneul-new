...
impl SignedCheckpointSummary {
    ...
    pub fn verify_with_transactions(&self, contents: &CheckpointContents) -> Result<(), HaneulError> {
        self.verify()?;
        ...
        fp_ensure!(
            ...
            HaneulError::from("Transaction digest mismatch")
        );
        Ok(())
    }
}
...
impl CheckpointFragment {
    pub fn verify(&self, _committee: &Committee) -> Result<(), HaneulError> {
        ...
        fp_ensure!(
            ...
            HaneulError::from("Authorities not in the committee")
        );
