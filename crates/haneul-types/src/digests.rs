...
pub const MAINNET_CHAIN_IDENTIFIER_BASE58: &str = "BmesG5C6R15WTWTL4feVxmf51fHXDSCMjmrF571aLiKC";
...
pub const HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str =
    "HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE";
...
static HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        ...
        match s.as_str() {
            ...
            "" => None,
            _ => panic!("unrecognized HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    ...
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
impl TransactionDigest {
    ...
    // TODO(https://github.com/GeunhwaJeong/haneul/issues/65): we can pick anything here
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::error::HaneulError;
...
impl CheckpointArtifactsDigest {
    ...
    pub fn from_artifact_digests(digests: Vec<Digest>) -> HaneulResult<Self> {
        let bytes =
            bcs::to_bytes(&digests).map_err(|e| HaneulError::from(format!("BCS error: {}", e)))?;
