...
use crate::crypto::{
    ...
    Ed25519HaneulSignature, EmptySignInfo, Signature, Signer, HaneulSignatureInner, ToFromBytes,
};
...
use crate::{
    HANEUL_CLOCK_OBJECT_ID, HANEUL_CLOCK_OBJECT_SHARED_VERSION, HANEUL_SYSTEM_STATE_OBJECT_ID,
...
impl VersionedProtocolMessage for SingleTransactionKind {
    fn check_version_supported(&self, _protocol_config: &ProtocolConfig) -> HaneulResult {
...
impl SingleTransactionKind {
    ...
    pub fn validity_check(&self, config: &ProtocolConfig, gas: &[ObjectRef]) -> UserInputResult {
        match self {
            ...
            SingleTransactionKind::PayHaneul(p) => p.validity_check(config, gas)?,
            SingleTransactionKind::PayAllHaneul(pa) => pa.validity_check(config, gas)?,
...
impl TransactionData {
    ...
    pub fn new_transfer_haneul_with_dummy_gas_price(
        recipient: HaneulAddress,
        sender: HaneulAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
    ...
    pub fn new_transfer_haneul(
        ...
        let kind = TransactionKind::Single(SingleTransactionKind::TransferHaneul(TransferHaneul {
            recipient,
            amount,
        }));
        Self::new(kind, sender, gas_payment, gas_budget, gas_price)
    ...
    pub fn new_pay_haneul_with_dummy_gas_price(
        sender: HaneulAddress,
        coins: Vec<ObjectRef>,
        recipients: Vec<HaneulAddress>,
        amounts: Vec<u64>,
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> Vec<HaneulAddress>;
...
impl SenderSignedData {
    ...
    fn get_signer_sig_mapping(&self) -> HaneulResult<BTreeMap<HaneulAddress, &GenericSignature>> {
...
pub struct ObjectInfoResponse {
    ...
    /// This should be only used for debugging purpose, such as from haneul-tool. No prod clients should
    /// rely on it.
...
pub enum ExecutionFailureStatus {
    ...
    InvalidTransferHaneul,
    InvalidTransferHaneulInsufficientBalance,
    InvalidCoinObject,
...
impl ExecutionFailureStatus {
    ...
    pub fn invalid_child_object_argument(child: ObjectID, parent: HaneulAddress) -> Self {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExecutionFailureStatus::InvalidGasObject => {
                write!(
                    f,
                    "Invalid Gas Object. Possibly not address-owned or possibly not a HANEUL coin."
                )
            }
            ...
            ExecutionFailureStatus::InvalidTransferHaneul => write!(
                f,
                "Invalid Transfer HANEUL. \
                Possibly not address-owned or possibly not a HANEUL coin."
            ),
            ExecutionFailureStatus::InvalidTransferHaneulInsufficientBalance => {
                write!(f, "Invalid Transfer HANEUL, Insufficient Balance.")
            }
            ...
            ExecutionFailureStatus::HaneulMoveVerificationError => write!(
                f,
...
impl VersionedProtocolMessage for TransactionEffects {
    ...
    fn check_version_supported(&self, protocol_config: &ProtocolConfig) -> HaneulResult {
        ...
        if supported.is_version_supported(protocol_config.version) {
            ...
            Err(HaneulError::WrongMessageVersion {
                error: format!(
...
pub struct ConsensusTransaction {
    /// Encodes an u64 unique tracking id to allow us trace a message between Haneul and Narwhal.
...
impl AuthorityCapabilities {
    pub fn new(
        ...
        let generation = SystemTime::now()
            ...
            .expect("Haneul did not exist prior to 1970")
            .as_millis()
            .try_into()
            .expect("This build of haneul is not supported in the year 500,000,000");
        Self {
            authority,
            generation,
            supported_protocol_versions,
