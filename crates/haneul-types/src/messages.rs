...
use crate::crypto::{
    ...
    SignatureScheme, HaneulAuthoritySignature, HaneulSignature, HaneulSignatureInner, ToFromBytes,
...
/// universally in the transactions storage in `HaneulDataStore`, shared by both authorities
...
impl<S> TransactionEnvelope<S> {
    ...
    pub fn verify_sender_signature(&self) -> HaneulResult<()> {
        if self.is_verified || self.signed_data.data.kind.is_system_tx() {
            return Ok(());
        }
        self.signed_data
    ...
    T: Deserialize<'de>,
{
        ...
        D: serde::de::Deserializer<'de>,
    {
        TransactionEnvelope::deserialize(DeserializeNameAdapter::new(
            ...
            // https://github.com/GeunhwaJeong/haneul/issues/1119
...
impl Transaction {
    ...
    pub fn verify(&self) -> Result<(), HaneulError> {
...
impl SignedTransaction {
    ...
    pub fn new_change_epoch(
        ...
        let signed_data = SenderSignedData {
            data,
            // Arbitrary keypair
            tx_signature: Ed25519HaneulSignature::from_bytes(&[0; Ed25519HaneulSignature::LENGTH])
                .unwrap()
                .into(),
        };
...
impl From<HaneulAddress> for AccountInfoRequest {
    fn from(account: HaneulAddress) -> Self {
...
impl ExecutionFailureStatus {
    ...
    pub fn missing_object_owner(child: ObjectID, parent: HaneulAddress) -> Self {
...
impl InputObjectKind {
    ...
    pub fn object_not_found_error(&self) -> HaneulError {
        match *self {
            ...
            Self::SharedMoveObject(object_id) => HaneulError::ObjectNotFound { object_id },
        }
    }
}
...
impl<'a> SignatureAggregator<'a> {
    ...
    pub fn try_new(transaction: Transaction, committee: &'a Committee) -> Result<Self, HaneulError> {
        transaction.verify()?;
...
impl CertifiedTransaction {
    ...
    pub fn verify(&self, committee: &Committee) -> Result<(), HaneulError> {
        // We use this flag to see if someone has checked this before
        // and therefore we can skip the check. Note that the flag has
        // to be set to true manually, and is not set by calling this
        // "check" function.
...
impl SignedEpoch {
    ...
    pub fn verify(&self, prev_epoch_committee: &Committee) -> HaneulResult {
