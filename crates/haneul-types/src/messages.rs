...
impl EpochInfo {
    pub fn new(
        ...
        fp_ensure!(
            ...
            HaneulError::from("Current epoch number and next epoch number are inconsistent")
        );
        Ok(Self {
            epoch,
    ...
    pub fn verify(&self) -> HaneulResult {
        let next_epoch = self.next_epoch_committee.epoch;
        fp_ensure!(
            next_epoch > 0 && next_epoch - 1 == self.epoch,
            HaneulError::from("Epoch number mismatch in epoch info")
        );
        Ok(())
    }
}
