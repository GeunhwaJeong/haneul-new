...
use crate::{
    error::HaneulResult, object::Owner, storage::ObjectStore, HANEUL_FRAMEWORK_ADDRESS,
...
pub const RESOLVED_HANEUL_RANDOMNESS_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
pub fn is_mutable_random(view: &CompiledModule, s: &SignatureToken) -> bool {
    match s {
        ...
        SignatureToken::Struct(idx) => resolve_struct(view, *idx) == RESOLVED_HANEUL_RANDOMNESS_STATE,
        _ => false,
    }
}
