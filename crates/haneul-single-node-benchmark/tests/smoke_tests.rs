...
async fn benchmark_send_funds_smoke_test() {
    ...
    let chain = match std::env::var("HANEUL_PROTOCOL_CONFIG_CHAIN_OVERRIDE").as_deref() {
        Ok("mainnet") => haneul_protocol_config::Chain::Mainnet,
        Ok("testnet") => haneul_protocol_config::Chain::Testnet,
        _ => haneul_protocol_config::Chain::Unknown,
    };
    let protocol_config = haneul_protocol_config::ProtocolConfig::get_for_version(
        haneul_protocol_config::ProtocolVersion::MAX,
        chain,
    );
