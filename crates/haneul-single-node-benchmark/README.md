# Haneul Single Node Benchmark

This crate contains a binary for performance benchmarking a single Haneul node.
...
```
cargo run --release --bin haneul-single-node-benchmark -- move
```
or
```
cargo run --release --bin haneul-single-node-benchmark -- no-move
...
```
cargo run --release --bin haneul-single-node-benchmark -- --component baseline <move/no-move>
```
...
### Profiling
If you are interested in profiling Haneul, you can start the benchmark, wait for it to print out "Started execution...", and then attach a profiler to the process.
