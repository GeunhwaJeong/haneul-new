...
use crate::{consumer::ProtobufDecoder, peers::HaneulNodeProvider};
use axum::{
    body::Body,
    body::Bytes,
...
/// we expect haneul-node to send us an http header content-length encoding.
...
pub async fn expect_haneullabs_proxy_header(
    ...
    match format!("{content_type}").as_str() {
        ...
        ct => {
            ...
            MIDDLEWARE_OPS
                .with_label_values(&["expect_haneullabs_proxy_header", "invalid-content-type"])
                .inc();
