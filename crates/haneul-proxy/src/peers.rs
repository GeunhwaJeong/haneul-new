...
use haneul_rpc::Client as HaneulRpcClient;
use haneul_rpc::field::{FieldMask, FieldMaskUtil};
use haneul_rpc::proto::haneul::rpc::v2::{GetObjectRequest, Object};
...
/// Cache of `HaneulAddress -> validator name` from the latest haneul system state poll.
...
type ValidatorNames = Arc<RwLock<BTreeMap<HaneulAddress, String>>>;

/// HaneulNodeProvider queries the haneul blockchain and keeps a record of known validators based on the response from
/// haneul_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct HaneulNodeProvider {
    haneul_nodes: AllowedPeers,
    bridge_nodes: AllowedPeers,
    hashi_nodes: AllowedPeers,
    static_nodes: AllowedPeers,
    haneul_validator_names: ValidatorNames,
    rpc_url: String,
...
impl HaneulNodeProvider {
    pub fn new(
        ...
        let haneul_nodes = Arc::new(RwLock::new(HashMap::new()));
        let bridge_nodes = Arc::new(RwLock::new(HashMap::new()));
        let hashi_nodes = Arc::new(RwLock::new(HashMap::new()));
        let haneul_validator_names = Arc::new(RwLock::new(BTreeMap::new()));
        Self {
            haneul_nodes,
            bridge_nodes,
            hashi_nodes,
            static_nodes,
            haneul_validator_names,
            rpc_url,
            rpc_poll_interval,
            hashi_object_id,
        }
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<AllowedPeer> {
        ...
        // check haneul validators
        if let Some(v) = self.haneul_nodes.read().unwrap().get(key) {
            return Some(AllowedPeer {
                name: v.name.to_owned(),
                public_key: v.public_key.to_owned(),
            });
    ...
    /// Get a mutable reference to the allowed haneul validator map
    pub fn get_haneul_mut(&mut self) -> &mut AllowedPeers {
        &mut self.haneul_nodes
    }
    ...
    async fn get_validators(url: String) -> Result<HaneulSystemStateSummary> {
        ...
        struct ResponseBody {
            result: HaneulSystemStateSummary,
        }
    ...
    async fn get_bridge_validators(url: String) -> Result<BridgeSummary> {
        let rpc_method = "haneulx_getLatestBridge";
    ...
    async fn update_haneul_validator_set(&self) {
        match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                ...
                let names: BTreeMap<HaneulAddress, String> = summary
                    .active_validators
                    .iter()
                    .map(|v| (v.haneul_address, v.name.clone()))
                    .collect();
                {
                    let mut nw = self.haneul_validator_names.write().unwrap();
                    *nw = names;
                }

                let validators = extract(summary);
                let mut allow = self.haneul_nodes.write().unwrap();
                allow.clear();
                allow.extend(validators);
                info!(
                    "{} haneul validators managed to make it on the allow list",
                    allow.len()
                );
            }
            Err(error) => {
    ...
    async fn update_hashi_committee_set(&self, hashi_object_id: &str) {
        let validator_names: BTreeMap<HaneulAddress, String> =
            self.haneul_validator_names.read().unwrap().clone();
        ...
        match Self::get_bridge_validators(self.rpc_url.to_owned()).await {
            ...
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to refresh haneul bridge peer list: {error}");
            }
        };
    }
    ...
    pub fn poll_peer_list(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // The haneul validator set must update first; bridge and hashi resolvers
                // read the cached `haneul_validator_names` for friendly labeling.
                cloned_self.update_haneul_validator_set().await;
                cloned_self
...
/// extract will get the network pubkey bytes from a HaneulValidatorSummary type.  This type comes from a
...
fn extract(
    summary: HaneulSystemStateSummary,
    ...
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            Ok(public_key) => {
                debug!(
                    "adding public key {:?} for haneul validator {:?}",
                    public_key, vm.name
                );
                Some((
                    public_key.clone(),
...
//
// Reads on-chain state via haneul-rpc gRPC, requesting the `json` rendering of
// each Move object and path-walking into the fields we need — keeps the
// resolver resilient to hashi adding sibling fields. See
// `haneul-types/src/object/rpc_visitor/mod.rs` for the JSON encoding rules
...
async fn resolve_hashi_committee(
    rpc_url: &str,
    hashi_object_id: &str,
    validator_names: &BTreeMap<HaneulAddress, String>,
    ...
    let mut client = HaneulRpcClient::new(rpc_url.to_owned())
        .with_context(|| format!("creating haneul-rpc client for {rpc_url}"))?;
    ...
    let mut active_addrs: std::collections::HashSet<HaneulAddress> =
    ...
    // haneul_rpc::Client is cheap to clone — each clone shares the underlying
...
async fn get_hashi_committee_snapshot(
    client: &mut HaneulRpcClient,
    hashi_object_id: Address,
) -> Result<CommitteeSetSnapshot> {
    let rpc_method = "haneul_rpc.LedgerService.GetObject:Hashi";
    let _timer = JSON_RPC_DURATION
        .with_label_values(&[rpc_method])
        .start_timer();
...
async fn get_committee_validator_addresses(
    ...
    committees_bag_id: Address,
    epoch: u64,
) -> Result<Vec<HaneulAddress>> {
    let rpc_method = "haneul_rpc.LedgerService.GetObject:Committee";
    let _timer = JSON_RPC_DURATION
        .with_label_values(&[rpc_method])
        .start_timer();
    ...
    for m in members {
        ...
        let addr: HaneulAddress = addr_str
            .parse()
...
async fn get_hashi_member_info(
    ...
    let rpc_method = "haneul_rpc.LedgerService.GetObject:MemberInfo";
    let _timer = JSON_RPC_DURATION
        .with_label_values(&[rpc_method])
        .start_timer();

    let key = haneul_address_to_sdk_address(validator_address);
...
/// Both haneul-types and haneul-sdk-types use 32-byte addresses; this swaps a
/// `HaneulAddress` into the `haneul_sdk_types::Address` shape that `derive_dynamic_child_id`
/// and `bcs::to_bytes` need for key encoding.
fn haneul_address_to_sdk_address(addr: HaneulAddress) -> Address {
...
mod tests {
    ...
    #[test]
    fn depend_on_haneul_haneul_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("haneul".into());
        // all fields here just satisfy the field types, with exception to active_validators, we use
        // some of those.
        let depends_on = HaneulSystemStateSummary {
            active_validators: vec![HaneulValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{HaneulSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{HaneulSystemStateSummary}");
            ...
            committee: BridgeCommitteeSummary {
                members: vec![(
                    ...
                    MoveTypeCommitteeMember {
                        haneul_address: HaneulAddress::ZERO,
    ...
    fn addr(byte: u8) -> HaneulAddress {
        // Build a deterministic test HaneulAddress from a single discriminator byte.
        let mut bytes = [0u8; 32];
        bytes[31] = byte;
        HaneulAddress::from_bytes(bytes).unwrap()
    }
    ...
    fn extract_hashi_keeps_members_with_valid_tls_key() {
        let names: BTreeMap<HaneulAddress, String> = [
    ...
    fn extract_hashi_falls_back_to_address_label_when_name_missing() {
        ...
        // (e.g. resolver ran before the haneul-validator-set poll finished, or the
