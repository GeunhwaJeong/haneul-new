...
/// HaneulNods a mapping of public key to HaneulPeer data
pub type HaneulPeers = Arc<RwLock<HashMap<Ed25519PublicKey, HaneulPeer>>>;

/// A HaneulPeer is the collated haneul chain data we have about validators
#[derive(Hash, PartialEq, Eq, Debug, Clone)]
pub struct HaneulPeer {
    pub name: String,
...
impl HaneulNodeProvider {
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<HaneulPeer> {
        debug!("look for {:?}", key);
        if let Some(v) = self.nodes.read().unwrap().get(key) {
            return Some(HaneulPeer {
    ...
    pub fn get_ref(&self) -> &HaneulPeers {
        &self.nodes
    }

    /// Get a mutable reference to the inner service
    pub fn get_mut(&mut self) -> &mut HaneulPeers {
        &mut self.nodes
    }
...
fn extract(summary: HaneulSystemStateSummary) -> impl Iterator<Item = (Ed25519PublicKey, HaneulPeer)> {
...
mod tests {
    ...
    use haneul_types::{
        ...
        id::ID,
        haneul_system_state::haneul_system_state_summary::{HaneulSystemStateSummary, HaneulValidatorSummary},
    ...
    fn depend_on_haneul_haneul_system_state_summary() {
        ...
        let depends_on = HaneulSystemStateSummary {
            ...
            active_validators: vec![HaneulValidatorSummary {
                ...
                voting_power: 1,
                operation_cap_id: ID::new(HANEUL_SYSTEM_STATE_OBJECT_ID),
                gas_price: 1,
                commission_rate: 1,
                next_epoch_stake: 1,
                next_epoch_gas_price: 1,
                next_epoch_commission_rate: 1,
                staking_pool_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
                staking_pool_activation_epoch: None,
                staking_pool_deactivation_epoch: None,
                staking_pool_haneul_balance: 1,
                rewards_pool: 1,
                pool_token_balance: 1,
                pending_stake: 1,
                pending_total_haneul_withdraw: 1,
                pending_pool_token_withdraw: 1,
                exchange_rates_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
                exchange_rates_size: 1,
            }],
            pending_active_validators_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
            pending_active_validators_size: 1,
            pending_removals: vec![],
            staking_pool_mappings_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
            staking_pool_mappings_size: 1,
            inactive_pools_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
            inactive_pools_size: 1,
            validator_candidates_id: HANEUL_SYSTEM_STATE_OBJECT_ID,
