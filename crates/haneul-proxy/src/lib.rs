...
mod tests {
    ...
    use crate::{config::RemoteWriteConfig, peers::HaneulNodeProvider};
    ...
    use haneul_tls::{CertVerifier, TlsAcceptor, TlsConnectionInfo};
    ...
    /// haneul-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn axum_acceptor() {
        ...
        let (client_priv_cert, client_pub_key) = admin::generate_self_cert("haneul".into());
        ...
        let mut allower = HaneulNodeProvider::new("".into(), Duration::from_secs(30));
