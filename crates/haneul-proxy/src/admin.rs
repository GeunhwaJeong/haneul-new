...
use crate::middleware::{
    expect_content_length, expect_haneullabs_proxy_header, expect_valid_public_key,
};
use crate::peers::{HaneulNodeProvider, HaneulPeer};
use crate::var;
use anyhow::Error;
use anyhow::Result;
...
use haneul_tls::{rustls::ServerConfig, AllowAll, CertVerifier, SelfSignedCertificate, TlsAcceptor};
use tokio::signal;
...
pub fn app(
    ...
    allower: Option<HaneulNodeProvider>,
) -> Router {
    ...
    let mut router = Router::new()
        ...
        .route_layer(middleware::from_fn(expect_haneullabs_proxy_header))
...
pub fn create_server_cert_enforce_peer(
    ...
    static_peers: Option<StaticPeerValidationConfig>,
) -> Result<(ServerConfig, Option<HaneulNodeProvider>), haneul_tls::rustls::Error> {
    let (Some(certificate_path), Some(private_key_path)) =
        (dynamic_peers.certificate_file, dynamic_peers.private_key)
    else {
        return Err(haneul_tls::rustls::Error::General(
            "missing certs to initialize server".into(),
        ));
    };
    let static_peers = load_static_peers(static_peers).map_err(|e| {
        haneul_tls::rustls::Error::General(format!("unable to load static pub keys: {}", e))
    })?;
    let allower = HaneulNodeProvider::new(dynamic_peers.url, dynamic_peers.interval, static_peers);
