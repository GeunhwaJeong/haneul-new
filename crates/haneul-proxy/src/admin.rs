...
use crate::middleware::{expect_haneullabs_proxy_header, expect_valid_public_key};
...
pub fn app(network: String, client: ReqwestClient, allower: Option<HaneulNodeProvider>) -> Router {
...
pub fn create_server_cert_enforce_peer(
    ...
    let (Some(certificate_path), Some(private_key_path)) = (peer_config.certificate_file, peer_config.private_key) else {
        return Err(haneul_tls::rustls::Error::General("missing certs to initialize server".into()));
    };
    let allower = HaneulNodeProvider::new(peer_config.url, peer_config.interval);
