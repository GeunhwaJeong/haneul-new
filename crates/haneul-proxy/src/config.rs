...
/// DynamicPeerValidationConfig controls what haneul-node, haneul-bridge, and hashi-server
/// binaries (each functioning as a node in a Haneul-anchored network) we'll accept
/// metrics from. Peer membership is determined dynamically via json-rpc calls to a
/// full node — haneul validator set and bridge committee come from fixed RPC methods,
