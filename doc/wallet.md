...
Welcome to the Haneul tutorial focusing on the toy Haneul wallet developed
...
Haneul wallet and how to execute wallet commands through its command-line
...
gas objects, which are Haneul [objects](objects.md) used to pay for Haneul
...
The genesis process creates a configuration file `wallet.conf` for the Haneul wallet, 
the config file contains information of the accounts and the Haneul network, 
Haneul wallet uses the network information to communicate with the Haneul network authorities 
...
`authorities` contains Haneul network authorities' name, host and port information, it is used to establish connections to the Haneul network.
...
Haneul's genesis process will create five accounts by default, if that's not enough, there are two ways to add accounts to the Haneul wallet if needed.
...
learned how to find out the user addresses available as part of Haneul
...
- `--package` - ID of the package object where the module containing
  ...
  genesis Haneul package containing the GAS module is defined in its
...
_published_ to to Haneul's [distributed
...
From now on, we can use package object id in the Haneul wallet's call
