...
Haneul has programmable objects created and managed by [Move](https://github.com/diem/move) packages (aka "smart contracts"). Move packages themselves are also objects. Thus, Haneul objects can be partitioned into two categories:
...
* *Immutable packages*: a set of Move bytecode modules with a distinct name. A package can depend on other pacakge objects that were previously published to the Haneul ledger.
...
## Further Reading
* Objects are modified and created by transactions--read more about the [transaction types](transactions.md) supported by Haneul
