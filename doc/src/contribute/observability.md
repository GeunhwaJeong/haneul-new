...
In a distributed and asynchronous system like Haneul, one cannot rely on looking at individual logs over time in a single thread.
...
Detailed span start and end logs can be generated by defining the `HANEUL_JSON_SPAN_LOGS` environment variable. Note that this causes all output to be in JSON format, which is not as human-readable, so it is not enabled by default.
...
> **Note:** Adding Tokio-console support may significantly slow down Haneul validators/gateways.

### Memory profiling

Haneul uses the [jemalloc memory allocator](https://jemalloc.net/) by default on most platforms, and there is code that enables automatic
...
(If you use the [Docker image](https://hub.docker.com/r/haneullabs/haneul-node), they are set automatically)
