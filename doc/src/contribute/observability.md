...
1. Start `./haneul node --config-path ...`
1. In the same `target/release` directory, run `jeprof --text haneul-node jeprof.66*.heap` where 66 is the starting PID of the current haneul-node process.
...
start of haneul-node, that should look like this:
```shell
   INFO haneul_node: Default Jemalloc conf: prof:true,lg_prof_interval:24,lg_prof_sample:19
```
