---
title: Profiling Haneul on macOS using XCode Instruments
---
...
1. Build whatever Haneul component you want to run normally with `cargo build` - these docs assume `target/debug/haneul`.
1. Sign the binary you wish to profile:
    ...
    1. Run:
       ```shell
          $ codesign -s - -v -f --entitlements ../debug.plist target/debug/haneul
          ```
1. Run the app and record a trace (select the most appropriate template; see xcode documentation for available templates):
   ```shell
        $ xcrun xctrace record --template 'Allocations' --launch -- ./target/debug/haneul start
        ```
