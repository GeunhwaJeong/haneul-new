...
*HaneulJSON* is a JSON-based format with restrictions that allow Haneul to align JSON inputs more closely with Move call arguments.

This table shows the restrictions placed on JSON types to make them HaneulJSON compatible:

| JSON    | HaneulJSON Restrictions                         | Move Type Mapping                                                                               |
|---------|----------------------------------------------|-------------------------------------------------------------------------------------------------|
...
| Array   | Must be homogeneous JSON and of HaneulJSON type | Vector                                                                                          |
| Null    | Not allowed                                  |
| Object  | Not allowed                                  |

## Type coercion reasoning

Due to the loosely typed nature of JSON/HaneulJSON and the strongly typed nature of Move types, you sometimes need to overload HaneulJSON types to represent multiple Move types.

For example `HaneulJSON::Number` can represent both *U8* and *U32*. This means you have to coerce and sometimes convert types.

Which type you coerce depends on the expected Move type. For example, if the Move function expects a U8, you must have received a `HaneulJSON::Number` with a value less than 256. More importantly, you have no way to easily express Move addresses in JSON, so you encode them as hex strings prefixed by `0x`.

Additionally, Move supports U128 and U256 but JSON doesn't. As a result Haneul allows encoding numbers as strings.

## Type coercion rules

| Move Type | HaneulJSON Representations | Valid Examples | Invalid Examples |
| --- | --- | --- | --- |
