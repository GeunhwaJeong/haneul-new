...
This section covers the main features of Haneul Move.
...
```toml
[package]
name = "haneul-by-example"
version = "0.1.0"

[dependencies]
Haneul = { git = "https://github.com/GeunhwaJeong/haneul.git", subdir = "crates/haneul-framework", rev = "main" }
...
[dependencies]
Haneul = { git = "https://github.com/GeunhwaJeong/haneul.git", subdir = "crates/haneul-framework", rev = "devnet-0.10.0" }
...
Shared object is an object that is shared using a `haneul::transfer::share_object` function and is accessible to everyone.
...
module examples::donuts {
    ...
    public entry fun buy_donut(
        shop: &mut DonutShop, payment: &mut Coin<HANEUL>, ctx: &mut TxContext
    ) {
        assert!(coin::value(payment) >= shop.price, ENotEnough);

        // Take amount = `shop.price` from Coin<HANEUL>
...
In Haneul Move, objects defined with only `key` ability can not be transferred by default. To enable
