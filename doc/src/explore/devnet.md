...
You may simply [install Haneul](../build/install.md) and then request test tokens as described below. To use the Haneul DevNet, you will need:
...
```shell
$ haneul client create-example-nft
```
...
The above command created an object with ID `ED883F6812AF447B9B0CE220DA5EA9E0F58012FE`. Note you may use `Haneul CLI client` to [view objects owned by the account](https://docs.haneul.io/build/wallet#view-objects-owned-by-the-account).
...
In order to create a sword and transfer it to another player, we use the following command to call the `sword_create` [function](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/move_tutorial/sources/m1.move#L44) in the `M1` [module](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/move_tutorial/sources/m1.move#L4) of the package we previously published.
...
Now run:
```shell
$ haneul client call --function sword_create --module M1 --package 0x<PACKAGE_ID> --args \"0x<FORGE_ID>\" 42 7 \"0x<PLAYER_ADDRESS>\" --gas-budget 30000
```
