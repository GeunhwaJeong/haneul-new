...
## Supported Browsers
The Haneul Explorer supports the following browsers:
...
## Choose a network
When you start Haneul Explorer, it displays the transactions for the Devnet network by default. You can also use the Explorer to view data for a local network running in your environment or a custom RPC endpoint URL.
...
1. Open your Haneul wallet.
1. Click the clipboard icon to copy your wallet address.
1. Open the Haneul Explorer. Select the **Coins** tab if it is not currently selected.
...
## Explorer start page
When you open the Haneul Explorer, the page displays the transactions and top validators on the network. The **Transactions** table lists the transactions on the network with the most recent transaction first.
 ...
 * **Type** - the type of transaction, one of Call, TransferHaneul, TransferObject, or Publish.
     ...
     * **TransferHaneul** - Indicates that the transaction transferred Haneul from one account to another.
 ...
 * **Gas** - Shows the Haneul used to pay for the gas required to complete the transaction.

You can click on a value in the **Transaction ID** or **Addresses** column to open a details page for the transaction or address that you click on. When you click a transaction ID, the page that opens depends on the type of transaction. Haneul Eplorer provides the following detail pages:
 * [Transaction details](#transaction-details-pages) for each transaction type
     * TransferHaneul transaction details
...
The **Events** tab lists the events the transaction generated and the details about each event. TransferHaneul transactions do not include events.
