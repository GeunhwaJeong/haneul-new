...
Welcome to the [Haneul Wallet Browser Chrome Extension](https://chrome.google.com/webstore/detail/haneul-wallet/opcgpfmipidbgpenhmajoajpbobppdil). The Haneul Wallet Browser Extension acts as your portal to the Web3 world. Follow this guide to install and use the extension.

## Purpose

Initially, the Haneul Wallet Browser Extension is aimed at Haneul developers for testing purposes. As such, the tokens are of no value (just like the rest of [DevNet](../build/devnet.md)) and will disappear each time we reset the network. In time, the Haneul Wallet Browser Extension will be production ready for real tokens.

This browser extension is a pared-down version of the [Haneul CLI client](../contribute/cli-client.md) that provides greater ease of use for the most commonly used features. If you need more advanced features, such as merging/splitting coins and making arbitrary [Move](../build/move/index.md) calls, instead use the [Haneul CLI client](../contribute/cli-client.md).
...
1. Visit its [link in the Chrome Webstore](https://chrome.google.com/webstore/detail/haneul-wallet/opcgpfmipidbgpenhmajoajpbobppdil).
1. Click **Install**.
