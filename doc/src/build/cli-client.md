...
to facilitate experimentation with Haneul features using a
...
`haneul client new-address` command covered later.

The network configuration is stored in `network.yaml` and can be used
subsequently to start the network. The `client.yaml` and `haneul.keystore`
are also created to be used by the Haneul client to manage the newly
created accounts.

By default, these files are placed in your home directory at
`~/.haneul/haneul_config` (created automatically if it does not yet exist). But you
...
```shell
$ haneul genesis --working-dir /path/to/haneul/config/dir
```
...
existing configuration files, pass the `--force` option to the `haneul
genesis` command and either run it in the default directory (`~/.haneul/haneul_config`) or specify
it once again, using the `--working-dir` argument:

```shell
$ haneul genesis --force --working-dir /path/to/haneul/config/dir
```

## Client configuration

The genesis process creates a configuration file `client.yaml`, and a keystore file `haneul.keystore` for the
Haneul client.  The config file contains information of the accounts and
the Haneul Network server. The keystore file contains all the public-private key pairs of the created accounts.
Haneul client uses the network information in `client.yaml` to communicate
with the Haneul network validators  and create transactions using the key
...
keystore:
  File: /Users/user/.haneul/haneul_config/haneul.keystore
gateway:
  embedded:
    epoch: 0
    validator_set:
    ...
    recv_timeout:
      ...
      nanos: 0
    buffer_size: 650000
    db_folder_path: /Users/user/.haneul/haneul_config/client_db
...
`gateway` variable contains the information of the Haneul network that the client will
be connecting to.

The `authorities` variable is part of the embedded gateway configuration. It contains
the Haneul network validator's name, host and port information. It is used to establish connections
to the Haneul network.
...
#### RPC gateway
You can also connect the client to the Haneul network via an [RPC Gateway](json-rpc.md#start-local-rpc-server);
...
The key pairs are stored in `haneul.keystore`. However, this is not secure
...
This command will look for the Haneul network configuration file
`network.yaml` in the `~/.haneul/haneul_config` directory. But you can
...
```shell
$ haneul start --config /Users/name/tmp/network.yaml
```
...
If you see errors when trying to start Haneul network, particularly if you made some custom changes
 (e.g,
[customized client configuration](#client-configuration)), you should [recreate Haneul genesis state](#recreating-genesis).

## Using the Haneul client

Now start a new terminal since you have the Haneul network running in the first terminal.

The following commands are supported by the Haneul client:
...
Use `haneul client -h` to see the most up-to-date list of commands.
...
terminal window than one used to execute `haneul start`). Assuming you
accepted the default location for configuration:
...
`client.yaml` in the `~/.haneul/haneul_config` directory. But you can
override this setting by providing a path to the directory where this
file is stored:

```shell
$ haneul console --config /path/to/client/config/file
```

The Haneul interactive client console supports the following shell functionality:
...
* *Environment variable substitution* -
  The Haneul console will substitute inputs prefixed with `$` with environment variables,
...
Since a Haneul CLI client manages multiple disjointed addresses, one might need to specify
...
```shell
$ haneul client switch --address 0x913cf36f370613ed131868ac6f9da2420166062e
```
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55 |     0      | j8qLxVk/Bm9iMdhPf9b7HcIMQIAM+qCd8LfPAwKYrFo= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
```
```shell
$ haneul client objects --address 0x913cf36f370613ed131868ac6f9da2420166062e
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55 |     0      | j8qLxVk/Bm9iMdhPf9b7HcIMQIAM+qCd8LfPAwKYrFo= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
```
...
All Haneul transactions require a gas object for payment, as well as a budget. However, specifying
...
```shell
$ haneul client gas --address 0x562f07cf6369e8d22dbf226a5bfedc6300014837
                Object ID                   |  Version   |  Gas Value
------------------------------------------------------------------------
...
not enough, there are two ways to add accounts to the Haneul CLI client if needed.
...
Restart the Haneul console after the modification; the new accounts will appear in the client if you query the addresses.
...
```shell
$ haneul client objects --address 0x66af3898e7558b79e115ab61184a958497d1905a
```
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55 |     0      | j8qLxVk/Bm9iMdhPf9b7HcIMQIAM+qCd8LfPAwKYrFo= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xc8add7b4073900ffb0a8b4fe7d70a7db454c2e19 |     0      | uCZNPmDWOksKhCKwEaMtST5T4HbTjcgXGHRXP4qTLC8= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xd1949864f94d87cf25e1fd7b1c8ab4bf685f7801 |     0      | OsTryyECAPW9mnSbWlYWELX+QlRg5er7s/DlkgqhDww= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xddb6119c320f52f3fef9fbc272af305d985b6883 |     0      | gBCDdel7iJZnXpuf4g9dqIPT4XjaAY/4knNcDxbTons= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xe1fe79ac8d900342e617e0986f54ff64e4e323de |     0      | qjsWIzAaomo0eqFwQt99EkARsiC/aw2hPDH8quM6pYg= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 5 results.
```
...
```shell
haneul-client-object
Get object info

USAGE:
    haneul client object [OPTIONS] --id <ID>
...
```bash
$ haneul client object --id 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55
```
...
> `--json` flag in the `haneul client` command to see the raw JSON representation
> of the object.
...
```shell
$ haneul client objects --address 0xc72cf3adcc4d11c03079cef2c8992aea5268677a
...
USAGE:
    haneul client transfer [OPTIONS] --to <TO> --object-id <COIN_OBJECT_ID> --gas-budget <GAS_BUDGET>
...
```shell
$ haneul client transfer --to 0xf456ebef195e4a231488df56b762ac90695be2dd --object-id 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55 --gas-budget 100
```
...
$ haneul client objects --address 0xc72cf3adcc4d11c03079cef2c8992aea5268677a
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x66eaa38c8ea99673a92a076a00101ab9b3a06b55 |     1      | j8qLxVk/Bm9iMdhPf9b7HcIMQIAM+qCd8LfPAwKYrFo= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
```

## Creating example NFTs

You may create an [NFT-like object](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/sources/devnet_nft.move#L16) on Haneul using the following command:
...
description: An NFT created by the Haneul Command Line Tool
id: 0x524f9fae3ca4554e01354415daf58a05e5bf26ac[1]
name: Example NFT
url: ipfs://bafkreibngqhl3gaa7daob4i2vccziay2jjlp435cf66vhono7nrvww53ty
```

The command will invoke the `mint` function in the `devnet_nft` module, which mints a Haneul object with three attributes: name, description, and image URL with [default values](https://github.com/GeunhwaJeong/haneul/blob/27dff728a4c9cb65cd5d92a574105df20cb51887/haneul/src/wallet_commands.rs#L39) and transfers the object to your address. You can also provide custom values using the following instructions:
...
USAGE:
    haneul client create-example-nft [OPTIONS]
...
```shell
haneul-client-merge-coin
Merge two coin objects into one coin

USAGE:
    haneul client merge-coin [OPTIONS] --primary-coin <PRIMARY_COIN> --coin-to-merge <COIN_TO_MERGE> --gas-budget <GAS_BUDGET>
...
```shell
$ haneul client objects --address 0x3cbf06e9997b3864e3baad6bc0f0ef8ec423cd75
```
...
```
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x1e90389f5d70d7fa6ce973155460e1c04deae194 |     0      | BC5O8Bf6Uw8S1LV1y4RCI6+kz1KhZG/aOpeqq9kTAvs= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x351f08f03709cebea85dcd20e24b00fbc1851c92 |     0      | 9aYvavAzY6chYbOUtMtJj0g/5GNc+KBsqptCX5pmQ2Y= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x3c720502f9eabb17a52a999859fbbaeb408b1d14 |     0      | WUPT6P40veMZ/C7GiQpv92I4EH+hvh5BbkBt+7p9yH0= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x7438af4677b9cea2094848f611143346183c11d1 |     0      | 55B56RG/kCeHrN6GXdIq0IvnyYD/hng9J7I7FNRykQ4= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x9d5f2b2564ad2255c24a03556785bddc85381508 |     0      | rmyYjq/UEED0xR0hE3Da8OYgBAu3MYxKQ3v76pGTDek= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 5 results.
```

Then we merge:
```shell
$ haneul client merge-coin --primary-coin 0x1e90389f5d70d7fa6ce973155460e1c04deae194 --coin-to-merge 0x351f08f03709cebea85dcd20e24b00fbc1851c92 --gas-budget 1000
```
...
Type Arguments : ["0x2::haneul::HANEUL"]
----- Merge Coin Results ----
...
```shell
$ haneul client objects --address 0x08da15bee6a3f5b01edbbd402654a75421d81397
```
...
```shell
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x4a2853304fd2c243dae7d1ba58260bb7c40724e1 |     0      | uNcjv6KP8AXgQHTFmiEPV3tpWZcYHb1HmBR0B2pMsAo= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x692c179dc434ceb0eaa51cdd198bb905b5ab27c4 |     0      | /ug6IGGld90PqnmL9qijciCqn25V11nn5/PAsKjxMY0= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x7f7b7c1589aceb073a7c8740b1d47d05e4d89e3c |     0      | N5+qKRenKWqb7Y6WKZuFD+fRDB6pj/OtIyri+FSQ3Q0= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xe42558e82e315c9c81ee5b9f1ac3db819ece5c1d |     0      | toHeih0DeFrqxQhGzVUi9EkVwAZSbLx6hv2gpMgNBbs= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xfa322fee6a7f4c266ad4840e85bf3d87689b6de0 |     0      | DxjnkJTSl0o6HlzeOX5K/If61bbFwvRDydjzd2bq8ho= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 5 results.
```
...
```shell
$ haneul client split-coin --coin-id 0x4a2853304fd2c243dae7d1ba58260bb7c40724e1 --amounts 1000 5000 3000 --gas-budget 1000
```
...
$ haneul client objects --address 0x08da15bee6a3f5b01edbbd402654a75421d81397
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x1da8193ac29f94f8207b0222bd5941b7814c1668 |     1      | nAMEV3NZ0zscjO10QQUt1drLvhNXTk4MVLAg1FXTQxw= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x3653bae7851c36e0e5e827b7c1a2978ef78efd7e |     1      | blMuVATrI89PRvqA4Kuv6rNkbuAb+bYhmkMocY7pavw= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x4a2853304fd2c243dae7d1ba58260bb7c40724e1 |     1      | uhfauig0guMidpxFyCO6FzhzDfucss+eA6xWzAVF3sU= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x692c179dc434ceb0eaa51cdd198bb905b5ab27c4 |     1      | sWTy2PUbt3UFEKx1Km32dEG7cQscSK+eVc3ChaZCkkA= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x7f7b7c1589aceb073a7c8740b1d47d05e4d89e3c |     0      | N5+qKRenKWqb7Y6WKZuFD+fRDB6pj/OtIyri+FSQ3Q0= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xd5b694f67410d5b6cd293128cd48953aaa0a3dce |     1      | 4V0BC6eopxkN6wIOdm2FVgwN3psNbPvLKQ9/zrYtsDM= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xe42558e82e315c9c81ee5b9f1ac3db819ece5c1d |     0      | toHeih0DeFrqxQhGzVUi9EkVwAZSbLx6hv2gpMgNBbs= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xfa322fee6a7f4c266ad4840e85bf3d87689b6de0 |     0      | DxjnkJTSl0o6HlzeOX5K/If61bbFwvRDydjzd2bq8ho= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 8 results.
```
...
```shell
$ haneul client split-coin --coin-id 0x4a2853304fd2c243dae7d1ba58260bb7c40724e1 --count 3 --gas-budget 1000
```
...
immediately ready to be called from Haneul CLI. Please see our
...
coins as this can be accomplished with a built-in Haneul client
...
```shell
$ haneul client objects --address 0x48ff0a932b12976caec91d521265b009ad5b2225
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x471c8e241d0473c34753461529b70f9c4ed3151b |     0      | MCQIALghS9kQUWMclChmsd6jCuLiUxNjEn9VRV+AhSA= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x53b50e3020a01e1fd6acf832a871feee240183f0 |     0      | VIbuA4fcsitOUmJLQ+FugZWIn7bg6LnVO8eTIAUDzkg= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x5c846224b8704683a1c576aec7c8d9c3413d87c1 |     0      | KO0Fr9uCPnT3KxOEishyzas33le4J9fAGg7iEOOzo7A= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x6fe4cf8d2c21f23f2aacf60f30c98ff9e2c78226 |     0      | p2evKbTirwEoF1PxGIu5USAsSdkxzh1sUD/OxBfpdNE= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xa28dd252ab5b984a8c1da699bbe10e7f09947a12 |     0      | 6VT+8479aijA8tYmab7YatVgjXm1TWy5jItooC416YQ= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 5 results.
```
...
the haneul module using the following Haneul client command:

```shell
$ haneul client call --function transfer --module haneul --package 0x2 --args 0x471c8e241d0473c34753461529b70f9c4ed3151b 0x3cbf06e9997b3864e3baad6bc0f0ef8ec423cd75 --gas-budget 1000
```
...
* `--args` - a list of function arguments formatted as
  [HaneulJSON](haneul-json.md) values (hence the preceding `0x` in address
  and object ID):
...
$ haneul client objects --address 0x48ff0a932b12976caec91d521265b009ad5b2225
                 Object ID                  |  Version   |                    Digest                    |   Owner Type    |               Object Type
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x53b50e3020a01e1fd6acf832a871feee240183f0 |     1      | st6KVE+nTPsQgtEtxSbgJZCzSSuSB2ZsJAMbXFNLw/k= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x5c846224b8704683a1c576aec7c8d9c3413d87c1 |     0      | KO0Fr9uCPnT3KxOEishyzas33le4J9fAGg7iEOOzo7A= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x6fe4cf8d2c21f23f2aacf60f30c98ff9e2c78226 |     0      | p2evKbTirwEoF1PxGIu5USAsSdkxzh1sUD/OxBfpdNE= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xa28dd252ab5b984a8c1da699bbe10e7f09947a12 |     0      | 6VT+8479aijA8tYmab7YatVgjXm1TWy5jItooC416YQ= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 4 results.
...
```shell
$ haneul client object --id 0x471c8e241d0473c34753461529b70f9c4ed3151b
```
...
*published* to Haneul's [distributed ledger](../learn/how-haneul-works.md#architecture).
...
which we can publish using Haneul client's `publish` command.
...
```shell
$ haneul client publish --path $PATH_TO_PACKAGE/my_move_package --gas-budget 30000
```
...
From now on, we can use the package object ID (`0xdbcee02bd4eb326122ced0a8540f15a057d82850`) in the Haneul client's call
...
> [build your package locally](../build/move/build-test.md#building-a-package) (using the `haneul move build` command)
