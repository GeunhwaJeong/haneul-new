...
Learn how to set up, configure, and use the Haneul Client Command Line Interface (CLI). You can use the CLI to experiment with Haneul features using a command line interface.

## Set up

The Haneul Client CLI installs when you install Haneul. See the [Install Haneul](install.md) topic for prerequisites and installation instructions.

## Using the Haneul client

The Haneul Client CLI supports the following commands:
...
| `new-address`        | Generate new address and keypair with keypair scheme flag {ed25519 or secp256k1 or secp256r1} with optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0 for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1 |
| `new-env`            | Add new Haneul environment.                                                                                                                                                                                                                      |
| `object`             | Get object information.                                                                                                                                                                                                                       |
| `objects`            | Obtain all objects owned by the address.                                                                                                                                                                                                      |
| `pay`                | Pay HANEUL to recipients following specified amounts, with input coins. Length of recipients must be the same as that of amounts.                                                                                                                |
| `pay_all_haneul`        | Pay all residual HANEUL coins to the recipient with input coins, after deducting the gas cost. The input coins also include the coin for gas payment, so no extra gas coin is required.                                                          |
| `pay_haneul`            | Pay HANEUL coins to recipients following specified amounts, with input coins. Length of recipients must be the same as that of amounts. The input coins also include the coin for gas payment, so no extra gas coin is required.                 |
| `publish`            | Publish Move modules.                                                                                                                                                                                                                         |
| `split-coin`         | Split a coin object into multiple coins.                                                                                                                                                                                                      |
| `switch`             | Switch active address and network.                                                                                                                                                                                                            |
| `transfer`           | Transfer object.                                                                                                                                                                                                                              |
| `transfer-haneul`       | Transfer HANEUL, and pay gas with the same HANEUL coin object. If amount is specified, transfers only the amount. If not specified, transfers the object.                                                                                           |
...
Use `haneul client -h` to see a list of supported commands.
...
The console command looks for the client configuration file `client.yaml` in the `~/.haneul/haneul_config` directory. If you have this file stored in a different directory, provide the updated path to the command to override this setting.

```shell
haneul console --client.config /workspace/config-files
```
...
Haneul sets a default address to use for commands. It uses the active address for commands that require an address. To view the current active address, use the `active-address` command.
...
```shell
haneul client switch --address 0xa3c00467938b392a12355397bdd3d319cea5c9b8f4fc9c51b46b8e15a807f030
```
...
All Haneul transactions require a gas object for gas fees. If you don't specify a gas object, Haneul uses a gas object with sufficient HANEUL to cover the gas fee.
...
```shell
haneul client gas 0x4e049913233eb918c11638af89d575beb99003d30a245ac74a02e26e45cb80ee
```

## Create new account addresses

Haneul Client CLI includes 1 address by default. You can create new addresses for the client with the `new-address` command, or add existing accounts to the client.yaml.
...
Restart the Haneul console after you save the changes to the client.yaml file.
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 0x1aa482ad8c6240cda3097a4aa13ad5bfb27bf6052133c01f79c8b4ea0aaa0601 |     1      | OpU8HmueEaLzK6hkNSQkcahG8qo73ag4vJPG+g8EQBs= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x3fd0e889ee56152cdbd5fa5b5dab78ddc66d127930f5173ae7b5a9ac3e17dd6d |     1      | lRamSZkLHnfN9mcrkoVzmXwHxE7GnFHNnqe8dzWEUA8= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x51ec7820e82035a5de7b4f3ba2a3813ea099dca1867876f4177a1fa1d1efe022 |     1      | 1NO7XtdmojnOch4gcCsUHDdV1n2bPYv5je83yXd5Suw= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0x727b37454ab13d5c1dbb22e8741bff72b145d1e660f71b275c01f24e7860e5e5 |     1      | 9C1lxL45JIxwX35rL69OtAFUf3kz39Dq6jiguVvpCeM= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
 0xe638c76768804cebc0ab43e103999886641b0269a46783f2b454e2f8880b5255 |     1      | idJrGmd6ZkzJVQeKtu8XlUt2dA397GURgCUXJOLQhxI= |  AddressOwner   |      0x2::coin::Coin<0x2::haneul::HANEUL>
Showing 5 results.
```
...
```shell
haneul client objects 0x338567a5fe29132d68fade5172870d8ac1b607fd00eaace1e0aa42896d7f97d4
```

To view more information about an object, use the `object` command and specify the `objectId`.

```shell
haneul client object <OBJECT-ID>
```
...
```
----- 0x2::coin::Coin<0x2::haneul::HANEUL> (0x3fd0e889ee56152cdbd5fa5b5dab78ddc66d127930f5173ae7b5a9ac3e17dd6d[0x1]) -----
...
```shell
haneul client object <OBJECT-ID> --json
```
...
```shell
haneul client transfer [OPTIONS] --to <TO> --object-id <OBJECT-ID> --gas-budget <GAS-BUDGET-AMOUNT>
...
```shell
haneul client transfer --to 0xcd2630011f6cb9aef960ed42d95b04e063c44a6143083ef89a35ea02b85c61b7 --object-id 0x33e3e1d64f76b71a80ec4f332f4d1a6742c537f2bb32473b01b1dcb1caac9427 --gas-budget <GAS-BUDGET-AMOUNT>
```
...
```shell
haneul client merge-coin [OPTIONS] --primary-coin <PRIMARY-COIN> --coin-to-merge <COIN-TO-MERGE> --gas-budget <GAS-BUDGET-AMOUNT>
...
```shell
haneul client objects 0x8f603d8a00ae87c43dc090e52bffc29a4b312c28ff3afd81c498caffa2a6b768
```

Use the IDs returned from the previous command in the `merge-coin` command.

```shell
haneul client merge-coin --primary-coin 0x33e3e1d64f76b71a80ec4f332f4d1a6742c537f2bb32473b01b1dcb1caac9427 --coin-to-merge 0x11af4b844ff94b3fbef6e36b518da3ad4c5856fa686464524a876b463d129760 --gas-budget <GAS-BUDGET-AMOUNT>
```

### Split coins

```shell
haneul client split-coin [OPTIONS] --coin-id <COIN-ID> --gas-budget <GAS-BUDGET-AMOUNT> (--amounts <AMOUNTS>... | --count <COUNT>)
...
```shell
haneul client objects 0xcd2630011f6cb9aef960ed42d95b04e063c44a6143083ef89a35ea02b85c61b7
```
...
```shell
haneul client split-coin --coin-id 0x11af4b844ff94b3fbef6e36b518da3ad4c5856fa686464524a876b463d129760 --amounts 1000 5000 3000 --gas-budget <GAS-BUDGET-AMOUNT>
```
...
```shell
haneul client split-coin --coin-id 0x11af4b844ff94b3fbef6e36b518da3ad4c5856fa686464524a876b463d129760 --count 3 --gas-budget <GAS-BUDGET-AMOUNT>
```

## Calling Move code

The genesis state of the Haneul platform includes Move code that is
immediately ready to be called from Haneul CLI.
...
```shell
haneul client call --function transfer --module haneul --package 0x2 --args 0x1b9c00a93345ce5f12bea9ffe04748d6696c30631735193aea95b8f9082c1062 0x33e3e1d64f76b71a80ec4f332f4d1a6742c537f2bb32473b01b1dcb1caac9427 --gas-budget <GAS-BUDGET-AMOUNT>
```
...
```shell
haneul client call --function transfer --module haneul --package 0x2 --args $OBJECT_ID $RECIPIENT --gas-budget <GAS-BUDGET-AMOUNT>
```
...
* `--type-args` - a list of types to let the Haneul Move compiler know how to fill
...
2. The `--type-args` flag is used if the entry function being called is
  ...
  - a non-generic type or struct defined in a published, public Haneul Move package, specified thusly:
  ...
  When, upon calling `haneul client call`, an error similar to
  ```
...
You must publish packages to the Haneul [distributed ledger](../learn/how-haneul-works.md#architecture) for the code you developed to be available in Haneul. To publish packages with the Haneul client, use the `publish` command.
...
which you can then publish using the Haneul client `publish` command.
...
```shell
haneul client objects 0x4e049913233eb918c11638af89d575beb99003d30a245ac74a02e26e45cb80ee
```
...
```shell
haneul client publish $PATH_TO_PACKAGE/my_move_package --gas 0x33e3e1d64f76b71a80ec4f332f4d1a6742c537f2bb32473b01b1dcb1caac9427 --gas-budget <GAS-BUDGET-AMOUNT>
```
...
From now on, use the package object ID (`0x53e4567ccafa5f36ce84c80aa8bc9be64e0d5ae796884274aef3005ae6733809`) in the Haneul client call
...
**Important:** If the publishing attempt results in an error regarding verification failure, [build your package locally](../build/move/build-test.md#building-a-package) (using the `haneul move build` command) to get a more verbose error message.
...
`haneul client verify-source ./code/MyPackage`
