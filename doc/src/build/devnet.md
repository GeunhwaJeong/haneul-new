...
Use the Haneul Devnet network to experiment with the version of Haneul running on the Devnet network.

The Haneul Devnet network consists of:
...
You can [request test HANEUL tokens](#request-test-tokens) through the Haneul [devnet-faucet](https://discordapp.com/channels/916379725201563759/971488439931392130) Discord channel. These coins have no financial value. With each Haneul release, the network resets and removes all assets (coins and NFTs).
...
Haneul provides the following tools to interact with Haneul Devnet:
...
If you previously ran `haneul genesis -f` to create a local network, it created a Haneul client configuration file (client.yaml) that connects to `localhost` `http://0.0.0.0:9000`. See [Connect to custom RPC endpoint](#connect-to-custom-rpc-endpoint) to update the client.yaml file.
...
The first time you start Haneul client, it displays the following message:

```
Config file ["/Users/dir/.haneul/haneul_config/client.yaml"] doesn't exist, do you want to connect to a Haneul Full node server [y/N]?
```
...
Press **Enter** to connect to Haneul Devnet. To use a custom RPC server, enter the URL to the custom RPC endpoint and then press **Enter**.
...
If you previously used `haneul genesis` with the force option (`-f` or `--force`), your client.yaml file already includes two RPC endpoints: localnet `http://0.0.0.0:9000` and devnet `https://fullnode.devnet.haneul.io:443`). You can view the defined environments with the `haneul client envs` command, and switch between them with the `haneul client switch` command.

If you previously installed a Haneul client that connected to the Devnet network, or created a local network, you can modify your existing `client.yaml` to change the configured RPC endpoint:
...
## Request test tokens
 ...
 1. Request test HANEUL tokens in the Haneul [#devnet-faucet](https://discord.com/channels/916379725201563759/971488439931392130) Discord channel.
...
To view the created object in [Haneul Explorer](https://explorer.haneul.io), append the object ID to the following URL https://explorer.haneul.io/objects/.
...
To view details about the object in Haneul Explorer, copy the object ID and search for it in the Explorer search field.
...
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519HaneulSignature(Ed25519HaneulSignature([0, 125, 78, 25, 52, 44, 203, 148, 143, 172, 110, 105, 88, 189, 236, 21, 138, 189, 112, 187, 194, 114, 84, 187, 38, 11, 110, 190, 132, 156, 55, 133, 83, 217, 107, 116, 84, 218, 245, 76, 35, 142, 168, 225, 28, 203, 249, 10, 215, 121, 50, 75, 57, 182, 174, 91, 9, 101, 132, 93, 135, 17, 130, 185, 1, 179, 217, 77, 101, 114, 102, 91, 188, 47, 119, 202, 180, 98, 71, 241, 49, 221, 180, 213, 109, 5, 155, 216, 106, 151, 199, 153, 67, 200, 193, 58, 158])))]
...
You can check the details of each object using the `haneul client object <OBJECT_ID>` command or by using the Haneul Explorer.
...
To demonstrate this, `<PLAYER_ADDRESS>` represents the address of the player to receive a sword. You can use an address of someone you know, or create another address for testing with the following Haneul Client CLI command:
```shell
...
```shell
haneul client call --function sword_create --module my_module --package 0x<PACKAGE_ID> --args \"0x<FORGE_ID>\" 42 7 \"0x<PLAYER_ADDRESS>\" --gas-budget 30000
```

The response resembles the following:
```shell
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519HaneulSignature(Ed25519HaneulSignature([0, 178, 50, 21, 4, 228, 131, 133, 216, 36, 226, 72, 149, 116, 225, 128, 3, 228, 203, 17, 185, 167, 107, 91, 123, 253, 21, 24, 10, 91, 152, 215, 199, 215, 144, 183, 170, 21, 66, 83, 27, 161, 252, 224, 42, 52, 97, 242, 186, 35, 32, 2, 222, 97, 167, 67, 197, 244, 60, 114, 32, 70, 41, 35, 2, 179, 217, 77, 101, 114, 102, 91, 188, 47, 119, 202, 180, 98, 71, 241, 49, 221, 180, 213, 109, 5, 155, 216, 106, 151, 199, 153, 67, 200, 193, 58, 158])))]
Transaction Kind : Programmable
...
To see your object in the current [Haneul Explorer](https://explorer.haneul.io), paste the object ID of the created object you observed in your own command output in the search field and press **Enter**.
