...
Haneul returns a message similar to the following (depending on the key scheme you selected) that includes the address and recovery phrase for the address:
...
You can check the details of each object using the `haneul client object <OBJECT_ID>` command. For example, checking the forge object returns:
