...
The ability to upgrade Haneul Move packages provides the opportunity to iterate your package development, whether to continuously improve features or address logic defects. The `haneul client upgrade` command offers an approachable way to upgrade packages when the CLI active address owns the `UpgradeCap` object associated with those packages.

Using the Haneul Client CLI is useful to get started with upgrades, or in the early stages of package development, but protecting the ability to upgrade a package on chain using a single key can pose a security risk for several reasons:
...
You can make a package *immutable* when it goes live to mitigate this risk using the Haneul Move `haneul::package::make_immutable` function to destroy its `UpgradeCap`. Making the package immutable, however, prevents future bug fixes and new features, which might not be practical or desired.

To address the security risk a single key poses while still providing the opportunity to upgrade live packages, Haneul offers *custom upgrade policies*. These policies protect `UpgradeCap` access behind arbitrary Haneul Move code and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.
...
You can change the current policy by calling one of the functions in `haneul::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's [`UpgradeCap`](./custom-upgrade-policy.md#upgradecap) and a policy can become only more restrictive. For example, after you call `haneul::package::only_dep_upgrades` to restrict the policy to become additive, calling `haneul::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
These are the functions that `haneul client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
bytecode or source code for ahead of time. Haneul calculates the digest as follows:
...
The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and Haneul added the new package to the set of created
...
After Haneul creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction block (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
module policy::day_of_week {
    ...
    fun week_day(ctx: &TxContext): u8 {
        let days_since_unix_epoch =
            haneul::tx_context::epoch_timestamp_ms(ctx) / MS_IN_DAY;
        // The unix epoch (1st Jan 1970) was a Thursday so shift days
        // since the epoch by 3 so that 0 = Monday.
        ((days_since_unix_epoch + 3) % 7 as u8)
    }
...
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519HaneulSignature(Ed25519HaneulSignature([0, 251, 96, 164, 70, 48, 195, 251, 181, 82, 206, 254, 167, 84, 165, 40, 29, 254, 102, 165, 152, 81, 244, 203, 199, 97, 33, 107, 29, 95, 120, 212, 34, 19, 233, 109, 179, 72, 246, 219, 23, 254, 108, 222, 210, 250, 166, 172, 208, 133, 108, 252, 36, 165, 71, 97, 210, 206, 144, 138, 237, 169, 15, 218, 13, 92, 225, 85, 204, 230, 61, 45, 147, 106, 193, 13, 195, 116, 230, 99, 61, 161, 251, 251, 68, 154, 46, 172, 143, 122, 101, 212, 120, 80, 164, 214, 54])))]
Transaction Kind : Programmable
Inputs: [Pure(HaneulPureValue { value_type: Some(Address), value: "<SENDER>" })]
Commands: [
...
Following best practices, use the Haneul Client CLI to call `haneul::package::make_immutable` on the `UpgradeCap` to make the policy immutable.
...
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519HaneulSignature(Ed25519HaneulSignature([0, 123, 97, 9, 252, 127, 238, 10, 88, 175, 157, 155, 98, 11, 23, 234, 52, 167, 230, 45, 218, 171, 31, 174, 87, 107, 174, 117, 236, 65, 117, 18, 42, 74, 56, 149, 82, 107, 216, 199, 223, 142, 135, 165, 200, 80, 151, 32, 110, 75, 133, 128, 150, 66, 13, 40, 173, 228, 211, 94, 222, 201, 248, 221, 10, 92, 225, 85, 204, 230, 61, 45, 147, 106, 193, 13, 195, 116, 230, 99, 61, 161, 251, 251, 68, 154, 46, 172, 143, 122, 101, 212, 120, 80, 164, 214, 54])))]
Transaction Kind : Programmable
...
If you don't have a package available, use the `haneul move new` command to create the template for a new package called `example`.
...
Next, add boilerplate code to get the keypair for the currently active address in the Haneul Client CLI:
...
const keyPair = (() => {
    const keystore = JSON.parse(
        readFileSync(
            path.join(homedir(), '.haneul', 'haneul_config', 'haneul.keystore'),
            'utf8',
        )
    );
...
const { modules, dependencies } = JSON.parse(
    execSync(
        `${HANEUL} move build --dump-bytecode-as-base64 --path ${packagePath}`,
        { encoding: 'utf-8'},
    ),
);
```
...
import { HaneulClient, getFullnodeUrl } from '@haneullabs/haneul.js/client';
import { fromB64 } from '@haneullabs/haneul.js/utils';
import { TransactionBlock, UpgradePolicy } from '@haneullabs/haneul.js/transactions';
