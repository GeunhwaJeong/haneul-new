---
title: Haneul Programmable Transaction Blocks with the TS SDK
---

One of Haneul’s most powerful core developer primitives is Programmable Transaction Blocks. For conventional blockchains, a transaction is the fundamental unit of execution, and each transaction is typically simplistic and close to the VM execution. On Haneul, the fundamental, atomic unit of execution is elevated to the level of a complex, composable sequence of transactions where:

* Any public on-chain Move function across all smart contracts is accessible to the programmable transaction block.
* Typed outputs from earlier on-chain Move calls can be chained as typed inputs to later on-chain Move calls. These types can be arbitrary Haneul objects that carry a rich set of attributes and properties. Programmable transaction blocks can be highly heterogeneous. A single block can extract a **Player** object from a smart contract wallet, use it to make a move in a **Game**, then send a **Badge** object won by the move to a multi-game **TrophyCase**, all without publishing any new smart contract code. The natural compositionality of these blocks allow existing contracts to seamlessly interoperate with both old and new code (for example, the **Game** does not have to know/care that the user stores their **Player** in a Multisig wallet or their **Badge** in a **TrophyCase**).
* Chained transactions in a Programmable Transaction block execute and fail atomically. This [example Defi transaction block](https://explorer.haneul.io/txblock/94u4E3hLbpbURU7Ja3NRSwk4Xvm22iq9GsTC5MrtLrnN?network=testnet&ref=blog.haneul.io) is a programmable transaction block with 12 operations - it performs five swaps across three distinct pools, mutating 20 existing objects and creating seven new ones in the process.
* Each Programmable Transaction block supports up to 1,024 transactions, which enables unbounded expressivity and efficiency. You can use these blocks for homogeneous batching (such as for payments or NFT mints), and heterogeneous chains of single-sender operations as described in the two preceding examples. Both modes leverage Haneul's high-speed execution, and allow developers to push already low transaction fees even lower by packing more productive work into a single block.

With the power and convenience of Programmable Transaction blocks, developers on Haneul are constructing increasingly sophisticated blocks customized for their applications. Haneul’s programmability was highly expressive even before Perogrammable Transaction blocks. Now, a single execution can perform up to 1,024 heterogeneous operations. On most other blockchains, each of the 1,024 operations would be an individual transaction.

## Get Started

To get started using Programmable Transaction blocks, make sure that you have the latest TypeScript SDK installed.

This example starts by constructing a transaction block to send Haneul. If you are familiar with the legacy Haneul transaction types, this is similar to a `payHaneul` transaction. To construct transactions, import the `TransactionBlock` class, and construct it:
...
Inputs are how you provide external values to transaction blocks. For example, defining an amount of Haneul to transfer, or which object to pass into a Move call, or a shared object. There are currently two ways to define inputs:
...
- `txb.moveCall({ target, arguments, typeArguments  })` - Executes a Move call. Returns whatever the Haneul Move call returns.
...
**Note:** The gas budget is represented in Haneul, and should take the gas price of the transaction block into account.
...
To serialize a transaction block for sending to a wallet, Haneul recommends using the `txb.serialize()` function, which returns an opaque string representation of the transaction block that can be passed from the wallet standard dapp context to your wallet. This can then be converted back into a `TransactionBlock` using `TransactionBlock.from()`.
