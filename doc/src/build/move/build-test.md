---
title: Build and Test the Haneul Move Package
---
...
An individual Move unit test is encapsulated in a public function that has no parameters, no return values, and has the `#[test]` annotation. The testing framework executes such functions when you call the `haneul move test` command from the package root (`my_move_package` directory as per our running example):
...
[Transfer module](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-framework/packages/haneul-framework/sources/transfer.move):
...
---
**Tip:**
Use a filter string to run only a matching subset of the unit tests. With a filter string provided, the `haneul move test` checks the fully qualified (`<address>::<module_name>::<fn_name>`) name for a match.

Example:
```
haneul move test sword
```
...
The previous testing example is largely *pure Move* and isn't specific to Haneul beyond using some Haneul packages, such as `haneul::tx_context` and `haneul::transfer`. While this style of testing is already useful for writing Move code for Haneul, you might also want to test additional Haneul-specific features. In particular, a Move call in Haneul is encapsulated in a Haneul
...
Haneul-specific testing is supported through the [test_scenario module](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-framework/packages/haneul-framework/sources/test/test_scenario.move)
that provides Haneul-related testing functionality otherwise unavailable in pure Move and its [testing framework](https://github.com/move-language/move/blob/main/language/documentation/book/src/unit-testing.md).

The `test_scenario` module provides a scenario that emulates a series of Haneul transactions, each with a potentially different user executing them. A test using this module typically starts the first transaction using the `test_scenario::begin` function. This function takes an address of the user executing the transaction as its argument and returns an instance of the `Scenario` struct representing a scenario.

An instance of the `Scenario` struct contains a per-address object pool emulating Haneul object storage, with helper functions provided to manipulate objects in the pool. After the first transaction finishes, subsequent test transactions start with the `test_scenario::next_tx` function. This function takes an instance of the `Scenario` struct representing the current scenario and an address of a user as arguments.

Update your `my_module.move` file to include [entry functions](index.md#entry-functions) callable from Haneul that implement sword creation and transfer. With these in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put these functions after the accessors (Part 5 in comments).
...
The code of the new functions uses struct creation and Haneul-internal modules (`TxContext` and `Transfer`) in a way similar to what you have seen in the previous sections. The important part is for the entry functions to have correct signatures as described in [Write Smart Contracts with Haneul Move](index.md#entry-functions).
