...
Haneul provides a list of Move library functions that allows us to manipulate objects in Haneul.

## Object ownership
Objects in Haneul can have different ownership types. Specifically, they are:
...
### Transfer to object
We can also transfer an object to be owned by another object. Note that the ownership is only tracked in Haneul. From Move's perspective, these two objects are still more or less independent, in that the child object isn't part of the parent object in terms of data store.
...
[object_owner.move](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-core/src/unit_tests/data/object_owner/sources/object_owner.move).
...
let info = haneul::object::new(ctx);
```

To obtain the current transaction sender's address:
```
haneul::tx_context::sender(ctx)
```
