...
zkLogin is a Haneul primitive that lets wallets and apps link Haneul objects with an OAuth credential—enabling users to perform transactions using both the credential and attached objects.
...
1. **Simple Onboarding**: With the familiar OAuth login flow, zkLogin enables users to easily create a Haneul wallet and engage with Haneul—eliminating the need for cryptographic key management.
...
1. **Privacy-Focused**: Zero knowledge proofs prevent third parties from linking Haneul addresses and OAuth identifiers.

1. **Natively Supported**: zkLogin is a protocol-level signature scheme, benefiting from [crypto agility](https://docs.haneul.io/devnet/learn/cryptography/haneul-signatures). Unlike smart contract-based alternatives, it will smoothly integrate with sponsored transactions and MultiSig.
...
If you want to understand how zkLogin works, including how the zero-knowledge proof is generated, and how Haneul verifies an zkLogin transaction, see [this section](#how-zklogin-works).
...
```bash
curl -X POST http://salt.api-devnet.haneul-labs.com/get_salt -H 'Content-Type: application/json' -d '{"token": "$JWT_TOKEN"}'
...
Here's an example request and response for the Haneul Labs-maintained proving service (the endpoint is currently experimental and only available in devnet).
...
```bash
curl -X POST http://prover-devnet.haneul-labs.com:8080/zkp -H 'Content-Type: application/json' -d '{"jwt":"$JWT_TOKEN","extendedEphemeralPublicKey":"84029355920633174015103288781128426107680789454168570548782290541079926444544","maxEpoch":"10","jwtRandomness":"100681567828351849884072155819400689117","salt":"248191903847969014646285995941615069143","keyClaimName":"sub"}'
...
First, sign the transaction bytes with the ephemeral private key. This is the same as [traditional KeyPair signing](https://haneul-typescript-docs.vercel.app/typescript/cryptography/keypairs).
  ```typescript
  const ephemeralKeyPair = new Ed25519Keypair();
...
2. Validators: Entities that have already participated in securing the Haneul Network.
...
Once all contributions are concluded, the ultimate CRS and a verification script will be made available for public examination. The Haneul Foundation will run the verification to ensure the integrity of the ceremony and we also invite anyone to verify the same.
...
Interestingly though, because Haneul native supports Multisig wallets, one can always include one or more zkLogin signers inside a Multisig wallet for additional security, such as using the zkLogin part as 2FA.
