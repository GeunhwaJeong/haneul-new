...
If you wish to use the Haneullabs ran ZK Proving Service, please contact us for whitelisting your registered client ID. Only valid JWT token authenticated with whitelisted client IDs are accepted.
...
```bash
curl -O https://github.com/haneul-foundation/zklogin-ceremony-contributions/blob/main/phase2/final.zkey
```
...
1. If you want to compile the prover from scratch (for performance reasons), please see our fork of [rapidsnark](https://github.com/GeunhwaJeong/rapidsnark#compile-prover-in-server-mode). You'd need to compile and launch the prover in server mode.
...
The following command runs the same signature verify function that is used in Haneul validators. You can examine a zkLogin signature with its transaction bytes (both in Base64) to validate. `$EPOCH` is the current epoch of the network and it should be smaller than the max_epoch specified in the signature.

```bash
$HANEUL_BINARY keytool zk-login-sig-verify --sig $ZKLOGIN_SIG --tx-bytes $TX_BYTES --provider Google --curr-epoch $EPOCH
```