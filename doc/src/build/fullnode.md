...
After Haneul starts performing transaction pruning on Full nodes to remove historical transactions and their effects, 
...
The archive is a history of all transaction data on Haneul, trailing behind the latest checkpoint by 10 minutes. 
You should enable this for all nodes as a best practice. To configure a Haneul node to automatically fall back to a 
...
state-archive-read-config:
  - object-store-config:
      object-store: "S3"
      # Use haneullabs-testnet-archives for testnet 
      # Use haneullabs-mainnet-archives for mainnet
      bucket: "haneullabs-<testnet|mainnet>-archives"
...
```
To configure a Haneul node to automatically fall back to a GCS archive, add the following to your `fullnode.yaml` file:
```yaml
state-archive-read-config:
  - object-store-config:
      object-store: "GCS"
