---
title: Run a Haneul Full Node with Indexer
---
...
The Haneul Full node indexer offloads functions from the Haneul Full node, processing most API endpoints to lessen the computational burden of the Full node. Without the indexer running, you might not have all available RPC functions available to optimize transaction processing on the network.        

## Features

Haneul Full nodes with indexer:

 * Track and verify the state of the blockchain, independently and locally.
 * Serve read requests from clients.
 * Maximize Haneul network throughput.
...
Complete instances of a Haneul Full node include an indexer with Postgres database integration that adds support for all JSON-RPC endpoints. The indexer processes transactions that don't require immediate validation, allowing the Full node to more effectively interact with the network.  
...
Follow the instructions here to run your own Haneul Full node with indexer.
...
To get a Full node with indexer running in Docker, follow the instructions in the [Docker Readme](https://github.com/GeunhwaJeong/haneul/blob/main/docker/fullnode-x/README.md) for `fullnode-x`.
...
### Setting up a Haneul indexer from source

Before setting up your Haneul indexer, make sure you have the following prerequisites on your system:
...
With Postgres and Diesel CLI installed, follow these steps to set up the indexer:
 ...
 1. Open a Terminal or Console to the `haneul/crates/haneul-indexer` directory. 
...
Refer to the `haneul-indexer` module [Readme file](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-indexer/README.md) for additional information and troubleshooting.

After completing these steps, you're ready to start your Haneul Full node and indexer. 

### Starting services

At this point, your Haneul Full node and indexer are ready to connect to the Haneul network.
 ...
 1. To start the Haneul indexer binary, open a new Terminal or Console to the `haneul/crates/haneul-indexer` directory.
 1. Using the database connection URL you used when setting up the indexer, run the following command:
    ```sh
    cargo run --bin haneul-indexer -- --db-url "<DATABASE-URL>" --rpc-client-url "http://0.0.0.0:9000"
    ```

If your setup is successful, your Haneul Full node is now connected to the appropriate network and your indexer is processing transactions.
...
Whenever Haneul releases a new version, the network resets and restarts as a new network with no data. You must update your Full node and indexer with each Haneul release to ensure compatibility with the network.
