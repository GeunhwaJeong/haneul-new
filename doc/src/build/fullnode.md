---
title: Run a Haneul Fullnode
---

We welcome you to run your own Haneul fullnode! Haneul fullnodes run a service that
...
over time, and we're seeking feedback in the form of [Haneul Code Bug issues filed in
GitHub](https://github.com/GeunhwaJeong/haneul/issues/new/choose) for any problems
encountered.

## Features

Haneul fullnodes exist to:
...
Today Haneul fullnodes sync with validators to be able to learn about newly
...
The Haneul fullnode is essentially a read-only view of the network state. Unlike
...
Follow the instructions here to run your own Haneul fullnode.
...
We recommend running Haneul fullnodes on Linux. The Haneul team supports the Ubuntu and
Debian distributions and tests against
[Ubuntu version 18.04 (Bionic Beaver)](https://releases.ubuntu.com/18.04/).

That said, you are welcome to run a Haneul fullnode on the operating system of your
choosing and submit changes to accommodate that environment. See [Install Haneul](../build/install.md)
...
Note, you will fork the Haneul repository here rather than clone it as described in
...
[Fullnode Docker README](https://github.com/GeunhwaJeong/haneul/tree/main/docker/fullnode#readme)
to run a Haneul fullnode using Docker, including [resetting the environment](https://github.com/GeunhwaJeong/haneul/tree/main/docker/fullnode#reset-the-environment).
...
issues building and installing the Haneul binaries, update all packages above and re-install.
...
1. Check out the `devnet` branch:
    ...
    ```
1. Make a copy of the [fullnode configuration template](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-config/data/fullnode-template.yaml):
   ```shell
   $ cp crates/haneul-config/data/fullnode-template.yaml fullnode.yaml
   ```
1. Download the latest
   [`genesis`](https://github.com/GeunhwaJeong/haneul-genesis/raw/main/devnet/genesis.blob)
...
1. Optional: You can skip this set of steps if you are willing to accept the default paths to
    ...
    1. Update the `db-path` field with the path to where the fullnode's database
       ...
       `./haneuldb` relative to your current directory:
       ```yaml
       db-path: "/path/to/haneuldb"
       ```
...
The [Haneul Explorer](https://explorer.devnet.haneul.io/) lets you configure where
...
Observability](https://docs.haneul.io/contribute/observability).
...
1. Restart your Haneul fullnode:
    ```shell
    $ cargo run --release --bin haneul-node -- --config-path fullnode.yaml
    ```
