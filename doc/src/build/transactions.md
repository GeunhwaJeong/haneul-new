...
Native transactions are optimized versions of common Haneul operations. Each native transaction is semantically equivalent to a specific Move call but has a lower gas cost.
...
This transaction type is similar to the Transfer Object transaction type, but the input object type must be a HANEUL coin--that is, an object of type `haneul::coin::Coin<haneul::haneul::HANEUL>`. The benefit of this transaction type is that a separate coin object is not needed for gas. The gas payment is taken from the HANEUL coin being transferred.
Optionally, an amount can be specified for partial transfers.

In addition to the common metadata above, a transfer HANEUL transaction includes the following fields:

* Input: An object reference pointing to a `haneul::coin::Coin<haneul::haneul::HANEUL>` object owned by the sender.
...
Inputs: A list of unique object references pointing to mutable objects owned by the sender. The objects must all have the same type: `haneul::coin::Coin<T>` with arbitrary `T`--that is, any fungible token. The list must contain at least two objects. All objects except the first one will be destroyed, and the new value of the first object will be its old value plus the sum of the value of all destroyed objects. The gas input object from above cannot also appear as an object input.
...
* Transactions are executed by Haneul [validators](../learn/architecture/validators.md).
