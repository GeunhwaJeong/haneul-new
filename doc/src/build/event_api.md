...
API. You can use this service with Haneul client to filter and subscribe to a real-time event stream generated from Move or
from the Haneul network.

The client provides an [event filter](#event-filters) to limit the scope of events. Haneul returns a notification with the
event data and subscription ID for each event that matches the filter.

## Move event

Move calls emit Move events. You can [define custom events](https://examples.haneul.io/basics/events.html) in Move
contracts.
...
## Haneul event query filter

You can use the `EventFilter` object to query a Haneul node and retrieve events that match query criteria.
...
```shell
>> {"jsonrpc":"2.0", "id": 1, "method": "haneul_subscribeEvent", "params": [{"All":[{"Package":"<PACKAGE-MODULE-ID>"}, {"MoveModule": {"package": "<PACKAGE-ID>", "module": "nft"}}]}]}
