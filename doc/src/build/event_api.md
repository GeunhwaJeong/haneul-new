...
Haneul [Full nodes](fullnode.md) support publish / subscribe using [JSON-RPC](json-rpc.md) notifications via the WebSocket
API. This service allows clients to filter and subscribe to a real-time event stream generated from Move or from the Haneul
network.
...
List of events emitted by the Haneul node.

## Move event

Move event are emitted from move call, user can [define custom events](https://examples.haneul.io/basics/events.html) in
the move contract.
...
Haneul [full node](fullnode.md) supports publish / subscribe using [JSON-RPC](json-rpc.md) notifications via the WebSocket
API.
...
Haneul event publish / subscribe uses `EventFilter` to enable fine control of the event subscription stream;
...
by the [Haneul CLI client](cli-client.md#creating-example-nfts) `create-example-nft` command:
