---
title: Haneul Events API
---

Haneul [Full nodes](fullnode.md) support publish / subscribe using [JSON-RPC](json-rpc.md) notifications via the WebSocket API. You can use this service with Haneul client to filter and subscribe to a real-time event stream generated from Move or from the Haneul network.

The client provides an [event filter](#event-filters) to limit the scope of events. Haneul returns a notification with the event data and subscription ID for each event that matches the filter.

# Event types

A Haneul node emits the following types of events:
...
Move calls emit Move events. You can [define custom events](https://examples.haneul.io/basics/events.html) in Move contracts.
...
## Haneul event query criteria

You can use the `EventQuery` criteria object to query a Haneul node and retrieve events that match query criteria.
...
The following example demonstrates how to subscribe to Move events (`MoveEvent`) emitted by the `0x2::devnet_nft` package from the [Haneul Client CLI](cli-client.md#creating-example-nfts) `create-example-nft` command:

```shell
>> {"jsonrpc":"2.0", "id": 1, "method": "haneul_subscribeEvent", "params": [{"All":[{"EventType":"MoveEvent"}, {"Package":"0x2"}, {"Module":"devnet_nft"}]}]}
...
```shell
>> {"jsonrpc":"2.0", "id": 1, "method": "haneul_unsubscribeEvent", "params": [3121662727959200]}
