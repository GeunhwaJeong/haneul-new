...
Haneul Kiosk is a primitive, or building block (a module in the Haneul Framework), you can use to build a trading platform for digital assets. Haneul Kiosk supports adding digital assets that you can store and list for sale to other users. You can also define rules for the kiosk as part of a transfer policy that controls how purchasers can use the asset after purchase. 
To add digital assets to your kiosk to list for sale, you must first create and publish a package to Haneul as part of a programmable transaction block. The package must define a type (T). You can then create a transfer policy (`TransferPolicy`) using the `Publisher` object. The transfer policy which determines the conditions that must be met for the purchase to succeed. You can specify different requirements for each asset and transaction. To learn more about transfer policies, see [Transfer Policy](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/transfer_policy.md). To learn more about using the Publisher object, see [Publisher](https://examples.haneul.io/basics/publisher.html) in Haneul by Example.

To extend and customize Kiosk functionality, Haneul Kiosk also supports building extensions that take full advantage of the highly accessible, composable, and dynamic nature of objects on Haneul.

## Haneul Kiosk architecture
...
Haneul Kiosk is built on the following principles:
...
* **Foundation for commerce applications on Haneul** - Haneul Kiosk defines a base framework to build on, and each change or enhancement goes through rigorous testing to ensure compatibility within the community.
* **Permissioned expandability** - The base implementation of Haneul Kiosk is generic. Any community or third-party extensions and modifications must maintain and ensure asset security. For example, any transferring of ownership for a kiosk must be clearly communicated to users and require their consent. Any extensions built on Haneul Kiosk must use an explicit entry call to install them transparently.

## Haneul Kiosk for collectors and traders

Anyone can create a Haneul Kiosk. As a kiosk owner, you can sell any asset with a type (T) that has a shared `TransferPolicy` available, or you can use a kiosk to store assets even without a shared policy. You can’t sell or transfer any assets from your kiosk that do not have an associated transfer policy available. 
...
## Create a Haneul Kiosk

To create a Haneul Kiosk, you need to have an active address on the Haneul network you use. The address is the owner of the `Kiosk` object you create.
To create a base Haneul kiosk (`Kiosk` object):
...
For a full list of the available parameters, see [Haneul Kiosk](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/kiosk.md) in the Haneul Framework documentation.

## Place an item in your Haneul Kiosk

To add an asset to your kiosk, use the `place` function. Only the kiosk owner has permission to perform actions on a `Kiosk` object - you must hold the `KioskOwnerCap` to [`place`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/kiosk.md#0x2_kiosk_place) or [`withdraw`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/kiosk.md#function-withdraw) an item from a kiosk.

To place an item in a Haneul kiosk:

```rust
kiosk::place(Kiosk, KioskOwnerCap, id)
```

To place an item in your kiosk as a [dynamic field](https://docs.haneul.io/build/programming-with-objects/ch5-dynamic-fields):

```rust
kiosk::place(Kiosk, KioskOwnerCap, Item)
```

If you also want to list an item after you place it in a kiosk, you can use the [`place_and_list`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/kiosk.md#0x2_kiosk_place_and_list) function to place the item in the kiosk and then list it.
To place and list an item in a Haneul kiosk:
...
As a kiosk owner, you can use the [`list`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/kiosk.md#0x2_kiosk_list) function to list any item you placed in the kiosk. Buyers can purchase only items that have an associated transfer policy ([`TransferPolicy`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/transfer_policy.md) object) that is shared and available to them. Use the `list` function to list assets already placed in your kiosk.

To list an item in a Haneul kiosk:
...
## Remove an item from your Haneul kiosk
...
To take an item from your Haneul kiosk:
