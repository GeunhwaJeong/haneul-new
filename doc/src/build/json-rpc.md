...
curl --location --request POST $HANEUL_RPC_HOST \
--header 'Content-Type: application/json' \
--data-raw '{ "jsonrpc":"2.0", "method":"haneul_syncAccountState", "params":["{{address}}"], "id":1}'
```
...
curl --location --request POST $HANEUL_RPC_HOST \
--header 'Content-Type: application/json' \
--data-raw '{ "jsonrpc":"2.0", "method":"haneul_getOwnedObjects", "params":["{{address}}"], "id":1}' | json_pp
```
...
genesis from `wallet.conf`. Ensure you have run [`haneul_syncAccountState`](#haneul_syncaccountstate)
...
curl --location --request POST $HANEUL_RPC_HOST \
--header 'Content-Type: application/json' \
--data-raw '{ "jsonrpc":"2.0", "method":"haneul_getObjectInfoRaw", "params":["{{object_id}}"], "id":1}' | json_pp
```
...
2022-04-25T18:50:06.031765Z  INFO haneul::haneul_commands: Address : 51DEF2379C833A065B21A5675B09D8A04537C699
...
would for [`haneul_transferCoin`](#haneul_transfercoin) - please not additional
...
#### 2, Sign the transaction
Follow the instructions to [sign the transaction](json-rpc.md#2-sign-the-transaction-using-the-haneul-signtool).
