...
To publish a Move module, you also need to include `{{vector_of_compiled_modules}}` along with the `{{vector_of_dependency_ids}}`. To generate the values for these fields, use the `haneul move` command. The `haneul move` command supports printing the bytecode as base64 and dependency object IDs:

```
haneul move <move-module-path> build --dump-bytecode-as-base64
```
...
```
haneul move $PATH_TO_PACKAGE/my_move_package build --dump-bytecode-as-base64
