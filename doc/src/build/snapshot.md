...
Full nodes do not take snapshots by default. To enable this feature:
 ...
 1. Open your node config file. For Full node operators, this file is titled `fullnode.yaml`. If you installed your Full node using the `cargo` command, the file location defaults to the `.haneul/haneul_config` directory. If you installed from source, the `fullnode.yaml` file is in your `haneul` root folder.
 ...
 1. Save the haneul-node.yaml file and restart the node.
...
To restore from a snapshot, follow these steps:
 ...
 1. Place the snapshot into the directory that the `db-config` value points to in your node config file. For example, if the `db-config` value points to `/opt/haneul/db/authorities_db` and you want to restore from epoch 10, then copy the snapshot to the directory with this command: 
