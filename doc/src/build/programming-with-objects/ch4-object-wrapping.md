...
In many programming languages, you organize data structures in layers by nesting complex data structures in another data structure. In Haneul Move, you can organize data structures by putting a field of `struct` type in another, like the following:
...
In the preceding example, `Bar` is a normal struct, but it is not a Haneul object since it doesn't have the `key` ability. This is common usage to organize data with good encapsulation.

To put a Haneul object struct type as a field in another Haneul object struct type, change `Bar` into:
...
In Haneul Move code, you can put a Haneul object as a field of a non-Haneul object struct type. For example, the preceding code sample defined `Foo` to not have `key`, but `Bar` to have `key, store`. This case can happen only temporarily in the middle of a Haneul Move execution, and cannot persist on-chain. This is because a non-Haneul object cannot flow across the Move-Haneul boundary, and one must unpack the non-Haneul object at some point and handle the Haneul object fields in it.
...
There are a few common ways to wrap a Haneul object into another Haneul object, and their use cases are typically different. This section describes three different ways to wrap a Haneul object with typical use cases.
...
You can also enable a swap/trade between your object and others' objects. For example, define a function that takes two objects from two addresses and swaps their ownership. But this doesn't work in Haneul! Recall from [Using Objects](ch2-using-objects.md) that only object owners can send a transaction to mutate the object. So one person cannot send a transaction that would swap their own object with someone else's object.

Haneul supports multi-signature (multi-sig) transactions so that two people can sign the same transaction for this type of use case. But a multi-sig transaction doesn't work in this scenario. 
...
`ObjectWrapper` defines a Haneul object type, wraps the object to swap as `to_swap`, and tracks the original owner of the object in `original_owner`. You might need to also pay the third party some fee for this swap. To define an interface to request a swap by someone who owns an `Object`:
...
In the preceding entry function, you must pass the object by value so that it's fully consumed and wrapped into `ObjectWrapper`to request swapping an `object`. The example also provides a fee (in the type of `Coin<HANEUL>`). The function also checks that the fee is sufficient. The example turns `Coin` into `Balance` when it's put into the `wrapper` object. This is because `Coin` is a Haneul object type and used only to pass around as Haneul objects (such as entry function arguments or objects sent to addresses). For coin balances that need to be embedded in another Haneul object struct, use `Balance` instead because it's not a Haneul object type and is much less expensive to use.
...
Find the full source code in [trusted_swap.move](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/objects_tutorial/sources/trusted_swap.move).

To view a more complex example of how to use direct wrapping, see [escrow.move](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/defi/sources/escrow.move).
...
Find the source code in [simple_warrior.move](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/objects_tutorial/sources/simple_warrior.move).

To view a more complex example, see [hero.move](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/examples/games/sources/hero.move).
