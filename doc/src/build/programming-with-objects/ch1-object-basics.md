...
Find the full code online in [ColorObject.move](../../../../haneul_programmability/examples/objects_tutorial/sources/ColorObject.move).

To compile the code, make sure you have [installed Haneul](../install.md) so that `haneul-move` is in `PATH`. In the code [root directory](../../../move_code/objects_tutorial/) (where `Move.toml` is), run:
```
haneul-move build
```
...
Again, you can find the full code in [ColorObject.move](../../../../haneul_programmability/examples/objects_tutorial/sources/ColorObject.move).
...
Now let's try to call `create` in actual transactions and see what happens. To do this, we need to start Haneul and the wallet. Follow the [Wallet guide](../wallet.md) to start the Haneul network and set up the wallet.
