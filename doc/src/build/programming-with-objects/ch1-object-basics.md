...
To define a struct that represents a Haneul object type, we must add a `key` capability to the definition, and the first field of the struct must be the `id` of the object with type `VersionedID` from the [ID library](../../../../haneul_programmability/framework/sources/ID.move):
```rust
...
> :books: In both core Move and Haneul Move, the [key ability](https://github.com/diem/move/blob/main/language/documentation/book/src/abilities.md#key) denotes a type that can appear as a key in global storage. However, the structure of global storage is a bit different: core Move uses a (type, `address`)-indexed map, whereas Haneul Move uses a map keyed by object IDs.

> :bulb: The `VersionedID` type is internal to Haneul, and you most likely won't need to deal with it directly. For curious readers, it contains the unique `ID` of the object and the version of the object. Each time a mutable object is used in a transaction, its version will increase by 1.
...
All of the APIs for adding objects to persistent storage live in the [`Transfer`](../../../../haneul_programmability//framework/sources/Transfer.move) module. One key API is:
```rust
...
To compile the code, make sure you have followed the [Haneul installation guide](../install.md) so that `haneul-move` is in `PATH`. In the code [root directory](../../../move_code/objects_tutorial/) (where `Move.toml` is), you can run:
```
haneul-move build
```

### Writing Unit Tests
After defining the `create` function, we want to test this function in Move using unit tests, without having to go all the way to sending Haneul transactions. Since Haneul manages global storage separately outside of Move, there is no direct way to retrieve objects from global storage within Move. This poses a question: after calling the `create` function, how do we check that the object is properly transferred?

To assist easy testing in Move, we provide a comprehensive testing framework in the [TestScenario](../../../../haneul_programmability/framework/sources/TestScenario.move) module, which allows us to interact with objects that are put into the global storage, so that we can test the behavior of any function directly in Move unit tests. A lot of this is also covered in our [Move testing doc](../move.md#haneul-specific-testing).

The idea of `TestScenario` is to emulate a series of Haneul transactions, each sent from a particular address. A developer writing a test starts the first transaction using the `TestScenario::begin` function that takes the address of the user sending this transaction as argument and returns an instance of the `Scenario` struct representing a test scenario.
...
Now let's try to call `create` in actual transactions and see what happens. To do this we need to start Haneul and the wallet. Please follow the [Wallet guide](../wallet.md) to start the Haneul network and setup the wallet.
