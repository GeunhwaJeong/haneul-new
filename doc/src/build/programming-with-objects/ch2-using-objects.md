...
The [Object Basics](./ch1-object-basics.md) chapter covered how to define, create and take ownership of a Haneul object in Haneul Move. This chapter describes how to use objects that you own in Haneul Move calls.

Haneul authentication mechanisms ensure only you can use objects owned by you in Haneul Move calls. To use an object in Haneul Move calls, pass them as parameters to an [entry function](../move/index.md#entry-functions). Similar to Rust, there are a few ways to pass parameters, as described in the following sections.
...
You can also pass objects by value into an entry function. By doing so, the object is moved out of Haneul storage. It is then up to the Haneul Move code to decide where this object should go.

Since every [Haneul object struct type](./ch1-object-basics.md#define-haneul-object) must include `UID` as its first field, and the [UID struct](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-framework/packages/haneul-framework/sources/object.move) does not have the `drop` ability, the Haneul object struct type cannot have the [drop](https://github.com/move-language/move/blob/main/language/documentation/book/src/abilities.md#drop) ability either. Hence, any Haneul object cannot be arbitrarily dropped and must be either consumed (for example, transferred to another owner) or deleted by [unpacking](https://move-book.com/advanced-topics/struct.html#destructing-structures), as described in the following sections.

There are two ways to handle a pass-by-value Haneul object in Move:
 * delete the object
 * transfer the object
...
However, the `id` field of a Haneul object requires special handling. You must call the following API in the [object](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-framework/packages/haneul-framework/sources/object.move) module to signal Haneul that you intend to delete this object:
...
Choose an address other than the active address. If you have only one address, create another address using the [Haneul Client CLI](../cli-client.md#create-a-new-account-address).
...
```shell
$ haneul client call --gas-budget <GAS-BUDGET-AMOUNT> --package $PACKAGE --module "color_object" --function "transfer" --args \"$OBJECT\" \"$RECIPIENT\"
```

Now let's see what objects the `RECIPIENT` owns:

```shell
$ haneul client objects $RECIPIENT
```
...
```shell
$ haneul client call --gas-budget <GAS-BUDGET-AMOUNT> --package $PACKAGE --module "color_object" --function "delete" --args \"$OBJECT\"
```
...
```shell
$ haneul client switch --address $RECIPIENT
```
