...
> :books: Since every [Haneul object struct type](./ch1-object-basics.md#define-haneul-object) must include `VersionedID` as a field, and the [VersionedID struct](../../../../haneul_programmability/framework/sources/ID.move) does not have the `drop` ability, the Haneul object struct type [must not](https://github.com/diem/move/blob/main/language/documentation/book/src/abilities.md#drop) have `drop` ability either. Hence, any Haneul object cannot be arbitrarily dropped and must be either consumed or unpacked.
...
However, the `id` field of a Haneul object requires special handling. We must call the following API in the [ID](../../../../haneul_programmability/framework/sources/ID.move) module to signal Haneul that we intend to delete this object:
```rust
