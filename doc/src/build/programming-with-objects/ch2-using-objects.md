...
However, the `id` field of a Haneul object requires special handling. We must call the following API in the [ID](https://github.com/GeunhwaJeong/haneul/blob/main/haneul_programmability/framework/sources/ID.move) module to signal Haneul that we intend to delete this object:
```rust
