...
> :bulb: See `haneul::bag::ObjectTable` for the object-preserving version of `Table`.
...
> :bulb: See `haneul::bag::ObjectBag` for the object-preserving version of `Bag`.

---

The following sections explain the collection APIs. `haneul::table` will be used as the basis for code examples, with explanations where other modules differ.
...
This function takes the collection by value. If it contains no entries, it will be deleted, otherwise the call will abort. `haneul::table::Table` also has a convenience function,
