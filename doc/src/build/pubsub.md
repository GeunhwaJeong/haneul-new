...
Haneul [Full nodes](fullnode.md) supports publish / subscribe using [JSON-RPC](json-rpc.md) notifications via the WebSocket API. This service allows clients to filter and subscribe to a real-time event stream generated from Move or from the Haneul network.
...
Haneul event pubsub uses `EventFilter` to enable fine control of the event subscription stream;
...
### Subscribe
Here is an example of subscribing to a stream of `MoveEvent` emitted by the `0x2::devnet_nft` package, which is created by the [Haneul CLI client](cli-client.md#creating-example-nfts) `create-example-nft` command:
```shell
