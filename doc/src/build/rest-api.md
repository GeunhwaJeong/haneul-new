...
This document will walk you through setting up your own local Haneul REST Server and using the Haneul REST API to interact with a local Haneul network. This guide is useful for users that are interested in Haneul network interactions via API. For a similar guide on Haneul network interactions via CLI, refer to [wallet](https://github.com/GeunhwaJeong/haneul/blob/df4bbfa2d6672b884e9afc25b71d1f6243428dde/doc/src/build/wallet.md) documentation.  
...
```shell
export HANEUL_GATEWAY_HOST=http://127.0.0.1:5000
```

To initialize and start the network, you need to invoke the /haneul/genesis and /haneul/start endpoint as mentioned below.

## Connect to remote REST Server

Coming soon: Connect to Haneul devnet, testnet, or mainnet
...
The recomended way to test the Haneul REST API is to use Postman. 
...
each with five gas objects. These are Haneul [objects](https://github.com/GeunhwaJeong/haneul/blob/cef5136b9af3dfdb767d0cc77d61356f1df6ff96/doc/src/build/objects.md) used
to pay for Haneul [transactions](https://github.com/GeunhwaJeong/haneul/blob/5c48a87ceee35ccbf0d6276bb3ef17bf5a0eb7d5/doc/src/build/transactions.md#native-transaction),
such as object transfers or smart contract (Move) calls.

```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/haneul/genesis | json_pp
```
...
```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/haneul/start | json_pp
```
...
```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/haneul/stop
```
...
```shell
curl --location --request GET $HANEUL_GATEWAY_HOST/docs | json_pp
```
...
```shell
curl --location --request GET $HANEUL_GATEWAY_HOST/addresses | json_pp
```
...
```shell
curl --location --request GET $HANEUL_GATEWAY_HOST'/objects?address={{address}}' | json_pp
```
...
```shell
curl --location --request GET $HANEUL_GATEWAY_HOST'/object_info?objectId={{object_id}}' | json_pp
```
...
```shell
curl --location --request GET $HANEUL_GATEWAY_HOST'/object_schema?objectId={{object_id}}' | json_pp
```
...
Refer to [transactions](https://github.com/GeunhwaJeong/haneul/blob/5c48a87ceee35ccbf0d6276bb3ef17bf5a0eb7d5/doc/src/build/transactions.md#native-transaction) documentation for more information about a native transfer.

```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/transfer \
...
```
Notes:
- Non-coin objects cannot be transferred natively and will require a Move call. Refer to [Move](https://github.com/GeunhwaJeong/haneul/blob/df4bbfa2d6672b884e9afc25b71d1f6243428dde/doc/src/build/move.md#move-structs) documentation to learn more about coin objects.
...
```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/call \
--header 'Content-Type: application/json' \
--data-raw '{
...
- To learn more about what `args` are accepted in a Move call, refer to [haneul-json](https://github.com/GeunhwaJeong/haneul/blob/6b6cc14f14a8cd71b87b560524373bd0faa2689c/doc/src/build/haneul-json.md) documentation for further information.
...
```shell
curl --location --request POST $HANEUL_GATEWAY_HOST/sync \
--header 'Content-Type: application/json' \
--data-raw '{
    "address": "{{address}}"
}' | json_pp
