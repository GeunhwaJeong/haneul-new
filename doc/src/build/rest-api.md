...
Welcome to the Haneul REST API quick start.

This document walks you through setting up your own local Haneul REST Server and using the Haneul REST API to interact with a local Haneul network. This guide is useful for developers interested in Haneul network interactions via API. For a similar guide on Haneul network interactions via CLI, refer to the [wallet](wallet.md) documentation.
...
Follow the instructions to [create](wallet.md#genesis) and [start](wallet.md#starting-the-network) the Haneul network.
...
```shell
$ export HANEUL_GATEWAY_HOST=http://127.0.0.1:5001
```

## Haneul REST API

In the following sections we will show how to use Haneul's REST API using
...
```shell
$ curl --location --request POST $HANEUL_GATEWAY_HOST/api/sync_account_state \
...
```shell
$ curl --location --request GET $HANEUL_GATEWAY_HOST'/api/objects?address={{address}}' | json_pp
```
...
```shell
$ curl --location --request GET $HANEUL_GATEWAY_HOST'/api/object_info?objectId={{object_id}}' | json_pp
```
...
```shell
$ curl --location --request POST $HANEUL_GATEWAY_HOST/api/new_transfer \
--header 'Content-Type: application/json' \
--data-raw '{
...
```
#### 2, Sign the transaction using the Haneul signtool
```shell
$ haneul signtool --address <owner_address> --data <tx_bytes>
```
The signing tool will create and print out the signature and public key information.
You will see output resembling:
```shell
2022-04-04T21:42:44.915471Z  INFO haneul::haneul_commands: Data to sign : 
2022-04-04T21:42:44.915626Z  INFO haneul::haneul_commands: Address : 8D6E335822A070961E1F54895912F5B354B5537D
2022-04-04T21:42:44.915915Z  INFO haneul::haneul_commands: Public Key Base64: jAs+VmaKuEf4fpLNiLIHknEpft3zmy9b1v3AOYn/v4g=
2022-04-04T21:42:44.915919Z  INFO haneul::haneul_commands: Signature : FULrl7iI1aQnb5OgCksYWJw6/Fv44wZZfeMXXN7CZJyooR2+Iu0WIVWUO61+r47aEidLeIewV8iLXFW7GPoYAw==
```

#### 3, Execute the transaction using the transaction data, signature and public key.
```shell
$ curl --location --request POST $HANEUL_GATEWAY_HOST/api/execute_transaction \
--header 'Content-Type: application/json' \
--data-raw '{
    "tx_bytes": "{{tx_bytes}}",
    "signature": "{{signature}}",
...
```shell
$ curl --location --request POST $HANEUL_GATEWAY_HOST/api/move_call \
--header 'Content-Type: application/json' \
--data-raw '{
...
Follow the instructions to [sign the transaction](rest-api.md#2-sign-the-transaction-using-the-haneul-signtool).
...
NOTE: A Publish endpoint is in the works, but for now the only way to add a new module is to have it included as part of genesis. To do this, add your Move module to `haneul_programmability/framework/sources` before you hit the genesis endpoint. Once you have done this you will be able to use `"packageObjectId": "0x2"` in the call endpoint to find your Move module.
...
```shell
$ curl --location --request POST $HANEUL_GATEWAY_HOST/publish \
--header 'Content-Type: application/json' \
--data-raw '{
    "sender": "{{owner_address}}",
...
## Postman setup
The recommended way to test the Haneul REST API is to use Postman.
...
Coming soon - alternative ways of working with Haneul's REST API. Connect to Haneul devnet, testnet, or mainnet!
