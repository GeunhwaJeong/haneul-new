...
- `budget` is the maximum amount a user pays for a transaction. The value for `budget` is expressed in GEUNHWA, a fractional unit of HANEUL. Each GEUNHWA equals 10^-9 HANEUL - 1 HANEUL equals 1 Billion MISTS. The sum of all coins in `payment` must be greater than or equal to the amount specified for `budget`.
