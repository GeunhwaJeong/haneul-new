---
title: Haneul Gas Fees
---

To learn more about gas fees in the context of Haneul Tokenomics, see [Gas in Haneul](../learn/tokenomics/gas-in-haneul.md).

This topic provides a summary of the changes to the Haneul Gas model for Haneul Mainnet.
...
- `payment` is a vector of gas coin. It accepts only HANEUL coins, and the coins must be unique. On execution, Haneul merges the coins together into a single coin object, specifically the first one. After execution, Haneul deletes all of the coins except the first coin that holds the balance of the coins combined minus the gas cost for the transaction.
...
- `budget` is the maximum amount a user pays for a transaction. The value for `budget` is expressed in GEUNHWA, a fractional unit of HANEUL. Each GEUNHWA equals 10^-9 HANEUL - 1 HANEUL equals 1 Billion GEUNHWA. The sum of all coins in `payment` must be greater than or equal to the amount specified for `budget`.
...
Gas usage is proportional to the resources used. Specifically, it is proportional to **execution**, in terms of computation and memory, and **DB access**, in terms of Haneul object reads and writes.

There are 2 main values defined in the system that contribute to gas consumption:

- **RGP** (Reference Gas Price): set up at the beginning of each epoch by the validator committee. RGP is a multiplier applied to each unit of computation. Initial **RGP** at genesis is 1,000 GEUNHWA.
- **Storage Price**: set up in the system and intended to be more stable than RGP, it is a multiplier for the number of bytes written to storage. This is a one-time charge for the life of the object on the network. Initial **Storage Price** at genesis is **76** GEUNHWA.

## Transaction Output: GasCostSummary

On transaction output, Haneul provides the following information about gas:
...
The remaining values provide an insight into the storage charges. Each time Haneul writes to or reads from an object in the database, the object is conceptually _deleted_ and _restored_ to the DB. A transaction could also delete objects and create new objects.
...
1. Out of gas in execution, all effects are reverted. **However** there is still a charge for storage as input objects are accessed. If there is any storage budget left (gas budget higher than max computation budget) Haneul uses that budget, charges for storage of the input objects, and returns an **Insufficient Gas** (go to step 3). On success, `GasCostSummary` has all fields greater than 0 and reflects the true cost of the transaction.
2. Execution is successful, but charging for storage causes *Out of Gas*.
   ...
   Note that this behavior described will be implemented before Haneul Mainnet, but is not currently supported. As of release .30, *Out Of Gas* occurrence while charging storage collects the entire budget and assigns it to `computation_cost`. The transaction aborts with an `InsufficientGas` error.
3. If storage charges for input objects fail, Haneul gets all of the budget assigned to `computation_cost`, and leaves the object storage value unchanged. In this case, the values related to storage in the `GasCostSummary` are all zeroed and the `computation_cost` is the only value set in `GasCostSummary`. The transaction aborts with an `InsufficientGas`.
