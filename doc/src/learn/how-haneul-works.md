...
Haneul also supports smart contracts written in Move, a language designed for blockchains with strong inherent security and a more understandable programming model.
...
But contrary to a blockchain, Haneul does not stop the flow of transactions in order to witness the convergence until the end of an epoch.
...
2. Each Haneul validator performs validity checks on transactions and adds a signature to valid ones. Each signature is weighted proportionally to the amount staked with the validator. 
3. The quorum driver collects signatures with a combined weight greater than or equal to 2/3 of the total stake (quorum of stake) into a _certificate_ and broadcasts it to all Haneul validators.
...
2. After Haneul sequences the transaction, it executes it to produce effects using the same flow as (5) above.
