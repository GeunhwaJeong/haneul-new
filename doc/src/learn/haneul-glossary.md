...
A certificate is the mechanism proving a transaction was approved or certified. Validators vote on transactions, and aggregators collect a Byzantine-resistant-majority of these votes into a certificate and broadcasts it to all Haneul validators, thereby ensuring finality.
...
For more information, see [Genesis](../build/haneul-local-network.md#genesis).
...
For more information, see [Haneul Objects](../learn/objects.md).
...
Single-writer objects are owned by one address. In Haneul, transactions affecting only single-writer objects owned by the same address may proceed with only a verification of the sender’s address, greatly speeding transaction times. These are *simple transactions*. See [Single-Writer Apps](single-writer-apps.md) for example applications of this simple transaction model.
