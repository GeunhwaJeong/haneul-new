...
to process them correctly despite some of the validators operating Haneul not following the protocol correctly (fault tolerance).
...
* The Haneul system operates through a protocol between a set of independent validators. Yet all its security properties are preserved
...
The Haneul system is operated by a set of validators that process transactions. They implement the Haneul protocol that allows them to
reach agreement on valid transactions submitted and processed in the system. 

The agreement protocols Haneul uses tolerate a fraction of validators not following the Haneul protocol correctly, through
...
remaining validators that correctly follow the Haneul protocol. This is achieved throught the use of cryptographic
...
Haneul validators provide facilities for users to read all asserts they store, as well as the historical record of
...
Haneul uses the established Delegated Proof-of Stake model to periodically determine its set of validators. Users can lock and delegate their HANEUL tokens in each epoch to determine the committee of validators that operate the Haneul network in the next epoch. Anyone with over a minimum
...
This mechanism ensures that validators are accountable to Haneul users and can be rotated out at the first sign
