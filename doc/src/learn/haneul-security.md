...
assets, and the assurances Haneul provides for them. Smart contract designers can also learn about the overall
Haneul security architecture to ensure the asset types they design leverage Haneul to provide a secure experience
to the asset holders. 

Before diving into this be sure to read [How Haneul Works?](how-haneul-works.md) to familiarize yourself with 
the basic components of Haneul.

## Security features

We designed Haneul to provide very high security guarantees to asset owners. We ensure that assets on Haneul can be used
...
to process them correctly despite some of the authorities operating Haneul not following the protocol correctly (fault tolerance). 
...
* The Haneul system operates through a protocol between a set of independent authorities. Yet all its security properties are preserved 
  when a small subset of the authorities do not follow the protocol.
* All operations in Haneul can be audited to ensure any assets have been correctly processed. This implies all operations on Haneul
  are visible to all, and users may wish to use multiple different addresses to protect their privacy.
* Authorities are determined periodically through users of Haneul locking and delegating HANEUL tokens to one or more authorities.

## Security architecture

The Haneul system is operated by a set of authorities that process transactions. They implement the Haneul protocol that allows them to 
reach agreement on valid transactions submitted and processed in the system. 

The agreement protocols Haneul uses tolerate a fraction of authorities not following the Haneul protocol correctly, through 
the use of Byzantine fault tolerant broadcast and consensus. Specifically, each authority has some voting power,
assigned to it through the process of users delegating / voting for them using their HANEUL tokens. Haneul maintains
...
A Haneul transaction is valid and can proceed only if the owner of all owned assets it operates on digitally signs it with their private 
...
A private signature key also corresponds to a public address on the Haneul network that can be used to send a user assets or
...
All assets have a type that is defined within a Haneul Smart Contract. Haneul provides a few system contracts, such as these used to 
manage the HANEUL native token, yet also allows anyone to write and submit custom smart contracts. A transaction on an asset type 
...
they trust have audited, and understand the logic they define for operations on their assets. Haneul smart contracts are 
...
The Move smart contract language that Haneul uses is designed with ease of audit and verification in mind. You may be 
...
between different users in a safe way, according to the smart contract that defined the type of the shared asset. Haneul allows
...
remaining authorities that correctly follow the Haneul protocol. This is achieved throught the use of cryptographic 
Byzantine fault tolerant agremment protocols for broadcast and consensus defined by the Haneul protocol. These protocols
...
All transactions in Haneul have to be associated with a gas asset to cover the cost of processing by Haneul. A valid 
...
Haneul authorities provide facilities for users to read all asserts they store, as well as the historical record of
...
The extreme public auditability of Haneul also implies that all transactions and assets within Haneul are publicly
...
Haneul uses the established Delegated Proof-of Stake model to periodically determine its set of authorities. Users can lock and delegate their HANEUL tokens in each epoch to determine the committee of authorities that operate the Haneul network in the next epoch. Anyone with over a minimum 
amount of delegated stake may operate an authority. 

Authorities operate the network and provide
rewards to users that delegated their Haneul to support them as validators, through gas fee income. Authorities with poor reliability, and in turn the users that delegated their stake to them, may receive a lower reward. But user stake cannot be confiscated away either by malicious authorities or anyone in the network.

This mechanism ensures that authorities are accountable to Haneul users and can be rotated out at the first sign 
...
they are willing to operate, Haneul users also have a meaningful say on the future evolution of the Haneul system.
