...
Note: As of Mar 12, 2022, an unoptimized single-worker Haneul authority running on an 8-core M1 Macbook Pro can process 17,500 token transfer transactions per second (TPS). Performance scales linearly with the number of cores–the same machine processes 3,000 TPS in a single core configuration and increases by about 2,000 TPS with each additional core. . 

We will publish a full performance report for optimized Haneul networks of various sizes when our testnet is released.
