...
Haneul wallet and execute wallet commands through its command line
...
clone the Haneul repository to the current directory and build:
...
This will create `haneul` and `wallet` binaries in `target/release`
directory.
...
`gateway` contains the information of the Haneul network that the wallet will be connecting to, 
currently only `Embedded` gateway type is supported.

The `authorities` variable is part of the embedded gateway configuration, it contains Haneul network 
authorities' name, host and port information. It is used to establish connections to the Haneul network.
...
Haneul's genesis process will create five accounts by default; if that's
not enough, there are two ways to add accounts to the Haneul wallet if needed.

#### Use `new-address` command to generate new account

To create a new account, execute the `new-address` command in Haneul interactive console:
...
Restart the Haneul wallet after the modification; the new accounts will appear in the wallet if you query the addresses.
...
```
To view the objects owned by the accounts created in genesis, run the following command in the Haneul interactive console (substitute the address with one of the genesis addresses in your wallet): 
```shell
haneul>-$ objects --address 0DC70EA8CA82FE6966900C697B73A235B8E2346F
```
...
haneul>-$ objects --address 830F66EA8EA867DDCA479535BC12CE2852E571F2
Showing 0 results.
```
...
```shell
haneul>-$ transfer --to 830F66EA8EA867DDCA479535BC12CE2852E571F2 --object-id 1E1D62BDE28964F6FC0CE3503B5058C4DC04F1DE --gas 27073453E1B556D1B6C2E4BE94FF2A3928D788BF
...
```shell
haneul>-$ sync --address 830F66EA8EA867DDCA479535BC12CE2852E571F2
...
haneul>-$ merge-coin --primary-coin 1E1D62BDE28964F6FC0CE3503B5058C4DC04F1DE --coin-to-merge 6A506E6779CF5936145628C689045A25643ACBDC --gas B7FD91A802DAF5523CAAB69FF4652FAFA6FF4ADF --gas-budget 1000
----- Certificate ----
...
haneul>-$ split-coin --coin-id 1E1D62BDE28964F6FC0CE3503B5058C4DC04F1DE --amounts 1000 5000 3000 --gas B7FD91A802DAF5523CAAB69FF4652FAFA6FF4ADF --gas-budget 1000
----- Certificate ----
...
`haneul>-$`) that can be started as follows:
...
Welcome to the Haneul interactive shell.
...
``` shell
haneul>-$ call --function transfer --module GAS --package 0x2 --args "0xF18F5B785D5766CD85BC2247F8C73F07BFF901BB" "0x3111A757BE55F195201FD9140DCE55EAEB719D35" --gas 60DADCE6E5081C3EFCA162694D7EFD8D99D46636 --gas-budget 1000
```
...
- `--package` - ID of the package object where the module containing
  ...
  that the ID of the genesis Haneul package containing the GAS module is
...
``` shell
haneul>-$ object --id F18F5B785D5766CD85BC2247F8C73F07BFF901BB
...
``` shell
haneul>-$ publish --path /PATH_TO_PACKAGE/my_move_package --gas 60DADCE6E5081C3EFCA162694D7EFD8D99D46636 1000
```
...
From now on, we can use the package object ID in the Haneul wallet's call
...
```shell
./haneul genesis --config <Path to genesis config file>
```
...
```json
{
  ...
  "haneul_framework_lib_path": "<Paths to Haneul framework lib>",
