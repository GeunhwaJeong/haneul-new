...
* Input: An object reference pointing a mutable object owned by the sender. The object must be of type `Haneul::Coin::Coin<T>` with arbitrary `T`--that is, any fungible token. The gas input object from above cannot also appear as an object input.
