...
`haneul client upgrade` offers a simple way to upgrade packages when the
...
when it goes live (using `haneul::package::make_immutable` to burn its
...
by `haneul client upgrade`.  Custom upgrade policies work by guarding
...
[here](https://github.com/GeunhwaJeong/haneul/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/haneul-types/src/move_package.rs#L232-L251),
...
- The signature of our custom `authorize_upgrade` can be different
  from the signature of `haneul::package::authorize_upgrade` as long as
...
`haneul::package::make_immutable` on its `UpgradeCap`:
...
And run the following in its directory to add the Haneul TypeScript SDK
as a dependency:
...
defining some constants: `HANEUL` the location of the `haneul` CLI binary,
