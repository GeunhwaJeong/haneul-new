...
concrete examples to demonstrate how Move can be used in Haneul.
...
a Haneul object, and must have a unique numeric ID in addition to a
...
In order for a Move struct type to define a Haneul object type such
`Coin`, its first field must be `id: VersionedID` (`VersionedID` is a
struct type defined in the ID
[module](../haneul_programmability/framework/sources/ID.move)), and must
also have the `key` (which allows the object to be persisted in Haneul's
...
[module](../haneul_programmability/framework/sources/Coin.move), that is
the `value` function.
...
this package, please make sure that you have cloned the Haneul repository
to the current directory and built Haneul binaries as described
...
to import the ID
[package](../haneul_programmability/framework/sources/ID.move) from Haneul
...
Haneul included support for Move's testing
...
[module](../haneul_programmability/framework/sources/Transfer.move):
