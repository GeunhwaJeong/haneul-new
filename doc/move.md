...
the [Move](https://github.com/GeunhwaJeong/awesome-move) language.
This tutorial provides a brief explanation of the Move language and
includes concrete examples to demonstrate how Move can be used in Haneul.
...
blockchains with vastly different data and execution models. [Haneul](../README.md),
...
[Celo](https://www.businesswire.com/news/home/20210921006104/en/Celo-Sets-Sights-On-Becoming-Fastest-EVM-Chain-Through-Collaboration-With-Haneullabs-Labs)).
...
to following the Haneul tutorial, which we strived to make self-contained.
Further, Haneul does differ in some ways from Move, which we explore here.

In Haneul, Move is used to define, create and manage programmable Haneul
[objects](objects.md) representing user-level assets.  Haneul
imposes additional restrictions on the code that can be written in
Move, effectively using a subset of Move (a.k.a. *Haneul Move*), which
makes certain parts of the original Move documentation not applicable
to smart contract development in Haneul (e.g., there is no concept of a
[script](https://github.com/diem/move/blob/main/language/documentation/book/src/modules-and-scripts.md#scripts)
in Haneul Move). Consequently, it's best to simply follow this tutorial
and relevant Move documentation links provided in the tutorial.

Before we look at the Move code included with Haneul, let's talk briefly
about Move code organization, which applies both to code included with
Haneul and the custom code written by the developers.
...
The Haneul platform includes _framework_ Move code that is needed to
bootstrap Haneul operations. In particular, Haneul supports multiple
user-defined coin types, which are custom assets define in the Move
language. Haneul framework code contains the `Coin` module supporting
creation and management of custom coins. The `Coin` module is
located in the
[haneul_programmability/framework/sources/Coin.move](../haneul_programmability/framework/sources/Coin.move)
...
[haneul_programmability/framework/Move.toml](../haneul_programmability/framework/Move.toml)
file.
...
(`Haneul`). The combination of the package name and the module name
...
package in Haneul is also a Haneul object and must have a unique numeric
...
represent different types of user-defined coins as Haneul objects:
...
In order for a Move struct type to define a Haneul object type such as
`Coin`, its first field must be `id: VersionedID`, which is a
struct type defined in the
[ID module](../haneul_programmability/framework/sources/ID.move). The
Move struct type must
also have the `key` ability, which allows the object to be persisted
in Haneul's global storage. Abilities of a Move struct are listed after
...
`Coin<GAS>`, which represents a token used to pay for gas used in Haneul
computations - in this case, the concrete type used to parameterize the
`Coin` struct is the `GAS` struct in the
[Coin module](../haneul_programmability/framework/sources/Coin.move):
...
[Coin module](../haneul_programmability/framework/sources/Coin.move), that is
the `value` function.
...
[module](../haneul_programmability/framework/sources/GAS.move) to
...
for now - since the function is part of Haneul framework, you can trust
...
- have parameters ordered as follows:
  - one or more Haneul objects (or vectors of objects)
  - one or more primitive types (or vectors of such types)
  - a mutable reference to an instance of the `TxContext` struct
  defined in the
  [TxContext module](../haneul_programmability/framework/sources/TxContext.move)
...
You can see how the `transfer` function is called from a sample Haneul
...
this package, first clone the Haneul repository to the current directory
and [build Haneul binaries](wallet.md#build-the-binaries).

The directory structure used in this tutorial should at the moment
look as follows (assuming Haneul has been cloned to a directory called
"haneul"):
...
will be represented by Haneul objects; in particular, we want a sword to
...
to import the
[ID package](../haneul_programmability/framework/sources/ID.move) from
Haneul framework to gain access to the `VersionedID` struct type defined
in this package.
...
name, package version, local dependency path to locate Haneul framework
...
Haneul includes support for the
...
[Transfer module](../haneul_programmability/framework/sources/Transfer.move):
...
[TestScenario module](../haneul_programmability/framework/sources/TestScenario.move)
...
point of view of a Haneul developer. First, let us create
[entry functions](#entry-functions) callable from Haneul that implement
