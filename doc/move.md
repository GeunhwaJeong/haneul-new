...
Welcome to the Haneul tutorial focusing on building smart contracts using
...
demonstrate how Move can be used in the context of Haneul.
...
vastly different data and execution models. Haneul,
...
strict prerequisite to following the Haneul tutorial which we strived to
make self-contained.

More importantly, Haneul imposes additional restrictions on the code that
can be written in Move, effectively using a subset of Move (aka Haneul
Move), which makes certain parts of the original Move documentation
not applicable to smart contract development in Haneul.

## Setup

This tutorial uses Haneul Wallet CLI (command-line interface) to
demonstrate capabilities of the Haneul platform. Please see the main
[README](../README.md) for instructions on how to install Haneul and how
to setup the Haneul Wallet (use default configurations when following the
...
_genesis state_ of the Haneul platform.

- 5 [addresses](overview.md) representing individual users, each
address initialized with 5 gas objects (all Haneul [objects](objects.md),
including gas objects, are "owned" by addresses representing Haneul
users)
- 4 Haneul [authorities](authorities.md) running locally in the
`target/release` subdirectory of your local copy of the Haneul
repository
- some essential Move code needed to bootstrap the Haneul platform
...
In order for a package to be available in Haneul, that is for functions
defined in its modules to be callable from Haneul or other Move
functions, the package must be _published_ (a published package
becomes a Haneul object). We will discuss publishing
...
pre-published during Haneul's initial setup.

## Your First Move Call

The genesis state of the Haneul platform includes Move code that is
needed to initialize Haneul operations, for example to create and
manipulate gas objects. In particular the gas object is defined in the
genesis GAS module located in the
[haneul_programmability/framework/sources/GAS.move](../haneul_programmability/framework/sources/GAS.move)
...
discussed [earlier](#move-code-organization), a package in Haneul is also
an object, and must have a unique numeric ID in addition to a unique
name, so that it can be identified by the Haneul platform. For user-level
packages, this ID is assigned when the package is
[published](#package-publishing), but for the packages pre-published
during Haneul setup it is assigned in the manifest file:
...
Since as part of Haneul genesis we have user accounts populated with gas
...
[module](../haneul_programmability/framework/sources/GAS.move):
...
- `c` - it represents a gas object whose ownership is to be
  ...
  [module](../haneul_programmability/framework/sources/Coin.move)
  parameterized with another struct `GAS` defined in the GAS genesis
  [module](../haneul_programmability/framework/sources/GAS.move) (you can
...
- `_ctx` - it is a mutable reference to an instance of the `TxContext`
  ...
  [module](../haneul_programmability/framework/sources/TxContext.move)
...
of Haneul genesis will do what they are intended to do correctly.

### Transferring Gas Objects with Move

Let's first see the user addresses available as part of Haneul genesis
(accounts in Haneul are identifier. We use the following Wallet CLI
...
the GAS module using the following Haneul Wallet command:
...
- `object-args` - a list of arguments of Haneul object type (in this case
  there is only one representing the `c` parameter of the `transfer`
  function)
- `pure-args` - a list of arguments of Haneul primitive types or vectors
...
- `--gas` - an object containing gas that will be used to pay for this
  ...
  owned by this address as at this point the only objects in Haneul are
  gas objects
...
is a required argument for all functions callable from Haneul and is
...
subdirectory of the Haneul repository, as specified in the setup
