...
if [ -z "$RELEASED_COMMIT" ]; then
  ...
  # haneul-private commit that does not resolve in this repo: approximate its
...
git checkout $RELEASED_COMMIT -- crates/haneul-protocol-config/src/snapshots || exit 1
