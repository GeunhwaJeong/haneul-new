...
# check out all files in crates/haneul-protocol-config/src/snapshots at origin commit
echo "Checking out $NETWORK snapshot files"
git checkout $ORIGIN_COMMIT -- crates/haneul-protocol-config/src/snapshots || exit 1
...
grep -lE 'scoring_decision_mad_divisor|scoring_decision_cutoff_value' crates/haneul-protocol-config/src/snapshots/$NETWORK_PATTERN | xargs $SED -Ei '/(scoring_decision_mad_divisor|scoring_decision_cutoff_value)/d'
git add .
...
cargo test --package haneul-protocol-config snapshot_tests || exit 1
