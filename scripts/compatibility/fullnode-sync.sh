...
function cleanup {
    ...
    [ ! -z $HANEUL_NODE_PID ] && kill $HANEUL_NODE_PID && echo "Shutdown haneulnode process running on pid $HANEUL_NODE_PID"
}
...
while getopts "hvn:e:p:t:" OPT; do
    case $OPT in
        ...
        h)
            ...
            >&2 echo ""
            >&2 echo "Options:"
            >&2 echo " -p                 Path to haneul binary to run. If unspecified, will build from source."
...
[ ! -d "${HANEUL_RUN_PATH}/haneuldb" ] && mkdir -p ${HANEUL_RUN_PATH}/haneuldb
...
if [[ ! -f "${HANEUL_RUN_PATH}/genesis.blob" ]]; then
    echo "Copying genesis.blob for ${NETWORK}"
    curl -fLJO https://github.com/GeunhwaJeong/haneul-genesis/raw/main/${NETWORK}/genesis.blob
    mv ./genesis.blob ${HANEUL_RUN_PATH}/genesis.blob
    echo "Done"
fi
...
if [[ -z $HANEUL_BIN_PATH ]]; then
    echo "Building haneul..."
    cargo build --release --bin haneul-node
    HANEUL_BIN_PATH="target/release/haneul-node"
    echo "Done"
fi


echo "Starting haneulnode..."
RUST_LOG=$LOG_LEVEL $HANEUL_BIN_PATH --config-path ${HANEUL_RUN_PATH}/fullnode.yaml &
HANEUL_NODE_PID=$!
