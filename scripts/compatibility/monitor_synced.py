...
def await_started(start_checkpoint):
    for i in range(STARTUP_TIMEOUT_SEC):
        if get_local_metric(Metric.CHECKPOINT) != start_checkpoint:
            print(f"haneul-node started successfully after {i} seconds")
            return
        print("Awaiting haneul-node startup...")
        time.sleep(1)
    print(f"haneul-node failed to start after {STARTUP_TIMEOUT_SEC} seconds")
