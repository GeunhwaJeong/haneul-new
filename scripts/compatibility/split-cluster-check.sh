...
if [ -f "$WORKING_DIR/haneul-node-release" ] && [ -f "$WORKING_DIR/haneul-release" ] && [ -f "$WORKING_DIR/haneul-node-candidate" ]; then
  echo "Binaries already built, skipping build"
else
  echo "Building haneul-node and haneul at $RELEASE_COMMIT"

  # remember current commit
  CURRENT_COMMIT=$(git rev-parse HEAD)

  git checkout $RELEASE_COMMIT || exit 1
  cargo build --bin haneul-node --bin haneul || exit 1
  cp ./target/debug/haneul-node "$WORKING_DIR/haneul-node-release"
  cp ./target/debug/haneul "$WORKING_DIR/haneul-release"

  echo "Building haneul-node at $RELEASE_CANDIDATE_COMMIT"
  git checkout $RELEASE_CANDIDATE_COMMIT || exit 1
  cargo build --bin haneul-node || exit 1
  cp ./target/debug/haneul-node "$WORKING_DIR/haneul-node-candidate"

  echo "returning to $CURRENT_COMMIT"
  git checkout $CURRENT_COMMIT || exit 1
fi

export HANEUL_CONFIG_DIR="$WORKING_DIR/config"
rm -rf "$HANEUL_CONFIG_DIR"

"$WORKING_DIR/haneul-release" genesis --epoch-duration-ms 20000 --committee-size 4
...
while IFS= read -r -d '' file; do
  CONFIGS+=("$file")
done < <(find "$HANEUL_CONFIG_DIR" -name "127.0.0.1*.yaml" -print0)

export RUST_LOG=haneul=debug,info

# 2 release nodes
"$WORKING_DIR/haneul-node-release" --config-path "${CONFIGS[0]}" > "$LOG_DIR/node-0.log" 2>&1 &
"$WORKING_DIR/haneul-node-release" --config-path "${CONFIGS[1]}" > "$LOG_DIR/node-1.log" 2>&1 &

# 2 candidate nodes
"$WORKING_DIR/haneul-node-candidate" --config-path "${CONFIGS[2]}" > "$LOG_DIR/node-2.log" 2>&1 &
"$WORKING_DIR/haneul-node-candidate" --config-path "${CONFIGS[3]}" > "$LOG_DIR/node-3.log" 2>&1 &

# and a fullnode
"$WORKING_DIR/haneul-node-release" --config-path "$HANEUL_CONFIG_DIR/fullnode.yaml" > "$LOG_DIR/fullnode.log" 2>&1 &
