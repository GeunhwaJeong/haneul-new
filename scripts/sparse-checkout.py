...
def read_sparse_config(sparse_file=".sparse"):
    ...
    if os.path.isfile(sparse_file):
        ...
        # haneul-benchmark tests use haneul-surfer which requires move sources to be checked out
        if "crates/haneul-surfer" not in crates and "crates/haneul-benchmark" in crates:
            crates.append("crates/haneul-surfer")
...
def update_git_sparse_checkout(crates_to_checkout):
    ...
    # if we don't have haneul-framework, we probably need to add the move sources in order for tests
    # to run
    if "crates/haneul-framework" not in crates_to_checkout:
        default_directories.append("crates/haneul-framework/packages")
