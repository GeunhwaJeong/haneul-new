...
# This script attempts to update the Fastcrypto pointer in Haneul
...
function latest_fc_revision() {
	...
	cd "$FC_CHECKOUT"
	git clone --depth 1 https://github.com/GeunhwaJeong/fastcrypto
	cd fastcrypto
	git rev-parse HEAD
}
...
function current_fc_revision() {
	cd "$TOPLEVEL"
	readarray -t <<< "$(find ./ -iname '*.toml' -exec $GREP -oPe 'git = "https://github.com/GeunhwaJeong/fastcrypto", *rev *= *\"\K[0-9a-fA-F]+' '{}' \;)"
	watermark=${MAPFILE[0]}
	for i in "${MAPFILE[@]}"; do
	    if [[ "$watermark" != "$i" ]]; then
        	not_equal=true
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current Fastcrypto revision in Haneul, aborting" && exit 1
	echo "$watermark"
}
...
if [[ "$CURRENT_FC" != "$LATEST_FC" ]]; then
	echo "About to replace $CURRENT_FC with $LATEST_FC as the Narwhal pointer in Haneul"
else
	exit 0
fi
