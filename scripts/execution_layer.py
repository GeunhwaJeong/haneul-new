...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `haneul-execution/src/lib.rs` based on the current set of "
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "haneul-execution" / "src" / "lib.rs"
...
def impl(feature):
    ...
    return Path() / "haneul-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature):
    """Add dependencies for 'feature' to haneul-execution's manifest."""
    toml_path = Path() / "haneul-execution" / "Cargo.toml"
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "haneul-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of haneul-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "haneul-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `haneul-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `haneul-execution` crate that count as
