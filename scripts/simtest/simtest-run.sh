...
MSIM_TEST_SEED="$SEED" MSIM_WATCHDOG_TIMEOUT_MS=60000 scripts/simtest/cargo-simtest simtest --package haneul --package haneul-core --profile simtestnightly
...
for SUB_SEED in `seq 1 $NUM_CPUS`; do
  ...
  # https://github.com/GeunhwaJeong/haneul/pull/8244.
  #
  ...
  MSIM_TEST_SEED="$SEED" MSIM_TEST_NUM=1 MSIM_WATCHDOG_TIMEOUT_MS=60000 scripts/simtest/cargo-simtest simtest --package haneul-benchmark --test-threads 1 --profile simtestnightly & # > "$LOG_FILE" 2>&1 &
done
