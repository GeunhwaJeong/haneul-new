...
LATEST_RS = "haneul-execution/src/latest.rs"
GAS_RS = "crates/haneul-types/src/gas.rs"
GAS_V2_RS = "crates/haneul-types/src/gas_model/gas_v2.rs"
GAS_TABLES_RS = "crates/haneul-types/src/gas_model/tables.rs"
...
impl Executor {
    pub(crate) fn new(protocol_config: &ProtocolConfig, silent: bool) -> Result<Self, HaneulError> {
        ...
        let base_runtime = Arc::new(haneul_adapter_${cut_pkg}::adapter::new_move_runtime(
            haneul_move_natives_${cut_pkg}::all_natives(silent, protocol_config),
            protocol_config,
        )?);
...
TPL_FN_NORMAL_BODY = """\
        ...
        let base = {
            use haneul_adapter_${cut_pkg} as base_adapter;
...
mod latest_dual_replay {
    ...
    type View<'a> = (&'a InnerTemporaryStore, &'a HaneulGasStatus, &'a TransactionEffects);
...
def do_inject(repo: Path, state: State) -> None:
    ...
    _ensure_haneul_execution_serde_json_dep(repo)
...
# The dual-exec body clones `HaneulGasStatus` (gas.rs enum). That cascades through
...
GAS_CLONE_TARGETS = [
    (GAS_RS, "HaneulGasStatus", "enum"),
    (GAS_V2_RS, "HaneulGasStatus", "struct"),
    (GAS_V2_RS, "HaneulCostTable", "struct"),
...
# on `TransactionEffects`. `haneul-execution` doesn't depend on `serde_json` by default
# at HEAD, so the cargo check fails until we add it. Idempotent.
RE_HANEUL_EXECUTION_DEPS_HEADER = re.compile(r"^\[dependencies\]\s*$", re.MULTILINE)
HANEUL_EXECUTION_CARGO_TOML = "haneul-execution/Cargo.toml"


def _ensure_haneul_execution_serde_json_dep(repo: Path) -> None:
    path = repo / HANEUL_EXECUTION_CARGO_TOML
    src = path.read_text()
    if re.search(r"^serde_json\b", src, re.MULTILINE):
        return
    m = RE_HANEUL_EXECUTION_DEPS_HEADER.search(src)
    if not m:
        raise InjectError(
            f"anchor `[dependencies]` not found in {HANEUL_EXECUTION_CARGO_TOML}"
        )
    insert_at = m.end()
...
def do_build(repo: Path) -> None:
    cmd = ["cargo", "check", "-p", "haneul-execution", "-p", "haneul-types"]
    r = subprocess.run(cmd, cwd=repo, capture_output=True, text=True)
    if r.returncode != 0:
...
def do_commit(repo: Path, state: State) -> str:
    ...
    #   - haneul-execution/<cut>/             generated crate tree
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("build", help="cargo check -p haneul-execution -p haneul-types")
