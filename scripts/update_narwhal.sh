...
function latest_nw_revision() {
	...
	cd "$NW_CHECKOUT"
	git clone --depth 1 https://github.com/GeunhwaJeong/narwhal
	cd narwhal
	git rev-parse HEAD
}
...
function current_nw_revision() {
	cd "$TOPLEVEL"
	readarray -t <<< "$(find ./ -iname '*.toml' -exec $GREP -oPe 'git = "https://github.com/GeunhwaJeong/narwhal", *rev *= *\"\K[0-9a-fA-F]+' '{}' \;)"
	watermark=${MAPFILE[0]}
	for i in "${MAPFILE[@]}"; do
	    if [[ "$watermark" != "$i" ]]; then
        	not_equal=true
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current NW revision in Haneul, aborting" && exit 1
	echo "$watermark"
}
...
if [[ "$CURRENT_NW" != "$LATEST_NW" ]]; then
	echo "About to replace $CURRENT_NW with $LATEST_NW as the Narwhal pointer in Haneul"
else
	exit 0
fi
