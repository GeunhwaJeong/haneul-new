...
type UseSignTransactionBlockArgs = PartialBy<HaneulSignTransactionBlockInput, 'account' | 'chain'>;
...
export function useSignTransactionBlock({
	...
	return useMutation({
		...
		mutationFn: async (signTransactionBlockArgs) => {
			...
			const walletFeature = currentWallet.features['haneul:signTransactionBlock'];
			if (!walletFeature) {
