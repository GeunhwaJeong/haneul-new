...
type UseSignAndExecuteTransactionBlockResult = HaneulSignAndExecuteTransactionBlockOutput;
...
export function useSignAndExecuteTransactionBlock({
	...
	return useMutation({
		...
		mutationFn: async (signAndExecuteTransactionBlockArgs) => {
			...
			const walletFeature = currentWallet.features['haneul:signAndExecuteTransactionBlock'];
			if (!walletFeature) {
