...
Haneul Wallet Adapter is a set of libraries that makes it easy to connect your dApp to Haneul wallets.
...
```bash
npm install @haneullabs/wallet-adapter-all-wallets @haneullabs/wallet-adapter-react @haneullabs/wallet-adapter-react-ui
```
...
To add a **Connect Wallet** button to your page, use the `@haneullabs/wallet-adapter-react-ui` package:
...
To get access to the currently connected wallet, use the `useWallet()` hook from `@haneullabs/wallet-adapter-react` to interact with the wallet, such as proposing transactions:
...
All available wallet adapters are currently exported via the `@haneullabs/wallet-adapter-all-wallets` package.
You can also install individual wallet adapters that you plan on using in your project.

- **[Haneul Wallet](https://docs.haneul.io/devnet/explore/wallet-browser)** - `@haneullabs/wallet-adapter-haneul-wallet`
