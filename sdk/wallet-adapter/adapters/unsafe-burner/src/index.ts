...
export class UnsafeBurnerWalletAdapter implements WalletAdapter {
	...
	constructor(client?: HaneulClient, faucetUrl?: string) {
	...
	async connect() {
		...
		try {
			requestHaneulFromFaucetV0({
				...
				recipient: this.#keypair.getPublicKey().toHaneulAddress(),
			});
		} catch (e) {
			console.warn(
				'Failed to request Haneul from the faucet. This may prevent transactions from being submitted.',
			);
			console.warn(e);
		}
		this.connecting = false;
