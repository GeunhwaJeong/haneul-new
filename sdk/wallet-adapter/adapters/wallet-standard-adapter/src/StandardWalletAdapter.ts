...
import {
	...
	HaneulSignAndExecuteTransactionBlockVersion,
	HaneulSignTransactionBlockVersion,
...
const haneulSignTransactionBlockLatestVersion: HaneulSignTransactionBlockVersion = '1.0.0';
const haneulSignAndExecuteTransactionBlockLatestVersion: HaneulSignAndExecuteTransactionBlockVersion =
	'1.0.0';
...
export class StandardWalletAdapter implements WalletAdapter {
	...
	signMessage: WalletAdapter['signMessage'] = (messageInput) => {
		if (!this.#wallet.features['haneul:signMessage']) {
			throw new Error('Wallet does not support the deprecated `signMessage` method.');
		}

		return this.#wallet.features['haneul:signMessage'].signMessage(messageInput);
	};

	signPersonalMessage: WalletAdapter['signPersonalMessage'] = (messageInput) => {
		if (!this.#wallet.features['haneul:signPersonalMessage']) {
			throw new Error('Wallet does not support the new `signPersonalMessage` method.');
		}
		return this.#wallet.features['haneul:signPersonalMessage'].signPersonalMessage(messageInput);
	};

	signTransactionBlock: WalletAdapter['signTransactionBlock'] = (transactionInput) => {
		const version = this.#wallet.features['haneul:signTransactionBlock'].version;
		if (!isFeatureCompatible(version, haneulSignTransactionBlockLatestVersion)) {
			throw new Error(
				`Version mismatch, signTransaction feature version ${version} is not compatible with version ${haneulSignTransactionBlockLatestVersion}`,
			);
		}
		return this.#wallet.features['haneul:signTransactionBlock'].signTransactionBlock(transactionInput);
	};

	signAndExecuteTransactionBlock: WalletAdapter['signAndExecuteTransactionBlock'] = (
		transactionInput,
	) => {
		const version = this.#wallet.features['haneul:signAndExecuteTransactionBlock'].version;
		if (!isFeatureCompatible(version, haneulSignAndExecuteTransactionBlockLatestVersion)) {
			throw new Error(
				`Version mismatch, signAndExecuteTransactionBlock feature version ${version} is not compatible with version ${haneulSignAndExecuteTransactionBlockLatestVersion}`,
			);
		}
