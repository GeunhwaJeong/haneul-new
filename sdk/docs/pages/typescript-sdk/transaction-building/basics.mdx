...
## Haneul Programmable Transaction Blocks with the TS SDK
...
txb.transferObjects([coin], txb.pure('0xSomeHaneulAddress'));
```

Note that you can attach multiple transactions of the same type to a transaction block as well. For
example, to get a list of transfers, and iterate over them to transfer coins to each of them:
...
Haneul to transfer, or which object to pass into a Move call, or a shared object. There are currently
...
Haneul supports following transactions:
