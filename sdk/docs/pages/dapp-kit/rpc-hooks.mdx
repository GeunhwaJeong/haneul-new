...
[JSON RPC specification](https://docs.haneul.io/haneul-api-ref).

## useHaneulClientQuery

Load data from the Haneul RPC using the `useHaneulClientQuery` hook. This hook is a wrapper around the
...
You can fetch a variable number of Haneul RPC queries using the `useHaneulClientQueries` hook. This hook
is a wrapper around the
[useQueries](https://tanstack.com/query/latest/docs/react/reference/useQueries) hook from
@tanstack/react-query.

The `queries` value is an array of query option objects identical to the `useHaneulClientQuery` hook.
...
For RPC methods that support pagination, dApp Kit also implements a `useHaneulClientInfiniteQuery`
...
For RPC methods that mutate state, dApp Kit implements a `useHaneulClientMutation` hook. Use this hook
