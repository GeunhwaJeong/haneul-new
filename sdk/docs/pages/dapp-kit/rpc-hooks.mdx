...
Haneul dApp Kit ships with hooks for each of the rpc methods defined in the
[JSON RPC specification](https://docs.haneul.io/haneul-jsonrpc)

## `useHaneulClientQuery`

You can load data from the Haneul RPC using the `useHaneulClientQuery` hook. This hook is a wrapper around
...
function MyComponent() {
	const { data, isPending, isError, error, refetch } = useHaneulClientQuery(
		'getOwnedObjects',
		{ owner: '0x123' },
		{
			gcTime: 10000,
...
You can fetch a variable numer of Haneul RPC queries using the `useHaneulClientQueries` hook. This hook is
...
`queries` value is an array with query option objects identical to the `useHaneulClientQuery` hook.
...
```tsx
import { useHaneulClientQueries } from '@haneullabs/dapp-kit';

function MyComponent() {
	const { data, isPending, isError } = useHaneulClientQueries({
		queries: [
			{
...
For RPC methods that support pagination dApp Kit also implements a `useHaneulClientInfiniteQuery` hook.
...
```tsx
import { useHaneulClientInfiniteQuery } from '@haneullabs/dapp-kit';

function MyComponent() {
	const { data, isPending, isError, error, isFetching, fetchNextPage, hasNextPage } =
		useHaneulClientInfiniteQuery('getOwnedObjects', {
			owner: '0x123',
		});
...
For RPC methods that mutate state dApp Kit implements a `useHaneulClientMutation` hook. This hook can
...
```tsx
import { useHaneulClientMutation } from '@haneullabs/dapp-kit';

function MyComponent() {
	const { mutate } = useHaneulClientMutation('dryRunTransactionBlock');
...
To get the HaneulNS name for a given address, use the `useResolveHaneulNSName` hook.

```tsx
import { useResolveHaneulNSName } from '@haneullabs/dapp-kit';

function MyComponent() {
	const { data, isPending } = useResolveHaneulNSName('0x123');
