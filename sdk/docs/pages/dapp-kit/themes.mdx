...
The components in dApp kit can be customized by using by providing a theme to the `WalletProvider`
component.

```tsx
import { lightTheme, WalletProvider } from '@haneullabs/dapp-kit';
...
To define a custom theme, you will need to implement the `ThemeVars` interface with CSS variable
values for your theme:

```tsx
import { ThemeVars } from '@haneullabs/dapp-kit';
