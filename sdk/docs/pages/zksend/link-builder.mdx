...
<Callout type="warning">
	...
	necessary licenses. The information provided by Haneul Labs is not legal advice, and developers
...
- **`client`** (optional) - The `@haneullabs/haneul.js` client used to fetch data to construct the link. If
...
// Adds the link creation transactions to the transaction block
...
`zksend_address` query parameter to the URL, containing the Haneul address of the user that claimed the
...
`getSentTransactionBlocksWithLinks` will return a list of transaction blocks sent by the provided
address. Each result will include the transaction that was sent, along with an array containing any
links that were created or regenerated by that transaction block.

```ts
import { getSentTransactionBlocksWithLinks } from '@haneullabs/zksend';

const { data, hasNextPage, nextCursor } = await getSentTransactionBlocksWithLinks({
	address: addressOfCreator,
});

for (const { transactionBlock, links } of data) {
	// get the claimable assets for this link (will be empty if the link has been claimed)
...
By default `getSentTransactionBlocksWithLinks` will not load the assets for claimed links. This can
be changed by passing `loadClaimedAssets: true` to the function.
