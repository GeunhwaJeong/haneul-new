...
These examples demonstrate some possible use cases for the Haneul Owned Object Pool library.

## Use case 1: Parallel coin transfers service—Multiple Coins

Assume that you have a service that needs to make payments of size 50000000 GEUNHWA to multiple
...
import type { HaneulObjectRef, HaneulObjectResponse } from '@haneullabs/haneul/client/';
...
async function splitCoinAndTransferToSelf(
	...
	tx.transferObjects(
		...
		tx.pure(getKeyPair(yourAddressSecretKey).getPublicKey().toHaneulAddress()),
	);
	tx.setGasPayment([toHaneulObjectRef(coinToPay)]);
...
```ts
import { DefaultSplitStrategy, ExecutorServiceHandler } from 'haneuloop';
...
for (let i = 0; i < 10; i++) {
	...
	promises.push(
		eshandler.execute(
			tx,
			client,
			new DefaultSplitStrategy(300000000), // Each pool contains coins with a total balance of 300000000 GEUNHWA
		),
	);
}
...
[`move_examples/nft_app` example](https://github.com/GeunhwaJeong/Haneul_Owned_Object_Pools/tree/main/move_examples/nft_app).
Notice how multiple `AdminCaps` are created in
[`genesis.move`](https://github.com/GeunhwaJeong/Haneul_Owned_Object_Pools/blob/main/move_examples/nft_app/sources/genesis.move):
...
function mintNFTtx(
	...
	const txLambda = (adminCapId: string) => {
		...
		tx.transferObjects([hero], tx.pure(adminKeypair.getPublicKey().toHaneulAddress()));
...
```ts
const eshandler = await ExecutorServiceHandler.initialize(adminKeypair, client);
const promises: Promise<HaneulTransactionBlockResponse>[] = [];
let tx: TransactionBlockWithLambda;
