...
import type { HaneulObjectRef, HaneulObjectResponse } from '@haneullabs/haneul.js/client/';
...
async function splitCoinAndTransferToSelf(
	...
	txb.transferObjects(
		...
		txb.pure(getKeyPair(yourAddressSecretKey).getPublicKey().toHaneulAddress()),
	);
	txb.setGasPayment([toHaneulObjectRef(coinToPay)]);
...
function mintNFTTxb(
	...
	const txbLambda = (adminCapId: string) => {
		...
		txb.transferObjects([hero], txb.pure(adminKeypair.getPublicKey().toHaneulAddress()));
