...
```sh
git clone https://github.com/GeunhwaJeong/Haneul_Owned_Object_Pools.git
```
...
```sh
haneul client switch --env testnet && haneul client envs
```

Also, make sure that your admin account (the account `haneul client active-address` identifies) owns
enough HANEUL. You can get HANEUL for a test network from the
[Haneul faucet](https://docs.haneul.io/guides/developer/getting-started/get-coins).
...
[`.test.env.example.`](https://github.com/GeunhwaJeong/coin_management_system/blob/main/test/.env.example):
...
# Example: "https://fullnode.testnet.haneul.io:443"
HANEUL_NODE= ...
...
<Callout type="default">
	You can run `cat ~/.haneul/haneul_config/haneul.keystore` to see your addresses' secret keys. Each secret's
	corresponding address is in the same row line that appears in `haneul client addresses`.
</Callout>
