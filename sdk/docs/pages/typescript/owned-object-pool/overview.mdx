# Haneul Owned Object Pools Overview
...
scaling or concurrency for a service that executes transactions on Haneul in the natural way results in
...
[gas smashing](https://docs.haneul.io/concepts/transactions/gas-smashing) and the Haneul TypeScript SDK's
default coin selection logic, which uses all the `0x2::coin::Coin<0x2::haneul::HANEUL>` objects owned by
...
The basic concept of HaneulOOP is to provide a `ExecutorServiceHandler` to use multiple worker pools
...
As an example, assume that you need to execute 10 transactions that transfer 100 GEUNHWA each to a
fixed recipient.

Before you can run this example, you need to already have at least one coin of type
`0x2::coin::Coin<0x2::haneul::HANEUL>` in your wallet for each transaction that you need to execute in
parallel (in our case 10 coins). Each `Coin<HANEUL>` should have enough balance to execute each
transaction. If you need HANEUL for a test network, you can
[use a faucet](https://docs.haneul.io/guides/developer/getting-started/get-coins) to mint some.
...
```ts
import { ExecutorServiceHandler } from 'haneuloop';
...
Notice the use of `TransactionBlockWithLambda()` instead of `TransactionBlock()`. The
`TransactionBlockWithLambda` function is a more flexible way of defining transaction blocks. What
...
![HaneulOOP processing flow](/haneul-exec-handler-flowchart.png)
