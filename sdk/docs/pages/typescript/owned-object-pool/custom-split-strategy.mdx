...
`0x2::coin::Coin<0x2::haneul::HANEUL>`) from the `mainPool` that their sum of balances surpasses a minimum
threshold, and creates a new worker pool only containing these gas coins.

It fulfils the minimum requirement needed for a transaction block to be executed, as the client
should always be able to pay for the gas of the transaction.
...
[`splitStrategies.ts` file](https://github.com/GeunhwaJeong/Haneul_Owned_Object_Pools/blob/main/src/splitStrategies.ts).
