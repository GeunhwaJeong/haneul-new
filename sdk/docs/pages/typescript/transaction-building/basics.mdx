# Haneul Programmable Transaction Blocks Basics

This example starts by constructing a transaction block to send HANEUL. To construct transactions
...
txb.transferObjects([coin], '0xSomeHaneulAddress');
```
...
transaction. Haneul supports following transactions:
...
[BCS](https://haneul-typescript-docs.vercel.app/bcs), which can be done using `txb.pure` eg,
...
txb.transferObjects([coin], txb.pure.address('0xSomeHaneulAddress'));
txb.transferObjects([coin], txb.pure(bcs.Address.serialize('0xSomeHaneulAddress')));
```
...
When building a transaction block, Haneul expects all objects to be fully resolved, including the
...
In most cases, building requires your HaneulClient to fully resolve input values.
