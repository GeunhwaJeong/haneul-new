...
To establish a connection to a network, import `HaneulClient` from `@haneullabs/haneul/client` and pass the
...
For local development, you can run `cargo run --bin haneul -- start --with-faucet --force-regenesis` to
spin up a local network with a local validator, a Full node, and a faucet server. Refer to
[the Local Network guide](https://docs.haneul.io/guides/developer/getting-started/local-network) for
more information.

## Manually calling unsupported RPC methods

You can use `HaneulClient` to call any RPC method the node you're connecting to exposes. Most RPC
...
[RPC documentation](https://docs.haneul.io/references/haneul-api).
...
If you are using `HaneulClient` in an environment that does not support the `WebSocket` API, including
Node.js, you may need to provide your own implementation of the `WebSocket` API when creating your
HaneulClient. You can either use a global polyfill for the WebSocket class, or pass a compatible
WebSocket implementation into HaneulHTTPTransport (eg, using the `ws` package)
...
In addition to the RPC methods mentioned above, `HaneulClient` also exposes some methods for working
with Transactions.
