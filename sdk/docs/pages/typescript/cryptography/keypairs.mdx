...
The Haneul TypeScript SDK provides `Keypair` classes that handle logic for signing and verification
using the cryptographic key pairs associated with a Haneul address.

The Haneul TypeScript SDK supports three signing schemes:
...
[Signatures](https://docs.haneul.io/concepts/cryptography/transaction-auth/signatures) topic.

To use, import the key pair class your project uses from the `@haneullabs/haneul/keypairs` folder. For
example, to use the Ed25519 scheme, import the `Ed25519Keypair` class from
`@haneullabs/haneul/keypairs/ed25519`.

```typescript
import { Ed25519Keypair } from '@haneullabs/haneul/keypairs/ed25519';
```

To create a random key pair (which identifies a Haneul address), instantiate a new `Keypair` class. To
...
to retrieve its associated Haneul address. You can access a `Keypair` from its `PublicKey` or construct
it from the bytes (as a `Uint8Array`) of the `PublicKey`, as in the following code:

```typescript
import { Ed25519Keypair, Ed25519PublicKey } from '@haneullabs/haneul/keypairs/ed25519';
...
const address = publicKey.toHaneulAddress();
...
then verifies the message to retrieve the public key. The code then uses `toHaneulAddress()` to check
...
if (publicKey.verifyAddress(keypair.getPublicKey().toHaneulAddress())) {
...
```typescript
// import HaneulClient to create a network client and the getFullnodeUrl helper function
...
// see Network Interactions with HaneulClient for more info on creating clients
...
run into issues if you try to compare the address returned by `publicKey.toHaneulAddress()` directly
...
[legacy versions of the zklogin address](https://sdk.haneul-labs.com/typescript/zklogin#legacy-addresses).

## Deriving a key pair from a mnemonic

The Haneul TypeScript SDK supports deriving a key pair from a mnemonic phrase. This can be useful when
...
```typescript
import { decodeHaneulPrivateKey, encodeHaneulPrivateKey } from '@haneullabs/haneul/cryptography';
import { Ed25519Keypair } from '@haneullabs/haneul/keypairs/ed25519';

const encoded = encodeHaneulPrivateKey(
	[
...
const { schema, secretKey } = decodeHaneulPrivateKey(encoded);
// use schema to choose the correct key pair
const keypair = Ed25519Keypair.fromSecretKey(secretKey);
```

See [SIP-15](https://github.com/haneul-foundation/sips/blob/main/sips/sip-15.md) for additional context
and motivation.
...
```typescript
const secretKey = 'haneulprivkey1qzse89atw7d3zum8ujep76d2cxmgduyuast0y9fu23xcl0mpafgkk8g4368';
...
```typescript
import { Ed25519Keypair } from '@haneullabs/haneul/keypairs/ed25519';
import { fromHex } from '@haneullabs/haneul/utils';
