...
The Haneul TypeScript SDK provides a `MultiSigPublicKey` class to support
[Multi-Signature](https://docs.haneul.io/concepts/cryptography/transaction-auth/multisig) (MultiSig)
...
transaction is equal to or greater than the threshold value, then the Haneul network considers the
transdaction valid.

```typescript
import { Ed25519Keypair } from '@haneullabs/haneul.js/keypairs/ed25519';
import { MultiSigPublicKey } from '@haneullabs/haneul.js/multisig';
...
and TransactionBlocks like any other keypair or signer class. This is often easier than manually
...
const signer1 = multiSigPublicKey.getSigner([kp1, kp2]);
const signer2 = multiSigPublicKey.getSigner([kp3]);

const message = new TextEncoder().encode('hello world');
const { signature } = await signer1.signPersonalMessage(message);
const isValid = await multiSigPublicKey.verifyPersonalMessage(message, combinedSignature);
```
