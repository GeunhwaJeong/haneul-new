...
To fund transactions, the `SerialTransactionExecutor` will select all of the senders HANEUL coins for
...
- `client`: An instance of `HaneulClient` used to execute transactions.
...
import { SerialTransactionExecutor } from '@haneullabs/haneul/transactions';
...
<Callout type="warning">
	Using HaneulClient or wallets to execute additional transactions while `ParallelTransactionExecutor`
...
```ts
import { getFullnodeUrl, HaneulClient } from '@haneullabs/haneul/client';
import { ParallelTransactionExecutor } from '@haneullabs/haneul/transactions';
