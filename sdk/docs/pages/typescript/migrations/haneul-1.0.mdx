...
- [A new GraphQL Client for the newly released Haneul GraphQL API](../graphql)
...
The majority of changes in this release are in the `@haneullabs/haneul` package, but the changes to API
...
## Changes to `@haneullabs/haneul`

As part of the 1.0 release the `@haneullabs/haneul.js` package has been renamed to `@haneullabs/haneul`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @haneullabs/haneul.js
...
```diff
- import { HaneulClient } from '@haneullabs/haneul.js'
+ import { HaneulClient } from '@haneullabs/haneul'
```
...
coming to other parts of the HANEUL ecosystem in the future.
...
- `haneulClient.waitForTransactionBlock({ transactionBlock })` ->
  `haneulClient.waitForTransaction({ transaction })`
- `haneulClient.signAndExecuteTransactionBlock({ transactionBlock })` ->
  `haneulClient.signAndExecuteTransaction({ transaction })`

There are a few methods on HaneulClient that retain the `TransactionBlock` name because they correspond
to specific JSON RPC calls. The following methods are unchanged:

- `haneulClient.getTransactionBlock`
- `haneulClient.queryTransactionBlocks`
- `haneulClient.executeTransactionBlock`
- `haneulClient.dryRunTransactionBlock`
- `haneulClient.devInspectTransactionBlock`
...
```diff
- import { Transactions } from '@haneullabs/haneul/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_haneullabs_haneul.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_haneullabs_haneul.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_haneullabs_haneul.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_haneullabs_haneul.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_haneullabs_haneul.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromHaneulBytes` has been moved to `@haneullabs/haneul/verify`

```diff
- import { publicKeyFromHaneulBytes } from '@haneullabs/haneul/multisig'
...
We previously deprecated the `generic` helper exported from `@haneullabs/bcs`. This helper has now been
entirely removed.
...
`haneulClient.executeTransactionBlock` for returning additional data.
...
`showRawEffects` to true if using `haneulClient.executeTransactionBlock`, or by returning an object
...
```diff
- import { getSentTransactionBlocksWithLinks, ZkSendLinkBuilder } from '@haneullabs/zksend'
+ import { getSentTransactionsWithLinks, ZkSendLinkBuilder } from '@haneullabs/zksend'
...
[wallet-standard documentation](https://docs.haneul.io/standards/wallet-standard).

The old `haneul:signTransactionBlock` and `haneul:signAndExecuteTransactionBlock` methods have been
deprecated in favor of the new `haneul:signTransaction` and `haneul:signAndExecuteTransaction` methods,
...
```ts
import { signAndExecuteTransaction, signTransaction } from '@haneullabs/wallet-standard';
