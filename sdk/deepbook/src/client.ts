...
import type { OrderArguments, PaginatedEvents, PaginationArguments } from '@haneullabs/haneul.js/client';
...
export class DeepBookClient {
	 ...
	 * @param coinId the coin used for the deposit. You can omit this argument if you are depositing HANEUL, in which case
	...
	async deposit(
		...
		if (coinId === undefined && !hasHaneul) {
			throw new Error('coinId must be specified if neither baseAsset nor quoteAsset is HANEUL');
		}
	...
	async placeLimitOrder(
		...
		const args = [
			...
			txb.object(HANEUL_CLOCK_OBJECT_ID),
	...
	async getUserPosition(
		...
		txb.moveCall({
			...
			arguments: [txb.object(normalizeHaneulObjectId(poolId)), txb.object(cap)],
		});
