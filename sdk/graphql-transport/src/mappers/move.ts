...
export function mapNormalizedType(type: OpenMoveTypeSignatureBody): HaneulMoveNormalizedType {
	switch (type) {
		case 'address':
			return 'Address';
		case 'bool':
...
export function moveDataToRpcContent(data: MoveData, layout: MoveTypeLayout): MoveValue {
	...
	if ('UID' in data) {
		return {
			id: normalizeHaneulAddress(data.UID.map((byte) => byte.toString(16).padStart(2, '0')).join('')),
		};
	}

	if ('ID' in data) {
		return normalizeHaneulAddress(data.ID.map((byte) => byte.toString(16).padStart(2, '0')).join(''));
	}
	...
	if ('Struct' in data) {
		...
		// https://github.com/GeunhwaJeong/haneul/blob/5849f6845a3ab9fdb4c17523994adad461478a4c/crates/haneul-json-rpc-types/src/haneul_move.rs#L481
