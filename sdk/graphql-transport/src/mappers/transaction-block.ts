...
function mapObjectChanges(
	transactionBlock: Rpc_Transaction_FieldsFragment,
	effects: HaneulTransactionBlockResponse['effects'],
) {
	const changes: HaneulObjectChange[] = [];
...
function mapTransactionInput(input: typeof bcs.CallArg.$inferType): HaneulCallArg {
	if (input.Pure) {
		return {
			type: 'pure',
...
function mapTransaction(transaction: typeof bcs.Command.$inferType): HaneulTransaction {
...
function mapTransactionArgument(arg: typeof bcs.Argument.$inferType): HaneulArgument {
	switch (arg.$kind) {
		case 'GasCoin': {
			return 'GasCoin';
		}
