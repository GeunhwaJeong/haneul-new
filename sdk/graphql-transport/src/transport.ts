...
export interface HaneulClientGraphQLTransportOptions {
	url: string;
...
export class HaneulClientGraphQLTransport implements HaneulTransport {
	#options: HaneulClientGraphQLTransportOptions;
	#fallbackTransport?: HaneulTransport;
	#fallbackMethods: (keyof typeof RPC_METHODS)[];

	constructor(options: HaneulClientGraphQLTransportOptions) {
		...
		if (options.fallbackFullNodeUrl) {
			this.#fallbackTransport = new HaneulHTTPTransport({
	...
	async request<T = unknown>(input: HaneulTransportRequestOptions): Promise<T> {
		...
		switch (input.method) {
			case 'rpc.discover':
				...
				break;
			case 'haneulx_getLatestAddressMetrics':
	...
	async subscribe<T = unknown>(
		input: HaneulTransportSubscribeOptions<T>,
	...
	async #unsupportedMethod<T = unknown>(input: HaneulTransportRequestOptions): Promise<T> {
		if (!this.#fallbackTransport) {
			throw new UnsupportedMethodError(input.method);
		}
