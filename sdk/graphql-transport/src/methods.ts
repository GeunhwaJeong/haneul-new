...
import {
	...
	GetLatestHaneulSystemStateDocument,
...
import type { HaneulClientGraphQLTransport } from './transport.js';
...
export const RPC_METHODS: {
	[K in keyof HaneulClient as HaneulClient[K] extends (...args: any[]) => Promise<any>
		? K
		: never]?: HaneulClient[K] extends (...args: any[]) => infer R
		? (
				transport: HaneulClientGraphQLTransport,
				inputs: any[],
	...
	async getRpcApiVersion(transport) {
		...
		return {
			info: {
				version: res.headers.get('x-haneul-rpc-version') ?? undefined,
			},
		};
	},
	...
	async getNormalizedMoveModulesByPackage(transport, [pkg]) {
		...
		const modules: Record<string, HaneulMoveNormalizedModule> = {};
	...
	async getStakes(transport, [owner]) {
		const stakes = await transport.graphqlQuery(
			...
			(data) => data.address?.stakedHaneuls?.nodes,
		);

		return mapGraphQLStakeToRpcStake(stakes);
	},
	async getStakesByIds(transport, [stakedHaneulIds]) {
		const stakes = await transport.graphqlQuery(
			{
			...
			(data) => data.objects?.nodes.map((node) => node?.asMoveObject?.asStakedHaneul!).filter(Boolean),
		);

		return mapGraphQLStakeToRpcStake(stakes);
	},
	async getLatestHaneulSystemState(transport) {
		const systemState = await transport.graphqlQuery(
			{
				query: GetLatestHaneulSystemStateDocument,
			},
			(data) => data.epoch,
		);
		...
		return {
			...
			results: results?.map((result) => ({
				mutableReferenceOutputs: result.mutatedReferences?.map(
					(ref): [HaneulArgument, number[], string] => [
	...
	async resolveNameServiceAddress(transport, [name]): Promise<string | null> {
		...
		return data.resolveHaneulnsAddress?.address ?? null;
	},
	async resolveNameServiceNames(transport, [address, cursor, limit]) {
		const haneulnsRegistrations = await transport.graphqlQuery(
			{
			...
			(data) => data.address?.haneulnsRegistrations,
		);

		return {
			hasNextPage: haneulnsRegistrations.pageInfo.hasNextPage,
			nextCursor: haneulnsRegistrations.pageInfo.endCursor ?? null,
			data: haneulnsRegistrations?.nodes.map((node) => node.domain) ?? [],
		};
	},
};
