...
This package provides a `HaneulTransport` that enables `HaneulClient` to make requests using the RPC 2.0
(GraphQL) API instead of the JSON RPC API.

## Install

```bash
npm install --save @haneullabs/graphql-transport
```
...
The following methods are currently unsupported in HaneulClientGraphQLTransport, and will either error,
...
Some supported methods in `HaneulClientGraphQLTransport` do not support the full set of parameters
...
- `getLatestHaneulSystemState`, `getCurrentEpoch`, `getValidatorsApy` and `getCommitteeInfo`:
