...
describe('GraphQL HaneulClient compatibility', () => {
	...
	beforeAll(async () => {
		...
		await toolbox.client
			...
			.then(function (objects) {
				const data = objects.data[0].data as HaneulObjectData;
	...
	test('tryGetPastObject', async () => {
		...
		const fullNodeClient = new HaneulClient({
	...
	test.skip('getLatestHaneulSystemState', async () => {
		const rpc = await toolbox.client.getLatestHaneulSystemState();
		const graphql = await graphQLClient!.getLatestHaneulSystemState();
