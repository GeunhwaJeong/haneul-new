	...
	test(
		...
		async () => {
			...
			const claimLink = await ZkSendLink.fromUrl(linkUrl, {
				contract: ZK_BAG_CONFIG,
				network: 'testnet',
				claimApi: 'https://zksend-git-mh-contract-claims-haneullabs-labs.vercel.app/api',
			});
...
describe('Non contract links', () => {
	...
	test(
		...
		async () => {
			...
			txb.transferObjects([coin], linkKp.toHaneulAddress());
...
async function createBears(totalBears: number) {
	...
	txb.transferObjects(bears, txb.pure.address(keypair.toHaneulAddress()));
