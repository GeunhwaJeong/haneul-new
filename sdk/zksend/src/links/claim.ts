...
import type { PureArg } from '@haneullabs/haneul.js/bcs';
import { bcs } from '@haneullabs/haneul.js/bcs';
import { getFullnodeUrl, HaneulClient } from '@haneullabs/haneul.js/client';
import type { CoinStruct, HaneulTransaction } from '@haneullabs/haneul.js/client';
...
export class ZkSendLink {
	...
	async claimAssets(address: string) {
		...
		const { digest } = await this.#executeSponsoredTransactionBlock(
			await this.#createSponsoredTransactionBlock(txb, address, this.keypair.toHaneulAddress()),
			this.keypair,
		);
	...
	async createRegenerateTransaction(
		...
		const to = txb.pure.address(newLinkKp.toHaneulAddress());
	...
	async #listNonContractClaimableAssets() {
		...
		if (this.#ownedObjects.length === 0 && !this.#hasHaneul) {
			return {
				balances,
				nfts,
				coins,
	...
	async #loadOwnedObjects() {
		...
		if (this.#hasHaneul || this.#ownedObjects.length > 0) {
			this.claimed = false;
