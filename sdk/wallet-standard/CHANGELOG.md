...
- 805ff4d4c2: Fix bug where transaction was passed as both transaction and transactionBlock to haneul:signTransactionBlock
...
- 437f0ca2ef: Add isHaneulChain utility which is useful for type-safe dApp interfaces in wallets
...
- 956ec28eb: Change `signMessage` to return message bytes. Add support for haneul:signMessage in the wallet standard
...
- dbe73d5a4: Add an optional `contentOptions` field to `HaneulSignAndExecuteTransactionOptions` to specify which fields to include in `HaneulTransactionBlockResponse` (e.g., transaction, effects, events, etc). By default, only the transaction digest will be included.
