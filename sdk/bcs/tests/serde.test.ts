...
describe("BCS: Serde", () => {
  ...
  it("should serde structs", () => {
    ...
    bcs.registerAddressType("address", HANEUL_ADDRESS_LENGTH, "hex");
  ...
  it("should structs and nested enums", () => {
...

    {
      ...
      expect(result.item.balance.value.toString(10)).toEqual(
        value.item.balance.value
      );
    }
  });

  it("should serde HaneulObjectRef", () => {
    const bcs = new BCS(getHaneulMoveConfig());
    ...
    expect(serde(bcs, "HaneulObjectRef", value)).toEqual(value);
  });
});
