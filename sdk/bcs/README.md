...
To install, add the [`@haneullabs/bcs`](https://www.npmjs.com/package/@haneullabs/bcs) package to your project:
...
| ------------------- | -------- | ------------------------------------------------------------------------- |
| `vectorType`        | +        | Defines the type of the vector (`vector<T>` in HaneulMove, `Vec<T>` in Rust) |
| `addressLength`     | +        | Length of the built-in `address` type. 20 for HaneulMove, 32 for Core Move   |
...
const bcs = new BCS({
  vectorType: "vector<T>",
  addressLength: HANEUL_ADDRESS_LENGTH,
  addressEncoding: "hex",
  genericSeparators: ["<", ">"],
  types: {
...
For Haneul Move there's already a pre-built configuration which can be used through the `getHaneulMoveConfig()` call.

```ts
// Example: Haneul Move Config
import { BCS, getHaneulMoveConfig } from "@haneullabs/bcs";
...
```ts
// Example: Rust Config
import { BCS, getRustConfig } from "@haneullabs/bcs";
...
| 'address'                   | `BCS.ADDRESS`                 | Address type (also used for IDs in Haneul Move)           |
...
import { BCS, getHaneulMoveConfig, BcsWriter } from "@haneullabs/bcs";
const bcs = new BCS(getHaneulMoveConfig());
...
In this library, all of the primitive Move types are present as built-ins, however, there's a set of special types in Haneul which can be simplified to a primitive.
