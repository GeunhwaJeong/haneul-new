...
For Haneul Move there's already a pre-built configuration which can be used through the
`getHaneulMoveConfig()` call.
...
import { BCS, BcsWriter, getHaneulMoveConfig } from '@haneullabs/bcs';
...
special types in Haneul which can be simplified to a primitive.
