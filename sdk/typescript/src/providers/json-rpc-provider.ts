...
export class JsonRpcProvider {
  ...
  async getCoins(input: {
    ...
    try {
      if (
        !input.owner ||
        !isValidHaneulAddress(normalizeHaneulAddress(input.owner))
      ) {
    ...
    try {
      return await this.client.requestWithType(
        ...
        [
          ...
          } as HaneulTransactionResponseQuery,
          input.cursor,
          input.limit,
  ...
  async getTransactionDigestsInRangeDeprecated(
    ...
    try {
      return await this.client.requestWithType(
        'haneul_getTransactionsInRangeDeprecated',
        [start, end],
        GetTxnDigestsResponse,
        this.options.skipDataValidation,
      );
  ...
  async getDynamicFields(
    ...
    try {
      if (
        !input.parentId ||
        !isValidHaneulObjectId(normalizeHaneulObjectId(input.parentId))
      ) {
        throw new Error('Invalid Haneul Object id');
      }
      const resp = await this.client.requestWithType(
