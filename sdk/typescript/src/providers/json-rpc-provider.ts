...
export class JsonRpcProvider extends Provider {
   ...
   *
   * @param endpoint URL to the Haneul RPC endpoint, or a `Network` enum
  ...
  async getTransactionAuthSigners(
    digest: TransactionDigest
  ): Promise<HaneulTransactionAuthSignersResponse> {
    try {
      return await this.client.requestWithType(
  ...
  async getCheckpointSummaryByDigest(
    ...
    try {
      const resp = await this.client.requestWithType(
        'haneul_getCheckpointSummaryByDigest',
        [digest],
        CheckpointSummary,
        this.options.skipDataValidation
      );
  ...
  async getCheckpointContentsByDigest(
    ...
    try {
      const resp = await this.client.requestWithType(
        'haneul_getCheckpointContentsByDigest',
        [digest],
        CheckpointContents,
        this.options.skipDataValidation
      );
