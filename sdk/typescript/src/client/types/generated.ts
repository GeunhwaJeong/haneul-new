...
export interface DevInspectResults {
	...
	results?: HaneulExecutionResult[] | null;
}
...
export interface HaneulObjectData {
	 ...
	 * have Display defined See more details in <https://forums.haneul.io/t/nft-object-display-proposal/4872>
	 */
	...
	/**
	 * The amount of HANEUL we would rebate if this object gets deleted. This number is re-calculated each
...
export interface HaneulExecutionResult {
...
export interface HaneulMoveAbilitySet {
	abilities: HaneulMoveAbility[];
}
export interface HaneulMoveModuleId {
	address: string;
	name: string;
}
export interface HaneulMoveNormalizedField {
	name: string;
	type: HaneulMoveNormalizedType;
}
export interface HaneulMoveNormalizedFunction {
	isEntry: boolean;
...
export interface HaneulMoveNormalizedModule {
	address: string;
	exposedFunctions: {
		[key: string]: HaneulMoveNormalizedFunction;
	};
	fileFormatVersion: number;
	friends: HaneulMoveModuleId[];
	name: string;
	structs: {
		[key: string]: HaneulMoveNormalizedStruct;
	};
}
export interface HaneulMoveNormalizedStruct {
...
export interface HaneulMoveStructTypeParameter {
...
/**
 * This is the JSON-RPC type for the HANEUL system state object. It flattens all fields to make them
 * top-level fields such that it as minimum dependencies to the internal data structures of the HANEUL
 * system state type.
 */
...
/**
 * This is the JSON-RPC type for the HANEUL validator. It flattens all inner structures to top-level
 * fields so that they are decoupled from the internal definitions.
 */
export interface HaneulValidatorSummary {
