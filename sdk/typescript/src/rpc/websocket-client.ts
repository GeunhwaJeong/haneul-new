...
import { HaneulEventFilter, HaneulEventEnvelope, SubscriptionId } from '../types';
...
export class WebsocketClient {
  ...
  /**
   * @param endpoint Haneul node endpoint to connect to (accepts websocket & http)
      ...
      let newSubsArr: (FilterSubHandler | null)[] = await Promise.all(
        Array.from(this.eventSubscriptions.values()).map(async (sub) => {
          ...
          /**
            ...
            we skip calling haneul_unsubscribeEvent for the old sub id, because:
