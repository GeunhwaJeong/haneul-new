...
export function isTxContext(param: OpenMoveTypeSignature): boolean {
	...
	return (
		!!struct &&
		normalizeHaneulAddress(struct.package) === normalizeHaneulAddress('0x2') &&
...
export function getPureBcsSchema(typeSignature: OpenMoveTypeSignatureBody): BcsType<any> | null {
	...
	if ('datatype' in typeSignature) {
		const pkg = normalizeHaneulAddress(typeSignature.datatype.package);

		if (pkg === normalizeHaneulAddress(MOVE_STDLIB_ADDRESS)) {
			if (
