...
function expectType(typeName: string, argVal?: HaneulJsonValue) {
...
export function getPureSerializationType(
	...
	argVal: HaneulJsonValue | undefined,
): string | undefined {
	if (typeof normalizedType === 'string' && allowedTypes.includes(normalizedType)) {
		if (normalizedType in ['U8', 'U16', 'U32', 'U64', 'U128', 'U256']) {
			...
			if (argVal && !isValidHaneulAddress(argVal as string)) {
	...
	if ('Struct' in normalizedType) {
		if (isSameStruct(normalizedType.Struct, RESOLVED_ASCII_STR)) {
			...
			return 'utf8string';
		} else if (isSameStruct(normalizedType.Struct, RESOLVED_HANEUL_ID)) {
			return 'address';
		} else if (isSameStruct(normalizedType.Struct, RESOLVED_STD_OPTION)) {
			const optionToVec: HaneulMoveNormalizedType = {
