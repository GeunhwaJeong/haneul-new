...
import type { HaneulGraphQLClient } from '../../graphql/client.js';
...
export type NamedPackagesPluginOptions = {
	/**
	 * The HaneulGraphQLClient to use for resolving names.
	 ...
	 */
	haneulGraphQLClient: HaneulGraphQLClient;
	/**
 ...
 * ```
 * Transaction.registerGlobalSerializationPlugin("namedPackagesPlugin", namedPackagesPlugin({ haneulGraphQLClient }));
 * ```
 *
...
export const namedPackagesPlugin = ({
	...
	return async (
		...
		(await Promise.all(batches.map((batch) => query(haneulGraphQLClient, batch)))).forEach((res) => {
	...
	async function query(client: HaneulGraphQLClient, requests: NameResolutionRequest[]) {
