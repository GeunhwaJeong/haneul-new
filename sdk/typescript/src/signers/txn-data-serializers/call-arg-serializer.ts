...
import { bcs, CallArg, MoveCallTx, ObjectArg } from '../../types/haneul-bcs';
...
export class CallArgSerializer {
  ...
  /**
   * Deserialize Call Args used in `Transaction` into `HaneulJsonValue` arguments
   */
  async deserializeCallArgs(txn: MoveCallTx): Promise<HaneulJsonValue[]> {
    const userParams = await this.extractNormalizedFunctionParams(
  ...
  private getPureSerializationType(
    ...
    if (
      ...
      if (normalizedType in ['U8', 'U16', 'U32', 'U64', 'U128', 'U256']) {
        ...
        this.checkArgVal(
          (t: any) => typeof t === 'string' && isValidHaneulAddress(t),
          argVal,
          'valid HANEUL address'
        );
      }
  ...
  private isTxContext(param: HaneulMoveNormalizedType): boolean {
