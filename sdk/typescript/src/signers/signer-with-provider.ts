...
export abstract class SignerWithProvider implements Signer {
  ...
  abstract getAddress(): Promise<HaneulAddress>;
  ...
  async signAndExecuteTransaction(
    ...
    switch (transaction.kind) {
      ...
      case 'transferHaneul':
        return this.transferHaneul(transaction.data);
      case 'transferObject':
  ...
  async signAndExecuteTransactionWithRequestType(
    ...
    switch (transaction.kind) {
      ...
      case 'transferHaneul':
        return this.transferHaneulWithRequestType(transaction.data, requestType);
      case 'transferObject':
  ...
  /**
   * @deprecated This method will be removed soon after we deprecate gateway. Prefer to use
   * `transferHaneulWithRequestType`
   *
   * Serialize and Sign a `TransferHaneul` transaction and submit to the Gateway for execution
   */
  async transferHaneul(
    transaction: TransferHaneulTransaction
  ): Promise<HaneulTransactionResponse> {
    const signerAddress = await this.getAddress();
    const txBytes = await this.serializer.newTransferHaneul(
      signerAddress,
      transaction
    );
    return await this.signAndExecuteTransaction(txBytes);
   ...
   */
  async pay(transaction: PayTransaction): Promise<HaneulTransactionResponse> {
    const signerAddress = await this.getAddress();
  ...
  /**
   *
   * Serialize and sign a `TransferHaneul` transaction and submit to the Fullnode
   * for execution
   */
  async transferHaneulWithRequestType(
    transaction: TransferHaneulTransaction,
    requestType: ExecuteTransactionRequestType = 'WaitForLocalExecution'
  ): Promise<HaneulExecuteTransactionResponse> {
    const signerAddress = await this.getAddress();
