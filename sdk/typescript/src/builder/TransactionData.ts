...
import { normalizeHaneulAddress, HaneulObjectRef } from '../types';
...
// crates/haneul-protocol-config/src/lib.rs
...
export class TransactionDataBuilder {
  ...
  build({ onlyTransactionKind }: { onlyTransactionKind?: boolean } = {}) {
    ...
    const transactionData = {
      sender: prepareHaneulAddress(this.sender),
      expiration: this.expiration ? this.expiration : { None: true },
      gasData: {
        payment: this.gasConfig.payment,
        owner: prepareHaneulAddress(this.gasConfig.owner ?? this.sender),
