...
import { getObjectReference, getObjectType, HANEUL_TYPE_ARG } from '../types';
...
export async function convertToTransactionBuilder(
  ...
  switch (kind) {
    ...
    case 'splitCoin': {
      ...
      if (getObjectType(coinToSplit) === '0x2::coin::Coin<0x2::haneul::HANEUL>') {
        // For Haneul amounts, we'll just split off the gas, to avoid overlapping
        // gas and input
    ...
    case 'transferHaneul': {
      ...
      const object = await provider.getObject(data.haneulObjectId);
