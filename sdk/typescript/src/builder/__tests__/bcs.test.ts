...
it('can serialize transaction data with a programmable transaction', () => {
	let haneul = normalizeHaneulAddress('0x2').replace('0x', '');
	let txData = {
		V1: {
			sender: normalizeHaneulAddress('0xBAD').replace('0x', ''),
			...
			kind: {
				ProgrammableTransaction: {
					...
					transactions: [
						...
						{
							kind: 'MoveCall',
							target: `${haneul}::display::add_multiple`,
							typeArguments: [`${haneul}::capy::Capy`],
							arguments: [
						...
						{
							kind: 'MoveCall',
							target: `${haneul}::display::update_version`,
							typeArguments: [`${haneul}::capy::Capy`],
							arguments: [
