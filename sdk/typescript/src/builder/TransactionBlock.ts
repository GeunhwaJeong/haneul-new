...
import { fromB64, isSerializedBcs } from '@haneullabs/bcs';
...
export class TransactionBlock {
	 ...
	 * @deprecated Use `isTransactionBlock` from `@haneullabs/haneul.js/transactions` instead.
	 */
	 ...
	 * @deprecated Either use the helper methods on the `TransactionBlock` class, or import `Transactions` from `@haneullabs/haneul.js/transactions`.
	 */
	 ...
	 * * @deprecated Either use the helper methods on the `TransactionBlock` class, or import `Inputs` from `@haneullabs/haneul.js/transactions`.
	 */
	static get Inputs() {
		return Inputs;
	}
	...
	setGasPayment(payments: HaneulObjectRef[]) {
		this.#blockData.gasConfig.payment = payments.map((payment) => mask(payment, HaneulObjectRef));
	}
	...
	object(value: string | ObjectCallArg) {
		...
		return (
			inserted ??
			this.#input('object', typeof value === 'string' ? normalizeHaneulAddress(value) : value)
		);
	}
	...
	async #prepareTransactions(options: BuildOptions) {
		...
		inputs.forEach((input) => {
			if (input.type === 'object' && typeof input.value === 'string') {
				...
				objectsToResolve.push({ id: normalizeHaneulAddress(input.value), input });
				return;
			}
		});
