...
import { OpenRpcType, type OpenRpcSpec, OpenRpcTypeRef } from './open-rpc';
/** @ts-ignore */
...
const openRpcSpec: OpenRpcSpec = JSON.parse(
	await fs.readFile(
		path.resolve(packageRoot, '../../crates/haneul-open-rpc/spec/openrpc.json'),
		'utf-8',
	),
);
...
const options: {
	...
	types: {
		...
		MoveFunctionArgType: {
			alias: 'HaneulMoveFunctionArgType',
		},
		PaginatedHaneulObjectResponse: {
			alias: 'PaginatedObjectsResponse',
		},
		TransactionBlockResponseOptions: {
			alias: 'HaneulTransactionBlockResponseOptions',
		},
		TransactionBlockResponse: {
			alias: 'HaneulTransactionBlockResponse',
		},
		PaginatedTransactionBlockResponse: {
			alias: 'PaginatedTransactionResponse',
		},
		TransactionBlockEffects: {
			alias: 'TransactionEffects',
		},
		ObjectResponseQuery: {
			alias: 'HaneulObjectResponseQuery',
		},
		ObjectDataOptions: {
			alias: 'HaneulObjectDataOptions',
		},
		ObjectData: {
			alias: 'HaneulObjectData',
		},
		Owner: {
			alias: 'ObjectOwner',
		},
		TransactionBlockResponseQuery: {
			alias: 'HaneulTransactionBlockResponseQuery',
		},
		TransactionBlockKind: {
			alias: 'HaneulTransactionBlockKind',
		},
		Event: {
			alias: 'HaneulEvent',
		},
		HaneulProgrammableMoveCall: {
			alias: 'MoveCallHaneulTransaction',
		},
		EventFilter: {
			alias: 'HaneulEventFilter',
		},
		ValidatorApys: {
			alias: 'ValidatorsApy',
		},
		Data: {
			alias: 'HaneulParsedData',
		},
		Coin: {
			alias: 'CoinStruct',
		},
		ObjectChange: {
			alias: 'HaneulObjectChange',
		},
		ObjectRef: {
			alias: 'HaneulObjectRef',
		},
		EventID: {
			alias: 'EventId',
		},
...
async function createObjectMembers(type: Extract<OpenRpcTypeRef, { type: 'object' }>) {
	...
	if (type.additionalProperties === true) {
		...
		members.push(
			ts.factory.createIndexSignature(
				...
				await generateTypeReference(type.additionalProperties),
			),
		);
	}
...
async function generateTypeReference(type: OpenRpcTypeRef): Promise<ts.TypeNode> {
	if (type === true) {
	...
	if (Array.isArray(type)) {
		return ts.factory.createUnionTypeNode(
			await Promise.all(type.map((type) => generateTypeReference(type))),
		);
	}
	...
	if ('$ref' in type) {
		...
		if (isNamedType(ref)) {
			return ts.factory.createTypeReferenceNode(normalizeName(name));
		}

		return generateTypeReference(ref);
	}
	...
	if ('allOf' in type) {
		...
		return ts.factory.createIntersectionTypeNode(
			await Promise.all(types.map(generateTypeReference)),
		);
	}
		...
		case 'array':
			if (Array.isArray(type.items)) {
				return ts.factory.createTupleTypeNode(
					await Promise.all(type.items.map(generateTypeReference)),
				);
			}
			return ts.factory.createArrayTypeNode(await generateTypeReference(type.items));
		case 'object':
			return withDescription(
				type,
				ts.factory.createTypeLiteralNode(await createObjectMembers(type)),
			);
		case 'null':
