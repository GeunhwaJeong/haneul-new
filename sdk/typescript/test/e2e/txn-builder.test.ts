...
describe('Transaction Builders', () => {
	...
	it('SplitCoins + TransferObjects', async () => {
		...
		const coin_0 = coins[0].data as HaneulObjectData;
	...
	it('MergeCoins', async () => {
		const coins = await toolbox.getGasObjectsOwnedByAddress();
		const coin_0 = coins[0].data as HaneulObjectData;
		const coin_1 = coins[1].data as HaneulObjectData;
		const tx = new TransactionBlock();
	...
	it(
		...
		async () => {
			...
			const [{ haneulAddress: validatorAddress }] = await toolbox.getActiveValidators();
	...
	it('TransferObject', async () => {
		const coins = await toolbox.getGasObjectsOwnedByAddress();
		const tx = new TransactionBlock();
		const coin_0 = coins[2].data as HaneulObjectData;
		...
		async () => {
			...
			const capId = (
				publishTxn.objectChanges?.find(
					(a) =>
						is(a, HaneulObjectChangeCreated) &&
...
async function validateTransaction(client: HaneulClient, signer: Keypair, tx: TransactionBlock) {
	tx.setSenderIfNotSet(signer.getPublicKey().toHaneulAddress());
