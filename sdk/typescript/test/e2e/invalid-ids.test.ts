...
describe('Object id/Address/Transaction digest validation', () => {
	...
	it('Test all functions with invalid Haneul Address', async () => {
		...
		expect(toolbox.client.getOwnedObjects({ owner: '' })).rejects.toThrowError(
			/Invalid Haneul address/,
		);
	});

	it('Test all functions with invalid Object Id', async () => {
		//empty id
		expect(toolbox.client.getObject({ id: '' })).rejects.toThrowError(/Invalid Haneul Object id/);
		...
		expect(toolbox.client.multiGetObjects({ ids: objectIds })).rejects.toThrowError(
			/Invalid Haneul Object id 0xWRONG/,
		);
	});
	...
	it('Validates tx.pure.address and tx.pure.id', async () => {
		...
		expect(() => tx.pure.address('')).toThrowError(/Invalid Haneul address/);
		expect(() => tx.pure.id('')).toThrowError(/Invalid Haneul address/);
	});
});
