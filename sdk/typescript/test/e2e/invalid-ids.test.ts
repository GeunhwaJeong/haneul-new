...
//Test that with invalid object id/address/digest, functions will throw an error before making a request to the rpc server
  it('Test all functions with invalid Haneul Address', async () => {
    //empty id
    expect(toolbox.provider.getObjectsOwnedByAddress('')).rejects.toThrowError(/Invalid Haneul address/);
    
    //wrong id
    expect(toolbox.provider.getTransactionsForAddress('Wrong')).rejects.toThrowError(/Invalid Haneul address/);
  })

  it('Test all functions with invalid Object Id', async () => {
    //empty id
    expect(toolbox.provider.getObject('')).rejects.toThrowError(/Invalid Haneul Object id/);
    
    //more than 20bytes
    expect(toolbox.provider.getObjectsOwnedByObject('0x4ce52ee7b659b610d59a1ced129291b3d0d4216322')).rejects.toThrowError(/Invalid Haneul Object id/);
    expect(toolbox.provider.getTransactionsForObject('4ce52ee7b659b610d59a1ced129291b3d0d421632')).rejects.toThrowError(/Invalid Haneul Object id/);
    
    //wrong batch request
    let objectIds = ['0xBABE', '0xCAFE', '0xWRONG', '0xFACE'];
    expect(toolbox.provider.getObjectBatch(objectIds)).rejects.toThrowError(/Invalid Haneul Object id 0xWRONG/);;
  })
