...
const HANEUL_BIN =
	import.meta.env.VITE_HANEUL_BIN ?? path.resolve(__dirname, '../../../../../target/debug/haneul');
...
export async function setupWithFundedAddress(
	...
	await retry(() => requestHaneulFromFaucetV1({ host: DEFAULT_FAUCET_URL, recipient: address }), {
		backoff: 'EXPONENTIAL',
	...
	execSync(`${HANEUL_BIN} client --yes --client.config ${configPath}`, { encoding: 'utf-8' });
...
export async function publishPackage(packagePath: string, toolbox?: TestToolbox) {
	...
	const { modules, dependencies } = JSON.parse(
		execSync(
			`${HANEUL_BIN} move --client.config ${toolbox.configPath} build --dump-bytecode-as-base64 --path ${packagePath} --install-dir ${tmpobj.name}`,
			{ encoding: 'utf-8' },
		),
	);
...
export async function payHaneul(
	client: HaneulClient,
	signer: Keypair,
...
export async function executePayHaneulNTimes(
	client: HaneulClient,
	signer: Keypair,
	nTimes: number,
	...
	for (let i = 0; i < nTimes; i++) {
		...
		txns.push(await payHaneul(client, signer, numRecipientsPerTxn, recipients, amounts));
	}
	return txns;
}
