...
describe('Invoke any RPC endpoint', () => {
	...
	it('haneulx_getOwnedObjects', async () => {
		...
		const gasObjects = await toolbox.provider.call('haneulx_getOwnedObjects', [toolbox.address()]);
		expect(gasObjects.data).toStrictEqual(gasObjectsExpected.data);
	});

	it('haneul_getObjectOwnedByAddress Error', async () => {
		expect(toolbox.provider.call('haneulx_getOwnedObjects', [])).rejects.toThrowError();
	});

	it('haneulx_getCommitteeInfo', async () => {
		const committeeInfoExpected = await toolbox.provider.getCommitteeInfo();

		const committeeInfo = await toolbox.provider.call('haneulx_getCommitteeInfo', []);
