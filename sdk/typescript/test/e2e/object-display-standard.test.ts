...
import { ObjectId, getObjectDisplay, HaneulObjectData } from '../../src';
...
describe('Test Object Display Standard', () => {
	...
	it('Test getting Display fields with error object', async () => {
		...
		const data = resp[0].data as HaneulObjectData;
	...
	it('Test getting Display fields for object that has no display object', async () => {
		const coin = (await toolbox.getGasObjectsOwnedByAddress())[0].data as HaneulObjectData;
