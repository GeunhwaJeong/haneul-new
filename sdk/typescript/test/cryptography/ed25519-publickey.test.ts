...
// ../haneul/target/debug/haneul client new-address ed25519
// ../haneul/target/debug/haneul keytool list
...
describe('Ed25519PublicKey', () => {
  ...
  it('toBuffer', () => {
    ...
    expect(new Ed25519PublicKey(key.toBuffer()).equals(key)).toBe(true);
  });

  it('toHaneulAddress', () => {
    const key = new Ed25519PublicKey(new Uint8Array(BASE64_KEY_BYTES));
    expect(key.toHaneulAddress()).toEqual(
      '98fc1c8179b95274327069cf3b0ed051fb14e0bc'
    );
  });

  TEST_CASES.forEach((address, base64) => {
    it(`toHaneulAddress from base64 public key ${address}`, () => {
      const key = new Ed25519PublicKey(base64);
      expect(key.toHaneulAddress()).toEqual(address);
    });
  });
});