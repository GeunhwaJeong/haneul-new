...
import { fromB64, toB58, toB64 } from '@haneullabs/bcs';
...
// Test case generated against rust keytool cli. See https://github.com/GeunhwaJeong/haneul/blob/edd2cd31e0b05d336b1b03b6e79a67d8dd00d06b/crates/haneul/src/unit_tests/keytool_tests.rs#L165
const TEST_CASES = [
	[
...
describe('secp256k1-keypair', () => {
	...
	it('signature of data is same as rust implementation', async () => {
		...
		// Assert the signature is the same as the rust implementation. See https://github.com/GeunhwaJeong/fastcrypto/blob/0436d6ef11684c291b75c930035cb24abbaf581e/fastcrypto/src/tests/secp256k1_tests.rs#L115
	...
	it('create keypair from secret key and mnemonics matches keytool', () => {
		for (const t of TEST_CASES) {
			...
			expect(imported.getPublicKey().toHaneulAddress()).toEqual(t[2]);
	...
	it('incorrect purpose node for secp256k1 derivation path', () => {
		expect(() => {
			Secp256k1Keypair.deriveKeypair(TEST_MNEMONIC, `m/44'/8282'/0'/0'/0'`);
		}).toThrow('Invalid derivation path');
	});

	it('incorrect hardened path for secp256k1 key derivation', () => {
		expect(() => {
			Secp256k1Keypair.deriveKeypair(TEST_MNEMONIC, `m/54'/8282'/0'/0'/0'`);
		}).toThrow('Invalid derivation path');
	});
	...
	it('signs TransactionBlocks', async () => {
		...
		txb.setSender(keypair.getPublicKey().toHaneulAddress());
		txb.setGasPrice(5);
