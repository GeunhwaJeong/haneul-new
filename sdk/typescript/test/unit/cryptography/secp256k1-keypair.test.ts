...
describe('secp256k1-keypair', () => {
  ...
  it('derive secp256k1 keypair from path and mnemonics', () => {
    // Test case generated against rust: /haneul/crates/haneul/src/unit_tests/keytool_tests.rs#L149
    ...
    expect(keypair.getPublicKey().toHaneulAddress()).toEqual(
  ...
  it('incorrect purpose node for secp256k1 derivation path', () => {
    expect(() => {
      Secp256k1Keypair.deriveKeypair(`m/44'/8282'/0'/0'/0'`, TEST_MNEMONIC);
    }).toThrow('Invalid derivation path');
  });

  it('incorrect hardened path for secp256k1 key derivation', () => {
    expect(() => {
      Secp256k1Keypair.deriveKeypair(`m/54'/8282'/0'/0'/0'`, TEST_MNEMONIC);
    }).toThrow('Invalid derivation path');
  });
});
