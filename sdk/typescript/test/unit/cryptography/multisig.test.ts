...
describe('Multisig scenarios', () => {
	it('multisig address creation and combine sigs using Secp256r1Keypair', async () => {
		...
		txb.setSender(k3.getPublicKey().toHaneulAddress());
		txb.setGasPrice(5);
...
describe('Multisig address creation:', () => {
	...
	it('`toMultiSigAddress()` with zklogin identifiers', async () => {
		...
		const multisigAddress = multisigPublicKey.toHaneulAddress();
...
describe('MultisigKeypair', () => {
	test('signTransactionBlock', async () => {
		...
		expect(signerPubKey.toHaneulAddress()).toEqual(publicKey.toHaneulAddress());
		...
		expect(signerPubKey2.toHaneulAddress()).toEqual(publicKey.toHaneulAddress());
	...
	test('unknown signers', async () => {
		...
		expect(() => publicKey.getSigner(k2)).toThrow(
			new Error(`Signer ${pk2.toHaneulAddress()} is not part of the MultiSig public key`),
		);
	});
});
