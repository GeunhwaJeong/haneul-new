...
import { normalizeHaneulAddress, HANEUL_ADDRESS_LENGTH } from '../../../src/utils/haneul-types.js';
...
describe('Publickey', () => {
	...
	it('`toHaneulPublicKey()` should return a valid haneul representation', async () => {
		expect(pk2.toHaneulPublicKey()).toEqual(toBase64(pk2.toHaneulBytes()));
	...
	it('`toHaneulBytes()` should return the correct byte representation of the public key with the signature scheme flag', async () => {
		const pk1HaneulBytes = new Uint8Array(pk1.toRawBytes().length + 1);
		pk1HaneulBytes.set([0x00]);
		pk1HaneulBytes.set(pk1.toRawBytes(), 1);

		expect(pk1.toHaneulBytes()).toEqual(pk1HaneulBytes);
		expect(pk1.toHaneulBytes()).toEqual(
			new Uint8Array([
		...
		const pk2HaneulBytes = new Uint8Array(pk2.toRawBytes().length + 1);
		pk2HaneulBytes.set([0x01]);
		pk2HaneulBytes.set(pk2.toRawBytes(), 1);

		expect(pk2.toHaneulBytes()).toEqual(pk2HaneulBytes);
		expect(pk2.toHaneulBytes()).toEqual(
			new Uint8Array([
		...
		const pk3HaneulBytes = new Uint8Array(pk3.toRawBytes().length + 1);
		pk3HaneulBytes.set([0x02]);
		pk3HaneulBytes.set(pk3.toRawBytes(), 1);

		expect(pk3.toHaneulBytes()).toEqual(pk3HaneulBytes);
		expect(pk3.toHaneulBytes()).toEqual(
			new Uint8Array([
	...
	it('`toHaneulAddress()` should correctly return haneul address associated with Ed25519 publickey', async () => {
		const pk1HaneulAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk1.toHaneulBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
		const pk2HaneulAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk2.toHaneulBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
		const pk3HaneulAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk3.toHaneulBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
