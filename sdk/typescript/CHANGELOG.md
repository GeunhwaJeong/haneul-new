...
- cdcfa76c43: Add a new client method for retrieving epoch metrics (haneulx_getEpochMetrics)

### Patch Changes

- Updated dependencies [fce0a08d0f]
  - @haneullabs/bcs@0.9.0
...
- faa13ded9: Ensure that TransactionBlocks can be copied via structuredClone to workaround bug in haneul wallet
...
- ba8e3b857: Rename TransactionBlock generated type in @haneullabs/haneul.js/client to HaneulTransactionBlock to avoid conflicting names in exports
...
- cc6441f46: The Haneul TS SDK has been broken up into a set of modular exports, and all exports from the root of
...
- 001148443: Introduce new `@haneullabs/haneul.js/faucet` export, which should be used for all faucet interactions. This deprecates the previous `requestHaneulFromFaucet` APIs that existed on the `JsonRpcProvider` and `Signer` classes.
...
- acc2edb31: Update schema for `HaneulSystemState` and `DelegatedStake`
- 941b03af1: Change functions in transactions.ts of ts-sdk such that: `getTotalGasUsed` and `getTotalGasUsedUpperBound` of ts-sdk return a `bigint`,fields of `gasCostSummary` are defined as `string`, `epochId` is defined as `string`. In `haneul-json-rpc` the corresponding types are defined as `BigInt`. Introduce `HaneulEpochId` type to `haneul-json-rpc` types that is a `BigInt`.
...
- f1e42f792: Consolidate get_object and get_raw_object into a single get_object endpoint which now takes an additional config parameter with type `HaneulObjectDataOptions` and has a new return type `HaneulObjectResponse`. By default, only object_id, version, and digest are fetched.
- 272389c20: Support for new versioned TransactionData format
- 3de8de361: Remove `getHaneulSystemState` method. Use `getLatestHaneulSystemState` method instead.
- be3c4f51e: Add `display` field in `HaneulObjectResponse` for frontend rendering. See more details in https://forums.haneul.io/t/nft-object-display-proposal/4872
...
- c82e4b454: Introduce BigInt struct to haneul-json-rpc-types to serialize and deserialize amounts to/from string. Change ts-sdk to serialize amounts of PayHaneul and Pay as string.
- 7a2eaf4a3: Changing the HaneulObjectResponse struct to use data/error fields instead of details/status
...
- d5ef1b6e5: Added dependencies to publish command, dependencies now also returned from the haneul move CLI with the `--dump-bytecode-as-base64` flag
...
- 3de8de361: Remove `getValidators` API. Use `getLatestHaneulSystemState` instead.
...
- 65f1372dd: Rename `provider.getTransactionWithEffects` to `provider.getTransaction`. The new method takes in an additional parameter `HaneulTransactionBlockResponseOptions` to configure which fields to fetch(transaction, effects, events, etc). By default, only the transaction digest will be returned.
...
- 3304eb83b: Refactor Rust HaneulTransactionBlockKind to be internally tagged for Json serialization with tag="type" and HaneulEvent to be adjacently tagged with tag="type" and content="content"
...
- 64fb649eb: Remove old `HaneulExecuteTransactionResponse` interface, and `CertifiedTransaction` interface in favor of the new unified `HaneulTransactionBlockResponse` interfaces.
...
- ebe6c3945: Support deserializing `payHaneul` and `payAllHaneul` transactions
...
- 5cd51dd38: Deprecate haneul_executeTransaction in favor of haneul_executeTransactionSerializedSig
...
- d2015f815: Rebuilt type-narrowing utilties (e.g. `isHaneulObject`) on top of Superstruct, which should make them more reliable.
...
- 3b510d0fc: adds coin transfer method to framework that uses pay and payHaneul
...
- e45b188a8: Introduce PayHaneul and PayAllHaneul native transaction types to TS SDK.
...
- 2dc594ef7: Introduce getCoinDenominationInfo, which returns denomination info of a coin, now only supporting HANEUL coin.
...
- 4575c0a02: Fix type definition of HaneulMoveNormalizedType
