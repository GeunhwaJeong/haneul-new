...
- 929db4976a: Add normalizeHaneulNSName and isValidHaneulNSName utils, and add a format option to
...
- cc6441f46: The Haneul TS SDK has been broken up into a set of modular exports, and all exports from
  ...
  The ability to request Haneul from a faucet was not added to `HaneulClient`, instead you will need to
...
- 001148443: Introduce new `@haneullabs/haneul.js/faucet` export, which should be used for all faucet
  interactions. This deprecates the previous `requestHaneulFromFaucet` APIs that existed on the
...
- 941b03af1: Change functions in transactions.ts of ts-sdk such that: `getTotalGasUsed` and
  ...
  `string`, `epochId` is defined as `string`. In `haneul-json-rpc` the corresponding types are defined
  as `BigInt`. Introduce `HaneulEpochId` type to `haneul-json-rpc` types that is a `BigInt`.
...
- 33cb357e1: Change functions in json-rpc-provider.ts of ts-sdk such that:
  ...
  `cursor` of `getCheckpoints` is defined as a `string`. Introduce `HaneulCheckpointSequenceNumber`
  type in haneul-json-rpc-types that is a `BigInt` to use instead of `CheckpointSequenceNumber` of
  haneul-types.
...
- be3c4f51e: Add `display` field in `HaneulObjectResponse` for frontend rendering. See more details in
  https://forums.haneul.io/t/nft-object-display-proposal/4872
...
- c82e4b454: Introduce BigInt struct to haneul-json-rpc-types to serialize and deserialize amounts
  to/from string. Change ts-sdk to serialize amounts of PayHaneul and Pay as string.
...
- d5ef1b6e5: Added dependencies to publish command, dependencies now also returned from the haneul move
...
- 64fb649eb: Remove old `HaneulExecuteTransactionResponse` interface, and `CertifiedTransaction`
