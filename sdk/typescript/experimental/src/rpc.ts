...
/**
 * Result of a `haneul_getObject` RPC call.
 * Extends `HaneulObjectRef` and can be used as one.
 */
export interface OwnedObjectResponse extends HaneulObjectRef {
  objectId: string;
  version: number;
  digest: string;
  type: string;
...
/**
 * Object where keys are objectId's and values are HaneulObjectRef.
 */
export type RefsById = { [key: string]: HaneulObjectRef };
...
export type TxResponse = {
   ...
   * Only @PublishTx
   */
  createdByType: { [key: string]: HaneulObjectRef[] };
  /**
   * Lists deleted objects indexed by objectid. Only available in
   * @MoveCallTx
   */
  deleted: { [key: string]: HaneulObjectRef };
  /**
  ...
  /**
   * Updated HaneulObjectRef for the gas object that was used when executing
   * the transaction.
   */
  gas: HaneulObjectRef;
  /**
...
export class RpcClient {
  ...
  public async fetchObj(id: string): Promise<any> {
    let res = await this.req("haneul_getObject", [id]);
    return res;
  }
   ...
   * @param id ID of the object to fetch
   */
  public async fetchObjRef(id: string): Promise<HaneulObjectRef> {
  ...
  public async myObjects(address: string): Promise<OwnedObjectResponse[]> {
    return this.req("haneul_getObjectsOwnedByAddress", [address]);
  }
  ...
  public async sendTx(args: any[]): Promise<TxResponse> {
    let res = await this.reqTx("haneul_executeTransaction", args);
    if (res.error) {
 ...
 */
function groupRefs(
  array: { reference: HaneulObjectRef }[] | HaneulObjectRef[]
): RefsById {
  // @ts-ignore
  return array.reduce(
    (acc: any, obj: any) =>
