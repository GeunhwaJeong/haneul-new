...
import { bcs as BCS, toB64 } from "@haneullabs/bcs";
 ...
 * the Gateway service to the `haneul_executeTransaction` method.
 */
 ...
 * data can be considered safe enough. Especially if HaneulClient is run by HaneulWallet.
 */
export class HaneulClient {
   ...
   */
  static local(gasRef: HaneulObjectRef | null = null): HaneulClient {
   ...
   * @param gasRef Optional gas setting
   */
  static devnet(gasRef: HaneulObjectRef | null = null): HaneulClient {
    return new this(
      "https://gateway.devnet.haneul.io",
      "https://fullnode.devnet.haneul.io",
      gasRef
    );
  }
   ...
   */
  protected objects: Map<string, HaneulObjectRef> = new Map();
  ...
  constructor(
     ...
     */
    protected gasRef: HaneulObjectRef | null = null
  ) {
   ...
   */
  public async ref(id: string, force: boolean = false): Promise<HaneulObjectRef> {
    if (force) {
  ...
  public async myObjectsByType(
    address: string
  ): Promise<{ [key: string]: HaneulObjectRef[] }> {
    let objects = await this.rpc.myObjects(address);
    let byType: { [key: string]: HaneulObjectRef[] } = {};
   ...
   * @param id ID of the object
   */
  public async fetchObject(id: string): Promise<HaneulObjectRef> {
   ...
   */
  updateLocalStorage(objects: { [key: string]: HaneulObjectRef }) {
   ...
   */
  gasOrFail(): HaneulObjectRef {
    if (this.gasRef === null) {
      throw new Error(
        `To send transactions, please provide gas with haneul.setGas(id) method`
      );
    }
...
/**
 * Get the Haneul address from a keypair using a `sha3_256` hash substring.
 */
