...
const domainName = 'test.haneul';
const walletAddress = '0xfce343a643991c592c4f1a9ee415a7889293f694ab8828f78e3c81d11c9530c6';

describe('HaneulNS Client', () => {
	const client = new HaneulnsClient(new HaneulClient({ url: getFullnodeUrl('testnet') }), {
		networkType: 'testnet',
		contractObjects: {
			...
			haneulns: '0x4acaf19db12fafce1943bbd44c7f794e1d81d00aeb63617096e5caa39499ba88',
		},
	});

	const nonExistingDomain = walletAddress + '.haneul';
	const nonExistingWalletAddress = walletAddress.substring(0, walletAddress.length - 4) + '0000';

	beforeEach(async () => {
		await client.getHaneulnsContractObjects();
	});
	...
	describe('getNameObject', () => {
		it('returns related data of the name', async () => {
			expect(
				...
				// avatar: 'https://api-testnet.haneulfrens.haneul.io/haneulfrens/0x4e3ba002444df6c6774f41833f881d351533728d585343c58cca1fec1fef74ef/svg',
