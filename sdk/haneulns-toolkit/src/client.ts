...
import { DataFields, NameObject, NetworkType, HaneulNSContract } from './types/objects';
...
class HaneulnsClient {
	...
	async getHaneulnsContractObjects() {
		if ((this.contractObjects as HaneulNSContract)?.packageId) return;
		...
		try {
			...
			throw new Error(`Error getting HaneulNS contract objects, ${(error as Error).message}`);
		}
	...
	async getNameObject(
		...
		await this.getHaneulnsContractObjects();

		const registryResponse = await this.getDynamicFieldObject(
			(this.contractObjects as HaneulNSContract).registry,
		...
		if (nameObject.nftId && (includeAvatar || options?.showOwner)) {
			const [owner, avatarNft] = await Promise.all([
				...
				includeAvatar ? getAvatar(this.haneulClient, nameObject.avatar) : Promise.resolve(null),
			]);
	 ...
	 *
	 * @param domain a domain name ends with `.haneul`
	 */
	 ...
	 *
	 * @param address a Haneul address.
	 */
