...
//! lock_service is a single-threaded atomic Haneul Object locking service.
...
enum LockServiceCommands {
    Acquire {
        ...
        resp: oneshot::Sender<HaneulResult>,
    },
    Initialize {
        refs: Vec<ObjectRef>,
        resp: oneshot::Sender<HaneulResult>,
...
type HaneulLockResult = Result<Option<Option<TransactionDigest>>, HaneulError>;
...
enum LockServiceQueries {
    GetLock {
        object: ObjectRef,
        resp: oneshot::Sender<HaneulLockResult>,
    },
    CheckLocksExist {
...
impl LockServiceImpl {
    ...
    fn try_open_db<P: AsRef<Path>>(path: P, db_options: Option<Options>) -> Result<Self, HaneulError> {
    ...
    fn get_lock(&self, object: ObjectRef) -> Result<Option<Option<TransactionDigest>>, HaneulError> {
    ...
    fn locks_exist(&self, objects: &[ObjectRef]) -> HaneulResult {
        ...
        locks
            ...
            .ok_or_else(|| {
                ...
                HaneulError::TransactionLockDoesNotExist
            })
    }
    ...
    /// Otherwise, HaneulError(TransactionLockDoesNotExist, ConflictingTransaction) is returned.
    fn acquire_locks(
        &self,
    ...
    fn initialize_locks(&self, objects: &[ObjectRef]) -> HaneulResult {
        ...
        if !existing_locks.is_empty() {
            ...
            return Err(HaneulError::TransactionLockExists {
                refs: existing_locks,
            });
        }
    ...
    fn delete_locks(&self, objects: &[ObjectRef]) -> HaneulResult {
...
/// Atomic Haneul Object locking service.
...
impl LockService {
    ...
    /// namely each HaneulDataStore creates its own LockService.
    pub fn new<P: AsRef<Path>>(path: P, db_options: Option<Options>) -> Result<Self, HaneulError> {
    ...
    pub async fn acquire_locks(
        ...
        let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
    ...
    /// Initialize a lock to None (but exists) for a given list of ObjectRefs.
    /// If the lock already exists and is locked to a transaction, then return TransactionLockExists
    pub async fn initialize_locks(&self, refs: Vec<ObjectRef>) -> HaneulResult {
        let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
        self.sender
    ...
    /// Removes locks for a given list of ObjectRefs.
    pub async fn remove_locks(&self, refs: Vec<ObjectRef>) -> HaneulResult {
        let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
        self.sender
    ...
    /// * Some(None) - lock exists and is initialized, but not locked to a particular transaction
    /// * Some(Some(tx_digest)) - lock exists and set to transaction
    pub async fn get_lock(&self, object: ObjectRef) -> HaneulLockResult {
        let (os_sender, os_receiver) = oneshot::channel::<HaneulLockResult>();
        self.query_sender
    ...
    /// Checks multiple object locks exist.
    /// Returns Err(TransactionLockDoesNotExist) if at least one object lock is not initialized.
    pub async fn locks_exist(&self, objects: Vec<ObjectRef>) -> HaneulResult {
        let (os_sender, os_receiver) = oneshot::channel::<HaneulResult>();
        self.query_sender
...
mod tests {
    ...
    use haneul_types::base_types::{ObjectDigest, ObjectID, ObjectRef, TransactionDigest};
    ...
    fn test_lockdb_acquire_init_multiple() {
        ...
        assert_eq!(
            ...
            Err(HaneulError::TransactionLockDoesNotExist)
        );
        ...
        assert!(matches!(
            ...
            Err(HaneulError::TransactionLockExists { .. })
        ));
        ...
        assert!(matches!(
            ...
            Err(HaneulError::ConflictingTransaction { .. })
        ));
    }
    ...
    async fn test_lockservice_conc_acquire_init() {
        ...
        assert!(inner_res
            .iter()
            .filter(|r| r.is_err())
            .all(|r| matches!(r, Err(HaneulError::ConflictingTransaction { .. }))));
    }
}
