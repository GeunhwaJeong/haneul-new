    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn spawn(mut self, duration: Duration) -> (AuthorityName, Result<(), HaneulError>) {
        ...
        let result = tokio::task::spawn(async move { self.gossip_timeout(duration).await })
            .await
            .map(|_| ())
            .map_err(|_err| HaneulError::GenericAuthorityError {
    ...
    async fn gossip_timeout(&mut self, duration: Duration) -> Result<(), HaneulError> {
                ...
                digest = &mut queue.next() , if !queue.is_empty() => {
                    ...
                    if !self.state._database.effects_exists(&digest)? {
                        ...
                        if let Some(certificate) = response.certified_transaction {
                            ...
                            return Err(HaneulError::ByzantineAuthoritySuspicion { authority :  self.peer_name });
                        }
                    }
                },
            };
