...
impl AuthorityState {
    ...
    pub fn set_batch_sender(
        ...
        if self.batch_channels.is_some() {
            return Err(HaneulError::AuthorityUpdateFailure);
        }
    ...
    pub fn subscribe(&self) -> Result<BroadcastReceiver, HaneulError> {
        self.batch_channels
            .as_ref()
    ...
    fn check_one_lock(
        ...
        object: &Object,
        mutable_object_addresses: &HashSet<HaneulAddress>,
    ) -> HaneulResult {
        match object_kind {
            ...
            InputObjectKind::OwnedMoveObject((object_id, sequence_number, object_digest)) => {
                ...
                if let TransactionKind::Transfer(t) = &transaction.data.kind {
                    if object_kind.object_id() == t.object_ref.0 {
                        fp_ensure!(
                            ...
                            HaneulError::TransferSharedError
                        );
                    }
                }
                ...
                match object.owner {
                    ...
                    Owner::SharedMutable => {
                        return Err(HaneulError::UnsupportedSharedObjectError);
                    }
                };
            }
    ...
    async fn check_locks(
        &self,
        transaction: &Transaction,
    ) -> Result<Vec<(InputObjectKind, Object)>, HaneulError> {
                    ...
                    Some(lock) => {
                        if let Some(object) = self._database.get_object(object_id)? {
                            if object.version() != lock {
                                lock_errors.push(HaneulError::InvalidSequenceNumber);
                            }
                        }
                    }
                    None => lock_errors.push(HaneulError::InvalidSequenceNumber),
                }
            }
            fp_ensure!(
    ...
    ) -> Result<(VecDeque<UpdateItem>, bool), HaneulError> {
    ...
    ) -> Result<(u64, TransactionInfoResponse), HaneulError> {
        self._database
