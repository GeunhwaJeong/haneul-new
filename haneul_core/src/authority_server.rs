...
impl AuthorityServer {
    ...
    pub async fn spawn_batch_subsystem(
        ...
        min_batch_size: u64,
        max_delay: Duration,
    ) -> HaneulResult<tokio::task::JoinHandle<HaneulResult<()>>> {
        ...
        A: RwChannel<'b>,
    {
        ...
        while let Some(item) = items.pop_front() {
            ...
            channel
                ...
                .await
                .map_err(|_| HaneulError::CannotSendClientMessageError)?;
        }
        ...
        loop {
            match subscriber.recv().await {
                ...
                Err(RecvError::Closed) => {
                    ...
                    return Err(HaneulError::SubscriptionServiceClosed);
                }
                Err(RecvError::Lagged(number_skipped)) => {
                    // We tell the client they are too slow to consume, and
                    // stop.
                    return Err(HaneulError::SubscriptionItemsDroppedError(number_skipped));
                }
            }
        }
    ...
    fn batch_verify_one_chunk(
        &self,
        one_chunk: Vec<Result<(SerializedMessage, BytesMut), HaneulError>>,
    ) -> Result<VecDeque<(SerializedMessage, BytesMut)>, HaneulError> {
        ...
        let load_verification: Result<VecDeque<(SerializedMessage, BytesMut)>, HaneulError> =
            one_chunk
                .into_iter()
                .map(|mut item| {
    ...
    A: 'static + RwChannel<'a> + Unpin + Send,
{
    async fn handle_messages(&self, mut channel: A) -> () {
        /*
            ...
            taken to turn all errors into HaneulError.
        */
        ...
        while let Some(one_chunk) = channel
            ...
            .map(|msg_bytes_result| {
                msg_bytes_result
                    .map_err(|_| HaneulError::InvalidDecoding)
                    .and_then(|msg_bytes| {
