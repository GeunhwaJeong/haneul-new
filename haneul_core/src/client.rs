...
pub struct ClientAddressManager<A> {
    ...
    address_states: BTreeMap<HaneulAddress, ClientState<A>>,
}
...
pub trait Client {
    ...
    async fn try_complete_pending_transactions(&mut self) -> Result<(), HaneulError>;
...
impl<A> ClientState<A> {
    ...
    fn tx_digests_for_object(
        &self,
        object_id: &ObjectID,
    ) -> Result<Vec<TransactionDigest>, HaneulError> {
    ...
    pub fn certificates(
        &self,
        object_id: &ObjectID,
    ) -> Result<Vec<CertifiedTransaction>, HaneulError> {
        ...
        self.store
            ...
            .map(|(opt_cert, tx_digest)| {
                opt_cert.ok_or(HaneulError::CertificateNotfound {
