...
/// ```
/// use signature::Error;
/// use haneul_core::client::AsyncTransactionSigner;
...
pub struct ClientAddressManager<A> {
    ...
    address_states: BTreeMap<HaneulAddress, ClientState>,
}
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub fn create_account_state(&mut self, address: HaneulAddress) -> HaneulResult {
        fp_ensure!(
            !self.address_states.contains_key(&address),
            HaneulError::AccountExists
        );
    ...
    fn get_account(&self, address: &HaneulAddress) -> HaneulResult<&ClientState> {
        ...
        self.address_states
            .get(address)
            .ok_or(HaneulError::AccountNotFound)
    }

    /// Get all the states
    pub fn get_managed_address_states(&self) -> &BTreeMap<HaneulAddress, ClientState> {
        &self.address_states
    }
...
#[async_trait]
pub trait Client {
    /// Send object to a Haneul account.
    ...
    async fn sync_client_state(&mut self, account_addr: HaneulAddress) -> Result<(), anyhow::Error>;
    ...
    fn get_owned_objects(&self, account_addr: HaneulAddress) -> Vec<ObjectRef>;
