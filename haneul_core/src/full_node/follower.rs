    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn follow(&mut self) -> Result<(), HaneulError> {
        let mut start = 0;
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn update_state(
        ...
        let signed_effects = tx_resp
            .clone()
            .signed_effects
            .ok_or(HaneulError::ByzantineAuthoritySuspicion { authority: name })?;
        ...
        while let Some((name, digest)) = recv.next().await {
            if !d.state.store.effects_exists(&digest)? {
                ...
                if response.clone().certified_transaction.is_some() {
                    ...
                    let err: Result<(), HaneulError> =
                        Err(HaneulError::ByzantineAuthoritySuspicion { authority: name });
