...
use std::io;
use haneul_network::network::{parse_recv_bytes, NetworkClient};
use haneul_network::transport::TcpDataStream;
...
impl AuthorityAPI for AuthorityClient {
    ...
    async fn handle_batch_streaming(
        ...
        // BatchInfoResponseItem, then send a Result<BatchInfoResponseItem, HaneulError to the channel
...
impl AuthorityClient {
    ...
    pub async fn handle_batch_streaming_as_stream(
        &self,
        request: BatchInfoRequest,
    ) -> Result<impl Stream<Item = Result<BatchInfoResponseItem, HaneulError>>, io::Error> {
        let tcp_stream = self
            .0
            .connect_for_stream(serialize_batch_request(&request))
            .await?;
        ...
        let stream = framed_read
            .map(|item| {
                item
                    // Convert io error to HaneulCLient error
                    .map_err(|err| HaneulError::ClientIoError {
                        error: format!("io error: {:?}", err),
                    })
                    // If no error try to deserialize
                    .and_then(|bytes| match deserialize_message(&bytes[..]) {
                        Ok(SerializedMessage::Error(error)) => Err(HaneulError::ClientIoError {
