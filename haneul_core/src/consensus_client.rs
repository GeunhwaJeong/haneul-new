...
use haneul_types::messages::{ConfirmationTransaction, ConsensusOutput, ConsensusSync};
use haneul_types::serialize::{deserialize_message, serialize_consensus_sync, SerializedMessage};
use haneul_types::{fp_bail, fp_ensure};
...
impl ConsensusClient {
    ...
    pub fn new(state: Arc<AuthorityState>) -> HaneulResult<Self> {
        // Ensure there is a single consensus client modifying the state.
        let status = state
            .consensus_guardrail
            .fetch_add(1, AtomicOrdering::SeqCst);
        fp_ensure!(status == 0, HaneulError::OnlyOneConsensusClientPermitted);
    ...
    /// and right order.
    async fn synchronize(&mut self, connection: &mut TcpDataStream) -> HaneulResult<()> {
        ...
        connection
            ...
            .send(bytes)
            .await
            .map_err(|e| HaneulError::ClientIoError {
                error: e.to_string(),
            })
    }

    /// Process a single sequenced certificate.
    async fn handle_consensus_message(&mut self, bytes: Bytes) -> HaneulResult<ProcessingOutcome> {
        ...
        let confirmation = match deserialize_message(&*consensus_message) {
            ...
            Ok(_) => {
                debug!("{}", HaneulError::UnexpectedMessage);
                return Err(HaneulError::UnexpectedMessage);
            }
            Err(e) => {
    ...
    async fn run(&mut self, address: SocketAddr, buffer_size: usize) -> HaneulResult<()> {
        // TODO: We may also move this logic to `haneul-network::transport` to expose a 'stream client'
        ...
        'main: loop {
            ...
            loop {
                ...
                match self.handle_consensus_message(bytes).await {
                    ...
                    Err(HaneulError::StorageError(e)) => {
                        error!("{e}");
                        ...
                        fp_bail!(HaneulError::StorageError(e));
                    }
