...
use haneul_types::messages::{ConfirmationTransaction, ConsensusOutput};
...
impl ConsensusClient {
    ...
    async fn synchronize(&mut self) -> HaneulResult<()> {
        // TODO [issue #932]: [liveness-critical] Implement the synchronizer.
        Ok(())
    }

    /// Process a single sequenced certificate.
    async fn handle_consensus_message(&mut self, bytes: Bytes) -> HaneulResult<()> {
