...
/// ```
/// use signature::Error;
/// use haneul_core::gateway_state::AsyncTransactionSigner;
...
///     async fn sign(&self, address: &HaneulAddress, data: TransactionData) -> Result<Signature, Error> {
...
pub struct GatewayState<A> {
    ...
    address_states: BTreeMap<HaneulAddress, AccountState>,
}
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    fn get_or_create_account(&mut self, address: HaneulAddress) -> HaneulResult<&AccountState> {
    ...
    /// Get all the states
    #[cfg(test)]
    pub fn get_managed_address_states(&self) -> &BTreeMap<HaneulAddress, AccountState> {
        &self.address_states
    }
...
pub struct AccountState {
    /// Haneul address of this account.
    address: HaneulAddress,
...
pub trait GatewayAPI {
    /// Send coin object to a Haneul address.
    ...
    async fn sync_account_state(&mut self, account_addr: HaneulAddress) -> Result<(), anyhow::Error>;
    ...
    fn get_owned_objects(&mut self, account_addr: HaneulAddress) -> Vec<ObjectRef>;

    /// Fetch objects from authorities
    async fn download_owned_objects_not_in_db(
        &mut self,
        account_addr: HaneulAddress,
    ) -> Result<BTreeSet<ObjectRef>, HaneulError>;
}
...
impl AccountState {
    ...
    #[cfg(test)]
    pub fn new(path: PathBuf, address: HaneulAddress) -> Self {
        AccountState {
            address,
    ...
    pub fn new_for_manager(address: HaneulAddress, store: gateway_store::AccountStore) -> Self {
    ...
    pub fn update_object_ref(&self, object_ref: &ObjectRef) -> HaneulResult {
    ...
    ) -> HaneulResult<BTreeSet<ObjectRef>> {
        let result = self
            .store
            .objects
    ...
    pub fn clear_object_refs(&self) -> HaneulResult {
        self.store.object_refs.clear()?;
        Ok(())
    }

    pub fn insert_object(&self, object: Object) -> HaneulResult {
        self.store
            .objects
    ...
    pub fn remove_object_info(&self, object_id: &ObjectID) -> Result<(), HaneulError> {
    ...
    /// TODO: always return true for immutable objects https://github.com/GeunhwaJeong/haneul/issues/305
    fn can_lock_or_unlock(&self, transaction: &Transaction) -> Result<bool, HaneulError> {
    ...
    /// Double-locking can cause equivocation. TODO: https://github.com/GeunhwaJeong/haneul/issues/335
    pub fn lock_pending_transaction_objects(
        &self,
        transaction: &Transaction,
    ) -> Result<(), HaneulError> {
        if !self.can_lock_or_unlock(transaction)? {
            return Err(HaneulError::ConcurrentTransactionError);
        }
        self.store
            .pending_transactions
            .multi_insert(
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn get_framework_object_ref(&mut self) -> Result<ObjectRef, anyhow::Error> {
        let info = self
            .get_object_info(ObjectID::from(HANEUL_FRAMEWORK_ADDRESS))
            .await?;
        Ok(info.reference()?)
    }
    ...
    /// https://github.com/GeunhwaJeong/haneul/issues/346
    async fn execute_transaction(
        ...
        for object_kind in &transaction.input_objects()? {
            ...
            fp_ensure!(
                ...
                HaneulError::UnexpectedSequenceNumber {
                    object_id,
        ...
        // TODO: https://github.com/GeunhwaJeong/haneul/issues/349
        // https://github.com/GeunhwaJeong/haneul/issues/211
        // https://github.com/GeunhwaJeong/haneul/issues/346
    ...
    async fn update_objects_from_transaction_info(
    ...
    ) -> Result<(CertifiedTransaction, TransactionEffects), HaneulError> {
        ...
        // https://github.com/GeunhwaJeong/haneul/issues/331
        let _failed = self
    ...
    async fn try_complete_pending_transactions(
        ...
        // TODO: https://github.com/GeunhwaJeong/haneul/issues/330
        for transaction in unique_pending_transactions {
            self.execute_transaction(transaction.clone())
                .await
                .map_err(|e| HaneulError::ErrorWhileProcessingTransactionTransaction {
                    err: e.to_string(),
                })?;
        }
        Ok(())
    ...
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    async fn publish(
        ...
        let package = package.ok_or(HaneulError::InconsistentGatewayResult {
