...
async fn test_hero() {
    ...
    hero_path.push("../haneul_programmability/examples/");
    let modules = haneul_framework::build_move_package(&hero_path, build_config, false).unwrap();
...
#[cfg(test)]
fn init_state_parameters() -> (Committee, HaneulAddress, KeyPair, Arc<AuthorityStore>) {
...
#[cfg(test)]
async fn init_state_with_ids<I: IntoIterator<Item = (HaneulAddress, ObjectID)>>(
    objects: I,
) -> AuthorityState {
...
#[cfg(test)]
async fn init_state_with_object_id(address: HaneulAddress, object: ObjectID) -> AuthorityState {
...
async fn call_move(
    ...
    let response = send_and_confirm_order(authority, order).await?;
