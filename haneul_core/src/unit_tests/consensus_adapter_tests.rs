...
async fn listen_to_sequenced_transaction() {
    let (tx_haneul_to_consensus, rx_haneul_to_consensus) = channel(1);
    let (tx_consensus_to_haneul, rx_consensus_to_haneul) = channel(1);
    ...
    ConsensusListener::spawn(
        /* rx_consensus_input */ rx_haneul_to_consensus,
        /* rx_consensus_output */ rx_consensus_to_haneul,
    );
