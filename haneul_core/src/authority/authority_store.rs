    ...
    > HaneulDataStore<ALL_OBJ_VER, USE_LOCKS, S>
{
    ...
    /// Get many objects
    pub fn get_objects(&self, _objects: &[ObjectID]) -> Result<Vec<Option<Object>>, HaneulError> {
    ...
    pub fn insert_genesis_object(&self, object: Object) -> HaneulResult {
    ...
    pub fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> HaneulResult {
        // Insert object
    ...
    /// must be new.
    pub fn bulk_object_insert(&self, objects: &[&Object]) -> HaneulResult<()> {
    ...
    pub fn set_transaction_lock(
        ...
        let mut need_write = false;
        {
            ...
            for lock in locks {
                ...
                let lock = lock.ok_or(HaneulError::TransactionLockDoesNotExist)?;
                if let Some(previous_tx_digest) = lock {
                    if previous_tx_digest != tx_digest {
                        ...
                        return Err(HaneulError::ConflictingTransaction {
        ...
        // new locks must be atomic, and no writes should happen in between.
        {
            ...
            if USE_LOCKS {
                ...
                for object_lock in locks {
                    object_lock.ok_or(HaneulError::TransactionLockDoesNotExist)?;
                }
            }
