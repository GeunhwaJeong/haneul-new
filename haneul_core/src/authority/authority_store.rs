...
impl AuthorityStore {
    ...
    pub fn get_order_lock(&self, object_ref: &ObjectRef) -> Result<Option<SignedOrder>, HaneulError> {
        let order_option = self
            .order_lock
            .get(object_ref)?
            .ok_or(HaneulError::OrderLockDoesNotExist)?;
    ...
    pub fn init_order_lock(&self, object_ref: ObjectRef) -> Result<(), HaneulError> {
    ...
    pub fn set_order_lock(
        ...
        // new locks must be atomic, and not writes should happen in between.
        {
            ...
            for (obj_ref, lock) in mutable_input_objects.iter().zip(locks) {
                ...
                let lock = lock.ok_or(HaneulError::OrderLockDoesNotExist)?;
                ...
                if let Some(previous_tx_digest) = lock {
                    if previous_tx_digest != tx_digest {
                        ...
                        return Err(HaneulError::ConflictingOrder {
    ...
    pub fn update_state(
        ...
        // new locks must be atomic, and no writes should happen in between.
        {
            ...
            for object_lock in locks {
                object_lock.ok_or(HaneulError::OrderLockDoesNotExist)?;
            }
