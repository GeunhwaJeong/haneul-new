...
import { type AddressOwner, DefaultRpcClient } from '../../utils/HaneulRpcClient';
import {
...
type HaneulIdBytes = { bytes: number[] };

function handleSpecialDemoNameArrays(data: {
    name?: HaneulIdBytes | string;
    player_name?: HaneulIdBytes | string;
    monster_name?: HaneulIdBytes | string;
    farm_name?: HaneulIdBytes | string;
}): string {
    let bytesObj: HaneulIdBytes = { bytes: [] };

    if ('player_name' in data) {
        bytesObj = data.player_name as HaneulIdBytes;
        const ascii = asciiFromNumberBytes(bytesObj.bytes);
        delete data.player_name;
        return ascii;
    } else if ('monster_name' in data) {
        bytesObj = data.monster_name as HaneulIdBytes;
        const ascii = asciiFromNumberBytes(bytesObj.bytes);
        delete data.monster_name;
        return ascii;
    } else if ('farm_name' in data) {
        bytesObj = data.farm_name as HaneulIdBytes;
        const ascii = asciiFromNumberBytes(bytesObj.bytes);
        delete data.farm_name;
        return ascii;
    } else if ('name' in data) {
        bytesObj = data.name as HaneulIdBytes;
