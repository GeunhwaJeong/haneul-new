...
import { isValidTransactionDigest, isValidHaneulAddress } from '@haneullabs/haneul.js';
...
function isInputValid(category: SearchCategory, input: string): boolean {
    ...
    switch (category) {
        case 'objects':
        case 'addresses':
            return isValidHaneulAddress(input) || isGenesisLibAddress(input);
        case 'transactions':
            return isValidTransactionDigest(input);
        case 'all':
            return (
                isValidHaneulAddress(input) ||
