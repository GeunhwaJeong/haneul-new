# Use Docker to Run a Haneul Full node Locally

Follow the steps in this Readme to install and configure a Haneul Full node for testing locally using Docker. The instructions were validated on the following operating system/processor combinations:
 ...
 * Download the Full node [docker-compose.yaml](https://github.com/GeunhwaJeong/haneul/blob/main/docker/fullnode/docker-compose.yaml) file.


## Configure Haneul Full node

Download the latest version of the Haneul Full node configuration file [fullnode-template.yaml](https://github.com/GeunhwaJeong/haneul/raw/main/crates/haneul-config/data/fullnode-template.yaml). Use the following command to download the file:

```shell
wget https://github.com/GeunhwaJeong/haneul/raw/main/crates/haneul-config/data/fullnode-template.yaml
```

### Download the Haneul genesis blob

The genesis blob contains the information that defined the Haneul network configuration. Before you can start the Full node, you need to download the most recent file to ensure compatibility with the version of Haneul you use. Use the following command to download the [genesis.blob](https://github.com/GeunhwaJeong/haneul-genesis/raw/main/devnet/genesis.blob) from the `devnet` branch of the Haneul repository:
