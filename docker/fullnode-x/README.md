...
1. First start postgres so you can create the DB with the `diesel` command:
	...
	- `psql -U postgres -p 5432 -h localhost -c 'create database haneul_indexer_testnet'`
	- run these in haneul.git/crates/haneul-indexer:
      * `diesel setup --database-url=postgres://postgres:admin@localhost:5432/haneul_indexer_testnet`
