...
echo
echo "Building haneul-node docker image"
...
docker build -f "$DOCKERFILE" "$REPO_ROOT" \
	...
	--platform "$PLATFORM" \
	--output type=oci,rewrite-timestamp=true,force-compression=true,tar=false,dest=$OCI_OUTPUT/haneul-node,name=haneul-node \
	"$@"
