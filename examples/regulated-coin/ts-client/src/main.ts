    ...
    program.command('deny-list-add')
        ...
        .action((options) => {
            ...
            txb.moveCall({
                ...
                arguments: [
                    txb.object(HANEUL_DENY_LIST_OBJECT_ID),
...
async function executeTx(txb: Transaction) {
    ...
    console.log("Connecting to Haneul network: ", HANEUL_NETWORK);
    const haneulClient = new HaneulClient({url: HANEUL_NETWORK});
    ...
    haneulClient.signAndExecuteTransaction({
        signer: adminKeypair,
        transaction: txb,
