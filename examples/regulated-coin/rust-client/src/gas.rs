...
use haneul_sdk::rpc_types::{
    HaneulObjectDataFilter, HaneulObjectDataOptions, HaneulObjectResponseQuery, HaneulRawData,
};
use haneul_sdk::types::base_types::{ObjectID, ObjectRef, HaneulAddress};
use haneul_sdk::types::gas_coin::GasCoin;
...
pub async fn select_gas(
    ...
    if let Some(gas) = input_gas {
        ...
        let object = read_api
            .get_object_with_options(gas, HaneulObjectDataOptions::new())
            .await?
    ...
    let gas_objs = read_api
        .get_owned_objects(
            ...
            Some(HaneulObjectResponseQuery {
                filter: Some(HaneulObjectDataFilter::StructType(GasCoin::type_())),
    ...
    for obj in gas_objs {
        let HaneulRawData::MoveObject(raw_obj) = &obj
            .data
            .as_ref()
