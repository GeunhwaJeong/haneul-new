...
enum CliCommand {
    ...
    /// Transfer coin from the haneul client's active address
    Transfer {
    ...
    /// Burn coin inside the haneul client's active address
    Burn {
        /// The Coin to burn
        #[arg(value_parser)]
        coin: String,
...
async fn cli_parse() -> Result<(AppConfig, AppCommand)> {
    ...
    let command = match command {
        CliCommand::DenyListAdd { address } => {
            AppCommand::DenyListAdd(HaneulAddress::from_str(&address)?)
        }
        CliCommand::DenyListRemove { address } => {
            AppCommand::DenyListRemove(HaneulAddress::from_str(&address)?)
        }
        CliCommand::MintAndTransfer { balance, address } => {
            AppCommand::MintAndTransfer(balance, HaneulAddress::from_str(&address)?)
        }
        CliCommand::Transfer { coin, address } => AppCommand::Transfer(
            ObjectID::from_hex_literal(&coin)?,
            HaneulAddress::from_str(&address)?,
        ),
