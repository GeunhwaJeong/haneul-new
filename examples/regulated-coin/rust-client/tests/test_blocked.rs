...
use haneul_config::{haneul_config_dir, HANEUL_CLIENT_CONFIG, HANEUL_KEYSTORE_FILENAME};
use haneul_keys::keystore::{AccountKeystore, FileBasedKeystore};
use haneul_sdk::rpc_types::{ObjectChange, HaneulTransactionBlockResponse};
...
fn cmd_haneul_client_switch(new_addr: HaneulAddress) -> Result<()> {
    println!("SWITCHING TO ADDRESS: {new_addr}");
    let haneul_client_switch = format!("haneul client switch --address {new_addr}");
    let _ = std::process::Command::new("sh")
        .arg("-c")
        .arg(haneul_client_switch)
        .output()?;
    Ok(())
}
...
fn get_other_address(different_from: HaneulAddress) -> Result<HaneulAddress> {
    ...
    Ok(keystore
        .keys()
        .into_iter()
        .find(|pub_key| HaneulAddress::from(pub_key) != different_from)
        .map(|pub_key| HaneulAddress::from(&pub_key))
...
async fn get_config() -> Result<AppConfig> {
    ...
    let wallet_context =
        WalletContext::new(&haneul_config_dir()?.join(HANEUL_CLIENT_CONFIG), None, None).await?;
...
async fn test_is_blocked() -> Result<()> {
    ...
    cmd_haneul_client_switch(deny_addr)?;
    ...
    let resp2 = run_as_deny_addr(coin.0, admin_addr).await; // Notice we do not use '?' so that
                                                            // cmd_haneul_client_switch runs again
    cmd_haneul_client_switch(admin_addr)?;
    assert!(resp2.is_err());
