...
// Define the USDC token type on Haneul Testnet
// This is the unique identifier for the USDC token on Haneul
...
function HomeContent() {
  ...
  // Get the Haneul client for interacting with the Haneul network
  ...
  const handleSendTokens = async () => {
    ...
    try {
      ...
      // This uses the HaneulClient to get coins of the specified type owned by the current address
      const { data: coins } = await haneulClient.getCoins({
      ...
      // Transaction is used to construct and execute transactions on Haneul
  ...
  return (
    <main className="mainwrapper">
      <div className="outerwrapper">
        <h1 className="h1">Haneul USDC Sender (Testnet)</h1>
        <ConnectButton />
