...
accept and process payments on Haneul. We'll focus on highlighting the trade-offs
...
using the function `fun make_payment(payment_id: u64, coin: Coin<HANEUL>, to: address)`.
...
using the unpacked HANEUL coin.
...
- Payments can still be made using the `identified_payment::make_payment`
  function that uses `haneul::transfer::transfer` under the hood, so payments can
...
object is `key`-only, then the `haneul::transfer::receive` function can be called
in the module that defines the object, but not elsewhere. Elsewhere, the
`haneul::transfer::public_receive` function must be called and can only be used on
