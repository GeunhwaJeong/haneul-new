...
module common::identified_payment {
    ...
    public fun make_payment(payment_id: u64, coin: Coin<HANEUL>, to: address, ctx: &mut TxContext) {
    ...
    public fun make_shared_payment(register_uid: &mut UID, payment_id: u64, coin: Coin<HANEUL>, ctx: &mut TxContext) {
        let payment_amount = coin::value(&coin);
        let identified_payment = IdentifiedPayment {
            id: object::new(ctx),
            payment_id,
    ...
    public fun unpack(identified_payment: IdentifiedPayment): (u64, Coin<HANEUL>) {
    ...
    /// `haneul::transfer::receive` function can only be called on the object from
