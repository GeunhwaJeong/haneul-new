...
/// A betting game that depends on Haneul randomness.
///
/// Anyone can create a new game for the current epoch by depositing HANEUL as the initial balance. The creator can
...
public fun create(reward: Coin<HANEUL>, ctx: &mut TxContext) {
...
public fun close(game: Game, ctx: &mut TxContext): Coin<HANEUL> {
...
entry fun play(game: &mut Game, r: &Random, coin: &mut Coin<HANEUL>, ctx: &mut TxContext) {
    assert!(ctx.epoch() == game.epoch, EInvalidEpoch);
