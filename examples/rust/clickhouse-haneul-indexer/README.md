# ClickHouse Haneul Indexer

A simple example of how to build a custom Haneul indexer that writes transaction data to ClickHouse.
...
- Fetch checkpoints from the Haneul testnet
...
```
Haneul Network → Checkpoints → Concurrent Pipeline → ClickHouse Store → ClickHouse DB
```
