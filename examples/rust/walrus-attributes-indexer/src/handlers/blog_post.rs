...
use haneul_indexer_alt_framework::postgres;
use haneul_indexer_alt_framework::types::base_types::{ObjectID, SequenceNumber, HaneulAddress};
...
use haneul_indexer_alt_framework::FieldCount;
use haneul_indexer_alt_framework::Result;
...
pub enum ProcessedWalrusMetadata {
    Upsert {
        ...
        /// ID of the Blob object on Haneul, used during reads to fetch the actual blob content. If
...
impl Handler for BlogPostPipeline {
    ...
    fn batch(
    ...
    ) -> haneul_indexer_alt_framework::pipeline::BatchStatus {
        for value in values {
            match value {
                ProcessedWalrusMetadata::Upsert {
                    dynamic_field_id, ..
        ...
        haneul_indexer_alt_framework::pipeline::BatchStatus::Pending
    }
...
impl BlogPostPipeline {
    pub fn new(type_string: &str) -> Result<Self> {
        let metadata_type = parse_haneul_struct_tag(type_string)?;
    ...
    fn to_stored(&self) -> Result<StoredBlogPost> {
        match self {
            ProcessedWalrusMetadata::Upsert {
                ...
                // This is meant to validate that the publisher address stored is a valid HaneulAddress
