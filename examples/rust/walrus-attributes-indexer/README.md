...
This is an extension of the [Custom Indexer guide](https://docs.haneul.io/guides/developer/advanced/custom-indexer) to show how to index Walrus blobs and their associated `Metadata` dynamic fields.

## Quickstart

Given a service that allows users to upload blog posts to Walrus and creates an associated `Metadata` dynamic field with `view_count`, `title`, and `publisher` (Haneul address that created the Walrus blob), you can write a corresponding indexer that commits these attributes to a store of your choice to emulate a blog post platform. Then, users can:
...
# Set a path: value attribute pair on the Metadata dynamic field of a Blob object on Haneul.
walrus set-blob-attribute {Haneul blob object id} --attr "title" {title} --attr "view_count" {view_count}
```
...
As of writing, the SequentialConfig is defined [here](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs) and the committer config defaults are:
```
...
The ingestion config is defined [here](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-indexer-alt-framework/src/ingestion/mod.rs) with defaults configured to:
```
...
# Haneul object ID: 0xcfb3d474c9a510fde93262d4b7de66cad62a2005a54f31a63e96f3033f465ed3
