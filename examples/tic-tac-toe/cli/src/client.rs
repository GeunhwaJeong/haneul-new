...
use haneul_sdk::{
    rpc_types::{
        DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, ObjectChange, HaneulData,
        HaneulExecutionStatus, HaneulObjectData, HaneulObjectDataFilter, HaneulObjectDataOptions,
        HaneulObjectResponse, HaneulObjectResponseQuery, HaneulProtocolConfigValue,
...
pub struct Connection {
    /// The Haneul CLI config file, (default: ~/.haneul/haneul_config/client.yaml)
    #[clap(long)]
...
impl Client {
    ...
    pub(crate) fn new(conn: Connection) -> Result<Self> {
        let Some(config) = conn.config.or_else(|| {
            ...
            default.extend([".haneul", "haneul_config", "client.yaml"]);
            Some(default)
        }) else {
            bail!(
                "Cannot find wallet config. No config was supplied, and the default path \
                 (~/.haneul/haneul_config/client.yaml) does not exist.",
            );
        };
    ...
    pub(crate) async fn turn_cap(&mut self, game: &Game) -> Result<ObjectRef> {
        ...
        let query = Some(HaneulObjectResponseQuery::new(
            Some(HaneulObjectDataFilter::StructType(turn_cap_type.clone())),
            Some(HaneulObjectDataOptions::new().with_bcs()),
        ));
        ...
        loop {
            ...
            for HaneulObjectResponse { data, error } in response.data {
    ...
    pub(crate) async fn new_shared_game(&mut self, opponent: HaneulAddress) -> Result<ObjectID> {
        let player = self.wallet.active_address()?;
    ...
    pub async fn new_owned_game(&mut self, opponent_key: PublicKey) -> Result<ObjectID> {
        ...
        let opponent = HaneulAddress::from(&opponent_key);
        ...
        let admin = HaneulAddress::from(&admin_key);
        let admin_bytes =
    ...
    async fn max_gas_budget(&self) -> Result<u64> {
        ...
        let Some(Some(HaneulProtocolConfigValue::U64(max))) = cfg.attributes.get("max_tx_gas") else {
    ...
    async fn execute_transaction(&self, tx: Transaction) -> Result<HaneulTransactionBlockResponse> {
        let response = self
            .wallet
    ...
    async fn client(&self) -> Result<HaneulClient> {
        self.wallet
            .get_client()
            .await
