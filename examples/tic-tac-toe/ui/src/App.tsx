...
function App() {
    ...
    const chain = account?.chains?.find((c) => c.startsWith("haneul:"))?.replace(/^haneul:/, "");
    if (chain) {
...
function Content() {
    ...
    const addr = normalizeHaneulObjectId(path);
    ...
    if (packageId === null) {
        ...
        return <Root />;
    } else if (isValidHaneulObjectId(addr)) {
        return <Game id={addr} />;
    } else {
        return (
            <Error title="Invalid Game ID">
                <code>"{path}"</code> is not a valid HANEUL object ID.
            </Error>
        );
    }
}
