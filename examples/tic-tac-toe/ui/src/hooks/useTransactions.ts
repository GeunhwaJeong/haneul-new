...
import { ObjectRef, Transaction } from "@haneullabs/haneul/transactions";
...
export class Transactions {
    ...
    newMultiSigGame(player: PublicKey, opponent: PublicKey): Transaction {
        ...
        const game = tx.moveCall({
            ...
            arguments: [
                tx.pure.address(player.toHaneulAddress()),
                tx.pure.address(opponent.toHaneulAddress()),
                tx.pure(bcs.vector(bcs.u8()).serialize(admin.toRawBytes()).toBytes()),
            ],
        });

        tx.transferObjects([game], admin.toHaneulAddress());
