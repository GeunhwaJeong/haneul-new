...
fun test_successful_swap() {
    ...
    // Alice gives the custodian their object to hold in escrow.
    {
        ...
        let l1: Locked<Coin<HANEUL>> = ts.take_from_sender();
    ...
    // Bob does the same.
    {
        ...
        let l2: Locked<Coin<HANEUL>> = ts.take_from_sender();
    ...
    {
        let c: Coin<HANEUL> = ts.take_from_address_by_id(ALICE, i2);
        ts::return_to_address(ALICE, c);
    };

    // Bob gets the object from Alice
    {
        let c: Coin<HANEUL> = ts.take_from_address_by_id(BOB, i1);
...
fun test_object_tamper() {
    ...
    // with it.
    {
        ...
        let l: Locked<Coin<HANEUL>> = ts.take_from_sender();
...
fun test_return_to_sender() {
    ...
    {
        ts.next_tx(CUSTODIAN);
        return_to_sender<Coin<HANEUL>>(ts.take_from_sender());
    };
    ...
    {
        let c: Coin<HANEUL> = ts.take_from_address_by_id(ALICE, cid);
        ts::return_to_address(ALICE, c)
    };
