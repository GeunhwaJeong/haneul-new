...
module escrow::shared {
    ...
    use haneul::event;
    use haneul::dynamic_object_field::{Self as dof};
    ...
    fun test_coin(ts: &mut Scenario): Coin<HANEUL> {
        coin::mint_for_testing<HANEUL>(42, ts::ctx(ts))
    }
    ...
    fun test_successful_swap() {
        ...
        // return.
        {
            ...
            let l2: Locked<Coin<HANEUL>> = ts::take_from_sender(&ts);
            let c = swap<Coin<HANEUL>, Coin<HANEUL>>(
                escrow,
                k2,
                l2,
                ts::ctx(&mut ts),
        ...
        {
            let c: Coin<HANEUL> = ts::take_from_address_by_id(&ts, ALICE, i2);
            ts::return_to_address(ALICE, c);
        };

        // Bob gets the object from Alice
        {
            let c: Coin<HANEUL> = ts::take_from_address_by_id(&ts, BOB, i1);
    ...
    fun test_object_tamper() {
        ...
        // hide their tampering.
        {
            ...
            let l: Locked<Coin<HANEUL>> = ts::take_from_sender(&ts);
    ...
    fun test_return_to_sender() {
        ...
        // ...but has a change of heart and takes it back
        {
            ts::next_tx(&mut ts, ALICE);
            let escrow = ts::take_shared(&ts);
            let c = return_to_sender<Coin<HANEUL>>(escrow, ts::ctx(&mut ts));
        ...
        {
            let c: Coin<HANEUL> = ts::take_from_address_by_id(&ts, ALICE, cid);
            ts::return_to_address(ALICE, c)
        };
