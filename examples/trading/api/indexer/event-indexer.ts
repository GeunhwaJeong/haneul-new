...
import { EventId, HaneulClient, HaneulEvent, HaneulEventFilter } from '@haneullabs/haneul.js/client';

import { CONFIG } from '../config';
import { prisma } from '../db';
import { getClient } from '../haneul-utils';
import { handleEscrowObjects } from './escrow-handler';
import { handleLockObjects } from './locked-handler';

type HaneulEventsCursor = EventId | null | undefined;

type EventExecutionResult = {
	cursor: HaneulEventsCursor;
	hasNextPage: boolean;
};
...
type EventTracker = {
	...
	callback: (events: HaneulEvent[], type: string) => any;
};
...
const runEventJob = async (client: HaneulClient, tracker: EventTracker, cursor: HaneulEventsCursor) => {
