...
export const HANEUL_BIN = `haneul`;

export const getActiveAddress = () => {
	return execSync(`${HANEUL_BIN} client active-address`, { encoding: 'utf8' }).trim();
};

/** Returns a signer based on the active address of system's haneul. */
...
export const getClient = (network: Network) => {
	return new HaneulClient({ url: getFullnodeUrl(network) });
};
