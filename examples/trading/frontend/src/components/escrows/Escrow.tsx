...
import { HaneulObjectDisplay } from "@/components/HaneulObjectDisplay";
...
export function Escrow({ escrow }: { escrow: ApiEscrowObject }) {
  ...
  const haneulObject = useHaneulClientQuery("getObject", {
    id: escrow?.itemId,
    options: {
      showDisplay: true,
      showType: true,
  ...
  const { data: haneulLockedObject } = useGetLockedObject({
      ...
      <div className="p-4 flex gap-3 flex-wrap">
            ...
            <Button
              ...
              onClick={() =>
                cancelEscrowMutation({
                  escrow,
                  haneulObject: haneulObject.data?.data!,
                })
              }
            >
              <Cross1Icon />
        ...
        {isToggled && lockedData.data && (
          <div className="min-w-[340px] w-full justify-self-start text-left">
            {haneulLockedObject?.data && (
              <LockedObject
                object={haneulLockedObject.data}
