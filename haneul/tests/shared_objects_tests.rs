...
async fn submit_shared_object_transaction(
    ...
    loop {
        ...
        let replies: Vec<_> = futures::future::join_all(futures)
            .await
            .into_iter()
            // Remove all `ConsensusConnectionBroken` replies. Note that the original Haneul error type
            // `HaneulError::ConsensusConnectionBroken(..)` is lost when the message is sent through the
            // network (it is replaced by `RpcError`). As a result, the following filter doesn't work:
            // `.filter(|result| !matches!(result, Err(HaneulError::ConsensusConnectionBroken(..))))`.
...
/// End-to-end shared transaction test for a Haneul validator. It does not test the client, wallet,
/// or gateway but tests the end-to-end flow from Haneul to consensus.
#[tokio::test]
...
async fn shared_object_sync() {
    ...
    for reply in replies {
        match reply {
            ...
            // Err(HaneulError::SharedObjectLockingFailure(_)) => (),
            Err(_) => (),
...
#[tokio::test]
//#[ignore] // cargo test gateway -p haneul --test shared_objects_tests -- --nocapture
