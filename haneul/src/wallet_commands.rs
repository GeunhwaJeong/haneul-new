...
use haneul_core::haneul_json::{resolve_move_function_args, HaneulJsonCallArg, HaneulJsonValue};
...
async fn call_move(
    ...
    for json_arg in json_args {
        if let HaneulJsonCallArg::Object(id) = json_arg {
    ...
    if std::env::var("SHARED").is_err() {
        fp_ensure!(
            ...
            HaneulError::UnsupportedSharedObjectError.into()
        );
    }
    let (cert, effects) = context
        .gateway
