...
use haneul_core::client::{Client, ClientAddressManager};
...
use crate::keystore::{Keystore, HaneulKeystoreSigner};
use anyhow::anyhow;
...
impl WalletContext {
    ...
    pub fn create_account_state(&mut self, owner: &HaneulAddress) -> HaneulResult {
        let signer = Box::pin(HaneulKeystoreSigner::new(self.keystore.clone(), *owner));
