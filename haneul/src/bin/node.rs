...
async fn main() -> anyhow::Result<()> {
    ...
    let config_path = options
        .config
        .unwrap_or(haneul_config_dir()?.join("node.conf"));
    ...
    module.merge(HaneulNode::new(&config_path)?.into_rpc())?;
    ...
    info!("Haneul RPC Gateway listening on local_addr:{}", addr);
