...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let network_config = match (network_config_path.exists(), cfg.force_genesis) {
        ...
        _ => {
            ...
            let adddress = HaneulAddress::from(genesis_conf.key_pair.public_key_bytes());
    ...
    let address = HaneulAddress::from(public_key_bytes);
    // Find the network config for this validator
    let authority = network_config
        .authorities
        .iter()
        .find(|x| HaneulAddress::from(&x.public_key) == address)
        .ok_or_else(|| {
            anyhow!(
