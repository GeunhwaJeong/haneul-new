...
use haneul::shell::{install_shell_plugins, AsyncHandler, CommandStructure, Shell};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    if !options.no_shell {
        ...
        println!("{}", HANEUL.cyan().bold());
        print!("--- ");
        app.write_long_version(&mut io::stdout())?;
        println!(" ---");
        println!("{}", context.config);
        println!();
        println!("Welcome to the Haneul interactive shell.");
        println!();
