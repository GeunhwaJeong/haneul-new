...
fn resolve_object_arg(idx: usize, arg: &HaneulJsonValue) -> Result<ObjectID, anyhow::Error> {
...
fn resolve_call_arg(
    ...
    arg: &HaneulJsonValue,
    param: &SignatureToken,
) -> Result<HaneulJsonCallArg, anyhow::Error> {
    Ok(match param {
        SignatureToken::Bool
        | SignatureToken::U8
        | SignatureToken::U64
        ...
        | SignatureToken::Vector(_) => HaneulJsonCallArg::Pure(resolve_primtive_arg(arg, param)?),

        SignatureToken::Struct(_)
        | SignatureToken::StructInstantiation(_, _)
        | SignatureToken::Reference(_)
        | SignatureToken::MutableReference(_) => {
            HaneulJsonCallArg::Object(resolve_object_arg(idx, arg)?)
        }
...
fn resolve_call_args(
    json_args: &[HaneulJsonValue],
    parameter_types: &[SignatureToken],
) -> Result<Vec<HaneulJsonCallArg>, anyhow::Error> {
    json_args
        .iter()
        .zip(parameter_types)
        .enumerate()
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    combined_args_json: Vec<HaneulJsonValue>,
) -> Result<Vec<HaneulJsonCallArg>, anyhow::Error> {
    ...
    if !function_signature.type_parameters.is_empty() {
        bail!(
            "{}::{} has type arguments, which are not yet supported in haneul_json",
            module.self_id(),
            function,
        )
    }
