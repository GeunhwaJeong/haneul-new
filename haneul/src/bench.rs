...
use haneul_core::{authority::*, authority_server::AuthorityServer};
use haneul_network::{network::NetworkClient, transport};
use haneul_types::crypto::{get_key_pair, AuthoritySignature, Signature};
...
#[structopt(
    name = "Haneul Benchmark",
    about = "Local end-to-end test and benchmark of the Haneul protocol"
)]
struct ClientServerBenchmark {
    ...
    port: u16,
    /// Size of the Haneul committee. Minimum size is 4 to tolerate one fault
...
impl ClientServerBenchmark {
    fn make_structures(&self) -> (AuthorityState, Vec<Bytes>) {
        ...
        let mut next_recipient: HaneulAddress = get_key_pair().0;
