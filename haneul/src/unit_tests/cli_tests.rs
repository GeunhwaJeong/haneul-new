...
use haneul::haneul_commands::{genesis, HaneulNetwork, HANEUL_NETWORK_CONFIG, HANEUL_WALLET_CONFIG};
...
async fn test_move_call_args_linter_command() -> Result<(), anyhow::Error> {
    ...
    let err_string = format!("{} ", resp.err().unwrap());
    assert!(err_string.contains("Expected argument of type 0x2::ObjectBasics::Object, but found type 0x2::Coin::Coin<0x2::HANEUL::HANEUL>"));
