...
use haneul_types::base_types::{encode_address_hex, get_key_pair};
use tokio::task;
...
async fn test_genesis() -> Result<(), anyhow::Error> {
    ...
    // Genesis
    HaneulCommand::Genesis.execute(&mut config).await?;
    ...
    let result = HaneulCommand::Genesis.execute(&mut config).await;
