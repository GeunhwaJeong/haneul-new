...
use serde_json::json;
use haneul_core::gateway_state::gateway_responses::PublishResponse;
use haneul_types::gas_coin::GasCoin;
use tokio::task::{self, JoinHandle};
use tracing::{error, info};

use haneul::config::{GenesisConfig, NetworkConfig, PersistedConfig};
use haneul::gateway::{EmbeddedGatewayConfig, GatewayType};
...
fn create_api() -> ApiDescription<ServerContext> {
    ...
    api.register(haneul_start).unwrap();
    api.register(haneul_stop).unwrap();
...
fn server_state_error() -> HttpError {
    custom_http_error(
        ...
        "Server state does not exist. Please make a POST request to `haneul/genesis/` and `haneul/start/` to bootstrap the network."
            .to_string(),
    )
}
...
async fn genesis(
    ...
    if context.server_state.lock().await.is_some() {
        return Err(custom_http_error(
            StatusCode::CONFLICT,
            String::from("Cannot run genesis on a existing network, please make a POST request to the `haneul/stop` endpoint to reset."),
        ));
    }
    ...
    let (network_config, accounts, mut keystore) =
        haneul_commands::genesis(genesis_conf).await.map_err(|err| {
            custom_http_error(StatusCode::BAD_REQUEST, format!("Genesis error: {:?}", err))
        })?;

    if genesis_params.custom {
        keystore = HaneulKeystore::load_or_create(&PathBuf::from("wallet.key")).map_err(|err| {
            custom_http_error(
                StatusCode::BAD_REQUEST,
...
async fn haneul_start(ctx: Arc<RequestContext<ServerContext>>) -> Result<Response<Body>, HttpError> {
    ...
    if !state.authority_handles.is_empty() {
        return Err(custom_http_error(
            ...
            String::from("Haneul network is already running."),
        ));
    }
    ...
    for authority in &state.config.authorities {
        let server = haneul_commands::make_server(authority, &committee, state.config.buffer_size)
            .await
            .map_err(|error| {
                custom_http_error(
                    StatusCode::CONFLICT,
...
/**
Stop haneul network and delete generated configs & storage.
...
/**
JSON representation of an object in the Haneul network.
 */
#[derive(Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
struct Object {
...
async fn get_object_info(
    state: &ServerState,
    object_id: ObjectID,
) -> Result<(ObjectRef, HaneulObject, Option<MoveStructLayout>), HttpError> {
...
async fn handle_publish(
    ...
    let sender: HaneulAddress = decode_bytes_hex(publish_params.sender.as_str())?;
...
async fn handle_move_call(
    ...
    let sender: HaneulAddress = decode_bytes_hex(call_params.sender.as_str())?;
