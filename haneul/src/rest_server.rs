...
async fn genesis(
    ...
    *server_context.client_db_path.lock().unwrap() = client_db_path;
    ...
    haneul_commands::genesis(&mut network_config, genesis_conf, &mut wallet_config)
        .await
        .map_err(|err| {
            custom_http_error(
...
async fn haneul_start(
    ...
    if network_config.authorities.is_empty() {
        return Err(custom_http_error(
            StatusCode::CONFLICT,
            String::from("No authority configured for the network, please make a POST request to the `haneul/genesis` endpoint."),
        ));
    }
    ...
    for authority in &network_config.authorities {
        let server = haneul_commands::make_server(
            authority,
            &committee,
            vec![],
            &[],
...
async fn get_addresses(
    ...
    let addresses: Vec<HaneulAddress> = wallet_context.config.accounts.clone();
...
async fn get_objects(
    ...
    let wallet_context = wallet_context.as_mut().ok_or_else(|| {
        custom_http_error(
            StatusCode::FAILED_DEPENDENCY,
            "Wallet Context does not exist. Please make a POST request to `haneul/genesis/` and `haneul/start/` to bootstrap the network."
                .to_string(),
        )
    })?;
