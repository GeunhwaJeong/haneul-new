...
public fun issue(bank: &mut Bank, amount: u64): (Balance<HANEUL>, Loan) {
    assert!(bank.holdings.value() >= amount);
    let loaned = bank.holdings.split(amount);
    (loaned, Loan { amount })
}

public fun repay(bank: &mut Bank, loan: Loan, repayment: Balance<HANEUL>) {
...
- This means a hot value’s type can be in one of the following cases:
  ...
  - `copy`
  - `key`
  - Note that a value cannot have both `key` and `copy` since `haneul::object::UID` does not have `copy`
