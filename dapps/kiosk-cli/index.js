...
const KNOWN_TYPES = {
  haneulfren:
    '0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::haneulfrens::HaneulFren<0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::capy::Capy>',
};

/** JsonRpcProvider for the Testnet */
const provider = new HaneulClient({ url: getFullnodeUrl('testnet') });
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of HANEUL')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount GEUNHWA>', 'The amount of HANEUL to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toHaneulAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toHaneulAddress();

  if (!isValidHaneulAddress(owner)) {
    throw new Error(`Invalid HANEUL address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidHaneulObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toHaneulAddress();

    if (!isValidHaneulAddress(sender)) {
      throw new Error(`Invalid HANEUL address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (HANEUL)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toHaneulAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidHaneulObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const kioskCap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toHaneulAddress();
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of HANEUL
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (target && target !== 'kiosk' && !isValidHaneulAddress(target)) {
    throw new Error(
  ...
  if (inputKioskId && !isValidHaneulObjectId(inputKioskId)) {
  ...
  if (target === 'kiosk') {
    ...
    const receiver = target || keypair.getPublicKey().toHaneulAddress();
...
/**
 * Formats the GEUNHWA into HANEUL.
 */
function formatAmount(amount) {
  ...
  if (amount <= GEUNHWA_PER_HANEUL) {
    return Number(amount) / Number(GEUNHWA_PER_HANEUL);
  }
