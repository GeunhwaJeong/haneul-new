...
const getCallArgDisplay = (argument: HaneulCallArg | undefined) => {
...
const getHaneulArgumentDisplay = (argument: HaneulArgument, inputs: HaneulCallArg[]) => {
	if (typeof argument === 'string') return argument;
...
const renderArguments = (callArgs: HaneulArgument[], inputs: HaneulCallArg[]) => {
	return (
		<div className="flex overflow-x-auto gap-3 my-3">
			{callArgs.map((arg, index) => (
				<div key={index} className="flex-shrink-0">
					{getHaneulArgumentDisplay(arg, inputs)}
				</div>
			))}
		</div>
	);
