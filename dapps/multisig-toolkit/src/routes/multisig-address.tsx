...
export default function MultiSigAddressGenerator() {
	...
	const { register, control, handleSubmit } = useForm({
		defaultValues: {
			pubKeys: [{ pubKey: 'Haneul Pubkey', weight: '' }],
			threshold: 1,
		},
	});
	...
	const onSubmit = (data: FieldValues) => {
		...
		console.log('pks:', pks);
		const multisigHaneulAddress = toMultiSigAddress(pks, data.threshold);
		console.log('multisigHaneulAddress', multisigHaneulAddress);
	...
	return (
		<div className="flex flex-col gap-4">
			...
			<form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
				<p>The following demo allow you to create Haneul MultiSig addresses.</p>
				<code>
					Haneul Pubkeys for playing with
				...
				<section>
					<Button
						type="button"
						onClick={() => {
							append({ pubKey: 'Haneul Pubkey', weight: '' });
						}}
					>
						New PubKey
					</Button>
...
/* 
➜  multisig-toolkit git:(jnaulty/multisig-create-address) ✗ haneul keytool multi-sig-address --pks ABr818VXt+6PLPRoA7QnsHBfRpKJdWZPjt7ppiTl6Fkq ANRdB4M6Hj73R+gRM4N6zUPNidLuatB9uccOzHBc/0bP --weights 1 2 --threshold 2
...
➜  multisig-toolkit git:(jnaulty/multisig-create-address) ✗ haneul keytool multi-sig-address --pks ABr818VXt+6PLPRoA7QnsHBfRpKJdWZPjt7ppiTl6Fkq ANRdB4M6Hj73R+gRM4N6zUPNidLuatB9uccOzHBc/0bP --weights 1 1 --threshold 2
...
➜  multisig-toolkit git:(jnaulty/multisig-create-address) ✗ haneul keytool multi-sig-address --pks ABr818VXt+6PLPRoA7QnsHBfRpKJdWZPjt7ppiTl6Fkq ANRdB4M6Hj73R+gRM4N6zUPNidLuatB9uccOzHBc/0bP --weights 1 1 --threshold 1
