...
import { StakedHaneul } from "./../types";
 ...
 */
const STAKED_HANEUL = "staking_pool::StakedHaneul";
...
export function useMyStake(account: string | null) {
  return useQuery(
    ...
    async () => {
      ...
      const search = objects.filter((v) => v.type.includes(STAKED_HANEUL));
      ...
      return Promise.all(
        search.map((obj) => getRawObjectParsedUnsafe<StakedHaneul>(provider, obj.objectId, "staking_pool::StakedHaneul"))
      );
    }
  );
}
