...
import { MoveActiveValidator } from "@haneullabs/haneul.js";
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="space-y-0.5">
          ...
          <div className="text-frenemies text-body font-medium">
            {formatBalance(totalStake, DEC)} HANEUL staked
          </div>
        </div>
      </div>
      <div>
          ...
          <div className="relative flex items-center">
            {stake ? (
              <div className="block w-full pr-28 bg-white rounded-lg py-2 pl-3 border-steel-darker/30 border font-bold">
                {formatBalance(stake?.data.staked.toString() || "0", DEC)} HANEUL
              </div>
            ) : (
              <input
                type="number"
            ...
            {delegation ? (
              ...
              <AddDelegation validator={metadata.haneul_address} amount={amount} />
            )}
          </div>
        </div>
      </div>

      {metadata.haneul_address.replace("0x", "") ===
