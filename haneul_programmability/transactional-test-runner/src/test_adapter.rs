...
pub struct HaneulTestAdapter<'a> {
    ...
    accounts: BTreeMap<String, (HaneulAddress, KeyPair)>,
    default_syntax: SyntaxChoice,
    object_enumeration: BiBTreeMap<ObjectID, [u8; HANEUL_ADDRESS_LENGTH]>,
    next_object: usize,
}
...
impl<'a> MoveTestAdapter<'a> for HaneulTestAdapter<'a> {
    ...
    fn init(
        ...
        let (additional_mapping, account_names) = match task_opt.map(|t| t.command) {
            Some((InitCommand { named_addresses }, HaneulInitArgs { accounts })) => {
    ...
    fn publish_module(
        ...
        let HaneulPublishArgs { sender } = extra;
    ...
    fn call_function(
        ...
        assert!(empty_args.is_empty(), "Use '--{}' instead", HANEUL_ARGS_LONG);
        let HaneulRunArgs { args, sender } = extra;
        let arguments = args
            .into_iter()
...
impl<'a> HaneulTestAdapter<'a> {
    ...
    pub(crate) fn fake_to_real_object_id(&self, id: [u8; HANEUL_ADDRESS_LENGTH]) -> Option<ObjectID> {
        self.object_enumeration.get_by_right(&id).copied()
    }

    pub(crate) fn real_to_fake_object_id(&self, id: &ObjectID) -> Option<[u8; HANEUL_ADDRESS_LENGTH]> {
        self.object_enumeration.get_by_left(id).copied()
    }

    fn enumerate_object(&mut self, id: ObjectID) -> [u8; HANEUL_ADDRESS_LENGTH] {
        const USIZE_LENGTH: usize = std::mem::size_of::<usize>();
        let mut fake_id = [0; HANEUL_ADDRESS_LENGTH];
        let next_id_bytes: [u8; USIZE_LENGTH] = self.next_object.to_be_bytes();
        fake_id[(HANEUL_ADDRESS_LENGTH - USIZE_LENGTH)..].clone_from_slice(&next_id_bytes);
    ...
    fn render_haneul_error(&self, haneul_error: HaneulError) -> anyhow::Error {
        const HEX_STR_LENGTH: usize = HANEUL_ADDRESS_LENGTH * 2;
        let error_string: String = format!("{}", haneul_error);
    ...
    fn remap_hex_str(&self, mut hex_str: String) -> String {
        ...
        let bytes: [u8; HANEUL_ADDRESS_LENGTH] = hex::decode(&hex_str).unwrap().try_into().unwrap();
...
pub(crate) static PRE_COMPILED: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    let fully_compiled_res = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: None,
            paths: vec![haneul_sources, haneul_deps],
