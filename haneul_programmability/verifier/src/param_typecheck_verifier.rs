...
/// functions (functions called directly from Haneul) have correct types.
///
...
pub fn check_params(module: &CompiledModule) -> HaneulResult {
    ...
    for func_def in module.function_defs.iter() {
        ...
        if func_def.visibility != Visibility::Public {
            // a non-public function cannot be called from Haneul
            continue;
        }
    ...
    for p in &params.0[0..params.len() - 1] {
        if is_primitive_type(p) {
                ...
                && !is_template_vector(p)
            {
                ...
                // we cannot pass non-object types from Haneul).
                return None;
            }
        }
    }
        ...
        SignatureToken::MutableReference(m) => match &**m {
            SignatureToken::Struct(idx) => {
                ...
                module_name == TX_CONTEXT_MODULE_NAME
                    && module_addr == &HANEUL_FRAMEWORK_ADDRESS
