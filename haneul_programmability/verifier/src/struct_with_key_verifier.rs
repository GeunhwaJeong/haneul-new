...
use haneul_types::{error::HaneulResult, fp_ensure, HANEUL_FRAMEWORK_ADDRESS};

pub fn verify_module(module: &CompiledModule) -> HaneulResult {
    verify_key_structs(module)
}

fn verify_key_structs(module: &CompiledModule) -> HaneulResult {
    ...
    for def in struct_defs {
        ...
        // A struct with key ability represents a haneul object.
        // We want to ensure that haneul objects cannot be arbitrarily dropped.
        ...
        // Chech that the struct type for "id" field must be HANEUL_FRAMEWORK_ADDRESS::ID::ID.
        ...
        fp_ensure!(
            id_type_struct_name == "VersionedID"
                && id_type_module_address == &HANEUL_FRAMEWORK_ADDRESS
