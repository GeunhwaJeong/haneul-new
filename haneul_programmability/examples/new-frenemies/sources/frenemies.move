...
module frenemies::frenemies {
    ...
    use haneul::haneul_system::HaneulSystemState;
    ...
    public entry fun register(
        name: String, registry: &mut Registry, old_registry: &mut old_frenemies::registry::Registry, state: &HaneulSystemState, ctx: &mut TxContext) {
    ...
    fun name_to_string(name: &old_frenemies::registry::Name): String {
        let name_bcs = haneul::bcs::new(bcs::to_bytes(name));
        let old_name = haneul::bcs::peel_vec_u8(&mut name_bcs);
    ...
    public entry fun update(
        scorecard: &mut Scorecard, state: &HaneulSystemState, leaderboard: &mut Leaderboard, ctx: &TxContext
    ) {
