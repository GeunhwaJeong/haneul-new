...
module frenemies::leaderboard {
    ...
    public(friend) fun update_epoch_info(self: &mut Leaderboard, state: &HaneulSystemState, cur_epoch: u64) {
        let validators = vector[];
        let active_validators = validator_set::active_validators(haneul_system::validators(state));
        let i = 0;
        ...
        while (i < len) {
            ...
            validator_insertion_sort(
                &mut validators,
                Validator {
                    addr: validator::haneul_address(validator_data),
    ...
    public fun next_epoch_stakes(state: &HaneulSystemState): vector<Validator> {
        let validators = validator_set::active_validators(haneul_system::validators(state));
        let next_epoch_stakes = vector[];
        let i = 0;
        let num_validators = vector::length(validators);
        while (i < num_validators) {
            let validator = vector::borrow(validators, i);
            let addr = validator::haneul_address(validator);
