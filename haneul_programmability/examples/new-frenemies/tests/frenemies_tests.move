...
module frenemies::frenemies_tests {
    ...
    public fun init_test(validators: vector<address>, players: vector<address>): Scenario {
        ...
        gtu::set_up_haneul_system_state(validators, &mut scenario);
        ...
        while (i < vector::length(&players)) {
            ...
            {
                let system_state = ts::take_shared<HaneulSystemState>(&mut scenario);
    ...
    fun double_register_ok() {
        ...
        {
            let system_state = ts::take_shared<HaneulSystemState>(scenario);
