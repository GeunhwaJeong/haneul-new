...
module defi::pool {
    ...
    entry fun create_pool_<P: drop, T>(
        ...
        transfer::transfer(
            create_pool(witness, token, haneul, /* share, */ fee_percent, ctx),
    ...
    public fun create_pool<P: drop, T>(
        ...
        let haneul_amt = coin::value(&haneul);
    ...
    public fun swap_haneul<P, T>(
        ...
        assert!(coin::value(&haneul) > 0, EZeroAmount);

        let haneul_balance = coin::into_balance(haneul);
        ...
        let output_amount = get_input_price(
            balance::value(&haneul_balance),
            haneul_reserve,
            token_reserve,
            pool.fee_percent
        );

        balance::join(&mut pool.haneul, haneul_balance);
    ...
    public fun add_liquidity<P, T>(
        ...
        let haneul_added = balance::value(&haneul_balance);
        ...
        let haneul_amt = balance::join(&mut pool.haneul, haneul_balance);
...
module defi::pool_tests {
    ...
    #[test] fun test_swap_haneul() { test_swap_haneul_(&mut scenario()) }
