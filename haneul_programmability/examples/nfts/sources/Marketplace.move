...
module NFTs::MarketplaceTests {
    ...
    fun list_kitty(scenario: &mut Scenario) {
        ...
        Marketplace::list<NFT<KITTY>, HANEUL>(&mkp, &mut bag, nft, 100, TestScenario::ctx(scenario));
    ...
    fun list_and_delist() {
        ...
        TestScenario::next_tx(scenario, &SELLER);
        {
            let mkp = TestScenario::remove_object<Marketplace>(scenario);
            let bag = TestScenario::remove_nested_object<Marketplace, Bag>(scenario, &mkp);
            let listing = TestScenario::remove_nested_object<Bag, Listing<NFT<KITTY>, HANEUL>>(scenario, &bag);

            // Do the delist operation on a Marketplace.
            let nft = Marketplace::delist<NFT<KITTY>, HANEUL>(&mkp, &mut bag, listing, TestScenario::ctx(scenario));
    ...
    fun buy_kitty() {
        ...
        TestScenario::next_tx(scenario, &BUYER);
        {
            ...
            let nft = Marketplace::buy<NFT<KITTY>, HANEUL>(&mut bag, listing, payment);
            let kitten = NFT::burn<KITTY>(nft);
