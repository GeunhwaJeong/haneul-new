...
///
/// Makes use of `haneul::dynamic_object_field` module by attaching `Listing`
...
module nfts::marketplaceTests {
    ...
    fun create_marketplace(scenario: &mut Scenario) {
        ...
        marketplace::create<HANEUL>(test_scenario::ctx(scenario));
    }
    ...
    fun mint_some_coin(scenario: &mut Scenario) {
        test_scenario::next_tx(scenario, ADMIN);
        let coin = coin::mint_for_testing<HANEUL>(1000, test_scenario::ctx(scenario));
    ...
    //     marketplace::list<Kitty, HANEUL>(mkp, &mut bag, nft, 100, test_scenario::ctx(scenario));
    ...
    //         let mkp = &mut mkp_val;
    //         let bag = test_scenario::take_child_object<Marketplace, Bag>(scenario, mkp);
    //         let listing = test_scenario::take_child_object<Bag, bag::Item<Listing<Kitty, HANEUL>>>(scenario, &bag);

    //         // Do the delist operation on a Marketplace.
    //         let nft = marketplace::delist<Kitty, HANEUL>(mkp, &mut bag, listing, test_scenario::ctx(scenario));
    ...
    //         let nft = marketplace::buy<Kitty, HANEUL>(&mut bag, listing, payment);
    //         let kitty_id = burn_kitty(nft);
