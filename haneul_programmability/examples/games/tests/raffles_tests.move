...
module games::raffles_tests {
    ...
    fun mint(addr: address, amount: u64, scenario: &mut Scenario) {
        transfer::public_transfer(coin::mint_for_testing<HANEUL>(amount, test_scenario::ctx(scenario)), addr);
    ...
    fun test_game_with_tickets() {
        ...
        assert!(raffle_with_tickets::get_cost_in_haneul(&game) == 10, 1);
        ...
        let coin = test_scenario::take_from_sender<Coin<HANEUL>>(scenario);
    ...
    fun test_small_raffle() {
        ...
        assert!(small_raffle::get_cost_in_haneul(&game) == 10, 1);
