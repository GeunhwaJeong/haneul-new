...
/// Basic raffles games that depends on Haneul randomness.
///
...
module games::raffle_with_tickets {
    ...
    public fun create(end_time: u64, cost_in_haneul: u64, ctx: &mut TxContext) {
        let game = Game {
            id: object::new(ctx),
            cost_in_haneul,
            participants: 0,
    ...
    public fun buy_ticket(game: &mut Game, coin: Coin<HANEUL>, clock: &Clock, ctx: &mut TxContext): Ticket {
        assert!(game.end_time > clock::timestamp_ms(clock), EGameAlreadyCompleted);
        assert!(coin::value(&coin) == game.cost_in_haneul, EInvalidAmount);
    ...
    public fun redeem(ticket: Ticket, game: Game, ctx: &mut TxContext): Coin<HANEUL> {
        ...
        let Game { id, cost_in_haneul: _, participants: _, end_time: _, winner: _, balance } = game;
        object::delete(id);
    ...
    #[test_only]
    public fun get_cost_in_haneul(game: &Game): u64 {
        game.cost_in_haneul
    }
...
module games::small_raffle {
    ...
    entry fun close(game: Game, r: &Random, clock: &Clock, ctx: &mut TxContext) {
        assert!(game.end_time <= clock::timestamp_ms(clock), EGameInProgress);
        let Game { id, cost_in_haneul: _, participants, end_time: _, balance, mut participants_table } = game;
        if (participants > 0) {
            let mut generator = new_generator(r, ctx);
    ...
    /// Anyone can play.
    public fun play(game: &mut Game, coin: Coin<HANEUL>, clock: &Clock, ctx: &mut TxContext) {
        assert!(game.end_time > clock::timestamp_ms(clock), EGameAlreadyCompleted);
        assert!(coin::value(&coin) == game.cost_in_haneul, EInvalidAmount);
