...
module abc::tests {
    ...
    use haneul::coin::{Coin, TreasuryCap};
    use haneul::test_scenario::{Self, Scenario, next_tx, ctx};
    ...
    fun test_take_(test: &mut Scenario) {
        ...
        next_tx(test, &user1); {
            ...
            haneul::transfer::transfer(coin, user2);
        };
    }
    ...
    fun test_put_back_(test: &mut Scenario) {
        ...
        next_tx(test, &user2); {
            let coin = test_scenario::take_owned<Coin<ABC>>(test);
            haneul::transfer::transfer(coin, admin);
        };
