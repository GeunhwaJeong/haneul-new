...
module capy::capy_item {
    ...
    public entry fun collect_profits(
        ...
        haneul::transfer::transfer(coin::from_balance(b, ctx), sender(ctx))
    }
    ...
    /// Buy an Item from the `ItemStore`. Pay `Coin<HANEUL>` and
    /// receive a `CapyItem`.
    public entry fun buy_and_take(
        s: &mut ItemStore, name: vector<u8>, payment: Coin<HANEUL>, ctx: &mut TxContext
    ) {
        let listing_mut = dof::borrow_mut<vector<u8>, ListedItem>(&mut s.id, name);
    ...
    public entry fun buy_mut(
        s: &mut ItemStore, name: vector<u8>, payment: &mut Coin<HANEUL>, ctx: &mut TxContext
    ) {
    ...
    public entry fun buy_mul_coin(
        s: &mut ItemStore, name: vector<u8>, coins: vector<Coin<HANEUL>>, ctx: &mut TxContext
    ) {
