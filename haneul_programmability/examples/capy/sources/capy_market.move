...
module capy::capy_market {
    ...
    entry fun take_profits<T: key + store>(
        ...
        let profit = dof::remove<address, Coin<HANEUL>>(&mut market.id, sender);
    ...
    public fun purchase<T: key + store>(
        ...
        if (dof::exists_(&market.id, owner)) {
            coin::join(dof::borrow_mut<address, Coin<HANEUL>>(&mut market.id, owner), paid)
        } else {
    ...
    /// Use `&mut Coin<HANEUL>` to purchase `T` from marketplace.
    entry fun purchase_and_take_mut<T: key + store>(
        market: &mut CapyMarket<T>,
        listing_id: ID,
        paid: &mut Coin<HANEUL>,
        ctx: &mut TxContext
    ) {
