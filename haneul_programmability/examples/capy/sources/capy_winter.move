...
module capy::capy_winter {
    ...
    public struct CapyPost has key { id: UID, balance: Balance<HANEUL> }
    ...
    entry fun buy_gift(post: &mut CapyPost, `type`: u8, payment: vector<Coin<HANEUL>>, ctx: &mut TxContext) {
    ...
    entry fun buy_premium(
        post: &mut CapyPost, ticket: PremiumTicket, payment: vector<Coin<HANEUL>>, ctx: &mut TxContext
    ) {
    ...
    fun merge_and_split(
        mut coins: vector<Coin<HANEUL>>, amount: u64, ctx: &mut TxContext
    ): (Coin<HANEUL>, Coin<HANEUL>) {
    ...
    fun get_link_url(id: &UID, `type`: u8): Url {
        ...
        res.append(haneul::hex::encode(object::uid_to_bytes(id)));
        res.append(b"?type=");
        res.push_back(ASCII_OFFSET + `type`);
