...
impl ResourceResolver for InMemoryStorage {
    ...
    fn get_resource(
        ...
        unreachable!("Should never be called in Haneul")
    }
}
...
fn test_object_basics() {
    ...
    let gas_object =
        Object::with_id_owner_for_testing(ObjectID::random(), base_types::HaneulAddress::default());
...
#[test]
fn test_wrap_unwrap() {
    let addr = base_types::HaneulAddress::default();
...
fn test_move_call_insufficient_gas() {
    ...
    let gas_object =
        Object::with_id_owner_for_testing(gas_object_id, base_types::HaneulAddress::default());
    storage.write_object(gas_object.clone());
    storage.flush();
    ...
    let err = response.unwrap().unwrap_err();
    assert!(matches!(err.1, HaneulError::InsufficientGas { .. }));
    // Provided gas_budget will be deducted as gas.
    assert_eq!(err.0, 50);
}
...
fn test_publish_module_insufficient_gas() {
    ...
    let gas_object = Object::with_id_owner_gas_for_testing(
        ...
        base_types::HaneulAddress::default(),
        30,
    );
    storage.write_object(gas_object.clone());
    storage.flush();
...
fn test_move_call_args_type_mismatch() {
    ...
    /*
    // Need to fix https://github.com/GeunhwaJeong/fastnft/issues/211
...
fn test_coin_transfer() {
    ...
    let addr1 = haneul_types::crypto::get_key_pair().0;
...
fn publish_from_src(
    ...
    let modules = haneul_framework::build_move_package(&module_path, build_config, false).unwrap();
