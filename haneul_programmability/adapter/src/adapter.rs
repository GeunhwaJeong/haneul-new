...
/// The return value indicates whether a system error has occured (i.e. issues with the haneul system, not with user transaction).
...
pub fn publish<E: Debug, S: ResourceResolver<Error = E> + ModuleResolver<Error = E> + Storage>(
    ...
    let gas_used_for_init =
        match store_package_and_init_modules(state_view, &vm, modules, sender, ctx, gas_budget) {
            Ok(ok) => ok,
            Err(err) => exec_failure!(gas::MIN_MOVE, err),
        };
    ...
    for e in events {
            ...
            .expect("Safe because event_type is derived from an EventType enum")
        {
            EventType::TransferToAddress => handle_transfer(
                HaneulAddress::try_from(recipient.as_slice()).unwrap(),
                type_,
                event_bytes,
                false, /* should_freeze */
                tx_digest,
