...
impl HaneulMoveVM {
    ...
    pub fn new(vm: MoveVM) -> HaneulMoveVM {
        HaneulMoveVM {
            movevm: vm,
...
pub fn new_move_vm(natives: NativeFunctionTable) -> Result<Arc<HaneulMoveVM>, HaneulError> {
    Ok(Arc::new(HaneulMoveVM {
        movevm: MoveVM::new(natives).map_err(|_| HaneulError::ExecutionInvariantViolation)?,
        package_cache: parking_lot::RwLock::new(HashMap::new()),
    }))
}
...
>(
    ...
    object_owner_map: HashMap<HaneulAddress, HaneulAddress>,
    ...
    match session.execute_function_for_effects(
        ...
        ExecutionResult::Fail { error, gas_used: _ } => Err(HaneulError::AbortedExecution {
...
>(
    ...
    ctx: &TxContext,
    mut object_owner_map: HashMap<HaneulAddress, HaneulAddress>,
) -> HaneulResult {
    ...
    for e in events {
        ...
        match event_type {
            ...
            EventType::ShareObject => Err(HaneulError::UnsupportedSharedObjectError),
...
>(
    ...
    state_view: &mut S,
    object_owner_map: &mut HashMap<HaneulAddress, HaneulAddress>,
