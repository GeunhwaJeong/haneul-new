...
/// The return value indicates whether a system error has occurred (i.e. issues with the haneul system, not with user transaction).
...
>(
    ...
    // TODO: Update Move gas constants to reflect the gas fee on haneul.
                ...
                if !newly_generated_ids.contains(obj_id) {
                    if let Some(object) = by_value_objects.remove(id.object_id()) {
                        ...
                        if matches!(object.owner, Owner::ObjectOwner { .. }) {
                            ...
                            return (gas_used, 0, Err(HaneulError::DeleteObjectOwnedObject));
                        }
