...
module Haneul::ValidatorSet {
    use Std::Option::{Self, Option};
    use Std::Vector;

    use Haneul::Balance::{Self, Balance};
    use Haneul::EpochRewardRecord;
    ...
    #[test_only]
    friend Haneul::ValidatorSetTests;
    ...
    /// Called by `HaneulSystem`, to remove a validator.
    ...
    /// Called by `HaneulSystem`, to add more stake to a validator.
    ...
    /// Called by `HaneulSystem`, to withdraw stake from a validator.
    ...
    /// Two validators duplicate if they share the same haneul_address or same IP or same name.
    ...
    fun distribute_reward(validators: &mut vector<Validator>, total_stake: u64, reward: &mut Balance<HANEUL>) {
