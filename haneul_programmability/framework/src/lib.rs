...
pub fn build_move_package(
    ...
    match build_config.compile_package(path, &mut Vec::new()) {
        ...
        Ok(package) => {
            ...
            {
                return Err(HaneulError::ModulePublishFailure { error: format!("Denpendent modules must have been published on-chain with non-0 addresses, unlike module {:?}", m.self_id()) });
            }
            Ok(package
                .transitive_compiled_modules()
...
pub fn build_and_verify_user_package(path: &Path, dev_mode: bool) -> HaneulResult {
...
pub fn run_move_unit_tests(path: &Path) -> HaneulResult {
