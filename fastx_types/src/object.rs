...
impl MoveObject {
    ...
    pub fn get_layout(
    ...
    ) -> Result<MoveStructLayout, HaneulError> {
        ...
        .map_err(|_e| HaneulError::ObjectSerializationError)?;
        match layout {
    ...
    pub fn to_json(&self, layout: &MoveStructLayout) -> Result<Value, HaneulError> {
        let move_value = MoveStruct::simple_deserialize(&self.contents, layout)
            .map_err(|_e| HaneulError::ObjectSerializationError)?;
        serde_json::to_value(&move_value).map_err(|_e| HaneulError::ObjectSerializationError)
    }
}
...
impl Object {
    ...
    pub fn transfer(&mut self, new_owner: Authenticator) {
        // TODO: these should be raised HaneulError's instead of panic's
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: HaneulAddress) -> Self {
    ...
    pub fn to_json(
        ...
        let owner =
            serde_json::to_value(&self.owner).map_err(|_e| HaneulError::ObjectSerializationError)?;
...
impl ObjectRead {
    ...
    pub fn object(&self) -> Result<&Object, HaneulError> {
        match &self {
    ...
    /// the object does exist or is deleted.
    pub fn reference(&self) -> Result<ObjectRef, HaneulError> {
        match &self {
            Self::Deleted(oref) => Err(HaneulError::ObjectDeleted { object_ref: *oref }),
            Self::NotExists(id) => Err(HaneulError::ObjectNotFound { object_id: *id }),
