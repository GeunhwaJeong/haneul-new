...
impl PublicKeyBytes {
    ...
    pub fn to_public_key(&self) -> Result<PublicKey, HaneulError> {
        // TODO(https://github.com/GeunhwaJeong/fastnft/issues/101): Do better key validation
        // to ensure the bytes represent a point on the curve.
        PublicKey::from_bytes(self.as_ref()).map_err(|_| HaneulError::InvalidAuthenticator)
    }
...
// TODO(https://github.com/GeunhwaJeong/fastnft/issues/101): more robust key validation
...
// Refer details to https://github.com/GeunhwaJeong/fastnft/pull/292.
...
impl TxContext {
    pub fn new(sender: &HaneulAddress, digest: TransactionDigest) -> Self {
        Self {
...
impl TransactionDigest {
    ...
    ///
    /// TODO(https://github.com/GeunhwaJeong/fastnft/issues/65): we can pick anything here    
    pub fn genesis() -> Self {
        Self::new([0; 32])
    }
    ...
    pub fn derive_id(&self, creation_num: u64) -> ObjectID {
        // TODO(https://github.com/GeunhwaJeong/fastnft/issues/58):audit ID derivation
...
impl SequenceNumber {
    ...
    pub fn decrement(self) -> Result<SequenceNumber, HaneulError> {
        let val = self.0.checked_sub(1);
        match val {
            None => Err(HaneulError::SequenceUnderflow),
...
impl Signature {
    ...
    pub fn check<T>(&self, value: &T, author: HaneulAddress) -> Result<(), HaneulError>
    where
        T: Signable<Vec<u8>>,
    {
        let mut message = Vec::new();
    ...
    where
        T: Signable<Vec<u8>>,
        I: IntoIterator<Item = &'a (HaneulAddress, Signature)>,
    {
        let mut msg = Vec::new();
        value.write(&mut msg);
...
impl TryFrom<&[u8]> for TransactionDigest {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, HaneulError> {
        let arr: [u8; TRANSACTION_DIGEST_LENGTH] = bytes
            .try_into()
            .map_err(|_| HaneulError::InvalidTransactionDigest)?;
        Ok(Self(arr))
    }
}
