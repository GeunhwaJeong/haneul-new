...
* We are adding new pay transaction types for generic payments [[NFT standard proposal](https://github.com/GeunhwaJeong/haneul/pull/4887)]. See below for more details.
...
* Adding `PayHaneul` transaction type, which takes multiple coins and transfer them to a single recipient, while paying gas with the input coins. Deprecating `TransferHaneul` as `PayHaneul` is a generalized version of it [**Breaking Change**].
...
* Intent signing support which will include an intent struct to be serialized and signed in addition to the transaction data [[Issues](https://github.com/GeunhwaJeong/fastcrypto/issues/26)].
* [Rust SDK] Removing reliance on String and Json Values in Rust Transaction Builder, making it more Rust friendly
* Adding denomination conversion functionality, which converts HANEUL to GEUNHWA.
* [TypeScript] Adding support for deserializing transactions.

## Haneul Move

* Crypto.move will be split into its individual modules organized by crypto primitives [[PR](https://github.com/GeunhwaJeong/haneul/pull/4653)].
...
* New autogenerated docs for the Haneul Framework [[link](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-framework/docs)].
* Enable shared coins via Safe module [[PR](https://github.com/GeunhwaJeong/haneul/pull/4680)].
...
## Haneul CLI
* Intent signing support: Same as in SDK. Also, making the Haneul keystore ChainID aware as well as password protected.
* Source verification for dependent packages enforced at publish time.

## Faucet
* As a precursor to integrating Faucet into Haneul CLI/SDKs/Wallet, rate-limiting will be enforced on the faucet server to no more than 20 requests in 24 hours, originating from the same ip address.
