...
This directory contains the assets required to build and update the [Haneul documentation](https://docs.haneul.io). The directory is split between `content` and `site`.
...
The Haneul documentation build generates an `llms.txt` file at `https://docs.haneul.io/llms.txt`. Use it as your entry point for understanding the documentation structure and locating specific topics. 

### Style guide skill file

A condensed, machine-readable version of the Haneul Documentation Style Guide is available as a skill file at `HANEUL_STYLE_GUIDE_SKILL.md`. This file contains all style rules from the [full style guide](https://docs.haneul.io/references/contribute/style-guide) in a format optimized for agent ingestion. Use it when writing or revising any documentation for this repository.
...
│   ├── concepts/         # Haneul architecture, objects, transactions, cryptography
│   ├── guides/           # Developer guides, tutorials, app examples
│   ├── references/       # API references, framework docs, CLI reference, contribution guides
│   ├── standards/        # Haneul standards (Closed-Loop Token, DeepBook, Kiosk, Wallet)
...
- **`docs/HANEUL_STYLE_GUIDE_SKILL.md`**: Condensed style guide rules. Ingest before writing or editing documentation.
...
- **Framework reference** (`/references/framework`): Generated from `cargo-doc` Markdown in `/haneul/crates`.
- **GraphQL reference** (`/references/haneul-api/haneul-graphql`): Generated from the GraphQL schema.
...
- Code samples should be sourced from GitHub using the `<ImportContent>` component when possible, rather than copied inline. See the [MDX Components page](https://docs.haneul.io/references/contribute/mdx-components) for component reference.
...
To run `docs.haneul.io` locally, open the `site` directory in a terminal or console. Use a package manager to install the required modules:
...
**Framework reference** (`/references/framework`): Generated by the `framework` plugin, which imports `cargo-doc` Markdown from `/haneul/crates` and transforms it into documentation pages. The build script removes this directory before each build and regenerates it from source. See the [components documentation](https://docs.haneul.io/references/contribute/mdx-components) for details.

**GraphQL reference** (`/references/haneul-api/haneul-graphql`): Generated by the `graphql-to-doc` Docusaurus plugin, then cleaned by `remove-no-desc.mjs`. These pages are generated from the GraphQL schema and should not be manually edited.
...
All documentation contributions must follow the [Haneul Documentation Style Guide](https://docs.haneul.io/references/contribute/style-guide). A machine-readable version is available in `SKILL.md` at the root of this directory. Key requirements include:
...
The Haneul documentation uses shared components from the [ML Shared Docusaurus repo](https://github.com/GeunhwaJeong/ML-Shared-Docusaurus/blob/master/README.md) and Haneul-specific components. For a complete reference of all custom components and plugins, see the [MDX Components page](https://docs.haneul.io/references/contribute/mdx-components).
...
- [Localization](https://docs.haneul.io/references/contribute/localize-haneul-docs)
