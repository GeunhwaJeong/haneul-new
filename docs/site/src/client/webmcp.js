...
if (typeof window !== 'undefined') {
	const registerTools = () => {
		...
		navigator.modelContext.provideContext({
			tools: [
				{
					name: 'search_haneul_docs',
					description:
						'Search the Haneul documentation site for pages matching a query. Returns page titles, URLs, and snippets.',
					inputSchema: {
						type: 'object',
						properties: {
							query: {
					...
					async execute({ query }) {
						...
						return {
							type: 'text',
							text: `Search Haneul docs for "${query}": ${searchUrl}`,
						};
					},
				},
				{
				...
				{
					name: 'get_haneul_api_reference',
					description:
						'Get a summary of available Haneul APIs including JSON-RPC, GraphQL, and gRPC endpoints with their documentation URLs.',
					inputSchema: {
						type: 'object',
						properties: {},
					},
