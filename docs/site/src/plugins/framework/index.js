...
function truncateToCharLimit(md, sourcePath) {
  ...
  if (budget <= 0) {
    ...
    console.warn(
      `[haneul-framework-plugin] WARNING: Frontmatter alone exceeds ${MAX_CHAR_LIMIT} chars; truncating raw.`,
    );
  ...
  const sourceUrl = sourcePath
    ? `https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/${sourcePath}`
    : null;
  ...
  console.warn(
    `[haneul-framework-plugin] WARNING: Truncated from ${md.length} to ${result.length} chars (limit ${MAX_CHAR_LIMIT}).`,
  );
    ...
    async loadContent() {
      ...
      allFiles.forEach((theseFiles) => {
        theseFiles.forEach((absFile) => {
          ...
          console.log(
            `[haneul-framework-plugin] ${targetRel}: ${reMarkdown.length} chars`,
          );
