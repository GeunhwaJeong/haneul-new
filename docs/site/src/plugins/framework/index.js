    ...
    async loadContent() {
          ...
          parts.forEach((part, i) => {
            if (!part.match(/\.md$/)) {
              ...
              if (!fs.existsSync(newDir)) {
                ...
                const topUnpref = top.replace(/^haneul_/, "");

                // Category label: "haneul:<lowercased dirname without haneul_ prefix>"
                const unprefixed = part.replace(/^haneul_/, "");
                const label = `haneul:${unprefixed.toLowerCase()}`;
      ...
      function buildIndexForDir(absDir) {
        ...
        const fm = [
          ...
          `slug: ${slug}`,
          `description: "${(`Documentation for the modules in the ${CRATE_PACKAGES_PATH[topDir?.replace(/^haneul_/, "")] ?? ""} crate. Select a module from the list to see its details.`).replace(/"/g, '\\"')}"`,
          "---",
          "",
        ].join("\n");
