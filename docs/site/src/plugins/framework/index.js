...
const BRIDGE_PATH = path.join(
  __dirname,
  "../../../../../crates/haneul-framework/docs/bridge",
);
const FRAMEWORK_PATH = path.join(
  __dirname,
  "../../../../../crates/haneul-framework/docs/haneul",
);
...
const prefixRootDir = (seg) => `haneul_${seg}`;

// map of crate dir -> prefixed dir, used to rewrite hrefs in HTML
const CRATE_PREFIX_MAP = {
  bridge: "haneul_bridge",
  haneul: "haneul_haneul",
  std: "haneul_std",
  haneul_system: "haneul_haneul_system",
};

const CRATE_PACKAGES_PATH = {
  bridge: "haneul/crates/haneul-framework/packages/bridge",
  haneul: "haneul/crates/haneul-framework/packages/haneul",
  std: "haneul/crates/haneul-framework/packages/std",
  haneul_system: "haneul/crates/haneul-framework/packages/haneul_system",
};
...
const toLowerTitleText = (s) =>
  s.replace(/^haneul_/, "").replace(/[-_]+/g, " ").toLowerCase();
...
function validateSourcePath(srcPath, label) {
  if (!fs.existsSync(srcPath)) {
    console.warn(
      `[haneul-framework-plugin] WARNING: Source path for "${label}" does not exist: ${srcPath}`,
    );
    return false;
  }
  try {
    ...
    if (!stat.isDirectory()) {
      console.warn(
        `[haneul-framework-plugin] WARNING: Source path for "${label}" is not a directory: ${srcPath}`,
      );
      return false;
    }
  } catch (err) {
    console.warn(
      `[haneul-framework-plugin] WARNING: Cannot stat source path for "${label}": ${err.message}`,
    );
    return false;
  }
  return true;
...
function safeWriteFile(filePath, content) {
  try {
    ...
    console.error(
      `[haneul-framework-plugin] ERROR: Failed to write ${filePath}: ${err.message}`,
    );
  }
}
...
function safeReadFile(filePath) {
  try {
    ...
    console.error(
      `[haneul-framework-plugin] ERROR: Failed to read ${filePath}: ${err.message}`,
    );
    return null;
  }
}
 ...
 *
 * MUST run BEFORE injectToc so the TOC placement logic finds the description
 * paragraph and positions itself after it.
 */
...
const frameworkPlugin = (_context, _options) => {
  return {
    name: "haneul-framework-plugin",

    async loadContent() {
      if (fs.existsSync(DOCS_PATH)) {
        try {
          fs.rmSync(DOCS_PATH, { recursive: true, force: true });
          console.log("[haneul-framework-plugin] Removed existing framework docs directory.");
        } catch (err) {
          console.error(
            `[haneul-framework-plugin] ERROR: Cannot remove existing directory ${DOCS_PATH}: ${err.message}`,
          );
          return;
        }
      }
      ...
      try {
        ...
        console.error(
          `[haneul-framework-plugin] ERROR: Cannot create output directory ${DOCS_PATH}: ${err.message}`,
        );
        return;
      }
      ...
      const recurseFiles = (dirPath, files = []) => {
        ...
        try {
          ...
          console.warn(
            `[haneul-framework-plugin] WARNING: Cannot read directory ${dirPath}: ${err.message}`,
          );
          return files;
        }
      ...
      const sourceDirs = [
        ...
        { path: FRAMEWORK_PATH, label: "haneul" },
        { path: STDLIB_PATH, label: "std" },
        // { path: DEEPBOOK_PATH, label: "deepbook" },
        { path: HANEULSYS_PATH, label: "haneul_system" },
      ];
      ...
      for (const src of sourceDirs) {
        if (validateSourcePath(src.path, src.label)) {
          ...
          if (files.length > 0) {
            ...
            console.warn(
              `[haneul-framework-plugin] WARNING: No .md files found in "${src.label}" at ${src.path}`,
            );
          }
        }
      }
      ...
      if (validSources === 0) {
        console.error(
          "[haneul-framework-plugin] ERROR: No valid source directories found. " +
      ...
      allFiles.forEach((theseFiles) => {
        theseFiles.forEach((absFile) => {
          ...
          reMarkdown = reMarkdown
            .replace(
              /href=(["'])(\.\.\/)(bridge|haneul|std|haneul_system)\/([^"']*)\1/g,
              (_m, q, up, seg, tail) =>
                `href=${q}${up}${CRATE_PREFIX_MAP[seg]}/${tail}${q}`,
            )
            .replace(
              /href='(\.\.\/)(bridge|haneul|std|haneul_system)\//g,
              (m, up, seg) =>
          ...
          // ── NEW: Inject fallback description for pages with no body context ──
          // Must run BEFORE injectToc so the TOC finds the description paragraph
          reMarkdown = injectFallbackDescription(reMarkdown);

          // ── NEW: Ensure frontmatter has a description for SEO / link previews ──
          ...
          parts.forEach((part, i) => {
            if (!part.match(/\.md$/)) {
              ...
              if (!fs.existsSync(newDir)) {
                ...
                const unprefixed = part.replace(/^haneul_/, "");
                ...
                try {
                  ...
                  console.error(
                    `[haneul-framework-plugin] ERROR: Failed to create category file ${catfile}: ${err.message}`,
                  );
                }
              }
            }
      ...
      console.log(
        `[haneul-framework-plugin] Processed ${processedCount} files` +
      ...
      function buildIndexForDir(absDir) {
        ...
        const dirName = relParts.length
          ...
          : "framework";
        const titleText = `haneul:${toLowerTitleText(dirName)}`;
        ...
        try {
          ...
          console.error(
            `[haneul-framework-plugin] ERROR: Cannot read directory for index: ${absDir}: ${err.message}`,
          );
          return;
        }
        ...
        const frameworkName = topDir.replace(/^haneul_/, "");
        const norm = (s) =>
          s
            .replace(/\.mdx?$/i, "")
            .toLowerCase()
        ...
        const topUnprefixed = topDir?.replace(/^haneul_/, "") ?? "";
        ...
        try {
          ...
          console.error(
            `[haneul-framework-plugin] ERROR: Failed to create index.md in ${absDir}: ${err.message}`,
          );
        }
      }
      ...
      function buildAllIndexes() {
        ...
        while (stack.length) {
          ...
          try {
            ...
            console.error(
              `[haneul-framework-plugin] ERROR: Cannot read directory ${dir}: ${err.message}`,
            );
            continue;
          }
          for (const ent of entries) {
