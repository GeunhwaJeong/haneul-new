...
const protocolInject = async function (source) {
  ...
  const haneulSorted = (array) => {
    return array.sort((a, b) => {
      const aStartsWithHaneul = a.name.startsWith("haneul");
      const bStartsWithHaneul = b.name.startsWith("haneul");

      if (aStartsWithHaneul && !bStartsWithHaneul) return -1;
      if (!aStartsWithHaneul && bStartsWithHaneul) return 1;
      return 0;
    });
  };
  ...
  let tocSorted = haneulSorted(toc);
  tocSorted.push({
  ...
  const files = haneulSorted(spec.files);
  const bordercolor = "border-haneul-blue-dark dark:border-haneul-blue";

  const tabStyle = `grid grid-cols-6 border border-solid border-b-0 border-haneul-blue-dark ${bordercolor}`;
  const tabRowStyle = `p-4 border-0 border-b border-solid ${bordercolor} col-span-full`;
  const tabHeaderStyle = `${tabRowStyle} bg-haneul-gray-50 dark:bg-haneul-ghost-dark`;
  const tabAltHeaderStyle = `${tabRowStyle} bg-haneul-ghost-white dark:bg-haneul-gray-95`;
      ...
      if (fields.length > 0) {
        for (const field of fields.entries()) {
          ...
          if (hasLabel) {
            ...
            let labelBg = "bg-haneul-ghost-white dark:bg-haneul-ghost-dark";
            if (field[1].label === "optional") {
              label = "Proto3 optional";
              labelBg = "bg-haneul-blue-light dark:bg-haneul-blue-dark";
            } else if (field[1].label === "repeated") {
              label = "Repeated []";
              labelBg = "bg-haneul-warning-light dark:bg-haneul-warning-dark";
            }
            content.push(
  ...
  content.push("\n## Scalar Value Types");
  const cellStyle =
    "m-2 min-w-24 max-w-[13rem] rounded-lg border border-solid align-center text-center relative border-haneul-gray-65";
  const titleStyle =
    "p-4 pb-2 font-bold text-haneul-ghost-dark dark:text-haneul-ghost-white bg-haneul-gray-50 dark:bg-haneul-ghost-dark border border-solid border-transparent rounded-t-lg";
  const valStyle =
    "p-4 pt-2 border border-solid border-transparent border-t-haneul-gray-65 whitespace-break-spaces";
