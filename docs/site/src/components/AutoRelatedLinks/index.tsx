...
const SPECIAL_CASING = new Map<string, string>([
  ...
  ["pnpm", "pnpm"], ["pip", "pip"], ["docker", "docker"], ["haneul", "Haneul"],
  ["move", "Move"], ["git", "git"], ["pyhaneul", "pyhaneul"], ["haneullabs", "Haneullabs"],
]);
...
function useMetaSafe(): Meta[] | null {
  try {
    const raw = usePluginData("haneul-description-plugin") as any;
    if (!raw) return null;
