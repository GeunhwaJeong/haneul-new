...
const indices = [
  { label: "Haneul", indexName: "haneul_docs" },
  { label: "HaneulNS", indexName: "haneulns_docs" },
  { label: "The Move Book", indexName: "move_book" },
  { label: "SDKs", indexName: "haneul_sdks" },
...
function HitItem({ hit }: { hit: any }) {
  ...
  return (
    <a
      href={hit.url}
      className="modal-result block px-4 py-3 -mx-2 rounded-lg no-underline hover:bg-haneul-gray-40 dark:hover:bg-haneul-gray-80 transition-colors"
    >
      {breadcrumb.length > 0 && (
        <div className="text-xs text-gray-500 dark:text-haneul-gray-55 mb-1 truncate">
          {breadcrumb.join(" > ")}
        </div>
      )}
      ...
      {hit.content && (
        <p
          className="text-xs text-gray-600 dark:text-haneul-gray-45 mt-1 mb-0 line-clamp-2"
...
function EmptyState({ label }: { label: string }) {
  ...
  if (results?.hits?.length === 0) {
    return (
      <p className="text-sm text-haneul-gray-5s dark:text-haneul-gray-50">
        No results in {label}
      </p>
    );
  }
...
export default function MultiIndexSearchModal({
  ...
  const activeMeta = {
    haneul_docs: null,
    haneulns_docs: { label: "HaneulNS Docs", url: "https://docs.haneulns.io" },
    move_book: {
      label: "The Move Book",
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-haneul-gray-90 w-full max-w-4xl rounded-xl shadow-2xl max-h-[min(600px,80vh)] flex flex-col overflow-hidden">
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto">
          <InstantSearch searchClient={searchClient} indexName={activeIndex}>
            <div className="bg-white dark:bg-haneul-gray-90 rounded-t sticky top-0 z-10 px-6">
              <div className="bg-white dark:bg-haneul-gray-90 h-8 flex justify-end">
                <button
                  onClick={onClose}
                  className="bg-transparent border-none outline-none text-xs text-gray-400 dark:text-haneul-gray-60 hover:text-gray-600 cursor-pointer"
                >
                  ESC
                </button>
              </div>
              ...
              />
              {query.length < 3 && (
                <p className="text-xs text-gray-400 dark:text-haneul-gray-60 pl-1 mb-2 -mt-6">
        ...
        <div className="h-12 px-6 bg-white dark:bg-haneul-gray-90 flex items-center justify-between text-xs border-t border-solid border-haneul-gray-50 dark:border-haneul-gray-80 border-b-transparent border-l-transparent border-r-transparent shrink-0">
          <a
            href={`/search?q=${encodeURIComponent(query)}`}
            className="text-gray-500 dark:text-haneul-gray-50 hover:text-haneul-blue dark:hover:text-haneul-blue-light no-underline"
          >
            View all results
          </a>
          {activeMeta && (
