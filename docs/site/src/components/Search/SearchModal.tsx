...
function HitItem({ hit }: { hit: any }) {
  ...
  return (
    <div className="modal-result">
      <a
        href={hit.url}
        className="text-blue-600 dark:text-haneul-blue dark:hover:text-haneul-blue-light font-medium"
      >
        {hit.title}
      </a>
      <a
        ...
        className="text-base text-blue-600 dark:text-haneul-blue dark:hover:text-haneul-blue-light underline pb-2"
      >
        {sectionTitle}
      </a>
      <p
        className="text-sm text-gray-600 dark:text-haneul-gray-50"
...
export default function MultiIndexSearchModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex justify-center p-4">
      <div className="bg-white dark:bg-haneul-gray-90 w-full max-w-3xl px-6 rounded-lg shadow-md max-h-[600px] flex flex-col">
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto">
          <InstantSearch searchClient={searchClient} indexName={activeIndex}>
            <div className="bg-white dark:bg-haneul-gray-90 rounded-t sticky top-0 z-10">
              ...
              />
              {query.length < 3 && (
                <p className="text-sm text-haneul-gray-5s dark:text-haneul-gray-50 pl-4 mb-2 -mt-6">
        ...
        <div className="h-14 bg-white dark:bg-haneul-gray-90 flex items-center justify-between text-sm border-t border-solid border-haneul-gray-50 border-b-transparent border-l-transparent border-r-transparent">
          <a
            href={`/search?q=${encodeURIComponent(query)}`}
            className="text-blue-600 dark:text-haneul-blue dark:hover:text-haneul-blue-light underline"
          >
