...
if (!toc || !toc.length || !proto) return null;  const [belowFold, setBelowFold] = useState(false);
  ...
  return (
    <div
      className={`max-xl:hidden sticky top-16 py-4 -mx-4 z-10 backdrop-blur-sm border-haneul-ghost-white dark:border-haneul-ghost-dark ${belowFold ? "border-solid border-x-0 border-t-0 border-b" : ""}`}
    >
      <style>
        {`
          h2, h3, h4 {
      ...
      <label
        className="m-2 text-xs bg-haneul-white rounded-lg backdrop-blur-none"
        htmlFor="proto"
      >
        Proto files
      </label>
