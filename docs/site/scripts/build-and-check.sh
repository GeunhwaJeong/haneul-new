...
docusaurus graphql-to-doc:beta && node scripts/remove-no-desc.mjs ../content/references/haneul-api/haneul-graphql/beta/reference || { echo "❌ graphql-to-doc step failed"; exit 1; }
...
TEMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/GeunhwaJeong/display-preview.git "$TEMP_DIR/display-preview"
