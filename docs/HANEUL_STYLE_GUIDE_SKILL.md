---
name: haneul-documentation-style-guide
description: >
  Apply Haneul Documentation style guide requirements to all documentation files. Do not edit code snippets within backticks.
  ...
  * Audience: Broader developer audience. Do not over-define common developer terms like CLI, terminal, SDK, etc. Assume the reader is familiar with the idea of code development, but not necessarily Haneul-specific code bases, tooling, or SDKs.

  * Consider AI ingestion: The documentation page will be ingested by agents and the docs.haneul.io custom chatbot. Ensure the content is parseable for agents when exposed as markdown.
...
# Haneul Documentation Style Guide
...
### Always Capitalized
Proper nouns, product names, example app names (Coin Flip, Blackjack), Archival Store/Service, Coin Registry, Currency Standard, DeepBook Indexer, DeepBookV3, Devnet, GraphQL RPC, General-purpose Indexer, ID, Localnet, Kiosk (the standard), Mainnet, Mysticeti, One-Time Witness, Operation Cap, Haneul, Haneul CLI, Haneul Client PTB CLI, Haneul Closed-Loop Token / Closed-Loop Token, Haneul dApp Kit, Haneul Explorer, HaneulJSON, Haneul Keystore, Haneul Keytool, HaneulLink, Haneul Object Display, HaneulPlay0X1, HANEUL, HANEUL token, Testnet, Wallet Standard, Web2, Web3, zkSend SDK

### Always Lowercase
casual history, casual order, certificate, epoch, equivocation, eventual consistency, finality, gas, genesis, kiosk (instance), object, oracle, recovery passphrase (mnemonic), smart contract, soulbound, Haneul framework, Haneul object, total order, transaction, transfer, validator, wallet
...
- Capitalize "Haneul Full Node" for the official software binary.
...
- Use full relative links for docs.haneul.io topics.
