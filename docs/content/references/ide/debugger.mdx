...
keywords: [ move trace debugger, debug move execution traces, debug move traces, move execution traces, vs code, debug in vs code, visual studio code, move extension, vs code extension, move vs code extension, debugging, debug interface ]
---
...
To use the debugger, you must also have the `haneul` binary installed with the `tracing` feature flag enabled. The `haneul` binaries in release tarballs, Homebrew, and Chocolatey have this feature enabled. See [Install Haneul](../../guides/developer/getting-started/haneul-install.mdx) for more information, including how to build from source.
