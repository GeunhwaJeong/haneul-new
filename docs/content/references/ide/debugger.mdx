...
The [Move Trace Debugger](https://marketplace.visualstudio.com/items?itemName=haneullabs.move-trace-debug) extension for Visual Studio Code provides a familiar [debugging interface](https://code.visualstudio.com/docs/debugtest/debugging) for Move unit tests and onchain transactions, including [PTB](/guides/developer/transactions/ptbs/prog-txn-blocks) debugging support. You can inspect the state of PTB commands, step through code execution, track local variable values, and set line breakpoints for Move code, either in unit tests or in onchain Move calls.

Debugging is enabled via trace generation: traces can be generated during [unit test execution](#debugging-unit-tests) or during onchain [transaction replay](#debugging-on-chain-transactions).
...
Use the debugger to debug unit tests and existing onchain transactions (including [PTB](/guides/developer/transactions/ptbs/prog-txn-blocks) debugging support) with the help of the [replay tool](/references/cli/replay).
...
- Check the version tags.
    ...
    If tags are not found, you can proceed directly to the next step, but there is no guarantee that the most recent version of source code in the repository corresponds to the version of the package onchain, which may affect debugging capabilities.
