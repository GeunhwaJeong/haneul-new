...
To execute these commands you must have `haneul` binary installed, in particular, to generate a trace, the `haneul` binary version must be trace-generation capable. See [Debugger](/references/ide/debugger#install) for installation instructions. The extension will search for the `haneul` binary on a system path but you can configure the extension to use a different path.
