...
1. Search for `haneullabs.move`.
1. Click **Install** on the Move extension by Haneul Labs.

Alternative install methods include:

  - Use <kbd>Ctrl</kbd> + <kbd>P</kbd> or <kbd>⌘</kbd> + <kbd>P</kbd> and type `ext install haneullabs.move`.
  - Use the command line: 

      ```sh
      $ code --install-extension haneullabs.move
      ```
...
- [Move Trace Debugger](https://marketplace.visualstudio.com/items?itemName=haneullabs.move-trace-analyze)
...
```sh
$ cargo install --git https://github.com/GeunhwaJeong/haneul.git haneul-move-lsp
```

By default, the Move extension expects to find the `move-analyzer` binary in `~/.haneul/bin`. You can either copy the binary to this location, or configure the extension to use a different path. 
...
These commands find the `Move.toml` file for the open Move source file and open a terminal to run the appropriate `haneul move` command.

To execute these commands you must have `haneul` binary installed, in particular, to generate a trace, the `haneul` binary version must be trace-generation capable. See [Debugger](./debugger.mdx#install) for installation instructions. The extension will search for the `haneul` binary on a system path but you can configure the extension to use a different path.
