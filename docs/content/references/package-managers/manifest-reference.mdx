...
For a basic project that only depends on `haneul` and `std`, the minimal lockfile is:
...
usdc = { git = "https://github.com/circlefin/stablecoin-haneul.git", subdir = "packages/usdc", rev = "releases/testnet" }

[dep-replacements.mainnet]
usdc = { git = "https://github.com/circlefin/stablecoin-haneul.git", subdir = "packages/usdc", rev = "releases/mainnet" }
```
