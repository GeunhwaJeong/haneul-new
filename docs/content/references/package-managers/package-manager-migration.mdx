...
Version 1.63 of the Haneul CLI introduced a major overhaul of the package management system. This document describes the changes between the old and new systems and gives guidance on migrating your packages. See [the package management user guide](/develop/manage-packages/move-package-management) for full details on the new system.
...
With any change of this magnitude there are bound to be some issues that crop up. Report any issues you encounter while using the new package system [here](https://github.com/GeunhwaJeong/haneul/issues/24540).
...
4. If necessary, replace `[dev-dependencies]` with [moded dependencies](/develop/manage-packages/move-package-management#advanced-dependency-configuration).
...
Publication workflows on Mainnet and Testnet should not change with the new package system. However, publishing on ephemeral networks like `localnet` or `devnet` should use the `haneul client test-publish` command to avoid polluting the `Published.toml` file with local addresses.

Instead of writing to `Published.toml`, the `test-publish` command writes the addresses to an ephemeral publication file (for example, `Pub.localnet.toml`). You can select the file to use with the `--pubfile-path` option. If you want to publish multiple packages, you should use the same ephemeral publication file for all of the packages, so that the dependency addresses will be set correctly. You can also automate this with the `--publish-unpublished-deps` [flag](/develop/manage-packages/move-package-management#ephemerally-publishing-dependencies).

:::caution
If you were using `--with-unpublished-deps`, you should almost certainly use `--publish-unpublished-deps` instead.
:::

The `test-publish` command requires a build environment (Testnet or Mainnet by default, although you can [define your own](/develop/manage-packages/move-package-management#environments)) to determine which dependencies to use for the ephemeral publication. Specify the build environment by passing the `--build-env <env>` flag.
...
Haneul advises against disabling implicit dependencies.
...
For [ephemeral publication](/develop/manage-packages/move-package-management#environments), the package system will use ephemeral files (often named `Pub.<env>.toml`); these files should not be committed to source control. Add `Pub.*.toml` to your `.gitignore` file.
