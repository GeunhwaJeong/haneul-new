...
The new package system is included in the 1.63 release of the Haneul CLI, which was released in January. However, you can install it before then using [`haneulup`](https://github.com/GeunhwaJeong/haneulup):

```sh
$ haneulup install haneul --nightly=haneul-pkg-alt
$ haneulup install haneul --nightly=ml/pkg-alt-integration mvr
```

This compiles `haneul` from the prototype branch, so it requires the Rust toolchain installed and might take a while to complete installation.
...
The package manager is integrated into the `haneul` binary, so most of the previous commands work the same way. There are a few changes though.

All operations must be done in a specific environment due to `dep-replacements` and how some details about how package conflicts are determined. By default, the `haneul` binary tries to choose the right environment based on the `haneul client` environment determined by `haneul client active-env`.
