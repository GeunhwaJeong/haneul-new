...
keywords: [ haneul cli, haneul client cli, haneul client cli, haneul client, haneul cli help, haneul cli reference, haneul cli commands, cli commands, cli reference ]
---
...
Commands:
  ...
  new-address                 Generate new address and key pair with key pair scheme flag {ed25519 | secp256k1 | secp256r1} with optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0
...
If you use one of the standard public RPCs (for example, `fullnode.devnet.haneul.io:443`), you can use the `faucet` command to request gas coins. If you use a custom faucet service, then pass in the URL to the faucet using the `--url` option.
...
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `haneul client envs` for a list of your defined networks and their aliases).
    ...
    ```
1. Use `haneul move build` to build the package. You must run this command at the same level as the package manifest file ([Move.toml](https://move-book.com/concepts/manifest.html)).
    ```sh
    $ haneul move build
    ```
    ...
    ```
1. Use `haneul move test` to run the unit tests.
    ```sh
    $ haneul move test
    ```
    ...
    Test result: OK. Total tests: 2; passed: 2; failed: 0
     ```
1. Use the `haneul client verify-bytecode-meter` to check if the module passes the bytecode meter.
    ```sh
    ...
    ╰────────┴────────────┴────────────╯
    ```
1. Use `haneul client gas` to verify that the active address has a gas coin for paying gas. 
    ```sh
    $ haneul client gas
    ```
...
1. Use `haneul client gas` to verify the current active address received the coins.
    ...
    ╰────────────────────────────────────────────────────────────────────┴─────────────╯
    ```
1. Use `haneul client publish` to publish the package, being sure to set an appropriate value for the `gas-budget` flag if you're using an older version of Haneul. The console responds with the details of the publish. You can use `haneul client object <OBJECT-ID>` to check the details of any of the objects from the process.
    ```sh
...
Usage: haneul client call [OPTIONS] --package <PACKAGE> --module <MODULE> --function <FUNCTION>
...
Options:
      ...
      --gas-budget <GAS_BUDGET>         An optional gas budget for this transaction (in GEUNHWA). If gas budget is not provided, the tool will first perform a dry
      ...
      --serialize-unsigned-transaction  Instead of executing the transaction, serialize the bcs bytes of the unsigned transaction data (TransactionData) using
                                        base64 encoding, and print out the string <TX_BYTES>. The string can be used to execute transaction with `haneul client
      ...
      --serialize-signed-transaction    Instead of executing the transaction, serialize the bcs bytes of the signed transaction data (SenderSignedData) using
                                        base64 encoding, and print out the string <SIGNED_TX_BYTES>. The string can be used to execute transaction with `haneul
