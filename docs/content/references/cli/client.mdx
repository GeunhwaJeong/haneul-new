---
title: Haneul Client CLI
description: The Haneul Client CLI provides command-level access to interact with the Haneul network.
---

The Haneul CLI `client` command provides command-level access to interact with the Haneul network. Typical uses for `haneul client` include publishing Move smart contracts, getting the information of an object, executing transactions, or managing addresses.

{@include: ../../snippets/cli-check-install.mdx}

## Commands

The following list itemizes all the available subcommands for the `haneul client` command.

```
Usage: haneul client [OPTIONS] [COMMAND]
...
Commands:
  ...
  envs                        List all Haneul environments
  ...
  new-address                 Generate new address and keypair with keypair scheme flag {ed25519 | secp256k1 | secp256r1} with optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0
                                  for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1. Word length can be { word12 | word15 | word18 | word21 | word24} default to word12 if not specified
  new-env                     Add new Haneul environment
  ...
  pay-all-haneul                 Pay all residual HANEUL coins to the recipient with input coins, after deducting the gas cost. The input coins also include the coin for gas payment, so no extra gas coin is required
  pay-haneul                     Pay HANEUL coins to recipients following following specified amounts, with input coins. Length of recipients must be the same as that of amounts. The input coins also include the coin for
  ...
  transfer-haneul                Transfer HANEUL, and pay gas with the same HANEUL coin object. If amount is specified, only the amount is transferred; otherwise the entire object is transferred
...
Append the `--json` flag to commands to format responses in JSON instead of the more human-friendly default Haneul CLI output. This can be useful for extremely large datasets, for example, as those results can have a troublesome display on smaller screens. In these cases, the `--json` flag is useful.
...
Use the `haneul client envs` command to find the network environments set up in the CLI. The information for these environments is also stored in the client.yaml file in the Haneul configuration directory (`~/.haneul/haneul_config`).
...
Use `client new-env` to add details for a new network environment. This example creates an environment pointer to Haneul Mainnet. Setting the `alias` value makes referencing the environment less prone to typographical errors. After running this command, Haneul updates your client.yaml file in `~/.haneul/haneul_config` with the new information.

```
$ haneul client new-env --alias=mainnet --rpc https://fullnode.mainnet.haneul.io:443

Added new Haneul env [mainnet] to config.
```

### Set current environment

Use the `haneul client switch` command to change the current network. This example switches the current network to `mainnet`.

```shell
$ haneul client switch --env mainnet

Active environment switched to [mainnet]
```

If you run `haneul client envs` after this command, you see the asterisk in the `active` column on the `mainnet` row of the table.

### Get current active address

Use the `haneul client active-address` command to reveal the current address. The CLI uses the current active address to execute address-specific CLI commands (like `haneul client objects`) when you don't provide them with a Haneul address value.
...
### Request a HANEUL coin from faucet

If you use one of the standard public RPCs (for example, `fullnode.testnet.haneul.io:443`), you can use the `faucet` command to request gas coins. If you use a custom faucet service, then pass in the URL to the faucet using the `--url` option.

```shell
$ haneul client faucet
Request successful. It can take up to 1 minute to get the coin. Run haneul client gas to check your gas coins.
```

### Get objects owned by an address

Use `haneul client objects` to list summary information about the objects the current active address owns. You can provide a Haneul address value to the command to list objects for a particular address. This example lists objects for the current active address.

```
$ haneul client objects 0x36df11369cf00ecf0be68d6ba965b0abe2e883bc5245911e3a29ebfa0aaf6b69
...
Use `haneul client object <OBJECT-ID>` to list object information for the ID you provide. This example displays the information for a Coin object.

```
$ haneul client object 0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f
╭───────────────┬─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
│ objectId  	  │  0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f                                             	  │
│ version   	  │  33363559                                                                                                       	  │
│ digest    	  │  3FzvXJVVVcXb9H6dEXdARaY9EmxXyyNFduet3X4eYV4x                                                                   	  │
│ objType   	  │  0x2::coin::Coin<0x2::haneul::HANEUL>                                                                                 	  │
...
Use the `haneul client dynamic-field <DYNAMIC-FIELD-ID>` command to list the details of the dynamic field with the ID you provide.

```
$ haneul client dynamic-field 0x5
╭─────────────┬───────────────────────────────────────────────────────────────────────────────────────────╮
│ hasNextPage │  false                                                                                	  │
...
│         	  │ │ │ bcsName	   │  LQM2cdzDY3                                                      	  │ │ │
│         	  │ │ │ type   	   │  DynamicField                                                    	  │ │ │
│         	  │ │ │ objectType │  0x3::haneul_system_state_inner::HaneulSystemStateInnerV2              	  │ │ │
...
### Send HANEUL or objects

In this example, let's see how to transfer HANEUL or transfer an object from one address to another. First of all, there two main commands for sending HANEUL or transferring objects: `pay` and `transfer`. 
Both `pay` and `transfer` have a few sister commands: `pay-haneul`, `pay-all-haneul`, `transfer-haneul`. 

The differences between these commands are:
- commands that end in `-haneul` deal with Haneul's native coin, and they use the input coints to pay for gas and for transferring HANEUL or the object.
...
- `pay-all-haneul` is a special case of `pay-haneul` that offers a way to transfer the entire balance after smashing.
- `transfer-haneul` is a legacy command and has been entirely superseded by `pay-haneul` or `pay-all-haneul` depending on whether an amount is specified or not.
...
```
haneul client gas hungry-spodumene
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
...
You want to send 0.5 HANEUL to `eloquent-amber`. Given that you have a few gas coins, you can use `pay`. If only one gas coin exists, then you need to use `transfer-haneul` or `pay-haneul`, or you would need to split the coin first to have another coin to use for paying gas. In this case, let's use the `pay-haneul` command as you do not need to provide a separate gas coin to be used for the gas fees. In the command below, you set the recipient to be `eloquent-amber`, which coin to use to transfer HANEUL from, and the amount of HANEUL to transfer.

{@include: ../../snippets/info-gas-budget.mdx}

```
haneul client pay-haneul --recipients eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amounts 500000000 --gas-budget 5000000
```
...
│ Gas Budget: 5000000 GEUNHWA                                                                                     │
│ Gas Price: 1000 GEUNHWA                                                                                         │
│ Gas Payment:                                                                                                 │
│  ┌──                                                                                                         │
...
│  └──                                                                                              │
│ Gas Cost Summary:                                                                                 │
│    Storage Cost: 1976000 GEUNHWA                                                                     │
│    Computation Cost: 1000000 GEUNHWA                                                                 │
│    Storage Rebate: 978120 GEUNHWA                                                                    │
│    Non-refundable Storage Fee: 9880 GEUNHWA                                                          │
│                                                                                                   │
...
Check the gas for the sender and the recipient's coins using `haneul client gas <address>`. The sender now has 199.49 HANEUL for that gas coin that was used. 0.5 HANEUL was transferred, and the remaining 0.01 HANEUL paid the gas fees.
```
haneul client gas
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ geunhwaBalance (GEUNHWA) │ haneulBalance (HANEUL) │
...
haneul client gas eloquent-amber
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ geunhwaBalance (GEUNHWA) │ haneulBalance (HANEUL) │
├────────────────────────────────────────────────────────────────────┼────────────────────┼──────────────────┤
...
If you want to transfer the whole object, you can use `haneul client pay-all-haneul` or `haneul client transfer-haneul` (without passing the amount): 
```
haneul client pay-haneul --recipient eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
or
haneul client transfer-haneul --to eloquent-amber --haneul-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
```
...
Use the `haneul client replay-transaction --tx-digest <TRANSACTION-DIGEST>` to re-execute a transaction locally and show the transaction effects.
...
```
$ haneul client replay-transaction --tx-digest 51MzJP2Uesvza8vXGpPCGbfLrY6UCfdvdoErN1z4oXPW
╭───────────────────────────────────────────────────────────────────────────────────────────────────╮
│ Transaction Effects                                                                               │
├───────────────────────────────────────────────────────────────────────────────────────────────────┤
...
Use `haneul client replay-batch --path <FILEPATH>` to replay several transactions listed in a newline-separated file. This
will verify that all transactions local execution results match the effects on-chain.

### Profile a transaction

Use the `haneul client profile-transaction --tx-digest <TRANSACTION-DIGEST>` command to re-execute a transaction locally
...
To enable the profiler, you must either install or build the Haneul Client binary locally with the `--features tracing` flag if it has not already been built with that feature enabled.

```shell
cargo install --locked --git https://github.com/GeunhwaJeong/haneul.git --branch <BRANCH-NAME> --features tracing haneul
```
...
One of the main uses of the `haneul client` command is to publish smart contracts on the Haneul network. This example switches the current environment to the Devnet network, then builds, tests, and publishes one of the existing Move examples available in the Haneul repository: [haneul/examples/move](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move)

This example also makes use of `haneul move` commands. To learn more about those commands, see [Haneul Move CLI](./move.mdx).

1. Open a terminal or console to the root of your local Haneul repository and navigate to the `move_tutorial` example.
    ```shell
    cd examples/move
    cd first_package
    ```
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `haneul client envs` for a list of your defined networks and their aliases).
    ```shell
    haneul client switch --env devnet
    ```
1. Use `haneul move build` to build the package. You must run this command at the same level as the package manifest file ([Move.toml](https://move-book.com/concepts/manifest.html)). The console responds with the status of the build.
    ```shell
    haneul move build
    ...
    ```
1. Use `haneul move test` to run the unit tests. The console responds with updates of its progress.
    ```shell
    haneul move test
    INCLUDING DEPENDENCY Haneul
    INCLUDING DEPENDENCY MoveStdlib
    ...
    Test result: OK. Total tests: 2; passed: 2; failed: 0
     ```
1. Use the `haneul client verify-bytecode-meter` to check if the module passes the bytecode meter. The console responds with the maximum allowed values, as well as the amount the package uses.
    ```shell
    $ haneul client verify-bytecode-meter
    ...
    ╰────────┴────────────┴────────────╯
    ```
1. Use `haneul client gas` to verify that the active address has a gas coin for paying gas. In the case of this example, the console responds with the information that the address is coinless.
    ```shell
    $ haneul client gas
    No gas coins are owned by this address
    ```
1. If you need coins, use `haneul client faucet` (not available for Mainnet). For more information on getting gas tokens, see [Get Haneul Tokens](/guides/developer/getting-started/get-coins.mdx).
    ```shell
    $ haneul client faucet
    ```
1. Use `haneul client gas` to verify the current active address received the coins.
    ```shell
    $ haneul client gas
    ╭────────────────────────────────────────────────────────────────────┬─────────────╮
    ...
    ╰────────────────────────────────────────────────────────────────────┴─────────────╯
    ```
1. Use `haneul client publish` to publish the package, being sure to set an appropriate value for the `gas-budget` flag. The console responds with the details of the publish. You can use `haneul client object <OBJECT-ID>` to check the details of any of the objects from the process.
    ```shell
    $ haneul client publish --gas-budget 100000000 .
...
Each command has its own help section. For example, `haneul client call --help` displays the following prompt:

```
Call Move function

Usage: haneul client call [OPTIONS] --package <PACKAGE> --module <MODULE> --function <FUNCTION> --gas-budget <GAS_BUDGET>
...
Options:
  	...
  	--serialize-unsigned-transaction  Instead of executing the transaction, serialize the bcs bytes of the unsigned transaction data (TransactionData) using base64 encoding, and print out
                                    	the string <TX_BYTES>. The string can be used to execute transaction with `haneul client execute-signed-tx --tx-bytes <TX_BYTES>`.
  	--serialize-signed-transaction	Instead of executing the transaction, serialize the bcs bytes of the signed transaction data (SenderSignedData) using base64 encoding, and print out the
                                    	string <SIGNED_TX_BYTES>. The string can be used to execute transaction with `haneul client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
  	--json                        	Return command outputs in json format
  -h, --help                        	Print help
```
