...
To enable the profiler, you must build the Haneul Client binary locally or install it with the `--features gas-profiler` flag set.
...
1. Open a terminal or console to the root of your local Haneul repository and navigate to the `move_tutorial` example.
    ...
    cd first_package
    ```
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `haneul client envs` for a list of your aliases).
    ```shell
