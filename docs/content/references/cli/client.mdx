...
To enable the profiler, you must either install or build the Haneul Client binary locally with the `--features gas-profiler` flag.

```shell
cargo install --locked --git https://github.com/GeunhwaJeong/haneul.git --branch <BRANCH-NAME> --features gas-profiler haneul
```
