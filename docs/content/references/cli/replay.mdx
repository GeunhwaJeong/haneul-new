---
title: Haneul Replay CLI
description: Use the Haneul CLI to replay a given transaction to view its effects.
---

Replay is a tool that allows you to replay a given transaction to check and analyze the transaction effects. The replay tool locally executes the transaction and compares its effects to the effects that exist on chain.

Using the replay tool, you can pass the `--trace` flag to generate execution traces for a transaction that can be used with the Move debugger. For more information about the Move debugger, see the [Move Debugger](/references/ide/debugger) page. You can run different [analyses on the transaction traces](/references/cli/trace-analysis) such as [gas profiling](/references/cli/trace-analysis).

## Usage

The main command for replaying a transaction follows. For all available flags and arguments, run `haneul replay --help`.

```sh
$ haneul replay --digest <digest>
```
...
│  └──                                                                                              │
│ Gas Cost Summary:                                                                                 │
│    Storage Cost: 11187200 GEUNHWA                                                                    │
│    Computation Cost: 1000000 GEUNHWA                                                                 │
│    Storage Rebate: 13031568 GEUNHWA                                                                  │
│    Non-refundable Storage Fee: 131632 GEUNHWA                                                        │
│                                                                                                   │
...
the gas cost, refer to the [gas in Haneul](/develop/transaction-payment/gas-in-haneul) page.
...
You can provide default values for the replay tool flags by specifying them in the `~/.haneul/haneul_config/replay.toml` file. You need to specify them in the `[flags]` section (the only section currently supported) using their long names with initial dashes (`--`) removed. For example, you can disable showing transaction effects with `show-effects = false` but not with `e = false`. Here is a complete example of a config file:
```toml
[flags]
show-effects = false
overwrite = true
