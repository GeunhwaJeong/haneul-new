...
With Replay, you can pass the `--trace` flag to generate execution traces that can be used with the Move debugger to debug that transaction locally. For more information about the Move debugger, see the [Move Debugger](../ide/debugger.mdx) page.

## Usage 
The main command for replaying a transaction follows. For all available flags and arguments, run `haneul client replay-transaction --help`.

```
haneul client replay-transaction --tx-digest <digest>
```
...
the gas cost, please refer to the [gas in haneul](../../concepts/tokenomics/gas-in-haneul.mdx) page.
