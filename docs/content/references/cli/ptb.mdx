...
<ImportContent source="console-output/haneul-client-ptb-help.mdx" mode="snippet" />
...
Besides using existing [traditional PTB](/guides/developer/transactions/ptbs/prog-txn-blocks/) related concepts, we introduce a few new and important concepts for this command.
...
Publishing a package is one of the most important commands you need when working with Haneul. While the CLI has a standalone `publish` command, PTBs also support publishing and upgrading packages. One main difference is that with `haneul client ptb`, you must explicitly transfer the `UpgradeCap` object that is returned when creating a package, or destroy it with a call to [`make_immutable`](/guides/developer/packages/index.mdx). Here is an example on how to publish a Move project on chain using the `haneul client ptb` command. It makes a call to the `haneul::tx_context::sender` to acquire the sender and assigns the result of that call to the `sender` variable, and then calls the publish command. The result of `publish` is bounded to `upgrade_cap` variable, and then this object is transferred to the sender.
