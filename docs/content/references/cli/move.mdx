---
title: Haneul Move CLI
description: The Haneul CLI move command provides commands for working with Move source code directly from a terminal or console.
keywords: [ haneul move, haneul move help, move, move reference, move commands, haneul move commands, use move cli, cli move, create a move project, interact with move project, move project, move project cli ]
---

The Haneul CLI `move` command provides several commands for working with Move source code. A typical usage of `haneul move` is to compile and test the Move code, or to generate a new Move project by using `haneul move new project_name`, which creates the needed directories and the `Move.toml` file.

<ImportContent source="cli-check-install.mdx" mode="snippet" />

## Commands

Typing `haneul move --help` into your terminal or console displays the following information on available commands.

```
Usage: haneul move [OPTIONS] <COMMAND>
...
To create a new Move project that automatically adds the necessary dependencies in a `Move.toml` file, run `haneul move new [<PROJECT-NAME>]`.

```sh
$ haneul move new smart_contract_test
```
```sh
...
Use `haneul move build` at the root of your Move project to build the package.
...
Use `haneul move test` to run the tests in a Move package.
...
This example uses [`first_package`](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/first_package) Move package.

To get a summary of the test coverage, you must first run the `haneul move test --coverage` command, and then the `haneul move coverage summary --test` to get a summary of the test coverage in the example project.

```sh
$ haneul move test --coverage
```
```sh
INCLUDING DEPENDENCY Haneul
INCLUDING DEPENDENCY MoveStdlib
...
$ haneul move coverage summary --test
+-------------------------+
...
Each command has its own help section. For example `haneul move build --help` displays the following prompt:

```sh
$ haneul move build --help
```
```sh
Usage: haneul move build [OPTIONS]
