...
description: The Haneul Move CLI provides commands for working with Move source code.
---

The Haneul Move CLI provides several commands for working with Move source code. A typical usage of `haneul move` is to compile and test the Move code, or to generate a new Move project by using `haneul move new project_name`, which creates the needed directories and the `Move.toml` file.
...
This example uses the `nfts` example from [haneul_programmability folder](https://github.com/GeunhwaJeong/haneul/tree/main/haneul_programmability/examples) in the Haneul repository.

To get the a summary of the test coverage, you must first run the `haneul move test --coverage` command, and then the `haneul move coverage summary` to get a summary of the test coverage in the `nfts` Move example project.
