...
A gRPC API layer that exposes access to this store, enabling point lookups of historical data (for example, LedgerService).

### Archival Store {#archival-store}

A long-term storage system that holds checkpoint-indexed Haneul data, that is, Bigtable.
...
Single-writer objects are owned by one address. In Haneul, transactions affecting only single-writer objects owned by the same address might proceed with only a verification of the sender's address, greatly speeding transaction times. These are basic transactions. See Single-Writer Apps for example applications of this basic transaction model.
...
A template engine that enables onchain management of offchain representation (display) for a type. 
...
A transaction in Haneul is a change to the blockchain. This might be a basic transaction affecting only single-writer, single-address objects, such as minting an NFT or transferring an NFT or another token. These transactions might bypass the consensus protocol in Haneul.
...
A transfer is switching the owner address of a token to a new one through a command in Haneul. This is accomplished through the Haneul CLI client command line interface. It is one of the more common of many commands available in the CLI client.
