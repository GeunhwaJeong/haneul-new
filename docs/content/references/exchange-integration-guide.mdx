...
description: Learn the primary tasks necessary to integrate the HANEUL token with a cryptocurrency exchange. 
keywords: [ integrate haneul, HANEUL token, HANEUL, integrate with exchange, integrate with cryptocurrency exchange, decentralized exchange, dex, integration with dex, dex integration, haneul staking, staking ]
---
...
The JSON-RPC API used in some examples in this guide is deprecated. For new integrations, use the gRPC or GraphQL APIs. See the [Haneul API documentation](/references/haneul-api) for migration guidance.

:::

## Requirements to configure a HANEUL integration {#requirements-haneul-integration}
...
## Configure a Haneul full node {#configure-full-node}
...
### Install a Haneul full node using Docker {#full-node-docker}
...
1.  Download the genesis.blob file:
    ```sh
    $ wget https://github.com/GeunhwaJeong/haneul-genesis/blob/main/mainnet/genesis.blob
    ```
    :::info
    
    Replace `mainnet` in the URL with the desired target branch if necessary. For example, `https://github.com/GeunhwaJeong/haneul-genesis/blob/main/testnet/genesis.blob` for Testnet.
...
## Install a Haneul full node from source {#install-full-node-source}
...
## Set up Haneul addresses {#setup-haneul-addresses}
...
## HANEUL Balance transfer {#haneul-balance-transfer}
...
## Haneul API operations for transfers {#api-operations-transfers}

Haneul supports the following API operations related to transferring HANEUL between addresses:

- [haneul_transferObject](/haneul-api-ref#haneul_transferObject)
...
- [haneul_payAllHaneul](/haneul-api-ref#haneul_payAllHaneul)
...
- [haneul_payHaneul](/haneul-api-ref#haneul_payHaneul)
...
- [haneul_pay](/haneul-api-ref#haneul_pay)
...
- [haneul_transferHaneul](/haneul-api-ref#haneul_transferHaneul)
...
Refer to Haneul Signatures for more details on signature validity requirements.

## HANEUL Staking {#haneul-staking}
...
- `request_add_stake_with_locked_coin`
  Add user stake to a validator's staking pool using a locked HANEUL coin.
