---
title: Haneul-Related Research Papers
sidebar_label: Research Papers
description: Research papers that are relevant to Haneul and that one or more Haneul team members have co-authored.
keywords: [ research papers, haneul papers, academic papers, research, haneul research, haneul publications, published papers, published research ] 
pagination_prev: null
---

This document contains a list of research papers that are relevant to Haneul and that one or more Haneul team members have co-authored.
...
but could be integrated in the future. Start with the [Haneul Smart Contract Platform](/doc/haneul.pdf) white paper, which contains
our latest design inspired by previous works below.

<div class="bg-haneul-blue-light dark:bg-haneul-gray-95 p-8 rounded-lg border border-solid border-haneul-blue-dark">

<p class="text-haneul-gray-80 text-xl font-bold italic">Latest</p>
...
- **Relevance:** Provides a performant leader election algorithm for partially-synchronous consensus protocols such as Bullshark. This work informed earlier designs for shared object support in Haneul before the adoption of Mysticeti.
- **Summary:** Advances in blockchains have influenced the State-Machine-Replication (SMR) world and many state-of-the-art blockchain-SMR solutions are
  ...
  Leader-rotation is that it cannot rely on consensus to determine a leader, since consensus itself needs a leader. Carousel uses the available onchain
...
- **Relevance:** Provides a partially-synchronous consensus protocol running over Narwhal. Bullshark was a predecessor to Mysticeti and informed the consensus design of Haneul before Mysticeti was adopted.
...
- **Relevance:** FastPay describes the core protocol at the heart of Haneul.
- **Summary:** FastPay allows a set of distributed validators, some of which are Byzantine, to maintain a high-integrity and availability
  ...
  side-infrastructure to support retail payments in fiat currencies. This is not the protocol Haneul uses, yet it proposes the basic safety mechanism
  that Haneul extends. FastPay is based on Byzantine Consistent Broadcast as its core primitive, foregoing the expenses of full atomic commit channels
...
blockchain technology has led to research on DAG-based consensus. This paper explores ideas pioneered by Carousel on providing Leader-Utilization
and presents HammerHead as an improvement over earlier leader-based DAG consensus protocols such as Bullshark. Unlike Carousel, which is built with a chained
...
- **Relevance:** Narwhal and Tusk provided the foundational DAG-based mempool and consensus design that informed earlier versions of Haneul's shared-object support. Haneul now uses Mysticeti as its consensus protocol.
...
- **Publication:** Not published
- **Relevance:** Less related to Haneul than the other papers, and the paper is in its early stages. It presents an algorithm to strengthen proof-of-Stake systems (like Haneul). The paper is, however, theoretical and not on our roadmap.
...
- **Relevance:** Less related to Haneul than the other papers, this provides a way to test implementations of consensus systems, such as Tusk and Bullshark.
...
- **Publication:** Not published yet (under submission)
- **Relevance:** Extends the FastPay design to support objects (rather than accounts), what Haneul actually uses. An additional contribution of this paper is
  to add strong privacy to FastPay transactions (but Haneul does not plan to do this).
- **Summary:** We introduce Zef, the first Byzantine-Fault Tolerant (BFT) protocol to support payments in anonymous digital coins at arbitrary scale. Zef
  ...
  sharded validators. Zef further introduces opaque coins represented as offchain certificates that are bound to user accounts. In order to hide the face
