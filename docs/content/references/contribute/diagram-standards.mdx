...
Excalidraw is the required tool for all diagrams with more than 5 nodes. It runs in the browser at [excalidraw.com](https://excalidraw.com) and produces clean SVG and PNG exports that conform to brand standards.
...
Because Haneul documentation is maintained as Docs as Code in GitHub, Mermaid.js is a strong choice for diagrams that change frequently or need to stay in sync with code. As plain text, Mermaid diagrams are easy to version control, review in pull requests, and update without a separate design tool.
...
Apply the following frontmatter to every Mermaid diagram to align colors with the Haneul brand palette:
...
Use Black, Gray 500, and Haneul Blue 500 to encode emphasis level: Black for primary elements, Gray 500 for secondary elements, and Haneul Blue 500 for tertiary elements and all arrows. If more than 3 fill colors are required to maintain clarity, expand into the Gray extended palette first, then the Blue extended palette. See the extended palette at [https://live.standards.site/haneul-media-kit/haneul-color](https://live.standards.site/haneul-media-kit/haneul-color).

### Typography

The Haneul Brand Kit specifies TWK Everett as the primary typeface. Inter is the approved backup for use when TWK Everett is unavailable.

For Excalidraw diagrams, use the Nunito font family, which is the closest available match. For all other tooling, download and use Inter from the Haneul Brand Kit site.
...
Use Black, Gray 500, and Haneul Blue 500 to encode emphasis level across all elements:

- **Primary elements:** Black fill, white label text.
...
All arrows use Haneul Blue 500 as the stroke color. The table below defines the 3 arrow styles and their C4 relationship types.
...
- Meet WCAG AA minimum contrast for all text and aim for AAA.
- Do not rely on color alone to distinguish elements. Supplement color with shape, arrow style, or label.
- Prefer white text on Black (`#000000`) or Gray 500 (`#6C7584`) fills. Avoid white text on Haneul Blue 500 for normal-size text because the contrast ratio of 3.31:1 fails AA for normal text.
...
- Step numbers are placed above the arrow in Haneul Blue 500 using small ALL CAPS labels.
