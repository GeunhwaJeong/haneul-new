...
description: Extensions to the Haneul docs rendering engine.
mdx:
  format: md
---

Haneul Docs uses the MDX format for its network documentation, which allows [JSX in markdown content](https://mdxjs.com/). Haneul uses features provided by Docusaurus, plugins from its community, and custom plugins in an effort to improve reader experience. None of these features are required in your markdown contributions to the Haneul documentation, but the Haneul community might include them if it improves experience.
...
Include the following in the frontmatter for pages that use the `{@include:}` and `{@inject:}` directives. This is not custom, but necessary for proper rendering of those pages.
...
---
title: A Guides
description: A guide on how to do this thing in Haneul.
effort: medium
---
```
...
Use the `Unsafelink` component to create a link that skips the link checker.

### When to use

The site is configured to throw an error when it comes across a broken link at build. Sometimes it's necessary to create a link to an auto-generated page, but this throws an error because the page does not exist when the link checker runs. Use this component to bypass that check.

:::info

The `onBrokenMarkdownLinks: "warn"` setting remains at `warn` so that starting the site locally is not blocked by broken links. This setting does not stop thrown errors at build.

:::

### How to use

Instead of a markdon link or anchor tag, pass the `href` value to the `UnsafeLink` component.
...
`Tabs` and child `TabItem` elements allow for the segmentation of content so that users see only the content that is relevant to them. Haneul documentation uses the default Docusaurus tab components. Check the [official documentation](https://docusaurus.io/docs/next/markdown-features/tabs) for all available options.  
...
1. Check your page in the Vercel preview of your PR (`haneul-core` preview link in Vercel comment) to make sure the content displays as expected.
...
Snippets are short pieces of reusable text, such as tips, tab components, or other plaintext. Snippets are not used for code snippets; see code inject for embedding code. The `docs/content/snippets` folder contains all the reusable content. Pages in that directory do not appear in the ToC and are hidden from search engines. Upon build, the process deletes the folder so the content doesn't get rendered to the final site as it's own page.
...
There must be an empty line above and below the `{@include} line for it to render properly. 

You must also add the following frontmatter to the page that includes the `{@include}` statement:
...
1. Check each page in the Vercel preview of your PR (`haneul-core` preview link in Vercel comment) to make sure the content displays. If the relative link you provide does not resolve correctly, the page displays error text instead of the expected content.

:::warning

Do not use links relative to the file structure in snippets (for example, `../concepts/file.mdx`). If your snippet requires a link internal to the Haneul docs site, use the relative HTML path instead (`/concepts/file`). 

:::

## Code inject

Haneul documentation uses a custom Docusaurus plugin to include code from the Haneul repo in its pages. 

Code inject formats code in a code block with syntax highlighting. Code inject can only be used for files located in the current repo, as they must be referenced via relative path.

### When to use

In an active repo, source code changes frequently. If you copy and paste that source code in a document, then the two sources can diverge quickly and drastically. In Haneul Docs, you can use the `{@inject}` directive to make a copy of the source when the site builds (merge PR), ensuring the source used in the content always matches the source code in the repo. If the code you inject is tested, this directive ensures the code in your document is also tested.
...
To include the entire source file, use the `{@inject: ...}` directive with a path to the file, based on the Haneul repo root. For example, to include the source file at `haneul/examples/move/hero/sources/example.move`, write:

``
{@inject: examples/move/hero/sources/example.move`
```

The file location is automatically used as the title for the codeblock.

You must also add the following frontmatter to the page that includes the `{@inject}` statement:
...
There must be an empty line above and below the `{@inject} line for it to render properly. 
...
- `highlight`: Highlight a single line out of the injected code. Use a word (such as a variable name) in the line you want highlighted. 

For example, `{@inject: examples/move/example.source#module=example::example noComments noTests noTitle singleSpace}`

## Summary and details 

For long portions of text or code that can be omitted, but should be available for readers looking for it, you can use the `<details>` syntax to create a collapsible component. The collapsible component takes up less space on the page, making it easier for readers to skim the page's content. 
...
Use `<details>` opening and closing tags to create the component, and the `<summary>` opening and closing tags to display the text shown when the component is closed:
...
There must be an empty line above and below the both the opening and closing `<summary>` and `<detail>` tags for it to render properly. 
...
The import statement can go anywhere, but it's typically added to the top of the page. Then use the component by providing an array of YouTube video IDs. Haneul videos should not have this problem, but be aware that videos might not have thumbnails like in the example.
...
It is recommended to use an actual ordered list when documenting processes. Each list item step should consist of a single instruction for the user. On occasion, however, you might want to have a group of instructions for each step, making it impractical for a standard ordered list. The step titles feature enables the use of numbered steps and sub-steps in a heading, where you can then group a set of instructions in a single step.

### When to use

Use when you detail a long process with multiple technologies, or a process that otherwise requires lengthy contextual content. Do not create a verbose ordered list or one where list item #1 requires several pages of depth before the reader gets to list item #2. If you find that you're creating such a process list, then it might be a good candidate for step headings. Having steps in the title of longer processes helps readers recall where they left off if they don't complete the task set, and enables them to easily navigate back using the ToC. 
