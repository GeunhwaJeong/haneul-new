...
| **Always lowercase**   | archival node, bridge node, dApp, full node, kiosk (when referring to an instance), object, oracle, recovery passphrase (mnemonic), soulbound, Haneul framework, Haneul object, validator node, wallet | Use lowercase when referring to general concepts rather than branded terms   |
...
- [Object and Package Versioning](/concepts/versioning): Versioning provides the ability to upgrade packages and objects on the Haneul network.
