...
This document defines the styles, vocabulary usage, and content formatting for the Haneul documentation.
...
Product names are proper nouns. Capitalize all words of a product name. When referring to a product, use only the product name without "the". When referring specifically to a Haneul wallet, specify the name of the wallet such as Slush Wallet. Use 'wallet' generically when referring to the concept of a wallet.
...
```mdx
<ImportContent source="src/lib/walrus.ts" mode="code" org="HaneulLabs" repo="walrus-sdk-relay-example-app" />
```
...
### Haneul docs {#prerequisites-haneul}

On Haneul documentation pages, use the following tab component for prerequisites:
