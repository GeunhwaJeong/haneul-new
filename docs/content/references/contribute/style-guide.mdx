...
| ---------------------- |---------------------------------------------- | -------------- |
| **Always capitalized** | [Proper nouns](#proper-nouns), [Product names](#product-names), DeepBook Indexer, DeepBookV3, Devnet, ID, Localnet, Kiosk (when referring to the standard), Mainnet, Mysticeti, Operation Cap, Haneul, Haneul CLI, Haneul Client PTB CLI, Haneul Closed-Loop Token / Closed-Loop Token, Haneul dApp Kit, Haneul Explorer, HaneulJSON, Haneul Keystore, Haneul Keytool, HaneulLink, Haneul Object Display, HaneulPlay0X1, HANEUL, HANEUL token, Testnet, Wallet Standard, Web2, Web3, zkSend SDK | Use capitalization consistently for branding and product-specific references |
| **Always lowercase**   | archival node, bridge node, dApp, full node, kiosk (when referring to an instance), object, oracle, recovery passphrase (mnemonic), Haneul framework, Haneul object, validator node, wallet | Use lowercase when referring to general concepts rather than branded terms   |
...
- [Object and Package Versioning](/docs/concepts/versioning.mdx): Versioning provides the ability to upgrade packages and objects on the Haneul network.
