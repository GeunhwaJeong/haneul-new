...
> You can mint non-fungible tokens (NFTs) using your Haneul Wallet. To view an NFT after you mint it, click the NFTs tab of your wallet.
...
- **Indicative –** Haneul network explorers display transactions on the network.
- **Imperative –** Open a Haneul network explorer to view transactions on the network.
- **Subjunctive –** We suggest that you view your transaction in a Haneul network explorer after you complete a transfer.
...
> You used to could view transaction history in Haneul Explorer.

**Rather than:**

> We used to could view transaction history in Haneul Explorer.
...
_Get Started with Haneul_ is better, but users want to get started with a specific task or user journey with Haneul. Instead of _Get Started with Haneul_, describe the specific task or journey, such as _Create a Haneul Full Node_ or _Haneul Validator Guide_. Use Get Started as a heading on the Documentation landing page to categorize tasks for new users.
...
The Build section of the documentation includes topics about: Building with Haneul, Using the CLI to Start a Network, Creating Smart Contracts, Haneul Tutorial, and Haneul Examples.
...
Haneul Explorer used to support the following browsers:
...
Always use relative links when linking to topics on docs.haneul.io.
...
> You can mint an NFT directly from your Haneul Wallet.
...
For consistency, Haneul docs must use the following terms and phrases with the displayed capitalization (unless sentence structure determines otherwise):

{/***Full node** A node running on the Haneul network. Written as Full node in a sentence.*/}

- **dApp:** A decentralized app that runs on a decentralized blockchain network.
- **Devnet:** A version of the Haneul network for development and experimentation. 
- **Testnet:** A version of the Haneul network for testing. 
- **Mainnet:** The Haneul production network that processes public user transactions. 
- **Localnet:** A local Haneul network. Define it parenthetically on first use in a topic: “a local Haneul network (Localnet).”
- **Layer 1:** Layer 1 refers to a base network, such as Bitcoin, Ethereum, or Haneul, and its underlying infrastructure. Layer 1 blockchains can validate and finalize transactions without the need for another network.
- **recovery passphrase (mnemonic):** The 12-word passphrase generated for a Haneul address. The parenthetical is optional, but if used then subsequent uses should just use "mnemonic".
- **open source:** Use 2 words without a hyphen.
- **Haneul:** Haneul is a boundless platform to build rich and dynamic on-chain assets from gaming to finance.
- **HANEUL token:** The HANEUL token is the Haneul platform's native asset.
- **HANEUL:** The unit of HANEUL tokens. For example, the list price for this NFT is 10 HANEUL.
