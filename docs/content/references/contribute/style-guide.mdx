...
title: Style Guide
slug: /style-guide
description: All contributions to the Haneul documentation must adhere to the style guide.
...
This document defines the styles, vocabulary usage, and content formatting for Haneul documentation. Entries are in alphabetical order. A style guide is never finished; expect continued iterations to add additional styles, additional information to existing styles, and infrequently a change to an existing style.
...
> ✅ You can view the transaction history in the Haneul Explorer.

> ❌ We can view the transaction history in the Haneul Explorer.
...
> ✅ You must install Cargo, Rust, Docker, and the Haneul CLI to create a Haneul node.
...
For consistency, the Haneul documentation must use the following terms and phrases with the indicated capitalization (unless sentence structure determines otherwise). This list is not exhaustive and will be updated over time. 

|     **Category**       |             **Words and terms**               |    **Notes**   |
| ---------------------- |---------------------------------------------- | -------------- |
| **Always capitalized** | [Proper nouns](#proper-nouns), [product names](#product-names), names of example dApps (Coin Flip, Blackjack, etc), Archival Store and Service, Archival Store, Archival Service, DeepBook Indexer, DeepBookV3, Devnet, ID, Localnet, Kiosk (when referring to the standard), Mainnet, Mysticeti, Operation Cap, Haneul, Haneul CLI, Haneul Client PTB CLI, Haneul Closed-Loop Token / Closed-Loop Token, Haneul dApp Kit, Haneul Explorer, HaneulJSON, Haneul Keystore, Haneul Keytool, HaneulLink, Haneul Object Display, HaneulPlay0X1, HANEUL, HANEUL token, Testnet, Wallet Standard, Web2, Web3, zkSend SDK | Use capitalization consistently for branding and product-specific references |
| **Always lowercase**   | casual history, casual order, certificate, dApp, epoch, equivocation, eventual consistency, finality, gas, genesis, kiosk (when referring to an instance), object, oracle, recovery passphrase (mnemonic), smart contract, soulbound, Haneul framework, Haneul object, total order, transaction, transfer, validator, wallet | Use lowercase when referring to general concepts rather than branded terms   |
| **Never hyphenated**   | key pair, layer 1, open source, use case                  | Keep these words separate, no hyphen                                         |
| **Always hyphenated**  | depth-first search, multi-writer objects, off-chain, off-device, on-chain, on-device, peer-to-peer, proof-of-stake, single-writer objects  | Maintain hyphen for clarity and correctness  |
...
- Capitalize _Haneul Full Node_ when referring to the official software binary/package that Haneul distributes.
    “Download and run the Haneul Full Node from GitHub to connect to Mainnet.”
...
Product names are proper nouns. Capitalize all words of a product name. When referring to a product, use only the product name without "the". When referring specifically to a Haneul wallet, use Haneul Wallet or Ethos Wallet and not just wallet. Users likely have multiple wallets, and we want to make it clear which wallet. Use wallet generically when referring to the concept of a wallet.

> There are several types of wallets to choose from.

> Never share the recovery passphrase for your wallet with anyone.

> The Haneul network supports the following wallets:
> - Haneul Wallet
> - Ethos Wallet
> - Coinbase Wallet
...
> Example: You can mint non-fungible tokens (NFTs) using your Haneul Wallet. To view an NFT after you mint it, click the NFTs tab of your wallet.
...
A better option is **Get Started with Haneul**, but even this is still too broad. Readers want guidance for a specific task or journey. Instead, use precise titles such as **Create a Haneul Full Node** or **Install Haneul Tooling**. These tell the reader exactly what they will learn or accomplish.
...
> ✅ Correct usage: 
> Page 1: Page title "Haneul Gas Profiling" with heading "Environment configuration"
> Page 2: Page title "Haneul Indexing" with heading "Environment configuration"

> ❌ Incorrect usage: 
> Page 1: Page title "Haneul Gas Profiling" with heading "Environment configuration"
> Page 2: Page title: "Haneul Features" with heading "Haneul gas profiling"
...
> ##### Section heading: Install `haneulup`

> Body content: Install `haneulup` using the command: ...
...
Haneul Explorer supports the following browsers:
...
- CLI commands when used within a sentence: If using the suggested location, type `export PATH=$PATH:~/haneul` and press Enter.
...
To get the latest version of the Haneul Wallet extension:

1. Open Google Chrome.
1. Click **Extensions**, then click **Manage Extensions**.
1. Click **Details** for the Haneul Wallet extension, then click **View in Chrome Web Store**.
...
Always use full, relative links when linking to topics on [docs.haneul.io].
...
> To learn more, see [Examples of Haneul Smart Contracts](/guides/developer/app-examples.mdx).
...
> Before you install Haneul, make sure to install the [prerequisites](/guides/developer/getting-started/haneul-install.mdx#prerequisites).
...
You can also just refer to a topic by title where it makes sense. See Installing Haneul for more information.
...
Running `target/release/haneul-node`
...
INFO: Haneul node is now running
```
...
You must install Rust before you can build Haneul from source.
