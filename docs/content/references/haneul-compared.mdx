...
description: Learn how Haneul's unique architecture compares with other blockchain networks. 
keywords: [ haneul compared to other chains, haneul compared to other blockchains, haneul compared to other networks, how haneul compares to other networks, haneul architecture, unique features of haneul, what makes haneul different, haneul key features ]
---

This page summarizes how Haneul compares with other blockchains and is intended for potential adopters of Haneul to decide whether it fits their use cases. See How Haneul Works for an introduction to the Haneul architecture.
...
This doesn't mean that Haneul as a platform never orders transactions with respect to each other, or that it allows owners to affect only their owned microcosm of objects. Haneul also processes transactions that have an effect on some shared state in a rigorous, consensus-ordered manner. See the [State-of-the-art consensus](#state-of-the-art-consensus) section for details on the consensus engine.
...
In summary, Haneul provides significant performance and usability enhancements. It introduces an innovative fastpath consensus for simple send transactions and utilizes a state-of-the-art consensus protocol that leads the industry with its low latency and high throughput.