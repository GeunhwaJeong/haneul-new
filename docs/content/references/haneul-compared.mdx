---
title: Comparison
slug: /haneul-compared
---

This page summarizes how Haneul compares with existing blockchains and is intended for potential adopters of Haneul to decide whether it fits their use cases. See How Haneul Works for an introduction to the Haneul architecture.
...
- Haneul's variant of Move and its object-centric data model make composable objects/NFTs possible
...
## Haneul's approach to validating new transactions {#validating}
...
Haneul further expands this approach to more involved transactions that may explicitly depend on multiple elements under their sender's control, using an object model and leveraging Move's strong ownership model. By requiring that dependencies be explicit, Haneul applies a "multi-lane" approach to transaction validation, making sure those independent transaction flows can progress without impediment from the others.

This doesn't mean that Haneul as a platform never orders transactions with respect to each other, or that it allows owners to affect only their owned microcosm of objects. Haneul also processes transactions that have an effect on some shared state in a rigorous, consensus-ordered manner. They're just not the default use case. See the [State-of-the-art consensus](#state-of-the-art-consensus) section for details on the Narwhal and Bullshark consensus engine.
...
1.  Sender collects a Byzantine-resistant-majority of these votes into a _certificate_ and broadcasts it to all Haneul validators.
...
## Where Haneul excels {#where-haneul-excels}
