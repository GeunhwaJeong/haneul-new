---
title: What is Permissioned Asset Standard?
description: Learn how the Permissioned Asset Standard (PAS) enforces restricted asset movement on Haneul through Accounts, Policies, and programmable approval logic.
keywords: [ PAS, Permissioned Asset Standard, Account, Policy, Request, approval, permissioned assets, closed-loop ]
---

On Haneul, any object with the `store` ability can be freely transferred by its owner. `Balance` and `Coin` both have `store`, meaning if you hold them, you can send them anywhere on the network with no restrictions.
...
All objects use derived addresses (`haneul::derived_object`), making them deterministic and queryable without onchain lookups.
...
PAS uses Haneul [Address Balances](/onchain-finance/asset-custody/address-balances/using-address-balances):
...
PAS provides the enforcement primitives (closed loop, type-safe approvals, atomic resolution), but it does not enforce your business or compliance policy. Deciding who can transfer and under what conditions is your responsibility, implemented in your approval witnesses before calling `request.approve()`. Because the approval witnesses are the only gate on asset movement, audit them carefully: a flawed or overly permissive witness undermines the entire closed loop. See [access control](/develop/security/best-practices#access-control).
