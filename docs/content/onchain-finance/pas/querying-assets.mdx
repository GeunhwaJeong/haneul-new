...
description: How to read PAS-managed balances for a wallet address using the PAS SDK and standard Haneul APIs.
keywords: [ PAS, query, balance, assets, derived address, explorer, wallet, portfolio ]
---

The Permissioned Asset Standard (PAS) does not expose dedicated read APIs in the SDK. This is by design. PAS stores assets in standard Haneul objects with deterministic addresses, so you query them using the same RPC methods you use for any other address-owned data (for example, balance queries and owned object queries).

A wallet address on Haneul can hold 2 kinds of balances:
...
const client = new HaneulGrpcClient({ network: 'mainnet' }).$extend(pas());
...
An Account can hold asset types that a PAS policy does not govern. For example, `Balance<HANEUL>` might end up in an Account through a direct `balance::send_funds` call. When you call `getAllBalances`, these show up alongside policy-managed assets.
