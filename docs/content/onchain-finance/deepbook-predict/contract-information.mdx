...
This page contains the current public integration targets for DeepBook Predict on Haneul Testnet. These values come from the `predict-testnet-4-16` branch of the [DeepBookV3 Predict package](https://github.com/GeunhwaJeong/deepbookv3/tree/predict-testnet-4-16/packages/predict).
...
|-----------|-------|
| Network | Testnet |
| Public server | `https://predict-server.testnet.haneul-labs.com` |
...
| Source branch | [`predict-testnet-4-16`](https://github.com/GeunhwaJeong/deepbookv3/tree/predict-testnet-4-16/packages/predict) |
...
The public server base URL is `https://predict-server.testnet.haneul-labs.com`. Use it to retrieve render-ready market, vault, portfolio, and history data.

The following example queries the market state for a Predict object:

```bash
$ curl https://predict-server.testnet.haneul-labs.com/predicts/0xc8736204d12f0a7277c86388a68bf8a194b0a14c5538ad13f22cbd8e2a38028a/state
```
...
When a UI needs lower-latency oracle state than the indexed server provides, use Haneul checkpoint or event streaming. Filter by the current Predict package ID and watch these event types:
...
| Area | Source |
|------|--------|
| Core shared object | [`packages/predict/sources/predict.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/packages/predict/sources/predict.move) |
| Manager account model | [`packages/predict/sources/predict_manager.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/packages/predict/sources/predict_manager.move) |
| Registry and admin entry points | [`packages/predict/sources/registry.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/packages/predict/sources/registry.move) |
| Oracle state machine | [`packages/predict/sources/oracle.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/packages/predict/sources/oracle.move) |
| Vault accounting | [`packages/predict/sources/vault/vault.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/packages/predict/sources/vault/vault.move) |
