...
Margin trading amplifies both gains and losses. Before using DeepBook Margin, you should understand the risks involved. This guide explains the key risks and provides concrete examples to help you make informed decisions.
...
### Example: Getting liquidated on HANEUL/USDC

Let's walk through a concrete example using the HANEUL/USDC trading pair, which has a **Liquidation Risk Ratio of 1.1**.

**Opening position:**
- You deposit 100 USDC as collateral
- You borrow 400 USDC and open a 5x long position on HANEUL at 1.50 USDC per HANEUL
...
| HANEUL Price (USDC) | HANEUL Value (USDC) | Total Assets (USDC) | Risk Ratio | Status |
|------------------|------------------|---------------------|------------|--------|
...
**What happens at 1.275 USDC per HANEUL:**
- Your risk ratio hits 1.1 (the liquidation threshold for HANEUL/USDC)
...
- **Long positions**: Price moves down (you borrowed USDC to buy HANEUL, and HANEUL drops)
- **Short positions**: Price moves up (you borrowed HANEUL to sell for USDC, and HANEUL rises)
...
These onchain protections do not remove your own data-hygiene responsibility. Monitor oracle freshness and be aware that the prices, risk ratios, and liquidation thresholds shown in a UI, wallet, or indexer can lag the onchain state. Never make margin decisions on stale or cached offchain data. Also understand the limits of liquidation automation: keepers and TPSL orders are best-effort and can be delayed or fail during congestion or extreme volatility, so do not assume a position always closes at your expected price. See [oracle and offchain data](/develop/security/best-practices#oracle-and-offchain-data).
