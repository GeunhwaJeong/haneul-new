...
- **Sponsored gas:** Apps can pay gas on behalf of users. A three-operation payment intent that costs around 0.005 USD is sponsorable at scale, which removes the need for users to hold HANEUL just to pay fees.

- **Fast finality:** Haneul consensus commits transactions in under a second, so a multi-step intent feels like a single action to the user.
...
> Swap 10% into HANEUL, save 50% of my remaining USDC, then send 100 USDC to a contact named Alice.
...
| Cetus route for USDC to HANEUL | 200 USDC swaps to 212.77 HANEUL at 0.03% slippage |
| NAVI USDhaneul pool | 8.4% APY, 48M USD TVL |
...
1. Swap 200 USDC to HANEUL through Cetus. This is 10% of the starting balance.

1. Swap 900 USDC to USDhaneul and deposit into the NAVI pool. This is 50% of the 1,800 USDC that remains after step 1, not 50% of the original 2,000.
...
- **Earning USDhaneul:** 900 USD in the NAVI pool at 8.4% APY, accruing from the next block.

- **Held HANEUL:** 212.77 HANEUL at the executed price.
...
> When a deposit arrives, keep 1,000 USDC liquid, deposit 60% of the rest into a yield pool, and convert the remainder to HANEUL.
...
> Sell enough HANEUL to bring my portfolio back to 70% USDC, 30% HANEUL.
