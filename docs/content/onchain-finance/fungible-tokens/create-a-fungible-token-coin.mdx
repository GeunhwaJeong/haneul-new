...
The Coin standard uses the [`haneul::coin`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/coin.md) module to define fungible assets called coins that have `key` and `store` abilities. Coins support wrapping and public transfer functionality.

The `Coin<T>` type represents open-loop fungible tokens, denominated by their type parameter `T`, which is also associated with metadata that applies to all instances of `Coin<T>`. The `haneul::coin` module exposes an interface over `Coin<T>` that treats it as fungible, meaning that a unit of `T` held in one instance of `Coin<T>` is interchangeable with any other unit of `T`, similar to how traditional fiat currencies operate.

The network's native coin, HANEUL, uses the Coin standard.
...
[View the full example](https://github.com/GeunhwaJeong/haneul-move-bootcamp/blob/main/I1) or learn more about the [Coin Standard](https://docs.haneul.io/onchain-finance/fungible-tokens/coin).

## Managing coins

Haneul supports two ways to hold fungible assets: address balances and coin objects. Address balances are the recommended default for most use cases. Coin objects remain available for scenarios that require explicit object-level control.

## Address balances

Address balances provide a canonical balance for each coin type tied directly to a Haneul address. Funds sent through `send_funds` merge automatically into a single balance per coin type, with no object management required.
...
- Gas payment directly from HANEUL address balances.
...
See [Address Balances Migration Guide](/onchain-finance/asset-custody/address-balances/using-address-balances) for the full reference on using address balances, including withdrawal patterns, gas payment, sponsored transactions, and balance queries.

## Coin objects

Coin objects are address-owned objects that represent a specific amount of a given coin type. Before address balances, all funds on Haneul were held as coin objects. They remain fully functional and are not deprecated.
...
You can convert coin objects into an address balance at any time by calling `coin::send_funds` with your own address as the recipient. See [Converting Existing Coins to Address Balances](/onchain-finance/asset-custody/address-balances/using-address-balances#converting-existing-coins-to-address-balances) for instructions.
...
Gas smashing applies only to `Coin<HANEUL>` objects, because HANEUL is the only coin type accepted for gas payment.

You can provide as many coins as you want (up to a maximum defined in the protocol configuration). Gas smashing is a useful tool when you need to consolidate HANEUL coins as part of a transaction.
...
See [Gas Smashing](/develop/transaction-payment/gas-smashing) for more details.
...
If your transactions use coin objects (or any other owned objects) as inputs, the original concurrency constraints apply. Each owned object has a version, and concurrent transactions cannot use the same mutable owned object version. Wait for the first transaction to finish, use independent owned objects, or combine related operations into one PTB.
