...
Coin uses `CoinMetadata` while Currency uses `Currency` object. Fungible tokens created on Haneul using the Coin or Currency standard are referred to as coins.
...
See [Haneul Tokenomics](/develop/haneul-architecture/tokenomics-overview) to learn more about the HANEUL native currency and its use on the network.
