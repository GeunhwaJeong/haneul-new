...
description: What changes with address balances on Haneul and what action to take for existing integrations.
keywords:
  [
    address balances,
    migration,
    ...
    compatibility coin reservations,
    SIP-58,
  ]
---

Address balances introduce a canonical balance system for fungible assets tied to Haneul addresses. This supplements the `Coin<T>` model with direct address-owned balances, simplifying transaction construction and eliminating coin selection complexity.

For the full specification, see [SIP-58: Haneul Address Balances](https://github.com/haneul-foundation/sips/blob/main/sips/sip-58.md). For implementation details on sending, withdrawing, and paying gas from address balances, see [Using Address Balances](/onchain-finance/asset-custody/address-balances/using-address-balances).
...
The `haneulx_getBalance` and `haneulx_getAllBalances` methods now return a `fundsInAddressBalance` field. The `totalBalance` field includes both coin objects and address balance funds:
...
The `balance` and `balances` fields on address types now include `addressBalance`, `coinBalance`, and `totalBalance` fields. See [GraphQL Reference - `IAddressable.balance`](/references/haneul-api/haneul-graphql/beta/reference/types/interfaces/iaddressable) for the full schema.
...
The Haneul framework provides a helper for this process:
...
Each event contains the target address, a balance type, an operation (`Split` for withdrawal, `Merge` for deposit), and the amount. See [haneul-types/src/effects/mod.rs](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/effects/mod.rs) for the full API.

## Backward compatibility

### Existing contracts

Contracts that accept `Coin<T>` or `Balance<T>` remain callable without changes. The `redeem_funds` functions (in the `haneul::coin` and `haneul::balance` modules respectively) convert withdrawals to the expected types within the PTB. For example:

```
inputs: [withdrawal<0x2::haneul::HANEUL>(100)]
commands:
0: 0x2::coin::redeem_funds(inputs(0))
1: 0x2::transfer::public_transfer(result(0))
```

### Compatibility coin reservations for legacy clients

A JSON-RPC compatibility layer presents compatibility coin reservations to clients that cannot upgrade. A compatibility coin reservation is a synthetic object reference that reserves address balance funds for backward compatibility with clients that still expect coin object references. It is not a persisted `Coin<T>` object.

These reservations exist primarily to preserve basic functionality for older clients that expect coin object references. Do not treat them as normal persisted coin objects. For new development, use address balances directly, as described in [Using Address Balances](/onchain-finance/asset-custody/address-balances/using-address-balances).

When a legacy client supplies a compatibility coin reservation as gas payment, the network resolves it as a withdrawal from the address balance rather than smashing it into a coin. And gas smashing applies depending on the position of the coin reservation in the list of gas payments. For more information, see [Gas Smashing](/develop/transaction-payment/gas-smashing).
