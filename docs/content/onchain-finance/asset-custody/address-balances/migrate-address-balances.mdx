...
Address balances introduce a canonical balance system for fungible assets tied to Haneul addresses. This supplements the `Coin<T>` model with direct address-owned balances, which simplifies transaction construction and eliminates coin selection complexity.
