...
description: Address balances introduce a canonical balance system for fungible assets tied to Haneul addresses. Learn how to send, withdraw, and query address balances.
keywords:
  [
    address balances,
    coin objects,
...
Address balances provide a canonical balance for each coin type tied directly to a Haneul address. Funds sent through `haneul::coin::send_funds` and `haneul::balance::send_funds` merge automatically into a single balance per coin type, with no object management required. For the full specification, see [SIP-58: Haneul Address Balances](https://github.com/haneul-foundation/sips/blob/main/sips/sip-58.md).
...
To send funds using [Haneul CLI](/references/cli/ptb):
...
| `--amount` | Amount to send in GEUNHWA. |
...
The Haneul framework provides `send_funds` in both the `balance` and `coin` modules:
...
Learn more at [haneul-framework/sources/balance.move](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/balance.move#L102)) and [haneul-framework/sources/coin.move](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/coin.move#L176).
...
For coin types other than HANEUL, pass the `type` parameter:
...
For explicit control over address balance withdrawals, use [`tx.withdrawal()`](https://haneul-typescript-docs-git-mh-address-balance-docs-haneullabs-labs.vercel.app/haneul/transactions/reference#address-balance-withdrawals). The withdrawal must be redeemed through `redeem_funds` to produce a usable `Coin<T>` or `Balance<T>`:
...
Use [`FundsWithdrawalArg`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/transaction.rs) to withdraw from an address balance in Rust. The helper `FundsWithdrawalArg::balance_from_sender(amount, balance_type)` provides a convenient constructor:
...
Learn more at [haneul-types/src/programmable_transaction_builder.rs](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/programmable_transaction_builder.rs).
...
When you pay gas from an address balance, you do not select or provide coin objects, so there is no gas coin selection or merge workflow. As a result, [gas smashing](/develop/transaction-payment/gas-smashing) does not apply, and the network resolves the gas fee as a withdrawal from your address balance.
...
In Rust, set `gas_data.payment` to an empty vector and use [`ValidDuring`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/transaction.rs) expiration:
...
console.log(balance.coinType); // e.g. "0x2::haneul::HANEUL"
```
...
Use [`GetBalance`](/references/fullnode-protocol#haneul-rpc-v2-GetBalanceRequest) and [`ListBalances`](/references/fullnode-protocol#haneul-rpc-v2-ListBalancesRequest) from `StateService`:
...
For more details, see [GraphQL Reference - `IAddressable.balance`](/references/haneul-api/haneul-graphql/beta/reference/types/interfaces/iaddressable).
...
See the [reference implementation](https://github.com/GeunhwaJeong/haneul/blob/e23cc4bb0215ec065785a19772f22eda8ada14ae/crates/haneul-types/src/balance_change.rs#L86) for the full algorithm.

### Accessing accumulator events

Accumulator events are embedded in [`TransactionEffects`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/effects/mod.rs):
...
The [`BalanceChange`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/balance_change.rs) struct represents the result:
...
Migration is optional. To consolidate coins into address balances using [Haneul CLI](/references/cli/ptb):

```bash
haneul client ptb \
    --merge-coins @<COIN_1> '[@<COIN_2>, @<COIN_3>]' \
    --move-call 0x2::coin::send_funds '<0x2::haneul::HANEUL>' @<COIN_1> @<YOUR_ADDRESS>
```

For HANEUL coins, you can smash them as gas and use `--move-call 0x2::coin::send_funds --gas-coin <COIN>`.
