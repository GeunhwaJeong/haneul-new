...
- `haneul:signTransaction` - Used to prompt the user to sign a transaction, and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `haneul:signAndExecuteTransaction` - Used to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.
- `haneul:reportTransactionEffects` - Used to report the effects of a transaction executed in the dApp
  to the wallet. this allows the wallet to update it's internal state to reflect the changes made by
  the transaction.
- `haneul:signTransactionBlock` - The previous version of `haneul:signTransaction`. It should still be
  implemented for compatibility with dApps that have not updated to the new feature.
- `haneul:signAndExecuteTransactionBlock` - The previous version of `haneul:signAndExecuteTransaction`. It
...
The wallet standard has recently been updated to reflect changes being implemented across the Haneul
ecosystem. With the migration the new GraphQL API the previous `haneul:signAndExecuteTransactionBlock`
feature will become harder to maintain, and has been closely tied to the JSON RPC options and data
structures.

The new `haneul:signAndExecuteTransaction` feature will be easier to implement for wallet builders
regardless of which API they are using to execute transactions. This simplicity comes at the expense
of flexibility in what is returned from the `haneul:signAndExecuteTransaction` feature.

The solution to this problem is to use the `haneul:signTransaction` feature to sign transactions, and
...
in `@haneullabs/dapp-kit` for the `useSignAndExecuteTransaction` hook, and enables dApps to take
...
To mitigate this, wallets can use the `haneul:reportTransactionEffects` feature so that apps can report
...
The `@haneullabs/haneul/transaction` SDK exports the `SerialTransactionExecutor` class, which can be used
