...
keywords: [ haneullabs/wallet-standard, wallet standards, wallet standard, browser extensions, exposing accounts, wallet events, register in window, wallet best practices, efficient transaction execution, wallet data, managing wallets, wallet-standard ]
---

Browser extension wallets built for Haneul use the [Wallet standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how dApps can automatically discover and interact with wallets.

If you are building a wallet, the helper library `@haneullabs/wallet-standard` provides types and utilities to help get started.
...
- `haneul:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
  message signature back to the dApp. Use this to verify the user’s public key.
- `haneul:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `haneul:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.
- `haneul:reportTransactionEffects` - Use to report the effects of a transaction executed in the dApp
  to the wallet. This allows the wallet to update its internal state to reflect the changes the transaction makes.
- `haneul:signTransactionBlock` - The previous version of `haneul:signTransaction`. Still
  implemented for compatibility with dApps that have not updated to the new feature.
- `haneul:signAndExecuteTransactionBlock` - The previous version of `haneul:signAndExecuteTransaction`. Still implemented for compatibility with dApps that have not updated to the new feature.
...
import {
  ...
  HaneulReportTransactionEffectsMethod
...
The Wallet standard has been updated from its original design to better support changes in the Haneul ecosystem. For example, the GraphQL service was introduced after Mainnet launched. The `haneul:signAndExecuteTransactionBlock` feature is closely tied to the JSON RPC options and data structures, so its continued maintenance becomes increasingly difficult as the GraphQL service becomes more ubiquitous.

Consequently, the Wallet standard introduced the `haneul:signAndExecuteTransaction` feature. The features of this method are more useful, regardless of which API you use to execute transactions. This usefulness comes at the expense
of flexibility in what `haneul:signAndExecuteTransaction` returns.

To solve this problem, use the `haneul:signTransaction` feature to sign transactions, and
leave transaction execution to the dApp. The dApp can query for additional data during
execution using whichever API it chooses. This is consistent with the default `@haneullabs/dapp-kit` uses for the `useSignAndExecuteTransaction` hook, and enables dApps to take
...
To mitigate this, wallets can use the `haneul:reportTransactionEffects` feature so that dApps can report
...
The `@haneullabs/haneul/transaction` SDK exports the `SerialTransactionExecutor` class, which you can use
...
Using the combination of `haneul:signTransaction` and `haneul:reportTransactionEffects`, dApps can use
...
Construct the transaction separately with the `@haneullabs/haneul` library and then sign it with the private key of the user. Use the `haneul:signTransaction` feature to achieve this:

```tsx
wallet.features[haneul:signTransaction].signTransaction(<Transaction>, <WalletAccount>);
```
...
To execute the transaction, use `HaneulClient` from `@haneullabs/haneul`:

```tsx
const client: HaneulClient
...
wallet.features['haneul:reportTransactionEffects'].reportTransactionEffects(
...
Many wallets abstract the above flow into one feature: `haneul:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
...
Haneul Labs offers a bare bones scaffold for React-based applications called `@haneullabs/dapp-kit`. See the [dApp Kit documentation](https://sdk.haneul-labs.com/dapp-kit) for more information.
