...
ActionRequest is defined in the `haneul::token` module and contains the following fields:
...
An example of a transaction implemented in TypeScript with haneul.js, confirming an `ActionRequest` with a TreasuryCap. Here the TreasuryCap is owned by the admin account and is used to mint and confirm the transfer request for the token:
