...
Token reuses the `TreasuryCap` defined in the `haneul::coin` module and therefore has the same initialization process. The `coin::create_currency` method guarantees the uniqueness of the `TreasuryCap` and forces the creation of a `CoinMetadata` object.
...
> *See [Create a Coin](/guides/developer/haneul-101/create-coin.mdx) guide for more details on how to create a Coin.*
