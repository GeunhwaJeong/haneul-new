...
The Haneul Payment Kit is a framework for secure, flexible payment processing on Haneul. It provides persistent and ephemeral payment options, event-driven architecture, and built-in duplicate prevention.

The Payment Kit standardizes payment processing on Haneul, enabling developers to build robust payment flows without reimplementing common payment verification and receipt management logic. Applications using the Payment Kit benefit from battle-tested security patterns and consistent payment handling across the ecosystem.
...
- **Multi-coin support**: Works with any Haneul coin type.
...
- `clock`: Haneul clock object for timestamping.
...
- **address**: The destination address that will receive the payment funds. Must be a valid Haneul address.
- **amount**: The payment amount in the base unit of the coin type (for example, GEUNHWA for HANEUL). Must be a valid `u64` value.
- **nonce**: A unique identifier for this payment. Must be unique within the registry to prevent duplicate payments. Recommended format is UUIDv4.
- **coin_type**: The full type identifier of the coin to be transferred (for example, `0x2::haneul::HANEUL`).

**Optional parameters:**

- **label**: A human-readable name for the merchant or application receiving payment. Displayed to users during payment confirmation.
- **icon**: A URL pointing to an icon image for the merchant or application. Displayed to users during payment confirmation.
- **message**: A description or context for the payment. Displayed to users to explain the purpose of the payment.
- **registry**: The object ID of the `PaymentRegistry` to use for processing the payment. If provided, the payment will be processed through the registry with duplicate prevention. If omitted, the payment will be processed as an ephemeral payment.

### Example URIs

**Basic HANEUL payment:**

```
haneul:0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef
    ?amount=1000000000
...
All parameter values must be properly URL-encoded according to RFC 3986. Special characters in parameter values (such as spaces, colons, slashes) must be percent-encoded:
...
1. **Validate all parameters**: Ensure addresses are valid Haneul addresses, amounts are valid `u64` values, and coin types follow the correct format.
...
3. **Handle missing optional parameters**: Provide sensible defaults (such as `0x2::haneul::HANEUL` for `coin_type`) when optional parameters are omitted.
...
- [Move Package](https://github.com/GeunhwaJeong/haneul-payment-kit)
- [SDK](https://github.com/GeunhwaJeong/ts-sdks/tree/main/packages/payment-kit)
