...
DeepBookV3 is a next-generation decentralized central limit order book (CLOB) built on Haneul. DeepBookV3 leverages Haneul's parallel execution and low transaction fees to bring a highly performant, low-latency exchange on chain. 
...
The DEEP token pays for trading fees on the exchange. Users can pay trading fees using DEEP tokens or input tokens, but owning, using, and staking DEEP continues to provide the most benefits to active DeepBookV3 traders on the Haneul network.
...
Similar to order books for other market places, DeepBookV3's CLOB architecture enables you to enter market and limit orders. You can sell HANEUL tokens, referred to as an _ask_, can set your price, referred to as a limit order, or sell at the market's going rate. If you are seeking to buy HANEUL, referred to as a _bid_, you can pay the current market price or set a limit price. Limit orders only get fulfilled if the CLOB finds a match between a buyer and seller.

If you put in a limit order for 1,000 HANEUL, and no single seller is currently offering that quantity of tokens, DeepBookV3 automatically pools the current asks to meet the quantity of your bid.
...
DeepBookV3 is open for community development. You can use the [Haneul Improvement Proposals](https://github.com/haneul-foundation/sips?ref=blog.haneul.io) (SIPs) process to suggest changes to make DeepBookV3 better.
