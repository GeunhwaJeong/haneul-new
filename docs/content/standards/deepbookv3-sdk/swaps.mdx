...
DeepBookV3 provides a swap-like interface commonly seen in automatic market makers (AMMs). The DeepBookV3 SDK provides functions to leverage the features of this interface. See [Swaps](../deepbookv3/contract-information/swaps.mdx) in the API section for more details.
...
<ImportContent source="packages/deepbook-v3/src/transactions/deepbook.ts" mode="code" org="HaneulLabs" repo="ts-sdks" fun="swapExactQuantity" signatureOnly/>
...
<ImportContent source="packages/deepbook-v3/src/transactions/deepbook.ts" mode="code" org="HaneulLabs" repo="ts-sdks" fun="swapExactQuantityWithManager" signatureOnly/>
...
swapExactBaseForQuote = (tx: Transaction) => {
  const [baseOut, quoteOut, deepOut] = this.deepBook.swapExactBaseForQuote({
    ...
    amount: 1, // amount of HANEUL to swap
...
swapExactQuoteForBase = (tx: Transaction) => {
  const [baseOut, quoteOut, deepOut] = this.deepBook.swapExactQuoteForBase({
    ...
    deepAmount: 1, // amount of DEEP to pay as fees, excess is returned
    minOut: 0.1, // minimum amount of HANEUL to receive or transaction fails
  })(tx);
