---
title: Coin Standard
description: The Haneul Coin standard enables you to create a broad range of fungible tokens on the Haneul network to satisfy a number of use cases. The Coin standard on Haneul is equivalent to the ERC-20 technical standard on Ethereum.
sidebar_label: Coin
keywords: [ haneul coin standard, erc-20, erc-20 equivalent, creating coins, coin standard, fungible tokens, fungible token standard, treasury capability, regulated coins, denylist object, coin metadata, create coin with SDK, use SDK to create coin, mint coin, minting coins ]
pagination_prev: null
---

The Coin standard is the technical standard used for smart contracts on Haneul for creating coins on the Haneul blockchain. The standardization of coin creation on Haneul means that wallets, exchanges, and other smart contracts can manage coins created on Haneul the same as they manage HANEUL, without any additional processing logic.

See [Haneul Tokenomics](../concepts/tokenomics.mdx) to learn more about the HANEUL native coin and its use on the Haneul network.

Although coins on Haneul follow the Coin standard, they can offer specialized abilities. For example, you can create a regulated token that allows its creator to add specific addresses to a deny list, so that the identified addresses cannot use the token as inputs to transactions.

See the [coin module](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/coin.md) documentation for all available options when creating a coin-type token on Haneul.

## Fungible tokens

In the Haneul blockchain ecosystem, the `Coin<T>` type represents open-loop fungible tokens (see `Token<T>` for closed-loop tokens). Coins are denominated by their type parameter, `T`, which is also associated with metadata (like name, symbol, decimal precision, and so on) that applies to all instances of `Coin<T>`. The `haneul::coin` module exposes an interface over `Coin<T>` that treats it as fungible, meaning that a unit of `T` held in one instance of `Coin<T>` is interchangeable with any other unit of `T`, much like how traditional fiat currencies operate.

:::info

The documentation refers to fungible tokens created on Haneul using the Coin standard as _coins_. For fungible tokens created on Haneul using the [Closed-Loop Token standard](./closed-loop-token.mdx), the documentation uses the term _tokens_. In practice, the terms for both these objects are often interchangeable.

:::

## Treasury capability

When you create a coin using the `coin::create_currency` function, the publisher of the smart contract that creates the coin receives a `TreasuryCap` object. The `TreasuryCap` object is required to mint new coins or to burn current ones. Consequently, only addresses that have access to this object are able to maintain the coin supply on the Haneul network.
...
The [regulated-coin-sample repository](https://github.com/GeunhwaJeong/regulated-coin-sample) provides an example of regulated coin creation.
...
| `symbol`      | Symbol for the coin. This might be the same as `name`, but is typically fewer than five all capital letters. For example, `HANEUL` is the `symbol` for the native coin on Haneul but its `name` is also `HANEUL`. |
...
The `coin` module provides the logic for creating and destroying coins on the Haneul network (as long as you own the associated `TreasuryCap`). These functions are the same for all coins and each requires the `TreasuryCap` as an input.

### Mint

Use the `coin::mint` function to create new tokens.

<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="mint" noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="burn" noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_enable_global_pause" noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_disable_global_pause" noComments />
...
Check out the following content for more information about coins and tokens on Haneul:

<RelatedLink to="/guides/developer/coin.mdx" />
<RelatedLink to="/standards/closed-loop-token.mdx" />
<RelatedLink href="https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/coin.md" label="`coin` module rustdoc documentation" desc="Automated documentation output for the Haneul framework `coin` module." />
<RelatedLink href="https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/token.md" label="`token` module rustdoc documentation" desc="Automated documentation output for the Haneul framework `token` module." />
