...
keywords: [ Object Display, object, display, display properties, object properties, haneul::display, display<T>, Haneul utility objects, Unique objects with dynamic representation, objects with unique static content ]
---

:::danger

Object Display V2 will be available in Haneul v1.68. You can check the [release schedule](https://www.haneul.io/networkinfo) to learn more about when v1.68 is available on Devnet, Testnet, and Mainnet. 
...
It is recommended to use the `haneul::display_registry` APIs when creating display from Move code. For the intermediate phase, use the `migrate_v1_to_v2` function to migrate a legacy display and receive the V2 `Display<T>` and `DisplayCap<T>`.
