...
description: Learn how to move assets through the Permissioned Asset Standard (PAS) on Haneul using Accounts, Requests, and Policies.
...
See the [KYC example `kyc_registry`](https://github.com/GeunhwaJeong/pas/blob/main/packages/examples/kyc/sources/kyc_registry.move) for a registry-gated transfer implementation.
...
See the [KYC example `treasury::burn`](https://github.com/GeunhwaJeong/pas/blob/main/packages/examples/kyc/sources/treasury.move) for a clawback-to-burn implementation.
...
The unrestricted path is `Balance<C>`-specific. It exists for assets like HANEUL that might accidentally end up in an Account. It aborts if a `Policy<Balance<C>>` exists for the type because you cannot bypass managed asset controls.
...
See the [Loyalty example `treasury::redeem`](https://github.com/GeunhwaJeong/pas/blob/main/packages/examples/loyalty/sources/treasury.move) for a registry-gated unlock (redemption) implementation.

### Unmanaged assets (no policy)

For example, if HANEUL is accidentally sent to an Account:

```move
let auth = account::new_auth(ctx);
let request = account.unlock_balance<HANEUL>(&auth, amount, ctx);

// No approval needed — resolves with empty approval set
let balance: Balance<HANEUL> = unlock_funds::resolve_unrestricted_balance(request, &namespace);
```
...
**Use case:** Burn, seize, or any operation where your contract needs the balance. See [`treasury::burn`](https://github.com/GeunhwaJeong/pas/blob/main/packages/examples/kyc/sources/treasury.move) (KYC) and [`treasury::redeem`](https://github.com/GeunhwaJeong/pas/blob/main/packages/examples/loyalty/sources/treasury.move) (Loyalty).
...
|---|---|---|
| [Loyalty](https://github.com/GeunhwaJeong/pas/tree/main/packages/examples/loyalty) | `send_funds` (permissionless), `unlock_funds` (per-address registry) | Transfers are open, and points stay in the system. Redemptions are gated by a `RedeemRegistry` and resolved in Move. Once unlocked, funds leave the managed system entirely. |
| [KYC](https://github.com/GeunhwaJeong/pas/tree/main/packages/examples/kyc) | `send_funds` (KYC registry), `clawback_funds` (issuer) | Every transfer checks the recipient against a `KYCRegistry`. Clawbacks are approved unconditionally, allowing the issuer to burn tokens from any account. |