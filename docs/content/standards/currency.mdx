...
description: The Haneul Currency Standard enables you to create a broad range of fungible tokens on the Haneul network using either the legacy coin creation or the centralized coin registry system. The Currency Standard on Haneul is equivalent to the ERC-20 technical standard on Ethereum.
sidebar_label: Currency
keywords: [ haneul Currency Standard, erc-20, erc-20 equivalent, creating coins, Currency Standard, fungible tokens, fungible token standard, treasury capability, regulated coins, denylist object, coin metadata, coin registry, create coin with SDK, use SDK to create coin, mint coin, minting coins, currency registry ]
pagination_prev: null
---

The Currency Standard is a technical standard used by Move smart contracts for creating currency on Haneul. The `haneul::coin_registry` module provides the logic that defines the standard, providing unified metadata, enhanced supply tracking, and improved regulatory features.

The standardization of currency creation on Haneul means that wallets, exchanges, and other smart contracts can manage currency created on Haneul the same as they manage HANEUL, without any additional processing logic.

See [Haneul Tokenomics](/concepts/tokenomics.mdx) to learn more about the HANEUL native currency and its use on the network.
...
The Coin Registry is a centralized system that provides unified currency management through the `haneul::coin_registry` module.
...
<details>
<summary>
`haneul::coin_registry` module
</summary>
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" />
</details>
...
- **Standard creation (recommended):**
    ...
    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="new_currency" signatureOnly noComments />
...
- **One-Time Witness (OTW) creation:**
    ...
    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="new_currency_with_otw" signatureOnly noComments />
...
When you create a currency using the `coin_registry::new_currency` or `coin_registry::new_currency_with_otw` functions, the publisher of the smart contract that creates the coin receives a `TreasuryCap` object upon finalization of the currency. The `TreasuryCap` object is required to mint new currency coins or to burn current ones (depending on currency supply state). Consequently, only addresses that have access to this object are able to maintain the currency supply on the Haneul network.
...
| `symbol`      | Symbol for the coin. This might be the same value as `name`, but is typically fewer than five letters, all capitals. For example, `HANEUL` is the `symbol` for the native coin on Haneul but its `name` is also `HANEUL`. **Only ASCII printable characters allowed**.|
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="mint" signatureOnly noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="burn,burn_balance" signatureOnly noComments />

For currencies with an unknown supply status, use the `coin::burn` function to destroy current coins.

<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="burn" signatureOnly noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_add" noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_remove" noComments />
...
You can use either the TypeScript or Rust SDK to manipulate the addresses held in the `DenyList` for your coin. The following examples are based on the [regulated coin sample](https://github.com/GeunhwaJeong/regulated-coin-sample).
...
tx.moveCall({
    ...
    arguments: [
        tx.object(<HANEUL-DENY-LIST-OBJECT-ID>),
...
- `<HANEUL-DENY-LIST-OBJECT-ID>` is `"0x403"`.
...
- `cmd.address()` returns the address to ban as a `HaneulAddress`.
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_enable_global_pause" noComments signatureOnly />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/coin.move" mode="code" fun="deny_list_v2_disable_global_pause" noComments signatureOnly />
...
- `decimals<T>()`: Get decimal precision.
    ...
    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="decimals" noTitle signatureOnly noComments />

- `name<T>()`: Get coin name.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="name" noTitle signatureOnly noComments />

- `symbol<T>()`: Get coin symbol.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="symbol" noTitle signatureOnly noComments />

- `description<T>()`: Get coin description.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="description" noTitle signatureOnly noComments />

- `icon_url<T>()`: Get icon URL.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="icon_url" noTitle signatureOnly noComments />

### Supply information

- `total_supply<T>()`: Get current total supply.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="total_supply" noTitle signatureOnly noComments />

- `is_supply_fixed<T>()`: Check if supply is fixed.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="is_supply_fixed" noTitle signatureOnly noComments />

- `is_supply_burn_only<T>()`: Check if supply is burn-only.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="is_supply_burn_only" noTitle signatureOnly noComments />

### Capability status

- `is_metadata_cap_claimed<T>()`: Check if metadata cap is claimed.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="is_metadata_cap_claimed" noTitle signatureOnly noComments />

- `is_metadata_cap_deleted<T>()`: Check if metadata cap is deleted.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="is_metadata_cap_deleted" noTitle signatureOnly noComments />

- `treasury_cap_id<T>()`: Get treasury cap object ID.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="treasury_cap_id" noTitle signatureOnly noComments />

- `metadata_cap_id<T>()`: Get metadata cap object ID.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="metadata_cap_id" noTitle signatureOnly noComments />

### Regulatory information

- `is_regulated<T>()`: Check if coin is regulated.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="is_regulated" noTitle signatureOnly noComments />

- `deny_cap_id<T>()`: Get deny cap object ID.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="deny_cap_id" noTitle signatureOnly noComments />
...
- `set_name<T>()`: Update coin name.
    ...
    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="set_name" noTitle noComments signatureOnly />

- `set_symbol<T>()`: Update coin symbol.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="set_symbol" noTitle noComments signatureOnly />

- `set_description<T>()`: Update coin description.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="set_description" noTitle noComments signatureOnly />

- `set_icon_url<T>()`: Update icon URL.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="set_icon_url" noTitle noComments signatureOnly />

Manage the metadata capability using:

- `claim_metadata_cap<T>()`: Claim the metadata capability one time.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="claim_metadata_cap" noTitle noComments signatureOnly />

- `delete_metadata_cap<T>()`: Permanently delete the capability to prevent future updates.

    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="delete_metadata_cap" noTitle noComments signatureOnly />
...
The Currency Standard replaces the legacy Coin Standard. Haneul provides a migration path from the `CoinMetadata<T>` system while maintaining backward compatibility.
...
1. **Metadata migration:** Use `migrate_legacy_metadata<T>()` to create a new `Currency<T>` entry based on existing `CoinMetadata<T>` information.
    <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="migrate_legacy_metadata" noTitle signatureOnly noComments />
...
1. **Regulatory migration:** For coins with deny list capabilities, use:
   - `migrate_regulated_state_by_metadata<T>()`: Migrate based on existing metadata.
        ...
        <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="migrate_regulated_state_by_metadata" noTitle signatureOnly noComments />

   - `migrate_regulated_state_by_cap<T>()`: Migrate based on deny capability.

        <ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="migrate_regulated_state_by_cap" noTitle signatureOnly noComments />
...
<RelatedLink href="/references/framework/haneul_haneul/coin" label="`coin` module rustdoc documentation" desc="Automated documentation output for the Haneul framework `coin` module." />
<RelatedLink href="/references/framework/haneul_haneul/token" label="`token` module rustdoc documentation" desc="Automated documentation output for the Haneul framework `token` module." />
