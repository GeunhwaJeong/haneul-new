...
To use the SDK in your projects, install the `@haneullabs/deepbook-v3` package, which includes the margin trading functionality.
...
class DeepBookMarginTrader {
	...
	getMarginManagers(): { [key: string]: MarginManager } {
		...
		const poolKey = process.env.POOL_KEY || 'HANEUL_DBUSDC';
...
(async () => {
	...
	traderClient.marginManager.borrowBase('MARGIN_MANAGER_1', 'HANEUL_DBUSDC', 100)(tx);
...
traderClient.marginManager.setMarginManagerReferral('MARGIN_MANAGER_1', referralId)(tx);

// Unset the referral for a margin manager for a specific pool
traderClient.marginManager.unsetMarginManagerReferral('MARGIN_MANAGER_1', 'HANEUL_DBUSDC')(tx);
```
