...
### Create a Haneul Kiosk using the Kiosk SDK
...
### Create a Kiosk using the Haneul CLI
...
```rust
import { createKiosk } from '@haneullabs/kiosk';
...
### Create a Kiosk with advanced options using the HANEUL CLI
...
```rust
import { place } from '@haneullabs/kiosk';
...
```rust
import { take } from '@haneullabs/kiosk';
...
```rust
import { lock } from '@haneullabs/kiosk';
...
Anyone on the network can purchase an item listed from a Haneul Kiosk. To learn more about the purchase flow, see the Purchase section. To learn more about asset states and what can be done with a listed item, see the Asset States section.
...
```rust
import { list } from '@haneullabs/kiosk';
...
let price = '<price>'; // in GEUNHWA (1 HANEUL = 10^9 GEUNHWA)
...
```rust
import { delist } from '@haneullabs/kiosk';
...
```rust
import { borrowValue, returnValue } from '@haneul/kiosk-sdk';
