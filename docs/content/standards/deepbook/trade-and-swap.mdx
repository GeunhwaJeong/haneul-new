...
When we create a pool in DeepBook, we need to specify the BaseAsset, QuoteAsset, tick_size, and lot_size. We also need to pay a transaction fee (in HANEUL token) to creating pool.
