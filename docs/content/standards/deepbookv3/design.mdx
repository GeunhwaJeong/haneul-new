...
- `BalanceManager`: Used to source a user's funds when placing orders. A single `BalanceManager` can be used between all pools. See [BalanceManager](./contract-information/balance-manager.mdx) to learn more.
