...
keywords: [ deepbook, deepbookv3, pool, balancemanager, tradeproof, query a pool, query pool, whitelist status, base quantity against quote, quote quantity against base, DEEP fees, input token fees, quote quantity against quote or base, check fee required, reserve mid price, reserve pool mid price ]mdx:
  format: md
---
...
{@inject: ../deepbook-ref/pool.move#fun=whitelisted noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_quote_quantity_out noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_base_quantity_out noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_quote_quantity_out_input_fee noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_base_quantity_out_input_fee noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_quantity_out noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_order_deep_required noComments}

### Retrieve mid price for a pool

Returns the mid price of the pool.

{@inject: ../deepbook-ref/pool.move#fun=mid_price noComments}

### Retrieve order IDs

Returns the `order_id` for all open orders for the `balance_manager` in the pool.

{@inject: ../deepbook-ref/pool.move#fun=account_open_orders noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_level2_range noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=get_level2_ticks_from_mid noComments}

### Retrieve balances

Get all balances held in this pool.

{@inject: ../deepbook-ref/pool.move#fun=vault_balances noComments}

### Retrieve pool ID

Get the ID of the pool given the asset types.

{@inject: ../deepbook-ref/pool.move#fun=get_pool_id_by_asset noComments}

### Retrieve order information

Returns the `Order` struct using the order ID.

{@inject: ../deepbook-ref/pool.move#fun=get_order noComments}

Returns a vector of `Order` structs using a vector of order IDs.

{@inject: ../deepbook-ref/pool.move#fun=get_orders noComments}

Returns a vector of `Order` structs for all orders that belong to a `BalanceManager` in the pool.

{@inject: ../deepbook-ref/pool.move#fun=get_account_order_details noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=locked_balance noComments}

### Retrieve pool parameters

Returns the trade parameters for the pool (`taker_fee`, `maker_fee`, `stake_required`).

{@inject: ../deepbook-ref/pool.move#fun=pool_trade_params noComments}
...
{@inject: ../deepbook-ref/pool.move#fun=pool_trade_params_next noComments}

Returns the quorum needed to pass proposal in the current epoch.

{@inject: ../deepbook-ref/pool.move#fun=quorum noComments}

Returns the book parameters for the pool (`tick_size`, `lot_size`, `min_size`).

{@inject: ../deepbook-ref/pool.move#fun=pool_book_params noComments}

Returns the `OrderDeepPrice` struct for the pool, which determines the conversion for DEEP fees.

{@inject: ../deepbook-ref/pool.move#fun=get_order_deep_price noComments}
