...
struct Order has store, drop {
    ...
    // Assuming a maximum TPS of 100K/s of Haneul chain, it would take (1<<63) / 100000 / 3600 / 24 / 365 = 2924712 years to reach the full capacity.
...
The following example uses the [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) to connect via websocket to listen to events.

```javascript
const local_client = new HaneulClient({ url: 'https://fullnode.mainnet.haneul.io:443/' });
...
deepbook_client.haneulClient.subscribeEvent({
