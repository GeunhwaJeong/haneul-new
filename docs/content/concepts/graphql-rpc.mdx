...
This document exlpains some of the common concepts when working with GraphQL such as pagination, fragments, or variables. For an introduction to GraphQL, see [GitHub's Introduction to GraphQL](https://docs.github.com/en/graphql/guides/introduction-to-graphql).

If you are interested in how to interact with the Haneul network via the GraphQL RPC, see [Getting Started](guides/developer/getting-started/graphql-rpc.mdx).
...
- `x-haneul-rpc-version` to specify which RPC version to use (note that this is currently unavailable)
- `x-haneul-rpc-show-usage` will return the response with extra query complexity information. 

By default, each request will return the service's version in the response header: `x-haneul-rpc-version`.

```bash
curl -v -X POST https://graphql-beta.mainnet.haneul.io \
...
Variables are a powerful way for passing different object IDs, Digests, Haneul addresses, and other kinds of data for full flexibility. The following is an example for querying the reference gas price for epoch `100`, by passing the epoch as a variable.
...
The following sections highlight the steps necessary for you to take to migrate your codebase to leverage the Haneul GraphQL RPC.
...
The goal is to return all `Coin<0x2::haneul::HANEUL>` objects an address owns.
...
A good start for writing your own queries is either by browsing the [examples](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-graphql-rpc/examples) folder or referring to the [Typescript SDK implementation](https://github.com/GeunhwaJeong/haneul/tree/main/sdk/typescript/src/graphql/queries).
-->
