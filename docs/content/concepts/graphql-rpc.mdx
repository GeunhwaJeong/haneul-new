...
- `x-haneul-rpc-version` to specify which RPC version to use (currently only one version is supported),
- `x-haneul-rpc-show-usage` returns the response with extra query complexity information.

By default, each request returns the service's version in the response header: `x-haneul-rpc-version`.
...
Different queries are subject to their own retention policies. For example, live object set queries only work for recent checkpoints (measured in minutes or hours), and transaction pagination might work going back to only a certain checkpoint (measured in weeks or months) with different retentions supported based on the filters applied. Data outside these ranges are pruned away. You can query the upper- and lower-bounds of that range for your query as follows:
