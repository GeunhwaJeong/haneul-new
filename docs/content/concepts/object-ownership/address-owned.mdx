...
You can use one of the following functions (defined in the [transfer module](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/transfer.move)) to create address-owned objects.
...
You should use the `haneul::transfer::transfer` function if you are defining a [custom transfer policy](../dynamic-fields/transfers/custom-rules.mdx) for the object. Use the `haneul::transfer::public_transfer` function to create an address-owned object if the object has the `store` capability.
...
If the address owner of the object corresponds to an object ID, then you must access and dynamically authenticate it during the execution of the transaction using the mechanisms defined in [Transfer to Object](../dynamic-fields/transfers/transfer-to-object.mdx).
...
An example of an object that is frequently address-owned is that of a [Coin object](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/coin.move). If address `0xA11CE` had a coin `C` with 100 HANEUL and wanted to pay address `0xB0B` 100 HANEUL, `0xA11CE` could do so by transferring `C` to `0xB0B`.

```move
haneul::transfer::public_transfer(C, @0xB0B);
```

This results in `C` having a new address owner of `0xB0B`, and `0xB0B` can later use that 100 HANEUL coin.
