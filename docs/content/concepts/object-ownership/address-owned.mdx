...
Use the `haneul::transfer::transfer` function if you are defining [custom transfer rules](/concepts/transfers/custom-rules.mdx) for the object. Use the `haneul::transfer::public_transfer` function to create an address-owned object if the object has the `store` capability.
