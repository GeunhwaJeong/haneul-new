...
Objects in Haneul can have different types of ownership, with two broad categories: immutable objects and mutable objects. An immutable object is an object that can't be mutated, transferred, or deleted. Immutable objects have no owner, so anyone can use them.

## Create immutable object

To convert an object into an immutable object, call the `public_freeze_object` function from the [transfer module](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/transfer.move):
...
You can see this function's use in one of the [color_object example module](https://github.com/GeunhwaJeong/haneul/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new (owned) `ColorObject`, then calls `public_freeze_object` to turn it into an immutable object.
...
You can only pass an immutable object as a read-only, immutable reference to Haneul Move entry functions as `&T`.
...
```rust
public entry fun copy_into(from: &ColorObject, into: &mut ColorObject);
```
...
Publish the `ColorObject` code on-chain using the Haneul Client CLI:
