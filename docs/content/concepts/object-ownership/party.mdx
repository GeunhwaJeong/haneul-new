...
`Coin`s can be party objects, including `Coin<HANEUL>`. However, you can't use a party object `Coin<HANEUL>` for gas payment. To use a party object `Coin<HANEUL>` for gas, you must first transfer it back to address-owned.
