...
A party object is an object that is transferred using the `haneul::transfer::party_transfer` or `haneul::transfer::public_party_transfer` function. It is accessible to the `Party` to which it is transferred.
...
Use one of the following functions (defined in the [transfer module](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/transfer.move)) to create party objects:
...
Use the `haneul::transfer::party_transfer` function if you are defining a [custom transfer policy](../transfers/custom-rules.mdx) for the object. Use the `haneul::transfer::public_party_transfer` function if the object has the `store` capability.
