...
To embed a struct type in a Haneul object struct (with a `key` ability), the struct type must have the `store` ability.

In the preceding example, `Bar` is a normal struct, but it is not a Haneul object since it doesn't have the `key` ability.
...
Now `Bar` is also a Haneul object type. If you put a Haneul object of type `Bar` into a Haneul object of type `Foo`, the object type `Foo` wraps the object type `Bar`. The object type `Foo` is the wrapper or wrapping object.

There are some interesting consequences of wrapping a Haneul object into another. When an object is wrapped, the object no longer exists independently on-chain. You can no longer look up the object by its ID. The object becomes part of the data of the object that wraps it. Most importantly, you can no longer pass the wrapped object as an argument in any way in Haneul Move calls. The only access point is through the wrapping object.
...
There are a few ways to wrap a Haneul object into another Haneul object, and their use cases are typically different. This section describes three different ways to wrap a Haneul object with typical use cases.

### Direct wrapping

**Direct wrapping** occurs when a Haneul object type contains another Haneul object type as a direct field. The most important properties achieved through direct wrapping are:
...
You can also enable a swap/trade between your object and others' objects. For example, define a function that takes two objects from two addresses and swaps their ownership. But this doesn't work in Haneul. Only object owners can send a transaction to mutate the object. So one person cannot send a transaction that would swap their own object with someone else's object.
...
`SwapRequest` defines a Haneul object type, wraps the `object` to swap, and tracks the original `owner` of the object. You might need to also pay the third party some fee for this swap. To define an interface to request a swap by someone who owns an `Object`:
...
In the preceding function, you must pass the object by value so that it's fully consumed and wrapped into `SwapRequest` to request swapping an `object`. The example also provides a fee (in the type of `Coin<HANEUL>`) and checks that the fee is sufficient. The example turns `Coin` into `Balance` when it's put into the `wrapper` object. This is because `Coin` is a Haneul object type and used only to pass around as Haneul objects (such as transaction inputs or objects sent to addresses). For coin balances that need to be embedded in other structs, use `Balance` instead because to avoid the overhead of carrying around an unnecessary `UID` field.
...
Find the full source code in the [trusted_swap](https://github.com/GeunhwaJeong/haneul/blob/main/examples/move/trusted_swap) example.

To view a more complex example of how to use direct wrapping, see the [escrow](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/escrow) example.

### Wrapping through `Option`

When Haneul object type `Bar` is directly wrapped into `Foo`, there is not much flexibility: a `Foo` object must have a `Bar` object in it, and to take out the `Bar` object you must destroy the `Foo` object. However, for more flexibility, the wrapping type might not always have the wrapped object in it, and the wrapped object might be replaced with a different object at some point.
...
Note that because `Sword` is a Haneul object type without `drop` ability, if the warrior already has a sword equipped, the warrior can't drop that sword. If you call `option::fill` without first checking and taking out the existing sword, an error occurs. In `equip_sword`, first check whether there is already a sword equipped. If so, remove it out and send it back to the sender. To a player, this returns an equipped sword to their inventory when they equip the new sword.

Find the source code in the [simple_warrior](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/simple_warrior) example.

To view a more complex example, see [hero](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/hero).

### Wrapping through `vector`

The concept of wrapping objects in a vector field of another Haneul object is very similar to wrapping through `Option`: an object can contain 0, 1, or many of the wrapped objects of the same type.
