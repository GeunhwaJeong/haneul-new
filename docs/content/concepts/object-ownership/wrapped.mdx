...
You can also enable a swap or trade between your object and others' objects. For example, define a function that takes two objects from two addresses and swaps their ownership. This doesn't work in Haneul. Only object owners can send a transaction to mutate the object. One person cannot send a transaction that would swap their own object with someone else's object.
