...
1. Use the GraphQL endpoint directly: `https://haneul-[network].haneul-labs.com/graphql`, changing `[network]` to the appropriate value. See the <a href="/references/haneul-api/haneul-graphql/reference/api/queries/verify-zklogin-signature" data-noBrokenLinkCheck="true">GraphQL documentation</a> for more details. This is recommended if you do not plan to run any servers or handle any JWK rotations. 

1. Use the [Haneul Keytool CLI](../../references/cli/keytool.mdx). This is recommended for debug usage. 

   ```sh
   $HANEUL_BINARY keytool zk-login-sig-verify --sig $ZKLOGIN_SIG --bytes $BYTES --intent-scope 3 --network devnet --curr-epoch 3
   ```
