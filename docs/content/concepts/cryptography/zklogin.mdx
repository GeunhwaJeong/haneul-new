...
- **Accessibility:** zkLogin is one of several native Haneul signature schemes thanks to [Haneul cryptography agility](/concepts/cryptography/transaction-auth/signatures.mdx). It integrates with other Haneul primitives, such as sponsored transactions and multisig.  
...
1. Self-Custodial without additional trust: Haneul leverages the nonce field in JWT to commit to ephemeral public key, so no persistent private key management is required with any trusted parties. In addition, the JWK itself is an oracle agreed upon by the quorum of stakes by the validators with trusting any source of authority.
