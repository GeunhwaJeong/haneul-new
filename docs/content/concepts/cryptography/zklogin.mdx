...
- **Accessibility:** zkLogin is one of several native Haneul signature schemes thanks to [Haneul cryptography agility](/concepts/transactions/transaction-auth.mdx). It integrates with other Haneul primitives, such as sponsored transactions and multisig.  
- **Rigor:** The code for zkLogin has been independently [audited](https://github.com/haneul-foundation/security-audits/blob/main/docs/zksecurity_zklogin-circuits.pdf) by 2 firms specializing in zero knowledge. The public zkLogin ceremony for creating the common reference string included contributions from more than 100 participants.  
...
#### Steps 9-10: The user signs the transaction with the ephemeral private key and submits it with the ephemeral signature, ZK proof, and other inputs to Haneul.  
...
1. The address flag: `zk_login_flag = 0x05` for zkLogin address. This serves as a domain separator as a signature scheme defined in [crypto agility](/concepts/transactions/transaction-auth.mdx).
...
1. Self-Custodial without additional trust: Haneul leverages the nonce field in JWT to commit to ephemeral public key, so no persistent private key management is required with any trusted parties. In addition, the JWK itself is an oracle agreed upon by the quorum of stakes by the validators without trusting any source of authority.
