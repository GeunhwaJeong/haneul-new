...
_Sign up for Twitch developer account_
...
and investigate its structure by pasting it in the [jwt.io](https://jwt.io) website.
...
User salt is used when computing the zkLogin Haneul address (see [definition](#address-definition)). There are several options for the application to maintain the user salt:
...
Here's an example request and response for the Haneul Labs-maintained proving service. If you wish to use the Haneullabs ran ZK Proving Service, please contact us for whitelisting your registered client ID. Only valid JWT token authenticated with whitelisted client IDs are accepted.

```bash
curl -X POST https://prover.haneul-labs.com/v1 -H 'Content-Type: application/json' \
-d '{"jwt":"$JWT_TOKEN", \
...
This is optional, but it enhances user experience by allowing zkLogin address holders to skip acquiring HANEUL prior to transaction initiation.
...
The application can currently call the Haneul Labs-maintained ZK Proving Service. Please reach out to whitelist the application's registered client ID to use the service.
