...
In previous releases, Haneul used a special `Signable` trait that attached the Rust struct name as a prefix to the serialized data. This is not ideal because it is:
...
The proof of possession is a BLS signature created using the authority protocol private key, committed over the following message: `intent || pubkey || address || epoch`. Here `intent` is serialized to `[5, 0, 0]` representing an intent with scope as `Proof of Possession`, `version` as `V0`, and `app_id` as `Haneul`. The `pubkey` value is the serialized public key bytes of the authority BLS protocol key. The `address` value is the account address associated with the authority account key. The `epoch` value is serialized to `[0, 0, 0, 0, 0, 0, 0, 0]`.
