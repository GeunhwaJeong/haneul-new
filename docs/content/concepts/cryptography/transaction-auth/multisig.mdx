...
Haneul supports multi-signature (multisig) transactions, which require multiple keys for authorization rather than a single, one-key signature. In technical terms, Haneul supports `k` out of `n` multisig transactions, where `k` is the threshold and `n` is the total weights of all participating parties. The maximum number of parties is 10. To learn more about the single key signatures that Haneul supports, see [Signatures](./signatures.mdx).

Valid participating keys for multisig are Pure Ed25519, ECDSA Secp256k1, and ECDSA Secp256r1. A ([u8](https://doc.rust-lang.org/std/primitive.u8.html)) weight is set for each participating keys and the threshold can be set as [u16](https://doc.rust-lang.org/std/primitive.u16.html). If the serialized multisig contains enough valid signatures of which the sum of weights passes the threshold, Haneul considers the multisig valid and the transaction executes.

## Applications of multisig

Haneul allows you to mix and match key schemes in a single multisig account. For example, you can pick a single Ed25519 mnemonic-based key and two ECDSA secp256r1 keys to create a multisig account that always requires the Ed25519 key, but also one of the ECDSA secp256r1 keys to sign. You could use this structure for mobile secure enclave stored keys as two-factor authentication.
...
![Supported structures in Haneul multisig](images/haneul_multisig_structures.png 'Supported structures in Haneul multisig')
_Multisig structures supported in Haneul._

## Example workflow

The following steps demonstrate how to create a multisig transaction and then submit it against a local network using the [Haneul CLI](/references/cli.mdx). A transaction can be the transfer of an object, the publish or upgrade of a package, the payment of HANEUL, and so on. To learn how to set up a local network, see [Connect to a Local Network](/guides/developer/getting-started/local-network.mdx).
...
Use `haneul client` to create Haneul addresses of different key schemes.
...
### Step 2: Add keys to Haneul keystore

Use `haneul keytool` to list the signatures you created in the previous step.
...
```shell
haneul keytool multi-sig-address --pks <PUBLIC-KEY-ED25519> <PUBLIC-KEY-SECPK1> <PUBLIC-KEY-SECP256R1> --weights 1 2 3 --threshold 3
```
...
This section demonstrates how to use an object that belongs to a multisig address and serialize a transfer to be signed. The `tx_bytes` value can be any serialized transaction data where the sender is the multisig address. Use the `--serialize-unsigned-transaction` flag for supported commands in `haneul client -h` (`publish`, `upgrade`, `call`, `transfer`, `transfer-haneul`, `pay`, `pay-all-haneul`, `pay-haneul`, `split`, `merge-coin`) to output the Base64 encoded transaction bytes.

```shell
haneul client transfer --to <HANEUL-ADDRESS> --object-id <OBJECT-ID> --gas-budget <GAS-AMOUNT> --serialize-unsigned-transaction
...
```shell
haneul keytool multi-sig-combine-partial-sig --pks <PUBLIC-KEY-1> <PUBLIC-KEY-2> <PUBLIC-KEY-3> --weights 1 2 3 --threshold 3 --sigs <SIGNATURE-1> <SIGNATURE-2>
...
Use `haneul client` to execute a transaction using multisig:

```shell
haneul client execute-signed-tx --tx-bytes <TX_BYTES> --signatures <SERIALIZED-MULTISIG>
```
