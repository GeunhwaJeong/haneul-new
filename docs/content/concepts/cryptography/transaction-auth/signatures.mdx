---
title: Signatures
description: Haneul supports multiple cryptography algorithms and primitives and can switch between them rapidly.
---
...
The signature must commit to the hash of the intent message of the transaction data, which you can construct by appending the 3-byte intent before the BCS serialized transaction data. To learn more on what an intent is and how to construct an intent message, see [Haneul Intent Signing](intent-signing.mdx).
...
The Authority on Haneul (collection of validators) holds three distinctive keypairs:
