...
3. Register the instance's platform configuration registers (PCRs) - measurements of the trusted computing base - using a Haneul smart contract.
4. Deploy the server to an AWS Nitro Enclave.
5. Register the deployed enclave using a Haneul smart contract and attestation document, including the enclave's ephemeral public key for signing responses.
