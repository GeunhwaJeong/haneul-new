...
mdx:
  format: md
---
The [Nautilus framework](https://github.com/GeunhwaJeong/nautilus) helps you deploy an AWS Nitro Enclave with all the necessary scaffolding, such as reproducible builds, signature formatting, and HTTPS traffic forwarding, so you can focus on implementing the off-chain computation logic inside the enclave.
...
Refer to the [Github repo](https://github.com/GeunhwaJeong/nautilus) for the code structure and related usage instructions that follow.
...
/move
  ...
  /twitter-example  # Entry point for on-chain logic as another example, which uses enclave functions to run your Nautilus application logic. 
...
For the latest structure, always refer to the [GitHub repository](https://github.com/GeunhwaJeong/nautilus).
...
```sh
# optionally
$ haneul client switch --env testnet # or appropriate network
...
$ haneul client call --function update_pcrs --module enclave --package $ENCLAVE_PACKAGE_ID --type-args "$APP_PACKAGE_ID::$MODULE_NAME::$OTW_NAME" --args $ENCLAVE_CONFIG_OBJECT_ID $CAP_OBJECT_ID 0x$PCR0 0x$PCR1 0x$PCR2

# optional, give it a name you like
$ haneul client call --function update_name --module enclave --package $ENCLAVE_PACKAGE_ID --type-args "$APP_PACKAGE_ID::$MODULE_NAME::$OTW_NAME" --args $ENCLAVE_CONFIG_OBJECT_ID $CAP_OBJECT_ID "weather enclave, updated 2025-05-13"
...
You can view an [example enclave config object](https://testnet.haneulvision.xyz/object/0x58a6a284aaea8c8e71151e4ae0de2350ae877f0bd94adc2b2d0266cf23b6b41d) containing PCR values on HaneulScan. Additionally see an [example enclave object](https://testnet.haneulvision.xyz/object/0xe0e70df5347560a1b43e5954267cadd1386a562095cb4285f2581bf2974c838d) that includes the enclave’s registered public key.
...
You can view an [example enclave config object](https://testnet.haneulvision.xyz/object/0xe33641a2dae5eb4acad3859e603ec4e25641af05f837c85058645c7d8d9d831a) containing PCR values. You can also view an [example enclave object](https://testnet.haneulvision.xyz/object/0x53db077721140910697668f9b2ee80fbecd104ac076d60fc1fb49ae57cd96c0d) that includes the registered enclave public key.

You can find the frontend code for the Twitter example [in this repository](https://github.com/GeunhwaJeong/nautilus-twitter/tree/main/frontend).
...
An example of a [created weather NFT](https://testnet.haneulvision.xyz/object/0xa78e166630c0ed004b3115b474fed15d71f27fc80b68e37d451494c6e815931e) is available on network scanners.
...
### Why did Haneul choose AWS Nitro Enclaves initially?
...
It is stored as part of the Haneul framework and used to verify AWS attestation documents. You can verify its hash by following the steps outlined on [AWS](https://docs.aws.amazon.com/enclaves/latest/user/verify-root.html#validation-process).

```sh
$ curl https://raw.githubusercontent.com/HaneulLabs/haneul/refs/heads/main/crates/haneul-types/src/nitro_root_certificate.pem -o cert_haneul.pem
$ sha256sum cert_haneul.pem
...
6eb9688305e4bbca67f44b59c29a0661ae930f09b5945b5d1d9ae01125c8d6c0 # check it matches from the one downloaded from the Haneul repo
```
