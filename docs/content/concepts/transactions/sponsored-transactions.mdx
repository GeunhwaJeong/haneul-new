...
description: A sponsored transaction is when one Haneul address pays the gas fee for a transaction submitted by another address. Sponsored transactions can help facilitate user onboarding and simplified asset management.
...
A Haneul sponsored transaction is one where a Haneul address (the sponsor's) pays the gas fees for a transaction that another address (the user's) initializes. You can use sponsored transactions to cover the fees for users on your site or app so that they don't get charged for them. This removes a significant obstacle that Web2 users encounter when entering Web3, as they often have to purchase tokens to perform a transaction on-chain. For example, you could sponsor gamers' early transactions to increase conversion rates.

Sponsored transactions also facilitate asset management as you don't need to maintain multiple accounts with HANEUL tokens to transfer funds.

You can use Haneul sponsored transactions to:
...
A user-initiated sponsored transaction involves the following steps:
 ...
 1. The user verifies and then signs `TransactionData` and sends the dual-signed transaction to Haneul network through a Full node or the sponsor.

### GasLessTransactionData

`GasLessTransactionData` is basically `TransactionData` without `GasData`. It is not a `haneul-core` data structure, but it is only an interface between user and sponsor.
...
A sponsor-initiated sponsored transaction involves the following steps:
 ...
 1. The user submits the dual-signed transaction to a Haneul Full node or sponsor to execute it.
...
## Create a Haneul gas station

On Haneul, a gas station is a concept to describe where you set up processes to sponsor user transactions. You can customize a Haneul gas station to support the specific user-facing functionality you need. Some example use cases for a Haneul gas station include:
...
## Code examples to create a Haneul gas station

The following Rust SDK code examples demonstrate how to implement a Haneul gas station that supports each of the sponsored transaction types described previously.
...
The following code block describes the `TransactionData` structure for sponsored transactions and `GasObject`. You can view the [source code](https://github.com/GeunhwaJeong/haneul/blob/224a28ed9dece21a952547896bd5d794bdf8b562/crates/haneul-types/src/transaction.rs) in the Haneul GitHub repository.
...
To learn more about transactions in Haneul, see [Transactions](../transactions.mdx).
