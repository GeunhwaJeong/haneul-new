...
## Transactions on Haneul

In Haneul, a transaction is more than a basic record of the flow of assets. Transactions in Haneul are composed of a number of commands that execute on inputs to define the result of the transaction. In essence, transactions on Haneul are programmable, therefore they are termed "programmable transaction blocks" (PTB for short) to reference their additional capabilities over traditional blockchain transactions. A programmable transaction block on Haneul can perform up to 1,024 unique operations in a single execution, whereas transactions on traditional blockchains would require 1,024 individual executions to accomplish the same result.

:::info

Unless otherwise noted in the documentation, "transaction" is synonymous with "programmable transaction block" when referring to transactions on the Haneul network.
...
For example, you might develop a PTB in which the first command extracts a `Player` object from a smart contract wallet, then the next command uses that `Player` to make a move in a game. Based on the outcome of that move, the next command might send a `Badge` object to a multi-game `TrophyCase` object. All of these events happen in a single PTB execution on the Haneul network.
...
For information on how to build PTBs, see [Building Programmable Transaction Blocks](/guides/developer/haneul-101/building-ptb.mdx). 
