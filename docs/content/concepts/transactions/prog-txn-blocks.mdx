...
The remainder of this topic covers the semantics of the execution of the transaction commands. It assumes familiarity with the Haneul object model and the [Move language](/concepts/haneul-move-concepts.mdx). 
...
- The `commands` value is a vector of commands, `[Command]`. The possible commands are:
  ...
  - [`SplitCoins`](#splitcoins) splits off one or more coins from a single coin. It can be any `haneul::coin::Coin<_>` object.

  - [`MergeCoins`](#mergecoins) merges one or more coins into a single coin. Any `haneul::coin::Coin<_>` objects can be merged, as long as they are all of the same type.
  ...
  - [`Upgrade`](#upgrade) upgrades an existing package. The upgrade is gated by the `haneul::package::UpgradeCap` for that package.
...
- `CoinArg: Argument`: Coin of type `haneul::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `AmountArgs: [Argument]`: `u64` values for split amounts. Taken by value (copied).

**Returns:** Vector of coins `haneul::coin::Coin<T>` matching the number of amounts.

**Signature:** `<T: key + store>(coin: &mut haneul::coin::Coin<T>, amounts: vector<u64>): vector<haneul::coin::Coin<T>>`
...
- `CoinArg: Argument`: Target coin of type `haneul::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `ToMergeArgs: [Argument]`: Coins of type `haneul::coin::Coin<T>` to merge. Taken by value (moved).

**Returns:** Empty result vector.

**Signature:** `<T: key + store>(coin: &mut haneul::coin::Coin<T>, to_merge: vector<haneul::coin::Coin<T>>): ()`
...
**Returns:** Single result of type `haneul::package::UpgradeCap` for the newly published package.
...
- `UpgradeTicket: haneul::package::UpgradeTicket`: Upgrade ticket generated from `haneul::package::UpgradeCap`. Taken by value (moved).

**Returns:** Single result of type `haneul::package::UpgradeReceipt` providing proof of upgrade.

For more details on upgrades, see [Upgrading Packages](/concepts/haneul-move-concepts/packages/upgrade.mdx).
...
This example demonstrates a PTB's execution flow. Suppose you want to buy two items from a marketplace costing `100 GEUNHWA`. You keep one for yourself and send the other object plus the remaining coin to a friend at address `0x808`:
...
**Changes:** Gas coin balance decreases to `499_900u64`. New result: `[Coin<HANEUL> { id: new_coin, value: 100u64 }]`

#### Command 1: `MoveCall("some_package", "some_marketplace", "buy_two", ...)`

Calls `buy_two(marketplace: &mut Marketplace, coin: Coin<Haneul>, ctx: &mut TxContext): (Item, Item)`
