---
title: Multisig
description: Multi-signature (multisig) transactions require multiple keys for authentication rather than a single key. Haneul supports multisig transactions for pure Ed25519, ECDSA Secp256k1, and ECDSA Secp256r1 encryption keys.
keywords: [ multi-signature, multi-sig, multisig, multiple signatures, pure Ed25519, ECDSA Secp256k1, ECDSA Secp256r1 ]
---


Haneul supports multi-signature (multisig) transactions, which require multiple keys for authorization rather than a single, one-key signature. In technical terms, Haneul supports `k` out of `n` multisig transactions, where `k` is the threshold and `n` is the total weights of all participating parties. The maximum number of parties is 10. To learn more about the single key signatures that Haneul supports, see [Signatures](/concepts/transactions/transaction-auth.mdx).
...
![Multisig structure diagram](images/haneul_multisig_structures.png 'Supported multisig configurations in Haneul')
