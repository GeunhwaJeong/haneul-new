...
1. After assembling the certificate, the client sends it back to all validators, who check its validity and acknowledge its receipt to the client.
    ...
    1. If the transaction involves [owned objects] exclusively, Haneul can process the transaction certificate immediately and execute it without waiting for the [consensus engine](/concepts/haneul-architecture/consensus.mdx). This is known as the **direct fast path**.
...
After the transaction is signed, the wallet or dApp submits the transaction to a Haneul full node on behalf of the user.
...
You open the wallet dApp on your phone and scan the coffee shop's QR code that contains the recipient on-chain address. The wallet dApp constructs a transaction that transfers 10 HANEUL from your Haneul address to the coffee shop's address. You review the transaction details and approve it. The wallet dApp then signs the transaction with your private key and submits it to a full node. The full node broadcasts the transaction to all validators in the network.
