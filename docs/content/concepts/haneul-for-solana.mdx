...
| Mutate contract state | Sending transactions with instruction data. Programs process instructions and modify account data. Can use Anchor's IDL (Interface Definition Language) for type-safe client interaction. | Sending transaction through runtime [programmable transaction block](/guides/developer/transactions/prog-txn-blocks.mdx) (PTB) construction. |
