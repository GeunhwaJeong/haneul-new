---
title: Solana -> Haneul
description: Build your first app on Haneul if you have Solana experience
keywords: [ solana, rust, anchor ]
pagination_prev: null
---

The biggest difference between Haneul and Solana development lies in the programming language. Haneul uses [**Move**](/concepts/haneul-move-concepts.mdx), while Solana relies on **Rust**, typically paired with a development framework that simplifies app development and management. **Anchor** is the most popular Solana framework.

| **Topic** | **Rust (Solana)** | **Move (Haneul)** |
|------|---------------|------------|
| Model | Account-based model where all data is stored in accounts. Programs declare read/write accounts upfront. Everything is an account (programs, data, wallets). | Object ownership inherent to Haneul, objects are first class citizens and encompass everything _owned_ on Haneul. |
...
| Mutate contract state | Sending transactions with instruction data. Programs process instructions and modify account data. Can use Anchor's IDL (Interface Definition Language) for type-safe client interaction. | Sending transaction through runtime [programmable transaction block](/guides/developer/transactions/ptbs/prog-txn-blocks) (PTB) construction. |
...
In Move, everything is an object. Objects serve as the fundamental data storage unit on Haneul, containing Move packages (smart contracts), addresses, coins, NFTs, and all other on-chain data.
...
| Coin names, name of the smallest unit | SOL, Lamport | HANEUL, GEUNHWA |
| Available frameworks for development | Anchor framework, Solana CLI, native Rust tooling | Haneul CLI |
