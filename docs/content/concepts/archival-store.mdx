...
The Archival Store and Service provide a scalable, consistent foundation for accessing historical data on Haneul beyond what Full nodes or indexer databases typically retain. They power rich developer experiences through [gRPC](./grpc-overview.mdx) and [GraphQL](./graphql-indexer.mdx) APIs while keeping storage and operations manageable.

:::note

GraphQL RPC will integrate with the Archival Service after the GraphQL RPC and General-purpose Indexer beta launch. Until then, the GraphQL RPC beta will only be available as a hosted service from the Haneul Foundation, and connects directly to the Archival Store (Bigtable) operated by the Haneul Foundation.
...
Access the Archival Service hosted by any participating provider. Haneul Foundation offers a public good service on a Bigtable-based store. This service has strict rate limits:
...
- Tool: [haneul-kv-store](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-kvstore)
...
- Reference implementation for Bigtable: [haneul-kv-rpc](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-kv-rpc).
...
- [GraphQL RPC](./graphql-indexer.mdx) will delegate historical lookups to the Archival Service using the planned integration after the upcoming beta release. It will enable cost-efficient setup of a General-purpose Indexer database with limited retention.
