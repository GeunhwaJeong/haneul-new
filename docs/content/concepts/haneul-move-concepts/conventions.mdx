...
For this reason, the `Move.lock` file should always be part of the package (don't add it to the `.gitignore` file). Use the [automated address management](/concepts/haneul-move-concepts/packages/automated-address-management.mdx) instead of the old `published-at` field in the manifest file.
