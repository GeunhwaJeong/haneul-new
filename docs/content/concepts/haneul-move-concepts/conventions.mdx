...
description: Recommended Move 2024 best practices for Haneul development.
---

This guide outlines recommended conventions and best practices for writing Move smart contracts on Haneul. Following these guidelines helps create more maintainable, secure, and composable code that aligns with ecosystem standards.

While these conventions are recommendations rather than strict rules, they represent patterns that have proven effective across many Haneul projects. They help create consistency across the ecosystem and make code easier to understand and maintain.

## Organization principles

### Package

A Haneul package consists of:
...
- `Move.lock` file that the Haneul Move toolchain automatically generates to lock the versions of the dependencies and track the different published and upgraded versions of the package that exist on the different networks.

For this reason, the `Move.lock` file should always be part of the package (don't add it to the `.gitignore` file). Use the [automated address management](/guides/developer/packages/automated-address-management.mdx) instead of the old `published-at` field in the manifest file.
...
Modules are the main building blocks of your Move code. They are used to organize and encapsulate related functionality. Design your modules around 1 object or data structure. A variant structure should have its own module to avoid complexity and bugs.
...
The following section covers common patterns and best practices specific to Move development on Haneul, including object ownership models and function design principles.

### Shared objects

Library modules that share objects should provide 2 functions: 1 to instantiate and return the object, and another 1 to share it. It allows the caller to pass it to other functions and run custom functionality before sharing it.
...
public fun withdraw(state: &mut State, account: &mut Account, ctx: &mut TxContext): Coin<HANEUL> {
...
public fun wrong_withdraw(state: &mut State, ctx: &mut TxContext): Coin<HANEUL> {
...
### Data storage in owned and shared objects

If your dApp data has a 1 to 1 relationship, it's best to use owned objects.
...
public fun new(deposit: Coin<HANEUL>, ctx: &mut TxContext): OwnedWallet {
    // Implementation omitted.
    abort(0)
}
...
public fun new_shared(deposit: Coin<HANEUL>, ctx: &mut TxContext) {
    // Implementation omitted.
