...
description: Packages published across Mainnet, Tesnet, and Devnet each have different addresses. These addresses are automatically tracked for you through automated address management.
...
When you publish or upgrade your package on multiple chains (Mainnet, Testnet, Devnet) separate addresses for each chain are tracked for you. This tracking is based on your active environment (run `haneul client active-env` if unsure). For example, if you set your active environment to an RPC connected to `testnet` and publish a package, the `Move.lock` records the address for that package and associates it with your active environment (`testnet`).
...
- Key-value entries correspond to data explained in [adopting automated address management](#adopting-automated-address-management-for-published-packages). Use the `haneul move manage-package` command as a frontend to manipulate these values.
:::
