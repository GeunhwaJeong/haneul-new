...
In the `[addresses]` section, you update the `haneul_package` address value to `0x0` again so the validator issues a new address for the upgrade package.
...
After upgrading, return the original `haneul_package` ID value in the `[addresses]` section:
