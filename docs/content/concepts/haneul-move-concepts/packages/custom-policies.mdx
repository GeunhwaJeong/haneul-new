...
You can make a package _immutable_ when it goes live to mitigate this risk using the Move `haneul::package::make_immutable` function to destroy its `UpgradeCap`. Making the package immutable, however, prevents future bug fixes and new features, which might not be practical or desired.

To address the security risk a single key poses while still providing the opportunity to upgrade live packages, Haneul offers _custom upgrade policies_. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.
...
You can change the current policy by calling one of the functions in `haneul::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `haneul::package::only_dep_upgrades` to restrict the policy to become additive, calling `haneul::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
const sender = execSync(`${HANEUL} client active-address`, {
	encoding: 'utf8',
}).trim();
const keyPair = (() => {
