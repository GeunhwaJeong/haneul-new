...
You can change the current policy by calling one of the functions in `haneul::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `haneul::package::only_dep_upgrades` to restrict the policy to become dependency-only, calling `haneul::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
