...
Refer to [when to use gRPC vs GraphQL](../guides/developer/getting-started/data-serving#when-to-use-grpc-vs-graphql-with-general-purpose-indexer) to access Haneul data.

## gRPC on Haneul

Protocol buffers define the gRPC interface. You can find the relevant **beta** `.proto` files at [haneul-apis on Github](https://github.com/GeunhwaJeong/haneul-apis/tree/main/proto), which apart from the gRPC messages (request and response payloads) include the following services:

| Service | Proto | Purpose |
| --- | --- | --- |
| `TransactionExecutionService` | `haneul/rpc/v2beta2/transaction_execution_service.proto` | Submit and execute signed transactions on the Haneul network. Wallets and apps use the service to send user actions to the network. |
| `LedgerService` | `haneul/rpc/v2beta2/ledger_service.proto` | Lookup specific checkpoints, transactions, objects and more from the current state and recent history of the Haneul network. “History” here refers to the recent past - limited to what a full node retains. |
| `LiveDataService` | `haneul/rpc/v2beta2/live_data_service.proto` | Query up-to-date on-chain data like balances, coin metadata, dynamic fields, or owned objects. Also supports dry-run simulations for transactions. |
| `SubscriptionService` | `haneul/rpc/v2beta2/subscription_service.proto` | Stream live updates for checkpoints. Ideal for building reactive systems such as indexers, bots, and dashboards. Refer to [Subscriptions for streaming data](#subscriptions-for-streaming-data). |
| `MovePackageService` | `haneul/rpc/v2beta2/move_package_service.proto` | Access metadata and content of Move packages deployed on the Haneul network. Useful for tooling, analysis, and smart contract introspection. |
| `SignatureVerificationService` | `haneul/rpc/v2beta2/signature_verification_service.proto` | Validate signatures outside transaction execution. Helps pre-verify payloads that may include [zklogin](./cryptography/zklogin.mdx) or other signatures, simulate authentication, or build custom signing workflows. |
| `NameService` | `haneul/rpc/v2beta2/name_service.proto` | Resolve human-readable HaneulNS names to their underlying name records, and perform reverse lookups from Haneul addresses back to linked names. |
...
Refer to the [list of RPC or data providers](https://www.notion.so/haneullabs/RPC-providers-offering-future-Haneul-data-primitives-2466d9dcb4e980a99a36e9aafd8c17e0?source=copy_link) that have enabled gRPC on their Full nodes. Reach out to a provider directly for access with higher rate limits.
...
haneul.rpc.v2beta2.LedgerService
haneul.rpc.v2beta2.LiveDataService
haneul.rpc.v2beta2.MovePackageService
haneul.rpc.v2beta2.NameService
haneul.rpc.v2beta2.SignatureVerificationService
haneul.rpc.v2beta2.SubscriptionService
haneul.rpc.v2beta2.TransactionExecutionService
```

#### List available APIs in the LedgerService

```shell
$ grpcurl <full node URL:port> list haneul.rpc.v2beta2.LedgerService
```

which should return something like:

```shell
haneul.rpc.v2beta2.LedgerService.BatchGetObjects
haneul.rpc.v2beta2.LedgerService.BatchGetTransactions
haneul.rpc.v2beta2.LedgerService.GetCheckpoint
haneul.rpc.v2beta2.LedgerService.GetEpoch
haneul.rpc.v2beta2.LedgerService.GetObject
haneul.rpc.v2beta2.LedgerService.GetServiceInfo
haneul.rpc.v2beta2.LedgerService.GetTransaction
```

#### Get the `events` and `effects` details of a particular transaction

```shell
$ grpcurl -d '{ "digest": "J4NvV5iQZQFm1xKPYv9ffDCCPW6cZ4yFKsCqFUiDX5L4" }' <full node URL:port> haneul.rpc.v2beta2.LedgerService/GetTransaction
```

#### Get the transactions in a particular checkpoint

```shell
$ grpcurl -d '{ "sequence_number": "164329987", "read_mask": { "paths": ["transactions"]} }' <full node URL:port> haneul.rpc.v2beta.LedgerService/GetCheckpoint
```

#### Get the latest information for a coin type

```shell
$ grpcurl -d '{ "coin_type": "0x2::haneul::HANEUL" }' <full node URL:port> haneul.rpc.v2beta2.LiveDataService/GetCoinInfo
```

#### List the objects owned by a particular address

```shell
$ grpcurl -d '{ "owner": "0x94096a6a54129234237759c66e6ef1037224fb3102a0ae29d33b490281c8e4d5" }' <full node URL:port> haneul.rpc.v2beta2.LiveDataService/ListOwnedObjects
```

#### List the dynamic fields in a particular object

```shell
$ grpcurl -d '{ "parent": "0xb57fba584a700a5bcb40991e1b2e6bf68b0f3896d767a0da92e69de73de226ac" }' <full node URL:port> haneul.rpc.v2beta2.LiveDataService/ListDynamicFields
```
...
```shell
$ buf curl --protocol grpc https://<full node URL>/haneul.rpc.v2beta2.SubscriptionService/SubscribeCheckpoints -d '{ "readMask": "sequenceNumber,digest,summary.timestamp" }'  --timeout 1m
```
...
Download all the `haneul/rpc/v2beta` proto files from [Github v2beta](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-rpc-api/proto) in the same folder.
...
const PROTO_PATH = path.join(__dirname, 'protos/haneul/rpc/v2beta/ledger_service.proto');
...
const LedgerService = haneulProto.haneul.rpc.v2beta.LedgerService;
...
Assuming you have the proto files from [Github v2beta](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-rpc-api/proto), run:

```shell
protoc --proto_path=./protos --go_out=. --go-grpc_out=. protos/haneul/rpc/v2beta/ledger_service.proto
```
...
import (
    ...
    pb "your_project/haneul/rpc/v2beta" // adjust path based on where your generated .pb.go files are
)
...
- If your generated files are in a nested directory like `haneul/rpc/v2beta`, your import in `main.go` should match:
```go
import pb "your_project/haneul/rpc/v2beta"
```
...
```shell
python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/haneul/rpc/v2beta/ledger_service.proto
```
...
```python
import grpc
from haneul.rpc.v2beta import ledger_service_pb2, ledger_service_pb2_grpc
...
- The import paths like `haneul.rpc.v2beta.ledger_service_pb2` depend on your proto structure.
...
- Generate client code from the official `.proto` definitions in [haneul-rpc-api](https://github.com/GeunhwaJeong/haneul-apis/tree/main/proto) to ensure compatibility and type safety.
...
Refer to [Considerations to enable gRPC](/guides/operator/haneul-full-node.mdx#considerations-to-enable-grpc-beta).
