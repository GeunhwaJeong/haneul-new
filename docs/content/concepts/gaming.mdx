...
description: Haneul offers features like dynamic NFTs, kiosks, soulbound assets, and on-chain randomness, to provide builders with the tools to create immersive, transparent, and fair gaming experiences.
...
Gaming on Haneul leverages blockchain technology to enhance in-game economies, ownership, and interactions. With features like dynamic NFTs, kiosks, soulbound assets, and on-chain randomness, Haneul provides builders with the tools to create immersive, transparent, and fair gaming experiences. Players benefit from true asset ownership, decentralized marketplaces, and seamless Web3 integration without compromising usability.
...
One of the aspects that defines a Web3 game is having objects within the game reside on a blockchain. Whether it's in-game currency, battle passes, skins, or any number of other objects, knowing how to create and integrate these items is the first step in building the desired user experience on Haneul.
...
Using Haneul, players can now have true ownership of their in-game currency. The currency exists as a token on the Haneul blockchain, where players can conceivably buy, sell, or swap that currency for anything else that also lives on chain. If you create an ecosystem of games, players can purchase currency in one of your games but spend it in another one that also uses that same currency. The possibilities are vast, but you must first learn how to create the currency. The following topics can get you started.
...
NFTs are able to represent many traditional game features. For example, in-game objects, battle passes, rewards, skins, game cards or keys, and loot boxes are just a few of the features that you can create using NFTs on Haneul. The following topics and sections provide more details.
...
On Haneul, everything is an object and all objects are NFTs. For the purpose of the gaming discussion, you can also think of these NFTs as game assets. 
...
For more information on the computation of gas fees, see [Haneul Gas Pricing](./tokenomics/gas-pricing.mdx).
...
See [Transfer to Object](./transfers/transfer-to-object.mdx) for more details on how to effectively transfer objects to other objects on Haneul.
...
The Haneul Object Display standard is a tool that helps define how objects appear in apps and interfaces. It works like a template where you can insert data from an object to control how it's displayed. Use it to manage how different types of data are shown, even if the data itself is stored on chain while the display happens off-chain (in apps or websites).
...
See [Haneul Object Display](../standards/display) for more details on the standard.
...
On Haneul, owned objects are either freely transferable or non-transferable. To ensure royalties, Haneul provides a standard called Kiosk. A kiosk is a shared object that restricts access to a single address or user. See [Haneul Kiosk](../standards/kiosk.mdx) for an in-depth look at the Kiosk standard. 

The kiosk "owner" (although a shared object has no owner from the perspective of Haneul, the smart contract ensures that only one address is permitted to access it) is allowed to:
...
An empty transfer policy, one that does not have rules, means that the asset is freely tradable. Since Kiosk only allows the “list” and “buy” pair of operations, a transfer is possible by setting the price to 0 HANEUL.
...
ShadowQuest wants to onboard players without adding complexity, especially those unfamiliar with Web3. By using [zkLogin](../concepts/cryptography/zklogin.mdx), users can sign in using social platforms like Google, Facebook, Twitch, and Apple. This automatically creates a Haneul wallet linked to their ShadowQuest account, making blockchain interactions seamless.
...
Dynamic NFTs on the Haneul network allow vehicles to securely update attributes and metadata over time. Every upgrade and customization is recorded on-chain, ensuring each vehicle's uniqueness and authenticity.
...
On-chain logic enables secure and transparent management of events and betting systems. The Haneul network ensures fairness, with immutable records of bets and outcomes.
...
By leveraging Haneul's on-chain randomness, ArcaneBattles introduces unpredictable elements such as random card draws, critical hit chances, and random effects from certain cards. This randomness is verifiable and secure, preventing manipulation by any party and maintaining fairness across all gameplay aspects.
...
- [Coin Standard](../standards/coin.mdx): The Haneul Coin standard enables you to create a broad range of fungible tokens on the Haneul network to satisfy a number of use cases. The Coin standed on Haneul is equivalent to the ERC-20 technical standard on Ethereum.
- [Create Coins and Tokens](../guides/developer/coin.mdx): Learn how to mint coins and tokens on the Haneul network.
- [Create a Non-Fungible Token](../guides/developer/nft.mdx): On Haneul, everything is an object. Moreover, everything is a non-fungible token (NFT) as its objects are unique, non-fungible, and owned.
- [Asset Tokenization](../guides/developer/nft/asset-tokenization.mdx): Learn how to tokenize assets on the Haneul blockchain. Asset tokenization refers to the process of representing real-world assets, such as real estate, art, commodities, stocks, or other valuable assets, as digital tokens on the blockchain network.
...
- [Kiosk](../standards/kiosk.mdx): Kiosk is a decentralized system for commerce applications on Haneul. Kiosk is a part of the Haneul framework, native to the system, and available to everyone.
- [Kiosk Apps](../standards/kiosk-apps.mdx): Kiosk apps are a way to extend the functionality of Haneul Kiosk while keeping the core functionality intact. You can develop apps to add new features to a kiosk without having to modify the core code or move the assets elsewhere.
- [zkLogin](../concepts/cryptography/zklogin.mdx): zkLogin is a Haneul primitive that enables you to send transactions from a Haneul address using an OAuth credential, without publicly linking the two.
- [Haneul Foundation blog](https://blog.haneul.io/tag/gaming/): Blog posts from the Haneul Foundation with the `gaming` tag.
- Guides for example games
    - [Coin Flip](../guides/developer/app-examples/coin-flip.mdx): Learn Haneul through a coin flip dApp that covers the full end-to-end flow of building a Haneul Move module and connecting it to a React Haneul dApp.
    - [Blackjack](../guides/developer/app-examples/blackjack.mdx): Learn Haneul using an example implementation of the popular casino game blackjack.
    - [Plinko](../guides/developer/app-examples/plinko.mdx): Learn Haneul through an example implementation of the popular casino game, Plinko.
- [Mysticon Legends](https://github.com/GeunhwaJeong/mysticon-legends) repo on GitHub: A blockchain-based game where players collect, train, and battle with mythical creatures called Mysticons.
- [Web3 Mini Games built on Haneul](https://mini-games.haneul.io/): A collection of mini games, to inspire the community of Haneul.