...
description: Learn about transfer policies on Haneul.
keywords: [ transfer, policy, transferpolicy, transfer receipt, transfer request, transferrequest ]
---

A [`TransferPolicy`](/references/framework/haneul_haneul/transfer_policy) on Haneul is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can use a `TransferPolicy` within a Haneul Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed.
...
The `TransferPolicy` module allows removing any rule at any time, as guaranteed by the constraints on the rule's configuration. This example module has no configuration and accepts a `Coin<HANEUL>` of any value.
...
The `haneul::transfer_policy` module provides public functions to access these fields: `paid()`, `item()`, and `from()`.
