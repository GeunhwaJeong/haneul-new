...
haneul::transfer::public_transfer(b, @0xADD);

// Transfers the object `c` to the object with object ID 0x0B
haneul::transfer::public_transfer(c, @0x0B);
```
...
To explain further, look at the core of the receiving interface in Move, which is defined in the `haneul::transfer` module in the Haneul framework:
...
Just like with [custom transfer policies](./custom-rules.mdx), Haneul allows for the definition of custom receivership rules for `key`-only objects. In particular, you can use the `haneul::transfer::receive` function only on objects defined in the same module as the call to `haneul::transfer::receive`--just like you can use the `haneul::transfer::transfer` function only on objects defined in the module where it's being used.

Similarly for objects that also have the `store` ability, anyone can use the `haneul::transfer::public_receive` function to receive them--just like `haneul::transfer::public_transfer` can transfer any objects that have the `store` ability on them.
...
When creating transactions, you interact with `Receiving` transaction inputs almost exactly as you would with other object arguments in the Haneul TypeScript SDK. For example, if in the [Simple Account](#simple-account) example that follows you want to send a transaction that receives a coin object with ID `0xc0ffee` that was sent to your account at `0xcafe`, you can do the following using either the Haneul TypeScript SDK or Haneul Rust SDK:
...
Because the `receive_object` function is generic over the object being received, it can only receive objects that are both `key` and `store`. `receive_object` must also use the `haneul::transfer::public_receive` function to receive the object and not `haneul::transfer::receive` because you can only use `receive` on objects defined in the current module.
...
module examples::soul_bound {
    ...
    public fun return_object(soul_bound: SoulBound, receipt: ReturnReceipt) {
        ...
        haneul::transfer::transfer(soul_bound, return_to);
    }
}
```
