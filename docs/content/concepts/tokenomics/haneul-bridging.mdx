...
Haneul supports bridging through [Haneul Bridge](#haneul-bridge), [Wormhole Connect](#wormhole-connect), [Wormhole Portal Bridge](#wormhole-portal-bridge), and [ZetaChain](#zetachain).

## Haneul Bridge

Haneul Bridge is the native bridge for the Haneul network. It is built into the core architecture of Haneul, providing optimal integration and operation. Haneul Bridge enables the movement of digital assets between Haneul and other blockchains, preserving security and maintaining interoperability. It supports secure and efficient transfer of native and wrapped ETH to and from Haneul. Haneul Bridge leverages Haneul’s capabilities to offer fast transactions, lower costs, and a decentralized architecture.

You can bridge tokens at [bridge.haneul.io](https://bridge.haneul.io/).
...
A limiter protects funds by constraining the total value of assets leaving Haneul Bridge in a 24-hour window. It tracks value hourly and aggregates from the last 24 hours, refreshing every hour.  

The limit applies globally and varies by direction (Ethereum → Haneul vs. Haneul → Ethereum). It also caps the maximum single transfer.  
...
The global limit is currently $16 million from Ethereum to Haneul and $7 million from Haneul to Ethereum per 24 hours.
...
On EVM chains and Haneul, Wormhole Connect can pay destination-chain gas automatically. Users only pay gas on the source chain.

### Wormhole Connect gas drop-off {#gas-drop-off}

Users can pay an extra fee on the source chain to receive a small amount of native gas on the destination chain. For example, bridging USDC from Ethereum to Haneul can include a fee in USDC to receive HANEUL for gas. This feature is supported on EVM chains and Haneul.  
