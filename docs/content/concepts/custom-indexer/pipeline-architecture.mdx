...
description: The `haneul-indexer-alt-framework` provides two distinct pipeline architectures. Understand the differences between the sequential and concurrent pipelines that the `haneul-indexer-alt-framework` provides to decide which best haneults your project needs.
keywords: [ indexer, index data, haneul-indexer, custom indexer, sequential pipeline, concurrent pipeline ]
mdx:
    format: md
---

The `haneul-indexer-alt-framework` provides two distinct pipeline architectures. Understanding their differences is crucial for choosing the right approach.
...
<li><span className="text-haneul-success-dark">✓</span> You want straightforward implementation with direct commits and simple queries.</li>
<li><span className="text-haneul-success-dark">✓</span> Team prefers predictable, easy-to-debug behavior.</li>
<li><span className="text-haneul-success-dark">✓</span> Current performance meets your requirements.</li>
<li><span className="text-haneul-success-dark">✓</span> Operational simplicity is valued.</li>
</ul>
...
<ul className="list-none pl-2">
<li><span className="text-haneul-success-dark">✓</span> Performance optimization is essential.</li>
<li><span className="text-haneul-success-dark">✓</span> Sequential processing can't keep up with your data volume.</li>
<li><span className="text-haneul-success-dark">✓</span> Your team is willing to handle the additional implementation complexity for the performance benefits.</li>
</ul>
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs#fun=batch}

`commit()`: Database write logic.

{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs#fun=commit}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/sequential/committer.rs#send}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/ingestion/regulator.rs#regulator}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#variable=MIN_EAGER_ROWS}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#variable=MAX_PENDING_ROWS}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/mod.rs#struct=CommitterConfig}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/mod.rs#struct=WatermarkPart}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/mod.rs#struct=CommitterConfig highlight=watermark_interval_ms}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#struct=PrunerConfig highlight=delay_ms}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#struct=PrunerConfig highlight=prune_concurrency}
...
{@inject: ../../../../crates/haneul-pg-db/src/schema.rs noComments}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/ingestion/mod.rs#struct=IngestionConfig highlight=ingest_concurrency}
    
**`Broadcaster` to `Processor`**: `checkpoint_buffer_size` slots → `Broadcaster` blocks, upstream pressure.

{@inject: ../../../../crates/haneul-indexer-alt-framework/src/ingestion/mod.rs#struct=IngestionConfig highlight=checkpoint_buffer_size}    
    
**`Processor` to `Collector`**: `FANOUT + PIPELINE_BUFFER` slots → All workers block on `send()`.

{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#variable=processor_tx}
    
**`Collector` to `Committer`**: `write_concurrency + PIPELINE_BUFFER` slots → `Collector` stops accepting.

{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#buff}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/pipeline/concurrent/collector.rs#collector}
...
{@inject: ../../../../crates/haneul-indexer-alt-framework/src/ingestion/regulator.rs#bound}
...
- [Custom Indexing Framework](../custom-indexing-framework.mdx): The `haneul-indexer-alt-framework` is a powerful Rust framework for building high-performance, custom blockchain indexers on Haneul, providing customizable, production-ready components for data ingestion, processing, and storage.
- [Life of a Transaction](../haneul-architecture/transaction-lifecycle.mdx): The life of a transaction on the Haneul network has some differences compared to those from other blockchains.
- [Build a Custom Indexer](../../guides/developer/advanced/custom-indexer.mdx): Establishing a custom indexer helps improve latency, allows pruning the data of your Haneul Full node, and provides efficient assemblage of checkpoint data. This guide walks you through the process.
