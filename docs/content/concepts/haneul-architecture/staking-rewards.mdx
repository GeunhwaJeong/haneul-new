...
Haneul is a Delegated-Proof-of-Stake (DPoS) system, where the voting power of validators is granted by stakes delegated to them from HANEUL token holders. When rewards are distributed at the end of each epoch, validators receive their share of the rewards based on their performance and their commission rate. The rewards auto-compound as validators staking pools receive new delegated stakes.

## Staking on Haneul versus other blockchains

The Haneul staking mechanism has a few features that might be different from staking on other blockchains:

- Staking is self-custodial on Haneul: stakers keep their staked HANEUL tokens in an owned object.
- Staking rewards auto-compound on Haneul due to its liquidity-pool-inspired design.
...
Because validator rewards are distributed to them as regular stake objects, staking rewards withdrawal works exactly the same for validators as for stakers. More precisely, a validator can call the  [`0x3::haneul_system::request_withdraw_stake`](https://github.com/GeunhwaJeong/haneul/blob/6234ae2cc8137f3a2a34cd0aa1ac4fd5f31260e2/crates/haneul-framework/packages/haneul-system/sources/haneul_system.move#L272) function to withdraw their stake and receive their rewards.
