...
To participate in consensus, validators must [stake HANEUL tokens](/guides/operator/validator/validator-rewards.mdx) on the network. Haneul uses delegated roof-of-stake (DPoS) to determine which validators operate the network and their voting power. Validators are incentivized to participate in good faith through a share of transaction fees, [staking rewards](/guides/operator/validator/validator-rewards.mdx), and slashing of stake and staking rewards in case of misbehavior.

### Epochs

An [_epoch_](/concepts/haneul-architecture/epochs.mdx) is a duration of time where the Haneul validators and their stakes do not change. An epoch is about 24 hours on both [Mainnet](/concepts/haneul-architecture/networks.mdx) and [Testnet](/concepts/haneul-architecture/networks.mdx). At an epoch boundary, [reconfiguration](/concepts/haneul-architecture/epochs.mdx) might occur and can change the set of validators participating in the network and their voting power. Conceptually, reconfiguration starts a new instance of the Haneul protocol with the previous epoch's final state as genesis and the new set of validators as the operators. Besides validator set changes, [tokenomics](/concepts/tokenomics.mdx) operations such as staking and un-staking, and distribution of staking rewards are also processed at epoch boundaries.
...
<RelatedLink to="/concepts/haneul-architecture/haneul-security.mdx" />

<RelatedLink href="https://docs.haneul.io/paper/mysticeti.pdf" label="MYSTICETI: Reaching the Latency Limits with Uncertified DAGs" desc="Whitepaper that documents the Mysticeti protocol." />