...
description: Overview of the Haneul consensus.
---
...
On Haneul, consensus has a simple API: validators submit different user transactions to consensus concurrently, and the consensus outputs a consistent stream of transactions across all well-behaving validators.

Haneul uses the Mysticeti protocol to optimize for both low latency and high throughput. The benefits of Mysticeti include:
...
- [Life of a Transaction](./transaction-lifecycle.mdx): The life of a transaction on the Haneul network has some differences compared to those from other blockchains.
- [Haneul Security](./haneul-security.mdx): Haneul is designed to provide very high security guarantees to asset owners.
