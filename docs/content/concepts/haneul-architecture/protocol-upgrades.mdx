...
description: The Haneul protocol, framework, and execution engine are frequently extended to include new functionality and bug fixes. The upgrade process ensures all clients use the same source.
keywords: [ protocol upgrades, framework upgrades, protocol config, feature flags, validator upgrades, protocol version ]
---

The Haneul protocol, framework, and execution engine are frequently extended to include new functionality and bug fixes. This functionality is added as new code released to validator operators as part of regular software releases.

As [consensus](/concepts/haneul-architecture/consensus.mdx) requires that all validators agree on the results of executing each transaction, how is code released that changes transaction execution, given that operators cannot all upgrade at the same instant? Further, how is it ensured that all transaction history can be replayed even after functionality has changed?

To solve this, Haneul uses a process called _protocol upgrades_.
...
1. A new release of the Haneul validator software is built and released to validator and full node operators.
...
Not all new Haneul functionality comes from validator code changes. Developers also extend the Haneul framework. For example, they might add new native functions to expose functionality to smart contracts. The process for framework updates is similar to protocol upgrades.

Instead of feature flags, Haneul objects coordinate framework changes. The Haneul framework is a special object with ID `0x2`. The Move source code for the framework is built into the validator binary.
...
<RelatedLink to="/concepts/haneul-architecture/consensus.mdx" />