...
description: The Haneul protocol, framework, and execution engine are frequently extended to include new functionality and bug fixes. The process to upgrade the network ensures clients use the same source.
---
...
This poses the following challenge: How do we release code that changes transaction execution, given that it is not possible to ensure that all operators upgrade their software at the same instant? Further, how do we ensure that all Haneul transaction history can be replayed even after functionality has changed?

To solve this problem, Haneul uses a process called protocol upgrades.

# Protocol upgrade process

An outline of the process used for protocol upgrades includes the following steps:

1. A Haneul developer codes the new feature, but restricts access to the feature using a feature flag. A feature flag is a boolean config variable that is initially set to false.
...
Not all new Haneul functionality comes in the form of changes to the validator code. There are also changes to the Haneul framework. For instance, Haneul developers periodically add new native functions to the framework to expose new functionality to smart contracts. The process for framework updates is similar to protocol upgrades.

Instead of using feature flags, however, Haneul objects are used to coordinate framework changes. The Haneul framework is a special object with ID `0x2`.
