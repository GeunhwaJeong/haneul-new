...
On Haneul, an _epoch_ is a period of time defined by the network. During this time, the Haneul [validator](/guides/operator/validator/validator-config.mdx) set and their stakes remain unchanged. On both [Mainnet](/concepts/haneul-architecture/networks.mdx) and [Testnet](/concepts/haneul-architecture/networks.mdx), an epoch is about 24 hours. Epochs on Devnet are 1 hour. This timeframe allows validators to process transactions efficiently without worrying about ad hoc validator changes.
...
- Inadvertent logic traps in [smart contract](/guides/developer/getting-started/hello-world.mdx) code, for example, when handling [sponsored transactions](/guides/developer/haneul-101/sponsor-txn.mdx#client-equivocation-risk).
...
1. **Distribution of gas rewards:** Computation gas fees are distributed to the validator staking reward pool, from which stakers can withdraw. Storage fees are allocated to a storage fund, playing a vital role in the [Haneul tokenomics](/concepts/tokenomics.mdx).
