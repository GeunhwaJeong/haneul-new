...
Checkpoints on Haneul are batches of transactions that represent consistent blockchain state snapshots. Each checkpoint contains complete transaction details, events, object changes, and execution results with guaranteed ordering. See [Life of a Transaction](/guides/developer/transactions/transaction-lifecycle) for more information on checkpoints.
...
To build your own pipelines for application-specific data, see [Building Your First Custom Indexer](/guides/developer/accessing-data/custom-indexer/build.mdx). You can also reach out to a [data indexer operator](/references/awesome-haneul.mdx#indexers--data-services) that might already have set one up for your needs.
