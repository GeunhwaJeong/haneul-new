...
- **Remote stores** connect to public checkpoint stores like [`https://checkpoints.mainnet.haneul.io`](https://checkpoints.mainnet.haneul.io/). This is the most straightforward way to get started without running your own infrastructure.
- **Local files** allow you to process checkpoint files dumped by a local Haneul full node, providing the lowest latency, but should be used for testing only as these files are not automatically cleaned up.
- **RPC endpoints** enable direct connection to a Haneul full node RPC endpoint, allowing you to use a trusted data source that you control or to connect to networks (like Devnet) that lack a remote store.
