...
- **General-purpose Indexer:** Ingests and transforms Haneul checkpoint data using configurable and parallel pipelines, and writes it into a Postgres-compatible database. Can be configured to use the Haneul remote checkpoint store and a full node as its sources.
...
- **GraphQL service:** Serves structured queries over indexed data. Follows the [GraphQL specification](https://graphql.org/) and the supported schema is documented in the [GraphQL API reference](/references/haneul-graphql). Also take a look at the [getting started guide](/guides/developer/advanced/graphql-rpc.mdx).
