...
description: The GraphQL RPC Beta service offers a structured way for your clients to interact with data on the Haneul blockchain. It accesses data processed by a general-purpose indexer and can connect to an archival store for historical network state.
...
- **General-purpose Indexer:** Ingests and transforms Haneul checkpoint data using configurable and parallel pipelines, then writes it into a Postgres-compatible database. It can be configured to use the Haneul remote checkpoint store and a full node as its sources.
...
- **GraphQL service:** Serves structured queries over indexed data. It follows the [GraphQL specification](https://graphql.org/) and the supported schema is documented in the [GraphQL API reference](/references/haneul-graphql). Refer to the [GraphQL getting started guide](/guides/developer/accessing-data/query-with-graphql.mdx).

- **Archival Service:** Enables point lookups for historical data from a key-value store. If unavailable, the GraphQL service falls back to the Postgres-compatible database for lookups, which might be limited by that database's retention policy. See [Archival Store and Service](/guides/developer/accessing-data/archival-store.mdx) for more information.
...
- Run the Indexer pipelines and GraphQL service, while using the [Archival Service](/guides/developer/accessing-data/archival-store.mdx) and a full node from an RPC provider or indexer operator.
...
<ImportContent source="crates/haneul-indexer-alt-schema/src/schema.rs" mode="code" />
</details>
...
The pipeline composition, concurrency, and deployment grouping is configured through a TOML config file. A built-in [`GenerateConfig` command](https://github.com/GeunhwaJeong/haneul/blob/58eabc882671ceb6266473a5531253a90b22a66f/crates/haneul-indexer-alt/src/main.rs#L109) is provided to output sample configuration files for different deployment setups. The configuration generated by this command includes all pipelines in a single indexer deployment.
...
This document focuses on the general-purpose Indexer that powers GraphQL. If you want to build your own pipelines for application-specific data (such as Deepbook order books, Walrus blob metadata, Seal access events, and so on), refer to the [Build Your First Custom Indexer](/guides/developer/accessing-data/custom-indexer/build.mdx).
...
Current implementation supports [GCP Bigtable](https://cloud.google.com/bigtable) which is a highly scalable and performant data store. If you plan to operate your own archival store, refer to `haneul-kvstore` and `haneul-kv-rpc` for indexer setup and RPC service implementation respectively. For the indexer setup, make sure to use the [custom indexing framework](/guides/developer/accessing-data/custom-indexer.mdx). If you're interested in contributing support for other scalable data stores, reach out on GitHub by creating a new issue.

<details>
<summary>
`main.rs` in `haneul-kvstore`
</summary>
<ImportContent source="crates/haneul-kvstore/src/main.rs" mode="code" />
</details>

<details>
<summary>
`main.rs` in `haneul-kv-rpc`
</summary>
<ImportContent source="crates/haneul-kv-rpc/src/main.rs" mode="code" />
</details>
...
<RelatedLink to="/guides/developer/accessing-data/custom-indexer" />
