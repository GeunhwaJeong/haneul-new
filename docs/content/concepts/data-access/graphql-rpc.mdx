...
GraphQL provides a flexible way to query the Haneul network. This page covers the core concepts for working with GraphQL on Haneul RPC, including request headers, query composition with variables and fragments, pagination strategies, query scope, and service limits.

For practical examples, see [Querying Haneul RPC with GraphQL](/guides/developer/accessing-data/query-with-graphql.mdx). For comprehensive GraphQL fundamentals, consult the introductory documentation from [GraphQL](https://graphql.org/learn/) and [GitHub](https://docs.github.com/en/graphql/guides/introduction-to-graphql).
...
- **GraphQL service:** Serves structured queries over indexed data. It follows the [GraphQL specification](https://graphql.org/) and the supported schema is documented in the [GraphQL API reference](/references/haneul-graphql).
...
The GraphQL service exposes a query surface conforming to GraphQL concepts. It allows pagination, filtering, and consistent snapshot queries. The service also supports runtime configuration for schema, query cost limits, and logging. The GraphQL schema is defined in the [GraphQL reference](/references/haneul-graphql). You can explore supported types and fields there, use the GraphiQL IDE to test queries, and read documentation on the up-to-date schema.
