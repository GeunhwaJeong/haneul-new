---
title: GraphQL for Haneul RPC (Beta)
beta: devnet, testnet, mainnet
description: Use GraphQL to make Haneul RPC calls. This feature is currently in Beta.
keywords: [ graphql, graphql headers, x-haneul-rpc-version, x-haneul-rpc-show-usage, variables, fragments, pagination, graphql limits, make rpc call, haneul rpc calls ]
---

GraphQL provides a flexible and powerful way to query the Haneul network. This page covers the core concepts you need to understand when working with GraphQL for Haneul RPC, including request headers, query composition with variables and fragments, pagination strategies, query scope, and service limits.

For practical examples and a quick start guide, see [Querying Haneul RPC with GraphQL](/guides/developer/accessing-data/query-with-graphql.mdx).
...
Switch any apps that still use the GraphQL Alpha endpoints (`https://haneul-mainnet.haneul-labs.com/graphql`, `https://haneul-testnet.haneul-labs.com/graphql`, and `https://haneul-devnet.haneul-labs.com/graphql`) to the GraphQL Beta endpoints as soon as possible to avoid disruption of service.

- GraphQL Beta Mainnet: `https://graphql.mainnet.haneul.io/graphql`
- GraphQL Beta Testnet: `https://graphql.testnet.haneul.io/graphql`
- GraphQL Beta Devnet: `https://graphql.devnet.haneul.io/graphql`
...
For more examples of scope usage, see [Querying Haneul RPC with GraphQL](/guides/developer/accessing-data/query-with-graphql.mdx).
...
- `address.coins` and `address.stakedHaneuls` are being deprecated in favor of `address.objects` with the corresponding `type` filter.
...
Dry-run now supports the same input schema as `gRPC`'s [`SimulateTransaction`](/references/fullnode-protocol#haneul-rpc-v2-Transaction) in JSON form. This means that it can also accept transactions where the inputs have not been fully resolved or serialized, perform gas estimation, and coin selection. The feature is now called `simulateTransaction`.

## Related links

<RelatedLink to="/guides/developer/accessing-data/query-with-graphql.mdx" />
<RelatedLink to="/references/haneul-graphql.mdx" />
<RelatedLink to="/concepts/data-access/graphql-indexer" />
<RelatedLink href="https://graphql.testnet.haneul.io/graphql" label="Haneul Testnet GraphiQL" desc="Haneul GraphiQL IDE for Testnet." />
<RelatedLink href="https://graphql.mainnet.haneul.io/graphql" label="Haneul Mainnet GraphiQL" desc="Haneul GraphiQL IDE for Mainnet." />