...
Commands:
  ...
  new-address                 Generate new address and keypair with keypair scheme flag {ed25519
                                  ...
                                  m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0 for secp256k1
                                  or m/74'/8282'/0'/0/0 for secp256r1. Word length can be { word12 |
  ...
  pay-all-haneul                 Pay all residual HANEUL coins to the recipient with input coins,
                                  after deducting the gas cost. The input coins also include the
                                  coin for gas payment, so no extra gas coin is required
  send-funds                  Send funds to an address balance using the haneul::coin::send_funds
                                  API. This sends funds to the recipient's address balance (not as a
                                  coin object)
  pay-haneul                     Pay HANEUL coins to recipients following specified amounts, with
  ...
  transfer-haneul                Transfer HANEUL, and pay gas with the same HANEUL coin object. If amount
  ...
  replay-transaction          Replay a given transaction to view transaction effects
                                  (deprecated; use `haneul replay` instead)
  replay-batch                Replay transactions listed in a file (deprecated; use `haneul replay`
                                  instead)
  replay-checkpoint           Replay all transactions in a range of checkpoints (deprecated; use
                                  `haneul replay` instead)
