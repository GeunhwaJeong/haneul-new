...
Usage: haneul replay [OPTIONS]

Options:
      --client.config <CONFIG>       Sets the file storing the state of our user accounts (an empty
                                     one will be created if missing)
      --client.env <ENV>             The Haneul environment to use. This must be present in the current
                                     config file
  -y, --yes                          Create a new haneul config without prompting if none exists
