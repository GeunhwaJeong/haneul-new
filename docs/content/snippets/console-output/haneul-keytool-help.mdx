...
Commands:
  ...
  convert
          ...
          key starting with "haneulprivkey"). Hex private key format import and export are both
          deprecated in Haneul Wallet and Haneul CLI Keystore. Use `haneul keytool import` if you wish to
          import a key to Haneul Keystore
  decode-or-verify-tx
  ...
  generate
          ...
          optional derivation path, default to m/44'/8282'/0'/0'/0' for ed25519 or m/54'/8282'/0'/0/0
          for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1. Word length can be { word12 | word15 |
          word18 | word21 | word24} default to word12 if not specified
  import
          Add a new key to Haneul CLI Keystore using either the input mnemonic phrase or a Bech32
          encoded 33-byte `flag || privkey` starting with "haneulprivkey", the key scheme flag {ed25519
          | secp256k1 | secp256r1} and an optional derivation path, default to m/44'/8282'/0'/0'/0'
          for ed25519 or m/54'/8282'/0'/0/0 for secp256k1 or m/74'/8282'/0'/0/0 for secp256r1.
  ...
  export
          Output the private key of the given key identity in Haneul CLI Keystore as Bech32 encoded
          string starting with `haneulprivkey`
  list
  ...
  load-keypair
          ...
          HaneulKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
          ...
          `privkey`
  multi-sig-address
          To MultiSig Haneul Address. Pass in a list of all public keys `flag || pk` in Base64. See
  ...
  multi-sig-combine-partial-sig
          ...
          as signature field for `haneul client execute-signed-tx`. The sum of weights of all
          signatures must be >= the threshold
  multi-sig-combine-partial-sig-legacy
          
  show
          Read the content at the provided file path. The accepted format can be [enum HaneulKeyPair]
  ...
  sign
          Create signature using the private key for the given address (or its alias) in haneul
  ...
  sign-kms
          ...
          a message and the base64 pubkey. Generate PubKey from pem using HaneulLabs/base64pemkey
          ...
          be used
  unpack
          This takes [enum HaneulKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It outputs
          the keypair into a file at the current directory where the address is the filename, and
          prints out its Haneul address, Base64 encoded public key, the key scheme, and the key scheme
          flag
  ...
  zk-login-sig-verify
          ...
          signature based its latest JWK fetched. Example request: haneul keytool zk-login-sig-verify
  ...
  zk-login-insecure-sign-personal-message
          ...
          zklogin signature for the given data and max epoch. e.g. haneul keytool
