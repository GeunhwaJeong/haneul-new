...
{@inject: crates/haneul-indexer-alt-framework/src/pipeline/processor.rs#variable=FANOUT}
    
Each worker calls your `Handler::process()` method independently. 

{@inject: crates/haneul-indexer-alt-framework/src/pipeline/processor.rs#variable=values}
