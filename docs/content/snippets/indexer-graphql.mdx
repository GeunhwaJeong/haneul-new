The GraphQL and `haneul-indexer-alt` (Indexer) stack are part of the [Haneul data access infrastructure](../guides/developer/haneul-101/data-serving.mdx). The stack provides access to on-chain data through a high-performance GraphQL service backed by a scalable and general-purpose indexer. This stack is optimized for developers who need flexible queries, structured output, and historical access to data (with configurable retention) across the Haneul network.
...
The General-purpose Indexer includes configurable checkpoint-based pipelines that extract data from the Haneul remote checkpoint store and full nodes. The pipelines write processed data to a database optimized for GraphQL query access.
