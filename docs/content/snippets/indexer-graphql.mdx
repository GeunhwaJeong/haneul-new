The GraphQL and `haneul-indexer-alt` (Indexer) stack are part of the [Haneul data access infrastructure](/concepts/data-access/data-serving.mdx). The stack provides access to onchain data through a high-performance GraphQL service backed by a scalable and general-purpose indexer. This stack is optimized for developers who need flexible queries, structured output, and historical access to data (with configurable retention) across the Haneul network.
