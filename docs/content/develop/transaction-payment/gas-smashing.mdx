...
In the basic case, gas smashing merges the real `Coin<HANEUL>` objects that you supply as gas payment. When you provide multiple `Coin<HANEUL>` objects, the network merges them into a single coin and deducts the gas fee from that coin.
...
The compatibility layer can present compatibility coin reservations to older clients. A compatibility coin reservation is a synthetic object reference that reserves address balance funds, not a persisted `Coin<HANEUL>` object. These reservations still participate in gas smashing, and the outcome depends on which gas payment you list first:

- If the first payment is a real `Coin<HANEUL>`, the network smashes every payment into that coin. Any coin reservation is fully withdrawn from the address balance into the resulting coin.
...
You can also use a coin reservation deliberately, even when you do not need the compatibility layer. If you pay gas from an address balance but want extra `HANEUL` available within the PTB, you have two options:
