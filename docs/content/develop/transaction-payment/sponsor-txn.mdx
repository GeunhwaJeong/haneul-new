...
description: Sponsored transactions let you pay gas fees on behalf of another user, reducing onboarding friction for users who don't own HANEUL.
...
A transaction on Haneul requires a gas payment to execute. The payment is a list of `0x2::coin::Coin<0x2::haneul::HANEUL>` objects.

Sponsored transactions let you pay gas fees on behalf of another user. This reduces onboarding friction because users can execute transactions without owning HANEUL or understanding gas mechanics. As a sponsor, you provide the gas payment object for the transaction.
...
![User-proposed sponsored transaction sequence](./images/sequence_sponsor-txn-user-proposed_v1.png)
...
1. A sponsor constructs a `TransactionData` object that contains the transaction details and associated gas fee data. The sponsor signs it to generate a `Signature` before sending it to a user. You can send unsigned `TransactionData` through email, SMS, or an application interface.
...
![Sponsor-proposed sponsored transaction sequence](./images/sequence_sponsor-txn-sponsor-proposed_v1.png)
...
![Wildcard gas payment sponsored transaction sequence](./images/sequence_sponsor-txn-wildcard_v1.png)
...
### Concurrent object use

Sponsored transactions still need valid, current object references for both the user's inputs and the sponsor's gas payment. If another inflight transaction already uses one of those same object versions, the sponsored transaction can be rejected and must be rebuilt or re-signed with updated references. If competing transactions split validator reservations, the object can be equivocated until the next epoch.

Both the user and the sponsor can introduce this conflict:

- The user can reuse one of the transaction inputs in another inflight transaction.

- The sponsor can reuse the same gas object in another inflight sponsored transaction.

To reduce this risk:

- Sponsors should use dedicated gas coin pools and avoid reusing the same gas object in multiple inflight transactions.

- Clients should avoid signing multiple transactions that use the same owned object inputs.

- Both parties must sign the entire `TransactionData`, including `GasData`. This prevents a third party, such as a malicious full node, from intercepting partially signed data and changing the gas payment.
...
The following example shows the complete sponsored transaction flow:
...
tx.setSender(user.toHaneulAddress());
tx.moveCall({ target: '0xPACKAGE::module::function' });

// 2. Sponsor sets gas payment
tx.setGasOwner(sponsor.toHaneulAddress());
...
Sponsorship is not the same as multi-sig. A sponsor pays gas but does not co-author the transaction intent. Multi-sig requires multiple parties to approve the same transaction. See [Multi-signature](/develop/transactions/transaction-auth/multisig) for details.
