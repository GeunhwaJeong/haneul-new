...
description: Recommended Move best practices for Haneul development.
keywords: ['Move best practices', 'Haneul conventions', 'smart contract patterns', 'Move coding standards', 'package organization', 'module design', 'Haneul Move development']
---
...
- `Move.lock` file that the Haneul Move toolchain automatically generates to lock the versions of the dependencies.
...
Here, public functions are the functions modifying state and view functions are often onchain getters or offchain helpers. The latter are not necessary because you can query objects to read their data. The `init` function should be the first function in the module, if it exists.
...
Structs support positional fields that can be used for basic wrappers, dynamic field keys, or as tuples.
...
To maintain composability, use capability objects instead of arrays of addresses for access control. For security guidance on capabilities, allowlists, revocation, and shared object authorization, see [Access control](/develop/security/best-practices) in the security best practices guide.

:::tip

For privileged functions, take an explicit capability parameter rather than relying on sender-only authorization. Only use `tx_context::sender()` as the authorization check where the signer is genuinely the only valid actor. When a capability grants rights over a specific object, validate the capability/object relationship (for example, match an ID stored in the capability). Design in a revocation or rotation path from the start, and emit events for privileged actions so they are auditable. See [Access control](/develop/security/best-practices#access-control) for the full guidance.
...
Document your code by explaining functions and structs in basic terms using the `///` syntax (doc comment). If you want to add technical insights for developers that might use your code, use the `//` syntax (regular comment).
