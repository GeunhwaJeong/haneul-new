...
The Haneul CLI cannot find the environment alias you use (such as `devnet` or `testnet`) in the `[environments]` section of your `Move.toml` file.
...
Look up the chain ID for your network with `haneul client chain-identifier`. For local or ephemeral deployments, use `haneul client test-publish` instead.
