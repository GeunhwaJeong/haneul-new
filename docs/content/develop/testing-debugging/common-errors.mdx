...
<AgentPrompt
  prompt={"Debug this Haneul transaction failure. Inspect object refs, gas objects, ownership, versions, and suggest the minimal code or PTB change to make it execute."}
/>
...
Errors in this section relate to Haneul addresses.
...
System packages like `std` and `haneul` are included implicitly and do not need to be listed.
...
The address you provided does not conform to a valid Haneul address format. Haneul addresses must:
...
- [Create a Haneul Address](/getting-started/onboarding/get-address)
...
This error occurs when your code receives a relative URL (`/path/to/file`) but expects an absolute URL (`https://example.com/path/to/file`). This error originates from the `haneul::url` module that provides a `Url` struct and functions to interact with URLs. Move does not validate the format of a URL itself.
...
- [`haneul::url`](/references/framework/haneul_haneul/url.md)

- [`haneul::url` source code](https://github.com/GeunhwaJeong/haneul/blob/c5f8b33b8090d75b1922f1497916b4010d9140d7/crates/haneul-framework/sources/url.move).
...
- [Move Concepts](/develop/write-move/haneul-move-concepts)
...
A module in your package does not have the address `0x0`. In Move, you must set all modules to have their self-address as `0x0` in the source code. When you publish the package, Haneul assigns it the correct address automatically.
...
- [Error definition](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/haneul-adapter-transactional-tests/tests/haneul/publish_module_non_zero_address.snap)

---

#### `The signer only expects one signature, not two`

A function you are calling in your Move module expects 1 signer argument, yet you provide 2. On Haneul, the number of signer arguments in your function's signature must match the number of signers you provide. 
...
- [Error definition](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/external-crates/move/crates/bytecode-verifier-transactional-tests/tests/script_signature/signer_double_signer.snap)
...
- [Install Haneul](/getting-started/onboarding/haneul-install)
...
#### `haneul move build` command hangs 
...
The network does not receive a response within a certain time period. This error is not specific to Haneul.
...
This error occurs when the network is down, your internet connection is unstable, or your Haneul client configuration has an error.
...
devnet => https://fullnode.devnet.haneul.io:443 (active)
```
...
A value in your transaction does not have the `drop` ability and you did not explicitly consume or transfer it. The Haneul runtime enforces resource safety and returns this error when you leave values unused at the end of a transaction. 

##### Solution 

In Haneul Move packages, you must either transfer every value that does not have the `drop` ability to another address, explicitly destroy it, or use it as input for another function to consume. 
...
- `UNKNOWN_VERSION`: Your module or enum uses a version or feature the system does not recognize. Ensure you are using supported features and the correct Haneul toolchain version. 
...
- [Error: `ZERO_SIZED_STRUCT`](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/haneul-verifier-transactional-tests/tests/one_time_witness/no_field.snap)

- [Error: `FIELD_MISSING_TYPE_ABILITY`](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/haneul-verifier-transactional-tests/tests/struct_with_key/key_struct_with_drop.snap)

- [Error: `CONSTRAINT_NOT_SATISFIED`](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/haneul-verifier-transactional-tests/tests/private_generics/receive_without_key.snap)
...
Wait for the first transaction to finish, then rebuild or re-sign the transaction so it uses the latest object references. For related operations, combine them into one [programmable transaction block](/develop/transactions/ptbs/building-ptb). For parallel execution, use SDK/client helpers that manage gas coins and object dependencies.

**Resources**

- [Signing and Sending Transactions](/develop/transactions/transaction-auth/auth-overview)
- [Object Versioning](/develop/objects/versioning)
- [Building Transactions](/develop/transactions/ptbs/building-ptb)
...
The Haneul network responds to an RPC or API request with `HTTP 403: Forbidden` status. This might happen if an RPC provider:
...
The Haneul network rejects a transaction due to invalid transaction inputs. 
...
The sender address either has insufficient HANEUL balance or the gas coin already serves as a transaction input object.

##### Solution

- Verify the wallet has enough HANEUL to cover the gas budget.
...
The Haneul CLI cannot match your active environment to an environment in your package configuration. The `mainnet` and `testnet` environments are implicitly available, so this error most commonly occurs when you are connected to a local or ephemeral network (Localnet, Devnet).

##### Solution

If you are on **Localnet or Devnet**, use `haneul client test-publish` instead of `haneul client publish`. Ephemeral networks should not be added to the `[environments]` section because their addresses change frequently and would pollute your `Published.toml`.
...
Look up the chain ID for your network with `haneul client chain-identifier`.
...
The Haneul RPC layer returns standard error codes. Use the following table to diagnose failures:
...
Haneul errors fall into two categories:
...
// import { getPureSerializationType } from '@haneullabs/haneul/transactions';
