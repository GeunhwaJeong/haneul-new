...
Your transaction tries to use one or more Haneul objects that another transaction already uses. When you use an object in a transaction, the system locks that object to prevent multiple, possibly conflicting modifications to an object. For DeFi objects, this prevents double-spending.
...
- [Avoiding Equivocation](/develop/haneul-architecture/epochs)
