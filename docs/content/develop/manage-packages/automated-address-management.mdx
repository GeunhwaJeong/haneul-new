...
<AgentPrompt
  prompt={"Migrate this package to current Haneul package address management using Move.lock / Published.toml conventions, then verify build and publish/test-publish behavior."}
/>
...
When you publish or upgrade your package on multiple Haneul networks (Mainnet, Testnet, Devnet), automated address management tracks separate addresses for each chain. Tracking is based on your active environment. You can run `haneul client active-env` to see your current environment. For example, if your active environment is `testnet` and you publish a package, the `Move.lock` records the address for that package and associates it with `testnet`.

Automated address management works for one package published to one or more Haneul networks. If a package is republished to a network, tracked addresses for that network are overwritten.
...
```sh
haneul client --switch --env <YOUR-CHAIN-ENVIRONMENT>
```

2. Run the `manage-package` command with details of your published package:

```sh
haneul move manage-package --environment "$(haneul client active-env)" \
                        --network-id "$(haneul client chain-identifier)" \
...
- Key-value pairs correspond to the data described in [adopting automated address management](#adopting-automated-address-management). Use the `haneul move manage-package` command as the frontend to modify these values.
