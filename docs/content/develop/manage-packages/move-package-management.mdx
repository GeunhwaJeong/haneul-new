...
You should only include an environment in your manifest if you expect other packages to link against your package on that network. You usually do not want to include local networks in your manifest, because they are typically short-lived and private. Instead, you should consider using the [`test-publish`](#test-publish) command to publish your package and its dependencies on a local network. The `test-publish` command gives you much more flexibility to manage your deployment.
...
Because the system always fetches after pinning, you do not need to connect to a network unless a dependency needs to be repinned because the manifest changed or you ran `update-deps`.
...
Because packages can have different dependencies in different environments, it is important to have a real environment present when building a package for ephemeral publication; the real environment is referred to as the build environment, and is specified by the `--build-env <env>` flag.
