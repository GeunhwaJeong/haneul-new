...
<AgentPrompt
  prompt={"Inspect this Move package's Move.toml, Move.lock, and dependencies. Fix dependency declarations, prefer MVR where appropriate, and verify haneul move build works."}
/>
...
If the CLI displays `Your active environment is not present in Move.toml`, the most likely cause is that you are on a local or ephemeral network (Localnet, Devnet). These networks should not be added to the `[environments]` section. Instead, use `haneul client test-publish` to publish on ephemeral networks.
...
Use `haneul client chain-identifier` to find the chain ID for your network. Note that `mainnet` and `testnet` are implicitly available and do not need to be listed.
...
When you run `haneul move build` or `haneul move test`, the system ensures you have all dependencies cached in your `~/.move` directory. It first checks whether the lockfile is current, and if the lockfile is not current, it repins all of the dependencies for your build environment. It then checks whether the packages are cached, and if not it downloads them.
...
Run `haneul move update-deps` to repin all of your dependencies. This redownloads the latest compatible versions without changing your manifest file.
...
You can also publish or upgrade using the `haneul client ptb` command. See [Building Programmable Transaction Blocks](/develop/transactions/ptbs/building-ptb) for details.
...
1. Start the local network: `haneul start --with-faucet --force-regenesis`
1. Publish with test-publish (no permanent record): `haneul client test-publish`
1. After each code change, re-run `haneul client test-publish` for a fresh deployment.
...
const { modules, dependencies } = JSON.parse(
  execSync('haneul move build --dump-bytecode-as-base64 --path ./your_package', {
    encoding: 'utf-8',
  }),
);
...
After running `haneul client publish --json`, the response includes:
...
```sh
$ haneul client publish --json 2>/dev/null | jq '.objectChanges[] | select(.type == "published")'
```

:::info Automatic framework dependencies

The `Haneul` and `MoveStdlib` packages are automatically included as dependencies. You do not need to list them in `[dependencies]`.
