---
title: Deploy Haneul Packages with GitHub Actions
sidebar_label: Deploy with GitHub Actions
description: Build a production-safe CI/CD pipeline that tests every pull request, automates Testnet publishing, and gates Mainnet publishing and upgrades behind manual approval.
keywords: [ GitHub Actions, CI/CD, publish, upgrade, deployment, UpgradeCap, haneulup ]
---
...
- [x] A Move package that builds and tests locally with the Haneul CLI.
...
When a step needs a key, map it from secrets to an environment variable and reference the variable. The Haneul CLI reads the imported key from its keystore, so the key value appears only once, during import.
...
## Pin the Haneul CLI to a release

Haneul ships updates regularly, and the CLI version determines build output and framework compatibility. Pin the CLI to a specific release so your pipeline produces the same result every run. Use [haneulup](https://github.com/GeunhwaJeong/haneulup) to install and pin a version. The `haneul@NETWORK-VERSION` form pins both the network release line and the exact version.

Add these steps to any job that runs the Haneul CLI. They install `haneulup`, install a pinned Haneul CLI, and add the binary directory to the path:

```yaml
- name: Install haneulup
  run: cargo install --git https://github.com/GeunhwaJeong/haneulup.git --locked

- name: Install pinned Haneul CLI
  run: |
    haneulup install haneul@${{ env.HANEUL_VERSION }}
    echo "$HOME/.haneulup/bin" >> "$GITHUB_PATH"
```

Pin to the release line that matches the target network. Use a Testnet release for the Testnet job and a Mainnet release for the Mainnet job, because the CLI must agree with the onchain Haneul framework version. The workflows below set `HANEUL_VERSION` per file, such as `testnet-1.51.4` or `mainnet-1.51.4`. Replace the version numbers with the current release you intend to pin.
...
jobs:
  build-and-test:
    ...
    steps:
      ...
      - name: Cache Cargo and haneulup
        uses: actions/cache@v4
        with:
          path: |
            ~/.cargo
            ~/.haneulup
          key: haneulup-${{ runner.os }}-${{ env.HANEUL_VERSION }}
      ...
      - name: Build
        run: haneul move build --path ${{ env.PACKAGE_PATH }}

      - name: Test
        run: haneul move test --path ${{ env.PACKAGE_PATH }}
```
...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Install pinned Haneul CLI
        run: |
          cargo install --git https://github.com/GeunhwaJeong/haneulup.git --locked
          haneulup install haneul@${{ env.HANEUL_VERSION }}
          echo "$HOME/.haneulup/bin" >> "$GITHUB_PATH"

      - name: Configure network and key
        env:
          HANEUL_PRIVATE_KEY: ${{ secrets.TESTNET_PRIVATE_KEY }}
        run: |
          haneul client new-env --alias testnet --rpc https://fullnode.testnet.haneul.io:443
          haneul client switch --env testnet
          haneul keytool import "$HANEUL_PRIVATE_KEY" ed25519
          haneul client switch --address ${{ vars.TESTNET_ADDRESS }}

      - name: Publish and capture IDs
        run: |
          haneul client publish --json --gas-budget 200000000 ${{ env.PACKAGE_PATH }} > publish.json
  ...
  prepare-upgrade:
    ...
    steps:
      ...
      - name: Connect to a dedicated Mainnet RPC
        run: |
          haneul client new-env --alias mainnet --rpc ${{ secrets.MAINNET_RPC_URL }}
          haneul client switch --env mainnet
          haneul client switch --address ${{ vars.MAINNET_SENDER_ADDRESS }}

      - name: Build the unsigned upgrade transaction
        run: |
          haneul client upgrade \
...
The artifact holds the Base64 transaction bytes. Sign those bytes with your hardware wallet, multisig, or custody service, then submit the signed transaction with `haneul client execute-signed-tx`, as shown earlier. A first Mainnet publish follows the same pattern with `haneul client publish --serialize-unsigned-transaction`.
