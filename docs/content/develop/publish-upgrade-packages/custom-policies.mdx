...
To address these risks while still providing the opportunity to [upgrade live packages](/develop/publish-upgrade-packages/upgrade), Haneul offers custom upgrade policies. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.
...
:::tip Custody and runbook

Treat the `UpgradeCap` owner as a privileged admin. Hold it under a multisig or timelock rather than a single key, emit an event on every upgrade, monitor onchain for upgrades to packages you depend on, and document an emergency process (including when to make the package immutable). See [Security Best Practices](/develop/security/best-practices) for more.
