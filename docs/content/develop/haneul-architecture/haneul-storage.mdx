---
title: Storage
description: Historical data and storage pricing provide insights into the cost of operations on the Haneul network.
...
There are 2 types of [snapshots](/operators/snapshots):
...
Compute is relatively fixed on Haneul, with hundreds of validators running 24-core, 128GB RAM machines. However, because Haneul achieves high throughput compared to other blockchains, storage costs can vary significantly. Periodically, a governance proposal on the network sets the storage fee. However, there are some underlying factors that contribute to the data storage measurement.

Currently, the storage fee is 76 GEUNHWA or 0.000000076 HANEUL per storage unit.
...
You can run this query in your browser at https://graphql.mainnet.haneul.io/graphql
...
The total cost of storage for a transaction is based on each storage unit used in the transaction. Every byte of data is equal to 100 storage units. For reference, a kilobyte is equal to 100,000 storage units. If a transaction included a 1 kilobyte object, the storage fee would equal 7.6 million GEUNHWA or 0.0076 HANEUL.
...
The design of the storage fund also incentivizes users to delete data if the cost of storing it exceeds the value of keeping it onchain. By offering a rebate on storage fees, the system encourages users to free up space.

If data is deleted from Haneul, it would stop using storage resources and a portion of the original storage fee, currently set at 99%, would be returned to the gas fee payer as a rebate. The non-refundable portion of the storage fee (1%) is never returned. Immutable objects lock both their storage fee forever and cannot receive a rebate, as they cannot be deleted or modified.
