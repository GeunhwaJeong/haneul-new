...
The borrow / hot-potato pattern is a safer way to hand out an asset by value: the type system forces the borrowed object to be returned to its `Referent` within the same transaction, so it cannot be retained or swapped. When you take an object back (or finalize any hot-potato flow), validate the returned object's identity and invariants before completing the operation rather than trusting that the correct object was returned. See [Security best practices](/develop/security/best-practices).
