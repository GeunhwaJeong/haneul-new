...
All functionality around [address-owned](/develop/objects/object-ownership/address-owned) objects works the same for objects owned by other objects.
...
These restrictions for accessing sent child objects are enforced dynamically by providing mutable access to the parent object's `UID` during the execution of the transaction. Because of this, you can transfer objects to and receive them from [address-owned](/develop/objects/object-ownership/address-owned) objects, [dynamic field](/develop/objects/dynamic-fields) objects, [wrapped](/develop/objects/object-ownership/wrapped) objects, and [shared](/develop/objects/object-ownership/shared) objects.

:::info

The transfer to object mechanism is not supported for [party objects](/develop/objects/object-ownership/party) whose owning address corresponds to an object ID.
...
Just like with [custom transfer policies](/develop/objects/transfers/custom-rules), Haneul allows for the definition of custom receivership rules for `key`-only objects. In particular, you can use the `transfer::receive` function only on objects defined in the same module as the call to [`transfer::receive`](/references/framework/haneul_haneul/transfer#haneul_transfer_receive), just like you can use the [`transfer::transfer`](/references/framework/haneul_haneul/transfer#haneul_transfer_transfer) function only on objects defined in the module where it's being used.
