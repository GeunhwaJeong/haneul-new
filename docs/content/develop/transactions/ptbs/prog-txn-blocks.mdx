...
- `GasCoin`: References the HANEUL coin used to pay for gas. When using address balance gas payment (empty gas payment list), `GasCoin` references a synthetic coin created from the sender or sponsor address balance. **This is separate from other inputs because it has special restrictions**:
    ...
    - Only by value through `TransferObjects` or new `haneul::coin::send_funds`
...
The most important consideration at this stage is the effects on the gas coin. At the beginning of execution, the maximum gas budget (in terms of HANEUL) is withdrawn from the gas coin. Any unused gas is returned to the gas coin at the end of execution, even if the coin has changed owners.
...
Object inputs have the type `T` of the underlying object. The exception is `ObjectArg::Receiving` inputs, which have type `haneul::transfer::Receiving<T>`. This wrapper indicates the object is owned by another object, not the sender. Call `haneul::transfer::receive` with the parent object to unwrap it and prove ownership.
...
PTBs can call any `public` function and any `entry` function, whether private (`entry fun f()`), or `public(package)` (`public(package) entry fun f()`). Non-entry private and `public(package)` functions cannot be called from PTBs. As a result, there is no reason to add `entry` to a `public` function.
...
**Private generics:**
Some framework functions have type parameters that can only be instantiated with types defined in the same module. Because PTBs are not modules, they cannot supply these types and therefore cannot call these functions directly. For example, certain `haneul::transfer` functions like `transfer` and `share_object` require a type defined in the calling module, and as such cannot be called from a PTB. Instead, use the `public_transfer` and `public_share_object` variants.
