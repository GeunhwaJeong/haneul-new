---
title: What is a PTB?
description: Programmable transaction blocks are a group of commands that complete a transaction on Haneul.
...
:::tip Local network prerequisite

If you connect to Localnet, verify your local node is running first. See [Connecting to a local network](/getting-started/onboarding/local-network).

:::

Transactions on Haneul are composed of groups of commands that execute on inputs to define the result of the transaction. Referred to as **programmable transaction blocks (PTBs)**, these groups of commands define all user transactions on Haneul. PTBs allow a user to call multiple Move functions, manage their objects, and manage their coins in a single transaction without publishing a new Move package. Designed with automation and transaction builders in mind, PTBs are a lightweight and flexible way of generating transactions. 
...
- `tx.moveCall({ target, arguments, typeArguments })`: Executes a Move call. Returns whatever the Haneul Move call returns.
...
Learn more about each [PTB command](/references/ptb-commands).

## Inputs and results

Inputs are the values that are provided to the PTB, and results are the values that are produced by the PTB commands. The inputs are either objects or basic Move values, and the results are arbitrary Move values, including objects. Inputs and results form the data flow through a PTB's execution.
...
Learn more about [inputs and results](/develop/transactions/ptbs/inputs-and-results).
...
- `GasCoin`: References the HANEUL coin used to pay for gas. When using address balance gas payment (empty gas payment list), `GasCoin` references a synthetic coin created from the sender or sponsor address balance. **This is separate from other inputs because it has special restrictions**:

    - `&mut` or `&` usage anywhere.
    - Only by value through `TransferObjects` or `haneul::coin::send_funds`
        - If you need an owned `Coin<HANEUL>` from the `GasCoin`, first use `SplitCoins`.
...
At the beginning of execution, the PTB runtime loads the input objects into the input array. The network already verifies the objects, checking rules like existence and valid ownership. The pure value bytes are also loaded into the array but not validated until usage.

The most important consideration at this stage is the effects on the gas coin. At the beginning of execution, the system withdraws the maximum gas budget (in terms of HANEUL) from the gas coin. The system returns any unused gas to the gas coin at the end of execution, even if the coin has changed owners.
...
In a PTB, if you create an object through a Move command and do not destroy, transfer, or use it in a subsequent command, the transaction fails with an error. 

### Pre-execution validation

When a transaction is signed, the network performs the following validations on specific commands:

- [`SplitCoins`](/references/ptb-commands) and [`MergeCoins`](/references/ptb-commands): Verifies that the argument arrays (`AmountArgs` and `ToMergeArgs`) are non-empty.

- [`MakeMoveVec`](/references/ptb-commands): Verifies that the type must be specified for an empty vector of `Args`.

- [`Publish`](/references/ptb-commands): Verifies that the `ModuleBytes` are not empty.
...
- If the signature expects a `T`, the runtime verifies the argument has type `T`, then copies the value if `T: copy` or moves it otherwise. Objects are always moved because `haneul::object::UID` does not have the `copy` ability.
...
Object inputs have the type `T` of the underlying object. `ObjectArg::Receiving` inputs are the exception and have type `haneul::transfer::Receiving<T>`. This wrapper indicates the object is owned by another object, not the sender. Call `haneul::transfer::receive` with the parent object to unwrap it and prove ownership.
...
**Return types:**
Move calls cannot return references (`&T` or `&mut T`).

**Private generics:**
Some framework functions have type parameters that can only be instantiated with types defined in the same module. Since PTBs are not modules, they cannot supply these types and therefore cannot call these functions directly. For example, certain `haneul::transfer` functions like `transfer` and `share_object` require a type defined in the calling module, and as such cannot be called from a PTB. Instead, use the `public_transfer` and `public_share_object` variants.
...
public fun issue(bank: &mut Bank, amount: u64): (Balance<HANEUL>, Loan) { ERROR }

public fun repay(bank: &mut Bank, loan: Loan, repayment: Balance<HANEUL>) { ERROR }
```
...
- Returns a single `haneul::package::UpgradeCap`.
...
- Takes exactly one PTB argument: a `haneul::package::UpgradeTicket` (by value).
- Returns a single `haneul::package::UpgradeReceipt`.
- Does not call `init` functions. Newly added modules in an upgrade cannot have `init` functions.
...
Any remaining HANEUL deducted from the gas coin at the beginning of execution is returned to the coin, even if the owner has changed. The maximum possible gas is deducted at the beginning of execution, and unused gas is returned at the end (all in HANEUL). Because the gas coin can only be taken by value with `TransferObjects`, it has not been wrapped or deleted.

The total effects (created, mutated, and deleted objects) are then passed out of the execution layer and applied by the Haneul network.
...
Suppose you want to buy 2 items from a marketplace costing `100 GEUNHWA`. You keep one item for yourself and send the other item plus the remaining coin to a friend at address `0x808`.
...
```rust
{
  ...
  commands: [
    ...
    MoveCall("haneul", "tx_context", "sender", [], []),
    TransferObjects([NestedResult(1, 1)], NestedResult(3, 0)),
  ]
}
```

The inputs include the friend's address, the marketplace object, and the coin split value. The commands split off the coin, call the marketplace function, send the gas coin and one object, grab your address (through `haneul::tx_context::sender`), and send the remaining object to yourself.
...
| State | Value |
| :---- | :---- |
| Gas coin balance | `499,900 GEUNHWA` |
| Result | `[Coin<HANEUL> { id: new_coin, value: 100 }]` |

**Command 1: `MoveCall("some_package", "some_marketplace", "buy_two", ...)`**

Calls `buy_two(marketplace: &mut Marketplace, coin: Coin<HANEUL>, ctx: &mut TxContext): (Item, Item)`. Uses `Input(1)` by mutable reference (marketplace) and `NestedResult(0, 0)` by value (coin is moved and consumed).
...
**Command 3: `MoveCall("haneul", "tx_context", "sender", [], [])`**
...
```rust
Gas Coin: Coin<HANEUL> { id: gas_coin, balance: 1_000_000u64 }
...
```rust
Gas Coin: Coin<HANEUL> { id: gas_coin, balance: 500_000u64 }
```
