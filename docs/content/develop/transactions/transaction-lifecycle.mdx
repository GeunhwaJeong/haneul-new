...
1. A user with a private key creates and signs a user transaction to either mutate objects they own, [shared objects](/develop/objects/object-ownership/shared), or create new objects.

1. Haneul sends the transaction to each [validator](/operators/validator), often through a [full node](/operators/full-node/haneul-full-node). Validators perform a series of validity and safety checks, sign it, and return the signed transaction to the client.

1. The client collects the responses from a set of validators that account for at least 2/3 of the [validator committee](/develop/haneul-architecture/consensus) to form a transaction certificate. As a result, Haneul validators do not need to propagate signatures on a best-effort basis (gossip signatures) or aggregate certificates. This effort is now the responsibility of the client or gateway.
...
1. After assembling the certificate, the client sends it back to all validators, who check its validity and acknowledge its receipt to the client.
    ...
    1. If the transaction involves [owned objects](/develop/objects/object-ownership/address-owned) or immutable objects exclusively, Haneul can process the transaction certificate immediately and execute it without waiting for the [consensus engine](/develop/haneul-architecture/consensus).
...
1. Subsequently, Haneul forms [checkpoints](/develop/haneul-architecture/checkpoint-verification) for every consensus commit, which it uses to drive the [reconfiguration protocol](/develop/haneul-architecture/epochs).
...
For transactions that involve owned objects only, this happens before consensus in under 1/2 a second. If a transaction includes shared objects, it happens shortly after consensus, which can take a few seconds. At this point, the transaction reached settlement finality because now you can process more transactions on the same input objects. See [Object Ownership](/develop/objects/object-ownership) for more information.

## Epoch change

Periodically (~24 hours), the Haneul [Testnet](/develop/haneul-architecture/networks) and [Mainnet](/develop/haneul-architecture/networks) networks enter the process of [epoch](/develop/haneul-architecture/epochs) change. During epoch change, the network calculates and distributes [staking rewards](/develop/haneul-architecture/tokenomics-overview), validator metadata takes effect, and other network processes are performed. All user transactions are delayed until the new epoch begins.
