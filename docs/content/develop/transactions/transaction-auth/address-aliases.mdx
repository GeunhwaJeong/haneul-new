...
description: Address aliases can be used to configure the set of keys that are allowed to sign transactions for a particular address on the Haneul network.
keywords: [ address aliases, aliases, key rotation, changing keys ]
---

Address aliases can be used to configure the set of keys that are allowed to sign transactions for a particular address on the Haneul network.

You can use address aliases to:
- Perform basic key rotation for an address.
...
Each address on Haneul has a set of aliases. New addresses begin as their own alias. That is, a new address `A` has the alias set `{A}`.

You can modify aliases for an address using the Move API in the <UnsafeLink href="/references/framework/haneul_haneul/address_alias">`haneul::address_alias` module</UnsafeLink>. These modification functions must be invoked as top-level PTB commands and cannot be called from other Move functions.
...
Transactions on Haneul are configured with two addresses: a `sender` and a `gas_owner` (which might or might not be the same). For each configured address, a signature is required from one of the signers in its alias set.
...
## Managing aliases with the Haneul CLI

You can manage address aliases using `haneul client ptb` to call entry functions in the `haneul::address_alias` module. The `AddressAliasState` singleton object lives at the address `0xa`.
...
```sh
$ haneul client ptb \
    --move-call haneul::address_alias::enable @0xa \
    --gas-budget 50000000
```
...
```sh
$ haneul client ptb \
    --move-call haneul::address_alias::add \
...
```sh
$ haneul client ptb \
    --move-call haneul::address_alias::replace_all \
        @<ADDRESS_ALIASES_OBJECT_ID> \
...
```sh
$ haneul client ptb \
    --move-call haneul::address_alias::remove \
        @<ADDRESS_ALIASES_OBJECT_ID> \
        @<ALIAS_TO_REMOVE> \
    --gas-budget 50000000
```
