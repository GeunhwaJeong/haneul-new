---
title: Querying Data with GraphQL RPC
...
<AgentPrompt
  prompt={"Convert this Haneul JSON-RPC query to GraphQL. Include the query, variables, pagination handling, and TypeScript caller code."}
/>

This guide provides practical examples for querying the Haneul network using GraphQL, including reads, transaction submission, and transaction simulation. For core concepts like headers, variables, fragments, pagination, scope, and limits, see the [corresponding concepts page](/develop/accessing-data/graphql/graphql-rpc).
...
Use `simulateTransaction` to preview transaction effects, estimate gas, or test transaction logic without committing the transaction onchain. Simulation accepts a JSON transaction matching the Haneul gRPC transaction schema, or a BCS-encoded transaction with `{"bcs": {"value": "<base64>"}}`.
