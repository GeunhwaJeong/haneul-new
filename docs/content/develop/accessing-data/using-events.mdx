...
Move code can interact with objects stored on Haneul. Some applications track an object's activity and trigger certain workflows within the application based on that activity, such as application updates based on how many times an NFT is minted or the amount of HANEUL a smart contract generates. 
...
<ImportContent source="crates/haneul-indexer-alt-e2e-tests/packages/emit_event/sources/emit_event.move" mode="code" fun="init" noComments />
...
:::info
The `haneul_sdk::HaneulClientBuilder` JSON-RPC client is deprecated. Use the `haneul-rust-sdk` crate with `HaneulGrpcClient` for new integrations.
:::

```rust
use haneul_rust_sdk::HaneulGrpcClient;

const PACKAGE_ID_CONST: &str = "0x279525274aa623ef31a25ad90e3b99f27c8dbbad636a6454918855c81d625abc";

#[tokio::main]
async fn main() -> Result<(), anyhow::Error> {
    let client = HaneulGrpcClient::new("https://fullnode.mainnet.haneul.io:443")?;

    let events = client
        .query_events(
            haneul_rust_sdk::types::EventFilter::MoveModule {
...
The [TypeScript SDK](https://sdk.haneul-labs.com/typedoc/modules/_haneullabs_haneul.graphql) also can be used to interact with the Haneul GraphQL service and filter events.
