...
You can use the following SDKs to interact with data on Haneul.

- [TypeScript SDK](https://sdk.haneul-labs.com/haneul/migrations/haneul-2.0/json-rpc-migration)

- [Rust SDK - gRPC](https://github.com/GeunhwaJeong/haneul-rust-sdk)

- [Rust SDK - GraphQL](https://docs.rs/haneul-graphql/latest/haneul_graphql/)

- Community-maintained [Python SDK](https://github.com/FrankC01/pyhaneul)
...
Directly connect to [JSON-RPC](/references/haneul-api) hosted on Haneul [full nodes](/operators/full-node/haneul-full-node) that are operated by [RPC providers](https://haneul.io/developers#dev-tools) (filter by `RPC`) or [data indexer operators](https://github.com/haneul-foundation/awesome-haneul?tab=readme-ov-file#indexers--data-services). The Mainnet, Testnet, or Devnet load balancer URLs abstract the Haneul Foundation-managed full nodes. Do not use those for production.

You can get real-time or historical data using JSON-RPC. The retention period for historical data depends on the [pruning strategy](/operators/data-management/managing-data#haneul-full-node-pruning-policies) that node operators implement, though the default configuration for all full nodes is to implicitly fall back on the [Archival Store](#archival-store-and-service) managed by the Haneul Foundation.
