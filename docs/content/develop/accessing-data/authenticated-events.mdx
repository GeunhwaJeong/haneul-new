...
description: Cryptographically verifiable Move events that can be verified by a light client without trusting any intermediary, suitable for trustless event consumption.
...
Authenticated Events provide a cryptographically verifiable stream of Move events from Haneul smart contracts. Unlike regular events, authenticated events can be verified by a light client without trusting any intermediary, making them suitable for applications requiring trustless event consumption.

## Why Authenticated Events?

Regular Haneul events are indexed and queryable, but verifying them requires trusting the RPC provider. Authenticated events solve this by:

1. **Cryptographic Commitments**: Events are committed into a Merkle Mountain Range (MMR) structure stored onchain (see [MMR section](#merkle-mountain-range-mmr) below)
2. **Checkpoint Verification**: The commitment is tied to Haneul checkpoints, which are signed by the validator committee
...
Authenticated events are fully backward-compatible with regular Haneul events. An authenticated event is a regular event with additional metadata for verification. Existing event consumers continue to work unchanged.
...
- Inclusion proofs are `O(log n)` in size. Event-level MMR inclusion proofs are not yet exposed by current RPC services (this is separate from OCS inclusion proofs exposed by the ProofService). These inclusion proofs can lead to many interesting applications, such as moving data storage offchain, efficient cross-chain reads, and proving inclusion in ZK.
...
If any event is missing, modified, or out of order, the computed MMR will not match the onchain state.
