...
Authenticated events provide a cryptographically verifiable stream of Move events from Haneul smart contracts. Unlike regular events, authenticated events can be verified by a light client without trusting any intermediary, making them suitable for applications that require trustless event consumption.
