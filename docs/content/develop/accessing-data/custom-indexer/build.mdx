...
description: Build a custom indexer using the `haneul-indexer-alt-framework` module. The example indexer demonstrates a sequential pipeline that extracts transaction digests from Haneul checkpoints and stores them in a local PostgreSQL database, the default example storage backend.
...
- Connects to a Haneul checkpoint source: Uses gRPC streaming from a full node as the primary source, with the durable GCS checkpoint bucket as the recommended production fallback. The public HTTPS endpoint at https://checkpoints.testnet.haneul.io is available as a convenience for recent data only (most recent 30 days).
...
Haneul provides checkpoint data through multiple sources:

- **GCS buckets (recommended for production and backfill):** `gs://haneullabs-testnet-checkpoints-use4` for Testnet or `gs://haneullabs-mainnet-checkpoints-use4` for Mainnet. These buckets have full checkpoint retention and are Requester Pays enabled, so configure GCS credentials and a billing project before using them.
- **HTTPS endpoints (recent data only, most recent 30 days):** `https://checkpoints.testnet.haneul.io` for Testnet or `https://checkpoints.mainnet.haneul.io` for Mainnet.
...
```sh
$ cargo new simple-haneul-indexer
$ cd simple-haneul-indexer
```
...
<ImportContent source="examples/rust/basic-haneul-indexer/Cargo.toml" mode="code" />

The manifest now includes the following dependencies:

- `haneul-indexer-alt-framework`: Core framework providing pipeline infrastructure.
...
```sh
$ createdb haneul_indexer
```

Get your connection details:

```sh
$ psql haneul_indexer -c "\conninfo"
```

If successful, your console should display a message similar to the following:

```sh
You are connected to database "haneul_indexer" as user "username" via socket in "/tmp" at port "5432".
```
...
```sh
$ PSQL_URL=postgres://username@localhost:5432/haneul_indexer
```
...
<ImportContent source="examples/rust/basic-haneul-indexer/diesel.toml" mode="code" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/schema.rs" mode="code" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/models.rs" mode="code" />

Key annotations:

- `FieldCount`: Required by `haneul-indexer-alt-framework` for memory optimization and batch processing efficiency. It is used to limit the max size of a batch so that we don't exceed the postgres limit on the number of bind parameters a single SQL statement can have.
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" struct="TransactionDigestHandler" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" tag="processordeps" />

After the `TransactionDigestHandler` struct, add the `Processor` code:

<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" tag="processor" />
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/processor.rs" mode="code" trait="Processor" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" tag="handlerdeps" />

Add the logic for `Handler` after the `Processor` code. The complete code is available at the end of this step.

<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" tag="handler" />
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/sequential/collector.rs" mode="code" tag="batch" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/handlers.rs" mode="code" />
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" trait="Handler" />
...
<ImportContent source="examples/rust/basic-haneul-indexer/src/main.rs" mode="code" />
...
```sh
$ cargo run -- \
    --remote-store-gcs haneullabs-testnet-checkpoints-use4 \
    --remote-store-header x-goog-user-project:YOUR_PROJECT_ID \
    --streaming-url https://fullnode.testnet.haneul.io:443
```
...
```sh
$ cargo run -- \
    --remote-store-url https://checkpoints.testnet.haneul.io \
    --first-checkpoint RECENT_CHECKPOINT
```

The public HTTPS checkpoint endpoints (`https://checkpoints.testnet.haneul.io` and `https://checkpoints.mainnet.haneul.io`) retain only the most recent 30 days of checkpoints and are suitable as a convenience for quick testing or recent-data workloads. When using one with a fresh database, set `--first-checkpoint` to a checkpoint that is still within the retained range; otherwise, the indexer starts at checkpoint 0 and retries missing checkpoints indefinitely. For production deployments and full-retention backfills, use the GCS buckets through `--remote-store-gcs`: `haneullabs-testnet-checkpoints-use4` for Testnet or `haneullabs-mainnet-checkpoints-use4` for Mainnet. These buckets are Requester Pays enabled. See [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays) and [Running a Remote Store](/operators/data-management/remote-store-setup) for more information.

:::info

Allow incoming network requests if your operating system requests it for the `basic-haneul-indexer` application.
...
To confirm your data is accurate, copy any transaction digest from your database and verify it on HaneulScan: [https://haneulscan.xyz/testnet/home](https://haneulscan.xyz/testnet/home)
