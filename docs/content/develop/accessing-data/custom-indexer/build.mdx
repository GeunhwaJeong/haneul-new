...
- Connects to Haneul Testnet: Uses the remote checkpoint store at https://checkpoints.testnet.haneul.io as a fallback source for recent checkpoints. The public HTTPS endpoint retains only the most recent 30 days.
...
Haneul provides checkpoint stores for both Mainnet and Testnet.

- Testnet: `https://checkpoints.testnet.haneul.io` for the most recent 30 days only
- Mainnet: `https://checkpoints.mainnet.haneul.io` for the most recent 30 days only

For full-retention backfill, use the network-specific GCS bucket with Requester Pays enabled: `gs://haneullabs-testnet-checkpoints-use4` for Testnet or `gs://haneullabs-mainnet-checkpoints-use4` for Mainnet.
...
```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.haneul.io --streaming-url https://fullnode.testnet.haneul.io:443
```

**Using remote checkpoint storage:**

```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.haneul.io
```

**Using a GCS checkpoint bucket directly:**

```sh
$ cargo run -- --remote-store-gcs haneullabs-testnet-checkpoints-use4
```

The public HTTPS checkpoint endpoints retain only the most recent 30 days of checkpoints. For full-retention backfills, use `gs://haneullabs-testnet-checkpoints-use4` through `--remote-store-gcs haneullabs-testnet-checkpoints-use4` for Testnet or `gs://haneullabs-mainnet-checkpoints-use4` through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` for Mainnet. These buckets are Requester Pays enabled and should be accessed through the indexing framework's `--remote-store-gcs` option rather than `--remote-store-url`. See [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays) and [Running a Remote Store](/operators/data-management/remote-store-setup).
