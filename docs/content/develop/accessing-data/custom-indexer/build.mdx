...
- Connects to Haneul Testnet: Uses the remote checkpoint store at https://checkpoints.testnet.haneul.io as a fallback source. Testnet still provides full checkpoint history.
...
- Testnet: `https://checkpoints.testnet.haneul.io` with full history
...
```sh
$ cargo run -- --remote-store-gcs haneullabs-mainnet-checkpoints-use4
```

The public HTTPS checkpoint endpoints retain only the most recent 30 days of checkpoints. For Mainnet backfills older than 30 days, use `gs://haneullabs-mainnet-checkpoints-use4` through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4`. This bucket is Requester Pays enabled and should be accessed through the indexing framework's `--remote-store-gcs` option rather than `--remote-store-url`. See [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays) and [Running a Remote Store](/operators/data-management/remote-store-setup).
