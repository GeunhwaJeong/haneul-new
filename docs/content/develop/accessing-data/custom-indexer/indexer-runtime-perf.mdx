---
title: Optimize Indexer Runtime and Performance
sidebar_label: Optimize Runtime and Performance
...
Configuration and resource monitoring deliver performant custom indexers. For example:
...
- Efficient data pruning keeps your tables performant over time. 

- Expose and extend Prometheus metrics to track indexer performance. 
...
- `ingest_concurrency`: How many checkpoint fetches run concurrently. Adaptive by default (scales 1 → 500 based on the slowest subscriber's fill fraction, 60%–85% dead band). Use `ConcurrencyConfig::Fixed` to pin it, or tune the adaptive bounds / `dead_band` if the defaults don't fit. Per-subscriber channel capacity is set per-pipeline on `{Sequential,Concurrent}Config::ingestion`.
- `retry_interval_ms`: Lower values reduce latency for live data, higher values reduce unnecessary retries.
- `streaming_backoff_initial_batch_size`: Number of checkpoints to process through polling after the initial streaming failure. Lower values restore streaming faster, higher values reduce connection attempts.
- `streaming_backoff_max_batch_size`: Maximum checkpoints to process through polling after repeated failures. The batch size increases exponentially from the initial size up to this maximum. Higher values reduce connection attempts during prolonged outages.
...
- `db_connection_pool_size`: Set size based on `write_concurrency` across all pipelines.
...
Use the following command-line arguments to focus processing. These values are for demonstration. Use values that match your environment and goals. 
...
Read the [README](https://github.com/tokio-rs/console/blob/main/README.md) in the Tokio GitHub repo for additional information.
...
`tokio-console` introduces runtime overhead. Use it carefully in production. Use it regularly during development and staging. Production usage requires careful evaluation. Before enabling in production, run performance benchmarks with and without Tokio console enabled to measure the impact on your specific workload. Consider enabling only during maintenance windows or targeted troubleshooting sessions while monitoring system resources.

## Metrics

The `haneul-indexer-alt-framework` provides built-in Prometheus metrics for monitoring indexer performance and health. All metrics are automatically exposed through HTTP and can be extended with custom metrics.
...
By default, metrics are served on `0.0.0.0:9184/metrics` in [Prometheus format](https://prometheus.io/docs/develop/haneul-architecture/object-model). You can override this address:
...
- Or, through the builder configuration:
...
Add a metric label prefix to differentiate between different indexer instances:
...
The framework only exposes metrics through HTTP, and you're responsible for collection and storage. For example, some indexers use the Alloy agent to scrape the `/metrics` endpoint and write to a metrics database for visualization and alerting.
