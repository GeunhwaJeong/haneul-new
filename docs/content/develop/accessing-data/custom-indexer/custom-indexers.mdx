---
title: What Are Custom Indexers?
description: The `haneul-indexer-alt-framework` is a powerful Rust framework for building high-performance, custom blockchain indexers on Haneul. It provides customizable, production-ready components for data ingestion, processing, and storage.
keywords: [ indexer, custom indexer, data processing, blockchain indexer, haneul-indexer-alt-framework ]
---

Haneul generates rich, complex data, including transactions, events, object changes, and more. While Haneul provides standard APIs to access this data, many applications need customized data processing for workflows like tracking specific events, aggregating analytics, building dashboards, or creating specialized databases. Custom indexers let you extract, transform, and store the specific blockchain data you need. Instead of querying Haneul APIs repeatedly or building complex filtering logic, you process the raw blockchain data once and store it in your preferred format and storage layer.

Custom indexers enable use cases such as tracking DEX trading volumes, monitoring NFT collection activity, building analytics dashboards, or aggregating cross-chain data. You build custom indexers using the `haneul-indexer-alt-framework`, a Rust framework that provides production-ready components for data ingestion, processing, and storage while giving you full control over your indexing logic.
...
Checkpoints on Haneul are batches of transactions that represent consistent blockchain state snapshots. Each checkpoint contains complete transaction details, events, object changes, and execution results with guaranteed ordering. See [Life of a Transaction](/develop/transactions/transaction-lifecycle) for more information on checkpoints.

![Overview](../images/context_custom-indexer-overview_v1.png)

### Architecture overview

The following diagram details the custom indexer architecture.

![Custom indexer architecture](../images/architecture_custom-indexer_v1.png)
...
- **Remote stores** can connect to public checkpoint repositories such as [`https://checkpoints.mainnet.haneul.io`](https://checkpoints.mainnet.haneul.io/) and [`https://checkpoints.testnet.haneul.io`](https://checkpoints.testnet.haneul.io/), but these HTTPS endpoints retain only the most recent 30 days of checkpoints and should be used only as fallback sources. For full-retention backfills, use `gs://haneullabs-mainnet-checkpoints-use4` or `gs://haneullabs-testnet-checkpoints-use4` with `--remote-store-gcs` and Requester Pays enabled.
...
For configuration examples and guidance on when to use each source, see [Checkpoint Data Sources](/develop/accessing-data/custom-indexer/indexer-data-integration#checkpoint-data-sources).
