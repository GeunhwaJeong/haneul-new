...
The `haneul-indexer-alt-framework` provides 2 distinct pipeline architectures. Understanding their differences is crucial for choosing the right approach.
...
<li><span className="text-haneul-success-dark">✓</span> You want straightforward implementation with direct commits and basic queries.</li>
...
<li><span className="text-haneul-success-dark">✓</span> Sequential processing cannot keep up with your data volume.</li>
...
With checkpoint 1004 now committed, `checkpoint_hi_inclusive` can advance from 1003 to 1004 because there are no gaps up to 1004. The `reader_lo` and `pruner_hi` values have not changed yet.
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="batch" noComments />

`commit()`: Database write logic.

<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="commit" noComments />
...
Sequential pipelines use channel-based flow control with a more streamlined topology due to fewer components:

- **Broadcaster → Processor:** `checkpoint_buffer_size` slots.
- **Processor → Committer:** `processor_channel_size` slots (defaults to `num_cpus / 2`).
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/pipeline/sequential/committer.rs" tag="send" />
...
<ImportContent source="crates/haneul-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="regulator" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MIN_EAGER_ROWS" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MAX_PENDING_ROWS" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/mod.rs" struct="WatermarkPart"/>
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" highlight="watermark_interval_ms" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="delay_ms" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="prune_concurrency" />
...
<ImportContent mode="code" source="crates/haneul-pg-db/src/schema.rs" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/ingestion/mod.rs" struct="IngestionConfig" highlight="checkpoint_buffer_size" />

**`Processor` to `Collector`**: `processor_channel_size` slots (defaults to `num_cpus / 2`) → All workers block on `send()`.

<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="processor_tx" />
    
**`Collector` to `Committer`**: `collector_channel_size` slots (defaults to `num_cpus / 2`) → `Collector` stops accepting.

<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs" tag="buff" />
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/pipeline/concurrent/collector.rs" tag="collector" /> 
...
<ImportContent mode="code" source="crates/haneul-indexer-alt-framework/src/ingestion/broadcaster.rs" fun="spawn_aborting" /> 
