...
keywords: [ json-rpc, migration, grpc, graphql, deprecation, haneul rpc, json-rpc migration, haneul rpc migration, replace json-rpc ]
---
...
The following table maps common JSON-RPC method families to their gRPC service and GraphQL equivalents. Method names on the gRPC side use the protobuf service in the [Haneul Full Node gRPC reference](/references/fullnode-protocol). GraphQL equivalents reference the GraphQL field or field path on the schema. Use this table to plan the migration, then consult the linked references for the exact request and response shapes.

| JSON-RPC method | gRPC service and operation | GraphQL field |
| --- | --- | --- |
| `haneul_getCheckpoint`, `haneul_getLatestCheckpointSequenceNumber` | `LedgerService.GetCheckpoint` | `Query.checkpoint` |
| `haneul_getCheckpoints` | No paginated gRPC list. Use `LedgerService.GetCheckpoint` for point lookups, or `SubscriptionService.SubscribeCheckpoints` for an ordered stream, depending on your use case. | `Query.checkpoints` |
| `haneul_getTransactionBlock` | `LedgerService.GetTransaction` | `Query.transaction` |
| `haneul_multiGetTransactionBlocks` | `LedgerService.BatchGetTransactions` | `Query.multiGetTransactions` |
| `haneul_getTotalTransactionBlocks` | `LedgerService.GetCheckpoint` on the latest or target checkpoint, then read `summary.total_network_transactions` | `Checkpoint.networkTotalTransactions` |
| `haneul_getObject`, `haneul_tryGetPastObject` | `LedgerService.GetObject` (current state and recent history) | `Query.object` |
| `haneul_multiGetObjects` | `LedgerService.BatchGetObjects` | `Query.multiGetObjects` |
| `haneulx_getBalance`, `haneulx_getAllBalances` | `StateService.GetBalance`, `StateService.ListBalances` | `Address.balance`, `Address.balances` |
| `haneulx_getCoins`, `haneulx_getAllCoins` | `StateService.ListOwnedObjects` with a type filter: `0x2::coin::Coin<T>` for one coin type, or the bare `0x2::coin::Coin` for all coin types | `Address.objects` with `filter: { type: "0x2::coin::Coin<T>" }` for one coin type, or `filter: { type: "0x2::coin::Coin" }` for all coin types |
| `haneulx_getCoinMetadata` | `StateService.GetCoinInfo` | `Query.coinMetadata` |
| `haneulx_getOwnedObjects` | `StateService.ListOwnedObjects` | `Address.objects` |
| `haneulx_getDynamicFields` | `StateService.ListDynamicFields` | `Object.dynamicFields` |
| `haneulx_getDynamicFieldObject` | Derive the dynamic field's object ID locally from the parent object ID and the field name, then call `LedgerService.GetObject`. `ListDynamicFields` is not needed when you already know the field name. | `Object.dynamicObjectField` |
| `haneulx_queryTransactionBlocks` | No direct filtered query. Enumerate finalized transactions through `SubscriptionService.SubscribeCheckpoints`, or fetch known transactions through `LedgerService.GetTransaction`. | `Query.transactions` with a `TransactionFilter` |
| `haneulx_queryEvents` | Read events from transaction data returned by `LedgerService.GetTransaction` or streamed by `SubscriptionService.SubscribeCheckpoints` | `Query.events` with an `EventFilter` |
| `haneul_subscribeEvent`, `haneul_subscribeTransaction` (WebSocket) | `SubscriptionService.SubscribeCheckpoints`, then read events and transactions from each checkpoint | No streaming subscription. Poll `Query.events` with an `EventFilter`, or `Query.transactions` with a `TransactionFilter`. |
| `haneulx_getReferenceGasPrice` | `LedgerService.GetEpoch` (read `reference_gas_price` from the response) | `Epoch.referenceGasPrice` |
| `haneulx_getStakes`, `haneulx_getStakesByIds` | No direct gRPC method. Read owned `StakedHaneul` objects through `StateService.ListOwnedObjects` filtered by type, then fetch full contents through `LedgerService.GetObject`. | `Address.objects` with `filter: { type: "0x3::staking_pool::StakedHaneul" }` |
| `haneulx_getValidatorsApy` | No direct gRPC method and no APY field. Read validator-set data and compute APY client-side. See [issue #23832](https://github.com/GeunhwaJeong/haneul/issues/23832) for the recommended computation. | No direct APY field. Read validator-set data through `Epoch.validatorSet` and compute APY client-side. See [issue #23832](https://github.com/GeunhwaJeong/haneul/issues/23832) for the recommended computation. |
| `haneul_executeTransactionBlock` | `TransactionExecutionService.ExecuteTransaction` | `Mutation.executeTransaction` |
| `haneul_dryRunTransactionBlock`, `haneul_devInspectTransactionBlock` | `TransactionExecutionService.SimulateTransaction` | `Query.simulateTransaction` |
| `unsafe_payHaneul`, `unsafe_pay`, `unsafe_payAllHaneul`, `unsafe_transferHaneul`, `unsafe_transferObject`, `unsafe_moveCall`, `unsafe_splitCoin`, `unsafe_mergeCoins`, `unsafe_batchTransaction` | Build a [programmable transaction block](/develop/transactions/ptbs/prog-txn-blocks) with the SDK, then execute it through `TransactionExecutionService.ExecuteTransaction` | Build a PTB with the SDK, then submit through `Mutation.executeTransaction` |
| `haneul_getNormalizedMoveModule`, `haneul_getNormalizedMoveFunction`, `haneul_getNormalizedMoveStruct` | `MovePackageService` | `Query.package`, then `MovePackage.module`, `MoveModule.function`, and `MoveModule.struct` |

The JSON-RPC `unsafe_*` builder methods do not have a one-to-one replacement on either side. Build transactions with [PTBs](/develop/transactions/ptbs/prog-txn-blocks) using the [TypeScript SDK](https://sdk.haneul-labs.com/typescript) or the [Rust SDK](https://github.com/GeunhwaJeong/haneul-rust-sdk), then submit the resulting bytes through `TransactionExecutionService.ExecuteTransaction`.
...
- **The Archival Service** is a separate gRPC service that serves unpruned point lookups for checkpoints, transactions, and objects. It is scoped to those point lookups, so it does not serve the rest of the full node RPC surface, such as transaction execution or live state queries. Reuse the same `LedgerService` methods, but point the client at an Archival Service endpoint such as `archive.mainnet.haneul.io:443` or your provider's archival URL. See [Archival Store](/develop/accessing-data/archival-store).
...
JSON-RPC offered WebSocket subscriptions for events and transactions. Haneul does not carry those forward. Use `SubscriptionService.SubscribeCheckpoints` over gRPC server streaming to receive finalized checkpoints in order, and read each checkpoint's transactions, effects, and events from the streamed payload. If your stream disconnects, resume from the last processed checkpoint sequence number. Backfill any missed range using `LedgerService` before you resubscribe. See [Subscriptions for streaming data](/develop/accessing-data/grpc/what-is-grpc#subscriptions-for-streaming-data).

### Public load balancer URLs are not production endpoints

The public URLs at `https://fullnode.<network>.haneul.io` and `https://graphql.<network>.haneul.io/graphql` are rate-limited and intended for development and public-good access. Do not point a production exchange, indexer, or wallet backend at them. Run your own [Haneul full node](/operators/full-node/haneul-full-node) or contract with a provider for a dedicated gRPC or GraphQL endpoint.
...
- **Coin selection.** Replace `haneulx_getCoins` with an owned-objects query filtered by coin type: `Address.objects` with `filter: { type: "0x2::coin::Coin<...>" }` in GraphQL, or `StateService.ListOwnedObjects` with the same type filter on gRPC. To list coins of every type (the `haneulx_getAllCoins` case), filter by the bare `0x2::coin::Coin` type without a type parameter. For transaction building, prefer [gas smashing](/develop/transaction-payment/gas-smashing) or [address-balance gas payments](/onchain-finance/asset-custody/address-balances/using-address-balances) rather than constructing coin lists by hand.
...
- **GraphQL:** [Example 2 in Migrating to GraphQL from JSON-RPC](/develop/accessing-data/graphql/query-with-graphql#migrating-to-graphql-from-json-rpc) shows the JSON-RPC `haneul_getTransactionBlock` call and the GraphQL `Query.transaction` equivalent side by side.
...
```sh
$ buf curl --protocol grpc https://<full node URL>/haneul.rpc.v2.StateService/GetBalance -d '{ "owner": "<ADDRESS>", "coin_type": "0x2::haneul::HANEUL" }'
```
...
- **GraphQL:** [Example 3 in Migrating to GraphQL from JSON-RPC](/develop/accessing-data/graphql/query-with-graphql#migrating-to-graphql-from-json-rpc) maps `haneulx_getCoins` to `Address.objects` with `filter: { type: "0x2::coin::Coin<0x2::haneul::HANEUL>" }`, including how cursor pagination changes.
...
query {
  events(
    filter: { type: "0x3164fcf73eb6b41ff3d2129346141bd68469964c2d95a5b1533e8d16e6ea6e13::Market::ChangePriceEvent<0x2::haneul::HANEUL>" }
  ) {
    nodes {
      sender {
        address
