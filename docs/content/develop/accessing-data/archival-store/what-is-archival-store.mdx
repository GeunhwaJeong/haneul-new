...
Archival endpoints are retention and availability tools, not inherently trustless proof: data they return is only as trustworthy as the operator serving it. For high-trust workflows, verify the data cryptographically (for example, through checkpoint verification or [authenticated events](/develop/accessing-data/authenticated-events)) rather than relying on the archival response alone. See [Security Best Practices](/develop/security/best-practices) for more.
