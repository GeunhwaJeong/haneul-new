...
You can reuse the same gRPC `LedgerService` methods for this lookup, but you must point the client at an Archival Service endpoint such as `archive.mainnet.haneul.io:443` or your provider's archival URL instead of the full node's URL.
