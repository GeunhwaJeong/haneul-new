---
title: What is gRPC?
description: Learn about gRPC and how it provides fast, type-safe access to Haneul network data.
keywords: [ grpc, g rpc, RPC, haneul full node grpc, full node rpc, what is grpc, grpc on haneul ]
---

The [Haneul Full Node gRPC API](/references/fullnode-protocol) provides a fast, type-safe, and efficient interface for interacting with the Haneul blockchain. Designed for power users, indexers, explorers, and decentralized apps, this API enables access to Haneul data with high performance and low latency.
...
The following gRPC services are available on Haneul. Protocol buffers define the gRPC interface. You can find the relevant `.proto` files at [`haneul-apis` on GitHub](https://github.com/GeunhwaJeong/haneul-apis/tree/main/proto), which apart from the gRPC messages (request and response payloads), include the following services:
...
| `LedgerService` | `haneul/rpc/v2/ledger_service.proto` | Look up specific checkpoints, transactions, objects, and more from the current state and recent history of the Haneul network. History refers to the recent past, limited to what a full node retains. |
| `StateService` | `haneul/rpc/v2/state_service.proto` | Query up-to-date onchain data like balances, coin metadata, dynamic fields, or owned objects. It also supports dry-run simulations for transactions. |
| `SubscriptionService` | `haneul/rpc/v2/subscription_service.proto` | Stream live updates for checkpoints. Ideal for building reactive systems such as indexers, bots, and dashboards. See Subscriptions for streaming data. |
...
| `SignatureVerificationService` | `haneul/rpc/v2/signature_verification_service.proto` | Validate signatures outside transaction execution. Helps pre-verify payloads that might include [zkLogin](/haneul-stack/zklogin-integration) or other signatures, simulate authentication, or build custom signing workflows. |
...
Streaming APIs are useful for building indexers, dashboards, or bots that need to react to real-time Haneul activity with minimal latency.

## Handling pruned data

If a full node does not return a specific object, transaction, or event, the data has likely been pruned based on the node's retention configuration. In that case, use a fallback mechanism in your application to query the same data from the [Archival Store and Service](/develop/accessing-data/archival-store). The Archival Store retains historical data for the network.

You can reuse the same gRPC `LedgerService` methods for this lookup. Change the endpoint in your client from the full node's address to the Archival Service endpoint. This ensures that your application can access both recent and historical data reliably, regardless of a full node's pruning settings.
