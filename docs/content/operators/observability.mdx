...
<AgentPrompt
  prompt={"Add structured logging for transaction digest, sender, package/function, status, gas, and user-visible errors so support can debug failed Haneul transactions."}
/>

Good observability capabilities are key to the development and growth of Haneul. The distributed and asynchronous nature of Haneul, with multiple client and validator processes distributed over a potentially global network, makes this more challenging.

The observability stack in Haneul builds on the [Tokio tracing](https://tokio.rs/blog/2019-08-tracing) library. The rest of this topic highlights specific aspects of achieving good observability through structured logging and metrics in Haneul.

:::info

Haneul operators, administrators, and developers primarily consume this output. The content of logs and traces does not represent the authoritative, certified output of validators and is subject to potentially byzantine behavior.
...
In the [Tokio tracing](https://tokio.rs/blog/2019-08-tracing) library, structured logging is implemented using [spans and events](https://docs.rs/tracing/0.1.31/tracing/index#core-concepts).
Spans cover a whole block of functionality, like one function call, a future or asynchronous task, and so on. They can be nested, and key-value pairs in spans give context to events or logs inside the function.
...
For more details, see the [EnvFilter](https://docs.rs/tracing-subscriber/latest/tracing_subscriber/filter/struct.EnvFilter) topic.

## Metrics {#metrics}

Haneul includes Prometheus-based metrics:

- `rpc_requests_by_route` and related metrics for RPC server API metrics and latencies
- Validator transaction metrics (see `AuthorityMetrics` in the Haneul source code)

## Viewing logs, traces, metrics {#viewing-logs-traces-metrics}

The tracing architecture uses [subscribers](https://github.com/tokio-rs/tracing#project-layout) that you can plug into the tracing library to process and forward output to different sinks for viewing. Multiple subscribers can be active at the same time.

You can feed JSON logs through a local sidecar log forwarder such as [Vector](https://vector.dev), and then onwards to destinations such as ElasticSearch.
...
Haneul serves metrics through a Prometheus scrape endpoint, by default at `<host>:9184/metrics`.
...
To see nested spans visualized with [Jaeger](https://www.jaegertracing.io), complete the following steps:
...
Adding Tokio-console support might significantly slow down Haneul validators and gateways.
