...
description: Database snapshots of the Haneul network enable full node operators a way to bootstrap a full node without having to execute all the transactions that occurred after genesis.
...
Database snapshots provide a point-in-time view of a database's store. In Haneul, the database snapshot captures a running database's view of the Haneul network from a particular node at the end of an epoch. While validators can enable snapshots, they are typically most valuable for full node operators.

Snapshots of the Haneul network enable full node operators to bootstrap a full node without having to execute all the transactions that occurred after genesis. You can upload snapshots to remote object stores like S3, Google Cloud Storage, Azure Blob Storage, and similar services. These services typically run the export process in the background so there is no degradation in performance for your full node. With snapshots stored in the cloud, you can recover quickly from catastrophic failures in your system or hardware.

To maintain a healthy Haneul network, Haneul encourages the Haneul community to bring up additional snapshots to ensure stronger data availability across the network.

## Supported snapshot types

Haneul supports two types of snapshots:

- **RocksDB snapshots** are a point-in-time view of a database store. The snapshot keeps the state of the system at the moment it generates the snapshot, including non-pruned data, additional indices, and other data.
- **Formal snapshots** are database agnostic with a minimalistic format. They contain only the data necessary to restore a node to a valid state at the specified epoch. Consequently, they have a much smaller storage footprint and are generally faster to restore from when compared to database snapshots. Formal snapshots are also supported natively by the Haneul protocol. You can cryptographically verify the contents of the formal snapshot against a commitment from the committee at the epoch you are restoring to. This verification happens automatically and by default during a formal snapshot restore (unless you explicitly bypass it).

:::info

Formal snapshots are not suitable for use if you are running an RPC node that does any historical data lookups. For more information on node data management, see [Data Management](/operators/data-management).
...
1. **Minimalism:** Formal snapshots currently contain only the end of epoch live object set (the set of all object versions eligible for use as input objects for future transactions). Haneul syncs all other critical chain information from the chain or derives it. Thus, formal snapshots contain only the necessary data required for a node to startup at epoch boundary and participate in the network.
...
You can restore a full node using either RocksDB snapshots or formal snapshots.

### Restoring using RocksDB snapshots

To restore from a RocksDB snapshot, follow these steps:

1. Choose the snapshot source for the epoch you want to restore. Haneul Labs managed DB snapshots are available in GCS for Testnet and in GCS or S3 for Mainnet, with one snapshot per epoch.
1. Download the snapshot to a temporary directory using cloud object store tooling. For example, if the `db-path` value points to `/opt/haneul/db/authorities_db/full_node_db` and you want to restore from epoch 10, use one of these commands:
...
1. Place the downloaded `epoch_[NUM]` directory at `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/haneul/db/authorities_db/full_node_db/live`.
...
When you restore a full node from a snapshot, write it to the path `/opt/haneul/db/authorities_db/full_node_db/live`. When restoring a validator node, you can shorten the database destination to `/opt/haneul/db/authorities_db/live`. Check the `db_path` field of your full node or Validator configs to confirm the path location.

:::

### Restoring using formal snapshots

To restore using a formal snapshot, use the `haneul-tool` binary. You can download `haneul-tool` along with other `haneul` binaries.
See [Install Haneul](/getting-started/onboarding/haneul-install) for more details.

To restore a node from a formal snapshot, complete the following steps:

1. If it's running, stop the node.
2. Run the command:
   ```
   haneul-tool download-formal-snapshot --latest --genesis "<PATH-TO-GENESIS-BLOB>" \
   ...
   ```
   - `--epoch`: The epoch that you want to download. Haneul Labs hosted buckets keep only the last 90 epochs. You can check the most recent epoch on Haneul explorers like [haneulvision](https://haneulvision.xyz/) or [haneulscan](https://haneulscan.xyz/).
   - `--latest`: Rather than explicitly passing an epoch through `--epoch`, you can pass the `--latest` flag, which automatically selects the latest snapshot.
   ...
   - `--snapshot-bucket`: Source snapshot bucket name, for example `haneullabs-mainnet-formal`. This cannot be used with `--no-sign-request`.
...
## Haneul Labs managed snapshots

Haneul Labs hosts two tiers of snapshot storage access: high throughput, Requester Pays enabled buckets, and free, permissionless buckets.
...
* Because the bucket is open to the internet, you do not need to provide any cloud credentials.
* The free buckets are only available for formal snapshots. For example, `haneul-tool download-formal-snapshot --no-sign-request` uses the free formal snapshot bucket.
* For DB snapshots, use cloud object store tooling with one of the Requester Pays buckets.

### Bucket names

**S3**

Testnet: DB snapshots are not hosted in S3. Formal snapshots: `s3://haneullabs-testnet-formal/`

Mainnet: DB snapshots: `s3://haneullabs-mainnet-snapshots/`; formal snapshots: `s3://haneullabs-mainnet-formal/`

**GCS**

Testnet: DB snapshots: `gs://haneullabs-testnet-snapshots/`; formal snapshots: `gs://haneullabs-testnet-formal/`

Mainnet: DB snapshots: `gs://haneullabs-mainnet-snapshots/`; formal snapshots: `gs://haneullabs-mainnet-formal/`

![Haneullabs Managed Snapshots](./images/managed-snapshots_opreator-guides_v1.png "A diagram that shows the current architecture of Haneullabs snapshot availability")
