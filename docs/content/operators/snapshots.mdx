...
1. Place the snapshot into the directory that the `db-path` value points to in your fullnode.yaml file. For example, if the `db-path` value points to `/opt/haneul/db/authorities_db/full_node_db` and you want to restore from epoch 10, then copy the snapshot to the directory with this command:

   You can use the AWS CLI (provided you have credentials to associate with the download):
   `aws s3 cp s3://<BUCKET_NAME>/epoch_10 /opt/haneul/db/authorities_db/full_node_db/live --recursive --request-payer requester`.

1. When using `aws s3 cp` (or `gsutil cp`) the database is copied to the location you pass to `--path`, in a directory named `epoch_[NUM]`. Rename the `epoch_[NUM]` directory to `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/haneul/db/authorities_db/full_node_db/live`.
...
2. Run the command:
   ...
   - `--snapshot-bucket`: Source snapshot bucket name, for example `haneullabs-mainnet-snapshots`. This cannot be used with `--no-sign-request`.
...
* The free buckets (for example, `haneul-tool download-formal-snapshot --no-sign-request`) are only available for formal snapshots. You must use one of the requester pays (S3 or GCS) buckets if downloading a db snapshot.

### Bucket names

**S3**

Testnet: `s3://haneullabs-testnet-snapshots/`, `s3://haneullabs-testnet-formal/`

Mainnet: `s3://haneullabs-mainnet-snapshots/`,  `s3://haneullabs-mainnet-formal/`

**GCS**

Testnet: `gs://haneullabs-testnet-snapshots/`, `gs://haneullabs-testnet-formal/`

Mainnet: `gs://haneullabs-mainnet-snapshots/`,  `gs://haneullabs-mainnet-formal/`
