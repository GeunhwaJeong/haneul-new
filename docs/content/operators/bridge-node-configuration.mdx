---
title: Haneul Bridge Validator Node Configuration
sidebar_label: Bridge Node Configuration
...
- [x] Install `haneul` and `haneul-bridge-cli`.
...
- A REST API URL where the bridge node listens to and serves requests, for example `https://bridge.example-haneul-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters.

To create a `BridgeAuthorityKey`, run
```sh
$ haneul-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
Treat bridge node key custody as a production requirement. Before you go live, confirm the following:

- **Generate keys securely.** Create the `BridgeAuthorityKey`, network key, and any admin keys in a secure environment, offline where possible, and never reuse a key across roles. See [user and operator key management](/develop/security/best-practices#user-and-operator-key-management).
- **Separate keys by role.** Keep the bridge authority key, network key, and administrative/account keys distinct. Provide a dedicated `bridge-client-key-path` rather than defaulting to the validator key, so a compromise of one key does not expose the others.
- **Document rotation and recovery.** Maintain a written procedure for rotating and recovering each key, including who holds custody and how compromised keys are replaced.
- **Treat WAF and rate limiting as required.** The [WAF protection](#web-application-firewall-waf-protection-for-bridge-node) and rate limiting described later on this page are production requirements, not optional hardening.

:::

### Registration

After you have both authority key file and REST API URL ready, you can register them by using Haneul CLI:

```sh
$ haneul validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
Both key and URL are changeable **before the committee is finalized**. If you want to update metadata, rerun `haneul validator register-bridge-committee`.
...
```sh
$ haneul-bridge-cli view-bridge-registration --haneul-rpc-url {HANEUL-FULLNODE-URL}
```
...
```sh
$ haneul validator update-bridge-committee-node-url
```
See the [offline signing section](#offline-signing) on this page for how to sign the transaction offline.
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `haneul-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `metrics:push-url` | The URL of the remote Haneul metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].haneul.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) for more details. |
...
| `haneul:bridge-client-key-path` | The file path of Bridge Client key. You can generate this key with `haneul-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `haneul:bridge-client-key-path`, the node defaults to use the Bridge Validator key to submit transactions on Haneul. This is not recommended for the sake of key separation. |

### BridgeClient

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the Haneul network. Thus when it's enabled, you need a Haneul account key with enough HANEUL balance.
...
```
This prints the newly created Haneul Address. Then fund this address with some HANEUL for operations.
...
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ...
    ```
- Use pre-built Docker image. Pull from Docker Hub: `haneullabs/haneul-tools:{SHA}`


### Run bridge node

Running bridge node is similar to running a Haneul node using systemd or Ansible. The command to start the bridge node is:
...
You can find a full list of bridge node metrics and their descriptions in the [`haneul-bridge` crate](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-bridge/src/metrics.rs).
