...
description: Learn how to set up, configure, and manage a Haneul validator node.
keywords: [ haneul validator node, validator nodes, validator node configuration, configure validator node, set up validator node, manage validator ]
sidebar_label: Deployment and Configuration
---

Validators on Haneul run specialized validator nodes that can execute more tasks than full nodes. Validators handle staking, gas price reference, and tallying rules.

## Validator requirements

To run a Haneul validator, you must set up and configure a Haneul validator node. Specific steps you must take include:

1. Install and configure Haneul.
...
2. Acquire 30M HANEUL by staking to the validator staking pool you created in the previous step. Call `request_add_stake` with the address of the validator (this is not the same as the staking pool ID).
...
You can deploy a Haneul node in a number of ways.

1. Use pre-built container images available in [Docker Hub](https://hub.docker.com/r/haneullabs/haneul-node/tags).
...
For more information on deploying a validator, refer to the [Haneul for Node Operators](https://github.com/GeunhwaJeong/haneul/blob/main/nre/haneul_for_node_operators.md) guide.
...
`haneul-node` runs with a single configuration file provided as an argument, for example:

```sh 
$ ./haneul-node --config-path /opt/haneul/config/validator.yaml
```

For more information on configuring a validator, refer to the [Haneul for Node Operators](https://github.com/GeunhwaJeong/haneul/blob/main/nre/haneul_for_node_operators.md) guide.
...
Load testing on Haneul validator networks suggests that the default Linux network buffer sizes are too small.
Increase them using one of the following two methods:

### Option 1: With `/etc/sysctl.d/`

Add these settings to a new `sysctl` file specifically for `haneul-node` or append them to an existing file. Modifications made in this way persist across system restarts.
