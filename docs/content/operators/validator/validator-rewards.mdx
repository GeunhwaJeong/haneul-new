...
description: Learn how Haneul calculates and distributes validator and staker rewards.
...
The total voting power on Haneul is always 10,000, regardless of the amount staked. Therefore, the quorum threshold is 6,667. There is no limit to the amount of HANEUL users can stake with a validator. Each validator has consensus voting power proportional to HANEUL in its staking pool, with one exception: the network caps the voting power of an individual validator at 1,000 (10% of the total). If a validator accumulates more than 10% of total stake, the validator's voting power remains fixed at 10%, and the network spreads the remaining voting power across the rest of the validator set.

## Staking on Haneul compared to other blockchains

The Haneul staking mechanism includes features that differ from other blockchains:

- Staking is self-custodial. You keep your staked HANEUL tokens in an owned object.
...
When you stake HANEUL tokens, the tokens are wrapped into `StakedHANEUL` objects. Your share of the staking pool is calculated using the timestamp of your `StakedHANEUL` object, which records when the deposit occurred, along with the change in exchange rates between the deposit epoch and the withdrawal epoch.
...
The network distributes validator rewards as regular stake objects, so withdrawal works the same for validators and stakers. A validator can call the [`request_withdraw_stake` function](https://github.com/GeunhwaJeong/haneul/blob/6234ae2cc8137f3a2a34cd0aa1ac4fd5f31260e2/crates/haneul-framework/packages/haneul-system/sources/haneul_system.move#L272) in the `haneul_system` module to withdraw stake and receive rewards.
...
You cannot withdraw a portion of your active stake. You must withdraw all staked HANEUL at the same time. You can, however, stake using multiple `StakedHaneul` objects by splitting your HANEUL into multiple coins. You can then perform a partial withdrawal from a validator by un-staking only some of the `StakedHANEUL` objects.

## Staker rewards 

Within a given validator staking pool, all stakers receive the same proportion of rewards through the pool's exchange rate appreciation. Because validators earn commissions on the stake they manage, they receive additional `StakedHANEUL` objects at the end of each epoch in proportion to the amount of commissions their staking pool earns.
...
Stake subsidies subsidize the network during its early phases and a 10% allocation of HANEUL tokens funds them. After this allocation depletes, gas fees collected through regular network operations make up the entirety of stake rewards.
...
Haneul provides stable and predictable gas prices during regular network operations. Validators set the network's reference gas price at the beginning of each epoch.
