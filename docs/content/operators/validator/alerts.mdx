...
When running a Haneul Validator node or Full node, you might want to configure alerting based on some or all of the following metrics.
...
| Key          | Value                                     |
| ------------ | ----------------------------------------- |
| **Name**     | `Haneullabs validators are not upgraded`      |
| **Summary**  | Validators are behind on protocol version |
| **Duration** | `1h`                                      |

```sh
min(haneul_configured_max_protocol_version{network="$network", host=~"Haneullabs-.*"})
  < quantile(0.34, haneul_configured_max_protocol_version{network="$network"})
```
