...
description: Use the Haneul CLI to interact with a full node's validator commands. 
keywords: [ haneul cli, full node, delegator, validator, operation cap, report validators, join a committee, leave a committee, report a validator, proof of possession, generate proof of possession ]
sidebar_label: Tools
---

This guide focuses on using the Haneul CLI `validator` commands. 
...
- [x] Complete all [Haneul installation prerequisites](/getting-started/onboarding/haneul-install).

- [x] Build the `haneul` binary, which you need for the genesis ceremony. You can do this on any computer.

<details className="nudge-details">

<summary>

View `haneul` binary installation details.
...
View Haneul account and CLI environment instructions.

</summary>

    - If this is the first time running this program, it asks you to provide a Haneul full node server URL and a meaningful environment alias. It also generates a random key pair in `haneul.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.

       By default, the `client.yaml` and `haneul.keystore` files are located in `~/.haneul/haneul_config`. For more information, refer to the [Haneul client CLI tutorial](/references/cli/client).
    ...
    - If you already set it up, just make sure:
      ...
      - `haneul.keystore` contains your account key pair.
...
```sh
$ haneul validator display-metadata
```
...
Use the Haneul CLI to perform validator tasks.

### Print help information

```sh
$ haneul validator --help
```
...
```sh
$ haneul validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
```sh
$ haneul validator become-candidate path/to/validator.info
```
...
Then, you must acquire 30M HANEUL and stake it to the validator staking pool. You can add Haneul to the validator staking pool with the command:

```sh
$ haneul client call --package 0x3 --module haneul_system --function request_add_stake --args 0x5 {haneul_object_id} {validator_address}
```

This creates a stake object (of type `0x3::staking_pool::StakedHaneul`). You can withdraw this object with the following command:

```sh
$ haneul client call --package 0x3 --module haneul_system --function request_withdraw_stake --args 0x5 {stake_object_id}
```
...
```sh
$ haneul validator join-committee
```
...
```sh
$ haneul validator leave-committee
```
...
```sh
$ haneul validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
...
```sh
$ haneul validator display-metadata <validator-address>
```
...
```sh
$ haneul validator update-metadata --help
```
...
```sh
$ haneul validator update-gas-price <gas-price>
```

If your account is a delegatee:

```sh
$ haneul validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ haneul validator report-validator <reportee-address>
```
