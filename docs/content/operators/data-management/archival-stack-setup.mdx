...
2. **`haneul-kvstore-alt`** (Indexer): Reads checkpoints from the remote checkpoint store and writes processed data to Bigtable.
3. **`haneul-kv-rpc`** (Archival Service): A gRPC server that reads from Bigtable and exposes the [`LedgerService`](/references/fullnode-protocol) API to clients.

```mermaid
graph LR
    A[Haneul Fullnode] --> B[haneul-kvstore-alt]
    B --> C[Bigtable]
    C --> D[haneul-kv-rpc]
    D --> E[gRPC Clients]
```
...
- Access to the Haneul checkpoint buckets for your target network:
   - **Mainnet fallback endpoint:** `https://checkpoints.mainnet.haneul.io` for the latest 30 days only.
   - **Testnet fallback endpoint:** `https://checkpoints.testnet.haneul.io` for the latest 30 days only.
   - **Mainnet full-retention backfill bucket:** `gs://haneullabs-mainnet-checkpoints-use4` through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4`. This bucket is Requester Pays enabled.
   - **Testnet full-retention backfill bucket:** `gs://haneullabs-testnet-checkpoints-use4` through `--remote-store-gcs haneullabs-testnet-checkpoints-use4`. This bucket is Requester Pays enabled.
- A [Haneul full node](/operators/full-node/haneul-full-node) with gRPC enabled. After the initial backfill, you can stream checkpoints from a full node instead of polling the bucket for lower latency. See [Steady-state operation](#steady-state-operation).
...
The public HTTPS checkpoint endpoints retain only the latest 30 days of checkpoints. For full-retention backfill beyond 30 days, use `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` on Mainnet or `--remote-store-gcs haneullabs-testnet-checkpoints-use4` on Testnet. These buckets are Requester Pays enabled. Using `--remote-store-gcs` with valid credentials and a billing project avoids throttling and is the supported path for full-retention backfill.
...
| Node count | Haneul Labs currently runs 5 nodes for Mainnet and 2 for Testnet as a starting point. Scale up to 15 nodes for backfill. Your steady-state node count depends on your read traffic. Enable autoscaling or monitor CPU utilization and scale manually. |
...
Create the following 11 tables, each with a single column family named `haneul` and a GC policy that keeps only the latest cell version:
...
for table in checkpoints checkpoints_by_digest transactions objects epochs \
  ...
  cbt -project <GCP_PROJECT> -instance <INSTANCE_ID> createfamily "$table" haneul
  cbt -project <GCP_PROJECT> -instance <INSTANCE_ID> setgcpolicy "$table" haneul maxversions=1
done
```
...
Bigtable supports both manual and automatic node scaling. Haneul Labs uses manual scaling with CPU monitoring for better operational control, but autoscaling is a reasonable option if you prefer hands-off management.
...
The indexer (`haneul-kvstore-alt`) reads checkpoints from the remote checkpoint store and writes data to Bigtable through 12 parallel pipelines.
...
haneul-kvstore-alt \
    ...
    --remote-store-gcs haneullabs-mainnet-checkpoints-use4
```

For Testnet, use `--chain testnet` with `--remote-store-gcs haneullabs-testnet-checkpoints-use4`.
...
For backfill, use a single indexer instance. One instance can generate enough load to fully saturate a 15-node cluster. The 16 CPU / 15-node configuration is the largest scale Haneul Labs has tested. Larger instance types and cluster sizes might work but could hit software bottlenecks that have not been identified yet.
...
No `--config` is needed. The framework defaults are sufficient for steady-state indexing. The full node only needs gRPC enabled. JSON-RPC is not required. Haneul Labs runs full nodes with a 2-week retention period and only uses the checkpoint bucket for backfills.
...
Haneul Labs plans to enable requester-pays on the public checkpoint buckets in the future. Streaming from your own full node avoids these costs.

:::

#### Running multiple instances

Bigtable writes are idempotent, so multiple indexer instances can safely process the same data. Haneul Labs runs **3 indexer instances** at steady state to enable rolling deployments without ever delaying updates from the chain.

During backfill, a single instance is sufficient. One instance can saturate a 15-node cluster.

## Archival service setup

The archival service (`haneul-kv-rpc`) is a gRPC server that reads from Bigtable and exposes the [`LedgerService`](/references/fullnode-protocol) API.
...
haneul-kv-rpc \
    my-bigtable-instance \
    "[::]:8000" \
    --credentials /etc/haneul-kv-rpc/bigtable-ro-sa.json \
...
The archival service implements the [`LedgerService`](/references/fullnode-protocol) gRPC API, which is the same API exposed by Haneul full nodes. Existing gRPC clients can query the archival service by changing the endpoint URL.
...
```yaml
scrape_configs:
  - job_name: haneul-kvstore
    static_configs:
      - targets: ['<INDEXER_HOST>:9184']
  - job_name: haneul-kv-rpc
    static_configs:
...
|-------------|-----------|
| Production | `info` |
| Debugging | `info,haneul_kvstore=debug,haneul_indexer_alt_framework=debug` |
