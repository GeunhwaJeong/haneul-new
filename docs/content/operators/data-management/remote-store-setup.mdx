...
- Access to a checkpoint source:
   - **Haneullabs's public buckets**: GCS bucket `haneullabs-mainnet-checkpoints` (Mainnet) or `haneullabs-testnet-checkpoints` (Testnet). Also available over HTTPS at `https://checkpoints.mainnet.haneul.io` and `https://checkpoints.testnet.haneul.io`.
...
The most straightforward way to populate your own remote store is to backfill from Haneullabs's public GCS buckets. Use `--remote-store-gcs` as the source and a destination flag (`--s3` or `--gcs`) to write to your store.
...
For Testnet, use `--remote-store-gcs haneullabs-testnet-checkpoints`.

You can also use `--remote-store-url https://checkpoints.mainnet.haneul.io` instead of `--remote-store-gcs`, but the GCS option is faster for backfill because it uses the GCS API directly rather than HTTPS.
