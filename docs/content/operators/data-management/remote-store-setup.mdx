...
- Access to a checkpoint source:
   - **Haneullabs's public checkpoint endpoints**: `https://checkpoints.mainnet.haneul.io` and `https://checkpoints.testnet.haneul.io`. The Mainnet HTTPS endpoint retains only the most recent 30 days of checkpoints and should be used only as a fallback source. Testnet still provides full history.
   - **Mainnet full-history backfill bucket**: `gs://haneullabs-mainnet-checkpoints-use4`. This bucket is Requester Pays enabled and must be accessed through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4`.
...
The easiest way to populate your own remote store on Mainnet is to backfill from Haneullabs's requester-pays GCS bucket. Use `--remote-store-gcs` as the source and a destination flag (`--s3` or `--gcs`) to write to your store.
...
The Mainnet bucket is `gs://haneullabs-mainnet-checkpoints-use4` and is Requester Pays enabled.

Do not use `--remote-store-url https://checkpoints.mainnet.haneul.io` for full-history backfill. The public HTTPS endpoint retains only the latest 30 days of checkpoints. Use `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` for Mainnet backfill instead.

:::info

`gs://haneullabs-mainnet-checkpoints-use4` is Requester Pays enabled. Configure credentials and a billing project when reading from this bucket. For the lowest steady-state cost and latency, stream from your own full node after backfill completes.
...
| Flag | Description |
|------|-------------|
| `--remote-store-gcs <BUCKET>` | Fetch checkpoints from a GCS bucket. Required for Mainnet full-history backfill from `gs://haneullabs-mainnet-checkpoints-use4`. |
