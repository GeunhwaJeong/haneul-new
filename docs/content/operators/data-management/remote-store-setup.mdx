...
The remote store is an object store (S3 or GCS) containing protobuf-encoded checkpoint blobs. Full nodes read from it for state sync and indexers read from it for backfills. The `haneul-checkpoint-blob-indexer` populates this store by reading checkpoints from a source and writing compressed protobuf blobs to a destination.

## Architecture overview

```mermaid
graph LR
    A[Source: Existing Remote Store / Full Node gRPC] --> B[haneul-checkpoint-blob-indexer]
...
- Access to a checkpoint source:
   - **Haneullabs's public checkpoint endpoints**: `https://checkpoints.mainnet.haneul.io` and `https://checkpoints.testnet.haneul.io`. Both HTTPS endpoints retain only the most recent 30 days of checkpoints and should be used only as fallback sources.
   - **Full-retention backfill buckets**: `gs://haneullabs-mainnet-checkpoints-use4` for Mainnet and `gs://haneullabs-testnet-checkpoints-use4` for Testnet. These buckets are Requester Pays enabled and must be accessed through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` or `--remote-store-gcs haneullabs-testnet-checkpoints-use4`.
...
- The `haneul-checkpoint-blob-indexer` binary.
...
The easiest way to populate your own remote store on Mainnet or Testnet is to backfill from Haneullabs's requester-pays GCS buckets. Use `--remote-store-gcs` as the source and a destination flag (`--s3` or `--gcs`) to write to your store.

#### Example: Backfill an S3 bucket from Haneullabs's GCS bucket (Mainnet)

```sh
haneul-checkpoint-blob-indexer \
...
The Mainnet bucket is `gs://haneullabs-mainnet-checkpoints-use4`. For Testnet, use `--remote-store-gcs haneullabs-testnet-checkpoints-use4` to read from `gs://haneullabs-testnet-checkpoints-use4`. Both buckets are Requester Pays enabled.

Do not use `--remote-store-url https://checkpoints.mainnet.haneul.io` or `--remote-store-url https://checkpoints.testnet.haneul.io` for full-retention backfill. The public HTTPS endpoints retain only the latest 30 days of checkpoints. Use `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` for Mainnet or `--remote-store-gcs haneullabs-testnet-checkpoints-use4` for Testnet instead.

:::info

`gs://haneullabs-mainnet-checkpoints-use4` and `gs://haneullabs-testnet-checkpoints-use4` are Requester Pays enabled. Configure credentials and a billing project when reading from either bucket. For the lowest steady-state cost and latency, stream from your own full node after backfill completes.
...
| Flag | Description |
|------|-------------|
| `--remote-store-gcs <BUCKET>` | Fetch checkpoints from a GCS bucket. Required for full-retention backfill from `gs://haneullabs-mainnet-checkpoints-use4` or `gs://haneullabs-testnet-checkpoints-use4`. |
...
```yaml
scrape_configs:
  - job_name: haneul-checkpoint-blob-indexer
    static_configs:
      - targets: ['<HOST>:9184']
```
