...
Internally at Haneul Labs, indexer instances use 8 CPUs and 16 GiB of memory for backfill, and scale down to 0.5 CPU and 1 GiB of memory for normal operation at network tip.
...
For steady-state operation, use the public full node gRPC endpoint pattern `https://fullnode.<network>.haneul.io:443`, for example:
...
$ haneul-indexer-alt indexer \
    --config events.toml \
    --database-url postgres://username:password@localhost:5432/database \
    --rpc-api-url https://fullnode.mainnet.haneul.io:443 \
    --streaming-url https://fullnode.mainnet.haneul.io:443
```

:::info

`https://checkpoints.mainnet.haneul.io` retains only the most recent 30 days of checkpoints. For Mainnet backfills older than 30 days or from genesis, use `gs://haneullabs-mainnet-checkpoints-use4` through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4`.

If the checkpoint bucket you are using has Requester Pays enabled, configure GCS credentials and a billing project when using `--remote-store-gcs`. In Haneul's object store config, the relevant fields are `google_service_account` and `google_project_id`. `google_project_id` is sent as the `x-goog-user-project` header required by GCS Requester Pays.

For more on Requester Pays, see [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays). For source selection and steady-state operation guidance, see [Running a Remote Store](/operators/data-management/remote-store-setup) and [Archival Stack Setup](/operators/data-management/archival-stack-setup). In general, `https://checkpoints.mainnet.haneul.io` should not be your default source in steady state if a full node gRPC endpoint is available.
...
Once caught up to network tip, you can manually set the max concurrency to a lower value. Haneullabs-internal indexers are capped at 20.
:::
...
```sh
$ haneul-indexer-alt-consistent-store restore \
    --azure <AZURE_BUCKET> \
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `<AZURE_BUCKET>` \*         | Name or URL of Azure bucket containing [managed snapshots](https://docs.haneul.io/operators/snapshots#haneullabs-labs-managed-snapshots).  |
| `<DATABASE_PATH>`           | Path to RocksDB database.                                                                                                                 |
| `<GCS_ACCOUNT>` \*          | Name or URL of GCS bucket containing [managed snapshots](https://docs.haneul.io/operators/snapshots#haneullabs-labs-managed-snapshots).    |
...
| `<S3_BUCKET>` \*            | Name or URL of AWS S3 bucket containing [managed snapshots](https://docs.haneul.io/operators/snapshots#haneullabs-labs-managed-snapshots). |
...
$ haneul-indexer-alt-consistent-store restore \
    ...
    --http https://formal-snapshot.mainnet.haneul.io \
...
`https://checkpoints.mainnet.haneul.io` retains only the most recent 30 days of checkpoints. For Mainnet backfills older than 30 days, use `gs://haneullabs-mainnet-checkpoints-use4` via the indexing framework's `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` option instead of `--remote-store-url`.
...
```sh
$ haneul-indexer-alt-consistent-store run \
    --config <CONFIG_FILE> \
    --database-path <DATABASE_PATH> \
    --remote-store-url <REMOTE_STORE_URL>
```
...
For Mainnet full-history backfill, use `gs://haneullabs-mainnet-checkpoints-use4` through `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` rather than `--remote-store-url`.
...
### Run `haneul-indexer-alt-graphql`

:::info

If you use the Haneul Foundation-hosted public good archival service on Testnet or Mainnet, you might encounter performance issues. The team plans to address these before the GraphQL RPC and Archival Service reach general availability.

:::

Use the following command to run a GraphQL RPC server node:
```sh
haneul-indexer-alt-graphql rpc \
...
haneul-indexer-alt-graphql rpc \
    ...
    --ledger-grpc-url https://archive.mainnet.haneul.io:443 \
...
haneul-indexer-alt-graphql generate-config > <PATH_TO_GRAPHQL_CONFIG_FILE>
```
...
Haneul Labs uses the following autovacuum settings for a 32-vCPU, 256 GiB Postgres instance. Adjust these proportionally for your setup:
