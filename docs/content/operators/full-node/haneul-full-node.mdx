...
These instructions are for advanced users. If you just need a local development environment, you should instead follow the instructions in [Create a Local Haneul Network](/getting-started/onboarding/local-network) to create a local full node, validators, and faucet.
...
This synchronization process requires listening to at a minimum `2f+1` validators to ensure that a full node has properly processed all new transactions. Haneul improves the synchronization process with the introduction of checkpoints and the ability to synchronize with other full nodes.
...
A Haneul full node has the potential to maintain the full history of the chain. That changes with gRPC as JSON-RPC gets phased out.
...
You must enable gRPC support on your full nodes. JSON-RPC is **deprecated**. See [Haneul's data serving roadmap](/develop/accessing-data/data-serving) for guidance on the overall transition plan.

To serve the [gRPC API](/develop/accessing-data/grpc), you must enable it on your full node by indexing gRPC-specific data. During the initial gRPC indexing phase, your full node might not be able to handle other traffic, including the [**deprecated** JSON-RPC](/references/haneul-api) requests. Plan your rollout carefully (such as using the `rolling upgrade` mechanism), and communicate any downtime to your customers and partners in advance. The time required to sync gRPC indexes depends on your full node's hardware and software configuration.
...
Refer to the [`RpcConfig` source](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-config/src/rpc_config.rs) for the complete list of available `rpc` options.
...
Instructions for building, installing, or downloading the `haneul-node` binary are available in the [Install from Binaries](/getting-started/onboarding/install-binaries) guide. 
...
Keep the following considerations in mind when running a Haneul full node, as they apply to all environments:

* [Genesis](/operators/genesis): You must download the genesis blob for the network that you want to connect to, and make it available to the `haneul-node`.
* [Data Storage](/operators/data-management/managing-data): Haneul full nodes can require a significant amount of disk space to store the blockchain history. If you plan to use your full node to serve RPC requests, you must also plan for the storage of index files, which requires a significant amount of disk space.
* [Monitoring](/operators/full-node/monitoring): Haneul full nodes expose metrics about the node's health and the state of the Haneul network.
* [Updates](/operators/full-node/updates): Haneul full nodes must be updated to the latest version to remain in sync with the network.
...
The Haneul repository includes a guide on running a full node through [Docker Compose](https://github.com/GeunhwaJeong/haneul/tree/main/docker/fullnode#readme).
...
1. Download the `docker-compose.yaml` file:
    ...
    ```sh
    $ curl -fLO https://raw.githubusercontent.com/HaneulLabs/haneul/branch-name/docker/fullnode/docker-compose.yaml
    ```

    The compose file pins a specific `haneullabs/haneul-node` image tag. Update that tag to the version and network you target before you start the node.

1. Download the `fullnode-template.yaml` file:

    ```sh
    $ curl -fLO https://github.com/GeunhwaJeong/haneul/raw/branch-name/crates/haneul-config/data/fullnode-template.yaml
    ```
...
You can also build `haneul-node` directly from the Haneul repository using [Cargo](https://doc.rust-lang.org/cargo/), the Rust package manager.

1. Install the prerequisites listed in [Software requirements](#software-requirements).

1. Clone the Haneul repository, replacing `branch-name` with the branch for your target network:
...
1. Build and run `haneul-node` from source:
...
1. Download the genesis blob for the network to use:
    - [Devnet genesis blob](https://github.com/GeunhwaJeong/haneul-genesis/raw/main/devnet/genesis.blob):
        ```sh
        $ curl -fLO https://github.com/GeunhwaJeong/haneul-genesis/raw/main/devnet/genesis.blob
        ```
    - [Testnet genesis blob](https://github.com/GeunhwaJeong/haneul-genesis/raw/main/testnet/genesis.blob):
        ```sh
        $ curl -fLO https://github.com/GeunhwaJeong/haneul-genesis/raw/main/testnet/genesis.blob
        ```
    - [Mainnet genesis blob](https://github.com/GeunhwaJeong/haneul-genesis/raw/main/mainnet/genesis.blob):
        ```sh
        $ curl -fLO https://github.com/GeunhwaJeong/haneul-genesis/raw/main/mainnet/genesis.blob
        ```
...
Instead, start your full node from a recent snapshot. You can find details on how to obtain a snapshot from the [Haneul Snapshots guide](/operators/snapshots).

After you have your full node config file set up and you have obtained a snapshot, start your full node by running the `haneul-node` binary with your `fullnode.yaml` configuration file:
...
By default, `haneul-node` writes its logs to standard error. When you run the node under systemd and do not override the output settings, `journalctl` captures these logs.

The `RUST_LOG` environment variable controls the log filter. The default level is `info`. You can raise or lower the level for individual components, for example `haneul_core=debug`. For more on filter syntax, see [Logging levels](/operators/observability#logging-levels) in the observability guide.

### Recommended systemd unit

The following unit file runs `haneul-node` and sends its output to the systemd journal. Adjust the binary path, configuration path, user, and `RUST_LOG` filter for your environment.

```ini title='/etc/systemd/system/haneul-node.service'
[Unit]
Description=Haneul Full Node
After=network-online.target
Wants=network-online.target

[Service]
User=haneul
ExecStart=/usr/local/bin/haneul-node --config-path /opt/haneul/fullnode.yaml
Environment=RUST_LOG=info,haneul_core=debug,consensus=debug,jsonrpsee=error
...
$ sudo systemctl enable --now haneul-node
```
...
```ini
Environment=RUST_LOG_FILE=/var/log/haneul/haneul-node.log
```

The `haneul` user that runs the node must own the log directory and have write permission:

```sh
$ sudo mkdir -p /var/log/haneul
$ sudo chown haneul:haneul /var/log/haneul
```

`haneul-node` rotates the log file daily and appends a date suffix to the file name. Daily rotation is built in, but the node does not delete, compress, or cap the number of old files. Handle retention and compression with `journald`, `logrotate`, or your operations tooling.
...
`haneul-node` exposes a private admin API that reports and updates the active log filter without a restart. The admin server binds to `127.0.0.1` on port 1337 by default. You can change the port with the `admin-interface-port` field in the node configuration file.
...
```sh
$ curl -X POST http://127.0.0.1:1337/logging --data 'info,haneul_core=debug'
```
...
- Confirm the systemd unit name. The service might not be named `haneul-node`. List the service units with `systemctl list-units --type=service | grep -i haneul`.
