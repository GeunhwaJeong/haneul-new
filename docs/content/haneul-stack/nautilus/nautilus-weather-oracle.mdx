...
<CodeWalkthrough source="K4/example/move/weather.move" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" language="move">
  <Step lines="13-18" title="`WeatherNFT` data struct">
    The `WeatherNFT` struct stores verified weather data as a transferable Haneul object. It has `key` and `store` abilities. The `location`, `temperature`, and `timestamp_ms` fields are all verified by the enclave signature before the contract creates the object.
  </Step>
