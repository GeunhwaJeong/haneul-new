...
description: Build a verified weather oracle on Haneul using Nautilus, where a Trusted Execution Environment fetches weather data, signs it cryptographically, and the Move contract mints an NFT only after verifying the signature.
...
Nautilus is a framework for secure offchain computation on Haneul. It uses AWS Nitro Enclaves to run application code in an isolated environment where:
...
For the full Nautilus framework and deployment tooling, see the [Nautilus documentation](/haneul-stack/nautilus/nautilus-overview).
...
The rest of the Nautilus framework (enclave runtime, attestation, key management) comes from the [Nautilus repo](https://github.com/GeunhwaJeong/nautilus). You only write the application logic.
...
[dependencies]
enclave = { git = "https://github.com/GeunhwaJeong/nautilus.git", subdir = "move/enclave", rev = "main" }
...
Follow the Nautilus documentation to [setup and deploy your enclave](/haneul-stack/nautilus/using-nautilus). After deploying the enclave to an AWS Nitro instance, call `GET /get_attestation` and submit the attestation document to the contract's `register_enclave` function. This stores the enclave's public key onchain.
...
The enclave returns a signed response containing the location, temperature, and timestamp. Submit the signed response to the `update_weather` function on Haneul. The contract verifies the signature and mints a `WeatherNFT` with the verified data.

To verify the NFT was created:

```bash
$ haneul client tx-block TX_DIGEST --json | jq '.objectChanges[] | select(.type == "created")'
```
...
<ImportContent source="K4/example/move/weather.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="update_weather" />
...
The `WeatherNFT` struct stores verified weather data as a transferable Haneul object.

<ImportContent source="K4/example/move/weather.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" struct="WeatherNFT" />
...
### Enclave data handler

The `process_data` function runs inside the TEE. It fetches weather data from the external API, validates freshness, and signs the response.

<ImportContent source="K4/example/rust/mod.rs" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="process_data" />

The handler fetches from `weatherapi.com`, extracts the location name and temperature, and validates that the data is no older than 1 hour (3,600,000 milliseconds). It creates a `WeatherResponse` struct (which must match the Move `WeatherResponse` for BCS compatibility) and signs it with the enclave's ephemeral key pair through `to_signed_response`.
