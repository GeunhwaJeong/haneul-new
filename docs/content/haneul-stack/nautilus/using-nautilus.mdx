...
[Nautilus](/haneul-stack/nautilus) is a verifiable offchain compute layer on Haneul. It enables builders to delegate sensitive or resource-intensive tasks to a self-managed [trusted execution environment (TEE)](https://en.wikipedia.org/wiki/Trusted_execution_environment) or a TEE marketplace like [Marlin Oyster](/haneul-stack/nautilus/community-dev-tools) while preserving trust onchain through smart contract-based verification.

The [Nautilus repo](https://github.com/GeunhwaJeong/nautilus) provides scaffolding such as reproducible builds, signature formatting, and HTTPS traffic forwarding. This allows you to focus on implementing the offchain computation logic inside the enclave.

An onchain template includes the minimal smart contract code required to register a Nautilus instance and its public key. 
...
1. Write and deploy a basic Nautilus offchain instance using AWS Nitro Enclaves. The example instance runs a server that fetches weather data for a specific location.

2. Write a Move smart contract that registers the enclave by verifying its attestation and public key, then verifies the Nautilus response (signature and payload) onchain and mints an NFT with the location and temperature data.
...
- [x] Install [AWS CLI v2](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install).

- [x] Install [Rust](https://rust-lang.org/tools/install/) and [cargo](https://doc.rust-lang.org/cargo/getting-started/installation).

- [x] Install [Make](https://www.gnu.org/software/make/).

- [x] [Install the Haneul CLI](/getting-started/onboarding/haneul-install).

- [x] Download or git clone the [Nautilus repo](https://github.com/GeunhwaJeong/nautilus).
...
Set up an AWS developer account and install the AWS CLI. For detailed instructions, see the [AWS Nitro Enclaves getting started guide](https://docs.aws.amazon.com/enclaves/latest/user/getting-started#launch-instance).
...
`AdministratorAccess` is used here only to simplify this walkthrough. In production, do not grant broad administrator permissions: scope an IAM role to the least privilege the enclave deployment actually needs (EC2, Nitro Enclaves, and KMS as applicable). Never log AWS credentials, enclave secrets, or attestation private material, and always verify the enclave's PCRs against the values registered onchain before trusting any output it produces. See the [Nautilus](/develop/security/best-practices#nautilus) section of the Security Best Practices guide.
...
```
{
    "UserId": "AROARMBANW4MXIS7SZ7S6:george.melas@haneul-labs.com",
    "Account": "094557288217",
    "Arn": "arn:aws:sts::094557288217:assumed-role/AWSReservedSSO_AdministratorAccess_bb69586677f0373c/george.melas@haneul-labs.com"
}
```
...
- `get_attestation`: Returns a signed attestation document over the enclave public key. Use this during onchain registration. This logic is built into the template and does not require modification.
...
Wait 2 to 3 minutes for EC2 initialization.
...
1. Modify the code to [inject a deterministic private key](https://github.com/GeunhwaJeong/nautilus/blob/main/src/nautilus-server/src/main.rs#L16) into the enclave. This allows you to run the server anywhere with the same private key without needing an AWS Nitro instance.
2. Use `make run-debug` instead of `make run`. Debug mode produces all-zero PCR values. You can register the enclave onchain with these all-zero PCRs once and reuse it for development without needing to update PCRs onchain. 
