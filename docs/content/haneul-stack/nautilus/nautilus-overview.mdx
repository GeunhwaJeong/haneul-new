...
It currently supports self-managed [AWS Nitro Enclave TEEs](https://aws.amazon.com/ec2/nitro/nitro-enclaves/) and Dockerized deployments through [Marlin](/haneul-stack/nautilus/community-dev-tools). You can verify AWS-signed enclave attestations onchain using Haneul smart contracts written in Move. Refer to the [Github repo](https://github.com/GeunhwaJeong/nautilus) for the reproducible build template.
...
- Deploy the offchain server to a self-managed TEE, such as AWS Nitro Enclaves, or use [Marlin Oyster](/haneul-stack/nautilus/community-dev-tools) to deploy Dockerized Nautilus apps.
...
Refer to [Nautilus design](/haneul-stack/nautilus/nautilus-design) and [using Nautilus](/haneul-stack/nautilus/using-nautilus) for details.
