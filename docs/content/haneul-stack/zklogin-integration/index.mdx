...
const FULLNODE_URL = 'https://fullnode.testnet.haneul.io:443'; // or mainnet: https://fullnode.mainnet.haneul.io:443
const haneulClient = new HaneulGrpcClient({ baseUrl: FULLNODE_URL, network: 'testnet' });
...
zkLogin uses the user salt to compute the zkLogin Haneul address (see [definition](/haneul-stack/zklogin-integration)). The salt must be a 16-byte value or an integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
...
2. Backend service that exposes an endpoint that returns a unique salt for each user consistently.
   ...
   - Option 4: Implement a service that keeps a master seed value, and derive a user salt with key derivation by validating and parsing the JWT. For example, use `HKDF(ikm = seed, salt = iss || aud, info = sub)` defined [here](https://github.com/GeunhwaJeong/fastcrypto/blob/e6161f9279510e89bd9e9089a09edc018b30fbfe/fastcrypto/src/hmac.rs#L121). This option does not allow any rotation on master seed or change in client ID (that is, aud), otherwise a different user address is derived and results in loss of funds.

The following is an example request and response for the Haneul Labs-maintained salt server (using option 4). If you want to use the Haneul Labs salt server, refer to the [Enoki docs](https://docs.enoki.haneul-labs.com/) and contact us. Only valid JWT authenticated with whitelisted client IDs are accepted.
...
User salt is used to disconnect the OAuth identifier (sub) from the onchain Haneul address to avoid linking Web2 credentials with Web3 credentials. While losing or misusing the salt could enable this link, it wouldn't compromise fund control or zkLogin asset authority. 
...
If you want to use the Haneullabs hosted ZK Proving Service for Mainnet, refer to the [Enoki docs](https://docs.enoki.haneul-labs.com/) and contact us for access.
...
- If you want to compile the prover from scratch (for performance reasons), see the fork of [rapidsnark](https://github.com/GeunhwaJeong/rapidsnark#compile-prover-in-server-mode). You need to compile and launch the prover in server mode.
...
The prover that Haneul Labs maintains is set to auto-scale to handle traffic surges. If you are not sure whether Haneul Labs can handle a specific number of requests or expect a sudden spike in the number of prover requests your application needs to make, reach out to us on [Discord](https://discord.gg/haneul). The plan is to horizontally scale the prover to handle any RPS you require.