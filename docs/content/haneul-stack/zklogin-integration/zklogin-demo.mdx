...
description: Build a React application that authenticates users with OAuth providers and derives Haneul wallet addresses using zero-knowledge proofs, enabling passwordless blockchain access.
...
repo: https://github.com/GeunhwaJeong/haneul-move-bootcamp
path: /K2
concepts:
  - zklogin
  - ephemeral_key_pairs
...
This example demonstrates how to integrate [zkLogin](/haneul-stack/zklogin-integration/zklogin) into a React application so users can authenticate with familiar OAuth providers (Google, Apple) and interact with the Haneul blockchain without managing private keys.

:::caution

This is a learning demo, not a production-ready integration. It runs entirely in the frontend, uses a hardcoded salt, and calls a public development prover. Before shipping a zkLogin app to production:

- Use a production prover (self-hosted or a managed service) instead of the public development endpoint, and proxy prover calls through your backend with correct CORS configuration.
- Run a real per-user salt service with availability and backups rather than a fixed salt; a lost salt permanently locks the user out of their address.
- Handle ephemeral keys carefully: store them in session (not local) storage, scope `maxEpoch` to a short session, and never persist them insecurely.
- Never log JWTs, ZK proofs, or salts, as these are sensitive credentials.
...
- Onboard users who do not have a Haneul wallet by letting them sign in with Google or Apple.

- Derive a deterministic Haneul address from a JWT token and a salt so users always get the same address.
...
- **Address derivation:** `jwtToAddress(jwt, salt)` deterministically maps an OAuth identity to a Haneul address. The same user with the same salt always gets the same address, enabling persistent wallet identity across sessions.
...
The example has 4 actors: a React frontend, an OAuth provider, a ZK prover service, and the Haneul network. There are no Move contracts; the application sends native HANEUL transfer transactions. The React frontend orchestrates the entire flow. It generates an ephemeral key pair, redirects the user to the OAuth provider (Google or Apple) in a popup, receives the JWT, sends proof inputs to the ZK prover service hosted by Haneul Labs, and uses the resulting proof to sign and execute transactions on the Haneul network through gRPC.
...
sequenceDiagram
  ...
  participant Haneul as Haneul network

  User->>Frontend: Click Generate Key Pair (Step 1)
  Frontend->>Haneul: Fetch current epoch
  Haneul-->>Frontend: Current epoch
  ...
  Frontend->>Haneul: Execute transaction with zkLogin signature
  Haneul-->>Frontend: Transaction digest
  Frontend-->>User: Display confirmation
```

The following steps walk through the flow:

1. The user clicks **Generate Key Pair**. The frontend fetches the current epoch from the Haneul network, creates an Ed25519 ephemeral key pair, generates randomness, and computes a nonce. The key pair is valid for the configured number of epochs past the current one.
...
3. The user clicks **Generate ZK Proof**. The frontend sends the JWT, extended ephemeral public key, max epoch, randomness (base64-encoded), and salt to the Haneul Labs prover service. The prover returns a zero-knowledge proof. The frontend then derives the Haneul address from the JWT and salt using `jwtToAddress`.

4. The user enters a recipient address and HANEUL amount. The frontend builds a `splitCoins` + `transferObjects` transaction, signs it with the ephemeral key pair, combines the signature with the ZK proof into a zkLogin signature using `getZkLoginSignature`, and executes the transaction through gRPC.

### How zkLogin works

The zkLogin system maps an OAuth identity (like a Google account) to a Haneul address using zero-knowledge proofs. The key insight is that the user's Haneul address is derived deterministically from 2 inputs: their OAuth `sub` claim (a unique user identifier from the provider) and an application-specific salt. The same user with the same salt always gets the same address.
...
For more details, see the [zkLogin documentation](/haneul-stack/zklogin-integration/zklogin).
...
- [x] [Install the latest version of Haneul](/getting-started/onboarding/haneul-install).

- [x] [Configure the Haneul client](/getting-started/onboarding/configure-haneul-client).

- [x] [Create a Haneul address](/getting-started/onboarding/get-address).

- [x] [Get HANEUL Testnet tokens](/getting-started/onboarding/get-coins).

- [x] Download and install an IDE. The following are recommended, as they offer Move extensions:

    - [VSCode](https://code.visualstudio.com/), corresponding [Move extension](https://marketplace.visualstudio.com/items?itemName=haneullabs.move)
...
- [x] A Haneul wallet ([Slush Wallet](https://slush.app/) or another compatible wallet)
...
```bash
$ git clone -b solution https://github.com/GeunhwaJeong/haneul-move-bootcamp.git
$ cd haneul-move-bootcamp/K2
```
...
```bash title='.env'
VITE_NETWORK=devnet
VITE_HANEUL_GRPC_URL=https://fullnode.devnet.haneul.io:443
...
Replace `YOUR_GOOGLE_CLIENT_ID` with the OAuth client ID from your Google Cloud project. The `VITE_SALT` can be any large random number; it determines the derived Haneul address. Use the same salt consistently to get the same address.
...
Open `http://localhost:5173` in a browser. The landing page explains what zkLogin is. Click **Get Started** to enter the 4-step wizard. Complete each step in order: generate a key pair, sign in with Google, generate the ZK proof, then send a test HANEUL transfer to verify the full flow.
...
<ImportContent source="K2/src/hooks/useEphemeral.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="useEphemeral" />
...
<ImportContent source="K2/src/hooks/useOauthPopup.tsx" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="useOauthPopup" />
...
The `useZkProof` hook prepares the proof payload and fetches it from the Haneul Labs prover service.

<ImportContent source="K2/src/hooks/useZkProof.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="useZkProof" />
...
The `useWallet` hook derives the deterministic Haneul address from the JWT and salt.

<ImportContent source="K2/src/hooks/useWallet.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="useWallet" />

`jwtToAddress` maps the JWT's `sub` claim and the salt to a Haneul address. `genAddressSeed` produces the address seed needed later for the zkLogin signature. The same JWT `sub` + salt always produces the same address, so the user's wallet persists across sessions.

### Transaction building and zkLogin execution

The `haneulWriteClient` builds a HANEUL transfer transaction and executes it with a zkLogin signature.

<ImportContent source="K2/src/services/haneul/writeClient.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="buildAndSignTransfer" />
...
The `fetchZkProof` utility calls the Haneul Labs prover endpoint.

<ImportContent source="K2/src/utils/zk.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="fetchZkProof" />

The function POSTs the proof payload to `https://prover-dev.haneul-labs.com/v1`. The error handling distinguishes network failures (which might indicate CORS issues) from prover errors. For production applications, consider proxying prover requests through a backend to avoid CORS restrictions.
...
- **Add sponsored transactions:** Instead of requiring the zkLogin wallet to hold HANEUL for gas, integrate a gas station service that sponsors transactions. Build the transaction with a separate gas owner and have the sponsor co-sign.
...
**Cause:** The `prover-dev.haneul-labs.com` endpoint might not allow requests from `localhost` or your domain.
...
**Symptom:** The wallet address is derived successfully but shows a 0 HANEUL balance.

**Cause:** The derived address has not received any HANEUL. The address is deterministic but starts with no funds.

**Fix:** Use the [Haneul faucet](https://faucet.haneul.io/) to request Devnet HANEUL for the derived address. Copy the address from the Step 3 panel and paste it into the faucet.
