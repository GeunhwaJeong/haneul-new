...
For uniqueness, each blob uploaded to Walrus also creates a corresponding [`Blob` NFT object on Haneul](https://docs.wal.app/dev-guide/haneul-struct#blob-and-storage-objects) with a unique ID. Furthermore, the associated `Blob` object can optionally have a `Metadata` [dynamic field](/develop/objects/dynamic-fields). 

`Metadata` dynamic fields are a key-value extension that augment an onchain object's data at runtime.. If set, this dynamic field acts as [a mapping of key-value attribute pairs](https://docs.wal.app/usage/client-cli.html?highlight=attribute#blob-attributes). 
