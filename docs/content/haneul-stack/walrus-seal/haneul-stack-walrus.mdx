...
The backend `encryptImages.ts` script handles encryption before upload. The frontend `usePostDecryption` hook handles decryption after reading. Both are covered in depth in [Encryption with Seal](/haneul-stack/walrus-seal/haneul-stack-seal), which builds on this same OnlyFins app.
