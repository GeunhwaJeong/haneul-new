...
keywords: [seal, encryption, access control, walrus, OnlyFins, Haneul Stack, threshold encryption, SealClient]
verified_against:
  haneul_ts_sdk: "^1.x"
  walrus_sdk: "^0.x"
  seal_sdk: "^0.x"
...
This guide continues from [Data Storage Using Walrus](/haneul-stack/walrus/haneul-stack-walrus) and builds on the same [OnlyFins](https://github.com/GeunhwaJeong/onlyfins-example-app) app. The Walrus guide covers how OnlyFins stores and reads images. This guide covers how it protects them: encrypting images before upload and enforcing onchain access control so only authorized users can decrypt them.

Seal is a decentralized secrets management (DSM) service that uses access control policies defined and validated on Haneul. You use Seal to encrypt sensitive data before storing it on Walrus, onchain, or in any other storage, and then issue decryption keys only to users who satisfy your onchain access conditions.

Seal has 3 core components:

- **Onchain access policies:** Move functions you write that define who is allowed to decrypt. [Key servers](https://seal-docs.wal.app/ServerOverview) evaluate your policy by running a `dry_run_transaction_block` on Haneul when a user requests decryption key shares. If the policy approves, the servers return the key shares.
...
In OnlyFins, the access policy is: the user must own a `ViewerToken` object for the specific post they want to unlock. The user receives a `ViewerToken`, a [Haneul object](/develop/haneul-architecture/object-model), after requesting access onchain. Seal key servers check this ownership before issuing key shares.
...
sequenceDiagram
    ...
    WAL-->>B: blob_id
    B->>HANEUL: createPosts PTB (blob_id, encryption_id)
    HANEUL-->>B: Post object created

    participant BR as BROWSER
    Note over BR: Request access
    BR->>HANEUL: usePayForContent PTB
    HANEUL-->>BR: ViewerToken minted

    Note over BR: Decrypt
    BR->>WAL: fetchFromWalrus(blob_id)
    WAL-->>BR: encrypted bytes
    BR->>KS: fetchKeys(ptbBytes, sessionKey)
    KS->>HANEUL: dry_run_transaction_block
    HANEUL-->>KS: policy result
    KS-->>BR: key shares
...
Your `seal_approve*` access policy is only as trustworthy as the upgrade policy of the package that defines it. Anyone who can upgrade the package can rewrite the policy and grant themselves decryption access, so govern that upgrade path carefully (for example, with an immutable package or a multisig-controlled `UpgradeCap`). Diversify your key servers across independent operators so no single operator can recover plaintext, choose a threshold that balances availability against collusion resistance, and use envelope encryption for large or sensitive data rather than encrypting it directly with Seal. See the [Seal encryption and access control](/develop/security/best-practices#seal-encryption-and-access-control) section of the Security Best Practices guide.
...
2. Store the encrypted blob on Walrus. The blob ID is stored on the `Post` Haneul object.
...
The `@haneullabs/seal` package provides `SealClient` for encryption and decryption, and `SessionKey` for managing user-approved access sessions.
...
The `@haneullabs/seal` package provides 2 primary classes:

- **`SealClient`:** Handles encryption, key fetching, and decryption. Can be instantiated standalone or as a Haneul client extension.
...
:::info
Some code examples in this guide use older Haneul transports: OnlyFins uses `HaneulClient` from `@haneullabs/haneul/client`, and the seal-demo standalone script uses `HaneulJsonRpcClient` from `@haneullabs/haneul/jsonRpc`, which is deprecated. `HaneulGrpcClient` from `@haneullabs/haneul/grpc` is the recommended transport for all new integrations. All `SealClient` constructor and encrypt/decrypt APIs are identical regardless of which transport you use. See the [Haneul SDK v2 migration guide](https://sdk.haneul-labs.com/haneul/migrations/haneul-2.0) for upgrade instructions.
:::
...
OnlyFins uses a purchase-gated pattern, but Seal's `seal_approve` mechanism is fully generic. The [seal-demo](https://github.com/GeunhwaJeong/haneul-move-bootcamp/tree/main/K5/seal-demo) in the Haneul Move Bootcamp provides 3 minimal working implementations of the most common patterns.

#### Owner-only access

The most basic policy: only the wallet address encoded in the identity decrypts the content. The entire `seal_approve` function is a single assertion comparing the [Binary Canonical Serialization (BCS)](https://sdk.haneul-labs.com/haneul/bcs)-encoded caller address to the `id` bytes:

<ImportContent
  source="K5/seal-demo/move/sources/private_seal.move"
  mode="code"
  org="HaneulLabs"
  repo="haneul-move-bootcamp"
  language="move"
/>

#### Time-lock access

Users can only decrypt data after a specific timestamp has passed. The `id` encodes a `u64` unlock time in milliseconds. The `seal_approve` function peeks the timestamp from the identity bytes and checks it against the Haneul [`Clock`](/haneul-stack/on-chain-primitives/access-time) object:
...
For subscription-based access (time-bound paid access) and other patterns, see the Seal repository's [`examples/move`](https://github.com/GeunhwaJeong/seal/tree/main/examples) directory.
...
The `encryptionId` is then stored as the `encryption_id` field on the `Post` Haneul object when `createPosts.ts` registers the post onchain. This links the Walrus blob, the Seal encryption, and the Haneul object together.
...
The `createAndSignSessionKey` utility in `sealUtils.ts` shows the core signing flow in isolation, with notes on signature format requirements for both Haneul wallets and non-Haneul wallets:
