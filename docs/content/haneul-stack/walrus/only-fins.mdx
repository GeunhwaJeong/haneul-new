...
description: Build a Web3 social platform with encrypted content, Seal-based access control, and Walrus storage on Haneul.
...
repo: https://github.com/GeunhwaJeong/onlyfins-example-app
path: /
concepts:
  - shared_objects
  - capability_pattern
...
- Let users authenticate with Google through zkLogin and interact without holding HANEUL for gas.
...
- **Capability pattern:** `ViewerToken` objects act as owned [capability](/develop/write-move/haneul-move-concepts) tokens that prove a user purchased access to a specific post.
...
- **Sponsored transactions:** A backend pays gas so users interact without holding HANEUL. The frontend sends unsigned transactions to the sponsor backend, which co-signs and returns the sponsored bytes.
...
- [x] A Haneul wallet ([Slush Wallet](https://slush.app/) or another compatible wallet) or willingness to use [Enoki](https://portal.enoki.haneul-labs.com/) zkLogin
...
```bash
$ git clone https://github.com/GeunhwaJeong/onlyfins-example-app.git
...
Then copy the value `exportedPrivateKey`. It should start with `haneulprivkey1...`
...
<ImportContent source="frontend/move/posts/sources/posts.move" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="create_post" />
...
<ImportContent source="frontend/move/posts/sources/posts.move" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="seal_approve_access" />
...
<ImportContent source="frontend/move/posts/sources/posts.move" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="grant_access" />
...
<ImportContent source="frontend/src/hooks/usePayForContent.ts" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="usePayForContent" />
...
<ImportContent source="frontend/src/hooks/usePostDecryption.ts" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="usePostDecryption" />
...
<ImportContent source="frontend/src/providers/SessionKeyProvider.tsx" mode="code" org="HaneulLabs" repo="onlyfins-example-app" fun="SessionKeyProvider" />
...
- **Add payment for access:** Require viewers to pay HANEUL or a custom token in the `grant_access` function before minting the `ViewerToken`. Transfer the payment to the post author.
...
**Fix:** Verify the blob ID in the post object with `haneul client object POST_ID`. Check that the Walrus aggregator URL in `constants.ts` is reachable. Re-upload the image if the blob ID is a placeholder.
