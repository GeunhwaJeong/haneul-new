...
description: Learn how to store and retrieve data on Walrus by walking through OnlyFins, a content platform built on Haneul and Walrus.
keywords: [walrus, blob storage, walrus SDK, OnlyFins, Haneul Stack, upload, read, seal]
last_verified: 2026-04-15
---

<AgentPrompt
  prompt={"Upload this app's media or NFT assets to Walrus, store the resulting blob references, and wire them into Haneul object metadata or Display fields."}
/>

Walrus is a decentralized storage and data availability protocol built specifically for large binary files, or blobs. It uses Haneul for coordination, payment, and governance, making it a natural storage layer for apps built on Haneul.

Each section explains a Walrus concept or operation and shows where it appears in the encrypted social media example [OnlyFins](only-fins.mdx). OnlyFins is a demo content platform where creators post images stored on [Walrus](https://docs.wal.app/), some encrypted with [Seal](https://seal-docs.wal.app/). Each post is a Haneul object. Users request access onchain to unlock encrypted posts, demonstrating how Haneul, Walrus, and Seal work together as a stack.

For patterns OnlyFins does not cover, the guide draws on [Walrus Onboarding](https://github.com/GeunhwaJeong/Walrus-Onboarding) modules and [walrus-pocs](https://github.com/GeunhwaJeong/walrus-pocs), a collection of focused Walrus proof-of-concept examples maintained by Haneul Labs. The [Walrus SDK documentation](https://sdk.haneul-labs.com/walrus) is the authoritative reference for all SDK options. The [Encryption with Seal](/haneul-stack/seal/haneul-stack-seal) guide builds on this same app.
...
flowchart LR
    ...
    subgraph haneul_chain ["Haneul"]
        HANEUL["HANEUL TRANSACTION\nregister blob"]
    end
    ...
    NODES -->|"blob_id"| HANEUL
    HANEUL -->|"blob_id + object_id"| APP
...
Both blob contents and their metadata are public. Blob IDs, blob attributes, and the onchain blob registration are all discoverable by anyone, so an attacker can enumerate and read everything you store. Never store secrets or private data unencrypted. For sensitive content, encrypt before upload using [Seal](/haneul-stack/seal/haneul-stack-seal) or envelope encryption, and treat blob IDs and attributes as public. See [Data encryption](#data-encryption) and the [Security Best Practices](/develop/security/best-practices) guide.
...
- **Object ID:** The Haneul object ID of the onchain blob registration record. Because blobs are [Haneul objects](/develop/haneul-architecture/object-model), they can be owned by addresses, transferred, wrapped in other objects, or managed through Haneul smart contracts.

In OnlyFins, the `image_blob_id` field on each `Post` Haneul object stores the blob ID. The frontend uses this to construct a fetch URL. The object ID lets the app manage blob lifecycle onchain, the same way it manages any other Haneul object.
...
For production environments, use the `send-object-to` parameter when uploading through a publisher. This instructs the publisher to transfer the resulting blob object to a specified Haneul address after upload, so you retain ownership of blobs stored through a shared publisher.
...
The Walrus TS SDK (`@haneullabs/walrus`) extends the Haneul TypeScript SDK client with Walrus-specific methods. It exposes high-level methods for reading and writing blobs, as well as lower-level methods for individual steps when you need more control.
...
Set up the client by extending a `HaneulGrpcClient` with the Walrus plugin:
...
The [Walrus SDK documentation](https://sdk.haneul-labs.com/walrus) is the single source of truth for all SDK patterns, options, and examples.

:::info
Some code examples in this guide use `HaneulClient` from `@haneullabs/haneul/client` rather than `HaneulGrpcClient` from `@haneullabs/haneul/grpc`. `HaneulGrpcClient` is the recommended transport for new integrations. All Walrus SDK and transaction APIs are identical regardless of which client you use. See the [Haneul SDK v2 migration guide](https://sdk.haneul-labs.com/haneul/migrations/haneul-2.0) for upgrade instructions.
:::

For React apps, wrap your component tree with a context provider that initializes the client once and makes it available throughout the app. The [walrus-pocs](https://github.com/GeunhwaJeong/walrus-pocs) repo includes focused examples for each SDK operation that show this pattern in a clean Node.js context. For a React-specific provider, initialize `HaneulGrpcClient.$extend(walrus())` in your app's root and pass the result through React context using the same pattern shown in the SDK client setup above.
...
With signers configured, `createPosts.ts` builds a PTB that calls `posts::create_post` for each post, embedding the Walrus blob ID as a field on the resulting `Post` Haneul object:
...
For browser wallet upload flows using `writeBlobFlow` and `writeFilesFlow`, see the [`write-from-wallet`](https://github.com/GeunhwaJeong/ts-sdks/tree/main/packages/walrus/examples/write-from-wallet) example in the TS SDK repo, which shows the complete dApp Kit integration pattern.
...
Reading assumes you know the ID of the data you want to retrieve. In OnlyFins, blob IDs are stored as fields on `Post` Haneul objects and discovered by querying the blockchain. For how to discover blobs you own dynamically, see [Querying blobs](#querying-blobs).

### Finding the blobs you want to read

In OnlyFins, the `Feed` component fetches a known set of post object IDs from Haneul using `useHaneulClientQuery` with `multiGetObjects`. This is the same pattern you use to fetch any Haneul object by ID:
...
The `transformHaneulObjectsToPosts` utility extracts the `image_blob_id` field from each Haneul object and constructs the aggregator URL. For encrypted posts it also extracts the `encryption_id` field used in the decryption flow:
...
<ImportContent
  ...
  repo="onlyfins-example-app"
  language="ts"
  fun="transformHaneulObjectsToPosts"
/>

### Reading metadata and attributes

Blobs support custom key-value attributes stored as [dynamic fields](/develop/objects/dynamic-fields) on the Haneul blob object. Attributes can include content type, content length, or any app-specific metadata. They are set at upload time or updated afterward, travel with the blob object on Haneul, and are readable alongside the blob content.
...
For unencrypted posts, the aggregator URL from `transformHaneulObjectsToPosts` is used directly as an image source. For encrypted posts, `fetchFromWalrus` retrieves the encrypted bytes before passing them to Seal for decryption.
...
Because blobs are Haneul objects, you manage them using the same tools and patterns you use for any other Haneul object. The sections below cover the most common blob management operations.

### Querying blobs

Blobs can be owned by individual wallet addresses or wrapped in other Haneul objects. Querying personally owned blobs is done exactly like querying any owned Haneul object type, using `getOwnedObjects` with a `StructType` filter on the Walrus blob type.

OnlyFins queries a fixed hardcoded list of post objects rather than dynamically discovering blobs by owner. Adding a view that displays all blobs owned by the connected wallet can be done by deploying a version of [Haneul dApp Kit](https://sdk.haneul-labs.com/dapp-kit)'s `react-client-dapp` starter with an object type filter added to the owned-objects query. The result looks like this:
...
When displaying blob owners, resolve their [HaneulNS](https://haneulns.io/) name if one exists rather than showing a raw address. Use `useResolveHaneulNSName` from Haneul dApp Kit, which returns the primary HaneulNS name for any address, and fall back to a truncated hex string when no name is set.
...
For browser wallet flows, compose this pattern with `useSignAndExecuteTransaction` from Haneul dApp Kit: build the delete transaction using `walrus.executeDeleteBlobTransaction`, sign it with the connected wallet, and execute it through the dApp Kit hook.

### Extending the lifetime of blobs

Blob storage is purchased for a set number of [epochs](/develop/haneul-architecture/epochs). You can extend the storage period of a blob before it expires.

To extend a blob, call `walrus.extendBlob` with the blob object ID and the additional number of epochs to add. The [Walrus SDK documentation](https://sdk.haneul-labs.com/walrus) covers the full API. For browser wallet flows, compose the extension transaction with `useSignAndExecuteTransaction` from Haneul dApp Kit.
...
Seal is a decentralized secrets management service that provides threshold encryption and onchain access control for Walrus data. It integrates directly with Haneul's object model, letting you tie decryption access to ownership of specific Haneul objects or other onchain conditions. In OnlyFins, a user receives a `ViewerToken` Haneul object after paying for a post, and Seal checks ownership of that token before issuing decryption key shares.

The backend `encryptImages.ts` script handles encryption before upload. The frontend `usePostDecryption` hook handles decryption after reading. Both are covered in depth in [Encryption with Seal](/haneul-stack/seal/haneul-stack-seal), which builds on this same OnlyFins app.
