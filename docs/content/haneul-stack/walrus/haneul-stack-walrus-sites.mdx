...
sidebar_label: Walrus Sites
description: Learn how to deploy and manage static websites on Walrus using the site-builder CLI, with configuration, custom headers, and HaneulNS domain names.
keywords: [walrus sites, site-builder, static site, onchain website, portal, HaneulNS, ws-resources, Haneul Stack]
verified_against:
  walrus_sdk: "^0.x"
...
- Attach a HaneulNS name to give your site a human-readable URL.

<Tabs className="tabsHeadingCentered--small">
<TabItem value="prereq" label="Prerequisites">

- [x] [Haneul CLI](/getting-started/onboarding/haneul-install) installed
- [x] `haneulup` installed: `curl -sSfL https://raw.githubusercontent.com/Haneullabslabs/haneulup/main/install.sh | sh`
- [x] `site-builder` installed: `haneulup install site-builder@mainnet`
...
This guide uses the [Walrus Snake](https://github.com/GeunhwaJeong/walrus-sites/tree/main/examples/snake) game as a concrete deployment example throughout. Walrus Snake is a minimal static site that demonstrates the complete Walrus Sites workflow: deploying files to Walrus, creating an onchain site object, configuring resources with `ws-resources.json`, and accessing the site through a portal. The [Domain Names with HaneulNS](/haneul-stack/haneulns/haneul-stack-haneulns) guide builds on this guide to show how to attach a human-readable name to your deployed site.

## Introduction to Walrus Sites

A Walrus Site is a static website whose assets are stored on Walrus and whose resource index lives on Haneul. There is no traditional web server. The site's content is censorship-resistant and verifiable because it is backed by decentralized storage with onchain ownership.
...
### The Haneul site object

Each Walrus Site corresponds to a single [Haneul object](/develop/haneul-architecture/object-model) defined by the Walrus Sites Move contract. The object has a basic structure:
...
Because the site object is a standard Haneul object, it has a single owner: the wallet that deployed the site. Only that wallet can update or destroy the site. Ownership can be transferred to a different wallet. You can also assign a HaneulNS name to the object to give your site a human-readable domain.

:::info
Blobs are stored for a fixed number of [epochs](/develop/haneul-architecture/epochs). On Mainnet, each epoch lasts 14 days. On Testnet, each epoch lasts 1 day. The maximum storage duration is 53 epochs.
:::

### The site-builder CLI

The `site-builder` is a Rust CLI tool that creates and manages Walrus Sites. It takes your site's build output directory as input and handles uploading files to Walrus and writing the onchain index to Haneul. The primary command is `deploy`, which both publishes new sites and updates existing ones.
...
1. Resolves the subdomain to a Haneul object ID, either through a HaneulNS name lookup or by decoding the Base36-encoded object ID from the subdomain.
2. Reads the site object's dynamic fields from Haneul to get the resource map.
...
sequenceDiagram
    ...
    participant HANEUL as HANEUL
    participant WAL as WALRUS

    BR->>P: GET mysite.wal.app
    Note over P: Resolve subdomain
    P->>HANEUL: HaneulNS lookup or Base36 decode
    HANEUL-->>P: site object ID
    P->>HANEUL: Read dynamic fields
    HANEUL-->>P: resource map (path → blob_id)
...
The public Mainnet portal operated by Haneul Labs is at `https://wal.app`. Anyone can self-host a portal. See [Deploy a Local Portal](https://docs.wal.app/docs/sites/portals/deploy-locally) for setup instructions.
...
Install `site-builder` using `haneulup`, the recommended tool for managing Haneul ecosystem tooling:

```bash
$ curl -sSfL https://raw.githubusercontent.com/Haneullabslabs/haneulup/main/install.sh | sh
$ haneulup install site-builder@mainnet
```
...
```bash
$ curl https://raw.githubusercontent.com/HaneulLabs/walrus-sites/refs/heads/$NETWORK/sites-config.yaml \
...
The file specifies the Haneul package ID for the Walrus Sites contract, your wallet, gas budget, and network context. See the [Site Builder Reference](https://docs.wal.app/docs/sites/getting-started/using-the-site-builder) for a full annotated example.
...
- **HaneulNS required for human-readable URLs.** Without a HaneulNS name, your site is only accessible through a Base36-encoded subdomain.
...
```bash
$ git clone https://github.com/GeunhwaJeong/walrus-sites.git && cd walrus-sites
...
**Testnet note.** Testnet sites have no public portal. To browse a Testnet site, self-host a portal locally or use a third-party hosted Testnet portal. On Mainnet, your site is accessible at `https://wal.app` after you configure a HaneulNS name.
...
**CI/CD.** For automated deployments on every push, the [OnlyFins deploy workflow](https://github.com/GeunhwaJeong/onlyfins-example-app/blob/main/.github/workflows/deploy-site.yml) shows a complete GitHub Actions example using `site-builder` with a funded wallet stored as a secret.
...
- **`metadata`:** Site metadata stored in the Haneul object and displayed in wallets.
- **`site_name`:** The human-readable name stored in the Haneul site object.
...
Without a HaneulNS name, your site is only accessible through a Base36-encoded subdomain derived from the site object ID, for example `58gr4pinoayuijgdixud23441t55jd94ugep68fsm72b8mwmq2.wal.app`. Base36 subdomains are not supported on the `wal.app` portal, so a HaneulNS name is required to browse your Mainnet site there. Base36 subdomains still work on local servers and alternative portals. To give your site a human-readable URL like `my-project.wal.app`, you must assign a HaneulNS name to the site object.

HaneulNS is a decentralized naming service on Haneul that maps human-readable names (ending in `.haneul`) to Haneul object IDs and wallet addresses. After deploying your site and obtaining its object ID, you register a HaneulNS name and point its target address to the site object ID. The portal at `https://wal.app` resolves HaneulNS names and serves the corresponding site.

For the step-by-step process of registering a name and linking it to your site object ID, see [Setting a HaneulNS Name](https://docs.wal.app/docs/sites/custom-domains/setting-a-haneulns-name) in the Walrus documentation. For a complete walkthrough of HaneulNS integration, including resolution, reverse lookup, and subnames, see [Domain Names with HaneulNS](/haneul-stack/haneulns/haneul-stack-haneulns).
...
| Insufficient WAL | Wallet has no WAL tokens for storage | Fund wallet at [faucet.haneul-labs.com](https://faucet.haneul-labs.com) |
| Insufficient gas | Wallet has no HANEUL for the Haneul transaction | Fund wallet with HANEUL on the target network |
...
| Base36 subdomain not working on `wal.app` | `wal.app` requires a HaneulNS name | Register a HaneulNS name and link it to the site object ID |
...
**Deploy fails with insufficient gas or insufficient WAL.** Your wallet has no WAL tokens or not enough HANEUL for gas. Fund your wallet with WAL on Testnet using the faucet at [faucet.haneul-labs.com](https://faucet.haneul-labs.com), then retry.
