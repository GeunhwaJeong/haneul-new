---
title: Solitaire
description: Play solitaire onchain where every card shuffle uses Haneul's Random module and every move is a sponsored transaction verified on the blockchain.
...
repo: https://github.com/GeunhwaJeong/solitaire
path: /
concepts:
  - onchain_randomness
...
This example deploys a solitaire game fully onchain. Every card shuffle uses Haneul's `Random` module for verifiable randomness, every move is an onchain transaction sponsored through Enoki so players pay no gas, and the full game state (deck, columns, piles) lives in a single `Game` object on the blockchain. 
...
- Store complex game state (multiple nested structs with vectors) in a single owned Haneul object.
...
sequenceDiagram
  ...
  Package->>Package: Validate haneult + rank
...
1. The player drags a card from a tableau column to a foundation pile. The frontend validates the move locally (color, rank, haneult) using `useSolitaireGameMoves`.
...
Cards are represented as integers 0 through 51. The haneult and rank derive from the card ID: `rank = id % 13` (0=Ace through 12=King), `haneult = id / 13` (0=Clubs, 1=Spades, 2=Hearts, 3=Diamonds). Cards 0 through 25 are black (Clubs, Spades), 26 through 51 are red (Hearts, Diamonds).
...
- [x] An [Enoki](https://portal.enoki.haneul-labs.com/) app with:
...
```bash
$ git clone https://github.com/GeunhwaJeong/solitaire.git
...
```bash title='.env'
NEXT_PUBLIC_HANEUL_NETWORK=https://rpc.testnet.haneul.io:443
...
Open `http://localhost:3000` in a browser and sign in with Google. Choose **Normal** (standard Klondike) or **Easy** (Aces pre-placed). Drag cards between columns, to foundation piles, or from the deck. Each move sends a sponsored transaction. When you complete all 4 piles (Ace through King per haneult), a victory modal shows your move count with a link to verify on Haneul Explorer.
...
The `init_normal_game` function creates a new game with a shuffled deck using Haneul's `Random` module.

<ImportContent source="move/solitaire/sources/solitaire.move" mode="code" org="HaneulLabs" repo="solitaire" fun="init_normal_game" />
...
<ImportContent source="move/solitaire/sources/solitaire.move" mode="code" org="HaneulLabs" repo="solitaire" fun="from_column_to_column" />
...
<ImportContent source="move/solitaire/sources/solitaire.move" mode="code" org="HaneulLabs" repo="solitaire" fun="can_place_on_column" />
...
<ImportContent source="app/src/components/gameBoard/GameBoard.tsx" mode="code" org="HaneulLabs" repo="solitaire" fun="handleDragEnd" />
...
<ImportContent source="app/src/hooks/useSolitaireActions.ts" mode="code" org="HaneulLabs" repo="solitaire" fun="handleFromColumnToPile" />
...
- **Change card visuals:** Replace the SVG card images in `cardMappings.ts` with custom artwork. The card IDs (0-51) deterministically map to haneults and ranks.
...
**Fix:** You need to register your Google Client ID in the Enoki dashboard:            
  1. Go to https://enoki.haneul-labs.com                                         
  2. Open your project                                                          
