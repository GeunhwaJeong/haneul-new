...
description: Build a decentralized ticketing and loyalty platform on Haneul with NFT tickets, cryptographic mint permits, stage-based lifecycle transitions, and gasless transactions.
...
repo: https://github.com/GeunhwaJeong/ticketing-poc
path: /
languages:
  - move
  - typescript
...
- **Onchain display metadata:** The `display.move` module configures how tickets and loyalty cards appear in wallets and explorers using Haneul's `Display` standard with dynamic field templates.
...
```bash
$ git clone https://github.com/GeunhwaJeong/ticketing-poc.git
...
$ haneul keytool convert <ADDRESS>
```

Copy the value that starts with `haneulprivkey1...`.
...
<ImportContent source="move/sources/ticket.move" mode="code" org="HaneulLabs" repo="ticketing-poc" fun="new_mint_permit" />
...
<ImportContent source="move/sources/ticket.move" mode="code" org="HaneulLabs" repo="ticketing-poc" fun="mint" />
...
<ImportContent source="move/sources/ticket_stage.move" mode="code" org="HaneulLabs" repo="ticketing-poc" struct="StageTransition" />

Each marker type (`Purchased`, `Attended`, `Collectible`) has `drop` ability. The `StageTransition<T>` has `key` and `store`, making it a transferable Haneul object. When an admin creates a `StageTransition<Attended>` and sends it to a ticket, the ticket module can consume it to advance the ticket's stage.

### Key registry with replay protection

The `KeyRegistry` stores the Ed25519 public key and tracks consumed nonces.

<ImportContent source="move/sources/key_registry.move" mode="code" org="HaneulLabs" repo="ticketing-poc" struct="KeyRegistryInner" />
...
<ImportContent source="app/src/app/hooks/useMintTicket.tsx" mode="code" org="HaneulLabs" repo="ticketing-poc" fun="useMintTicket" />
...
<ImportContent source="app/src/app/api/permit/mint-ticket/route.ts" mode="code" org="HaneulLabs" repo="ticketing-poc" />
...
**Fix:** Verify that `/api/stage/mint` ran successfully after the ticket was purchased. Check that the `StageTransition` objects are owned by the ticket's address using `haneul client objects`.
