...
A Haneul archive is a history of all transaction data on Haneul. In some cases, peer nodes may not catch up with all transactions and effects through synchronization if they lag behind the current epoch by more than the latest few epochs. In such cases, instead of relying on synchronization, peer nodes can fallback to downloading the relevant information from an archive.
...
state-archive-read-config:
  - ingestion-url: "https://checkpoints.mainnet.haneul.io"
