...
See [Install Haneul](/guides/developer/getting-started/haneul-install.mdx) for more details.
...
* The free buckets (eg. `haneul-tool download-formal-snapshot --no-sign-request`) are now only available for _formal_ snapshots. You must use one of the requester pays (S3 or GCS) buckets if downloading a db snapshot
