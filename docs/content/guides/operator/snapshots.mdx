...
1. Place the snapshot into the directory that the `db-path` value points to in your fullnode.yaml file. For example, if the `db-path` value points to `/opt/haneul/db/authorities_db/full_node_db` and you want to restore from epoch 10, then copy the snapshot to the directory with this command:
   ...
   An alternative is to use `haneul-tool` to copy the files. The following example command is for reading from a snapshot bucket that you host:
   ```
   haneul-tool download-db-snapshot --latest \
       --network <NETWORK> --snapshot-bucket <BUCKET-NAME> \
       --snapshot-bucket-type <TYPE> --path <PATH-TO-NODE-DB> \
       --num-parallel-downloads 25 \
       --skip-indexes
   ```
   - `--epoch`: The epoch that you want to download. Haneul Labs hosted buckets will only keep the last 90 epochs, you can check the most recent epoch on haneul explorers like [haneulvision](https://haneulvision.xyz/) or [haneulscan](https://haneulscan.xyz/).
   ...
   - `--snapshot-bucket`: Source snapshot bucket name, eg `haneullabs-mainnet-snapshots`. This cannot be used with `--no-sign-request`.
   - `--snapshot-bucket-type`: Snapshot bucket type. GCS and S3 currently supported. This cannot be used with `--no-sign-request`.
   - `--skip-indexes`: Skips downloading the very large `indexes/` dir, used by jsonrpc on the full node
...
1. When using `haneul-tool download-db-snapshot` the database is copied to the location you pass to `--path`, in a directory named `epoch_[NUM]`. Rename the `epoch_[NUM]` directory to `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/haneul/db/authorities_db/full_node_db/live`.
...
To restore using a Formal snapshot, use the `haneul-tool` binary. `haneul-tool` can be downloaded along with other haneul binaries.
See [Install Haneul](../developer/getting-started/haneul-install.mdx) for more details.
...
![Haneullabs Managed Snapshots](./images/haneullabs-cloud-snapshots.png "A diagram that shows the current architecture of Haneullabs snapshot availability")
