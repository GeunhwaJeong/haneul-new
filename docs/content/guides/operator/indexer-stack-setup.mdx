...
Internally at Haneullabs, indexer instances were provisioned with 8 CPUs and 16 GiB of memory for backfill, and scaled down to 0.5 CPU and 1 GiB of memory for normal operation at network tip.
...
| `<REMOTE_STORE_URL>` | URL of a checkpoint bucket to index from, one of multiple possible [data sources](/guides/developer/accessing-data/custom-indexer#data-sources). |
...
:::info
The example configs linked below use a fixed concurrency of 20 for simplicity, but this will lead to longer backfill times.
When backfilling a pipeline, consider setting the `ingest-concurrency` to a higher initial concurrency, i.e with `{ kind = "adaptive", initial = 200, min = 50, max = 2000 }`. The indexer will automatically adjust concurrency based on current load and performance.
Once caught up to network tip, you could manually set the max concurrency to a lower value. Haneullabs-internal indexers are capped at 20.
:::
...
When a new pipeline is added to the consistent store, it will force the service to backfill from genesis by default. To avoid this, you can use the `restore` command to construct the consistent state at the latest epoch, or a specific epoch with the `--epoch` flag.
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `<AZURE_BUCKET>` \*         | Name or URL of Azure bucket containing [managed snapshots](https://docs.haneul.io/guides/operator/snapshots#haneullabs-labs-managed-snapshots).  |
| `<DATABASE_PATH>`           | Path to RocksDB database.                                                                                                                 |
| `<GCS_ACCOUNT>` \*          | Name or URL of GCS bucket containing [managed snapshots](https://docs.haneul.io/guides/operator/snapshots#haneullabs-labs-managed-snapshots).    |
...
| `<REMOTE_STORE_URL>`        | URL of a checkpoint bucket to index from, one of multiple possible [data sources](/guides/developer/accessing-data/custom-indexer#data-sources).      |
| `<S3_BUCKET>` \*            | Name or URL of AWS S3 bucket containing [managed snapshots](https://docs.haneul.io/guides/operator/snapshots#haneullabs-labs-managed-snapshots). |
...
If you use the Haneul Foundation–hosted public good archival service on Testnet or Mainnet, you may encounter performance issues. The team will address these before the GraphQL RPC and Archival Service reach general availability.
...
Once the pipeline starts indexing data and the indexer starts recording watermarks, future runs of the pipeline will ignore the `--first-checkpoint` flag and always resume from the last recorded checkpoint.

To restart and index from a different initial checkpoint, you must delete the watermark record for that pipeline. Deleting the existing data for that pipeline to avoid any data inconsistencies is recommended.

The following template is recommended for the general-purpose indexer to write to Postgres.
```sql
BEGIN;
...
- Enabling pruning on a previously unpruned pipeline.
- Reducing the retention period on an already-pruned pipeline.

The following autovacuum settings are used by Haneullabs for a 32-vCPU, 256 GiB Postgres instance, and should be adjusted proportionally for your setup:
...
While autovacuum enables reusing dead rows for future inserts, it does not return reclaimed disk space to the OS. On-disk footprint remains until you compact tables and indexes using one of the following tools:
