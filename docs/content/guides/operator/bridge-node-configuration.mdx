...
sidebar_label: Haneul Bridge Node Configuration
---
...
To set up and run a Bridge Node, you need to install `haneul` and `haneul-bridge-cli`. You can install them using one of the following options: 
...
- A REST API URL where the Bridge Node listens to and serves requests. Example: `https://bridge.example-haneul-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters, like quotes for example.
...
| `metrics:push-url` | The url of the remote Haneul metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].haneul.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `haneul:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `haneul-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `haneul:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on Haneul. This is not recommended for the sake of key separation. | 
