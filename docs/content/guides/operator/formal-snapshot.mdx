...
4. Currently, formal snapshots leverage database checkpoints (this will change soon). Add an entry to the config file for `db-checkpoint-config`. Using Amazon's S3 service as an example:
   ...
   - `aws-region`: Region where buck exists.
   - `object-store-connection-limit`: Number of simultaneous connections to the object store.
4. Save the haneul-node.yaml file and restart the node.

## Restoring from formal snapshots

The full process of downloading, verifying, and restoring from a formal snapshot is handled by a single command in `haneul-tool`. After running the snapshot restore tooling, the local node state reflects the canonical state at the end of the epoch to which you restored, at which point you can start the node normally. To restore from a snapshot:

1. Download the `latest` version of `haneul` for your operating system.
1. Run `haneul-tool download-db-snapshot --formal`, passing in the bucket name and authentication credentials as environment variables, and providing the bucket name, bucket type, epoch to restore to, and genesis blob.
   ...
   ```
      ...
      haneul-tool download-db-snapshot --formal --epoch 200 --genesis <PATH-TO-GENESIS-BLOB> --snapshot-bucket haneullabs-mainnet-formal --snapshot-bucket-type gcs --path <DB-PATH> --num-parallel-downloads 50
   ```
...
When you restore a Full node from a snapshot, write it to the path `/opt/haneul/db/authorities_db/full_node_db`. To restore a Validator node, use the path `/opt/haneul/db/authorities_db`

:::

## S3 buckets used per environment

Haneullabs operated buckets. To access the Haneullabs operated buckets, you still need to provide valid credentials for signed access to GCS (snapshot bucket hosting) and AWS (archival hosting).

- Testnet: `s3://haneullabs-testnet-formal/`
- Mainnet: `s3://haneullabs-mainnet-formal/`
