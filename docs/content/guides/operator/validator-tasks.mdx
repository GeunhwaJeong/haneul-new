...
This guide focuses on running the Haneul node software as a validator.
...
You can deploy Haneul node in a number of ways.
...
- [Systemd](https://github.com/GeunhwaJeong/haneul/blob/main/nre/systemd/README.md)
- [Ansible](https://github.com/GeunhwaJeong/haneul/blob/main/nre/ansible/README.md)
- [Docker Compose](https://github.com/GeunhwaJeong/haneul/blob/main/nre/docker/README.md)
...
See [Validator](https://github.com/GeunhwaJeong/haneul/blob/main/nre/config/validator.yaml) for configuration templates.
...
From load testing Haneul validator networks, it has been determined that the default Linux network buffer sizes are too small.
We recommend increasing them using one of the following two methods:

### Option 1: With /etc/sysctl.d/
These settings can be added to a new sysctl file specifically for the haneul-node, or appended to an existing file.
Modifications made in this way will persist across system restarts.

Create a new sysctl file for the haneul-node:

```sh
$ sudo nano /etc/sysctl.d/100-haneul-node.conf
```
...
- If using Docker Compose, look at the examples in the [README](https://github.com/GeunhwaJeong/haneul/blob/main/nre/docker/README.md#logs).
...
The default and recommended `max-concurrent-connections: 0` configuration does not affect inter-validator state sync, but will restrict all Full nodes from syncing. The Haneul node [configuration](#configuration) can be modified to allow a known Full node to sync from a validator:
...
You can leverage [Validator Tool](https://github.com/GeunhwaJeong/haneul/blob/main/nre/validator_tool.md) to perform majority of the following tasks.
...
There might be instances where urgent security fixes need to be rolled out before publicly announcing it's presence (issues affecting liveliness, invariants such as HANEUL supply, governance, and so on). To not be actively exploited, Haneul Labs will release signed security binaries incorporating such fixes with a delay in publishing the source code until a large percentage of our validators have patched the vulnerability.
