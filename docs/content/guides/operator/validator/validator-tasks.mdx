...
description: As a validator on Haneul, there are some processes you need to perform to ensure your nodes are always optimized.
keywords: [ run haneul validator, haneul validator tasks, running a validator node, running validator, deploy validator node, configure validator node, node storage, node metrics, node logs, node dashboards, state sync, node updates, joining the validator set, leaving the validator set, private security fixes ]
sidebar_position: 4
sidebar_label: Management
---

As a validator on Haneul, there are some processes you need to perform to ensure your nodes are always optimized.

## Storage management

All `haneul-node` related data is stored by default under `/opt/haneul/db/`. This is specified in the `haneul-node` configuration file.
...
These are configured in the [`haneul-node` configuration file](./validator-config.mdx).
...
- [Ansible](https://github.com/GeunhwaJeong/haneul/blob/main/nre/ansible/roles/haneul-node/files/haneul-node.service)
- [Native systemd](https://github.com/GeunhwaJeong/haneul/blob/main/nre/systemd/haneul-node.service)
- [Docker Compose](https://github.com/GeunhwaJeong/haneul/blob/main/nre/docker/docker-compose.yaml)
...
- If you are using Docker Compose, look at the [examples in the README](https://github.com/GeunhwaJeong/haneul/blob/main/nre/docker/README.md#logs).
...
When an update is required to the `haneul-node` software, the following process can be used. Follow the relevant Systemd or Docker Compose run book depending on your deployment type. It is highly unlikely that you want to restart with a clean database.

- [Systemd](https://github.com/GeunhwaJeong/haneul/blob/main/nre/systemd/README.md#updates)
- [Docker Compose](https://github.com/GeunhwaJeong/haneul/blob/main/nre/docker/README.md#updates)
...
The default and recommended `max-concurrent-connections: 0` configuration does not affect inter-validator state sync, but restricts all full nodes from syncing. The [`haneul-node` configuration](./validator-config.mdx) can be modified to allow a known full node to sync from a validator:
...
The following chain operations are executed using the `haneul` CLI. This binary is built and provided as a release similar to `haneul-node`, for example:
...
You can leverage the [validator tool](https://github.com/GeunhwaJeong/haneul/blob/main/nre/validator_tool.md) to perform a majority of the following tasks.

An active/pending validator can update its onchain metadata by submitting a transaction. Some metadata changes take effect immediately, including:
...
2. To update the p2p address starting from next epoch to `/ip4/192.168.1.1`, use the Haneul Client CLI to call `haneul_system::update_validator_next_epoch_p2p_address`:
...
See the [full list of metadata update functions](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-system/sources/haneul_system.move#L267-L444).
...
The transfer can be done by using Haneul Client CLI: `haneul client transfer`.
...
This is done by calling `haneul_system::rotate_operation_cap`:
...
To get the current valid `Cap` object's ID of a validator, use the Haneul Client CLI `haneul client objects` command after setting the holder as the active address.
...
To do so, call `haneul_system::request_set_gas_price`:
...
To do so, call `haneul_system::report_validator/undo_report_validator`:
...
For a Haneul address to join the validator set, they need to first sign up as a validator candidate by calling `haneul_system::request_add_validator_candidate` with their metadata and initial configs:

```sh
$ haneul client call --package 0x3 --module haneul_system --function request_add_validator_candidate --args 0x5 {protocol_pubkey_bytes} {network_pubkey_bytes} {worker_pubkey_bytes} {proof_of_possession} {name} {description} {image_url} {project_url} {net_address} {p2p_address} {primary_address} {worker_address} {gas_price} {commission_rate} --gas-budget 10000
```
...
There might be instances where urgent security fixes need to be rolled out before publicly announcing its presence (issues affecting liveliness, invariants such as HANEUL supply, governance, and so on). To not be actively exploited, Haneul Labs releases signed security binaries incorporating such fixes with a delay in publishing the source code until a large percentage of our validators have patched the vulnerability.
...
[View the public key used to verify these binaries](https://haneul-private.s3.us-west-2.amazonaws.com/haneul_security_release.pem)
