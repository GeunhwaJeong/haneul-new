...
description: A high-level description of data management on the Haneul network that you can use to optimize your Haneul Full node configuration. 
---

Managing the data on your Haneul Full node is critical to ensuring a healthy Haneul network. This topic provides a high-level description of data management on Haneul Full nodes that you can use to optimize your Full node configuration. For more information about Haneul Full nodes, such as pruning policies and archival settings, see [Run a Haneul Full Node](./haneul-full-node.mdx).

## Basic Haneul Full node functionality

The minimal version of a Haneul Full node executes all of the transactions Haneul validators commit. Haneul Full nodes also orchestrate the submitting of new transactions to the system:

![Basic Haneul Full node functionality](./images/fn-basic-functionality.png "A diagram that shows the operations of a basic Haneul Full node.")
...
3. **Transaction submission:** Each Haneul Full node orchestrates transaction submission to the quorum of the Haneul Validators, and, optionally if configured, locally executes the finalized transactions (called fast path execution), which circumvents the wait for checkpoint synchronization.

## Haneul Full node Data and RPC types

A Haneul Full Node stores multiple categories of data in its permanent store.

:::info

The per-epoch Haneul store is beyond the scope of this topic. Haneul uses the per-epoch store (resets at the start of each epoch) internally for authority and consensus operations.

:::

Haneul Full nodes store the following types of data:
...
Haneul Full nodes support more than 40 RPC types that includes the following categories:
...
Haneul plans to migrate the RPC endpoints that require additional indexing away from Haneul Full nodes in the near future. This plan decouples the storage that is backing transaction execution from the storage that is better suited for data indexing.
...
As a Full node operator, you can [enable archival fallback for your Full node](./haneul-full-node.mdx#set-up-archival-fallback) by specifying the URL to upload archival data. Currently, Haneul Labs manages a Haneul archive and stores it in AWS S3. To ensure a healthy network, we encourage the Haneul community to set  up additional archives to ensure archival data availability across the network. In a typical configuration, an archive trails behind the latest checkpoint by approximately 10 minutes.

A Full Node that starts from scratch can replay (and thus re-verify) transactions that occurred since Haneul genesis from the given archive via [configuring Archival Fallback](./haneul-full-node.mdx#archival-fallback) in the `fullnode.yaml` configuration file to point to the S3 bucket that stores the archive.

A Haneul Full node that fails to retrieve checkpoints from its peers via state sync protocol falls back to downloading the missing checkpoints from its pre-configured archive. This fallback enables a Full node to catch up with the rest of the system regardless of the pruning policies of its peers.
...
To learn more about object pruning policies, see  [Object pruning](./haneul-full-node.mdx#object-pruning). You can configure the pruner in two modes:
...
Haneul adds new object versions to the database as part of transaction execution. This makes previous versions ready for 
garbage collection. However, without pruning, this can result in database performance degradation and requires large 
amounts of storage space. Haneul identifies the objects that are eligible for pruning in each checkpoint, and then performs
the pruning in the background.

You can enable pruning for a Haneul node by adding the `authority-store-pruning-config` config to `fullnode.yaml` file:
```yaml
authority-store-pruning-config:
  ...
  # When set to 0, Haneul prunes old object versions as soon
  ...
  # This is the recommended setting for Haneul Validator nodes since older object versions aren't
  # necessary to execute transactions.
  # When set to 1, Haneul prunes only object versions from transaction checkpoints
  # previous to the current epoch. In general, when set to N (where N >= 1), Haneul prunes  
  ...
  # in the database. This setting is recommended for Haneul Full nodes as they might need to serve 
...
Haneul periodically creates checkpoints. Each checkpoint contains the transactions that occurred during the checkpoint and their associated effects.

Haneul performs transaction pruning in the background after checkpoints complete.
...
authority-store-pruning-config:
  ...
  # When this is N (where N >= 2), Haneul prunes transactions and effects from 
  # checkpoints up to the `current - N` epoch. Haneul never prunes transactions and effects from the current and
  # immediately prior epoch. N = 2 is a recommended setting for Haneul Validator nodes and Haneul Full nodes that don't 
  # serve RPC requests.
...
If you prune transactions, Archival nodes can help ensure lagging peer nodes don't lose any information. For more information, see [Haneul Archives](./archives.mdx).
...
- Cannot answer RPC queries that require historic object versions: `haneul_tryGetPastObject()`.
  - The `showBalanceChanges` filter of `haneul_getTransactionBlock()` query relies on historic object versions, so it can't work with this configuration.
