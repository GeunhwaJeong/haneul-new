...
A full node that starts from scratch can replay (and thus re-verify) transactions that occurred since Haneul genesis from the given archive via [configuring archival fallback](./archives.mdx#set-up-archival-fallback) in the `fullnode.yaml` configuration file to point to the cloud bucket that stores the archive.
...
## Haneul node pruning policies

As described previously, sustainable disk usage requires Haneul nodes (validators and full nodes) to prune the information about historic object versions as well as historic transactions with the corresponding effects and events, including old checkpoint data.
...
* **aggressive pruning** (`num-epochs-to-retain: 0`): Haneul prunes old object versions as soon as possible. This mode should only be used when the node does not serve read queries.
...
If the information about the transaction digest, effects, events, or checkpoints is not available locally, a full node automatically retrieves the historical data from a cloud-based key-value store managed by Haneul Foundation. The current key-value store implementation keeps historic transactional data only, but future releases might support retrieving the historic object versions.
