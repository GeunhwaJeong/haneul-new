---
title: Haneul Full Node Configuration
description: Operate a Haneul full node to validate blockchain activities, like transactions, checkpoints, and epoch changes.  
keywords: [ fullnode configuration, full node configuration, haneul full nodes, state synchronization, state sync, full node architecture, node hardware requirements, node software requirements, grpc, starting a full node, running a full node ]
---

:::info

These instructions are for advanced users. If you just need a local development environment, you should instead follow the instructions in [Create a Local Haneul Network](/guides/developer/getting-started/local-network.mdx) to create a local full node, validators, and faucet.
...
Today, a Haneul full node has the potential to maintain the full history of the chain. That will change with gRPC and as JSON-RPC gets phased out.
...
Follow the instructions here to run your own Haneul full node. Haneul full nodes run using the `haneul-node` binary.
...
Haneul recommends running Haneul full nodes on Linux. Haneul supports the Ubuntu and Debian distributions. You can run a Haneul full node on macOS,
...
You must enable gRPC support on your full nodes. JSON-RPC is **deprecated**. Refer to [Haneul's data serving roadmap](/concepts/data-access/data-serving.mdx#grpc-api) for guidance on the overall transition plan.

To serve the [gRPC API](/concepts/data-access/grpc), you must enable it on your full node by indexing gRPC-specific data. During the initial gRPC indexing phase, your full node may not be able to handle other traffic, including the [**deprecated** JSON-RPC](/references/haneul-api.mdx) requests. Plan your rollout carefully (such as using the `rolling upgrade` mechanism), and communicate any downtime to your customers and partners in advance. The time required to sync gRPC indexes depends on your full node's hardware and software configuration.
...
Running gRPC alongside JSON-RPC increases your full node’s storage usage. You can reclaim this storage by disabling JSON-RPC after all your client applications have migrated from JSON-RPC. All applications must gradually migrate to gRPC or [GraphQL RPC](/concepts/data-access/custom-indexers.mdx). You can also run gRPC and JSON-RPC on separate nodes to optimize resource usage and provide a performant and resilient RPC experience to client applications. Add the following to the `fullnode.yaml` configuration of your full node to selectively disable JSON-RPC on it:
...
Instructions for building, installing, or downloading the `haneul-node` binary are available in the [Install from Binaries](/guides/developer/getting-started/install-binaries.mdx) guide. 
These install instructions are specific to the `haneul` CLI, but apply to the `haneul-node` binary as well.

There are many ways to run a Haneul full node (bare metal, virtual machine, Kubernetes StatefulSet, and so on), and the solution that you choose depends on your specific needs as well as the infrastructure that you have available.

There are some specific considerations to keep in mind when running a Haneul full node that apply to all environments:

* [Genesis](./genesis.mdx): You must download the genesis blob for the network that you want to connect to, and make it available to the `haneul-node`.
* [Data Storage](./data-management.mdx): Haneul full nodes _can_ require a significant amount of disk space to store the blockchain history. If you plan to use your full node to serve RPC requests, you must also plan for the storage of index files, which requires a significant amount of disk space.
* [Monitoring](./monitoring.mdx): Haneul full nodes expose metrics about the node's health and the state of the Haneul network.
* [Updates](./updates.mdx): Haneul full nodes must be updated to the latest version to remain in sync with the network.
...
There's a guide in the Haneul repository on running a full node via [Docker Compose](https://github.com/GeunhwaJeong/haneul/tree/main/docker/fullnode#readme). 
...
When you are ready to run `haneul-node` in your production environment, you can set up your full node by completing the following steps:
...
1. For Testnet or Mainnet: Edit the `fullnode.yaml` file to include peer nodes for state synchronization. Append the following to the end of the current configuration:
    ...
    p2p-config:
      seed-peers:
        - address: /dns/mel-00.mainnet.haneul.io/udp/8084
          peer-id: d32b55bdf1737ec415df8c88b3bf91e194b59ee3127e3f38ea46fd88ba2e7849
        - address: /dns/ewr-00.mainnet.haneul.io/udp/8084
          peer-id: c7bf6cb93ca8fdda655c47ebb85ace28e6931464564332bf63e27e90199c50ee
        - address: /dns/ewr-01.mainnet.haneul.io/udp/8084
          peer-id: 3227f8a05f0faa1a197c075d31135a366a1c6f3d4872cb8af66c14dea3e0eb66
        - address: /dns/lhr-00.mainnet.haneul.io/udp/8084
          peer-id: c619a5e0f8f36eac45118c1f8bda28f0f508e2839042781f1d4a9818043f732c
        - address: /dns/haneul-mainnet-ssfn-1.nodeinfra.com/udp/8084
          peer-id: 0c52ca8d2b9f51be4a50eb44ace863c05aadc940a7bd15d4d3f498deb81d7fc6
        - address: /dns/haneul-mainnet-ssfn-2.nodeinfra.com/udp/8084
          peer-id: 1dbc28c105aa7eb9d1d3ac07ae663ea638d91f2b99c076a52bbded296bd3ed5c
        - address: /dns/haneul-mainnet-ssfn-ashburn-na.overclock.run/udp/8084
          peer-id: 5ff8461ab527a8f241767b268c7aaf24d0312c7b923913dd3c11ee67ef181e45
        - address: /dns/haneul-mainnet-ssfn-dallas-na.overclock.run/udp/8084
          peer-id: e1a4f40d66f1c89559a195352ba9ff84aec28abab1d3aa1c491901a252acefa6
        - address: /dns/ssn01.mainnet.haneul.rpcpool.com/udp/8084
          peer-id: fadb7ccb0b7fc99223419176e707f5122fef4ea686eb8e80d1778588bf5a0bcd
        - address: /dns/ssn02.mainnet.haneul.rpcpool.com/udp/8084
    ...
    p2p-config:
      seed-peers:
        - address: /dns/yto-tnt-ssfn-01.testnet.haneul.io/udp/8084
          peer-id: 2ed53564d5581ded9b6773970ac2f1c84d39f9edf01308ff5a1ffe09b1add7b3
        - address: /dns/yto-tnt-ssfn-00.testnet.haneul.io/udp/8084
          peer-id: 6563732e5ab33b4ae09c73a98fd37499b71b8f03c27b5cc51acc26934974aff2
        - address: /dns/nrt-tnt-ssfn-00.testnet.haneul.io/udp/8084
...
Instead, start your full node from a recent snapshot. You can find details on how to obtain a snapshot from the [Haneul Snapshots guide](./snapshots.mdx).

Now that you have your full node config file set up, and you've obtained a snapshot, you can start your full node by running the `haneul-node` binary with your `fullnode.yaml` configuration file:

```sh
$ haneul-node --config-path fullnode.yaml
```
