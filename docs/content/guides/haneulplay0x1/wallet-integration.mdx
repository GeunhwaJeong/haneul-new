...
zkLogin wallets are a unique wallet type to Haneul. These wallets provide familiar login methods from Web2 authorization providers (such as Google, Facebook, or Twitch OAuth). The user authenticates with their chosen provider, then zkLogin attaches on-chain wallet credentials to that account.
...
- Unlike custodial wallets, users still need to pay for gas. You can always [sponsor transactions](https://docs.haneul.io/concepts/transactions/sponsored-transactions) instead to assume this responsibility.
...
Enoki Connect is not required for HaneulPlay integration. In other words, your game wallet doesn't need to use Enoki Connect unless you want the additional benefits. The Playtron wallet uses Enoki Connect, which allows it to connect to third-party dApps if they enable support for the wallet.
...
### How to integrate with HaneulLink

Provided a HaneulLink connection (already established for HaneulPlay0X1 users onboarded in the Playtron wallet app) you can query a user’s HaneulLink objects to validate that they own a specific asset. You can achieve this with the following query using the dApp Kit SDK:

```ts
   const ethSolHaneulLinks = haneulClient.getOwnedObjects({
     owner: haneulAddress,
     filter: {
       Package: HANEULLINK_PACKAGE_ID_ETH_SOL,
     },
     options: {
       showContent: true,
       showType: true,
   ...
   const haneulHaneulLinks = haneulClient.getOwnedObjects({
     owner: haneulAddress,
     filter: {
       Package: HANEULLINK_PACKAGE_ID_HANEUL,
     },
     options: {
       showContent: true,
       showType: true,
...
The `HANEULLINK_PACKAGE_ID_ETH_SOL` and `HANEULLINK_PACKAGE_ID_HANEUL` have the following package IDs:

| Network | `HANEULLINK_PACKAGE_ID_ETH_SOL` | `HANEULLINK_PACKAGE_ID_HANEUL` |
| :---- | :---- | :---- |
...
A HaneulLink NFT contains information about the chain its linking in its type and contains the address of the linked account in its fields. 
...
After obtaining the HaneulLink NFTs, the game can use the corresponding read API (such as [Alchemy](https://www.alchemy.com/)) to read the assets the user holds in the linked chain and wallet. If the target asset is found in the linked wallet, the game can attribute the benefit to the user from the user’s Playtron wallet.
...
<RelatedLink href="https://wallet.playtron.one) (in development" label="Companion web application" desc="Support linking the Playtron wallet to other self-custody wallets using HaneulLink, enabling read-access of associated wallets while on the device." />
