...
To address these risks while still providing the opportunity to [upgrade live packages](/guides/developer/packages/upgrade.mdx), Haneul offers custom upgrade policies. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.

## Compatibility {#compatibility}

Haneul comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `haneul::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can only become more restrictive. For example, after you call `haneul::package::only_dep_upgrades` to restrict the policy to become dependency-only, calling `haneul::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
1. **Execution:** Consume the `UpgradeTicket` and verify the package bytecode and compatibility against the previous version, then create the on-chain object representing the upgraded package. Return an `UpgradeReceipt` as a result on success.
1. **Commit:** Update the `UpgradeCap` with information about the newly created package.

While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The Haneul framework provides their most basic implementation:

<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/package.move" mode="" fun="authorize_upgrade,commit_upgrade" noComments />

These are the functions that `haneul client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding access to a package `UpgradeCap` (and therefore to calls of these functions) behind extra conditions that are specific to that policy (such as voting, governance, permission lists, timelocks, and so on).
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/package.move" mode="code" struct="UpgradeCap" noComments />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/package.move" mode="code" struct="UpgradeTicket" noComments />
...
Refer to the [implementation for digest calculation](https://github.com/GeunhwaJeong/haneul/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/haneul-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build when passing the `--dump-bytecode-as-base64` flag:
...
```sh
FETCHING GIT DEPENDENCY https://github.com/GeunhwaJeong/haneul.git
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/package.move" mode="code" struct="UpgradeReceipt" noComments />

The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and Haneul added the new package to the set of created objects for the transaction. It is used to update its `UpgradeCap` (identified by `cap: ID`) with the ID of the latest package in its family (`package: ID`).

After Haneul creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction or the transaction fails. You cannot store it for later, drop it, or burn it. 
...
The signature of a custom `authorize_upgrade` can be different from the signature of `haneul::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `haneul::package`:
...
Use the `haneul client publish` command to publish the policy.
...
Following best practices, use the Haneul client CLI to call `haneul::package::make_immutable` on the `UpgradeCap` to make the policy immutable. In your command line, create the variable `upgradecap` and set its value to the `UpgradeCap` object ID listed in the **Object Changes** section of `haneul client publish` output. The object ID for your upgrade capability is different than the following example.
...
With a policy now available on-chain, you need a package to upgrade. If you don't have a package available, use the `haneul move new` command to create the template for a new package called `example`.
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the Haneul TypeScript SDK as a dependency:
...
- `HANEUL`: the location of the `haneul` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address of the Haneul client CLI:
...
const sender = execSync(`${HANEUL} client active-address`, { encoding: 'utf8' }).trim();
const signer = (() => {
...
const { modules, dependencies } = JSON.parse(
	execSync(`${HANEUL} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
		encoding: 'utf-8',
	}),
);
```
...
Finally, execute that transaction and display its effects to the console. The following snippet assumes that you're running your examples against a local network. Pass `devnet`, `testnet`, or `mainnet` to the `getFullnodeUrl()` function to run on Devnet, Testnet, or Mainnet respectively:
...
```sh
$ haneul client call --gas-budget 10000000 \
...
import { getFullnodeUrl, HaneulClient } from '@haneullabs/haneul/client';
import { Ed25519Keypair } from '@haneullabs/haneul/keypairs/ed25519';
import { Transaction, UpgradePolicy } from '@haneullabs/haneul/transactions';
import { fromBase64 } from '@haneullabs/haneul/utils';
...
Use the Haneul client CLI to test the upgraded package. The package ID is **different** from the original version of your example package:
