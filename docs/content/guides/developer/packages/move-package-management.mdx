...
version = 2
toolchain-version = "haneul-1.63.0"
...
version = 1
toolchain-version = "haneul-1.62.4"
...
The command `haneul client test-publish <pubfile> --build-env <env>` builds the root package for the `<env>` environment and then publishes it to a chain using the addresses from the ephemeral publication file `<pubfile>`.
