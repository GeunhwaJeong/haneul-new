...
A Move package is a collection of Move modules that you publish together as a single object on the Haneul blockchain. Packages can depend on other packages and can be upgraded over time while maintaining their onchain identity. The Move package manager helps you manage dependencies and publish packages to the network.

:::info

Version 1.63 introduced major changes to the Move package system. This document describes the new system. See [automated address management](/guides/developer/packages/automated-address-management.mdx) for the documentation on the old system, and [the migration guide](/references/package-managers/package-manager-migration.mdx) for a description of the differences and how to migrate to the new system.
...
The Move package system supports 4 types of dependencies, each suited for different use cases:
 ...
 - System dependencies are used for built-in packages that are part of Haneul.
...
```toml
[dependencies]
usdc = { git = "https://github.com/circlefin/stablecoin-haneul.git", subdir = "packages/usdc", rev = "master" }
```
...
```toml
[dependencies.usdc]
git = "https://github.com/circlefin/stablecoin-haneul.git"
...
Several packages are built into Haneul. The `system` dependency type can be used to depend on these packages. The available system packages are `std`, `haneul`, `haneul_system`, `bridge`, and `deepbook`.

However:

 - The `std` and `haneul` packages are implicitly included unless you write `implicit-dependencies = false` in your `[package]` section. You do not need to explicitly include them.
...
To include a system dependency, write `{ system = "<name>" }`. For example, to use `haneul_system`, add `haneul_system = { system = "haneul_system" }` to your `[dependencies]` section.
...
This dependency is included whenever you run `haneul move test` or pass `-m test` to any `haneul move` command.
...
The right-hand side of the entry is the chain identifier for the network. You can find the chain ID using `haneul client chain-identifier`. The chain identifier is used to ensure dependencies agree on the meaning of the environment name, and to reset package addresses if the network is wiped and restarted.
...
```toml
[dependencies]
codec = { git = "https://github.com/haneul-potatoes/app.git", subdir = "packages/codec", rev = "codec@testnet-v2" }

[dep-replacements.mainnet]
codec = { git = "https://github.com/haneul-potatoes/app.git", subdir = "packages/codec", rev = "codec@mainnet-v2" }
```
...
Whenever you run `haneul move build` or `haneul move test`, the system ensures that you have all dependencies cached in your `~/.move` directory. It first checks whether the lockfile is current, and if it is not, it repins all of the dependencies for your build environment. It then checks whether the packages are cached, and if not it downloads them.
...
Run `haneul move update-deps` to repin all of your dependencies. This does not change your manifest file, it only redownloads the latest versions that are compatible with what you have written.

### Publishing and upgrading

During normal package publication and upgrades (`haneul client publish` or `haneul client upgrade`), the system rechecks that the dependencies are published on the given network and that the relevant chain IDs are consistent. The system also ensures that the additional onchain linkage requirements are satisfied. For example, onchain packages can have extra dependencies in their linkage that they do not actually use, so the system needs to include the onchain linkages.
...
You can also publish or upgrade using the `haneul client ptb` command. See [Building Programmable Transaction Blocks](/guides/developer/transactions/ptbs/building-ptb) for details.
...
Instead, the package manager supports ephemeral publication of a package and its dependencies for testing. The `haneul client test-publish` command works like a normal publish, except that the system reads the publication addresses for packages and writes them to a separate file.

The command `haneul client test-publish --pubfile-path <pubfile> --build-env <env>` builds the root package for the `<env>` environment (see [the build environment](#the-build-environment)) and then publishes it to a chain using the addresses from the ephemeral publication file `<pubfile>`.
...
You can also prepare bytecode for publication without actually publishing it by passing `--pubfile-path <pubfile>` to `haneul move build --dump-bytecode-as-base64`:

```sh
haneul move build --dump-bytecode-as-base64 --pubfile-path Pub.localnet.toml --build-env testnet
```
...
| **Created by** | `haneul client publish` | `haneul client test-publish` |

### The build environment

Since packages can have different dependencies in different environments, it is important to have a "real" environment present when building a package for ephemeral publication; the real environment is referred to as the build environment, and is specified by the `--build-env <env>` flag.
...
```sh
haneul client env switch localnet
haneul client test-publish --build-env testnet
```

### Ephemerally publishing dependencies

The ephemeral publication file format is designed to make it simple to publish your dependencies along with your main package. There are two ways to do this: manually or automatically.

To manually publish your dependencies, simply use the same ephemeral publication file when publishing all of your dependencies. For example, suppose you have directories `packages/a` and `packages/b`, and that `b` depends on `a`. You could run the following:

```sh
cd packages/a
haneul client test-publish --build-env testnet --pubfile-path ../Pub.localnet.toml

cd ../b
haneul client test-publish --build-env testnet --pubfile-path ../Pub.localnet.toml
```

When you publish `a`, the `packages/Pub.localnet.toml` file will be updated with an entry with `a`'s localnet address. Then when you publish `b`, it will take `a`'s address from `packages/Pub.localnet.toml`, and will also write `b`'s localnet address.
...
```sh
cd packages/b
haneul client test-publish --build-env testnet --publish-unpublished-deps`
```

This will create a fresh publication of each of `a`'s dependencies, and then publish `a`, and record all of the addresses to `Pub.localnet.toml`.
...
Once you have published your package, you should consider adding it to the Move registry so that others can easily depend on it. See [the Move registry documentation](https://docs.haneulns.io/move-registry/managing-package-info) for more information.
