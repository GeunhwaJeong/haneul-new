...
description: The life of a transaction on the Haneul network has some differences compared to those from other blockchains.
keywords: [ transaction lifecycle, transaction flow, transaction finality, settlement, consensus, certification, execution ]
---

At a high level, the following diagram outlines the life cycle of a transaction on the Haneul blockchain.

![Transaction life cycle](./images/txn-lifecycle.png)

The following steps align with those in the diagram:

1. A user with a private key creates and signs a user transaction to either mutate objects they own, [shared objects](/guides/developer/objects/object-ownership/shared.mdx), or create new objects.

1. Haneul sends the transaction to each [validator](/guides/operator/validator-index.mdx), often through a [full node](/guides/operator/haneul-full-node.mdx). Validators perform a series of validity and safety checks, sign it, and return the signed transaction to the client.

1. The client collects the responses from a set of validators that account for at least 2/3 of the [validator committee](/concepts/haneul-architecture/consensus.mdx) to form a transaction certificate. As a result, Haneul validators do not need to propagate signatures on a best-effort basis (gossip signatures) or aggregate certificates. This effort is now the responsibility of the client or gateway.
...
1. After assembling the certificate, the client sends it back to all validators, who check its validity and acknowledge its receipt to the client.
    ...
    1. If the transaction involves [owned objects](/guides/developer/objects/object-ownership/address-owned.mdx) or immutable objects exclusively, Haneul can process the transaction certificate immediately and execute it without waiting for the [consensus engine](/concepts/haneul-architecture/consensus.mdx).

1. All certificates are forwarded to the Haneul DAG-based consensus protocol, which also operated by the validators. Consensus outputs a complete order of certificates; the validators check and execute those that contain [shared objects].
...
1. Subsequently, Haneul forms [checkpoints](/concepts/cryptography/system/checkpoint-verification.mdx) for every consensus commit, which it uses to drive the [reconfiguration protocol](/concepts/haneul-architecture/epochs.mdx).

The [Haneul Lutris](https://docs.haneul.io/paper/haneul-lutris.pdf) paper provides additional details on how the safety and liveness protocols operate, as well as proof of security for the standard distributed systems model with Byzantine participants in partial synchrony.

## Transaction submission

All transactions on Haneul begin when they are submitted to the network. For example, imagine you want to transfer an NFT that you own in your wallet to your friend. First, you would create a transaction using a wallet or some other app. The transaction includes your gas object and a command to transfer the NFT object to your friend's address. Before the wallet or app submits the transaction to the network, it must also sign it.

After the transaction is signed, the wallet or app submits the transaction to a Haneul full node on behalf of the user.
...
- The transaction has a valid user [signature](/guides/developer/transactions/transaction-auth/auth-overview).
...
- The gas coin is a `Coin<HANEUL>` object, and it contains at least as much gas as specified in the transaction's gas budget.

If all the validity checks pass, the validator then attempts to lock all the owned input objects to the given transaction digest. This ensures each owned input object can only be used by one transaction at a time and is the way that Haneul prevents double-spending. In other words, this makes sure that you send your NFT to only one friend instead of trying to send the same NFT to all your friends.
...
The validator submits the transaction to Haneul's consensus layer, which orders the transaction with respect to other transactions using the same shared objects and then executes it. Transactions that don't overlap on inputs at all can be verified and executed completely in parallel.
...
At this point, the transaction has reached the end of its life cycle and is included in the permanent record of transaction activity on the Haneul network.
...
For transactions that involve owned objects only, this happens before consensus in under 1/2 a second. If a transaction includes shared objects, it happens shortly after consensus, which can take a few seconds. At this point, the transaction reached settlement finality because now you can process more transactions on the same input objects. See [Object Ownership](/guides/developer/objects/object-ownership) for more information.

## Epoch change

Periodically (~24 hours), the Haneul [Testnet](/concepts/haneul-architecture/networks.mdx) and [Mainnet](/concepts/haneul-architecture/networks.mdx) networks enter the process of [epoch](/concepts/haneul-architecture/epochs.mdx) change. During epoch change, the network calculates and distributes [staking rewards](/concepts/tokenomics/staking-unstaking.mdx), validator metadata takes effect, and other network processes are performed. All user transactions are delayed until the new epoch begins.
...
As a real-world example, suppose you want to pay the local coffee shop 10 HANEUL for your morning coffee. The following steps demonstrate how the coffee shop can be sure that the payment is complete.

### Transaction creation and signing

You open the wallet app on your phone and scan the coffee shop's QR code that contains the recipient on-chain address. The wallet app constructs a transaction that transfers 10 HANEUL from your Haneul address to the coffee shop's address. You review the transaction details and approve it. The wallet app then signs the transaction with your private key and submits it to a full node. The full node broadcasts the transaction to all validators in the network.
