...
This guide explores creating a [programmable transaction block (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks) on Haneul using the [TypeScript SDK](https://sdk.haneul-labs.com/typescript/install).
...
Learn more about [inputs](/guides/developer/transactions/ptbs/inputs-and-results).
...
You might need to explicitly call `setSender()` on the PTB to ensure that the `sender` field is populated. This is normally done by the signer before [signing the transaction](/guides/developer/transactions/ptbs/sign-and-send-txn), but will not be done automatically if you're building the PTB bytes yourself.
