...
This guide explores creating a [programmable transaction block (PTB)](/guides/developer/transactions/prog-txn-blocks.mdx) on Haneul using the [TypeScript SDK](https://sdk.haneul-labs.com/typescript/install).
...
After you have the transaction defined, you can directly execute it with a `HaneulClient` and `KeyPair` using `client.signAndExecuteTransaction`.
...
There are 2 ways to define inputs:
...
- For pure values: The `tx.pure(type, value)` function is used to construct an input for a non-object input.
  ...
  - In the new version, a more intuitive way of writing is provided, for example: `tx.pure.u64(100)`, `tx.pure.string('SomeString')`, `tx.pure.address('0xSomeHaneulAddress')`, `tx.pure.vector('bool', [true, false])`...

Learn more about [inputs](/concepts/transactions/inputs-and-results.mdx).
...
You might need to explicitly call `setSender()` on the PTB to ensure that the `sender` field is populated. This is normally done by the signer before [signing the transaction](/guides/developer/transactions/sign-and-send-txn.mdx), but will not be done automatically if you're building the PTB bytes yourself.
...
The Wallet Standard interface supports `Transaction` directly. All `signTransaction` and `signAndExecuteTransaction` calls from dApps into wallets must provide a `Transaction` class. You can then serialize this PTB class and send it to your wallet for execution.

To serialize a PTB to be sent to a wallet, use the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the Wallet Standard context to your wallet. You can then convert this back into a `Transaction` using `Transaction.from()`.
...
Learn more about [sponsored transactions](/guides/developer/transactions/sponsor-txn.mdx).