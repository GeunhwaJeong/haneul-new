...
This guide provide a practical example for building a custom indexer. Refer to [Custom Indexing Framework](/concepts/data-access/custom-indexers) and [Indexer Pipeline Architecture](/concepts/data-access/pipeline-architecture) for a conceptual overview of the indexer framework.
...
Use the `cargo run` command to run your indexer against Testnet. You can choose from multiple checkpoint data sources (see [Checkpoint Data Sources](/concepts/data-access/indexer-data-integration.mdx#checkpoint-data-sources) for details):
