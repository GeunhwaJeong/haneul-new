...
description: Learn how to integrate custom data sources and storage systems with Haneul indexers. Covers checkpoint data sources, custom store implementations, and Move event deserialization for building flexible indexing solutions.
keywords: [haneul custom indexer, checkpoint data sources, custom storage backend, move event deserialization, BCS serialization, indexer integration, bring your own store, postgresql indexer, blockchain data ingestion, haneul indexing framework]
---

Building a custom indexer on Haneul lets you take full control of data ingestion, storage, and processing. You can choose from multiple [checkpoint data sources](#checkpoint-data-sources) such as remote store, local files, or direct RPC API access, depending on whether you're indexing Mainnet data, testing against known checkpoints, or working on a local network.
...
The `haneul-indexer-alt-framework` supports multiple data sources for accessing Haneul blockchain data.You configure these sources through command-line arguments. They fall into two categories: **push-based** and **polling-based**.
...
**Endpoint format:** `https://fullnode.NETWORK.haneul.io:443` where `NETWORK` is one of the available networks:
...
The most direct stream source is a subscription to a remote checkpoint store. Haneul provides the following endpoints:
...
```sh
$ cargo run -- --rpc-api-url https://fullnode.testnet.haneul.io:443
```
...
- Development against local Haneul networks

## Bring your own store (BYOS) {#byos}

The `IndexerCluster` provides a convenient way to get started with PostgreSQL, but you might want to use a different database or storage system. This requires using the manual `Indexer` class and implementing custom `Store` and `Connection` traits from `haneul-indexer-alt-framework-store-traits`.

<details>
<summary>
`lib.rs` in `haneul-indexer-alt-framework-store-traits`
</summary>

<ImportContent source="crates/haneul-indexer-alt-framework-store-traits/src/lib.rs" mode="code" trait="Connection,Store" />
...
```rust
use haneul_indexer_alt_framework::store::{Store, Connection};
...
For a complete reference, study the `haneul-pg-db` implementation on `Connection`:

<ImportContent source="crates/haneul-pg-db/src/store.rs" mode="code" impl="Connection" />
    ...
    let indexer = Indexer::new(
        ...
        ClientArgs {
            ingestion: IngestionClientArgs {
                remote_store_url: Some("https://checkpoints.testnet.haneul.io".to_string()),
                ..Default::default()
            },
            ..Default::default()
        },
...
For a complete working example of BYOS with [ClickHouse](https://clickhouse.com/) (a high-performance columnar database for analytics), see the [example project in the Haneul repo](https://github.com/GeunhwaJeong/haneul/tree/main/examples/rust/clickhouse-haneul-indexer).

<details>
<summary>
ClickHouse example README
</summary>
<ImportContent source="examples/rust/clickhouse-haneul-indexer/README.md" mode="code" style="md" />
</details>
...
1. `store.rs` - ClickHouseStore implementing `Store` and `Connection` traits.
    ...
    <ImportContent source="examples/rust/clickhouse-haneul-indexer/src/store.rs" mode="code" />
    </details>
...
1. `handlers.rs` - `TxDigest` handler processing checkpoint data.
    ...
    <ImportContent source="examples/rust/clickhouse-haneul-indexer/src/handlers.rs" mode="code" />
    </details>
...
1. `main.rs` - Manual indexer setup with ClickHouse backend.
    ...
    <ImportContent source="examples/rust/clickhouse-haneul-indexer/src/main.rs" mode="code" />
    </details>

## Deserializing Move events {#deserialize}

When Move smart contracts execute on Haneul, they can emit events using the `haneul::event` module. These events are stored in checkpoints as **BCS-serialized bytes** that your indexer needs to deserialize to extract meaningful data.
...
1. Define the `Event` struct in Rust
    ...
    Define the same structure in Rust as declared in Move. You can do this manually or use [`move-binding`](https://github.com/GeunhwaJeong/move-binding) to auto-generate it from on-chain packages.
