...
The Haneul RPC provides a [`queryEvents`](/haneul-api-ref#haneulx_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the DeepBookV3 package on Mainnet for a specific type of event:
