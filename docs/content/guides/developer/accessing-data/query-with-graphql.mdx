...
This guide provides practical examples for querying the Haneul network using GraphQL. For core concepts like headers, variables, fragments, pagination, scope, and limits, see [GraphQL for Haneul RPC](/concepts/data-access/graphql-rpc.mdx).
...
Consult the [GraphQL reference](../../../references/haneul-graphql.mdx) for full documentation on the supported schema.
...
Find all the transactions that touched (modified, transferred, or deleted) a given object. Useful when you want to trace the flow of a `Coin`, `StakeHaneul`, `NFT`, or similar object.
...
The examples in the following sections demonstrate typical API calls. The calls are tabbed to show both the JSON-RPC call and the equivalent GraphQL structure. For a comprehensive list of all available GraphQL features, consult the [reference](/references/haneul-graphql).
