...
description: Practical guide to making queries of the Haneul RPC using the GraphQL service, with examples for common tasks.
keywords: [ rpc, haneul rpc, graphql, query transactions, query blocks, query block info, query dynamic field, execute transaction, graphql examples ]
---

This guide provides practical examples for querying the Haneul network using GraphQL. For core concepts like headers, variables, fragments, pagination, scope, and limits, see the [corresponding concepts page](/concepts/data-access/graphql-rpc).

Access the GraphQL service for Haneul RPC through an online IDE that provides a complete toolbox for fetching data and executing transactions on the network:
...
Both Mainnet and Testnet services are rate-limited to keep network throughput optimized. These services are provided as a public good from Haneul Foundation and are not meant for production-grade use. Refer to the [list of RPC or data providers](https://www.notion.so/haneullabs/RPC-providers-offering-future-Haneul-data-primitives-2466d9dcb4e980a99a36e9aafd8c17e0?source=copy_link) that have enabled gRPC on their full nodes.
...
Consult the [GraphQL reference](/references/haneul-graphql) for full documentation on the supported schema. The [official GraphQL introspection documentation](https://graphql.org/learn/introspection/) provides an overview on introspection and how to interact with it programmatically.

## Headers

The service accepts the following optional HTTP request headers:

- `x-haneul-rpc-version`: Specifies which RPC version to use. Currently only one version is supported.
- `x-haneul-rpc-show-usage`: Returns the response with extra query complexity information.

By default, each response contains the following HTTP response headers:

- `x-haneul-rpc-request-id`: A unique identifier for the request. This appears in application logs for debugging.
- `x-haneul-rpc-version`: The version of the service that handled the request.

```sh
$ curl -i -X POST https://graphql.testnet.haneul.io/graphql\
     --header 'x-haneul-rpc-show-usage: true'                 \
...
content-length: 179
x-haneul-rpc-request-id: f5442058-47ab-4360-8295-61c009f38516
x-haneul-rpc-version: 1.56.1-
...
When using the [GraphQL online integrated development environment (IDE)](https://graphql.mainnet.haneul.io/graphql), supply variables as a JSON object to the query in the **Variables** pane at the bottom of the main editing window. You receive a warning if you supply a variable but don't declare it.
...
```sh
$ curl -X POST https://haneul-testnet.haneul-labs.com/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo` of type <UnsafeLink href="/references/haneul-api/haneul-graphql/beta/reference/types/objects/page-info">PageInfo</UnsafeLink>, which indicates whether there are more pages before or after the page returned.
...
query NextBalancesPage($address: HaneulAddress!, $after: String!) {
...
```graphql
query FirstBalancesPageAtCheckpoint($address: HaneulAddress!, $cp: UInt53) {
  checkpoint(sequenceNumber: $cp) {
    query {
      address(address: $address) {
        balances(first: 10) {
...
```graphql
query ObjectsDynamicFields($id: HaneulAddress!, $v: UInt53!) {
...
```graphql
query NestedDynamicFields($id: HaneulAddress!, $r: UInt53!) {
...
```graphql
query WrappedDynamicField($id: HaneulAddress!, $r: UInt53!) {
...
```graphql
query LatestDynamicField($id: HaneulAddress!, $v: UInt53!) {
  object(address: $id, version: $v) {
    objectAt {
      version
    }
...
The GraphQL service for Haneul RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
...
Find all the transactions that touched (modified, transferred, or deleted) a given object. This might be useful when you want to trace the flow of a `Coin`, `StakeHaneul`, `NFT`, or similar object.
...
query ($address: HaneulAddress!) {
  transactions(filter: {
    function: "0x3::haneul_system::request_add_stake"
    sentAddress: $address
  }) {
    nodes {
      digest
...
Transaction execution takes 2 arguments, `transactionDataBcs` and `signatures`. `transactionDataBcs` is the serialized unsigned transaction data, which the [Haneul Client CLI](/references/cli/client) `client call` command can generate. Pass the `--serialize-unsigned-transaction` flag to the command to call a Move function. The [Haneul Keytool CLI](/references/cli/keytool) command `haneul keytool sign` can generate the `signatures`.
...
The examples in the following sections are tabbed to show both the JSON-RPC call and the equivalent GraphQL structure. For a comprehensive list of all available GraphQL features, consult the [reference](/references/haneul-graphql).
...
Return all `Coin<0x2::haneul::HANEUL>` objects that an address owns.
...
Find all versions of the Haneul framework and list their modules:
...
<RelatedLink to="/references/haneul-graphql.mdx" label="Haneul GraphQL RPC schema" />
<RelatedLink to="/guides/operator/indexer-stack-setup" label="Setup GraphQL RPC service and general-purpose indexer stack" />
<RelatedLink to="/guides/operator/haneul-full-node.mdx" />
<RelatedLink href="https://graphql.testnet.haneul.io/graphql" label="Haneul Testnet GraphiQL (public good)" desc="Haneul GraphiQL IDE for Testnet (Public good)." />
<RelatedLink href="https://graphql.mainnet.haneul.io/graphql" label="Haneul Mainnet GraphiQL (public good)" desc="Haneul GraphiQL IDE for Mainnet (Public good)." />
