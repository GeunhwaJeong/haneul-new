...
Every Haneul [object](/guides/developer/objects/object-model.mdx) must have the `key` ability. The `store` ability, on the other hand, is an optional ability you can add to Haneul objects. Objects with the `store` ability are transferable by anyone using the [`transfer::public_transfer`](/references/framework/haneul_haneul/transfer#haneul_transfer_public_transfer) function and can be [wrapped](/guides/developer/objects/object-ownership/wrapped.mdx) in other objects.
