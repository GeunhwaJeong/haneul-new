---
title: Object Transfer Policies
sidebar_label: Transfer Policies
...
A [`TransferPolicy`](https://docs.haneul.io/references/framework/haneul_haneul/transfer_policy) on Haneul is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed. A `TransferPolicy` can be used within a Haneul Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. 
...
This example module has no configuration and accepts a `Coin<HANEUL>` of any value.
...
- Amount paid in HANEUL

- The object ID that was used for selling, such as a kiosk

The `haneul::transfer_policy` module provides public functions to access the fields `paid()`, `item()`, and `from()`.
