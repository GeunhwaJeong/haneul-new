...
description: On Haneul, you can transfer objects to objects in the same way you can transfer objects to addresses.
...
An object must receive another object before it can use it. To receive the object, use a `Receiving(o: ObjectRef)` argument type in a [programmable transaction block (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks). This argument takes an object reference containing the object's `ObjectID`, `Version`, and `Digest`, similar to address-owned object arguments. However, `Receiving` PTB arguments are not passed as an owned value or mutable reference within the transaction.

The core receiving interface in Move is defined in the [`transfer`](https://docs.haneul.io/references/framework/haneul_haneul/transferx) module in the Haneul framework:
...
///
/// This function has custom rules that the Haneul Move bytecode verifier enforces to ensure
...
Each `Receiving` argument referring to a sent object of type `T` in a PTB results in exactly 1 argument with type `haneul::transfer::Receiving<T>`. You can then use this argument to receive the sent object of type `T` with the `transfer::receive` function.
...
Just like with [custom transfer policies](/guides/developer/objects/transfers/custom-rules.mdx), Haneul allows for the definition of custom receivership rules for `key`-only objects. In particular, you can use the `transfer::receive` function only on objects defined in the same module as the call to [`transfer::receive`](/references/framework/haneul_haneul/transfer#haneul_transfer_receive), just like you can use the [`transfer::transfer`](/references/framework/haneul_haneul/transfer#haneul_transfer_transfer) function only on objects defined in the module where it's being used.

Similarly for objects that also have the `store` ability, anyone can use the [`transfer::public_receive`](/references/framework/haneul_haneul/transfer#haneul_transfer_public_receive) function to receive them, just like [`transfer::public_transfer`](/references/framework/haneul_haneul/transfer#haneul_transfer_public_transfer) can transfer any objects that have the `store` ability on them.
...
Interact with `Receiving` transaction inputs as you would with other object arguments in the Haneul TypeScript SDK. For example, if in the [Simple Account](#simple-account) example you want to send a transaction that receives a coin object with ID `0xc0ffee` that was sent to your account at `0xcafe`, you can do the following using either the [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) or [Haneul Rust SDK](https://github.com/GeunhwaJeong/haneul-rust-sdk):
...
client
  ...
  .move_call(
     ...
     "account",
     "accept_payment",
     vec!["0x2::haneul::HANEUL"],
     vec![
       HaneulJsonValue::from_object_id("0xcafe"),
       HaneulJsonValue::from_object_id("0xc0ffee") // 0xcoffee is turned into the `Receiving<...>` argument of `accept_payment` by the SDK
     ])
  ...
```
