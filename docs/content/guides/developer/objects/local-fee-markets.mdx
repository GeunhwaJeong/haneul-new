...
Haneul's local fee market algorithm runs every time a new batch of sequenced transactions is received from consensus. When it runs, it:
...
Using a gas price that is at least 5 times the reference gas price increases the likelihood that the transaction is included in the earliest consensus commit possible. See [SIP-45](https://github.com/haneul-foundation/sips/blob/main/sips/sip-45.md) for details.

### Cost estimation of transactions

Haneul limits the per-commit execution capacity of each shared object. If transactions touching a shared object have a low estimated execution time, more of them can fit. If they have a high estimated execution time, fewer can fit.  
