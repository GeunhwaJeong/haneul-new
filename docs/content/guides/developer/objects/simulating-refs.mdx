...
description: Use the borrow module in the Haneul framework to include objects by reference in your programmable transaction blocks.
keywords: [ borrow module, haneul framework, objects, immutable reference, mutable reference, Haneul API, references, simulate a reference, simulate references ]
---


Everything on the Haneul blockchain is an object. When you develop Move packages for the Haneul network, you typically manipulate or use on-chain objects in some way through functionality available in the Haneul API. For most API functions, you provide an object by reference.

References are a key construct when programming in Move and on Haneul. Most of the functionality available in the Haneul API takes objects by reference.
...
- **By reference:** When you use an object by reference, operations over that object are determined by the logic the module that defines the object provides because you are using a reference to its data rather than having ownership of the object itself. The restrictions of references allow you to develop smart contracts with a high level of security and safety around assets. There are 2 types of references:
  ...
  - Mutable reference (`&mut`): You can alter the object (according to the API) but you cannot destroy or transfer it.
...
Using a `Referent` is a very explicit and intrusive change. Take this into consideration when designing a solution.
