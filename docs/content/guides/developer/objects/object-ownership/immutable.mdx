...
Objects on Haneul can either be immutable or mutable. An immutable object cannot be mutated, transferred, or deleted. These objects have no owner and are freely accessible to everyone.

## Create immutable objects

To make an object immutable, call the [`haneul::transfer::public_freeze_object`](/references/framework/haneul_haneul/transfer#haneul_transfer_public_freeze_object) function from the [transfer module](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/transfer.move):
...
You can see this function used in the [`color_object` example module](https://github.com/GeunhwaJeong/haneul/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new address-owned `ColorObject` object, then calls `public_freeze_object` to turn it into an immutable object.
...
After an object becomes immutable, the rules of who can use this object in Haneul Move calls change:

1. You can only pass an immutable object as a read-only, immutable reference to Haneul Move entry functions as `&T`.
...
Publish the `ColorObject` code on-chain using the Haneul CLI:
...
```sh
$ haneul client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "freeze_object" --args \"$OBJECT\"
```
...
```sh
$ haneul client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "update" --args \"$OBJECT\" 0 0 0
```
