---
title: Customizing a Nautilus server
description: Details on how to customize Nautilus server logic and use enclave data in a Move smart contract.
keywords: [ nautilus, using nautilus, deploy aws nitro enclave, aws nitro, enclaves, tee, ec2 ]
---

The Nautilus server logic lives in `src/nautilus-server`. To customize the application, refer to [`apps/weather-example` or `apps/twitter-example`](/guides/developer/nautilus/using-nautilus.mdx) as templates.

You can customize the Nautilus server to define `allowed_endpoints.yaml` to specify any external domains your application needs to access and create `mod.rs` to define your `process_data` logic and register any additional endpoints.
...
## Use the verified computation in Move
