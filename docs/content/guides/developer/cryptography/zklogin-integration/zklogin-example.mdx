...
keywords: [ zklogin, how to use zklogin, generate ephemeral key pair, fetch JWT, decode JWT, generate salt, generate user Haneul address, fetch ZK proof, a2ssemble zkLogin signature ]
---
...
The example runs in Haneul Devnet. All data the user generates is stored locally on the client-side (browser). The acquisition of the zero-knowledge proof (ZK proof) is done through a call to the [Haneul Labs-maintained proving service](../zklogin-integration.mdx#call-the-haneullabs-labs-maintained-proving-service). Therefore, running the example does not require an additional deployed backend server (or a Docker container).
