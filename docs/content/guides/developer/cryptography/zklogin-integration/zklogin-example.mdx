...
The example runs in Haneul Devnet. All data the user generates is stored locally on the client-side (browser). The acquisition of the zero-knowledge proof (ZK proof) is done through a call to the [Haneul Labs-maintained proving service](/guides/developer/cryptography/zklogin-integration/). Therefore, running the example does not require an additional deployed backend server (or a Docker container).
