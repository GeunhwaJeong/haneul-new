...
- **Optional verified identity:** You can opt in to verify the OAuth identifier used to derive a particular Haneul address. This creates the foundation for a verifiable on-chain identity layer.
...
3. A transaction is submitted on-chain with the ephemeral signature and the zero-knowledge proof. Haneul authorities execute the transaction after verifying the ephemeral signature and the proof.

4. Instead of deriving the Haneul address based on a public key, the zkLogin address is derived from `sub` (user identifier), `iss` (provider), `aud` (application), and `user_salt` (a value that unlinks the OAuth identifier from the on-chain address).
