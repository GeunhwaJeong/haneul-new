...
Here's an example request and response for the Haneul Labs-maintained salt server (using option 4). If you want to use the Haneul Labs salt server, please contact us for whitelisting your registered client ID. Only valid JWT authenticated with whitelisted client IDs are accepted.
