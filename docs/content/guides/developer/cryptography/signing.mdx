...
Move contracts in Haneul support verifications for several signature schemes on-chain. Not all signatures supported in on-chain verification are supported as user signature verification. See [Haneul Signatures](/concepts/transactions/transaction-auth.mdx) for valid signature schemes for transaction authorization.
