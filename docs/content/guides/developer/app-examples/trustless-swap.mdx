...
description: An app that performs atomic swaps on Haneul. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
---

{@include: ../../../snippets/app-examples-swap-source.mdx}

This guide demonstrates how to make an app that performs atomic swaps on Haneul. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- Read the basics of [shared versus owned objects](../haneul-101/shared-owned.mdx).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the Haneul repo to follow along instead. See [Write a Move Package](../first-app/write-package.mdx) to learn more about package structure and how to use the Haneul CLI to scaffold a new project.
...
The test starts with a helper function for creating an object, it doesn't matter what kind of object it is, as long as it has the `store` ability. The test uses `Coin<HANEUL>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that don't need to simulate multiple transactions but still need access to a `TxContext` can use `haneul::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `haneul move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- [Full source code](https://github.com/GeunhwaJeong/haneul/blob/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/contracts/escrow/sources/shared.move)
- Concepts: [Shared Objects](../../../concepts/object-ownership/shared)
- Concepts: [Shared Object Deletion](https://blog.haneul.io/ephemeral-shared-objects/)
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the Haneul RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
{@inject: examples/trading/contracts/escrow/sources/shared.move#use=haneul::event}
...
- Guide: [Using Events](../haneul-101/using-events.mdx)

:::

:::checkpoint

You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `haneul move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass: 
...
- [Full source code](https://github.com/GeunhwaJeong/haneul/tree/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/api)
- Reference: [JSON-RPC](https://docs.haneul.io/haneul-api-ref)
...
3. Deploy the Haneul package
  ...
  Next, configure the Haneul CLI to use `testnet` as the active environment. 
  ...
  Before being able to publish your package to Testnet, you need Testnet HANEUL tokens. To get some, run the following command: 

  ```bash
  haneul client faucet
  ```

  For other ways to get HANEUL in your Testnet account, see [Get HANEUL Tokens](/guides/developer/getting-started/get-coins).

  Now that you have an account with some Testnet HANEUL, you can deploy your contracts. 

  There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contrqcts expects built smart contracts in both the `escrow` and `demo` directories. Run `haneul move build` in both directories, if necessary. Be sure to update the Haneul dependency in the manifest to point to the correct source based on your environment. 
...
- Tooling: [Haneul Typescript SDK](https://sdk.haneul-labs.com/typescript). For basic usage on how to interact with Haneul with TypeScript.
...
- Tooling: [`@haneullabs/dapp`](https://sdk.haneul-labs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based Haneul dApp.
...
<details>
  ...
  {@inject: examples/trading/frontend/src/components/HaneulObjectDisplay.tsx}
</details>
...
Note that `dApp Kit` provides a set of hooks for making query and mutation calls to the Haneul blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
Fetch the owned objects directly from Haneul blockchain using the `useHaneulClientInfiniteQuery()` hook from `dApp Kit`. This hook is a thin wrapper around Haneul blockchain RPC calls, reference the documentation to learn more about these [RPC hooks](https://sdk.haneul-labs.com/dapp-kit/rpc-hooks). Basically, supply the RPC endpoint you want to execute, in this case it's the [`getOwnedObjects` endpoint](https://docs.haneul.io/haneul-api-ref#haneulx_getownedobjects). Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data will be re-fetched automatically.
...
Use the `useHaneulClient()` hook from `dApp Kit` to retrieve the Haneul client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the Haneul client instance of the Haneul TypeScript SDK.
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Haneul blockchain RPC.
...
This component uses tanstack's `useInfiniteQuery` instead of `useHaneulClientInfiniteQuery` since the data is being fetched from the example's API rather than Haneul. 
