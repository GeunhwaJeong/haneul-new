---
title: Plinko
description: Learn Haneul through an example implementation of the popular casino game, Plinko.
keywords: [ example app, example app, onchain randomness, enoki sponsorship, sponsored transactions, codegen, typescript codegen, example game, game app ]
---

Plinko is an example implementation of the popular casino game. The Plinko game on Haneul leverages onchain randomness to ensure fairness and transparency. Players drop Plinko balls onto a pegged board, where they randomly fall into slots representing different multipliers. This document details the game's mechanics, onchain randomness implementation, sponsored transactions through Enoki, and TypeScript integration using codegen.

Building an onchain Plinko game shares a lot of similarities with the [Coin Flip](./coin-flip.mdx) game and [Blackjack](./blackjack.mdx) game. For that reason, this example covers only the smart contracts (Move modules) and frontend logic.

<ImportContent source="prerequisites.mdx" mode="snippet" />

:::info

You can find the source files for this example in the [Plinko repo](https://github.com/GeunhwaJeong/plinko-poc/tree/main) on GitHub.

A version of the game is also deployed at [Haneullabs Plinko](https://plinko-poc.vercel.app/).

:::

## Gameplay

The Plinko game, implemented through smart contracts on the Haneul blockchain, leverages Haneul's native onchain randomness to ensure fairness and transparency. The game calculates the trace path for each ball using secure random values generated directly onchain, determining the game's outcome based on the number of Plinko balls a player chooses to drop.

The game mechanics involve a player starting a game by specifying the number of balls and staking a certain amount. The onchain randomness is provided through Haneul's Move randomness framework, which ensures each game's randomness is cryptographically secure and cannot be predicted or manipulated. The number of Plinko balls, chosen by the player, directly influences the game's complexity and potential payout, as each ball's final position is determined by the onchain random values that dictate the ball's path through the pegged board.
...
sequenceDiagram
    ...
    participant Blockchain as Haneul Blockchain
    ...
    Note over Enoki: executeSponsoredTransaction()
    Enoki->>+Blockchain: Submit sponsored + signed transaction
    Note over Blockchain: Executes plinko::finish_game,<br/>uses onchain randomness to calculate outcome<br/>(ball trace paths, payments), emits GameFinished event
...
The `plinko::plinko` module combines various Haneul blockchain features, such as coin handling, event emissions, onchain randomness, and game logic, to create fair and transparent onchain Plinko games.
...
public fun start_game(coin: Coin<HANEUL>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
/// finish_game Completes the game by calculating the outcome using onchain randomness
...
entry fun finish_game(
    ...
    let stake_per_ball = stake.value<HANEUL>() / num_balls;
    ...
    while (ball_index < num_balls) {
        ...
        // Divide by 100 to adjust for multiplier scale and HANEUL units
    ...
    let payout_coin: Coin<HANEUL> = coin::take(payout_balance_mut, total_funds_amount, ctx);
...
fun internal_start_game(
    ...
    let new_game = Game {
        ...
        stake: coin.into_balance<HANEUL>(),
        player: ctx.sender(),
        fee_bp,
    };
...
- `finish_game`: Completes the game using onchain randomness to calculate outcomes, traces the path each ball travels, and distributes the total winnings to the player. This is an entry function that takes the Random object from Haneul's randomness framework.
...
public fun initialize_house_data(
    ...
    let mut house_data = HouseData {
        ...
        house: ctx.sender(),
        max_stake: 10_000_000_000, // 10 HANEUL = 10^9.
        min_stake: 100_000_000, // 0.1 HANEUL.
        fees: balance::zero(),
...
The Plinko game demonstrates a frictionless user experience by leveraging [Enoki](https://docs.enoki.haneul-labs.com/) for transaction sponsorship. This allows players to interact with the game without needing to hold HANEUL tokens for gas fees, significantly reducing barriers to entry.
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const sponsored = await enokiClient.createSponsoredTransaction({
      network: process.env.NEXT_PUBLIC_HANEUL_NETWORK_NAME as
        | "mainnet"
        | "testnet"
        | "devnet",
...
The Plinko frontend uses [Haneullabs's codegen tool](https://www.npmjs.com/package/@haneullabs/codegen?activeTab=readme) to automatically generate TypeScript bindings from the Move smart contracts. This provides several advantages:
...
Navigate to the [setup folder](https://github.com/GeunhwaJeong/plinko-poc/tree/main/setup) of the Plinko repository and execute the `publish.sh` script. Refer to the [README instructions](https://github.com/GeunhwaJeong/plinko-poc/blob/main/README.md) for deploying the smart contracts and testing them locally.


## Frontend

The Plinko frontend is built with React and integrates with the Haneul blockchain using sponsored transactions through Enoki. The application provides an interactive and responsive gaming experience while abstracting away blockchain complexity from end users.
...
- **Sponsored transactions:** The frontend leverages Enoki's `/sponsor` and `/execute` APIs to create a gasless experience. Players can interact with the game without needing HANEUL tokens, as the backend sponsors all transaction fees.
...
The `MatterSim` and `PlinkoSettings` components are the foundation for the Plinko frontend. To see code for all components and source files, see the [Plinko repo](https://github.com/GeunhwaJeong/plinko-poc/tree/main/app/src).  

### Simulation component

The [`MatterSim` component](https://github.com/GeunhwaJeong/plinko-poc/blob/main/app/src/components/MatterSim.tsx) renders the game board with realistic physics for the dropping Plinko balls. It uses [Matter.js](https://brm.io/matter-js/), a 2D physics engine, to simulate the game dynamics.
...
The component receives ball paths (trace data) generated from onchain randomness, emitted in the `GameFinished` event. Each path consists of 12 bytes per ball, where each byte determines whether the ball moves left (odd) or right (even) at each peg. `MatterSim` applies physics principles like gravity for natural ball descent while using custom forces to guide balls along their predetermined paths. The physics simulation provides smooth, realistic movement that matches the onchain outcomes, creating an engaging visual experience where fairness is guaranteed by blockchain randomness.

## Plinko settings component

The [`PlinkoSettings` component](https://github.com/GeunhwaJeong/plinko-poc/blob/main/app/src/components/PlinkoSettings.tsx) is an integral part of the user interface in the Plinko game, enabling players to customize their gameplay experience according to their preferences. This React component allows users to select the number of Plinko balls they want to drop, set the bet size for each ball, and initiate the game round by pressing the **Play** button.
