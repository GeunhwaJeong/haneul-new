...
This is the third in a [three-part guide](../trustless-token-swap.mdx) on how to build a trustless atomic swap on Haneul.
...
- Understand [the mechanism behind the Escrow smart contract backend](../trustless-token-swap.mdx).
...
- Check out [Haneul Typescript SDK](https://sdk.haneul-labs.com/typescript) for basic usage on how to interact with Haneul with Typescript.
- Check out [Haneul dApp Kit](https://sdk.haneul-labs.com/dapp-kit) to learn basic building blocks for developing a dApp in the Haneul ecosystem with React.js.
- Check out [React Router](https://reactrouter.com/en/main) as we use it to navigate between different routes in our UI website.
- `dApp Kit` provides a set of hooks for making query and mutation calls to Haneul blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`. Please check out [@tanstack/react-query](https://tanstack.com/query/latest/docs/framework/react/overview) to learn the basic usage for managing, caching, mutating server state.
- This project is bootstrapped through `pnpm create @haneullabs/dapp`. Please check out [@haneullabs/create-dapp](https://sdk.haneul-labs.com/dapp-kit/create-dapp) for how to scaffold a React.js Haneul dApp project quickly.
...
The following code snippets are not the full source code. The snippets are meant to focus on relevant logic important to the functionality of the example and features of Haneul.
...
Use the `useHaneulClient()` hook from `dApp Kit` to retrieve the Haneul client instance configured in the [**Set up providers**](#set-up-providers) step. The `useSignTransactionBlock()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, the `executeTransactionBlock()` on the Haneul client instance of the Haneul TypeScript SDK. Use `react-hot-toast` as another dependency to toast transaction status to users.

## Generate demo data

:::info

The full source code of the demo bear smart contract is available at [Trading Contracts Demo directory](https://github.com/GeunhwaJeong/haneul/tree/main/examples/trading/contracts/demo)
...
As previously mentioned, the example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Haneul blockchain RPC.

When you execute a transaction call to mutate data on the Haneul blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs, however, you only need two of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
export function useCreateEscrowMutation() {
	...
	return useMutation({
		mutationFn: async ({ object, locked }: { object: HaneulObjectData; locked: ApiLockedObject }) => {
...
Fetch the owned `Locked` objects directly from Haneul blockchain using the `useHaneulClientInfiniteQuery()` hook from `dApp Kit`. This hook is a thin wrapper around Haneul blockchain RPC calls, reference the documentation to learn more about these [RPC hooks](https://sdk.haneul-labs.com/dapp-kit/rpc-hooks). Basically, supply the RPC endpoint you want to execute, in this case it's the [`getOwnedObjects` endpoint](https://docs.haneul.io/haneul-api-ref#haneulx_getownedobjects). Supply the connected wallet account and the `Locked` object struct type to the call. The struct type is usually identified by the format of `{PACKAGE_ID}::{{MODULE_NAME}}::{{STRUCT_TYPE}}`. The returned data is stored inside the cache at query key `getOwnedObjects`. Recall the previous section where you invalidate the data at this key after the mutation succeeds, the `useHaneulClientInfiniteQuery()` hook automatically re-fetches the data, thus you don't have to worry about the out-dated data living in your frontend application.

#### `LockedObject` and `Locked` component

The `<LockedObject />` (`src/components/locked/LockedObject.tsx`) component is mainly responsible for mapping an on-chain `HaneulObjectData` `Locked` object to its corresponding `ApiLockedObject`, which is finally delegated to the `<Locked />` component for rendering. The `<LockedObject />` fetches the locked item object ID if the prop `itemId` is not supplied by using `dApp Kit` `useHaneulClientQuery()` hook to call the `getDynamicFieldObject` RPC endpoint. Recalling that in this smart contract, the locked item is put into a dynamic object field.
...
- Query the escrowed item object directly from Haneul blockchain by using `useHaneulClientQuery('getObject')` as this is the only way to have its Object Display metadata.
...
const { data, fetchNextPage, hasNextPage, isLoading, isFetchingNextPage } = useInfiniteQuery({
	...
	queryFn: async ({ pageParam }) => {
		...
		const objects = await haneulClient.multiGetObjects({
		...
		return {
			haneulObjects: objects.map((x) => x.data),
			api: data,
		};
	},
