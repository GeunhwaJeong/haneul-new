...
description: Build a React frontend for the Trustless Swap app using dApp Kit, TanStack Query, and the Haneul TypeScript SDK.
effort: large
keywords:
...
- [x] [Implement the backend](/guides/developer/app-examples/trustless-swap.mdx#backend) to learn how to index on-chain data and expose it through an API.
...
- [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript). For basic usage on how to interact with Haneul with TypeScript.
...
- [`@haneullabs/dapp`](https://sdk.haneul-labs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based Haneul app.
...
`HaneulObjectDisplay.tsx`

</summary>

<ImportContent source="examples/trading/frontend/src/components/HaneulObjectDisplay.tsx" mode="code" />
...
The dApp Kit provides a set of hooks for making query and mutation calls to the Haneul blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
export function LockOwnedObjects() {
	...
	return (
		...
		>
			{data?.map((obj) => (
				<HaneulObjectDisplay key={obj.objectId} object={obj}></HaneulObjectDisplay>
			))}
		</InfiniteScrollArea>
	);
}
...
Fetch the owned objects directly from the Haneul blockchain using `useInfiniteQuery` from TanStack Query with the `useCurrentClient()` hook from dApp Kit. The `useCurrentClient()` hook returns the configured Haneul client, and you use its `core.listOwnedObjects()` method to fetch paginated owned objects. Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data is re-fetched automatically.
...
Use the `useCurrentClient()` hook from dApp Kit to retrieve the Haneul client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from dApp kit that helps to sign the transaction using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction, use the `executeTransaction()` on the client instance of the Haneul TypeScript SDK.
...
This example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Haneul blockchain RPC.
...
import { HaneulObjectData } from '@haneullabs/haneul/jsonRpc';
...
export function Locked({
	...
	return (
		<HaneulObjectDisplay object={haneulObject.data!} label={getLabel()} labelClasses={getLabelClasses()}>
					...
					<Button
						...
						onClick={() => {
							unlockMutation({
								...
								keyId: locked.keyId,
								haneulObject: haneulObject.data!,
							});
						}}
					>
