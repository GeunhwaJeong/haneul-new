...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your Haneul Move module and connecting it to your React Haneul app. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
[Example source code](https://github.com/GeunhwaJeong/haneul/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)

:::

## What the guide teaches

- **Shared objects:** The guide teaches you how to use [shared objects](/guides/developer/objects/object-ownership/shared.mdx), in this case to create globally accessible `Counter` objects.
- **Programmable transaction blocks (PTBs):** You learn how to use PTBs to interact with your Move module from your frontend.

## Directory structure

To begin, create a new folder on your system titled `react-e2e-counter` to hold all your project files. You can name this directory differently, but the rest of the guide references this file structure. Inside that folder, create two more folders: `move` and `src`. Inside the `move` folder, create a `counter` directory. Finally, create a `sources` folder inside `counter`. Different projects have their own directory structure, but it's common to split code into functional groups to help with maintenance. See ["Hello, World!"](/guides/developer/getting-started/hello-world.mdx) to learn more about package structure and how to use the Haneul CLI to scaffold a new project.

:::checkpoint

- You have the latest version of Haneul installed. If you run `haneul --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `haneul client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update Haneul using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the Haneul repo.
- Your active address has HANEUL. Run `haneul client balance` in your terminal or console. If there is no balance, [acquire HANEUL](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the Haneul dependency.

:::

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/Move.toml" mode="code" org="HaneulLabs" repo="ts-sdks" />

### `Counter` struct

To begin creating the smart contract that defines the onchain counter, create a `counter.move` file inside your `react-e2e-counter/move/counter/sources` folder. Define the module that holds your smart contract logic.
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" struct="Counter" org="HaneulLabs" repo="ts-sdks" noComments />

- The `Counter` type stores the address of its `owner`, its current `value`, and its own `id`.

### Creating `Counter`

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="create" org="HaneulLabs" repo="ts-sdks" noComments />

In the `create` function, a new `Counter` object is created and [shared](/guides/developer/objects/object-ownership/shared.mdx). 

### Incrementing and resetting `Counter`

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="increment" org="HaneulLabs" repo="ts-sdks" noComments />

The `increment` function accepts a mutable reference to any shared `Counter` object and increments its `value` field.

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="set_value" org="HaneulLabs" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" org="HaneulLabs" repo="ts-sdks" noComments />

:::checkpoint

Your smart contract is complete. You should be able to run the `haneul move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `haneul move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
```sh
$ pnpm create @haneullabs/dapp --template react-e2e-counter
```
...
```sh
$ yarn create @haneullabs/dapp --template react-e2e-counter
```
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/networkConfig.ts" mode="code" org="HaneulLabs" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/CreateCounter.tsx" mode="code" org="HaneulLabs" fun="create" repo="ts-sdks" noComments />

</details>

The `create` function now creates a new Haneul `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
Add the following code to your `src/Counter.tsx` file:
  ...
  import { useCurrentAccount, useCurrentClient, useDAppKit } from '@haneullabs/dapp-kit-react';
...
This snippet shows the key concepts. It uses the `useCurrentClient` hook to get the Haneul client, combined with TanStack Query's `useQuery` hook to fetch the object. The `useDAppKit` hook provides access to transaction signing.

Note that the gRPC API returns object content as BCS-encoded bytes. The snippet above includes inline BCS type definitions to parse the counter data. For production use, the [codegen package](https://sdk.haneul-labs.com/typescript/bcs#codegen) can generate these types automatically from your Move modules. The full template (shown below) uses the codegen package for BCS parsing.
...
The full template version of `src/Counter.tsx` (using the codegen package for BCS parsing):

</summary>

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/Counter.tsx" mode="code" org="HaneulLabs" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/App.tsx" mode="code" org="HaneulLabs" repo="ts-sdks" noComments />
