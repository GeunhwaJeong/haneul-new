...
First, import `Transaction` from `@haneullabs/haneul`, `COUNTER_PACKAGE_ID` from your constants.ts file created previously, and `useSignAndExecuteTransaction` from `@haneullabs/dapp-kit`.
	...
	signAndExecute(
		...
		},
		{
			onSuccess: async ({ digest }) => {
				const tx = await haneulClient.waitForTransaction({
					digest,
					options: {
						showEffects: true,
					},
...
Now you can use the `haneulClient` in your `create` function to wait until the `Transaction` is indexed:
...
export function Counter({ id }: { id: string }) {
	...
	function executeMoveCall(method: 'increment' | 'reset') {
		...
		signAndExecute(
			...
			{
				onSuccess: async (tx) => {
					await haneulClient.waitForTransaction({ digest: tx.digest });
					refetch();
				},
			},
		);
