...
Next, use the [Haneul Client CLI](references/cli/client.mdx) to generate a new Move package. If you have [Haneul installed](guides/developer/getting-started/haneul-install.mdx), the Haneul CLI is on your system. Run the following command in your terminal or console:

```bash
haneul move new counter
```
...
Now that you have your Move code, you need to publish it. The Client App with Haneul TypeScript SDK example and the app template use `testnet` by default, so configure your code to match the network you want to deploy to.

First, update the Haneul dependency in Move.toml by changing the `rev` from `framework/testnet` to `framework/devnet`.
...
Next, configure the Haneul CLI to use `devnet` as the active environment, as well. If you haven't already set up a `devnet` environment you can do so by running the following command in a terminal or console:
...
```bash
haneul client publish --gas-budget 10000000 counter
```

:::info

See [Haneul Client CLI](/references/cli/client.mdx) for more information about `client` commands in the Haneul CLI.
...
First, import `TransactionBlock` from `@haneullabs/haneul.js`, `COUNTER_PACKAGE_ID` from your constants.ts file created previously, and `useSignAndExecuteTransactionBlock` from `@haneullabs/dapp-kit`.

```ts
import { useSignAndExecuteTransactionBlock } from '@haneullabs/dapp-kit';
...
You now have a functional component that can create a new `Counter` object, but if you use it as is, you might run into some consistency issues where you successfully execute the `TransactionBlock`, but the data isn't yet indexed to read from an RPC node. To ensure the `TransactionBlock` is available, you can use the `waitForTransactionBlock` method of `HaneulClient`.
To get an instance of `HaneulClient`, you can use the `useHaneulClient` hook from dApp Kit:

```ts
import { useSignAndExecuteTransactionBlock, useHaneulClient } from '@haneullabs/dapp-kit';
...
Now you can use the `haneulClient` in your `create` function to wait until the `TransactionBlock` is indexed:
...
export function Counter({ id }: { id: string }) {
	const currentAccount = useCurrentAccount();
	const { data, refetch } = useHaneulClientQuery('getObject', {
		id,
		options: {
			showContent: true,
		},
...
export function Counter({ id }: { id: string }) {
	...
	function executeMoveCall(method: 'increment' | 'reset') {
		...
		signAndExecute(
			...
			{
				onSuccess: (tx) => {
					haneulClient.waitForTransactionBlock({ digest: tx.digest }).then(() => {
						refetch();
					});
				},
			},
...
Your counter app is now ready to count. To learn more about dApp Kit, check out the [dApp Kit docs](https://sdk.haneul-labs.com/dapp-kit).
