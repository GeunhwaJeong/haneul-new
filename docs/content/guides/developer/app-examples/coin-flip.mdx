...
This guide assumes you have [installed Haneul](../getting-started/haneul-install.mdx) and understand Haneul fundamentals.

## Backend

As with all Haneul dApps, a Move package on chain powers the logic of Satoshi Coin Flip. The following instruction walks you through creating and publishing the module.
...
The full source code for the Move modules, including comments and on overview of its cryptography, is available at the [Satoshi Coin Flip repository](https://github.com/GeunhwaJeong/satoshi-coin-flip).
...
- The second struct, `HouseCap`, is a [capability](concepts/haneul-move-concepts/patterns/capabilities.mdx) that initializes the house data.
- The third struct, `HOUSE_DATA`, is a [one-time witness](concepts/haneul-move-concepts/one-time-witness.mdx) that ensures only a single instance of this `HouseData` ever exists.
- The [`init` function](concepts/haneul-move-concepts/init.mdx) creates and sends the `Publisher` and `HouseCap` objects to the sender.
...
```rust title='house_data.move'
    public fun initialize_house_data(house_cap: HouseCap, coin: Coin<HANEUL>, public_key: vector<u8>, ctx: &mut TxContext) {
        ...
        let house_data = HouseData {
            ...
            public_key,
            max_stake: 50_000_000_000, // 50 HANEUL.
...
```rust title='house_data.move'
    ...
    public(friend) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<HANEUL> {
        &mut house_data.balance
    }

    public(friend) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<HANEUL> {
        &mut house_data.fees
    }
...
Lastly in this section, you also create structs for two [events](concepts/dynamic-fields/events.mdx) to emit. Indexers consume emitted events, which enables you to track these events through API services, or your own indexer. In this case, the events are for when a new game begins (`NewGame`) and for the outcome of a game when it has finished (`Outcome`).
...
See [Publish a Package](guides/developer/first-app/publish) for a more detailed guide on publishing packages or [Haneul Client CLI](/references/cli/client.mdx) for a complete reference of `client` commands in the Haneul CLI.

:::

Before [publishing your code](guides/developer/first-app/publish), you must first initialize the Haneul Client CLI. To do so, in a terminal or console at the root directory of the project enter `haneul client`. You will then see:

```
Config file ["[LINK_TO_PATH/.haneul/haneul_config/client.yaml"] doesn't exist, do you want to connect to a Haneul Full node server [y/N]?
```
...
The full source code for the frontend is available at the [Satoshi Coin Flip Frontend Example repository](https://github.com/haneul-foundation/satoshi-coin-flip-frontend-example).

:::

To expose the backend you have created to your users, you need a frontend (UI). In this section, you create a React frontend project using the [Haneul Typescript SDK](https://haneul-typescript-docs.vercel.app/typescript) and the [Haneul dApp Kit](https://haneul-typescript-docs.vercel.app/dapp-kit) that interacts with the deployed smart contracts.
...
First, initialize your frontend project. To do this rapidly, use the [`create-dapp` tool](https://haneul-typescript-docs.vercel.app/dapp-kit/create-dapp) to bootstrap the project using [dApp Kit](https://haneul-typescript-docs.vercel.app/dapp-kit). Run the following command in your terminal or console:

```
pnpm create @haneullabs/create-dapp
```
...
1. It asks you the starter template that you want to use. Currently, there are two variants:
   ...
   1. `react-e2e-counter`: This starter template contains a simple counter Haneul Move smart contract with the frontend template code interacting with it. This variant is meant for developers trying to learn how to use dApp Kit.
...
There are two constants that you need to put into `constants.ts` to make the app work – `PACKAGE_ID` and `HOUSECAP_ID`. You can get these from the terminal or console after running the Haneul CLI command to publish the package.
  ...
  onSubmit={(e) => {
    ...
    txb.moveCall({
      ...
      arguments: [
        ...
        // https://haneul-typescript-docs.vercel.app/typescript/transaction-building/basics#pure-values
        txb.pure(
          bcs
            .vector(bcs.U8)
...
To use a [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) in Haneul, create a `TransactionBlock`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
- Pass the BLS public key as bytes `vector<u8>`. When providing inputs that are not on-chain objects, serialize them as BCS using a combination of `txb.pure` and `bcs` imported from `@haneullabs/haneul.js/bcs`.
...
const [stakeCoin] = txb.splitCoins(txb.gas, [
  GEUNHWA_PER_HANEUL * BigInt(stake),
]);
