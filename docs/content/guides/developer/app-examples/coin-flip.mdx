---
title: Coin Flip
description: Learn Haneul through a coin flip dApp that covers the full end-to-end flow of building a Haneul Move module and connecting it to a React Haneul dApp.
...
- [Haneul dApp Kit](https://sdk.haneul-labs.com/dapp-kit) to learn basic building blocks for developing a dApp in the Haneul ecosystem with React.js.
- [`@haneullabs/dapp`](https://sdk.haneul-labs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Haneul dApp.
...
To use a [programmable transaction block](/guides/developer/transactions/prog-txn-blocks.mdx) (PTB) in Haneul, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
- Pass the BLS public key as bytes `vector<u8>`. When providing inputs that are not on-chain objects, serialize them as BCS using a combination of `txb.pure` and `bcs` imported from `@haneullabs/haneul/bcs`.
...
// This hook is to demonstrate how to use `@haneullabs/dapp-kit` React hook to query data
// besides using HaneulClient directly
export function useFetchCounterNft() {
  ...
  const { data, isLoading, isError, error, refetch } = useHaneulClientQuery(
    'getOwnedObjects',
    {
...
This hook logic is very basic: if there is no current connected wallet, return empty data; otherwise, fetch the `Counter` object and return it. dApp Kit provides a React hook, `useHaneulClientQuery()`, that enables interaction with [Haneul RPC](references/haneul-api.mdx) methods. Different RPC methods require different parameters. To fetch the object owned by a known address, use the [`getOwnedObjects` query](/haneul-api-ref#haneulx_getownedobjects).
...
Event subscriptions are deprecated. To learn future-safe methods to work with events, see [Using Events](/guides/developer/accessing-data/using-events.mdx).
...
export function HouseFinishGame() {
  ...
  useEffect(() => {
    ...
    const unsub = haneulClient.subscribeEvent({
      filter: {
  ...
  }, [housePrivHex, houseDataId, haneulClient]);

  return null;
}
```

To get the underlying `HaneulClient` instance from the SDK, use `useHaneulClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

`HaneulClient` exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. `HaneulClient::subscribeEvent()` is actually a thin wrapper around the RPC method [`haneulx_subscribeEvent`](/haneul-api-ref#haneulx_subscribeevent).
