---
title: Coin Flip
description: Learn Haneul through a coin flip app that covers the full end-to-end flow of building a Haneul Move module and connecting it to a React Haneul app.
...
This example walks you through building a Coin Flip app, covering the full end-to-end flow of building your Haneul Move module and connecting it to your React Haneul app. This Coin Flip app utilizes verifiable random functions (VRFs) to create a fair coin game on the Haneul blockchain. The user (human) plays against the house (module) and places a bet on either heads or tails. The user then either receives double their bet, or gets nothing, depending on the outcome of the game.
...
- [Move package repository](https://github.com/GeunhwaJeong/satoshi-coin-flip)
- [Frontend repository](https://github.com/haneul-foundation/satoshi-coin-flip-frontend-example)

:::

## What the guide teaches

- **Shared objects:** The guide teaches you how to use [shared objects](/guides/developer/objects/object-ownership/shared.mdx), in this case to create a globally accessible `HouseData` object.
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](/concepts/haneul-move-concepts.mdx) to ensure only a single instance of the `HouseData` object ever exists.
...
- **Address-owned objects:** The guide teaches you how to use [address-owned objects](/guides/developer/objects/object-ownership/address-owned) when necessary.
- **Events:** The guide teaches you how to emit events in your contracts, which can be used to track onchain activity. For more information on events, see [Using Events](/guides/developer/accessing-data/using-events.mdx) for practical usage of events on Haneul or [Events in The Move Book](https://move-book.com/programmability/events.html) to learn about event structure and how to emit them in Move.
- **Storage rebates:** The guide shows you best practices regarding [storage fee rebates](/concepts/tokenomics/tokenomics-overview.mdx).
- **MEV attack protection:** The guide introduces you to [MEV attacks](https://github.com/GeunhwaJeong/satoshi-coin-flip?tab=readme-ov-file#mev-attack-resistant-single-player-satoshi-smart-contract-flow), how to make your contracts MEV-resistant, and the trade-offs between protection and user experience.
...
This example uses several modules to create a package for the Satoshi Coin Flip game. The first module is `house_data.move`. You need to store the game's data somewhere, and in this module you create a [shared object](/guides/developer/objects/object-ownership/shared) for all house data.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the Haneul standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<HANEUL>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 HANEUL, 1 HANEUL = 10^9.
      min_stake: 1_000_000_000, // 1 HANEUL.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<HANEUL>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough HANEUL for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<HANEUL> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<HANEUL> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use haneul::haneul::HANEUL;
  use haneul::bls12381::bls12381_min_pk_verify;
...
- First, the function makes sure the `Game` object exists, then deletes it, as after the game concludes the metadata is no longer needed. Freeing up unnecessary storage is not only recommended, but [incentivized through rebates on storage fees](/concepts/tokenomics/tokenomics-overview.mdx).
...
```move title='single_player_satoshi.move'
  public fun start_game(guess: String, counter: &mut Counter, coin: Coin<HANEUL>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<HANEUL>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
You can read more about both versions of the game, and view the full source code for all the modules in the [Satoshi Coin Flip repository](https://github.com/GeunhwaJeong/satoshi-coin-flip).

Now that you have written our contracts, it's time to deploy them.

### Deployment {#deployment}

<ImportContent source="initialize-haneul-client-cli.mdx" mode="snippet" />

Next, configure the Haneul CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
To turn this into a complete app, you need to [create a frontend](#frontend).
...
To skip building the frontend and test out your newly deployed package, use the provided [Satoshi Coin Flip Frontend Example repository](https://github.com/haneul-foundation/satoshi-coin-flip-frontend-example) and follow the instructions in the example's `README.md` file
...
- [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) for basic usage on how to interact with Haneul using TypeScript.
- [Haneul dApp Kit](https://sdk.haneul-labs.com/dapp-kit) to learn basic building blocks for developing an app in the Haneul ecosystem with React.js.
- [`@haneullabs/dapp`](https://sdk.haneul-labs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Haneul app.
...
```sh
$ yarn create @haneullabs/dapp --template react-client-dapp
```
...
The UI interacts with the [Single Player smart contract](/guides/developer/app-examples/coin-flip.mdx) variant of the game. This section walks you through each step in the smart contract flow and the corresponding frontend code.

:::info

The following frontend code snippets include only the most relevant sections. Refer to the [Satoshi Coin Flip Frontend Example repository](https://github.com/haneul-foundation/satoshi-coin-flip-frontend-example) for complete source code.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      GEUNHWA_PER_HANEUL * BigInt(houseStake),
    ]);
    ...
    txb.moveCall({
      ...
      arguments: [
        ...
        houseStakeCoin,
        // This argument is not an onchain object, hence, we must serialize it using `bcs`
        // https://sdk.haneul-labs.com/typescript/transaction-building/basics#pure-values
        txb.pure(
          bcs
            .vector(bcs.U8)
    ...
    execInitializeHouse(
      ...
      },
      {
        ...
        onSuccess: (result: HaneulTransactionBlockResponse) => {
          let houseDataObjId;
...
To use a [programmable transaction block](/guides/developer/transactions/ptbs/prog-txn-blocks) (PTB) in Haneul, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
// This hook demonstrates how to use `@haneullabs/dapp-kit-react` hooks with TanStack Query
...
Great, now you can create the game with the created `Counter` object. Isolate the game creation logic into `PlayerCreateGame.tsx`. There is one more thing to keep in mind - to flag an input as an onchain object, you should use `txb.object()` with the corresponding object ID.
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [GEUNHWA_PER_HANEUL * BigInt(stake)]);
...
For more information on how to work with events in your dApp, see [Using Events](/guides/developer/accessing-data/using-events.mdx).
...
The component uses `HaneulGraphQLClient` to poll for `NewGame` events every 5 seconds, tracking the cursor to only process new events. When a new game is detected, it signs the VRF input with the BLS private key and calls `single_player_satoshi::finish_game()` to settle the game.

The cleanup function sets `cancelled = true` to stop polling when the component unmounts.

Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next Haneul project.
