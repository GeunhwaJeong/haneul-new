...
This guide and example demonstrate an atomic swap on Haneul, which is similar to an escrow but does not require a trusted third party. Instead, this example uses a [shared object](../../../concepts/object-ownership/shared.mdx) to act as the escrow between two Haneul users wanting to trade. Shared objects are a unique concept to Haneul. Any transaction and any signer can modify it, given the changes meet the requirements set forth by the package that defined the type.
