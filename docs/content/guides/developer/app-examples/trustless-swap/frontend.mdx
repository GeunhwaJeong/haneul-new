...
This is the third in a [three-part guide](../trustless-swap.mdx) on how to build a trustless atomic swap on Haneul.
...
Use the `useHaneulClient()` hook from `dApp Kit` to retrieve the Haneul client instance configured in the [**Set up providers**](#set-up-providers) step. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, the `executeTransaction()` on the Haneul client instance of the Haneul TypeScript SDK. Use `react-hot-toast` as another dependency to toast transaction status to users.
