...
- [health/](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/health): A simple healthcheck endpoint to check the API availability
...
To ensure that the game object is available in the Full node that the dealer is using, we need to call waitForTransactionBlock after the create-game transaction.
...
This leads to the need of exchanging the `txDigest` between the frontend and the backend, and use `waitForTransactionBlock` on each write-after-write or read-after-write case.
...
The main sub-directories are:
  ...
  - [styles/](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/styles): The global css files to style our app.
