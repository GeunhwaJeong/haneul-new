---
title: Blackjack
description: Learn Haneul using an example implementation of the popular casino game Blackjack.
...
The following documentation goes through an example implementation of the popular casino game Blackjack on Haneul. This guide walks through its components, providing a detailed look at the module's functions, structures, constants, and their significance in the overall gameplay mechanism.

A deployed version of the Blackjack game is online at [Haneullabs Blackjack](https://blackjack-haneul.vercel.app/).
...
For more details on building a backend and deploying to Haneul, check out the [Coin Flip app example](./coin-flip.mdx).

You can also find the [full repository for this example here](https://github.com/GeunhwaJeong/blackjack-haneul).
...
The [`single_player_blackjack.move`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/move/blackjack/sources/single_player_blackjack.move) module includes several constants that define game statuses and help track the game's progress:
...
The Blackjack game leverages Haneul's built-in [on-chain randomness module](/guides/developer/advanced/randomness-onchain.mdx) to generate unpredictable and verifiable card values, eliminating the need for external oracles or VRF (Verifiable Random Function) services. This approach ensures fairness and transparency in the game.
...
- [`health/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/health): A simple health check endpoint to check the API availability.
- [`helpers/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/helpers): Multiple helper functions used in various endpoints.
- [`services/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/services): The core logic of the backend, that signs and executes the transactions on the Haneul blockchain.
- [`utils/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/utils): Reusable methods for signing the transactions as the dealer, and sponsoring them with Shinami, to avoid gas coins equivocation.
...
An interesting aspect of developing an app on Haneul, that is coupled to using a full node with/without a load balancer, and requires attention by the developer, is the occurrence of read-after-write and write-after-write cases.
...
The main code of the frontend lies under the [`app/src/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/) directory.
The main sub-directories are:

- [`app/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/): The main code of the pages and the API endpoints.
- [`components/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components): The reusable components of the app, organized in sub-directories.
- [`hooks/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks): The custom hooks used in the app.
- [`helpers/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/helpers), [`utils/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/utils), [`lib/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/lib): Multiple helper functions and utilities.
- [`types/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/types): The types/interfaces used in the app.
- [`styles/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/styles): The global CSS files to style the app.
- [`__generated__/`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/__generated__): Auto-generated TypeScript bindings from Move contracts (see Codegen section below).

### Codegen

The project uses [`@haneullabs/codegen`](https://www.npmjs.com/package/@haneullabs/codegen) to automatically generate type-safe TypeScript bindings from the Move smart contracts. This ensures type safety when interacting with the blockchain from the frontend and backend.

The codegen configuration is defined in [`haneul-codegen.config.ts`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/haneul-codegen.config.ts):
...
- **Custom hooks:** To keep the code as structured as possible, multiple custom hooks are utilized to manage the complex state of the game board at each step. The [`useBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useBlackjackGame.ts) custom hook encapsulates the game state and logic, exposing all the required information (with fields such as `game`, `isInitialDealLoading`), and the required functionality (with methods such as `handleCreateGame` and `handleHit`) to display and play the game. Multiple additional custom hooks, such as [`useCreateBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useCreateBlackjackGame.ts), and [`useMakeMoveInBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useMakeMoveInBlackjackGame.ts) are encapsulating their own piece of state and logic to make the code readable and maintainable.

- **Component for game initialization:** The [`StartGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/StartGame.tsx) component is implemented to facilitate the creation of a new game. It uses the `handleCreateGame` function of the [`useBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useBlackjackGame.ts) hook to execute the create-game transaction.

- **Card displaying and management:** The [`DealerCards`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/DealerCards.tsx) and the [`PlayerCards`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/PlayerCards.tsx) components are used to display the total points and the cards owned by the dealer and the player respectively.

- **Game actions:** The [`GameActions`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/GameActions.tsx) component is used to display the **Hit** and **Stand** buttons, and trigger the corresponding actions, as they are exported by the [`useBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useBlackjackGame.ts) hook to execute the corresponding transactions.

- **`BlackjackBanner`**: The [`BlackjackBanner`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/BlackjackBanner.tsx) component is used as a custom view to display when the player wins with a blackjack.


### Enoki

The game uses [Enoki](https://docs.enoki.haneul-labs.com/) for transaction sponsorship, enabling users to play the game gas-free. The sponsorship workflow involves two key API endpoints:

#### [`/api/sponsor`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/sponsor/route.ts) endpoint
...
const sponsored = await enokiClient.createSponsoredTransaction({
  network: serverConfig.NEXT_PUBLIC_HANEUL_NETWORK_NAME,
...
#### [`/api/execute`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/app/api/execute/route.ts) endpoint

After the user signs the sponsored transaction, this endpoint executes it on Haneul:
...
- **Blockchain-based logic:** Both games are built on Haneul, leveraging its capabilities for decentralized applications. The core game logic for each resides in Move modules, ensuring secure and verifiable gameplay.
...
- **Randomness and fair play:** Both games emphasize randomness for fairness. Both games use Haneul's [on-chain randomness module](/guides/developer/on-chain-primitives/randomness-onchain.mdx), ensuring verifiable and truly random outcomes for every game.
...
The complete app example can be found in [the `blackjack-haneul` repository](https://github.com/GeunhwaJeong/blackjack-haneul).
