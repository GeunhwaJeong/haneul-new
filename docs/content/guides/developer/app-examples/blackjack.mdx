...
An interesting aspect of developing a dApp on Haneul, that is coupled to using a full node with/without a load balancer, and requires attention by the developer, is the occurrence of read-after-write and write-after-write cases.
...
- **Component for game initialization:** The [`StartGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/StartGame.tsx) component is implemented to facilitate the creation of a new game. It renders the [`CollectMouseRandomness`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/CollectMouseRandomness.tsx) to capture the randomness and uses the `handleCreateGame` function of the [`useBlackjackGame`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useBlackjackGame.ts) hook to execute the create-game transaction.

- **Randomness generation:** Fair game outcomes are also ensured by the [`CollectMouseRandomness`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/components/home/CollectMouseRandomness.tsx) component. This component uses the [`useMouseRandomness`](https://github.com/GeunhwaJeong/blackjack-haneul/blob/main/app/src/hooks/useMouseRandomness.ts) custom hook, and is in charge of capturing some user's mouse movements and generating a random bytes array. This array is converted to a hexadecimal string (`randomness`) and used in the create-game transaction.
...
- **Randomness and fair play:** Both games emphasize randomness for fairness. Blackjack uses a Counter NFT and player mouse movements to generate randomness, while Coin Flip only uses a Counter NFT as a unique input for the [Verifiable Random Function (VRF)](../cryptography/ecvrf.mdx) in each game.
...
The complete app example can be found in [the `blackjack-haneul` repo](https://github.com/GeunhwaJeong/blackjack-haneul).
