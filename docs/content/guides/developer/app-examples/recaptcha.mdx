...
As with all Haneul dApps, a Move package on chain powers the logic of the reCAPTCHA module. The following instruction walks you through creating and publishing the module.
...
module recaptcha::recaptcha {
    ...
    use haneul::ed25519::ed25519_verify;
...
1. The next eight lines use the use keyword to import types and functions from other modules, such as `haneul::clock::Clock`, `haneul::dynamic_field as df`, `haneul::ed25519::ed25519_verify`, `haneul::event::emit,haneul::math`, `haneul::object::{Self, UID}`, `haneul::transfer`, and `haneul::tx_context::{sender, TxContext}`. These modules are needed for the implementation of the reCAPTCHA verification and the interaction logic.
...
- It calls the `ed25519_verify` function from the `haneul::ed25519` module to check if the signature is valid for the given public key and message. The `ed25519_verify` function returns a boolean value that indicates the validity of the signature.
- It asserts that the signature is valid, otherwise it aborts the execution with the error code `EInvalidSignature`.
- It checks if the user's address exists as a key in the dynamic field of the registry object. The dynamic field is a way of storing key-value pairs in an object without declaring them in advance and it can be accessed, modified, or deleted using the `haneul::dynamic_field` module.
...
public fun interact(
    ...
    if (df::exists_with_type<address, u64>(&registry.id, sender(ctx))) {
        ...
        let current_timestamp = haneul::clock::timestamp_ms(clock);

        if (current_timestamp - *timestamp_ms <= registry.window) {
            emit(
                Interaction{
                    sender: sender(ctx),
                    timestamp_ms: haneul::clock::timestamp_ms(clock)
                }
            );
        } else {
...
- It checks if there is an existing interaction history for the sender address with the registry object. The interaction history is stored as a dynamic field in the registry object. The dynamic field is a way of storing key-value pairs in an object without declaring them in advance. The dynamic field can be accessed, modified, or deleted using the `haneul::dynamic_field` module.
...
Before [publishing your code](guides/developer/first-app/publish), you must first initialize the Haneul Client CLI. To do so, in a terminal or console at the root directory of the project enter `haneul client`. You receive the following response:
...
2. Install the dependencies with `pnpm install --save @haneullabs/dapp-kit @haneullabs/haneul.js axios react-google-recaptcha`.
...
import "./App.css";
import Axios from "axios";
import { ConnectButton, useCurrentWallet, useSignAndExecuteTransactionBlock, useCurrentAccount } from "@haneullabs/dapp-kit";
import { TransactionBlock } from "@haneullabs/haneul.js/transactions";
import { useEffect, useState } from "react";
import ReCAPTCHA from "react-google-recaptcha";
import { HANEUL_CLOCK_OBJECT_ID } from "@haneullabs/haneul.js/utils";
          ...
          onClick={async () => {
            ...
            transactionBlock.moveCall({
              ...
              arguments: [
                ...
                transactionBlock.object(HANEUL_CLOCK_OBJECT_ID),
              ],
            });
...
- Next, use hooks from `@haneullabs/dapp-kit` to get access to the current wallet, the current account, and the `signAndExecuteTransactionBlock` function. These help you connect to the wallet and execute transactions on the blockchain.
...
- Finally, return a JSX element that renders the app. The app consists of the following components:
  - A `ConnectButton` component from `@haneullabs/dapp-kit` that allows the user to connect to their wallet.
  - A button that allows the user to interact with the smart contract. This button is disabled unless the user passes the verification at least once. When the user clicks this button, create a new `TransactionBlock` object from `@haneullabs/haneul.js` and add a `moveCall` action that calls the `interact` function of the smart contract with the registry ID and the clock object ID as arguments. Then, use the `signAndExecuteTransactionBlock` function from `@haneullabs/dapp-kit` to sign and execute the transaction block on the blockchain. You also log the result to the console.
  - A button that allows the user to verify their identity to the smart contract. This button is disabled unless the user passes the reCAPTCHA challenge. When the user clicks this button, create a new `TransactionBlock` object from `@haneullabs/haneul.js` and add a `moveCall` action that calls the verify function of the smart contract with the registry ID, the signature, the public key, and the message as arguments. Then, use the `signAndExecuteTransactionBlock` function from `@haneullabs/dapp-kit` to sign and execute the transaction block on the blockchain. You also log the result to the console.
