...
2. Install the dependencies with `pnpm install --save @haneullabs/dapp-kit-react @haneullabs/haneul @tanstack/react-query axios react-google-recaptcha`.
...
- Next, use hooks from `@haneullabs/dapp-kit-react` to get access to the current wallet, the current account, and use TanStack Query with `useDAppKit` for the `signAndExecuteTransaction` function. These help you connect to the wallet and execute transactions on the blockchain.
