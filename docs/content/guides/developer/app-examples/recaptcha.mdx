...
2. Install the dependencies with `pnpm install --save @haneullabs/wallet-kit @haneullabs/haneul.js axios react-google-recaptcha`.
...
- Next, use the `useWalletKit` hook from `@haneullabs/wallet-kit` to get access to the current wallet, the current account, and the `signAndExecuteTransactionBlock` function. These help you connect to the wallet and execute transactions on the blockchain.
