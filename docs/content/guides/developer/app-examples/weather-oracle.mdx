...
keywords: [ weather app, weather forecast example, weather forecast dapp, haneul weather oracle, openweather api, randomness, nfts, example app, example dapp, oracle, create an oracle ] 
---
...
As with all Haneul apps, a Move package on chain powers the logic of Haneul Weather Oracle. The following instruction walks you through creating and publishing the module.
...
The `add_city` function lets the `AdminCap` holder add a new city to the Haneul Weather Oracle. The caller must present their `AdminCap` and pass a mutable reference to the main `Oracle` object. The function accepts city metadata—geoname ID, name, country and coordinates (latitude, longitude, and their positive flags). It then creates a `CityWeatherOracle` sub-object, initializes its weather fields to empty/zero, and inserts it into the oracle’s dynamic fields keyed by the geoname ID. After this, the city is registered and ready to receive updates from the backend.

To delete a city, call `remove_city`. The admin again provides the `AdminCap` and a **mutable** oracle reference, plus the city’s geoname ID. The function removes the corresponding `CityWeatherOracle` sub-object and deletes its dynamic field entry from the oracle. This cleanly unregisters the city and frees the associated onchain storage.
...
The `update` function accepts the city’s geoname ID and the new weather fields, then **writes** them to the corresponding `CityWeatherOracle` object. This keeps the onchain weather data current and accurate.
...
- Run `npm install @haneullabs/bcs @haneullabs/haneul axios csv-parse csv-parser dotenv pino retry-axios --save` to install the other dependencies and save them to your package.json file. These dependencies are:
  - **@haneullabs/bcs**: a library for blockchain services.
  - **@haneullabs/haneul**: a library for smart user interfaces.
...
- Imports the necessary modules and classes from the library, such as `Ed25519Keypair`, `HaneulClient`, and `Transaction`.
...
- Creates a haneul client object that connects to a Full node specified by the `FULLNODE` environment variable.
