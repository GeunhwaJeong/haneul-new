...
This guide demonstrates writing a module (smart contract) in Move, deploying it on Devnet, and adding a backend, which fetches the weather data from the OpenWeather API every 10 minutes and updates the weather conditions for each city. The dApp created in this guide is called Haneul Weather Oracle and it provides real-time weather data for over 1,000 locations around the world. 

You can access and use the weather data from the OpenWeather API for various applications, such as randomness, betting, gaming, insurance, travel, education, or research. You can also mint a weather NFT based on the weather data of a city, using the `mint` function of the HANEUL Weather Oracle smart contract.

This guide assumes you have [installed Haneul](../getting-started/haneul-install.mdx) and understand Haneul fundamentals.

## Move smart contract

As with all Haneul dApps, a Move package on chain powers the logic of Haneul Weather Oracle. The following instruction walks you through creating and publishing the module.
...
```bash
haneul move new weather_oracle
```
...
fun init(otw: WEATHER, ctx: &mut TxContext) {
    ...
    transfer::share_object(WeatherOracle {
        ...
        name: string::utf8(b"HaneulMeteo"),
...
- The first struct, `AdminCap`, is a [capability](concepts/haneul-move-concepts/patterns/capabilities.mdx).
- The second struct, `WEATHER`, is a [one-time witness](concepts/haneul-move-concepts/one-time-witness.mdx) that ensures only a single instance of this `Weather` ever exists.
...
- The [`init` function](concepts/haneul-move-concepts/init.mdx) creates and sends the `Publisher` and `AdminCap` objects to the sender. Also, it creates a [shared object](concepts/object-ownership/shared.mdx) for all the `CityWeatherOracle`s.
...
You have successfully deployed the Haneul Weather Oracle smart contract on the blockchain. Now, it's time to create an Express backend that can interact with it. The Express backend performs the following tasks:
...
The Express backend uses the [Haneul Typescript SDK](https://haneul-typescript-docs.vercel.app/typescript), a TypeScript library that enables you to interact with the Haneul blockchain and smart contracts. With the Haneul Typescript SDK, you can connect to the Haneul network, sign and submit transactions, and query the state of the smart contract. You also use the OpenWeather API to fetch the weather data for each city and update the smart contract every 10 minutes. Additionally, you can mint weather NFTs, if you want to explore that feature of the smart contract.
...
- Run `npm install @haneullabs/bcs @haneullabs/haneul.js axios csv-parse csv-parser dotenv pino retry-axios --save` to install the other dependencies and save them to your package.json file. These dependencies are:
    - **@haneullabs/bcs**: a library for blockchain services.
    - **@haneullabs/haneul.js**: a library for smart user interfaces.
...
The code in index.ts does the follwoing: 
...
Congratulations, you completed the Haneul Weather Oracle tutorial. You can carry the lessons learned here forward when building your next Haneul project.
