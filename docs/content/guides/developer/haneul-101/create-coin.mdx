...
The `Coin<T>` is a generic implementation of a coin on Haneul. The owner of the `TreasuryCap` gets control over the minting and burning of coins. Further transactions can be sent directly to the `haneul::coin::Coin` with `TreasuryCap` object as authorization.
...
For all `Coin` functions available on Devnet, see the [Haneul framework documentation](https://github.com/GeunhwaJeong/haneul/blob/devnet/crates/haneul-framework/docs/haneul-framework/coin.md) on the `devnet` branch.