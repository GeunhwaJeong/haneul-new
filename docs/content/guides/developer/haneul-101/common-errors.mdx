...
Indicates that the address you provided does not conform to a valid Haneul address format. Haneul addresses must:
...
- [Create a Haneul Address](/guides/developer/getting-started/get-address.mdx)
...
- [Error definition](https://github.com/GeunhwaJeong/haneul/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/haneul-graphql-e2e-tests/tests/stable/errors/clever_errors.snap)
...
- [Move Concepts](/concepts/haneul-move-concepts.mdx)
...
Indicates that a module in your package does not have the address `0x0`. In Move, you must set all modules to have their self-address as `0x0` in the source code. When you publish the package, Haneul automatically assigns it the correct address. 
...
Indicates that a function you are calling in your Move module expects 1 signer argument, yet you provided 2. On Haneul, the number of signer arguments in your function's signature must match the number of signers you provide. 
...
- [Install Haneul](/guides/developer/getting-started/haneul-install.mdx)
...
Indicates the network does not receive a response within a certain time period. This error is not specific to Haneul.
...
This error occurs when the network is down, unreachable due to an unstable internet connection, or there is a misconfiguration in your Haneul client configuration.
...
- [Configure a Haneul Client](/guides/developer/getting-started/configure-haneul-client.mdx)
...
This error occurs when a Move transaction does not have a `drop` ability, or you do not explicitly drop it. The Haneul runtime system enforces resource safety that throws this error when you leave values unused at the end of a transaction. 
...
Indicates that your transaction is trying to use one or more Haneul objects that another transaction already uses. When you use an object in a transaction, the system locks that object to prevent multiple, possibly conflicting modifications to an object. For DeFi objects, this prevents double-spending.
...
- [Signing and Sending Transactions](/guides/developer/haneul-101/sign-and-send-txn.mdx)
...
- [Avoiding Equivocation](/guides/developer/haneul-101/avoid-equivocation.mdx)

---

#### `Unable to process transaction. Unexpected status code: 403`

Indicates the Haneul network responds to an RPC or API request with `HTTP 403: Forbidden` status. This might happen if an RPC provider:
...
Indicates the Haneul network rejects a transaction due to invalid transaction inputs. 
...
- [Get HANEUL from Faucet](/guides/developer/getting-started/get-coins.mdx)
