---
title: Using Events
description: Monitor Haneul on-chain activity by subscribing to events that Move packages published on Haneul emit.
---
...
To support activity monitoring, Move provides a structure to emit events on the Haneul network. When you establish a connection with the Haneul network, you create a two-way interactive communication session between your client and a Haneul network node. With an open session, you can subscribe to specific events that the Haneul network adds to the stream to create real-time monitoring of events.
...
Firing events is not very useful in a vacuum. You also need the ability to listen for those events so that you can act on them. In Haneul, you subscribe to those events and provide logic that triggers when the event fires.

Haneul Full nodes support subscribe functionality using JSON-RPC notifications transmitted through the WebSocket API. You can interact with the RPC directly ([haneulx_subscribeEvent](/haneul-api-ref#haneulx_subscribeEvent), [haneulx_subscribeTransaction](/haneul-api-ref#haneulx_subscribeTransaction)) or you can use an SDK like the Haneul TypeScript SDK. The following excerpt from one of the examples uses the TypeScript SDK to create an asynchronous subscription to the filter identified in the filter.
...
```json
{
	...
	"type": "0xdee9::clob_v2::OrderPlaced<0x2::haneul::HANEUL, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
	"parsedJson": {
...
This example leverages the Haneul TypeScript SDK to subscribe to events the package with ID `<PACKAGE_ID>` emits. Each time the event fires, the code displays the response to the console.

#### TypeScript

To create the event subscription, you can use a basic Node.js app. You need the Haneul TypeScript SDK, so install the module using `npm install @haneullabs/haneul` at the root of your project. In your TypeScript code, import `JsonRpcProvider` and a connection from the library.
...
The [Haneul by Example](https://github.com/gdanezis/haneul-by-example/blob/main/src/05_reading_events/bin/main.rs) repo on GitHub contains a code sample that demonstrates how to subscribe to events using the `query_events` function. The package that `PACKAGE_ID_CONST` points to exists on Mainnet, so you can test this code using Cargo. To do so, clone the `haneul-by-example` repo locally and follow the [Example 05 directions](https://github.com/gdanezis/haneul-by-example/tree/main/src/05_reading_events). 
...
### Filtering events for subscription (deprecated)

:::warning

This section is deprecated beginning with Haneul client version 1.28. Use the [custom indexer](../advanced/custom-indexer.mdx) section to learn about how to stream checkpoints and filter events continuously.
