...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](../app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the Haneul network. The database is populated from polling the network to retrieve emitted events. 
