...
Haneul supports the following transaction commands:
...
To serialize a PTB for sending to a wallet, Haneul recommends using the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the Wallet Standard dApp context to your wallet. You can then convert this back into a `Transaction` using `Transaction.from()`.
