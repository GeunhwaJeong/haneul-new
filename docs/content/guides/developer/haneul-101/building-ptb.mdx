...
This guide explores creating a programmable transaction block (PTB) on Haneul using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the Haneul TypeScript SDK, follow the [install instructions](https://haneul-typescript-docs.vercel.app/typescript/install) on the Haneul TypeScript SDK site.

This example starts by constructing a PTB to send Haneul. If you are familiar with the legacy Haneul transaction types, this is similar to a payHaneul transaction. To construct transactions, import the TransactionBlock class, and construct it:
...
Note that you can attach multiple transaction commands of the same type to a PTB as well. For example, to get a list of transfers, and iterate over them to transfer coins to each of them:
interface Transfer {
  to: string;
  amount: number;
}
...
Inputs are how you provide external values to PTBs. For example, defining an amount of Haneul to transfer, or which object to pass into a Move call, or a shared object. 
...
Haneul supports following transaction commands:
...
The gas budget is represented in Haneul, and should take the gas price of the PTB into account.
...
To serialize a PTB for sending to a wallet, Haneul recommends using the `txb.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the wallet standard dApp context to your wallet. This can then be converted back into a `TransactionBlock` using `TransactionBlock.from()`.
