...
This guide explores creating a programmable transaction block (PTB) on Haneul using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the Haneul TypeScript SDK, follow the [install instructions](https://sdk.haneul-labs.com/typescript/install) on the Haneul TypeScript SDK site.

This example starts by constructing a PTB to send Haneul. If you are familiar with the legacy Haneul transaction types, this is similar to a `payHaneul` transaction. To construct transactions, import the `Transaction` class, and construct it:
...
To serialize a PTB for sending to a wallet, Haneul recommends using the `txb.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the wallet standard dApp context to your wallet. This can then be converted back into a `Transaction` using `Transaction.from()`.
