...
description: Sponsored transactions are a primitive on the Haneul blockchain that enable the execution of a transaction where you pay the gas fee for your users transactions.
...
A transaction on Haneul takes a payment to execute. The payment, also known as gas, is a list of `0x2::coin::Coin<0x2::haneul::Haneul>` objects. Although gas is a critical piece in Haneul tokenomics, it sometimes adds challenges when new Web3 users start to navigate on Haneul.
