---
title: Slush
description: Learn about Slush Wallet, its features, and how to integrate it into your wallet or app using the Slush Wallet SDK and Haneul dApp Kit.
keywords: [ slush, slush wallet, wallet, zkLogin, wallet integration, dapp kit, deep linking, registerSlushWallet ]
---

Slush is the official Haneul wallet built by Haneul Labs, available as a Chrome browser extension, an iOS app, an Android app, and a web app at [slush.app](https://slush.app).

## Slush basics

Slush Wallet gives users two ways to create and access an account:

- **Social login** uses [zero-knowledge login (zkLogin)](/guides/developer/cryptography/zklogin-integration/zklogin) to derive a Haneul address from an OAuth credential (Google, Apple, Facebook, or Twitch). No recovery passphrase is required. This is the recommended login method for users who are new to wallets, because key management is fully abstracted.
...
- [**Staking**](/concepts/tokenomics/staking-unstaking): Delegate HANEUL to a validator and earn staking rewards directly from the wallet.
- **Swaps**: Exchange tokens in-app without leaving the wallet.
- **App browser**: Browse and connect to Haneul apps from inside the mobile app.

## Slush Wallet SDK

The [`@haneullabs/slush-wallet` SDK](https://sdk.haneul-labs.com/slush-wallet) lets you add Slush Wallet support to your own wallet interface or app. The Slush browser extension and native mobile app work automatically through the [Wallet Standard](/standards/wallet-standard) and do not require this SDK. The SDK is only required if you want to support the Slush web app as a connection option.

### Install
```bash
pnpm install @haneullabs/slush-wallet
```
...
## Haneul dApp Kit integration

The [Haneul dApp Kit](https://sdk.haneul-labs.com/dapp-kit) provides built-in opt-in support for Slush Wallet. Pass `slushWalletConfig` to `createDAppKit` to enable it. The `name` field is required and is shown to the user during the connection flow.
...
For the full route reference, parameter tables, platform-specific implementation examples, and troubleshooting guidance, refer to the [Slush Wallet SDK docs on deep linking](https://sdk.haneul-labs.com/slush-wallet/deep-linking).