...
Update your `my_module.move` file to include a function callable from Haneul that implements `sword` creation. With this in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put this functions after the accessors (Part 5 in comments).
...
- No return values
- Private visibility
- Optionally, the parameter list starts by accepting the module's [one-time witness by value](../../../concepts/haneul-move-concepts/one-time-witness.mdx)
...
You can refer to the source code for the package (with all the tests and functions properly adjusted) in the [first_package](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/first_package/sources/example.move) module in the `haneul/examples` directory.

## Related links

- [Publish a Package](./publish.mdx): Continue the example by publishing your package to the Haneul network.
