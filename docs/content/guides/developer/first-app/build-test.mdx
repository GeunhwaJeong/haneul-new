...
Haneul includes support for the Move testing framework. Using the framework, you can write unit tests that analyze Move code much like test frameworks for other languages, such as the built-in Rust testing framework or the JUnit framework for Java.

An individual Move unit test is encapsulated in a public function that has no parameters, no return values, and has the `#[test]` annotation. The testing framework executes such functions when you call the `haneul move test` command from the package root (`my_move_package` directory as per the current running example):
...
<ImportContent source="examples/move/first_package/sources/example.move" mode="code" tag="first-test" />
...
<ImportContent source="examples/move/first_package/sources/example.move" mode="code" tag="test-dummy" noComments />
...
The previous testing example uses Move but isn't specific to Haneul beyond using some Haneul packages, such as `haneul::tx_context` and `haneul::transfer`. While this style of testing is already useful for writing Move code for Haneul, you might also want to test additional Haneul-specific features. In particular, a Move call in Haneul is encapsulated in a Haneul transaction, and you might want to test interactions between different transactions within a single test (for example, one transaction creating an object and the other one transferring it).

Haneul-specific testing is supported through the `test_scenario` module that provides Haneul-related testing functionality otherwise unavailable in pure Move and its testing framework.
...
Update your `example.move` file to include a function callable from Haneul that implements `sword` creation. With this in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put this functions after the accessors (Part 5 in comments).

<ImportContent source="examples/move/first_package/sources/example.move" mode="code" fun="sword_create" noComments />

The code of the new functions uses struct creation and Haneul-internal modules (`tx_context`) in a way similar to what you have seen in the previous sections. The important part is for the function to have correct signatures.

With the new function included, add another test function to make sure it behaves as expected.

<ImportContent source="examples/move/first_package/sources/example.move" mode="code" fun="test_sword_transactions" />
...
<ImportContent source="examples/move/first_package/sources/example.move" mode="code" fun="init" noComments />
...
<ImportContent source="examples/move/first_package/sources/example.move" mode="code" fun="new_sword" noComments />

Now, create a function to test the module initialization:

<ImportContent source="examples/move/first_package/sources/example.move" mode="code" fun="test_module_init" />
...
<ImportContent source="examples/move/first_package/sources/example.move" mode="code" />
