...
To publish your package to the Haneul network, you can use the `haneul client publish` command of the Haneul Client CLI. After the package becomes available on the network, you can then use the `haneul client call` command to access the functions available in the package.
...
While the `haneul move` command does not support publishing explicitly, you can still test module initializers using the testing framework by dedicating the first transaction to executing the initializer function.
...
`sword_create` function signature change. The changes required for the tests to run again is an exercise left for you. If you need help, you can refer to the source code for the package (with all the tests properly adjusted) in [my_module.move](https://github.com/GeunhwaJeong/haneul/tree/main/haneul_programmability/examples/move_tutorial/sources/my_module.move).