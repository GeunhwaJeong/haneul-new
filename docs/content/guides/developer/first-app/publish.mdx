...
Your currently active address now has three objects (or more, if you had objects prior to this example). Assuming you are using a new address, running the `haneul client objects` command reveals what those objects are.
...
Now that the package is on-chain, you can call its functions to interact with the package. Use the `haneul client call` command to make individual calls to package functions, or construct more advanced blocks of transactions using the `haneul client ptb` command. The `ptb` part of the command stands for [programmable transaction blocks (PTBs)](../../../concepts/transactions/prog-txn-blocks.mdx). PTBs allow you to group commands together in a single transaction for more efficient and cost-effective network activity.
