...
description: You must publish Move packages on the Haneul network before you can call its functions.
...
Before you can call functions in a Move package (beyond an emulated Haneul execution scenario), that package must be available on the Haneul network. When you publish a package, you are actually creating an immutable Haneul object on the network that anyone can access.

To publish your package to the Haneul network, use the `publish` CLI command in the root of your package. Use the `--gas-budget` flag to set a value for the maximum amount of gas the transaction can cost. If the cost of the transaction is more than the budget you set, the transaction fails and your package doesn't publish. 
...
Your currently active address now has three objects (or more, if you had objects prior to this example). Assuming you are using a new address, running the `haneul objects` command reveals what those objects are.
...
Now that the package is on chain, you can call its functions to interact with the package. You can use the `haneul client call` command to make individual calls to package functions, or you can construct more advanced blocks of transactions using the `haneul client ptb` command. The `ptb` part of the command stands for [programmable transaction blocks](../../../concepts/transactions/prog-txn-blocks.mdx). In basic terms, PTBs allow you to group commands together in a single transaction for more efficient and cost-effective network activity.

```mermaid
flowchart TB
    my_module["my_module::new_sword(&Forge, strength, magic)"]
    
    Haneul_client["Haneul client"]
...
After the transaction executes, you can check the status of the `Sword` object by using the `haneul client objects` command again. Provided you used your address as the `<TO-ADDRESS>`, you should now see a total of four objects:
...
Congratulations! You have successfully published a package to the Haneul network and modified the blockchain state by using a programmable transaction block. 

## Related links

- [Debugging](./debug.mdx): Print values to aid in logic debugging.
- [Package Upgrades](../../../concepts/haneul-move-concepts/packages.mdx): Upgrading packages published on the Haneul network.
...
- [Haneul Client PTB CLI](../../../references/cli/ptb.mdx): The `client ptb` command allows you to specify the transactions for execution in a programmable transaction block directly from your CLI or through bash scripts.
