...
description: Install the Haneul framework and required prerequisites on your system, including the Haneul command line interface to interact with the Haneul network.
...
The quickest way to install Haneul is using the binaries delivered with every release. If you require more control over the install process, you can install from source. To take advantage of containerization, you can utilize the Docker images in the `docker` folder of the haneul repository.
...
- Linux - Ubuntu version 22.04 (Jammy Jellyfish) or later
    - The [`haneul-node` binary from AWS](#aws-haneul-node) supports only version 22.04
- macOS - macOS Monterey or later
- Microsoft Windows - Windows 10 and 11

::::info[Fastest method]

## Quick install using Homebrew, Chocolatey, or haneulup {#install-homebrew}

{@include: ../../../snippets/quick-install.mdx}

:::note

If you use this method to install Haneul, you are all set. The quick install is suitable for most use cases. The remaining installation methods are for those wanting more control over the installation process.
...
1. Go to https://github.com/GeunhwaJeong/haneul.
...
1. If running the binaries for the first time, you might receive an error from MacOS that prevents the binaries from running. If you receive this error, close the dialog and type `xattr -d com.apple.quarantine ~/haneul/*` in your terminal and press Enter (be sure to adjust the path if different).
...
Running binaries other than `haneul` might require installing prerequisites itemized in the following section.

:::

#### Confirm the installation {#confirm-the-installation}

To confirm that Haneul installed correctly, type `haneul --version` in your console or terminal and press Enter. The response should provide the Haneul version installed. If the console or terminal responds with a command not found error, make sure the full path to your Haneul binaries is included in your `PATH` variable.

## Install from Cargo {#install-haneul-binaries-from-source}

Run the following command to install Haneul binaries from the `testnet` branch:

```sh
$ cargo install --locked --git https://github.com/GeunhwaJeong/haneul.git --branch testnet haneul --features tracing
```

Enabling the `tracing` feature is important as it adds Move test coverage and debugger support in the Haneul CLI. Without it these two features will not be able to be used.
...
If you previously installed the Haneul binaries, you can update them to the most recent release with the same command you used to install them (changing `testnet` to the desired branch):
...
The `haneul-node` binaries for Ubuntu 22.04 are available for download from AWS. You can use either the commit sha or version tag in the URL to retrieve the specific version of Haneul you want. Use one of these values to construct the AWS download URL.

The URL is in the form `https://haneul-releases.s3-accelerate.amazonaws.com/<SHA-OR-TAG>/haneul-node`, where you replace `<SHA-OR-TAG>` with the proper value. For example, the URL is `https://haneul-releases.s3-accelerate.amazonaws.com/00544a588bb71c395d49d91f756e8bfe96067eca/haneul-node` to download the release with the relevant commit sha. If you visit the URL using a browser, the binary downloads automatically.

After downloading, open a console to the file's location and change its permission to `755`.
```sh
$ chmod 755 haneul-node
```

Add the file's location to your PATH variable if it's directory is not already included. Follow the steps in [Haneul Full Node Configuration](../../operator/haneul-full-node) to complete the setup.

## Build from source

Follow the instructions in this topic to install the Rust crates (packages) required to interact with Haneul networks, including the Haneul CLI.

To install Haneul from source, you first need to install its [prerequisites](#prerequisites) for your operating system. After installing the supporting technologies, you can install [Haneul binaries from source](#install-haneul-binaries-from-source).

You can also download the [source code](/references/contribute/haneul-environment.mdx) to have local access to files.

#### Prerequisites {#prerequisites}

Your system needs the following prerequisites available to successfully install Haneul.
...
With Haneul installed, you can interact with Haneul networks using the Haneul CLI. For more details, see the [Haneul CLI](/references/cli.mdx) reference.

## Installing Haneul developer tools

If you use VSCode, you can install the [Move extension](https://marketplace.visualstudio.com/items?itemName=haneullabs.move) to get language server support for Move, as well as support for building, testing, and debugging Move code within the IDE.
You can install the extension either by searching the fully-qualified extension name, `Haneullabs.move`, in the extension view, or by pressing `Ctrl-P` or `Cmd-P` and typing `ext install haneullabs.move`.
Installing the Move extension also installs the appropriate `move-analyzer` binary for your operating system, as well as the [Move Trace Debugger](https://marketplace.visualstudio.com/items?itemName=haneullabs.move-trace-debug) extension, and [Move Syntax](https://marketplace.visualstudio.com/items?itemName=damirka.move-syntax) extension.
...
Now that you have Haneul installed, it's time to start developing. Check out the following topics to start working with Haneul:

- Read about the [Haneul CLI](/references/cli.mdx), the most straightforward way to start exploring Haneul networks.
