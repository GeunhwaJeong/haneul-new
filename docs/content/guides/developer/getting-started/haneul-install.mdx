...
If you use [Homebrew](https://brew.sh/), you can install Haneul with the following commands:

```bash
brew tap haneullabs/tap
brew install haneullabs/tap/haneul
```
...
1. Open the expanded folder and double-click the appropriate binary to install, beginning with haneul-`<OS>`-`<ARCHITECTURE>`:

    - haneul-faucet-`<OS>`-`<ARCHITECTURE>`: Local faucet to mint coins on local network.
    - haneul-indexer-`<OS>`-`<ARCHITECTURE>`: An indexer for a local Haneul network.
    - haneul-`<OS>`-`<ARCHITECTURE>`: Main Haneul binary.
    - haneul-node-`<OS>`-`<ARCHITECTURE>`: Run a local node.
    - haneul-test-validator-`<OS>`-`<ARCHITECTURE>`: Run test validators on a local network for development.
    - haneul-tool-`<OS>`-`<ARCHITECTURE>`: Provides utilities for Haneul.

    The Haneul binary takes several minutes to download and install files, so make sure you allocate enough time for it to complete. Actual time to complete depends on your network connection and computer specifications.
...
You can also download the [source code](./haneul-environment) to have local access to files.
