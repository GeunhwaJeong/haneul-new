...
The quick install is suitable for most use cases. The remaining installation methods are for those wanting more control over the installation process.
...
:::tip
Running binaries other than `haneul` might require installing prerequisites listed in the following section.
::::
...
:::info
The `tracing` feature enables Move test coverage and debugger support in the Haneul CLI. These features are not available unless you enable tracing.
:::
...
Add the file's location to your PATH variable if its directory is not already included. Follow the steps in [Configure a Haneul full node](../../operator/haneul-full-node.mdx) to complete the setup.
...
## Using Haneul from the command line
...
Now that you have Haneul installed:

- Explore the [Haneul CLI reference](/references/cli.mdx) to learn command line operations.
- [Connect to a network](./connect.mdx) to interact with the Haneul ecosystem.
- [Acquire test coins](./get-coins.mdx) for development purposes.
- [Build your first dApp](../first-app.mdx) using Haneul's programmable transaction model.
