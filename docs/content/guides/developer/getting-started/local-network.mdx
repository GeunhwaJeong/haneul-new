...
Haneul CLI provides the `haneul client faucet` command to get coins from the faucet. In the most basic case, run `haneul client faucet` and wait up to 60 seconds for the coins to reach your wallet. Use `haneul client gas` to check for the new coins.
