...
After the installation completes, run the following command to install Haneul Wallet:
...
You can also use a local Haneul Wallet to test with your local network.

**Note:** To run the command you must have `pnpm` installed. See [Install Haneul Wallet locally](#install-haneul-wallet-locally) for details.
