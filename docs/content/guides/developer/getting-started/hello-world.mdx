...
The most basic unit of storage on Haneul is an [object](/guides/developer/objects/object-model.mdx). Other blockchains typically structure storage using key-value stores. Haneul centers storage around objects with unique ID addresses on-chain. Every Haneul smart contract is an object that manipulates other objects.
