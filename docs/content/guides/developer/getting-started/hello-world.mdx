...
The most basic unit of storage on Haneul is an [_object_](/guides/developer/objects/object-model.mdx). Other blockchains typically structure storage using key-value stores. Haneul centers storage around objects with unique ID addresses on-chain. Every Haneul smart contract is an object that manipulates other objects.
...
[Move](/concepts/haneul-move-concepts.mdx) is the programming language Haneul uses to create smart contracts. It is platform agnostic and enables common libraries, tooling, and developer communities across blockchains with vastly different data and execution models. There are three ways to use Move in the context of Haneul: Move packages, Move modules, and Move objects.

A Haneul **Move package** is also referred to as a Move smart contract. It is a set of Move bytecode published to the Haneul network. It is immutable and cannot be changed or removed, however you can upgrade it. Upgrading creates a new version of the package object on-chain, leaving the original intact. All prior versions of a package still exist on-chain. Once you publish it, other packages can import and use the modules it provides. Anyone can view a package's contents and use a Haneul Explorer to see how its logic manipulates other objects. 

Every Move package on Haneul includes one or more **Haneul Move modules** that define the package's interaction with on-chain objects. A module's name is always unique within the package that contains it.
...
- [x] [Install the latest version of Haneul](/guides/developer/getting-started/haneul-install.mdx).

- [x] [Configure the Haneul client](/guides/developer/getting-started/configure-haneul-client.mdx).

- [x] [Create a Haneul address](/guides/developer/getting-started/get-address.mdx).

- [x] [Get HANEUL Testnet tokens](/guides/developer/getting-started/get-coins.mdx).
...
A unique aspect of programming applications on Haneul is the resource safety enforced by the Move Bytecode Verifier. Move packages must satisfy the following resource safety parameters:
...
You must build your package before you can publish it, but also before you test it. You cannot run tests (`haneul move test`) on your code until it has been built.
...
This should return `testnet`. If it does not return `testnet`, follow the [client configuration instructions](/guides/developer/getting-started/configure-haneul-client.mdx) before continuing.
...
If you do not have a balance, follow the [HANEUL faucet instructions](/guides/developer/getting-started/get-coins.mdx).
...
When you publish a Move package to the network, the network uploads and stores the bytecode as a Move package with a unique package ID and version number. The network consumes HANEUL tokens as gas and processes the transaction on-chain.
