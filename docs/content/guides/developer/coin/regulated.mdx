...
keywords: [ regulated coin list, regulated coins, regulate coin, allow list, deny list, stablecoins, token example, create a token, how to create token, haneul token standards ]
---

You can create regulated coins on Haneul, such as [stablecoins](../stablecoins.mdx). These coins are similar to other coins, like HANEUL, but include the ability to control access to the coin using a deny list. 

When creating standard coins, you call the `create_currency` function in the `coin` package of the Haneul framework, whether directly or via an SDK. When you create regulated coins, you call the `create_regulated_currency_v2` function in that same package instead. The `create_regulated_currency_v2` function actually leverages the `create_currency` function to create the coin, but adds an additional step that produces a `DenyCapV2` and transfers it to the publisher of the regulated coin package. The bearer of the transferrable `DenyCapV2` object can control access to the coin through a deny list. 

## DenyList

The `DenyList` is a singleton, shared object that the bearer of a `DenyCapV2` can access to specify a list of addresses that are unable to use a Haneul core type. The initial use case for `DenyList`, however, focuses on limiting access to coins of a specified type. This is useful when creating a regulated coin on Haneul that requires the ability to block certain addresses from using it as inputs to transactions. Regulated coins on Haneul satisfy any regulations that require the ability to prevent known bad actors from having access to those coins.
...
To learn about the features available, see the [Coin standard](../../../standards/coin.mdx) documentation and the `coin` module in the <a href="/references/framework/haneul/coin" data-noBrokenLinkCheck='true'>Haneul framework</a>.

## Regulated coin example

The regulated coin example is in the `examples/regulated-coin` directory of the Haneul repo. The example provides both TypeScript- and Rust-based command line access to an on-chain package that demonstrates some of the features of regulated coins on Haneul.

## Prerequisites

This topic assumes you are accessing the code from your own fork of the Haneul repo. To run the example project, you must have [Haneul installed](../getting-started/haneul-install.mdx).

You need at least one Haneul address to publish the contract to the network. At least one additional address is helpful if you want to transfer or test the deny list capability for the regulated coins.

You do not need a Haneul wallet to use this project, but having one available might help you visualize results.

This example assumes you're familiar with publishing packages on Haneul and the Move language. For more detailed guides on example dApps, see [App Examples](../app-examples.mdx). For more more information on the Move language, see [The Move Book](https://move-book.com/).
...
| --- | --- |
| `PACKAGE_ID` | Object ID of the package you publish. This data is part of the response that Haneul provides on publish. |
| `ADMIN_SECRET_KEY` | The secret key for the address that publishes the package. You can use `haneul keytool export --key-identity <HANEUL-ADDRESS>` or a wallet UI to get the value. Take care not to expose the value to the public. |
| `ADMIN_ADDRESS` | The address that publishes the contract. |
| `DENY_CAP_ID` | Deny capability object ID. This data is part of the response that Haneul provides on publish. |
| `TREASURY_CAP_ID` | The treasury cap object ID that allows the bearer to mint new coins. This data is part of the response that Haneul provides on publish. |
| `MODULE_NAME` | The name of the module you publish. |
| `COIN_NAME` | The name of your regulated coin.  |
| `HANEUL_FULLNODE_URL` | The URL to the Full node network that will process transactions. For Testnet this value is `https://fullnode.testnet.haneul.io:443`. |
...
For all `Coin` functions available, see the Haneul framework [`coin` module documentation](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/coin.md). The following functions are the most common. 

<details>
<summary>
  `coin::mint<T>`
</summary>
{@inject: crates/haneul-framework/packages/haneul-framework/sources/coin.move#fun=mint}
</details>
<details>
<summary>
  `coin::mint_balance<T>`
</summary>
{@inject: crates/haneul-framework/packages/haneul-framework/sources/coin.move#fun=mint_balance}
</details>
<details>
<summary>
  `coin::mint_and_transfer<T>`
</summary>
{@inject: crates/haneul-framework/packages/haneul-framework/sources/coin.move#fun=mint_and_transfer}
</details>
<details>
<summary>
  `coin::burn<T>`
</summary>
{@inject: crates/haneul-framework/packages/haneul-framework/sources/coin.move#fun=burn}
</details>
...
- [Source code](https://github.com/GeunhwaJeong/haneul/tree/main/examples/regulated-coin): The source code in GitHub for this example.
- [In-Game Tokens](./in-game-token.mdx): Example of how to create tokens for use as in-game currency.
- [Loyalty Tokens](./loyalty.mdx): Example of how to create tokens that reward brand or service loyalty on the Haneul network.
