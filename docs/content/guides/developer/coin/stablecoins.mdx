---
title: Stablecoins on Haneul
slug: /guides/developer/stablecoins
...
On Haneul, you can interact with various stablecoins such as USDC, USDT, Agora, and Ondo USDY.
...
For more detailed information on how to interact with USDC on Haneul, refer to the [USDC guide](#usdc-guide).
...
USDT is currently not issued natively on Haneul. For more information on bridging USDT to Haneul, refer to [HANEUL Bridging](/concepts/tokenomics/haneul-bridging.mdx).
...
## How to use USDC on Haneul {#usdc-guide}

:::info
While this example uses USDC, the same principles can be applied to any asset on Haneul that uses the [Haneul Currency Standard](/standards/currency.mdx).
:::
...
The USDC stablecoin source code is available in the [circlefin/stablecoin-haneul](https://github.com/circlefin/stablecoin-haneul/blob/master/packages/usdc/sources/usdc.move) repository.
...
:::warning
The `usdc` package uses a specific version of the `haneul` package, which causes a version conflict with the `Haneul` package in the Haneul framework. You can override the version of the `Haneul` package in your `Move.toml` file to use a different version. Add the `override = true` flag to the `Haneul` package in your `Move.toml` file.
:::

### Using USDC in Move

USDC uses the [Haneul Currency Standard](/standards/currency.mdx) and can be used just like any other coin type in the Haneul framework.
...
Then use the `USDC` type just as you would use the `HANEUL` type when accepting a `Coin<HANEUL>` parameter.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" fun="buy_sword_with_usdc" />

The following example demonstrates how to use the USDC stablecoin in a Move package and how it relates to using the `HANEUL` type as well as any generic coin types.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" />

### Using USDC in PTBs

Use USDC in your [PTBs](/guides/developer/transactions/building-ptb) just like any other coin type.
...
:::info
`0xa1ec7fc00a6f40db9693ad1415d0c193ad3906494428cf252621037bd7117e29` is the [Testnet address](https://haneulscan.xyz/testnet/coin/0xa1ec7fc00a6f40db9693ad1415d0c193ad3906494428cf252621037bd7117e29::usdc::USDC) for USDC, while `0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7` is the [Mainnet address](https://haneulscan.xyz/coin/0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7::usdc::USDC).
:::
