...
description: A quick reference document regarding best practices for Haneul developers.
...
- Follow the [Move conventions](/concepts/haneul-move-concepts/conventions.mdx) for consistent naming and coding style.
...
- If your function `f` needs a payment in, for example, HANEUL from the caller, use `fun f(payment: Coin<HANEUL>)` not `fun f(payment: &mut Coin<HANEUL>, amount: u64)`. This is safer for callers; they know exactly how much they are paying, and do not need to trust `f` to extract the right amount.

### Composability

- Use the [Haneul Object Display](https://move-book.com/programmability/display) to customize how your objects show up in wallets, apps, and explorers.
- Avoid “self-transfers." When possible, return the object from the current function, so it can be used in a different command in a [programmable transaction block](/guides/developer/haneul-101/working-with-ptbs.mdx).

### Package upgrades

- Read about [package upgrades](../../concepts/haneul-move-concepts/packages/upgrade.mdx) before publishing your package.
- Packages are immutable, so any published package can be called forever. Use [object versioning](/concepts/haneul-move-concepts/packages/upgrade.mdx#versioned-shared-objects) to prevent older versions from being called.
...
- Use the [`haneul::test_scenario`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/test/test_scenario.move) module to mimic multi-transaction, multi-sender test scenarios.
- Use the [`std::unit_test`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/move-stdlib/sources/unit_test.move) module for `assert_eq!` and `assert_ref_eq!` macros for better test error messages.
- Use the [`haneul::test_utils`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/test/test_utils.move#L5) module for black-hole function `destroy`.
- Use the [`std::debug`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/move-stdlib/sources/debug.move) module for debug printing via `print`.
...
- Haneul supports several signature schemes for transaction signing, including native multisig.
