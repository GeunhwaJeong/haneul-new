...
description: Represent real-world assets as on-chain fractional tokens using the tokenized_asset module on Haneul.
...
On Haneul, the `tokenized_asset` module handles this. It operates similarly to the [`coin` library](/references/framework/haneul-framework/coin): when it receives a new [One-Time Witness (OTW)](https://move-book.com/programmability/one-time-witness/) type, it creates a unique on-chain representation of a fractional asset. Each fraction is a `TokenizedAsset<T>` object with a balance that counts toward the asset's total supply.
...
:::info
The packages described in this document use the [Kiosk standard](/standards/kiosk) to ensure that tokenized assets operate within their defined transfer policy. This supports marketable tokenized assets with rules like royalties and commissions.
...
The `tokenized_asset` module is the core of the implementation. It handles asset creation, minting, splitting, joining, and burning. It operates similarly to the [`coin` library](/references/framework/haneul-framework/coin) on Haneul:
...
- Claim the [`Publisher`](/references/framework/haneul-framework/package) object using a One-Time Witness and store it in a shared `Registry`
- Create a [`TransferPolicy`](/standards/kiosk) and `TransferPolicyCap` for a given `TokenizedAsset<T>` type using `setup_tp`
- Create an empty [`Display`](/references/framework/haneul-framework/display) for a `TokenizedAsset<T>` type using `new_display`
...
For more information on WASM bytecode manipulation and how to deploy, see [Deploy a Tokenized Asset](/guides/developer/digital-assets/non-fungible-tokens/deploy-tokenized-asset.mdx).
