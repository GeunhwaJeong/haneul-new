...
On Haneul, the `tokenized_asset` module handles this. It operates similarly to the [`coin` library](/references/framework/haneul_haneul/coin): when it receives a new [One-Time Witness (OTW)](https://move-book.com/programmability/one-time-witness/) type, it creates a unique on-chain representation of a fractional asset. Each fraction is a `TokenizedAsset<T>` object with a balance that counts toward the asset's total supply.
