...
Regulated currencies are assets with access control managed through a **deny list**. A deny list identifies addresses that cannot send or recieve that asset. For example, stablecoins are a type of regulated currency.
...
To learn about available features, see [Currency Standard](/standards/currency.mdx) and the `coin_registry` module in the Haneul framework.

## Create regulated currency

To use regulatory features, you will need to [create a currency](/guides/developer/digital-assets/fungible-tokens/create-a-fungible-tokens). During currency initialization, use the `make_regulated()` function on the `CurrencyInitializer<T>` before calling `finalize()`. This adds deny list capabilities to the `Currency<T>` and tracks the regulatory status within the registry system:
