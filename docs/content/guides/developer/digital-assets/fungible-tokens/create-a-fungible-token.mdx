...
draft: true
description: Learn how to create currencies and mint coins and tokens on the Haneul network.
...
Assets created using the Currency Standard are free-flowing, [wrappable](/guides/developer/objects/object-ownership/wrapped.mdx), [freely transferable](/guides/developer/objects/transfers/custom-rules.mdx) fungible assets that you can store in any app. The Currency Standard uses the [`haneul::coin_registry`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/docs/haneul/coin_registry.md) module and the `Currency` object for metadata.

The `Currency<T>` type represents open-loop fungible tokens. Currencies are denominated by their type parameter `T`, which is also associated with metadata (like name, symbol, and decimal precision) that applies to all instances of `Currency<T>`. The `haneul::coin_registry` module exposes an interface over `Currency<T>` that treats it as fungible, meaning that a unit of `T` held in one instance of `Currency<T>` is interchangeable with any other unit of `T`, similar to how traditional fiat currencies operate.
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" struct="CurrencyInitializer" />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="finalize" signatureOnly />
...
<ImportContent source="crates/haneul-framework/packages/haneul-framework/sources/registries/coin_registry.move" mode="code" fun="finalize_registration" signatureOnly />