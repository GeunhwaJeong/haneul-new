...
description: Migrating from Coin objects to address balances on Haneul.
keywords:
  [
    address balances,
    migration,
...
Address balances introduce a canonical balance system for fungible assets tied to Haneul addresses. This replaces the UTXO-style `Coin<T>` model with direct address-owned balances, simplifying transaction construction and eliminating coin selection complexity.

For the full specification, see [SIP-58: Haneul Address Balances](https://github.com/haneul-foundation/sips/blob/main/sips/sip-58.md).
...
- No contracts need to be re-written. Contracts can continue to accept `Coin<T>` and `&mut Coin<T>` as before.
- Coins can still be sent via `transfer::public_transfer` or the `TransferObjects` command.
...
Initially, this will not happen often, as most of the ecosystem will continue to transfer coin objects, just as they do today.
But, since the possibility exists that funds can arrive at any wallet as an address balance transfer, wallet implementors should be prepared for this.
...
- Direct gas payment from HANEUL address balances.
...
[TypeScript SDK - `coinWithBalance`](https://haneul-typescript-docs-git-mh-address-balance-docs-haneullabs-labs.vercel.app/haneul/transactions/intents#coinwithbalance):
...
- `balance` (required): Amount in base units (GEUNHWA for HANEUL).
- `type` (optional): Coin type, defaults to `0x2::haneul::HANEUL`.
- `useGasCoin` (optional): Whether to split from gas coin for HANEUL (default `true`). Set to `false` for sponsored transactions.
- `forceAddressBalance` (optional): Force address balance withdrawals, skipping balance/coin lookups.
...
[TypeScript SDK - Transaction Reference](https://haneul-typescript-docs-git-mh-address-balance-docs-haneullabs-labs.vercel.app/haneul/transactions/reference#address-balance-withdrawals):
...
// Withdraw HANEUL from address balance and redeem to Coin
const [coin] = tx.moveCall({
	target: '0x2::coin::redeem_funds',
	typeArguments: ['0x2::haneul::HANEUL'],
	arguments: [tx.withdrawal({ amount: 1_000_000_000 })], // 1 HANEUL in GEUNHWA
});
...
- `type` (optional): Coin type, defaults to HANEUL.
...
[Source: `haneul-types/src/transaction.rs` - `FundsWithdrawalArg`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/transaction.rs):

```rust
use haneul_types::transaction::{FundsWithdrawalArg, WithdrawalTypeArg, Reservation, WithdrawFrom};
...
[Source: `haneul-types/src/programmable_transaction_builder.rs`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/programmable_transaction_builder.rs):
...
[Source: `haneul-framework/sources/funds_accumulator.move`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/funds_accumulator.move):
...
[Source: `haneul-framework/sources/coin.move` - `send_funds`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/coin.move#L176):

```move
// After: In Move contract
public fun send_payment(coin: Coin<HANEUL>, recipient: address) {
...
[Source: `haneul-framework/sources/balance.move` - `send_funds`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/balance.move#L102):
...
[Haneul CLI Reference - PTB](/references/cli/ptb):
...
haneul client ptb \
    ...
    --assign coin \
    --move-call 0x2::coin::send_funds '<0x2::haneul::HANEUL>' coin @<recipient_address>
...
### Checking if address balance gas payments are enabled
...
[Source: `haneul-types/src/transaction.rs` - `TransactionExpiration::ValidDuring`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/transaction.rs):
...
[TypeScript SDK - Core API](https://sdk.haneul-labs.com/haneul/clients/core):

```typescript
// Before: Get balance (RPC sums all coin objects)
const balance = await client.core.getBalance({ owner: address, coinType: '0x2::haneul::HANEUL' });
console.log(balance.totalBalance); // Sum of all Coin<HANEUL> objects
```
...
[JSON-RPC Reference - `haneulx_getBalance`](/haneul-api-ref#haneulx_getbalance):

```bash
curl -s https://fullnode.testnet.haneul.io \
    -H 'Content-Type: application/json' \
    -d '{"jsonrpc":"2.0","id":1,"method":"haneulx_getBalance","params":["<address>", "0x2::haneul::HANEUL"]}'
```
...
curl -s https://fullnode.testnet.haneul.io \
    ...
    -d '{
        ...
        "params":["<address>", "0x2::haneul::HANEUL"]
    }'
```
...
[JSON-RPC Reference - `haneulx_getAllBalances`](/haneul-api-ref#haneulx_getallbalances):
...
[gRPC Reference - `GetBalanceRequest`](/references/fullnode-protocol#haneul-rpc-v2-GetBalanceRequest):

```bash
buf curl --protocol grpc https://fullnode.testnet.haneul.io/haneul.rpc.v2.StateService/GetBalance \
    -d '{
        "owner": "<address>",
...
[gRPC Reference - `ListBalancesRequest`](/references/fullnode-protocol#haneul-rpc-v2-ListBalancesRequest):

```bash
# List all balances
buf curl --protocol grpc https://fullnode.testnet.haneul.io/haneul.rpc.v2.StateService/ListBalances \
    -d '{"owner": "<address>"}'
```

### GraphQL

[GraphQL Reference - `IAddressable.balance`](/references/haneul-api/haneul-graphql/beta/reference/types/interfaces/iaddressable) |
Copy the examples below into [GraphQL IDE (Testnet)](https://graphql.testnet.haneul.io/graphql) to try them out.
...
) -> BTreeMap<(HaneulAddress, TypeTag), i128> {
...
[Source: `haneul-types/src/balance_change.rs`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/balance_change.rs):

```rust
// After: Include accumulator events
use haneul_types::balance_change::{derive_balance_changes, BalanceChange};
...
[Reference implementation](https://github.com/GeunhwaJeong/haneul/blob/e23cc4bb0215ec065785a19772f22eda8ada14ae/crates/haneul-types/src/balance_change.rs#L86)

### Accessing accumulator events

Accumulator events are embedded in `TransactionEffects`.

[Source: `haneul-types/src/effects/mod.rs` - `TransactionEffectsAPI`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-types/src/effects/mod.rs):
...
haneul client ptb \
    ...
    --move-call 0x2::coin::send_funds '<0x2::haneul::HANEUL>' @<coin1> @<your_address>
```
...
[Source: `haneul-framework/sources/balance.move`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/balance.move):
...
[Source: `haneul-framework/sources/coin.move`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/haneul-framework/sources/coin.move):
