...
description: Use the asset tokenization packages to create fractional NFTs and fungible tokenized assets on Haneul, with support for Kiosk-based transfer policies.
keywords: [asset tokenization, fractional NFT, tokenized asset, kiosk, transfer policy, WASM, bytecode template]
---

This reference implementation uses the [Kiosk standard](/standards/kiosk.mdx) to ensure that tokenized assets operate within their defined policy. Use the implementation as presented to have marketable tokenized assets that support rules like royalties and commissions.
