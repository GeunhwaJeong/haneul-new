...
Renting non-fungible tokens (NFTs) is a mechanism that allows individuals to temporarily use the benefits of an NFT without ownership or possession. This implementation uses the [Kiosk Apps standard](/standards/kiosk-apps.mdx) to establish an infrastructure for rental transactions. This approach aligns with the [Ethereum ERC-4907](https://eips.ethereum.org/EIPS/eip-4907) renting standard, making it a suitable choice for Solidity-based use cases intended for implementation on Haneul.
...
This NFT rental example uses a single module, [`nft_rental.move`](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/nft-rental/sources/nft_rental.move).
...
The rental smart contract uses the [Kiosk Apps](/standards/kiosk-apps.mdx) standard. Both the lender and borrower must install a kiosk extension to take part. The creator of the borrowed asset type must create a rental policy and `ProtectedTP` object to allow the extension to manage rentals while enforcing royalties.
...
[View the full example on GitHub](https://github.com/GeunhwaJeong/haneul/tree/main/examples/move/nft-rental/sources/nft_rental.move).