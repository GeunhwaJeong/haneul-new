...
In this example, the Admin sends `LOYALTY` tokens to the users of your service as a reward for their contiunued buisness. The example creates a `GiftShop` where holders can spend `LOYALTY` tokens to buy `Gift`s.
