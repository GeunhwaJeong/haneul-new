...
Fungible tokens created on Haneul using the Coin or Currency standard are referred to as _coins_. 

For fungible tokens created on Haneul using the [Closed-Loop Token standard](/standards/closed-loop-token.mdx), the documentation uses the term _tokens_. In practice, the terms for both these objects are often interchangeable.
...
PAS uses Haneul [Address Balances](/guides/developer/address-balances-migration). PAS balances are managed by PAS on behalf of the wallet address. They are held in a derived Account object, however, transfers always target the wallet address, not the Account address. 
