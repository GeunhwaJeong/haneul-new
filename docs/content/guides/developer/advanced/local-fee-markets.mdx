...
1. Estimates each transaction's execution cost using the [`TotalGasBudgetWithCap`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-core/src/authority/shared_object_congestion_tracker.rs#L143) heuristic.
...
Haneul limits the per-commit execution capacity of each shared object. If transactions touching a shared object have a low estimated cost, more of them can fit. If they have a high estimated cost, not as many can fit.  
...
At the protocol level, Haneul is configured with a fixed per-commit limit and burst capacity for each shared object. On average, a shared object's activity cannot exceed the per-commit limit. However, short bursts of traffic might exceed the limit temporarily.
