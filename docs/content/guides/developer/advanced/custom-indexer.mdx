...
description: Establishing a custom indexer helps improve latency, allows pruning the data of your Haneul full node, and provides efficient assemblage of checkpoint data.
...
{@inject: examples/rust/basic-haneul-indexer/Cargo.toml}
...
{@inject: examples/rust/basic-haneul-indexer/diesel.toml}
...
{@inject: examples/rust/basic-haneul-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql}
...
{@inject: examples/rust/basic-haneul-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql}
...
{@inject: examples/rust/basic-haneul-indexer/src/schema.rs}
...
{@inject: examples/rust/basic-haneul-indexer/src/models.rs}
...
{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs#struct=TransactionDigestHandler}
...
{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs#processordeps}

After the `TransactionDigestHandler` struct, add the `Processor` code:

{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs#processor}
...
{@inject: crates/haneul-indexer-alt-framework/src/pipeline/processor.rs#trait=Processor}
...
{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs#handlerdeps}

Add the logic for `Handler` after the `Processor` code. The complete code is available at the end of this step.

{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs#handler}
...
{@inject: ../../../../../crates/haneul-indexer-alt-framework/src/pipeline/sequential/committer.rs#batch}
...
{@inject: examples/rust/basic-haneul-indexer/src/handlers.rs}
...
{@inject: crates/haneul-indexer-alt-framework/src/pipeline/concurrent/mod.rs#trait=Handler}
...
{@inject: examples/rust/basic-haneul-indexer/src/main.rs}
...
- [Haneul Indexer Alt](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-indexer-alt): The `haneul-indexer-alt` crate in the Haneul repo.
- [Move Registry](https://github.com/GeunhwaJeong/mvr/tree/main/crates/mvr-indexer): The indexer that the Move Registry (MVR) implements.
- [DeepBook Indexer](https://github.com/GeunhwaJeong/deepbookv3/tree/main/crates/indexer): The indexer that DeepBook implements.
- [Custom Indexing Framework](../../../concepts/custom-indexing-framework.mdx): The `haneul-indexer-alt-framework` is a powerful Rust framework for building high-performance, custom blockchain indexers on Haneul, providing customizable, production-ready components for data ingestion, processing, and storage.
- [Indexer Pipeline Architecture](../../../concepts/custom-indexer/pipeline-architecture.mdx): The `haneul-indexer-alt-framework` provides two distinct pipeline architectures. Understand the differences between the sequential and concurrent pipelines that the `haneul-indexer-alt-framework` provides to decide which best haneults your project needs.
