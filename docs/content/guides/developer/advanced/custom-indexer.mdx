...
description: You can build custom indexers using the Haneul micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. Establishing a custom indexer helps improve latency, allows pruning the data of your Haneul Full node, and provides efficient assemblage of checkpoint data.
---

You can build custom indexers using the Haneul micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. This stream can be one of the publicly available streams from Haneul Labs, one that you set up in your local environment, or a combination of the two.  

Establishing a custom indexer helps improve latency, allows pruning the data of your Haneul Full node, and provides efficient assemblage of checkpoint data.
...
The most straightforward stream source is to subscribe to a remote store of checkpoint contents. Haneul Labs provides the following buckets:

- Testnet: `https://storage.googleapis.com/haneullabs-testnet-checkpoints`
- Mainnet: `https://storage.googleapis.com/haneullabs-mainnet-checkpoints`

The checkpoint files are stored in the following format: `https://storage.googleapis.com/haneullabs-testnet-checkpoints/<checkpoint_id>.chk`. You can download the checkpoint file by sending an HTTP GET request to the relevant URL. Try it yourself for checkpoint 1 at [https://storage.googleapis.com/haneullabs-testnet-checkpoints/1.chk](https://storage.googleapis.com/haneullabs-testnet-checkpoints/1.chk).
...
The Haneul data ingestion framework provides a helper function to quickly bootstrap an indexer workflow.
...
To enable, add the following to your [Full node configuration](../../operator/haneul-full-node.mdx) file:
...
executor.run(
    ...
    Some("https://storage.googleapis/haneullabs-testnet-checkpoints".to_string()), // Remote Checkpoint Store
...
Find the following source code in the [Haneul repo](https://github.com/GeunhwaJeong/haneul/tree/main/examples/custom-indexer/rust). 
...
- [Haneul internal example](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneul-data-ingestion/src/): Haneul data ingestion daemon that runs internal pipelines.
- [Production example](https://github.com/GeunhwaJeong/haneul/tree/main/crates/haneulns-indexer/src): Haneul Name Service custom indexer.
- [Using Events](../haneul-101/using-events.mdx): Events in Haneul enable you to monitor on-chain activity in near-real time when coupled with a custom indexer.
