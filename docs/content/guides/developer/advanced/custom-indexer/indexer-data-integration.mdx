...
The `haneul-indexer-alt-framework` supports three data sources for accessing Haneul blockchain data. For all data sources, the [indexing framework](/concepts/data-access/custom-indexing-framework.mdx) is using a polling technique to check for new checkpoints. Data sources are configured through command-line arguments.
