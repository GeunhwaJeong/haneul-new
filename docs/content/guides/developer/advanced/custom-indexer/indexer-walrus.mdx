...
For uniqueness, each blob uploaded to Walrus also creates a corresponding [`Blob` NFT object on Haneul](https://docs.wal.app/dev-guide/haneul-struct.html#blob-and-storage-objects) with a unique ID. Furthermore, the associated `Blob` object can optionally have a `Metadata` [dynamic field](../../../../concepts/dynamic-fields.mdx). 
...
- [Build Your First Indexer](../custom-indexer.mdx): Establishing a custom indexer helps improve latency, allows pruning the data of your Haneul full node, and provides efficient assemblage of checkpoint data.
- [Dynamic Fields](../../../../concepts/dynamic-fields.mdx): Dynamic fields and dynamic object fields on Haneul are added and removed dynamically, affect gas only when accessed, and store heterogeneous values.
- [Indexer with Walrus example files](https://github.com/GeunhwaJeong/haneul/tree/main/examples/rust/walrus-attributes-indexer): Directory in the Haneul repo containing the files for this example.
