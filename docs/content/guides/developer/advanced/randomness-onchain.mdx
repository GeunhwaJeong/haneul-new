...
`Random` has a reserved address `0x8`. See <a href="/references/framework/haneul-framework/random" data-noBrokenLinkCheck='true'>random.move</a> for the Move APIs for accessing randomness on Haneul.
...
entry fun insecure_play(r: &Random, payment: Coin<HANEUL>, ...) {
  ...
...
- For a complete list of the current limits defined in the Protocol Config, see [this](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-protocol-config/src/lib.rs#L731).
...
See [this](https://github.com/GeunhwaJeong/haneul/blob/main/examples/move/random/random_nft/sources/example.move#L117-L142) for example implementation.
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<HANEUL>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<HANEUL>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
...
To protect against PTB-based composition attacks, Haneul rejects PTBs that have commands that are not `TransferObjects` or `MergeCoins` following a `MoveCall` command that uses `Random` as an input.
...
- [Haneul Framework Reference](../../../references/framework): The Haneul framework libraries include Move modules that provide the logic for Haneul and its standards. A Rust process creates the documentation for the modules directly from comments in the code.
  - <a href="/references/framework/haneul-framework/random" data-noBrokenLinkCheck="true">random.move</a>: Documentation for the `random` module.
- [Raffle example](https://github.com/GeunhwaJeong/haneul/blob/main/examples/move/random/raffles): GitHub repo containing a raffle smart contract demonstrating randomness.
- [Haneul Client CLI](../../../references/cli/client): Haneul provides command line tools to interact with the network, its features, and the Move programming language. Individual command groups are referred to as Haneul Client CLI, Haneul Keytool CLI, Haneul Move CLI, and Haneul Validator CLI.
