...
`Random` has a reserved address `0x8`. See <UnsafeLink href="/references/framework/haneul-framework/random">random.move</UnsafeLink> for the Move APIs for accessing randomness on Haneul.
...
{@inject: crates/haneul-protocol-config/src/lib.rs#struct=ProtocolConfig}
...
{@inject: examples/move/random/random_nft/sources/example.move#fun=reveal_alternative2_step1,reveal_alternative2_step2}
...
Write the function so the main processing path does the heavy work, while early-exit paths return quickly. Keep the following in mind:
  ...
  - Use [`profile-transaction`](../../../references/cli/client.mdx#profile-a-transaction) to benchmark the costs of a transaction.
...
- [Haneul Framework Reference](../../../references/framework): The Haneul framework libraries include Move modules that provide the logic for Haneul and its standards. A Rust process creates the documentation for the modules directly from comments in the code.
  - <UnsafeLink href="/references/framework/haneul-framework/random">random.move</UnsafeLink>: Documentation for the `random` module.
