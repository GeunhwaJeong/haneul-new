...
description: Learn how to tokenize assets on the Haneul blockchain. Asset tokenization refers to the process of representing real-world assets, such as real estate, art, commodities, stocks, or other valuable assets, as digital tokens on the blockchain network.
---
...
This pattern is similar to the [ERC1155](https://eips.ethereum.org/EIPS/eip-1155) multi-token standard with additional functionality. This makes it a suitable choice for Solidity based use cases that one might want to implement on Haneul.
...
As with all smart contracts on Haneul, Move provides the logic that powers asset tokenization.

### asset_tokenization package

:::info

This reference implementation uses the Kiosk primitive to ensure that TAs created operate within their creator's defined policy. We recommend you use the implementation as it is in order to have marketable TAs that support Rules (e.g: royalties, commissions etc).
If operating within the Kiosk primitive is not a requirement, then the unlock module & some of the proxy's methods related to Transfer Policies can be excluded.
...
- `Proxy`
  ...
  The PROXY struct represents the one-time witness (OTW) utilized to claim the publisher.
...
- `Registry`
  ...
  This is a shared object serves as a repository for the Publisher object, specifically intended to control and restrict access to the creation and management of transfer policies for tokenized assets. Mutable access to this object is exclusively granted to the actual publisher.
...
- `ProtectedTP`
  This is a shared object utilized to store an empty transfer policy. It is required to create one per type `<T>` generated by a user. Its involvement is apparent in the unlock module.
...
- `new_display`
  ...
  This function utilizes the publisher nested within the registry in and the sender's publisher to generate and return an empty Display for the type `TokenizedAsset<T>`, where 'T' is encapsulated within the Publisher object.
...
- `prove_join`
  ...
  A function utilized to demonstrate that the unlocked tokenized asset has been successfully burned and its balance has been incorporated into an existing tokenized asset.
...
- `asset_from_kiosk_to_burn`
  ...
  This helper function is intended to facilitate the burning of tokenized assets locked in kiosk. It assists in their unlocking while ensuring a promise that the circulating supply will be reduced, achieved by returning a `BurnPromise`.
...
- `prove_burn`
  ...
  This function ensures that the circulating supply of the asset cap has indeed been reduced by the balance of the burned tokenized asset.
...
This is similar to the launchpad approach and will serve as the template package whenever a new asset requires representation as a tokenized asset.
...
```
HANEUL_NETWORK = rpc endpoint of the network of your choice | auto filled by publish script
...
For more details on publishing, please check the setup folder's [README](https://github.com/GeunhwaJeong/asset-tokenization/tree/main/setup).
...
The fields that are automatically filled are: `HANEUL_NETWORK`, `ASSET_TOKENIZATION_PACKAGE_ID` and `REGISTRY`.
...
For more details regarding this process, please consult the setup folder's [README](https://github.com/GeunhwaJeong/asset-tokenization/tree/main/setup).
...
You can find a public facing reference to the WASM library in the [move-binary-format-wasm](https://github.com/GeunhwaJeong/haneul/tree/main/sdk/move-binary-format-wasm) Haneul repo subfolder.
...
Copy the output you receive and paste it in the return instruction of the `getBytecode` method, which is located inside the [bytecode-template.ts](https://github.com/GeunhwaJeong/asset-tokenization/blob/main/setup/src/utils/bytecode-template.ts) file.
...
The output format is similar to the template module but smaller in length. Similarly to what you did with the template module, you need to copy this output but this time paste it in the bytecode constant variable located in the [genesis_bytecode.ts](https://github.com/GeunhwaJeong/asset-tokenization/blob/main/setup/src/utils/genesis_bytecode.ts) file.
...
const tx = new Transaction();
  ...
  const [upgradeCap] = tx.publish({
    ...
    dependencies: [
      normalizeHaneulObjectId("0x1"),
      normalizeHaneulObjectId("0x2"),
  ...
  tx.transferObjects(
    [upgradeCap],
    tx.pure(signer.getPublicKey().toHaneulAddress(), "address")
  );
...
```
...
- **Create Transfer Policy**
  ...
  By searching the transaction digest on a Haneul network explorer, you can locate the created objects. Subsequently, select and save the `TransferPolicy ID` and the `ProtectedTP ID` from these objects into the respective fields within your .env file.
