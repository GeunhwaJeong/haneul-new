...
Best practices for writing Move smart contracts on Haneul.
...
- Use [Haneul Object Display](https://move-book.com/programmability/display) to customize how your objects appear in wallets, apps, and explorers.
...
- Use the [`std::debug`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/move-stdlib/sources/debug.move) module for debug printing through `print`.
...
- **Never** sign two concurrent transactions that touch the same owned object. Either use independent owned objects, or wait for one transaction to conclude before sending the next one. Violating this rule might lead to client [equivocation](/references/haneul-glossary#equivocation), which locks the owned objects involved in the two transactions until the end of the current epoch.
- Any `haneul client` command that crafts a transaction (for example, `haneul client publish` or `haneul client call`) can accept the `--serialize-output` flag to output a base64 transaction for signing.
