...
<CodeWalkthrough source="/move/hello-world/sources/greeting.move" org="HaneulLabs" repo="haneul-stack-hello-world" language="move">
  ...
  <Step lines="8-12" title="`Greeting` struct">
    The `Greeting` struct is a Haneul object (it has the `key` ability and a `UID` field). It contains a `text` field that stores a string. A struct with `key` is a resource type, meaning it cannot be copied and must be explicitly moved or destroyed.
  </Step>
