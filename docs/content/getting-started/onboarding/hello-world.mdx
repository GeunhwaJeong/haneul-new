...
description: Build and publish your first Move package using a basic 'Hello, World!' example.
keywords: [ first dapp, first package, move package, first move package, hello world, hello, world, what is move, what is a package, what is an object, move object, haneul move object, haneul move package, haneul move module, move module, build package, publish package, call package, make call to package, how to call package ]
pagination_prev: null
pagination_next: null
---

<AgentPrompt
  prompt={"Clone the https://github.com/GeunhwaJeong/haneul-stack-hello-world example app, build and publish the Move package at move/hello-world to Testnet, then call its entry function and show me the resulting object and transaction on HaneulVision."}
/>

You'll build a "Hello, World!" program to learn the fundamentals of programming on Haneul. You create programs on Haneul by writing and deploying smart contracts to the network.

The most basic unit of storage on Haneul is an [object](/develop/haneul-architecture/object-model). Other blockchains typically structure storage using key-value stores. Haneul centers storage around objects with unique ID addresses onchain. Every Haneul smart contract is an object that manipulates other objects.
...
- **Mutable** objects can be transferred, changed, and deleted. A Haneul address can own them, or they can be shared for public access.
...
Haneul has limits on the maximum transaction size (128KB) and number of objects (2,048) used in a transaction. For more information on limits, see [Building Against Limits](https://move-book.com/guides/building-against-limits/) in The Move Book.

## What is Move? 

[Move](/develop/write-move/haneul-move-concepts) is the programming language Haneul uses to create smart contracts. It is platform agnostic and enables common libraries, tooling, and developer communities across blockchains with vastly different data and execution models. There are three ways to use Move in the context of Haneul: Move packages, Move modules, and Move objects.

A Haneul **Move package** is also referred to as a Move smart contract. It is a set of Move bytecode published to the Haneul network. It is immutable and cannot be changed or removed, but you can upgrade it. Upgrading creates a new version of the package object onchain, leaving the original intact. All prior versions of a package still exist onchain. Once you publish it, other packages can import and use the modules it provides. Anyone can view a package's contents and use a Haneul Explorer to see how its logic manipulates other objects. 

Every Move package on Haneul includes one or more **Haneul Move modules** that define the package's interaction with onchain objects. A module's name is always unique within the package that contains it.

A Haneul Move module governs a Haneul **Move object**, which is typed data from a Haneul Move package. Each Move object value is a struct with fields that can contain primitive types, such as integers and addresses, other objects, and non-object structs.
...
To demonstrate creating objects, packages, and how to build your first Haneul application, start by cloning the "Hello, World!" example:

```
$ git clone \
  https://github.com/GeunhwaJeong/haneul-stack-hello-world.git
$ cd haneul-stack-hello-world/move/hello-world
```
...
<ImportContent source="/move/hello-world/Move.toml" mode="code" org="HaneulLabs" repo="haneul-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" org="HaneulLabs" repo="haneul-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" struct="Greeting" org="HaneulLabs" repo="haneul-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" fun="new" org="HaneulLabs" repo="haneul-stack-hello-world" />

Lastly, the package defines a function called `update_text` that can be called to update the text stored in `Greeting`:

<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" fun="update_text" org="HaneulLabs" repo="haneul-stack-hello-world" />

### Resource safety 

A unique aspect of programming applications on Haneul is the resource safety that the Move Bytecode Verifier enforces. Move packages must satisfy the following resource safety parameters:
...
[Learn more](https://github.com/GeunhwaJeong/haneul/blob/main/external-crates/move/move-execution/v1/crates/move-bytecode-verifier/README.md) about the Move Bytecode Verifier.
...
To build your "Hello, World!" package, first confirm your working directory is `~/haneul-stack-hello-world/move/hello-world`, then run the following command:

```
$ haneul move build
```

The build process fetches and compiles the dependencies defined in the `Move.toml` file. The Move compiler checks your `.move` code for type errors, syntax errors, and enforces [resource safety](#resource-safety), then translates your `.move` code into bytecode that Haneul can execute.

:::info

Both `haneul move test` and `haneul client publish` automatically build your package first, so running `haneul move build` separately is optional. It is useful for catching compilation errors early before you test or publish.
...
This should return `testnet`. If it does not return `testnet`, follow the [client configuration instructions](/getting-started/onboarding/configure-haneul-client) before continuing.

Then, check your balance of HANEUL tokens to confirm you have enough to publish to Testnet:
...
If you do not have a balance, follow the [HANEUL faucet instructions](/getting-started/onboarding/get-coins).
...
When you publish a Move package to the network, the network uploads and stores the bytecode as a Move package with a unique package ID and version number. The network consumes HANEUL tokens as gas and processes the transaction onchain.
...
```
$ haneul client call --package <PACKAGE_ID> --module greeting --function new  
```

Replace `<PACKAGE_ID>` with the package ID the output of the `haneul client publish` command returned. You must include the `--package`, `--module`, and `--function` flags. 
...
If you receive this error, you must wait until the current epoch is over before submitting your transaction again. You can see how long is left in the current epoch using [Haneul Explorer](https://haneulvision.xyz/) or another network explorer like [HaneulScan](https://haneulscan.xyz/mainnet/home). 
...
<div className="next-steps-module">
  ...
  <div className="next-steps-grid">
    ...
    >
      Learn more about accessing data on Haneul.
    </Card>
    <Card className="plausible-event-name=hello+community+button"
      title="Join the Community" 
      href="/getting-started/onboarding/next-steps"
    >
      Join the Haneul developer community, try out other example projects, or read more documentation.
    </Card>
  </div>
</div>
