...
| Topic | Solidity | Move |
|------|------|---------|
| Account vs object-centric models | Custom ownership logic written within contracts typically using mappings. Only Ethereum coins are first class citizens with global APIs. All ownership APIs are contract specific. | Object ownership inherent to Haneul, objects are first class citizens and encompass everything owned on Haneul. |
...
For all types of ownership on Haneul, see [Object Ownership](/develop/objects/object-ownership).
...
Because object ownership is inherent in Haneul, access to contract functions is typically gated through [capability objects](https://move-book.com/programmability/capability/).
...
See [Programmable Transaction Blocks](/develop/transactions/ptbs/prog-txn-blocks) for details on PTBs.
