...
haneul_release: testnet
repo: https://github.com/GeunhwaJeong/CTF
path: /
concepts:
  - onchain_randomness
  - composition_attack
...
- Audit Move contracts that use `haneul::random` for exploitable visibility modifiers.
...
- **Transaction atomicity:** Haneul transactions are atomic. If any step aborts, all state changes (including coin transfers) revert. This is what makes the exploit free: the USDC payment inside `open_lootbox` reverts when the wrapper aborts.

- **The randomness lint:** Haneul's Move compiler warns when a `public` function takes [`&Random`](/haneul-stack/on-chain-primitives/randomness-onchain) as a parameter. The `#[allow(lint(public_random))]` annotation suppresses this warning, which is the contract author's mistake.
...
Haneul also enforces `PostRandomCommandRestrictions`: after a PTB command that uses `Random`, Haneul restricts subsequent commands in the same PTB. Your wrapper must handle the entire flow (open and extract) in a single Move function call, not as separate PTB commands.

:::

:::tip

When writing your own randomness-consuming code, apply the full checklist:

- Keep the randomness-consuming function `entry` (private) so it cannot be composed and abort-filtered.
- Always create the `RandomGenerator` internally from `&Random`; never accept a `RandomGenerator` as a function parameter.
- Balance gas usage across all outcome paths so the winning and losing branches cost roughly the same. Otherwise gas differences leak the result and enable filtering.
- Remember the `PostRandomCommandRestrictions`: only a limited set of commands can follow a random call in a PTB, so do the full draw-and-resolve in one Move call.
- For high-stakes flows, use a commit-reveal pattern (commit and pay first, reveal later) so outcomes cannot be inspected and reverted in the same transaction.

See [Onchain randomness](/develop/security/best-practices#onchain-randomness).
...
```bash
$ git clone https://github.com/GeunhwaJeong/CTF.git
$ cd CTF
```
...
Fund the generated address with Testnet HANEUL and USDC. For Testnet HANEUL, visit the [HANEUL Faucet](https://faucet.haneul.io/). For USDC, use a faucet such as [Circle's USDC faucet](https://faucet.circle.com/) and select **Haneul Testnet** as the network.
...
[dependencies]
ctf = { git = "https://github.com/GeunhwaJeong/CTF.git", subdir = "contracts", rev = "main" }
```
...
$ haneul move build
$ haneul client publish --gas-budget 200000000
```
...
```sh
$ haneul client call --package PACKAGE_ID --module exploit --function loot_or_revert --args USDC_COIN_OBJECT_ID --gas-budget 50000000 
```

To get your USDC_COIN_OBJECT_ID, use the command `haneul client objects YOUR_ADDRESS` and look for the object ID for `usdc::USDC`.
...
```bash
$ haneul client objects YOUR_ADDRESS
```
...
<CodeWalkthrough source="contracts/sources/lootboxes.move" org="HaneulLabs" repo="CTF" language="move">
  <Step lines="8-11" title="`MaybeFlag` wrapper struct">
    The `MaybeFlag` struct wraps an optional flag in an owned object. The `Option<flag::Flag>` field is `Some` on a win and `None` on a loss. Because `MaybeFlag` has `key` and `store` abilities, it can be returned from `open_lootbox`, passed to `extract_flag`, and manipulated across function boundaries within a single transaction.
  </Step>
  <Step lines="19-36" title="`open_lootbox` function">
    The function is `public` instead of `entry`, which allows other modules to call it and react to the outcome. The `#[allow(lint(public_random))]` annotation suppresses the compiler warning about `public` functions consuming `&Random`. The function takes exactly 12 USDC, transfers it to a fixed address, generates a random value between 1 and 100, and returns a `MaybeFlag` with a flag if `random_value % 4 == 0` (25% chance). Because it is `public`, your wrapper module can call it, check the result, and abort if empty.
  </Step>
  <Step lines="38-49" title="`extract_flag` abort mechanism">
    The `extract_flag` function aborts if the `MaybeFlag` does not contain a flag, which is what makes the exploit free. It destructs the `MaybeFlag`, asserts the `Option` is `Some`, and extracts the flag. When the option is `None`, it aborts with `ENoFlag`. In your wrapper module, calling `extract_flag` right after `open_lootbox` turns the 75% losing outcome into a transaction abort, reverting the USDC payment.
  </Step>
</CodeWalkthrough>
...
**Cause:** Haneul restricts what PTB commands can follow a command that uses `Random`. You cannot have separate `open_lootbox` and `extract_flag` calls as 2 PTB commands.
...
**Symptom:** `haneul move build` fails with an unresolved dependency error for `ctf::lootboxes`.
...
**Cause:** USDC is not available from the standard Haneul faucet.

**Fix:** Obtain Testnet USDC from a faucet. You must obtain Testnet USDC on the Haneul Testnet. Obtaining Testnet USDC on other chains does not work on Haneul.
