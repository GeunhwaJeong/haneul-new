---
title: Plinko
description: Build a provably fair, gasless Plinko game on Haneul using onchain randomness and sponsored transactions through Enoki.
...
repo: https://github.com/GeunhwaJeong/plinko-poc
path: /
...
Plinko is a casino-style game where a ball is dropped through a pin board, randomly landing on a prize at the bottom. This Plinko example written in Move demonstrates Haneul's [onchain randomness](/haneul-stack/on-chain-primitives/randomness-onchain) and [sponsored transactions](/develop/transaction-payment/sponsor-txn). Players bet HANEUL tokens and drop balls through the pin board, where each ball lands in a multiplier bucket determined entirely by Haneul's onchain random number generator.
...
- Sponsor transactions through Enoki so users interact without holding HANEUL for gas.
...
- **Onchain randomness:** Haneul's `Random` module provides verifiable, unbiasable random numbers that the contract consumes directly. No external oracle is needed.
...
For more details on the randomness API, see [Onchain Randomness](/haneul-stack/on-chain-primitives/randomness-onchain).
...
- [x] [Enoki developer portal access](https://portal.enoki.haneul-labs.com/).
...
```bash
$ git clone https://github.com/GeunhwaJeong/plinko-poc.git
$ cd plinko-poc
```
...
```bash title='.env'
NEXT_PUBLIC_HANEUL_NETWORK_NAME=testnet
...
NEXT_PUBLIC_MIN_BET_AMOUNT=100000000 (0.1 HANEUL in GEUNHWA)
NEXT_PUBLIC_MAX_BET_AMOUNT=10000000000 (10 HANEUL in GEUNHWA)
...
```bash
$ haneul keytool export --key-identity YOUR_WALLET_ADDRESS
```

Then, convert it to base64 format and record the `publicBase64Key` value:

```bash
$ haneul keytool convert YOUR_BASE32_PRIVATE_KEY
```

To get an Enoki secret key, visit the [Enoki dashboard](https://portal.enoki.haneul-labs.com/).
...
<ImportContent source="plinko/sources/plinko.move" mode="code" org="HaneulLabs" repo="plinko-poc" fun="start_game" />
...
<ImportContent source="plinko/sources/plinko.move" mode="code" org="HaneulLabs" repo="plinko-poc" fun="finish_game" />
...
<ImportContent source="plinko/sources/house_data.move" mode="code" org="HaneulLabs" repo="plinko-poc" struct="HouseData" />
...
<ImportContent source="app/src/hooks/moveTransactionCalls.ts/useCreateGame.ts" mode="code" org="HaneulLabs" repo="plinko-poc" fun="useCreateGame" />
...
<ImportContent source="app/src/app/api/services/PlinkoGameService.ts" mode="code" org="HaneulLabs" repo="plinko-poc" fun="finishGame" />
...
**Cause:** The bet amount falls outside the range the house configured. The default minimum is 1 HANEUL and the default maximum is 10 HANEUL.

**Fix:** Adjust the bet to fall within the limits. Check the current limits with `haneul client object HOUSE_DATA_ID` and inspect the `min_stake` and `max_stake` fields.

### Insufficient house balance

**Symptom:** The `start_game` transaction aborts with `EInsufficientHouseBalance`.

**Cause:** The house does not hold enough HANEUL to cover the maximum possible payout for the bet.

**Fix:** Top up the house by calling `house_data::top_up` with additional HANEUL. Use the setup script or build the transaction manually.
...
**Fix:** Verify the game ID from the `NewGameStarted` event. Check the transaction status on Haneul Explorer. If the game was already finished, the trace is available in the `GameFinished` event on the original finish transaction.
