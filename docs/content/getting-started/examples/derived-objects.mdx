...
description: Create deterministic Haneul objects whose IDs you can predict offchain before the objects exist, using four derivation strategies in Move and the TypeScript SDK.
...
Derived objects are Haneul objects with deterministic IDs computed from a parent object and a key. You can predict a derived object's ID offchain before the transaction that creates it runs, which enables indexer-free lookups, batch queries, and state verification.
...
- **Derived objects:** Top-level Haneul objects whose IDs are deterministically computed from a parent UID and a serialized key. The same parent + key always produces the same ID.
...
sequenceDiagram
  ...
  participant Chain as Haneul network
...
Edit `.env` with the values from the publish step. You can get your USER_SECRET_KEY from the command `haneul keytool export --key-identity <ADDRESS>`
...
<ImportContent source="C5/contracts/derived_objects/sources/parent.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" struct="ParentObject" />
...
<ImportContent source="C5/contracts/derived_objects/sources/objects.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="create_incremental" />
...
The `create_address` function uses a Haneul address as the derivation key, which creates a 1-to-1 mapping between addresses and derived objects under this parent.

<ImportContent source="C5/contracts/derived_objects/sources/objects.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="create_address" />
...
The `deriveObjectIDFromParent` helper computes a derived object's ID using the Haneul SDK, without any network call.

<ImportContent source="C5/ts/src/helpers/deriveObjectID.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="deriveObjectIDFromParent" />
...
<ImportContent source="C5/ts/tests/derive.test.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="reverse search pagination" />
