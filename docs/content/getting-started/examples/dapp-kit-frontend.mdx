---
title: Haneul dApp Kit Frontend
description: Build a React frontend that connects a wallet, mints an NFT through a Move contract, queries owned objects, and refreshes the UI automatically using Haneul dApp Kit and React Query.
...
- Create a starter template for any Haneul app frontend that you can extend with your own contract calls.

## What you learn

This example teaches:

- **Provider setup:** The app wraps components in `QueryClientProvider` (React Query) and `DAppKitProvider` (Haneul). The dApp Kit config creates `HaneulGrpcClient` instances for each network. This is the foundation every Haneul React app needs.

- **Wallet hooks:** `useCurrentAccount()` returns the connected wallet address (or null). `useDAppKit()` provides `signAndExecuteTransaction` and the Haneul client. These hooks drive all wallet-dependent UI.
...
The example has 1 React app that connects to a deployed Move contract on Testnet. The React app renders a mint button, a wallet status card, and an owned-objects list. The wallet (Slush Wallet or any Haneul-compatible wallet) handles signing. The Haneul gRPC node serves object queries for the owned-objects list.
...
sequenceDiagram
  ...
  participant RPC as Haneul gRPC
...
$ cd haneul-move-bootcamp/E2/my-first-haneul-dapp
```
...
<ImportContent source="E2/my-first-haneul-dapp/src/dapp-kit.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" />

The `createDAppKit` factory takes an array of network configs, each with a `chain` identifier and a `createClient` factory. The `HaneulGrpcClient` uses binary protocol for faster communication than JSON-RPC. The module declaration augments dApp Kit's types so TypeScript knows about the configured networks.
...
<ImportContent source="E2/my-first-haneul-dapp/src/components/ui/MintNFTForm.tsx" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="MintNFTForm" />
...
<ImportContent source="E2/my-first-haneul-dapp/src/OwnedObjects.tsx" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" fun="OwnedObjects" />
...
<ImportContent source="E2/my-first-haneul-dapp/src/main.tsx" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" />
...
- **Add transaction history:** After each mint, store the transaction digest in local state or `localStorage`. Display a history section with links to the Haneul explorer.
...
**Fix:** Install Slush Wallet or another Haneul-compatible wallet. Enable it for `localhost:5173`. Refresh the page.
...
**Fix:** Verify the `StructType` matches `PACKAGE_ID::hero::Hero` exactly. Confirm `invalidateQueries` uses the same query key as the `useQuery` call.
...
**Fix:** Verify the URLs in `dapp-kit.ts` match the current Haneul gRPC endpoints. The standard endpoints are `https://grpc.testnet.haneul.io` for Testnet and `https://grpc.mainnet.haneul.io` for Mainnet.
