...
description: Build a real-time event indexer that subscribes to Haneul checkpoints through gRPC, filters for specific Move events, and decodes BCS-encoded event data in TypeScript.
...
- Subscribe to Haneul checkpoints through gRPC streaming for lower latency than JSON-RPC polling.

## What you learn

This example teaches:

- **Move events:** A struct with `copy` and `drop` abilities that you pass to `haneul::event::emit`. The network indexes events and makes them queryable by type. They carry data but do not create onchain objects.

- **gRPC checkpoint streaming:** The `HaneulGrpcClient.subscribeCheckpoint` method opens a persistent stream that yields each new checkpoint as it finalizes. The `readMask` parameter filters the response to only include the fields you need (in this case, `transactions.events`).
...
sequenceDiagram
  ...
  participant gRPC as Haneul gRPC
...
```bash
$ git clone https://github.com/GeunhwaJeong/haneul-move-bootcamp.git
$ cd haneul-move-bootcamp/K1
```
...
```bash
$ cd haneul-move-bootcamp/K1/backend
$ npm test
```
...
<ImportContent source="K1/contract/sources/indexer_sample.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" fun="register_user" />
...
<ImportContent source="K1/contract/sources/indexer_sample.move" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" struct="UserRegistered" />
...
<ImportContent source="K1/backend/indexer.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" fun="main" />
...
<ImportContent source="K1/backend/utils/parseEvent.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" fun="decodeBcsEvent" />
...
<ImportContent source="K1/backend/tests/registerUser.test.ts" mode="code" org="HaneulLabs" repo="haneul-move-bootcamp" fun="should successfully register a new user" />
...
**Cause:** The test wallet does not have enough HANEUL for the transaction.

**Fix:** Fund the test address with Testnet HANEUL using `haneul client faucet`. The registration transaction uses minimal gas (under 0.01 HANEUL).
