...
This capture the flag style example allows you to analyze a staking contract, find the vulnerability in its time-tracking logic, and exploit it to claim a flag NFT. The contract requires 168 hours (1 week) of staking and a minimum of 1 HANEUL to claim, but a flaw in how receipts merge their `hours_staked` values lets you reach the threshold instantly. 
...
- Practice building exploit transactions with the Haneul TypeScript SDK against a live Testnet contract.
...
The challenge has 2 components: the deployed Move contract on Testnet and the TypeScript script you write locally to exploit it. The `StakingPool` is a shared object that holds all staked HANEUL balances. When you call `stake`, it returns a `StakeReceipt` owned by your address. The exploit creates multiple receipts, merges them to accumulate `hours_staked` values, and calls `claim_flag` with the merged receipt that meets the 168-hour threshold.
...
sequenceDiagram
  ...
  Script->>Pool: stake(0.5 HANEUL) → Receipt A (0 hours)
  Script->>Pool: stake(0.5 HANEUL) → Receipt B (0 hours)
  ...
  Pool-->>Script: Flag NFT + staked HANEUL returned
...
4. After enough merges, the combined receipt has `hours_staked >= 168` and `amount >= 1_000_000_000` (1 HANEUL total across all merged stakes).

5. The script calls `claim_flag` with the merged receipt. The contract verifies the threshold, mints a `Flag` NFT, and returns the staked HANEUL.
...
Mitigation checklist for time-accumulation and receipt patterns:

- Derive elapsed time from monotonic onchain sources (`Clock`/timestamps) rather than a client-updatable counter, and never let accounting move backward.
- Make receipts non-mergeable, or key them by a canonical, unforgeable receipt ID so duplicates and merges cannot inflate accumulated state.
- Add invariant tests that assert accumulated time and amounts can never exceed real elapsed time across stake, update, merge, and claim paths.
- Emit events for stake, claim, and merge so the accounting is auditable and anomalies are detectable.
...
This creates a `keypair.json` file with a fresh Ed25519 key pair. [Fund this new address with Testnet HANEUL](https://faucet.haneul.io/).
...
If successful, the script claims a `Flag` NFT and returns your staked HANEUL. Verify the flag in your wallet:
...
<CodeWalkthrough source="contracts/sources/staking.move" org="HaneulLabs" repo="CTF" language="move">
  <Step lines="21-26" title="`StakeReceipt` struct">
    The receipt tracks staking state as an owned object the staker controls. The `hours_staked` field is the vulnerability target. It starts at `0` when the receipt is created and accumulates through `update_receipt` and `merge_receipts` calls. Because the receipt has `store` ability, it can be transferred and wrapped.
  </Step>
  <Step lines="35-51" title="Staking to create a receipt">
    The `stake` function deposits HANEUL into the pool and returns a fresh receipt. Each call creates a new `StakeReceipt` with `hours_staked: 0` and `last_update_timestamp` set to the current clock. The `amount` field records how much HANEUL the receipt represents. The exploit creates multiple receipts with small amounts that sum to at least 1 HANEUL when merged.
  </Step>
  <Step lines="73-102" title="`merge_receipts` vulnerability">
    This function adds the `hours_staked` of two receipts together without validating the sum. It destructs both receipts, sums their `amount` and `hours_staked` fields, and creates a new receipt with `last_update_timestamp` set to the current clock time. The new receipt has no link to the original stake times. This is where the exploit happens: merge enough receipts and the `hours_staked` exceeds 168 without any real time passing.
  </Step>
  <Step lines="104-126" title="`claim_flag` guard">
    The `claim_flag` function checks the time and amount thresholds before minting the flag. It computes `total_hours = receipt.hours_staked + hours_passed` (where `hours_passed` is the time since the last update). It asserts `total_hours >= 168` and `receipt.amount >= 1_000_000_000`. If both pass, it splits the staked amount from the pool, destroys the receipt, and mints a `Flag`.
  </Step>
</CodeWalkthrough>
...
**Cause:** The merged receipt's `amount` is below 1,000,000,000 (1 HANEUL).

**Fix:** Ensure the total staked across all merged receipts equals at least 1 HANEUL. Each `stake` call's coin value adds to the merged total.

### Insufficient gas or HANEUL balance

**Symptom:** The exploit script fails with an `InsufficientGas` or balance error.

**Cause:** The wallet does not have enough HANEUL for both the stake amount (minimum 1 HANEUL) and gas fees.

**Fix:** Request more Testnet HANEUL from the faucet. You need at least 2 HANEUL (1 for staking, 1 for gas across multiple transactions).
