...
<ImportContent source="contracts/sources/staking.move" mode="code" org="HaneulLabs" repo="CTF" struct="StakeReceipt" />
...
<ImportContent source="contracts/sources/staking.move" mode="code" org="HaneulLabs" repo="CTF" fun="merge_receipts" />
...
<ImportContent source="contracts/sources/staking.move" mode="code" org="HaneulLabs" repo="CTF" fun="claim_flag" />
...
The `stake` function deposits HANEUL into the pool and returns a fresh receipt.

<ImportContent source="contracts/sources/staking.move" mode="code" org="HaneulLabs" repo="CTF" fun="stake" />

Each call creates a new `StakeReceipt` with `hours_staked: 0` and `last_update_timestamp` set to the current clock. The `amount` field records how much HANEUL the receipt represents. The exploit creates multiple receipts with small amounts that sum to at least 1 HANEUL when merged.
