...
The RW cache role is `arn:aws:iam::011083325127:role/haneul-sccache-rw-github`. Its
trust (aud pinned, no wildcard) = these subjects:
`repo:HaneulLabs/haneul:ref:refs/heads/{main,devnet,testnet,mainnet}` **plus**
`repo:HaneulLabs/haneul:ref:refs/heads/releases/haneul-*-release` (added 2026-06-08).
...
| `push`             | `releases/haneul-*-release`             | `…:ref:refs/heads/releases/…`  | **YES** (added 2026-06-08) | OIDC RW (trusted)                   |
| `push`             | `extensions` (external.yml only)     | `…:ref:refs/heads/extensions`  | **NO — excluded**        | branch is 404; local-cache fallback   |
| `workflow_dispatch`| `main`, **no** `haneul_repo_ref`        | `…:ref:refs/heads/main`        | **YES**                  | OIDC RW                               |
| `workflow_dispatch`| `main` + `haneul_repo_ref=<feature>`    | `…:ref:refs/heads/main`        | sub matches, but **gate OFF** | builds the override ref (untrusted code) → must NOT hold RW; gate requires `haneul_repo_ref==''` → static/local |
...
> `${{ github.event.inputs.haneul_repo_ref || github.ref }}`. So a `workflow_dispatch`
> from `main` with `haneul_repo_ref` set to a feature ref has a **trusted `sub`** while
> **building untrusted code**. The `SCCACHE_ROLE` gate must therefore also require
> `haneul_repo_ref` to be empty, or that build would populate the RW cache from
> arbitrary code. `external.yml` has no such input, so it is unaffected.

## Per-call-site map

### `rust.yml` — 11 sites (`push`[main,devnet,testnet,mainnet,releases/haneul-*-release] + `pull_request` + `workflow_dispatch`)

`test` (L97), `test-tidehunter` (L146), `test-extra` (L211), `benchmark-smoke`
(L276), `windows-build` (L334), `windows-cli-tests` (L361), `simtest` (L386),
`simtest-mainnet` (L422), `move-test` (L458), `clippy` (L554), `haneul-excution-cut`
...
### `bridge.yml` — 2 sites (`push`[main,devnet,testnet,mainnet,releases/haneul-*-release] + `pull_request` + `workflow_dispatch`)
...
1. It also writes to `s3://haneul-releases` via the **separate** `release-s3` role in
...
2. **Environment-sub subtlety (not a tag-trust gap).** Once `release-build` declares
   ...
   `sub=repo:HaneulLabs/haneul:environment:release`, **not** a tag/branch ref. So adding
   `*-v*` tag trust to role #1 would do **nothing** for the sccache step. To give it
   OIDC, pick one in Phase 5:
   - **(a)** role #1 **also** trusts `repo:HaneulLabs/haneul:environment:release`; or
...
1. **`releases/haneul-*-release` branches** — **ADDED 2026-06-08** (StringLike; 4 exact
...
env:
  ...
  # workflow_dispatch building an override ref (haneul_repo_ref). Keep the ref set in
  # sync with the role trust. Drop the haneul_repo_ref clause for workflows lacking
  # that input (e.g. external.yml).
  SCCACHE_ROLE: ${{ (github.event_name != 'workflow_dispatch' || github.event.inputs.haneul_repo_ref == '') && (contains(fromJSON('["refs/heads/main","refs/heads/devnet","refs/heads/testnet","refs/heads/mainnet"]'), github.ref) || (startsWith(github.ref, 'refs/heads/releases/haneul-') && endsWith(github.ref, '-release'))) && 'arn:aws:iam::011083325127:role/haneul-sccache-rw-github' || '' }}
...
The `haneul_repo_ref` guard is **conservative** for jobs that ignore that input
