...
Spec for the AWS IAM roles that let `HaneulLabs/haneul` workflows authenticate to
...
2. **Always constrain `:sub`. Never use `repo:HaneulLabs/haneul:*`.** A wildcard
   `sub` would let **`pull_request`-triggered jobs assume the role** — GitHub's
   OIDC `sub` for a PR is `repo:HaneulLabs/haneul:pull_request`, so a fork or any PR
...
3. **Preferred: a dedicated GitHub Environment** (e.g. `aws-sccache`) with
   ...
   `repo:HaneulLabs/haneul:environment:<name>`, and the branch restriction is
...
4. **Alternative: explicit `ref` subjects** via `StringLike` when not using an
   environment. Wildcards *within* a ref (e.g. `…:ref:refs/heads/releases/haneul-*-release`)
...
## Role 1 — `haneul-github-actions-sccache`
...
> **`arn:aws:iam::011083325127:role/haneul-sccache-rw-github`** (created 2026-05-22),
> with the managed policy `haneul-sccache-rw` (exactly the least-privilege policy
...
> `repo:HaneulLabs/haneul:ref:refs/heads/{main,devnet,testnet,mainnet}` — `aud`
> pinned, no wildcard.
>
> **Gap to close if needed:** the live trust does **not** yet include
> `releases/haneul-*-release` branches or `*-v*` tags. sccache callers that run on a
  ...
  "Statement": [{
    ...
    "Condition": {
      "StringEquals": {
        ...
        "token.actions.githubusercontent.com:sub": "repo:HaneulLabs/haneul:environment:aws-sccache"
      }
    }
  }]
}
...
"Condition": {
  ...
  "StringLike": {
    "token.actions.githubusercontent.com:sub": [
      "repo:HaneulLabs/haneul:ref:refs/heads/main",
      "repo:HaneulLabs/haneul:ref:refs/heads/devnet",
      "repo:HaneulLabs/haneul:ref:refs/heads/testnet",
      "repo:HaneulLabs/haneul:ref:refs/heads/mainnet",
      "repo:HaneulLabs/haneul:ref:refs/heads/releases/haneul-*-release",
      "repo:HaneulLabs/haneul:ref:refs/tags/*-v*"
    ]
  }
}
```
...
### Permission policy (least-privilege; bucket `haneullabs-sccache`)
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::haneullabs-sccache/*"
    },
    {
...
role `haneul-github-actions-sccache-ro` (trust on `pull_request`, policy granting
...
`repo:HaneulLabs/haneul:pull_request`. AWS cannot tell a fork PR from a same-repo PR
...
## Role 2 — `haneul-github-actions-release-s3`

**Used by:** `release.yml` — uploads/downloads release binaries to
`s3://haneul-releases`. Runs on the `release: created` event (a release tag) and
...
> **`arn:aws:iam::011083325127:role/haneul-releases-rw-github`** (inline policy
> `haneul-releases-rw` = exactly the least-privilege policy below). Trust =
> `repo:HaneulLabs/haneul:environment:release` (the environment variant), aud
...
(`repo:HaneulLabs/haneul:ref:refs/heads/main`) *even when the `haneul_tag` input names
...
"Condition": {
  "StringEquals": {
    "token.actions.githubusercontent.com:aud": "sts.amazonaws.com",
    "token.actions.githubusercontent.com:sub": "repo:HaneulLabs/haneul:environment:release"
  }
}
```
...
"StringLike": {
  "token.actions.githubusercontent.com:sub": [
    "repo:HaneulLabs/haneul:ref:refs/tags/devnet-v*",
    "repo:HaneulLabs/haneul:ref:refs/tags/testnet-v*",
    "repo:HaneulLabs/haneul:ref:refs/tags/mainnet-v*",
    "repo:HaneulLabs/haneul:ref:refs/heads/main"
  ]
}
```

### Permission policy (bucket `haneul-releases`)
...
```json
{
  ...
  "Statement": [
    { "Sid": "ReleaseObjectRW", "Effect": "Allow",
      ...
      "Resource": "arn:aws:s3:::haneul-releases/*" },
...
> Note: `sccache-warmup.yml` writes a small summary to `s3://haneullabs-sccache`.
...
`release.yml` builds with sccache (`haneullabs-sccache`, Role 1) **and** uploads
artifacts to `haneul-releases` (Role 2). `configure-aws-credentials` (and therefore
...
`haneul-releases` credentials — its only remaining AWS credentials are the static
...
and performs every `haneul-releases` write. The two roles never coexist in one
job, so there is no credential sequencing to manage.

---

## Role 3 — `haneul-github-actions-kms-test`
...
> `repo:HaneulLabs/haneul:environment:haneul-typescript-aws-kms-test-env` subject (that
...
**dedicated test key**, job-scoped to environment `haneul-typescript-aws-kms-test-env`).
...
- If it must run on **PRs**, the minimal blast radius of a test-only KMS key may
  justify allowing `repo:HaneulLabs/haneul:pull_request` here — **owner's decision**.
  ...
  build:
    ...
    - uses: ./.github/actions/setup-sccache
      with:
        ...
        # always main.
        role-to-assume: ${{ github.ref == 'refs/heads/main' && 'arn:aws:iam::011083325127:role/haneul-sccache-rw-github' || '' }}
...
(`haneul-sccache-<run_id>-<attempt>`) and `allowed-account-ids: 011083325127` on
...
- Set **deployment branch/tag rules** to exactly the trusted refs (e.g. `main`,
  `releases/haneul-*-release`, the release tag patterns). Without them, *any* branch
...
3. **PR CI** (`haneul-ci-tests.yml` et al.) — introduce Role 1 (RW) for trusted
