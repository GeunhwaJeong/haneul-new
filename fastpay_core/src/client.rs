...
pub trait Client {
    ...
    async fn try_complete_pending_orders(&mut self) -> Result<(), HaneulError>;
    ...
    async fn download_owned_objects_not_in_db(&self) -> Result<BTreeSet<ObjectRef>, HaneulError>;
}
...
impl<A> ClientState<A> {
    ...
    /// https://github.com/GeunhwaJeong/fastnft/issues/332
    pub fn new(
        path: PathBuf,
        address: HaneulAddress,
        secret: StableSyncSigner,
    ...
    pub fn object_ref(&self, object_id: ObjectID) -> Result<ObjectRef, HaneulError> {
        self.store
            .object_refs
            .get(&object_id)?
            .ok_or(HaneulError::ObjectNotFound { object_id })
    }
    ...
    /// https://github.com/GeunhwaJeong/fastnft/issues/307
    ...
    pub fn remove_object_info(&mut self, object_id: &ObjectID) -> Result<(), HaneulError> {
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    /// https://github.com/GeunhwaJeong/fastnft/issues/346
    async fn execute_transaction(
        ...
        // TODO: https://github.com/GeunhwaJeong/fastnft/issues/349
        // https://github.com/GeunhwaJeong/fastnft/issues/211
        // https://github.com/GeunhwaJeong/fastnft/issues/346
    ...
    /// TODO: always return true for immutable objects https://github.com/GeunhwaJeong/fastnft/issues/305
    /// TODO: this function can fail. Need to handle it https://github.com/GeunhwaJeong/fastnft/issues/383
    fn can_lock_or_unlock(&self, order: &Order) -> Result<bool, HaneulError> {
    ...
    /// Double-locking can cause equivocation. TODO: https://github.com/GeunhwaJeong/fastnft/issues/335
    pub fn lock_pending_order_objects(&self, order: &Order) -> Result<(), HaneulError> {
    ...
    fn unlock_pending_order_objects(&self, order: &Order) -> Result<(), HaneulError> {
        if !self.can_lock_or_unlock(order)? {
            return Err(HaneulError::ConcurrentTransactionError);
        }
        self.store
    ...
    async fn update_objects_from_order_info(
    ...
    ) -> Result<(CertifiedOrder, OrderEffects), HaneulError> {
        ...
        // https://github.com/GeunhwaJeong/fastnft/issues/331
    ...
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    async fn try_complete_pending_orders(&mut self) -> Result<(), HaneulError> {
        ...
        // TODO: https://github.com/GeunhwaJeong/fastnft/issues/330
        for order in unique_pending_orders {
            self.execute_transaction(order.clone()).await.map_err(|e| {
                HaneulError::ErrorWhileProcessingTransactionOrder { err: e.to_string() }
            })?;
        }
        Ok(())
    }
