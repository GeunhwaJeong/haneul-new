        ...
        let numbers: futures::stream::FuturesUnordered<_> = authority_clients
            ...
            .map(|(name, client)| {
                ...
                async move {
                    match fut.await {
                        Ok(info) => {
                            // TODO(https://github.com/GeunhwaJeong/fastnft/issues/323): This assumes the object is not deleted.
    ...
    async fn receive_object(&mut self, certificate: &CertifiedOrder) -> Result<(), anyhow::Error> {
        ...
        match &certificate.order.kind {
            OrderKind::Transfer(transfer) => {
                ...
                // TODO(https://github.com/GeunhwaJeong/fastnft/issues/323): This assumes object is not deleted.
