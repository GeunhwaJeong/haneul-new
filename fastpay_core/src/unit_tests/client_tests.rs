...
use fastx_types::error::HaneulError::ObjectNotFound;
...
fn test_transfer_object_error() {
    ...
    assert!(matches!(
        ...
        Some(HaneulError::ObjectNotFound { .. })
    ));
...
async fn test_transfer_pending_orders() {
    ...
    //        Some(HaneulError::QuorumNotReached {errors, ..}) if matches!(errors.as_slice(), [HaneulError::ObjectNotFound{..}, ..])));
    // Pending order should be cleared
    assert!(sender_state.store().pending_orders.is_empty());
    ...
    assert!(result.is_err());
    //assert!(matches!(result.unwrap_err().downcast_ref(),
    //        Some(HaneulError::QuorumNotReached {errors, ..}) if matches!(errors.as_slice(), [HaneulError::LockErrors{..}, ..])));
    ...
    assert!(matches!(
        result.unwrap_err().downcast_ref(),
        Some(HaneulError::ConcurrentTransactionError)
    ));
