...
pub fn verify_module(
    ...
    if *module.address() == HANEUL_FRAMEWORK_ADDRESS
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            } else if ident == (HANEUL_FRAMEWORK_ADDRESS, EVENT_MODULE) {
                verify_private_event_emit(view, fhandle, type_arguments)?
            } else if ident == (HANEUL_FRAMEWORK_ADDRESS, COIN_REGISTRY_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (HANEUL_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{haneul}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{haneul}::transfer::public_{f}'",
            haneul = HANEUL_FRAMEWORK_ADDRESS,
            f = fident,
...
fn verify_private_event_emit(
    ...
    if fident == EMIT_AUTHENTICATED_IMPL_FUNCTION {
        ...
        if (module_id.address(), module_id.name()) != (&HANEUL_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            debug_assert!(
                false,
