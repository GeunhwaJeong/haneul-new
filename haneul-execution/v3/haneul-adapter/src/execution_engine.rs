...
mod checked {
    ...
    use haneul_types::randomness_state::{
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), haneul_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            ...
            let conservation_result = {
                temporary_store
                    .check_haneul_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy HANEUL, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_haneul_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
    ...
    fn setup_store_execution_time_estimates(
        ...
        let system_state = builder.obj(ObjectArg::HANEUL_SYSTEM_MUT).unwrap();
