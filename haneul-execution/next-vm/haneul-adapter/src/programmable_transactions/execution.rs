...
mod checked {
    ...
    use haneul_types::storage::{get_package_objects, PackageObjectArc};
    use haneul_types::{
        base_types::{
    ...
    fn publish_and_verify_modules(
        ...
        // TODO(https://github.com/GeunhwaJeong/haneul/issues/69): avoid this redundant serialization by exposing VM API that allows us to run the linker directly on `Vec<CompiledModule>`
        ...
        for module in modules {
            ...
            haneul_verifier::verifier::haneul_verify_module_unmetered(module, &BTreeMap::new())?;
        }
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = HaneulDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    fn check_private_generics(
        ...
        if module_ident == (&HANEUL_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in haneul::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&HANEUL_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call haneul::{m}::{f}. \
                Use the public variant instead, haneul::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_struct_type(*sidx) else {
                    invariant_violation!("haneul::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &HANEUL_FRAMEWORK_ADDRESS
    ...
    fn primitive_serialization_layout(
        ...
        Ok(match param_ty {
            ...
            Type::Struct(idx) => {
                ...
                let resolved_struct = get_struct_ident(&s);
                if resolved_struct == RESOLVED_HANEUL_ID {
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    HaneulAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
