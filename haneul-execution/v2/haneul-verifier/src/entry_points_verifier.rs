...
use haneul_types::randomness_state::is_mutable_random;
use haneul_types::{
...
fn verify_param_type(
    ...
    // Only `haneul::haneul_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `haneul::haneul_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
