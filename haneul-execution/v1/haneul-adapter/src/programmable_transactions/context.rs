...
mod checked {
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn new(
            ...
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_haneul_resolver()));
            ...
            move_vm_profiler::tracing_feature_enabled! {
                ...
                use crate::gas_meter::HaneulGasMeter;

                let tx_digest = tx_context.digest();
                let remaining_gas: u64 = move_vm_types::gas::GasMeter::remaining_gas(&HaneulGasMeter(
                    gas_charger.move_gas_status_mut(),
                ))
                .into();
                HaneulGasMeter(gas_charger.move_gas_status_mut()).set_profiler(GasProfiler::init(
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = HaneulDataStore::new(&self.linkage_view, &self.new_packages);
        ...
        pub fn publish_module_bundle(
            ...
            self.vm.get_runtime().publish_module_bundle(
                ...
                sender,
                &mut data_store,
                &mut HaneulGasMeter(self.gas_charger.move_gas_status_mut()),
            )
        }
    }
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = HaneulDataStore::new(linkage_view, new_packages);
    ...
    pub(crate) struct HaneulDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> HaneulDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl DataStore for HaneulDataStore<'_, '_> {
