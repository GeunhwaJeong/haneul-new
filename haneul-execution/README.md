...
The `haneul-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into Haneul.
...
- `./haneul-execution`, (latest version and cuts/copies of haneul-specific
  crates).
...
code must be via `haneul-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `haneul-execution`.**
...
`haneul-execution tests::test_encapsulation` is a test that detects
...
`./crates/haneul-types` are shared across all versions.  Changes to
...
After the crates have been copied, the `haneul-execution` crate must be
modified. The following two sections detail the necessary changes, and
[this commit](https://github.com/GeunhwaJeong/haneul/commit/060f916698b03655f0647a6ec73c5321edb16459) shows a worked example of the manual changes necessary to cut `v0`.


### Depending on the new crates

`haneul-execution` needs to depend on the following new crates:

- `haneul-adapter-<FEATURE>`
- `haneul-move-natives-<FEATURE>`
- `haneul-verifier-<FEATURE>`
- `move-vm-runtime-<FEATURE>`


### New implementations of `Executor` and `Verifier`

`haneul-execution` exposes each version of the execution layer through
...
`./haneul-execution/src/lib.rs` and copy `./haneul-execution/src/latest.rs`
to `./haneul-execution/src/<feature>.rs`
...
```rust
use haneul_adapter_<feature>::adapter::{
    default_verifier_config, new_move_vm, run_metered_move_bytecode_verifier,
};
use haneul_adapter_<feature>::execution_engine::execute_transaction_to_effects;
use haneul_adapter_<feature>::programmable_transactions;
use haneul_adapter_<feature>::type_layout_resolver::TypeLayoutResolver;
use haneul_move_natives_<feature>::all_natives;
use haneul_verifier_<feature>::meter::HaneulVerifierMeter;
```
...
`haneul-execution` crate (`executor` and `verifier`) which switch on
