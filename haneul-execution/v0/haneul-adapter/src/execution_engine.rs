...
use haneul_types::{is_system_package, HANEUL_FRAMEWORK_PACKAGE_ID, HANEUL_SYSTEM_PACKAGE_ID};
...
fn execute_transaction<Mode: ExecutionMode>(
    ...
    if protocol_config.gas_model_version() > 1 {
        ...
        // === begin HANEUL conservation checks ===
        // For advance epoch transaction, we need to provide epoch rewards and rebates as extra
        // information provided to check_haneul_conserved, because we mint rewards, and burn
        ...
        if !is_genesis_tx && !Mode::allow_arbitrary_values() {
            // ensure that this transaction did not create or destroy HANEUL, try to recover if the check fails
            let conservation_result = {
                let mut layout_resolver = TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                temporary_store.check_haneul_conserved(advance_epoch_gas_summary, &mut layout_resolver, enable_expensive_checks)
            };
            if let Err(conservation_err) = conservation_result {
                ...
                let mut layout_resolver = TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                if let Err(recovery_err) = temporary_store.check_haneul_conserved(advance_epoch_gas_summary, &mut layout_resolver, enable_expensive_checks) {
                    // if we still fail, it's a problem with gas
                    // charging that happens even in the "aborted" case--no other option but panic.
                    // we will create or destroy HANEUL otherwise
                    panic!(
                        "HANEUL conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_ctx.digest(),
                        recovery_err,
                        gas_status.summary()
                    )
          ...
          // === end HANEUL conservation checks ===
        (cost_summary, result)
    } else {
