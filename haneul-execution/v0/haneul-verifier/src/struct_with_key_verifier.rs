    ...
    for def in struct_defs {
        ...
        let uid_field_type = match uid_field_type {
            ...
            _ => {
                return Err(verification_failure(format!(
                    ...
                    {:?} type found",
                    name, HANEUL_FRAMEWORK_ADDRESS, uid_field_type
                )))
            }
        };
        // check that the struct type for "id" field must be HANEUL_FRAMEWORK_ADDRESS::object::UID.
        ...
        fp_ensure!(
            uid_type_struct_name == UID_STRUCT_NAME
                && uid_type_module_address == &HANEUL_FRAMEWORK_ADDRESS
