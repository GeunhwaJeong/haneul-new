...
use haneul_types::execution::{ExecutionResults, LoadedChildObjectMetadata};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    ) -> HaneulResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
            ...
            match kind {
                WriteKind::Mutate => {
                    ...
                    match &old_obj.owner {
                        ...
                        Owner::Immutable => {
                            assert!(is_epoch_change, "Immutable objects cannot be written, except for Haneul Framework/Move stdlib upgrades at epoch change boundaries");
    ...
    pub fn check_haneul_conserved_expensive(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, _)) = input {
                total_input_haneul += self.get_input_haneul(&id, version, layout_resolver)?;
            }
            if let Some(object) = output {
    ...
    fn get_module(&self, module_id: &ModuleId) -> Result<Option<Vec<u8>>, Self::Error> {
        ...
        match &package.data {
            Data::Package(c) => Ok(c
                ...
                .cloned()),
            _ => Err(HaneulError::BadObjectType {
    ...
    fn get_resource(
        ...
        let object = match self.read_object(&ObjectID::from(*address)) {
            ...
            None => match self.read_object(&ObjectID::from(*address)) {
                ...
                Some(x) => {
                    if !x.is_immutable() {
                        fp_bail!(HaneulError::ExecutionInvariantViolation);
                    }
                    x
                }
            },
