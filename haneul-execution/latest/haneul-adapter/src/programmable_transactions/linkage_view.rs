...
impl<'state> LinkageView<'state> {
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, HaneulError> {
...
impl PackageStore for LinkageView<'_> {
    fn get_package(&self, package_id: &ObjectID) -> HaneulResult<Option<Rc<MovePackage>>> {
