...
mod checked {
    ...
    use haneul_types::execution::{ExecutionTiming, ResultWithTimings, SharedInput};
    use haneul_types::execution_status::{ExecutionErrorKind, ExecutionStatus};
    use haneul_types::gas::GasCostSummary;
    use haneul_types::gas::HaneulGasStatus;
    #[cfg(msim)]
    use haneul_types::haneul_system_state::advance_epoch_result_injection::maybe_modify_result_for;
    ...
    mod legacy {
        ...
        pub(super) fn execute_transaction_inner<Mode: ExecutionMode>(
            ...
            // Post-execution system-invariant checks, run after gas charging: HANEUL conservation
        ...
        fn execute_transaction<Mode: ExecutionMode>(
            ...
            // charging gas. For HANEUL this bounds each per-key gross Merge/Split total to the total supply;
            ...
            // events remain). Bounding HANEUL to the supply (which is ~8.4B HANEUL below u64::MAX) leaves enough
        ...
        /// - HANEUL conservation / balance-accumulator authorization, via [`run_conservation_checks`]. A
        ///   violation is recoverable: the tx is aborted (and conserves HANEUL) rather than panicking.
        ...
        /// Run the HANEUL-conservation and balance-accumulator invariant checks
        ...
        /// mints or burns HANEUL, which is unrecoverable, so we panic. The checks themselves are read-only;
        ...
        fn run_conservation_checks<Mode: ExecutionMode>(
            ...
            if let Err(recovery_err) = temporary_store.check_conservation_invariants::<Mode>(
                ...
                // "aborted" case — no other option but panic. We would create or destroy HANEUL
    ...
    fn gasless_withdrawal_reservations(
        transaction_kind: &TransactionKind,
        tx_ctx: &TxContext,
    ) -> Option<BTreeMap<(HaneulAddress, TypeTag), u64>> {
        ...
        let mut reservations = BTreeMap::<(HaneulAddress, TypeTag), u64>::new();
