...
pub struct TemporaryStore<'backing> {
    ...
    /// `check_haneul_address_balance_changes` inside `run_conservation_checks` to gate
...
impl<'backing> TemporaryStore<'backing> {
    ...
    pub fn check_gasless_execution_requirements(
        &self,
        withdrawal_reservations: Option<&BTreeMap<(HaneulAddress, TypeTag), u64>>,
    ) -> Result<(), String> {
        ...
        let allowed_types =
            haneul_types::transaction::get_gasless_allowed_token_types(self.protocol_config);
        ...
        let net_totals = haneul_types::balance_change::signed_balance_changes_from_events(
            &self.execution_results.accumulator_events,
        )
        .fold(
            BTreeMap::<(HaneulAddress, TypeTag), i128>::new(),
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&HANEUL_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping HANEUL on the floor by overriding it.
...
impl TemporaryStore<'_> {
    ...
    pub fn check_ownership_invariants(
        ...
        let objects_authenticated_for_mutation: HashSet<HaneulAddress> = self
            .input_objects
            .iter()
            .filter_map(|(id, obj)| {
                match &obj.owner {
                    ...
                    Owner::Immutable => {
                        assert!(
                            ...
                            "Immutable objects cannot be written, except for \
                             Haneul Framework/Move stdlib upgrades at epoch change boundaries"
                        );
        ...
        let haneul_balance_type =
            haneul_types::balance::Balance::type_tag(haneul_types::gas_coin::GAS::type_tag());
        ...
        for event in self.execution_results.accumulator_events.iter() {
            let amount = match event.write.value {
                ...
                AccumulatorValue::IntegerTuple(_, _) | AccumulatorValue::EventDigest(_) => {
                    assert!(
                        !haneul_types::balance::Balance::is_balance_type(&event.write.address.ty),
...
impl TemporaryStore<'_> {
    ...
    /// `HaneulGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl TemporaryStore<'_> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_haneul_system_state_wrapper(self.store.as_object_store())
...
impl TemporaryStore<'_> {
    fn get_input_haneul(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_haneul(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input HANEUL in HANEUL conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Some(obj) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in HANEUL conservation checking"
                );
            };
            obj.get_total_haneul(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input HANEUL in HANEUL conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys HANEUL. This should hold for all txes
    ...
    ///
    /// 1. all HANEUL in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all HANEUL charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "HANEUL conservation failed -- no storage charges in gas summary \
            ...
            // all HANEUL in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "HANEUL conservation failed -- {} HANEUL in storage rebate field of input objects, \
                        {} HANEUL in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all HANEUL charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "HANEUL conservation failed -- {} HANEUL charged for storage, \
                        {} HANEUL in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys HANEUL.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_haneul_conserved` above:
    ///
    /// * all HANEUL in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of HANEUL, and burn `epoch_rebates`
    /// amount of HANEUL. We need these information for this check.
    pub fn check_haneul_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        ...
        if total_input_haneul != total_output_haneul {
            return Err(ExecutionError::invariant_violation(format!(
                "HANEUL conservation failed: input={}, output={}, \
                    this transaction either mints or burns HANEUL",
                total_input_haneul, total_output_haneul,
            )));
        }
        Ok(())
    }
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(
        ...
        let ExecutionResults::V2(mut results) = results else {
            panic!("ExecutionResults::V2 expected in haneul-execution v1 and above");
        };
    ...
    fn check_coin_deny_list(
        ...
        if result.num_non_gas_coin_owners > 0
            && !self.input_objects.contains_key(&HANEUL_DENY_LIST_OBJECT_ID)
        {
            self.loaded_per_epoch_config_objects
                .write()
                .insert(HANEUL_DENY_LIST_OBJECT_ID);
        }
        result
    }
