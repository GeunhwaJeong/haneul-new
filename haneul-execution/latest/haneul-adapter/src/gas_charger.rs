...
pub mod checked {
    ...
    pub struct GasCharger {
        ...
        address_balance_gas_payer: Option<HaneulAddress>,
    }
        ...
        pub fn charge_gas<T>(
            ...
            if self.smashed_gas_coin.is_some() || self.address_balance_gas_payer.is_some() {
                ...
                let is_move_abort = execution_result
                    ...
                    .map(|err| {
                        matches!(
                            err.kind(),
                            haneul_types::execution_status::ExecutionFailureStatus::MoveAbort(_, _)
                        )
                    })
                    .unwrap_or(false);
                // bucketize computation cost
            ...
            if let Some(payer_address) = self.address_balance_gas_payer {
                let is_insufficient_balance_error = execution_result
                    .as_ref()
                    .err()
                    .map(|err| matches!(err.kind(), haneul_types::execution_status::ExecutionFailureStatus::InsufficientBalanceForWithdraw))
                    .unwrap_or(false);
                ...
                if is_insufficient_balance_error {
                    ...
                    if net_change != 0 {
                        let accumulator_event = AccumulatorEvent::from_balance_change(
                            AccumulatorObjId::new_unchecked(HANEUL_ACCUMULATOR_ROOT_OBJECT_ID),
                            payer_address,
                            net_change,
                        );
