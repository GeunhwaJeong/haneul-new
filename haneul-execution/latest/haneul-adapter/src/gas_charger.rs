...
pub mod checked {
    ...
    use haneul_types::gas::{GasCostSummary, HaneulGasStatus, deduct_gas};
    use haneul_types::gas_model::gas_predicates::refresh_gas_payment_location;
    use haneul_types::{
    ...
    /// A single source of HANEUL used to pay for gas: either a coin object or a withdrawal
    /// reservation from an address balance.
    #[derive(Debug)]
    pub enum PaymentMethod {
        Coin(ObjectRef),
        AddressBalance(HaneulAddress, /* withdrawal reservation */ u64),
    }
    ...
    pub enum PaymentLocation {
        Coin(ObjectID),
        AddressBalance(HaneulAddress),
    }

    /// A resolved gas payment: the location that will receive the final charge or refund,
    /// paired with the total HANEUL available after smashing. Produced by
    ...
    impl GasCharger {
        ...
        pub fn new_unmetered(tx_digest: TransactionDigest) -> Self {
            Self {
                ...
                gas_status: HaneulGasStatus::new_unmetered(),
            }
        }
    ...
    /// HANEULPR-753 fix). Removed at the next execution-version cut.
    mod legacy {
        use super::*;
        ...
        impl super::GasCharger {
            ...
            pub(crate) fn legacy_charge_gas<T, E: ExecutionErrorTrait>(
                ...
                if !matches!(&self.payment, PaymentMetadata::Unmetered) {
                    ...
                    let is_move_abort = execution_result
                        ...
                        .map(|err| {
                            matches!(
                                err.kind(),
                                haneul_types::execution_status::ExecutionErrorKind::MoveAbort(_, _)
                            )
                        })
                        .unwrap_or(false);
                    // bucketize computation cost
                ...
                .map(|err| {
                    matches!(
                        err.kind(),
                        haneul_types::execution_status::ExecutionErrorKind::InsufficientFundsForWithdraw
                    )
                })
                .unwrap_or(false)
