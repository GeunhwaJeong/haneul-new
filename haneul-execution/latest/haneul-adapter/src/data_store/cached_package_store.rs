...
impl<'state, 'runtime> CachedPackageStore<'state, 'runtime> {
    ...
    /// subsequent lookups.
    pub fn get_package(&self, object_id: &ObjectID) -> HaneulResult<Option<Arc<VerifiedPackage>>> {
        self.fetch_package(object_id)
    }

    pub fn get_move_package(&self, object_id: &ObjectID) -> HaneulResult<Option<Rc<MovePackage>>> {
    ...
    fn fetch_package(&self, id: &ObjectID) -> HaneulResult<Option<Arc<VerifiedPackage>>> {
        ...
        match self
            ...
            .map_err(|e| {
                HaneulErrorKind::ExecutionError(
