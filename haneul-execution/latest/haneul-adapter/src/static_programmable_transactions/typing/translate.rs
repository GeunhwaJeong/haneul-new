...
/// `haneul::coin::Coin<T>` and swaps references to that input to the conversion result.
...
/// Returns the inner type `T` if the type is `haneul::coin::Coin<T>`, else `None`
...
/// Returns the inner type `T` if the type is `haneul::balance::Balance<T>`, else `None`
...
/// Returns the inner type `T` if the type is `haneul::funds_accumulator::Withdrawal<T>`, else `None`
