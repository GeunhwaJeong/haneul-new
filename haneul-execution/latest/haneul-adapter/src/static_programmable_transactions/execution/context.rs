...
use haneul_types::{
    ...
    balance::Balance,
    base_types::{
        HaneulAddress, MoveObjectType, ObjectID, RESOLVED_ASCII_STR, RESOLVED_UTF8_STR,
...
macro_rules! object_runtime {
    ($context:ident) => {
        $context
            ...
            .get::<haneul_move_natives::object_runtime::ObjectRuntime>()
            .map_err(|e| {
                $context
                    .env
...
/// or as an address balance via `haneul::coin::send_funds`. This is needed at the end to
...
pub(crate) enum GasCoinTransfer {
    ...
    TransferObjects,
    /// Sent with the `haneul::coin::send_funds` command
    SendFunds {
    ...
    Mode: ExecutionMode,
{
        ...
        let gas = match payment_location {
            ...
            Some(gas_payment) => {
                ...
                let (gas_metadata, gas_value) = match gas_payment.location {
                    PaymentLocation::AddressBalance(haneul_address) => {
                        assert_invariant!(
                        ...
                        let metadata = InputObjectMetadata {
                            ...
                            owner: Owner::AddressOwner(haneul_address),
    ...
    pub fn argument_updates(
        &mut self,
        args: Vec<T::Argument>,
    ) -> Result<Vec<(haneul_types::transaction::Argument, Vec<u8>, TypeTag)>, Mode::Error> {
        args.into_iter()
    ...
    ) -> Result<Option<(haneul_types::transaction::Argument, Vec<u8>, TypeTag)>, Mode::Error> {
...
fn balance_change_accumulator_event(
    ...
    let balance_type = Balance::type_tag(haneul_types::gas_coin::GAS::type_tag());
