...
use haneul_types::transaction::{GasData, TransactionKind};
...
impl InvariantChecker {
    ...
    pub(crate) fn check_haneul_conserved(
        &self,
    ...
    pub(crate) fn check_haneul_conserved_expensive(
        &self,
        store: &TemporaryStore<'_>,
        gas_summary: &GasCostSummary,
        ...
        // settling.
        total_input_haneul += store.execution_results.settlement_input_haneul as u128;
        total_output_haneul += store.execution_results.settlement_output_haneul as u128;

        for (id, input, output) in get_modified_objects(store) {
            if let Some(input) = input {
                total_input_haneul +=
                    get_input_haneul(store, &id, input.version, layout_resolver)? as u128;
            }
            if let Some(object) = output {
...
impl InvariantChecker {
    ...
    pub(crate) fn check_conservation_invariants<Mode: ExecutionMode>(
        ...
        self.check_haneul_conserved(store, simple_conservation_checks, cost_summary)
            .and_then(|()| {
                if enable_expensive_checks {
                    let mut layout_resolver =
                        TypeLayoutResolver::new(move_vm, store.protocol_config(), Box::new(store));
                    self.check_haneul_conserved_expensive(store, cost_summary, &mut layout_resolver)
                } else {
                    Ok(())
                }
            })
    ...
    pub(crate) fn check_ownership_invariants(
        ...
        let objects_authenticated_for_mutation: HashSet<HaneulAddress> = store
            .input_objects
            .iter()
            .filter_map(|(id, obj)| {
                match &obj.owner {
