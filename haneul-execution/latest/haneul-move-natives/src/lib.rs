...
pub fn all_natives(silent: bool, protocol_config: &ProtocolConfig) -> NativeFunctionTable {
    let haneul_framework_natives: &[(&str, &str, NativeFunction)] = &[
        ...
        (
            ...
            "record_settlement_haneul_conservation",
            make_native!(accumulator::record_settlement_haneul_conservation),
        ),
        ...
        _ => {
            return Err(
                PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                    .with_message("Haneul verifier guarantees this is a struct".to_string()),
            );
        }
    };
        ...
        match $cache_info {
            ...
            CacheInfo::Loaded(bytes_opt) => {
                ...
                if config.object_runtime_charge_cache_load_gas() {
                    let bytes = bytes_opt.unwrap_or(HANEUL_ADDRESS_LENGTH as usize);
