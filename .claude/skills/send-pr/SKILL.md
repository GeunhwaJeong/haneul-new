...
- For unit tests: `HANEUL_SKIP_SIMTESTS=1 cargo nextest run --no-capture -p <package>`
- For anything in `haneul-e2e-tests`: `cargo simtest --no-capture -p haneul-e2e-tests <test_filter>`
...
If any commit on this branch touches `crates/haneul-protocol-config/src/lib.rs`, run:
```
