---
name: dual-replay
description: Run Haneul dual execution replay between base and tip commits, recover failed steps, build, and commit replay instrumentation.
---
...
2. Apply the dual-exec changes manually for this run, following the shape in the `TPL_*` template constants in `scripts/dual_replay.py`. Substitute `${cut_pkg}` → the underscored cut name (e.g. `replay_cut` → `haneul_adapter_replay_cut`).
...
`cargo check -p haneul-execution -p haneul-types` failed. The script already handles the common scaffolding (Clone derives on the gas types it knows about, the `serde_json` dep on `haneul-execution`). What's left is genuinely case-by-case. Read the captured stderr and apply the smallest fix that compiles:
...
- **Cascading `Clone` derives.** The script derives `Clone` on `HaneulGasStatus` (gas.rs), `gas_v2::HaneulGasStatus`, `gas_v2::HaneulCostTable`, `gas_v2::ComputationBucket`, and `tables::GasStatus`. If a new field type in any of these (or a transitively-reached type) is not `Clone`, add `#[derive(Clone)]` to it — and if this becomes a recurring fix, add the type to `GAS_CLONE_TARGETS` in `scripts/dual_replay.py` so the script handles it directly.
- **Missing gas-model setter.** If the template calls `set_gas_model_version` on `HaneulGasStatus` and the method doesn't exist, add a narrow one. Keep it minimal.
