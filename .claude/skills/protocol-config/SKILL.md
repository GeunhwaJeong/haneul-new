...
The protocol config is defined in `crates/haneul-protocol-config/src/lib.rs`. It controls blockchain behavior across all Haneul networks. The critical constraint is:
...
```bash
git fetch origin
git branch -r | grep 'origin/releases/haneul-v' | grep -E '\-release$' | sort -V | tail -1
```

This returns something like `origin/releases/haneul-v1.66.0-release`.

### 2. Check the max protocol version in the release branch

```bash
git show <release-branch>:crates/haneul-protocol-config/src/lib.rs | grep 'const MAX_PROTOCOL_VERSION'
```

For example:
```bash
git show origin/releases/haneul-v1.66.0-release:crates/haneul-protocol-config/src/lib.rs | grep 'const MAX_PROTOCOL_VERSION'
```

### 3. Check the max protocol version in your feature branch

```bash
grep 'const MAX_PROTOCOL_VERSION' crates/haneul-protocol-config/src/lib.rs
```
...
```bash
cargo insta test -p haneul-protocol-config --accept
```
...
- Protocol config: `crates/haneul-protocol-config/src/lib.rs`
- Snapshots: `crates/haneul-protocol-config/src/snapshots/`
