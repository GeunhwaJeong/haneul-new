---
name: simtest-debug
description: Debug deterministic Haneul simtest failures with structured experiments, logging-only changes, and NOTEBOOK.md observations.
---
...
Example: `/debug-simtest MSIM_TEST_SEED=1768248386016 RUST_LOG=haneul=debug,info cargo simtest --test address_balance_tests test_deposit_and_withdraw`
...
Run the repro command. If `RUST_LOG=...` is missing, add `RUST_LOG=haneul=debug,info`. if `--no-capture` is missing, add it.
