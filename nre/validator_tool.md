...
2. Build the `haneul` binary, which you will need for the genesis ceremony. This step can be done on any machine you like. It does not have to be done on the machine on which you will run the validator.
    ...
    3. Build haneul binary

           cargo build --bin haneul

    4. Remember the path to your binary:

           export HANEUL_BINARY="$(pwd)/target/debug/haneul"

3. Run the following command to set up your Haneul account and CLI environment. 

    1. If this is the first time running this program, it will ask you to provide a Haneul Fullnode Server URL and a meaningful environment alias. It will also generate a random key pair in `haneul.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.
...
``` bash
$HANEUL_BINARY validator display-metadata
```
...
``` bash
$HANEUL_BINARY validator display-metadata <validator-address>
```
...
``` bash
$HANEUL_BINARY validator update-metadata --help
```
...
#### Operation Cap
Operation Cap allows a validator to authorizer another account to perform certain actions on behalf of this validator. Read about [Operation Cap here](haneul_for_node_operators.md#operation-cap).
...
```bash
$HANEUL_BINARY validator update-gas-price <gas-price>
```

if the account itself is a validator and holds the Operation Cap. Or 

```bash
$HANEUL_BINARY validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```bash
$HANEUL_BINARY validator report-validator <reportee-address>
```
...
```bash
$HANEUL_BINARY validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```

This will generate a `validator.info` file and key pair files. The output of this command includes:
  1. Four key pair files (Read [more here](haneul_for_node_operators.md#key-management)). ==Set their permissions with the minimal visibility (chmod 600, for example) and store them securely==. They are needed when running the validator node as covered below.
    a. If you follow this guide thoroughly, this key pair is actually copied from your `haneul.keystore` file.
  2. `validator.info` file that contains your validator info. **Double check all information is correct**.

Then run 

``` bash
$HANEUL_BINARY validator become-candidate {path-to}validator.info
```
...
``` bash
$HANEUL_BINARY validator join-committee
```
...
``` bash
$HANEUL_BINARY validator leave-committee
```
...
Example:
```bash
$HANEUL_BINARY validator update-gas-price 1000 --serialize-unsigned-transaction
```
Output:
```
Serialized transaction for signing: <BASE64_STRING>
```
You can then sign this payload using `haneul keytool sign` and execute it using `haneul client execute-signed-tx`.

**Note:** If you are using a multisig account, ensure that your active address in the Haneul CLI is set to your multisig address before running these commands. You can set it using:
```bash
$HANEUL_BINARY client switch --address <MULTISIG_ADDRESS>
```
...
``` bash
$HANEUL_BINARY validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
