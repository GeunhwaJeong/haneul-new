...
function latest_mi_revision() {
	...
	cd "$MI_CHECKOUT"
	git clone --depth 1 https://github.com/GeunhwaJeong/haneullabs-infra
	cd haneullabs-infra
	git rev-parse HEAD
}
...
function current_mi_revision() {
	cd "$TOPLEVEL"
	readarray -t <<< "$(find ./ -iname '*.toml' -exec $GREP -oPie 'git = "https://github.com/[mM]ystenLabs/haneullabs-infra(\.git)?", *rev *= *\"\K[0-9a-fA-F]+' '{}' \;)"
	watermark=${MAPFILE[0]}
	for i in "${MAPFILE[@]}"; do
	    if [[ "$watermark" != "$i" ]]; then
        	not_equal=true
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current haneullabs-infra revision in NW, aborting" && exit 1
	echo "$watermark"
}
...
# Debug prints for haneullabs-infra
CURRENT_MI=$(current_mi_revision)
LATEST_MI=$(latest_mi_revision)
if [[ "$CURRENT_MI" != "$LATEST_MI" ]]; then
	echo "About to replace $CURRENT_MI with $LATEST_MI as the haneullabs-infra pointer in Narwhal"
else
	exit 0
fi
