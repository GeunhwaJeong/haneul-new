...
After setting up the testbed, running a benchmark on AWS is similar to running it locally (see [Run Local Benchmarks](https://github.com/GeunhwaJeong/narwhal/tree/main/benchmark#local-benchmarks)). Locate the task `remote` in [fabfile.py](https://github.com/GeunhwaJeong/haneul/blob/main/narwhal/benchmark/fabfile.py):
