...
pub fn execute<E: Debug, S: ResourceResolver<Error = E> + ModuleResolver<Error = E> + Storage>(
    ...
    match session.execute_function_for_effects(
        ...
        ExecutionResult::Success {
            ...
            return_values,
            mutable_ref_values,
            gas_used: _, // TODO(https://github.com/GeunhwaJeong/fastnft/issues/45): charge
        } => {
...
pub fn publish<E: Debug, S: ResourceResolver<Error = E> + ModuleResolver<Error = E> + Storage>(
    ...
    for module in modules {
        ...
        // TODO(https://github.com/GeunhwaJeong/fastnft/issues/69):
...
>(
    ...
    // TODO(https://github.com/GeunhwaJeong/fastnft/issues/96): implement freeze and immutable objects
    for e in events {
            ...
            Data::Move(m) => {
                ...
                match &param_type {
                    Type::MutableReference(inner_t) => {
                        // (https://github.com/GeunhwaJeong/fastnft/issues/96): check m.mutability
                    ...
                    Type::Struct { .. } => {
                        // TODO(https://github.com/GeunhwaJeong/fastnft/issues/96): check m.mutability
