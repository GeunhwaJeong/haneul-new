...
use fastx_types::{
    base_types::{
        Authenticator, ObjectID, HaneulAddress, TransactionDigest, TxContext, TX_CONTEXT_MODULE_NAME,
...
pub fn new_move_vm(natives: NativeFunctionTable) -> Result<Arc<MoveVM>, HaneulError> {
    Ok(Arc::new(
        MoveVM::new(natives).map_err(|_| HaneulError::ExecutionInvariantViolation)?,
    ))
}
...
>(
    ...
    let mut gas_status =
        match get_gas_status(cost_table, Some(gas_budget)).map_err(|e| HaneulError::GasBudgetTooHigh {
            error: e.to_string(),
        }) {
            Ok(ok) => ok,
            Err(err) => {
...
pub fn publish<E: Debug, S: ResourceResolver<Error = E> + ModuleResolver<Error = E> + Storage>(
    ...
    if modules.is_empty() {
        exec_failure!(
            gas::MIN_MOVE_PUBLISH_GAS,
            HaneulError::ModulePublishFailure {
...
>(
    ...
    // TODO(https://github.com/GeunhwaJeong/fastnft/issues/69): avoid this redundant serialization by exposing VM API that allows us to run the linker directly on `Vec<CompiledModule>`
    ...
    session
        ...
        .map_err(|e| HaneulError::ModulePublishFailure {
            error: e.to_string(),
        })?;
    ...
    for e in events {
            ...
            .expect("Safe because event_type is derived from an EventType enum")
        {
            EventType::TransferToAddress => handle_transfer(
                Authenticator::Address(HaneulAddress::try_from(recipient.borrow()).unwrap()),
                type_,
                event_bytes,
                false, /* should_freeze */
                tx_digest,
    ...
    match type_ {
        TypeTag::Struct(s_type) => {
            ...
            if let Authenticator::Object(owner_object_id) = recipient {
                ...
                if parent == obj_id {
                    return Err(HaneulError::CircularObjectOwnership);
                }
...
fn resolve_and_type_check(
    ...
    ctx: &TxContext,
) -> Result<TypeCheckSuccess, HaneulError> {
    ...
    let (function_signature, module_id) = match package_object.data {
        ...
        Data::Move(_) => {
            return Err(HaneulError::ModuleLoadFailure {
    ...
    if !function_signature.return_.is_empty() {
        return Err(HaneulError::InvalidFunctionSignature {
...
fn type_check_struct(arg_type: &StructTag, param_type: &Type) -> Result<(), HaneulError> {
